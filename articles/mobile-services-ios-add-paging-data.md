<properties urlDisplayName="Add paging to data" pageTitle="Aggiungere paging ai dati (iOS) | Mobile Developer Center" metaKeywords="" description="Informazioni su come usare il paging per gestire la quantità di dati restituiti all'app per iOS da Servizi mobili." metaCanonical="" services="mobile-services" documentationCenter="Mobile" title="Refine Mobile Services queries with paging" authors="krisragh" solutions="" manager="dwrede" editor="" />
...
Questo argomento illustra come usare il paging per gestire la quantità di dati restituiti all'app per iOS da Servizi mobili di Azure. In questa esercitazione verranno usate le proprietà di query **fetchLimit** e **fetchOffset** sul client per richiedere "pagine" di dati specifiche.
...
2. Premere il pulsante **Run** (Comando + R) per compilare il progetto e avviare l'app, quindi immettere il testo nella casella di testo e fare clic sull'icona con il segno più  plus (**+**).
