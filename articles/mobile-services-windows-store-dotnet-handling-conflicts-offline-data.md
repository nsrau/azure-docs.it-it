<properties urlDisplayName="Handle Conflicts with Offline Data" pageTitle="Gestire i conflitti con dati offline in Servizi mobili (Windows Store) | Mobile Developer Center" metaKeywords="" description="Learn how to use Azure Mobile Services handle conflicts when syncing offline data in your Windows Store application" metaCanonical="" disqusComments="1" umbracoNaviHide="1" documentationCenter="Mobile" title="Handling conflicts with offline data in Mobile Services" authors="wesmc" manager="dwrede" />
...
Questa esercitazione è basata sulle procedure e sull'app di esempio dell'esercitazione precedente [Introduzione ai dati offline]. Prima di iniziare questa esercitazione è consigliabile completare [Introduzione ai dati offline in Servizi mobili].


In questa esercitazione vengono descritte le operazioni di base seguenti:

1. [Scaricare il progetto dell'app di Windows Store] 
2. [Testare l'app in un servizio mobile]
...
1. Scaricare il file ZIP per l'[repository Github degli esempi di Servizi mobili] ed estrarlo in una directory di lavoro. 
...
1. Nel portale di gestione di Azure individuare la chiave applicazione del servizio facendo clic su **Gestisci chiavi** sulla barra dei comandi della scheda **Dashboard**. Copiare la **chiave applicazione**.
...
3. In Visual Studio premere **F5** per compilare ed eseguire ancora l'app.
...
[Aggiornare il database per i servizi mobili back-end .NET]: #dotnet-backend  
...
[Gestione di conflitti nel database]: /it-it/documentation/articles/mobile-services-windows-store-dotnet-handle-database-conflicts/#test-app
