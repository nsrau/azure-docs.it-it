<properties 
	pageTitle="Configurazione di una macchina virtuale di SQL Server di Azure per l'analisi scientifica dei dati" 
...
1.  Accedere al [portale di gestione di Azure](http://manage.windowsazure.com/) con il proprio account. 
	Se non si dispone di un account Azure, provare la [versione di valutazione gratuita di Azure](http://www.windowsazure.com/pricing/free-trial/).
...
3.  Nella pagina relativa alla **creazione di una macchina virtuale** selezionare l'immagine di una macchina virtuale contenente SQL Server in base alle esigenze dei dati di cui si dispone, quindi fare clic sulla freccia Avanti nella parte inferiore destra della pagina. Per informazioni aggiornate sulle immagini di SQL Server supportate in Azure, vedere l'argomento [Introduzione a SQL Server in Macchine virtuali di Azure](http://go.microsoft.com/fwlink/p/?LinkId=294720) nel set di documentazione [SQL Server in Macchine virtuali di Azure](http://go.microsoft.com/fwlink/p/?LinkId=294719).

	![Select SQL Server VM][1]

4.  Nella prima pagina di **configurazione della macchina virtuale** fornire le seguenti informazioni:

    -   Specificare un nome in **NOME MACCHINA VIRTUALE**.
    -   Nella casella **NOME NUOVO UTENTE** digitare un nome utente univoco per l'account amministratore locale della macchina virtuale.
...
9.  Attendere durante la preparazione della macchina virtuale in Azure. Lo stato della macchina virtuale attraverserà le seguenti fasi:
...
3.  Nella finestra di dialogo **Sicurezza di Windows** inserire la password per l'account amministratore locale specificata in uno dei passaggi precedenti. (potrebbe essere richiesta la verifica delle credenziali della macchina virtuale).
...
3.  In Esplora oggetti di SQL Server Management Studio fare clic con il pulsante destro del mouse sul nome dell'istanza di SQL Server (il nome della macchina virtuale), quindi fare clic su **Proprietà**.

    ![Server Properties][7]

4.  Nella pagina **Sicurezza**, in **Autenticazione server** selezionare **Autenticazione di SQL Server e di Windows**, quindi fare clic su **OK**.

    ![Select Authentication Mode][8]

5.  Nella finestra di dialogo di SQL Server Management Studio fare clic su **OK** per confermare il requisito di riavvio di SQL Server.

6.  In Esplora oggetti fare clic con il pulsante destro del mouse sul server, quindi fare clic su **Riavvia**. (Se SQL Server Agent è in esecuzione, anch'esso dovrà essere riavviato).

    ![Restart][9]

7.  Nella finestra di dialogo di SQL Server Management Studio fare clic su **Sì** per confermare il riavvio di SQL Server.
...
2.  Fare clic con il pulsante destro del mouse sulla cartella **Sicurezza**, scegliere **Nuovo**, quindi selezionare **Accedi...**.
...
5.  Nella casella **Password** digitare una password per il nuovo utente. Digitare di nuovo tale password nella casella **Conferma password**.
...
8.  Per forzare un utente a creare una nuova password dopo il primo uso dell'account di accesso, selezionare **Richiedi modifica della password all'accesso successivo** (scelta consigliata se tale account di accesso sarà usato da un altro utente. Se si tratta del proprio nome di accesso, non selezionare questa opzione). Per abilitare questa casella di controllo, è necessario che sia selezionata l'opzione Imponi scadenza password. Si tratta di un'opzione predefinita quando si seleziona l'autenticazione di SQL Server.

9.  Nell'elenco **Database predefinito** selezionare un database predefinito per effettuare l'accesso. Quello predefinito per tale opzione è **master**. Se il database utente non è ancora stato creato, lasciare questa opzione impostata su **master**.
...
1.  Nel portale di gestione di Azure (oppure nel passaggio precedente) selezionare **MACCHINE VIRTUALI**.
...
2.  Nella finestra di dialogo **Connetti al server** oppure **Connetti al motore di database**, nella casella **Nome server** immettere il nome DNS della macchina virtuale (determinato nell'attività precedente) e un numero di porta di endpoint pubblica nel formato  *DNSName,portnumber* come **tutorialtestVM.cloudapp.net,57500**.

3.  Nella casella **Autenticazione** selezionare **Autenticazione di SQL Server**.

4.  Nella casella **Accedi** digitare il nome di un account di accesso creato in un'attività precedente.

5.  Nella casella **Password** digitare la password dell'account di accesso creato in un'attività precedente.

6.  Fare clic su **Connetti**.

##<a name="amlconnect"></a>Connettersi al motore di database da Azure Machine Learning

Nelle fasi successive del processo di analisi scientifica dei dati cloud verrà utilizzato [Azure Machine Learning Studio](https://studio.azureml.net) per creare e distribuire modelli di Machine Learning. Per inserire dati dai database delle macchine virtuali di SQL Server direttamente in Azure Machine Learning per il training o l'assegnazione di punteggi, usare il modulo Lettore in un nuovo esperimento di [Azure Machine Learning Studio](https://studio.azureml.net). In questo argomento vengono fornite informazioni più dettagliate con collegamenti alla mappa del processo di analisi scientifica dei dati cloud. Per un'introduzione, vedere [Cos'è Azure Machine Learning Studio?](machine-learning-what-is-ml-studio.md).
...
3.	Nella casella di testo **Nome server database** immettere `tcp:<nome DNS della macchina virtuale>,1433`
