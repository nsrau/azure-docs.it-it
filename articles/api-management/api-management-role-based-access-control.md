...
| Lettore del servizio di gestione API | ✓ | | || Dispone di accesso in sola lettura a servizi ed entità Gestione API. |
| Operatore del servizio di gestione API | ✓ | | ✓ | | Può gestire i servizi di Gestione API, ma non le entità.|
| Editor del servizio di gestione API<sup>*</sup> | ✓ | ✓ | |  | Può gestire le entità di Gestione API, ma non i servizi.|
| Gestione contenuto di Gestione API<sup>*</sup> | ✓ | | | ✓ | Può gestire il portale per sviluppatori. Accesso in sola lettura a servizi ed entità.|
...
<sup>\*Il ruolo Editor servizi sarà disponibile dopo la migrazione di tutta l'interfaccia utente di amministrazione dal portale dell'editore esistente al portale di Azure.The Service Editor role will be available after we migrate all the admin UI from the existing publisher portal to the Azure portal. Il ruolo Gestione contenuto sarà disponibile dopo il refactoring del portale editore per contenere solo le funzionalità correlate alla gestione del portale per sviluppatori.</sup>  
...
> [!NOTE]
> Per poter visualizzare un'istanza di Gestione API nel portale ```Microsoft.ApiManagement/service/read``` di Azure, un ruolo personalizzato deve includere l'azione.

Quando si crea un ruolo personalizzato, è più semplice iniziare con uno dei ruoli predefiniti. Modificare gli attributi per aggiungere **Actions**, **NotActions** o **AssignableScopes** e quindi salvare le modifiche come nuovo ruolo. L'esempio seguente inizia con il ruolo "Lettore del servizio gestione API" e crea un ruolo personalizzato denominato "Editor API calcolatrice". È possibile assegnare il ruolo personalizzato a un'API specifica. Di conseguenza, questo ruolo può accedere solo a tale API. 
...
Per altre informazioni sul controllo degli accessi in base al ruolo in Azure, vedere gli articoli seguenti:
  ...
  * [Ruoli personalizzati in Controllo degli accessi in base al ruolo di AzureCustom roles in Azure](../role-based-access-control/custom-roles.md)
  * [Operazioni del provider di risorse di Azure Resource ManagerAzure Resource Manager resource provider operations](../role-based-access-control/resource-provider-operations.md#microsoftapimanagement)
