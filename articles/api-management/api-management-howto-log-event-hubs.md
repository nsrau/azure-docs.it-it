...
Per creare un nuovo hub eventi, accedere al [portale di Azure](https://manage.windowsazure.com), quindi fare clic su **Nuovo**->**Servizi app**->**Bus di servizio**->**Hub eventi**->**Creazione rapida**. Immettere un nome e un'area per l'hub eventi, selezionare una sottoscrizione e quindi uno spazio dei nomi. Se non è stato ancora creato uno spazio dei nomi, è possibile crearne uno immettendo un nome nella casella di testo **Spazio dei nomi**. Al termine della configurazione di tutte le proprietà, fare clic su **Crea un nuovo hub eventi** per creare l'hub eventi.

![Creare un hub eventi][create-event-hub]

Passare quindi alla scheda **Configura** per il nuovo hub eventi e creare due **criteri di accesso condivisi**. Denominare il primo criterio **Invio** e assegnare al criterio le autorizzazioni **Send**.
...
Ogni criterio di accesso condiviso consente alle applicazioni di inviare e ricevere eventi verso e dall'hub eventi. Per accedere alle stringhe di connessione per questi criteri, passare alla scheda **Dashboard** dell'hub eventi e fare clic su **Informazioni di connessione**.
...
-	Sostituire `{new logger name}` con il nome desiderato per il nuovo logger. Si farà riferimento a questo nome quando si configura il criterio [log-to-eventhub](https://msdn.microsoft.com/library/azure/dn894085.aspx#log-to-eventhub).
...
-	`credentials` contiene i valori `name` e `connectionString` dell'Hub eventi di Azure.

Quando si effettua la richiesta, se viene creato il logger, verrà visualizzato un codice di stato `201 Created`.

>[AZURE.NOTE]Per altri codici restituiti possibili e i relativi motivi, vedere [Creare un logger](https://msdn.microsoft.com/library/azure/mt592020.aspx#PUT). Per informazioni su come eseguire altre operazioni, ad esempio operazioni di creazione di elenco, aggiornamento ed eliminazione, vedere la documentazione relativa all'entità [Logger](https://msdn.microsoft.com/library/azure/mt592020.aspx).
...
Per configurare i criteri, accedere al [portale di Azure](https://manage.windowsazure.com), passare al servizio Gestione API e fare clic sul **portale di pubblicazione** o su **Gestisci** per accedere al portale di pubblicazione.

![Portale di pubblicazione][publisher-portal]

Scegliere **Criteri** dal menu di Gestione API a sinistra, selezionare il prodotto e l'API desiderati, quindi fare clic su **Aggiungi criterio**. In questo esempio si aggiunge un criterio all'**API Echo** nel prodotto **Unlimited**.

![Aggiungi criteri][add-policy]

Posizionare il cursore nella sezione relativa ai criteri `inbound` e fare clic sul criterio **Log to EventHub** per inserire il modello di istruzione dei criteri `log-to-eventhub`.
