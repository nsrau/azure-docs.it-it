---
title: Usare Gestione API di Azure con reti virtuali interneUse Azure API Management with internal virtual networks
...
Quando Gestione API viene distribuito in modalità rete virtuale interna, tutti gli endpoint del servizio (il gateway proxy, il portale per sviluppatori, la gestione diretta e Git) sono visibili solo all'interno di una rete virtuale a cui si controlla l'accesso. Nessuno degli endpoint di servizio è registrato nel server DNS pubblico.

> [!NOTE]
> Poiché non sono presenti voci DNS per gli endpoint del servizio, questi endpoint non saranno accessibili fino a quando DNS non [sarà configurato](#apim-dns-configuration) per la rete virtuale.
...
Dopo l'esito positivo della distribuzione, nel pannello Panoramica dovrebbero essere visualizzati l'indirizzo IP virtuale **privato** e l'indirizzo IP virtuale **pubblico** del servizio Gestione API. L'indirizzo IP virtuale **privato** è un indirizzo IP con carico `gateway`bilanciato dall'interno della subnet delegata di Gestione API tramite la quale è possibile accedere agli endpoint `portal` `management` e , e `scm` . L'indirizzo IP virtuale **pubblico** viene utilizzato `management` **solo** per il traffico del piano di controllo all'endpoint sulla porta 3443 e può essere bloccato al servicetag ApiManagement.The public virtual IP address is used only for control plane traffic to endpoint over port 3443 and can be locked down to the [ApiManagement][ServiceTags] servicetag.
...
Quando si crea un servizio Gestione API, ad esempio "contosointernalvnet", gli endpoint del servizio seguenti vengono configurati per impostazione predefinita:When you create an API Management service, named "contosointernalvnet", for example, the following service endpoints are configured by default:

   * Gateway o proxy: contosointernalvnet.azure-api.net

   * Il portale per sviluppatori: contosointernalvnet.portal.azure-api.net
...
* Un indirizzo IP virtuale *privato* con bilanciamento del carico dall'intervallo di subnet verrà riservato e utilizzato per accedere agli endpoint del servizio Gestione API dall'interno della rete virtuale. Questo indirizzo IP privato è disponibile nel pannello Panoramica per il servizio nel portale di Azure.This *private* IP address can be found on the Overview blade for the service in the Azure portal. Questo indirizzo deve essere registrato con i server DNS utilizzati dalla rete virtuale.
* Un indirizzo IP pubblico con bilanciamento del carico (VIP) verrà riservato anche per fornire l'accesso all'endpoint del servizio di gestione tramite la porta 3443.A load balanced *public* IP address (VIP) will also be reserved to provide access to the management service endpoint over port 3443. Questo indirizzo IP pubblico è disponibile nel pannello Panoramica per il servizio nel portale di Azure.This *public* IP address can be found on the Overview blade for the service in the Azure portal. L'indirizzo IP *pubblico* viene utilizzato solo `management` per il traffico aereo di controllo verso l'endpoint sulla porta 3443 e può essere bloccato al servicetag [ApiManagement.][ServiceTags]
* Gli indirizzi IP dell'intervallo IP della subnet (DIP) verranno assegnati a ogni macchina virtuale nel servizio e verranno usati per accedere alle risorse all'interno della rete virtuale. Un indirizzo IP pubblico (VIP) verrà utilizzato per accedere alle risorse all'esterno della rete virtuale. Se gli elenchi di restrizioni IP vengono utilizzati per proteggere le risorse all'interno della rete virtuale, l'intero intervallo per la subnet in cui è distribuito il servizio Gestione API deve essere specificato per concedere o limitare l'accesso al servizio.
* Gli indirizzi IP pubblici e privati con carico bilanciato sono disponibili nel pannello Panoramica nel portale di Azure.The load balanced public and private IP addresses can be found on the Overview blade in the Azure portal.
...
* [Domande frequenti sulla rete virtualeVirtual network FAQs](../virtual-network/virtual-networks-faq.md)
