...
1. Impostare la voce ' URL di risposta ' su quella annotata quando è stata configurata la redirect_uri della concessione del codice di autenticazione in gestione API in precedenza.
   ...
   Ora che l'autorizzazione utente OAuth 2,0 è abilitata nel `Echo API`, la console per sviluppatori ottiene un token di accesso per l'utente, prima di chiamare l'API.

1. Passare a qualsiasi operazione nel `Echo API` nel portale per sviluppatori e selezionare **prova** per visualizzare la console per sviluppatori.
1. Nella sezione **Autorizzazione** è presente un nuovo elemento che corrisponde al server di autorizzazione appena aggiunto.
...
1. Una volta eseguito l'accesso, viene aggiunta un'intestazione `Authorization: Bearer` alla richiesta, con un token di accesso da Azure AD B2C codificato in Base64. 
...
* Provare a usare il API Graph Azure AD per assegnare attestazioni personalizzate e usare un criterio di gestione API per convalidare che siano presenti nel token.
