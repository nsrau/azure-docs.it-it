...
Per installare lo strumento in un Mac, scaricare ed eseguire il [programma di installazione di Azure SDK](http://go.microsoft.com/fwlink/?LinkId=252249).
...
* [Comandi per la gestione dei dischi dati delle macchine virtuali di Azure](#Commands_to_manage_your_Azure_virtual_machine_data_disks)
...
**vm create [opzioni] &lt;nome_dns> &lt;immagine> &lt;nome_utente> [password]**
...
**-n, --vm-name** Specifica il nome della macchina virtuale. Per impostazione predefinita, questo parametro prende il nome del servizio di hosting. Se -vmname non viene specificato, il nome per la nuova macchina virtuale viene generato come &lt;nome_servizio>&lt;, dove &lt;id> è il numero di macchine virtuali esistenti nel servizio più 1. Se ad esempio si utilizza questo comando per aggiungere una nuova macchina virtuale a un servizio di hosting denominato MyService con una macchina virtuale già esistente, la nuova macchina virtuale viene denominata MyService2.<br />
...
**vm create-from &lt;nome_dns> &lt;file_ruolo **
...
**vm capture &lt;nome_vm> &lt;nome_immagine_destinazione>**
...
**vm export [opzioni] &lt;nome_vm> &lt;percorso_file>**
...
**vm endpoint create &lt;nome_vm> &lt;porta_bc> [porta_vm]**
...
**vm endpoint create-multiple [opzioni] &lt;nome_vm> &lt;porta_bc>[:&lt;porta_vm>[:&lt;protocollo>[:&lt;nome_set bc>[:&lt;protocollo_prob>:&lt;porta_prob_bc>[:&lt;percorso_prob>]]]]] ]{1-*}**
...
**vm endpoint delete &lt;nome_vm> &lt;porta_bc>**
...
**vm endpoint update [opzioni] &lt;nome_vm> &lt;nome_endpoint>**
...
**vm endpoint show [opzioni] &lt;nome_vm>**
...
**vm disk create &lt;nome> [percorso_origine]**

Questo comando carica e registra un disco di Azure. È necessario specificare --blob-url, --location o --affinity-group. Se si utilizza questo comando con [percorso_origine], viene caricato il file con estensione vhd specificato e viene creata una nuova immagine. È quindi possibile collegare questa immagine a una macchina virtuale utilizzando il comando vm disk attach.
...
**vm disk upload [opzioni] &lt;percorso_origine> &lt;url_blob> &lt;chiave_account_archiviazione>**
...
**vm disk attach &lt;nome_vm> &lt;nome_immagine_disco>**
...
**vm disk attach-new &lt;nome_vm> &lt;dimensioni_in_gb> [url_blob]**
...
**vm disk detach &lt;nome_vm> &lt;lun>**

Questo comando scollega un disco dati collegato a una macchina virtuale di Azure. &lt;lun> identifica il disco da scollegare. Per ottenere un elenco di dischi associati a un disco prima che questo venga scollegato, usare il comando vm disk-list &lt;nome_vm>.
...
**service create [opzioni] &lt;nome_servizio>**
...
**service show [opzioni] &lt;nome_servizio>**
...
**service cert create &lt;prefisso_dns> &lt;file> [password]**
...
**site cert add [opzioni] &lt;percorso_certificato> [nome]**
...
**site deployment user set [opzioni] [nome_utente] [password]**
...
+ **--job-name** &lt;nome_processo> obbligatorio. Nome del processo Web.
+ **--job-type** &lt;tipo_processo> obbligatorio. Tipo di processo Web. Il valore valido è "triggered" o "continuous".
+ **-q** o **--quiet**: non chiede alcuna conferma. Utilizzare questa opzione negli script automatici.
+ **--slot** &lt;slot>: nome dello slot da riavviare.

**site job upload [opzioni] &lt;nome_processo> &lt;tipo_processo> <file_processo> [nome]**
...
+ **--job-name** &lt;nome_processo>: obbligatoria. Nome del processo Web.
+ **--job-type** &lt;tipo_processo>: obbligatoria. Tipo di processo Web. Il valore valido è "triggered" o "continuous".
+ **--job-file** &lt;file_processo>: obbligatoria. File del processo.
+ **--slot** &lt;slot>: nome dello slot da riavviare.
...
+ **-l `<ubicazione>`** o **--location `<ubicazione>`**: crea il servizio in un'ubicazione specifica, definita come `<ubicazione>`. Eseguire il comando azure mobile locations per ottenere le ubicazioni disponibili.
+ **--sqlLocation `<ubicazione>`**: crea il server SQL in una '<ubicazione>' specifica. L'ubicazione predefinita è quella del servizio mobile.
...
Questo comando restituisce i log del servizio mobile, escludendo dal filtro tutti i tipi di log ad eccezione di  `error`.
...
+ **-p `&lt;autorizzazioni>`** o **--permissions `&lt;autorizzazioni>`**: elenco delimitato da virgole di coppie `<operazione>`=`<autorizzazione>`, dove `<operazione>` è  `insert`, `read`, `update` o  `delete` e '&lt;autorizzazione>' è  `public`, `application` (predefinita),  `user` o  `admin`.
...
I comandi in questa sezione vengono usati per gestire gli script del server appartenenti a un servizio mobile. Per altre informazioni, vedere [Uso degli script del server in Servizi mobili](http://www.windowsazure.com/develop/mobile/how-to-guides/work-with-server-scripts/).
...
Il nome del file deve essere composto dai nomi della tabella e dell'operazione e deve trovarsi nella sottocartella table relativa al percorso in cui viene eseguito il comando. È anche possibile usare il parametro **-f `<file>`** o il parametro **--file `<file>`** per specificare un nome file diverso e un percorso di file che contiene lo script da registrare.
...
+ **-t `<ora>`** **--startTime `<ora>`** L'orario di inizio della prima esecuzione dello script in formato ISO. Il valore predefinito è `now`.
...
+ **-t** o **--blobtype** &lt;tipo_blob>: tipo di BLOB di archiviazione, Page o Block.
