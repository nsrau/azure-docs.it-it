...
<p>Non è necessario registrare l'app con Windows Store per usare l'account Microsoft per l'autenticazione prima di pubblicare l'app. Quando l'app di Windows Store non richiede l'accesso Single Sign-On o le notifiche push, è sufficiente registrare l'app con Live Connect per usare un accesso con account Microsoft.  Per altre informazioni, vedere <a href="/it-it/develop/mobile/how-to-guides/register-for-microsoft-authentication">Registrazione delle app di Windows Store per l'uso delle credenziali di accesso di un account Microsoft</a>.</p>
</div>
