...
- **SQL Server in Azure Arc** : se la SQL Server è ospitata in una macchina di [Azure Arc](../azure-arc/index.yml) , è possibile distribuire l'agente di log Analytics usando la raccomandazione "log Analytics Agent deve essere installato nei computer Azure Arc basati su Windows (anteprima)". In alternativa, è possibile seguire la procedura manuale nella [documentazione di Azure Arc](../azure-arc/servers/manage-vm-extensions.md#enable-extensions-from-the-portal).
...
1. Dall'intestazione laterale del Centro sicurezza, aprire la pagina **raccomandazioni** e selezionare le **vulnerabilità di raccomandazione nei server SQL sui computer da correggere (anteprima)**. Per altre informazioni, vedere [raccomandazioni del Centro sicurezza](security-center-recommendations.md). 
    ...
    :::image type="content" source="./media/security-center-advanced-iaas-data/all-servers-view.png" alt-text="I risultati della valutazione della vulnerabilità nei computer SQL Server devono essere corretti (anteprima)":::
...
Gli avvisi di Azure Defender sono disponibili nella pagina degli avvisi del Centro sicurezza, nella scheda sicurezza della risorsa, nel [dashboard di Azure Defender](azure-defender-dashboard.md)o tramite il collegamento diretto nei messaggi di posta elettronica di avviso.
