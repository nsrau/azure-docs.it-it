...
description: Questo documento consente di usare il controllo adattivo delle applicazioni nel Centro sicurezza di Azure per whitelist le applicazioni in esecuzione nelle macchine di Azure.This document helps you use adaptive application control in Azure Security Center to whitelist applications running in Azure machines.
...
Il controllo adattivo delle applicazioni è una soluzione end-to-end intelligente, automatizzata e end-to-end del Centro sicurezza di Azure che consente di controllare quali applicazioni possono essere eseguite nelle macchine Azure e non Azure (Windows e Linux). Tra gli altri vantaggi, questo aiuta a indurire le macchine contro il malware. Il Centro sicurezza usa l'apprendimento automatico per analizzare le applicazioni in esecuzione nei computer e crea un elenco Consenti da questa funzionalità. Questa funzionalità semplifica notevolmente il processo di configurazione e gestione dei criteri degli elenchi di applicazioni consentiti, consentendo di:
...
> [!NOTE]
> Per le macchine non Azure e Linux, i controlli adattivi delle applicazioni sono supportati solo in modalità di controllo.

## <a name="how-to-enable-adaptive-application-controls"></a>Come si abilitano i controlli delle applicazioni adattivi?

I controlli adattivi delle applicazioni consentono di definire un set di applicazioni che possono essere eseguite su gruppi di computer configurati. Questa funzionalità è disponibile sia per Azure che per Windows non Azure (tutte le versioni, classiche o di Azure Resource Manager) e per le macchine Linux.This feature is available for both Azure and non-Azure Windows (all versions, classic, or Azure Resource Manager) and Linux machines. Utilizzare la procedura seguente per configurare gli elenchi di applicazioni consentite:Use the following steps to configure your application allow lists:
...
1. Selezionare la scheda **Consigliato** per un elenco di gruppi con i suggerimenti per il controllo delle applicazioni:

   ![Consigliato](./media/security-center-adaptive-application/security-center-adaptive-application-fig3.png)

   L'elenco include:

   - **Nome gruppo**: Il nome della sottoscrizione e del gruppo
...
4. Esaminare le applicazioni in ognuno degli elenchi e deselezionare quelle a cui non si vuole applicare il controllo delle applicazioni. Ogni elenco include:
   ...
   - **EXPLOITABLE**: un'icona di avviso indica se un'applicazione specifica può essere utilizzata da un utente malintenzionato per ignorare un elenco di applicazioni consentite. È consigliabile esaminare queste applicazioni prima di approvarle.
...
5. Dopo avere selezionato le opzioni desiderate, scegliere **Crea**. <br>
   Dopo aver selezionato Crea, Centro sicurezza di Azure crea automaticamente le regole appropriate nella soluzione predefinita dell'elenco Consenti dell'applicazione disponibile nei server Windows (AppLocker).
...
1. Per modificare e monitorare un gruppo configurato con un criterio elenco consenti applicazione, tornare alla pagina **Controlli applicazioni adattivi** e selezionare **CONFIGURATO** in Gruppi di **macchine virtuali:**
   ...
   - **Nome gruppo**: il nome della sottoscrizione e del gruppo
   - **Macchine virtuali e computer:** il numero di macchine virtuali nel gruppo
   - **Modalità**: La modalità di controllo registrerà i tentativi di esecuzione di applicazioni che non sono nell'elenco Consenti; L'applicazione non consentirà l'esecuzione delle applicazioni a meno che non siano nell'elenco Consenti
...
4. In **Estensione dei criteri** aggiungere il percorso di qualsiasi applicazione da consentire. Dopo aver aggiunto questi percorsi, il Centro sicurezza aggiorna i criteri dell'elenco Consenti applicazioni nelle macchine virtuali all'interno del gruppo selezionato di VMS e crea le regole appropriate per queste applicazioni, oltre alle regole già presenti.
...
## <a name="move-a-vm-from-one-group-to-another"></a>Spostare una macchina virtuale da un gruppo a un altroMove a VM from one group to another

 Quando si sposta una macchina virtuale da un gruppo a un altro, i criteri di controllo dell'applicazione applicati vengono modificati alle impostazioni del gruppo in cui è stata spostata. È anche possibile spostare una macchina virtuale da un gruppo configurato a un gruppo non configurato, il che comporta la rimozione di qualsiasi criterio di controllo dell'applicazione precedentemente applicato alla macchina virtuale.

 1. Nella pagina **Controlli dell'applicazione adattivi,** nella scheda **CONFIGURATO** fare clic sul gruppo a cui appartiene attualmente la macchina virtuale da spostare.
1. Fare clic su **Macchine virtuali e computer configurati**.
1. Fare clic sui tre puntini nella riga della macchina virtuale da spostare e fare clic su **Sposta**. Viene visualizzata la finestra Sposta computer in **un gruppo diverso.**

    ![Protezione](./media/security-center-adaptive-application/adaptive-application-move-group.png)

 1. Selezionare il gruppo in cui spostare la macchina virtuale, fare clic su **Sposta computer**e quindi su **Salva**.
...
In this document, you learned how to use adaptive application control in Azure Security Center to whitelist applications running in Azure and non-Azure VMs. Per ulteriori informazioni sul Centro sicurezza di Azure, vedere gli argomenti seguenti:
