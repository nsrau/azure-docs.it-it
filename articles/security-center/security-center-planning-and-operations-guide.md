...
Questa guida illustra una serie di passaggi e attività che è possibile eseguire per ottimizzare l'uso del Centro sicurezza in base ai requisiti di sicurezza e al modello di gestione cloud dell'organizzazione. Per poter sfruttare tutti i vantaggi del Centro sicurezza, è importante sapere come i vari utenti o team dell'organizzazione useranno il servizio per soddisfare le esigenze di gestione e sviluppo sicuri, di monitoraggio, di governance e di risposta agli eventi imprevisti. Le aree principali da considerare quando si intende usare il Centro sicurezza sono:
...
- Può anche usare un sistema di creazione di ticket per tenere traccia delle modifiche e popola le raccomandazioni usando [API](https://msdn.microsoft.com/library/mt704034.aspx).
...
- Può usare un dashboard esistente e popola gli avvisi usando [API](https://msdn.microsoft.com/library/mt704034.aspx).
...
Il Centro sicurezza usa il [controllo degli accessi in base al ruolo](../active-directory/role-based-access-control-configure.md), che offre [ruoli predefiniti](../active-directory/role-based-access-built-in-roles.md) che possono essere assegnati a utenti, gruppi e servizi in Azure. Quando un utente apre il Centro sicurezza, vengono visualizzate unicamente le informazioni relative alle risorse a cui ha accesso. All'utente viene infatti assegnato il ruolo Proprietario, Collaboratore o Lettore per la sottoscrizione o il gruppo di risorse a cui appartiene la risorsa. Con gli utenti fittizi precedenti, sarebbe necessario il controllo degli accessi in base al ruolo illustrato di seguito:
...
## Criteri di sicurezza e raccomandazioni
I criteri di sicurezza definiscono il set di controlli consigliato per le risorse all'interno della sottoscrizione o di un gruppo di risorse specificato. Nel Centro sicurezza è possibile definire i criteri in base ai requisiti di sicurezza della società e al tipo di applicazioni o al livello di riservatezza dei dati.
...
**Protezione degli endpoint**: se in una macchina virtuale non è abilitata una soluzione di protezione endpoint, il Centro sicurezza visualizza una raccomandazione a installarla. Se in locale è già stata adottata una soluzione di protezione endpoint preferita, è necessario stabilire se usare lo stesso antimalware per le macchine virtuali di Azure. Il Centro sicurezza di Azure offre diverse opzioni di protezione endpoint. È possibile usare un antimalware Microsoft gratuito o scegliere tra un elenco di soluzioni di protezione endpoint di partner integrati. Per altre informazioni su come distribuire antimalware con il Centro sicurezza di Azure, vedere [Install Endpoint Protection in Azure Security Center](security-center-install-endpoint-protection.md) (Installare Endpoint Protection nel Centro sicurezza di Azure).

**Aggiornamenti di sistema**: il Centro sicurezza di Azure identifica le macchine virtuali in cui non sono presenti aggiornamenti della sicurezza o aggiornamenti critici del sistema operativo per IaaS e Servizi cloud (PaaS). Valutare chi sarà responsabile dell'applicazione degli aggiornamenti necessari e le relative modalità di applicazione. Molte organizzazioni usano WSUS, Windows Update o un altro strumento.
...
**Crittografia dei dischi**: se sono presenti dischi di macchina virtuale non crittografati, il Centro sicurezza di Azure visualizza una raccomandazione ad applicare Crittografia dischi di Azure, che usa BitLocker per Windows e DM-Crypt per Linux per la crittografia del volume dei dischi di dati e del sistema operativo. La raccomandazione reindirizza a una [guida dettagliata](security-center-disk-encryption.md) con istruzioni su come eseguire tale crittografia.
...
La pianificazione dei requisiti sarà diversa per ogni scenario. Per informazioni dettagliate sui singoli scenari, vedere il [white paper su Crittografia dischi di Azure](https://gallery.technet.microsoft.com/Azure-Disk-Encryption-for-a0018eb0).

**Web application firewall**: il Centro sicurezza di Azure identifica le macchine virtuali che eseguono applicazioni Web e visualizza una raccomandazione a installare un web application firewall (WAF). Valutare le soluzioni partner disponibili per determinare la soluzione più adatta all'organizzazione e come verrà concessa la relativa licenza. I partner potrebbero supportare modelli Bring Your Own License e/o con pagamento in base al consumo. Per altre informazioni su come distribuire un web application firewall nelle VM di Azure con il Centro sicurezza di Azure, vedere [Aggiungere un web application firewall nel Centro sicurezza di Azure](security-center-add-web-application-firewall.md).

**Firewall di nuova generazione**: consente di effettuare il provisioning di un'appliance virtuale di fornitori leader come Check Point e presto Cisco e Fortinet. Estende le protezioni di rete oltre i gruppi di sicurezza di rete, incorporati in Azure. Il Centro sicurezza individuerà le distribuzioni per cui è consigliabile un firewall di nuova generazione e consentirà di effettuare il provisioning di un'appliance virtuale.
...
Il Centro sicurezza visualizza una raccomandazione a specificare i dettagli dei contatti di sicurezza per la sottoscrizione di Azure. Queste informazioni verranno usate da Microsoft per contattare l'utente se il Microsoft Security Response Center (MSRC) rileva che un'entità illegale o non autorizzata ha effettuato l'accesso ai dati del cliente. Per altre informazioni su come abilitare questa raccomandazione, vedere [Provide security contact details in Azure Security Center](security-center-provide-security-contact-details.md) (Specificare i dettagli dei contatti di sicurezza nel Centro sicurezza di Azure).
...
> [AZURE.NOTE] Per risolvere i problemi relativi all'agente di monitoraggio della sicurezza di Azure, vedere [Guida alla risoluzione dei problemi del Centro sicurezza di Azure](security-center-troubleshooting-guide.md).
...
> [AZURE.NOTE] Per un elenco delle VM supportate, vedere [Domande frequenti sul Centro sicurezza di Azure](security-center-faq.md).
...
Quando si abilita per la prima volta la raccolta dati nel Centro sicurezza di Azure per l'ambiente Azure corrente, esaminare tutte le raccomandazioni nel pannello **Raccomandazioni** o nelle singole risorse (**Macchina virtuale**, **Rete**, **SQL** e **Applicazione**).
...
2.	Per conoscere gli eventuali rischi di sicurezza identificati per la nuova risorsa, visualizzare **Raccomandazioni**.
...
> [AZURE.NOTE] Per tale scopo è possibile usare la [guida alla gestione degli eventi imprevisti della sicurezza nei computer](http://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-61r2.pdf) del National Institute of Standards and Technology (NIST).
