...
Se si apre la console di gestione dei servizi (Services. msc), viene visualizzato anche il servizio Log Analytics Agent in esecuzione come illustrato di seguito:

![Servizi](./media/security-center-troubleshooting-guide/security-center-troubleshooting-guide-fig5.png)

Per visualizzare la versione dell'agente disponibile, aprire **Gestione attività**, nella scheda **processi** individuare il **servizio agente di log Analytics**, fare clic con il pulsante destro del mouse su di esso e scegliere **Proprietà**. Nella scheda **Dettagli** cercare la versione del file, come illustrato di seguito:

![File](./media/security-center-troubleshooting-guide/security-center-troubleshooting-guide-fig6.png)

## <a name="log-analytics-agent-installation-scenarios"></a>Scenari di installazione dell'agente Log Analytics
...
## <a name="monitoring-agent-health-issues"></a>Monitoraggio dei problemi di integrità dell'agente<a name="mon-agent"></a>
...
| Stato dell'alimentazione disattivato | La macchina virtuale è stata arrestata.  È possibile installare l'agente di Log Analytics solo in una macchina virtuale che esegue. | Riavviare la VM. |
...
| L'installazione non è riuscita - L'agente locale è già installato | Installazione dell'agente di Log Analytics non riuscita. Il Centro sicurezza ha identificato un agente locale (Log Analytics o System Center Operations Manager) già installato nella macchina virtuale. Per evitare la configurazione multihosting, in cui la macchina virtuale invia una segnalazione a due aree di lavoro separate, l'installazione dell'agente di Log Analytics è stata arrestata. | È possibile risolvere il problema in due modi: [installare manualmente l'estensione](../azure-monitor/learn/quick-collect-azurevm.md#enable-the-log-analytics-vm-extension) e connetterla all'area di lavoro specifica. In alternativa, configurare l'area di lavoro da usare come area di lavoro predefinita e abilitare il provisioning automatico dell'agente.  Vedere [Abilitare il provisioning automatico](security-center-enable-data-collection.md). |
| L'agente non riesce a connettersi all'area di lavoro | Agente di Log Analytics installato ma non riuscito a causa della connettività di rete.  Verificare che sia disponibile l'accesso a Internet o che un proxy HTTP corretto sia stato configurato per l'agente. | Vedere Requisiti di rete di Monitoring Agent. |
...
Alcuni problemi possono essere identificati usando le linee guida contenute in questo articolo, altri sono documentati nel [forum](https://social.msdn.microsoft.com/Forums/en-US/home?forum=AzureSecurityCenter)pubblico del Centro sicurezza. Tuttavia, se è necessario ulteriore risoluzione dei problemi, è possibile aprire una nuova richiesta di supporto utilizzando **portale di Azure** come illustrato di seguito:
