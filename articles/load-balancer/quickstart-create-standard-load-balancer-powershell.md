...
Per accedere alle macchine virtuali con una connessione RDP, sono necessari indirizzi IP pubblici per le macchine virtuali. 

Usare [New-AzPublicIpAddress](/powershell/module/az.network/new-azpublicipaddress) per creare indirizzi IP pubblici standard per:

#### <a name="vm1"></a>VM1

* Denominata **myVMPubIP1**.
* Nel gruppo di risorse **myResourceGroupLB**.
* Nella posizione **eastus**.
* SKU **Standard**.
* Allocazione **static** per l'indirizzo IP.
...
Creare un gruppo di sicurezza di rete con [New-AzNetworkSecurityRuleConfig](/powershell/module/az.network/new-aznetworksecurityruleconfig):

* Denominato **myNSGRuleRDP**.
* Descrizione **Allow RDP**.
* Accesso **Allow**.
* Protocollo **TCP**.
* Direzione **Inbound**.
* Priorità **1000**.
...
1. Ottenere gli indirizzi IP pubblici delle tre macchine virtuali usando [Get-AzPublicIPAddress](/powershell/module/az.network/get-azpublicipaddress).
...
2. Creare connessioni Desktop remoto con **myVM1**, **myVM2** e **myVM3** usando gli indirizzi IP pubblici delle macchine virtuali.

3. Immettere le credenziali per ogni VM per avviare la sessione RDP.

4. Avviare Windows PowerShell in ogni macchina virtuale e usare i comandi seguenti per installare il server IIS e aggiornare il file htm predefinito.
...
Per ottenere l'indirizzo IP pubblico del servizio di bilanciamento del carico, usare [Get-AzPublicIPAddress](/powershell/module/az.network/get-azpublicipaddress):

* Denominato **MyPublicIP**.
* Nel gruppo di risorse **myResourceGroupLB**.
...
Quando non servono più, è possibile usare il comando [Remove-AzResourceGroup](/powershell/module/az.resources/remove-azresourcegroup) per rimuovere il gruppo di risorse, il servizio di bilanciamento del carico e tutte le risorse correlate. 
