...
#### <a name="default-snat-ports-allocated"></a><a name="snatporttable"></a>Porte SNAT predefinite allocate
...
| V| NAT in uscita per scenari con Load Balancer Standard interno| Quando si usa un Load Balancer Standard interno, NAT in uscita non è disponibile se non è stata dichiarata in modo esplicito una connettività in uscita. È possibile definire la connettività in uscita usando una regola in uscita per creare una connettività in uscita per le macchine virtuali dietro una Load Balancer Standard interna con i passaggi seguenti: 1. Creare un Load Balancer Standard pubblico. 2. Creare un pool back-end e posizionare le macchine virtuali in un pool back-end del Load Balancer pubblico oltre al Load Balancer interno. 3. Configurare una regola in uscita per il Load Balancer pubblico per programmare la conversione NAT in uscita per queste macchine virtuali.|
