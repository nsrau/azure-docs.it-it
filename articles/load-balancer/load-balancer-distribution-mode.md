...
Quando l' `LoadBalancerDistribution` elemento non è presente, Azure Load Balancer usa l'algoritmo di cinque tuple predefinito.
...
Come descritto in precedenza, impostare l' `LoadBalancerDistribution` elemento su SourceIP per l'affinità con due Tuple, sourceIPProtocol per l'affinità a tre Tuple o None per nessuna affinità (affinità a cinque tuple).
