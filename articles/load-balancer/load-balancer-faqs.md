...
Vedere la [Guida alla tecnologia](/azure/architecture/guide/technology-choices/load-balancing-overview)  del servizio di bilanciamento del carico per i servizi di bilanciamento del carico disponibili e gli usi consigliati per ognuno di essi.

## <a name="where-can-i-find-load-balancer-arm-templates"></a>Dove è possibile trovare Load Balancer modelli ARM?
Vedere l' [elenco dei modelli di avvio rapido Azure Load Balancer](/azure/templates/microsoft.network/loadbalancers#quickstart-templates) per i modelli ARM delle distribuzioni comuni.
