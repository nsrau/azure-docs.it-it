...
Usare il comando [az network lb create](https://docs.microsoft.com/cli/azure/network/lb?view=azure-cli-latest) per creare un servizio Azure Load Balancer pubblico denominato **myLoadBalancer**, che include un pool front-end denominato **myFrontEnd** e un pool back-end denominato **myBackEndPool**, associato all'indirizzo IP pubblico **myPublicIP** creato nel passaggio precedente. Usare `--sku basic` per creare una risorsa Load Balancer Basic. Microsoft consiglia di scegliere SKU Standard per i carichi di lavoro di produzione.
