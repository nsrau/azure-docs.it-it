...
# Analisi dei log per il servizio di bilanciamento del carico di Azure 
...
Accedere al [portale di Azure](http://portal.azure.com). Se non lo si è già fatto, [creare un bilanciamento del carico](load-balancer-internet-arm-ps.md) prima di continuare.
...
Questo log viene generato solo se è stato abilitato per ogni bilanciamento del carico, come indicato in precedenza. I dati vengono archiviati nell'account di archiviazione specificato quando è stata abilitata la registrazione. Vengono registrati i dati seguenti:
