...
Ogni indirizzo IP pubblico da tutti i front-end di una regola in uscita rende disponibili fino a 51.200 porte temporanee da usare come porte SNAT.  Load Balancer alloca le porte SNAT in multipli di 8. Se si specifica un valore non divisibile per 8, l'operazione di configurazione viene rifiutata.  Se si tenta di allocare più porte SNAT di quante ne siano disponibili in base al numero di indirizzi IP pubblici, l'operazione di configurazione viene rifiutata.  Ad esempio, se si allocano 10.000 porte per ogni macchina virtuale e 7 VM in un back-end pool si condivide un singolo indirizzo IP pubblico, la configurazione è rifiutato (7 x 10.000 porte > 51.200 SNAT porte SNAT).  È possibile aggiungere altri indirizzi IP al front-end della regola in uscita per abilitare questo scenario.
