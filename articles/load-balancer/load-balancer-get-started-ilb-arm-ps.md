...
Per ulteriori informazioni sui componenti del servizio di bilanciamento del carico, vedere [Azure Load Balancer Components](components.md).
...
* Una regola di bilanciamento del carico: consente di bilanciare il carico di tutto il traffico in ingresso sulla porta pubblica 80 alla porta locale 80 nel pool di indirizzi back-end.
* Una [regola di bilanciamento del carico delle porte a disponibilità elevata](load-balancer-ha-ports-overview.md) per bilanciare il carico di tutto il traffico in ingresso verso tutte le porte per semplificare gli scenari a disponibilità elevata per la ILB standard.
