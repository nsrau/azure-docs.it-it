...
* Pool di indirizzi back end: configurazione delle interfacce di rete che ricevono il traffico con carico bilanciato proveniente dal pool di indirizzi IP front end.
...
Per altre informazioni sui componenti del servizio di bilanciamento del carico con Azure Resource Manager, vedere [Supporto di Azure Resource Manager per il bilanciamento del carico](load-balancer-arm.md).
...
Assicurarsi di avere la versione di produzione più recente del modulo Azure per PowerShell e di avere configurato correttamente PowerShell per l'accesso alla sottoscrizione di Azure.
...
Gestione risorse di Azure richiede che tutti i gruppi di risorse specifichino un percorso che viene usato come percorso predefinito per le risorse in questo gruppo di risorse. Assicurarsi che tutti i comandi per creare un servizio di bilanciamento del carico usino lo stesso gruppo di risorse.

Nell'esempio precedente sono stati creati un gruppo di risorse denominato **NRP-RG** e la località **West US**.

## <a name="create-a-virtual-network-and-a-private-ip-address-for-a-front-end-ip-pool"></a>Creare la rete virtuale e un indirizzo IP privato per un pool di indirizzi IP front-end
...
## <a name="create-a-front-end-ip-pool-and-back-end-address-pool"></a>Creare un pool di indirizzi IP front-end e un pool di indirizzi back-end

Configurazione di un pool di indirizzi IP front-end per il traffico di rete in ingresso del servizio di bilanciamento del carico e di un pool di indirizzi back-end per ricevere il traffico sottoposto a bilanciamento del carico.

### <a name="step-1"></a>Passaggio 1

Creare un pool IP front-end usando l'indirizzo IP privato 10.0.2.5 per la subnet 10.0.2.0/24 che è l'endpoint del traffico di rete in ingresso.
...
## <a name="create-load-balancing-rules-nat-rules-probe-and-load-balancer"></a>Creare regole di bilanciamento del carico, regole NAT, probe e il servizio di bilanciamento del carico

Dopo aver creato il pool di indirizzi IP front-end e il pool di indirizzi back-end, creare le regole che fanno parte della risorsa di bilanciamento carico:
...
* Regola NAT secondo cui tutto il traffico in ingresso sulla porta 3441 passa alla porta 3389.
* Una seconda regola NAT secondo cui tutto il traffico in ingresso sulla porta 3442 passa alla porta 3389.
...
In questo passaggio viene creata un'interfaccia di rete che fa parte del pool di back-end del servizio di bilanciamento del carico e che associa la prima regola NAT per RDP per questa interfaccia di rete:
...
Il risultato finale mostra l'output seguente:
...
Se non è ancora stata eseguita questa operazione, caricare la risorsa di bilanciamento del carico in una variabile. La variabile usata viene chiamata $lb e usa gli stessi nomi della risorsa di bilanciamento del carico creata nei passaggi precedenti.
...
Nell'esempio seguente si aggiunge a un servizio di bilanciamento del carico esistente una nuova regola NAT in ingresso usando la porta 81 nel front-end e la porta 8181 per il pool back-end
