...
## Configurazione di Azure Load Balancer
...
Tutte le risorse nel cloud richiedono un indirizzo IP pubblico per poter essere raggiungibili da Internet. Nell'infrastruttura cloud Microsoft Azure usa indirizzi IP non instradabili per le proprie risorse. Azure usa Network Address Translation (NAT) con indirizzi IP pubblici per comunicare con Internet.

## Modelli di distribuzione di Azure

È importante comprendere le differenze tra il [modello di distribuzione](../resource-manager-deployment-model.md) classica di Azure e quello di Azure Resource Manager. Azure Load Balancer viene configurato in modo diverso in ogni modello.

### Modello di distribuzione classica di Azure

In questo modello al servizio cloud vengono assegnati un indirizzo IP pubblico e un nome FQDN. Le macchine virtuali distribuite entro il limite di un servizio cloud possono essere raggruppate per usare un servizio di bilanciamento del carico. Il servizio di bilanciamento del carico esegue la conversione delle porte e distribuisce il traffico di rete ricevuto sull'indirizzo IP pubblico del servizio cloud.
...
Nel modello di distribuzione di Resource Manager non è necessario creare un servizio cloud. Il bilanciamento del carico può essere creato, in modo esplicito, per instradare il traffico tra le macchine virtuali.

Nel modello Resource Manager un indirizzo IP pubblico è la risorsa e può essere associato a un'etichetta di dominio o a un nome DNS. In questo caso l'indirizzo IP pubblico è associato alla risorsa del servizio di bilanciamento del carico. Le regole del servizio di bilanciamento del carico e le regole NAT in ingresso usano l'indirizzo IP pubblico come endpoint Internet per le risorse che ricevono il traffico di rete con carico bilanciato.
