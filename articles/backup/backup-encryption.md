...
Per altre informazioni, vedere [Crittografia di Archiviazione di Azure per dati inattivi](../storage/common/storage-service-encryption.md). Vedere le domande [frequenti su backup di Azure](./backup-azure-backup-faq.md#encryption) per rispondere a eventuali domande sulla crittografia.
...
Backup di Azure consente anche di eseguire il backup delle macchine virtuali di Azure che usano la chiave per la [crittografia del servizio di archiviazione](../storage/common/storage-service-encryption.md). La chiave usata per crittografare i dischi è archiviata nel Azure Key Vault e gestita dall'utente. Crittografia del servizio di archiviazione (SSE) con chiavi gestite dal cliente differisce da crittografia dischi di Azure, perché ADE USA BitLocker (per Windows) e DM-crypt (per Linux) per eseguire la crittografia in-Guest, SSE crittografa i dati nel servizio di archiviazione e consente di usare qualsiasi sistema operativo o immagine per le macchine virtuali. Per altri dettagli, vedere [crittografia dei dischi gestiti con chiavi gestite dal cliente](../virtual-machines/windows/disk-encryption.md#customer-managed-keys) .
