...
In questo articolo viene fornita un'introduzione sull'utilizzo del servizio Backup di Microsoft Azure per proteggere i server DPM di System Center e i carichi di lavoro. Le informazioni dell'articolo riguardano:

- Il funzionamento relativo al backup del server DPM
- I prerequisiti per eseguire un backup senza problemi
- Gli errori tipici rilevati e come gestirli
- Scenari supportati

DPM di System Center esegue il backup di file e dati delle applicazioni. I dati sottoposti a backup su DPM possono essere archiviati su nastro, disco oppure sottoposti a backup in Azure utilizzando il servizio Backup di Microsoft Azure. DPM interagisce con Backup di Azure nei modi seguenti:
...
3. In **Name** immettere un nome descrittivo per identificare l'insieme di credenziali. È necessario che il nome sia univoco per ogni sottoscrizione.
...
5. In **Subscription** immettere la sottoscrizione di Azure da utilizzare con l'insieme di credenziali per il backup.
...
- Per installare l'agente su più server DPM, è possibile inserire il file del programma di installazione in una risorsa di rete condivisa oppure utilizzare criteri di gruppo o prodotti di gestione come System Center Configuration Manager.
...
3. Nella pagina **Impostazioni di installazione**, selezionare la **cartella di installazione** e il **percorso cache**.
    ...
    Il percorso della cartella cache predefinito è <system drive>:\Programmi\Azure Backup Agent. Nel percorso della cache, tramite la procedura di installazione viene creata una cartella denominata **Scratch** all'interno della cartella **Azure Backup Agent**. Il percorso della cache deve disporre di almeno 2,5 GB di spazio libero oppure di una quantità pari al 10% della dimensione dei dati che verranno sottoposti a backup in Azure. Solo gli amministratori del sistema locale e i membri del gruppo Administrators possono accedere alla directory della cache al fine di impedire attacchi di tipo Denial of Service.

4. Nella pagina **Configurazione proxy**, regolare le impostazioni personalizzate del proxy affinché l'agente possa connettersi ad Azure. Se non si configurano le impostazioni, nel server DPM verranno utilizzate quelle predefinite per l'accesso a Internet. Tenere presente che se si usa un server proxy che necessita di autenticazione, è necessario inserire i dettagli in questa pagina.
5. È opportuno attivare gli aggiornamenti nella pagina **Acconsentire esplicitamente gli aggiornamenti Microsoft**. Se gli aggiornamenti automatici sono stati già attivati per il server, è possibile ignorare questo passaggio. Tenere presente le impostazioni di Microsoft Update sono valide per tutti gli aggiornamenti dei prodotti Microsoft e non si riferiscono esclusivamente ad Azure Backup Agent.
6. Viene visualizzata la pagina **Installazione**. La procedura di installazione consente di verificare che venga installato il software richiesto; successivamente, completa l'installazione. Al termine dell'operazione, viene visualizzato un messaggio di conferma in merito all'installazione dell'agente di Backup di Azure. A questo punto, è possibile verificare la disponibilità di aggiornamenti. È consigliabile abilitare la verifica relativa alla disponibilità degli aggiornamenti.
...
11. Fare clic su **Register**.
    ...
    >[AZURE.NOTE]Backup di Azure non conserva mai la passphrase; per questo motivo, se viene persa, i dati non possono essere ripristinati oppure recuperati. Si consiglia di salvare la chiave in un percorso esterno.

Quando si specifica una passphrase e si fa clic su **Fine**, è necessario attendere alcuni secondi prima che l'agente registri il server di produzione nell'insieme di credenziali di backup. Al termine della registrazione nell'insieme di credenziali, viene visualizzata la pagina riepilogativa denominata **Registrazione server**.

## Requisiti e limitazioni

- DPM può essere eseguito come server fisico o come macchina virtuale Hyper-V installata in System Center 2012 SP1 o System Center 2012 R2. Inoltre, è possibile eseguirlo come macchina virtuale di Azure in System Center 2012 R2 (con almeno aggiornamento cumulativo 3 di DPM 2012 R2) oppure come macchina virtuale di Windows in VMWare con System Center 2012 R2 e almeno il relativo aggiornamento cumulativo 5.
- Se DPM viene eseguito con System Center 2012 SP1, è necessario installare l'aggiornamento cumulativo 2 per System Center Data Protection Manager SP1. Tale procedura di installazione è richiesta prima di poter eseguire quella di Azure Backup Agent.
- È necessario che nel server DPM siano installati Windows PowerShell e .Net Framework 4.5.
- DPM può eseguire il backup della maggior parte dei carichi di lavoro del servizio Backup di Azure. Per visualizzare un elenco completo degli elementi supportati, consultare gli elementi supportati di Backup di Azure riportati di seguito.
- Utilizzando l'opzione "Copia su nastro", non è possibile ripristinare i dati memorizzati in Backup di Azure.
- È necessario disporre di un account di Azure su cui sia abilitata la funzionalità Backup di Azure. Se non si dispone di un account, è possibile creare un account di valutazione gratuita in pochi minuti. Informazioni sui [prezzi di Backup di Azure](https://azure.microsoft.com/pricing/details/backup/).
- Per utilizzare Backup di Azure, è necessario che Azure Backup Agent sia installato sui server da sottoporre a backup. Ogni server deve disporre di almeno 2,5 GB di spazio di archiviazione locale dedicato al percorso della cache. Tuttavia, per il percorso della cache si consiglia di disporre di almeno 15 GB di spazio di archiviazione locale
...
Il backup in Azure è supportato per i seguenti tipi di file:
