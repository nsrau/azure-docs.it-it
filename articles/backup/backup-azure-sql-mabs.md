---
title: Eseguire il backup di SQL Server tramite il server di backup di AzureBack up SQL Server by using Azure Backup Server
description: In questo articolo viene appresa la configurazione per eseguire il backup dei database di SQL Server utilizzando il server di backup di Microsoft Azure (MABS).
ms.topic: conceptual
ms.date: 03/24/2017
...
---
# <a name="back-up-sql-server-to-azure-by-using-azure-backup-server"></a>Eseguire il backup di SQL Server in Azure usando il server di backup di AzureBack up SQL Server to Azure by using Azure Backup Server

Questo articolo consente di configurare i backup dei database di SQL Server utilizzando Microsoft Azure Backup Server (MABS).

Per eseguire il backup di un database di SQL Server e ripristinarlo da Azure:To back up a SQL Server database and recover it from Azure:
...
1. Ripristinare il database in Azure.Recover the database in Azure.

## <a name="before-you-start"></a>Prima di iniziare

Prima di iniziare, assicurarsi di aver installato e preparato il server di Backup di [Azure.](backup-azure-microsoft-azure-backup.md)
...
1. In Server di Backup di Azure selezionare l'area di lavoro **Protezione.In** Azure Backup Server, select the Protection workspace.
1. Selezionare **Nuovo** per creare un gruppo protezione dati.

    ![Creare un gruppo protezione dati nel server di Backup di AzureCreate a protection group in Azure Backup Server](./media/backup-azure-backup-sql/protection-group.png)
...
1. Fare clic su **Avanti**. MABS mostra lo spazio di archiviazione complessivo disponibile. Mostra anche il potenziale utilizzo dello spazio su disco.

    ![Impostare l'allocazione dei dischi in MABS](./media/backup-azure-backup-sql/pg-storage.png)

    Per impostazione predefinita, MABS crea un volume per ogni origine dati (database di SQL Server). Il volume viene utilizzato per la copia di backup iniziale. In questa configurazione, Gestione dischi logici (LDM) limita la protezione MABS a 300 origini dati (database sql Server). Per porre rimedio a questa limitazione, selezionare **Condividi percorso dati nel pool di archiviazione DPM**. Se si utilizza questa opzione, MABS utilizza un singolo volume per più origini dati. Questa configurazione consente a MABS di proteggere fino a 2.000 database di SQL Server.

    Se si seleziona **Aumenta automaticamente i volumi**, MABS può tenere conto dell'aumento del volume di backup man mano che i dati di produzione aumentano. Se non si seleziona **Aumenta automaticamente i volumi,** MABS limita l'archiviazione dei backup alle origini dati nel gruppo protezione dati.
1. Se sei un amministratore, puoi scegliere di trasferire automaticamente questo backup iniziale **in rete** e scegliere l'ora del trasferimento. In alternativa, scegliere di trasferire **manualmente** il backup. Quindi selezionare **Avanti**.
    ...
    ![Scegliere un metodo di creazione della replica in MABSChoose a replica-creation method in MABS](./media/backup-azure-backup-sql/pg-manual.png)

    La copia di backup iniziale richiede il trasferimento dell'intera origine dati (database di SQL Server). I dati di backup vengono spostati dal server di produzione (computer SQL Server) a MABS. Se questo backup è di grandi dimensioni, il trasferimento dei dati in rete potrebbe causare la congestione della larghezza di banda. Per questo motivo, gli amministratori possono scegliere di utilizzare supporti rimovibili per trasferire manualmente il **backup**iniziale. Oppure possono trasferire i dati **automaticamente in rete** in un momento specificato.
...
1. Scegliere quando eseguire una verifica di coerenza. Quindi selezionare **Avanti**.
    ...
    MABS può eseguire una verifica di coerenza sull'integrità del punto di backup. Calcola il checksum del file di backup nel server di produzione (il computer SQL Server in questo esempio) e i dati di backup per tale file in MABS. Se il controllo rileva un conflitto, si presuppone che il file di cui è stato eseguito il backup in MABS sia danneggiato. MABS corregge i dati di cui è stato eseguito il backup inviando i blocchi che corrispondono alla mancata corrispondenza del checksum. Poiché la verifica coerenza è un'operazione che richiede un utilizzo intensivo delle prestazioni, gli amministratori possono scegliere di pianificare la verifica di coerenza o di eseguirla automaticamente.
...
1. Gli amministratori possono scegliere le pianificazioni di backup e i criteri di conservazione adatti ai criteri dell'organizzazione.
    ...
    >
    > Usare Data Protection Manager (DPM) per pianificare i backup di Azure al termine dei backup su disco locale. Quando si segue questa procedura, il backup del disco più recente viene copiato in Azure.When follow this practice, the latest disk backup is copied to Azure.
    >
