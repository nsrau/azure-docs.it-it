...
L'URI POST ha parametri `{subscriptionId}`, `{vaultName}`, `{vaultresourceGroupName}`e `{fabricName}`. In questo esempio, il valore per i diversi parametri sarà il seguente:

- `{fabricName}` è *Azure*

- `{vaultName}` è *azurefilesvault*

- `{vaultresourceGroupName}` è *risorsa*
