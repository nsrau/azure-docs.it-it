...
2. Fornire le **credenziali di insieme** associate al server DPM, la cui data è in fase di ripristino, scegliere il **server DPM** dall'elenco di server DPM registrati con l'archivio di backup e fornire il **passphrase di crittografia** associato al server DPM contenente i dati in fase di ripristino.
...
8. Scegliere l'opzione relativa a **Crea copia**, **Salta**, o **Sovrascrittura**.
    - **Crea copia** creerà una copia del file nel caso in cui si verifichi un conflitto di nome.
    - **Salta** ignorerà il ripristino del file nel caso in cui si verifichi un conflitto di nome.
    - **Sovrascrittura** sovrascriverà la copia esistente nel percorso specificato nel caso di un conflitto di nomi.
