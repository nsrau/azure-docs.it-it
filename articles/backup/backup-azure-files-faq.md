...
Verificare se la condivisione file è già protetta nello stesso insieme di credenziali di Servizi di ripristino oppure se è stata eliminata di recente.
...
Quando si elimina una condivisione file, viene visualizzato l'elenco di backup che verranno eliminati se si procede, con la relativa richiesta di conferma. Non è possibile ripristinare una condivisione file eliminata.
...
È possibile accedere a tutti gli snapshot creati da Backup di Azure visualizzandoli nel portale, in PowerShell o nell'interfaccia della riga di comando. È possibile montare gli snapshot usando la procedura illustrata [qui](../storage/files/storage-how-to-use-files-snapshots.md#mount-a-file-share).
