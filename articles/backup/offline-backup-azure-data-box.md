...
1. Eseguire *MARSAgentInstaller. exe*e seguire *solo* i passaggi per [installare e registrare l'agente](https://docs.microsoft.com/azure/backup/backup-configure-vault#install-and-register-the-agent) nell'insieme di credenziali dei servizi di ripristino in cui si desidera archiviare i backup.
...
1. Selezionare **rileva dispositivo** nella pagina di **rilevamento del dispositivo data box** . Questa azione consente di eseguire l'analisi dell'agente MARS per i dischi Azure Data Box collegati localmente e di rilevarli.
    ...
    >
    >Se ad esempio il percorso del disco è `\\mydomain\myserver\disk1\` e *disk1* contiene una directory denominata *PageBlob*, viene `\\mydomain\myserver\disk1\`il percorso immesso nella pagina della procedura guidata dell'agente Mars.
    >
    >Se si [configura un dispositivo Azure Data Box 100 TB](#set-up-azure-data-box-devices), immettere `\\<DeviceIPAddress>\<StorageAccountName>_PageBlob` come percorso di rete del dispositivo.
...
Se nessun altro server dispone di un seeding offline configurato e nessun altro server dipende dall'applicazione `AzureOfflineBackup_<Azure User Id>`, eliminare questa applicazione. Selezionare **portale di Azure** > **Azure Active Directory** > **registrazioni app**.
...
1. Passare alla scheda **Gestisci applicazione certificato computer** > **Personal** e cercare il certificato con il nome `CB_AzureADCertforOfflineSeeding_<ResourceId>`.
...
5. Accedere al computer del registro di sistema *\ HKEY_LOCAL_MACHINE \Software\microsoft\windows Azure Backup\Config\CloudBackupProvider.* Fare clic con il pulsante destro del mouse su **CloudBackupProvider**e aggiungere un nuovo valore stringa con il nome `AzureADAppCertThumbprint_<Azure User Id>`.
    ...
    >
    >* Da PowerShell connesso ad Azure eseguire il comando `Get-AzureRmADUser -UserPrincipalName “Account Holder’s email as defined in the portal”`.
