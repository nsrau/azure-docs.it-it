...
4. Per visualizzare informazioni dettagliate su un avviso specifico, nell'elenco di eventi fare clic sull'avviso per aprire la sezione **Dettagli**.
    ...
    Per personalizzare gli attributi visualizzati nell'elenco, vedere [Visualizzare altri attributi degli eventi](backup-azure-monitor-vms.md)
...
### <a name="what-alert-types-are-available-for-azure-iaas-vm-backup"></a>Tipi di avviso disponibili per il backup di macchine virtuali IaaS di Azure
   ...
   | Avviso | Nessun avviso di avviso è attualmente disponibile per i backup di macchine Virtuali di Azure (ad esempio: errore di alcuni writer durante la creazione di uno snapshot) |
