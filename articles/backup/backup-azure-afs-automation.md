---
title: Distribuire e gestire i backup per le condivisioni file di Azure tramite PowerShell
...
Se non si ha familiarità con il servizio Backup di Azure, vedere l'articolo [Informazioni su Backup di Azure](backup-introduction-to-azure-backup.md) per una panoramica del servizio. Prima di iniziare, assicurarsi di conoscere le funzionalità di anteprima per il backup di condivisioni file di Azure documentate [qui](backup-azure-files.md).
...
> [!NOTE]
> Come indicato [qui](https://docs.microsoft.com/powershell/azure/install-azurerm-ps?view=azurermps-6.13.0), il supporto per nuove funzionalità nel modulo AzureRM è terminato nel novembre 2018. Di conseguenza, il supporto per il backup di condivisioni file di Azure viene fornito con il nuovo modulo PS "Az". È anche previsto l'onboarding nella versione di disponibilità generale del modulo Az.

Per iniziare:

1. [Scaricare la versione più recente di PowerShell 'Az'](https://docs.microsoft.com/powershell/azure/install-az-ps?view=azurermps-6.13.0) (la versione minima richiesta è: 0.7.0)
...
2. Cercare i cmdlet PowerShell di Azure Backup disponibili digitando il comando seguente:
    ...
    ![Elenco di Servizi di ripristino](./media/backup-azure-afs-automation/list-of-recoveryservices-ps-az.png)
...
5. Se si sta usando Backup di Azure per la prima volta, è necessario usare il cmdlet **Register-AzResourceProvider** per registrare il provider di Servizi di ripristino di Azure con la propria sottoscrizione.
...
Nei passaggi seguenti viene descritto come creare un insieme di credenziali dei servizi di ripristino.
...
Usare **Set-AzRecoveryServicesVaultContext** per impostare il contesto dell'insieme di credenziali. Una volta impostato il contesto dell'insieme di credenziali, si applica a tutti i cmdlet successivi. L'esempio seguente imposta il contesto per l'insieme di credenziali, *testvault*.
...
> [!NOTE]
> È previsto che l'impostazione del contesto dell'insieme di credenziali venga deprecato, in base alle linee guida di Azure PowerShell. Si consiglia agli utenti di passare l'ID dell'insieme di credenziali, come indicato di seguito

In alternativa, è possibile archiviare/recuperare l'ID dell'insieme di credenziali in cui si vuole eseguire un'operazione di PowerShell e passarlo al comando pertinente.
...
Prima di tutto di recuperare l'oggetto criteri pertinente con il cmdlet **Get-AzRecoveryServicesBackupProtectionPolicy**. È possibile usare questo cmdlet per ottenere un criterio specifico o per visualizzare i criteri associati a un tipo di carico di lavoro.
...
Il comando attende fino a quando il processo di configurazione protezione dati non viene completato e fornisce un output simile al seguente.
...
Il comando restituisce un processo di cui tenere traccia con un ID, come nell'esempio seguente.
...
### <a name="modify-protection-policy"></a>Modificare i criteri di protezione

Se si vuole modificare il criterio di protezione della condivisione file di Azure, usare **Enable-AzRecoveryServicesBackupProtection** con l'elemento di backup pertinente e il nuovo criterio di protezione.
...
## <a name="restore-azure-file-sharesazure-files"></a>Ripristinare condivisioni file di Azure/file di Azure
...
### <a name="fetching-recovery-points"></a>Recupero dei punti di recupero

Usare il cmdlet **Get-AzRecoveryServicesBackupRecoveryPoint** per elencare tutti i punti di recupero dell'elemento di backup. Nello script seguente la variabile **$rp** è una matrice di punti di recupero per l'elemento di backup selezionato negli ultimi sette giorni. La matrice viene ordinata in ordine inverso di tempo con il punto di ripristino più recente in posizione 0 nell'indice. Per scegliere il punto di ripristino, usare l'indicizzazione standard della matrice di PowerShell. Nell'esempio $rp [0] seleziona il punto di ripristino più recente.
...
Dopo aver selezionato il punto di recupero pertinente, ripristinare la condivisione file o il file nel percorso originale/alternativo, come illustrato di seguito.
...
#### <a name="restoring-an-azure-file-share"></a>Ripristino di una condivisione file di Azure
...
* ***TargetFileShareName***: condivisioni file nell'account di archiviazione in cui ripristinare il contenuto sottoposto a backup
* ***TargetFolder***: cartella nella condivisione file in cui vengono ripristinati i dati. Se il contenuto sottoposto a backup deve essere ripristinato nella cartella radice, fornire i valori della cartella di destinazione come stringa vuota
...
#### <a name="restoring-an-azure-file"></a>Ripristino di un file di Azure

Se si vuole ripristinare un singolo file invece di un'intera condivisione file, il singolo file deve essere identificato in modo univoco specificando i parametri seguenti.

* ***TargetStorageAccountName***: account di archiviazione in cui ripristinare il contenuto sottoposto a backup. L'account di archiviazione di destinazione deve trovarsi nella stessa posizione dell'insieme di credenziali.
* ***TargetFileShareName***: condivisioni file nell'account di archiviazione in cui ripristinare il contenuto sottoposto a backup
* ***TargetFolder***: cartella nella condivisione file in cui vengono ripristinati i dati. Se il contenuto sottoposto a backup deve essere ripristinato nella cartella radice, fornire i valori della cartella di destinazione come stringa vuota
* ***SourceFilePath***: percorso assoluto del file da ripristinare all'interno della condivisione file, sotto forma di stringa. È lo stesso percorso usato nel cmdlet di PowerShell ```Get-AzStorageFile```.
* ***SourceFileType***: indica se è selezionato un file o una directory. Accetta "Directory" o "File"
* ***ResolveConflict***: istruzione in caso di conflitto con i dati ripristinati. Accetta "Overwrite" o "skip"

Come si può notare, i parametri aggiuntivi riguardano solo il singolo file da ripristinare.

```powershell
Restore-AzRecoveryServicesBackupItem -RecoveryPoint $rp[0] -TargetStorageAccountName "TargetStorageAcct" -TargetFileShareName "DestAFS" -TargetFolder "testAzureFS_restored" -SourceFileType File -SourceFilePath "TestDir/TestDoc.docx" -ResolveConflict Overwrite
```

Verrà anche restituito un processo di cui tenere traccia con un ID, come illustrato in precedenza.

### <a name="restore-azure-file-shares-to-original-location"></a>Ripristinare condivisioni file di Azure nel percorso originale

In caso di ripristino nel percorso originale, non occorre specificare alcun parametro relativo alla destinazione. Solo ```ResolveConflict``` deve essere specificato
...
Le operazioni di backup e ripristino su richiesta restituiscono un processo con un ID, come mostrato [sopra](#trigger-an-on-demand-backup). Usare il cmdlet ```Get-AzRecoveryServicesBackupJobDetails``` per tenere traccia dell'avanzamento del processo e ottenere altri dettagli.
