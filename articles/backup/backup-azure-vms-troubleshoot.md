...
Un'altra procedura che può essere utile è eseguire il comando seguente da un prompt dei comandi con privilegi elevati (come amministratore).
...
2. Eliminare tutti i certificati **a** cui è stato rilasciato il modello di distribuzione classica o il **Generatore di certificati CRP di Windows Azure**:
   ...
   * In **Personal** > **certificati**personali, eliminare tutti i certificati in cui è stato **rilasciato a** è il modello di distribuzione classica o il **Generatore di certificati CRP di Windows Azure**.
...
* Installare la versione più recente dell'agente dal repository di distribuzione. Per informazioni dettagliate sul nome del pacchetto, vedere l' [Archivio dell'agente Linux](https://github.com/Azure/WALinuxAgent).
* Per le macchine virtuali create con il modello di distribuzione classica, [aggiornare la proprietà della macchina virtuale](https://docs.microsoft.com/azure/virtual-machines/troubleshooting/install-vm-agent-offline#use-the-provisionguestagent-property-for-classic-vms) e verificare che l'agente sia installato. Questo passaggio non è necessario per le macchine virtuali di Resource Manager.
...
* Per aggiornare l'agente di macchine virtuali Linux, seguire le istruzioni riportate nell'articolo [aggiornamento dell'agente di macchine virtuali Linux](../virtual-machines/linux/update-agent.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json).
...
* **Se più di quattro VM condividono lo stesso servizio cloud, distribuire le macchine virtuali in più criteri di backup**. Distribuire gli orari del backup in modo che non più di quattro backup delle macchine virtuali vengano avviati nello stesso momento. Provare a separare di almeno un'ora gli orari di inizio dei criteri.
