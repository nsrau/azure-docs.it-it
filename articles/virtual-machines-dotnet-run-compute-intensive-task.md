...
<h2>Per accedere in remoto alla macchina virtuale</h2>
...
Per iniziare a usare le code del bus di servizio in Azure, è innanzitutto necessario
...
1. Nel computer di sviluppo (che non deve essere la macchina virtuale creata dall'utente) scaricare [Azure SDK per .NET](http://azure.microsoft.com/develop/net/).
...
6. Modificare i segnaposto **your\_service\_bus\_namespace**, **your\_service\_bus\_owner** e **your\_service\_bus\_key** per usare rispettivamente i valori **namespace**, **Default Issuer** e **Default Key** del bus di servizio.
...
3. Aggiungere la libreria System Runtime Serialization. In Esplora soluzioni di Visual Studio, fare clic con il pulsante destro del mouse su **TSPClient**, scegliere su **Aggiungi riferimento**, quindi fare clic sulla scheda **.NET** e selezionare **System.Runtime.Serialization** come riferimento.
4. Usare il codice di esempio fornito al termine di questa sezione per i contenuti di **Program.cs**.
5. Modificare i segnaposto **your\_service\_bus\_namespace**, **your\_service\_bus\_owner** e **your\_service\_bus\_key** per usare rispettivamente i valori **namespace**, **Default Issuer** e **Default Key** del bus di servizio.
5. Compilare l'applicazione. Verrà creato **TSPClient.exe** nella cartella **bin** del progetto (**bin\release** o **bin\debug**, a seconda che la destinazione sia una build di rilascio o di debug). È possibile eseguire questo codice dal computer di sviluppo oppure copiare questo eseguibile e Microsoft.ServiceBus.dll in un computer sul quale verrà eseguita l'applicazione client (non è necessario che vena eseguita sulla macchina virtuale)
