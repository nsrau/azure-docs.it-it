...
Per altre informazioni, vedere integrare Hub eventi di Azure con collegamento privato di https://docs.microsoft.com/azure/event-hubs/private-link-serviceAzure:.
...
**Linee guida**: implementare sottoscrizioni e/o gruppi di gestione distinti per lo sviluppo, il test e la produzione. Gli spazi dei nomi di hub eventi devono essere separati da una rete virtuale con gli endpoint di servizio abilitati e contrassegnati in modo appropriato.
...
**Linee guida**: identificazione dei dati, classificazione e funzionalità di prevenzione della perdita non ancora disponibili per hub eventi di Azure. Implementare una soluzione di terze parti, se necessario ai fini della conformità.
...
**Linee guida**: usare gli alias di criteri di Azure nello spazio dei nomi "Microsoft. EventHub" per creare criteri personalizzati per avvisare, controllare e applicare le configurazioni di sistema. Sviluppare inoltre un processo e una pipeline per la gestione delle eccezioni dei criteri.
...
Usare identità gestite per fornire servizi di Azure con un'identità gestita automaticamente in Azure Active Directory (AD). Le identità gestite consentono di eseguire l'autenticazione a qualsiasi servizio che supporti l'autenticazione Azure AD, incluso Azure Key Vault, senza credenziali nel codice.
