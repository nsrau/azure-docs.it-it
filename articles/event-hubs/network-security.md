...
Questa funzionalità è utile negli scenari in cui gli hub eventi di Azure devono essere accessibili solo da determinati siti noti. Le regole del firewall consentono di configurare le regole per accettare il traffico proveniente da indirizzi IPv4 specifici. Ad esempio, se si usano hub eventi con [Azure Express Route][express-route], è possibile creare una **regola del firewall** per consentire il traffico proveniente solo dagli indirizzi IP dell'infrastruttura locale. 
...
L'integrazione di Hub eventi con [Virtual Network) Service Endpoints][vnet-sep] consente l'accesso sicuro alle funzionalità di messaggistica da carichi di lavoro come le macchine virtuali associate a reti virtuali, con il percorso del traffico di rete protetto in entrambe le estremità.
