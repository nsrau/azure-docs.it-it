...
Non è necessario creare una soluzione personalizzata per soddisfare questi requisiti. Gli SDK di hub eventi di Azure forniscono questa funzionalità. In .NET o Java SDK si usa un client di elaborazione eventi (EventProcessorClient) e in Python e Java script SDK si usa EventHubConsumerClient. Nella versione precedente di SDK, era l'host processore di eventi (EventProcessorHost) che supportava queste funzionalità.
...
I record di proprietà della partizione nell'archivio Checkpoint tengono traccia dello spazio dei nomi di hub eventi, nome dell'hub eventi, gruppo di consumer, identificatore del processore di eventi (noto anche come proprietario), ID partizione e ora dell'Ultima modifica.



| Spazio dei nomi di Hub eventi               | Nome dell'hub eventi | **Gruppo di consumer** | Proprietario                                | Partition ID | Ora dell'Ultima modifica  |
| ---------------------------------- | -------------- | :----------------- | :----------------------------------- | :----------- | :------------------ |
