...
Acquisizione hub eventi consente di specificare il proprio account di archiviazione BLOB di Azure e il proprio account di archiviazione BLOB di Azure o l'account di archiviazione di Azure Data Lake, usati per archiviare i dati acquisiti. Questi account possono trovarsi nella stessa area dell'hub eventi o in un'altra area, aumentando così la flessibilità della funzionalità Acquisizione di Hub eventi.
...
Nel caso in cui il BLOB di archiviazione di Azure sia temporaneamente non disponibile, Event Hubs Capture conserverà i dati per il periodo di conservazione dei dati configurato nell'hub eventi e riempirà i dati una volta che l'account di archiviazione sarà nuovamente disponibile.

### <a name="scaling-to-throughput-units"></a>Ridimensionamento alle unità elaborate

Il traffico di Hub eventi è controllato dalle [unità di velocità effettiva](event-hubs-scalability.md#throughput-units). Una singola unità elaborata consente 1 MB al secondo o 1000 eventi al secondo in ingresso e il doppio in uscita. Hub eventi Standard può essere configurato con 1-20 unità elaborate e altre possono essere acquistate con una [richiesta di supporto][support request] per l'aumento della quota. L'uso superiore rispetto alle unità elaborate acquistate è limitato. Acquisizione di Hub eventi copia i dati direttamente dalla memoria di Hub eventi interna, ignorando le quote in uscita di unità elaborate e salvando l'uscita per altri lettori di elaborazione, ad esempio l'analisi di flusso o Spark.
...
- [Abilitare l'acquisizione di Hub eventi tramite il portale di AzureEnable Event Hubs Capture using the Azure portal](event-hubs-capture-enable-through-portal.md)
...
[Streaming su scala: acquisizione di hub eventi]

### <a name="use-apache-spark"></a>Usare Apache Spark

[Apache Spark][Apache Spark] è un "motore di analisi unificato per l'elaborazione di dati su larga scala". Supporta diversi linguaggi, tra cui SQL, e può accedere facilmente ad Archiviazione BLOB di Azure. Sono disponibili alcune opzioni per eseguire Apache Spark in Azure e ognuna offre un facile accesso all'archiviazione BLOB di Azure:There are a few options to run Apache Spark in Azure, and each provides easy access to Azure Blob storage:

- [HDInsight: indirizzare i file nell'archiviazione di AzureHDInsight: Address files in Azure storage][HDInsight: Address files in Azure storage]
- [Azure Databricks: Archiviazione BLOB di AzureAzure Databricks: Azure Blob storage][Azure Databricks: Azure Blob Storage]
...
In Apache Avro sono disponibili guide introduttive complete per [Java][Java] e [Python][Python]. È anche possibile leggere l'articolo Introduzione all'acquisizione [di Hub eventi.](event-hubs-capture-python.md)
...
Si noti che Capture non utilizza la quota in uscita come viene fatturata separatamente. 
...
Informazioni su come abilitare questa funzionalità usando il portale di Azure e il modello di Azure Resource Manager:Learn how to enable this feature using the Azure portal and Azure Resource Manager template:

- [Usare il portale di Azure per abilitare Acquisizione di Hub eventi](event-hubs-capture-enable-through-portal.md)
- [Usare un modello di Azure Resource Manager per abilitare l'acquisizione di Hub eventiUse an Azure Resource Manager template to enable Event Hubs Capture](event-hubs-resource-manager-namespace-event-hub-enable-capture.md)
...
[Streaming su scala: acquisizione di hub eventi]:https://github.com/yorek/streaming-at-scale/tree/master/event-hubs-capture
