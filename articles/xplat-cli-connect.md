...
>[AZURE.NOTE]  Con l'accesso interattivo l'autenticazione e l'autorizzazione vengono eseguite usando Azure Active Directory. Se si usa un'identità dell'account Microsoft, il processo consente di accedere al dominio predefinito di Azure Active Directory. Se è stata effettuata l'iscrizione per un account Azure gratuito, Azure Active Directory ha creato automaticamente un dominio predefinito per l'account.
...
> [AZURE.NOTE] La modifica della sottoscrizione predefinita ha effetto immediato ed è una modifica globale. I nuovi comandi dell'interfaccia della riga di comando di Azure, sia che vengano eseguiti dalla stessa istanza della riga di comando o da un'istanza diversa, usano la nuova sottoscrizione predefinita.
