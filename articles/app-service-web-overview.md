<properties
	pageTitle="Panoramica delle app Web"
	description="Informazioni sul servizio app per app Web"
...
È possibile sfruttare la potenza del [servizio app per app Web](http://go.microsoft.com/fwlink/?LinkId=529714) per usare i linguaggi e i framework che si conoscono meglio e su cui si basa il proprio lavoro, per distribuire velocemente le applicazioni nel cloud di Azure e per migliorare continuamente il codice senza doversi mai più preoccupare delle tematiche legate all'infrastruttura.

![Web Marketplace](./media/app-service-web-overview/marketplace.png)

## Molto più di semplici app Web##
...
- **Scalabilità automatica**: il servizio app Web consente di aumentare o scalare orizzontalmente le app in modo rapido, per gestire qualsiasi carico di lavoro proveniente dalla clientela. È possibile selezionare manualmente il numero e le dimensioni delle VM o configurare la scalabilità automatica per adattare la capacità dei server ai carichi o alla pianificazione.
...
- **Slot di distribuzione**: implementare la [pre-distribuzione][Slots] per verificare il codice in un ambiente pre-produzione identico a quello dell'app Web di produzione nel servizio app di Azure. Una volta soddisfatti, rilasciare una nuova versione dell'app con tempo di inattività pari a zero eseguendo un'operazione di scambio. 
- **Test in produzione**: portare la pre-distribuzione al livello successivo ed eseguire il [test A/B][Tip] per verificare il nuovo codice con una frazione configurabile del proprio traffico live. 
- **Processi Web**: eseguire qualsiasi programma o script nelle VM del servizio app Web. È possibile eseguire i processi in modo continuo o in base a una pianificazione e ridimensionarli per essere eseguiti in più VM. Usare [WebJobs SDK][Webjobs] di Azure per eseguire l'integrazione con l'archiviazione o con il bus di servizio di Azure.
