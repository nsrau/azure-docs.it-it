...
> * [Versione 3](../latest/offline-widevine-for-android.md)
...
Non è necessario duplicare il contenuto. È possibile usare semplicemente una sola copia del contenuto e un singolo ContentKeyAuthorizationPolicy, ma due ContentKeyAuthorizationPolicyOption distinti:

1. IContentKeyAuthorizationPolicyOption 1: usa la licenza persistente e proprietà contentkeyauthorizationpolicyrestriction 1 che contiene un'attestazione, ad esempio license_type = "persistente"
2. IContentKeyAuthorizationPolicyOption 2: usa una licenza non permanente e proprietà contentkeyauthorizationpolicyrestriction 2 che contiene un'attestazione, ad esempio license_type = "non persistente"

In questo modo, quando giunge una richiesta di licenza dall'app client, dal punto di vista della richiesta non vi è alcuna differenza. Tuttavia, per utenti/dispositivi finali diversi, il servizio token di protezione deve avere la logica di business per emettere token JWT diversi contenenti attestazioni diverse (uno dei due license_type precedenti). Il valore dell'attestazione nel token JWT verrà usato dal servizio licenze per determinare se rilasciare una licenza permanente o non permanente.
...
Nella panoramica dell' [architettura WIDEVINE DRM](https://storage.googleapis.com/wvdocs/Widevine_DRM_Architecture_Overview.pdf)di Google vengono definiti i tre livelli di sicurezza seguenti:
...
|---|---| 
| **Livello di sicurezza 1**: tutte le operazioni di elaborazione, crittografia e controllo del contenuto vengono eseguite all'interno dell'ambiente di esecuzione attendibile. In alcuni modelli di implementazione è possibile che l'elaborazione delle informazioni di sicurezza venga eseguita in chip diversi.|**security_level=5**: le operazioni di crittografia e decodifica e l'intera gestione dei file multimediali (con e senza compressione) devono essere eseguite all'interno di un ambiente di esecuzione affidabile basato su hardware.<br/><br/>**security_level=4**: le operazioni di crittografia e decodifica del contenuto devono essere eseguite all'interno di un ambiente di esecuzione affidabile basato su hardware.|
