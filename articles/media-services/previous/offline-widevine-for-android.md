...
Non è necessario duplicare il contenuto. È possibile utilizzare semplicemente una singola copia del contenuto e una singola ContentKeyAuthorizationPolicy, ma due ContentKeyAuthorizationPolicyOption separati:

1. IContentKeyAuthorizationPolicyOption 1: utilizza la licenza permanente e ContentKeyAuthorizationPolicyRestriction 1 che contiene un'attestazione, ad esempio license_type , "Persistent"
2. IContentKeyAuthorizationPolicyOption 2: utilizza la licenza non persistente e ContentKeyAuthorizationPolicyRestriction 2 che contiene un'attestazione, ad esempio license_type , ovvero "Nonpersistent"

In questo modo, quando giunge una richiesta di licenza dall'app client, dal punto di vista della richiesta non vi è alcuna differenza. Tuttavia, per l'utente finale/dispositivo diverso, il servizio token di sicurezza deve disporre della logica di business per emettere token JWT diversi contenenti attestazioni diverse (uno dei due license_type precedenti). Il valore dell'attestazione nel token JWT verrà usato dal servizio licenze per determinare se rilasciare una licenza permanente o non permanente.
...
Per i livelli di sicurezza Widevine, nella documentazione del [documento Widevine DRM Architecture Overview](https://storage.googleapis.com/wvdocs/Widevine_DRM_Architecture_Overview.pdf) di Google, definisce tre diversi livelli di sicurezza. mentre nella [documentazione di Servizi multimediali di Azure relativa al modello di licenza Widevine](https://docs.microsoft.com/azure/media-services/media-services-widevine-license-template-overview) ne vengono presentati cinque. Qual è la relazione o la corrispondenza tra i due diversi set di livelli di sicurezza?

### <a name="answer"></a>Risposta

In [Widevine DRM Architecture Overview di](https://storage.googleapis.com/wvdocs/Widevine_DRM_Architecture_Overview.pdf)Google, definisce i seguenti tre livelli di sicurezza:
