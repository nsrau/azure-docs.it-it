<properties 
	pageTitle="Introduzione a Servizi multimediali tramite il portale di gestione di Azure" 
...
# Guida introduttiva: Introduzione a Servizi multimediali tramite il portale di gestione di Azure
...
3. Per specificare il numero di unità di streaming, selezionare la scheda RIDIMENSIONA e spostare il dispositivo di scorrimento **capacità riservata**.
	...
	![Pagina Ridimensiona](./media/media-services-portal-get-started/media-services-origin-scale.png)
...
- Codificare il file in formato intermedio (di origine) in un set di file MP4 o Smooth Streaming a velocità in bit adattiva (i passaggi per la codifica sono descritti più avanti in questa esercitazione).
- Ottenere almeno un'unità di streaming su richiesta per l'endpoint di streaming da cui si pianifica la distribuzione dei contenuti. Per altre informazioni, vedere l'articolo sulla procedura per [scalare unità riservate di streaming su richiesta](media-services-manage-origins.md#scale_streaming_endpoints/).
...
5. Scegliere una delle **configurazioni di codifica**.
	...
	+ **Riproduzione tramite HTML5 (IE/Chrome/Safari)**. Questo set di impostazioni genera un singolo file MP4 con le seguenti caratteristiche: audio stereo a 16 bit e frequenza di campionamento di 44,1 kHz con codifica CBR a 128 kbps (compressione AAC) e video 720p con codifica CBR a 4500 kbps tramite compressione H.264 Main Profile.
...
Un URL SAS ha il seguente formato:
