...
Come suggerisce il nome, se si usa il set di impostazioni **AdaptiveStreaming** durante la creazione di un oggetto Transform di codifica, si otterrà un output adatto al recapito tramite protocolli di streaming quali HLS, DASH, CMAF e così via. Quando si usa il set di impostazioni **AdaptiveStreaming**, il codificatore determina in modo intelligente il numero di livelli video da generare e a quale risoluzione e velocità in bit. L'asset di output contiene i file MP4 in cui l'audio con codifica AAC e il video con codifica H.264 non sono di tipo Interleaved.
...
Questa sezione illustra tre esempi dei livelli video di output prodotti dal codificatore di Servizi multimediali in seguito alla codifica con il set di impostazioni **AdaptiveStreaming**. In tutti i casi, l'output conterrà file MP4 solo audio con audio stereo con codifica a 128 kbps.
