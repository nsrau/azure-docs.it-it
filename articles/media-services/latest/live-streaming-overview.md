...
1. Assicurarsi che **StreamingEndpoint** sia in esecuzione. 
2. Creare un **LiveEvent**. 
  
    Quando si crea l'evento, è possibile impostarne l'avvio automatico. In alternativa, è possibile avviare l'evento quando si è pronti ad avviare lo streaming.<br/> Quando l'avvio automatico è impostato su true, l'evento live verrà avviato subito dopo la creazione. Ciò significa che la fatturazione inizia non appena l'evento live è in esecuzione. È necessario chiamare esplicitamente Stop sulla risorsa LiveEvent per interrompere la fatturazione. Per altre informazioni, vedere la sezione [Stati e fatturazione di LiveEvent](live-event-states-billing.md).
...
6. Creare un **LiveOutput** e usare il nome dell'asset creato.
     ...
     Il **LiveOutput** archivierà il flusso nell'**Asset**.
7. Creare un **StreamingLocator** con i tipi **StreamingPolicy** predefiniti.

    Se si prevede di crittografare il contenuto, rivedere la [Panoramica della protezione del contenuto](content-protection-overview.md).
...
11. Se si vuole interrompere la creazione del **LiveEvent** visualizzabile, è necessario arrestare lo streaming dell'evento ed eliminare lo **StreamingLocator**.

Per altre informazioni, vedere l'[Esercitazione sullo streaming live](stream-live-tutorial-with-api.md).
...
Per informazioni sulle nuove funzionalità per lo streaming live in v3, vedere [Materiale sussidiario sulla migrazione per aggiornare Servizi multimediali da v2 a v3](migrate-from-v2-to-v3.md).
...
>
> Se l'asset **LiveOutput** è stato pubblicato usando **StreamingLocator**, il **LiveEvent** (fino alla lunghezza dell'intervallo DVR) continuerà a essere visualizzabile fino alla scadenza o all'eliminazione di **StreamingLocator**, a seconda del valore raggiunto per primo.
