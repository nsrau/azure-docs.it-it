---
title: Segnalare le tracce audio descrittive con Servizi multimediali di Azure v3 Documenti Microsoft
description: Segui i passaggi di questa esercitazione per caricare un file, codificare il video, aggiungere tracce audio descrittive e trasmettere i contenuti in streaming con Servizi multimediali v3.
...
---
# <a name="signal-descriptive-audio-tracks"></a>Segnalare tracce audio descrittive

È possibile aggiungere una traccia audio al video per aiutare i clienti con problemi di vista a seguire la registrazione ascoltando la narrazione. In Servizi multimediali v3, si segnalano tracce audio descrittive annotando la traccia audio nel file manifesto.

Questo articolo mostra come codificare un video, caricare un file MP4 solo audio (codec AAC) contenente audio descrittivo nella risorsa di output e modificare il file .ism per includere l'audio descrittivo.
...
- Esaminare [l'imballaggio dinamico](dynamic-packaging-overview.md).
- Esamina [l'esercitazione Caricare, codificare e riprodurre in streaming i video.](stream-files-tutorial-with-api.md)
...
La funzione **CreateInputAsset** crea un nuovo [asset](https://docs.microsoft.com/rest/api/media/assets) di input e carica al suo interno il file video locale specificato. Questa **risorsa** viene usata come input per il processo di codifica. In Servizi multimediali v3, l'input per un **processo** può essere un **Asset**oppure può essere contenuto disponibile per l'account di Servizi multimediali tramite URL HTTPS. 

Se si desidera apprendere come eseguire la codifica da un URL HTTPS, vedere [questo articolo](job-input-from-http-how-to.md) .  
...
* Crea un **asset** 
* Ottiene un URL di [sAS](https://docs.microsoft.com/azure/storage/common/storage-dotnet-shared-access-signature-part-1) scrivibile per il contenitore dell'asset [nell'archiviazione](https://docs.microsoft.com/azure/storage/blobs/storage-quickstart-blobs-dotnet#upload-blobs-to-a-container)
...
Se è necessario passare il nome dell'asset di input creato `Name` ad altri metodi, `CreateInputAssetAsync`assicurarsi di utilizzare la proprietà nell'oggetto asset restituito da , ad esempio, inputAsset.Name. 

## <a name="create-an-output-asset-to-store-the-result-of-the-encoding-job"></a>Creare una risorsa di output per archiviare il risultato del processo di codificaCreate an output asset to store the result of the encoding job
...
Se è necessario passare il nome dell'asset di output creato `Name` ad altri metodi, `CreateIOutputAssetAsync`assicurarsi di utilizzare la proprietà nell'oggetto asset restituito da , ad esempio, outputAsset.Name. 
...
Il processo in genere passa attraverso i seguenti stati: **Pianificato**, **In coda**, **Elaborazione**, **Finito** (lo stato finale). Se nel corso del processo si verifica un errore, viene restituito lo stato **Error**. Se il processo è in fase di annullamento, vengono restituiti lo stato **Canceling** e, al termine, lo stato **Canceled**.

Per ulteriori informazioni, vedere [Gestione degli eventi della griglia di eventi](reacting-to-media-services-events.md).

## <a name="upload-the-audio-only-mp4-file"></a>Caricare il file MP4 solo audio

Caricate il file MP4 aggiuntivo solo audio (codec AAC) contenente l'audio descrittivo nella risorsa di output.  
...
Di seguito è riportato `UpoadAudioIntoOutputAsset` un esempio di chiamata alla funzione:Here is an example of a call to the function:

```csharp
await UpoadAudioIntoOutputAsset(client, config.ResourceGroup, config.AccountName, outputAsset.Name, "audio_description.m4a");
```

## <a name="edit-the-ism-file"></a>Modificare il file .ism

Al termine del processo di codifica, l'asset di output conterrà i file generati dal processo di codifica. 

1. Nel portale di Azure passare all'account di archiviazione associato all'account di Servizi multimediali. 
1. Individuare il contenitore con il nome dell'asset di output. 
1. Nel contenitore individuare il file con estensione ism e fare clic su **Modifica BLOB** (nella finestra di destra). 
1. Modificare il file .ism aggiungendo le informazioni sul file MP4 solo audio caricato (codec AAC) contenente l'audio descrittivo e premere **Salva** al termine.

    Per segnalare le tracce audio descrittive, è necessario aggiungere i parametri "accessibilità" e "ruolo" al file .ism. È responsabilità del cliente impostare correttamente questi parametri per segnalare una traccia audio come descrizione audio. Ad esempio, `<param name="accessibility" value="description" />` `<param name="role" value="alternate" />` aggiungete e al file .ism per una traccia audio specifica, come illustrato nell'esempio seguente.
...
## <a name="get-a-streaming-locator"></a>Ottenere un localizzatore di streamingGet a streaming locator
...
1. Aprire un browser Web [https://aka.ms/azuremediaplayer/](https://aka.ms/azuremediaplayer/)e passare a .
2. Nella casella **URL:** incolla uno dei valori dell'URL di streaming ottenuti dall'applicazione. 
