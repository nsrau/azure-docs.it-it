...
**DRM**|**Sistema operativo nativo/app nativa**|**Browser desktop supportati**|**Browser per dispositivi mobili supportati**|**Formati di streaming supportati**
----|------|----|-----|----
**PlayReady**|Windows, Windows Phone, Xbox|IE11 in Windows 8.1+, Microsoft Edge|Windows Phone 8.1+|DASH, Smooth Streaming
**Widevine**|Android|Chrome 34+ in tutti i PC e Mac|Android 4.3+|DASH
...
Molti-a-1|Per ogni asset sono necessarie più chiavi simmetriche. <br/><br/>Ad esempio, se è necessario applicare la protezione CENC dinamica con DRM multiplo per MPEG-DASH e la crittografia AES-128 dinamica per HLS, sono necessarie due chiavi simmetriche distinte, entrambe con il proprio ContentKeyType. Per la chiave simmetrica usata per la protezione CENC dinamica, è consigliabile usare ContentKeyType.CommonEncryption, invece per la chiave simmetrica usata per la crittografia AES-128 dinamica, è consigliabile usare ContentKeyType.EnvelopeEncryption.<br/><br/>Un altro esempio è quello della protezione CENC del contenuto DASH dove, in teoria, una chiave simmetrica può essere usata per proteggere il flusso video e un'altra chiave simmetrica per proteggere il flusso audio. 
...
------|-------
**Lettore**|[Azure Media Player](http://azure.microsoft.com/services/media-services/media-player/)
...
In altre parole, sia il provider di identità (IdP) che il servizio token di sicurezza saranno Azure AD. Come lettore si userà l'[API Azure Media Player](http://amp.azure.net/libs/amp/latest/docs/). Sia Servizi multimediali di Azure che Azure Media Player supportano DASH e CENC con DRM multiplo.
...
1. Creare un lettore usando l'[API Azure Media Player](http://amp.azure.net/libs/amp/latest/docs/). L'[API ProtectionInfo di Azure Media Player](http://amp.azure.net/libs/amp/latest/docs/) consente di specificare la tecnologia DRM da usare in una piattaforma DRM diversa.
...
- Informazioni per gli sviluppatori sono disponibili in [Guida per gli sviluppatori di Azure Active Directory](../active-directory/active-directory-developers-guide.md).
- Informazioni per gli amministratori sono disponibili in [Amministrare la directory di Azure AD](../active-directory/active-directory-administer.md).
...
1. L'URL di **issuer** deve terminare con **"/"**.  

	**Audience** deve essere l'ID client dell'applicazione lettore e si deve anche aggiungere **"/"** alla fine dell'URL di issuer.

		<add key="ida:audience" value="[Application Client ID GUID]" />
		<add key="ida:issuer" value="https://sts.windows.net/[AAD Tenant ID]/" /> 

	In [JWT Decoder](http://jwt.calebb.net/) dovrebbero essere visualizzati **aud** e **iss** come nel token JWT sotto:
...
3. Usare l'emittente corretta durante la configurazione della protezione CENC dinamica:
...
##Argomenti aggiuntivi sulla progettazione e l'implementazione
...
1. Acquisizione della licenza DRM da parte del client, che deve usare HTTPS.
...
Della [coppia di chiavi pubblica-privata](https://login.windows.net/common/discovery/keys):
...
###Uso di Encrypted Media Extensions (EME) per PlayReady

In un browser moderno con il supporto EME/PlayReady, ad esempio IE 11 in Windows 8.1 e versioni successive il browser Microsoft Edge in Windows 10, PlayReady sarà il sistema DRM sottostante per EME.
...
![Utenti non idonei](./media/media-services-cenc-with-multidrm-access-control/media-services-unentitledusers.png.png)
