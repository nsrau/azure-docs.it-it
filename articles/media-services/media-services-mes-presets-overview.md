...
`Media Encoder Standard` definisce un set di impostazioni di codifica che è possibile usare per la creazione di processi di codifica  
  
 Le stringhe dei codici JSON o XML dovrebbero essere basate sui set di impostazioni illustrati nei file seguenti. I set di impostazioni con valori personalizzati possono essere trasmessi al codificatore (i valori devono essere validi). Per una spiegazione del significato di ogni elemento di questo set di impostazioni e dei valori possibili per ciascuno, vedere l'argomento [Media Encoder Standard schema](media-services-mes-schema.md) (Schema di Media Encoder Standard).  
...
>  Quando si usa un set di impostazioni per codifiche 4K, si dovrebbe ottenere il tipo di unità riservata `S3`. Per altre informazioni, vedere [Come scalare la codifica](https://azure.microsoft.com/en-us/documentation/articles/media-services-portal-encoding-units).  
 ...
 I nomi delle impostazioni corrispondono ai set di impostazioni illustrati negli argomenti seguenti.  
