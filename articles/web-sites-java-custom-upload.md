...
In questo argomento viene illustrato come caricare un sito Web Java personalizzato in Azure. Sono incluse informazioni applicabili a qualsiasi sito Web Java e sono forniti alcuni esempi per specifiche applicazioni.

Si noti che Azure consente di creare siti Web Java utilizzando l'interfaccia di configurazione di Azure e la raccolta di applicazioni di Azure, come illustrato in [Introduzione alla creazione di siti Web di Azure con Java](../web-sites-java-get-started). Questa esercitazione è adatta per gli scenari in cui si preferisce non utilizzare l'interfaccia di configurazione di Azure o la raccolta di applicazioni di Azure.
...
-   La porta HTTP utilizzata dal processo Java viene assegnata in modo dinamico. Il processo deve utilizzare la porta indicata dalla variabile di ambiente `HTTP_PLATFORM_PORT`.
...
-   Le applicazioni che richiedono directory con autorizzazioni di scrittura devono essere situate nella directory del contenuto del sito Web di Azure, ovvero **D:\\home**.
...
Per ulteriori informazioni su Hudson, vedere <http://hudson-ci.org>.
...
Per ulteriori informazioni su Liferay, vedere <http://www.liferay.com>.
