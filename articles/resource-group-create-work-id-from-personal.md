<properties
   pageTitle="Creazione di un'identità di lavoro o scuola in Azure Active Directory"
   description="Istruzioni per creare un'identità di lavoro o scuola dalla propria identità personale e usarla per varie finalità, ad esempio con i modelli dei gruppi di risorse o con l'accesso basato sui ruoli."
...
Accedere al [portale di Azure](https://manage.windowsazure.com) usando l'identità dell'account Microsoft personale. Dopo l'accesso, scorrere verso il basso nel pannello blu sul lato sinistro e fare clic su **ACTIVE DIRECTORY**.
...
A questo punto occorre verificare il dominio "predefinito" facendo clic su **DOMINI**.

![](./media/resource-group-create-work-id-from-personal/domainclicktoseeyourdefaultdomain.png)

Da qui si dovrebbe vedere che, al momento della creazione dell'account Azure, Azure Active Directory ha creato un dominio personale predefinito, che è un hash dell'id personale usato come sottodominio di **onmicrosoft.com**. Questo è il dominio a cui verrà ora aggiunto un nuovo utente.

## Creazione di un nuovo utente nel dominio predefinito

Fare clic su **UTENTI** e cercare il proprio account personale. Nella colonna **ORIGINATO DA** dovrebbe essere evidente che si tratta di un `Microsoft account`. Si desidera creare un utente nel dominio predefinito **.onmicrosoft.com** di Azure Active Directory.
...
Nella parte inferiore della pagina fare clic su **+AGGIUNGI UTENTE**. Nella finestra di dialogo visualizzata digitare il nuovo nome utente e selezionare **Nuovo utente nell'organizzazione** per **Tipo di utente**. In questo esempio il nuovo nome utente è `ahmet`. Assicurarsi di selezionare il dominio predefinito individuato sopra come dominio per l'indirizzo di posta elettronica di `ahmet`. Al termine, fare clic sulla freccia Avanti.
...
Copiare l'indirizzo di posta elettronica del nome utente o immettere un indirizzo di posta elettronica a cui inviare le informazioni. In entrambi i casi, saranno necessarie le informazioni di accesso.

![](./media/resource-group-create-work-id-from-personal/receivedtemporarypassworddialog.png)

Ora verrà visualizzato il nuovo utente, in questo caso `Ahmet the Developer`, originato da Azure Active Directory. Si è creata la nuova identità di lavoro o scuola con Azure Active Directory, ma questa identità non ha ancora le autorizzazioni per usare le risorse di Azure.

![](./media/resource-group-create-work-id-from-personal/defaultdirectoryusersaftercreate.png)

Se si invia un messaggio di posta elettronica con le informazioni, potrebbe essere simile al seguente:
...
A questo punto è necessario aggiungere il nuovo utente come co-amministratore della sottoscrizione, in modo che possa accedere al portale di gestione. A tale scopo, fare clic sull'icona **Impostazioni** nel pannello in basso a sinistra.

![](./media/resource-group-create-work-id-from-personal/thesettingswidget.png)

Nella parte superiore dell'area principale delle impostazioni fare clic su **AMMINISTRATORI**. Dovrebbe essere visualizzata solo l'identità dell'account Microsoft personale. Nella parte inferiore della pagina fare clic su **+AGGIUNGI** per specificare un co-amministratore. Immettere qui l'indirizzo di posta elettronica del nuovo utente che è stato creato, compreso il dominio predefinito. Come mostrato di seguito, verrà visualizzato un segno di spunta verde accanto all'utente per la directory predefinita. Ricordarsi di selezionare tutte le sottoscrizioni che l'utente dovrà essere in grado di gestire.
...
Verrà immediatamente chiesto di creare una nuova password. Tuttavia, al termine...
...
A questo punto è possibile eseguire molte altre operazioni, ad esempio usare la nuova identità di Azure Active Directory con i [modelli dei gruppi di risorse di Azure](xplat-cli-azure-resource-manager.md).
