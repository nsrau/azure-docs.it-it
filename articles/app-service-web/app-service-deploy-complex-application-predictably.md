<properties
	...
	description="Come distribuire un'applicazione complessa in Azure come unità singola e in modo prevedibile usando modelli di gruppo di risorse JSON e script di PowerShell."
...
In DevOps la ripetibilità e la prevedibilità sono fondamentali per la distribuzione riuscita di un'applicazione complessa su vasta scala. [Servizio app di Azure](/services/app-service/) permette di creare applicazioni complesse, incluse app Web, per dispositivi mobili, per le API e app per la logica. [Gestione risorse di Azure](../resource-group-overview.md) abilita la gestione di tutti i componenti di tale applicazione come unità. A questo punto è possibile distribuire l'applicazione complessa usando modelli JSON e semplici script PowerShell.
...
In DevOps la ripetibilità e la prevedibilità sono fondamentali per la distribuzione riuscita di un'applicazione complessa su vasta scala. In questa esercitazione è stata distribuita un'applicazione in Azure come singolo gruppo di risorse usando il modello di Gestione risorse di Azure. È auspicabile che si siano acquisite le conoscenze necessarie per iniziare a convertire l'applicazione complessa in Azure in un modello, in modo da poterla distribuire in modo prevedibile in Azure.
