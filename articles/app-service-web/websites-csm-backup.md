...
Verranno ora esaminati alcuni esempi relativi all'uso dell'API REST per il backup e il ripristino di un'app. Ogni esempio include un URL e il corpo di una richiesta HTTP. L'URL di esempio contiene segnaposto racchiusi tra parentesi graffe, ad esempio {subscription-id}. È necessario sostituirli con le informazioni corrispondenti della propria app. Come riferimento, ecco una descrizione di ogni segnaposto visualizzato negli URL di esempio.
