---
title: Distribuire la prima app Web PHP in Azure in cinque minuti | Microsoft Docs
...
1. Nel pannello dell'app Web scorrere verso il basso fino alla voce **Opzioni di distribuzione** oppure cercarla e quindi fare clic su di essa.
   
    ![](./media/app-service-web-get-started-languages/deploy-web-app-deployment-options.png)
2. Fare clic su **Scegliere l'origine** > **Repository Git locale** > **OK**.
3. Tornare al pannello dell'app Web e fare clic su **Credenziali per la distribuzione**.
4. Impostare le credenziali per la distribuzione e fare clic su **Salva**.
5. Tornare al pannello dell'app Web e scorrere verso il basso fino alla voce **Proprietà** oppure cercarla e quindi fare clic su di essa. Fare clic sul pulsante **Copia** accanto a **URL GIT**.
   
    ![](./media/app-service-web-get-started-languages/deploy-web-app-properties.png)
   
    A questo punto è possibile distribuire il codice con Git.
6. Nel terminale della riga di comando passare a una directory di lavoro (`CD`) e clonare l'app di esempio come segue:
   
        git clone https://github.com/Azure-Samples/app-service-web-php-get-started.git
   
    ![Clonare il codice dell'app di esempio per la prima app Web in Azure](./media/app-service-web-get-started-languages/php-git-clone.png)
   
    Per *&lt;github\_sample\_url>* usare uno degli URL seguenti, a seconda del framework preferito.
7. Passare al repository dell'app di esempio. Ad esempio:
   
        cd app-service-web-html-get-started
8. Configurare l'URL GIT copiato dal portale in uno dei passaggi precedenti come Git remoto per l'app Azure.
   
        git remote add azure <giturlfromportal>
9. Distribuire il codice di esempio nell'app Azure nello stesso modo in cui si effettua il push di qualsiasi codice con Git:
   
        git push azure master
   
    ![Effettuare il push di codice nella prima app Web in Azure](./media/app-service-web-get-started-languages/php-git-push.png)
   
    Se è stato usato uno dei framework di linguaggio, verrà visualizzato un output diverso. Ciò dipende dal fatto che `git push` non solo inserisce il codice in Azure, ma attiva anche le attività di distribuzione nel motore di distribuzione. Se nella radice del progetto (repository) sono presenti file package.json, lo script di distribuzione ripristina automaticamente i pacchetti necessari. È anche possibile [abilitare l'estensione Composer](web-sites-php-mysql-deploy-use-git.md#composer) per elaborare automaticamente i file composer.json nell'app PHP.
...
## Passaggi successivi
[Creare, configurare e distribuire un'app Web Laravel in Azure](app-service-web-php-get-started.md). Seguendo questa esercitazione è possibile acquisire le competenze di base necessarie per eseguire qualsiasi app Web PHP in Azure, ad esempio:
