---
title: Guida all&quot;implementazione API | Documentazione Microsoft
...
---
# <a name="api-implementation-guidance"></a>Guida all'implementazione API
...
## <a name="considerations-for-implementing-a-restful-web-api"></a>Considerazioni per l'implementazione di un'API Web RESTful
...
## <a name="considerations-for-implementing-request-routing"></a>Considerazioni per l'implementazione del routing delle richieste
In un servizio implementato usando l'API Web ASP.NET, ogni richiesta viene indirizzata a un metodo di una classe *controller*. Il framework API Web offre due opzioni principali per l'implementazione del routing: routing *basato su convenzioni* e routing *basato su attributi*. Quando si determina il modo migliore per indirizzare le richieste dell’API Web, tenere presente quanto segue:
...
* **Comprendere le limitazioni e i requisiti del routing basato su convenzioni**.
    ...
    Le route possono essere generiche e comprendere valori letterali, ad esempio *api*, e variabili, ad esempio *{controller}* e *{id}*. Il routing basato su convenzioni prevede che alcuni elementi della route possano essere facoltativi. Il framework API Web determina quale metodo richiamare nel controller creando una corrispondenza tra il metodo HTTP nella richiesta per la parte iniziale del nome del metodo dell'API, quindi creando una corrispondenza tra i parametri facoltativi. Ad esempio, se un controller denominato *orders* contiene i metodi *GetAllOrders()* o *GetOrderByInt(int id)* la richiesta GET *http://www.adventure-works.com/api/orders/* viene indirizzata al metodo *GetAlllOrders()* e la richiesta GET *http://www.adventure-works.com/api/orders/99* viene indirizzata al metodo *GetOrderByInt(int id)*. Se non esiste alcun metodo corrispondente che inizia con il prefisso Get nel controller, il framework API Web risponde con un messaggio HTTP 405 (metodo non consentito). Inoltre, il nome del parametro (id) specificato nella tabella di routing deve essere lo stesso nome del parametro per il metodo *GetOrderById*. In caso contrario il framework API Web risponderà con un messaggio HTTP 404 (Not Found).

    Le stesse regole si applicano alle richieste POST, PUT e DELETE HTTP: una richiesta PUT che aggiorna i dettagli dell'ordine 101 verrebbe indirizzata all'URI *http://www.adventure-works.com/api/orders/101*, il corpo del messaggio conterrà i nuovi dettagli dell'ordine e queste informazioni verranno passate come parametro a un metodo nel controller degli ordini con un nome che inizia con il prefisso *Put*, ad esempio *PutOrder*.

    La tabella di routing predefinita non corrisponderà a una richiesta che fa riferimento a risorse figlio di un'API Web RESTful, ad esempio *http://www.adventure-works.com/api/customers/1/orders*, che permette di trovare i dettagli di tutti gli ordini effettuati dal cliente 1. Per gestire questi casi, è possibile aggiungere route personalizzate alla tabella di routing:
    ...
    Questa route indirizza le richieste che corrispondono all'URI al metodo *GetOrdersForCustomer* nel controller *Customers*. Questo metodo deve accettare un parametro singolo denominato *custI*:
...
* **Preferenza per il routing basato su attributi**.
    ...
    Il routing basato su attributi può anche fungere da documentazione per gli sviluppatori che hanno la necessità di gestire il codice in futuro. In questo modo è immediatamente chiaro quale metodo appartiene a quale route e l'attributo *HttpGet* chiarisce il tipo di richiesta HTTP alla quale il metodo risponde.
...
* **Distinguere i metodi che non devono essere indirizzati**.
    ...
    Se si usa il routing basato su convenzioni, è opportuno indicare i metodi che non corrispondono alle azioni HTTP assegnando loro l'attributo *NonAction*. In genere ciò si applica ai metodi helper definiti per l'utilizzo da parte di altri metodi all'interno del controller. Questo attributo eviterà che questi metodi siano associati e richiamati da una richiesta HTTP non corretta.
* **Considerare vantaggi e compromessi della collocazione dell'API in un sottodominio**.

    Per impostazione predefinita, l'API Web ASP.NET organizza le API della directory */api* in un dominio, ad esempio *http://www.adventure-works.com/api/orders*. Questa directory si trova nello stesso dominio come tutti gli altri servizi esposti dallo stesso host. Può essere utile suddividere l'API Web in un proprio sottodominio in esecuzione in un host separato, con URI simili a *http://api.adventure-works.com/orders*. Questa separazione consente di partizionare e scalare in modo più efficace l'API Web senza influire sulle altre applicazioni Web o i servizi in esecuzione nel dominio *www.adventure-works.com*.
...
* **Fornire collegamenti per supportare la navigazione di tipo HATEOAS e l'individuazione di risorse**.
  ...
  * Richiesta HTTP DELETE all'URI *http://adventure-works.com/customers/2* per eliminare i dettagli del cliente. La richiesta non prevede informazioni aggiuntive o la restituzione dei dati nel corpo del messaggio di risposta.
  * Richiesta HTTP GET all'URI *http://adventure-works.com/customers/2/orders* per trovare tutti gli ordini del cliente. I dati possono essere restituiti come XML o JSON.
...
* **Acquisire le eccezioni e restituire una risposta significativa ai client**.
  ...
  > [!TIP]
  > Non includere informazioni che potrebbero essere utili a un utente malintenzionato che voglia introdursi nell'API Web. Per altre informazioni, visitare la pagina [Exception Handling in ASP.NET Web API](http://www.asp.net/web-api/overview/error-handling/exception-handling) (Gestione delle eccezioni nell'API Web ASP.NET) del sito Web Microsoft.
  >
  > [!NOTE]
...
* **Gestire le eccezioni in modo coerente e registrare le informazioni sugli errori**.
  ...
  * Eccezioni generate durante la serializzazione del contenuto di un messaggio di risposta.
    ...
    Per gestire questi casi, potrebbe essere necessario implementare un approccio più personalizzato. È inoltre necessario incorporare la registrazione di errori che permette di rilevare informazioni dettagliate su ogni eccezione. La registrazione degli errori può contenere informazioni dettagliate, purché non sia accessibile sul web ai client. L'articolo [Web API Global Error Handling](http://www.asp.net/web-api/overview/error-handling/web-api-global-error-handling) (Gestione globale degli errori nell'API Web) nel sito Web Microsoft mostra come eseguire questa operazione.
...
* **Fornire supporto asincrono per le richieste a esecuzione prolungata**.
  ...
  > [!NOTE]
  > Per altre informazioni sull'uso di processi Web con il sito Web di Azure, vedere [Eseguire attività in background con processi Web](app-service-web/web-sites-create-web-jobs.md) nel sito Web Microsoft.
  >
  >
...
## <a name="publishing-and-managing-a-web-api-by-using-the-azure-api-management-service"></a>Pubblicare e gestire un'API Web mediante il servizio di gestione API di Azure
...
## <a name="supporting-developers-building-client-applications"></a>Supporto di sviluppatori che creano applicazioni client
...
### <a name="documenting-the-rest-operations-for-a-web-api"></a>Documentare le operazioni REST per un'API Web
...
### <a name="implementing-a-client-sdk"></a>Implementazione di un client SDK
...
Per altre informazioni, vedere [Installare Application Insights per ASP.NET](application-insights/app-insights-asp-net.md) nel sito Web Microsoft.
...
* La pagina [Eseguire attività in background con processi Web](app-service-web/web-sites-create-web-jobs.md) sul sito Web Microsoft fornisce informazioni ed esempi sull'uso dei processi Web per eseguire operazioni in background su un sito Web di Azure.
...
* La pagina [Metodi di routing del traffico di Gestione traffico](traffic-manager/traffic-manager-routing-methods.md) sul sito Web Microsoft riepiloga come Gestione traffico di Azure può essere usato per le richieste di bilanciamento del carico tra più istanze di un sito Web che ospita un'API Web.
