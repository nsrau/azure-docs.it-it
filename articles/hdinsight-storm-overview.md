<properties title="Panoramica di Storm in HDInsight" pageTitle="Informazioni su Apache Storm in HDInsight (hadoop)" description="Informazioni su come usare Apache Storm in HDInsight (Hadoop)" metaKeywords="Azure hdinsight storm, Azure hdinsight realtime, azure hadoop storm, azure hadoop realtime, azure hdinsight real-time, azure hadoop storm real-time, aure hadoop real-time" services="hdinsight" solutions="" documentationCenter="big-data" authors="larryfr" videoId="" scriptId="" manager="paulettm" editor="cgronlun"/>

<tags ms.service="hdinsight" ms.workload="big-data" ms.tgt_pltfrm="na" ms.devlang="na" ms.topic="article" ms.date="09/30/2014" ms.author="larryfr" />

# Panoramica di HDinsight Storm
...
## Informazioni su Azure HDInsight Storm

HDInsight Storm viene offerto come cluster gestito integrato nell'ambiente Azure, in cui può essere usato come componente di una soluzione Azure più completa. Ad esempio, Storm può usare i dati provenienti da servizi come le code del bus di servizio o l'hub eventi e usare Siti Web o Servizi cloud per fornire la visualizzazione dei dati. I cluster HDInsight Storm possono anche essere configurati in una rete virtuale di Azure. Ciò riduce la latenza nella comunicazione con le altre risorse della stessa rete virtuale e può anche consentire una comunicazione protetta con le risorse all'interno di un data center privato.

Per iniziare a usare Storm, vedere [Introduzione a Storm in HDInsight][Introduzione a Storm in HDInsight].
...
-   **Spout**: usa i dati dall'origine dati e genera uno o più **flussi**
    ...
    > [WACOM.NOTE] In molti casi i dati vengono letti da una coda come Kafka, le code del bus di servizio di Azure o gli hub eventi. La coda assicura la persistenza dei dati in caso di interruzione.
...
Per altre informazioni sui componenti Storm, vedere l'[esercitazione su Storm][esercitazione su Storm] nel sito Web apache.org.

## Scenari: quali sono i casi di utilizzo di Storm?

Di seguito sono descritti alcuni scenari comuni in cui è possibile usare Apache Storm. Per informazioni sugli scenari reali, leggere l'articolo che descrive [in che modo le società stanno usando Storm][in che modo le società stanno usando Storm].
...
Per altre informazioni su RPC distribuito e DRPCClient fornito con Storm, vedere [RPC distribuito][RPC distribuito].

### Machine Learning online

Storm può essere usato con una soluzione Machine Learning sottoposta precedentemente a training con l'elaborazione batch, ad esempio una soluzione basata su Mahout. Tuttavia, il suo modello di calcolo distribuito generico consente anche soluzioni Machine Learning basate su flussi. Ad esempio, il [progetto Scalable Advanced Massive Online Analysis (SAMOA)][progetto Scalable Advanced Massive Online Analysis (SAMOA)] è una libreria Machine Learning che usa l'elaborazione del flusso e può funzionare con Storm.

## Quali linguaggi di programmazione si possono usare?

Il cluster HDInsight Storm fornisce il supporto predefinito per .NET, Java e Python. Storm [supporta altri linguaggi][supporta altri linguaggi], ma la gran parte di essi richiedono l'installazione di un altro linguaggio di programmazione nel cluster HDInsight da aggiungere alle altre modifiche alla configurazione.

### .NET

SCP è un progetto che consente agli sviluppatori .NET di progettare e implementare una topologia (con Spout e Bolt). Il supporto per SCP viene fornito per impostazione predefinita con i cluster HDInsight Storm.

Per altre informazioni sullo sviluppo con SCP, vedere [Sviluppare applicazioni di elaborazione di flussi di dati con SCP.NET e C# tramite Storm in HDInsight][Sviluppare applicazioni di elaborazione di flussi di dati con SCP.NET e C# tramite Storm in HDInsight].
...
Per altre informazioni su Trident, vedere l'[esercitazione su Trident][esercitazione su Trident] nel sito Web apache.org.

Per esempi sulle topologie Java non elaborate e Trident, vedere la directory **%storm\_home%\\contrib\\storm-starter** nel cluster HDInsight Storm.
...
Per altre informazioni, vedere la sezione sulle [garanzie relative all'elaborazione dati][garanzie relative all'elaborazione dati] nel sito Web apache.org

### BasicBolt

Il modello che prevede la lettura di una tupla di input, la generazione di nessuna o più tuple e l'inserimento della tupla di input subito dopo la fine del metodo di esecuzione è così frequente che Storm fornisce l'interfaccia [IBasicBolt][IBasicBolt] per automatizzarlo.

### Join

La creazione di un join di due flussi di dati varia a seconda delle applicazioni. Ad esempio, è possibile creare un join di ciascuna tupla da più flussi a un nuovo flusso singolo oppure è possibile creare un solo join di più batch di tuple per una specifica finestra. In entrambi i casi il join può essere eseguito usando [fieldsGrouping][fieldsGrouping], che rappresenta un metodo per definire in che modo le tuple vengono indirizzate ai Bolt.
...
Per un esempio su come usare le tuple tick, vedere [Analisi dei dati dei sensori con Storm e HDInsight][Analisi dei dati dei sensori con Storm e HDInsight]

Trident invece si basa sull'elaborazione di batch di tuple.

### Memorizzazione nella cache

La memorizzazione nella cache viene spesso usata come meccanismo per velocizzare l'elaborazione perché conserva le risorse usate di frequente in memoria. Poiché una topologia viene distribuita in più nodi e in più processi in ogni nodo, è opportuno usare [fieldsGrouping][fieldsGrouping] per assicurarsi che le tuple che contengono i campi usati per la ricerca nella cache vengano sempre indirizzate allo stesso processo. Ciò consente di evitare la duplicazione delle voci della cache nei processi.

### Streaming dei primi N elementi

Quando la topologia dipende dal calcolo di un valore "primi N", ad esempio le prime cinque tendenze su Twitter, è necessario calcolare il valore "primi N" in parallelo, quindi unire l'output dei calcoli in un valore globale. A questo scopo usare [fieldsGrouping][fieldsGrouping] per l'indirizzamento in base al campo al Bolt parallelo che esegue il partizionamento dei dati in base al valore del campo e infine eseguire l'indirizzamento a un Bolt che determina globalmente il valore "primi N".

Per un esempio, vedere [RollingTopWords][RollingTopWords].

## Passaggi successivi

-   [Introduzione a Storm in HDInsight][Introduzione a Storm in HDInsight]

-   [Analisi dei dati dei sensori con Storm e HDInsight][1]
...
-   [Sviluppare applicazioni di elaborazione di flussi di dati con SCP.NET e C# tramite Storm in HDInsight][Sviluppare applicazioni di elaborazione di flussi di dati con SCP.NET e C# tramite Storm in HDInsight]
  ...
  [Introduzione a Storm in HDInsight]: /it-it/documentation/articles/hdinsight-storm-getting-started
  [esercitazione su Storm]: https://storm.incubator.apache.org/documentation/Tutorial.html
  [in che modo le società stanno usando Storm]: https://storm.incubator.apache.org/documentation/Powered-By.html
  [RPC distribuito]: https://storm.incubator.apache.org/documentation/Distributed-RPC.html
  [progetto Scalable Advanced Massive Online Analysis (SAMOA)]: http://yahooeng.tumblr.com/post/65453012905/introducing-samoa-an-open-source-platform-for-mining
  [supporta altri linguaggi]: https://storm.incubator.apache.org/about/multi-language.html
  ...
  [esercitazione su Trident]: https://storm.incubator.apache.org/documentation/Trident-tutorial.html
  [garanzie relative all'elaborazione dati]: https://storm.apache.org/about/guarantees-data-processing.html
  ...
  [Analisi dei dati dei sensori con Storm e HDInsight]: /it-it/documentation/articles/hdinsight-storm-sensor-data-analysis.md
  [RollingTopWords]: https://github.com/nathanmarz/storm-starter/blob/master/src/jvm/storm/starter/RollingTopWords.java
  [1]: /it-it/documentation/articles/hdinsight-storm-sensor-data-analysis
