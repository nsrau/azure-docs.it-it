---
title: 'Esercitazione: Elaborare fatture EDIFACT mediante Servizi BizTalk di Microsoft Azure | Microsoft Docs'
...
2. Premere **F5** per compilare e avviare l'applicazione **client dell'esercitazione**.
...
### Creare i partner commerciali
Per iniziare, creare i partner commerciali per Contoso e Northwind.

1. Nel portale di Servizi BizTalk, nella scheda **Partner**, fare clic su **Aggiungi**.
...
2. Nella pagina **Impostazioni generali** del nuovo accordo specificare i valori mostrati nell'immagine seguente, quindi fare clic su **Continua**.
   ...
   Dopo che si è fatto clic su **Continua**, diventano disponibili due schede, **Receive Settings** e **Send Settings**.
3. Creare l'accordo di invio tra Contoso e Northwind. Questo accordo definisce le modalità con cui Contoso invia la fattura EDIFACT a Northwind.
   ...
   2. Mantenere i valori predefiniti nelle schede **Inbound URL**, **Trasforma** e **Batch**.
   ...
   4. Nella scheda **Trasporto** specificare i dettagli delle code del bus di servizio. Per l'accordo sul lato invio, viene usata la coda **northwindreceive** per inviare la fattura EDIFACT a Northwind, mentre la coda **suspended** viene usata per instradare gli eventuali messaggi che presentano problemi durante l'elaborazione e vengono sospesi. Queste code sono state create nel **Passaggio 1: Creare le code del bus di servizio** di questo argomento.
      ...
      In **Impostazioni trasporto > Tipo trasporto** e in **Message Suspension Settings > Tipo trasporto** selezionare Bus di servizio di Azure e specificare i valori mostrati nell'immagine.
4. Creare l'accordo di ricezione tra Contoso e Northwind. Questo accordo definisce le modalità con cui Contoso riceve l'acknowledgement da Northwind.
   ...
   3. Nella scheda **Protocollo**, nella sezione **Schemi**, caricare lo schema **EFACT\_4.1\_CONTRL.xsd**. Questo schema è disponibile con il pacchetto di esempio.
...
5. Fare infine clic su **Distribuisci** per distribuire l'accordo. Notare gli endpoint in cui vengono distribuiti gli accordi di invio e ricezione.
   ...
   * Nella scheda **Receive Settings** in **Trasporto**, notare l'endpoint. Per inviare un messaggio da Northwind a Contoso usando il bridge di ricezione EDI è necessario inviare un messaggio a questo endpoint.

## Passaggio 3: creare e distribuire il progetto di Servizi BizTalk
...
* **EFACT\_D93A\_INVOIC.XSD**: schema della fattura EDIFACT standard.
* **EFACT\_4.1\_CONTRL.XSD**: schema dell'acknowledgement EDIFACT che Northwind invia a Contoso.
* **INHOUSEINVOICE\_to\_D93AINVOIC.TRFM**: la trasformazione che mappa lo schema della fattura interna allo schema di fattura EDIFACT standard.

### Creare il progetto di Servizi BizTalk
...
8. Aggiornare il file EDIBridge.config in modo da includere i dettagli di configurazione
   ...
   * In *<behaviors>* specificare lo spazio dei nomi ACS e la chiave associati alla sottoscrizione a Servizi BizTalk.
...
## Passaggio 4: testare la soluzione
