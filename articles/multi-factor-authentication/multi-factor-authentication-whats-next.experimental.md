...
Se si ha una delle licenze seguenti, è possibile utilizzare la pagina delle impostazioni del servizio MFA:
- Azure MFA
- Azure AD Premium 
...
2. Per segnalare un illecito, immettere il codice dell'illecito, il cui valore predefinito è 0, e quindi premere #. Viene quindi segnalato che è stato inviato un avviso di illecito.
3. Terminare la chiamata.

### <a name="view-fraud-reports"></a>Visualizzare le segnalazioni di illeciti
1. Accedere al [portale di Azure classico](https://manage.windowsazure.com).
2. A sinistra selezionare Active Directory.
3. Nella parte superiore selezionare **Provider di Autenticazione a più fattori** per visualizzare un elenco dei provider Multi-Factor Authentication.
...
7. Fare clic su **Esegui** per visualizzare un report degli avvisi di illecito. Fare clic su **Esporta in CSV** per esportare il report.
...
3. Se a sinistra è visualizzato il nome del provider Azure MFA con **+** accanto a esso, fare clic su **+**. Vengono visualizzati diversi gruppi di replica del server MFA e il gruppo predefinito di Azure. Selezionare il gruppo appropriato.
...
6. Immettere il nome utente, la durata del bypass e il motivo del bypass. quindi fare clic su **Bypass**.
...
6. Specificare l'intervallo di date che si desidera visualizzare nel report. È anche possibile specificare nomi utente, numeri di telefono e lo stato dell'utente.
7. Fare clic su **Esegui** per visualizzare un report dei bypass. Fare clic su **Esporta in CSV** per esportare il report.

## <a name="custom-voice-messages"></a>Messaggi vocali personalizzati
I messaggi vocali personalizzati consentono di usare registrazioni o messaggi introduttivi personalizzati per la verifica in due passaggi. È possibile usare messaggi vocali personalizzati in aggiunta alle registrazioni di Microsoft o al posto di esse.
...
* I messaggi di autenticazione devono avere una durata inferiore a 20 secondi. I messaggi che durano più di 20 secondi non lasciano agli utenti tempo sufficiente per rispondere prima del timeout della verifica.
...
10. Una volta caricato il messaggio vocale, viene confermato il corretto caricamento del file.

Per attivare il messaggio per gli utenti:
...
4. Fare clic su **Nuova cache** nella pagina di configurazione della memorizzazione nella cache.
...
Se la funzionalità Indirizzi IP attendibili è abilitata, la verifica in due passaggi *non* è necessaria per i flussi dei browser. Le password delle app *non* sono necessarie per le applicazioni rich client meno recenti, a condizione che l'utente non abbia già creato una password. Se si inizia a usare una password dell'app, questa rimane obbligatoria. 
...
* Quando viene avviata l'autenticazione a più fattori, è possibile usare la password con alcuni client non basati su browser. Non è possibile usare le password delle app per attività amministrative. Assicurarsi di creare un account di servizio con una password complessa per l'esecuzione degli script di PowerShell e di non abilitare tale account per la verifica in due passaggi.

> [!WARNING]
> Le password di app non funzionano in ambienti ibridi dove i client comunicano con endpoint di individuazione automatica sia locali che nel cloud. Le password di dominio sono necessarie per l'autenticazione locale e le password delle app sono necessarie per l'autenticazione nel cloud.

### <a name="naming-guidance-for-app-passwords"></a>Indicazioni per la denominazione delle password dell'app
Per le password dell'app è consigliabile usare nomi che riflettano il dispositivo in cui vengono usate. Ad esempio, se si usa un portatile con app non basate su browser, creare una sola password dell'app denominata Portatile e usare tale password. Creare quindi un'altra password dell'app denominata Desktop per le stesse applicazioni nel computer desktop. 
...
Azure AD supporta la federazione (Single Sign-On) con Active Directory Domain Services (AD DS) di Windows Server in locale. Se l'organizzazione è federata con Azure AD e si prevede di usare Azure Multi-Factor Authentication, le informazioni sulle password delle app sono importanti. Questa sezione si applica solo agli utenti federati (SSO).
...
Per impostazione predefinita, gli utenti non possono creare password delle app, ma è possibile abilitare questa funzionalità. Per consentire agli utenti di creare password dell'app, seguire questa procedura:
...
In conclusione, la memorizzazione di MFA nei dispositivi attendibili consente una riduzione del numero di autenticazioni nelle app Web (che in genere richiedono l'autenticazione ogni volta). Comporta però un aumento del numero di autenticazioni per i client di autenticazione moderni (che in genere richiedono l'autenticazione ogni 90 giorni).
