...
2. [Creare un provider di Multi-Factor Authentication](#creating-an-azure-multi-factor-auth-provider) o [assegnare licenze agli utenti](#assigning-an-azure-ad-premium-or-enterprise-mobility-license-to-users)
	- È quindi necessario eseguire una di queste operazioni. Creare un provider di Azure Multi-Factor Authentication e assegnarlo alla directory o assegnare licenze agli utenti. Le licenze sono disponibili per gli utenti che hanno Azure MFA, Azure AD Premium o EMS. Azure Multi-Factor Authentication è incluso in Azure Active Directory Premium. È incluso anche in Enterprise Mobility Suite. Se è disponibile un numero di licenze sufficiente, non è necessario creare un provider di autenticazione. 
...
## Creazione di un provider di Multi-Factor Authentication di Azure
...
3. Nella parte superiore della pagina Active Directory, selezionare i provider di Multi-Factor Authentication. Nella parte inferiore fare clic su **Nuovo**.
...
5. Compilare i campi seguenti, quindi selezionare Crea.
	1. Nome: il nome del provider di Multi-Factor Authentication.
...
Dopo aver abilitato gli utenti, inviare un messaggio di posta elettronica in cui viene richiesto di fornire le informazioni sul contatto. Ecco un modello di messaggio di posta elettronica che è possibile usare. Include un collegamento a un video che gli utenti possono guardare.
