...
Se il catalogo SSIS è ospitato nel database SQL di Azure con endpoint del servizio di rete virtuale/Istanza gestita (anteprima), è possibile aggiungere il runtime di integrazione Azure-SSIS per:
...
La rete virtuale può essere distribuita con il modello di distribuzione classico o con il modello di distribuzione Azure Resource Manager. Se si prevede di aggiungere il runtime di integrazione Azure-SSIS alla *stessa rete virtuale* già aggiunta da Istanza gestita (anteprima), verificare che il runtime di integrazione Azure-SSIS si trovi in una *subnet diversa* da quella usata per Istanza gestita (anteprima).   
...
5. Nella pagina **Rete virtuale (classico)** selezionare **Proprietà**.
    ![ID della risorsa di rete virtuale in versione classica](media/join-azure-ssis-integration-runtime-virtual-network/classic-vnet-resource-id.png)
...
5. Nella pagina **Impostazioni generali** della finestra **Integration Runtime Setup** (Configurazione runtime di integrazione) selezionare **Avanti**. 
...
8. È ora possibile avviare il runtime di integrazione tramite il pulsante **Inizia** nella colonna **Azioni** corrispondente al runtime di integrazione Azure-SSIS di interesse. L'avvio di un runtime di integrazione Azure-SSIS richiede circa dai 20 ai 30 minuti. 
