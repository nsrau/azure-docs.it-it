...
scheduler | La proprietà “scheduler” viene utilizzata per definire la pianificazione per l’attività. Le relative proprietà secondarie sono le stesse indicate nella [proprietà availability in un set di dati](data-factory-create-datasets.md#Availability). | No | 
| pipelineMode | Metodo per la pianificazione delle esecuzioni per la pipeline. I valori consentiti sono: scheduled (predefinito), onetime.<br/><br/>"Scheduled" indica che la pipeline verrà eseguita a intervalli di tempo specificati in base al periodo di attività, ovvero all'ora di inizio e di fine. "Onetime" indica che la pipeline verrà eseguita una sola volta. Al momento, dopo aver creato una pipeline monouso non è possibile modificarla o aggiornarla. Per maggiori dettagli su questa impostazione, vedere [Pipeline monouso](data-factory-scheduling-and-execution.md#onetime-pipeline). | No | 
| expirationTime | Periodo di tempo dopo la creazione durante il quale la pipeline è valida e deve rimanere sottoposta a provisioning. Quando viene raggiunta la scadenza, la pipeline viene automaticamente eliminata se non ha esecuzioni attive, non riuscite o in sospeso. | No | 
| datasets | Elenco dei set di dati da utilizzare per le attività definite nella pipeline. Può essere utilizzato per definire i set di dati specifici di questa pipeline e non definiti all'interno della data factory. I set di dati definiti all'interno di questa pipeline possono essere usati solo da questa pipeline e non possono essere condivisi. Vedere [Set di dati con ambito](data-factory-create-datasets.md#scoped-datasets) per i dettagli. | No | 
