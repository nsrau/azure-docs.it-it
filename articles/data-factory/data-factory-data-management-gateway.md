...
> [AZURE.NOTE] Attualmente il gateway supporta solo l'attività di copia e l'attività di stored procedure in Data Factory. Non è possibile usare il gateway da un'attività personalizzata per accedere alle origini dati locali. 
...
6. Selezionare la **cartella** per l'installazione del gateway e fare clic su **Avanti**. 
...
#### <a name="if-you-haven't-already-created-a-logical-gateway-in-the-portal"></a>Se non è ancora stato creato un gateway logico nel portale
...
### <a name="configure-proxy-server-settings-in-diahost.exe.config"></a>Configurare le impostazioni del server proxy in diahost.exe.config
...
### <a name="to-disable/enable-auto-update-feature"></a>Per abilitare o disabilitare la funzionalità di aggiornamento automatico
...
- Se vengono visualizzati errori relativi alla connessione dell'archivio dati o al driver, attenersi alla procedura seguente: 
    ...
    ![Test della connessione](./media/data-factory-data-management-gateway/TestConnection.png)
...
#### <a name="gateway-cannot-connect-to-cloud-service-through-service-bus"></a>Il gateway non può connettersi al servizio cloud tramite il bus di servizio
...
#### <a name="error:-cloud-service-cannot-connect-to-gateway-through-service-bus."></a>Errore: il servizio cloud non può connettersi al gateway tramite il bus di servizio.
Seguire questi passaggi per fare in modo che il gateway torni online:
...
3. Selezionare il gateway nella sezione **GATEWAY DATI** del pannello **Servizi collegati**.
...
8. Incollare la **chiave** nella casella di testo nella pagina **Registra gateway** di **Gestione configurazione di Gateway di gestione dati** sul computer. (Facoltativo) Selezionare la casella di controllo **Mostra chiave del gateway** per visualizzare il testo della chiave. 
...
5. Fare clic sul pulsante **Crittografa** sulla barra dei comandi per avviare l'applicazione ClickOnce **Gestione credenziali**. Verrà visualizzata la finestra di dialogo **Impostazione credenziali** . 
