...
Nella pagina **Impostazioni avanzate** selezionare la casella di controllo **Configura Integration Runtime self-hosted come proxy per la Azure-SSIS Integration Runtime** , selezionare il servizio di integrazione self-hosted e il servizio collegato di archiviazione BLOB di Azure per la gestione temporanea e specificare il nome di un contenitore BLOB per il **percorso di gestione temporanea** , se lo si desidera.
