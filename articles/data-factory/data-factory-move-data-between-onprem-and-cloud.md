...
Questa procedura dettagliata illustra come creare un'istanza di Data Factory con una pipeline che sposta i dati da un database di SQL Server locale a un BLOB di Azure. Come parte della procedura dettagliata, verrà installato e configurato il Gateway di gestione dati nel computer.

## Procedura dettagliata: copiare i dati locali nel cloud
...
6. Attendere finché i valori non sono impostati come segue:
	...
	Nella scheda **Home** è inoltre possibile eseguire le operazioni seguenti: - **Registrare** un gateway con una chiave dal Portale di Azure tramite il pulsante Registra - **Interrompere** il servizio host del Gateway di gestione dati in esecuzione sul gateway. - **Pianificare gli aggiornamenti** da installare in un preciso momento del giorno. - Visualizzare la data dell’**ultimo aggiornamento** del gateway.

8. Passare alla scheda **Impostazioni**. Il certificato specificato nell’apposita sezione viene utilizzato per crittografare e decrittografare le credenziali per l'archivio dati locale specificato nel portale (facoltativo ai fini di questa esercitazione). Fare clic su **Modifica** per utilizzare il proprio certificato. Per impostazione predefinita, il gateway usa il certificato generato automaticamente dal servizio Data Factory.
...
9. (facoltativo) Passare alla scheda **Diagnostica**, selezionare l'opzione **Abilita registrazione** se si vuole abilitare la registrazione dettagliata che è possibile usare per risolvere i problemi del gateway. Le informazioni sulla registrazione si trovano nel **Visualizzatore eventi**, nel nodo **Registri applicazioni e servizi** -> **Gateway di gestione dati**.
