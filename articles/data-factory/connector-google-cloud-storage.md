...
- **Per data factory la creazione dell'interfaccia utente grafica**: `s3:ListAllMyBuckets` e `s3:ListBucket` / `s3:GetBucketLocation` per le autorizzazioni per le operazioni bucket sono necessarie anche per operazioni quali test della connessione e Esplora/Esplora file percorsi. Se non si vuole concedere queste autorizzazioni, ignorare la connessione di test nella pagina di creazione del servizio collegato e specificare il percorso direttamente nelle impostazioni del set di dati.
...
| Proprietà   | Descrizione                                                  | Obbligatoria |
| ---------- | ------------------------------------------------------------ | -------- |
| type       | La proprietà Type `location` nel set di dati deve essere impostata su **AmazonS3Location**. | Sì      |
...
| Versione | La versione dell'oggetto S3 se è stato abilitato il controllo delle versioni S3. |No |
