...
Le proprietà seguenti sono supportate per HDFS in `location` impostazioni nel set di dati basato sul formato:

| Proprietà   | Descrizione                                                  | Obbligatoria |
| ---------- | ------------------------------------------------------------ | -------- |
| type       | La proprietà Type `location` nel set di dati deve essere impostata su **HdfsLocation**. | Sì      |
...
Le proprietà seguenti sono supportate per HDFS in `storeSettings` impostazioni in origine copia basata sul formato:
...
### <a name="option-1-join-self-hosted-integration-runtime-machine-in-kerberos-realm"></a><a name="kerberos-join-realm"></a>Opzione 1: aggiungere un computer Integration Runtime self-hosted nell'area di autenticazione Kerberos
