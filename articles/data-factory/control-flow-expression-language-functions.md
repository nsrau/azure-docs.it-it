...
|string|Converte il parametro in una stringa. Ad esempio, l'espressione seguente restituisce `'10'`: `string(10)`. È anche possibile convertire un oggetto in una stringa, ad esempio se il parametro **foo** è un oggetto con una sola proprietà `bar : baz`, l'espressione seguente restituisce `{"bar" : "baz"}`.`string(pipeline().parameters.foo)`<br /><br /> **Numero di parametro**: 1<br /><br /> **Nome**: Value<br /><br /> **Descrizione**: obbligatoria. Valore convertito in una stringa.|  
