...
| [Servizio Azure Machine Learning](#azure-machine-learning-service-linked-service) | [Pipeline di esecuzione Azure Machine Learning](transform-data-machine-learning-service.md) |
...
| connectVia                   | Runtime di integrazione da usare per inviare le attività a questo servizio collegato di HDInsight. Per il servizio collegato di HDInsight su richiesta, supporta solo il runtime di integrazione di Azure. Se non diversamente specificato, viene usato il runtime di integrazione di Azure predefinito. | No       |
...
| Nome utente          | Specifica il nome dell'utente da utilizzare per connettersi a un cluster HDInsight esistente. | SÌ      |
...
| connectVia        | Il runtime di integrazione da usare per inviare le attività a questo servizio collegato. È possibile usare il runtime di integrazione di Azure o il runtime di integrazione self-hosted. Se non diversamente specificato, viene usato il runtime di integrazione di Azure predefinito. | No       |
...
## <a name="azure-machine-learning-service-linked-service"></a>Servizio collegato di Azure Machine Learning Service
Si crea un servizio collegato del servizio Azure Machine Learning per connettere un'area di lavoro del servizio Azure Machine Learning a un data factory.

> [!NOTE]
> Attualmente è supportata solo l'autenticazione basata su entità servizio per il servizio collegato Azure Machine Learning servizio.
...
| mlWorkspaceName        | Nome dell'area di lavoro del servizio Azure Machine Learning | SÌ  |
...
È possibile creare **Azure Databricks servizio collegato** per registrare l'area di lavoro di databricks che si userà per eseguire i carichi di lavoro di databricks (notebook, jar, Python). 
> [!IMPORTANT]
