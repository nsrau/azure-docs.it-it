...
4. Sostituire **nomeserver** con il nome del server di Database SQL di Azure, **nomedatabase** con il database in cui sono state create la tabella e la stored procedure, ****username@servername** con l'account utente che ha accesso al database e **password** con la password per l'account utente.
...
10. Quando lo stato di una sezione è **Pronto**, eseguire una query **select * from sampledata** sul database SQL di Azure per verificare che i dati siano stati inseriti nella tabella dalla stored procedure.
