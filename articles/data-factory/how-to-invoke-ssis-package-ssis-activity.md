---
title: Eseguire un pacchetto SSIS tramite l'attività SSIS in Azure Data Factory | Microsoft Docs
description: Questo articolo descrive come eseguire un pacchetto di SQL Server Integration Services (SSIS) da una pipeline di Azure Data Factory usando l'attività SSIS.
...
---
# <a name="run-an-ssis-package-using-the-ssis-activity-in-azure-data-factory"></a>Eseguire un pacchetto SSIS tramite l'attività SSIS in Azure Data Factory
Questo articolo descrive come eseguire un pacchetto SSIS da una pipeline di Azure Data Factory usando un'attività SSIS. 

> [!NOTE]
> Questo articolo si applica alla versione 2 del servizio Data Factory, attualmente in versione di anteprima. L'attività SSIS non è disponibile nella versione 1 del servizio Data Factory, disponibile a livello generale. Per un metodo alternativo per l'esecuzione di un pacchetto SSIS con la versione 1 del servizio Data Factory, vedere [Eseguire pacchetti SSIS tramite attività stored procedure nella versione 1](v1/how-to-invoke-ssis-package-stored-procedure-activity.md).
...
In questa sezione verrà usata l'interfaccia utente di Data Factory per creare una pipeline di Data Factory con un'attività SSIS che esegue un pacchetto SSIS.
...
In questo passaggio viene usata l'interfaccia utente di Data Factory per creare una pipeline. Si aggiunge un'attività SSIS alla pipeline e la si configura per eseguire il pacchetto SSIS. 
...
3. Nella scheda **Generale** scheda delle proprietà per l'attività SSIS, fornire un nome e una descrizione per l'attività. Facoltativamente, impostare i valori per timeout e ripetizione.

    ![Impostare le proprietà nella scheda Generale](media/how-to-invoke-ssis-package-ssis-activity/ssis-activity-general.png)

4. Nella scheda **Impostazioni** delle proprietà per l'attività SSIS selezionare il runtime di integrazione Azure-SSIS associato al database `SSISDB` in cui è distribuito il pacchetto. Fornire il percorso del pacchetto nel database `SSISDB` nel formato `<folder name>/<project name>/<package name>.dtsx`. Facoltativamente specificare l'esecuzione a 32 bit e un livello di registrazione predefinito o personalizzato e fornire un percorso di ambiente nel formato `<folder name>/<environment name>`.
...
3. Fare clic sul collegamento **View Activity Runs** (Visualizza le esecuzioni di attività) nella colonna **Azioni**. Verrà visualizzata una sola esecuzione attività dal momento che la pipeline ha una sola attività (l'attività SSIS).
...
* Data Factory versione 2 consente attualmente di creare data factory solo nelle aree Stati Uniti orientali, Stati Uniti orientali 2, Europa occidentale e Asia sud-orientale. Gli archivi dati (Archiviazione di Azure, database SQL di Azure e così via) e le risorse di calcolo (HDInsight e così via) usati dalla data factory possono trovarsi in altre aree.
