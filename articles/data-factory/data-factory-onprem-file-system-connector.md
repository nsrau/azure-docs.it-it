...
Per trovare la **versione** del gateway installato, è possibile avviare [Gestione configurazione di Gateway di gestione dati](data-factory-data-management-gateway.md#data-management-gateway-configuration-manager) nel computer e passare alla scheda **?**.

> [AZURE.NOTE] Per lo scenario della cartella locale, specificando la proprietà "host" come "localhost", l'esecuzione dell'attività di copia funzionerà con qualsiasi versione del gateway, ma è possibile usare la procedura guidata per l'impostazione. Per usare questo scenario, è consigliabile [aggiornare il gateway alla versione 2.0 o successiva](data-factory-data-management-gateway.md#update-data-management-gateway) per usare le nuove configurazioni indicate in precedenza tramite JSON e la procedura Copia guidata.
...
Proprietà | Descrizione | Obbligatorio
-------- | ----------- | --------
folderPath | Sottopercorso alla cartella. Usare il carattere di escape "\" per i caratteri speciali nella stringa. Per ottenere alcuni esempi, vedere [Servizio collegato di esempio e definizioni del set di dati](#sample-linked-service-and-dataset-definitions).<br/><br/>È possibile combinare il servizio con **partitionBy** per ottenere percorsi di cartelle basati su data e ora di inizio/fine delle sezioni. | Sì
