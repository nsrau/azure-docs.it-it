...
Per un'esercitazione relativa all'utilizzo di HDInsight, vedere [Introduzione all'utilizzo di Azure HDInsight](/en-us/manage/services/hdinsight/get-started-hdinsight/).
...
1.  Aprire Internet Explorer, quindi passare alla [pagina di installazione di Microsoft HDInsight Emulator per Azure](http://www.microsoft.com/web/gallery/install.aspx?appid=HDINSIGHT).
...
8.  Fare clic su **Esci** per chiudere la finestra di Installazione guidata piattaforma Web 4.6.
    ...
    Per informazioni sui problemi noti relativi all'installazione e all'esecuzione di HDInsight Server, vedere le [Note sulla versione di HDInsight Emulator](http://gettingstarted.hadooponazure.com/releaseNotes.html). Il file di log dell'installazione è disponibile in **C:\\HadoopFeaturePackSetup\\HadoopFeaturePackSetupTools\\gettingStarted.winpkg.install.log**.

Esecuzione di un processo MapReduce relativo al conteggio di parole
-------------------------------------------------------------------
...
Verranno utilizzati anche alcuni comandi fs. Per ulteriori informazioni sui comandi Hadoop, vedere il [manuale relativo ai comandi Hadoop](http://hadoop.apache.org/docs/r1.1.1/commands_manual.html).
...
3.  Per generare e importare i dati in HDFS, eseguire il comando seguente:
    ...
    Se si desidera invece caricare i dati nell'archivio BLOB di Azure, vedere [Connessione all'archivio BLOB di Azure](#blobstorage).
...
4.  Eseguire il comando seguente:
	...
	<tr><td>w3c_scenarios.jar</td><td>Il file con estensione jar si trova nella cartella C:\Hadoop\GettingStarted\Java.</td></tr>
...
6.  Eseguire il comando seguente per eseguire il file di script **w3ctotalhitsbypate.hql** di HiveQL.
	...
	<tr><td>C:\Hadoop\hive-0.9.0\bin\hive.cmd</td><td>Script del comando Hive.</td></tr>
	<tr><td>C:\Hadoop\GettingStarted\Hive\w3c\w3ctotalhitsbypage.hql</td><td> È possibile sostituire il file di script Hive con uno dei file seguenti: 
	<ul>
...
3.  Eseguire il comando seguente per inviare il processo Pig:
	...
	<table border="1">
	<tr><td>File</td><td>Descrizione</td></tr>
	<tr><td>C:\Hadoop\pig-0.9.3-SNAPSHOT\bin\pig.cmd</td><td>	Script del comando Pig.</td></tr>
	<tr><td>C:\Hadoop\GettingStarted\Pig\w3c\TotalHitsForPage.pig</td><td> È possibile sostituire il file di script Pig Latin con uno dei file seguenti:
	<ul>
...
Azure HDInsight utilizza l'archivio BLOB di Azure come file system predefinito. Per ulteriori informazioni, vedere [Utilizzo dell'archivio BLOB di Azure con HDInsight](/en-us/manage/services/hdinsight/howto-blob-store/).
...
><pre><code>Il processo non può accedere al file perché è in uso da un altro processo.
</code></pre>

><p>Tale errore è dovuto al fatto che anche uno dei servizi Hive di Hadoop utilizza la porta 10000. Per risolvere il problema, eseguire la procedura seguente:</p>

><ol>
<li>Interrompere i due servizi Hive di Hadoop che utilizzano services.msc, ovvero Apache Hadoop Hiveserver e Apache Hadoop Hiveserver2.</li>
<li>Avviare l'emulatore di archiviazione BLOB. </li>
<li>Riavviare i due servizi Hive di Hadoop. </li>
</ol>
...
Per istruzioni sulla creazione di un account di archiviazione, vedere [Come creare un account di archiviazione](/en-us/manage/services/storage/how-to-create-a-storage-account/).
