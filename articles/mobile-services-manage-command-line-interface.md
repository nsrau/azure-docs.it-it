...
Questo argomento descrive come usare gli strumenti da riga di comando di Azure per automatizzare la creazione e la gestione di Servizi mobili di Azure e illustra come installare e iniziare a usare gli strumenti da riga di comando per eseguire le seguenti attività di Servizi mobili:

-	[Creare un nuovo servizio mobile] 
-	[Creare una nuova tabella]
-   [Registrare uno script in un'operazione su tabella][Registrare un nuovo script di tabella]
...
+ Se non si ha un account, è possibile creare un account di valutazione gratuito in pochi minuti. Per informazioni dettagliate, vedere la pagina relativa alla <a href="http://www.windowsazure.com/pricing/free-trial/" target="_blank">versione di valutazione gratuita di Azure</a>.
...
<!--+  È necessario scaricare e installare gli strumenti da riga di comando di Azure sul computer locale. A tale scopo, seguire le istruzioni contenute nella prima sezione di questo argomento. 

+ (Facoltativo) Per poter eseguire le richieste HTTP direttamente dalla riga di comando è necessario usare cURL o uno strumento equivalente. cURL viene eseguito su varie piattaforme. Individuare e installare cURL per la propria piattaforma specifica dalla <a href=http://go.microsoft.com/fwlink/p/?LinkId=275676 target="_blank">pagina di download di cURL</a>.-->

<h2><a name="install"></a>Installare gli strumenti da riga di comando di Azure</h2>

Nel seguente elenco vengono fornite informazioni per l'installazione degli strumenti da riga di comando in base al sistema operativo in uso.
...
* **Linux**: installare la versione più recente di [Node.js][nodejs-org] (vedere l'articolo sull'[installazione di Node.js mediante Gestione pacchetti][install-node-linux]), quindi eseguire il seguente comando:
...
Importare quindi il file `.publishsettings` eseguendo il seguente comando, sostituendo prima `<path-to-settings-file>` con il percorso del proprio file `.publishsettings`:
...
Usare il seguente comando per creare una nuova istanza del servizio mobile nella sottoscrizione, dove `<service-name>` è il nome del nuovo servizio mobile, `<server-admin>` è il nome dell'account di accesso del nuovo server e `<server-password>` è la password per il nuovo account di accesso:
...
Il seguente comando restituisce un elenco di tutti i servizi mobili in una sottoscrizione di Azure:
...
È possibile usare gli strumenti da riga di comando per eliminare un servizio mobile esistente unitamente al database SQL e al server ad esso associati. Usare il seguente comando per eliminare il servizio mobile, dove `<service-name>` è il nome del servizio mobile da eliminare:
...
> [AZURE.NOTE] Se non si specifica il parametro <code>-q</code> insieme a <code>-a</code> o  <code>-d</code>, l'esecuzione viene sospesa e viene richiesto di selezionare le opzioni di eliminazione per il database SQL. Usare solo il parametro <code>-a</code> quando nessun altro servizio usa il database o il server. In caso contrario usare il parametro <code>-d</code> per eliminare solo i dati che appartengono al servizio mobile da eliminare.

<h2><a name="create-table"></a>Come creare una tabella nel servizio mobile</h2>

Il seguente comando crea una tabella nel servizio mobile specificato, dove `<service-name>` è il nome del servizio mobile e `<table-name>` è il nome della tabella da creare:
...
Usare il seguente comando per creare una nuova tabella con autorizzazione `read` pubblica e con autorizzazione `delete` concessa solo agli amministratori:

		azure mobile table create <service-name> <table-name> -p read=public,delete=admin

La seguente tabella illustra il valore dell'autorizzazione dello script rispetto al valore dell'autorizzazione nel [portale di gestione di Azure].
...
Usare il seguente comando per restituire un elenco di tutte le tabelle in un servizio mobile, dove `<service-name>` è il nome del servizio mobile:
...
Il seguente comando consente di eliminare una tabella nel servizio mobile, dove `<service-name>` è il nome del servizio mobile e `<table-name>` è il nome della tabella da eliminare:
...
Il seguente comando carica e registra una funzione in un'operazione su una tabella, dove `<service-name>` è il nome del servizio mobile, `<table-name>` è il nome della tabella e `<operation>` è l'operazione su tabella, che può essere di tipo `read`, `insert`, `update` o `delete`:

		azure mobile script upload <service-name> table/<table-name>.<operation>.js

Si noti che questa operazione comporta il caricamento di un file JavaScript (con estensione js) dal computer locale. Il nome del file deve essere composto dai nomi della tabella e dell'operazione e deve trovarsi nella sottocartella `table` relativa al percorso in cui viene eseguito il comando. Ad esempio, la seguente operazione carica e registra un nuovo script `insert` che appartiene alla tabella `TodoItems`:

		azure mobile script upload todolist table/todoitems.insert.js

La dichiarazione di funzione nel file di script deve corrispondere all'operazione su tabella registrata. Questo significa che, per uno script `insert`, lo script caricato contiene una funzione con la seguente firma:

		function insert(item, user, request) {
		    ...
		} 

Per altre informazioni sulla registrazione di script, vedere [Riferimento agli script server dei Servizi mobili][Informazioni di riferimento sugli script del server di Servizi mobili].

<!--<h2><a name="test-service"></a>Testare il nuovo servizio mobile</h2>

Quando si automatizza la creazione del servizio mobile, è possibile usare facoltativamente cURL o un altro generatore di richieste da riga di comando per 

## <a name="nextsteps"> </a>Passaggi successivi
Passaggi successivi qui...
-->
<!-- Anchors. -->
