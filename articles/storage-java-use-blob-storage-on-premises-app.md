...
-   **CloudStorageAccount**: utilizzata per inizializzare l'oggetto account con la chiave e il nome account di archiviazione di Azure e per creare l'oggetto client BLOB.
-   **CloudBlobClient**: utilizzata per accedere al servizio BLOB.
-   **CloudBlobContainer**: utilizzata per creare un contenitore BLOB, elencare i BLOB nel contenitore ed eliminare il contenitore.
-   **CloudBlockBlob**: utilizzata per caricare un file di immagine locale nel contenitore.
...
Creare il contenitore. Questo metodo creerà il contenitore se non esiste (e restituirà **true**). Se il contenitore esiste, restituirà **false**. Un'alternativa a **createIfNotExist** è il metodo **create** (che restituirà un errore se il contenitore esiste già).
