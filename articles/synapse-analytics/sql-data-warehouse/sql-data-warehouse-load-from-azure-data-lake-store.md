...
> [!NOTE]  
> Un'alternativa al caricamento è [l'istruzione COPY](https://docs.microsoft.com/sql/t-sql/statements/copy-into-transact-sql?view=azure-sqldw-latest) attualmente in anteprima pubblica.  L'istruzione COPY offre la massima flessibilità. Per fornire commenti e suggerimenti sull'istruzione COPY, sqldwcopypreview@service.microsoft.cominviare un messaggio di posta elettronica alla lista di distribuzione seguente: .
>
> [!div class="checklist"]
...
* Un account di data lake storage. Vedere [Introduzione ad Archiviazione dati di Azure](../../data-lake-store/data-lake-store-get-started-portal.md). Per questo account di archiviazione, è necessario configurare o specificare una delle credenziali seguenti da caricare: una chiave dell'account di archiviazione, un utente dell'applicazione directory di Azure o un utente AAD con il ruolo RBAC appropriato per l'account di archiviazione. 
...
Usare il comando [CREATE EXTERNAL DATA SOURCE](/sql/t-sql/statements/create-external-data-source-transact-sql) per archiviare il percorso dei dati. Se si esegue l'autenticazione con AAD pass-through, il parametro CREDENTIAL non è obbligatorio. 
...
Per un elenco completo, vedere la documentazione di T-SQL di [CREATE EXTERNAL FILE FORMAT](/sql/t-sql/statements/create-external-file-format-transact-sql)
