...
<properties urlDisplayName="Notify iOS app users by using Web API" pageTitle="Registrare l'utente corrente per le notifiche push mediante l'API Web - Hub di notifica"metaKeywords ="applicazione di registrazione di Azure, hub di notifica, notifiche push di Azure, app per iOS per le notifiche push" description="Learn how to request push notification registration in an iOS app with Azure Notification Hubs when registeration is performed by ASP.NET Web API." metaCanonical="" services="notification-hubs" documentationCenter="" title="Register the current user for push notifications by using ASP.NET" authors="yuaxu" solutions="" manager="dwrede" editor="" />
...
10.	Nel file PushToUserAppDelegate.m aggiungere il metodo del gestore seguente:
...
9. Nel metodo **viewDidLoad** del file PushToUserViewController.m inizializzare l'etichetta installationId come segue:
...
12. Copiare il codice seguente nel metodo del gestore **login** creato da XCode:
	...
	Questo metodo ottiene un ID di installazione e un canale per le notifiche push e li invia, insieme al tipo di dispositivo, al metodo API Web di autenticazione che crea una registrazione in Hub di notifica. Questa API Web è stata definita in [Usare Hub di notifica per inviare notifiche agli utenti].
