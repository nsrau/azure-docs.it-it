...
Usare il `helm version` comando per verificare la versione di Helm installata:
...
Usare il `kubectl get services` comando per ottenere l' *indirizzo IP esterno* del servizio. Ad esempio, il comando seguente mostra l' *IP esterno* per il servizio *My-nginx-ingress-controller* :
