...
Quando si crea un cluster del servizio contenitore di AZURE nel portale di Azure del portale o tramite il [az aks create][az-aks-create] comando, Azure può generare automaticamente un'entità servizio.
...
Per creare manualmente un'entità servizio con la CLI di Azure, usare il [az ad sp create-for-rbac][az-ad-sp-create] comando. Nell'esempio seguente il parametro `--skip-assignment` impedisce il completamento di qualsiasi assegnazione predefinita aggiuntiva:
...
Usare un'entità servizio esistente quando si crea un cluster AKS usando il [az aks create][az-aks-create] comando, utilizzare il `--service-principal` e `--client-secret` parametri per specificare la `appId` e `password` dall'output del [az ad sp create-for-rbac][az-ad-sp-create] comando:
...
Se si usa Registro Azure Container come archivio di immagini del contenitore, è necessario concedere le autorizzazioni per il cluster servizio Azure Kubernetes per leggere ed eseguire il pull di immagini. All'entità servizio del cluster servizio Azure Kubernetes deve essere delegato il ruolo *Lettore* nel Registro di sistema. Per informazioni dettagliate, vedere [accesso Grant AKS al registro contenitori di AZURE][aks-to-acr].
...
- Creare un [ruolo personalizzato][rbac-custom-role] e definire le autorizzazioni del ruolo seguente:
...
- In alternativa, assegnare il [collaboratore rete][rbac-network-contributor] ruolo incorporato nella subnet all'interno della rete virtuale
...
- In alternativa, assegnare il [collaboratore Account di archiviazione][rbac-storage-contributor] ruolo incorporato nel gruppo di risorse
...
- Quando si usa la [az aks create][az-aks-create] comando per generare automaticamente, l'entità servizio del servizio vengono scritte le credenziali dell'entità nel file `~/.azure/aksServicePrincipal.json` nel computer usato per eseguire il comando.
- Quando si elimina un cluster AKS che è stato creato da [az aks create][az-aks-create], non viene eliminata l'entità servizio creata automaticamente.
    - Per eliminare l'entità servizio, eseguire una query per il cluster *servicePrincipalProfile.clientId* e quindi eliminare con [delete di az ad app][az-ad-app-delete]. Sostituire il gruppo di risorse e i nomi di cluster seguenti con i propri valori:
