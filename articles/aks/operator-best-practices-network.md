...
* **Rete Kubenet** : Azure gestisce le risorse di rete virtuale perché il cluster viene distribuito e utilizza il [kubenet][kubenet] plug-in Kubernetes.
* **La rete di Azure CNI** : consente di distribuire in una rete virtuale esistente e Usa le [Azure contenitore Networking Interface (CNI)][cni-networking] plug-in Kubernetes. Ai pod vengono assegnati IP singoli che possono essere instradati ad altri servizi di rete o a risorse locali.
...
Per altre informazioni sulla delega dell'identità del servizio AKS, vedere [delegare l'accesso ad altre risorse di Azure][sp-delegation].
...
Per calcolare l'indirizzo IP richiesto, vedere [networking configurare CNI Azure nel servizio contenitore di AZURE][advanced-networking].
...
Un controller di ingresso è un daemon che viene eseguito in un nodo del servizio Azure Kubernetes e controlla le richieste in arrivo. Il traffico viene quindi distribuito in base alle regole definite nella risorsa in ingresso. Il controller in ingresso più comune è basato su [NGINX]. Servizio contenitore di AZURE non limita a un controller specifico, pertanto è possibile utilizzare altri controller, ad esempio [Contour][contour], [HAProxy][haproxy], o [Traefik][traefik].
...
**Procedure consigliate** : per analizzare il traffico in ingresso per potenziali attacchi, usare un web application firewall (WAF), ad esempio [Barracuda WAF for Azure][barracuda-waf] o Gateway applicazione di Azure. Queste risorse di rete più avanzate possono anche instradare il traffico oltre le sole connessioni HTTP e HTTPS o la terminazione SSL di base.
...
Le risorse in ingresso e il servizio di bilanciamento del carico continuano a essere eseguiti nel cluster del servizio Azure Kubernetes per perfezionare ulteriormente la distribuzione del traffico. Il gateway applicazione può essere gestito centralmente come controller in ingresso con una definizione delle risorse. Per iniziare, [creare un controller di ingresso di Gateway applicazione][app-gateway-ingress].
...
Anche la rete di gestione per il bastion host dovrebbe essere protetta. Usa un' [Azure ExpressRoute][expressroute] or [VPN gateway][vpn-gateway] per connettersi a una rete locale e controllare l'accesso usando gruppi di sicurezza di rete.

## <a name="next-steps"></a>Passaggi successivi

Questo articolo è stato incentrato sulla sicurezza e la connettività di rete. Per altre informazioni sui concetti di base di rete in Kubernetes, vedere [concetti per le applicazioni in Azure Kubernetes Service (AKS) di rete][aks-concepts-network]
