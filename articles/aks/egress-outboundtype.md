...
* Versione API di `2020-01-01` o superiore
...
* Durante la fase di anteprima, `outboundType` può essere definito solo in fase di creazione del cluster e non può essere aggiornato in seguito.
* Durante la fase di anteprima, `outboundType` cluster AKS devono usare Azure CNI. Kubenet è configurabile. l'utilizzo richiede associazioni manuali della tabella di route alla subnet AKS.
* L'impostazione `outboundType` richiede cluster AKS con una `vm-set-type` di `VirtualMachineScaleSets` e `load-balancer-sku` di `Standard`.
* Per impostare `outboundType` su un valore di `UDR` è necessaria una route definita dall'utente con connettività in uscita valida per il cluster.
* L'impostazione di `outboundType` su un valore di `UDR` implica che l'IP di origine in ingresso indirizzato al servizio di bilanciamento del carico potrebbe **non corrispondere** all'indirizzo di destinazione in uscita del cluster.

## <a name="overview-of-outbound-types-in-aks"></a>Panoramica dei tipi in uscita in AKS

Un cluster AKS può essere personalizzato con una `outboundType` univoca di tipo bilanciamento del carico o routing definito dall'utente.
...
Di seguito è riportata una topologia di rete distribuita nei cluster AKS per impostazione predefinita, che usa un `outboundType` di `loadBalancer`.
...
A questo punto, sostituire il `APPID` e `PASSWORD` di seguito con l'AppID dell'entità servizio e la password dell'entità servizio generate automaticamente dall'output del comando precedente. Si fa riferimento all'ID della risorsa VNET per concedere le autorizzazioni all'entità servizio in modo che AKS possa distribuire le risorse al suo interno.
...
Infine, il cluster AKS può essere distribuito nella subnet esistente dedicata al cluster. La subnet di destinazione da distribuire in viene definita con la variabile di ambiente, `$SUBNETID`.
...
Distribuire l'applicazione Azure vote App copiando il file YAML riportato di seguito in un file denominato `example.yaml`.
