---
title: Distribuire l'istanza del contenitore abilitato per GPUDeploy GPU-enabled container instance
description: Informazioni su come distribuire istanze di contenitori di Azure per eseguire app contenitore con utilizzo intensivo di calcolo usando le risorse GPU.
ms.topic: article
ms.date: 02/19/2020
...
Per eseguire determinati carichi di lavoro con utilizzo intensivo di calcolo in istanze del contenitore di Azure, distribuire i [gruppi di contenitori](container-instances-container-groups.md) con le *risorse GPU.* Le istanze di contenitore nel gruppo possono accedere a una o più GPU NVIDIA Tesla durante l'esecuzione dei carichi di lavoro dei contenitori, ad esempio CUDA e applicazioni di Deep Learning.

In questo articolo viene illustrato come aggiungere risorse GPU quando si distribuisce un gruppo di contenitori utilizzando un [file YAML](container-instances-multi-container-yaml.md) o un modello di [Resource Manager.](container-instances-multi-container-group.md) È anche possibile specificare le risorse GPU quando si distribuisce un'istanza del contenitore usando il portale di Azure.You can also specify GPU resources when you deploy a container instance using the Azure portal.
...
**Tipi di sistemi di oggetti oS supportati**: Solo Linux

**Ulteriori limitazioni**: Le risorse GPU non possono essere utilizzate quando si distribuisce un gruppo di contenitori in una [rete virtuale.](container-instances-vnet.md)
...
* **Conteggio** - Il numero di GPU: **1**, **2**o **4**.
...
* **Driver CUDA**: poiché viene effettuato il pre-provisioning delle istanze di contenitore con risorse della GPU con i driver NVIDIA CUDA e i runtime dei contenitori, è possibile usare le immagini dei contenitori sviluppati per carichi di lavoro CUDA.
  ...
  Supportiamo CUDA 9.0 in questa fase. Ad esempio, è possibile utilizzare le seguenti immagini di base per il file Docker:
...
Un altro modo per distribuire un gruppo di contenitori con le risorse GPU consiste nell'usare un [modello di Resource Manager](container-instances-multi-container-group.md). Per iniziare, creare un file denominato `gpudeploy.json` e quindi copiarvi il codice JSON seguente. In questo esempio viene distribuita un'istanza del contenitore con una GPU V100 che esegue un processo di training [TensorFlow](https://www.tensorflow.org/) sul set di dati MNIST. Le richieste di risorse sono sufficienti per eseguire il carico di lavoro.
