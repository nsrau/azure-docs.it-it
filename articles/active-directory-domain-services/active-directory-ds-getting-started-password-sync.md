...
### Abilitare la sincronizzazione password per i tenant solo cloud
Se l'organizzazione è un tenant di Azure AD solo cloud, gli utenti che devono usare Servizi di dominio Azure AD dovranno modificare le loro password. Questa operazione attiva la generazione in Azure AD degli hash delle credenziali richiesti da Servizi di dominio Azure AD per l'autenticazione Kerberos e NTLM. È possibile impostare come scadute le password per tutti gli utenti nel tenant che dovranno usare Servizi di dominio Azure AD oppure richiedere a tali utenti finali di cambiare le loro password.
...
4. Verrà visualizzata la pagina **Cambia password**. L'utente può quindi immettere la password esistente (precedente) e continuare con la modifica della password.
...
Dopo la modifica, la nuova password verrà sincronizzata in Servizi di dominio Azure AD dopo breve tempo. Al termine della sincronizzazione delle password, gli utenti possono quindi accedere al dominio con la password appena modificata.


### Abilitare la sincronizzazione password per i tenant sincronizzati
...
Sarà necessario installare la versione disponibile a livello generale di Azure AD Connect in un computer aggiunto a un dominio. Se si dispone di un'istanza esistente del programma di installazione di Azure AD Connect, sarà necessario aggiornarla per usare la build disponibile a livello generale di Azure AD Connect.

  [Scaricare Azure AD Connect - Versione disponibile a livello generale](http://download.microsoft.com/download/B/0/0/B00291D0-5A83-4DE7-86F5-980BC00DE05A/AzureADConnect.msi).

> [AZURE.WARNING]L'installazione della versione disponibile a livello generale di Azure AD Connect è NECESSARIA per abilitare la sincronizzazione delle credenziali di password legacy (obbligatorio per l'autenticazione NTLM e Kerberos) nel tenant di Azure AD. Questa funzionalità non è disponibile nelle versioni precedenti di Azure AD Connect.
