...
> [!NOTE]
> È possibile usare Criteri di gruppo Modelli amministrativi copiando i nuovi modelli nella workstation di gestione. Copiare i *.admx* file con estensione ADMX `%SYSTEMROOT%\PolicyDefinitions` in e copiare i file con *estensione adml* specifici delle impostazioni `%SYSTEMROOT%\PolicyDefinitions\[Language-CountryRegion]`locali in `Language-CountryRegion` , dove corrisponde alla lingua e all'area dei file con *estensione adml* .
>
