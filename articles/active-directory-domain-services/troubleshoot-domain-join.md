...
1. Provare a eseguire il ping del dominio usando il nome di dominio del dominio gestito di Azure AD DS `ping aaddscontoso.com`, ad esempio.
    * Se la risposta al ping ha esito negativo, provare a effettuare il ping degli indirizzi IP per il dominio visualizzato nella pagina panoramica nel portale per il dominio gestito di `ping 10.0.0.4`Azure AD DS, ad esempio.
