...
- Per impostazione predefinita, vengono memorizzate nella cache solo le richieste con un corpo di dimensioni inferiori a 14 Kb. Usare la funzionalità di dimensioni corpo richiesta inseribile nella cache per impostare la dimensione massima del corpo della richiesta.
...
**Scopo:** definisce la soglia per determinare se una risposta POST può essere memorizzato nella cache.
...
- Questa funzionalità è applicabile solo se le risposte POST sono idonee per la memorizzazione nella cache. Usare la funzionalità di metodi HTTP inseribili nella cache per abilitare la memorizzazione nella cache di richiesta POST.
...
**Scopo:** controlla la generazione delle `Cache-Control` intestazioni da parte del POP quando la funzionalità Max-Age esterna è attiva.
...
Valore|Risultato
--|--
Overwrite|Garantisce che vengano eseguite le azioni seguenti:<br/> -Sovrascrivere il `Cache-Control` intestazione generata dal server di origine. <br/>-Aggiunge il `Cache-Control` intestazione prodotta dalla funzionalità External Max-Age per la risposta.
Pass-through|Assicura che il `Cache-Control` intestazione prodotta dalla funzionalità External Max-Age non venga mai aggiunta alla risposta. <br/> Se il server di origine produce un' `Cache-Control` intestazione, vengono passati all'utente finale. <br/> Se il server di origine non produce un `Cache-Control` intestazione, quindi questa opzione può causare l'intestazione della risposta non contenga un `Cache-Control` intestazione.
Add if Missing (Aggiungi se mancante)|Se un `Cache-Control` intestazione non è stata ricevuta dal server di origine, quindi questa opzione aggiunge il `Cache-Control` intestazione prodotta dalla funzionalità External Max-Age. Questa opzione è utile per garantire che tutti gli asset venga assegnata una `Cache-Control` intestazione.
Rimuovere| Questa opzione assicura che un `Cache-Control` intestazione non è inclusa con la risposta di intestazione. Se un `Cache-Control` intestazione è già stata assegnata, quindi viene rimossa dalla risposta di intestazione.
...
**Scopo:** determina se la chiave di cache include o esclude i parametri di stringa di query associati a una richiesta.
...
--|--
 ...
 Escludi tutto  |Indica che vengono esclusi dalla chiave di cache tutti i parametri della stringa di query. Questa configurazione consente di duplicare il valore predefinito "standard-cache" nella pagina di memorizzazione nella cache della stringa di Query comportamento di memorizzazione nella cache.  
...
Per duplicare la stringa di query "no-cache" nella pagina di memorizzazione nella cache della stringa di Query comportamento di memorizzazione nella cache, creare una regola contenente una condizione di corrispondenza carattere jolly Query URL e una funzionalità Bypass Cache. Impostare la condizione di corrispondenza URL Query Wildcard (Carattere jolly query URL) su un asterisco (*).
...
- **Richiesta di esempio:** http://wpc.0001.&lt; dominio&gt;/800001/Origin/folder/asset.htm?sessionid=1234 & lingua = EN & userid = 01
...
- **Tipo:** escludere
- **Uno o più parametri:** sessionid userid
...
Opzione|DESCRIZIONE
--|--
Percorso originale| Consente di definire il percorso relativo dei tipi di richieste di cui viene riscritta la chiave di cache. È possibile definire un percorso relativo, selezionare un percorso origine di base e quindi definendo un modello di espressione regolare.
...
**Comportamento predefinito:** chiave di cache della richiesta è determinata dall'URI della richiesta.
...
**Scopo:** consente una nota da aggiungere all'interno di una regola.
...
**Scopo:** determina ciò che accade quando una richiesta genera un mancato riscontro nella cache parziale su un POP.
...
**Scopo:** definisce i formati di file per i file compressi nel server.
...
- Prima di aggiungere questa funzionalità a una regola, assicurarsi di impostare l'opzione di compressione disabilitata nella pagina di compressione per la piattaforma su cui viene applicata la regola.
...
**Scopo:** determina il formato e il contenuto che verrà assegnato al campo del log personalizzato in un file di log non elaborati.
...
Per impostazione predefinita, il campo del log personalizzato è denominato "x-ec_custom-1". Il nome di questo campo può essere personalizzato dalla pagina delle impostazioni di Log non elaborati.
...
**Scopo:** determina se una risposta può includere [intestazioni della risposta X-EC-Debug](cdn-http-debug-headers.md), che fornisce informazioni sui criteri di cache per l'asset richiesto.
...
Valore|Risultato
-|-
Attivato|Le richieste per le intestazioni di risposta di debug della cache restituiranno una risposta che include l'intestazione X-EC-Debug.
Disabled|L'intestazione della risposta X-EC-Debug verrà esclusa dalla risposta.
...
**Scopo:** determina l'intervallo max-age predefinito per POP per la riconvalida della cache di server di origine. In altre parole, la quantità di tempo che deve trascorrere prima che un POP verifichi se un asset memorizzato nella cache corrisponde all'asset archiviato sul server di origine.

Informazioni chiave:

- Questa azione verrà eseguita solo per le risposte da un server di origine che non ha assegnato un'indicazione di validità massima nella `Cache-Control` oppure `Expires` intestazione.
- Questa azione non viene eseguita per gli asset che non sono considerati memorizzabili nella cache.
- Questa azione non riguarda i tipi di riconvalida della cache dal browser al POP. Questi tipi di riconvalida vengono determinati dalle `Cache-Control` oppure `Expires` intestazioni inviate al browser, che possono essere personalizzati con la funzionalità External Max-Age.
...
- Impostare l'unità di tempo su "Off" per assegnare un intervallo di validità massima interna predefinita di 7 giorni per le richieste di cui non è stata assegnata un'indicazione di validità massima nella loro `Cache-Control` oppure `Expires` intestazione.
...
**Scopo:** controlla la generazione delle `Expires` intestazioni da un POP quando la funzionalità Max-Age esterna è attiva.
...
Valore|Risultato
--|--
Overwrite|Garantisce che vengano eseguite le azioni seguenti:<br/>-Sovrascrivere il `Expires` intestazione generata dal server di origine.<br/>-Aggiunge il `Expires` intestazione prodotta dalla funzionalità External Max-Age per la risposta.
Pass-through|Assicura che il `Expires` intestazione prodotta dalla funzionalità External Max-Age non venga mai aggiunta alla risposta. <br/> Se il server di origine produce un' `Expires` intestazione, ne verrà pass-through all'utente finale. <br/>Se il server di origine non produce un' `Expires` intestazione, quindi questa opzione può causare l'intestazione della risposta non contenga un' `Expires` intestazione.
Add if Missing (Aggiungi se mancante)| Se un' `Expires` intestazione non è stata ricevuta dal server di origine, quindi questa opzione aggiunge il `Expires` intestazione prodotta dalla funzionalità External Max-Age. Questa opzione è utile per garantire che tutti gli asset venga assegnati un' `Expires` intestazione.
Rimuovere| Assicura che un' `Expires` intestazione non è inclusa con la risposta di intestazione. Se un' `Expires` intestazione è già stata assegnata, quindi viene rimossa dalla risposta di intestazione.
...
**Scopo:** determina l'intervallo max-age per browser di riconvalida della cache dal POP. In altre parole, la quantità di tempo che deve trascorrere prima che un browser cerchi una nuova versione di un asset da un POP.

Abilitazione di questa funzionalità genererà `Cache-Control: max-age` e `Expires` intestazioni dai POP e inviarli al client HTTP. Per impostazione predefinita, queste intestazioni sovrascriveranno quelle create dal server di origine. Tuttavia, la modalità di gestione intestazione Cache-Control e le funzionalità di gestione intestazione scade consente di modificare questo comportamento.

Informazioni chiave:

- Questa azione non riguarda i tipi di riconvalida della cache dal POP al server di origine. Questi tipi di riconvalida vengono determinati dalle `Cache-Control` e `Expires`  intestazioni ricevute dal server di origine e può essere personalizzate con il Default Internal Max-Age e le funzionalità Force Internal Max-Age.
...
- Se si imposta questa funzionalità su un valore negativo, il POP inviare un `Cache-Control: no-cache` e un `Expires` ora in cui viene impostato nel passato con ogni risposta inviata al browser. Anche se un client HTTP non memorizza la risposta nella cache, questa impostazione non influisce sulla capacità dei POP di memorizzare nella cache la risposta ottenuta dal server di origine.
- Impostando l'unità di tempo su "Off", questa funzionalità viene disabilitata. Il `Cache-Control` e `Expires`  intestazioni memorizzato nella cache con la risposta del server di origine verranno pass-through al browser.
...
**Scopo:** determina se le richieste possono essere reindirizzate al nome host definito nell'intestazione Location restituita da un server di origine cliente.
...
**Scopo:** determina l'intervallo max-age per POP per la riconvalida della cache di server di origine. In altre parole, la quantità di tempo che deve trascorrere prima che un POP verifichi se un asset memorizzato nella cache corrisponde all'asset archiviato sul server di origine.

Informazioni chiave:

- Questa funzionalità sostituirà l'intervallo di validità massima definito nelle `Cache-Control` oppure `Expires` le intestazioni generate da un server di origine.
- Questa funzionalità non riguarda i tipi di riconvalida della cache dal browser al POP. Questi tipi di riconvalida vengono determinati dalle `Cache-Control` oppure `Expires` intestazioni inviate al browser.
...
- Impostando l'unità di tempo su "Off", questa funzionalità viene disabilitata e agli asset non verrà assegnato un intervallo di validità massima interna. Nell'intestazione originale non sono contenute istruzioni di memorizzazione nella cache, quindi l'asset verrà memorizzato nella cache in base all'impostazione attiva nella funzionalità Default Internal Max-Age.
...
**Scopo:** determina i tipi di formati di file H.264 che possono essere utilizzati per lo streaming di contenuti.

Informazioni chiave:

- Definire un set delimitato da spazi di estensioni di file H.264 consentite nell'opzione estensioni File. L'opzione estensioni File sostituirà il comportamento predefinito. Garantire il supporto di file MP4 e F4V includendo queste estensioni durante la configurazione dell'opzione. 
...
**Scopo:** determina se no-cache di un client HTTP verranno inoltrate le richieste al server di origine.

Una richiesta no-cache si verifica quando il client HTTP invia un' `Cache-Control: no-cache` e/o `Pragma: no-cache` intestazione nella richiesta HTTP.
...
Lo stato della cache che viene segnalato per una richiesta che può essere inoltrata a un server di origine grazie a questa funzionalità è `TCP_Client_Refresh_Miss`. Report stati della Cache, che è disponibile nel modulo di report principale, fornisce informazioni statistiche in base allo stato della cache. Consente di monitorare il numero e la percentuale di richieste inoltrate a un server di origine a causa di questa funzionalità.
...
**Scopo:** determina la risposta che verrà restituita ai client quando una richiesta genera un codice di stato 416 intervallo non Impossibile attenersi al richiesto.
...
Valore|Risultato
-|-
Attivato|Impedisce ai POP di rispondere a una richiesta di intervallo di byte non valida con un codice di stato 416 intervallo non Impossibile attenersi al richiesto. I server verranno invece recapitare l'asset richiesto e restituiranno un messaggio 200 OK al client.
Disabled|Ripristina il comportamento predefinito. Il comportamento predefinito prevede di rispettare il codice di stato 416 intervallo non Impossibile attenersi al richiesto.
...
**Scopo:** controlla per quanto tempo dopo la normale scadenza un asset memorizzato nella cache può essere servito da un POP quando il POP non è in grado di riconvalidare l'asset memorizzato nella cache con il server di origine.

In genere, quando il periodo di validità massima degli asset scade, il POP invia una richiesta di riconvalida al server di origine. L'origine server risponde quindi con entrambi un 304 non modificato per fornire al POP un nuovo lease sull'asset memorizzato nella cache, o con 200 OK per fornire al POP una versione aggiornata dell'asset memorizzato nella cache.
...
Si noti che questo intervallo di tempo inizia quando validità massima dell'asset scade, non quando la riconvalida ha esito negativo. Pertanto, il periodo massimo durante il quale un asset può essere servito senza riconvalida è la quantità di tempo specificato dalla combinazione di max-age più max-stale. Ad esempio, se un asset è stato memorizzato nella cache alle 9.00 con una validità massima di 30 minuti e un max-stale pari a 15 minuti, quindi un tentativo di riconvalida non riuscito alle 9:44 comporterebbe un utente finale riceve l'asset memorizzato nella cache non aggiornato, mentre un tentativo di riconvalida non riuscito alle 9:46 riceverebbe inglese d utente riceve un 504-Timeout Gateway.

Qualsiasi valore configurato per questa funzionalità è stata sostituita da `Cache-Control: must-revalidate` oppure `Cache-Control: proxy-revalidate` intestazioni ricevute dal server di origine. Se una di queste intestazioni viene ricevuta dal server di origine quando un asset viene inizialmente memorizzato nella cache, il POP non servirà un asset memorizzato nella cache non aggiornato. In tal caso, se il POP non è in grado di riconvalidare all'origine durante l'intervallo di validità massima dell'asset è scaduta, il POP restituisce un errore di Timeout Gateway 504.

Informazioni chiave:

- Per configurare questa funzionalità:
    - Selezionare il codice di stato in base al quale verrà applicato il tempo di non aggiornamento massimo.
    - Specificare un valore intero e quindi selezionare l'unità di tempo desiderata, ad esempio secondi, minuti, ore e così via. Questo valore definisce il max-stale interno che verrà applicate.
...
- Assicurarsi che il valore specificato nell'opzione nome sia una corrispondenza esatta per l'intestazione di richiesta desiderata.
- Ai fini dell'identificazione di un'intestazione non viene fatta distinzione tra maiuscole e minuscole. Ad esempio, una delle varianti seguenti del `Cache-Control` nome intestazione può essere utilizzato per facilitarne l'identificazione:
    - cache-control
    - CACHE-CONTROL
    - cachE-Control
...
- Assicurarsi che il valore specificato nell'opzione nome sia una corrispondenza esatta per l'intestazione di risposta desiderata. 
- Ai fini dell'identificazione di un'intestazione non viene fatta distinzione tra maiuscole e minuscole. Ad esempio, una delle varianti seguenti del `Cache-Control` nome intestazione può essere utilizzato per facilitarne l'identificazione:
    - cache-control
    - CACHE-CONTROL
    - cachE-Control
...
**Scopo:** determina se il contenuto memorizzato nella cache saranno idoneo per la riconvalida anticipata prima della scadenza relativa durata (TTL).
...
**Scopo:** definisce il set di [intestazioni delle richieste HTTP specifiche di Verizon](cdn-verizon-http-headers.md) che verranno inoltrate da un POP a un server di origine.
...
**Scopo:** determina come viene gestita dai POP di richiesta di un client HTTP per un asset della cache con 0 byte.
...
Per impostazione predefinita, la memorizzazione nella cache è abilitata solo per le 200 risposte OK.

Definire un set delimitato da spazi dei codici di stato desiderati.

Informazioni chiave:

- Abilitare la funzionalità Ignora origine No-Cache. Se questa funzionalità non è abilitata, quindi le risposte non 200 OK potrebbero non essere memorizzata nella cache.
...
- Questa funzionalità non può essere usata per disabilitare la memorizzazione nella cache delle risposte che generano un codice di 200 stato-OK.

**Comportamento predefinito:** memorizzazione nella cache è abilitata solo per le risposte che generano un codice di 200 stato-OK.
...
L'opzione Nome intestazione definisce il nome dell'intestazione di richiesta personalizzata in cui è archiviato indirizzo IP del client.

Questa funzionalità consente a un server di origine del cliente di trovare gli indirizzi IP dei client tramite un'intestazione di richiesta personalizzata. Se la richiesta viene gestita dalla cache, al server di origine non verrà comunicato l'indirizzo IP del client. Pertanto, è consigliabile che questa funzionalità utilizzabile con le risorse che non vengono memorizzati nella cache.

Assicurarsi che il nome di intestazione specificato non coincida con nessuno dei nomi seguenti:

- Nomi di intestazioni di richiesta standard. Un elenco di nomi di intestazioni standard è reperibile nel [RFC 2616](https://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html).
...
**Scopo:** determina se contenuto scaduto memorizzato nella cache verrà distribuito quando si verifica un errore durante la riconvalida della cache o quando si recupera il contenuto richiesto dal server di origine del cliente.
...
- Il comportamento di questa funzionalità varia in base all'unità di tempo selezionata.
    ...
    - **OFF:** selezionare "Off" per richiedere la riconvalida prima di una richiesta di possono essere serviti contenuti non aggiornati.
...
La chiave di crittografia utilizzato per crittografare e decrittografare i valori dei token è determinata dalla chiave primaria e le opzioni di Backup della chiave nella pagina di autenticazione basata su Token. Tenere presente che le chiavi di crittografia sono specifiche della piattaforma.
...
-------------|-------------|--------
301|Spostato in modo permanente|Questo codice di stato reindirizza gli utenti non autorizzati all'URL specificato nell'intestazione Location.
302|Trovato|Questo codice di stato reindirizza gli utenti non autorizzati all'URL specificato nell'intestazione Location. Questo codice di stato costituisce il metodo standard per eseguire un'operazione di reindirizzamento.
307|Reindirizzamento temporaneo|Questo codice di stato reindirizza gli utenti non autorizzati all'URL specificato nell'intestazione Location.
401|Non autorizzata|La combinazione di questo codice di stato con l'intestazione di risposta WWW-Authenticate consente di richiedere a un utente per l'autenticazione.
403|Accesso negato|Questo messaggio è standard 403 Forbidden messaggio di stato viene visualizzato un utente non autorizzato durante il tentativo di accedere ai contenuti protetti.
...
Questa funzionalità supporta il reindirizzamento degli URL a un URL definito dall'utente quando è configurato per restituire un codice di stato 3xx. Per specificare l'URL definito dall'utente, seguire questa procedura:

1. Selezionare un codice di risposta 3xx per la funzionalità Token Auth Denial Code.
2. Selezionare l'opzione Nome intestazione facoltativo "Location".
3. Impostare l'opzione valore intestazione facoltativo sull'URL desiderato.
...
Il reindirizzamento degli URL è applicabile solo per i codici di risposta 3xx.

L'opzione valore intestazione facoltativo supporta caratteri alfanumerici, virgolette e spazi.

#### <a name="authentication"></a>Authentication

Questa funzionalità supporta la possibilità di includere l'intestazione WWW-Authenticate quando si risponde a una richiesta non autorizzata di contenuti protetti con l'autenticazione basata su Token. Se l'intestazione WWW-Authenticate è stata impostata su "basic" nella configurazione, quindi verrà richiesto all'utente non autorizzato per le credenziali dell'account.
...
2. Nell'opzione Nome intestazione facoltativo, selezionare "WWW-Authenticate".
3. Impostare l'opzione valore intestazione facoltativo su "basic".

L'intestazione WWW-Authenticate è applicabile solo per i codici di risposta 401.
...
**Scopo:** determina se sono tra maiuscole e minuscole nei confronti tra URL eseguiti dall'autenticazione basata su Token.
...
**Scopo:** determina se il parametro di stringa di query di autenticazione basata su Token deve essere rinominato.

Informazioni chiave:

- L'opzione valore definisce il nome di parametro di stringa di query tramite il quale si può specificare un token.
...
Valore|Risultato
----|----
Attivato|L'opzione valore definisce il nome di parametro di stringa di query tramite il quale devono essere definiti i token.
...
Source & Pattern (Origine e modello)| Queste impostazioni definiscono un modello di URI di richiesta che identifica il tipo di richieste che possono essere reindirizzate. Verranno reindirizzate solo le richieste il cui URL soddisfa entrambi i criteri seguenti: <br/> <br/> **Origine (o un punto di accesso al contenuto):** selezionare un percorso relativo che identifica un server di origine. Questo percorso è costituito dalla sezione _/XXXX/_ e dal nome dell'endpoint. <br/><br/> **Origine (modello):** deve essere definito un modello che identifica le richieste in base al percorso relativo. Questo criterio di espressione regolare deve definire un percorso che inizia dopo l'accesso al contenuto selezionato in precedenza puntare direttamente (vedere sopra). <br/> - Assicurarsi che i criteri dell'URI di richiesta, ad esempio origine e modello, definiti in precedenza non entrino in conflitto con le condizioni di corrispondenza definite per questa funzionalità. <br/> - Specificare un criterio; se si usa un valore vuoto come criterio, viene eseguito il confronto di tutte le stringhe.
Destination| Definire l'URL a cui verranno reindirizzate le richieste precedenti. <br/><br/> Costruire l'URL in modo dinamico usando: <br/> -Un criterio di espressione regolare <br/>- [Variabili HTTP](cdn-http-variables.md) <br/><br/> Sostituire i valori acquisiti nel modello di origine nel modello di destinazione utilizzando $_n_ in cui _n_ identifica un valore in base all'ordine in cui è stato acquisito. Ad esempio, $1 rappresenta il primo valore acquisito nel modello di origine e $2 rappresenta il secondo valore. <br/> 
Si consiglia di utilizzare un URL assoluto. poiché l'uso di un URL relativo può reindirizzare gli URL CDN su un percorso non valido.
...
- Tutte le richieste corrispondenti verranno reindirizzate per l'URL CNAME periferico definito nell'opzione destinazione. 
...
- La variabile schema richiesta (% {scheme}) viene sfruttata nell'opzione destinazione, che assicura che lo schema della richiesta rimanga invariato dopo il reindirizzamento.
...
Opzione|DESCRIZIONE
-|-
 Source & Pattern (Origine e modello) | Queste impostazioni definiscono un modello di URI di richiesta che identifica il tipo di richieste che possono essere riscritte. Verranno riscritte solo le richieste il cui URL soddisfa entrambi i criteri seguenti: <br/><br/>  - **Origine (o un punto di accesso al contenuto):** selezionare un percorso relativo che identifica un server di origine. Questo percorso è costituito dalla sezione _/XXXX/_ e dal nome dell'endpoint. <br/><br/> - **Origine (modello):** deve essere definito un modello che identifica le richieste in base al percorso relativo. Questo criterio di espressione regolare deve definire un percorso che inizia dopo l'accesso al contenuto selezionato in precedenza puntare direttamente (vedere sopra). <br/> Assicurarsi che i criteri dell'URI di richiesta, ad esempio origine e modello, definiti in precedenza non entrino in conflitto con le condizioni di corrispondenza definite per questa funzionalità. Specificare un criterio; se si usa un valore vuoto come criterio, tutte le stringhe vengono confrontate. 
 Destination  |Definire l'URL relativo in cui verranno riscritte le richieste precedenti: <br/>    1. Se si seleziona un punto di accesso al contenuto che identifica un server di origine. <br/>    2. Definendo di un percorso tramite: <br/>        -Un criterio di espressione regolare <br/>        - [Variabili HTTP](cdn-http-variables.md) <br/> <br/> Sostituire i valori acquisiti nel modello di origine nel modello di destinazione utilizzando $_n_ in cui _n_ identifica un valore in base all'ordine in cui è stato acquisito. Ad esempio, $1 rappresenta il primo valore acquisito nel modello di origine e $2 rappresenta il secondo valore. 
