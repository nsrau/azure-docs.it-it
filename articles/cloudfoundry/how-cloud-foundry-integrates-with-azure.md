...
description: Viene descritto come Cloud Foundry possono utilizzare servizi di Azure per migliorare l'esperienza aziendale
...
ms.locfileid: "57550174"
---
# <a name="integrate-cloud-foundry-with-azure"></a>Integrare Cloud Foundry con Azure

[Cloud Foundry](https://docs.cloudfoundry.org/) è una piattaforma PaaS in esecuzione sulla piattaforma IaaS dei provider di servizi cloud. Offre un'esperienza coerente di distribuzione delle applicazioni tra i diversi provider di servizi cloud. Può anche si integrano con diversi servizi di Azure, di livello aziendale a disponibilità elevata, scalabilità e risparmi sui costi.
Esistono [6 sottosistemi di Cloud Foundry](https://docs.cloudfoundry.org/concepts/architecture/), che possono essere ridimensionati online in modo flessibile, tra cui: Routing, l'autenticazione, gestione del ciclo di vita delle applicazioni, gestione del servizio di messaggistica e il monitoraggio. Per ognuno dei sottosistemi, è possibile configurare Cloud Foundry per usare il servizio di Azure corrispondente. 
...
Bosh Usa CPI di Azure (Cloud Provider Interface) per la creazione del disco e routine di eliminazione. Per impostazione predefinita, vengono usati dischi non gestiti. Il cliente deve creare manualmente gli account di archiviazione e quindi configurare gli account nei file manifesto CF. Si tratta a causa della limitazione sul numero di dischi per account di archiviazione.
Ora è disponibile il servizio [Managed Disks](https://azure.microsoft.com/services/managed-disks/), che offre archiviazione su disco gestita, sicura e affidabile per le macchine virtuali. I clienti non devono più gestire l'account di archiviazione per la scalabilità e la disponibilità elevata. Azure organizza automaticamente i dischi. Se si tratta di un nuovo oggetto o una distribuzione esistente, CPI Azure gestirà la creazione o la migrazione del disco gestito durante una distribuzione cloud Foundry. È supportato con 1.11 Pivotal cloud Foundry. Per informazioni, è anche possibile vedere le [indicazioni sull'uso di Managed Disks](https://github.com/cloudfoundry-incubator/bosh-azure-cpi-release/tree/master/docs/advanced/managed-disks) con Cloud Foundry open source. 
...
Azure Load Balancer è un servizio di bilanciamento del carico di livello 4. Viene utilizzato per distribuire il traffico tra le istanze dei servizi in un set con bilanciamento del carico. La versione Standard fornisce [funzionalità avanzate](https://docs.microsoft.com/azure/load-balancer/load-balancer-overview) in aggiunta a quanto disponibile nella versione Basic. Ad esempio: 1. Il limite massimo per il pool back-end passa da 100 a 1000 macchine virtuali.  2. Gli endpoint supportano ora più set di disponibilità invece di un unico set di disponibilità.  3. Le funzionalità aggiuntive come le porte a disponibilità elevata, i dati di monitoraggio più completi e così via. Se si spostano in zona di disponibilità di Azure, load balancer standard è necessaria. Per una nuova distribuzione, è consigliabile iniziare con il servizio Load Balancer Standard. 
...
Il Nozzle di Analitica di Log di Azure è un componente di Foundry Cloud, che inoltra le metriche dal [firehose loggregator di Cloud Foundry](https://docs.cloudfoundry.org/loggregator/architecture.html) al [monitoraggio di Azure registra](https://azure.microsoft.com/services/log-analytics/). Questo strumento consente di raccogliere, visualizzare e analizzare le metriche relative a prestazioni e integrità del sistema CF tra più distribuzioni.
Fare clic su [qui](https://docs.microsoft.com/azure/cloudfoundry/cloudfoundry-oms-nozzle) per informazioni su come distribuire il Nozzle di Analitica di Log di Azure in Open Source e ambiente di Pivotal Cloud Foundry e quindi accedere ai dati da Azure Monitor i log di console. 
> [!NOTE]
> Dalla versione 2.0 di Pivotal cloud Foundry, le metriche sull'integrità BOSH per le macchine virtuali vengono inoltrate al Loggregator Firehose per impostazione predefinita e sono integrate nella console di log di monitoraggio di Azure.
...
Mentre le macchine virtuali serie F e D erano comunemente consigliate per l'ambiente di produzione Pivotal Cloud Foundry, la nuova [serie B](https://azure.microsoft.com/blog/introducing-b-series-our-new-burstable-vm-size/) con possibilità di burst offre nuove opzioni. Macchine virtuali serie B che prevedono burst sono ideali per carichi di lavoro che non necessitano delle prestazioni complete della CPU in modo continuo, ad esempio server web, database di piccole dimensioni e lo sviluppo e ambienti di test. Questi carichi di lavoro hanno in genere requisiti relativi alle prestazioni che prevedono burst. Il costo è di € 0,011/ora (B1) rispetto a € 0,043/ora (F1). Vedere l'elenco completo di [dimensioni di macchine virtuali](https://docs.microsoft.com/azure/virtual-machines/linux/sizes-general) e [prezzi](https://azure.microsoft.com/pricing/details/virtual-machines/linux/) per informazioni dettagliate. 
#### <a name="managed-standard-disk"></a>Managed Disks Standard: 
Per prestazioni affidabili nell'ambiente di produzione erano consigliati i dischi Premium.  Con [Managed Disks](https://azure.microsoft.com/services/managed-disks/), l'archiviazione Standard può offrire un livello di affidabilità simile, con prestazioni diverse. Per carico di lavoro non sensibili alle prestazioni, ad esempio sviluppo/Test o di ambiente non critici, managed disks standard offrono un'opzione alternativa con costi più bassi.  
...
#### <a name="use-azure-first-party-services"></a>Usare i servizi di terze parti prima di Azure: 
...
Le funzionalità di integrazione di Azure sono disponibili con innanzitutto [Open Source Cloud Foundry](https://github.com/cloudfoundry-incubator/bosh-azure-cpi-release/tree/master/docs/advanced/), prima che sia disponibile in Pivotal Cloud Foundry. Le funzionalità contrassegnate con * non son ancora disponibili tramite PCF. Integrazione di cloud Foundry con Azure Stack non viene trattato in questo documento sia.
