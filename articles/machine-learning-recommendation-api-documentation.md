...
API documentation" pageTitle="Raccomandazioni per l'apprendimento automatico di Azure: 
documentazione dell'API" description="Raccomandazioni su Azure Machine Learning - 
Documentazione sull'API" metaKeywords="" services="machine-learning" solutions="" documentationCenter="" authors="AharonGumnik" manager="paulettm" editor="cgronlun" videoId="" scriptId="" />
...
#Raccomandazioni per l'apprendimento automatico di Azure: <br>documentazione dell'API

Questo documenta illustra le API Recommendations di Azure ML.

##Sommario

- [1. Panoramica generale](#1-general-overview)
- [2. Limitazioni](#2-limitations)
- [3. API: informazioni generali](#3-apis--general-information)
	- [3.1. Autenticazione](#31-authentication)
	- [3.2. URI del servizio](#32-service-uri)
	- [3.3. Versione dell'API](#33-api-version)
- [4. Modello Basic](#4-model-basic)
	- [4.1. Creare il modello](#41-create-a-model)
	- [4.2. Ottenere il modello](#42-get-model)
	- [4.3. Ottenere tutti i modelli](#43-get-all-models)
	- [4.4. Aggiornare il modello](#44-update-model)
	- [4.5. Eliminare il modello](#45-delete-model)
- [5. Modello Advanced](#5-model-advanced)
	- [5.1. Modello Data Insight](#51-model-data-insight)
	- [5.2. Modello Insight](#52-model-insight)
	- [5.3. Ottenere un esempio del modello](#53-get-model-sample)
- [6. Modello Business Rules](#6-model-business-rules)
	- [6.1. Ottenere le regole del modello](#61-get-model-rules)
	- [6.2. Aggiungere una regola](#62-add-rule)
	- [6.3. Eliminare una regola](#63-delete-rule)
	- [6.4. Eliminare tutte le regole](#64-delete-all-rules)
- [7. Catalogo](#7-catalog)
	- [7.1. Importare i dati del catalogo](#71-import-catalog-data)
	- [7.2. Ottenere il catalogo](#72-get-catalog)
	- [7.3. Ottenere gli elementi del catalogo in base al token](#73-get-catalog-items-by-token)
- [8. Dati di utilizzo](#8-usage-data)
	- [8.1. Importare i dati di utilizzo](#81-import-usage-data)
		- [8.1.1. Caricamento del file](#811-uploading-file)
		- [8.1.2. Uso dell'acquisizione dei dati](#812-using-data-acquisition)
	- [8.2. Elencare i file di utilizzo del modello](#82-list-model-usage-files)
	- [8.3. Ottenere statistiche di utilizzo](#83-get-usage-statistics)
	- [8.4. Ottenere un esempio del file di utilizzo](#84-get-usage-file-sample)
	- [8.5. Ottenere il file di utilizzo del modello](#85-get-model-usage-file)
	- [8.6. Eliminare il file di utilizzo](#86-delete-usage-file)
	- [8.7. Eliminare tutti i file di utilizzo](#87-delete-all-usage-files)
- [9. Compilazione](#9-build)
	- [9.1. Compilare un modello](#91-build-a-model)
	- [9.2. Ottenere lo stato delle compilazioni di un modello](#92-get-builds-status-of-a-model)
	- [9.3. Ottenere lo stato delle compilazioni di un utente](#93-get-builds-status-of-a-user)
	- [9.4. Eliminare una compilazione](#94-delete-build)
	- [9.5. Annullare una compilazione](#95-cancel-build)
	- [9.6. Ottenere i parametri della compilazione](#96-get-build-parameters)
- [10. Raccomandazione](#10-recommendation)
	- [10.1. Ottenere raccomandazioni](#101-get-recommendations)
- [11. Notifiche](#11-notifications)
	- [11.1. Ottenere notifiche](#111-get-notifications)
	- [11.2. Eliminare le notifiche di modello](#112-delete-model-notifications)
	- [11.3. Eliminare le notifiche utente](#113-delete-user-notifications)
- [12. Note legali](#12-legal)
...
1.	<ins>Modello Basic</ins>API che consentono di eseguire operazioni di base sul modello (ad esempio, create, aggiornare ed eliminare un modello).
...
4.	<ins>Catalogo</ins>: API che consentono di eseguire operazioni di base sul catalogo di un modello. Un catalogo contiene informazioni sui metadati relativi agli elementi dei dati di utilizzo.
5.	<ins>Dati di utilizzo</ins>: API che consentono di eseguire operazioni di base sui dati di utilizzo del modello. Nella forma di base, i dati di utilizzo sono costituiti da righe che includono coppie di <userId>,<itemId>.
6.	<ins>Compilazione</ins>: API che consentono di attivare una compilazione di modello ed eseguire operazioni di base associate alla compilazione. È possibile attivare una compilazione di modello solo se si dispone di dati di utilizzo rilevanti.
7.	<ins>Raccomandazione</ins>: quando la build di un modello è completa, è possibile usare le indicazioni facendo riferimento a queste API.
...
* Numero massimo di elementi contenuti in un catalogo: 100.000
* Numero massimo di punti di utilizzo mantenuti: 5.000.000. I punti di utilizzo meno recenti vengono eliminati nel momento in cui vengono caricati o riportati nuovi punti di utilizzo.
...
Gli URI radice del servizio per ogni API Recommendations di Azure ML sono disponibili [qui.](https://api.datamarket.azure.com/amla/recommendations/v1/)
...
|	apiVersion		| 1.0 |
|||
| Corpo richiesta | <ins>Per aggiungere una regola BlockList:</ins><br>`<ApiFilter xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><ModelId>24024f7e-b45c-419e-bfa2-dfd947e0d253</ModelId><Type>BlockList</Type><Value>{"ItemsToExclude":["2406E770-769C-4189-89DE-1C9283F93A96"]}</Value></ApiFilter>`<br><br><ins>Per aggiungere una regola BlockList:</ins><br>`<ApiFilter xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><ModelId>24024f7e-b45c-419e-bfa2-dfd947e0d253</ModelId><Type>Upsale</Type><Value>{"ItemsToUpsale":["2406E770-769C-4189-89DE-1C9283F93A96"]}</Value></ApiFilter>`|
...
Corpo della richiesta
	...
	Esempio di clic di raccomandazione:
	<Event xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  	<ModelId>2779c063-48fb-46c1-bae3-74acddc8c1d1</ModelId>
  	<SessionId>11112222</SessionId>
  	<EventData>
	...
	Esempio di rimozione dal carrello:
	<Event xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  	<ModelId>2779c063-48fb-46c1-bae3-74acddc8c1d1</ModelId>
  	<SessionId>11112222</SessionId>
  	<EventData>
...
###8,2.	Elencare i file di utilizzo del modello
...
| eventTypes |	Stringa separata da virgole di tipi di evento specifici o null per ottenere tutti gli eventi.  |
| apiVersion | 1.0 |
|||
| Corpo richiesta | NONE |
...
###8,4.	Ottenere un esempio del file di utilizzo
...
###8,5.	Ottenere il file di utilizzo del modello
...
###8,6.	Eliminare il file di utilizzo
...
###8,7.	Eliminare tutti i file di utilizzo
...
|:--------			|:--------								|
| modelId |	Identificatore univoco del modello.  |
| userDescription | Identificatore testuale del catalogo. Tenere presente che, se si usano degli spazi, è necessario codificarli con il simbolo %20. Vedere l'esempio sopra riportato.<br>Lunghezza massima: 50 |
| apiVersion | 1.0 |
|||
| Corpo richiesta | Se questo valore non viene specificato, la compilazione verrà eseguita con i parametri predefiniti<br><br>Per impostare i parametri della compilazione, inviare il seguente codice XML nel corpo (vedere la sezione Ottenere i parametri della compilazione per una descrizione dei parametri):<br>`<BuildParametersList xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><NumberOfModelIterations>40</NumberOfModelIterations><NumberOfModelDimensions>20</NumberOfModelDimensions><UseFeaturesInModel>false</UseFeaturesInModel><AllowColdItemPlacement>false</AllowColdItemPlacement><MinItemAppearance>5</MinItemAppearance><MinUserAppearance>5</MinUserAppearance><EnableFeatureCorrelation>false</EnableFeatureCorrelation><RefreshFeatureScoreOnBuild>false</RefreshFeatureScoreOnBuild><ComputeUpd>false</ComputeUpd><EnableModelingInsights>true</EnableModelingInsights><ModelingFeatureList /><ReasoningFeatureList /></BuildParametersList>`|
...
XML OData
	...
	La risposta include una voce per ciascuna compilazione e in ogni voce sono riportati i dati seguenti:
...
Consente di eliminare una compilazione.
...
|:--------			|:--------								|
| modelId | Identificatore univoco del modello. |
| itemIds | Elenco separato da virgole degli elementi per i quali aggiungere raccomandazioni.<br>Lunghezza massima: 200 |
| numberOfResults | Numero dei risultati richiesti. |
...
##11. Notifiche
L'API Recommendations di Azure ML crea una notifica quando nel sistema si verificano continui errori. Sono possibili tre tipi di notifica.
...
|	Nome parametro	|	Valori validi						|
|:--------			|:--------								|
| modelId | Parametro facoltativo: se omesso, vengono restituite le tutte le notifiche relative a tutti i modelli. <br>Valore valido: Identificatore univoco del modello.|
| apiVersion | 1.0 |
|||
| Corpo richiesta | NONE |
...
##12. Note legali
Questo documento viene fornito "così com'è". Le informazioni e le indicazioni riportate nel presente documento, inclusi URL e altri riferimenti a siti Internet, sono soggette a modifica senza preavviso.<br><br>
...
(c) 2014 Microsoft. Tutti i diritti riservati.
