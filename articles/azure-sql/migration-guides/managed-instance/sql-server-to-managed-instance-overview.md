...
> [!IMPORTANT]
> Eventuali discrepanze nei [requisiti della rete virtuale dell'istanza gestita](/azure/azure-sql/managed-instance/connectivity-architecture-overview#network-requirements) possono impedire la creazione di nuove istanze o l'utilizzo di quelle esistenti. Altre informazioni sulla [creazione di nuove](/azure/azure-sql/managed-instance/virtual-network-subnet-create-arm-template?branch=release-ignite-arc-data)   reti e sulla configurazione di reti [esistenti](/azure/azure-sql/managed-instance/vnet-existing-add-subnet?branch=release-ignite-arc-data)   . 
...
|[Importazione/esportazione guidata/BACPAC](/azure/azure-sql/database/database-import?tabs=azure-powershell)| [BacPac](/sql/relational-databases/data-tier-applications/data-tier-applications#bacpac) è un file di Windows con un' `.bacpac` estensione che incapsula lo schema e i dati di un database. BACPAC può essere usato per esportare i dati da un SQL Server di origine e per importare il file di nuovo in Azure SQL Istanza gestita.  |  
|[Azure Data Factory](/azure/data-factory/connector-azure-sql-managed-instance)| L' [attività di copia](/azure/data-factory/copy-activity-overview) in Azure Data Factory esegue la migrazione dei dati da database SQL Server di origine a SQL istanza gestita usando connettori predefiniti e un [Integration Runtime](/azure/data-factory/concepts-integration-runtime).</br> </br> ADF supporta un'ampia gamma di [connettori](/azure/data-factory/connector-overview) per spostare i dati da origini SQL Server a SQL istanza gestita. |
| | |
...
Eseguire la migrazione di progetti e pacchetti di SQL Server Integration Services (SSIS) in SSISDB al Istanza gestita SQL di Azure tramite il [servizio migrazione del database di Azure (DMS)](/azure/dms/how-to-migrate-ssis-packages-managed-instance). 
...
In alternativa, è anche possibile eseguire la migrazione dei modelli tabulari Analysis Services locali per [Power bi Premium utilizzando i nuovi endpoint di lettura/scrittura XMLA](https://docs.microsoft.com/power-bi/admin/service-premium-connect-tools). 
> [!NOTE]
