...
description: Usare la replica geografica attiva per creare database secondari leggibili di singoli database nel database SQL di Azure nella stessa area del Data Center o in aree diverse.
...
La replica geografica attiva è una funzionalità del database SQL di Azure che consente di creare database secondari leggibili di singoli database in un server nello stesso data center o in un'altra area.
...
![replica geografica attiva](./media/active-geo-replication-overview/geo-replication.png )
...
- Il[portale di Azure](active-geo-replication-configure-portal.md)
...
![relazione di replica geografica](./media/active-geo-replication-overview/geo-replication-relationship.png)
...
## <a name="cross-subscription-geo-replication"></a>Replica geografica tra sottoscrizioni

Per configurare la replica geografica attiva tra due database appartenenti a sottoscrizioni diverse (indipendentemente dal fatto che si trovino nello stesso tenant), è necessario seguire la procedura speciale descritta in questa sezione.  La procedura è basata sui comandi SQL e richiede:

- Creazione di un account di accesso con privilegi in entrambi i server
...
Il client che esegue le modifiche richiede l'accesso alla rete al server primario. Anche se lo stesso indirizzo IP del client deve essere aggiunto all'elenco Consenti sul server secondario, la connettività di rete al server secondario non è strettamente necessaria.

### <a name="on-the-master-of-the-primary-server"></a>Nel database master del server primario

1. Aggiungere l'indirizzo IP all'elenco Consenti del client che esegue le modifiche. per ulteriori informazioni, vedere [configurare il firewall](firewall-configure.md).
1. Creare un account di accesso dedicato per configurare la replica geografica attiva (e modificare le credenziali in base alle esigenze):
...
1. Prendere nota del SID del nuovo account di accesso usando questa query:
...
1. Creare un utente per lo stesso account di accesso:
...
> [!IMPORTANT]
> Per script di esempio, vedere [Configurare un database singolo ed eseguirne il failover usando la replica geografica attiva](scripts/setup-geodr-and-failover-database-powershell.md) e [Configurare un database in pool ed eseguirne il failover usando la replica geografica attiva](scripts/setup-geodr-and-failover-elastic-pool-powershell.md).
