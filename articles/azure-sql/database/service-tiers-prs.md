---
title: Ritiro del livello di servizio Premium RS
description: Il livello di servizio Premium RS per il database SQL di Azure è in fase di ritiro e il supporto per esso sta per scadere. vedere Opzioni di migrazione.
...
- Livelli di servizio vCore
  ...
  Livelli di servizio **Utilizzo generico** e **Business critical** nel [modello di acquisto basato su vCore](service-tiers-vcore.md). Questi due livelli di servizio sono in disponibilità generale. Il modello di acquisto basato su vCore offre anche il livello di servizio di **iperscalabilità** che si adatta alle esigenze del carico di lavoro con scalabilità automatica fino a 100 TB per database. Il livello di servizio Hyperscale garantisce prestazioni I/O paragonabili a quelle del livello di servizio Premium nel [modello di acquisto basato su DTU](service-tiers-dtu.md) a un prezzo più vicino a quello del livello di servizio Premium RS.
...
I livelli di servizio del modello di acquisto basato su vCore danno diritto a offerte speciali che consentono di risparmiare fino all'80% rispetto ai prezzi con licenza inclusa. Usa le tue licenze SQL Server Standard o Enterprise Edition con Software Assurance attivo per risparmiare fino al 55% rispetto ai prezzi con licenza inclusa con i [vantaggio Azure Hybrid](https://azure.microsoft.com/pricing/hybrid-benefit/) per SQL Server. È possibile combinare il Vantaggio Azure Hybrid con l'offerta [Capacità di riserva del database SQL di Azure](reserved-capacity-overview.md) e risparmiare fino all'80% impegnandosi in anticipo per un periodo di uno o tre anni.  È possibile attivare entrambi i vantaggi già da oggi sul portale di Azure.
...
- Per informazioni dettagliate sulle dimensioni di calcolo specifiche e sulle opzioni relative alle dimensioni di archiviazione disponibili per database singoli, vedere [Limiti delle risorse basate su vCore del database SQL per database singoli](resource-limits-vcore-single-databases.md)
- Per informazioni dettagliate sulle dimensioni di calcolo specifiche e sulle opzioni di archiviazione disponibili per i pool elastici, vedere [limiti delle risorse basate su vCore del database SQL per i pool elastici](resource-limits-vcore-elastic-pools.md).
