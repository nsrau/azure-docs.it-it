---
title: Risolvere i problemi della sincronizzazione dati SQL
description: Informazioni su come identificare, risolvere i problemi e risolvere i problemi comuni con sincronizzazione dati SQL in Azure.
...
ms.locfileid: "91616466"
---
# <a name="troubleshoot-issues-with-sql-data-sync"></a>Risolvere i problemi della sincronizzazione dati SQL
[!INCLUDE[appliesto-sqldb](../includes/appliesto-sqldb.md)]

Questo articolo descrive come risolvere i problemi noti con sincronizzazione dati SQL in Azure. Le possibili risoluzioni a un problema vengono fornite qui.
...
- [Viene visualizzato il messaggio seguente: "Impossibile inserire il valore NULL nella colonna \<column> . La colonna non ammette valori null. Che cosa significa e come si può risolvere il problema?](#sync-nulls)
...
> [!NOTE]
> Se queste informazioni non hanno consentito di cambiare lo stato di elaborazione del gruppo di sincronizzazione, il supporto tecnico Microsoft può reimpostare lo stato del gruppo di sincronizzazione. Per fare in modo che lo stato del gruppo di sincronizzazione venga reimpostato, creare un post nella [pagina dedicata alle domande e risposte di Microsoft del database SQL di Azure](https://docs.microsoft.com/answers/topics/azure-sql-database.html). Nel post includere l'ID sottoscrizione e l'ID gruppo di sincronizzazione per il gruppo da reimpostare. Un tecnico del supporto tecnico Microsoft risponderà al post e comunicherà quando lo stato sarà stato reimpostato.
...
### <a name="i-see-inconsistent-primary-key-data-after-a-successful-sync"></a><a name="sync-pkdata"></a> Vengono visualizzati dati della chiave primaria incoerenti dopo una sincronizzazione riuscita
...
Per risolvere i problemi con l'agente client, vedere [Troubleshoot Data Sync Agent issues](sql-data-sync-agent-overview.md#agent-tshoot) (Risolvere i problemi relativi all'agente di sincronizzazione dati).
...
- [Non è possibile annullare la registrazione di un database di SQL Server](#setup-unreg)
...
### <a name="i-get-a-disk-out-of-space-message"></a><a name="setup-space"></a> Viene visualizzato un messaggio di spazio su disco non sufficiente
...
- **Risoluzione**. Per annullare la registrazione di un database di SQL Server, selezionare il database e selezionare **forza eliminazione**.
...
### <a name="i-dont-have-sufficient-privileges-to-start-system-services"></a><a name="setup-perms"></a> Non si hanno privilegi sufficienti per avviare i servizi di sistema
...
### <a name="what-happens-when-i-restore-a-lost-or-corrupted-database"></a><a name="setup-restore"></a> Cosa accade quando si ripristina un database perso o danneggiato?
...
-   Aggiornare lo schema di sincronizzazione
    -   Con Transact-SQL- [automatizzare la replica delle modifiche dello schema in sincronizzazione dati SQL in Azure](sql-data-sync-update-sync-schema.md)
