...
L'estensione SQL Server IaaS viene installata quando si esegue la registrazione della macchina virtuale di SQL Server con il [provider di risorse per VM SQL](sql-vm-resource-provider-register.md). Se necessario, è possibile installare SQL Server IaaS Agent manualmente usando il seguente comando di PowerShell: 
