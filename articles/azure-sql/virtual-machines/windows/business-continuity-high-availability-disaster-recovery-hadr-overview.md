...
| --- | --- |
| **Gruppi di disponibilità** |Le repliche di disponibilità in esecuzione nelle macchine virtuali di Azure nella stessa area assicurano disponibilità elevata.  È necessario configurare una macchina virtuale da usare come controller di dominio perché il servizio Clustering di failover di Windows richiede un dominio di Active Directory.<br/><br/> Per una maggiore ridondanza e disponibilità, le macchine virtuali di Azure possono essere distribuite in diverse [zone di disponibilità](../../../availability-zones/az-overview.md), come illustrato nella [panoramica dei gruppi di disponibilità](availability-group-overview.md). Se le macchine virtuali di SQL Server in un gruppo di disponibilità vengono distribuite in zone di disponibilità, usare [Load Balancer Standard](../../../load-balancer/load-balancer-standard-overview.md) come listener, come descritto negli articoli [Interfaccia della riga di comando delle macchine virtuali di Azure SQL](availability-group-az-cli-configure.md) e [Modelli di avvio rapido di Azure](availability-group-quickstart-template-configure.md).<br/> ![Gruppi di disponibilità](./media/business-continuity-high-availability-disaster-recovery-hadr-overview/azure-only-ha-always-on.png)<br/>Per altre informazioni, vedere [Configurare manualmente un gruppo di disponibilità Always On nelle macchine virtuali di Azure tramite Resource Manager](availability-group-azure-marketplace-template-configure.md). |
| **Istanze del cluster di failover** |Le istanze del cluster di failover, che richiedono l'archiviazione condivisa, possono essere create in 4 modi diversi.<br/><br/>1. Un cluster di failover a due nodi in esecuzione su macchine virtuali di Azure con archiviazione collegata che usa [Spazi di archiviazione diretta in Windows Server 2016 \(S2D\)](failover-cluster-instance-storage-spaces-direct-manually-configure.md) per offrire una SAN virtuale basata su software.<br/><br/> 2. Un cluster di failover a due nodi in esecuzione in macchine virtuali di Azure che usa la [condivisione di file Premium](failover-cluster-instance-premium-file-share-manually-configure.md). Le condivisioni di file Premium sono condivisioni di file a bassa latenza costante basate su unità SSD completamente supportate per l'uso con l'istanza del cluster di failover.<br/><br/>3. Un cluster di failover a due nodi in esecuzione su macchine virtuali di Azure con archiviazione supportata da una soluzione di clustering di terze parti. Per un esempio specifico che usa SIOS DataKeeper, vedere l'articolo relativo alla [disponibilità elevata per una condivisione file basata su WSFC e sul software di terze parte SIOS DataKeeper](https://azure.microsoft.com/blog/high-availability-for-a-file-share-using-wsfc-ilb-and-3rd-party-software-sios-datakeeper/).<br/><br/>4. Un cluster di failover a due nodi in esecuzione su macchine virtuali di Azure con archiviazione a blocchi condivisa su una destinazione iSCSI remota tramite ExpressRoute. Ad esempio, NetApp Private Storage (NPS) espone una destinazione iSCSI tramite ExpressRoute con Equinix a macchine virtuali di Azure.<br/><br/>Per le soluzioni di replica dei dati e di archiviazione condivisa di terze parti, contattare il fornitore per eventuali problemi di accesso ai dati durante il failover.<br/><br/>|
...
Se si ha [Software Assurance](https://www.microsoft.com/licensing/licensing-programs/software-assurance-default?rtc=1&activetab=software-assurance-default-pivot:primaryr3), è possibile implementare piani di ripristino di emergenza ibridi con SQL Server senza sostenere costi di licenza aggiuntivi per l'istanza di ripristino di emergenza passiva.

Nell'immagine seguente il programma di installazione usa SQL Server in esecuzione in una macchina virtuale di Azure con 12 core come replica di ripristino di emergenza per una distribuzione di SQL Server locale con 12 core. In passato, sarebbe stato necessario concedere in licenza 12 core di SQL Server per la distribuzione locale e della macchina virtuale di Azure. Il nuovo vantaggio consente di beneficiare della replica passiva in esecuzione in una macchina virtuale di Azure. È ora necessario concedere in licenza solo 12 core di SQL Server in esecuzione in locale, a condizione che vengano soddisfatti i criteri di ripristino di emergenza per la replica passiva nella macchina virtuale di Azure.
...
Per altre informazioni, vedere le [condizioni di licenza del prodotto](https://www.microsoft.com/licensing/product-licensing/products). 

Per abilitare questo vantaggio, andare alla [risorsa macchina virtuale SQL Server](manage-sql-vm-portal.md#access-the-sql-virtual-machines-resource), selezionare **Configura** nelle impostazioni e scegliere l'opzione **Ripristino di emergenza** in **Licenza di SQL Server**. Selezionare la casella di controllo per confermare che la macchina virtuale di SQL Server verrà usata come replica passiva e quindi selezionare **Applica** per salvare le impostazioni. 

![Configurare la replica di ripristino di emergenza in Azure](./media/business-continuity-high-availability-disaster-recovery-hadr-overview/dr-replica-in-portal.png)
...
Per altre informazioni, vedere [Configurare manualmente un gruppo di disponibilità Always On nelle macchine virtuali di Azure tramite Resource Manager](availability-group-azure-marketplace-template-configure.md).
...
* [Configurare un listener ILB per i gruppi di disponibilità in Azure](availability-group-listener-powershell-configure.md).
* [Configurare un listener esterno per i gruppi di disponibilità in Azure](../../../virtual-machines/windows/sqlclassic/virtual-machines-windows-classic-ps-sql-ext-listener.md).
...
Se è necessario creare una macchina virtuale di Azure con SQL Server, vedere [Provisioning di una macchina virtuale di SQL Server in Azure](create-sql-vm-portal.md).

Per ottenere le prestazioni migliori di SQL Server in esecuzione su una VM di Azure, vedere le indicazioni fornite in [Procedure consigliate per le prestazioni per SQL Server in Macchine virtuali di Azure](performance-guidelines-best-practices.md).
