...
Le dichiarazioni XMD ([XML Document Transformation][XML Document Transformation]) consentono di trasformare il file [ApplicationHost.config][ApplicationHost.config] in Siti Web di Microsoft Azure. È inoltre possibile usare le dichiarazioni XDT per aggiungere estensioni private al sito, in modo da consentire azioni di amministrazione del sito personalizzate. In questo articolo è disponibile un'estensione del sito PHP Manager di esempio, che consente la gestione di impostazioni PHP mediante un'interfaccia Web.

<!-- MINI TOC -->

-   [Trasformazione della configurazione del sito in ApplicationHost.config][Trasformazione della configurazione del sito in ApplicationHost.config]
-   [Estensione del sito][Estensione del sito] 
    -   [Panoramica delle estensioni private del sito][Panoramica delle estensioni private del sito]
    -   [Esempio di estensioni del sito: PHP Manager][Esempio di estensioni del sito: PHP Manager]
        -   [App Web di PHP Manager][App Web di PHP Manager]
        -   [File applicationHost.xdt][File applicationHost.xdt]
    -   [Distribuire le estensioni del sito][Distribuire le estensioni del sito]

## <span id="transform"></span></a>Trasformazione della configurazione del sito in ApplicationHost.config
...
Per avvalersi di tale funzionalità per la trasformazione, è necessario creare un file ApplicationHost.xdt con contenuto XDT e posizionarlo sotto la radice del sito. Quindi, nella pagina **Configura** del portale di Microsoft Azure, configurare l'impostazione app `WEBSITE_PRIVATE_EXTENSIONS` su 1 (potrebbe essere necessario riavviare il sito).
...
Per esempi aggiuntivi, vedere [][]<https://github.com/projectkudu/kudu/wiki/Azure-Site-Extensions></a>.
...
## <span id="extend"></span></a>Estensione del sito

### <span id="overview"></span></a>Panoramica delle estensioni private del sito
...
4.  **Abilitazione** dell'estensione del sito: impostare l'impostazione app `WEBSITE_PRIVATE_EXTENSIONS` su 1
...
**Nota**: informazioni aggiuntive relative a questi elementi chiave sono disponibili all'indirizzo [][]<https://github.com/projectkudu/kudu/wiki/Azure-Site-Extensions></a>. L'esempio dettagliato incluso illustra la procedura per la creazione e l'abilitazione di un'estensione privata del sito. Il codice sorgente per l'esempio di PHP Manager seguente è disponibile per il download all'indirizzo [][1]<https://github.com/projectkudu/PHPManager></a>.

### <span id="SiteSample"></span></a>Esempio di estensioni del sito: PHP Manager
...
#### <span id="PHPwebapp"></span></a>App Web di PHP Manager
...
#### <span id="XDT"></span></a>File applicationHost.xdt
...
### <span id="deploy"></span></a>Distribuire le estensioni del sito

Per installare l'estensione del sito, è possibile usare FTP per copiare tutti i file dell'app Web nella cartella `\SiteExtensions\[your-extension-name]` del sito in cui si vuole installare l'estensione. Assicurarsi di copiare anche il file ApplicationHost.xdt nello stesso percorso.

Nel portale Siti Web di Azure passare quindi alla scheda **Configura** per il sito Web associato all'estensione. Nella sezione **impostazioni app**, aggiungere la chiave `WEBSITE_PRIVATE_EXTENSIONS` e assegnarle un valore di `1`.

![TransformSiteappSettings][TransformSiteappSettings]

Infine, nel portale di Microsoft Azure, riavviare il sito Web per abilitare l'estensione.
...
<!-- IMAGES -->
  ...
  [XML Document Transformation]: http://msdn.microsoft.com/it-it/library/dd465326.aspx
