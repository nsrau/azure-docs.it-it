---
title: Analizzare il report di Deployment Planner per il ripristino di emergenza di VMware con Azure Site Recovery
description: Questo articolo descrive come analizzare il report generato da Recovery Deployment Planner for VMware disaster recovery to Azure, usando Azure Site Recovery.This article describes how to analyze the report generated by the Recovery Deployment Planner for VMware disaster recovery to Azure, using Azure Site Recovery.
...
---
# <a name="analyze-the-deployment-planner-report-for-vmware-disaster-recovery-to-azure"></a>Analizzare il report di Deployment Planner per il ripristino di emergenza VMware in AzureAnalyze the Deployment Planner report for VMware disaster recovery to Azure
...
>[!Note]
>Deployment Planner v2.5 in poi consiglia il posizionamento dello storage per i computer che verranno replicati direttamente nei dischi gestiti.
...
**Tipo di archiviazione di replica:** un disco gestito standard o premium, usato per replicare tutte le macchine virtuali corrispondenti indicate nella colonna **Macchine virtuali da posizionare.**

**Tipo di account di archiviazione log:** tutti i log di replica vengono archiviati in un account di archiviazione standard.

**Prefisso consigliato per l'account di archiviazione:** prefisso a tre caratteri suggerito che può essere usato per denominare l'account di archiviazione della cache. È possibile usare un prefisso personalizzato, ma il suggerimento dello strumento segue la [convenzione di denominazione delle partizioni per gli account di archiviazione](https://aka.ms/storage-performance-checklist).

**Nome account registro suggerito:** il nome dell'account di archiviazione dopo aver incluso il prefisso suggerito. Sostituire il nome nelle parentesi acute (< e >) con l'input personalizzato.

**Riepilogo del posizionamento:** riepilogo dei dischi necessari per proteggere le macchine virtuali in base al tipo di archiviazione. Include il numero totale di macchine virtuali, la dimensione totale di cui è stato eseguito il provisioning su tutti i dischi e il numero totale di dischi.
...
**Asrseeddisk (Disco gestito) creato per**la replica: nome del disco creato quando si abilita la replica. Archivia i dati e i relativi snapshot in Azure.It stores the data and its snapshots in Azure.
...
* Dimensioni del disco dati errate o dimensioni del disco del sistema operativo errate. [Esaminare](vmware-physical-azure-support-matrix.md#azure-vm-requirements) i limiti di supporto. 
...
* La varianza media dei dati supera il limite di variazione dei dati di Site Recovery supportato di 20 MB/s per la dimensione media di I/O per il disco.
...
Tipo di sistema **operativo:** è il tipo di sistema operativo della macchina virtuale. Può essere Windows o Linux o un altro ancora, in base al modello scelto da VMware vSphere durante la creazione della VM.
...
**Destinazione di archiviazione della replica** | **Dimensione media dei / o del disco di origine** |**Media scissioni dei dati del disco di origine** | **Variazione totale dei dati del disco di origine al giorno**
---|---|---|---
