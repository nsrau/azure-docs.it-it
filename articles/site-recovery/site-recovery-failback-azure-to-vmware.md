---
title: Eseguire il failback di server fisici e macchine virtuali VMware nel sito locale | Documentazione Microsoft
...
5. Se è stata selezionata una rete di *distribuzione classica* , verrà richiesto di creare una nuova VM tramite la raccolta di Azure e installarvi il server di elaborazione.
   ...
   2. Installare il server di elaborazione seguendo la procedura [specificata qui](site-recovery-vmware-to-azure-classic.md#step-5-install-the-management-server)
...
> [!NOTE]
> Per installare un server di destinazione master in Linux, seguire le istruzioni riportate nella procedura successiva. Assicurarsi di usare solo il sistema operativo minimo CentOS 6.6 per il sistema operativo MT.
> 
> 
...
3. Se le macchine di origine da proteggere eseguono Linux con il file system Reiser o XFS per il dispositivo di avvio o radice, è necessario scaricare e installare pacchetti aggiuntivi come indicato di seguito:
   ...
   * # <a name="yum-install-device-mapper-multipath---this-is-required-to-enable-multipath-packages-on-the-mt-server"></a>YUM installa device-mapping-multipath   Questa operazione è necessaria per abilitare i pacchetti Multipath sul server MT.
