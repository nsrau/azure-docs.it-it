...
1. Aprire il file C:\ProgramData\ASR\home\svsystems\var\configurator_register_host_static_info.log. (La cartella ProgramData potrebbe essere una cartella nascosta. Se la cartella ProgramData non è visualizzata, in Esplora file, nella sezione **Mostra/Nascondi** della scheda **Visualizza** selezionare la casella di controllo **Elementi nascosti.** Gli errori potrebbero essere causati da più problemi.
...
4. Se non si trova la stringa **Motivo=>NULL**, aprire il file C:\ProgramData\ASRSetupLogs\UploadedLogs\ASRUnifiedAgentInstaller.log nella macchina di origine. (La cartella ProgramData potrebbe essere una cartella nascosta. Se la cartella ProgramData non è visualizzata, in Esplora file, nella sezione **Mostra/Nascondi** della scheda **Visualizza** selezionare la casella di controllo **Elementi nascosti.** Gli errori potrebbero essere causati da più problemi. 

5. Cercare la **richiesta di post stringa: (7) - Impossibile connettersi al server**. Se si trova la stringa:
...
## <a name="failure-to-activate-windows-license-from-server-standard-evaluation-to-server-standard"></a>Impossibile attivare la licenza di Windows da Server Standard EVALUATION a Server Standard

1. Come parte della distribuzione del server di configurazione tramite OVF, viene utilizzata una licenza di valutazione, valida per 180 giorni. È necessario attivare la licenza prima che scada. In caso contrario, ciò può comportare l'arresto frequente del server di configurazione e quindi causare un ostacolo alle attività di replica.
...
4. Per rimuovere i dettagli degli host registrati e le informazioni relative alla voce non aggiornata dal server di configurazione, eseguire il comando seguente usando il computer di origine e l'indirizzo IP della voce non aggiornata. 
    ...
    Se si dispone di una voce del server di origine "OnPrem-VM01" con un indirizzo IP di 10.0.0.4, utilizzare invece il comando seguente.
...
- Verificare che il flag **Gli utenti possono registrare applicazioni** sia impostato su *Sì* in AAD. Per altre informazioni, vedere [Procedura: Usare il portale per creare un'applicazione Azure AD e un'entità servizio in grado](../active-directory/develop/howto-create-service-principal-portal.md#required-permissions)di accedere alle risorse.
...
TCP \<Sostituire IP con CS IP qui \<>:52739 Sostituire IP con IP CS qui>:443 SYN_SENT 
