...
EFI/UEFI guest/server| Parziale (solo migrazione ad Azure per Windows Server 2012 e versioni successive) </br></br> ** Vedere la nota alla fine della tabella.
...
Volume con disco con striping > 1 TB guest/server<br/><br/> Gestione volumi logici (LVM) guest/server - Spazi di archiviazione | No Aggiunta/rimozione a caldo disco guest/server | No Esclusione disco guest/server | Sì Percorsi multipli (MPIO) guest/server (MPIO) | N/D
