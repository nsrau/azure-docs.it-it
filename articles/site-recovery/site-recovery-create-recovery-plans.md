---
title: Creare/personalizzare piani di ripristino in Azure Site Recovery
...
In questo articolo viene descritto come creare e personalizzare un piano di ripristino per il failover in [Azure Site Recovery.](site-recovery-overview.md) Prima di iniziare, vedere [altre informazioni](recovery-plan-overview.md) sui piani di ripristino.

## <a name="create-a-recovery-plan"></a>Creare un piano di ripristino

1. Nell'insieme di credenziali dei servizi di ripristino, selezionare Piani di **ripristino (Ripristino del sito)** > **- Piano di ripristino**.
...
3. Scegliere un'origine e una destinazione in base ai computer nel piano e selezionare **Resource Manager** per il modello di distribuzione. Nella posizione di origine devono essere presenti computer abilitati per il failover e il ripristino. 
   ...
   --- | --- | ---
   Da Azure ad Azure | Selezionare l'area di AzureSelect the Azure region | Selezionare l'area di AzureSelect the Azure region
   ...
   Hyper-V (gestito da VMM) in Azure  | Selezionare il server VMM | Selezionare Azure
    ...
    -  È possibile usare solo un piano di ripristino per il failover dal percorso di origine ad Azure.You can only use a recovery plan for failover from the source location to Azure. Non è possibile usare un piano di ripristino per il failback da Azure.You can't use a recovery plan for failback from Azure.
    ...
    - È possibile includere macchine virtuali VMware e macchine virtuali Hyper-V gestite da VMM, nello stesso piano.
...
- Se si esegue la replica in Azure è possibile integrare i runbook di automazione di Azure nel piano di ripristino. [Scopri di più](site-recovery-runbook-automation.md).
...
- Se si aggiunge un'azione manuale, quando viene eseguito il piano di ripristino, si arresta nel punto in cui è stata inserita l'azione manuale. Una finestra di dialogo richiede all'utente di specificare che l'azione manuale è stata completata.
...
1. Nel piano di ripristino fare clic sul passaggio a cui aggiungere l'azione e specificare quando deve essere eseguita l'azione:
...
4. Se si desidera aggiungere uno script, eseguire le operazioni seguenti:
    1. Se si aggiunge uno script VMM, selezionare l'opzione **Failover to VMM script** (Failover nello script VMM) e in **Percorso script** digitare il percorso relativo della condivisione. Se, ad esempio, la \\ \<condivisione si trova nel percorso di nomeServer VMM>, MSSCVMMLibrary, RPScripts, specificare il percorso:
