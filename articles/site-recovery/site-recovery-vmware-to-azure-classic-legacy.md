---
title: Eseguire la replica di macchine virtuali VMware e server fisici in Azure con Azure Site Recovery (legacy) | Microsoft Docs
...
6. Se si usa Linux:
   ...
   4. Estrarre i file dal programma di installazione compresso con gzip eseguendo: **tar –xvzf Microsoft-ASR\_UA\_8.4.0.0\_RHEL6-64*** ![Server di destinazione master Linux](./media/site-recovery-vmware-to-azure-classic-legacy/linux-tar.png)
...
> [!NOTE]
> Al termine della registrazione, possono essere necessari fino a 15 minuti perché il server di destinazione master venga elencato nel portale. Per aggiornarlo immediatamente, fare clic su **Aggiorna** nella pagina **Server di configurazione**.
> 
> 
...
5. Si noti che potrebbe essere necessario riavviare il server per completare l'installazione. In **Server di configurazione** > **Dettagli server** verificare che il server di elaborazione sia visualizzato e risulti registrato correttamente nell'insieme di credenziali.
...
2. È possibile modificare il nome assegnato al computer in Azure dopo il failover e le dimensioni della macchina virtuale di Azure. È anche possibile selezionare la rete di Azure alla quale verrà connesso il computer dopo il failover.
   ...
   > [!NOTE]
   > [Migration of networks](../resource-group-move-resources.md) all'interno dei gruppi di risorse con la stessa sottoscrizione o all'interno delle sottoscrizioni non è supportata per le reti usate per la distribuzione di Site Recovery.
   > 
   > 
