...
2. Eliminare i mapping di rete usati dai cloud nel server VMM da eliminare. In **Infrastruttura** > di Site Recovery**per System Center VMM** > **Network Mapping**fare clic con il pulsante destro del mouse sul mapping di rete > **Elimina**.
3. Prendere nota dell'ID del server VMM.
4. Annullare l'associazione dei criteri di replica dai cloud nel server VMM da rimuovere.  In **Infrastruttura** > di Site Recovery per**i criteri**di replica VMM >  di System**Center**fare doppio clic sul criterio associato. Fare clic con il pulsante destro del mouse sul cloud > **Annulla associazione**.
5. Eliminare il server VMM o il nodo attivo. In **Infrastruttura** > di ripristino del sito per i**server VMM****di System Center** > fare clic con il pulsante destro del mouse sul server > **Elimina**.
...
2. Annullare l'associazione dei criteri per il sito Hyper-V. In **Infrastruttura** > di Site Recovery per**i criteri**di replica dei siti >  **Hyper-V**fare doppio clic sul criterio associato. Fare clic con il pulsante destro del mouse sul cloud > **Annulla associazione**.
3. Eliminare gli host Hyper-V. In **Infrastruttura** > di Site Recovery**per** > host Hyper-V Sites**Hyper-V**fare clic con il pulsante destro del mouse sul server > **Elimina**.
4. Eliminare il sito Hyper-V dopo che sono stati rimossi tutti gli host dal sito. In **Infrastruttura** > di Site Recovery per siti > **Hyper-V****Siti Hyper-V**fare clic con il pulsante destro del mouse sul sito > **Elimina**.
...
> [!NOTE]
> Se è già stato eseguito il failover di una macchina virtuale ed è in esecuzione in Azure, si noti che la disabilitazione della protezione non rimuove/influisce sulla macchina virtuale di cui è stato eseguito il failover.
## <a name="disable-protection-for-a-azure-vm-azure-to-azure"></a>Disabilitare la protezione per una macchina virtuale di Azure (da Azure ad Azure)Disable protection for a Azure VM (Azure to Azure)

-  In **Elementi** > **protetti Elementi replicati**fare clic con il pulsante destro del mouse sul computer > **Disabilita replica.**
> [!NOTE]
...
2. In **Disabilita replica** è possibile selezionare le opzioni seguenti:
   - **Disabilita replica e rimuovi (scelta consigliata):** questa opzione consente di rimuovere l'elemento replicato da Azure Site Recovery e la replica per il computer viene interrotta. La configurazione della replica nella macchina virtuale locale verrà rimossa e la fatturazione di Site Recovery per questo server protetto viene interrotta.
