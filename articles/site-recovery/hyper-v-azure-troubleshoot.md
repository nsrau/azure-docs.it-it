---
title: Risolvere i problemi di ripristino di emergenza Hyper-V con Azure Site RecoveryTroubleshoot Hyper-V disaster recovery with Azure Site Recovery
...
4. Controllare eventuali problemi che vengono visualizzati nell'accesso Hyper-V-VMMS\Admin alla macchina virtuale. Questo registro si trova in **Registri applicazioni e servizi** > **di Microsoft** > **Windows**.
5. Nella macchina virtuale guest verificare che WMI sia abilitato e accessibile.
   ...
   - [Risolvere i problemi](https://technet.microsoft.com/library/ff406382.aspx#H22) relativi agli script e ai servizi WMI.
...
2. Verificare se la replica è stata sospesa:
   ...
   - Se è fondamentale, fare clic con il pulsante destro del mouse sulla macchina virtuale > l'integrità della**replica della visualizzazione di** **replica** > .
...
2. Fare clic su **Visualizza stato di replica** per visualizzare i dettagli:
    ...
    - Se la replica è sospesa, fare clic con il pulsante destro del mouse sulla replica VM > **resume** > **replication**.
...
7. Controllare se nella macchina virtuale viene riscontrata una varianza elevata:
    ...
    - È anche possibile [verificare gli obiettivi di scalabilità delle risorse di archiviazione](https://docs.microsoft.com/azure/storage/common/storage-scalability-targets).
...
1. Controllare i log eventi per individuare eventuali errori e raccomandazioni per il servizio Copia Shadow del volume:
    - Nel server host Hyper-V aprire il registro eventi di amministrazione di Hyper-V in Applicazioni e registri dei servizi**del** > **Windows** >  > **Visualizzatore** >  **eventi**microsoft Windows**Hyper-V** > **Admin**.
...
2. Per generare gli snapshot VSS per la macchina virtuale, verificare che nella macchina virtuale sia installato Integration Services di Hyper-V e che il servizio di integrazione Backup (VSS) sia abilitato.
    ...
    - È possibile controllare questo da una sessione di PowerShell con privilegi elevati nell'host Hyper-V con il comando **Get-VMIntegrationService -VMName\<VMName nomeVM>-Nome VSS** È anche possibile ottenere queste informazioni accedendo alla macchina virtuale guest. [Scopri di più](https://docs.microsoft.com/windows-server/virtualization/hyper-v/manage/manage-hyper-v-integration-services).
...
Tutti gli eventi di replica di Hyper-V vengono registrati nel registro di Hyper-V-VMMS-Admin, che si trova in **Registri applicazioni e servizi** > di**Microsoft** > **Windows**. È anche possibile abilitare un registro analitico per il servizio Virtual Machine Management di Hyper-V, come indicato di seguito:

1. Rendere i log analitici e di debug visibili nel Visualizzatore eventi. Per rendere disponibili i registri, nel Visualizzatore eventi fare clic su **Visualizza** > **mostra registri analitici e di debug. .**. Il registro analitico viene visualizzato in **Hyper-V-VMMS**.
