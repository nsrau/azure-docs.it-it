...
> [!NOTE]
> In PowerShell per Azure Site Recovery il supporto per lo scenario "*Ripristino di emergenza per le macchine virtuali di Azure*" è attualmente disponibile in anteprima. Non tutte le funzionalità dello scenario disponibili tramite il portale sono disponibili tramite Azure PowerShell. Di seguito sono riportate alcune delle funzionalità dello scenario che attualmente non sono supportate da Azure PowerShell:
> - La possibilità di replicare le macchine virtuali di Azure che usano dischi gestiti.
...
####<a name="1-create-a-site-recovery-fabric-object-to-represent-the-primarysource-region"></a>1. Creare un oggetto di infrastruttura di Site Recovery per rappresentare l'area primaria (di origine)

L'oggetto di infrastruttura nell'insieme di credenziali rappresenta un'area di Azure. L'oggetto di infrastruttura primaria è l'oggetto di infrastruttura creato per rappresentare l'area di Azure a cui appartengono le macchine virtuali protette per l'insieme di credenziali. Nell'esempio descritto in questo articolo, la macchina virtuale da proteggere si trova nell'area Stati Uniti orientali.
...
####<a name="2-create-a-site-recovery-fabric-object-to-represent-the-recovery-region"></a>2. Creare un oggetto di infrastruttura di Site Recovery per rappresentare l'area di ripristino
...
####<a name="3-create-a-site-recovery-protection-container-in-the-primary-fabric"></a>3. Creare un contenitore di protezione di Site Recovery nell'infrastruttura primaria
...
```
####<a name="4-create-a-site-recovery-protection-container-in-the-recovery-fabric"></a>4. Creare un contenitore di protezione di Site Recovery nell'infrastruttura di ripristino
...
####<a name="5-create-a-replication-policy"></a>5. Creare un criterio di replica
...
```
####<a name="6-create-a-protection-container-mapping-between-the-primary-and-recovery-protection-container"></a>6. Creare un mapping del contenitore di protezione tra i contenitori di protezione principale e di ripristino
...
####<a name="7-create-a-protection-container-mapping-for-failback-reverse-replication-after-a-failover"></a>7. Creare un mapping del contenitore di protezione per il failback (replica inversa dopo un failover)
...
Per le macchine virtuali che non usano i dischi gestiti, l'account di archiviazione di destinazione è l'account di archiviazione nell'area di ripristino in cui vengono replicati i dischi della macchina virtuale. L'account di archiviazione di destinazione può essere un account di archiviazione standard o un account di archiviazione Premium. Selezionare il tipo di account di archiviazione necessario in base alla frequenza di modifica dei dati (frequenza di scrittura I/O) per i dischi e in base ai limiti supportati da Azure Site Recovery per il tipo di archiviazione.
...
Replicare la macchina virtuale di Azure.
