...
- Site Recovery supporta crittografia dischi di Azure versione 0,1, che dispone di uno schema che richiede Azure Active Directory (Azure AD). Site Recovery supporta anche la versione 1,1, che non richiede Azure AD. [Altre informazioni sull'estensione schemi per crittografia dischi di Azure](../virtual-machines/extensions/azure-disk-enc-windows.md#extension-schemata).
...
Passare a **Insieme di credenziali di Site Recovery** > **Infrastruttura di Site Recovery** > **Criteri di replica**. Selezionare il criterio che si desidera modificare e salvare le modifiche. Qualsiasi modifica verrà applicata anche a tutte le repliche esistenti.
...
L'opzione **elaborata più recente** esegue il failover di tutte le macchine virtuali nel piano al punto di ripristino più recente che Site Recovery elaborato. Per vedere il punto di ripristino più recente per una macchina virtuale specifica, selezionare **Punti di ripristino più recenti** nelle impostazioni della macchina virtuale. Questa opzione offre un RTO basso poiché non viene impiegato tempo per la gestione di dati non elaborati.
