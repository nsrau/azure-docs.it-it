...
7. Selezionare l'account di archiviazione di Azure da usare per la replica dei dati. Si noti che:
   ...
   * Se si vuole usare un account di archiviazione diverso rispetto a quelli disponibili, è possibile creare un *collegamento segnaposto per la creazione di un account di archiviazione con Resource Manager*, come illustrato nella guida introduttiva. Per creare un account di archiviazione con Resource Manager, fare clic su **Crea nuovo**. Per creare un account di archiviazione con il modello di distribuzione classica, usare il [portale di Azure](../storage/common/storage-create-storage-account.md).
...
13. Abilitare la **Coerenza tra più VM** per raccogliere le macchine in un gruppo di replica e specificare un nome per il gruppo. Fare quindi clic su **OK**. Si noti che:
    ...
    * I computer in un gruppo di replica vengono replicati insieme e hanno punti di ripristino condivisi coerenti con l'arresto anomalo del sistema e coerenti con l'app in caso di failover.
...
3. In **Calcolo e rete** > **Proprietà di calcolo** è possibile specificare le dimensioni di destinazione e il nome della VM di Azure. Se necessario, modificare il nome in modo che sia conforme ai requisiti di Azure.
    ![Abilitare la replica](./media/site-recovery-vmware-to-azure/vmproperties.png)

4.  È possibile selezionare un [gruppo di risorse](https://docs.microsoft.com/azure/virtual-machines/windows/infrastructure-resource-groups-guidelines) di cui il computer diventerà parte dopo il failover. È possibile modificare questa impostazione in qualsiasi momento prima del failover. Dopo il failover, se si esegue la migrazione del computer a un gruppo di risorse diverso, le impostazioni di protezione di un computer non potranno essere applicate.
...
Vedere altre informazioni sul [Vantaggio Azure Hybrid Use](https://aka.ms/azure-hybrid-use-benefit-pricing).
