...
Se si amplia la distribuzione a oltre 200 computer di origine oppure la varianza totale giornaliera è superiore a 2 TB, sono necessari server di elaborazione aggiuntivi per gestire il volume di traffico. Sono state migliorate del prodotto nella versione 9.24 fornire [linee guida del prodotto](vmware-azure-manage-process-server.md#process-server-selection-guidance) sui casi in cui configurare un server di elaborazione scale-out. [Configurare il server di elaborazione](vmware-azure-set-up-process-server-scale.md) per proteggere le nuove macchine di origine oppure [bilanciare il carico](vmware-azure-manage-process-server.md#balance-the-load-on-process-server).
