...
-   Selezionare le dimensioni della partizione, ad esempio digitare '+10G' per creare una partizione da 10 gigabyte. In alternativa, premere `<enter>` per creare un'unica partizione che occupa l'intero disco:
...
2.  Quando si modifica /etc/fstab è consigliabile utilizzare l'**UUID** anziché il nome del dispositivo per fare riferimento al file system. Servirsi dell'utility `blkid` per determinare l'UUID del nuovo file system:
