...
Quando si esegue la sottoscrizione per gli eventi di un gruppo di risorse, l'endpoint riceve tutti gli eventi da tale gruppo. Gli eventi possono includere l'evento che si desidera visualizzare, ad esempio l'aggiornamento di una macchina virtuale, ma anche eventi probabilmente non importanti, come ad esempio la scrittura di una nuova voce nella cronologia di distribuzione. È possibile ricevere tutti gli eventi in corrispondenza dell'endpoint e scrivere il codice che elabora gli eventi che si desidera gestire, oppure impostare un filtro quando si crea la sottoscrizione dell'evento.
