...
* Il valore di ogni coppia chiave-valore nella sezione argomenti ha esattamente lo stesso schema JSON usato per `Topic.Properties` sull'API durante la creazione di argomenti.
* Il valore di ogni coppia chiave-valore nella sezione **EventSubscriptions** ha esattamente lo stesso schema JSON usato per `EventSubscription.Properties` sull'API durante la creazione di argomenti.
