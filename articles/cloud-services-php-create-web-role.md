...
Questa guida descrive come creare ruoli Web o di lavoro PHP in un ambiente di sviluppo Windows, scegliere una versione specifica di PHP tra le versioni incorporate disponibili, modificare la configurazione di PHP, abilitare le estensioni e, infine, come eseguire la distribuzione in Azure. Descrive inoltre come configurare un ruolo Web o di lavoro per l'uso del runtime PHP (con configurazione ed estensioni personalizzate) fornito dall'utente.
...
Per aggiungere un ruolo Web PHP a un progetto, avviare il seguente comando dalla directory radice del progetto:

	PS C:\myProject> Add-AzurePHPWebRole roleName

Per un ruolo di lavoro, usare il seguente comando:
...
È possibile impostare la versione del runtime PHP su una qualsiasi delle versioni PHP elencate. Ad esempio, per impostare la versione PHP (per un ruolo denominato  `roleName`) su 5.4.0, usare il seguente comando:

	PS C:\myProject> Set-AzureServiceProjectRole roleName php 5.4.0

> [AZURE.NOTE] È possibile che in futuro siano disponibili ulteriori versioni PHP e che le versioni già disponibili subiscano delle modifiche.
...
3. Aggiungere un file  `php.ini` alla cartella  `php`. Abilitare eventuali estensioni personalizzate e impostare qualsiasi direttiva PHP in questo file. Se, ad esempio, si desidera attivare  `display_errors` e abilitare l'estensione  `php_mongo.dll`, il contenuto del file  `php.ini` sarà il seguente:
...
1. Creare un progetto di servizio Azure e aggiungere un ruolo Web PHP come descritto nelle precedenti sezioni [Procedura: Creare un progetto di servizi cloud](#CreateProject) e [Procedura: Aggiungere ruoli Web o di lavoro PHP](#AddRole) .
2. Creare una cartella  `php` nella cartella  `bin` che si trova nella directory radice del proprio ruolo Web, quindi aggiungere il proprio runtime PHP tutti i binari, i file di configurazione, le sottocartelle e così via) alla cartella  `php`.
...
6. Pubblicare l'applicazione come descritto nella seguente sezione [Procedura: Pubblicare l'applicazione](#Publish) .
...
<h3><a name="OwnPHPWorkerRole"></a>Configurare un ruolo di lavoro per l'uso del proprio runtime PHP</h3>

Per configurare un ruolo di lavoro per l'uso di un runtime PHP fornito dall'utente, seguire questa procedura.

1. Creare un progetto di servizio Azure e aggiungere un ruolo di lavoro PHP come descritto nelle precedenti sezioni [Procedura: Creare un progetto di servizi cloud](#CreateProject) e [Procedura: Aggiungere ruoli Web o di lavoro PHP](#AddRole) .
2. Creare una cartella  `php` nella directory radice del proprio ruolo Web, quindi aggiungere il proprio runtime PHP (tutti i binari, i file di configurazione, le sottocartelle e così via) alla cartella  `php`.
...
Per eseguire il progetto negli emulatori, avviare il seguente comando dalla directory radice del progetto:
...
Per arrestare gli emulatori, avviare il seguente comando:
...
[execution model-web sites]: /it-it/develop/net/fundamentals/compute/#WebSites
...
[configurazione del servizio (file con estensione cscfg)]: http://msdn.microsoft.com/library/windowsazure/ee758710.aspx
