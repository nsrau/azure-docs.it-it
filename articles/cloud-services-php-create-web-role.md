...
-   [Procedura: Utilizzare il proprio runtime PHP](#OwnPHP)
...
In Azure sono disponibili tre modelli di calcolo per l'esecuzione di applicazioni: [Siti Web di Azure](/en-us/develop/net/fundamentals/compute/#WebSites), [Macchine virtuali di Azure](/en-us/develop/net/fundamentals/compute/#VMachine) e [Servizi cloud di Azure](/en-us/develop/net/fundamentals/compute/#CloudServices). Tutti e tre i modelli supportano PHP. Servizi cloud, che include ruoli Web e di lavoro, fornisce la tecnologia *PaaS (Platform as a Service)*. Nell'ambito di un servizio cloud, un ruolo Web fornisce un server Web IIS (Internet Information Services) dedicato su cui ospitare applicazioni Web front-end, mentre un ruolo di lavoro consente di eseguire attività asincrone, a esecuzione prolungata o perpetue, indipendenti dall'interazione o dall'input degli utenti.

Per ulteriori informazioni, vedere la pagina relativa alla [definizione di servizio cloud](/en-us/manage/services/cloud-services/what-is-a-cloud-service/).

Download di Azure SDK per PHP
-----------------------------

[Azure SDK per PHP](/en-us/develop/php/common-tasks/download-php-sdk/) è composto da diversi componenti. In quest'articolo ne verranno presi in considerazione solo due: Azure PowerShell e gli emulatori di Azure. È possibile installare questi due componenti tramite Installazione guidata piattaforma Web Microsoft all'indirizzo: [Installazione di Azure PowerShell e degli emulatori di Azure](http://go.microsoft.com/fwlink/?LinkId=253447&clcid=0x409).

Procedura: Creare un progetto di servizi cloud
----------------------------------------------

Il primo passaggio nella creazione di un ruolo Web o di lavoro PHP consiste nel creare un progetto di servizio Azure, che funge da contenitore logico per i ruoli Web e di lavoro e contiene la [definizione del servizio (.csdef)](http://msdn.microsoft.com/en-us/library/windowsazure/ee758711.aspx) del progetto e i file di [configurazione del servizio(.cscfg)](http://msdn.microsoft.com/en-us/library/windowsazure/ee758710.aspx).

Per creare un nuovo sito del progetto Azure, eseguire il comando seguente:

    PS C:PS C:\>New-AzureServiceProject myProject
    gt;New-AzureServiceProject myProject

Questo comando consente di creare una nuova directory (`myProject`) a cui è possibile aggiungere ruoli Web e di lavoro.

Procedura: Aggiungere ruoli Web o di lavoro PHP
-----------------------------------------------
...
Il parametro `roleName` è facoltativo: se omesso, il nome del ruolo verrà generato automaticamente. Il primo ruolo Web creato sarà `WebRole1`, il secondo sarà `WebRole2` e così via. Il primo ruolo di lavoro creato sarà `WorkerRole1`, il secondo sarà `WorkerRole2` e così via.

Procedura: Specificare la versione PHP incorporata
--------------------------------------------------
...
Il risultato del comando sopra riportato sarà simile al seguente. In questo esempio, il flag `IsDefault` è impostato su `true` per PHP 5.3.17, a indicare che questa sarà la versione di PHP installata per impostazione predefinita.
    ...
    Versione runtime     PackageUri                      IsDefault
    ------- -------     ----------                      ---------

Node 0.6.17 http://nodertncu.blob.core... False Node 0.6.20 http://nodertncu.blob.core... True Node 0.8.4 http://nodertncu.blob.core... False
...
È possibile impostare la versione del runtime PHP su una qualsiasi delle versioni PHP elencate. Ad esempio, per impostare la versione PHP (per un ruolo denominato `roleName`) su 5.4.0 utilizzare il comando seguente:

    PS C:\myProject> Set-AzureServiceProjectRole roleName php 5.4.0

**Nota**

È possibile che in futuro siano disponibili ulteriori versioni PHP e che le versioni già disponibili subiscano delle modifiche.

Procedura: Personalizzare il runtime PHP incorporato
----------------------------------------------------
...
2.  Nella cartella `php` creare un'altra cartella denominata `ext`. Inserire tutti i file di estensione `dll` (ad esempio `php_mongo.dll`) che si desidera abilitare in questa cartella.
...
Qualsiasi impostazione non esplicitamente impostata nel file `php.ini` fornito verrà automaticamente impostata sui rispettivi valori predefiniti. Si tenga tuttavia presente che è possibile aggiungere un file `php.ini` completo.

Procedura: Utilizzare il proprio runtime PHP
--------------------------------------------

In alcuni casi, invece di selezionare un runtime PHP incorporato e configurarlo come sopra descritto, può essere consigliabile fornire un proprio runtime PHP. Ad esempio, è possibile utilizzare lo stesso runtime PHP in un ruolo Web o di lavoro utilizzato nell'ambiente di sviluppo, in modo che sia più semplice assicurarsi che il comportamento dell'applicazione non cambi nell'ambiente di produzione.

### Configurazione di un ruolo Web per l'utilizzo del proprio runtime PHP

Per configurare un ruolo Web per l'utilizzo di un runtime PHP fornito dall'utente, eseguire la procedura seguente.

1.  Creare un progetto di servizio Azure e aggiungere un ruolo Web PHP come descritto nelle sezioni [Procedura: Creazione di un progetto di servizi cloud](#CreateProject) e [Procedura: Aggiunta di ruoli Web e di lavoro PHP](#AddRole) sopra riportate.
...
3.  (FACOLTATIVO) Se il proprio runtime PHP utilizza i [Driver Microsoft per PHP per SQL Server](http://php.net/sqlsrv) sarà necessario configurare il proprio ruolo Web per installare [SQL Server Native Client 2012](http://msdn.microsoft.com/en-us/sqlserver/aa937733.aspx) quando viene eseguito il provisioning. A tale scopo, aggiungere il programma di installazione `sqlncli.msi` alla cartella `bin` nella directory radice del proprio ruolo Web. È possibile scaricare il programma di installazione all'indirizzo: [sqlncli.msi x64 installer](http://go.microsoft.com/fwlink/?LinkID=239648). Lo script di avvio descritto nel passaggio successivo eseguirà il programma di installazione senza avvisi quando viene eseguito il provisioning del ruolo. Se il proprio runtime PHP non utilizza i Driver Microsoft per PHP per SQL Server è possibile rimuovere la seguente riga dallo script illustrato nel passaggio successivo:

         msiexec /i sqlncli.msi /qn IACCEPTSQLNCLILICENSETERMS=YES

4.  Il passaggio successivo consiste nel definire un'attività di avvio per configurare [Internet Information Services (IIS)](http://www.iis.net/) per l'utilizzo del runtime PHP nella gestione delle richieste di pagine `.php`. A tale scopo, aprire il file `setup_web.cmd` (nel file `bin` della directory radice del proprio ruolo Web) in un editor di testo e sostituirne il contenuto con lo script seguente:
...
6.  Pubblicare l'applicazione come descritto nella sezione [Procedura: Pubblicazione dell'applicazione](#Publish) di seguito riportata.

**Nota**

Lo script `download.ps1` (nella cartella `bin` della directory radice del proprio ruolo Web) può essere eliminato dopo aver eseguito i passaggi sopra descritti per l'utilizzo del proprio runtime PHP.

### Configurazione di un ruolo di lavoro per l'utilizzo del proprio runtime PHP

Per configurare un ruolo di lavoro per l'utilizzo di un runtime PHP fornito dall'utente, eseguire la procedura seguente.

1.  Creare un progetto di servizio Azure e aggiungere un ruolo di lavoro PHP come descritto nelle sezioni [Procedura: Creazione di un progetto di servizi cloud](#CreateProject) e [Procedura: Aggiunta di ruoli Web e di lavoro PHP](#AddRole) sopra riportate.
...
3.  (FACOLTATIVO) Se il proprio runtime PHP utilizza i [Driver Microsoft per PHP per SQL Server](http://php.net/sqlsrv) sarà necessario configurare il proprio ruolo di lavoro per installare [SQL Server Native Client 2012](http://msdn.microsoft.com/en-us/sqlserver/aa937733.aspx) quando viene eseguito il provisioning. A tale scopo, aggiungere il programma di installazione `sqlncli.msi` alla directory radice del proprio ruolo di lavoro. È possibile scaricare il programma di installazione all'indirizzo: [sqlncli.msi x64 installer](http://go.microsoft.com/fwlink/?LinkID=239648). Lo script di avvio descritto nel passaggio successivo eseguirà il programma di installazione senza avvisi quando viene eseguito il provisioning del ruolo. Se il proprio runtime PHP non utilizza i Driver Microsoft per PHP per SQL Server è possibile rimuovere la seguente riga dallo script illustrato nel passaggio successivo:

         msiexec /i sqlncli.msi /qn IACCEPTSQLNCLILICENSETERMS=YES

4.  Il passaggio successivo consiste nel definire un'attività di avvio che consente di aggiungere l'eseguibile `php.exe` alla variabile di ambiente PATH del ruolo di lavoro durante il provisioning del ruolo. A tale scopo, aprire il file `setup_web.cmd` (nella directory radice del proprio ruolo di lavoro) in un editor di testo e sostituirne il contenuto con lo script seguente:
...
Per pubblicare l'applicazione è necessario prima importare le impostazioni di pubblicazione con il cmdlet **Import-PublishSettingsFile**, quindi sarà possibile pubblicare l'applicazione con il cmdlet **Publish-AzureServiceProject**. Informazioni dettagliate sull'utilizzo di ciascuno dei cmdlet sono disponibili rispettivamente nelle sezioni [Procedura: Importazione delle impostazioni di pubblicazione](/en-us/develop/php/how-to-guides/powershell-cmdlets/#ImportPubSettings) e [Procedura: Distribuzione di un servizio cloud in Azure](/en-us/develop/php/how-to-guides/powershell-cmdlets/#Deploy).
