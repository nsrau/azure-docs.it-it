...
---
# <a name="profiling-live-azure-web-apps-with-application-insights-preview"></a>Profilatura delle app Web di Azure attive con Application Insights (anteprima)

*Questa funzione di Application Insights è disponibile in anteprima.*
...
In [https://portal.azure.com](https://portal.azure.com) aprire la risorsa di Application Insights per l'App Web. Aprire **Prestazioni** e fare clic su **Configura**. Selezionare l'app e seguire la procedura guidata.

![Nel pannello Prestazioni fare clic su Configura.][performance-blade]

* *Non è presente un pulsante Configura? Usare la [procedura manuale](#manual-installation).*
...
Il profiler del servizio Microsoft usa una combinazione della strumentazione e del metodo di campionamento per analizzare le prestazioni dell'applicazione. Quando la raccolta dettagliata è in corso, il profiler del servizio campiona il puntatore all'istruzione di ciascuna delle CPU del computer ogni millisecondo. Ogni esempio acquisisce lo stack di chiamate completo del thread attualmente in esecuzione, fornendo informazioni utili e dettagliate sulle operazioni che il thread stava eseguendo a livelli di astrazione alti e bassi. Il profiler del servizio raccoglie anche altri eventi, ad esempio gli eventi di commutazione di contesto, TPL e del pool di thread per tenere traccia della causalità e della correlazione delle attività. 
...
Quando si configura il profiler, gli aggiornamenti seguenti vengono eseguiti nelle impostazioni dell'App Web. È possibile eseguire l'operazione manualmente:
...
L'applicazione ASP.NET Core è attualmente supportata in .NET Core Runtime.

L'applicazione deve inoltre includere i componenti seguenti per abilitare la profilatura.

1. [Application Insights per ASP.NET Core 2.0](https://github.com/Microsoft/ApplicationInsights-aspnetcore/releases/tag/v2.0.0)
2. [System.Diagnostics.DiagnosticSource 4.4.0-beta-25022-02](https://dotnet.myget.org/feed/dotnet-core/package/nuget/System.Diagnostics.DiagnosticSource/4.4.0-beta-25022-02)
    * In Visual Studio selezionare il menu "Strumenti -> Gestione pacchetti NuGet -> 	Impostazioni di Gestione pacchetti".
    * Nella finestra di dialogo Opzioni selezionare "Gestione pacchetti NuGet -> Origini pacchetti".
    * Fare clic sul pulsante "+" per aggiungere una nuova origine pacchetto con nome "Core-DotNet-MyGet" e il valore "https://dotnet.myget.org/F/dotnet-core/api/v3/index.json".
    * Fare clic sul pulsante "Aggiorna" e chiudere la finestra di dialogo Opzioni.
    * In Esplora soluzioni fare clic con il pulsante destro del mouse sul progetto ASP.NET Core e scegliere Gestisci pacchetti NuGet.
    * Fare clic sulla scheda "Sfoglia", selezionare "Origine pacchetto: Core-DotNet-MyGet" e selezionare "Includi versione preliminare".
    * Cercare "System.Diagnostics.DiagnosticSource" e scegliere "__4.4.0-beta-25022-02__" per l'installazione.
