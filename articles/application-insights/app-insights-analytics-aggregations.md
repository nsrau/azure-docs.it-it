...
![risultato](./media/app-insights-analytics-aggregations/03.png)
...
![risultato](./media/app-insights-analytics-aggregations/04.png)
...
#### Richieste di conteggio

Trovare i numeri di richieste di ogni nome nell'ora precedente:

    requests | where timestamp > ago(1h)
    | summarize req_count = sum(itemCount) by name

> [AZURE.NOTE] Anziché contare semplicemente i punti dati della richiesta, utilizzare la proprietà itemCount. Questo consente di compensare gli eventuali [campionamenti](app-insights-sampling.md) che potrebbe essere in esecuzione. Ad esempio, se il campionamento è impostato su 33%, itemCount sarà 3 in ogni punto dati.

#### Min e Max

Individuare il timestamp minimo e massimo di tutti i record nel flusso di richieste. Poiché non è presente alcuna clausola group-by, l'output include una sola riga:
...
Vedere anche l'[operatore `mvexpand`](app-insights-analytics-queries.md#mvexpand-operator) per la funzione opposta.
