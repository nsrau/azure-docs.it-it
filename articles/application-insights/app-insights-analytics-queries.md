...
* *plain\_query:* query non preceduta da una clausola let.
...
* `bagexpansion=array`: i contenitori delle proprietà vengono espansi in strutture di matrici a due elementi `[`*key*`,`*value*`]`, che consentono l'accesso uniforme alle chiavi e ai valori oltre che, ad esempio, l'esecuzione di un'aggregazione di conteggi di valori univoci sui nomi delle proprietà. 
...
Gli argomenti devono essere valori numerici, date o intervalli di tempo. Non possono fare riferimento alle colonne di nessuna tabella. Per calcolare l'intervallo basato su una tabella di input, usare la [*funzione* range](app-insights-analytics-scalars.md#range), ad esempio con l'[operatore mvexpand](#mvexpand-operator).
...
[Altri esempi](app-insights-analytics-aggregations.md).
...
* *Aggregation:* chiamata a una [funzione di aggregazione](app-insights-analytics-aggregations.md), ad esempio `count()` o `avg()` con nomi di colonna come argomenti. Vedere l'[elenco delle funzioni di aggregazione](app-insights-analytics-aggregations.md).
...
* Potrebbe essere visualizzato `asc` o `desc` (valore predefinito) per controllare se la selezione nell'intervallo è effettivamente in ordine crescente o decrescente.
...
* *Predicate:* [espressione](app-insights-analytics-scalars.md#boolean) `boolean` sulle colonne di *T*. Viene valutato per ogni riga in *T*.
