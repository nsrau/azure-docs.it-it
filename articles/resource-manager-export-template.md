...
La costruzione di un modello di Azure Resource Manager può risultare difficile da comprendere. Fortunatamente, Resource Manager consente di esportare un modello da risorse esistenti nella sottoscrizione. Il modello generato può essere usato per ottenere informazioni sulla sintassi del modello o per automatizzare la ridistribuzione della soluzione in base alle esigenze.
...
4. Resource Manager recupera i cinque file seguenti:
   ...
   - Modello che definisce l'infrastruttura per la soluzione. Quando è stato creato l'account di archiviazione tramite il portale, Resource Manager ha usato un modello per distribuirlo e ha salvato tale modello come riferimento futuro.

   - File dei parametri che può essere usato per passare i valori durante la distribuzione. Contiene i valori specificati durante la prima distribuzione, ma è possibile modificare qualsiasi valore durante la ridistribuzione del modello.

   - File di script di Azure PowerShell che può essere usato per distribuire il modello.

   - File di script dell'interfaccia della riga di comando di Azure che può essere usato per distribuire il modello.

   - Classe .NET che può essere usato per distribuire il modello.
     ...
     Si noti che il modello definisce i parametri per il nome dell'account di archiviazione, il tipo e la posizione. È presente anche un parametro che indica se la crittografia è abilitata. Il valore predefinito è **false**. Nella sezione **resources** viene visualizzata la definizione per l'account di archiviazione da distribuire.

Le parentesi quadre contengono un'espressione che viene valutata durante la distribuzione. Le espressioni tra parentesi nel modello vengono usate per ottenere i valori dei parametri durante la distribuzione. È possibile usare molte altre espressioni. Esempi di altre espressioni sono disponibili più avanti in questo articolo. Per un elenco completo, vedere [Funzioni del modello di Azure Resource Manager](resource-group-template-functions.md).
...
1. Nel pannello Gruppo di risorse selezionare **Aggiungi** e quindi scegliere la **rete virtuale** tra le risorse disponibili.
...
## Esportare il modello per il gruppo di risorse
...
2. Verranno visualizzati di nuovo i cinque file che è possibile usare per ridistribuire la soluzione, ma questa volta il modello è leggermente diverso. Questo modello ha solo due parametri, uno per il nome dell'account di archiviazione e uno per il nome della rete virtuale.
     ...
     Resource Manager non ha recuperato i modelli usati durante la distribuzione. Ha generato invece un nuovo modello basato sulla configurazione corrente delle risorse. Resource Manager non sa quali valori si vogliono passare come parametri, quindi la maggior parte dei valori è hardcoded in base ai valori nel gruppo di risorse. Ad esempio, la posizione dell'account di archiviazione e il valore di replica sono impostati su:
...
- Nella seconda parte di questa esercitazione verrà personalizzato il modello appena scaricato con l'aggiunta di altri parametri e il modello verrà ridistribuito tramite uno script. Vedere [Personalizzare il modello di Resource Manager esportato](resource-manager-customize-template.md).
