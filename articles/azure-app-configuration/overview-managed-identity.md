...
1. Creare un'identità assegnata dall'utente denominata `myUserAssignedIdentity` usando l'interfaccia della riga di comando.
    ...
    Nell'output di questo comando prendere nota del valore della proprietà `id`.

1. Eseguire il comando [AZ appconfig Identity Assign] per assegnare la nuova identità assegnata dall'utente a questo archivio di configurazione. Usare il valore della proprietà `id` annotata nel passaggio precedente.
