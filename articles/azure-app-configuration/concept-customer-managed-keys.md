...
2. Concedere le autorizzazioni di `GET`, `WRAP`e `UNWRAP` di identità nei criteri di accesso dell'Key Vault di destinazione.
...
- [Creare un'app .NET Framework con la configurazione app Azure](quickstart-dotnet-app.md)
- [Creare un'app Java Spring con app Azure configurazione](quickstart-java-spring-app.md)
...
1. Creare una Azure Key Vault usando l'interfaccia della riga di comando di Azure.  Si noti che sia `vault-name` che `resource-group-name` sono forniti dall'utente e devono essere univoci.  In questi esempi vengono usati `contoso-vault` e `contoso-resource-group`.
...
1. Creare una chiave di Key Vault. Specificare un `key-name` univoco per questa chiave e sostituire i nomi del Key Vault (`contoso-vault`) creati nel passaggio 1. Specificare se si preferisce la crittografia `RSA` o `RSA-HSM`.
...
1. Creare un'identità gestita assegnata dal sistema usando l'interfaccia della riga di comando di Azure, sostituendo il nome dell'istanza di configurazione dell'app e del gruppo di risorse usato nei passaggi precedenti. L'identità gestita verrà usata per accedere alla chiave gestita. Si usa `contoso-app-config` per illustrare il nome di un'istanza di configurazione dell'app:
    
    ```azurecli
    az appconfig identity assign --na1. me contoso-app-config --group contoso-resource-group --identities [system]
    ```
    
    L'output di questo comando include l'ID principale ("principalId") e l'ID tenant ("tenandId") dell'identità assegnata dal sistema.  Questa operazione verrà usata per concedere all'identità l'accesso alla chiave gestita.
