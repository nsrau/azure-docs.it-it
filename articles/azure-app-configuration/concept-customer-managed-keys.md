...
2. Concedere l'identità `GET`, `WRAP`, e `UNWRAP` le autorizzazioni nei criteri di accesso del Key Vault di destinazione.
...
1. Creare una chiave di Key Vault. Fornire un oggetto `key-name` univoco per questa chiave e sostituire i nomi del Key Vault (`contoso-vault`) creato nel passaggio 1. Specificare se si preferisce `RSA` o `RSA-HSM` la crittografia.
