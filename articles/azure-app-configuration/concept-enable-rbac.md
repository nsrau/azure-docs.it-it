...
1. L'identità dell'entità di sicurezza viene autenticata e viene restituito un token OAuth 2,0.  Il nome della risorsa per richiedere un token `https://login.microsoftonline.com/{tenantID}` è `{tenantID}` dove corrisponde al Azure Active Directory ID tenant a cui appartiene l'entità servizio.
