...
In questa esercitazione viene illustrato come creare un sito Web di Azure PHP-MySQL e distribuirlo tramite Git. Verranno utilizzati [PHP][1], lo strumento da riga di comando MySQL (parte di [MySQL][install-mysql]), un server Web e [Git][2] installato nel computer. Le istruzioni di questa esercitazione possono essere eseguite in qualsiasi sistema operativo, tra cui Windows, Mac e Linux. Dopo aver completato questa guida, si disporrà di un sito Web PHP/MySQL in esecuzione in Azure.
...
In questa esercitazione si presuppone che siano presenti [PHP][1], lo strumento da riga di comando MySQL (parte di [MySQL][install-mysql]), un server Web e [Git][2] installato nel computer.

> [WACOM.NOTE] 
> Se l'esercitazione viene eseguita in Windows, è possibile configurare il computer per PHP e configurare automaticamente IIS (il server Web incorporato in Windows) installando [Azure SDK per PHP][3].

## <a  id="create-web-site-and-set-up-git" ></a>Creazione di un sito Web di Azure e configurazione della pubblicazione Git
...
3.  Fare clic su **Web Site** e quindi su **Custom Create**.
    ...
    Immettere un valore per **URL**, scegliere **Create a New MySQL Database** nell'elenco a discesa **Database**, quindi selezionare il data center per il sito Web nell'elenco a discesa **Region**.
    Fare clic sulla freccia nella parte inferiore della finestra di dialogo.
...
1.  (Facoltativo) Se l'URL dell'archivio remoto Git è stato dimenticato o smarrito, passare alla scheda Development nel portale.
    ...
    ![Recupero dell'URL di Git](./media/web-sites-php-mysql-deploy-use-git/git-instructions.png)
...
3.  Passare a **http://[nome sito].azurewebsites.net/index.php** per iniziare a utilizzare l'applicazione. Queste informazioni verranno archiviate nel dashboard dell'account:
...
Per pubblicare le modifiche apportate all'applicazione, attenersi alla procedura seguente:
