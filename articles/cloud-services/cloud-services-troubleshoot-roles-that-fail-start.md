<properties 
   pageTitle="Risoluzione dei problemi dei ruoli che non vengono avviati | Microsoft Azure"
...
# Passaggi comuni per la risoluzione dei problemi dei ruoli del servizio cloud che non vengono avviati

Ecco alcuni problemi e soluzioni comini correlati ai ruoli del servizio cloud di Azure che non vengono avviati.
...
**Sintomo:** i sintomi di file DLL o assembly mancati includono i seguenti.

- L'istanza del ruolo passa in ciclo tra gli stati **Inizializzazione** / **Occupato** / **Arresto**.
- L'istanza del ruolo è passata allo stato **Pronto** ma se si passa all'applicazione Web la pagina non viene visualizzata.

Risoluzione: sono disponibili tre metodi consigliati per esaminare questi problemi.

## Diagnosi dei problemi di DLL mancanti in un ruolo Web
...
Per visualizzare errori più completi, configurare il file web.config per il ruolo Web in modo da disattivare la modalità di errore personalizzata e ridistribuire il servizio.
...
È possibile usare Desktop remoto per accedere al ruolo e visualizzare errori più completi in remoto. Seguire questa procedura per visualizzare gli errori usando Desktop remoto:
...
Per ottenere risultati ottimali con questo metodo di diagnosi, è consigliabile usare un computer o una macchina virtuale in cui è presente un'installazione pulita di Windows. Per simulare in modo ottimale l'ambiente Azure, è consigliabile usare Windows Server 2008 R2 x64.
...
4. Copiare la cartella con estensione csx e il file con estensione cscfg nel computer che si usa per eseguire il debug dei problemi.
...
## Risoluzione di DLL e assembly mancanti
...
Dopo avere verificato che tutti gli errori sono stati corretti, sarà possibile distribuire il servizio senza selezionare l'impostazione **Abilita IntelliTrace per ruoli .NET 4**.
