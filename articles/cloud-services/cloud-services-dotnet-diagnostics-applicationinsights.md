...
* Usare [Cerca](../application-insights/app-insights-diagnostic-search.md) per eseguire ricerche nei vari log di traccia inviati da Diagnostica di Azure. Se è presente un'eccezione non gestita in un ruolo che ha causato l'arresto anomalo e il riciclo del ruolo, ad esempio, tali informazioni verranno visualizzate nel canale *Applicazione* del *registro eventi di Windows*. È possibile usare la funzionalità di ricerca per esaminare l'errore del registro eventi di Windows e l'analisi dello stack completa per l'eccezione. In questo modo sarà possibile trovare la causa radice del problema.
