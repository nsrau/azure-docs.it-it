...
A supporto del nostro impegno a usare la crittografia migliore, Microsoft ha annunciato piani per avviare la migrazione da TLS 1.0 e 1.1 a giugno 2017.   Dopo tale annuncio iniziale, Microsoft ha dichiarato l'intenzione di disabilitare Transport Layer Security (TLS) 1.0 e 1.1 per impostazione predefinita nelle versioni supportate di Microsoft Edge e Internet Explorer 11 nella prima metà del 2020.  Annunci simili da Apple, Google e Mozilla indicano la direzione in cui il settore è diretto.   

Per altre informazioni, vedere [Preparazione per TLS 1.2 in Microsoft Azure](https://azure.microsoft.com/updates/azuretls12/)

## <a name="tls-configuration"></a>Configurazione TLS  
L'immagine server cloud di Windows Server 2019 è configurata con TLS 1.0 e TLS 1.1 disabilitato a livello di registro di sistema. Ciò significa che le applicazioni distribuite a questa versione di Windows E che usano lo stack di Windows per la negoziazione TLS non consentiranno la comunicazione TLS 1.0 e TLS 1.1.   
...
Usare il codice seguente come esempio per creare uno script che abiliti i protocolli e i pacchetti di crittografia precedenti. Ai fini di questa documentazione, questo script verrà denominato: **TLSsettings.ps1**. Archiviare questo script sul desktop locale per facilitarne l'accesso nei passaggi successivi. 
...
## <a name="step-2-create-a-command-file"></a>Passaggio 2: Creare un file dei comandi 
...
## <a name="step-3-add-the-startup-task-to-the-roles-service-definition-csdef"></a>Passaggio 3: Aggiungere l'attività di avvio alla definizione di servizio del ruolo (csdef) 
...
3) Selezionare **Elemento esistente**
4) In Esplora file passare al desktop in cui sono stati archiviati il file **TLSsettings.ps1** e **RunTLSSettings.cmd** 
...
Per assicurarsi che gli script vengano caricati con tutti gli aggiornamenti inseriti da Visual Studio, l'impostazione *Copia in directory di output* deve essere impostata su *Copia sempre*

1) In WebRole o WorkerRole fare clic con il pulsante destro del mouse su RunTLSSettings.cmd
2) Selezionare **Proprietà**
3) Nella scheda Proprietà modificare il valore di *Copia in directory di output* su *Copia sempre*
4) Ripetere i passaggi per **TLSsettings.ps1**

## <a name="step-6-publish--validate"></a>Passaggio 6: Pubblicare e convalidare
