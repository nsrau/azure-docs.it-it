...
1. Immettere un **Nome di accesso** e una **Password** per l'amministratore. 
	...
	Se è stata selezionata l'opzione **Nuovo server di database SQL**, in questo passaggio non sarà necessario immettere un nome e una password esistenti, bensì un nuovo nome e una nuova password, definiti ora e da usare in seguito per l'accesso al database. Se è stato selezionato un server creato in precedenza, sarà richiesta la password dell'account utente di amministrazione già creato.

1. Fare clic su **Crea database SQL**.

	![New SQL Database](./media/cloud-services-dotnet-get-started/newdb.png)	

1. Dopo che Azure ha completato la creazione del database, fare clic sulla scheda **Database SQL** nel riquadro sinistro del portale, quindi fare clic sul nome del nuovo database.
...
1. Nel progetto ContosoAdsWeb aprire il file di trasformazione  *Web.Release.config* per il file  *Web.config* dell'applicazione, eliminare il blocco di commento che include un elemento '<connectionStrings>' e sostituirlo incollando il seguente codice. 
...
6. Selezionare e copiare la stringa di connessione, senza le virgolette tra cui è racchiusa, per usarla nei seguenti passaggi per la configurazione del progetto di ruolo di lavoro.
...
Ad esempio, il file ServiceDefinition.csdef include le seguenti definizioni:
...
Il file  *ServiceConfiguration.Cloud.cscfg* invece include i valori immessi per tali impostazioni in Visual Studio:
...
3. Nel passaggio **Impostazioni** della procedura guidata fare clic su **Avanti**.
	...
	Le impostazioni predefinite della scheda **Avanzate** sono corrette per questa esercitazione. Per informazioni sulla scheda Avanzate, vedere [Procedura guidata Pubblica l'applicazione Azure](http://msdn.microsoft.com/library/hh535756.aspx).
...
Per creare l'applicazione Contoso Ads sono necessari i seguenti passaggi:
...
2. Nel riquadro sinistro della finestra di dialogo **Nuovo progetto** espandere **Visual C#**, scegliere i modelli **Cloud**, quindi selezionare il modello **Servizio cloud Microsoft Azure**.
...
11. Nella finestra di dialogo **Aggiungi nuovo progetto** scegliere **Desktop di Windows** in **Visual C#** nel riquadro sinistro, quindi fare clic sul modello **Libreria di classi**.  
...
13. Cercare il pacchetto  *Azure Storage* e, se è incluso nell'elenco, fare clic su **Aggiorna** per ottenere la versione più recente della libreria del client di archiviazione.
...
16. Trovare il pacchetto NuGet  *EntityFramework* e installarlo nei tre progetti.
...
4. Aggiungere una nuova impostazione denominata  *StorageConnectionString*. Impostare **Tipo** su  *ConnectionString* e **Valore** su  *UseDevelopmentStorage=true*.
...
5. Nella finestra delle proprietà del **[Ruolo]* ContosoAdsWorker* aggiungere un'altra stringa di connessione:
...
In questa sezione i file di codice saranno copiati dalla soluzione scaricata alla nuova soluzione. Le seguenti sezioni illustrano e spiegano parti chiave del codice.
...
Il codice chiamato dal metodo  `Application_Start` crea un  *images* contenitore BLOB e una  *images* coda, se non esistono già. In questo modo, ogni volta che si inizia a usare un nuovo account di archiviazione o a usare l'emulatore di archiviazione in un nuovo computer, il contenitore BLOB e la coda necessari saranno creati automaticamente.
...
Il file  *Views\Home\Index.cshtml* visualizza i collegamenti di categoria nella home page. I collegamenti passano il valore Integer dell'enumerazione  `Category` in una variabile stringaquery alla pagina di indice delle inserzioni.
...
Il file  *Details.cshtml* mostra l'immagine con dimensioni originali:
