...
Questo articolo offre una panoramica dell'uso dei ruoli Web e di lavoro con [Python Tools per Visual Studio][]. Si apprenderà come usare Visual Studio per creare e distribuire un servizio cloud di base che usa Python.
...
I modelli di Visual Studio dovrebbero aver creato un file **ps.cmd** nella cartella **./bin**. Questo script della shell chiama gli script wrapper di PowerShell precedenti e consente la registrazione in base al nome del wrapper di PowerShell chiamato. Se questo file non è stato creato, di seguito è indicato il contenuto necessario.
