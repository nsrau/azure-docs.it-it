...
Per decidere se utilizzare o meno Archiviazione tabelle di Azure, è necessario esaminare i costi e le prestazioni. Archiviazione tabelle è una soluzione molto più economica rispetto all'archiviazione degli stessi dati in un database SQL. Tuttavia, è necessario considerare attentamente in quale misura i dati utilizzano le funzionalità relazionali di SQL quali join, filtro, query e così via. Se l’utilizzo di tali funzionalità da parte dei dati è ridotto, questi costituiscono un buon candidato per l'archiviazione in una tabella di Azure.
...
Per visualizzare un esempio di analisi di progettazione che esamina il database AdventureWorks per eseguirne la federazione e mostra le considerazioni in ogni fase della progettazione, vedere la prima parte [del post di blog relativo all’approccio orientato alla scalabilità per la progettazione di database con le federazioni, dedicata alla scelta delle federazioni e della chiave della federazione](http://go.microsoft.com/fwlink/?LinkId=252671).
...
Dopo avere stabilito la modalità di federazione delle tabelle, è necessario valutare la posizione delle tabelle di riferimento, nonché degli oggetti di database. Nella [seconda parte del blog, dedicata all’annotazione e alla distribuzione degli schemi per le federazioni](http://go.microsoft.com/fwlink/?LinkId=252672), viene fornita una discussione approfondita di questo argomento. L'esecuzione di query più avanzate è descritta nella [Parte 2](http://go.microsoft.com/fwlink/?LinkId=252673).
...
Prevedere una capacità di memorizzazione nella cache sufficiente in modo da memorizzare gli oggetti utilizzati con maggiore frequenza. In Database SQL sono presenti tabelle di riferimento che vengono usate di frequente per la conversione di codici numerici in stringhe di caratteri descrittive più lunghe. Queste tabelle spesso includono dati quali nomi di paesi e città, i valori di codice postale validi, nomi dei reparti aziendali e così via. Per le tabelle più piccole può essere utile memorizzare l'intera tabella nella cache, mentre per altri utenti è possibile archiviare solo i valori utilizzati con maggiore frequenza. L'aumento di prestazioni deriva dall'esecuzione di query multijoin che coinvolgono tali dati: per ogni valore trovato nella cache vengono risparmiati diversi accessi al disco. Per informazioni introduttive e discussioni relative a prestazioni e servizio di caching in Azure, vedere l'articolo [Introduzione del servizio di memorizzazione nella cache di Microsoft Azure](http://go.microsoft.com/fwlink/?LinkId=252680). Per un post di blog più recente, vedere l'argomento riguardante le [considerazioni relative alle prestazioni del servizio Cache di #Azure](http://go.microsoft.com/fwlink/?LinkId=252681).
...
Azure dispone di due tecnologie di coda alternative: le code di archiviazione di Azure e il bus di servizio.
...
* [Servizio HDInsight di Azure](/documentation/services/hdinsight/)
