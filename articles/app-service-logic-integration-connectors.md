...
> [AZURE.NOTE] Se il contenuto XML di input ha un semplice nodo con un attributo (come "<authorid= "1">abc</author>"), l'output JSON della libreria è { "author": { "@id" : "1", "#text": "abc"}}. Per gestire l'attributo "Id" viene aggiunta una nuova chiave "#text" per il contenuto testuale del nodo. Per gestire questo tipo di nodo, aggiungere una chiave costante. Questo comportamento è previsto da progettazione nella libreria Newtonsoft.Json. Quando viene inserita in SQL, usare "JSONOutput.Author.#text"; non usare "JsonOutput.Author".
...
A tale scopo, usare queste app per API senza scrivere alcun codice. Di seguito è riportata la procedura. 
...
Gruppo di risorse | Creare un nuovo gruppo di risorse o usare un gruppo esistente. Tutte le app per le API e i connettori per app per la logica, app Web e app mobili devono trovarsi nello stesso gruppo di risorse. <br/><br/>Questa proprietà è illustrata nell'articolo [Uso dei gruppi di risorse](azure-preview-portal-using-resource-groups.md). 
Sottoscrizione | Proprietà di sola lettura che elenca la sottoscrizione corrente.
Località | La località geografica che ospita il servizio di Azure. 
...
Attesa |  Immettere una durata o un orario specifico per l'esecuzione delle app Web, app mobili o app per la logica.
...
[Creare un'app per la logica EAI con VETR](app-service-logic-create-EAI-logic-app-using-VETR.md)<br/>
[Trasformare i documenti XML](app-service-logic-transform-xml-documents.md)<br/>
[Usare BizTalk Rules](app-service-logic-use-biztalk-rules.md)<br/>
