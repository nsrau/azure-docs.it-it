...
Dopo aver effettuato la connessione al cluster del servizio contenitore di Azure, è\/possibile accedere al controller di dominio e alle API REST correlate tramite http: /localhost:local-port. Gli esempi riportati in questo documento presuppongono il tunneling sulla porta 80. Ad esempio, gli endpoint Marathon possono essere raggiunti\/con URI che iniziano con http: /localhost/marathon/v2/. 
...
> [!NOTE]
> L'URI è\/http: /localhost/marathon/v2/apps/ seguito dall'ID dell'applicazione da ridimensionare. Se si utilizza l'esempio Nginx fornito qui, l'URI sarà http:\//localhost/marathon/v2/apps/nginx.
