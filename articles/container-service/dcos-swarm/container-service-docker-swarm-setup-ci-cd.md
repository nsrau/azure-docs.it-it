...
1. Per ogni immagine, configurare un passaggio che usi il comando `docker build`.
    ...
    Per l'operazione di compilazione, selezionare il registro contenitori di Azure, l'azione **Compila un'immagine** e il Dockerfile che definisce ogni immagine. Impostare **Build context** (Contesto compilazione) come directory radice del file Docker e definire **Nome immagine**. 
...
Servizi di Azure DevOps consente di [gestire le versioni in vari ambienti](https://azure.microsoft.com/services/devops/pipelines/). È possibile abilitare la distribuzione continua per assicurarsi che l'applicazione venga distribuita in ambienti diversi (ad esempio sviluppo, test, preproduzione e produzione) in modo uniforme. È possibile creare un nuovo ambiente che rappresenti il cluster Docker Swarm del servizio Azure Container.
...
1. Per configurare l'origine artefatto, fare clic su **artefatti**per  >  **collegare un'origine artefatto**. In questo caso, è possibile collegare questa nuova pipeline di versione alla compilazione definita nel passaggio precedente. In questo modo, il file docker-compose.yml è disponibile nel processo di rilascio.
