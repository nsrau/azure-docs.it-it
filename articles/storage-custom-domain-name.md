...
| Account di archiviazione | **URL predefinito:** http://accountarchiviazionepersonale.blob.core.windows.net        
                             **URL di dominio personalizzato:** http://www.contoso.com                              |
| BLOB                     | **URL predefinito:** http://mystorageaccount.blob.core.windows.net/mycontainer/myblob  
                            **URL del dominio personalizzato:** http://www.contoso.com/mycontainer/myblob           |
| Contenitore radice       | **URL predefinito:** http://accountarchiviazionepersonale.blob.core.windows.net/myblob 
                            oppure                                                                                  
                             http://accountarchiviazionepersonale.blob.core.windows.net/$root/myblob                
                             **URL di dominio personalizzato:** http://www.contoso.com/myblob                       
                            oppure                                                                                  
...
-   [Registrazione di un dominio personalizzato per l'account di archiviazione usando il sottodominio intermedio asverify][Registrazione di un dominio personalizzato per l'account di archiviazione usando il sottodominio intermedio asverify]
...
Se invece il dominio personalizzato supporta un'applicazione per cui non sono consentiti tempi di inattività, eseguire la procedura descritta in [Registrazione di un dominio personalizzato per l'account di archiviazione usando il sottodominio intermedio asverify][Registrazione di un dominio personalizzato per l'account di archiviazione usando il sottodominio intermedio asverify].
...
5.  Accedere al sito Web del registrar DNS e passare alla pagina di
    gestione dei DNS. Queste informazioni possono essere disponibili in una sezione come **Domain
    Name**, **DNS** o **Name Server Management**.

6.  Individuare la sezione per la gestione dei record CNAME. Può essere necessario passare in una
    pagina di impostazioni avanzate e cercare il termine **CNAME**, **Alias**,
    o **Subdomains**.

7.  Creare un nuovo record CNAME e specificare un alias di sottodominio, ad esempio **www** o **photos**. Specificare
    quindi un nome host, che corrisponde all'endpoint del servizio BLOB, nel formato **mystorageaccount.blob.core.windows.net**, dove **mystorageaccount** è il nome dell'account di archiviazione. Il formato del nome host viene definito automaticamente nella finestra di dialogo **Manage Custom Domain**.
...
## <a name="register-asverify"></a>Registrazione di un dominio personalizzato per l'account di archiviazione usando il sottodominio intermedio asverify
...
4.  Nella parte inferiore dello schermo fare clic su **Manage Domain** per visualizzare la finestra di dialogo **Manage Custom Domain**. Nella parte superiore della finestra di dialogo verranno visualizzate informazioni su come creare il record CNAME usando il sottodominio **asverify**.
...
7.  Creare un nuovo record CNAME e fornire un alias di sottodominio che include il sottodominio asverify. Ad esempio, il formato del sottodominio specificato sarà **asverify.www** or **asverify.photos**. Specificare
    quindi un nome host, che corrisponde all'endpoint del servizio BLOB, nel formato **asverify.accountarchiviazionepersonale.blob.core.windows.net**, dove **accountarchiviazionepersonale** è il nome dell'account di archiviazione. Il formato del nome host viene definito automaticamente nella finestra di dialogo **Manage Custom Domain**.
...
-   [Come eseguire il mapping del contenuto della rete CDN a un dominio personalizzato][Come eseguire il mapping del contenuto della rete CDN a un dominio personalizzato]
  ...
  [Registrazione di un dominio personalizzato per l'account di archiviazione usando il sottodominio intermedio asverify]: #register-asverify
