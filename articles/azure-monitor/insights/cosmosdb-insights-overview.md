...
Nell'esempio riportato di seguito si noterà che `eastus-billingint` riceve prevalentemente le richieste di lettura, ma con un numero ridotto di richieste di Upsert e di creazione. Mentre `westeurope-billingint` è di sola lettura dal punto di vista della richiesta, almeno nelle ultime quattro ore in cui la cartella di lavoro è attualmente a cui è stato definito l'ambito tramite il parametro intervallo di tempo.
