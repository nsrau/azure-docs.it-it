...
- L'abilitazione della funzionalità di integrità viene eseguita in tutte le macchine virtuali connesse all'area di lavoro di Log Analytics, anche quando l'operazione viene avviata e completata da una singola macchina virtuale.
- Se dopo la disabilitazione del monitoraggio per una macchina virtuale tramite i metodi supportati si prova nuovamente a eseguire l'onboarding, è consigliabile farlo nella stessa area di lavoro.  Se viene usata una nuova area di lavoro, quando si visualizza lo stato di integrità per tale macchina virtuale, potrebbe verificarsi un comportamento anomalo.
...
- I riepiloghi degli avvisi inclusi in Monitoraggio di Azure per la macchina virtuale riguardano solo gli avvisi generati per problemi di integrità rilevati con macchine virtuali di Azure monitorate.
...
- Le soglie per i criteri di integrità seguenti per una macchina virtuale Windows non possono essere modificate, perché i relativi stati di integrità sono già impostati su **in esecuzione** o **disponibile**. Quando viene eseguita una query sullo stato di integrità dall'[API di monitoraggio del carico di lavoro](https://github.com/Azure/azure-rest-api-specs/tree/master/specification/workloadmonitor/resource-manager), per lo stato di integrità viene visualizzato il valore di *OperatoreDiConfronto* **MinoreDi** o **MaggioreDi** con un valore di *soglia* pari a **4** per il servizio o l'entità se:
