---
title: Eseguire l'onboarding di Monitoraggio di Azure per le macchine virtuali (anteprima) | Microsoft Docs
...
Questo articolo descrive come configurare Monitoraggio di Azure per le macchine virtuali per monitorare l'integrità del sistema operativo delle macchine virtuali e dei set di scalabilità di macchine virtuali di Azure e delle macchine virtuali dell'ambiente, inclusi individuazione e mapping delle dipendenze delle applicazioni ospitate in tali macchine.  
...
Prima di iniziare, verificare di aver compreso quanto descritto nelle sottosezioni seguenti.
...
* Installare le soluzioni **ServiceMap** e **InfrastructureInsights**. Questa operazione può essere eseguita solo usando un modello di Azure Resource Manager fornito in questo articolo.   
* Configurare l'area di lavoro di Log Analytics per raccogliere i contatori delle prestazioni.

Per configurare l'area di lavoro per questo scenario su larga scala, vedere [Configurare l'area di lavoro di Log Analytics per la distribuzione su larga scala](#setup-log-analytics-workspace).
...
La mappa di Monitoraggio di Azure per le macchine virtuali ottiene i dati da Microsoft Dependency Agent. Dependency Agent usa l'agente di Log Analytics per la connessione a Log Analytics e quindi in un sistema deve essere installato e configurato l'agente di Log Analytics con Dependency Agent. Quando si abilita Monitoraggio di Azure per le macchine virtuali per una singola macchina virtuale di Azure o quando si usano i metodi per la distribuzione su larga scala, l'estensione Dependency Agent della macchina virtuale di Azure consente di installare l'agente come parte di tale esperienza di onboarding. Con un ambiente ibrido, Dependency Agent può essere scaricato e installato manualmente o usando un metodo di distribuzione automatizzata in tali macchine virtuali ospitate all'esterno di Azure.  
...
L'accesso seguente deve essere concesso agli utenti per poter abilitare e accedere alle funzionalità di Monitoraggio di Azure per le macchine virtuali.  
  
- Per abilitare la soluzione, è necessario essere aggiunti come membri del ruolo Collaboratore di Log Analytics.  

- Per visualizzare i dati sulle prestazioni, l'integrità e la mappa, è necessario essere aggiunti come membri del ruolo con autorizzazioni di lettura dei dati di monitoraggio per la macchina virtuale di Azure e l'area di lavoro di Log Analytics configurata con Monitoraggio di Azure per le macchine virtuali.   

Per altre informazioni su come controllare l'accesso a un'area di lavoro di Log Analytics, vedere [Gestire le aree di lavoro](../../log-analytics/log-analytics-manage-access.md).
...
5. Nella pagina **Insights (anteprima)** selezionare **Prova adesso**.
...
In questa sezione vengono fornite le istruzioni per eseguire la distribuzione su larga scala di Monitoraggio di Azure per le macchine virtuali tramite Criteri di Azure o Azure PowerShell.  

Vengono riepilogati i passaggi da eseguire per preconfigurare l'area di lavoro di Log Analytics prima di poter procedere con l'onboarding delle macchine virtuali.

1. Creare una nuova area di lavoro se non ne esiste già una che può essere usata per supportare Monitoraggio di Azure per le macchine virtuali. Prima di creare una nuova area di lavoro, vedere [Gestire le aree di lavoro](../../log-analytics/log-analytics-manage-access.md?toc=/azure/azure-monitor/toc.json) per conoscere le considerazioni sui costi, la gestione e la conformità.       
2. Abilitare i contatori delle prestazioni nell'area di lavoro per la raccolta in macchine virtuali Windows e Linux.
3. Installare e abilitare le soluzioni **ServiceMap** e **InfrastructureInsights** nell'area di lavoro.  
...
Se l'area di lavoro di Log Analytics a cui fa riferimento la soluzione non è configurata per la raccolta dei contatori delle prestazioni richiesti dalla soluzione, sarà necessario abilitare i contatori. Questa operazione può essere eseguita manualmente come descritto [qui](../../azure-monitor/platform/data-sources-performance-counters.md) oppure scaricando ed eseguendo uno script di PowerShell disponibile in [Azure Powershell Gallery](https://www.powershellgallery.com/packages/Enable-VMInsightsPerfCounters/1.1).
...
Esaminare i requisiti e i metodi di distribuzione per l'[agente di Log Analytics per Linux e Windows](../../azure-monitor/platform/log-analytics-agent.md).  
...
2. Scaricare e installare l'agente Dependency di Monitoraggio di Azure per le macchine virtuali per [Windows](https://aka.ms/dependencyagentwindows) o [Linux](https://aka.ms/dependencyagentlinux).
