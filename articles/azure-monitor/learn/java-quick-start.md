...
1. Avviare **Eclipse** > Fare clic su **Help** (?) > Selezionare **Install New Software** (Installa nuovo software).
   ...
   ![Modulo per una nuova risorsa di Application Insights](./media/java-quick-start/000-j.png)
...
4. Selezionare **Enable telemetry with Application Insights** (Abilita la telemetria con Application Insights) > Selezionare la risorsa di Application Insights e il valore associato per **Instrumentation Key** (Chiave di strumentazione) da collegare all'app Java.
   ...
   ![Menu di configurazione di Eclipse per Azure](./media/java-quick-start/0007-j.png)

5. Dopo aver configurato il plug-in Application Insights, è necessario [pubblicare/ripubblicare](https://docs.microsoft.com/azure/app-service/app-service-web-get-started-java#deploy-the-app) di nuovo l'applicazione prima di iniziare a inviare dati di telemetria.
