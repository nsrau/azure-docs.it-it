---
title: Risolvere i problemi relativi a Azure Application Insights ProfilerTroubleshoot problems with Azure Application Insights Profiler
...
* La profilatura per ASP.NET applicazioni Core 3.x non è ancora supportata.
  * Se è necessario disporre di Profiler, una soluzione alternativa consiste [nell'utilizzare Application Insights Profiler per ASP.NET Core](https://github.com/microsoft/ApplicationInsights-Profiler-AspNetCore). Il profiler è etichettato per Linux, ma funziona anche con le applicazioni .NET Core 3.0 e versioni successive in Windows. Per informazioni dettagliate, vedere [Versioni supportate](https://github.com/microsoft/ApplicationInsights-Profiler-AspNetCore#supported-versions).
...
* Profiler non è supportato nei piani di servizio app gratuiti o condivisi. Se si utilizza uno di questi piani, provare a scalare fino a uno dei piani di base e Profiler dovrebbe iniziare a funzionare.
...
* L'app Web deve avere le seguenti impostazioni:
...
Se non riesci a capire perché Profiler non funziona per te, puoi scaricare il serviceprofilerhelp@microsoft.comregistro e inviarlo al nostro team per assistenza, . 
...
1. Crea queste impostazioni dell'app:
...
1. Accedere alla macchina virtuale (VM) e quindi aprire il file di log in questa posizione. (L'unità potrebbe essere c: o d: e la versione del plugin potrebbe essere diversa.)
...
1. Usando il percorso disponibile nel file *config.json* precedente, controllare il file di log di Profiler, che visualizza le informazioni di debug che indicano le impostazioni usate da Profiler, nonché i messaggi di errore e di stato di Profiler.  
    ...
    Se Profiler è in esecuzione mentre l'applicazione riceve richieste, viene visualizzato il seguente messaggio: *Attività rilevata da iKey*. 

    Quando viene caricata la traccia, viene visualizzato il seguente messaggio: *Inizia a caricare*la traccia . 


## <a name="edit-network-proxy-or-firewall-rules"></a>Modificare le regole proxy o firewall di rete

Se l'applicazione si connette a Internet tramite un proxy o un firewall, potrebbe essere necessario modificare le regole per consentire all'applicazione di comunicare con il servizio Application Insights Profiler. Gli indirizzi IP usati da Application Insights Profiler sono inclusi nel tag del servizio Monitoraggio di Azure.The IPs used by Application Insights Profiler are included in the Azure Monitor service tag.
