...
---
# <a name="data-collection-retention-and-storage-in-application-insights"></a>Data collection, retention, and storage in Application Insights
...
* È possibile scrivere codice che invia dati di telemetria personalizzati aggiuntivi per agevolare la diagnostica e il monitoraggio dell'utilizzo. Questa estensibilità è un'ottima funzionalità di Application Insights. Sarebbe possibile, per errore, scrivere questo codice in modo che includa dati personali e altri dati sensibili. Se l'applicazione funziona con tali dati, è necessario applicare un processo di revisione approfondita a tutto il codice scritto.
...
* I dati vengono archiviati nei server di [Microsoft Azure](https://azure.com) negli Stati Uniti o in Europa. (Ma l'app può essere eseguita ovunque.) Azure ha processi di [sicurezza elevati e soddisfa un'ampia gamma di standard di conformità.](https://azure.microsoft.com/support/trust-center/) Solo lo sviluppatore dell’app e il team designato hanno accesso ai dati. Il personale Microsoft può avere accesso limitato ai dati solo in determinate circostanze con il consenso dello sviluppatore. È criptato in transito e a riposo.
*   Esaminare i dati raccolti, in quanto ciò può includere dati consentiti in alcune circostanze ma non in altri.  Un buon esempio è Device Name. Il nome del dispositivo da un server non ha alcun impatto sulla privacy ed è utile, ma un nome di dispositivo da un telefono o laptop può avere un impatto sulla privacy ed essere meno utile. Un SDK sviluppato principalmente per i server di destinazione, raccoglie il nome del dispositivo per impostazione predefinita e potrebbe essere necessario sovrascrivere sia gli eventi normali che le eccezioni.
...
* L'SDK, che viene integrato nell'app [in fase di sviluppo](../../azure-monitor/app/asp-net.md) o [in esecuzione](../../azure-monitor/app/monitor-performance-live-website-now.md). Sono disponibili diversi SDK per diversi tipi di applicazione. C'è anche un SDK per le [pagine web](../../azure-monitor/app/javascript.md), che viene caricato nel browser dell'utente finale insieme alla pagina.
...
* [Eccezioni](../../azure-monitor/app/asp-net-exceptions.md) e arresti anomali `build id`- dump dello **stack**, , tipo di CPU. 
...
I punti dati non elaborati, ovvero gli elementi su cui è possibile eseguire query in Analytics e controllare nella ricerca, vengono conservati per un massimo di 730 giorni. È possibile [selezionare una durata](https://docs.microsoft.com/azure/azure-monitor/app/pricing#change-the-data-retention-period) di conservazione di 30, 60, 90, 120, 180, 270, 365, 550 o 730 giorni. Se è necessario mantenere i dati più lunghi di 730 giorni, è possibile usare [l'esportazione continua](../../azure-monitor/app/export-telemetry.md) per copiarli in un account di archiviazione durante l'inserimento dei dati. 

I dati conservati per più di 90 giorni comporterà costi aggiuntivi. Per altre informazioni sui prezzi di Application Insights, vedere la [pagina dei prezzi di Monitoraggio di Azure](https://azure.microsoft.com/pricing/details/monitor/).Learn more about Application Insights pricing on the Azure Monitor pricing page .
...
Gli snapshot di debug vengono [archiviati](../../azure-monitor/app/snapshot-debugger.md) per 15 giorni. I criteri di conservazione sono impostati per ogni singola applicazione. Se è necessario aumentare questo valore, è possibile richiedere un aumento aprendo un caso di supporto nel portale di Azure.
...
* È possibile selezionare la località quando si crea una nuova risorsa di Application Insights. Ulteriori informazioni sulla disponibilità di Application Insights per area [sono disponibili qui](https://azure.microsoft.com/global-infrastructure/services/?products=all).

#### <a name="does-that-mean-my-app-has-to-be-hosted-in-the-usa-europe-or-southeast-asia"></a>Significa che la mia app deve essere ospitata negli Stati Uniti, in Europa o nel sud-est asiatico?
...
I dati vengono archiviati nei server di Microsoft Azure. Per gli account nel portale di Azure, le restrizioni relative agli account sono descritte nel [documento Sicurezza, Privacy e conformità](https://go.microsoft.com/fwlink/?linkid=392408)di Azure.
...
Tutti i dati vengono crittografati inattivi e si spostano tra i data center.
...
Sì, utilizziamo https per inviare dati al portale da quasi tutti gli SDK, inclusi server Web, dispositivi e pagine Web HTTPS. 
...
I canali di telemetria che utilizzano l'archiviazione locale creano file temporanei nelle directory TEMP o APPDATA, che sono limitati all'account specifico che esegue l'applicazione. Ciò può verificarsi quando un endpoint è stato temporaneamente non disponibile o si raggiunge il limite della limitazione delle richieste. Una volta risolto il problema, il canale di telemetria riprenderà l'invio di tutti i dati nuovi e persistenti.

Questi dati persistenti non vengono crittografati localmente. Se si tratta di un problema, esaminare i dati e limitare la raccolta di dati privati. Per ulteriori informazioni, vedere [Come esportare ed eliminare dati privati.](https://docs.microsoft.com/azure/application-insights/app-insights-customer-data#how-to-export-and-delete-private-data)

Se un cliente deve configurare questa directory con requisiti di sicurezza specifici, può essere configurata per framework. Assicurarsi che il processo che esegue l'applicazione abbia accesso in scrittura a questa directory, ma assicurarsi anche che la directory sia protetta per evitare la lettura da parte di utenti malintenzionati dei dati di telemetria.
...
Per ulteriori informazioni, vedere [Configurazione personalizzata AspNetCore](https://github.com/Microsoft/ApplicationInsights-aspnetcore/wiki/Custom-Configuration).
...
[L'archiviazione delle](https://developer.mozilla.org/en-US/docs/Web/API/Window/sessionStorage) sessioni HTML5 viene utilizzata per rendere persistenti i dati. Vengono utilizzati due `AI_buffer` buffer `AI_sent_buffer`separati: e . I dati di telemetria in batch `AI_buffer`e in attesa di essere inviati vengono archiviati in . La telemetria appena inviata `AI_sent_buffer` viene inserita fino a quando il server di inserimento non risponde che è stato ricevuto correttamente. Quando la telemetria viene ricevuta correttamente, viene rimossa da tutti i buffer. In caso di errori temporanei (ad esempio, un utente `AI_buffer` perde la connettività di rete), la telemetria rimane in fino a quando non viene ricevuto correttamente o il server di inserimento risponde che la telemetria non è valida (schema non valido o troppo vecchio, ad esempio).

I buffer di telemetria [`enableSessionStorageBuffer`](https://github.com/microsoft/ApplicationInsights-JS/blob/17ef50442f73fd02a758fbd74134933d92607ecf/legacy/JavaScript/JavaScriptSDK.Interfaces/IConfig.ts#L31) `false`possono essere disabilitati impostando su . Quando l'archiviazione della sessione è disattivata, viene invece utilizzato un array locale come archivio permanente. Poiché l'SDK JavaScript viene eseguito su un dispositivo client, l'utente ha accesso a questa posizione di archiviazione tramite gli strumenti di sviluppo del browser.

### <a name="opencensus-python"></a>OpenCensus Python

Per impostazione predefinita OpenCensus Python `%username%/.opencensus/.azure/`SDK utilizza la cartella utente corrente. Le autorizzazioni per accedere a questa cartella sono limitate all'utente corrente e agli amministratori. (Vedere [implementazione](https://github.com/census-instrumentation/opencensus-python/blob/master/contrib/opencensus-ext-azure/opencensus/ext/azure/common/storage.py) qui.) La cartella con i dati persistenti verrà denominata in base al file Python che ha generato i dati di telemetria.

È possibile modificare il percorso del file `storage_path` di archiviazione passando il parametro nel costruttore dell'esportatore in uso.
...
Il [PCI Security Standards Council](https://www.pcisecuritystandards.org/) ha fissato una scadenza del [30 giugno 2018](https://www.pcisecuritystandards.org/pdfs/PCI_SSC_Migrating_from_SSL_and_Early_TLS_Resource_Guide.pdf) per disabilitare le versioni precedenti di TLS/SSL e l'aggiornamento a protocolli più sicuri. Al termine del supporto legacy di Azur, non sarà possibile inviare i dati ad Application Insights se l'applicazione e/o i client non potranno comunicare tramite il protocollo TLS 1.2. L'approccio adottato per testare e convalidare il supporto TLS dell'applicazione varierà a seconda del sistema operativo/piattaforma nonché del linguaggio/framework usati dall'applicazione.

Non è consigliabile impostare in modo esplicito l'applicazione in modo che utilizzi solo TLS 1.2, a meno che non sia necessario, in quanto ciò può interrompere le funzionalità di sicurezza a livello di piattaforma che consentono di rilevare e sfruttare automaticamente i protocolli più recenti più sicuri man mano che diventano disponibili, ad esempio TLS 1.3. È consigliabile eseguire un controllo approfondito del codice dell'applicazione per verificare la presenza di hardcoding di versioni TLS/SSL specifiche.
...
|.NET | Supportato, la configurazione varia a seconda della versione. | Per informazioni dettagliate sulla configurazione per .NET 4.7 e versioni precedenti, fare riferimento a [queste istruzioni](https://docs.microsoft.com/dotnet/framework/network-programming/tls#support-for-tls-12).  |
|Monitoraggio stato | Supportato, è necessaria la configurazione | Status Monitor si basa sulla [configurazione](https://docs.microsoft.com/windows-server/security/tls/tls-registry-settings) + del sistema operativo[.NET Configuration](https://docs.microsoft.com/dotnet/framework/network-programming/tls#support-for-tls-12) per supportare TLS 1.2.
|Node.js |  Supportato, nella versione 10.5.0, potrebbe essere necessaria la configurazione. | Utilizzare la [documentazione ufficiale di Node.js TLS/SSL](https://nodejs.org/api/tls.html) per qualsiasi configurazione specifica dell'applicazione. |
...
Per eseguire un test preliminare per verificare se il sistema Linux è in grado di comunicare tramite TLS 1.2., aprire il terminale ed eseguire:
...
Gli SDK sono diversi a seconda delle piattaforme e sono disponibili vari componenti da installare. (Fare riferimento a [Application Insights - panoramica][start].) Ogni componente invia dati diversi.
...
| DeviceContext |`Id`, IP, Impostazioni locali, Modello di dispositivo, rete, tipo di rete, nome OEM, risoluzione dello schermo, Istanza ruolo, Nome ruolo, Tipo di dispositivo |
...
| Dependencies |Tipo(SQL, HTTP, ...), stringa di connessione o URI, sincronizzazione/async, durata, esito positivo, istruzione SQL (con Monitoraggio stato) |
| **Eccezioni** |Tipo, **messaggio,** stack di chiamate, file di origine, numero di riga,`thread id` |
| Crashes |`Process id`, `parent process id`, `crash thread id`; patch dell'applicazione, `id`, build;  tipo di eccezione, indirizzo, motivo; simboli e registri offuscati, indirizzi iniziali e finali binari, nome e percorso binari, tipo cpu |
...
> [!NOTE]
> Il client IP viene utilizzato per dedurre la posizione geografica, tuttavia per impostazione predefinita i dati IP non vengono più memorizzati e tutti gli zeri vengono scritti nel campo associato. Per comprendere meglio la gestione dei dati personali si consiglia questo [articolo](../../azure-monitor/platform/personal-data-mgmt.md#application-data). Se avete bisogno di memorizzare i dati di indirizzo IP il nostro [articolo di raccolta di indirizzi IP](https://docs.microsoft.com/azure/azure-monitor/app/ip-collection) vi guiderà attraverso le vostre opzioni.

## <a name="credits"></a>Credits
Questo prodotto include i dati GeoLite2 [https://www.maxmind.com](https://www.maxmind.com)creati da MaxMind, disponibili da .
