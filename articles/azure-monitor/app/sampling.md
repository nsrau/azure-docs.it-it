...
* È anche possibile impostare il campionamento manualmente. Ciò può essere configurato nel portale nel *pagina utilizzo e costi stimati*.  SDK di ASP.NET nel file applicationinsights. config. In Asp.NET Core SDK tramite codice. O nel SDK Java di applicationinsights. XML file.
...
* **Il campionamento adattivo** regola automaticamente il volume di dati di telemetria inviato dal SDK nell'app ASP.NET/ASP.NET Core. A partire da 2.0.0-beta3 ASP.NET Web SDK, il campionamento adattivo è il metodo di campionamento predefinito. Il campionamento adattativo è attualmente disponibile solo per la telemetria lato server di ASP.NET. Per le applicazioni ASP.NET Core destinate a Framework completo, il campionamento adattivo è disponibile dalla versione 1.0.0 di Microsoft.ApplicationInsights.AspNetCore SDK. Per le applicazioni ASP.NET Core destinate a NetCore, il campionamento adattivo è disponibile da 2.2.0-beta1 Microsoft.ApplicationInsights.AspNetCore SDK.
...
Il campionamento adattivo è disponibile per il SDK di Application Insights per ASP.NET 2.0.0-beta3 e versioni successive. In 2.2.0-beta1 Microsoft.ApplicationInsights.AspNetCore SDK e versioni successive ed è abilitato per impostazione predefinita. 
...
[Informazioni su](../../azure-monitor/app/sampling.md#aspnet-core-sampling) sulla configurazione di campionamento adattivo per le applicazioni ASP.NET Core. 
...
## <a name="fixed-rate-sampling-for-aspnet-aspnet-core-and-java-web-sites"></a>Campionamento a frequenza fissa per siti web ASP.NET, ASP.NET Core e Java
...
* Se si usa una delle versioni più recenti degli SDK precedente, il campionamento adattivo è abilitato per impostazione predefinita con cinque elementi di telemetria al secondo.
  Esistono 2 AdaptiveSamplingTelemetryProcessors aggiunto per impostazione predefinita, una include tipo di evento di campionamento e l'altra consente di escludere il tipo di evento dal campionamento. Questa configurazione indica che il SDK tenterà di limitare gli elementi di telemetria su cinque elementi di telemetria dei tipi di evento e cinque gli elementi di telemetria di tutti gli altri tipi.
  Garantire che gli eventi vengono campionati separatamente rispetto ad altri tipi di dati di telemetria. Gli eventi vengono in genere usati per la telemetria business e molto probabilmente non devono dipendere da volumi di dati di telemetria diagnostica.
  Usare gli esempi nella sezione precedente del documento per modificare questo comportamento predefinito.

*Possibile i dati di telemetria da raccogliere più volte?*

*  No. SamplingTelemetryProcessors ignora gli elementi da considerazioni di campionamento se l'elemento è già campionato. Lo stesso vale per il campionamento per inserimento come, che non verranno applicate campionamento agli elementi già il campionamento nel SDK stesso.'
