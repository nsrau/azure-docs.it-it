...
* [Attivare la registrazione](#debug-data-from-the-sdk) aggiungendo un elemento `<SDKLogger />` nel nodo radice del file ApplicationInsights. XML (nella cartella Resources del progetto) e verificare la presenza di voci precedute da ai: info/warn/Error per eventuali log sospetti. 
...
Per abilitare la registrazione dell'SDK con le app Spring boot con Application Insights Spring boot Starter, aggiungere il codice seguente al file `application.properties`:
