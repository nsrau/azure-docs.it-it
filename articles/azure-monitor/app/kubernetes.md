...
> [!NOTE]
> Poiché il servizio mesh solleva i dati fuori rete, non è possibile intercettare il traffico crittografato. Per il traffico che non lascia il cluster, usare un protocollo non crittografato (ad esempio, HTTP). Per il traffico esterno che deve essere crittografato, valutare la possibilità di [impostare la terminazione TLS](https://kubernetes.io/docs/concepts/services-networking/ingress/#tls) sul controller di ingresso.
