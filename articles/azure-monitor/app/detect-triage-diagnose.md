...
ms.locfileid: "54000190"
---
# <a name="overview-of-application-insights-for-devops"></a>Panoramica di Application Insights per DevOps

Con [Application Insights](../../application-insights/app-insights-overview.md) è possibile capire velocemente le prestazioni dell'app e come viene usata quando è attiva. Se è presente un problema, consente di scoprirlo, aiuta a valutarne l'impatto e a determinarne la causa.
...
![Ciclo DevOps](./media/detect-triage-diagnose/00-devcycle.png)
...
![Rilevare-Valutare-Diagnosticare](./media/detect-triage-diagnose/01-pipe1.png)

## <a name="detect-poor-availability"></a>Rilevare scarsa disponibilità
Marcela Markova è un senior developer del team di OBS e accetta di essere responsabile del monitoraggio delle prestazioni online. Imposta alcuni [test di disponibilità](../../azure-monitor/app/monitor-web-app-availability.md):
...
![Visualizzazione dei test Web eseguiti nel periodo precedente](./media/detect-triage-diagnose/04-webtests.png)
...
Nella pagina Panoramica in Application Insights è presente un grafico che mostra una serie di [metriche chiave](../../application-insights/app-insights-web-monitor-performance.md).

![Schermata panoramica dei grafi degli indicatori KPI](./media/detect-triage-diagnose/overview-graphs.png)
...
![Alcuni criteri di misurazione](./media/detect-triage-diagnose/002-servers.png)
...
Quindi imposta un [avviso](../../azure-monitor/app/metrics-explorer.md) se i tempi di risposta sono superiori alla soglia tipica. Questo le consentirà di conoscere la velocità dei tempi di risposta.

![Pannello Aggiungi avviso](./media/detect-triage-diagnose/07-alerts.png)
...
![messaggio di posta elettronica sulla diagnostica proattiva](./media/detect-triage-diagnose/21.png)
...
![richiesta non riuscita](./media/detect-triage-diagnose/23.png)

## <a name="detect-exceptions"></a>Rilevare le eccezioni
Con una configurazione minima, le [eccezioni](../../azure-monitor/app/asp-net-exceptions.md) vengono segnalate automaticamente ad Application Insights. Possono anche essere acquisite in modo esplicito con l'inserimento di chiamate a [TrackException()](../../azure-monitor/app/api-custom-events-metrics.md#trackexception) nel codice:  
...
In effetti, la strategia della banca è addirittura più estesa: vengono inviati i dati di telemetria in tutti i casi in cui il cliente è insoddisfatto riguardo all'operazione desiderata, corrispondenti o meno a un'eccezione del codice. Ad esempio, se il sistema esterno di bonifico interbancario restituisce un messaggio che indica che non è possibile completare l'operazione per qualche ragione operativa (nessun errore del cliente), allora si tiene traccia di quell'evento.
...
Eccezioni ed eventi vengono visualizzati nel pannello [Ricerca diagnostica](diagnostic-search.md). È possibile esaminarli per visualizzare le proprietà aggiuntive e l'analisi dello stack.

![In Ricerca diagnostica usare i filtri per visualizzare determinati tipi di dati](./media/detect-triage-diagnose/appinsights-333facets.png)


## <a name="monitor-proactively"></a>Monitorare in modo proattivo
Marcela non resta ferma in attesa di avvisi. Subito dopo ogni ridistribuzione, esamina i [tempi di risposta](../../application-insights/app-insights-web-monitor-performance.md), sia la figura nel suo complesso che la tabella delle richieste più lente, nonché il numero di eccezioni.  

![Grafico dei tempi di risposta e griglia dei tempi di risposta del server.](./media/detect-triage-diagnose/response-time.png)
...
![Numero di visualizzazioni della pagina](./media/detect-triage-diagnose/funnel.png)
...
[Altre informazioni sul monitoraggio dell'utilizzo](../../application-insights/app-insights-usage-overview.md).
...
* App già distribuite, ospitate in [IIS](../../azure-monitor/app/monitor-web-app-availability.md), [J2EE](../../azure-monitor/app/java-live.md) o [Azure](../../application-insights/app-insights-overview.md).
