...
Se si utilizza una sola parte della combinazione, la scalabilità automatica agirà solo in una singola direzione (scalabilità orizzontale o in) fino a raggiungere il numero massimo o il numero minimo di istanze definito nel profilo. Questo non è ottimale, sarebbe opportuno che la risorsa possa aumentare in orari di utilizzo elevato per garantire la disponibilità. Allo stesso modo, in casi di uso non elevato si desidera che la risorsa sia ridotta per ottenere risparmi sui costi.
...
*Non è consigliabile* impostare la scalabilità automatica, ad esempio gli esempi seguenti con valori di soglia uguali o simili per out e in condizioni:We do not recommend autoscale settings like the examples below with the same or similar threshold values for out and in conditions:

* Aumentare le istanze di 1 conteggio quando il conteggio dei thread >600
* Diminuire le istanze di 1 conteggio quando il conteggio thread <600
...
4. Prima di ridurre le prestazioni, la scalabilità automatica cerca di stimare quale sarà lo stato finale in caso di riduzione del numero di istanze. Ad esempio, 575 x 3 (conteggio corrente delle istanze) = 1.725 / 2 (numero finale di istanze dopo la riduzione delle prestazioni) = 862,5 thread. Ciò significa che la scalabilità automatica dovrebbe aumentare immediatamente anche dopo la scalabilità verticale, se il numero medio di thread rimane lo stesso o addirittura cade solo una piccola quantità. Se, tuttavia, aumentasse di nuovo le prestazioni, l'intero processo si ripeterebbe, generando un loop infinito.
5. Per evitare questa situazione (definita "instabile"), la scalabilità automatica non riduce affatto le prestazioni, ma ignora la condizione e la valuta nuovamente la volta successiva che il processo del servizio viene eseguito. Lo stato di sbattimento può confondere molte persone perché la scalabilità automatica non sembra funzionare quando il numero medio di thread è 575.
...
4. Una terza istanza della coda di archiviazione viene aggiunta a causa di un'azione di scalabilità orizzontale.  L'azione successiva di aumento del numero di istanze verrà eseguita solo dopo che il conteggio totale dei messaggi nella coda avrà raggiunto il numero di 150, poiché 150/3 = 50.
...
Esaminiamo usando un esempio:Let's review using an example:

L'immagine seguente illustra un'impostazione di scalabilità automatica con un profilo predefinito con un numero minimo di istanze = 2 e un numero massimo = 10. In questo esempio, le regole sono configurate per la scalabilità orizzontale quando il numero di messaggi nella coda è maggiore di 10 e la scalabilità orizzontale quando il numero di messaggi nella coda è inferiore a tre. Ora la risorsa può quindi essere ridimensionata tra le due e le 10 istanze.
...
In alcuni casi potrebbe essere necessario impostare più regole in un profilo. Le regole di scalabilità automatica seguenti vengono usate dal motore di scalabilità automatica quando sono impostate più regole.

In *caso di scalabilità*orizzontale, la scalabilità automatica viene eseguita se viene soddisfatta una regola.
...
* Se la CPU è 76% e la memoria è 50%, si scalabilia orizzontalmente.
* Se la CPU è 50% e la memoria è 76%, si scalabilia orizzontalmente.
...
Il numero di istanze predefinito è importante perché la scalabilità automatica ridimensiona il servizio in base al conteggio quando le metriche non sono disponibili. Selezionare quindi un conteggio di istanze predefinito sicuro per i carichi di lavoro.
...
* La scalabilità automatica emette un'operazione di ridimensionamento.
