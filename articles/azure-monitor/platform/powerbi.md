...
Iniziare creando una [ricerca log](../../azure-monitor/log-query/log-query-overview.md) che restituisca i dati di Log Analytics che si vuole usare per compilare il set di dati di Power BI.  Esportare quindi la query nella [lingua di Power Query (M)](https://msdn.microsoft.com/library/mt807488.aspx), in modo che possa essere usata da Power BI Desktop.
...
2. Se si usa il portale di ricerca log, fare clic su **Power BI**.  Se si usa il portale di Analytics, selezionare **Esporta** > **Power BI Query (M)** (Query Power BI - M).  Entrambe le opzioni esportano la query in un file di testo denominato **PowerBIQuery.txt**. 
    ...
    ![Esportare la ricerca log](media/powerbi/export-logsearch.png) ![Esportare la ricerca log](media/powerbi/export-analytics.png)
...
* Informazioni su [ricerche dei log](../../azure-monitor/log-query/log-query-overview.md) per compilare query che possono essere esportate in Power BI.
