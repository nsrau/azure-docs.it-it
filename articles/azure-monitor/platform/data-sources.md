...
|:---|:---|:---|
| [Tenant di Azure](#azure-tenant) | i dati relativi al funzionamento dei servizi di Azure a livello di tenant, ad esempio Azure Active Directory. | Visualizzare i dati di AAD nel portale o configurare la raccolta in monitoraggio di Azure usando un'impostazione di diagnostica del tenant. |
...
I log di diagnostica a livello di metrica e di risorsa offrono informazioni sul funzionamento _interno_ delle risorse di Azure. Sono disponibili per la maggior parte dei servizi di Azure e il monitoraggio di soluzioni e informazioni dettagliate raccoglie dati aggiuntivi per servizi specifici.
...
| Log di monitoraggio di Azure | Copiare le metriche della piattaforma nei log per la tendenza e altre analisi usando Log Analytics. | [Diagnostica di Azure diretta a Log Analytics](diagnostic-logs-stream-log-store.md) |
...
I [log di diagnostica](diagnostic-logs-overview.md) forniscono informazioni dettagliate sul funzionamento _interno_ di una risorsa di Azure.  I log di diagnostica non sono abilitati per impostazione predefinita. È necessario abilitarli e specificare una destinazione per ogni risorsa. 

I requisiti di configurazione e il contenuto dei log di diagnostica variano in base al tipo di risorsa e non a tutti i servizi, ma creano log di diagnostica. Per informazioni dettagliate su ogni servizio e collegamenti a procedure di configurazione dettagliate, vedere [servizi, schemi e categorie supportati per i log di diagnostica di Azure](diagnostic-logs-schema.md) . Se il servizio non è elencato in questo articolo, il servizio attualmente non scrive nei log di diagnostica.

| Destination | Descrizione | Riferimenti |
|:---|:---|:---|
| Log di monitoraggio di Azure | Inviare i log di diagnostica ai log di monitoraggio di Azure per l'analisi con altri dati di log raccolti. Alcune risorse possono scrivere direttamente in monitoraggio di Azure mentre altre scrivono in un account di archiviazione prima di essere importate in un'area di lavoro Log Analytics. | [Trasmettere log di diagnostica di Azure all'area di lavoro Log Analytics in monitoraggio di Azure](diagnostic-logs-stream-log-store.md)<br>[Usare il portale di Azure per raccogliere i log da archiviazione di Azure](azure-storage-iis-table.md#use-the-azure-portal-to-collect-logs-from-azure-storage)  |
| Archiviazione | Inviare i log di diagnostica ad archiviazione di Azure per l'archiviazione. | [Archiviare i log di diagnostica di Azure](archive-diagnostic-logs.md) |
| Hub eventi | Eseguire lo streaming dei log di diagnostica in altre posizioni tramite hub eventi. |[Trasmettere log di diagnostica di Azure a un hub eventi](diagnostic-logs-stream-event-hubs.md) |
