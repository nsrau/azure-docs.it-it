...
![Logo di AIM AIOps](./media/partners/aims.jpg)

MIRA AIOps (intelligenza artificiale per le operazioni IT) consente di automatizzare l'analisi delle metriche delle prestazioni di Azure (per l'infrastruttura e i servizi) per fornire informazioni dettagliate di utilità pratica per migliorare l'efficienza, la scalabilità in modo appropriato, il controllo dei costi e la fornitura di informazioni aziendali.  MIRA a usare Machine Learning per alleviare i team operativi IT da noiose attività manuali.  MIRA inoltre a supportare le tecnologie locali per un controllo ibrido trasparente.  Gli obiettivi sono disponibili in Azure Marketplace e come edizione Community gratuita completamente funzionante.

[La documentazione di AIOps per Azure](https://www.aims.ai/platform/azure)
...
[Documenti sull'integrità del cloud](https://www.cloudhealthtech.com/products/azure-management)
...
Azure consente ai clienti di eseguire la migrazione e modernizzare le applicazioni per l'esecuzione nel cloud, in coordinamento con molte soluzioni dei partner. Uno di questi partner è Datadog, che offre agli utenti strumenti di osservabilità e sicurezza per comprendere l'integrità e le prestazioni delle applicazioni in ambienti ibridi e multicloud.  Tuttavia, la configurazione delle integrazioni necessarie spesso richiede lo spostamento tra il portale di Azure e Datadog.  Questo processo aggiunge complessità, richiede tempo e rende difficile la risoluzione dei problemi se le cose non funzionano.  Per ridurre il carico di gestione su più portali, Microsoft ha collaborato con Datadog per creare una soluzione Datadog integrata sulla piattaforma cloud di Azure.  Disponibile tramite Azure Marketplace, questa soluzione offre un'esperienza uniforme per l'uso della soluzione di monitoraggio cloud di Datadog in Azure.  

Con la nuova integrazione di Azure-Datadog, le organizzazioni possono ora eseguire il mapping completo dei sistemi legacy e basati sul cloud, monitorando i dati in tempo reale durante ogni fase della transizione cloud e assicurandosi che le applicazioni migrate soddisfino gli obiettivi di prestazioni. Questa integrazione combina la presenza globale di Azure, la flessibilità, la sicurezza e la conformità con le funzionalità di registrazione e monitoraggio di Datadog per creare la migliore esperienza per le aziende. Grazie a questa esperienza unificata, sarà possibile: 

- Effettuare il provisioning di una nuova organizzazione Datadog e configurare le risorse di Azure per l'invio di log e metriche a Datadog, una configurazione completamente gestita senza infrastruttura per la configurazione e il funzionamento dei clienti.  
- Invia facilmente log e metriche a Datadog. Il processo di invio dei log è stato completamente automatizzato; anziché creare una pipeline di invio di log con impostazioni di diagnostica, Hub eventi e funzioni, è possibile configurare tutti gli elementi con pochi clic.
-   Installare facilmente l'agente Datadog negli host VM con un solo clic.
...
[Integrazione Grafana di monitoraggio di Azure](./grafana-plugin.md)

## <a name="influxdata"></a>InfluxData

![Logo di InfluxData](./media/partners/Influxdata.png)

InfluxData è il creatore di InfluxDB, il database di serie temporali open source. La nostra tecnologia è progettata per gestire i volumi elevati di dati timestamp prodotti da dispositivi, applicazioni, reti, contenitori e computer. Ci troviamo in una missione per aiutare gli sviluppatori e le organizzazioni, ad esempio IBM, Visa, Siemens, Tesla e NASA, a archiviare e analizzare i dati in tempo reale, offrendo loro la possibilità di creare applicazioni di monitoraggio e analisi trasformative e di scalabilità. Microsoft Azure le funzionalità di raccolta di metriche ed eventi di monitoraggio possono essere espanse con l'agente Telegraf di InfluxData, che consente agli utenti Microsoft di trarre vantaggio da una soluzione di database Time Series leader e sfruttare i maggiori contributi open source per la Telegraf.

[Integrazione di Telegraf dei dati di monitoraggio di Azure](./collect-custom-metrics-linux-telegraf.md) 
...
![Logo di Moogsoft](./media/partners/moogsoft.png)
...
[Scopri di più sul servizio adesso](https://www.servicenow.com/solutions/aiops.html)
...
[Documentazione di Sumologic](https://www.sumologic.com/azure) 
