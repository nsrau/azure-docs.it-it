...
ms.locfileid: "57309690"
---
# <a name="stream-the-azure-activity-log-to-event-hubs"></a>Trasmettere il log attività di Azure a Hub eventi
È possibile trasmettere il [log attività di Azure](../../azure-monitor/platform/activity-logs-overview.md) quasi in tempo reale a qualsiasi applicazione:
...
1. Passare alla sezione **Log attività** usando la casella di ricerca **Tutti i servizi** sul lato sinistro del portale.
   ...
   ![Selezione di Log attività nell'elenco dei servizi disponibili nel portale](./media/activity-logs-stream-event-hubs/activity-logs-portal-navigate-v2.png)
2. Selezionare il pulsante **Esporta in Hub eventi**  nella parte superiore del log.
   
   ![Pulsante Esporta nel portale](./media/activity-logs-stream-event-hubs/activity-logs-portal-export-v2.png)
...
3. Nella sezione che viene visualizzata selezionare **Tutte le aree**. Non selezionare aree specifiche.
   ...
   ![Sezione di esportazione](./media/activity-logs-stream-event-hubs/export-audit.png)
...
1. Usare `Get-AzLogProfile` per determinare se esiste già un profilo di log.  Se esiste un profilo di log, individuare la proprietà *name*.
...
Lo schema del log attività è disponibile in [Monitorare l'attività di sottoscrizione con il log attività di Azure](../../azure-monitor/platform/activity-logs-overview.md). Ogni evento si trova in una matrice di BLOB JSON denominati *record*.
