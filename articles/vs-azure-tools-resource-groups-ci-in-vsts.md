<properties
	pageTitle="Integrazione continua in VS Team Services con i progetti di gruppi di risorse di Azure | Microsoft Azure"
...
1.	Configurare l'autorizzazione di Azure in VS Team Services.
1.	Aggiungere un'istruzione di compilazione di Azure PowerShell che fa riferimento allo script di PowerShell nel progetto di distribuzione Gruppo di risorse di Azure.
1.	Impostare il valore del parametro *-ArtifactsStagingDirectory* per l'interazione con un progetto compilato in VS Team Services.
...
1.	Aggiungere un'istruzione di compilazione di **Azure PowerShell** alla definizione di compilazione e quindi scegliere il pulsante **Aggiungi istruzione di compilazione**.

    ![][1]

1.	Scegliere la categoria **Distribuzione**, selezionare l'attività **Azure PowerShell** e quindi scegliere il pulsante **Aggiungi**.
...
1.	Scegliere l'istruzione di compilazione di **Azure PowerShell** e quindi compilare i relativi valori.
    ...
    1.	Scegliere il collegamento**Gestisci** accanto alla casella di riepilogo a discesa **Sottoscrizione Azure**.

        ![][3]

    1. Scegliere **Azure** nella casella di riepilogo a discesa **Nuovo endpoint servizio**.

        ![][4]

    1.	Nella finestra di dialogo **Aggiungi sottoscrizione di Azure** selezionare l'opzione **Entità servizio**.

        ![][5]

    1.	Aggiungere le informazioni sulla sottoscrizione di Azure nella finestra di dialogo **Aggiungi sottoscrizione di Azure**. È necessario specificare gli elementi seguenti:
        -	ID sottoscrizione
        -	Nome sottoscrizione
        -	ID entità servizio
    ...
    1.	Aggiungere un nome a propria scelta nella casella **Sottoscrizione**. Questo valore verrà visualizzato in seguito nell'elenco a discesa **Sottoscrizione Azure** in VS Team Services.

    1.	Se non si conosce l'ID sottoscrizione di Azure, è possibile usare uno dei seguenti comandi per ottenerlo.
    ...
    1.	Aggiungere i valori per ID entità servizio, Chiave entità servizio e ID Tenant nella finestra di dialogo **Aggiungi sottoscrizione di Azure** e quindi scegliere **OK**.

        Ora è disponibile un'entità servizio valida da usare per eseguire lo script di Azure PowerShell.

1.	Modificare la definizione di compilazione e scegliere l'istruzione di compilazione **Azure PowerShell**. Selezionare la sottoscrizione nella casella di riepilogo a discesa **Sottoscrizione Azure**. Se la sottoscrizione non viene visualizzata, scegliere il pulsante **Aggiorna** accanto al collegamento **Gestisci**.

    ![][8]

1.	Fornire un percorso per lo script di PowerShell Deploy-AzureResourceGroup.ps1. A tale scopo, scegliere il pulsante con i puntini di sospensione (...) accanto alla casella **Percorso script**, passare allo script di PowerShell Deploy-AzureResourceGroup.ps1 nella cartella **Script** del progetto, selezionarlo e quindi scegliere **OK**.

    ![][9]

1. Dopo aver selezionato lo script, aggiornare il percorso dello script in modo che venga eseguito da Build.StagingDirectory, la stessa directory su cui è impostato *ArtifactsLocation*. A questo scopo è possibile aggiungere "$(Build.StagingDirectory)/" all'inizio del percorso dello script.

    ![][10]

1.	Nella casella **Argomenti script** immettere i parametri seguenti, su una sola riga. Quando si esegue lo script in Visual Studio, è possibile vedere come vengono usati i parametri nella finestra **Output**. Si può usare questa finestra come punto di partenza per impostare i valori dei parametri nell'istruzione di compilazione.
...
1.	Dopo aver aggiunto tutti gli elementi richiesti per l'istruzione di compilazione di Azure PowerShell, scegliere il pulsante **Accoda compilazione** per compilare il progetto. La schermata **Compilazione** mostra l'output dello script di PowerShell.
