...
### CORS
Invece di un'impostazione app **MS\_CrossDomainOrigins** delimitata da virgole, ora è disponibile un pannello nel portale di gestione di Azure per la configurazione di CORS. In alternativa, è possibile eseguirne la configurazione usando strumenti di Gestione risorse come Azure PowerShell, CLI o [Esplora risorse](https://resources.azure.com/). Impostare la proprietà **cors** sul tipo di risorsa **Microsoft.Web/sites/config** per la risorsa **&lt;nome del sito&gt;/web**. Ad esempio:
...
Per altre informazioni sull'SDK 2.8.1, leggere il [post di blog](https://azure.microsoft.com/blog/announcing-the-azure-sdk-2-8-1-for-net) relativo all'annuncio.
...
2. Connettere e distribuire il progetto nell'app per le API vuota usando uno dei meccanismi di distribuzione supportati dal servizio app. Per altre informazioni, vedere la [Documentazione sulla distribuzione del servizio app di Azure](../app-service-web/web-sites-deploy.md). 
...
- Client: [SDK del client per dispositivi mobili Azure](http://www.nuget.org/packages/Microsoft.Azure.Mobile.Client/)
- Server: [estensione dell'autenticazione .NET per le app per dispositivi mobili di Microsoft Azure](http://www.nuget.org/packages/Microsoft.Azure.Mobile.Server.Authentication/) 

Se invece si usano gli SDK alfa del servizio app, gli SDK seguenti sono deprecati:

- Client: [SDK AppService di Microsoft Azure](http://www.nuget.org/packages/Microsoft.Azure.AppService)
