...
- **Integrazione con app per la logica**: le app per le API create possono essere utilizzate dalle [app per la logica del servizio app](../app-service-logic/app-service-logic-what-are-logic-apps.md). Per informazioni sulle modifiche in corso sulle modalità di integrazione di App per le API con App per la logica, vedere [App per le API del servizio app: modifiche apportate](app-service-api-whats-changed.md).
