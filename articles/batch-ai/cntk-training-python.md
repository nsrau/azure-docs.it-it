---
title: Eseguire il training di un modello CNTK con Azure Batch per intelligenza artificiale - Python | Microsoft Docs
description: Eseguire il training di una rete neurale di Microsoft Cognitive Toolkit (CNTK) con Azure Batch per intelligenza artificiale usando Python SDK
services: batch-ai
...
Questo articolo illustra come usare Azure Python SDK per eseguire il training di un modello di Microsoft Cognitive Toolkit (CNTK) di esempio usando il servizio Batch per intelligenza artificiale.
...
* Azure Python SDK - Vedere le [istruzioni per l'installazione](/python/azure/python-sdk-azure-install). Questo articolo richiede almeno il pacchetto azure-mgmt-batchai versione 2.0.0.
...
Creare una condivisione file denominata `batchaiquickstart`.
...
Creare una directory nella condivisione denominata `mnistcntksample`.

```Python
mnist_dataset_directory = 'mnistcntksample'
service.create_directory(azure_file_share_name, mnist_dataset_directory, fail_on_exist=False)
```
Scaricare il [pacchetto di esempio](https://batchaisamples.blob.core.windows.net/samples/BatchAIQuickStart.zip?st=2017-09-29T18%3A29%3A00Z&se=2099-12-31T08%3A00%3A00Z&sp=rl&sv=2016-05-31&sr=b&sig=hrAZfbZC%2BQ%2FKccFQZ7OC4b%2FXSzCF5Myi4Cj%2BW3sVZDo%3D) e decomprimerlo nella directory corrente. Il codice seguente carica i file necessari nella condivisione file di Azure:
...
Un'area di lavoro è una raccolta di primo livello di tutti i tipi di risorse di Batch per intelligenza artificiale. I cluster di Batch per intelligenza artificiale e gli esperimenti vengono creati in un'area di lavoro.
...
## <a name="create-experiment-and-training-job"></a>Creare l'esperimento e il processo di training

Dopo aver creato il cluster, creare un esperimento, ovvero un contenitore logico per un gruppo di processi correlati. Configurare quindi un processo di training e inviarlo nell'esperimento:
...
Per altre informazioni sull'uso di Batch per intelligenza artificiale con diversi framework, vedere le [recipe di training](https://github.com/Azure/BatchAI).