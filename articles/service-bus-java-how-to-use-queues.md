...
-   [Configurazione dell'applicazione per l'utilizzo del bus di servizio](#bkmk_ConfigApp)
-   [Procedura: Creare un provider di token di sicurezza](#bkmk_HowToCreateQueue)
-   [Procedura: Creare una coda](#bkmk_HowToCreateQueue)
...
Per eseguire operazioni di gestione per le code del bus di servizio, è possibile utilizzare la classe **ServiceBusContract** class. Un oggetto **ServiceBusContract** è costruito con una configurazione appropriata che incapsula le autorizzazioni dei token necessarie a gestirlo. La classe **ServiceBusContract** rappresenta l'unico punto di comunicazione con Azure.
...
Il modo principale per ricevere i messaggi da una coda consiste nell'utilizzare un oggetto **ServiceBusContract**. È possibile utilizzare i messaggi ricevuti in due diverse modalità: **ReceiveAndDelete** e **PeekLock**.
...
Nell'esempio seguente viene illustrato come ricevere ed elaborare messaggi utilizzando la modalità **PeekLock** non predefinita. Nell'esempio seguente viene creato un ciclo infinito e i messaggi vengono elaborati non appena arrivano in "TestQueue":
