<properties urlDisplayName="Call a custom API from the client" pageTitle="Chiamare un'API personalizzata da un client HTML - Servizi mobili" metaKeywords="" description="Informazioni su come definire un'API personalizzata e chiamarla da un'app HTML che usa Servizi mobili di Microsoft Azure." metaCanonical="" services="mobile-services" documentationCenter="Mobile" title="Call a custom API from the client" authors="glenga"  solutions="" writer="jparrel" manager="dwrede" editor=""  />
...
Questo argomento illustra come chiamare un'API personalizzata da un'app HTML. Un'API personalizzata consente di definire endpoint personalizzati che espongono la funzionalità del server di cui non è possibile eseguire il mapping a un'operazione di inserimento, aggiornamento, eliminazione o lettura. L'uso di un'API personalizzata offre maggiore controllo sulla messaggistica, incluse la lettura e l'impostazione delle intestazioni del messaggio HTTP e la definizione di un formato del corpo del messaggio diverso da JSON.
...
<h2><a name="update-app"></a>Aggiornare l'app per la chiamata all'API personalizzata</h2>
