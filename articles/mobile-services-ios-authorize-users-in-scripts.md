...
Questo argomento illustra come usare gli script del server per autorizzare gli utenti autenticati per l'accesso ai dati in Servizi mobili di Azure da un'app per iOS.  In questa esercitazione gli script verranno registrati con Servizi Mobili per filtrare le query in base all'ID utente di un utente autenticato, per garantire che ogni utente possa visualizzare solo i relativi dati.
