...
I tag di servizio seguenti sono disponibili per l'utilizzo in [le regole dei gruppi di sicurezza di rete](https://docs.microsoft.com/azure/virtual-network/security-overview#security-rules). Tag di servizio con asterisco alla fine (vale a dire AzureCloud *) possono essere usati anche nelle [regole del Firewall di Azure rete](https://docs.microsoft.com/azure/firewall/service-tags). 

* **VirtualNetwork** (Resource Manager) (**VIRTUAL_NETWORK** per la versione classica): Questo tag include lo spazio di indirizzi della rete virtuale (tutti gli intervalli CIDR definiti per la rete virtuale), tutti gli spazi indirizzi locali, connessi [peering](virtual-network-peering-overview.md) reti virtuali o rete virtuale connessa a un [virtuale gateway di rete](../vpn-gateway/vpn-gateway-about-vpngateways.md?toc=%2fazure%2fvirtual-network%2ftoc.json) e risolvere i prefissi utilizzati in [route definite dall'utente](virtual-networks-udr-overview.md). Tenere presente che questo tag può contenere route predefinita. 
...
* **AzureCloud*** (solo Resource Manager): Questo tag identifica lo spazio indirizzi IP per Azure che include tutti gli [indirizzi IP pubblici dei data center](https://www.microsoft.com/download/details.aspx?id=41653). Se si specifica *AzureCloud* come valore, verrà consentito o impedito il traffico verso gli indirizzi pubblici di Azure. Se si desidera solo consentire l'accesso al cloud Azure in una determinata [regione](https://azure.microsoft.com/regions), è possibile specificare tale area nel formato seguente AzureCloud. [ nome dell'area]. Questo tag consigliati per la regola di sicurezza in uscita. 
* **AzureTrafficManager*** (solo Resource Manager): Questo tag identifica lo spazio indirizzi IP per gli indirizzi IP probe di Gestione traffico di Azure. Per altre informazioni sugli IP probe di Gestione traffico, consultare [Domande frequenti su Gestione traffico di Azure](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-faqs). Questo tag consigliati per la regola di sicurezza in ingresso.  
* **Archiviazione*** (solo Resource Manager): Questo tag identifica lo spazio indirizzi IP per il servizio Archiviazione di Azure. Se si specifica *Storage* come valore, verrà consentito o impedito il traffico verso il servizio di archiviazione. Se si desidera solo consentire l'accesso all'archiviazione in una determinata [regione](https://azure.microsoft.com/regions), è possibile specificare tale area nel seguente formato di archiviazione. [ nome dell'area]. Il tag rappresenta il servizio, ma non istanze specifiche del servizio. Ad esempio, il tag rappresenta il servizio Archiviazione di Azure, ma non uno specifico account di archiviazione di Azure. Questo tag consigliati per la regola di sicurezza in uscita. 
* **SQL*** (solo Resource Manager): Questo tag identifica i prefissi degli indirizzi del Database SQL di Azure, Database di Azure per MySQL, Database di Azure per PostgreSQL e i servizi di Azure SQL Data Warehouse. Se si specifica *Sql* come valore, verrà consentito o impedito il traffico verso SQL. Se si desidera solo consentire l'accesso a Sql in una determinata [regione](https://azure.microsoft.com/regions), è possibile specificare tale area nel seguente formato Sql. [nome area]. Il tag rappresenta il servizio, ma non istanze specifiche del servizio. Ad esempio, il tag rappresenta il servizio Database SQL di Azure, ma non uno specifico server o database SQL. Questo tag consigliati per la regola di sicurezza in uscita. 
* **AzureCosmosDB*** (solo Resource Manager): Questo tag identifica i prefissi di indirizzo del servizio Azure Cosmos DB. Se si specifica *AzureCosmosDB* come valore, verrà consentito o impedito il traffico verso AzureCosmosDB. Se si vuole solo consentire l'accesso ad AzureCosmosDB in un'[area](https://azure.microsoft.com/regions) specifica, è possibile specificare l'area nel formato seguente AzureCosmosDB.[nome dell'area]. Questo tag consigliati per la regola di sicurezza in uscita. 
* **AzureKeyVault*** (solo Resource Manager): Questo tag identifica i prefissi di indirizzo del servizio Azure KeyVault. Se si specifica *AzureKeyVault* come valore, verrà consentito o impedito il traffico verso AzureKeyVault. Se si vuole solo consentire l'accesso ad AzureKeyVault in un'[area](https://azure.microsoft.com/regions) specifica, è possibile specificare l'area nel formato seguente AzureKeyVault.[nome dell'area]. Il tag non ha dipendenze **AzureActiveDirectory** tag. Questo tag consigliati per la regola di sicurezza in uscita.  
* **Hub eventi*** (solo Resource Manager): Questo tag identifica i prefissi di indirizzo del servizio Azure EventHub. Se si specifica *EventHub* come valore, verrà consentito o impedito il traffico verso EventHub. Se si vuole solo consentire l'accesso ad EventHub in un'[area](https://azure.microsoft.com/regions) specifica, è possibile specificare l'area nel formato seguente EventHub.[nome dell'area]. Questo tag consigliati per la regola di sicurezza in uscita. 
* **Bus di servizio*** (solo Resource Manager): Questo tag identifica i prefissi degli indirizzi del servizio del bus di servizio di Azure usando il livello di servizio Premium. Se si specifica *ServiceBus* come valore, verrà consentito o impedito il traffico verso ServiceBus. Se si vuole solo consentire l'accesso ad ServiceBus in un'[area](https://azure.microsoft.com/regions) specifica, è possibile specificare l'area nel formato seguente ServiceBus.[nome dell'area]. Questo tag consigliati per la regola di sicurezza in uscita. 
* **MicrosoftContainerRegistry*** (solo Resource Manager): Questo tag identifica i prefissi di indirizzo del servizio Registro Container di Microsoft. Se si specifica *MicrosoftContainerRegistry* come valore, verrà consentito o impedito il traffico verso MicrosoftContainerRegistry. Se si vuole solo consentire l'accesso ad MicrosoftContainerRegistry in un'[area](https://azure.microsoft.com/regions) specifica, è possibile specificare l'area nel formato seguente MicrosoftContainerRegistry.[nome dell'area]. Questo tag consigliati per la regola di sicurezza in uscita. 
* **AzureContainerRegistry*** (solo Resource Manager): Questo tag identifica i prefissi di indirizzo del servizio Registro Azure Container. Se si specifica *AzureContainerRegistry* come valore, verrà consentito o impedito il traffico verso AzureContainerRegistry. Se si vuole solo consentire l'accesso a AzureContainerRegistry in un'[area](https://azure.microsoft.com/regions) specifica, è possibile specificare l'area nel formato seguente AzureContainerRegistry.[nome dell'area]. Questo tag consigliati per la regola di sicurezza in uscita. 
* **AppService*** (solo Resource Manager): Questo tag identifica i prefissi di indirizzo del servizio Azure AppService. Se si specifica *AppService* come valore, verrà consentito o impedito il traffico verso AppService. Se si vuole solo consentire l'accesso ad AppService in un'[area](https://azure.microsoft.com/regions) specifica, è possibile specificare l'area nel formato seguente AppService.[nome dell'area]. Questo tag consigliati per la regola di sicurezza in uscita per i front-end di App Web.  
* **AppServiceManagement*** (solo Resource Manager): Questo tag identifica i prefissi degli indirizzi il traffico di gestione per le distribuzioni di ambiente del servizio App dedicato. Se si specifica *AppServiceManagement* come valore, verrà consentito o impedito il traffico verso AppServiceManagement. Questo tag consigliati per la regola di sicurezza in ingresso/in uscita. 
* **ApiManagement*** (solo Resource Manager): Questo tag identifica che i prefissi degli indirizzi il traffico di gestione per gestione API di AZURE dedicato le distribuzioni. Se si specifica *ApiManagement* come valore, verrà consentito o impedito il traffico verso ApiManagement. Questo tag consigliati per la regola di sicurezza in ingresso/in uscita. 
* **AzureConnectors*** (solo Resource Manager): Questo tag identifica i prefissi degli indirizzi dei connettori di App per la logica per le connessioni di tipo probe/back-end. Se si specifica *AzureConnectors* come valore, verrà consentito o impedito il traffico verso AzureConnectors. Se si vuole solo consentire l'accesso ad AzureConnectors in un'[area](https://azure.microsoft.com/regions) specifica, è possibile specificare l'area nel formato seguente AzureConnectors.[nome dell'area]. Questo tag consigliati per la regola di sicurezza in ingresso. 
* **GatewayManager** (solo Resource Manager): Questo tag identifica i prefissi degli indirizzi il traffico di gestione per le distribuzioni dedicate i gateway VPN/App. Se si specifica *GatewayManager* come valore, verrà consentito o impedito il traffico verso GatewayManager. Questo tag consigliati per la regola di sicurezza in ingresso. 
* **AzureDataLake*** (solo Resource Manager): Questo tag identifica i prefissi di indirizzo del servizio Azure Data Lake. Se si specifica *AzureDataLake* come valore, verrà consentito o impedito il traffico verso AzureDataLake. Questo tag consigliati per la regola di sicurezza in uscita. 
* **AzureActiveDirectory*** (solo Resource Manager): Questo tag identifica i prefissi di indirizzo del servizio AzureActiveDirectory. Se si specifica *AzureActiveDirectory* come valore, verrà consentito o impedito il traffico verso AzureActiveDirectory. Questo tag consigliati per la regola di sicurezza in uscita.
* **AzureMonitor*** (solo Resource Manager): Questo tag identifica i prefissi degli indirizzi del Log Analitica, Application Insights, AzMon e metriche personalizzate (endpoint GiG). Se si specifica *AzureMonitor* come valore, verrà consentito o impedito il traffico verso AzureMonitor. Per Log Analitica, il tag non ha dipendenze **archiviazione** tag. Questo tag consigliati per la regola di sicurezza in uscita.
* **ServiceFabric*** (solo Resource Manager): Questo tag identifica i prefissi di indirizzo del servizio ServiceFabric. Se si specifica *ServiceFabric* come valore, verrà consentito o impedito il traffico verso ServiceFabric. Questo tag consigliati per la regola di sicurezza in uscita. 
* **AzureMachineLearning*** (solo Resource Manager): Questo tag identifica i prefissi di indirizzo del servizio AzureMachineLearning. Se si specifica *AzureMachineLearning* come valore, verrà consentito o impedito il traffico verso AzureMachineLearning. Questo tag consigliati per la regola di sicurezza in uscita. 
* **BatchNodeManagement*** (solo Resource Manager): Questo tag identifica i prefissi degli indirizzi il traffico di gestione per le distribuzioni di Azure Batch dedicato. Se si specifica *BatchNodeManagement* per il valore, il traffico è consentito o negato dal servizio Batch nei nodi di calcolo. Questo tag consigliati per la regola di sicurezza in ingresso/in uscita. 
...
### <a name="service-tags-in-on-premises"></a>Tag di servizio in locale  
È possibile scaricare e integrare con un firewall locale nell'elenco dei tag di servizio con i dettagli di prefisso nelle seguenti pubblicazioni settimanale per Azure [pubbliche](https://www.microsoft.com/download/details.aspx?id=56519), [governo degli Stati Uniti](https://www.microsoft.com/download/details.aspx?id=57063), [Cina ](https://www.microsoft.com/download/details.aspx?id=57062), e [Germania](https://www.microsoft.com/download/details.aspx?id=57064) cloud.

È anche a livello di programmazione possono recuperare queste informazioni usando il **API del servizio di individuazione Tag** (anteprima pubblica) - [REST](https://aka.ms/discoveryapi_rest), [Azure PowerShell](https://aka.ms/discoveryapi_powershell), e [ Interfaccia della riga di comando Azure](https://aka.ms/discoveryapi_cli). 

> [!NOTE]
> Seguire le pubblicazioni settimanale (versione precedente) per Azure [pubbliche](https://www.microsoft.com/en-us/download/details.aspx?id=41653), [Cina](https://www.microsoft.com/en-us/download/details.aspx?id=42064), e [Germania](https://www.microsoft.com/en-us/download/details.aspx?id=54770) cloud verrà deprecato entro il 30 giugno 2020. Per iniziare a usare le pubblicazioni aggiornate come descritto in precedenza. 
