<properties 
   pageTitle="Connessione ibrida con applicazione a 2 livelli | Microsoft Azure"
...
In questo scenario, il solo traffico da locale ad Azure verrà usato per gestire i firewall connettendosi a **AZF3**; questo traffico deve passare attraverso il firewall interno, **AZF2**. È quindi necessaria una sola route in **GatewaySubnet** come illustrato di seguito.
...
|Destination|Hop successivo|Spiegazione|
|---|---|---|
|10\.0.2.0/24|10\.0.3.10|Consente al traffico verso **azsn2** di passare dal server app al server Web tramite **AZF2**|
...
|Destination|Hop successivo|Spiegazione|
|---|---|---|
|192\.168.2.0/24|192\.168.1.4|Consente il traffico verso **onpremsn2** tramite **OPFW**|
...
|192\.168.1.0/24|192\.168.2.4|Consente il traffico verso **onpremsn1** tramite **OPFW**|
...
Per altre informazioni sull'inoltro IP, vedere [Informazioni sulle route definite dall'utente e sull'inoltro IP](./virtual-networks-udr-overview/#ip-forwarding).
...
- **Route**: tutto il traffico verso 10.0.0.0/16 (**azurevnet**) deve essere inviato tramite tunnel **ONPREMAZURE**.
...
- **Route**: tutto il traffico verso 10.0.0.0/16 (**onpremvnet**) deve essere inviato all'indirizzo IP del gateway di Azure (ovvero 10.0.0.1) tramite **port1**.
- **Criteri**: consentire tutto il traffico bidirezionale tra **port1** e **port2**.
