...
Azure crea automaticamente route di sistema e assegna le route a ogni subnet in una rete virtuale. Non è possibile creare route di sistema, né rimuovere route di sistema, ma è possibile sostituire alcune route di sistema con [route personalizzate.](#custom-routes) Azure crea route di sistema predefinite per ogni subnet e aggiunge route [predefinite facoltative](#optional-default-routes) aggiuntive a subnet specifiche o a ogni subnet quando si usano funzionalità di Azure specifiche.
...
* **Nessuno**: il traffico indirizzato al tipo di hop successivo **Nessuno** viene eliminato e non instradato all'esterno della subnet. Azure crea automaticamente route predefinite per i prefissi degli indirizzi seguenti:<br>
    ...
    * **10.0.0.0/8 e 192.168.0.0/16**: riservato per uso privato in RFC 1918.<br>
...
* **ExpressRoute:** è necessario usare BGP per annunciare route locali al router Microsoft Edge.ExpressRoute : You must use BGP to advertise on-premises routes to the Microsoft Edge router. Se si distribuisce un gateway di rete virtuale di tipo ExpressRoute, non è possibile creare route definite dall'utente per forzare il traffico verso il gateway di rete virtuale ExpressRoute. È possibile usare route definite dall'utente per forzare il traffico da ExpressRoute, ad esempio verso un'appliance virtuale di rete.<br>
...
La tabella di route per *Subnet2* contiene tutte le route predefinite create da Azure, il peering di rete virtuale facoltativo e le route facoltative del gateway di rete virtuale. Azure ha aggiunto le route facoltative a tutte le subnet della virtuale di rete quando sono stati aggiunti il gateway e il peering alla rete virtuale. Azure ha rimosso le route per i prefissi di indirizzi 10.0.0.0/8, 192.168.0.0/16 e 100.64.0.0/10 dalla tabella di route *Subnet1* quando la route definita dall'utente per il prefisso di indirizzo 0.0.0.0/0 è stata aggiunta a *Subnet1*.  
