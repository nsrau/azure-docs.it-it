---
title: Distribuire le macchine virtuali con funzionalità Multi-NIC tramite l’interfaccia della riga di comando di Azure in Gestione risorse | Microsoft Docs
...
3. Recuperare l'ID per la subnet `BackEnd` in cui verranno create le macchine virtuali. È necessario farlo poichè le schede di rete associate a questa subnet si trovano in un gruppo di risorse diverso.
   ...
   > [!TIP]
   > Il primo comando precedente usa [grep](http://tldp.org/LDP/Bash-Beginners-Guide/html/sect_04_02.html) e [manipolazione delle stringhe](http://tldp.org/LDP/abs/html/string-manipulation.html) (in particolare, la rimozione di sotto stringhe).
   > 
   > 
