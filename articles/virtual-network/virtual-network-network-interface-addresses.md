...
6. Specificare le impostazioni seguenti, quindi fare clic su **OK** per chiudere il pannello **Aggiungi configurazione IP**:
    ...
    |Metodo di assegnazione di indirizzi IP privati|Sì|Gli indirizzi [**dinamici**](#dynamic) possono cambiare se la macchina virtuale viene riavviata dopo essere stata arrestata (deallocata). Azure assegna un indirizzo disponibile dallo spazio di indirizzi della subnet a cui l'interfaccia di rete è connessa. Gli indirizzi [**statici**](#static) non vengono rilasciati fino a quando non viene eliminata l'interfaccia di rete. Dall'intervallo di indirizzi della subnet specificare un indirizzo IP non attualmente usato da un'altra configurazione IP.|
