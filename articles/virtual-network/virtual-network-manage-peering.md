...
6. <a name="add-peering"></a>Nel pannello **Aggiungi peering** immettere o selezionare i valori per le impostazioni seguenti:
    ...
    - **Consentire il traffico inoltrato:** selezionare questa casella per consentire il traffico *inoltrati* da una rete virtuale (che non hanno origine dalla rete virtuale) al flusso per la rete virtuale tramite un peering. Si consideri ad esempio tre reti virtuali denominate Spoke1, Spoke2 e Hub. Un peering esiste tra ogni rete virtuale spoke e la rete virtuale di Hub, ma non esistono peering tra le reti virtuali spoke. Un dispositivo di rete virtuale viene distribuito nella rete virtuale Hub e le route definite dall'utente vengono applicate a ogni rete virtuale spoke instradare il traffico tra le subnet tramite il dispositivo di rete virtuale. Se questa casella di controllo non è selezionata per il peering tra ogni rete virtuale spoke e la rete virtuale di hub, non traffico tra le reti virtuali spoke perché l'hub è recapitarli il traffico tra le reti virtuali. L'abilitazione di questa funzionalità consente il traffico inoltrato attraverso il peering, tuttavia non crea route definite dall'utente né appliance virtuali di rete. Le route definite dall'utente e le appliance virtuali di rete vengono create separatamente. Informazioni sulle [route definite dall'utente](virtual-networks-udr-overview.md#user-defined).
    - **Consenti transito gateway:** selezionare questa casella se un gateway di rete virtuale è collegato a questa rete virtuale e si vuole consentire il flusso attraverso il gateway del traffico dalla rete virtuale con peering. Questa rete virtuale, ad esempio, potrebbe essere collegata a una rete locale tramite un gateway di rete virtuale. Il gateway può essere un gateway ExpressRoute o VPN. Selezionando questa casella consente il traffico dalla rete virtuale peered a passare attraverso il gateway associato a questa rete virtuale alla rete locale. Se si seleziona questa casella, la rete virtuale con peering non può avere un gateway configurato. La rete virtuale peered deve avere il **usare gateway remoto** casella di controllo selezionata quando si configura il peering dalla rete virtuale a questa rete virtuale. Se si lascia questa casella deselezionata (impostazione predefinita), il traffico dalla rete virtuale con peering viene ancora trasmesso a questa rete virtuale, ma non può essere trasmesso attraverso un gateway di rete virtuale collegato a questa rete virtuale. 
    
    Oltre a inoltrare il traffico a una rete locale, un gateway VPN può inoltrare il traffico di rete tra reti virtuali che sono il peering con la rete virtuale, che il gateway si trova, senza dover eseguire il peering tra loro reti virtuali. Ciò è utile quando si desidera utilizzare un gateway VPN in un hub (vedere l'esempio di hub e spoke descritto per **Consenti inoltrato il traffico**) la rete virtuale per instradare il traffico tra reti virtuali spoke che non è stato effettuato il peering tra loro. Altre informazioni sui [gateway di rete virtuale](../vpn-gateway/vpn-gateway-about-vpngateways.md?toc=%2fazure%2fvirtual-network%2ftoc.json#s2smulti). Questo scenario richiede l'implementazione di route definite dall'utente che specificano il gateway di rete virtuale come il tipo dell'hop successivo. Informazioni sulle [route definite dall'utente](virtual-networks-udr-overview.md#user-defined). È possibile specificare solo un gateway VPN come un tipo di hop successivo in una route definita dall'utente, è possibile specificare un gateway ExpressRoute come il tipo dell'hop successivo in una route definita dall'utente.
...
Solo una per il peering per la rete virtuale può avere questa impostazione è abilitata.
È possibile usare questa impostazione se si dispone già di un gateway configurato nella rete virtuale.
