...
Informazioni su come creare, modificare o eliminare un peering reti virtuali. Il peering reti virtuali consente di connettere due reti virtuali nella stessa località di Azure tramite la rete backbone di Azure. Dopo che è stato eseguito il peering, le due reti virtuali vengono ancora gestite come risorse separate. Le risorse connesse all'una o all'altra delle due reti virtuali comunicano con la stessa latenza e larghezza di banda che userebbero se fossero connesse alla stessa rete virtuale. Se non si ha familiarità con il peering reti virtuali, è consigliabile leggere l'articolo [Panoramica del peering reti virtuali](virtual-network-peering-overview.md) e completare l'esercitazione [Create a virtual network peering](virtual-network-create-peering.md) (Creare un peering reti virtuali), prima di completare le attività di questo articolo.

## <a name="before"></a>Prima di iniziare

Prima di completare i passaggi di qualsiasi sezione di questo articolo, eseguire le attività seguenti:

- Se non si ha familiarità con il peering reti virtuali di Azure, è consigliabile completare l'esercitazione [Create a virtual network peering](virtual-network-create-peering.md) (Creare un peering reti virtuali) prima di leggere questo articolo.
...
## <a name="about-peering"></a>Informazioni sul peering 

Considerare i vincoli seguenti prima di eseguire il peering delle reti virtuali:
...
- Un peering viene stabilito tra due reti virtuali. I peering non sono transitivi. Se si creano peering tra:
    - Rete virtuale 1 e rete virtuale 2
    - Rete virtuale 2 e rete virtuale 3

  Non vengono stabiliti peering tra la rete virtuale 1 e la rete virtuale 3 tramite la rete virtuale 2. Per configurare un peering tra la rete virtuale 1 e la rete virtuale 3, è necessario creare un peering tra la rete virtuale 1 e la rete virtuale 3.
...
- È possibile eseguire il peering di reti virtuali distribuite tramite modelli di distribuzione diversi, che sono nella stessa sottoscrizione. È possibile eseguire il peering di reti virtuali distribuite tramite modelli di distribuzione diversi in sottoscrizioni diverse nella versione di anteprima.
- Le sottoscrizione in cui si trovano entrambe le reti virtuali devono essere associate allo stesso tenant di Azure Active Directory. Se non si ha già un tenant di AD, è possibile [crearne uno](../active-directory/develop/active-directory-howto-tenant.md?toc=%2fazure%2fvirtual-network%2ftoc.json#start-from-scratch) rapidamente. 
- <a name="roles-permissions"></a>L'account deve avere il ruolo o le autorizzazioni necessarie per creare un peering. Se, ad esempio, si deve eseguire il peering di due reti virtuali denominate *Vnet1* e *VNet2*, all'account deve essere assegnato il ruolo o le autorizzazioni minime seguenti per ogni rete virtuale:
...
## <a name="create-peering"></a>Creare un peering

>[!NOTE]
>I requisiti, i vincoli e le considerazioni da tenere presenti per creare correttamente un peering sono diversi. Prima di creare un peering, assicurarsi di avere acquisito familiarità con le considerazioni elencate nella sezione [Informazioni sul peering](#about-peering) di questo articolo.
>

1. Accedere al [portale](https://portal.azure.com) con un account che abbia [ruoli e autorizzazioni](#roles-permissions) necessari.
...
## <a name="change-subnet"></a>Visualizzare o modificare le impostazioni dei peering
...
## <a name="delete-subnet"></a>Eliminare un peering
...
- Creare una topologia di rete hub-spoke|[Più elementi](/azure/architecture/reference-architectures/hybrid-networking/hub-spoke?toc=%2fazure%2fvirtual-network%2ftoc.json#vnet-peering)|
