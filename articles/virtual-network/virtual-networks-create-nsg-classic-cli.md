...
In questo articolo viene illustrato il modello di distribuzione classica. È inoltre possibile [creare gruppi di sicurezza di rete nel modello di distribuzione di Gestione risorse](virtual-networks-create-nsg-arm-cli.md).
...
3. Eseguire il comando **`azure network nsg rule create`** per creare una regola che neghi l'accesso a Internet.
    ...
    Output previsto:
        ...
        data:    Priority                        : 200
        info:    network nsg rule create command OK
4. Eseguire il comando **`azure network nsg subnet add`** per collegare il gruppo di sicurezza di rete alla subnet back-end.
