...
La figura precedente illustra come vengono elaborate le regole del gruppo di sicurezza di rete.
...
Come illustrato dalle regole predefinite seguenti, il traffico che ha origine e termina in una rete virtuale è consentito sia in ingresso che in uscita. Mentre la connettività a Internet è consentita per la direzione in uscita, per impostazione predefinita è bloccata per la direzione in ingresso. È disponibile una regola predefinita per consentire al servizio di bilanciamento del carico di Azure di eseguire il probe dell'integrità delle VM e delle istanze del ruolo. È possibile eseguire l'override di questa regola se non si sta usando un set con carico bilanciato.
...
Per altre informazioni sulla pianificazione della sicurezza di rete in Azure, veder le procedure consigliate in [Servizi cloud Microsoft e sicurezza di rete](../best-practices-network-security.md). 
...
> [!IMPORTANT]
> Assicurarsi di visualizzare tutti i [limiti relativi ai servizi di rete in Azure](../azure-subscription-service-limits.md#networking-limits) prima di progettare la soluzione. Alcun limiti possono essere aumentati aprendo un ticket di supporto.
> 
> 
...
* Gli elenchi di controllo di accesso basati su endpoint e i gruppi di sicurezza di rete non sono supportati nella stessa istanza VM. Se si vuole usare un gruppo di sicurezza di rete ed è già presente un elenco di controllo di accesso basato su endpoint, rimuovere prima l'elenco di controllo di accesso. Per informazioni su come eseguire questa operazione, vedere [Come gestire gli elenchi di controllo di accesso per gli endpoint tramite PowerShell](virtual-networks-acl-powershell.md).
...
### <a name="nsg-for-frontend-subnet"></a>Gruppi di sicurezza di rete per la subnet FrontEnd
**Regole in ingresso**
...
### <a name="nsg-for-backend-subnet"></a>Gruppo di sicurezza di rete per la subnet BackEnd
**Regole in ingresso**
...
### <a name="nsg-for-single-vm-nic-in-frontend-for-rdp-from-internet"></a>Gruppo di sicurezza di rete per VM singola (scheda di interfaccia di rete) in FrontEnd per RDP da Internet
**Regole in ingresso**
...
> [!NOTE]
> Si noti che l'intervallo di indirizzi di origine per questa regola è **Internet** e non l'indirizzo VIP per il servizio di bilanciamento del carico. La porta di origine è **\*** e non 500001. Non confondere le regole NAT/regole di bilanciamento del carico con le regole dei gruppi di sicurezza di rete. Le regole dei gruppi di sicurezza di rete sono sempre correlate all'origine e alla destinazione finale del traffico, **NON** al servizio di bilanciamento del carico tra le due. 
> 
> 

### <a name="nsg-for-management-nics-in-backend"></a>Gruppo di sicurezza di rete per le schede di interfaccia di rete di gestione in BackEnd
**Regole in ingresso**
...
### <a name="nsg-for-database-access-nics-in-back-end"></a>Gruppo di sicurezza di rete per le schede di interfaccia di rete di accesso al database nel back-end
**Regole in ingresso**
