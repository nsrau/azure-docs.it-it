...
Azure DocumentDB supporta l'esecuzione di query su documenti con il familiare linguaggio SQL (Structured Query Language) su documenti JSON gerarchici. DocumentDB è effettivamente privo di schema. Grazie al suo impegno nei confronti del modello di dati JSON direttamente nel motore del database, fornisce l'indicizzazione automatica dei documenti JSON senza richiedere schemi espliciti o la creazione di indici secondari.
Nella progettazione del linguaggio di query per DocumentDB sono stati presi in considerazione due obiettivi:

-   **Adottare SQL**: invece di inventare un nuovo linguaggio di query, si è preferito adottare il linguaggio SQL. In fondo, SQL è uno dei linguaggi di query più familiari e popolari. Il linguaggio SQL di Azure DocumentDB fornisce un modello di programmazione formale per le query complesse sui documenti JSON.
-   **Estendere SQL**: poiché un database di documenti JSON può eseguire JavaScript direttamente nel motore di database, l'obiettivo era usare il modello di programmazione di JavaScript come base per il linguaggio di query SQL. Il linguaggio di query di Azure DocumentDB è radicato nel sistema di tipi, nella valutazione delle espressioni e nella chiamata di funzioni di JavaScript. A sua volta ciò rappresenta un modello di programmazione naturale per le proiezioni relazionali, la navigazione gerarchica attraverso i documenti JSON, i self join, la chiamata di funzioni definite dall'utente (UDF) scritte interamente in JavaScript, tra le altre funzionalità.
...
In questa esercitazione verranno presentate le capacità e la grammatica del linguaggio di query di DocumentDB per mezzo di vari esempi. Verrà inoltre esaminata la possibilità di eseguire query su DocumentDB con le API REST e gli SDK (incluso LINQ).

# Per iniziare

Per osservare il linguaggio SQL di DocumentDB in azione, si inizierà con alcuni semplici documenti JSON e si procederà eseguendo alcune query elementari. Considerare questi due documenti JSON come se riguardassero due famiglie. Da notare che, con DocumentDB, non è necessario creare alcuno schema o indici secondari in maniera esplicita. Basta semplicemente inserire i documenti JSON in una raccolta di DocumentDB ed eseguire successivamente una query.
...
Di seguito è illustrato un secondo documento con una sottile differenza: `givenName` e `familyName` vengono usati invece di `firstName` e `lastName`.
...
A questo punto è possibile provare a eseguire alcune query a fronte di questi dati per comprendere alcuni aspetti chiave del linguaggio SQL di DocumentDB. Ad esempio, la query seguente restituisce i documenti in cui il campo ID corrisponde a `AndersenFamily`. Poiché si tratta di un `SELECT *`, l'output della query è il documento JSON completo:
...
Lo scopo degli indici di database è gestire le query in varie forme con un consumo di risorse ridotto al minimo (ad esempio CPU, I/O), offrendo al contempo una buona velocità effettiva e basse latenze. Spesso, la scelta dell'indice o degli indici giusti per l'interrogazione di un database richiede una lunga pianificazione e sperimentazione. Questo approccio costituisce una sfida per i database senza schema, in cui i dati non si conformano a un rigido schema ed evolvono rapidamente.
...
-   Indicizzazione dei documenti senza richiedere uno schema: il sistema di indicizzazione non richiede alcuna informazione sullo schema né formula alcuna ipotesi a priori sullo schema dei documenti.
...
-   Supporto per la multi-tenancy: Dato il modello basato sulla prenotazione per la governance delle risorse tra tenant, gli aggiornamenti dell'indice vengono eseguiti mantenendosi nel budget delle risorse di sistema (CPU, memoria, IOPS) allocate per ogni replica.
...
Le regole di associazione applicate per ogni query sono le seguenti:

-   È possibile effettuare l'aliasing della raccolta, come in `SELECT f.id FROM Families AS f` o semplicemente `SELECT f.id FROM Families f`. In questo caso, `f` è l'equivalente di `Families`. `AS` è una parola chiave facoltativa per effettuare l'aliasing dell'identificatore.
...
-   Tutte le proprietà che è necessario referenziare devono essere **complete**. In assenza di una rigorosa aderenza allo schema, ciò viene applicato per evitare eventuali associazioni ambigue. Di conseguenza, la sintassi di `SELECT id FROM Families f` non è valida perché la proprietà `id` non è associata.

## Documenti secondari

È anche possibile ridurre il database di origine a un sottoinsieme di dimensioni inferiori. Ad esempio, se si è interessati a enumerare un solo sottoalbero in ogni documento, la sottoradice potrebbe quindi diventare l'origine, come nell'esempio seguente.
...
Se nell'esempio precedente veniva usata una matrice come origine, è anche possibile usare un oggetto come origine, come illustrato nell'esempio seguente. Qualsiasi valore JSON valido (diverso da Undefined) che è disponibile nell'origine verrà considerato per essere incluso nel risultato della query. Se alcune famiglie non hanno un valore `address.state` verranno escluse dal risultato della query.
...
La query seguente richieste documenti che contengano una proprietà nome il cui valore sia `AndersenFamily`. Qualsiasi altro documento che non contiene una proprietà nome o il cui valore non corrisponda a `AndersenFamily` verrà escluso.
...
I seguenti operatori binari sono attualmente supportati e possono essere usati nelle query come illustrato negli esempi riportati di seguito:
...
Oltre agli operatori binari e unari, sono consentiti anche i riferimenti di proprietà. Ad esempio, `SELECT * FROM Families f WHERE f.isRegistered` restituirebbe i documenti JSON contenenti la proprietà `isRegistered` e il valore della proprietà è uguale al valore `true` JSON. Qualsiasi altro valore (false, null, Undefined, <number>, <string>, <object>, <array> ecc.) condurrà all'esclusione del documento di origine dal risultato.

## Operatori di confronto e uguaglianza

Nella tabella seguente è illustrato il risultato dei confronti di uguaglianza nel linguaggio SQL di DocumentDB tra due tipi JSON qualsiasi.
...
Per altri operatori di confronto come \>, \>=, !=, \< e \<=
...
Funzionano con valori booleani. Le tabelle di veridicità logica per questo operatori sono illustrate di seguito.
...
Oltre ai riferimenti di proprietà, la clausola **SELECT** supporta anche le espressioni scalari come le costanti, le espressioni aritmetiche, le espressioni logiche ecc. Ad esempio, ecco una semplice query "Hello World".
...
Ecco un esempio più complesso in cui viene usata un'espressione scalare:
...
Un'altra funzione fondamentale del linguaggio SQL di DocumentDB è la creazione di matrici/oggetti. Nell'esempio precedente si è osservato che è stato creato un nuovo oggetto JSON. In maniera analoga, è possibile creare matrici come illustrato di seguito.
...
L'esempio seguente estende ciò mostrando come restituire valori primitivi JSON, cioè a livello foglia dell'albero JSON.
...
È supportato l'operatore speciale (\*) per proiettare il documento così com'è. Quando usato, deve essere l'unico campo proiettato. Benché una query come `SELECT * FROM Families f` sia valida, `SELECT VALUE * FROM Families f` e `SELECT *, f.id FROM Families f` non lo sono.
...
Al linguaggio SQL di DocumentDB è stato aggiunto un nuovo costrutto con la parola chiave **IN** per fornire supporto all'iterazione nelle matrici JSON. L'origine FROM fornisce supporto per l'iterazione. Esaminare l'esempio seguente:
...
Può essere usato per filtrare ulteriormente ciascuna voce individuale della matrice, come nell'esempio seguente.
...
La sintassi supportata nel linguaggio è <from_source1> JOIN <from_source2> JOIN ... JOIN <from_sourcen>. In generale, verrebbe restituito un set di tuple **N** (tupla con valori **N**). Ciascuna tupla avrà valori prodotti dall'iterazione di tutti gli alias della raccolta sui rispettivi set. In altri termini, si tratta del prodotto incrociato completo dei set che partecipano al join.

Negli esempi riportati di seguito viene illustrato come funzionano i JOIN. Nell'esempio seguente, il risultato è vuoto in quanto il prodotto incrociato di ciascun documento dall'origine e un set vuoto è a sua volta vuoto.
...
Nell'esempio seguente è illustrato un join più convenzionale:
...
Per prima cosa occorre notare che l'origine `from_source` del **JOIN** in questo caso è un iteratore. Dunque, il flusso in questo caso è il seguente:
...
-   Infine, proiettare la proprietà nome dell'oggetto **f** radice del progetto da sola.
...
La vera utilità del JOIN consiste nel formare tuple dal prodotto incrociato in una forma che altrimenti sarebbe difficile proiettare. In più, come si vedrà nell'esempio riportato di seguito, sarebbe possibile filtrare la combinazione di una tupla che consenta all'utente di scegliere una condizione soddisfatta dalle tuple nell'insieme.
...
Questo esempio è un'estensione naturale di quello precedente e illustra l'esecuzione di un doppio join. Il prodotto incrociato può quindi essere visualizzato come lo pseudo-codice seguente.
...
`AndersenFamily` ha un figlio che ha un animale domestico. Il prodotto incrociato genera dunque una riga (1*1*1) da questa famiglia. Tuttavia, la famiglia WakefieldFamily ha due figli, ma un solo figlio, "Jesse", ha animali domestici. Poiché ne ha due, il prodotto incrociato genera dunque 1*1*2 = 2 righe da questa famiglia.
...
-   Capacità di eseguire operazioni CRUD e query transazionali con prestazioni elevate a fronte dei documenti in una raccolta grazie alla stretta integrazione del runtime JavaScript direttamente nel motore di database.
-   Modellazione naturale del flusso di controllo, definizione dell'ambito delle variabili e integrazione di primitivi di gestione delle eccezioni con transazioni di database. Per altri dettagli sul supporto di DocumentDB per l'integrazione di JavaScript, vedere la documentazione relativa alla programmabilità lato server di JavaScript.

## Funzioni definite dall'utente (UDF)

Oltre ai tipi specificati in precedenza, il linguaggio SQL di DocumentDB offre il supporto per le funzioni definite dall'utente (UDF). In particolare, le UDF scalari sono supportate laddove gli sviluppatori possono passare zero o molti argomenti e restituire un unico argomento. Verrà quindi eseguito un controllo per verificare che ciascuno di questi argomenti sia un valore JSON legale.

La grammatica SQL di DocumentDB viene estesa per supportare la logica delle applicazioni personalizzata usando le funzioni definite dall'utente. È possibile registrare le UDF con Azure DocumentDB e quindi referenziarle come parte di una query SQL. In effetti, le UDF sono progettate espressamente per essere richiamate dalle query. Come corollario a questa scelta, le UDF non hanno accesso all'oggetto di contesto a cui possono invece accedere altri tipi di Javascript (stored procedure, trigger). Poiché le query vengono eseguite in sola lettura, è possibile eseguirle sulle repliche primarie o secondarie. Di conseguenza, a differenza di altri tipi di JavaScript, le UDF vengono progettate per l'esecuzione sulle repliche secondarie.
...
DocumentDB SQL non esegue conversioni implicite, a differenza di JavaScript. Ad esempio, una query come `SELECT * FROM Person p WHERE p.Age = 21` corrisponde a documenti che contengono la proprietà Age, il cui valore è 21. Qualsiasi altro documento la cui proprietà Age corrisponda alla stringa "21" o
ad altre possibili variazioni infinite, come "021", "21.0", "0021", "00021" ecc. non troverà corrispondenza.
Ciò avviene in contrasto con JavaScript, che consente di eseguire implicitamente il cast dei valori di stringa al numero (in base all'operatore, ad esempio: ==). Questa scelta è di importanza cruciale per la ricerca di indici corrispondenti nel linguaggio SQL di DocumentDB.
...
Il provider di query di DocumentDB esegue con il massimo impegno un mapping da una query LINQ in una query SQL di DocumentDB. Nella descrizione seguente si suppone che il lettore abbia una certa familiarità con LINQ.
...
-   Espressioni aritmetiche: includono espressioni aritmetiche comuni su valori numerici e booleani. Per ottenere l'elenco completo, vedere la specifica SQL sopra riportata.
...
Gli operatori sopra riportati possono essere composti in modo da formare query più potenti. Poiché DocumentDB supporta le raccolte annidate, tale composizione può essere concatenata o annidata.
...
Gli esempi seguenti illustrano il verbo POST per una query di DocumentDB a fronte di una raccolta contenente i due documenti di esempio esaminati finora. La query ha un semplice filtro sulla proprietà nome JSON. Notare l'uso delle intestazioni `x-ms-documentdb-isquery` e Content-Type: `application/sql` per denotare che l'operazione è una query.
...
Nell'esempio seguente viene illustrato come usare queryDocuments nell'API del server JavaScript per eseguire query dall'interno di stored procedure e trigger.
