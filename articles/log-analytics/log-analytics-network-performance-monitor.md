<properties
    pageTitle="Soluzione di monitoraggio delle prestazioni di rete in OMS | Microsoft Azure"
...
>[AZURE.NOTE] Si tratta di una [soluzione di anteprima](log-analytics-add-solutions.md#log-analytics-preview-solutions-and-features).
...
>[AZURE.NOTE] Lo script EnableRules.ps1 configura le regole del firewall Windows solo nel computer in cui viene eseguito lo script. Se si dispone di un firewall di rete, è necessario assicurarsi che consenta il traffico destinato alla porta TCP usata dal monitoraggio delle prestazioni di rete.
