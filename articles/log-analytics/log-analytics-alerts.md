<properties 
   pageTitle="Avvisi in Log Analytics | Microsoft Azure"
...
![Avvisi di Log Analytics](media/log-analytics-alerts/overview.png)
...
Le azioni runbook avviano il runbook tramite un [webhook](../automation/automation-webhooks.md). Quando si crea la regola di avviso, viene creato automaticamente un nuovo webhook per il runbook con il nome **OMS Alert Remediation** seguito da un GUID. La regola di avviso non invia un payload e non è possibile popolare i parametri del runbook, quindi è necessario usare un runbook che non richiede parametri.

È anche possibile avviare un runbook da un avviso con un'azione di webhook, come descritto nella sezione precedente. Le azioni webhook offrono il vantaggio di fornire un payload per inviare dati al runbook. Il vantaggio delle azioni runbook è dato dal fatto che non è necessario creare il webhook personalmente o comprendere come usarlo.
...
4.	Fare clic su **Salva** per salvare la regola di avviso. Verrà avviata immediatamente l'esecuzione.
...
## Impostazione di intervalli di tempo 
...
I record degli avvisi creati dalle regole di avviso in Log Analytics hanno un **Tipo** di **Avviso** e **SourceSystem** di **OMS**. Includono le proprietà elencate nella tabella seguente.
...
Sono disponibili altri tipi di record di avviso creati dalla [soluzione Gestione avvisi](log-analytics-solution-alert-management.md) e dalle [esportazioni di Power BI](log-analytics-powerbi.md). Questi record hanno un **Tipo** di **Avviso** ma vengono distinti da relativo **SourceSystem**.
