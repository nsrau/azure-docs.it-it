...
[Monitoraggio prestazioni rete](/azure/networking/network-monitoring-overview) è una soluzione di [monitoraggio delle reti ibrida](log-analytics-network-performance-monitor-performance-monitor.md) basata sul cloud che consente di monitorare le prestazioni di rete tra vari punti dell'infrastruttura di rete. Con questa soluzione è anche possibile monitorare la connettività di rete agli [endpoint di servizio e applicazione](log-analytics-network-performance-monitor-service-endpoint.md), nonché [monitorare le prestazioni di Azure ExpressRoute](log-analytics-network-performance-monitor-expressroute.md). 
...
* Eseguire lo script [EnableRules.ps1](https://aka.ms/npmpowershellscript) di PowerShell sul nodo. Per altre informazioni, vedere queste [istruzioni](log-analytics-network-performance-monitor.md#configure-log-analytics-agents-for-monitoring).
...
Monitoraggio prestazioni rete può monitorare la connettività tra reti in qualsiasi parte del mondo, da un'area di lavoro ospitata in una delle [aree supportate](log-analytics-network-performance-monitor.md#supported-regions).
...
Monitoraggio prestazioni rete può monitorare la connettività ai servizi in qualsiasi parte del mondo, da un'area di lavoro ospitata in una delle [aree supportate](log-analytics-network-performance-monitor.md#supported-regions).
...
Questa situazione può verificarsi se le connessioni di peering e di circuito sono distribuite tra più sottoscrizioni. Monitoraggio prestazioni rete individua solo le connessioni di peering private di ExpressRoute in cui le reti virtuali connesse al circuito ExpressRoute sono nella stessa sottoscrizione a cui è collegata l'area di lavoro di Monitoraggio prestazioni rete. Inoltre, Monitoraggio prestazioni rete individua le connessioni di peering Microsoft in cui il circuito ExpressRoute connesso è nella stessa sottoscrizione a cui è collegata l'area di lavoro di Monitoraggio prestazioni rete. Questo concetto può essere chiarito con l'esempio seguente:

 Si supponga di avere due reti virtuali, la rete A nella sottoscrizione A e la rete B nella sottoscrizione B, connesse entrambe a un circuito ExpressRoute nella sottoscrizione C. Si supponga inoltre che esista un'altra rete virtuale C nella sottoscrizione C e che il circuito ExpressRoute abbia una connessione di peering Microsoft nella sottoscrizione C. 

Con questi presupposti,

* Se l'area di lavoro di Monitoraggio prestazioni rete è collegata alla sottoscrizione A, sarà possibile monitorare la connettività tramite ExpressRoute solo alla rete virtuale A.
* Se l'area di lavoro di Monitoraggio prestazioni rete è collegata alla sottoscrizione B, sarà possibile monitorare la connettività tramite ExpressRoute solo alla rete virtuale B.
* Se l'area di lavoro di Monitoraggio prestazioni rete è collegata alla sottoscrizione C, sarà possibile monitorare la connettività tramite ExpressRoute alla rete virtuale C e alla connessione di peering Microsoft.

Il supporto per più sottoscrizioni sarà presto disponibile. Sarà così possibile monitorare tutte le connessioni di peering Microsoft e le connessioni private di ExpressRoute in sottoscrizioni diverse da una stessa area di lavoro.
...
- Altre informazioni su Monitoraggio prestazioni rete sono disponibili in [Soluzione Monitoraggio prestazioni rete in Azure](log-analytics-network-performance-monitor.md).
