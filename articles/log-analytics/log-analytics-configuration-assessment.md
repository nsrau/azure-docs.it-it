...
>[AZURE.NOTE] La soluzione Configuration Assessment non può più essere aggiunta alle aree di lavoro di Log Analytics dal 5 luglio 2016 e non è più disponibile per gli utenti esistenti dal 1° agosto 2016. Per i clienti che usano questa soluzione per SQL Server o Active Directory, è consigliabile usare le soluzioni [SQL Server Assessment](log-analytics-sql-assessment.md), [Active Directory Assessment](log-analytics-ad-assessment.md) e [Stato replica di Active Directory](log-analytics-ad-replication-status.md). Per i clienti che usano Configuration Assessment per Windows, Hyper-V e System Center Virtual Machine Manager, è consigliabile usare le funzionalità di raccolta eventi e rilevamento modifiche per ottenere una visualizzazione completa degli eventuali problemi presenti negli ambienti in uso.
