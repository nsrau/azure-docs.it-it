...
description: Configurare e usare la soluzione DNS Analytics in Log Analytics per raccogliere informazioni dettagliate relative alla sicurezza, alle prestazioni e alle operazioni nell&quot;infrastruttura DNS.
...
Questo articolo descrive come configurare e usare la soluzione DNS Analytics in Log Analytics per raccogliere informazioni dettagliate relative alla sicurezza, alle prestazioni e alle operazioni nell'infrastruttura DNS.
...
| [Gruppo di gestione SCOM](log-analytics-om-agents.md) | Sì | La soluzione raccoglie le informazioni DNS dagli agenti di un gruppo di gestione di SCOM connesso. Non è necessaria una connessione diretta dall'agente SCOM a OMS. I dati vengono inoltrati dal gruppo di gestione al repository OMS. |
...
- Aggiungere la soluzione DNS Analytics all'area di lavoro di OMS da [Azure Marketplace](https://aka.ms/dnsanalyticsazuremarketplace) oppure seguendo la procedura descritta nell'articolo relativo all'[aggiunta di soluzioni di Log Analytics dalla raccolta soluzioni](log-analytics-add-solutions.md).
...
- **Nomi di dominio consentiti**: la soluzione non elabora tutte le query di ricerca, ma gestisce un elenco elementi consentiti per i suffissi dei nomi di dominio. Le query di ricerca che si risolvono nei nomi di dominio corrispondenti ai suffissi di questo elenco non vengono elaborate dalla soluzione. L'esclusione dall'elaborazione dei nomi di dominio consentiti consente di ottimizzare i dati inviati a Log Analytics. L'elenco elementi consentiti predefinito include i nomi di dominio pubblici più diffusi, come www.google.com e www.facebook.com. L'intero elenco predefinito può essere visualizzato usando la barra di scorrimento.

È possibile modificare l'elenco per aggiungere (o rimuovere) qualsiasi suffisso di nome di dominio per cui non si è interessati a visualizzare informazioni dettagliate relative alla ricerca.
...
Se per la connessione all'area di lavoro di OMS si usa Microsoft Monitoring Agent (MMA), viene installato il Management Pack seguente.

- Microsoft DNS Data Collector Intelligence Pack (Microsft.IntelligencePacks.Dns)

Se il gruppo di gestione di SCOM è connesso all'area di lavoro di OMS, quando si aggiunge questa soluzione vengono installati i Management Pack seguenti in SCOM. Per questi Management Pack non è necessaria alcuna configurazione o manutenzione.
...
Dopo aver aggiunto la soluzione DNS Analytics all'area di lavoro, il riquadro della soluzione nella pagina di panoramica di OMS offre un breve riepilogo dell'infrastruttura DNS, che include il numero di server DNS in cui vengono raccolti i dati e il numero di richieste effettuato dai client per risolvere domini dannosi nelle ultime 24 ore. Quando si fa clic sul riquadro, viene aperto il dashboard della soluzione.
...
**Query di DNS Analytics di esempio**: contiene un elenco delle query di ricerca più comuni che recuperano direttamente dati di analisi non elaborati.

![Query di esempio](./media/log-analytics-dns/queries.png)

È possibile usare queste query come punto di partenza per creare le proprie query per l'elaborazione di report personalizzati.

- **Elenco di server:** questo collegamento apre la pagina Ricerca log per DNS contenente un elenco di tutti i server DNS con il nome di dominio completo, il nome del dominio, il nome della foresta e gli indirizzi IP server associati.
- **Elenco di zone DNS:** questo collegamento apre la pagina Ricerca log per DNS contenente un elenco di tutte le zone DNS con il nome della zona, lo stato di aggiornamento dinamico, i server dei nomi e lo stato della firma DNSSEC.
- **Record di risorse inutilizzati:** questo collegamento apre la pagina Ricerca log per DNS e visualizza un elenco di tutti i record di risorse non usati o non aggiornati. L'elenco contiene il nome e il tipo dei record di risorse, il server DNS associato, la data e l'ora di generazione dei record e il nome della zona. È possibile usare questo elenco per identificare i record di risorse DNS non più in uso e quindi intervenire sulla base di tali informazioni per rimuovere tali voci dai server DNS.
- **Carico query server DNS:** questo collegamento apre la pagina Ricerca log per DNS che offre una panoramica del carico DNS nei server DNS in modo da consentire la pianificazione della capacità per i server. È possibile passare alla scheda **Metrica** per ottenere una visualizzazione grafica, che consente di comprendere la distribuzione del carico DNS nei server DNS e mostra le tendenze nella frequenza di query DNS per ogni server.
    ![Ricerca log: risultati relativi alle query nei server DNS](./media/log-analytics-dns/dns-servers-query-load.png)  
- **Carico query zone DNS:** questo collegamento apre la pagina Ricerca log per DNS in cui vengono visualizzate le statistiche al secondo delle query di tutte le zone DNS nei server DNS gestiti dalla soluzione. Fare clic sulla scheda **Metrica** per passare dalla visualizzazione di record dettagliati a una visualizzazione grafica dei risultati.
- **Eventi di configurazione:** questo collegamento apre la pagina Ricerca log per DNS in cui vengono visualizzati tutti gli eventi di modifica della configurazione DNS e i messaggi associati. È quindi possibile filtrare questi eventi in base a data e ora dell'evento, ID evento, server DNS o categoria di attività. Ciò consente di controllare le modifiche apportate a specifici server DNS in date e ore specifiche.
- **Log analitico DNS:** questo collegamento apre la pagina Ricerca log per DNS in cui vengono visualizzati tutti gli eventi di analisi per tutti i server DNS gestiti dalla soluzione. È quindi possibile filtrare questi eventi in base a data e ora dell'evento, ID evento, server DNS, IP client che ha eseguito la query di ricerca e categoria di attività del tipo di query. Gli eventi di analisi dei server DNS consentono il rilevamento delle attività nel server DNS. Viene registrato un evento di analisi ogni volta che il server invia o riceve informazioni DNS.

### <a name="dns-log-search"></a>Ricerca log per DNS

Nella pagina Search è possibile creare una query e quindi, durante la ricerca, filtrare i risultati usando controlli facet. È anche possibile creare query avanzate per trasformare, filtrare e creare report sui risultati. Per iniziare, usare le query seguenti.

Nel campo della query di ricerca digitare `Type=DnsEvents` per visualizzare tutti gli eventi DNS generati dai server DNS gestiti dalla soluzione. Nei risultati sono elencati i dati di log per tutti gli eventi relativi a query di ricerca, registrazioni dinamiche e modifiche di configurazione.

![Ricerca log: eventi DNS](./media/log-analytics-dns/log-search-dnsevents.png)  

- Per visualizzare i dati di log per query di ricerca, applicare il filtro **LookUpQuery** per il sottotipo nel controllo facet sul lato sinistro. Verrà visualizzato un elenco/tabella contenente gli eventi relativi a query di ricerca per il periodo di tempo selezionato.
- Per visualizzare i dati di log per registrazioni dinamiche, applicare il filtro **DynamicRegistration** per il sottotipo nel controllo facet sul lato sinistro. Verrà visualizzato un elenco/tabella contenente tutti gli eventi relativi a registrazioni dinamiche per il periodo di tempo selezionato.
- Per visualizzare i dati di log per modifiche di configurazione, applicare il filtro **ConfigurationChange** per il sottotipo nel controllo facet sul lato sinistro. Verrà visualizzato un elenco/tabella contenente tutti gli eventi relativi a modifiche di configurazione per il periodo di tempo selezionato.

Nel campo della query di ricerca digitare `Type=DnsInventory` per visualizzare tutti i dati relativi all'inventario DNS per i server DNS gestiti dalla soluzione. Nei risultati vengono elencati i dati di log relativi ai server DNS, alle zone DNS e ai record di risorse.
![Ricerca log: inventario DNS](./media/log-analytics-dns/log-search-dnsinventory.png)

## <a name="feedback"></a>Commenti e suggerimenti

È possibile fornire commenti e suggerimenti in diversi modi.

- **UserVoice**: è possibile pubblicare idee sulle funzionalità di DNS Analytics che possono essere migliorate. Visitare la [pagina UserVoice relativa a OMS](https://aka.ms/dnsanalyticsuservoice).
- **Partecipare alla coorte**: Microsoft è sempre interessata a includere nelle coorti nuovi clienti, che otterranno un accesso in anteprima alle nuove funzionalità e contribuiranno al futuro miglioramento di DNS Analytics. Per partecipare, compilare questo [rapido sondaggio](https://aka.ms/dnsanalyticssurvey).
