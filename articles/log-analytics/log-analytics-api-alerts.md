...
# API REST degli avvisi di Log Analytics
...
## Pianificazioni
Una ricerca salvata può avere una o più pianificazioni. La pianificazione definisce la frequenza con cui viene eseguita la ricerca e l'intervallo di tempo in cui vengono identificati i criteri. Le pianificazioni includono le proprietà elencate nella tabella seguente.
...
Utilizzare il metodo Get per recuperare una determinata pianificazione per una ricerca salvata.
...
### Creazione o modifica di una pianificazione
Utilizzare il metodo Put con un ID pianificazione univoco per la ricerca salvata per creare una nuova pianificazione. Se si utilizza un ID pianificazione esistente, tale pianificazione verrà modificata. Quando si crea una pianificazione nella console di OMS, viene creato un GUID per l'ID pianificazione.
...
### Creazione o modifica di azioni
Utilizzare il metodo Put con un ID attività univoco per la pianificazione per creare una nuova attività. Se si usa un ID attività esistente, l’attività verrà modificata. Quando si crea un'attività nella console di OMS, un GUID è destinato all’ID attività.

Il formato della richiesta per la creazione di una nuova attività varia in base al tipo di attività e alcuni esempi sono riportati nelle sezioni seguenti.
...
Utilizzare il metodo Put con l'ID azione per creare una nuova azione di soglia per una pianificazione. Se si utilizza un nuovo ID per l'azione, verrà creata una nuova azione. Se si utilizza un ID già esistente per la pianificazione, tale azione verrà modificata.
...
Utilizzare il metodo Put con l'ID azione per creare una nuova azione di posta elettronica per una pianificazione. Se si utilizza un nuovo ID per l'azione, verrà creata una nuova azione. Se si utilizza un ID già esistente per la pianificazione, tale azione verrà modificata.

Nell'esempio seguente viene creata una notifica tramite posta elettronica con una determinata soglia, per cui il messaggio viene inviato quando i risultati della ricerca salvata superano la soglia.
...
Utilizzare il metodo Put con l'ID azione per creare una nuova azione correttiva per una pianificazione. Se si utilizza un nuovo ID per l'azione, verrà creata una nuova azione. Se si utilizza un ID già esistente per la pianificazione, tale azione verrà modificata.

Nell'esempio seguente viene creata una correzione con una determinata soglia, per cui il runbook viene avviato quando i risultati della ricerca salvata superano la soglia.
...
#### Creazione o modifica di un’azione webhook
Utilizzare il metodo Put con l'ID azione per creare una nuova azione webhook per una pianificazione. Se si utilizza un nuovo ID per l'azione, verrà creata una nuova azione. Se si utilizza un ID già esistente per la pianificazione, tale azione verrà modificata. Nell'esempio seguente viene creata un'azione webhook e un’azione di avviso con una determinata soglia, in modo che il webhook verrà attivato quando i risultati della ricerca salvata supereranno tale soglia.
