...
La configurazione del proxy può essere impostata nel file: `/etc/opt/microsoft/omsagent/proxy.conf`. Questo file può essere creato o modificato direttamente, ma le autorizzazioni devono essere aggiornate per concedere al gruppo omiuser l'autorizzazione di lettura per il file. ad esempio:
```
...
2. Verificare di avere installato la versione più recente dell'agente OMS per Linux.  Ora la versione più recente notifica l'utente se la differenza di tempo causa l'errore di onboarding.
