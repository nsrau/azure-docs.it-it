 ...
 * Fare clic con il pulsante destro del mouse in Esplora soluzioni e scegliere Aggiungi Application Insights. Informazioni dettagliate dei passaggi che includono la creazione di una risorsa di Application Insights. [Altre informazioni](app-insights-start-monitoring-app-health-usage.md)
2. Aggiungere il pacchetto di Listener di traccia al progetto.
 * In Visual Studio fare clic con il pulsante destro del mouse sul progetto e scegliere Gestisci pacchetti NuGet. Selezionare `Microsoft.ApplicationInsights.TraceListener` [ulteriori](app-insights-asp-net-trace-logs.md)
3. Caricare il progetto ed eseguire la generazione di dati del log.
4. Nel [portale Azure preview](http://portal.azure.com/), individuare la nuova risorsa di Application Insights e aprire **ricerca**. Si noterà che i dati di registro, insieme a richiesta, l'utilizzo e altri dati di telemetria. Alcuni dati di telemetria potrebbe richiedere alcuni minuti per arrivare: fare clic su Aggiorna. [Ulteriori informazioni](app-insights-diagnostic-search.md)
