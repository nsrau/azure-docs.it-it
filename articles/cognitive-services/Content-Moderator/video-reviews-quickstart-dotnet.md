...
Aggiungere le proprietà private seguenti allo spazio dei nomi **VideoReviews**, Class **Program**. Aggiornare i `AzureEndpoint` `CMSubscriptionKey` campi e con i valori dell'URL dell'endpoint e della chiave di sottoscrizione. È possibile trovarli nella scheda **avvio rapido** della risorsa nel portale di Azure.
...
- **Metadati**. Oggetto IList\<VideoFrameBodyItemMetadataItem>. **VideoFrameBodyItemMetadataItem** è semplicemente una coppia chiave/valore. Le chiavi valide includono:
