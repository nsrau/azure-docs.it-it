...
La funzione `addProducts()` crea un collegamento per ognuno dei risultati di shopping restituiti eseguendo l'iterazione dell'elenco delle azioni `Products` e accodando un tag `<img>` all'elemento `<div>` esterno per ognuno.
