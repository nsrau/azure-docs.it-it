...
È possibile eseguire questa guida introduttiva come Jupyter Notebook in [MyBinder](https://mybinder.org). Per avviare Binder, selezionare il pulsante seguente:
...
1. Modificare il valore di `face_api_url` impostando l'indirizzo in cui si sono ottenute le chiavi di sottoscrizione, se necessario.
...
Il codice seguente usa la libreria `requests` di Python per chiamare l'API di rilevamento del viso. I risultati vengono restituiti come oggetto JSON. La chiave API viene passata tramite il dizionario `headers`. I tipi di caratteristiche da riconoscere vengono passati tramite il dizionario `params`.
