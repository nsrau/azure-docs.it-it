...
Quando si usa l'API [Rilevamento viso] , è possibile assegnare la versione del modello con il parametro `detectionModel`. I valori disponibili sono:
...
Questo codice crea un **gruppo** con ID `mypersongroupid` e vi aggiunge una **persona** . Quindi aggiunge un volto a questa **persona** usando il modello di `detection_02`. Se non si specifica il parametro *detectionModel* , l'API utilizzerà il modello predefinito `detection_01`.
...
Questo codice crea un oggetto **facet** denominato `My face collection` e ne aggiunge una faccia con il modello di `detection_02`. Se non si specifica il parametro *detectionModel* , l'API utilizzerà il modello predefinito `detection_01`.
...
Il modo migliore per confrontare le prestazioni dei modelli `detection_01` e `detection_02` consiste nell'utilizzarli in un set di dati di esempio. È consigliabile chiamare l'API di [rilevamento viso] su un'ampia gamma di immagini, in particolare immagini di molti visi o di visi difficili da visualizzare, usando ogni modello di rilevamento. Prestare attenzione al numero di visi restituiti da ogni modello.
