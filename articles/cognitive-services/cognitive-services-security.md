...
Per impostare le variabili di ambiente, usare uno dei comandi seguenti, `ENVIRONMENT_VARIABLE_KEY` dove è la chiave denominata `value` e è il valore archiviato nella variabile di ambiente.
...
Per ottenere una variabile di ambiente, è necessario leggerla in memoria. A seconda del linguaggio usato, prendere in considerazione i frammenti di codice seguenti. Questi frammenti di codice illustrano come ottenere la `ENVIRONMENT_VARIABLE_KEY` variabile di ambiente in base a e assegnarla `value`a una variabile denominata.
