...
Il riconoscimento vocale personalizzato USA sia **modelli di base** che **modelli personalizzati**. Ogni lingua dispone di uno o più **modelli di base**. In genere, quando un nuovo modello di riconoscimento vocale viene rilasciato al servizio di riconoscimento vocale normale, viene anche importato nel servizio Riconoscimento vocale personalizzato come nuovo **modello di base**. Vengono aggiornati ogni 3-6 mesi e i modelli meno recenti diventano in genere meno utili nel corso del tempo, perché il modello più recente ha in genere un'accuratezza notevolmente superiore.

Al contrario, i **modelli personalizzati** vengono creati adattando un modello di base scelto a uno scenario specifico del cliente. È possibile utilizzare un modello personalizzato specifico per un lungo periodo di tempo dopo che è stato raggiunto un modello che soddisfi le proprie esigenze o per mantenerne il training nel tempo con dati aggiuntivi. 
...
Le modifiche apportate ai modelli di base per aggiungere nuove funzionalità e migliorare le prestazioni possono causare problemi con la compatibilità con le versioni precedenti e anche le modifiche apportate all'accuratezza osservata con un modello specifico in un determinato set di dati di test. Per gestire il lavoro richiesto per la gestione di modelli ed endpoint, vedere le seguenti righe temporali per le sequenze temporali di scadenza del modello e dell'endpoint.
