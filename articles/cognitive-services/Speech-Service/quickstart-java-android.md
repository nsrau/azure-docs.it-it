...
Questo articolo spiega come sviluppare un'applicazione Java per Android usando Speech SDK di Servizi cognitivi di Azure per la trascrizione del riconoscimento vocale.

L'applicazione è basata sul pacchetto Maven di Speech SDK e su Android Studio 3.3. Speech SDK è attualmente compatibile con i dispositivi Android con processori ARM a 32 o 64 bit e compatibili con Intel x86/x64.
...
Per completare questa guida di avvio rapido, è necessaria una chiave di sottoscrizione di servizi Voce. È possibile ottenerne una gratuitamente. Per altre informazioni, vedere [Provare gratuitamente i servizi Voce](get-started.md).
...
![Interfaccia utente](media/sdk/qs-java-android-11-gui.png)
...
1. Sostituire inoltre la stringa `YourServiceRegion` con l'[area](regions.md) associata alla sottoscrizione. Ad esempio, usare `westus` per la sottoscrizione di valutazione gratuita.
