...
Sono supportate lingue diverse a seconda delle funzioni dei servizi vocali. Le tabelle seguenti riepilogano questo supporto della lingua.
...
L'API Riconoscimento vocale Microsoft supporta le lingue seguenti. Livelli diversi di personalizzazione sono disponibili per ogni lingua.
 ...
 en-AU | Inglese (Australia) | Yes | Yes | Yes
 en-CA | Inglese (Canada) | Yes | Yes | Yes
 en-GB | Inglese (Regno Unito) | Yes | Yes | Yes
 en-IN | Inglese (India) | No  | Yes | Yes
 en-NZ | Inglese (Nuova Zelanda) | Yes | Yes | Yes  
 it-IT | Inglese (Stati Uniti) | Yes | Yes | Yes
 es-ES | Spagnolo (Spagna) | Yes | Yes | No 
 es-MX | Spagnolo (Messico) | Yes | Yes | No 
 fi-FI | Finlandese (Finlandia) | No  | No  | No 
 fr-CA | Francese (Canada) | Yes | Yes | No 
...
te-IN | Telugu (India) |Femmina | "Microsoft Server Speech Text to Speech Voice (tr-TR, Chitra)"
...
È possibile ottenere l'elenco delle lingue a livello di codice usando l'endpoint Languages dell'API REST. L'elenco fornisce il codice di ogni lingua e il nome della lingua in inglese o in qualsiasi altra lingua supportata. L'elenco viene aggiornato dal servizio Microsoft Translator quando viene resa disponibile una nuova lingua.
