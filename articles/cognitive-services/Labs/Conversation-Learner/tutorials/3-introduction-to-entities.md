---
title: Come usare le entità con un'applicazione di Conversation Learner - Servizi cognitivi Microsoft| Microsoft Docs
titleSuffix: Azure
description: Informazioni su come usare le entità con un'applicazione di Conversation Learner.
...
Si noti che si tratta di un'entità di tipo "custom" (personalizzato) ed è quindi possibile eseguirne il training.  Sono disponibili anche entità predefinite il cui comportamento non può essere modificato. Queste entità sono illustrate in un'altra esercitazione.
...
4. In Required Entities (Entità obbligatorie) si noti che l'entità city è stata aggiunta automaticamente perché vi viene fatto riferimento.
...
3. Fare clic su Score Actions (Punteggio azioni) e selezionare "I don't know what city you want".
    - Si noti che la risposta in cui l'entità city è obbligatoria non può essere selezionata perché l'entità city non è definita nella memoria del bot.
...
5. Fare clic su Score Actions (Punteggio azioni).
    - Si noti che il valore di city è ora presente nella memoria del bot.
...
1. Digitare e premere INVIO. Si noti che l'entità city e il relativo valore sono presenti in memoria e disponibili.
