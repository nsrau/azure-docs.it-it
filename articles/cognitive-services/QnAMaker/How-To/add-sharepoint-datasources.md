...
È possibile aggiungere tutte supportate QnA Maker [tipi di file](../Concepts/data-sources-supported.md) da un sito di SharePoint per l'articolo della knowledge base. Potrebbe essere necessario concedere [autorizzazioni](#permissions) se la risorsa del file è protetto.

1. La libreria con il sito di SharePoint, selezionare il menu di puntini di sospensione del file, `...`.
...
### <a name="images-with-sharepoint-files"></a>Immagini con i file di SharePoint

Se i file includono immagini, quelli non vengono estratti. È possibile aggiungere l'immagine dal portale di QnA Maker, dopo che il file viene estratto in coppie di domande e risposte.

Aggiungere l'immagine con la sintassi di markdown seguente: 

```markdown
![Explanation or description of image](URL of public image)
```

Il testo nelle parentesi quadre, `[]`, illustra l'immagine. L'URL tra parentesi, `()`, il collegamento diretto all'immagine. 

Quando si testa la coppia di domande e risposte nel Pannello di test interattiva, nel portale di QnA Maker, viene visualizzata l'immagine, anziché il testo di markdown. Questo modo si convalida l'immagine può essere recuperato pubblicamente dall'applicazione client.

## <a name="permissions"></a>Autorizzazioni

Concessione di autorizzazioni, si verifica quando viene aggiunto un file protetto da un sito di SharePoint a una knowledge base. A seconda della configurazione di SharePoint remota e le autorizzazioni della persona aggiunta del file, ciò potrebbe implicare:
...
Dopo il gestore di QnA Maker consente di selezionare l'account, l'amministratore di Azure Active Directory riceverà una notifica che è necessario consentire QnA Maker app (non gestione QnA Maker) di accedere alla risorsa di SharePoint. La gestione di Azure Active Directory sarà necessario eseguire questa operazione per tutte le risorse di SharePoint, ma non tutti i documenti in tale risorsa. 
