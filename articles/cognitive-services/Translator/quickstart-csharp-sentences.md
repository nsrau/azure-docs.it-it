...
Nella classe `Program` creare una funzione denominata `BreakSentence`. Questa classe incapsula il codice usato per chiamare la risorsa BreakSentence e stampa i risultati nella console.
...
Aggiungere le righe seguenti alla funzione `BreakSentence`. Si noterà che, oltre a `api-version`, è possibile definire la lingua di input. In questo esempio la lingua impostata è quella inglese.
...
L'ultimo passaggio consiste nel chiamare `BreakSentence()` nella funzione `Main`. Individuare `static void Main(string[] args)` e aggiungere le righe seguenti:
