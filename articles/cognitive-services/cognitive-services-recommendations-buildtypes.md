---
title: "Tipi di compilazione e qualità dei modelli: API Raccomandazioni | Documentazione Microsoft"
...
---
# <a name="build-types-and-model-quality"></a>Tipi di compilazione e qualità dei modelli
<a name="TypeofBuilds"></a>

## <a name="supported-build-types"></a>Tipi di compilazione supportati
L'API Raccomandazioni supporta due tipi di compilazione: *raccomandazione* e *FBT*. Ogni tipo viene compilato usando algoritmi diversi e presenta vantaggi differenti. Questo documento descrive i tipi di compilazione e le tecniche per confrontare la qualità dei modelli generati.
...
### <a name="recommendation-build-type"></a>Tipo di compilazione di elementi consigliati
...
**La compilazione di raccomandazioni supporta il posizionamento di *elementi ad accesso sporadico* **
...
#### <a name="recommendations-build-parameters"></a>Parametri di compilazione di elementi consigliati
...
### <a name="fbt-build-type"></a>Tipo di compilazione FBT
La compilazione FBT (Frequently Bought Together) esegue un'analisi che conta il numero di volte in cui due o tre prodotti diversi ricorrono insieme. Ordina quindi i set in base a una funzione di somiglianza, ad esempio **co-occorrenze**, **Jaccard** e **accuratezza**.

**Jaccard** e **accuratezza** sono modi per normalizzare le co-occorrenze.  Ciò significa che gli elementi verranno restituiti solo se sono stati acquistati con l'elemento seme.
...
#### <a name="fbt-build-parameters"></a>Parametri della compilazione FBT
| Nome | Descrizione | Tipo,   <br> valori validi, <br>  (valore predefinito) |
| --- | --- | --- |
| *FbtSupportThreshold* |Indica il livello conservativo del modello. Numero di co-occorrenze di elementi da considerare per la creazione del modello. |Integer,  <br> 3-50 <br>  (6) |
| *FbtMaxItemSetSize* |Limita il numero di elementi in un set frequente. |Integer    <br> 2-3 <br>  (2) |
| *FbtMinimalScore* |Punteggio minimo che un set frequente deve avere per essere incluso nei risultati restituiti. Più alto è il valore, migliori saranno i risultati. |Double <br> 0 e oltre <br>  (0) |
| *FbtSimilarityFunction* |Definisce la funzione di somiglianza da usare per la compilazione. L'**accuratezza** predilige la casualità, la **co-occorrenza** predilige la prevedibilità e la funzione **Jaccard** rappresenta un compromesso tra la prime due. |Stringa,   <br>  <i>cooccurrence, lift, jaccard</i><br> Valore predefinito: <i>jaccard</i> |

<a name="SelectBuild"></a>

## <a name="build-evaluation-and-selection"></a>Valutazione e selezione della compilazione
...
L'obiettivo di una valutazione offline consiste nel prevedere la precisione, ovvero il numero di utenti che acquisterà uno degli elementi consigliati, e la diversità degli elementi consigliati, ovvero il numero di elementi consigliati.
Nell'ambito della valutazione delle metriche di precisione e diversità il sistema trova un campione di utenti e suddivide le relative transazioni in due gruppi: il set di dati di training e il set di dati di test.

> [!NOTE]
> Per usare le metriche offline è necessario avere il timestamp nei dati di utilizzo.
> I dati temporali sono infatti necessari per suddividere correttamente l'utilizzo tra i set di dati di training e quello di test.
> 
...
### <a name="precision-at-k"></a>Precisione su K elementi
...
#### <a name="users-in-test"></a>Utenti nel test
...
#### <a name="percentile-buckets"></a>Bucket percentili
...
#### <a name="total-items-recommended"></a>Totale elementi consigliati
La metrica totale elementi consigliati indica il numero di elementi consigliati. Alcuni potrebbero essere duplicati.

<a name="ImplementingEvaluation"></a>

### <a name="offline-evaluation-metrics"></a>Metriche di valutazione offline
...
* È possibile selezionare facoltativamente il parametro *splitterStrategy* (*RandomSplitter* o *LastEventSplitter*).
  *RandomSplitter* suddivide i dati di utilizzo in set di training e set di test basati sui valori percentuali e di seme casuali *randomSplitterParameters* specificati per il test.
  *LastEventSplitter* suddivide i dati di utilizzo in set di training e set di test in base all'ultima transazione per ogni utente.

Verrà attivata una compilazione che usa solo un subset dei dati per il training, mentre il resto dei dati viene usato per calcolare le metriche di valutazione.  Dopo aver completato la compilazione, per ottenere l'output della valutazione è sufficiente chiamare l'[API Get build metrics](https://westus.dev.cognitive.microsoft.com/docs/services/Recommendations.V4.0/operations/577eaa75eda565095421666f) passando i rispettivi oggetti *modelId* e *buildId*.
