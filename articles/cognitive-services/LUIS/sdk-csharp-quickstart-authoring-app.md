...
Introduzione alla libreria client di creazione di Language Understanding (LUIS) per .NET. Seguire questi passaggi per installare il pacchetto e provare il codice di esempio per le attività di base.  Language Understanding (LUIS) consente di applicare l'intelligence dell'apprendimento automatico personalizzato al testo in linguaggio naturale parlato di un utente per prevedere il significato generale ed estrarre informazioni pertinenti e dettagliate. 
...
### <a name="get-your-language-understanding-luis-authoring-key"></a>Ottenere la chiave di creazione per Language Understanding (LUIS)

Ottenere la [chiave di creazione](luis-how-to-account-settings.md) e [creare una variabile di ambiente](https://docs.microsoft.com/azure/cognitive-services/cognitive-services-apis-create-account#configure-an-environment-variable-for-authentication) per la chiave denominata `COGNITIVESERVICE_AUTHORING_KEY`.
