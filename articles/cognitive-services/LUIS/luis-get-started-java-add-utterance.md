---
title: "Esercitazione: Come aggiungere espressioni a un'app di Language Understanding con Java | Microsoft Docs"
...
ms.locfileid: "36266040"
---
# <a name="tutorial-add-utterances-to-app-using-java"></a>Esercitazione: Aggiungere espressioni all'app con Java 
In questa esercitazione si scrive un programma per aggiungere un'espressione a una finalità usando le API di creazione in Java.
...
* Versione più recente di Oracle [**Java/JDK**](http://www.oracle.com/technetwork/java/javase/downloads/index.html).
...
* [**ID dell'applicazione**](./luis-get-started-create-app.md) di Language Understanding esistente. L'ID applicazione viene visualizzato nel dashboard delle applicazioni. L'applicazione di Language Understanding con le finalità e le entità usate nel file `utterances.json` deve esistere prima di eseguire il codice in `AddUtterances.java`. Il codice riportato in questo articolo non crea le finalità e le entità. Aggiunge solo le espressioni per le entità e le finalità esistenti. 
* **ID della versione** all'interno dell'applicazione che riceve le espressioni. L'ID predefinito è "0.1"
* Creare un file di testo denominato `AddUtterances.java`.

> [!NOTE] 
> Il file `add-utterances.cs` completo e un file `utterances.json` di esempio sono disponibili nel [repository GitHub **LUIS-Samples**](https://github.com/Microsoft/LUIS-Samples/tree/master/documentation-samples/authoring-api-samples/java).


## <a name="write-the-java-code"></a>Scrivere il codice Java

Aggiungere le dipendenze Java al file.
...
Creare la classe `AddUtterances`
...
Questa classe contiene tutti i frammenti di codice che seguono.

Aggiungere le costanti di Language Understanding alla classe. Copiare il codice seguente e immettere la propria chiave di creazione, insieme all'ID applicazione e all'ID versione.
...
Aggiungere il metodo per chiamare l'API di Language Understanding. 
...
Aggiungere il metodo per la risposta HTTP dall'API di Language Understanding.
...
Aggiungere la gestione di output/stampa.
...
> [!NOTE]
> L'applicazione di Language Understanding con le finalità e le entità usate nel file `utterances.json` deve esistere prima di eseguire il codice in `AddUtterances.java`. Il codice riportato in questo articolo non crea le finalità e le entità. Aggiunge solo le espressioni per le entità e le finalità esistenti.
...
Quando si chiama l'app con l'argomento `-train`, viene creato un file `training-results.json` che indica che la richiesta di training dell'app di Language Understanding è stata inserita in coda. 
...
Chiamare l'app con l'argomento `-status` per verificare lo stato del training e scrivere i dettagli dello stato in un file.
