...
# <a name="tutorial-for-custom-decision-service"></a>Tutorial for Custom Decision Service
...
## <a name="prepare-action-set-feeds"></a>Prepare action set feeds

Custom Decision Service consumes action sets via RSS or Atom feeds provided by the customer. In our scenario, you need to provide two feeds: one for politics and one for sports. For concreteness, suppose they are served from `http://www.domain.com/feeds/<feed-name>`.
...
## <a name="register-a-new-app"></a>Register a new app
...
## <a name="use-the-performance-dashboard"></a>Use the performance dashboard
