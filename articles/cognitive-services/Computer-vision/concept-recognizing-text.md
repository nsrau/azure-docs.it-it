---
title: Riconoscimento di testo stampato e scritto a mano - Visione artificiale
titleSuffix: Azure Cognitive Services
description: Concetti relativi al riconoscimento di testo stampato e scritto a mano presente nelle immagini tramite l'API Visione artificiale.
services: cognitive-services
author: deken
manager: cgronlun
ms.service: cognitive-services
ms.component: computer-vision
ms.topic: conceptual
ms.date: 08/29/2018
ms.author: v-deken
ms.openlocfilehash: 49cba0e9b6958beb07b6f074e6dc748679514525
ms.sourcegitcommit: 776b450b73db66469cb63130c6cf9696f9152b6a
ms.translationtype: HT
ms.contentlocale: it-IT
ms.lasthandoff: 09/18/2018
ms.locfileid: "45985312"
---
# <a name="recognizing-printed-and-handwritten-text"></a>Riconoscimento di testo stampato e scritto a mano

Visione artificiale può rilevare ed estrarre testo stampato o scritto a mano dalle immagini di diversi oggetti con superfici e sfondi diversi, ad esempio ricette, poster, biglietti da visita, lettere e lavagne.

Il riconoscimento di testo consente di risparmiare tempo e fatica. Se si acquisisce il testo da un'immagine invece di trascriverlo, si può migliorare la produttività. Il riconoscimento di testo rende anche possibile la digitalizzazione delle note e di conseguenza l'implementazione di una ricerca semplice e rapida e la riduzione della quantità di carta usata.

## <a name="text-recognition-requirements"></a>Requisiti per il riconoscimento di testo

Visione artificiale può riconoscere testo stampato e scritto a mano nelle immagini che soddisfano i requisiti seguenti:

- L'immagine deve essere presentata in formato JPEG, PNG, o BMP.
- La dimensione del file dell'immagine deve essere inferiore a 4 megabyte (MB).
- Le dimensioni dell'immagine devono essere comprese tra 40x40 e 3200x3200 pixel.

> [!NOTE]
> Questa tecnologia è attualmente disponibile in anteprima e solo per testo in lingua inglese.

## <a name="next-steps"></a>Passaggi successivi

Concetti relativi all'[estrazione di testo con OCR](concept-extracting-text-ocr.md).