...
description: Quando si invia una richiesta di ricerca a Ricerca Web Bing, viene restituito un oggetto `SearchResponse` nel corpo della risposta.
...
A seconda del dispositivo usato dall'utente, viene in genere visualizzato un subset delle anteprime con un'opzione che consente di [spostarsi tra le pagine](./paging-search-results.md) delle immagini rimanenti.
...
Se si immette un'espressione matematica o una query di conversione unità, la risposta può contenere una risposta [Computation](/rest/api/cognitiveservices-bingsearch/bing-web-api-v7-reference#computation). La risposta `computation` contiene l'espressione normalizzata e il relativo risultato.
...
Se la query richiede l'ora di uno stato o paese/area geografica, Bing determina la città primaria entro il confine geografico della località e la restituisce nel `primaryCityTime` campo. Se il confine contiene più fusi orari, i fusi orari rimanenti vengono restituiti nel campo `otherCityTimes`.
...
Modificare quindi l'endpoint API Ricerca Web Bing nel file HTML in: \
