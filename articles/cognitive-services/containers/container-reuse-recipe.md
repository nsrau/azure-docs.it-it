...
Tutti gli `docker run` esempi in questo documento presuppongono una console di Windows con un `^` carattere di continuazione di riga. Tenere presente quanto segue per uso personale:
...
* `docker run`gli esempi usano la directory dall' `c:` unità per evitare eventuali conflitti di autorizzazione in Windows. Se è necessario usare una directory specifica come directory di input, potrebbe essere necessario concedere l'autorizzazione per il servizio Docker.
...
* Se il servizio cognitivo non richiede montaggi di input, non aggiungere le `COPY` righe alla Dockerfile.
...
* Imposta l'endpoint di fatturazione `{BILLING_ENDPOINT}` dalla chiave dell'ambiente dell'host tramite `ENV` .
...
* Copia il file del modello di Language Understanding (LUIS) dal file system dell'host usando `COPY` .
* Il contenitore LUIS supporta più di un modello. Se tutti i modelli sono archiviati nella stessa cartella, è necessario disporre di un' `COPY` istruzione.
* Eseguire il file Docker dall'elemento padre relativo della directory di input del modello. Per l'esempio seguente, eseguire i `docker build` `docker run` comandi e dal padre relativo di `/input` . Il primo del `/input` `COPY` comando è la directory del computer host. Il secondo `/input` è la directory del contenitore.
...
1. Creare un oggetto `Dockerfile` con il testo dalla ricetta per il riutilizzo. Un oggetto `Dockerfile` non ha un'estensione.
