---
title: Guida introduttiva all'API Ricerca entità per C# | Microsoft Docs
...
---
# <a name="entity-search-sdk-c-quickstart"></a>Guida introduttiva all'SDK di Ricerca entità per C#

L'API Ricerca entità Bing contiene la funzionalità dell'API REST per query di entità e analisi dei risultati.

Il [codice sorgente per gli esempi dell'SDK di Ricerca entità Bing per C#](https://github.com/Azure-Samples/cognitive-services-dotnet-sdk-samples/tree/master/BingSearchv7/BingEntitySearch) è disponibile su GitHub.

## <a name="application-dependencies"></a>Dipendenze dell'applicazione

Per configurare un'applicazione console tramite l'SDK di Ricerca entità Bing, passare all'opzione `Manage NuGet Packages` da Esplora soluzioni in Visual Studio.  Aggiungere il pacchetto `Microsoft.Azure.CognitiveServices.Search.EntitySearch`.

L'installazione del [pacchetto di Ricerca entità NuGet](https://www.nuget.org/packages/Microsoft.Azure.CognitiveServices.Search.EntitySearch/1.2.0) comporta anche l'installazione di dipendenze, inclusi gli assembly seguenti:
* Microsoft.Rest.ClientRuntime
* Microsoft.Rest.ClientRuntime.Azure
* Newtonsoft.Json

## <a name="entity-search-client"></a>Client di Ricerca entità
Per creare un'istanza del client `EntitySearchAPI`, aggiungere le direttive using:
```
...
L'applicazione console seguente cerca una singola entità in base alla query "Satya Nadella" e visualizza una breve descrizione.
```
using System;
using System.Linq;
using System.Text;
...
## <a name="ambiguous-results"></a>Risultati ambigui
Il codice seguente cerca e gestisce la disambiguazione dei risultati per una query ambigua "William Gates".
```
...
## <a name="entitydata-places"></a>Località EntityData
Il codice seguente cerca un singolo store "Microsoft Store" e visualizza il numero di telefono.
```
...
```
## <a name="entityscenario-list"></a>Elenco EntityScenario
Il codice seguente cerca un elenco di "Seattle restaurants" e visualizza il nome e il numero di telefono.
```
...
```
## <a name="error-results"></a>Risultati dell'errore
Il codice seguente attiva una richiesta non valida e mostra come leggere la risposta di errore.
```
