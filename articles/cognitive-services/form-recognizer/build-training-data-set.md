...
Se si aggiunge il contenuto seguente al corpo della richiesta, l'API eseguirà il training con i documenti presenti nelle sottocartelle. Il campo `"prefix"` è facoltativo e il set di dati di training viene limitato ai file i cui percorsi iniziano con la stringa specificata. Il valore `"Test"`, ad esempio, fa sì che l'API esamini solo i file o le cartelle che iniziano con la parola "test".
