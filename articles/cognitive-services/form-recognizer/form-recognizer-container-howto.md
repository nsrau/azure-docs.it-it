...
Usare il comando [docker run](https://docs.docker.com/engine/reference/commandline/run/) per eseguire il contenitore. Per informazioni dettagliate su come ottenere `{COMPUTER_VISION_ENDPOINT_URI}`i `{COMPUTER_VISION_API_KEY}`valori, `{FORM_RECOGNIZER_ENDPOINT_URI}` e `{FORM_RECOGNIZER_API_KEY}` , vedere raccolta dei [parametri obbligatori](#gathering-required-parameters) .

[Examples](form-recognizer-container-configuration.md#example-docker-run-commands) Sono disponibili esempi `docker run` di comando.
