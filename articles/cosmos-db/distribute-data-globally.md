...
Guardare il video seguente per vedere la funzionalità di distribuzione globale chiavi in mano di Azure Cosmos DB in azione.
...
**Garanzie di coerenza associate ai modelli di coerenza specifici in Azure Cosmos DB**
...
**Relazione della capacità di lettura per un modello di coerenza specifico in Azure Cosmos DB** 
...
**Contenitore di un cliente con provisioning elastico della velocità effettiva (compresa tra 1 milione e 10 milioni di richieste al secondo)**
...
* Per implementare la replica globale nell'account Azure Cosmos DB tramite il portale di Azure, vedere [Come eseguire la replica del database globale di Azure Cosmos DB con il portale di Azure](tutorial-global-distribution-sql-api.md).
