...
**Crea app attive-attive globali.** Con il suo nuovo protocollo di replica multimaster, ogni area supporta sia le scritture che le letture. La funzionalità multimaster consente inoltre di:
...
Usando le API multihoming Cosmos DB di Azure, l'applicazione è a conoscenza dell'area più vicina e può inviare richieste a tale area. L'area più vicina viene identificata senza che sia necessario apportare modifiche alla configurazione. Quando si aggiungono e rimuovono aree da e verso l'account Cosmos di Azure, non è necessario ridistribuire o sospendere l'applicazione, ma continua a essere sempre a disponibilità elevata.

**Compilare applicazioni a reattività elevata.** L'applicazione può eseguire letture e scritture quasi in tempo reale su tutte le aree scelte per il database. Il database Cosmos di Azure gestisce internamente la replica dei dati tra aree con garanzie del livello di coerenza del livello selezionato.
