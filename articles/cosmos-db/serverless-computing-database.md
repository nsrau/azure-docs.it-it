---
title: Elaborazione di database senza server con Azure Cosmos DB e Funzioni di AzureServerless database computing with Azure Cosmos DB and Azure Functions
...
* Creare un trigger di Funzioni di Azure basato su eventi **per Cosmos DB**. Questo trigger si basa sui flussi del feed di [modifiche](change-feed.md) per monitorare il contenitore Cosmos di Azure per le modifiche. Quando vengono apportate delle modifiche a un contenitore, il flusso del feed di modifiche viene inviato al trigger, che richiama la funzione di Azure.
* In alternativa, associare una funzione di Azure a un contenitore CosmoS di Azure usando **un'associazione**di input. Le associazioni di input leggono i dati dal contenitore quando viene eseguita una funzione.
* Associare una funzione a un contenitore Cosmos di Azure usando **un'associazione**di output. Le associazioni di output scrivono i dati in un contenitore al termine di una funzione.
...
Il trigger Funzioni di Azure, l'associazione di input e l'associazione di output per Azure Cosmos DB possono essere usati nelle combinazioni seguenti:The Azure Functions trigger, input binding, and output binding for Azure Cosmos DB can be used in the following combinations:

* Un trigger Funzioni di Azure per Cosmos DB può essere usato con un'associazione di output a un contenitore Cosmos di Azure diverso. Dopo che una funzione ha eseguito un'azione su un elemento nel feed di modifiche è possibile scriverla in un altro contenitore. Scriverla nello stesso contenitore da cui proviene creerebbe in realtà un ciclo ricorsivo. In alternativa, è possibile usare un trigger di Funzioni di Azure per Cosmos DB per eseguire in modo efficace la migrazione di tutti gli elementi modificati da un contenitore a un contenitore diverso, con l'uso di un'associazione di output.
...
### <a name="iot-use-case---azure-functions-trigger-and-output-binding-for-cosmos-db"></a>Caso d'uso IoT - Trigger di Funzioni di Azure e associazione di output per Cosmos DB
...
**Attuazione:** Usare un trigger di Funzioni di Azure e un'associazione di output per Cosmos DBUse an Azure Functions trigger and output binding for Cosmos DB

1. Un **trigger Funzioni di Azure per Cosmos DB** viene usato per attivare eventi correlati agli avvisi dell'auto, ad esempio la luce del motore di controllo in arrivo in un'auto connessa.
2. Quando compare la luce del motore di controllo, i dati del sensore vengono inviati ad Azure Cosmos DB.
3. Azure Cosmos DB crea o aggiorna nuovi documenti di dati del sensore, quindi tali modifiche vengono trasmesse al trigger di Funzioni di Azure per Cosmos DB.
...
7. **L'associazione** di output nella funzione aggiorna il record dell'auto in un altro contenitore Cosmos di Azure per archiviare le informazioni sull'evento del motore di controllo.

La figura seguente mostra il codice scritto nel portale di Azure per questo trigger.

![Creare un trigger di Funzioni di Azure per Cosmos DB nel portale di AzureCreate an Azure Functions trigger for Cosmos DB in the Azure portal](./media/serverless-computing-database/cosmos-db-trigger-portal.png)
...
1. Utilizzando un [trigger timer](../azure-functions/functions-bindings-timer.md), è possibile recuperare le informazioni sul saldo del conto bancario archiviate in un contenitore Cosmos di Azure a intervalli di tempo utilizzando **un'associazione**di input .
...
### <a name="gaming-use-case---azure-functions-trigger-and-output-binding-for-cosmos-db"></a>Caso d'uso del gioco - Associazione di generazione e trigger di Funzioni di Azure per Cosmos DB 
...
1. Usando un database [grafico](graph-introduction.md) del database del database cosmos di Azure per archiviare tutti gli utenti, è possibile creare una nuova funzione con un trigger Funzioni di Azure per Cosmos DB. 
