...
Le transazioni di database offrono un modello di programmazione sicuro e prevedibile per gestire le modifiche simultanee ai dati. I database relazionali tradizionali, come SQL ServerSQL Server, consentono di scrivere la logica di business usando stored procedure e/o trigger, inviarla al server per l'esecuzione direttamente all'interno del motore di database. Con i database relazionali tradizionali, è necessario gestire due linguaggi di programmazione diversi il linguaggio di programmazione dell'applicazione (non transazionale), ad esempio JavaScript, Python, C, Java e così via e il linguaggio di programmazione transazionale (ad esempio T-SQL) che viene eseguito in modo nativo dal database.

Il motore di database in Azure Cosmos DB supporta transazioni conformi ACID con l'isolamento dello snapshot. Tutte le operazioni di database nell'ambito della [partizione logica](partition-data.md) di un contenitore vengono eseguite transazionalmente all'interno del motore di database ospitato dalla replica della partizione. Tali operazioni includono sia operazioni di scrittura (aggiornamento di uno o più elementi all'interno della partizione logica) e lettura. Nella tabella seguente vengono illustrate le diverse operazioni e i tipi di transazione:
...
Ogni elemento archiviato in un contenitore di Azure Cosmos ha un sistema definito proprietà `_etag`. Il valore di `_etag` viene automaticamente generato e aggiornato dal server ogni volta che viene aggiornato l'elemento. `_etag`può essere utilizzato con `if-match` l'intestazione della richiesta fornita dal client per consentire al server di decidere se un elemento può essere aggiornato in modo condizionale. Il valore `if-match` dell'intestazione corrisponde `_etag` al valore di nel server, l'elemento viene quindi aggiornato. Se il valore `if-match` dell'intestazione della richiesta non è più aggiornato, il server rifiuta l'operazione con un messaggio di risposta "HTTP 412 Precondizione failure". Il client può quindi recuperare nuovamente l'elemento per acquisire la versione corrente dell'elemento nel `_etag` server o eseguire l'override della versione dell'elemento nel server con il proprio valore per l'elemento. Inoltre, `_etag` può essere utilizzato `if-none-match` con l'intestazione per determinare se è necessaria una rilettura di una risorsa.

Il valore `_etag` dell'elemento cambia ogni volta che l'elemento viene aggiornato. Per le operazioni `if-match` di sostituzione degli elementi, deve essere espresso in modo esplicito come parte delle opzioni di richiesta. Per un esempio, vedere il codice di esempio in [GitHub](https://github.com/Azure/azure-cosmos-dotnet-v3/blob/master/Microsoft.Azure.Cosmos.Samples/Usage/ItemManagement/Program.cs#L578-L674). `_etag`vengono controllati in modo implicito tutti gli elementi scritti toccati dalla stored procedure. Se viene rilevato un conflitto, la stored procedure eseguirà il rollback della transazione e genererà un'eccezione. Con questo metodo, tutte o nessuna scrittura all'interno della stored procedure viene applicata in modo atomico. Si tratta di un segnale per l'applicazione per riapplicare gli aggiornamenti e ripetere la richiesta del client originale.
