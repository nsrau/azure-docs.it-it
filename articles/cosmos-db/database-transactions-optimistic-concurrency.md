...
Il valore `_etag` dell'elemento cambia ogni volta che l'elemento viene aggiornato. Per le operazioni di sostituzione di elementi, `if-match` deve essere espressa in modo esplicito come parte delle opzioni di richiesta. Per un esempio, vedere il codice di esempio in [GitHub](https://github.com/Azure/azure-cosmos-dotnet-v3/blob/master/Microsoft.Azure.Cosmos.Samples/Usage/ItemManagement/Program.cs#L578-L674). `_etag` valori vengono verificati in modo implicito per tutti gli elementi scritti con il stored procedure. Se viene rilevato un conflitto, il stored procedure eseguirà il rollback della transazione e genererà un'eccezione. Con questo metodo, tutte o nessuna scrittura all'interno della stored procedure viene applicata in modo atomico. Si tratta di un segnale per l'applicazione per riapplicare gli aggiornamenti e ripetere la richiesta del client originale.
