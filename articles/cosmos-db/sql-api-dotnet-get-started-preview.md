...
Un database si può creare usando la funzione [**CreateDatabaseIfNotExistsAsync**](/dotnet/api/microsoft.azure.cosmos.cosmosclient.createdatabaseifnotexistsasync) o [**CreateDatabaseAsync**](/dotnet/api/microsoft.azure.cosmos.cosmosclient.createdatabaseasync) della classe ``CosmosDatabases``. Un database è un contenitore logico di elementi partizionati tra contenitori.
...
Un contenitore può essere creato usando la funzione [**CreateContainerIfNotExistsAsync**](/dotnet/api/microsoft.azure.cosmos.database.createcontainerifnotexistsasync) o [**CreateContainerAsync**](/dotnet/api/microsoft.azure.cosmos.database.createcontainerasync) della classe **CosmosContainers**. Un contenitore è costituito da elementi, che nel caso dell'API SQL sono documenti JSON, e dalla logica dell'applicazione JavaScript lato server associata, come stored procedure, funzioni definite dall'utente e trigger.
