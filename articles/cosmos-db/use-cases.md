...
[Azure Cosmos DB](../cosmos-db/introduction.md) è il servizio di database di Microsoft distribuito a livello globale. Esso è progettato per consentire ai clienti di ridimensionare in modo elastico e indipendente velocità effettiva e archiviazione tra più aree geografiche. Azure Cosmos DB è il primo servizio di database distribuito oggi a livello globale in grado di offrire [contratti di servizio](https://azure.microsoft.com/support/legal/sla/cosmos-db/) comprendenti velocità effettiva, latenza, disponibilità e coerenza. 

Il progetto Azure Cosmos DB è stato avviato nel 2011 come "Progetto Florence" per far fronte ai problemi critici di sviluppo riscontrati dalle applicazioni Internet di grandi dimensioni all'interno di Microsoft. Avendo osservato che questi problemi non riguardano soltanto tali applicazioni, è stato deciso di rendere Azure Cosmos DB generalmente disponibile per gli sviluppatori esterni nel 2015 sotto forma di [Azure DocumentDB](https://azure.microsoft.com/blog/documentdb-moving-to-general-availability/). Il servizio viene ampiamente usato all'interno di Microsoft ed è uno dei servizi in rapida crescita più utilizzati dagli sviluppatori esterni che lavorano su Azure. 

Azure Cosmos DB è un database multimodello distribuito a livello globale che viene usato in un'ampia gamma di applicazioni e casi di utilizzo. È un'ottima scelta per le applicazioni che richiedono tempi di risposta nell'ordine dei millisecondi e che devono essere ridimensionate rapidamente e a livello globale. Supporta diversi modelli di dati (chiave-valore, documenti, grafi e a colonne) oltre a molte API per l'accesso ai dati, tra cui [MongoDB](mongodb-introduction.md), [DocumentDB SQL](documentdb-introduction.md), [Gremlin](graph-introduction.md) e [tabelle di Azure ](table-introduction.md), in modo nativo ed estendibile. 
...
* Il supporto di Azure Cosmos DB per i livelli di coerenza di tipo eventuale, prefisso coerente e sessione e con obsolescenza associata consente completa flessibilità e un basso rapporto costo-prestazioni. Nessun servizio di database offre maggiore flessibilità riguardo alla coerenza dei livelli. 
...
* Il modello di velocità effettiva riservata di Azure Cosmos DB consente un approccio in termini di numero di letture/scritture invece che di CPU/memoria/operazioni di I/O al secondo dell'hardware sottostante.
...
I casi di utilizzo IoT condividono di solito alcuni modelli di inserimento, elaborazione e archiviazione dei dati. Come prima cosa, tali sistemi devono ricavare grandi quantità di informazioni dai sensori dei dispositivi localizzati. In secondo luogo,  elaborano e analizzano i dati streaming per ottenere informazioni in tempo reale. I dati vengono quindi archiviati offline per l'analisi batch. Microsoft Azure offre servizi avanzati che possono essere applicati per i casi d'uso di IoT, inclusi Azure Cosmos DB, Hub eventi di Azure, analisi di flusso di Azure, Hub di notifica di Azure, Azure Machine Learning, Azure HDInsight e PowerBI. 

![Architettura di riferimento di IoT per Azure Cosmos DB](./media/use-cases/iot.png)

Con gli Hub eventi di Azure si possono acquisire grandi quantità di dati con una velocità effettiva elevata e una bassa latenza. I dati inseriti, che devono essere elaborati per ottenere informazioni in tempo reale, possono essere incanalati in Analisi di flusso di Azure per l'analisi in tempo reale ed essere caricati in Azure Cosmos DB per l'esecuzione di query ad hoc. Una volta su Azure Cosmos DB, essi sono pronti per l'esecuzione delle query. I dati in Azure Cosmos DB possono essere utilizzati anche come riferimento durante l'analisi in tempo reale, oppure possono essere ulteriormente affinati ed elaborati connettendoli con HDInsight per processi Pig, Hive o MapReduce. I risultati affinati vengono quindi ricaricati in Azure Cosmos DB per la creazione di report.   
...
Azure Cosmos DB viene ampiamente usato nelle piattaforme di e-commerce di Microsoft che eseguono Windows Store e XBox Live. Viene inoltre applicato nel settore della vendita al dettaglio per l'archiviazione dei dati dei cataloghi. Gli scenari di utilizzo dei dati di un catalogo includono l'archiviazione e l'esecuzione di query di un set di attributi per entità come persone, luoghi e prodotti. Gli account utente, i cataloghi di prodotti, i registri di periferica dei dispositivi per IoT e i sistemi di distinta base sono alcuni esempi di dati di catalogo. Gli attributi di questi dati possono variare e cambiare nel corso del tempo per rispondere ai requisiti dell'applicazione.  

Consideriamo l'esempio di un catalogo di prodotti per un fornitore di parti di ricambio per automobili. Ogni ricambio può avere propri attributi oltre ai comuni attributi condivisi da tutti i ricambi.  Inoltre, gli attributi di un ricambio specifico possono cambiare l'anno seguente, quando viene rilasciato ad esempio un nuovo modello. Azure Cosmos DB supporta schemi flessibili e dati gerarchici ed è pertanto adatto per l'archiviazione dei dati di un catalogo di prodotti.
