...
* Azure Cosmos Emulator supporta un account Azure Cosmos in modalità di [velocità effettiva con provisioning](set-throughput.md) ; Attualmente non supporta un account Azure Cosmos in modalità senza [Server](serverless.md) .
...
* Emulatore supporta la dimensione massima della proprietà ID di 254 caratteri.
...
:::image type="content" source="./media/local-emulator/database-local-emulator-start.png" alt-text="Fare clic sul pulsante Start o premere WINDOWS sulla tastiera, iniziare a digitare Azure Cosmos Emulator e selezionare l'emulatore dall'elenco di applicazioni":::
...
:::image type="content" source="./media/local-emulator/database-local-emulator-ssl_certificate.png" alt-text="Certificato TLS/SSL dell'emulatore locale di Azure Cosmos DB":::
...
| GenCert | Genera e installa un nuovo certificato TLS/SSL autofirmato, includendo facoltativamente un elenco di nomi DNS aggiuntivi separati da virgole per l'accesso all'emulatore tramite la rete. | Microsoft.Azure.Cosmos.Emulator.exe /GenCert=\<dns-names\> |\<dns-names\>: elenco facoltativo di nomi DNS aggiuntivi delimitati da virgole  |
