...
## <a name="connections-from-azure-paas-service"></a>Connessioni dal servizio PaaS di Azure 
In Azure i servizi PaaS come Analisi di flusso di Azure e Funzioni di Azure vengono usati in combinazione con Azure Cosmos DB. Per consentire l'accesso a un account del database di Azure Cosmos DB da questi tipi di servizi il cui indirizzo IP non è prontamente disponibile, è necessario aggiungere l'indirizzo IP 0.0.0.0 all'elenco di indirizzi IP consentiti associato all'account del database di Azure Cosmos DB [configurando il criterio di controllo dell'accesso agli indirizzi IP](#configure-ip-policy).  In questo modo si garantisce che i servizi PaaS di Azure possano accedere a un account Azure Cosmos DB che dispone di questa regola. 
