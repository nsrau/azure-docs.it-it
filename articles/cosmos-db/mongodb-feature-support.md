...
L'API di Azure Cosmos DB per MongoDB offre anche un'esperienza di aggiornamento facilitato per gli account idonei. Per altre informazioni, vedere la [Guida all'aggiornamento della versione di MongoDB](mongodb-version-upgrade.md).
...
Cosmos DB supporta la pipeline di aggregazione per MongoDB 3.2 nella versione di anteprima pubblica. Per istruzioni sull'onboarding nell'anteprima pubblica, vedere il [blog di Azure](https://azure.microsoft.com/blog/azure-cosmosdb-extends-support-for-mongodb-aggregation-pipeline-unique-indexes-and-more/).
