---
title: Strumento di migrazione del database per Azure Cosmos DB
description: Informazioni sull'uso degli strumenti open source di migrazione dati di Azure Cosmos DB per importare dati in Azure Cosmos DB da varie origini, tra cui file JSON, CSV, MongoDB, SQL Server, archiviazione tabelle e Amazon DynamoDB. Conversione da CSV a JSON.
...
* Default. Questo criterio risulta ottimale quando si eseguono query di uguaglianza su stringhe. È utile anche se si usano query di tipo ORDER BY, di intervallo e di uguaglianza per i numeri. Questo criterio ha un overhead di archiviazione indice inferiore rispetto a intervallo.
...
4. Quindi, scegliere se registrare tutti i messaggi di errore, quelli critici o nessuno. Infine, definire la frequenza con cui il messaggio di trasferimento su schermo viene aggiornato con lo stato di avanzamento.
   ...
   ![Screenshot della schermata di configurazione avanzata](./media/import-data/AdvancedConfiguration.png)
