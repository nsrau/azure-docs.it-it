...
Lo strumento di migrazione dati può essere usato solo per l'importazione in Azure Cosmos DB di dati da usare con l'API DocumentDB. L'importazione di dati da usare con l'API Table o l'API Graph non è attualmente supportata. 

Per importare dati da usare con l'API MongoDB, vedere l'articolo su [come eseguire la migrazione di dati per l'API MongoDB in Azure Cosmos DB](mongodb-migrate.md).
...
## <a id="Overviewl"></a>Panoramica dello strumento di migrazione dati
...
## <a id="Install"></a>Installare lo strumento di migrazione dati
...
## <a id="JSON"></a>Per importare file JSON
...
## <a id="MongoDB"></a>Per importare da MongoDB
...
## <a id="MongoDBExport"></a>Per importare file di esportazione MongoDB
...
## <a id="SQL"></a>Per importare da SQL Server
...
## <a id="CSV"></a>Per importare file CSV ed eseguire la conversione da CSV a JSON
...
## <a id="AzureTableSource"></a>Per importare dall'archivio tabelle di Azure
L'opzione dell'utilità di importazione dell'origine Archiviazione tabelle di Azure consente di importare da una singola tabella di Archiviazione tabelle di Azure e, facoltativamente, di filtrare le entità tabelle da importare. Si noti che non è possibile usare lo strumento di migrazione dati per importare in Azure Cosmos DB dati dell'archivio tabelle di Azure da usare con l'API Table. L'importazione in Azure Cosmos DB è attualmente supportata solo per l'uso con l'API DocumentDB.
...
## <a id="DynamoDBSource"></a>Per importare da Amazon DynamoDB
...
## <a id="BlobImport"></a>Per importare file dall'archivio BLOB di Azure
...
## <a id="DocumentDBSource"></a>Per importare da una raccolta dell'API DocumentDB di Azure Cosmos DB
...
La stringa di connessione dell'account Azure Cosmos DB può essere recuperata dal pannello Chiavi del portale di Azure, come descritto in [How to manage an Azure Cosmos DB account](manage-account.md) (Come gestire un account Azure Cosmos DB), ma è necessario aggiungere il nome del database alla stringa di connessione nel formato seguente:
...
## <a id="HBaseSource"></a>Per importare da HBase
...
## <a id="DocumentDBBulkTarget"></a>Per importare nell'API DocumentDB (importazione in blocco)
...
Facoltativamente, è possibile specificare il campo dell'origine di importazione da usare come proprietà ID del documento di Azure Cosmos DB durante l'importazione. Se i documenti non contengono questa proprietà, lo strumento di importazione genererà un GUID come valore della proprietà ID.
...
## <a id="DocumentDBSeqTarget"></a>Per importare nell'API DocumentDB (importazione di record sequenziali)
...
Quando si consente all'utilità di migrazione di creare raccolte durante l'importazione, è possibile specificare i criteri di indicizzazione delle raccolte. Nella sezione delle opzioni avanzate per l'importazione in blocco e l'importazione di record sequenziali di Azure Cosmos DB, passare alla sezione relativa ai criteri di indicizzazione.
...
> [!NOTE]
> Se non si specifica un criterio di indicizzazione, verrà applicato il criterio predefinito. Per altre informazioni sui criteri di indicizzazione, vedere l'articolo relativo ai [criteri di indicizzazione di Azure Cosmos DB](indexing-policies.md).
> 
> 
