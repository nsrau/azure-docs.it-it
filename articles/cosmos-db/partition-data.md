...
I contenitori sono risorse logiche e possono comprendere una o più partizioni fisiche o server. Il numero di partizioni è determinato da Azure Cosmos DB in base alle dimensioni di archiviazione e alla velocità effettiva del contenitore assegnata tramite provisioning. 

Una partizione *fisica* è una quantità fissa di archiviazione riservata basata su unità SSD. Ogni partizione fisica viene replicata per assicurare disponibilità elevata. Una o più partizioni fisiche costituiscono un contenitore. Le partizioni fisiche vengono completamente gestite da Azure Cosmos DB, quindi non è necessario scrivere codice complesso o occuparsi personalmente della gestione. I contenitori di Azure Cosmos DB sono illimitati in termini di risorse di archiviazione e di velocità effettiva. 
...
> [!NOTE]
> Per ottenere la velocità effettiva totale di un contenitore, è necessario scegliere una chiave di partizione che permetta di distribuire in modo uniforme le richieste tra tutti i valori di chiave di partizione distinti.
> 
> 
