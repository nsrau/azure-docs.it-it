...
* [Node.js][Node.js] 0.10.29 o versione successiva. È consigliabile usare Node.js 6.10 o versione successiva.
...
6. È possibile visualizzare la nuova applicazione passando con il browser all'indirizzo [http://localhost:3000](http://localhost:3000).
    ...
    Quindi, per arrestare l'applicazione, premere CTRL+C nella finestra del terminale e fare clic su **y** per interrompere il processo batch.
...
2. Nella directory **models** (modelli) creare un nuovo file denominato **task-model.js**. Questo file conterrà il modello per le attività create dall'applicazione.
3. Nella stessa directory **models** (modelli) creare un altro nuovo file denominato **cosmosdb-manager.js**. Questo file conterrà porzioni di codice utile e riutilizzabile, che verrà usato nell'applicazione. 
4. Copiare il codice seguente in **cosmosdb-manager.js**
    ```nodejs
    ...
    ```
5. Salvare e chiudere il file **cosmosdb-manager.js**.
6. All'inizio del file **task-model.js** aggiungere il codice seguente per fare riferimento a **DocumentDBClient** e **cosmosdb-manager.js** creati in precedenza: 
...
8. Aggiungere quindi il codice seguente per definire metodi aggiuntivi nell'oggetto Task che consentano l'interazione con i dati archiviati in Azure Cosmos DB.
    ...
    module.exports = TaskModel;
    ```
9. Salvare e chiudere il file **task-model.js**. 
...
4. Sostituire le due righe seguenti nel file **app.js** :
    ...
    ```
5. Queste righe definiscono una nuova istanza dell'oggetto **TaskModel** con una nuova connessione ad Azure Cosmos DB usando i valori letti dal file **config.js**, inizializzano l'oggetto attività e quindi associano le azioni del modulo ai metodi nel controller **TaskList**. 
