...
Ogni passaggio nella definizione di un'app per la logica è un'azione.  Sono inclusi trigger, passaggi del flusso di controllo come condizioni, ambiti e cicli ForEach, cicli Do until, chiamate a connettori e chiamate ad azioni native.
I trigger sono semplicemente azioni speciali progettate per creare una nuova istanza di un'app per la logica quando si verifica un determinato evento.  Diversi comportamenti dei trigger potrebbero influire sulla misurazione dell'app per la logica.
...
### <a name="convert-from-consumption-to-app-service-plan-pricing"></a>Passare da prezzi al consumo al piano tariffario del servizio app
