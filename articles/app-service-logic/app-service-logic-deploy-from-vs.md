...
Si consiglia di usare **parametri** in tutta la definizione. L'uso dei parametri è utile se si vuole effettuare la distribuzione in un ambiente di sviluppo e di produzione. In tal caso, è necessario inserire i dati di configurazione di tutte le specifiche dell'ambiente nel file `*.parameters.json` e i parametri anziché le stringhe effettive.
