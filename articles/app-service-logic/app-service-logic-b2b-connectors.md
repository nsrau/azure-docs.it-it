---
title: Connettori Business to Business e app per le API nelle app per la logica | Microsoft Docs
...
> [!NOTE]
> Per creare un connettore AS2 e un'app per le API X12 o EDIFACT è necessaria un'istanza di BizTalk Trading Partner Management (TPM). Quando si crea l'app per le API TPM, si crea l'istanza di TPM.
> 
> 
...
> [!IMPORTANT]
> Per aggiungere connettori e app per le API create in precedenza, creare le app per la logica, mobili o Web nello stesso gruppo di risorse.
> 
> 
...
3. In **Raccolta** selezionare l'app per le API B2B, che verrà aggiunta automaticamente all'app. È anche possibile creare una nuova app per le API B2B.
   ...
   > [!IMPORTANT]
   > Il connettore AS2 e le app per le API X12 ed EDIFACT richiedono un'istanza di TPM. Pertanto, quando si creano nuove app per le API B2B, creare prima l'app per le API TPM e quindi creare il connettore AS2 e l'app per le API X12 o EDIFACT.
   > 
   > 
