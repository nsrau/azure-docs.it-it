<properties
   pageTitle="Gestione delle eccezioni in App per la logica | Microsoft Azure"
...
>[AZURE.TIP] Le azioni eseguite dopo l'esito negativo di un'azione precedente e completate correttamente verranno contrassegnate come `Succeeded`. Con questo comportamento, se tutti gli errori in un flusso di lavoro vengono rilevati l'esecuzione stessa verrà contrassegnata come `Succeeded`.
...
1. **Filtra matrice** con qualsiasi elemento `@result()` con stato `Failed` come condizione. La matrice dei risultati di tutte le azioni di `My_Scope` viene così filtrata in modo da ottenere una matrice solo dei risultati delle azioni non riuscite.
1. Esecuzione di un'azione **ForEach** sugli output **Matrice filtrata**. Verrà così eseguita un'azione *per ogni* risultato di azione non riuscita precedentemente filtrato.
...
1. Invio di HTTP POST nel corpo della risposta dell'elemento `foreach`, ovvero `@item()['outputs']['body']`. La forma dell'elemento `@result()` è uguale alla forma di `@actions()` e può essere analizzata nello stesso modo.
1. Inclusione anche di due intestazioni personalizzate con il nome dell'azione non riuscita `@item()['name']` e l'ID rilevamento client dell'esecuzione non riuscita `@item()['clientTrackingId']`.
