...
3. Configurare il connettore SharePoint Online e fare clic su Crea. Di seguito sono elencati i parametri che è necessario fornire per creare il connettore:
  ...
  <tr>
    <td>URL sito</td>
    <td>Sì</td>
    <td>Specificare l'URL completo del sito Web di SharePoint. Ad esempio, https://microsoft.sharepoint.com/teams/wabstest </td>
  </tr>
  <tr>
    <td>URL relativi raccolta/elenco documenti</td>
    <td>Sì</td>
    <td>Specificare gli URL degli elenchi o delle raccolte di documenti, relativi all'URL del sito di SharePoint, che possono essere modificati dal connettore. Ad esempio, elenchi/attività, documenti condivisi.</td>
  </tr>
</table>![][1]
...
Scegliendo il valore Default vengono usate le credenziali predefinite per l'esecuzione del microservizio SharePoint e non è necessario inserire nome utente e password. I campi Nome utente e Password sono obbligatori per altri tipi di autenticazione. <br><br>Nota: l'autenticazione anonima non è supportata.</td>
  ...
  <tr>
    <td>Nome utente</td>
    <td>No</td>
    <td>Specificare un nome utente valido per la connessione al sito di SharePoint, se la modalità di autenticazione non è Predefinito.</td>
  </tr>
  <tr>
    <td>Password</td>
    <td>No</td>
    <td>Specificare una password valida per la connessione al sito di SharePoint, se la modalità di autenticazione non è Predefinito.</td>
  </tr>
  <tr>
    <td>URL relativi raccolta/elenco documenti</td>
    <td>Sì</td>
  ...
  <tr>
    <td>Stringa di connessione del bus di servizio</td>
    <td>No</td>
    <td>Deve trattarsi di una stringa di connessione valida allo spazio dei nomi del bus di servizio.<br><br>

È necessario installare un agente listener in un server che può accedere a SharePoint Server. <br>Per installare l'agente, passare alla pagina di riepilogo dell'app per le API e fare clic su 'Connessione ibrida'.</td>
  </tr>
</table>![][2]
...
5. È necessario installare un agente listener in un server che può accedere a SharePoint Server. Per installare l'agente, passare alla pagina di riepilogo dell'app per le API e fare clic su 'Connessione ibrida'.
...
4. Se si usa il connettore di SharePoint Online, per eseguire operazioni per conto dell'utente è necessario autenticare e autorizzare le app per la logica. Per avviare l'autorizzazione, fare clic su Autorizza nel connettore SharePoint.
...
7. Se il connettore SharePoint viene configurato come trigger, vengono mostrati i trigger. In caso contrario, viene visualizzato l'elenco delle azioni ed è possibile scegliere l'operazione appropriata da eseguire.
	...
	<b>Nota:</b> per i trigger seguenti si presuppone che l'utente abbia specificato 'Documenti condivisi, Elenchi/attività' nelle impostazioni pacchetto del connettore, dove 'Documenti condivisi' è una raccolta documenti ed 'Elenchi/attività' è un elenco.

##  Trigger
Usare i trigger se si vuole avviare un'app per la logica

**NOTA**: i trigger elimineranno i file dopo averli letti. Per mantenere i file, specificare un valore per il percorso dell'archivio.
...
<table>
  ...
  <tr>
    ...
    <td>No</td>
    <td>Specificare una visualizzazione valida usata per filtrare i documenti da prelevare. Ad esempio: 'Approved Orders'. Per elaborare tutti i documenti esistenti, lasciare vuoto questo campo. </td>
  </tr>
  <tr>
...
**Output:** <table> <tr> <td><b>Nome</b></td> <td><b>Descrizione</b></td> </tr> <tr> <td>Nome </td> <td>Nome del documento.</td> </tr> <tr> <td>Contenuto</td> <td>Contenuto del documento.</td> </tr> <tr> <td>ContentTransferEncoding</td> <td>Codifica trasferimento contenuto del messaggio. ("Nessuna"|"Base64")</ </tr> </table>
...
**Input:** <table> <tr> <td><b>Nome</b></td> <td><b>Obbligatorio</b></td> <td><b>Descrizione</b></td> </tr> <tr> <td>Nome visualizzazione</td> <td>No</td> <td>Specificare una visualizzazione valida usata per filtrare gli elementi dell'elenco. Ad esempio: 'Approved Orders'. Per elaborare tutti i nuovi elementi, lasciare vuoto questo campo. </td> </tr> <tr> <td>Percorso archiviazione</td> <td>No</td> <td>Specificare un URL di cartella valido, relativo al sito di SharePoint, in cui archiviare gli elementi elenco elaborati. </td> </tr> <tr> <td>Query Caml</td> <td>No, Avanzate</td> <td>Specificare una query Caml valida per filtrare gli elementi elenco. Esempio: <Where><Geq><FieldRef Name='ID'/><Value Type='Number'>10</Value></Geq></Where></td> </tr> </table>
...
<table>
  ...
  <tr>
    ...
    <td>Codifica di trasferimento contenuto del messaggio ("Nessuna"|"Base64").</td>
  </tr>
</table>
...
<table>
  ...
  <tr>
    <td>Nome visualizzazione</td>
    <td>No</td>
    <td>Specificare una visualizzazione valida usata per filtrare gli elementi dell'elenco. Ad esempio: 'Approved Orders'. Per elaborare tutti i nuovi elementi, lasciare vuoto questo campo. </td>
  </tr>
  <tr>
    <td>Percorso archiviazione</td>
    <td>No</td>
...
<b>Nota:</b> tutti i parametri della raccolta documenti vengono popolati in modo dinamico. I parametri obbligatori sono visibili e i parametri facoltativi si trovano nella sezione Avanzate.
...
<table>
  ...
  <tr>
    ...
    <td>No</td>
    <td>Specificare l'URL del documento, relativo a 'Documenti condivisi'. Esempio: specpersonali1,cartellapersonale/ordini</td>
  </tr>
</table>
...
<b>Nota:</b> tutti i parametri della raccolta documenti vengono popolati in modo dinamico. Si trovano nella sezione Avanzate.
...
<b>Nota:</b> tutti i parametri dell'elenco vengono popolati in modo dinamico. I parametri obbligatori sono visibili e i parametri facoltativi si trovano nella sezione Avanzate.
