<properties
	pageTitle="Segnalazione avanzata di Windows universale con MobileApps Engagement"
...
# Segnalazione avanzata con Engagement SDK per le app di Windows universali
...
## Specifica della configurazione di Engagement in fase di esecuzione
...
> [AZURE.IMPORTANT] Se la pagina esegue l'override del metodo `OnNavigatedTo`, accertarsi di chiamare `base.OnNavigatedTo(e)`. In caso contrario, l'attività non verrà segnalata, ovvero `EngagementPage` chiama `StartActivity` all'interno del relativo metodo `OnNavigatedTo`.
