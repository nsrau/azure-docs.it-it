...
30 minuti si otterrà un'applicazione in esecuzione nel cloud e che usa più risorse di Windows
Azure.
...
## <span class="short-header">CONFIGURAZIONE DELL'AMBIENTE</span>CONFIGURAZIONE DELL'AMBIENTE DI SVILUPPO
...
## <span class="short-header">CREAZIONE DI UNO SPAZIO DEI NOMI</span>CREAZIONE DI UNO SPAZIO DEI NOMI SERVIZIO
...
## <span class="short-header">CREAZIONE DI UN SERVER LOCALE</span>CREAZIONE DI UN SERVER LOCALE
...
5.  In **Esplora soluzioni** fare clic con il pulsante destro del mouse su **ProductsServer**, quindi
    scegliere **Proprietà**.
6.  Fare clic sulla scheda **Applicazione** sulla sinistra, quindi assicurarsi che nell'elenco a discesa **Framework di destinazione
     sia presente** .NET**Framework 4** o **.NET Framework 4.5** . In caso contrario, selezionarlo dall'elenco a discesa e fare clic su **Sì**
...
10. Nella colonna di destra fare clic sulla casella di ricerca, digitare "**WindowsAzure**" e selezionare l'elemento **Windows
...
11. Aggiungere una nuova classe per il contratto dei prodotti. In **Esplora soluzioni**
     fare clic con il pulsante destro del mouse sul progetto **ProductsServer**, scegliere **Aggiungi** e quindi fare clic su
    **Classe**.

    ![][15]

12. Nella casella **Nome** digitare il nome **ProductsContracts.cs**. Fare
    clic su **Aggiungi**.
13. In **ProductsContract.cs** sostituire la definizione dello spazio dei nomi con il
...
14. In Program.cs sostituire la definizione dello spazio dei nomi con il codice
...
15. In **Esplora soluzioni** fare doppio clic sul file **app.config** per
    ...
    **\<system.ServiceModel\>** con il codice XML seguente. Assicurarsi di
    sostituire *yourServiceNamespace* con il nome dello
    spazio dei nomi servizio e *yourIssuerSecret* con la chiave recuperata
...
3.  Da **Modelli installati**, in **Visual C\#**, fare clic su **Applicazione Web ASP.NET**. Assegnare al progetto il nome **ProductsPortal**. Fare
    clic su **OK**.
...
3.  L'applicazione dovrebbe risultare in esecuzione in un browser:
    ...
    configurazione necessarie per la corretta esecuzione dell'applicazione nel
    cloud.

    1.  Per rendere l'applicazione distribuibile nel cloud, fare clic con il pulsante
        destro del mouse sul progetto **ProductsPortal** in **Esplora soluzioni**, fare
        clic su **Converti** e quindi su **Converti nel progetto di servizio cloud di Azure**.
    ...
    ## <span class="short-header">COMBINAZIONE DEI VARI COMPONENTI</span>COMBINAZIONE DEI VARI COMPONENTI
    ...
    6.  Aprire il file **HomeController.cs** nell'editor di Visual Studio
        ...
        codice seguente. Assicurarsi di sostituire *yourServiceNamespace* con il nome dello
        spazio dei nomi servizio e *yourIssuerSecret* con la chiave in uso.
        In tal modo il client potrà chiamare il servizio locale
        e restituire il risultato della chiamata.
    ...
    8.  Individuare il progetto **ProductsServer**, quindi fare doppio clic sulla
        soluzione **ProductsServer.csproj** per aggiungerla.
    ...
    10. Sul lato sinistro fare clic su **Progetto di avvio**. Sul
        ...
        **ProductsPortal** siano visualizzati in quest'ordine e che per **ProductsServer**
        e **ProductsPortal.Azure** sia impostata l'azione **Avvia**,
        mentre per **ProductsPortal** sia impostata l'azione **Nessuna**. Ad
        esempio:
    ...
    12. Nell'elenco a discesa **Projects** fare clic su
        **ProductsServer**. Assicurarsi che **ProductsPortal** sia deselezionato e
        che **ProductsPortal.Azure** sia selezionato. Fare quindi clic su **OK**:

        ![][27]

    ## <span class="short-header">ESECUZIONE DELL'APPLICAZIONE</span>ESECUZIONE DELL'APPLICAZIONE
    ...
    ## <span class="short-header">DISTRIBUZIONE DELL'APPLICAZIONE</span>DISTRIBUZIONE DELL'APPLICAZIONE IN AZURE
    ...
    4.  Fare clic su **Avanti**. Se la sottoscrizione non contiene già servizi
        ospitati, verrà chiesto di crearne uno. Il servizio ospitato
        funge da contenitore per l'applicazione all'interno della sottoscrizione di Windows
    ...
    5.  Selezionare il servizio ospitato in cui pubblicare
        l'applicazione. Mantenere i valori predefiniti illustrati di seguito per le
        rimanenti impostazioni. Fare clic su **Avanti**:

        ![][30]

    6.  Nell'ultima pagina fare clic su **Pubblica** per avviare il processo di
        distribuzione:
    ...
    7.  Durante la pubblicazione è possibile monitorare le
    ...
    8.  Al termine della distribuzione, per visualizzare il sito Web
...
delle macchine virtuali utilizzate per l'hosting di queste istanze del ruolo Web.
...
-   [Come utilizzare le code del bus di servizio][]
  ...
  [installare NuGet]: http://visualstudiogallery.msdn.microsoft.com/27077b70-9dad-4c64-adcf-c7cf6bc9970c
  ...
  [Procedure relative al bus di servizio]: /en-us/documentation/services/service-bus/
