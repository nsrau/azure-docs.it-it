...
|Numero S.  |Nome risorsa  |Azure FarmBeats Component  |
|---------|---------|---------|
...
|6 |      EventHub Namespace    |  Hub di dati       |
|7  |    Azure Data Factory V2       |     Data hub/ Accelerator      |
|8  |Account Batch    |Hub di dati   |
|9  |       Account di archiviazione     |  Data hub/ Accelerator        |
|10  |    App per la logica        |     Hub di dati      |
|11  |    API connection        |     Hub di dati      |
|12|      Servizio app      |  Data hub/Accelerator       |
|13 |    Piano di servizio app        |     Data hub/ Accelerator      |
|14 |Azure Maps account     |Acceleratore    |
...
Passare a https://scihub.copernicus.eu/dhus/#/self-registration. In the registration page, provide a first name, last name, username, password, and email.
...
**Case 2**: This method is the preferred step when you don't have enough rights to create and configure an Azure AD app registration within your subscription. Request your  admin to use the [custom script](https://aka.ms/FarmBeatsAADScript), which will help IT admin automatically generate and configure the Azure AD app registration on the Azure portal. As an output to running this custom script using PowerShell environment the IT admin needs to share an Azure Active Directory Application Client ID and password secret with you. Prendere nota dei valori seguenti.
...
4. First-time users will be prompted to select a subscription to create a storage account and Microsoft Azure Files share. Selezionare **Create storage** (Crea risorsa di archiviazione).
...
The steps for upgrade are similar to the first-time installation. Seguire questa procedura:
...
4. Selezionare il nome del gruppo di risorse. Type in the name again to double-check, and select Delete to remove the resource group, and all its underlying resources.
