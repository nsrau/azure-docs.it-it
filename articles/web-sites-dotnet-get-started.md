...
* È possibile [attivare i benefici per gli abbonati MSDN](/pricing/member-offers/msdn-benefits-details/?WT.mc_id=A261C142F). Con la sottoscrizione MSDN ogni mese si accumulano crediti che è possibile usare per i servizi di Azure a pagamento.</li>
...
<!-- Operazione necessaria: reintegrare il digramma dopo la modifica del sito Web in app Web in didascalie
Il diagramma seguente illustra cosa avviene in questi due passaggi.
...
3. In Visual Studio viene fornito un **Nome app Web** predefinito, che verrà usato in Azure come prefisso dell'URL dell'applicazione. Se si preferisce, immettere un nome di app Web diverso.

	L'URL completo sarà costituito da quanto immesso in questa casella e da *.azurewebsites.net* (come mostrato accanto alla casella di testo **Nome app Web**). Ad esempio, se il nome è  `MyExample6442`, the URL will be `MyExample6442.azurewebsites.net`. L'URL deve essere univoco. Se l'URL immesso è già stato usato da un altro utente, a destra, invece di un segno di spunta verde, verrà visualizzato un punto esclamativo rosso e sarà necessario specificare un nome diverso.
...
2. Aprire il file  *Views/Home/Index.cshtml* o *.vbhtml* in **Esplora soluzioni**, modificare l'intestazione **h1** da "ASP.NET" a "ASP.NET e Azure" e salvare il file. 
...
1. Nel browser passare a [http://portal.azure.com]() e accedere con le credenziali di Azure.
...
* Come gestire un'app Web in Visual Studio
	...
	Per informazioni sulle funzioni di gestione delle app Web che è possibile eseguire in **Esplora server**, vedere [Risoluzione dei problemi delle app Web di Azure in Visual Studio](troubleshoot-web-sites-in-visual-studio.md).
...
* Come scegliere tra Servizio app, Servizi cloud e Macchine virtuali per le applicazioni Web
	...
	In Azure è possibile eseguire applicazioni Web nel servizio app per app Web come descritto in questa esercitazione oppure in Servizi cloud o Macchine virtuali. Per altre informazioni, vedere [Modelli di esecuzione di Azure](/develop/net/fundamentals/compute/) e [Confronto tra app Web, servizi cloud e macchine virtuali di Azure: Macchine virtuali di Azure](/manage/services/web-sites/choose-web-app-service/).

## Modifiche apportate
* Per una guida relativa al passaggio da Siti Web al servizio app, vedere: [Servizio app di Azure e l'impatto sui servizi di Azure esistenti](http://go.microsoft.com/fwlink/?LinkId=529714)
* Per una guida sul passaggio dal vecchio al nuovo portale, vedere: [Riferimenti per l'esplorazione del portale di anteprima](http://go.microsoft.com/fwlink/?LinkId=529715)
