<properties 
   pageTitle="Autenticazione di Azure Active Directory e Resource Manager | Microsoft Azure"
...
1. **Accesso solo app**: per le app che eseguono servizi daemon e processi pianificati. All'identità dell'app viene concesso l'accesso diretto alle risorse. Questo approccio funziona per le app che richiedono l'accesso "offline" a lungo termine in Azure.
...
## Operazioni eseguite dall'app Web
...
Per informazioni sulla creazione di un'app Active Directory con un certificato, vedere [Usare Azure PowerShell per creare un'entità servizio per accedere alle risorse](resource-group-authenticate-service-principal.md#create-service-principal-with-certificate) o [Usare l'interfaccia della riga di comando di Azure per creare un'entità servizio per accedere alle risorse](resource-group-authenticate-service-principal-cli.md#create-service-principal-with-certificate).

## Ottenere l'ID tenant dall'ID sottoscrizione
...
## Ottenere il token di accesso utente + app
...
I parametri della stringa di query disponibili per questa richiesta sono descritti nell'argomento [Richiedere un codice di autorizzazione](./active-directory/active-directory-protocols-oauth-code.md#request-an-authorization-code).
...
Se non si vuole solo accedere ad Azure Resource Manager per conto dell'utente, ma anche consentire all'utente di accedere all'applicazione usando l'account Azure AD, inviare una richiesta di autorizzazione OpenID Connect. Con OpenID Connect l'applicazione riceve anche un id\_token da Azure AD, che può essere usato dall'app per consentire l'accesso dell'utente.

I parametri della stringa di query disponibili per questa richiesta sono descritti nell'argomento [Inviare la richiesta di accesso](./active-directory/active-directory-protocols-openid-connect-code.md#send-the-sign-in-request).
...
I parametri della stringa di query disponibili per questa richiesta sono descritti nell'argomento [Usare un codice di autorizzazione](./active-directory/active-directory-protocols-oauth-code.md#use-the-authorization-code-to-request-an-access-token).
...
Quando si usano le credenziali certificato, creare un token Web JSON (JWT) e una firma (RSA SHA256) usando la chiave privata della credenziale certificato dell'applicazione. I tipi di attestazione per il token sono illustrati in [Attestazioni nei token JWT](./active-directory/active-directory-protocols-oauth-code.md#jwt-token-claims). Come riferimento, vedere [Active Directory Auth Library (.NET) code](https://github.com/AzureAD/azure-activedirectory-library-for-dotnet/blob/dev/src/ADAL.PCL.Desktop/CryptographyHelper.cs) (Codice di Active Directory Authentication Library - .NET) per firmare i token JWT per l'asserzione client.
...
I parametri da usare con la richiesta di aggiornamento sono descritti in [Aggiornamento dei token di accesso](./active-directory/active-directory-protocols-oauth-code.md#refreshing-the-access-tokens).
...
## Verificare se l'utente può assegnare l'accesso alla sottoscrizione
...
Per ogni sottoscrizione da connettere, chiamare l'API di [Resource Manager per elencare le autorizzazioni](https://msdn.microsoft.com/library/azure/dn906889.aspx) per determinare se all'utente sono concessi i diritti di gestione degli accessi per la sottoscrizione.
...
## Ottenere il token di accesso solo app
...
1. Registrare la connessione nella struttura dei dati delle "sottoscrizioni connesse" dell'applicazione, rendendo persistente l'ID della sottoscrizione.
...
I parametri della stringa di query disponibili per questa richiesta sono descritti nell'argomento [Richiedere un token di accesso](./active-directory/active-directory-protocols-oauth-service-to-service.md#request-an-access-token).
...
Chiamare l'[API per la definizione del ruolo di Resource Manager](https://msdn.microsoft.com/library/azure/dn906879.aspx) per elencare tutti i ruoli di Controllo degli accessi in base al ruolo di Azure e quindi scorrere il risultato per trovare la definizione di ruolo desiderata in base al nome.
...
A questo punto è disponibile tutto quanto è necessario per assegnare il ruolo Controllo degli accessi in base al ruolo appropriato all'entità servizio usando l'API di [Resource Manager per creare un'assegnazione di ruolo](https://msdn.microsoft.com/library/azure/dn906887.aspx).
