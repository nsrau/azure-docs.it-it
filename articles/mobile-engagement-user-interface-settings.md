<properties 
 ...
 description="Panoramica dell'interfaccia utente per la sezione Impostazioni di Azure Mobile Engagement" 
...
Le opzioni del menu Impostazioni disponibili per un'applicazione variano a seconda della piattaforma dell'applicazione e delle autorizzazioni di cui si dispone per l'applicazione. Le impostazioni sono le seguenti: Dettagli, Progetti, Push nativo, Velocità push, SDK, Rilevamento, Info App, Pressione commerciale e Autorizzazioni. Solo l'opzione di menu Info app della sezione Impostazioni dell'interfaccia utente contiene elementi che possono essere gestiti con la funzionalità "Tag" dell'API dispositivo. "Glossario" in "Concetti" contiene le definizioni dei termini e delle abbreviazioni, ad esempio: APNS, GCM, IDFA, API, SDK, chiave dell'API, chiave SDK, ID applicazione (ID app), ID AppStore, piano di tag, ID utente, ID dispositivo, delegato app, traccia di stack e collegamento diretto.

**Vedere anche:** 

- [Documentazione API - API dispositivo][Link 4], [Concetti - Glossario][Link 6][Guida per la risoluzione dei problemi - Assistenza][Link 2]
 
 ![settings1][46]

## Dettagli:
Consente di modificare il nome e la descrizione dell'applicazione 
Visualizzare il proprietario dell'applicazione e le autorizzazioni del ruolo. 
Configurazione analisi: Consente di visualizzare o modificare il giorno da cui iniziano le settimane e il tempo di conservazione in giorni:
...
## Push nativo:
Consente di registrare un nuovo certificato o di eliminare un certificato esistente per l'utilizzo con push nativo. 
Il push nativo consente a Azure Mobile Engagement di effettuare il push all'applicazione in qualsiasi momento, anche quando non è in esecuzione. 
Dopo aver fornito le credenziali o i certificati per almeno un servizio di push nativo, è possibile selezionare "In qualsiasi momento" in fase di creazione di campagne di copertura, nonché utilizzare il parametro "notifier" nell'API PUSH.

**Vedere anche:** 

- [Documentazione API - API Copertura] [Link 4], [Documentazione API - API Push] [Link 4][Documentazione dell'interfaccia utente - Copertura - Nuova campagna di push][Link 17]
 
**Servizio notifiche Push Apple (APNS)**

Per abilitare il push nativo tramite il Servizio notifiche Push Apple è necessario registrare il certificato. Occorre specificare il tipo di certificato: sviluppo (DEV) o produzione (PROD) e quindi caricare il certificato e la password.
...
Per abilitare il push nativo tramite il Servizio notifica di Windows, è necessario fornire le credenziali dell'applicazione. 
Sono necessari l'identificatore di sicurezza del pacchetto (SID) e la chiave segreta.
 
![settings5][50]
 
**Push nativo Android con GCM o ADM**

**Google Cloud Messaging per Android (GCM)**

Per abilitare il push nativo tramite GCM, è necessario seguire le istruzioni fornite in http://developer.android.com/guide/google/gcm/gs.html. 
È necessario, quindi, incollare una chiave API semplice del server, configurata senza restrizioni IP. 
Richiede l'integrazione con SDK per Android v1.12.0+.
...
Per abilitare il push nativo utilizzando ADM, è necessario fornire <credenziali OAuth> costituite da ID client e segreto client (richiede l'integrazione con SDK per Android v2.1.0 +).
...
## Velocità di push:
Mostra la velocità di push corrente dell'applicazione e consente di definire la velocità di push dell'applicazione. La velocità di push definisce il numero massimo di push al secondo che saranno eseguiti dal modulo Copertura. Questa impostazione può essere utile nel caso in cui i server sono sovraccarichi a causa di un numero eccessivo di richieste al secondo dopo l'attivazione di una campagna.
...
- Utilizzare un Server Active Directory di terze parti (attualmente Azure Mobile Engagement supporta [SmartAd](http://smartadserver.fr/) e [Surikate](http://www.surikate.com/)).
- Utilizzare un servizio di attribuzione di terze parti (Azure Mobile Engagement supporta attualmente [Mobile App Tracking]( http://www.mobileapptracking.com/) e [Trademob](http://www.trademob.com/) potrebbe essere supportato a breve).
- Utilizzare un referrer di installazione di terze parti (Azure Mobile Engagement supporta attualmente [Google Play Install Referrer](https://developers.google.com/app-conversion-tracking/docs/third-party-trackers/) - solo Android).
- Utilizzare la segnalazione manuale.
...
È inoltre possibile creare un URL di rilevamento che consente agli utenti di scaricare una delle applicazioni dall'interno della sezione di contenuto di una nuova campagna di copertura.

**Vedere anche:** 

- [Documentazione dell'interfaccia utente - Copertura - Contenuto del push][Link 17]
...
La registrazione di tag di informazioni dell'applicazione consente di segmentare le campagne di copertura creando criteri di destinatari di copertura specifici. È possibile visualizzare il nome e il tipo di tag di info app esistenti o aggiungere nuove info app in base al nome e al tipo: stringa, data, numero intero o booleano.
...
- Quota per segmento: Poiché una quota deve applicarsi a un gruppo di utenti, è necessario creare un segmento per identificare gli utenti a cui applicare una quota. Potrebbe essere un segmento di  'utenti pesanti'definito in base al numero di sessioni create o a qualsiasi elemento ritenuto interessante per definire il gruppo di utenti. Se un dispositivo corrisponde a più segmenti con una quota definita, viene applicata solo la quota con il numero massimo più alto di push all'ora.
...
- "Creatore campagna di copertura" (un utente che può creare le campagne di copertura) 
- "Responsabile campagna di copertura" (un utente che può creare campagne di copertura e attivarle, sospendere, terminarle ed eliminarle)

> Nota: quando un utente ha un ruolo di progetto e dispone anche di un gruppo di autorizzazioni per una determinata applicazione, viene utilizzato il concetto più permissivo. Di conseguenza, quando si utilizzano le autorizzazioni, è consigliabile impostare il ruolo di progetto degli utenti su "Visualizzatore" (il ruolo più restrittivo) e aggiungere autorizzazioni più permissive a livello di applicazione.
