...
Questo è il terzo passaggio della procedura dettagliata [Sviluppare una soluzione predittiva con Azure Machine Learning][Sviluppare una soluzione predittiva con Azure Machine Learning]:
...
# Passaggio 3: Creare un nuovo esperimento di Azure Machine Learning
...
2.  Trascinare il modulo **Editor metadati** nell'area di disegno e rilasciarlo sotto al set di dati.
...
5.  Verificare che "Tutte le colonne" sia selezionato nell'elenco a discesa, quindi fare clic su **OK**. Questo indicherà a **Editor metadati** di lavorare su tutte le colonne di dati.
...
Il prossimo passaggio dell'esperimento consiste nel generare set di dati separati che verranno usati per le attività di training e di test del modello. Per eseguire questa operazione, viene impiegato il modulo **Dividi**.

1.  Trovare il modulo **Dividi**, trascinarlo nell'area di disegno, quindi connetterlo all'ultimo modulo **Editor metadati**.
...
1.  Selezionare e trascinare il modulo **Esecuzione script R** nell'area di disegno dell'esperimento e connetterlo alla porta di output sinistra del modulo **Dividi**.
