...
description: Quando si usa la gestione delle risorse delegate di Azure, è importante considerare la sicurezza e il controllo degli accessi.
ms.date: 03/24/2020
ms.topic: conceptual
...
Quando si usa la gestione delle risorse delegate di [Azure,](azure-delegated-resource-management.md)è importante considerare la sicurezza e il controllo degli accessi. Gli utenti nel tenant avranno accesso diretto alle sottoscrizioni dei clienti e ai gruppi di risorse, pertanto è consigliabile adottare le misure necessarie per mantenere la sicurezza del tenant. Ti consigliamo anche di assicurarti di consentire solo l'accesso necessario per gestire in modo efficace le risorse dei tuoi clienti. Questo argomento fornisce indicazioni utili a tale scopo.

## <a name="require-azure-multi-factor-authentication"></a>Richiedere Azure Multi-Factor Authentication

[Azure Multi-Factor Authentication](../../active-directory/authentication/concept-mfa-howitworks.md) (noto anche come verifica in due passaggi) consente di impedire agli utenti malintenzionati di accedere a un account richiedendo più passaggi di autenticazione. È consigliabile richiedere Multi-Factor Authentication per tutti gli utenti nel tenant del provider di servizi, inclusi gli utenti che avranno accesso alle risorse dei clienti.
...
Per semplificare la gestione, è consigliabile usare i gruppi di utenti di Azure AD per ogni ruolo necessario per gestire le risorse dei clienti. Ciò consente di aggiungere o rimuovere singoli utenti al gruppo in base alle esigenze, invece di assegnare le autorizzazioni direttamente a tale utente.
