...
In questa esercitazione verrà illustrato come distribuire un'applicazione Web ASP.NET in un sito Web di Azure utilizzando la procedura guidata Pubblica sito Web in Visual Studio 2013 o Visual Studio Express 2013 per il Web. Se si preferisce utilizzare Visual Studio 2012, vedere la [versione precedente di questa esercitazione](/en-us/develop/net/tutorials/get-started-vs2012/).
...
Nel passaggio successivo verrà creato il sito Web di Azure e il database SQL che verranno utilizzati dall'applicazione.
...
3.  Nel passaggio **New Web Site** della procedura guidata immettere nella casella **URL** una stringa da utilizzare come URL univoco per l'applicazione. L'URL completo sarà costituito da quanto immesso in questa casella e dal suffisso visualizzato sotto la casella di testo. Nell'illustrazione è visualizzato "contactmgr11", ma poiché è probabile che questo URL sia già in uso, sarà necessario sceglierne un altro.

4.  Nell'elenco a discesa **Region** scegliere l'area geografica più vicina.

5.  Nell'elenco a discesa **Database** scegliere **Create a free 20 MB SQL database**.

    ![Passaggio Create a New Web Site della procedura guidata New Web Site - Create with Database](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/rrCWS.png)
...
7.  Nella casella **Name** immettere *ContactDB*.
...
11. Fare clic sul segno di spunta nella parte inferiore della finestra di dialogo per indicare che le operazioni sono state completate.
    ...
    ![Passaggio Database Settings della procedura guidata New Web Site - Create with Database](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/dntutmobile-setup-azure-site-004.png)

    Nell'immagine seguente viene mostrato l'utilizzo di un database SQL Server e un accesso esistenti.

    ![Passaggio Database Settings della procedura guidata New Web Site - Create with Database](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/rxPrevDB.png)
...
3.  Nella finestra di dialogo **Nuovo progetto** espandere **Visual C\#** e selezionare **Web**, quindi selezionare **Applicazione Web ASP.NET MVC 5**. Mantenere l'opzione **.NET Framework 4.5** predefinita. Assegnare all'applicazione il nome **ContactManager** e fare clic su **OK**. ![Finestra di dialogo Nuovo progetto](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/rr4.PNG)
...
5.  Nella finestra di dialogo **Modifica autenticazione** fare clic su **Nessuna autenticazione**, quindi fare clic su **OK**.
    ...
    L'applicazione di esempio da creare non includerà funzionalità che richiedono l'accesso degli utenti. Per informazioni su come implementare la funzionalità di autenticazione e autorizzazione, vedere la sezione [Passaggi successivi](#nextsteps) alla fine dell'esercitazione.
...
1.  In Visual Studio fare clic con il pulsante destro del mouse sul progetto in **Esplora soluzioni** e scegliere **Pubblica** dal menu di scelta rapida.
    ...
    ![Opzione Pubblica nel menu di scelta rapida del progetto](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/PublishVSSolution.png)
...
2.  Nella scheda **Profilo** della procedura guidata **Pubblica sito Web** fare clic su **Importa**.
    ...
    ![Importazione delle impostazioni di pubblicazione](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/ImportPublishSettings.png)

    Verrà visualizzata la finestra di dialogo **Import Publish Profile**.

3.  Selezionare Import from an Azure Web Site. Se non lo si è già fatto, è innanzitutto necessario eseguire l'accesso. Fare clic su **Sign In**. Immettere l'utente associato alla sottoscrizione corrente e seguire le istruzioni visualizzate.
...
7.  Nella scheda **Preview** fare clic su **Start Preview**.
    ...
    ![Pulsante StartPreview nella scheda Preview](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/dntutmobile-deploy1-publish-007.png)
...
9.  Nel browser predefinito verrà automaticamente aperto l'URL del sito distribuito.
    ...
    ![Pagina iniziale Elenco azioni in esecuzione in Azure](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/rxz2.png)
...
5.  Impostare il nome del controller su **HomeController**. Selezionare **Contact** come classe del modello. Fare clic sul pulsante **Nuovo contesto dati** e accettare il contesto predefinito "ContactManager.Models.ContactManagerContext" per **Nuovo tipo di contesto dati**. Fare clic su **Aggiungi**.
    ...
    Verrà visualizzata una finestra di dialogo con il messaggio: "Un file con nome HomeController esiste già. Sostituirlo?" Fare clic su **Sì**. Il file HomeController creato con il nuovo progetto verrà sovrascritto. Per l'elenco dei contatti, verrà utilizzato il nuovo file HomeController.
...
7.  In **Console di Gestione pacchetti** immettere il comando seguente:
    ...
    ![Comandi di Console di Gestione pacchetti](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/dntutmobile-migrations-package-manager-console.png)
...
1.  Aprire il file *Views\\Home\\Index.cshtml*. Nel passaggio successivo, il markup generato verrà sostituito con codice che utilizza [jQuery](http://jquery.com/) e [Knockout.js](http://knockoutjs.com/). Questo nuovo codice recupera l'elenco dei contatti utilizzando l'API Web e JSON e quindi associa i dati dei contatti all'interfattia utente tramite knockout.js. Per ulteriori informazioni, vedere la sezione [Next Steps](#nextsteps) alla fine di questa esercitazione.
...
4.  Nella finestra di dialogo **Aggiungi nuovo elemento** immettere **Stile** nella casella di ricerca in alto a destra e quindi selezionare **Foglio di stile**.
	...
	![Finestra di dialogo Aggiungi nuovo elemento](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/rxStyle.png)
...
1.  Premere CTRL+F5 per eseguire l'applicazione.
    ...
    ![Pagina di indice](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/dntutmobil-intro-finished-web-app.png)
...
3.  Nel browser aggiungere **/api/contacts** all'URL.
    ...
    ![Pagina di indice con elementi dell'elenco azioni](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/rxFFchrome.png)
    ...
    ![Finestra di dialogo Salva dell'API Web](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/dntutmobile-webapi-contacts-in-notepad.png)

    **Avviso di sicurezza**: a questo punto, l'applicazione non è sicura ed è vulnerabile agli attacchi di richiesta intersito falsa (Cross-Site Request Forgery, CSRF). Questa vulnerabilità verrà rimossa più avanti nell'esercitazione. Per ulteriori informazioni, vedere l'articolo che descrive come [prevenire gli attacchi di richiesta intersito falsa (CSRF)](http://www.asp.net/web-api/overview/security/preventing-cross-site-request-forgery-(csrf)-attacks).

XSRFAggiunta della protezione XSRF
----------------------------------

La richiesta intersito falsa (nota anche come XSRF o CSRF) è un attacco contro applicazioni ospitate sul Web in base al quale un sito Web dannoso può influenzare l'interazione tra un browser client e un sito Web considerato attendibile da tale browser. Questi attacchi sono possibili in quanto i Web browser inviano automaticamente token di autenticazione con ogni richiesta a un sito Web. Un classico esempio è un cookie di autenticazione, ad esempio un ticket di autenticazione basata su form di ASP.NET. Tuttavia, i siti Web che utilizzano un meccanismo di autenticazione persistente, ad esempio Autenticazione di Windows, autenticazione di base e così via, possono essere presi di mira da questi attacchi.
...
1.  In **Esplora soluzioni** fare clic con il pulsante destro del mouse sul progetto, quindi scegliere **Pubblica**.
    ...
    ![Opzione Pubblica](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/rxP.png)
...
6.  Fare clic su **Pubblica**. Al termine della distribuzione, nel browser verrà aperta la pagina iniziale dell'applicazione.
    ...
    ![Pagina di indice senza contatti](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/dntutmobil-intro-finished-web-app.png)
...
![Pagina di indice con contatti](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/dntutmobile-webapi-added-contact.png)
...
-   [Debug di WAWS](http://www.windowsazure.com/en-us/documentation/articles/web-sites-dotnet-troubleshoot-visual-studio/)
