<properties
	pageTitle="Database NoSQL - Introduzione a DocumentDB Node.js SDK | Microsoft Azure"
	description="Informazioni su come connettersi a un account DocumentDB, creare un database NoSQL ed eseguire query in DocumentDB mediante Node.js in Windows, Linux o OS/X."
...
#Introduzione a DocumentDB Node.js SDK  

> [AZURE.SELECTOR]
- [.NET](documentdb-get-started.md)
- [Node.js](documentdb-nodejs-get-started.md)

Questa è una Guida introduttiva a DocumentDB Node.js SDK. Dopo aver seguito questa esercitazione, si otterrà un'applicazione console che consente di creare e ridefinire le query delle risorse DocumentDB.
...
Successivamente, usare i pulsanti di voto all'inizio o alla fine di questa pagina per fornire commenti e suggerimenti. Se si desidera contattarci, è possibile includere l'indirizzo di posta elettronica nel commento per il follow-up.
...
La procedura è completa. Dopo avere completato l'installazione, è possibile iniziare a scrivere il codice.

##<a id="Config"></a> Passaggio 3: Impostare le configurazioni dell'app

Aprire il file *config.js* in un editor di testo.

Creare quindi un oggetto vuoto denominato *config* e impostare le proprietà *config.endpoint* e *config.authKey* sull'endpoint e la chiave di autorizzazione di DocumentDB. Entrambe le configurazioni sono disponibili nel [portale di anteprima di Azure](https://portal.azure.com).
...
Aggiungere l'*ID database*, l'*ID raccolta* e i *documenti JSON* all'oggetto *config*. Aggiungere il codice seguente dopo la posizione in cui si configurano le proprietà *config.endpoint* e *config.authKey*. Se sono già disponibili dati da archiviare nel database, è possibile usare l'[utilità di migrazione dei dati](documentdb-import-data.md) di DocumentDB invece di aggiungere le definizioni dei documenti.
...
Le definizioni del database, della raccolta e del documento verranno usate come *ID database*, *ID raccolta* e dati dei documenti di DocumentDB.

Esportare infine l'oggetto *config*, in modo che sia possibile fare riferimento a questo oggetto nel file *app.js*.
...
È possibile creare una [raccolta](documentdb-resources.md#collections) mediante la funzione [createCollection](https://azure.github.io/azure-documentdb-node/DocumentClient.html) della classe **DocumentClient**. Una raccolta è un contenitore di documenti JSON e di logica dell'applicazione JavaScript associata. La raccolta appena creata verrà mappata a un [livello di prestazioni S1](documentdb-performance-levels.md). Aggiungere una funzione per la creazione della nuova raccolta nel file app.js con l'*ID* specificato nell'oggetto *config*. Anche in questo caso è prima di tutto necessario verificare che non esista già una raccolta con lo stesso ID *FamilyCollection*. Se esiste, verrà restituita la raccolta esistente, invece di procedere con la creazione di una nuova raccolta.
...
È possibile creare un [documento](documentdb-resources.md#documents) mediante la funzione [createDocument](https://azure.github.io/azure-documentdb-node/DocumentClient.html) della classe **DocumentClient**. I documenti sono contenuto JSON definito dall'utente (arbitrario). È ora possibile inserire un documento in DocumentDB.
...
##<a id="DeleteDatabase"></a>Passaggio 9: Eliminare il database
...
##<a id="Build"></a>Passaggi 10: Operazioni finali
...
##<a id="Run"></a>Passaggio 11: Eseguire l'applicazione

È ora possibile eseguire l'applicazione.

Nel terminale individuare il file *app.js* ed eseguire il comando **node app.js**.
...
A questo punto È stata creata la prima applicazione DocumentDB!
...
Nel file *config.js* aggiornare quindi i valori config.endpoint e config.authKey, come illustrato in [Passaggio 3: Impostare le configurazioni dell'app](#Config).
...
-	Per informazioni dettagliate sul modello di programmazione, vedere la sezione Sviluppo in [Documentazione di DocumentDB](../../services/documentdb/).
