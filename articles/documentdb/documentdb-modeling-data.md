...
Con questo approccio ora è stato **denormalizzato** il record della persona, in cui sono state **incorporate** tutte le informazioni relative a questa persona, ad esempio i dettagli contatto e gli indirizzi, in un singolo documento JSON.
...
In un database relazionale le relazioni *many:many* vengono spesso modellate con le tabelle join, che creano un join dei record delle altre tabelle. 
...
Per altre informazioni su Azure DocumentDB, fare riferimento alla pagina della [documentazione del servizio]( ../../services/documentdb/). 
