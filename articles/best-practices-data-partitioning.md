...
> Per ulteriori informazioni e istruzioni sulla maggior parte di queste considerazioni e tecniche consigliate per la progettazione di archivi dati che implementano il partizionamento orizzontale, vedere il [Modello di partizionamento orizzontale]
...
2. In base all'analisi, si determineranno gli obiettivi di scalabilità attuali e futuri, ad esempio le dimensioni dei dati e il carico di lavoro e si distribuiranno i dati in tutte le partizioni per soddisfare l'obiettivo di scalabilità. Nella strategia di partizionamento orizzontale è importante scegliere la chiave di partizionamento appropriata per assicurarsi che la distribuzione sia uniforme. Per altre informazioni, vedere il [Modello di partizionamento].
...
Un singolo database SQL ha un limite per il volume dei dati che può contenere e la velocità effettiva è vincolata da fattori di architettura e dal numero di connessioni simultanee che supporta. Database SQL di Azure offre un database elastico per supportare la scalabilità orizzontale per un database SQL. Usando un database elastico è possibile partizionare i dati in partizioni distribuite in più database SQL, nonché aggiungere o rimuovere partizioni quando il volume di dati da gestire aumenta e si riduce. Usando un database elastico è anche possibile ridurre i conflitti distribuendo il carico tra i database.

> [AZURE.NOTE]Il database elastico è attualmente in anteprima a partire da dicembre 2015. È una sostituzione per le federazioni di Database SQL Azure che verranno ritirate. Le installazioni esistenti di federazione di Database SQL Azure possono essere migrate al database elastico usando l’[Utilità di migrazione di federazioni]. In alternativa, è possibile implementare un proprio meccanismo di partizionamento orizzontale se lo scenario non si presta naturalmente alle funzionalità offerte dal database elastico.
...
- Se si eseguono regolarmente query per cercare i dati che utilizzano campi diversi dalle chiavi di partizione e di riga, si consiglia di implementare l’ [Index Table Pattern].
...
- Le risorse disponibili per un database di DocumentDB sono soggette a limitazioni di quota dell'account DocumentDB. Ogni database può contenere un numero di raccolte (anche in questo caso, esiste un limite) e ogni raccolta è associata a un livello di prestazioni che regola il limite di velocità RU (velocità effettiva riservata) per la raccolta. Per ulteriori informazioni, visitare la pagina[DocumentDB limiti e quote] sul sito Web Microsoft.
...
Per un esempio di una soluzione che supporta la migrazione online, vedere il documento online [Scalabilità tramite lo strumento di suddivisione-unione del database elastico] nel sito Web Microsoft.
...
- La pagina [Indicazioni di coerenza dati], disponibile nel sito Web Microsoft, descrive le strategie per la gestione della coerenza in un ambiente distribuito, ad esempio il cloud.
- La pagina[Indicazioni di partizionamento dati] sul sito Web Microsoft fornisce una panoramica generale di progettazione di partizioni per soddisfare diversi criteri in una soluzione distribuita.
- Il [Modello di partizionamento orizzontale], descritto nel sito Web Microsoft, riepiloga alcune strategie comuni per i dati di partizionamento orizzontale.
- [Index Table Pattern] descritto sul sito Web Microsoft viene illustra come creare gli indici secondari sui dati. Questo approccio consente a un'applicazione di recuperare rapidamente i dati utilizzando le query che non fanno riferimento alla chiave primaria di una raccolta.
- [Materialized View Pattern] descritto sul sito Web Microsoft descrive come generare visualizzazioni preinserite che riepilogano i dati per supportare velocemente le operazioni di query. Questo approccio può essere utile in un archivio dati partizionati se le partizioni che contengono dati riepilogati vengono distribuite tra più siti.
- L’articolo [Uso della rete CDN per Azure] contiene altre informazioni sulla configurazione e sull’uso della rete CDN con Azure.
...
- L'argomento [Scalabilità tramite lo strumento di suddivisione-unione del database elastico] nel sito Web Microsoft contiene informazioni sull'uso del servizio di suddivisione/unione per gestire partizioni con database elastico.
...
- La pagina[Esecuzione di transazioni del gruppo di entità] del sito Web Microsoft fornisce informazioni dettagliate sull'implementazione di operazioni transazionali su entità archiviate in archiviazione tabelle di Azure.
- L'articolo [Guida alla progettazione della tabella di archiviazione di Azure] nel sito Web Microsoft contiene informazioni dettagliate sul partizionamento dei dati nell'archiviazione tabelle di Azure.
- La pagina [Uso della rete CDN per Azure] sul sito Web Microsoft illustra come replicare dati in archiviazione Blob di Azure tramite la rete per la distribuzione di contenuti (rete CDN).
...
- La pagina [Che cos'è la Ricerca di Azure?] nel sito Web Microsoft contiene una descrizione completa delle funzionalità disponibili con il servizio Ricerca di Azure.
- La pagina [Limiti dei servizi in Ricerca di Azure] nel sito Web Microsoft contiene informazioni sulla capacità di ogni istanza del servizio Ricerca di Azure.
- La pagina [Tipi di dati supportati (ricerca di Azure)] sul sito Web Microsoft riepiloga i tipi di dati che è possibile utilizzare nei documenti e indici in cui è possibile eseguire ricerche.
- La pagina [Cache Redis di Azure] nel sito Web Microsoft offre un'introduzione a Cache Redis di Azure.
- La pagina [Partizionamento: come suddividere i dati tra più istanze di Redis] sul sito Web Redis fornisce informazioni sull'implementazione del partizionamento con Redis.
- La pagina [Esecuzione Redis in una macchina virtuale CentOS Linux in Azure] sul sito Web Microsoft illustra un esempio che mostra come creare e configurare un nodo Redis in esecuzione come una macchina virtuale di Azure.
- La pagina [Tipi di dati] sul sito Web di Redis descrive i tipi di dati che sono disponibili con Redis e Cache Redis di Azure.
...
[Accesso ai dati per le soluzioni altamente scalabili mediante l’utilizzo di SQL NoSQL e Polyglot Persistence]: https://msdn.microsoft.com/library/dn271399.aspx
[Indicazioni di coerenza dati]: http://aka.ms/Data-Consistency-Primer
[Indicazioni di partizionamento dati]: https://msdn.microsoft.com/library/dn589795.aspx
...
[Capacità di gestire DocumentDB]: documentdb/documentdb-manage.md
...
[Partizionamento: come suddividere i dati tra più istanze di Redis]: http://redis.io/topics/partitioning
...
[Esecuzione di transazioni del gruppo di entità]: https://msdn.microsoft.com/library/azure/dd894038.aspx
[Esercitazione del cluster Redis]: http://redis.io/topics/cluster-tutorial
[Esecuzione Redis in una macchina virtuale CentOS Linux in Azure]: http://blogs.msdn.com/b/tconte/archive/2012/06/08/running-redis-on-a-centos-linux-vm-in-windows-azure.aspx
...
[ServicePoint Class]: cdn/cdn-how-to-use-cdn.md
