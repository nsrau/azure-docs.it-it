---
title: Creare cluster di Azure HDInsight con Data Lake Storage Gen1 - portaleCreate Azure HDInsight clusters with Data Lake Storage Gen1 - portal
...
Informazioni su come usare il portale di Azure per creare un cluster HDInsight con un account Azure Data Lake Storage Gen1 come risorsa di archiviazione predefinita o risorsa di archiviazione aggiuntiva. Anche se lo spazio di archiviazione aggiuntivo è facoltativo per un cluster HDInsight, è consigliabile archiviare i dati aziendali negli account di archiviazione aggiuntivi.
...
In questa sezione viene creato un cluster HDInsight con account Data Lake Storage Gen1 come risorsa di archiviazione predefinita o aggiuntiva. Questo articolo è incentrato solo sulla configurazione degli account Data Lake Storage Gen1. Per informazioni generali sulla creazione di cluster e le relative procedure, vedere [Creare cluster Hadoop basati su Linux in HDInsight](../hdinsight/hdinsight-hadoop-provision-linux-clusters.md).
...
Per creare un cluster HDInsight con un account Data Lake Storage Gen1 come account di archiviazione predefinito:To create an HDInsight cluster with a Data Lake Storage Gen1 account as the default storage account:
...
Per creare un cluster HDInsight con un account Data Lake Storage Gen1 come account di archiviazione aggiuntivo:To create a HDInsight cluster with a Data Lake Storage Gen1 account as an additional storage account:
...
3. Nel pannello **Archiviazione**, in **Tipo di archiviazione primario** selezionare **Archiviazione di Azure** e quindi immettere le informazioni seguenti:
    ...
    * **Account di archiviazione aggiuntivi:** aggiungere altri account di archiviazione di Azure come spazio di archiviazione aggiuntivo.
    * **Accesso all'archivio dati:** configurare l'accesso tra l'account Data Lake Storage Gen1 e il cluster HDInsight.Data Lake Store access - Configure access between the Data Lake Storage Gen1 account and HDInsight cluster. Per istruzioni, vedere [Configure Data Lake Storage Gen1 access](#configure-data-lake-storage-gen1-access).
...
Per creare un'entità servizio dal portale di Azure:To create a service principal from the Azure portal:

1. Selezionare **l'accesso data Lake Store** dal pannello Archiviazione.Select Data Lake Store access from the Storage blade.
1. Nel pannello di **accesso Gen1 di Data Lake Storage** selezionare Crea **nuovo**.
1. Selezionare **Entità servizio**e quindi seguire le istruzioni per creare un'entità servizio.
...
Per usare un'entità servizio esistente dal portale di Azure:To use an existing service principal from the Azure portal:

1. Selezionare **l'accesso data Lake Store**.
1. Nel pannello di **accesso Gen1 di Data Lake Storage** selezionare Usa **esistente**.
...
La configurazione è diversa a seconda che l'account venga usato come risorsa di archiviazione predefinita o come account di archiviazione aggiuntivo:The configuration is different depending on whether the account is used as the default storage or an additional storage account:
...
Per assegnare l'autorizzazione a livello radice dell'account Data Lake Storage Gen1:

1. Nel pannello di **accesso Gen1 di Data Lake Storage** selezionare **Access**. Viene aperto il pannello per la **Selezionare le autorizzazioni file**. Questo pannello elenca tutti gli account Data Lake Storage Gen1 nella sottoscrizione.
1. Passare il mouse (non fare clic) sul nome dell'account Data Lake Storage Gen1 per rendere visibile la casella di controllo e selezionarla.
   ...
   Per impostazione predefinita, __l'opzione READ__, __WRITE__e __EXECUTE__ sono tutte selezionate.

1. Fare clic su **Seleziona** nella parte inferiore della pagina.
1. Selezionare **Esegui** per assegnare l'autorizzazione.
1. Selezionare **Fatto**.

Per assegnare l'autorizzazione a livello di radice del cluster HDInsight:To assign permission at the HDInsight cluster root level:

1. Nel pannello di **accesso Gen1 di Data Lake Storage** selezionare **Access**. Viene aperto il pannello per la **Selezionare le autorizzazioni file**. Questo pannello elenca tutti gli account Data Lake Storage Gen1 nella sottoscrizione.
1. Nel pannello **Seleziona autorizzazioni file** selezionare il nome dell'account Gen1 di Data Lake Storage per visualizzarne il contenuto.
...
![Assegnare le autorizzazioni dell'entità servizio al cluster HDInsightAssign service principal permissions to the HDInsight cluster](./media/data-lake-store-hdinsight-hadoop-use-portal/hdi.adl.3-1.png "Assegnare le autorizzazioni dell'entità servizio al cluster HDInsightAssign service principal permissions to the HDInsight cluster")

## <a name="verify-cluster-setup"></a><a name="verify-cluster-set-up"></a>Verificare la configurazione del clusterVerify cluster setup
...
* Per verificare che l'archiviazione associata per il cluster sia l'account Data Lake Storage Gen1 specificato, selezionare **Account di archiviazione** nel riquadro sinistro.
...
* Per verificare che l'entità servizio sia associata correttamente al cluster HDInsight, selezionare **accesso Data Lake Storage Gen1** nel riquadro sinistro.
