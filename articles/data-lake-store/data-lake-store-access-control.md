...
| **Scrittura (W)** |È possibile scrivere o aggiungere in un file |Per creare elementi figlio in una cartella sono necessarie le autorizzazioni **Scrittura + Esecuzione**. |
...
La forma **RWX** viene usata per indicare **Lettura + Scrittura + Esecuzione**. Esiste una forma numerica ridotta in cui **Lettura=4**, **Scrittura=2** ed **Esecuzione=1** e la somma rappresenta le autorizzazioni. Di seguito sono riportati alcuni esempi.
...
## <a name="the-super-user"></a>Superuser
...
> [!NOTE]
> Per un nuovo account Data Lake Store, l'impostazione predefinita di mask per l'ACL di accesso e l'ACL predefinito della cartella radice ("/") è RWX.
> 
> 
...
### <a name="a-child-file-or-folders-access-acl"></a>ACL di accesso di una cartella o un file figlio
...
## <a name="common-questions-for-acls-in-data-lake-store"></a>Domande comuni sugli ACL in Data Lake Store
...
### <a name="who-is-set-as-the-owning-group-of-a-file-or-folder-at-creation"></a>Quale gruppo viene impostato come proprietario di un file o una cartella al momento della creazione?
