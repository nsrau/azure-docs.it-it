<properties title="Learn how to configure an Azure website that uses Traffic Manager to use a domain name registered with Directnic" pageTitle="Configurare un nome di dominio Directnic per un sito Web di Azure usando Gestione traffico" metaKeywords="Windows Azure, Windows Azure Web Sites, Directnic, Traffic Manager" description="Informazioni su come configurare un sito Web di Azure che usa Gestione traffico per l'uso di un nome di dominio registrato con Directnic" services="web-sites" documentationCenter="" authors="larryfr,jroth" manager="wpickett" />
