...
![diagramma dell'architettura](./media/apache-storm-sensor-data-analysis/devicesarchitecture.png)
...
La topologia legge i dati dall'hub eventi usando la classe `org.apache.storm.eventhubs.spout.EventHubSpout` e scrive i dati in HBase usando la classe `org.apache.storm.hbase.bolt.HBaseBolt`. Per la comunicazione con il sito Web viene usato [socket.io-client.java](https://github.com/nkzawa/socket.io-client.java).

Nel diagramma seguente viene illustrato il layout della topologia:

![diagramma della topologia](./media/apache-storm-sensor-data-analysis/sensoranalysis.png)
...
Prima di usare questo esempio, è necessario preparare l'ambiente di sviluppo. È anche necessario creare un hub eventi di Azure, che viene usato da questo esempio.

Nell'ambiente di sviluppo devono anche essere installati gli elementi seguenti:
...
Per creare un hub eventi, seguire i passaggi indicati nel documento [Creare un hub eventi](../../event-hubs/event-hubs-create.md).

> [!IMPORTANT]
> Salvare il nome dell'hub eventi, lo spazio dei nomi e la chiave per RootManageSharedAccessKey. Queste informazioni vengono usate per configurare la topologia di Storm.

Non è necessario un cluster HDInsight. I passaggi di questo documento forniscono un modello di Azure Resource Manager che crea le risorse necessarie per questo esempio. Il modello crea le risorse seguenti:
...
3. Aprire un Web browser e immettere `http://localhost:3000/` come indirizzo. Viene visualizzata una pagina simile all'immagine seguente:
    ...
    ![dashboard Web](./media/apache-storm-sensor-data-analysis/emptydashboard.png)
...
2. Aprire il file `app.js` in un editor di testo e aggiungere le informazioni relative all'hub eventi ottenute in precedenza:
   ...
   > [!NOTE]
   > Questo esempio presuppone che si usi `RootManageSharedAccessKey` per accedere all'hub eventi.
...
2. Per avviare la topologia in modalità locale, usare il comando seguente:
   ...
   Una volta avviata, la topologia legge le voci dall'hub eventi e le invia al dashboard in esecuzione sul computer locale. Nel dashboard Web dovrebbero comparire linee simili a quelle nell'immagine seguente:
    ...
    ![dashboard con dati](./media/apache-storm-sensor-data-analysis/datadashboard.png)
...
2. Immettere i valori seguenti dalla sezione **Distribuzione personalizzata**:
    ...
    ![Parametri di HDInsight](./media/apache-storm-sensor-data-analysis/parameters.png)
...
![Gruppo di risorse per la rete virtuale e i cluster](./media/apache-storm-sensor-data-analysis/groupblade.png)
...
1. Usare uno degli esempi seguenti per recuperare il quorum di Zookeeper per il cluster HBase:
    ...
    >
    > Questo esempio richiede Azure PowerShell. Per altre informazioni sull'uso di Azure PowerShell, vedere [Getting started with Azure PowerShell](https://docs.microsoft.com/powershell/scripting/Getting-Started-with-Windows-PowerShell?view=powershell-6) (Introduzione ad Azure PowerShell)
...
Per altri esempi di topologie Storm con HDInsight, vedere [Topologie di esempio per Storm in HDInsight](apache-storm-example-topology.md).

Per altre informazioni su Apache Storm, visitare il sito Web [Apache Storm](https://storm.incubator.apache.org/) .

Per altre informazioni su HBase in HDInsight, vedere la [panoramica su HBase con HDInsight](../hbase/apache-hbase-overview.md)
...
Per altre informazioni sulla creazione di topologie in Java, vedere [Sviluppo di topologie basate su Java per Apache Storm in HDInsight](apache-storm-develop-java-topology.md).

Per altre informazioni sulla creazione di topologie in .NET, vedere [Sviluppare topologie C# per Apache Storm in HDInsight tramite Visual Studio](apache-storm-develop-csharp-visual-studio-topology.md).
