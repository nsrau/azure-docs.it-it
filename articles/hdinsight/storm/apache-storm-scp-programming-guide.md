...
SCP è una piattaforma per la compilazione di applicazioni di elaborazione dati dalle prestazioni elevate, affidabili, coerenti e in tempo reale. Si basa su [Apache Storm](https://storm.incubator.apache.org/) , un sistema di elaborazione dei flussi progettato dalle community OSS. Storm è stato progettato da Nathan Marz ed è stato open source da Twitter. Il sistema si avvale di [Apache Zookeeper](https://zookeeper.apache.org/), un altro progetto Apache per il coordinamento e la gestione dello stato altamente affidabili delle applicazioni distribuite.
...
L'estensione e la personalizzazione vengono eseguite in modo tale che non è necessario creare un fork dei progetti OSS ed è possibile sfruttare gli ecosistemi derivati basati su Storm.
...
In Storm, una topologia applicazione definisce un grafo di elaborazione. Ogni nodo in una topologia contiene logica di elaborazione e i collegamenti tra i nodi indicano il flusso dei dati. I nodi per l'inserimento dei dati di input nella topologia, denominati _spout_, possono essere usati per porre in sequenza i dati. I dati di input potrebbero risiedere in log di file, database transazionale, contatore delle prestazioni del sistema e così via. I nodi con flussi di dati sia di input che di output, denominati _bolt_, eseguono il filtraggio, la selezione e l'aggregazione dei dati effettivi.

SCP supporta le attività più appropriate, almeno una volta e esattamente una volta l'elaborazione dei dati. In un'applicazione di elaborazione di flussi distribuiti possono verificarsi diversi errori durante l'elaborazione dei dati, ad esempio l'interruzione della rete, l'errore del computer o l'errore del codice utente e così via. L'elaborazione at-least-once assicura che tutti i dati vengano elaborati almeno una volta, riproducendo automaticamente gli stessi dati in caso di errore. L'elaborazione at-least-once è semplice e affidabile e si adatta a molte applicazioni. Quando tuttavia un'applicazione richiede il conteggio esatto, l'elaborazione at-least-once non è sufficiente, perché potenzialmente nella topologia dell'applicazione potrebbero essere riprodotti gli stessi dati. In tal caso, esattamente una volta che l'elaborazione è progettata per assicurarsi che il risultato sia corretto anche quando i dati possono essere riprodotti ed elaborati più volte.

SCP consente agli sviluppatori .NET di sviluppare applicazioni di elaborazione dati in tempo reale sfruttando Java Virtual Machine (JVM) con Storm dietro le quinte. .NET e la JVM comunicano tramite socket locali TCP. Fondamentalmente ogni beccuccio/Bolt è una coppia di processi .NET/Java, in cui la logica utente viene eseguita nel processo .NET come plug-in.
...
* Progettare la topologia, quindi inviarla ed eseguirla. La topologia definisce i vertici e i flussi di dati tra i vertici. SCP recupererà le specifiche della topologia e le implementerà in un cluster Storm, in cui ogni vertice viene eseguito in un nodo logico. Il failover e la scalabilità verranno gestiti dall'utilità di pianificazione dell'attività Storm.
...
ISCPPlugin è l'interfaccia comune per tutti i tipi di plug-in. Attualmente si tratta di un'interfaccia fittizia.
...
Quando viene chiamato `NextTuple()` , il codice utente C# può emettere una o più tuple. Se non c'è nulla da emettere, questo metodo deve restituire senza emettere alcun elemento. Si noti che `NextTuple()`, `Ack()`e `Fail()` vengono tutti chiamati in un ciclo ridotto in un singolo thread in C# fase di elaborazione. Quando non sono presenti tuple da emettere, è cortese che NextTuple la sospensione per un breve periodo di tempo, ad esempio 10 millisecondi, in modo da non sprecare una quantità eccessiva di CPU.

`Ack()` e `Fail()` vengono chiamati solo se il meccanismo di acknowledgement è abilitato nel file delle specifiche. Il `seqId` viene utilizzato per identificare la tupla riconosciuta o non riuscita. Dunque, se l'acknowledgement è abilitato in una topologia non transazionale, nello Spout deve essere usata la funzione di emissione seguente:

    public abstract void Emit(string streamId, List<object> values, long seqId); 

Se ACK non è supportato nella topologia non transazionale, il `Ack()` e il `Fail()` possono essere lasciati come funzioni vuote.

Il `parms` parametro di input in queste funzioni è un dizionario vuoto, è riservato per un uso futuro.
...
Analogamente alla controparte non transazionale, `NextTx()`, `Ack()`e `Fail()` vengono tutti chiamati in un ciclo ridotto in un singolo thread in C# fase di elaborazione. Quando non sono presenti dati da emettere, è cortese avere `NextTx` sospensione per un breve periodo di tempo (10 millisecondi), in modo da non sprecare una quantità eccessiva di CPU.
...
Storm riprodurrà automaticamente una transazione in caso di errore, quindi `Fail()` non deve essere chiamato in modo normale. Se SCP può verificare i metadati emessi dallo Spout transazionale, tuttavia, può chiamare `Fail()` quando i metadati non sono validi.
...
`Execute()` viene chiamato quando ci sono nuove tuple che arrivano al Bolt. `FinishBatch()` viene chiamato al termine di questa transazione. Il parametro di output `parms` è riservato per l'utilizzo futuro.
...
In SCP.NET è disponibile un semplice set di oggetti chiave che gli sviluppatori possono usare per la programmazione. Si tratta di **context**, **StateStore**e **SCPRuntime**. Sono descritti nella parte restante di questa sezione.

### <a name="context"></a>Context

Context fornisce un ambiente di esecuzione per l'applicazione. Ogni istanza di ISCPPlugin (ISCPSpout/ISCPBolt/ISCPTxSpout/ISCPBatchBolt) dispone di un'istanza Context corrispondente. Le funzionalità fornite da context possono essere divise in due parti: (1) la parte statica, disponibile nell'intero C# processo, (2) la parte dinamica, disponibile solo per l'istanza di contesto specifica.
...
`TopologyContext` viene fornito per ottenere il contesto della topologia, è particolarmente utile per i componenti con più parallelismo. Esempio:
...
`Initialize()` viene usato per inizializzare l'ambiente di runtime SCP. In questo metodo, il C# processo si connette al lato Java e ottiene i parametri di configurazione e il contesto della topologia.

`LaunchPlugin()` viene usato per avviare il ciclo di elaborazione dei messaggi. In questo ciclo, il C# plug-in riceve messaggi dal lato Java (incluse Tuple e segnali di controllo) e quindi elabora i messaggi, forse chiamando il metodo di interfaccia fornito dal codice utente. Il parametro di input per il metodo `LaunchPlugin()` è un delegato che può restituire un oggetto che implementa l'interfaccia ISCPSpout/IScpBolt/ISCPTxSpout/ISCPBatchBolt.

    public delegate ISCPPlugin newSCPPlugin(Context ctx, Dictionary\<string, Object\> parms); 

Per ISCPBatchBolt, è possibile ottenere `StormTxAttempt` da `parms`e usarlo per valutare se si tratta di un tentativo riprodotto. Il controllo del tentativo di riproduzione viene spesso eseguito nel Bolt del commit e viene illustrato nell'esempio `HelloWorldTx`.
...
SCP Topology Specification è un linguaggio specifico di dominio per la descrizione e la configurazione di topologie SCP. Si basa sul linguaggio DSL Clojure di Storm (<https://storm.incubator.apache.org/documentation/Clojure-DSL.html>) ed è esteso da SCP.
...
| Funzioni nuove | Parametri | Description |
| --- | --- | --- |
...
| SCP-TX-beccuccio |exec-name<br />args<br />fields |Consente di definire uno Spout transazionale. Esegue l'applicazione con ***exec-name*** usando ***args***.<br /><br />***fields*** corrisponde ai campi di output per lo Spout. |
...
| SCP-beccuccio |exec-name<br />args<br />fields<br />parametri |Consente di definire uno Spout non transazionale. Esegue l'applicazione con ***exec-name*** usando ***args***.<br /><br />***fields*** corrisponde ai campi di output per lo Spout.<br /><br />***parameters*** è facoltativo e può essere usato per specificare parametri come ad esempio "nontransactional.ack.enabled". |
...
| --- | --- |
| : nome |Consente di definire il nome della topologia. |
| : topologia |Consente di definire la topologia usando le funzioni precedenti e quelle predefinite. |
...
| "non transazionale. ACK. Enabled" |Abilitazione o meno dell'acknowledgement per la topologia non transazionale. |
...
Il parametro ***Resource-dir*** è facoltativo, è necessario specificarlo quando si vuole collegare un' C# applicazione e questa directory contiene l'applicazione, le dipendenze e le configurazioni.

Anche il parametro ***classpath*** è facoltativo. Viene usato per specificare il CLASSPATH Java se il file delle specifiche contiene il beccuccio o il Bolt Java.
...
Gli utenti possono generare Tuple C# nei processi, la piattaforma deve serializzare la tupla in byte [], trasferirla sul lato Java e Storm trasferirà la tupla alle destinazioni. Nel frattempo, nei componenti C# downstream, i processi riceveranno le tuple dal lato Java e la convertono ai tipi originali per piattaforma, tutte queste operazioni sono nascoste dalla piattaforma.
...
Nell'oggetto Context SCP.NET sono stati aggiunti due metodi. Vengono usati per generare Tuple o Tuple per specificare StreamId. StreamId è una stringa e deve essere coerente in C# e nelle specifiche di definizione della topologia.
...
Il raggruppamento dei campi predefiniti in Storm non funziona correttamente in SCP.NET. Sul lato Proxy Java tutti i dati dei campi sono in effetti di tipo byte[] e per il raggruppamento dei campi viene usato il codice hash dell'oggetto byte[]. Il codice hash dell'oggetto byte[] corrisponde all'indirizzo dell'oggetto in memoria. Pertanto, il raggruppamento non sarà corretto per gli oggetti a due byte che condividono lo stesso contenuto, ma non lo stesso indirizzo.
...
Il sistema Storm nativo è scritto in Java. E SCP.NET lo ha migliorato per consentire C# agli sviluppatori di C# scrivere codice per gestire la logica di business. Supporta però anche le topologie ibride, che contengono non C# solo bocchette/Bolt, ma anche il becco/Bolt Java.
...
Per inviare una topologia che contiene Spout o Bolt Java, è necessario prima compilare gli Spout o Bolt Java e ottenere i file Jar. Quindi, è necessario specificare il classpath Java che contiene i file Jar al momento dell'invio della topologia. Esempio:
...
Il componente SCP include il lato C# Java e il lato. Per interagire con Spout/Bolt Java nativi, la serializzazione/deserializzazione deve essere effettuata tra il lato Java e il lato C#, come illustrato nel grafico seguente.
...
1. Serializzazione sul lato Java e deserializzazione C# sul lato
   ...
   Questa implementazione predefinita deve gestire la maggior parte dei casi purché il tipo di dati non sia troppo complesso. Per alcuni casi, poiché il tipo di dati utente è troppo complesso o perché le prestazioni dell'implementazione predefinita non soddisfano i requisiti dell'utente, gli utenti possono inserire la propria implementazione.
...
2. Serializzazione C# in lato e deserializzazione sul lato Java
   ...
   L'utente può anche collegare la propria implementazione di C# serializer e deserializzatore Java. Questo codice è l'interfaccia per C# il serializzatore:
...
**HelloWorld** è un semplice esempio per illustrare un SCP.NET. Usa una topologia non transazionale con uno spout denominato **generator** e due bolt denominati **splitter** e **counter**. Lo spout **generator** genera frasi casuali e le emette verso **splitter**. La barra di divisione Bolt * * divide le frasi in parole e genera queste parole nel Bolt del **contatore** . Il Bolt “counter” usa un dizionario per registrare il numero di occorrenze di ogni parola.
...
Nel beccuccio, se ACK è abilitato, viene usato un dizionario per memorizzare nella cache le tuple che non sono state riconosciute. Se viene chiamato Fail(), la tupla in errore viene riprodotta:
...
Per ottenere una semantica di **una sola volta** , il **numero** di Bolt del commit del commit deve valutare se si tratta di una transazione rieseguita. In questo esempio, contiene una variabile membro statica:
...
Quando viene chiamato `FinishBatch()`, il `lastCommittedTxId` verrà aggiornato se non si tratta di una transazione rieseguita.
