---
title: Scrittura in un archivio con Apache Storm/Data Lake Storage - Azure HDInsight
description: Informazioni su come usare Apache Storm per eseguire operazioni di scrittura in un archivio compatibile con HDFS per HDInsight.
services: hdinsight
...
È possibile scaricare il progetto che contiene questa topologia qui: [https://github.com/Azure-Samples/hdinsight-storm-azure-data-lake-store](https://github.com/Azure-Samples/hdinsight-storm-azure-data-lake-store).

Per compilare questo progetto, è necessaria la seguente configurazione per l'ambiente di sviluppo:

* [Java JDK 1.8](https://aka.ms/azure-jdks) o versione successiva. Per HDInsight 3.5 o una versione successiva è necessario Java 8.
...
* `abfs://`: usato con un account di Azure Data Lake Storage Gen2.
* `adl://`: usato con un account di Azure Data Lake Storage Gen1.
...
| `abfs:///` | L'account di archiviazione predefinito è una directory in un account di Azure Data Lake Storage Gen2 |
| `adl:///` | L'account di archiviazione predefinito è una directory in un account di Azure Data Lake Storage Gen1. Durante la creazione del cluster, si specifica la directory in Data Lake Storage che rappresenta la radice dell'HDFS del cluster. Ad esempio, la directory `/clusters/myclustername/`. |
...
| `abfs://CONTAINER@ACCOUNT.dfs.core.windows.net/` | Un account di archiviazione di Azure non predefinito (aggiuntivo) associato al cluster. |
| `adl://STORENAME/` | La radice di Data Lake Storage usata dal cluster. Questo schema permette di accedere ai dati situati all'esterno della directory che contiene il file system del cluster. |

Per altre informazioni, vedere il riferimento [HdfsBolt](https://storm.apache.org/releases/current/javadocs/org/apache/storm/hdfs/bolt/HdfsBolt.html) su Apache.org.

### <a name="example-configuration"></a>Configurazione di esempio

Il seguente YAML è un estratto del file `resources/writetohdfs.yaml` incluso nell'esempio. Questo file definisce la topologia di Storm usando il framework [Flux](https://storm.apache.org/releases/1.1.2/flux.html) per Apache Storm.
...
Per impostazione predefinita, Storm in HDInsight non include i componenti che HdfsBolt usa per comunicare con Archiviazione di Azure o Data Lake Storage nel classpath di Storm. Usare la seguente azione script per aggiungere questi componenti alla directory `extlib` per Storm nel cluster:

* URI script: `https://hdiconfigactions.blob.core.windows.net/linuxstormextlibv01/stormextlib.sh`
* Nodi validi: Nimbus, Supervisor
* Parametri Nessuna
...
1. Scaricare il progetto di esempio dal [ https://github.com/Azure-Samples/hdinsight-storm-azure-data-lake-store ](https://github.com/Azure-Samples/hdinsight-storm-azure-data-lake-store) all'ambiente di sviluppo.
...
4. Usare il testo seguente come contenuto del file `dev.properties`:
    ...
    > [!IMPORTANT]  
    > In questo esempio si presuppone che il cluster usi un account di archiviazione di Azure come archivio predefinito. Se il cluster usa Azure Data Lake Storage Gen2, usare invece `hdfs.url: abfs:///`. Se il cluster usa Azure Data Lake Storage Gen1, usare invece `hdfs.url: adl:///`.
    
    Per salvare il file, usare __CTRL + X__, poi __Y__e infine premere __Invio__. I valori in questo file impostano l'URL di Data Lake Storage e il nome della directory in cui vengono scritti i dati.
...
Dopo aver appreso come usare Apache Storm per scrivere in Archiviazione di Azure e Azure Data Lake Storage, è possibile vedere altri [esempi di Apache Storm per HDInsight](apache-storm-example-topology.md).
