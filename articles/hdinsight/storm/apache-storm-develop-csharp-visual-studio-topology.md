...
È possibile sviluppare topologie C# con SCP.NET usando [Visual Studio](https://visualstudio.microsoft.com/downloads/). Le istruzioni riportate in questo articolo usano Visual Studio 2019, ma è possibile usare anche le versioni precedenti di Visual Studio.
...
Per creare un progetto di topologia C# in Visual Studio:
...
* **Java beccuccio** e **Bolt C#**: definiti nella `HybridTopology_javaSpout_csharpBolt` classe.

  Una versione transazionale è definita nella `HybridTopologyTx_javaSpout_csharpBolt` classe.

* **Beccuccio C#** e **Java Bolt**: definiti nella `HybridTopology_csharpSpout_javaBolt` classe.

  Una versione transazionale è definita nella `HybridTopologyTx_csharpSpout_javaBolt` classe.
...
Quando si usa una topologia C# con un cluster HDInsight basato su Linux, i componenti Bolt e beccucci che usano **ConfigurationManager** per leggere le impostazioni di configurazione in fase di esecuzione possono restituire eccezioni del puntatore null.
...
Quando si usa una topologia C# con un cluster HDInsight basato su Linux, è possibile che si verifichi l'errore seguente:
...
1. Usare il codice seguente come contenuto della `LocalTest` classe:
...
3. Per ottenere altre informazioni, selezionare una **porta** nella sezione **eXecutors (esecutori** ) della pagina per visualizzare il log di lavoro di Storm negli ultimi minuti.
