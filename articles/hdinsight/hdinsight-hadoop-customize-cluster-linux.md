---
title: Personalizzare cluster HDInsight mediante azioni script | Microsoft Docs
...
* Possono essere limitate all'**esecuzione solo in alcuni tipi di nodi**, ad esempio nodi head o di ruolo di lavoro.
...
* Possono essere **persistenti** o **ad hoc**.
    ...
    Gli script **persistenti** vengono applicati ai nodi di ruoli di lavoro e vengono eseguiti automaticamente nei nuovi nodi creati durante l'aumento delle prestazioni di un cluster.
...
* Vengono eseguite con **privilegi a livello radice** sui nodi del cluster.
...
2. Nel pannello del cluster HDInsight selezionare il riquadro **Azioni script**.
   ...
   > [!NOTE]
   > È inoltre possibile selezionare **Tutte le impostazioni** e quindi **Azioni script** dal pannello Impostazioni.
   > 
   > 
...
4. Immettere le informazioni seguenti dal pannello Aggiungi azione script.
   ...
   * **CON SALVATAGGIO PERMANENTE**: selezionare questa voce per salvare lo script in modo permanente e applicarlo ai nuovi nodi di lavoro quando si aumentano le prestazioni del cluster.
...
6. È anche possibile usare **...** a destra delle voci nel pannello Azioni script per eseguire azioni come riesecuzione, persistenza o, nel caso di azioni persistenti, eliminazione.
...
2. Nella barra in alto nella pagina fare clic sulla voce **ops**. Verrà visualizzato un elenco delle operazioni correnti e precedenti eseguite nel cluster tramite Ambari.
