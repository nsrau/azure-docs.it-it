...
Quando si verifica il problema, nei log `ambari-agent` viene visualizzato quanto segue.
```
...
1. Passare a hive Component Summary (Riepilogo componenti hive) nel portale di Ambari e fare clic sulla scheda `Configs`.

2. Passare alla sezione `Advanced hive-log4j` in impostazioni avanzate.

3. Impostare `log4j.appender.RFA` parametro come RollingFileAppender. 
