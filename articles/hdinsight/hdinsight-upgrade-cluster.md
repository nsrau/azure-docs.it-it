---
title: Eseguire la migrazione del cluster a una versione più recenteMigrate cluster to a newer version
titleSuffix: Azure HDInsight
description: Informazioni sulle linee guida per eseguire la migrazione del cluster di Azure HDInsight a una versione più recente.
...
---
# <a name="migrate-hdinsight-cluster-to-a-newer-version"></a>Eseguire la migrazione del cluster HDInsight a una versione più recenteMigrate HDInsight cluster to a newer version

Per sfruttare le funzionalità più recenti di HDInsight, è consigliabile eseguire regolarmente la migrazione dei cluster HDInsight alla versione più recente. HDInsight non supporta gli aggiornamenti sul posto in cui un cluster esistente viene aggiornato a una versione più recente del componente. È necessario creare un nuovo cluster con il componente e la versione della piattaforma desiderati e quindi eseguire la migrazione delle applicazioni per l'utilizzo del nuovo cluster. Seguire le linee guida seguenti per eseguire la migrazione delle versioni del cluster HDInsight.Follow the below guidelines to migrate your HDInsight cluster versions.
...
![Diagramma del flusso di lavoro di aggiornamento HDInsightHDInsight upgrade workflow diagram](./media/hdinsight-upgrade-cluster/upgrade-workflow-diagram.png)
...
## <a name="workload-specific-guidance"></a>Indicazioni specifiche sul carico di lavoroWorkload specific guidance

I documenti seguenti forniscono indicazioni su come eseguire la migrazione di carichi di lavoro specifici:The following documents provide guidance on how to migrate specific workloads:

* [Eseguire la migrazione di HBaseMigrate HBase](./hbase/apache-hbase-migrate-new-version.md)
* [Migrare Kafka](./kafka/migrate-versions.md)
* [Eseguire la migrazione di Hive/Query interattivaMigrate Hive/Interactive Query](./interactive-query/apache-hive-migrate-workloads.md)
...
* [Note sulla versione di HDInsightHDInsight release notes](./hdinsight-version-release.md)
