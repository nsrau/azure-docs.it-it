---
title: MapReduce e Remote Desktop con Hadoop in HDInsight | Microsoft Docs
...
1. Dal desktop di HDInsight avviare la **riga di comando di Hadoop**. Viene aperta una finestra del prompt dei comandi nella directory **c:\\apps\\dist\\hadoop-&lt;numero versione>**.
   ...
   > [!NOTE]
   > Quando Hadoop viene aggiornato, il numero di versione viene modificato. La variabile di ambiente **HADOOP\_HOME** può essere usata per trovare il percorso. Ad esempio, `cd %HADOOP_HOME%` modifica le directory nella directory di Hadoop, senza richiedere il nome della versione.
   > 
   > 
...
4. Al termine del processo, usare il seguente comando per elencare i file di output archiviati in **wasbs://example/data/WordCountOutput**:
