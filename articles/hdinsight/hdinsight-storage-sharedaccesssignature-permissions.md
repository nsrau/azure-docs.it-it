...
Le firme di accesso condiviso sono una funzionalità degli account di archiviazione di Azure che consente di limitare l'accesso ai dati. Ad esempio, concedendo l'accesso in sola lettura ai dati. Questo documento illustra come usare la firma di accesso condiviso per abilitare l'accesso in sola lettura e solo elenco a un contenitore BLOB da HDInsight.
...
* Un cluster HDInsight basato su Linux OPPURE [Azure PowerShell][powershell]. Se è disponibile un cluster basato su Linux esistente, è possibile usare Ambari per aggiungere una firma di accesso condiviso al cluster. In caso contrario, è possibile usare Azure PowerShell per creare un cluster e aggiungere una firma di accesso condiviso durante la creazione di quest'ultimo.
...
3. I criteri di accesso archiviati cui viene fatto riferimento nella firma di accesso condiviso vengono eliminati e ciò corrisponde a un altro modo per revocare la firma. Se si ricreano i criteri di accesso archiviati specificando esattamente lo stesso nome, tutti i token di firma di accesso condiviso relativi ai criteri precedenti resteranno validi, a condizione che l'ora di scadenza indicata nella firma di accesso condiviso non sia trascorsa. Se si intende revocare la firma di accesso condiviso, verificare di usare un nome diverso per ricreare i criteri di accesso archiviati con scadenza nel futuro.
...
* Per i cluster HDInsight **basati su Linux** , usare SSH per connettersi al cluster. Per informazioni sull'uso di SSH con i cluster basati su Linux, vedere uno degli argomenti seguenti:
  ...
  * [Usare SSH con HDInsight (Hadoop) da Bash in Windows 10, Linux, Unix o OS X](hdinsight-hadoop-linux-use-ssh-unix.md)
