...
* [Un cluster Azure HDInsight basato su Linux con HBase](hdinsight-hbase-tutorial-get-started-linux.md#create-hbase-cluster)
  ...
  > [!NOTE]
  > Le procedure descritte in questo documento sono state testate con le versioni del cluster HDInsight 3.2, 3.3, 3.4 e 3.5. I valori predefiniti specificati negli esempi sono per un cluster HDInsight 3.4.
...
* **Familiarità con SSH e SCP** o **Azure PowerShell**. Questo documento descrive la procedura per l'uso di SSH/SCP e di Azure PowerShell durante l'esecuzione dell'esempio.
    ...
    Per altre informazioni sull'uso di SSH e SCP con HDInsight, vedere gli articoli seguenti:
...
2. Usare il comando **mvn** , che viene installato con Maven, per generare lo scaffolding per il progetto.
    ...
    Verrà creata una nuova directory nella directory corrente, con il nome specificato dal parametro **artifactID** (in questo esempio **hbaseapp**). La directory contiene gli elementi seguenti:
...
5. Creare una nuova directory denominata **conf** nella directory **hbaseapp**. La directory viene usata per contenere le informazioni di configurazione per la connessione a HBase.
