<properties
	pageTitle="Sviluppare programmi MapReduce Java per Hadoop | Microsoft Azure"
...
Questa esercitazione illustra in modo dettagliato uno scenario end-to-end per lo sviluppo di un processo MapReduce Hadoop in Java usando Apache Maven. L'esercitazione mostra anche come eseguire il test dell'applicazione in HDInsight Emulator per Azure e quindi distribuirla ed eseguirla in un cluster HDInsight basato su Windows.

##<a name="prerequisites"></a>Prerequisiti

Prima di iniziare questa esercitazione, è necessario aver completato quanto segue:

- Installare HDInsight Emulator. Per istruzioni, vedere [Introduzione a HDInsight Emulator][hdinsight-emulator]. Verificare che tutti i servizi necessari siano in esecuzione. Nel computer in cui è installato HDInsight Emulator avviare la riga di comando di Hadoop dal collegamento sul desktop, passare a **C:\\hdp** ed eseguire il comando **start\_local\_hdp\_services.cmd**.
...
2. Aggiornare il modello a oggetti dei progetti (POM, Project Object Model)
...
2. Aggiungere il codice seguente al file __pom.xml__. Il codice deve essere racchiuso tra tag `<project>...</project>` nel file, ad esempio tra `</dependencies>` e `</project>`.
	...
	Verrà configurato il [plug-in Maven Shade](http://maven.apache.org/plugins/maven-shade-plugin/), usato per prevenire la duplicazione della licenza nel file JAR compilato da Maven. Il motivo per cui viene usato questo plug-in è che i file di licenza duplicati causano un errore in fase di esecuzione sul cluster HDInsight. L'uso del plug-in Maven Shade con l'implementazione di `ApacheLicenseResourceTransformer` previene il verificarsi di questo errore.

	Il plug-in Maven Shade produrrà anche un file uberjar (o fatjar), che contiene tutte le dipendenze richieste dall'applicazione.
...
Per testare il processo MapReduce in HDInsight Emulator, è necessario eseguire le procedure seguenti:

1. Caricare i file di dati in Hadoop Distributed File System (HDFS) nell'emulatore
2. Creare un gruppo di utenti locale
3. Eseguire un processo MapReduce per il conteggio delle parole
4. Recuperare i risultati del processo

HDInsight Emulator usa HDFS come file system predefinito. Se si desidera, è possibile configurare HDInsight Emulator per l'uso dell'archivio BLOB di Azure. Per informazioni dettagliate, vedere [Introduzione a HDInsight Emulator][hdinsight-emulator-wasb].
...
Per la corretta esecuzione del processo MapReduce nel cluster è necessario creare un gruppo di utenti denominato hdfs, a cui si dovrà aggiungere anche un utente Hadoop e l'utente locale con il quale eseguire l'accesso all'emulatore. Usare i comandi riportati di seguito da un prompt dei comandi con privilegi elevati:
...
2. Eseguire il comando seguente per eliminare la struttura di cartelle /WordCount/Output da HDFS. /WordCount/Output è la cartella di output del processo MapReduce per il conteggio delle parole. Se tale cartella esiste già, il processo MapReduce avrà esito negativo. Questo passaggio è necessario se si esegue il processo per la seconda volta.
...
2. Eseguire il comando seguente:
	...
	Se il processo viene completato correttamente, l'output che si ottiene dovrebbe essere analogo alla schermata seguente:
...
Il processo MapReduce per il conteggio delle parole è stato sviluppato e testato correttamente nell'emulatore. Il passaggio successivo consiste nella distribuzione e nell'esecuzione del processo in Azure HDInsight.
...
**Per creare un account di archiviazione BLOB e un contenitore**
...
2. Impostare le prime tre variabili, quindi eseguire i comandi seguenti:
	...
	Le variabili **$storageAccountName\_Data** e **$containerName\_Data** sono le stesse definite nella procedura precedente.
...
6. Eseguire i comandi seguenti per visualizzare l'elenco dei file caricati:
...
2. Impostare le prime tre variabili, quindi eseguire i comandi seguenti:
	...
	Le variabili **$storageAccountName\_Data** e **$containerName\_Data** sono le stesse definite nella procedura precedente. I file di dati e l'applicazione verranno pertanto caricati nello stesso contenitore nello stesso account di archiviazione.
...
5. Eseguire il comando seguente per copiare le applicazioni:
...
6. Eseguire i comandi seguenti per visualizzare l'elenco dei file caricati:
	...
	Il file JAR dovrebbe essere incluso nell'elenco visualizzato.

##<a name="run"></a>Eseguire il processo MapReduce in Azure HDInsight

In questa sezione verrà creato uno script di Azure PowerShell che esegue le attività seguenti:

1. Effettua il provisioning di un cluster HDInsight
...
2. Passare alla directory **C:\\Tutorials\\WordCountJava**. La cartella predefinita per Azure PowerShell è **C:\\Windows\\System32\\WindowsPowerShell\\v1.0**. I cmdlet eseguiti consentiranno di scaricare il file di output nella cartella corrente. Non si dispone di autorizzazioni per il download dei file nelle cartelle di sistema.
