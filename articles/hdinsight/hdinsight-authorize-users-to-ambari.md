...
Gli utenti di Active Directory possono accedere ai nodi del cluster utilizzando le credenziali di dominio. Possono inoltre utilizzare le credenziali di dominio per autenticare le interazioni del cluster con altri endpoint approvati, ad esempio [Hue](https://gethue.com/), Ambari Views, ODBC, JDBC, PowerShell e LE API REST.
...
![Gestione dashboard Apache Ambari](./media/hdinsight-authorize-users-to-ambari/manage-apache-ambari.png)
...
1. Nella pagina di gestione selezionare **Utenti**.
    ...
    ![Utenti della pagina di gestione Apache Ambari](./media/hdinsight-authorize-users-to-ambari/apache-ambari-management-page-users.png)

1. Selezionare **: Crea utente locale**.

1. Specificare **nome utente** e **password**. Selezionare **salva**.

### <a name="add-users-through-powershell"></a>Aggiungere utenti tramite PowerShellAdd users through PowerShell

Modificare le variabili riportate `NEWUSER`di `PASSWORD` seguito sostituendo `CLUSTERNAME`, e con i valori appropriati.
...
Modificare le variabili riportate `NEWUSER`di `USERPASSWORD` seguito sostituendo `CLUSTERNAME`, `ADMINPASSWORD`, e con i valori appropriati. Lo script è progettato per essere eseguito con bash. Sarebbero necessarie lievi modifiche per un prompt dei comandi di Windows.
...
1. Nella pagina di gestione selezionare il collegamento **Views** (Viste) nell'intestazione del menu **Views** (Viste) a sinistra.
    ...
    ![Viste Apache Ambari visualizza i link](./media/hdinsight-authorize-users-to-ambari/apache-ambari-views-link.png)
...
1. Per aggiungere un utente, selezionare il pulsante **Add User** (Aggiungi utente).
   ...
   * Iniziare a digitare il nome utente. Verrà visualizzato un elenco a discesa dei nomi definiti in precedenza.
     ...
     ![Completamenti automatici automatici utente Apache Ambari](./media/hdinsight-authorize-users-to-ambari/ambari-user-autocomplete.png)
   ...
   * Per salvare le modifiche, selezionare la **casella di controllo blu**.
     ...
     ![Apache Ambari concede le autorizzazioni utente](./media/hdinsight-authorize-users-to-ambari/user-entered-permissions.png)
...
1. Per aggiungere un gruppo, selezionare il pulsante **Add Group** (Aggiungi gruppo).
   ...
   * Per salvare le modifiche, selezionare la **casella di controllo blu**.
     ...
     ![Apache Ambari concede autorizzazioni](./media/hdinsight-authorize-users-to-ambari/ambari-group-entered.png)
...
![Collegamenti al menu dei ruoli Apache Ambari](./media/hdinsight-authorize-users-to-ambari/cluster-roles-menu-link.png)
...
![Autorizzazioni per i collegamenti al menu dei ruoli Apache Ambari](./media/hdinsight-authorize-users-to-ambari/roles-menu-permissions.png "Autorizzazioni per i collegamenti al menu dei ruoli Apache Ambari")
...
![I ruoli Apache Ambari bloccano la vista](./media/hdinsight-authorize-users-to-ambari/ambari-roles-block-view.png)
...
* La categoria Users (Utenti) visualizza un elenco di tutti gli utenti, che consente di selezionare un ruolo per ogni utente nell'elenco a discesa.
    ...
    ![Visualizzazione elenco ruoli Apache Ambari - utenti](./media/hdinsight-authorize-users-to-ambari/roles-list-view-users.png)
...
* La categoria Groups (Gruppi) visualizza tutti i gruppi e il ruolo e il ruolo assegnato a ogni gruppo. In questo esempio l'elenco di gruppi è sincronizzato da gruppi di Azure AD specificati nella proprietà **Access user group** (Gruppo utenti di accessi) delle impostazioni di dominio del cluster. Vedere [Creazione di un cluster HDInsight con ESP abilitato](./domain-joined/apache-domain-joined-configure-using-azure-adds.md#create-an-hdinsight-cluster-with-esp).
    ...
    ![Visualizzazione elenco ruoli Apache Ambari - gruppi](./media/hdinsight-authorize-users-to-ambari/roles-list-view-groups.png)
...
![Utente Apache Ambari con sola vista](./media/hdinsight-authorize-users-to-ambari/ambari-user-views-only.png)
...
![Visualizzazione del cruscotto Apache Ambari](./media/hdinsight-authorize-users-to-ambari/user-cluster-user-role.png)
