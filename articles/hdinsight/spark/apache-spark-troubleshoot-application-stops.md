...
Il valore `livy.server.session.timeout` controlla per quanto tempo Apache Livio deve attendere il completamento di una sessione. Quando la lunghezza della sessione raggiunge il valore `session.timeout`, la sessione di Livio e l'applicazione vengono automaticamente terminate.
