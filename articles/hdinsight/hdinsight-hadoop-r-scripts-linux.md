---
title: Installare GNU R in HDInsight basato su Linux | Microsoft Docs
...
---
# <a name="install-and-use-gnu-r-on-linux-based-hdinsight-hadoop-clusters"></a>Installare e usare GNU R nei cluster Hadoop di HDInsight basati su Linux

È possibile installare GNU R in qualsiasi tipo di cluster in Hadoop su HDInsight usando la personalizzazione dei cluster **Azione script**. Gli analisti ed esperti di dati possono usare R per elaborare grandi quantità di dati nei cluster Hadoop distribuiti in HDInsight.

> [!WARNING]
> In questo documento vengono offerte istruzioni sull'installazione di GNU R in HDInsight come azione personalizzata. __Non__ vengono offerte le stesse funzionalità __Microsoft R Server in HDInsight__. È consigliabile usare R Server in HDInsight invece dei passaggi descritti in questo documento.
>
> Per informazioni su come usare R Server in HDInsight, vedere [Introduzione all'uso di R Server su HDInsight](hdinsight-hadoop-r-server-get-started.md). 
...
Questa sezione fornisce istruzioni su come usare lo script quando si crea un cluster usando il Portale di Azure. 
...
2. Nel pannello **Configurazione facoltativa** selezionare **Azioni script** e usare le informazioni seguenti per completare il modello:
...
3. Immettere il programma R seguente. Questo codice genera i numeri da 1 a 100 e li moltiplica per 2.
...
4. Usare quindi il comando seguente per visualizzare il percorso temporaneo in cui è stato archiviato l'output di MapReduce:
    ...
    Il comando restituisce un valore analogo a `/tmp/file5f615d870ad2`. Per visualizzare l'output effettivo, usare il comando seguente:
...
5. Per uscire da R, immettere il comando seguente:
