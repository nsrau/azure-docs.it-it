...
La risorsa di archiviazione predefinita per i cluster HDInsight è Azure Blob (WASB) con il file system HDFS mappato a un contenitore BLOB. Ciò assicura che i dati caricati nella risorsa di archiviazione del cluster o scritti in essa durante il corso dell'analisi siano resi persistenti. Un comodo metodo per copiare i dati da e verso un BLOB è ricorrere all’utilità [AzCopy](../storage/storage-use-azcopy.md).

Oltre a BLOB, esiste anche la possibilità di aggiungere l’utilizzo dell’[archiviazione di Azure Data Lake](https://azure.microsoft.com/services/data-lake-store/) (ADLS) al cluster. Se si aggiunge l'utilizzo di ADLS è possibile combinare l'utilizzo di BLOB e di ADLS per l'archiviazione HDFS.

È anche possibile ricorrere a [File di Azure](../storage/storage-how-to-use-files-linux.md) come opzione di archiviazione per l'utilizzo sul nodo perimetrale. File di Azure consente di montare una condivisione file creata su un account di Archiviazione di Azure nel file system Linux. Per altre informazioni sulle opzioni di archiviazione dei dati per R Server su cluster HDInsight, vedere l'articolo dedicato alle [opzioni di archiviazione per R Server su cluster HDInsight](hdinsight-hadoop-r-server-storage.md).
...
È inoltre possibile modificare la quantità di memoria disponibile per R Server utilizzando switch Hadoop nella chiamata di **RxHadoopMR**, come mostrato di seguito.
 
	hadoopSwitches = "-libjars /etc/hadoop/conf -Dmapred.job.map.memory.mb=6656"  

### Dimensionamento del cluster

Un cluster esistente può essere dimensionato, in aumento o in riduzione, mediante il Portale di Azure. In questo modo è possibile ottenere la capacità aggiuntiva necessaria per le elaborazioni più estese e ridurre nuovamente il cluster quando la capacità rimane inutilizzata. Per istruzioni su come dimensionare un cluster, vedere [Gestire cluster HDInsight](hdinsight-administer-use-portal-linux.md).
