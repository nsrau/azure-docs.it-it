...
È necessario aggiungere due configurazioni al `mapred-site` personalizzato in Ambari.

1. Da un Web browser passare a `https://CLUSTERNAME.azurehdinsight.net`, dove `CLUSTERNAME` è il nome del cluster.

1. Dall'interfaccia utente di Ambari passare a **MapReduce2** > **configs** > **Advanced** > **mapred-site personalizzato**.
...
1. Elencare tutti gli ID applicazione delle applicazioni Yarn attualmente in esecuzione con il comando seguente:
    ...
    Prendere nota dell'ID applicazione della colonna `APPLICATIONID` i cui log devono essere scaricati.
...
1. Scaricare i log dei contenitori Yarn per tutti i master applicazioni con il comando seguente. Verrà creato il file di log denominato `amlogs.txt` nel formato di testo.
