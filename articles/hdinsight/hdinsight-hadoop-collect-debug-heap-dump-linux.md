...
> [!NOTE]
> È consigliabile usare Ambari per modificare gli script e le impostazioni di mapred-site.xml, in quanto Ambari gestirà la replica delle modifiche tra i nodi del cluster. Per i passaggi specifici, vedere la sezione [Uso di Ambari](#using-ambari) .
> 
> 
...
1. Aprire l'interfaccia utente Web di Ambari per il cluster. L'URL sarà https://YOURCLUSTERNAME.azurehdinsight.net.
   ...
   > [!NOTE]
   > È possibile che Ambari richieda il nome utente e la password una seconda volta. In questo caso, immettere di nuovo lo stesso nome dell'account e la stessa password.
   > 
   > 
...
4. Trovare la voce **\*\_OPTS** per il servizio per cui si vogliono abilitare i dump dell'heap e aggiungere le opzioni da abilitare. Nella figura seguente è stato aggiunto `-XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=/tmp/` alla voce **HADOOP\_NAMENODE\_OPTS**:
   ...
   > [!NOTE]
   > Quando si abilitato i dump dell'heap per il processo figlio di mapping o riduzione, si cercheranno invece i campi con etichetta **mapreduce.admin.map.child.java.opts** e **mapreduce.admin.reduce.child.java.opts**.
   > 
   > 
...
6. Selezionare ogni servizio che richiede un riavvio e usare il pulsante **Service Actions** per **Disattivare la modalità di manutenzione**. Ciò impedisce che vengano generati avvisi quando si riavvia questo servizio.
