...
1. Usare il comando seguente per caricare il file con estensione zip nel nodo head del cluster HDInsight:
   ...
   > [!NOTE]
   > Se è stata usata una password per l'autenticazione a SSH, verrà richiesto di specificarla. Se è stata usata una chiave pubblica, può essere necessario usare il parametro `-i` e specificare il percorso alla chiave privata corrispondente. Ad esempio, `scp -i ~/.ssh/id_rsa FILENAME.csv USERNAME@CLUSTERNAME-ssh.azurehdinsight.net:`.
   > 
   > 
...
3. Dopo aver installato FreeTDS, usare il comando seguente per connettersi al server del database SQL. Sostituire **serverName** con il nome server del database SQL. Sostituire **adminLogin** e **adminPassword** con le credenziali di accesso per il database SQL. Sostituire **databaseName** con il nome del database.
...
5. Per uscire dall'utilità psql, immettere `exit` at the `1>` .
