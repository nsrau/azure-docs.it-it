...
1. Passare a **hive** > **configs** > **Advanced** > Advanced**Advanced hive-log4j**. Esaminare le impostazioni seguenti:
    ...
    * `hive.root.logger=DEBUG,RFA`. Questo è il valore predefinito, modificare il [livello](https://logging.apache.org/log4j/2.x/log4j-api/apidocs/org/apache/logging/log4j/Level.html) di registrazione `INFO` in per stampare le voci di log less.
...
* `/mnt/resource`potrebbe essere compilato con file orfani, come nel caso del riavvio di Resource Manager. Se necessario, pulire `/mnt/resource/hadoop/yarn/log` manualmente e `/mnt/resource/hadoop/yarn/local`.
