...
Un processo potrebbe non riuscire con un messaggio di errore simile a: `/usr/hdp/2.6.3.2-14/hadoop/libexec/hadoop-config.sh: fork: No space left on device.`
...
1. Determinare quale cartella del nodo preoccupante contribuisce alla maggior parte dello spazio su disco. Eseguire prima la connessione SSH al nodo, quindi eseguire `df` per elencare l'utilizzo del disco per tutti i montaggi. In genere è `/mnt` un disco temporaneo usato da OSS. È possibile immettere in una cartella, quindi digitare `sudo du -hs` per visualizzare le dimensioni dei file riepilogati in una cartella. Se viene visualizzata una cartella simile a `/mnt/resource/hadoop/yarn/local/usercache/livy/appcache/application_1537280705629_0007`, significa che l'applicazione è ancora in esecuzione. La causa potrebbe essere la persistenza RDD o file shuffle intermedi.
