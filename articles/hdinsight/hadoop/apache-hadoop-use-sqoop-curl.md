...
Per i comandi in questa sezione, sostituire `USERNAME` con l'utente per l'autenticazione nel cluster e sostituire `PASSWORD` con la password per l'account utente. Sostituire `CLUSTERNAME` con il nome del cluster.
...
1. Per verificare lo stato del processo, usare il seguente comando. Sostituire `JOBID` con il valore restituito nel passaggio precedente. Se, ad esempio, il valore restituito è `{"id":"job_1415651640909_0026"}`, viene `job_1415651640909_0026``JOBID`. Rivedere la posizione dei `jq` in base alle esigenze.
