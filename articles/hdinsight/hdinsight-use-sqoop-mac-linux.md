...
- **Database SQL di Azure**: in questo documento vengono fornite istruzioni per la creazione di un database SQL di esempio. Per altre informazioni sui database SQL, vedere [Introduzione all'uso del database SQL di Azure][sqldatabase-get-started].

* **SQL Server**: i passaggi descritti in questo documento possono essere usati inoltre, con alcune modifiche, con SQL Server; tuttavia, sia il cluster HDInsight che SQL Server devono essere nella stessa rete virtuale di Azure. Per altre informazioni sui requisiti specifici per l'uso di questo articolo con SQL Server, vedere la sezione [Utilizzo di SQL Server](#using-sql-server).
...
1. Aprire un terminale o un prompt dei comandi e usare il comando seguente per creare un nuovo server di database SQL di Azure:
...
4. Usare il comando seguente per esportare dati dalla tabella **hivesampletable** alla tabella **mobiledata**:
...
5. Al termine dell'esecuzione del comando, usare le informazioni seguenti per connettersi al database tramite TSQL:
    ...
    Una volta connessi, usare le istruzioni seguenti per verificare che i dati siano stati esportati nella tabella **mobiledata**:
...
* È necessario creare il database **sqooptest** in SQL Server usando un'utilità, ad esempio **SQL Server Management Studio** o **tsql**; la procedura per l'uso dell'interfaccia della riga di comando di Azure funziona solo per un database SQL di Azure

    Le istruzioni TSQL per creare la tabella **mobiledata** sono simili a quelle usate per database SQL, fatta eccezione per la creazione di un indice clustrer (non è obbligatorio per SQL Server):
