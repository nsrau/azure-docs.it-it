...
* Un cluster Apache Storm in HDInsight versione 3.4. Per la creazione di un cluster, usare uno degli articoli introduttivi seguenti:
  
  * Un cluster HDInsight. Per informazioni sulla creazione di un nuovo cluster, vedere [Introduzione a Storm in un cluster HDInsight](hdinsight-apache-storm-tutorial-get-started-linux.md).
...
* Editor di testo o ambiente di sviluppo integrato (IDE) Java.
  ...
  * Un client SSH. Per altre informazioni sull'uso di SSH con HDInsight, vedere gli articoli seguenti:
    ...
    * [Usare SSH con Hadoop basato su Linux in HDInsight da Linux, Unix, OS X o Bash in Windows 10](hdinsight-hadoop-linux-use-ssh-unix.md).
...
**com.microsoft.example.EventHubReader** legge i dati dall'hub eventi, scritti da EventHubWriter, e li memorizza nell'archiviazione predefinita dei cluster nella directory /devicedata.
...
* Rinominare i file di licenza per le dipendenze. Se non si esegue questa operazione, può verificarsi un errore in fase di runtime.
...
È necessario popolare il file `EventHubs.properties` con le informazioni relative all'hub eventi e al cluster HDInsight.
...
5. Selezionare **Configura**e quindi creare due nuovi criteri di accesso usando le informazioni seguenti.
    ...
    Dopo avere creato le autorizzazioni, selezionare l'icona **Salva** nella parte inferiore della pagina. Vengono creati i criteri di accesso condivisi che sono usati per inviare (writer) e rimanere in ascolto (reader) in questo hub eventi.
...
2. Usare il comando seguente per compilare il progetto e creare il pacchetto:
    ...
    Vengono scaricate le dipendenze necessarie, viene compilato il progetto e quindi viene creato il pacchetto. L'output viene archiviato nella directory **/target** come **EventHubExample-1.0-SNAPSHOT.jar**.
...
3. Per avviare le topologie, usare il comando seguente:
    ...
    Le topologie vengono avviate e denominate con i nomi descrittivi "reader" e "writer".
...
4. Attendere un minuto o due per consentire alle topologie di scrivere e leggere gli eventi dall'hub eventi, quindi usare il comando seguente per verificare che EventHubReader memorizzi i dati nell'archiviazione di HDInsight:
   ...
   > Le dimensioni di alcuni file potrebbero essere pari a 0, perché sono stati creati da EventHubReader, ma non contengono ancora dati archiviati.
    ...
    Vengono restituiti dati analoghi ai seguenti:
...
### <a name="if-using-a-windows-based-hdinsight-cluster"></a>Se si usa un cluster HDInsight basato su Windows

> [!IMPORTANT]
> Linux è l'unico sistema operativo usato in HDInsight versione 3.4 o successiva. Per altre informazioni, vedere [HDInsight deprecato in Windows](hdinsight-component-versioning.md#hdi-version-32-and-33-nearing-deprecation-date).

1. Nel browser Web, aprire https://NOMECLUSTER.azurehdinsight.net. Quando richiesto, immettere le credenziali di amministratore per il cluster di HDInsight. Viene visualizzato il Dashboard di Storm.
...
7. In **Console query** selezionare **Editor Hive** e sostituire la stringa predefinita `select * from hivesampletable` con la seguente:
    ...
    Fare clic su **Seleziona** per eseguire la query. Vengono restituite 10 righe dai dati scritti nell'archiviazione del cluster da EventHubReader. Dopo il completamento della query, dovrebbero essere visualizzati dati simili ai seguenti:
...
È possibile esportare e importare i checkpoint persistenti in WASB (il servizio Archiviazione di Azure usato dai cluster HDInsight basati su Windows). Gli script per eseguire questa operazione si trovano in Storm, nel cluster HDInsight, in **c:\apps\dist\storm-0.9.3.2.2.1.0-2340\zkdatatool-1.0\bin**.
