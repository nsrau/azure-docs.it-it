...
Non è possibile completare l'operazione di riavvio in un server di area di Apache HBase. Dalla directory `region-server.log` nei `/var/log/hbase` nodi di lavoro in cui l'area del server di avvio non riesce, è possibile che venga visualizzato un messaggio di errore simile al seguente:
