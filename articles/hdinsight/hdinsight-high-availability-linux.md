---
title: Funzionalità a disponibilità elevata di HDInsight (Hadoop) basati su Linux | Microsoft Docs
...
Al nodo perimetrale fornito con Server R nei cluster HDInsight è anche possibile accedere direttamente usando SSH tramite la porta 22. Ad esempio, `ssh username@RServer.mycluster.ssh.azurehdinsight.net` si connetterà al nodo perimetrale per un Server R nel cluster HDInsight denominato **mycluster**.
...
* **SSH**: una volta connessi a un nodo head tramite SSH, è quindi possibile usare SSH dal nodo head per connettersi ad altri nodi del cluster.
