...
---
# <a name="what-is-apache-storm-on-azure-hdinsight"></a>Informazioni su Apache Storm in Azure HDInsight
...
* __Hub eventi__: per un esempio dell'uso di Hub eventi con un cluster Storm, vedere i documenti seguenti:
...
* __Database SQL__, __Cosmos DB__, __Hub eventi__, e __HBase__: Strumenti Azure Data Lake per Visual Studio include esempi di modelli. Per altre informazioni, vedere [Sviluppare una topologia C# per Storm in HDInsight](hdinsight-storm-develop-csharp-visual-studio-topology.md).
...
Storm in HDInsight viene fornito con supporto continuo di livello aziendale. Storm in HDInsight offre anche un contratto di servizio con disponibilità del 99,9%. Questo garantisce che il cluster Storm avrà connettività esterna per almeno il 99,9% del tempo.
...
Il modello che prevede la lettura di una tupla di input, la generazione di nessuna o più tuple e l'inserimento della tupla di input subito dopo la fine del metodo di esecuzione è un modello comune. Storm mette a disposizione l'interfaccia [IBasicBolt](https://storm.apache.org/releases/1.0.3/javadocs/org/apache/storm/topology/IBasicBolt.html) per automatizzarlo.
