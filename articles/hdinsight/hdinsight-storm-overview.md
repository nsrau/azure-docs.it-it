...
> [AZURE.NOTE] È anche possibile effettuare il provisioning di cluster Storm usando l'[interfaccia della riga di comando di Azure](../xplat-cli-install.md) o [Azure PowerShell](../powershell-install-configure.md).
...
__Per cluster Storm basati su Linux in HDInsight__, è possibile connettersi al cluster con SSH e usare il comando `storm` per avviare e gestire le topologie. Inoltre, è possibile utilizzare Ambari per monitorare il servizio Storm e l'interfaccia utente Storm per monitorare e gestire le topologie in esecuzione.
...
Gli esempi di storm-starter si trovano nella directory __ /usr/hdp/current/storm-client/contrib/storm-starter__ nei cluster basati su Linux e nella directory **%storm_home%\contrib\storm-starter** nei cluster basati su Windows.
