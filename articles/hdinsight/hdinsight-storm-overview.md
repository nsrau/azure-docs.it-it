...
**Per cluster Storm basati su Linux in HDInsight**, è possibile connettersi al cluster con SSH e usare il comando `storm` per avviare e gestire le topologie. Inoltre, è possibile utilizzare Ambari per monitorare il servizio Storm e l'interfaccia utente Storm per monitorare e gestire le topologie in esecuzione.

Per ulteriori informazioni sull'utilizzo dei cluster Storm basati su Linux, vedere [Introduzione ad Apache Storm in HDInsight basato su Linux](hdinsight-apache-storm-tutorial-get-started-linux.md).

**Per Storm basato su Windows in cluster HDInsight**, HDInsight Tools per Visual Studio consente di creare topologie C# e topologie ibride C#/Java e di inviarle quindi al cluster Storm in HDInsight.  
...
Per altre informazioni su HDInsight Tools per Visual Studio, vedere [Introduzione all'uso di HDInsight Tools per Visual Studio](hdinsight-hadoop-visual-studio-tools-get-started.md).
...
* [Sviluppare una topologia Java che utilizza gli hub eventi di Azure](hdinsight-storm-develop-java-event-hub-topology.md)
...
* **Spout**: utilizza i dati dall'origine dati e genera uno o più **flussi**.
  ...
  > [!NOTE]
  > In molti casi i dati vengono letti da una coda, ad esempio Kafka, code del bus di servizio di Azure o hub eventi. La coda assicura la persistenza dei dati in caso di interruzione.
  > 
  > 
...
Gli esempi di storm-starter si trovano nella directory ** /usr/hdp/current/storm-client/contrib/storm-starter** nei cluster basati su Linux e nella directory **%storm_home%\contrib\storm-starter** nei cluster basati su Windows.
