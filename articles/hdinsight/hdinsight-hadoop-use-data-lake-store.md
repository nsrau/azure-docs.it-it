...
Per analizzare i dati nel cluster HDInsight, è possibile archiviare i dati in [Archiviazione di Azure](../storage/common/storage-introduction.md), Archiviazione data server di Azure gen [1](../data-lake-store/data-lake-store-overview.md)o Azure Data Lake Storage [Gen 2.](../storage/blobs/data-lake-storage-introduction.md) Tutte le opzioni di archiviazione consentono l'eliminazione sicura dei cluster HDInsight utilizzati per i calcoli, senza perdita di dati utente.
...
|------------------------|------------------------------------|---------------------------------------|------|
| HDInsight versione 4.0 | No | No |ADLS Gen1 non è supportato con HDInsight 4.0ADLS Gen1 is not supported with HDInsight 4.0 |
...
L'uso di Data Lake Storage Gen1 come account di archiviazione aggiuntivo non influisce sulle prestazioni o sulla possibilità di leggere o scrivere nell'archiviazione di Azure dal cluster.
...
Se si vuole configurare Azure Data Lake Storage Gen1 come risorsa di archiviazione predefinita per un nuovo cluster e il certificato per l'entità servizio viene archiviato in Archiviazione delle chiavi di Azure, sono necessari alcuni passaggi aggiuntivi per convertire il certificato nel formato corretto. I frammenti di codice seguenti mostrano come eseguire la conversione.
...
È possibile usare Data Lake Storage Gen1 anche come risorsa di archiviazione aggiuntiva per il cluster. In questi casi la risorsa di archiviazione predefinita del cluster può essere un BLOB del servizio di archiviazione di Azure o un account Data Lake Storage. Se si eseguono processi HDInsight sui dati archiviati in Data Lake Storage come spazio di archiviazione aggiuntivo, è necessario usare il percorso completo dei file. Ad esempio:

    adl://mydatalakestore.azuredatalakestore.net/<file_path>

Si noti che al momento non esiste alcun **cluster_root_path** nell'URL. Questo perché Data Lake Storage non è uno storage predefinito in questo caso, quindi tutto quello che devi fare è fornire il percorso dei file.
...
Per altre informazioni sulle nozioni di base del modello di controllo di accesso per Data Lake Storage Gen1, vedere Controllo di accesso in Archiviazione data lake di [AzureGen1.](../data-lake-store/data-lake-store-access-control.md)
...
Gli esempi sono basati su una [connessione ssh](./hdinsight-hadoop-linux-use-ssh-unix.md) al nodo head del cluster. Negli esempi vengono utilizzati tutti e tre gli schemi URI. Sostituire `DATALAKEACCOUNT` `CLUSTERNAME` e con i valori pertinenti.
...
Per identificare il percorso completo dell'archivio predefinito configurato, `fs.defaultFS` passare a**Configurazioni** **HDFS** > e immettere nella casella di input del filtro.
...
* [Utilizzo del portale](../storage/data-lake-storage/quickstart-create-connect-hdi-cluster.md)
