...
- Con lo stato attivo della mappa, usare il segno`+`più () o il segno`=`di uguale () per ingrandire un livello.
- Con la mappa concentrata, usare il segno meno, il`-`trattino () o il`_`carattere di sottolineatura () per eseguire lo zoom indietro di un livello.
- Utilizzare il controllo zoom con un mouse, un tocco o una scheda tastiera/invio.
- Premere e tenere premuto `Shift` il pulsante e premere il pulsante sinistro del mouse sulla mappa e trascinare per creare un'area in cui ingrandire la mappa.
...
- Se si creano controlli personalizzati, è utile verificare se i controlli incorporati utilizzano uno stile a contrasto elevato. Gli sviluppatori possono aggiungere una classe CSS sul div del contenitore della mappa da controllare. Le classi CSS che verrebbero aggiunte sono `high-contrast-dark` e. `high-contrast-light` Per controllare l'uso di JavaScript, usare:
...
| Segno più (`+`) o <sup>*</sup>segno di uguale (`=`) | Fare zoom avanti |
| Segno meno, trattino`-`() o <sup>*</sup>carattere di sottolineatura (`_`) | Fare zoom indietro | 
| `Shift`+ Trascinare il mouse sulla mappa per l'area di estrazione | Ingrandisci area |
