---
title: Copertura per gli itinerari in Mappe di Azure | Microsoft Docs
description: Informazioni sulla copertura per gli itinerari in Mappe di Azure
author: jingjing-z
ms.author: jinzh
ms.date: 04/04/2019
ms.topic: conceptual
ms.service: azure-maps
services: azure-maps
manager: ''
ms.openlocfilehash: ef62ec78bc7fc01bd2e6dd7c5561b15be85540d2
ms.sourcegitcommit: 3102f886aa962842303c8753fe8fa5324a52834a
ms.translationtype: MT
ms.contentlocale: it-IT
ms.lasthandoff: 04/23/2019
ms.locfileid: "60768802"
---
# <a name="azure-maps-routing-coverage"></a>Copertura per gli itinerari in Mappe di Azure

Questo articolo contiene informazioni sulla copertura per gli itinerari in Mappe di Azure. Quando si esegue la ricerca ottimale route dal percorso A B, le mappe di Azure può offrire tempi di viaggio estremamente precisi, live istruzioni informazioni e la route aggiornata travel. La route prenderà in termini di ricerca aggiuntive di account, ad esempio il traffico corrente, tipo del veicolo e sulle condizioni per evitare. La possibilità di ottimizzare l'itinerario dipende l'area come mappe di Azure dispone di vari livelli di informazioni e accuratezza per aree geografiche diverse. La tabella seguente elenca le aree e quale tipo di informazioni è possibile richiedere per loro.
 
Vedere le informazioni sulla copertura per la [**geocodifica**](geocoding-coverage.md).       
Consultare le informazioni sulla copertura per il [**traffico**](traffic-coverage.md).  
Consultare le informazioni sulla copertura per il [**routing**](render-coverage.md).
<br>  
**Legenda**

| Simbolo             | Significato                                |
|--------------------|----------------------------------------|
| ✓                  | Paese fornito con dati dettagliati.   |
| Ø                  | Paese fornito con dati semplificati. |
| Paese mancante | Dati del paese non specificati.          |

<br>  

La tabella seguente riporta informazioni sulla copertura per gli itinerari in Mappe di Azure.
<br>  


## <a name="africa"></a>Africa

| Paese/Area geografica                 | Routing.         | Routing del traffico | Routing del veicolo |
|--------------------------------|:---------------:|:--------------------:|:------------:|
| Algeria                        |        ✓       |                       |             |
| Angola                         |        ✓       |                       |             |
| Benin                          |        ✓       |                       |             |
| Botswana                       |        ✓       |                       |             |
| Burkina Faso                   |        ✓       |                       |             |
| Burundi                        |        ✓       |                       |             |
| Camerun                       |        ✓       |                       |             |
| Cabo Verde                     |        ✓       |                       |             |
| Congo - Brazzaville Kinshasa    |        ✓       |                       |             |
| Congo - Kinshasa                |        ✓       |                       |             |
| Côte d’Ivoire (Costa d'Avorio)                  |        ✓       |                       |             |
| Egitto                          |        ✓       |         ✓            |             |
| Gabon                          |        ✓       |                       |             |
| Gambia                         |        ✓       |                       |             |
| Ghana                          |        ✓       |                       |             |
| Kenya                          |        ✓       |         ✓            |             |
| Lesotho                        |        Ø        |         ✓            |             |
| Malawi                         |        ✓       |                       |              |
| Mali                           |        ✓       |                       |             |
| Mauritania                     |        ✓       |                       |             |
| Mauritius, Mayotte e Réunion |        ✓      |                       |             |
| Marocco                        |        ✓       |                       |             |
| Mozambico                     |        ✓       |         ✓            |             |
| Namibia                        |        ✓       |                       |             |
| Niger                          |        ✓       |                       |             |
| Nigeria                        |        ✓       |         ✓            |             |
| Ruanda                         |        ✓       |                       |             |
| Senegal                        |        ✓       |                       |             |
| Seychelles                     |        ✓       |                       |             |
| Sud Africa                   |        ✓       ||         ✓            |             |
| Swaziland                      |        ✓       |                       |             |
| Tanzania                       |        ✓       |                       |             |
| Togo                           |        ✓       |                       |             |
| Tunisia                        |        ✓       |                       |             |
| Uganda                         |        ✓       |                       |             |
| Sahara occidentale                 |        ✓       |                       |             |
| Zambia                         |        ✓       |                       |             |
| Zimbabwe                       |        ✓       |                       |             |


## <a name="americas"></a>Americhe

| Paese/Area geografica                 | Routing.         | Routing del traffico | Routing del veicolo |
|--------------------------------|:---------------:|:--------------------:|:------------:|
| Argentina                      |        ✓       |         ✓            |     ✓       |
| Antigua e Barbuda            |        ✓       |                      |             |
| Bahamas                        |        ✓       |                      |             |
| Barbados                       |        ✓       |                      |             |
| Belize                         |        ✓       |                      |             |
| Brasile                         |        ✓       |         ✓            |     ✓       | 
| Canada                         |        ✓       |         ✓            |     ✓      |
| Cile                          |        ✓       |         ✓            |     ✓      |
| Colombia                       |        ✓       |         ✓            |             |
| Costa Rica                     |        ✓       |                      |             |
| Cuba                           |        ✓       |                      |             |
| Dominica                       |        ✓       |                      |             |
| Repubblica dominicana             |        ✓       |                      |             |
| Ecuador                        |        ✓       |                      |             |
| El Salvador                    |        ✓       |                      |             |
| Guayana francese                  |        ✓       |                      |             |
| Grenada                        |        ✓       |                      |             |
| Guatemala                      |        ✓       |                      |             |
| Guiana                         |        ✓       |                      |             |
| Haiti                          |        ✓       |                      |             |
| Honduras                       |        ✓       |                      |             |
| Giamaica                        |        ✓       |                      |             |
| Messico                         |        ✓       |         ✓            |     ✓      | 
| Nicaragua                      |        ✓       |                      |             |
| Panama                         |        ✓       |                      |             |
| Paraguay                       |        ✓       |                      |             |
| Perù                           |        ✓       |         ✓            |             |
| Saint Kitts e Nevis            |        ✓       |                      |             |
| Saint Lucia                      |        ✓       |                      |             |
| Saint Vincent & Grenadine       |        ✓       |                      |             |
| Suriname                       |        ✓       |                      |             |
| Trinidad e Tobago              |        ✓       |                      |             |
| Stati Uniti                  |        ✓       |         ✓            |     ✓      | 
| + Isole Samoa americane                |        ✓       |                      |             |
| + Isole Marianne Settentrionali      |        ✓       |                      |             |
| +Portorico                   |        ✓       |                      |             |
| + STATI UNITI Isole Vergini americane           |        ✓       |                      |             |
| Uruguay                        |        ✓       |         ✓            |     ✓      | 
| Venezuela                      |        ✓       |                      |             |


## <a name="asia-pacific"></a>Asia/Pacifico

| Paese/Area geografica                 | Routing.         | Routing del traffico | Routing del veicolo |
|--------------------------------|:---------------:|:--------------------:|:------------:|
| Australia                      |        ✓       |         ✓            |     ✓       |
| + Isola Christmas              |        ✓       |                      |             |
| Brunei                         |        ✓       |         ✓            |             |
| Cambogia                       |        ✓       |                      |             |
| Figi                           |        ✓       |                      |             |
| Guam                           |        ✓       |                      |             |
| RAS di Hong Kong                  |        ✓       |         ✓            |             |
| India                          |        ✓       |         ✓            |             |
| Indonesia                      |        ✓       |         ✓            |             |
| Corea del Sud                          |        Ø       |                      |             |
| Laos                           |        ✓       |                      |             |
| RAS di Macao                      |        ✓       |         ✓            |             |
| Malaysia                       |        ✓       |         ✓            |             |
| Myanmar                        |        ✓       |                      |             |
| Nuova Zelanda                    |        ✓       |         ✓            |     ✓       | 
| Filippine                    |        ✓       |         ✓            |             |
| Singapore                      |        ✓       |         ✓            |             |
| Taiwan                         |        ✓       |         ✓            |             |
| Thailandia                       |        ✓       |         ✓            |             |
| Vietnam                        |        ✓       |         ✓            |             |


## <a name="europe"></a>Europa

| Paese/Area geografica                 | Routing.         | Routing del traffico | Routing del veicolo |
|--------------------------------|:---------------:|:--------------------:|:------------:|
| Albania                        |        ✓       |                      |             |
| Andorra                        |        ✓       |         ✓            |             |
| Austria                        |        ✓       |         ✓            |     ✓       | 
| Bielorussia                        |        ✓       |                      |             |
| Belgio                        |        ✓       |         ✓            |     ✓      | 
| Bolivia                        |        ✓       |                      |             |
| Bosnia ed Erzegovina             |        ✓       |                      |             |
| Bulgaria                       |        ✓       |         ✓            |     ✓      | 
| Croazia                        |        ✓       |         ✓            |     ✓      |
| Cipro                         |        ✓       |                      |             |
| Repubblica ceca                 |        ✓       |         ✓            |     ✓      |
| Danimarca                        |        ✓       |         ✓            |     ✓      |
| + Isole Faroe                 |        ✓       |                      |             |
| Estonia                        |        ✓       |         ✓            |     ✓      |
| Finlandia                        |        ✓       |         ✓            |     ✓      |
| Francia                         |        ✓       |         ✓            |     ✓      | 
| + Guadalupa                    |        ✓       |                      |             |
| +Martinique                    |        ✓       |                      |             |
| + Saint Barthélemy                |        ✓       |                      |             |
| +St. Martin                    |        ✓       |                      |             |
| + Saint Pierre e Miquelon         |        ✓       |                      |             |
| Georgia                        |        ✓       |                      |             |
| Germania                        |        ✓       |         ✓            |     ✓      | 
| Grecia                         |        ✓       |         ✓            |     ✓      |  
| Isola di Guernsey                       |        ✓       |                      |             |
| Ungheria                        |        ✓       |         ✓            |     ✓      | 
| Islanda                        |        ✓       |         ✓            |             |
| Irlanda                        |        ✓       |         ✓            |     ✓      | 
| Italia                          |        ✓       |         ✓            |     ✓      | 
| Jersey                         |        ✓       |                      |             |
| Lettonia                         |        ✓       |         ✓            |     ✓      | 
| Liechtenstein                  |        ✓       |         ✓            |             |
| Lituania                      |        ✓       |         ✓            |     ✓      | 
| Lussemburgo                     |        ✓       |         ✓            |     ✓      | 
| Macedonia del Nord                |        ✓       |                      |             |
| Malta                          |        ✓       |         ✓            |             |
| Moldova                        |        ✓       |                      |             |
| Principato di Monaco                         |        ✓       |         ✓            |             |
| Montenegro                     |        ✓       |                      |     ✓      | 
| Paesi Bassi                    |        ✓       |         ✓            |     ✓      | 
| +Aruba                         |        ✓       |                      |             | 
| + Paesi Bassi Caraibi         |        ✓       |                      |             |
| +Curaçao                       |        ✓       |                      |             |
| + Maarten Saint                  |        ✓       |                      |             |
| Norvegia                         |        ✓       |         ✓            |     ✓      | 
| Polonia                         |        ✓       |         ✓            |     ✓      | 
| Portogallo                       |        ✓       |         ✓            |     ✓      |  
| Romania                        |        ✓       |         ✓            |     ✓      | 
| Federazione russa             |        ✓       |         ✓            |     ✓      | 
| San Marino                     |        ✓       |         ✓            |             |
| + Maarten Saint                  |        ✓       |                      |             |
| Serbia                         |        ✓       |                      |     ✓      | 
| Slovacchia                       |        ✓       |         ✓            |     ✓      | 
| Slovenia                       |        ✓       |         ✓            |     ✓      |
| Spagna                          |        ✓       |         ✓            |     ✓      | 
| Svezia                         |        ✓       |         ✓            |     ✓      | 
| Svizzera                    |        ✓       |         ✓            |     ✓      | 
| Turchia                         |        ✓       |         ✓            |     ✓      | 
| Ucraina                        |        ✓       |         ✓            |             | 
| Regno Unito                 |        ✓       |         ✓            |     ✓      | 
| +Anguilla                      |        ✓       |                      |             |
| +Bermuda                       |        ✓       |                      |             |
| + Le Isole Vergini britanniche        |        ✓       |                      |             |
| + Isole Cayman                |        ✓       |                      |             |
| +Gibilterra                      |        ✓       |         ✓            |             | 
| +Montserrat                    |        ✓       |                      |             |
| + Isole Turks e Caicos isole        |        ✓       |                      |             |
| Città del Vaticano                   |        ✓       |         ✓            |             |
| Isola di Man                    |        ✓       |                      |             |


## <a name="middle-east"></a>Medio Oriente

| Paese/Area geografica                 | Routing.         | Routing del traffico | Routing del veicolo |
|--------------------------------|:---------------:|:--------------------:|:------------:|
| Bahrain                        |        ✓       |         ✓            |             |
| Iraq                           |        ✓       |                      |             |
| Israele                         |        ✓       |         ✓            |             |
| Giordania                         |        ✓       |                      |             |
| Kuwait                         |        ✓       |         ✓            |             |
| Libano                        |        ✓       |                      |             |
| Oman                           |        ✓       |         ✓            |             |
| Qatar                          |        ✓       |         ✓            |             |
| Arabia Saudita                   |        ✓       |         ✓            |             |
| Emirati Arabi Uniti           |        ✓       |         ✓            |             |
| Yemen                          |        ✓       |                      |             |


## <a name="next-steps"></a>Passaggi successivi

Per altre informazioni sul routing di mappe di Azure, vedere la [Routing](https://docs.microsoft.com/rest/api/maps/route) le pagine di riferimento.
