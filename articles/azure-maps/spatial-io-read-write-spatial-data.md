...
Nel codice seguente viene illustrato come leggere un set di dati spaziali ed eseguirne il rendering sulla mappa utilizzando `SimpleDataLayer` la classe. Il codice usa un file GPX a cui punta un URL.
...
Lo strumento seguente illustra la maggior parte delle opzioni di scrittura che è possibile usare con `atlas.io.write` la funzione.

<br/>

<iframe height='700' scrolling='no' title='Opzioni di scrittura dei dati spaziali' src='//codepen.io/azuremaps/embed/YzXxXPG/?height=700&theme-id=0&default-tab=result&embed-version=2&editable=true' frameborder='no' allowtransparency='true' allowfullscreen='true' style='width: 100%;'>Vedere le <a href='https://codepen.io/azuremaps/pen/YzXxXPG/'>Opzioni di scrittura dei dati spaziali</a> di Pen da<a href='https://codepen.io/azuremaps'>@azuremaps</a>mappe di Azure () in <a href='https://codepen.io'>CodePen</a>.
</iframe>
...
Nel codice seguente viene illustrato come leggere la stringa `POINT(-122.34009 47.60995)` di testo nota ed eseguirne il rendering sulla mappa utilizzando un livello Bubble.
...
GML è una specifica del file XML spaziale spesso utilizzata come estensione di altre specifiche XML. I dati GeoJSON possono essere scritti come XML con i tag GML `atlas.io.core.GmlWriter.write` usando la funzione. Il codice XML che contiene GML può essere letto utilizzando `atlas.io.core.GmlReader.read` la funzione. La funzione Read ha due opzioni:
...
La `atlas.io.read` funzione utilizzerà per impostazione `atlas.io.core.GmlReader.read` predefinita la funzione quando rileva che i dati di input sono XML, ma i dati non sono uno degli altri formati XML spaziali di supporto.
