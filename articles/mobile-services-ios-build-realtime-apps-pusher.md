...
1.  [Creazione di un account Pusher](#sign-up)
2.  [Aggiornamento dell'app](#update-app)
3.  [Installazione degli script del server](#install-scripts)
4.  [Test dell'app](#test-app)
...
Creazione di un account Pusher
------------------------------
...
La libreria [libPusher](http://go.microsoft.com/fwlink/p?LinkId=276999) consente di accedere a Pusher da iOS.

1.  Scaricare la libreria libPusher [da qui](http://go.microsoft.com/fwlink/p/?LinkId=276998).
...
8.  Nell'area **Link Binary With Libraries** aggiungere le librerie seguenti:
...
Installazione degli script del server
-------------------------------------
...
5.  Sostituire i segnaposto nello script precedente con i valori copiati dalla finestra di dialogo Connection Info:
    ...
    -   **`**your_app_id**`**: valore di app\_id
    -   **`**your_app_key**`**: valore di app\_key
    -   **`**your_app_key_secret**`**: valore di app\_key\_secret
...
Per ulteriori informazioni sulla registrazione e l'utilizzo di script del server, vedere [Riferimento per gli script del server di Servizi mobili](http://go.microsoft.com/fwlink/p/?LinkId=262293).
