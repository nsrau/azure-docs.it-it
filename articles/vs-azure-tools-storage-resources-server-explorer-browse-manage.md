...
3. Premere INVIO sulla tastiera, oppure fare clic o toccare all'esterno del campo del nome per salvare il contenitore BLOB.
...
* Eliminare un BLOB.
  ...
  > [!NOTE]
  > L'eliminazione di un file da un contenitore BLOB non comporta l'eliminazione del file sottostante. Viene solo rimosso dal contenitore BLOB.
  > 
  > 
* Aprire un BLOB.
...
2. Nella finestra di dialogo **Carica nuovo file** selezionare il pulsante **Sfoglia** per specificare il file da caricare e quindi immettere un nome di cartella nella casella **Cartella (facoltativa)**.
...
3. Fare doppio clic sulla cartella o premere INVIO per visualizzare il contenuto della cartella. Dalla cartella del contenitore è possibile tornare alla radice del contenitore selezionando il pulsante **Apri directory padre** (freccia).
...
* Selezionare il pulsante **Carica BLOB** quando il contenitore è aperto nella relativa visualizzazione.
...
> [!NOTE]
> Le code dei servizi di archiviazione sono diverse dalle code del bus di servizio di Azure. Per altre informazioni sulle code del bus di servizio, vedere[Code, argomenti e sottoscrizioni del bus di servizio](https://docs.microsoft.com/en-us/azure/service-bus-messaging/service-bus-queues-topics-subscriptions).
> 
> 
...
4. Per eseguire la query, scegliere l'icona a forma di triangolo verde.
...
Se è stato creato un account di archiviazione con l'opzione di replica **Archiviazione con ridondanza geografica e accesso in lettura**, è possibile visualizzare gli endpoint secondari:

* Aprire il menu di scelta rapida per il nome account e quindi selezionare **Proprietà**.
