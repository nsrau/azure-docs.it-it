...
7.  Modificare la riga di avvio del kernel in Grub per includere i parametri seguenti. In questo modo si garantisce inoltre che tutti i messaggi della console vengano inviati alla prima porta seriale, agevolando così il supporto di Azure nella risoluzione dei problemi di debug:
...
1.  È consigliabile modificare l'impostazione di /etc/sysconfig/network/dhcp o equivalente da DHCLIENT\_SET\_HOSTNAME="yes" a DHCLIENT\_SET\_HOSTNAME="no".

2.  In /etc/sudoers impostare come commento la riga seguente, se esistente:

        Defaults targetpw

3.  Verificare che il server SSH sia installato e configurato per l'esecuzione all'avvio.

4.  Non creare l'area di swap sul disco del sistema operativo.
...
5.  Eseguire i comandi seguenti per effettuare il deprovisioning della macchina virtuale e prepararla per il provisioning in Azure:

        waagent -force -deprovision
        export HISTSIZE=0
        logout

6.  Verificare che l'agente Linux di Azure venga eseguito all'avvio:

        systemctl enable waagent.service

7.  Fare clic su **Arresto** nella console di gestione di Hyper-V.
