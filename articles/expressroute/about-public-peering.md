---
title: Creare e gestire il peering pubblico di Azure ExpressRouteCreate and manage Azure ExpressRoute public peering
description: Informazioni sul peering pubblico di Azure e sulla gestione
...
---
# <a name="create-and-manage-expressroute-public-peering"></a>Creare e gestire il peering pubblico ExpressRouteCreate and manage ExpressRoute public peering
...
Questo articolo consente di creare e gestire la configurazione del routing di peering pubblico per un circuito ExpressRoute.This article helps you create and manage public peering routing configuration for an ExpressRoute circuit. È inoltre possibile controllare lo stato, aggiornare o eliminare e deprovisioning i peering. Questo articolo si applica ai circuiti di Resource Manager creati prima del peering pubblico deprecato. Se si dispone di un circuito esistente in precedenza (creato prima che il peering pubblico sia deprecato), è possibile gestire/configurare il peering pubblico usando [Azure PowerShell,](#powershell) [l'interfaccia della riga di comando](#cli)di Azure e il portale di [Azure.](#portal)

>[!NOTE]
>Il peering pubblico è deprecato. Non è possibile creare peering pubblico su nuovi circuiti ExpressRoute.You cannot create public peering on new ExpressRoute circuits. Se si dispone invece di un nuovo circuito ExpressRoute, usare [il peering Microsoft](expressroute-circuit-peerings.md#microsoftpeering) per i servizi di Azure.If you have a new ExpressRoute circuit, instead, use Microsoft peering for your Azure services.
>

## <a name="connectivity"></a>Connettività

La connettività viene sempre attivata dalla rete WAN verso i servizi di Microsoft Azure. I servizi di Microsoft Azure non potranno attivare connessioni alla rete dell'utente tramite questo dominio di routing. Se il circuito ExpressRoute è abilitato per il peering pubblico di Azure, è possibile accedere agli [intervalli IP pubblici usati in Azure](../virtual-network/virtual-network-ip-addresses-overview-arm.md#public-ip-addresses) nel circuito.

Dopo aver abilitato il peering pubblico, è possibile connettersi alla maggior parte dei servizi di Azure.Once public peering is enabled, you can connect to most Azure services. Non è consentito scegliere in modo selettivo i servizi per cui vengono annunciate route.

* Servizi come Archiviazione di Azure, Database SQL e Siti Web sono disponibili negli indirizzi IP pubblici.
...
In questa sezione vengono illustrati i servizi disponibili tramite il peering pubblico. Poiché il peering pubblico è deprecato, non è prevista l'aggiunta di servizi nuovi o aggiuntivi al peering pubblico. Se si usa il peering pubblico e il servizio che si desidera utilizzare è supportato solo tramite il peering Microsoft, è necessario passare al peering Microsoft. Per un elenco dei servizi supportati, vedere [Peering Microsoft.See Microsoft peering](expressroute-faqs.md#microsoft-peering) for a list of supported services.

**Supportati:**

* Power BI
* È supportata la maggior parte dei servizi di Azure. Rivolgersi direttamente al servizio che si desidera utilizzare per verificare il supporto.
...
Per convalidare la disponibilità per un servizio specifico, è possibile consultare la documentazione relativa a tale servizio per verificare se è presente un intervallo riservato pubblicato per tale servizio. È quindi possibile cercare gli intervalli IP del servizio di destinazione e confrontarli con gli intervalli elencati nel [file AZURE IP Ranges and Service Tags – Public Cloud XML file](https://www.microsoft.com/download/details.aspx?id=56519). In alternativa, è possibile aprire un ticket di supporto per il servizio in questione per chiarimenti.
...
> [!NOTE]
> Il peering pubblico di Azure ha 1 indirizzo IP NAT associato a ogni sessione BGP. Per un numero maggiore di 2 indirizzi IP NAT, passare a Peering Microsoft.For greater than 2 NAT IP addresses, move to Microsoft peering. Il peering Microsoft consente di configurare allocazioni NAT personalizzate e di usare filtri di route per annunci di prefisso selettivo. Per ulteriori informazioni, vedere [Passare a Microsoft peering](https://docs.microsoft.com/azure/expressroute/how-to-move-peering).
>
...
## <a name="azure-powershell-steps"></a><a name="powershell"></a>Passaggi di Azure PowerShellAzure PowerShell steps


[!INCLUDE [CloudShell](../../includes/expressroute-cloudshell-powershell-about.md)]

Poiché il peering pubblico è deprecato, non è possibile configurare il peering pubblico in un nuovo circuito ExpressRoute.Because public peering is deprecated, you cannot configure public peering on a new ExpressRoute circuit.

1. Verificare di disporre di un circuito ExpressRoute di cui è stato eseguito il provisioning e abilitato. Usare l'esempio seguente:
...
## <a name="azure-cli-steps"></a><a name="cli"></a>Passaggi dell'interfaccia della riga di comando di AzureAzure
...
Per configurare il peering, usare i passaggi di PowerShell o dell'interfaccia della riga di comando contenuti in questo articolo. Per gestire un peering, puoi usare le sezioni seguenti. Per riferimento, questi passaggi sono simili alla gestione di un [peering Microsoft nel portale.](expressroute-howto-routing-portal-resource-manager.md#msft)
...
Selezionare la riga per il peering, quindi modificare le proprietà di peering.

### <a name="to-delete-azure-public-peering"></a><a name="delete"></a>Per eliminare un peering pubblico di Azure

Rimuovere la configurazione di peering selezionando l'icona di eliminazione.
