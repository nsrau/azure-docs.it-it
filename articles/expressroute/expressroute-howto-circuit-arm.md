...
- La famiglia SKU determina il tipo di fatturazione. Specificare "Metereddata" per un piano dati a consumo e "Unlimiteddata" per un piano dati senza limiti. **Nota:** è possibile modificare il tipo di fatturazione da "Metereddata" a "Unlimiteddata", ma non è possibile passare da "Unlimiteddata" a "Metereddata".
...
È possibile recuperare queste informazioni in qualsiasi momento usando il cmdlet `Get-AzureRmExpressRouteCircuit`. Se si esegue la chiamata senza parametri, verranno elencati tutti i circuiti. La chiave di servizio verrà indicata nel campo "ServiceKey".
...
"ServiceProviderProvisioningState" offre informazioni sullo stato attuale del provisioning sul lato provider del servizio, mentre "Status" indica lo stato sul lato Microsoft. Per altre informazioni sullo stato del provisioning dei circuiti, vedere l'articolo relativo ai [flussi di lavoro](expressroute-workflows.md#expressroute-circuit-provisioning-states).
...
La verifica dello stato e della condizione della chiave del circuito comunicano all'utente quando il provider ha abilitato il circuito. Dopo la configurazione del circuito, "ServiceProviderProvisioningState" risulta "Provisioned", come illustrato nell'esempio seguente:
...
Per istruzioni dettagliate, vedere la sezione relativa alla [configurazione di routing del circuito ExpressRoute](expressroute-howto-routing-arm.md) per creare e modificare i peering del circuito.
...
Per le opzioni relative alla larghezza di banda supportate per il provider, vedere la pagina [Domande frequenti su ExpressRoute](expressroute-faqs.md). È possibile scegliere qualsiasi dimensione maggiore della dimensione del circuito esistente.
...
Vedere le istruzioni relative allo [spostamento dei circuiti ExpressRoute dal modello di distribuzione classica a quello di Resource Manager](expressroute-howto-move-arm.md).
