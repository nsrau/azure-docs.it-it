...
# Creare e modificare la configurazione di routing di ExpressRoute

> [AZURE.SELECTOR]
[PowerShell Classic](expressroute-howto-routing-classic.md)
[PowerShell Resource Manager](expressroute-howto-routing-arm.md)

Questo articolo descrive le procedure per creare e gestire la configurazione di routing per un circuito ExpressRoute usando i cmdlet di PowerShell e il modello di distribuzione ARM. La procedura seguente mostra anche come controllare lo stato e aggiornare, eliminare o effettuare il deprovisioning dei peering per un circuito ExpressRoute.
...
1. **Importare il modulo PowerShell per ExpressRoute.**
 	...
 	È necessario installare il programma di installazione più recente Powershell da [PowerShell Gallery](http://www.powershellgallery.com/) e importare i moduli di Gestione risorse di Azure nella sessione di PowerShell per iniziare a utilizzare i cmdlet di ExpressRoute. È necessario eseguire PowerShell come amministratore.

	    Install-Module AzureRM

		Install-AzureRM

	Importare tutti i moduli AzureRM.* all'interno dell'intervallo della versione semantica nota

		Import-AzureRM

	È possibile anche solo importare un modulo selezionato all'interno dell'intervallo di versione semantica nota
...
>[AZURE.IMPORTANT]Assicurarsi che tutte le reti virtuali siano scollegate dal circuito ExpressRoute prima di eseguire questo cmdlet.
