...
ExpressRoute Direct offre la possibilità di connettersi direttamente alla rete globale di Microsoft in località peer distribuite in modo strategico in tutto il mondo. ExpressRoute Direct offre una connettività dual e 10 Gbps, che supporta la connettività attiva/attiva su larga scala.
...
## <a name="onboard-to-expressroute-direct"></a>Eseguire l'onboarding a ExpressRoute DirectOnboard to ExpressRoute Direct

Prima di usare ExpressRoute Direct, è necessario registrare la sottoscrizione. Per eseguire la registrazione inviare un messaggio di posta elettronica all'indirizzo <ExpressRouteDirect@microsoft.com> con l'ID della sottoscrizione, includendo i dettagli seguenti:
...
| --- | --- |
| Usa i provider di servizi per abilitare il caricamento rapido e la connettività nell'infrastruttura esistente | Richiede un'infrastruttura 100 Gbps/10 Gbps e una gestione completa di tutti i livelli
...
| SKU di circuiti da 50 Mbps a 10 Gbps | Il cliente può selezionare una combinazione dei seguenti SKU di circuito su 100 Gbps ExpressRoute Direct: <ul><li>5 Gbps</li><li>10 Gbps</li><li>40 Gbps</li><li>100 Gbps</li></ul> Il cliente può selezionare una combinazione dei seguenti SKU di circuito su 10 Gbps ExpressRoute Direct:<ul><li>1 Gbps</li><li>2 Gbps</li><li>5 Gbps</li><li>10 Gbps</li></ul>
| Ottimizzato per tenant singolo | Ottimizzato per singolo tenant con più Business Unit e ambienti di lavoro multipli
...
* Interfacce del router Microsoft Enterprise Edge (MSEE):
    * Porte Ethernet a 10 o 100 Gigabit doppie solo su coppia di router
    * Connettività in fibra LR in modalità singola
    * IPv4 e IPv6
    * MTU IP 1500 byte

* Connettività Switch/Router Layer 2/Layer 3:
    * Deve supportare 1 802.1Q (Dot1Q) tag o due Tag 802.1Q (QinQ) incapsulamento
    * Ethertype - 0x8100
    * È necessario aggiungere il tag VLAN esterno (STAG) in base all'ID VLAN specificato da Microsoft - *applicabile solo su QinQ*
    ...
    * Facoltativo: supporto [del rilevamento dell'inoltro bidirezionale (BFD, BD),](https://docs.microsoft.com/azure/expressroute/expressroute-bfd) configurato per impostazione predefinita in tutti i peering privati nei circuiti ExpressRouteOptional: Bidirectional Forwarding Detection (BFD) support, which is configured by default on all Private Peerings on ExpressRoute circuits
