<properties 
	pageTitle="Come usare il servizio di archiviazione BLOB (Ruby) | Microsoft Azure" 
...
In questa guida verranno illustrati diversi scenari comuni di uso del servizio BLOB di Azure. Gli esempi sono scritti usando l'API Ruby.
Gli scenari presentati includono **caricamento, visualizzazione in elenchi, download** ed **eliminazione** di BLOB.
Per altre informazioni sui BLOB, vedere la sezione [Passaggi successivi](#next-steps) .
...
L'oggetto **Azure::BlobService** consente di usare contenitori e BLOB. Per creare un contenitore, usare il metodo **create\_container()**.
...
È sufficiente modificare la chiamata <strong>create\_container()</strong> in modo da passare l'opzione **:public\_access\_level**:
...
Per scaricare i BLOB, usare il metodo **get\_blob()** per recuperare i contenuti. 

Nel seguente esempio viene illustrato l'uso di **get\_blob()** per scaricare i contenuti di "image-blob" e scriverli in un file locale.

	blob, content = azure_blob_service.get_blob(container.name,"image-blob")
	File.open("download.png","wb") {|f| f.write(content)}

## <a id="DeleteBlob"></a>Procedura: Eliminare un BLOB
Infine, per eliminare un BLOB, usare il metodo **delete\_blob()**. Nell'esempio seguente viene illustrato come eliminare un BLOB.
