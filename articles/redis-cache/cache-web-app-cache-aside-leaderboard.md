---
title: Esercitazione per creare un'app Web con Cache Redis che usa il modello cache-aside | Microsoft Docs
description: Informazioni su come creare un'app Web con Cache Redis che usa il modello cache-aside
...
## <a name="configure-the-app-for-redis-cache"></a>Configurare l'app per Cache Redis
...
In questo test sia l'applicazione che il database vengono eseguiti in locale. Tuttavia, Cache Redis è ospitata da remoto in Azure. Pertanto, le prestazioni della cache saranno probabilmente leggermente inferiori rispetto a quelle del database. Per prestazioni ottimali, l'applicazione client e la cache Redis di Azure devono trovarsi nella stessa posizione. Nella sezione successiva verranno distribuite tutte le risorse in Azure per vedere le prestazioni migliorate attraverso l'uso di una cache.
