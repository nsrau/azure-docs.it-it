...
Per usare il provider di cache Redis di output, configurare prima di tutto la cache, quindi configurare l'applicazione ASP.NET usando il pacchetto NuGet del provider di cache Redis di output. Questo argomento fornisce indicazioni sulla configurazione dell'applicazione per l'uso del provider di cache Redis di output. Per altre informazioni sulla creazione e sulla configurazione di un'istanza della Cache Redis di Azure, vedere [Creare una cache](cache-dotnet-how-to-use-azure-redis-cache.md#create-a-cache).
...
![Gestione di pacchetti NuGet con la Cache Redis di Azure](./media/cache-asp.net-output-cache-provider/IC729541.png)

Digitare **RedisOutputCacheProvider** nella casella di testo **Cerca online**, selezionarlo nei risultati e fare clic su **Installa**.

![Provider di cache di output della Cache Redis di Azure](./media/cache-asp.net-output-cache-provider/IC751727.jpg)
...
-	**applicationName**: le chiavi vengono archiviate in Redis come <AppName>\_<SessionId>\_Data. Ciò consente a più applicazioni di condividere la stessa chiave. Questo parametro è facoltativo e se non lo si specifica, verrà usato un valore predefinito.
