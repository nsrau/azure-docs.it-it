...
-   [Notifiche push multipiattaforma][Notifiche push multipiattaforma]
-   [Sviluppo di app multipiattaforma][Sviluppo di app multipiattaforma]
-   [Condivisione del codice in progetti di Visual Studio][Condivisione del codice in progetti di Visual Studio]

Per informazioni di carattere generale su Servizi mobili, fare riferimento al [Centro sviluppatori di Servizi mobili][Centro sviluppatori di Servizi mobili].
...
Per ulteriori informazioni, vedere [Hub di notifica di Azure][Hub di notifica di Azure].
...
-   **Registrazione modello**
    Hub di notifica supporta anche registrazioni modello specifiche della piattaforma. Grazie alle registrazioni modello, è possibile inviare una notifica a un'app in esecuzione su qualsiasi piattaforma registrata eseguendo una sola chiamata all'API. Per ulteriori informazioni, vedere [Utilizzo di Hub di notifica per inviare notifiche agli utenti tra piattaforme diverse][Utilizzo di Hub di notifica per inviare notifiche agli utenti tra piattaforme diverse].
...
In un servizio mobile back-end .NET le notifiche vengono inviate chiamando il metodo [SendAsync][SendAsync] sull'oggetto [PushClient][PushClient] ottenuto dalla proprietà [ApiServices.Push][ApiServices.Push]. La notifica push inviata (nativa o modello) dipende dallo specifico oggetto derivato da [IPushMessage][IPushMessage] che viene passato al metodo [SendAsync][SendAsync], come mostrato nella tabella seguente:

| Piattaforma | [APNS][APNS]             | [GCM][GCM]               | [WNS][WNS]                | [MPNS][MPNS]            |
|-------------|----------------------|-----------------------|------------------------|---------------------|
| Nativa      | [ApplePushMessage][ApplePushMessage] | [GooglePushMessage][GooglePushMessage] | [WindowsPushMessage][WindowsPushMessage] | [MpnsPushMessage][MpnsPushMessage] |
...
Quando si usano registrazioni client modello anziché registrazioni client native, è possibile inviare la stessa notifica con una singola chiamata a [SendAsync][SendAsync] fornendo un oggetto [TemplatePushMessage][TemplatePushMessage], come mostrato di seguito:
...
In un servizio mobile back-end JavaScript le notifiche vengono inviate chiamando il metodo **send** sull'oggetto specifico della piattaforma ottenuto dall'[oggetto push][oggetto push] globale, come mostrato nella tabella seguente:

| Piattaforma | [APNS][1]        | [GCM][2]        | [WNS][3]        | [MPNS][4]        |
|-------------|------------------|-----------------|-----------------|------------------|
| Nativa      | [oggetto apns][oggetto apns] | [oggetto gcm][oggetto gcm] | [oggetto wns][oggetto wns] | [oggetto mpns][oggetto mpns] |
...
Quando si usano registrazioni client modello anziché registrazioni client native, è possibile inviare la stessa notifica con una singola chiamata della funzione **send** nell'[oggetto push][oggetto push] globale fornendo un payload di messaggio modello, come mostrato di seguito:
