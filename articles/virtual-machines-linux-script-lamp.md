...
È possibile distribuire l'estensione CustomScript tramite il portale di Azure, PowerShell o l'interfaccia della riga di comando multipiattaforma di Azure(xplat-cli).

In questo esempio verrà distribuita una semplice applicazione LAMP su Ubuntu utilizzando l'interfaccia xplat-cli.

## Prerequisiti

Per questa procedura dettagliata, creare due macchine virtuali di Azure che eseguono Ubuntu 14.04. In questo esempio si chiamano  *script-vm* e  *lamp-vm*. Quando si esegue questa procedura, utilizzare sempre nomi univoci. In una macchina virtuale vengono eseguiti i comandi CLI e nell'altra viene distribuita l'applicazione LAMP.
...
A questo punto, è possibile distribuire l'estensione CustomScript di Linux alla macchina virtuale remota utilizzando l'interfaccia xplat-cli:
...
Di seguito sono riportate alcune risorse aggiuntive per xplat-cli, Linux e l'estensione CustomScript:
