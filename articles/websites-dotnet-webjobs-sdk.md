<properties pageTitle="Cos'&egrave; Azure WebJobs SDK" metaKeywords="Azure Azure WebJobs SDK, Azure storage, Azure queues, Azure tables" description="Introduzione ad Azure WebJobs SDK. Spiega che cos'&egrave; l'SDK e gli scenari tipici in cui &egrave; utile con alcuni esempi di codice." metaCanonical="" services="web-sites,storage" documentationCenter=".NET" title="Cos'&egrave; Azure WebJobs SDK" authors="tdykstra" solutions="" manager="wpickett" editor="jimbe" />
...
## <span id="scenarios"></span></a>Scenari
...
-   Altre attività con esecuzione prolungata da eseguire in un thread in background, ad esempio l'[invio di messaggi di posta elettronica][invio di messaggi di posta elettronica].

## <span id="code"></span></a>Esempi di codice
...
## <span id="workerrole"></span></a>Uso di WebJobs SDK al di fuori dei processi Web
...
Tuttavia il dashboard è disponibile solo come estensione del sito per un sito Web di Azure. Se si vuole eseguirlo all'esterno di un processo Web ma usare comunque il dashboard, è possibile configurare un sito Web di Azure in modo da usare lo stesso account di archiviazione a cui fa riferimento la stringa di connessione del dashboard di WebJobs SDK, nel quale verranno quindi visualizzati i dati relativi all'esecuzione della funzione dal programma che viene eseguito altrove. Sarà possibile accedere al dashboard usando l'URL https://*{nomesitoweb}*.scm.azurewebsites.net/azurejobs/\#/functions. Per altre informazioni, vedere il post di blog riguardo l'[accesso a un dashboard per lo sviluppo locale con WebJobs SDK][accesso a un dashboard per lo sviluppo locale con WebJobs SDK], in cui tuttavia viene usato un vecchio nome per la stringa di connessione.

## <span id="nostorage"></span></a>Uso di WebJobs SDK per richiamare una funzione qualsiasi
...
-   Per altre informazioni, vedere le sezioni relative a [come chiamare manualmente una funzione][come chiamare manualmente una funzione] e [come scrivere i log][come scrivere i log].

## <span id="nextsteps"></span></a>Passaggi successivi

Per altre informazioni su WebJobs SDK, vedere le [risorse consigliate per i Processi Web Azure][risorse consigliate per i Processi Web Azure].
  ...
  [processi Web]: /it-it/documentation/articles/web-sites-create-web-jobs/
  [esercitazione di introduzione a WebJobs SDK]: ../websites-dotnet-webjobs-sdk-get-started/
  [invio di messaggi di posta elettronica]: https://github.com/victorhurdugaci/AzureWebJobsSamples/tree/master/SendEmailOnFailure
  ...
  [come chiamare manualmente una funzione]: ../websites-dotnet-webjobs-sdk-storage-queues-how-to/#manual
  [come scrivere i log]: ../websites-dotnet-webjobs-sdk-storage-queues-how-to/#logs
  [risorse consigliate per i Processi Web Azure]: http://go.microsoft.com/fwlink/?linkid=390226
