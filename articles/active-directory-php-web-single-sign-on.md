...
6. Sostituire il markup generato con il seguente codice, quindi salvare il file **index.php**:
...
8. In Gestione IIS espandere la cartella **Siti** nel menu di sinistra, fare clic con il pulsante destro del mouse su **Sito Web predefinito**, quindi scegliere **Aggiungi applicazione...**.
...
In questo passaggio verranno visualizzate informazioni simili alle seguenti:
		...
		Usage                 : Verify 
> [AZURE.NOTE] 
> È consigliabile salvare questo output, in particolare la chiave simmetrica generata. Questa chiave viene visualizzata solo durante la creazione dell'entità servizio e non sarà recuperabile in seguito. Gli altri valori sono necessari per usare l'API Graph per leggere e scrivere informazioni nella directory.
...
3. Nel nuovo file **federation.ini** immettere le seguenti informazioni, specificando i valori con le informazioni salvate nel passaggio 2 durante la creazione dell'entità servizio:
...
6. Nel nuovo file **secureResource.php** immettere il seguente codice, sostituendo il percorso **c:\phpLibraries** con il percorso radice in cui è stato scaricato il codice di esempio. Il percorso radice dovrebbe includere il file **simpleSAML.php** e la cartella **federation**:
...
## Riepilogo
In questa esercitazione è stato illustrato come creare e configurare un'applicazione PHP a singolo tenant che utilizza le funzionalità Single Sign-On di Azure Active Directory.

Un esempio in cui viene illustrato l'uso di Azure Active Directory e Single Sign-On per i siti Web PHP è disponibile all'indirizzo <https://github.com/WindowsAzure/azure-sdk-for-php-samples/tree/master/WAAD.WebSSO.PHP>.


[Passaggio 1: Creare un'applicazione PHP]: #createapp
