...
È possibile creare un nuovo sito Web di Azure in molti modi differenti tramite il portale di gestione di Azure. Quando si fa clic sul pulsante **New** in basso nel portale, viene visualizzata la finestra di dialogo seguente:

![GlobalWebCreate][GlobalWebCreate]

Per la creazione di un nuovo sito Web sono disponibili tre opzioni: **Quick Create**, **Custom Create** e **From Gallery**. Con ciascuna di tali opzioni occorre selezionare un'area Azure che si allinei alla maggioranza della base utenti.
...
Per informazioni dettagliate sui passaggi necessari per creare un nuovo sito Web, vedere [Introduzione a Siti Web di Azure e ASP.NET][Introduzione a Siti Web di Azure e ASP.NET].
...
Alcuni strumenti di controllo codice sorgete e client FTP utilizzano l'accesso con nome utente/password. Le credenziali di un nuovo sito Web non vengono create automaticamente. È tuttavia possibile eseguire facilmente questa operazione facendo clic su **Reset your deployment credentials**. Al termine, sarà possibile usare qualsiasi client FTP per la distribuzione del proprio sito Web tramite queste credenziali con l'opzione **Nome host FTP** sulla stessa pagina **Dashboard**.

![GlobalWebFTPSettings][GlobalWebFTPSettings]

Si noti che il nome nella sezione Utente FTP/distribuzione è una combinazione del nome del sito Web e del nome utente fornito dall'utente. Quindi, se l'indirizzo del proprio sito fosse "<http://contoso.azurewebsite.net>" e se il proprio nome utente fosse "mioutente", il nome utente per la distribuzione e l'FTP diverrebbe "contoso\\mioutente".
...
<div class="dev-callout">
<strong>Nota</strong>
<p>&Egrave; importante mantenere il file <i>publishsettings</i> al sicuro e fuori dal controllo del codice sorgente, in quanto contiene nomi utente e password per la distribuzione nonch&eacute; per eventuali stringhe di connessione ai database collegati.</p>
</div>
...
Per altre informazioni su questa opzione, vedere [Sviluppo e distribuzione di un sito Web con Microsoft WebMatrix][Sviluppo e distribuzione di un sito Web con Microsoft WebMatrix].
...
<div class="dev-callout">
<strong>Nota</strong>
<p>Nella discussione seguente vi sono due tipi di record DNS di interesse. In primo luogo, un record CNAME pu&ograve; reindirizzare da un unico URL, ad esempio &quot;www.contoso.com&quot;, a un URL differente, come &quot;contoso.azurewebsites.net&quot;. In secondo luogo, un record A pu&ograve; mappare un URL, come &quot;www.contoso.com&quot;, a un indirizzo IP, come 172.16.48.1.</p>
</div>
...
Nel portale di gestione di Azure verificare che l'esecuzione sia in modalità **Condiviso** o Standard nella scheda **Scala** (i domini personalizzati non sono supportati per i siti Web in modalità **Gratuito**). Passare quindi alla scheda **Configure** e fare clic sul pulsante **Manage Domains**. In tal modo sarà possibile associare il sito Web al nome di dominio personalizzato.
...
| Tipo  | Host        | Risposta    | TTL  |
|-------|-------------|-------------|------|
| **A** | contoso.com | 172.16.48.1 | 8000 |

Per altre informazioni, vedere [Configurazione di un nome di dominio personalizzato per un sito Web di Azure][Configurazione di un nome di dominio personalizzato per un sito Web di Azure].
...
<tr>
  ...
  [Sviluppo e distribuzione di un sito Web con Microsoft WebMatrix]: /it-it/develop/net/tutorials/get-started/
  ...
  [Configurazione di un certificato SSL per un sito Web di Azure]: /it-it/develop/net/common-tasks/enable-ssl-web-site/
  [sito Web OpenSSL]: http://www.openssl.org/
  ...
  [voce di Wikipedia su SNI SSL]: http://en.wikipedia.org/wiki/Server_Name_Indication
  [Dettagli prezzi - Siti Web]: /it-it/pricing/details/web-sites/#service-ssl
