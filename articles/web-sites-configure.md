<properties 
	pageTitle="Configurazione delle app Web nel servizio app di Azure" 
...
# Configurazione delle app Web nel servizio app di Azure #
...
- **PHP**: impostare la versione PHP oppure selezionare **DISATTIVATO **per disabilitare PHP. 
...
- **Python**: selezionare la versione di Python oppure **DISATTIVATO** per disabilitare Python.
...
**Web Socket**. Impostare **ATTIVATO** per abilitare il protocollo WebSocket; ad esempio, se l'app Web utilizza [ASP.NET SignalR](http://www.asp.net/signalr) o [socket.io](web-sites-nodejs-chat-app-socketio.md).

**Sempre attivata**. Per impostazione predefinita, le app Web vengono scaricate se restano inattive per un determinato periodo di tempo. Ciò consente al sistema di conservare le risorse. Nella modalità di base o in quella standard, è possibile abilitare l'opzione **Sempre attivata** affinché l'app sia sempre caricata. Se l'app esegue continuamente processi Web, è necessario abilitare l'opzione **Sempre attivata**; in caso contrario, l'esecuzione dei processi potrebbe essere inaffidabile.

**Versione pipeline gestita**. Consente di impostare la [modalità pipeline]( IIShttp://www.iis.net/learn/get-started/introduction-to-iis/introduction-to-iis-architecture#Application). Lasciare questa opzione impostata su Integrato (predefinita), tranne nel caso in cui un'app meno recente richieda una versione precedente di IIS.

**Scambio automatico**. Se si abilita l'opzione Scambio automatico per uno slot di distribuzione, il servizio app immette automaticamente l'app Web in produzione quando si esegue un aggiornamento di quello slot. Per ulteriori informazioni, vedere [Eseguire la distribuzione negli slot di memorizzazione temporanea per le app Web del servizio app di Azure] (web-sites-staged-publishing.md).
...
Per le app.NET, tali stringhe vengono inserite nelle impostazioni della configurazione .NET `connectionStrings` durante il runtime, sostituendo le voci esistenti in cui la chiave è uguale al nome del database collegato. 
...
Ad esempio, se una stringa di connessione MySql venisse denominata `connectionstring1`, l'accesso avverrebbe tramite la variabile di ambiente `MYSQLCONNSTR_connectionString1`.
...
- **Percorso del processore script**. Il percorso assoluto del processore script. Le richieste di file che corrispondono a questo modello saranno elaborate dal processore script. Utilizzare il percorso  `D:\home\site\wwwroot` per fare riferimento alla directory radice del sito.
...
Il nome utente FTP completo è "app\nomeutente", in cui *app* rappresenta il nome della propria app Web. Il nome utente viene elencato nel pannello dell'app Web, in **Elementi essenziali**.  
...
Nella modalità di base o standard, è possibile caricare i certificati SSL di un dominio personalizzato. Per ulteriori informazioni, vedere [Abilitazione di HTTPS per un'app Web](web-sites-configure-ssl-certificate.md). 
...
- Impostare una distribuzione continua. Consultare [Utilizzo di Git per distribuire le app Web nel servizio app di Azure](web-sites-publish-source-control.md)
- Slot di distribuzione. Consultare [Distribuzione negli ambienti di gestione temporanea per le app Web del servizio app di Azure](web-sites-staged-publishing.md)
...
- [Abilitare HTTPS](web-sites-configure-ssl-certificate.md)
- [Ridurre un'app Web nel servizio app di Azure](web-sites-scale.md)
- [Monitoraggio degli elementi di base per le app Web nel servizio app di Azure](web-sites-monitor.md)
