<properties urlDisplayName="How to configure" pageTitle="Come configurare i siti Web - Gestione servizi di Azure" metaKeywords="Azure websites, configuring Azure websites, Azure SQL database, Azure MySQL" description="Informazioni su come configurare siti Web in Azure, incluso come configurare un sito Web per l'uso del database SQL o del database MySQL." metaCanonical="" services="web-sites" documentationCenter="" title="How to Configure Websites" authors="mwasson" solutions="" manager="wpickett" editor="mollybos" />
...
La scheda **Configura** include le sezioni seguenti:
...
- **PHP**: Impostare la versione PHP oppure impostare su **DISATTIVA** per disabilitare PHP. 
- **Java**: Selezionare la versione visualizzata per abilitare Java oppure <strong>DISATTIVA</strong> per disabilitare Java. 
- Se si abilita Java, usare l'opzione <strong>Contenitore Web</strong> per scegliere tra le versioni Tomcat e Jetty.
- **Python**: Impostare la versione Python oppure impostare su **DISATTIVA** per disabilitare Python.

Per motivi tecnici l'abilitazione di Java per il sito Web disabilita le opzioni .NET, PHP e Python.

<strong>Modalità pipeline gestita</strong>. Imposta la [modalità pipeline](http://www.iis.net/learn/get-started/introduction-to-iis/introduction-to-iis-architecture#Application) di IIS. Lasciare questa opzione impostata su Integrato (l'impostazione predefinita) salvo il caso in cui il sito Web richieda una versione precedente di IIS.
...
<strong>Web Socket</strong>. Impostare **ATTIVA**per abilitare il protocollo WebSocket, ad esempio se il sito Web usa [ASP.NET SignalR](http://www.asp.net/signalr) o [socket.io](http://azure.microsoft.com/it-it/documentation/articles/web-sites-nodejs-chat-app-socketio/).

<strong>Always On</strong>. Per impostazione predefinita i siti Web vengono scaricati se restano inattivi per un determinato periodo di tempo. Ciò consente al sistema di conservare le risorse. In modalità Basic o Standard è possibile abilitare  <strong>Always On</strong> per fare in modo che il sito resti sempre caricato. Se nel sito vengono eseguiti processi Web continui, è necessario abilitare **Always On**, altrimenti l'esecuzione dei processi Web potrebbe non avvenire in modo affidabile.
...
In modalità Basic o Standard è possibile caricare certificati SSL per un dominio personalizzato. Per altre informazioni, vedere [Abilitare HTTPS per un sito Web di Azure](href="http://www.windowsazure.com/it-it/documentation/articles/web-sites-configure-ssl-certificate/). 
...
Visualizzare o aggiungere nomi di dominio aggiuntivi per il sito Web. Per altre informazioni, vedere [Configurazione di un nome di dominio personalizzato per un sito Web di Azure](http://www.windowsazure.com/it-it/documentation/articles/web-sites-custom-domain-name/).

### Associazioni SSL

Se sono stati caricati certificati SSL, è possibile associarli a nomi di dominio personalizzati. Per altre informazioni, vedere [Abilitare HTTPS per un sito Web di Azure](href="http://www.windowsazure.com/it-it/documentation/articles/web-sites-configure-ssl-certificate/).
...
- <strong>File system</strong>. Consente di eseguire la scrittura dei log nel file system del sito Web. La registrazione del file system ha una durata di 12 ore. È possibile accedere ai log dalla condivisione FTP del sito Web (vedere [Credenziali FTP](http://azure.microsoft.com/it-it/documentation/articles/web-sites-manage#ftp-credentials)).
- <strong>Archiviazione tabelle</strong>. Consente di scrivere i log nell'archiviazione tabelle di Azure. Non è presente alcun limite di tempo e la registrazione rimane abilitata fino alla relativa disabilitazione. 
- <strong>Archiviazione BLOB</strong>. Consente di scrivere log nell'archivio BLOB di Azure. Non è presente alcun limite di tempo e la registrazione rimane abilitata fino alla relativa disabilitazione.
...
- Se si sceglie <strong>File system</strong>Se si sceglie File system, i log vengono salvati nel sito FTP indicato in "Registri di diagnostica FTP" nella pagina Dashboard (vedere [Credenziali FTP](http://azure.microsoft.com/it-it/documentation/articles/web-sites-manage#ftp-credentials)). 
...
<strong>Messaggi di errore dettagliati</strong>. Se abilitato, i messaggi di errore dettagliati vengono salvati come file HTM. Per visualizzare i file, passare al sito FTP indicato in "Registri di diagnostica FTP" nella pagina Dashboard. I file vengono salvati in /LogFiles/DetailedErrors nel sito FTP (vedere [Credenziali FTP](http://azure.microsoft.com/it-it/documentation/articles/web-sites-manage#ftp-credentials)).

<strong>Traccia delle richieste non riuscite</strong>. Se abilitato, le richieste non riuscite vengono registrate in file XML. Per visualizzare i file, passare al sito FTP indicato in "Registri di diagnostica FTP" nella pagina Dashboard. (vedere [Credenziali FTP](http://azure.microsoft.com/it-it/documentation/articles/web-sites-manage#ftp-credentials)). I file vengono salvati in /LogFiles/W3SVC*xxx*, dove xxx è un identificatore univoco. Questa cartella contiene un file XSL e uno o più file XML. Assicurarsi di scaricare il file XSL, perché fornisce la funzionalità di formattazione e filtro dei contenuti dei file XML.
...
Per configurare applicazioni e directory virtuali associate al sito Web, specificare ogni directory virtuale e il corrispondente percorso fisico relativo alla radice del sito. Facoltativamente è possibile selezionare la casella di controllo <strong>Applicazione</strong> per contrassegnare una directory virtuale come applicazione nella configurazione del sito.
...
1. Nel [portale di gestione](http://manage.windowsazure.com) selezionare **Siti Web** per visualizzare l'elenco dei siti Web creati dall'account attualmente connesso.
...
4. Fare clic su **Collega una risorsa** per visualizzare la procedura guidata di ****collegamento delle risorse.

5. Fare clic su **Crea una nuova risorsa** per visualizzare un elenco dei tipi di risorsa che è possibile collegare al sito Web.

6. Fare clic su **Database SQL** per visualizzare la procedura guidata di ****collegamento del database.

7. Completare i campi richiesti nelle pagine 3 e 4 della procedura guidata di ****collegamento del database, quindi fare clic sul segno di spunta **Fine** a pagina 4.
...
In alternativa, è possibile creare il sito Web con l'opzione **Creazione personalizzata**. Nel menu a discesa **Database** scegliere **Crea un nuovo database MySQL** oppure **Usare un database MySQL esistente**. 
...
Per informazioni sulla configurazione di SSL per un dominio personalizzato in Azure, vedere [Abilitare HTTPS per un sito Web di Azure](http://www.windowsazure.com/it-it/documentation/articles/web-sites-configure-ssl-certificate/). 

##<a name="next"></a>Passaggi successivi

* [Come scalare siti Web](http://www.windowsazure.com/it-it/documentation/articles/web-sites-scale/)
