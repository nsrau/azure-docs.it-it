...
Questa guida illustra diversi scenari di utilizzo comuni del
servizio di archiviazione BLOB di Azure. Gli esempi sono scritti in C\# e in
usa la libreria client di archiviazione di Azure per .NET. Gli scenari presentati includono
**caricamento**, **visualizzazione dell'elenco**, **download** ed **eliminazione** di BLOB. Per
altre informazioni sui BLOB, vedere la sezione [Passaggi successivi][].

> [WACOM.NOTE] Questa guida fa riferimento alla libreria client di archiviazione di Azure per .NET 2.x e versioni successive. La versione consigliata è la libreria client di archiviazione di Azure per .NET 4.x, disponibile tramite [NuGet](https://www.nuget.org/packages/WindowsAzure.Storage/) o come parte di [Azure SDK per .NET](/it-it/downloads/). Vedere [Procedura: Accedere all'archiviazione BLOB a livello di codice][] seguente per informazioni su come ottenere la libreria client di archiviazione.
...
##<a name="create-account"></a><span  class="short-header">Creare un account di archiviazione di Azure</span>
[WACOM.INCLUDE [create-storage-account](../includes/create-storage-account.md)]

##<a name="setup-connection-string"></a><span  class="short-header">Configurare una stringa di connessione di archiviazione</span>

[WACOM.INCLUDE [storage-configure-connection-string](../includes/storage-configure-connection-string.md)]

## <a name="configure-access"> </a><span  class="short-header">Procedura: Accedere all'archiviazione BLOB a livello di codice</span>

###Recupero dell'assembly
Per recuperare l'assembly M`Microsoft.WindowsAzure.Storage.dll`, è consigliabile usare NuGet. Fare clic con il pulsante destro del mouse sul progetto in **Esplora soluzioni** e scegliere **Gestisci pacchetti NuGet**.  Cercare online "WindowsAzure.Storage" e fare clic su **Installa** per installare il pacchetto Archiviazione di Azure e le dipendenze.
...
Aggiungere le seguenti dichiarazioni di spazi dei nomi all'inizio di qualsiasi file C\#
...
le informazioni dell'account di archiviazione. Se si intende usare un 
...
Il tipo **CloudBlobClient** consente di recuperare oggetti che rappresentano
...
## <a name="create-container"> </a><span  class="short-header">Procedura: Creare un contenitore</span>

Ogni BLOB nell'archiviazione di Azure deve risiedere in un contenitore. Questo esempio illustra come creare un contenitore, nel caso in cui non esista già:
...
Per impostazione predefinita, il contenitore è privato ed è necessario specificare la
chiave di accesso alle risorse di archiviazione per scaricare BLOB da questo
contenitore. Se i file all'interno del contenitore dovranno essere disponibili
a tutti, è possibile impostare il contenitore su pubblico usando il
codice seguente:
...
Chiunque acceda a Internet può visualizzare i BLOB di un contenitore pubblico, ma può
modificarli o eliminarli solo se ha la chiave di accesso appropriata.

## <a name="upload-blob"> </a><span  class="short-header">Procedura: Caricare un BLOB in un contenitore</span>
...
Per caricare un file in un BLOB in blocchi, ottenere un riferimento a un contenitore e usarlo per ottenere
un riferimento a un BLOB in blocchi. A questo punto, è possibile caricare qualsiasi
flusso di dati nel riferimento al BLOB chiamando il metodo **UploadFromStream**. Questa operazione creerà il BLOB se non esiste già
o lo sovrascrive in caso contrario. L'esempio seguente illustra come caricare un BLOB in un contenitore e presuppone che il contenitore sia già stato creato.
...
##<a name="list-blob"> </a><span  class="short-header">Procedura: Elencare i BLOB in un contenitore</span>

Per elencare i BLOB in un contenitore, ottenere prima un riferimento al contenitore. È
quindi possibile usare il metodo **ListBlobs** del contenitore per recuperare i BLOB e/o le directory
al suo interno. Per accedere al set completo di proprietà e metodi per un 
oggetto **IListBlobItem**, restituito, è necessario eseguirne il cast in un oggetto **CloudBlockBlob**, 
**CloudPageBlob** o **CloudBlobDirectory**.  Se il tipo è sconosciuto, è possibile usare un 
controllo del tipo per identificare quello in cui eseguire il cast.  Il codice seguente 
illustra come recuperare e visualizzare l'URI di ogni elemento del 
contenitore `photos`:
...
Come illustrato in precedenza, il servizio BLOB include anche il concetto di directory all'interno di
contenitori. È quindi possibile organizzare i BLOB in una struttura più simile a quella delle
cartelle. Si consideri, ad esempio, il seguente set di BLOB in blocchi in un contenitore
denominato 'photos':
...
Quando si chiama il metodo **ListBlobs** sul contenitore 'photos', come nell'esempio precedente, la raccolta restituita
conterrà gli oggetti **CloudBlobDirectory** e **CloudBlockBlob**
che rappresentano le directory e i BLOB contenuti nel livello principale. L'output risultante è il seguente:
...
, indipendentemente dalla directory.  La chiamata a **ListBlobs** è la seguente:
...
## <a name="download-blobs"> </a><span  class="short-header">Procedura: Scaricare BLOB</span>

Per scaricare BLOB, recuperare prima un riferimento al BLOB e quindi chiamare il metodo **DownloadToStream**. L'esempio
seguente usa il metodo **DownloadToStream** per trasferire il contenuto del BLOB
...
##<a name="delete-blobs"> </a><span  class="short-header">Procedura: Eliminare BLOB</span>

Per eliminare un BLOB, ottenere prima un riferimento al BLOB su cui chiamare il metodo
**Delete**.
...
##<a name="list-blobs-async"> </a><span  class="short-header">Procedura: Elencare BLOB nelle pagine in modo asincrono</span> 

Se si sta elencando un numero elevato di BLOB oppure si vuole controllare il numero di risultati restituiti in un'operazione di elenco, è possibile elencare i BLOB nelle pagine dei risultati. Questo esempio illustra come restituire i risultati nelle pagine in modo asincrono, così che l'esecuzione non venga bloccata in attesa della restituzione di un set di risultati di grandi dimensioni.
...
Poiché il metodo di esempio chiama un metodo asincrono, deve essere preceduto dalla parola chiave `async` e deve restituire un oggetto **Task**. La parola chiave await specificata per il metodo **ListBlobsSegmentedAsync** sospende l'esecuzione del metodo di esempio fino al completamento dell'attività di elenco.
...
## <a name="next-steps"></a><span  class="short-header">Passaggi successivi</span>
