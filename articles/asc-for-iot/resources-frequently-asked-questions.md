...
Azure provides an integrated view for monitoring and managing your IoT security as part of your overall security solution through Azure Security Center. Se sei uno sviluppatore di applicazioni, puoi usare la visualizzazione Hub IoT per gestire la sicurezza delle applicazioni IoT.If you are an application developer, you can use IoT Hub view to manage your IoT application security.

## <a name="what-is-azures-unique-value-proposition-for-iot-security"></a>Qual è la proposta di valore unica di Azure per la sicurezza IoT?
...
Sì. Centro sicurezza di Azure per IoT si basa sulla connettività e sull'infrastruttura IoT di Azure.Azure Security Center for IoT relies on Azure IoT connectivity and infrastructure.

## <a name="do-i-have-to-install-an-agent"></a>Devo installare un agente?

L'installazione dell'agente nei dispositivi IoT non è obbligatoria per abilitare il Centro sicurezza di Microsoft Azure per l'IoT.Agent installation on your IoT devices isn't mandatory in order to enable the Microsoft Azure Security Center for IoT. È possibile scegliere tra le tre opzioni seguenti, ottenendo diversi livelli di monitoraggio della sicurezza e funzionalità di gestione in base alla selezione effettuata:
...
Il Centro sicurezza di Azure per l'agente IoT offre la copertura delle minacce a livello di dispositivo per la configurazione, il comportamento e l'accesso del dispositivo (tramite l'analisi della configurazione), il processo & la connettività. Il Security Center di Azure per l'agente di sicurezza IoT non analizza l'attività o i dati correlati all'azienda.

## <a name="where-can-i-get-the-azure-security-center-for-iot-security-agent"></a>Dove è possibile ottenere il Centro sicurezza di Azure per l'agente di sicurezza IoT?
...
Connettività, accesso, configurazione del firewall, elenco processi & linea di base del sistema operativo vengono raccolti dall'agente.

## <a name="how-much-data-will-the-agent-generate"></a>Quanti dati genererà l'agente?

La generazione dei dati dell'agente è guidata dal dispositivo, dall'applicazione, dal tipo di connettività e dalla configurazione dell'agente cliente. A causa dell'elevata variabilità tra dispositivi e soluzioni IoT, è consigliabile distribuire prima l'agente in un ambiente di laboratorio o di test per osservare, apprendere e impostare la configurazione specifica che si adatta alle proprie esigenze, misurando al contempo la quantità di dati generati. Dopo aver avviato il servizio, l'agente del Centro sicurezza di Azure per ioT fornisce consigli operativi per ottimizzare la velocità effettiva dell'agente per facilitare il processo di configurazione e personalizzazione.

## <a name="how-can-i-control-my-billing"></a>Come posso controllare la mia fatturazione?

Il Centro sicurezza di Azure per IoT offre scansioni agente configurabili, buffer di dati e la possibilità di creare avvisi personalizzati che aumentano o riducono la quantità di dati generati dall'agente.
...
1. Verificare che il tipo di agente si adatti alla piattaforma del sistema [operativo designata del dispositivo](how-to-deploy-agent.md)
...
L'agente di sicurezza è progettato per essere rieseguito automaticamente a ogni riavvio del dispositivo.
...
L'agente utilizza le risorse della macchina come qualsiasi altra applicazione/processo e non deve interrompere la normale attività del dispositivo. L'utilizzo delle risorse nel dispositivo su cui viene eseguito l'agente è associato all'installazione e alla configurazione. È consigliabile testare la configurazione dell'agente in un ambiente indipendente, insieme all'interoperabilità con le altre applicazioni e funzionalità IoT, prima di tentare la distribuzione in un ambiente di produzione.

## <a name="im-making-some-maintenance-on-the-device-can-i-turn-off-the-agent"></a>Sto facendo un po' di manutenzione sul dispositivo. Posso spegnere l'agente?
