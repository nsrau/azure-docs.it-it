...
Per completare questa esercitazione, è necessaria un'applicazione Azure IoT Central. Se è stato completato l'avvio rapido [Creare un'applicazione Azure IoT Central](quick-deploy-iot-central-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json), è possibile usare nuovamente l'applicazione creata nella guida. In caso contrario, completare i passaggi seguenti per creare un'applicazione Azure IoT Central vuota:
...
2. Immettere l'indirizzo di posta elettronica e la password usati per accedere alla sottoscrizione di Azure:
    ...
    ![Immettere l'account dell'organizzazione](./media/tutorial-define-device-type-experimental/sign-in.png)

3. Per iniziare a creare una nuova applicazione Azure IoT Central, fare clic su **Nuova applicazione**:

    ![Pagina Application Manager di Azure IoT Central](./media/tutorial-define-device-type-experimental/iotcentralhome.png)
...
4. Per creare una nuova applicazione Azure IoT Central:
    ...
    * Scegliere **Versione di valutazione**. Non è necessaria una sottoscrizione di Azure per creare una versione di valutazione dell'applicazione.
       ...
       Per altre informazioni su directory e sottoscrizioni, vedere la [guida introduttiva per la creazione di un'applicazione](quick-deploy-iot-central-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json).
    ...
    * Fare clic su **Create**(Crea).
    ...
    ![Pagina Crea applicazione di Azure IoT Central](./media/tutorial-define-device-type-experimental/iotcentralcreate.png)

    Per altre informazioni, vedere la [guida introduttiva per la creazione di un'applicazione](quick-deploy-iot-central-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json).
...
Per aggiungere un nuovo modello di dispositivo all'applicazione, è necessario passare alla pagina **Modelli di dispositivo**. A tale scopo, fare clic su **Modelli di dispositivo** nel menu di spostamento a sinistra.

![Pagina Modelli di dispositivo](./media/tutorial-define-device-type-experimental/devicetemplates.png)
...
1. Nella pagina **Modelli di dispositivo**, fare clic su **+ Nuovo**:

    ![Pagina Modelli di dispositivo, Crea modello di dispositivo](./media/tutorial-define-device-type-experimental/newtemplate.png)

3. Nella pagina **Modello di dispositivo personalizzato**, immettere **Condizionatore connesso** come nome del dispositivo e quindi fare clic su **Crea**. È anche possibile caricare un'immagine del dispositivo che sia visibile agli operatori in Device Explorer:

    ![Dispositivo personalizzato](./media/tutorial-define-device-type-experimental/createcustomdevice.png)
...
4. Nel modello di dispositivo **Condizionatore connesso**, assicurarsi di trovarsi nella scheda **Misure** dove si definiscono i dati di telemetria. Ogni modello di dispositivo che viene definito dispone di schede separate in cui è possibile:
    ...
    * Personalizzare il _dashboard_ del dispositivo per gli operatori.
    ...
    ![Misure del condizionatore](./media/tutorial-define-device-type-experimental/airconmeasurements.png)

    > [!NOTE]
    > Per modificare il nome del modello di dispositivo, fare clic sul nome del modello nella parte superiore della pagina.

5. Per aggiungere la misura di telemetria della temperatura, fare clic su **+ Nuova misura**. Quindi scegliere **Telemetria** come tipo di misura:

    ![Misure del condizionatore connesso](./media/tutorial-define-device-type-experimental/airconmeasurementsnew.png)

6. Ogni tipo di dati di telemetria definito per un modello di dispositivo include [opzioni di configurazione](howto-set-up-template-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json), ad esempio:
    ...
    * Parametri di simulazione.
    ...
    È anche possibile scegliere un colore per la visualizzazione dei dati di telemetria. Per salvare la definizione dei dati di telemetria, fare clic su **Salva**:

    ![Configurare la simulazione di Temperatura](./media/tutorial-define-device-type-experimental/temperaturesimulation.png)
...
7. Dopo un breve periodo di tempo, la scheda **Misure** mostra un grafico dei dati di telemetria sulla temperatura dal condizionatore connesso simulato. Usare i controlli per gestire la visibilità e l'aggregazione o per modificare la definizione dei dati di telemetria:
    ...
    ![Visualizzare la simulazione di temperatura](./media/tutorial-define-device-type-experimental/viewsimulation.png)

8. È anche possibile personalizzare il grafico usando i controlli **Riga**, **In pila**, e **Modifica intervallo di tempo**:

    ![Personalizzare il grafico](./media/tutorial-define-device-type-experimental/customizechart.png)
...
1. Per aggiungere la misura dell'evento **Errore motore ventola**, fare clic su **Nuova misura**. Quindi scegliere **Evento** come tipo di misura:

    ![Misure del condizionatore connesso](./media/tutorial-define-device-type-experimental/eventnew.png)

2. Ogni tipo di evento definito per un modello di dispositivo include [opzioni di configurazione](howto-set-up-template-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json), ad esempio:
   ...
   * Gravità.
    ...
    Per salvare la definizione dell'evento, fare clic su **Salva**:

    ![Configurare la misura Evento](./media/tutorial-define-device-type-experimental/eventconfiguration.png)
...
3. Dopo un breve periodo di tempo, la scheda **Misure** mostra un grafico degli eventi generato casualmente dal condizionatore connesso simulato. Usare i controlli per gestire la visibilità o per modificare la definizione dell'evento:
    ...
    ![Visualizzare una simulazione dell'evento](./media/tutorial-define-device-type-experimental/eventview.png)

1. Per visualizzare altri dettagli sull'evento, fare clic sull'evento nel grafico:

    ![Visualizzare i dettagli evento](./media/tutorial-define-device-type-experimental/eventviewdetail.png)
...
1. Per aggiungere una misura di stato **Modalità ventola**, fare clic su **+ Nuova misura**. Quindi scegliere **Stato** come tipo di misura:

    ![Misure di stato del condizionatore connesso](./media/tutorial-define-device-type-experimental/statenew.png)

2. Ogni tipo di stato definito per un modello di dispositivo include [opzioni di configurazione](howto-set-up-template-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json), ad esempio:
   ...
   * Colore per ogni valore.
    ...
    Per salvare la definizione della misura di stato, fare clic su **Salva**:

    ![Configurare una misura Stato](./media/tutorial-define-device-type-experimental/stateconfiguration.png)
...
3. Dopo un breve periodo di tempo, la scheda **Misure** mostra un grafico degli stati generato casualmente dal condizionatore connesso simulato. Usare i controlli per gestire la visibilità o per modificare la definizione dello stato:
    ...
    ![Visualizzare una simulazione di stato](./media/tutorial-define-device-type-experimental/stateview.png)

4. Se sono presenti troppi punti dati inviati dal dispositivo in un periodo limitato, la misura stato viene visualizzata con un oggetto visivo diverso. Fare clic sul grafico per visualizzare tutti i punti dati all'interno di tale periodo di tempo in ordine cronologico. È anche possibile restringere l'intervallo di tempo per visualizzare la misura più in dettaglio.
...
* Le _proprietà_ sono usate per definire i metadati associati al dispositivo. Sono disponibili due categorie di proprietà:
    ...
    * Le _proprietà dispositivo_ sono usate per permettere a un dispositivo di inviare valori di proprietà all'applicazione. Queste proprietà possono essere modificate solo dal dispositivo. Per un operatore, le proprietà dispositivo sono di sola lettura. In questo scenario relativo a un condizionatore connesso, il numero di serie del dispositivo e la versione del firmware sono proprietà del dispositivo segnalate dal dispositivo.
    ...
    Per altre informazioni, vedere [Proprietà](howto-set-up-template-experimental.md#properties) nella guida pratica alla configurazione di un modello di dispositivo.
...
2. È possibile creare impostazioni di tipi diversi, ad esempio numeri o testo. Fare clic su **Numero** per aggiungere un'impostazione numerica al dispositivo.
...
3. Per configurare l'impostazione **Imposta temperatura**, usare le informazioni nella tabella seguente:
    ...
    Fare quindi clic su **Save** (Salva):

    ![Configurare l'impostazione Imposta temperatura](./media/tutorial-define-device-type-experimental/configuresetting.png)
...
4. È possibile personalizzare il layout della scheda **Impostazioni** spostando e ridimensionando i riquadri delle impostazioni:
    ...
    ![Personalizzare il layout delle impostazioni](./media/tutorial-define-device-type-experimental/settingslayout.png)
...
1. È possibile creare proprietà del dispositivo di tipi diversi, ad esempio numeri o testo. Per aggiungere una proprietà di posizione al modello di dispositivo, scegliere **Posizione**. Per configurare la proprietà della posizione, usare le informazioni nella tabella seguente:
    ...
    ![Configurare le proprietà del dispositivo](./media/tutorial-define-device-type-experimental/configureproperties.png)
...
1. Per configurare la proprietà relativa alla data dell'ultimo utilizzo, usare le informazioni nella tabella seguente:
    ...
    ![Configurare le proprietà del dispositivo](./media/tutorial-define-device-type-experimental/configureproperties2.png)
...
1. Per configurare la versione del firmware, usare le informazioni nella tabella seguente:
    ...
    ![Configurare la versione del firmware](./media/tutorial-define-device-type-experimental/configureproperties3.png)
...
1. Per configurare il numero di serie, usare le informazioni nella tabella seguente:
    ...
    ![Configurare il numero di serie](./media/tutorial-define-device-type-experimental/configureproperties4.png)
...
1. Fare clic su **+ Nuovo comando** per aggiungere un comando al dispositivo e iniziare a configurare il nuovo comando.
...
1. Per configurare il nuovo comando, usare le informazioni nella tabella seguente:
    ...
    ![Prepararsi per aggiungere un'impostazione](./media/tutorial-define-device-type-experimental/commandsecho1.png)
...
1. Configurare il componente **Grafico a linee** usando le informazioni nella tabella seguente:
    ...
    | Misure     | Temperatura (fare clic su **Visibilità** accanto a **Temperatura**) |

    ![Impostazioni Grafico a linee](./media/tutorial-define-device-type-experimental/linechartsettings.png)

    Fare quindi clic su **Salva**.

1. Fare clic sul componente **Cronologia eventi** usando le informazioni nella tabella seguente:

    | Impostazione      | Valore       |
    | ------------ | ----------- |
    | Title        | Eventi motore ventola |
    | Intervallo di tempo   | Ultimi 30 minuti |
    | Misure     | Errore motore ventola (fare clic su **Visibilità** accanto a **Errore motore ventola**) |

    ![Impostazioni Grafico a linee](./media/tutorial-define-device-type-experimental/dashboardeventchartsetting.png)
...
1. Configurare il componente **Cronologia dello stato** usando le informazioni nella tabella seguente:
    ...
    | Intervallo di tempo   | Ultimi 30 minuti |
    | Misure | Modalità ventola (fare clic su **Visibilità** accanto a **Modalità ventola**) |

    ![Impostazioni Grafico a linee](./media/tutorial-define-device-type-experimental/dashboardstatechartsetting.png)

    Fare quindi clic su **Salva**.

1. Per aggiungere le impostazioni e le proprietà del dispositivo al dashboard, scegliere **Impostazioni e proprietà**. Fare clic su **Aggiungi/Rimuovi** per aggiungere le impostazioni o le proprietà che si vuole visualizzare nel dashboard.
...
1. Configurare il componente **Impostazioni e proprietà** usando le informazioni nella tabella seguente:
    ...
    ![Configurare le impostazioni delle proprietà relative alla temperatura](./media/tutorial-define-device-type-experimental/propertysettings4.png)
...
1. È ora possibile vedere i dati simulati per il Condizionatore connesso nel dashboard. È possibile modificare i riquadri e il layout per il dashboard:
    ...
    ![Visualizzare il dashboard](./media/tutorial-define-device-type-experimental/dashboard.png)
