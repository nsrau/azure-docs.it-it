...
3. Immettere un nome per il nuovo modello di dispositivo e selezionare **Create** (Crea).
...
Per aggiungere una nuova misura di telemetria, selezionare il pulsante **+ Nuova misura**. Selezionare **Telemetry** (Telemetria) come tipo di misura e immettere i dettagli nel modulo **Create Telemetry** (Crea dati di telemetria).

> [!NOTE]
> Quando si connette un dispositivo reale, prestare attenzione al nome della misura segnalata dal dispositivo. Il nome deve corrispondere esattamente al valore di **Field Name** (Nome campo) della misura.
...
Dopo aver selezionato **Salva**, la misura **Temperature** (Temperatura) viene visualizzata nell'elenco delle misure. Un operatore può vedere la visualizzazione dei dati di temperatura che il dispositivo sta raccogliendo.

![Grafico della misura](./media/howto-set-up-template/measurementsgraph.png)

### <a name="create-an-event-measurement"></a>Creare una misura di evento
Per aggiungere una nuova misura di evento, selezionare il pulsante **+ Nuova misura**. Selezionare **Event** (Evento) come tipo di misura e immettere i dettagli nel modulo **Create Event** (Crea evento).
...
Dopo aver selezionato **Salva**, la misura **Fan Motor Error** (Errore motore ventola) viene visualizzata nell'elenco delle misure. Un operatore può vedere la visualizzazione dei dati dell'evento che il dispositivo sta inviando.
...
Per aggiungere una nuova misura di stato, selezionare il pulsante **+ Nuova misura**. Selezionare **State** (Stato) come tipo di misura e immettere i dettagli nel modulo **Create State** (Crea stato).
...
Dopo aver selezionato **Salva**, la misura **Fan Mode** (Modalità ventola) viene visualizzata nell'elenco delle misure. L'operatore può vedere la visualizzazione dei dati di stato che il dispositivo sta inviando.
...
![Dettagli per la misura di stato "Static Fan Mode" (Modalità ventola statica)](./media/howto-set-up-template/statemeasurementsdetail.png)
...
> [!NOTE]
> Quando si connette un dispositivo reale, prestare attenzione al nome dell'impostazione segnalata dal dispositivo. Il nome deve corrispondere esattamente al valore di **Field Name** (Nome campo) per un'impostazione.
...
Dopo aver creato un riquadro, provare la nuova impostazione. In primo luogo disattivare la modalità progettazione nella parte superiore destra della schermata.
...
Le proprietà sono i metadati associati al dispositivo, ad esempio posizione del dispositivo e numero di serie. È possibile aggiungere più proprietà al modello di dispositivo, che verranno visualizzate come riquadri nella scheda **Properties** (Proprietà). Un operatore può specificare i valori delle proprietà quando crea un dispositivo e modificare questi valori in qualsiasi momento. Esistono sei tipi di proprietà che è possibile aggiungere: number (numero), text (testo), date (data), toggle (attivazione/disattivazione), device property (proprietà dispositivo) e label (etichetta).

Sono disponibili due categorie di proprietà:

- Proprietà **del dispositivo** che il dispositivo segnala.
- Proprietà **dell'applicazione** che sono archiviate esclusivamente nell'applicazione. Il dispositivo non riconosce le proprietà dell'applicazione.

> [!NOTE]
> Per le proprietà del dispositivo, quando si connette un dispositivo reale, prestare attenzione al nome della proprietà segnalata dal dispositivo. Il nome deve corrispondere esattamente al valore di **Field Name** (Nome campo) per una proprietà. Per le proprietà dell'applicazione, il nome del campo può essere un nome qualsiasi, purché sia univoco nel modello di dispositivo.
...
Dopo aver creato un riquadro, è possibile modificare il valore della proprietà. In primo luogo, disattivare la modalità progettazione nella parte superiore destra della schermata.
...
- **Location as a Device property** (Posizione come proprietà del dispositivo) segnalata dal dispositivo.
...
1. Nella scheda **Properties** (Proprietà) verificare che **Design Mode** (Modalità progettazione) sia **attivato**.
...
4. Selezionare **Salva**. 
   ...
   ![Proprietà di posizione con l'indirizzo di installazione aggiunto](./media/howto-set-up-template/locationcloudproperty3.png)
...
Ora che è stata configurata la proprietà di posizione, sarà possibile [aggiungere una mappa per visualizzare la posizione nel dashboard del dispositivo](#add-an-azure-maps-location-in-the-dashboard).
...
Quando si esegue un comando, può essere in uno fra tre stati, a seconda del fatto che il dispositivo abbia ricevuto il comando o meno. 

Ad esempio, è possibile aggiungere un nuovo comando **Echo**:

![Modulo "Configure Command" (Configura comando) con i dettagli per Echo](./media/howto-set-up-template/commandsecho.png)

Dopo aver selezionato **Salva**, il comando **Echo** (Eco) viene visualizzato come riquadro ed è pronto a essere usato per eseguire l'eco del dispositivo.

Dopo aver creato un riquadro, provare il nuovo comando.
...
1. Nella scheda **Dashboard** verificare che **Design Mode** (Modalità progettazione) sia **attivato**.
...
3. Assegnare un titolo e scegliere la proprietà di posizione configurata in precedenza come proprietà del dispositivo.
...
Ora, quando un operatore visualizza il dashboard, può vedere tutti i riquadri del dashboard che sono stati configurati, inclusa la mappa della posizione.

![Riquadri sul dashboard](./media/howto-set-up-template/locationcloudproperty7map.png) 
