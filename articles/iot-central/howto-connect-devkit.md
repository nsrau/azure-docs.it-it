...
1. Aggiungere un **dispositivo reale** da Device Explorer, selezionare **+ nuovo > reale** per aggiungere un dispositivo reale.
    ...
    * Immettere un **nome di dispositivo**o usare il nome suggerito
    ...
    ![Aggiungi dispositivo](media/howto-connect-devkit/add-device.png)
...
1. Per ottenere i dettagli di connessione del dispositivo, **ID ambito**, **ID dispositivo**e **chiave primaria**, selezionare **Connetti** nella pagina dispositivo.
    ...
    ![Dettagli connessione](media/howto-connect-devkit/device-connect.png)
...
Utilizzare Visual Studio Code per aprire la `MXCHIP/mxchip_advanced` cartella `iot-central-firmware` nella cartella:

![Visual Studio Code](media/howto-connect-devkit/vscodeview.png)

Per vedere come vengono inviati i dati di telemetria all'applicazione IoT Central di Azure, aprire il file di telemetria **. cpp** nella `src` cartella:
...
- La funzione `TelemetryController::sendTelemetryPayload` chiama `sendTelemetry` in **AzureIOTClient. cpp** per inviare il payload JSON all'hub Internet usato dall'applicazione IoT Central di Azure.

Per vedere come vengono segnalati i valori delle proprietà all'applicazione IoT Central di Azure, aprire il file di telemetria **. cpp** nella `src` cartella:

- La funzione `TelemetryController::loop` Invia la proprietà segnalata approssimativamente ogni 30 secondi. Usa la `sendReportedProperty` funzione nel file di origine **AzureIOTClient. cpp** .

- La funzione `TelemetryController::loop` Invia la proprietà segnalata **dieNumber** quando l'accelerometro del dispositivo rileva un doppio tocco. Usa la `sendReportedProperty` funzione nel file di origine **AzureIOTClient. cpp** .

Per visualizzare il modo in cui il dispositivo risponde ai comandi chiamati dall'applicazione IoT Central, aprire il file **registeredMethodHandlers. cpp** nella `src` cartella:
...
| Name          | `Display name`   | NORMALE | ATTENZIONE | PERICOLO | 
| ------------- | -------------- | ------ | ------- | ------ | 
...
| `Display name` | Nome campo | Tipo restituito | Nome visualizzato del campo di input | Nome campo di input | Tipo di campo di input |
| ------------ | ---------- | ----------- | ------------------------ | ---------------- | ---------------- |
