...
Usare il cmdlet `Get-AzPrivateLinkService` per ottenere le connessioni all'endpoint privato e i relativi stati.  
```azurepowershell
...
Usare il cmdlet `Approve-AzPrivateEndpointConnection` per approvare una connessione all'endpoint privato. 
...
Usare il cmdlet `Deny-AzPrivateEndpointConnection` per rifiutare una connessione all'endpoint privato. 
```azurepowershell
Deny-AzPrivateEndpointConnection -Name myPrivateEndpointConnection -ResourceGroupName myResourceGroup -ServiceName myPrivateLinkService 
```
#### <a name="remove-private-endpoint-connection"></a>Rimuovere la connessione all'endpoint privato 
 
Usare il cmdlet `Remove-AzPrivateEndpointConnection` per rimuovere una connessione all'endpoint privato. 
```azurepowershell
...
Usare `az network private-link-service update` per gestire le connessioni agli endpoint privati. Lo stato della connessione è specificato nel parametro ```azurecli connection-status```. 
```azurecli
