...
3. Dalla shell della riga di comando di PowerShell passare alla cartella che contiene lo script scaricato ed eseguirlo modificando i valori per i parametri *-AutomationAccountName*, *-AAResourceGroupName*, *-OMSResourceGroupName*, *-HybridGroupName*, *-SubscriptionId* e *-WorkspaceName*.
...
Microsoft Monitoring Agent connette i computer a Operations Management Suite. Quando si installa l'agente nel computer locale e lo si connette all'area di lavoro, viene eseguito il download dei componenti necessari per il ruolo di lavoro ibrido per runbook.
...
Per importare il modulo, aprire una sessione di PowerShell in modalità amministratore ed eseguire i comandi seguenti:
...
È possibile ottenere le informazioni necessarie per questo cmdlet facendo clic sull'opzione **Chiavi** in **Impostazioni account** nell'account di automazione.
...
* **EndPoint** è il valore del campo **URL** della pagina **Chiavi**.
* **Token** è il valore **Chiave di accesso primaria** nella pagina **Chiavi**.
...
1. In **Automazione processi** selezionare **Gruppi di lavoro ibridi**. Selezionare il gruppo da eliminare. Dopo aver selezionato il gruppo specifico, viene visualizzato il pannello delle proprietà del **Gruppo di lavoro ibrido**.

   ![Pannello del gruppo di ruoli di lavoro ibridi per runbook](media/automation-hybrid-runbook-worker/automation-hybrid-runbook-worker-group-properties.png)

1. Nel pannello delle proprietà del gruppo selezionato fare clic su **Elimina**. Viene visualizzato un messaggio che chiede di confermare l'azione. Selezionare **Sì** se si è certi che si vuole procedere.
