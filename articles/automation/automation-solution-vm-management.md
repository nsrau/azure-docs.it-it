<properties
    pageTitle="Soluzione Avvio/Arresto di macchine virtuali durante gli orari di minore attività (anteprima) | Microsoft Azure"
...
Runbook | Descrizione|
--------|------------|
CleanSolution-MS-Mgmt-VM | Quando si elimina la soluzione dalla sottoscrizione, questo runbook rimuove tutte le risorse e le pianificazioni contenute.|  
...
---------|------------|
StartByResourceGroup-Schedule-MS-Mgmt | Permette di pianificare il runbook StartByResourceGroup.|
StopByResourceGroup-Schedule-MS-Mgmt | Permette di pianificare il runbook StopByResourceGroup.|
...
1. Nella schermata home del portale di Azure selezionare il riquadro **Marketplace**. Se il riquadro non è aggiunto alla schermata home, selezionare **Nuovo** nel riquadro di spostamento a sinistra.
...
5.  Nel pannello **Aggiungi soluzione** selezionare **Area di lavoro**. Qui è possibile selezionare un'area di lavoro OMS collegata alla stessa sottoscrizione di Azure in cui si trova l'account di Automazione o creare una nuova area di lavoro OMS. Se non è disponibile un'area di lavoro OMS, è possibile selezionare **Crea nuova area di lavoro** e seguire questa procedura nel pannello **Area di lavoro OMS**:
   ...
   - Per il **gruppo di risorse**, è possibile e selezionare un gruppo di risorse esistente o crearne uno nuovo.
   - Selezionare una **località**. Le uniche località attualmente selezionabili sono l'**Australia sud-orientale**, gli **Stati Uniti orientali**, l'**Asia sud-orientale** e l'**Europa occidentale**.
...
8. Infine, nel pannello **Aggiungi soluzione** selezionare **Configurazione**. Verrà visualizzato il pannello **Parametri**. Nel pannello **Parametri** viene richiesto di:
   ...
   - Selezionare una **pianificazione**, ovvero una data e ora ricorrenti per l'avvio e l'arresto delle macchine virtuali nei gruppi di risorse di destinazione.
...
Nell'account di Automazione è possibile accedere alla soluzione e gestirla selezionando il riquadro **Soluzioni** e, nel pannello **Soluzioni**, scegliendo quindi la soluzione **Start-Stop-VM[Workspace]** (Avvio-Arresto-VM - Area di lavoro) dall'elenco.<br><br> ![Elenco delle soluzioni di automazione](media/automation-solution-vm-management/vm-management-solution-autoaccount-solution-list.png)

Verrà visualizzato il pannello della soluzione **Start-Stop-VM[Workspace]** (Avvio-Arresto-VM - Area di lavoro), in cui è possibile rivedere informazioni importanti come il riquadro **StartStopVM** (Avvio e arresto di VM) nell'area di lavoro OMS, che permette di visualizzare un conteggio e una rappresentazione grafica dei processi di runbook della soluzione che sono stati avviati e completati.<br><br> ![Pannello delle soluzioni di automazione delle macchine virtuali](media/automation-solution-vm-management/vm-management-solution-solution-blade.png)

### Configurazione delle notifiche tramite posta elettronica
...
2. Nel pannello **Impostazioni** nella sezione **Risorse di automazione** selezionare **Asset**.
...
Query | Descrizione|
----------|----------|
Trova i processi completati per il runbook StartVM | Category=JobLogs RunbookName\_s="StartByResourceGroup-MS-Mgmt-VM" ResultType=Succeeded | measure count() by JobId\_g|
Trova i processi completati per il runbook StopVM | Category=JobLogs RunbookName\_s="StartByResourceGroup-MS-Mgmt-VM" ResultType=Failed | measure count() by JobId\_g
Visualizza lo stato dei processi nel tempo per i runbook StartVM e StopVM | Category=JobLogs RunbookName\_s="StartByResourceGroup-MS-Mgmt-VM" OR "StopByResourceGroup-MS-Mgmt-VM" NOT(ResultType="started") | measure Count() by ResultType interval 1day|
