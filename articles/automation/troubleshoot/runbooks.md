...
ms.locfileid: "82855102"
---
# <a name="troubleshoot-runbook-errors"></a>Risolvere gli errori di Runbook

 Questo articolo descrive vari errori di Runbook che possono verificarsi e come risolverli.
...
## <a name="diagnose-runbook-issues"></a>Diagnosticare i problemi di Runbook
...
1. Assicurarsi che lo script Runbook venga eseguito correttamente nel computer locale.
    ...
    Per i moduli di formazione e di riferimento per il linguaggio, vedere la [documentazione di PowerShell](/powershell/scripting/overview) o la [documentazione di Python](https://docs.python.org/3/). Eseguendo lo script in locale è possibile individuare e risolvere gli errori comuni, ad esempio:

      * Moduli mancanti
      * Errori di sintassi
      * Errori di logica

1. Esaminare i [flussi di errore](../automation-runbook-output-and-messages.md#runbook-output)Runbook.

    Esaminare questi flussi per i messaggi specifici e confrontarli con gli errori documentati in questo articolo.

1. Verificare che i nodi e l'area di lavoro di automazione dispongano dei moduli richiesti.

    Se il Runbook importa i moduli, verificare che siano disponibili per l'account di automazione usando la procedura descritta in [importare i moduli](../shared-resources/modules.md#import-modules). Aggiornare i moduli di PowerShell alla versione più recente seguendo le istruzioni riportate in [Update Azure PowerShell Modules in automazione di Azure](../automation-update-azure-modules.md). Per ulteriori informazioni sulla risoluzione dei problemi, vedere [risoluzione dei problemi](shared-resources.md#modules)relativi ai moduli.

1. Se il Runbook è sospeso o non riesce in modo imprevisto:
    ...
    * [Controllare gli Stati dei processi](../automation-runbook-execution.md#job-statuses) per determinare gli Stati Runbook correnti e alcune possibili cause del problema.
    * [Aggiungere un output aggiuntivo](../automation-runbook-output-and-messages.md#message-streams) a Runbook per identificare cosa accade prima della sospensione del Runbook.
    * [Gestire tutte le eccezioni](../automation-runbook-execution.md#exceptions) generate dal processo.

1. Eseguire questo passaggio se il processo Runbook o l'ambiente in Hybrid Runbook Worker non risponde.

    Se si esegue il manuali operativi in un ruolo di lavoro ibrido per Runbook anziché in automazione di Azure, potrebbe essere necessario [risolvere i problemi del ruolo di lavoro ibrido](https://docs.microsoft.com/azure/automation/troubleshoot/hybrid-runbook-worker).

## <a name="scenario-runbook-fails-with-a-no-permission-or-forbidden-403-error"></a><a name="runbook-fails-no-permission"></a>Scenario: Runbook ha esito negativo senza autorizzazione o errore 403 non consentito

### <a name="issue"></a>Problema

Il Runbook ha esito negativo senza autorizzazione o errore 403 vietato o equivalente.

### <a name="cause"></a>Causa

Gli account RunAs potrebbero non avere le stesse autorizzazioni per le risorse di Azure dell'account di automazione corrente. 

### <a name="resolution"></a>Soluzione

Verificare che l'account RunAs disponga [delle autorizzazioni per accedere a tutte le risorse](https://docs.microsoft.com/azure/role-based-access-control/role-assignments-portal) utilizzate nello script.
...
Questi errori si verificano se il nome dell'asset delle credenziali non è valido. Potrebbero anche verificarsi se il nome utente e la password usati per configurare l'asset delle credenziali di automazione non sono validi.

### <a name="resolution"></a>Soluzione

Per determinare il problema, attenersi alla seguente procedura:
...
1. Verificare se è possibile usare il nome utente e la password archiviati nelle credenziali di automazione di Azure nell'editor di PowerShell ISE locale. Eseguire i cmdlet seguenti in PowerShell ISE.
...
1. Se l'autenticazione non riesce in locale, non sono state configurate correttamente le credenziali di Azure Active Directory (Azure AD). Per configurare correttamente l'account Azure AD, vedere il post di Blog relativo all' [autenticazione in Azure con Azure Active Directory](https://azure.microsoft.com/blog/azure-automation-authenticating-to-azure-using-azure-active-directory/).

1. Se l'errore sembra essere temporaneo, provare ad aggiungere la logica di ripetizione dei tentativi alla routine di autenticazione per rendere l'autenticazione più affidabile.
...
Questo errore può verificarsi quando non si utilizza un account RunAs o l'account RunAs è scaduto. Per altre informazioni, vedere [gestire gli account RunAs di automazione di Azure](https://docs.microsoft.com/azure/automation/manage-runas-account).
...
Se si sta tentando di accedere alle risorse in un'altra sottoscrizione, attenersi alla procedura seguente per configurare le autorizzazioni:
...
* Il Azure AD utente che sta tentando di ottenere i dettagli della sottoscrizione non è configurato come amministratore della sottoscrizione.
* Il cmdlet non è disponibile.

### <a name="resolution"></a>Soluzione

Seguire questa procedura per determinare se è stata eseguita l'autenticazione in Azure e si ha accesso alla sottoscrizione che si sta tentando di selezionare:

1. Per assicurarsi che lo script funzioni autonomo, testarlo all'esterno di automazione di Azure.
1. Assicurarsi che lo script esegua il cmdlet [Connect-AzAccount](https://docs.microsoft.com/powershell/module/Az.Accounts/Connect-AzAccount?view=azps-3.7.0) prima di eseguire `Select-*` il cmdlet.
...
1. Se il messaggio di errore viene ancora visualizzato, modificare il codice aggiungendo il `AzContext` parametro per `Connect-AzAccount`, quindi eseguire il codice.
...
Viene visualizzato l'errore seguente quando si esegue l'autenticazione in Azure con il nome utente e la password di Azure:
...
Se si dispone dell'autenticazione a più fattori per l'account Azure, non è possibile usare un utente Azure Active Directory per l'autenticazione in Azure. È invece necessario usare un certificato o un'entità servizio per l'autenticazione.
...
## <a name="scenario-runbook-fails-with-a-task-was-canceled-error-message"></a><a name="task-was-cancelled"></a>Scenario: Runbook ha esito negativo con il messaggio di errore "un'attività è stata annullata"
...
1. Nell'account di automazione selezionare **moduli**, quindi selezionare **Aggiorna moduli di Azure**.
1. L'aggiornamento richiede circa 15 minuti. Al termine, eseguire di nuovo il Runbook che ha avuto esito negativo.
...
## <a name="scenario-term-not-recognized-as-the-name-of-a-cmdlet-function-or-script"></a><a name="not-recognized-as-cmdlet"></a>Scenario: termine non riconosciuto come nome di un cmdlet, di una funzione o di uno script
...
* Il modulo che contiene il cmdlet viene importato ma non aggiornato.
...
* Per un modulo non Azure, verificare che il modulo sia stato importato nell'account di automazione.

## <a name="scenario-cmdlet-fails-in-pnp-powershell-runbook-on-azure-automation"></a>Scenario: il cmdlet non riesce in Runbook di PowerShell PnP in automazione di Azure
...
Se il flusso contiene oggetti, `Start-AzAutomationRunbook` il flusso di output non viene gestito correttamente.

### <a name="resolution"></a>Soluzione

Implementare una logica di polling e usare il cmdlet [Get-AzAutomationJobOutput](https://docs.microsoft.com/powershell/module/Az.Automation/Get-AzAutomationJobOutput?view=azps-3.7.0) per recuperare l'output. Un esempio di questa logica è definito qui:
...
## <a name="scenario-400-bad-request-status-when-calling-a-webhook"></a><a name="expired webhook"></a>Scenario: 400 stato di richiesta non valida quando si chiama un webhook
...
Se il webhook è disabilitato, è possibile riabilitarlo tramite il portale di Azure. Se il webhook è scaduto, è necessario eliminarlo e ricrearlo. Il [rinnovo di un webhook](../automation-webhooks.md#renew-a-webhook) è possibile solo se il webhook non è scaduto. 
...
## <a name="scenario-runbook-job-fails-because-allocated-quota-was-exceeded"></a><a name="quota-exceeded"></a>Scenario: il processo Runbook non riesce perché è stata superata la quota allocata
...
1. Selezionare **Impostazioni**e quindi selezionare **prezzi**.
1. Selezionare **Abilita** nella parte inferiore della pagina per aggiornare l'account al livello Basic.

## <a name="scenario-runbook-job-start-attempted-three-times-but-fails-to-start-each-time"></a><a name="job-attempted-3-times"></a>Scenario: l'avvio del processo Runbook è stato eseguito tre volte, ma non viene avviato ogni volta
...
* **Socket di rete.** Azure sandbox è limitato a 1.000 socket di rete simultanei. Per altre informazioni, vedere [limiti del servizio di automazione](../../azure-resource-manager/management/azure-subscription-service-limits.md#automation-limits).
* **Il modulo non è compatibile.** Le dipendenze del modulo potrebbero non essere corrette. In questo caso, il Runbook restituisce in genere `Command not found` un `Cannot bind parameter` messaggio o.
...
* **Il modulo non è compatibile.** Aggiornare i moduli di Azure seguendo i passaggi descritti in [come aggiornare i moduli Azure PowerShell in automazione di Azure](../automation-update-azure-modules.md).
* **Nessuna autenticazione con Active Directory per sandbox.** Quando si esegue l'autenticazione a Azure AD con un Runbook, assicurarsi che il modulo di Azure AD sia disponibile nell'account di automazione. Assicurarsi di concedere all'account RunAs le autorizzazioni necessarie per eseguire le attività automatizzate da Runbook.
...
* **Troppa quantità di dati di eccezione.** Il flusso di output del processo prevede un limite di 1 MB. Assicurarsi che Runbook racchiuda le chiamate a un eseguibile o a un sottoprocesso `try` usando `catch` i blocchi e. Se le operazioni generano un'eccezione, fare in modo che il codice scriva il messaggio dall'eccezione in una variabile di automazione. Questa tecnica impedisce che il messaggio venga scritto nel flusso di output del processo.

## <a name="scenario-powershell-job-fails-with-cannot-invoke-method-error-message"></a><a name="cannot-invoke-method"></a>Scenario: il processo di PowerShell non riesce con il messaggio di errore "Impossibile richiamare il metodo"
...
Questo errore potrebbe indicare che manuali operativi eseguiti in una sandbox di Azure non possono essere eseguiti in [modalità Full Language](/powershell/module/microsoft.powershell.core/about/about_language_modes).
...
Per altre informazioni su questo comportamento e altri comportamenti dei manuali operativi di automazione di Azure, vedere [esecuzione di Runbook in automazione di Azure](../automation-runbook-execution.md).
...
Il Runbook viene visualizzato in uno stato interrotto dopo l'esecuzione per tre ore. È anche possibile che venga visualizzato questo errore:
...
## <a name="scenario-error-in-job-streams-about-the-get_serializationsettings-method"></a><a name="get-serializationsettings"></a>Scenario: errore nei flussi del processo relativi al metodo get_SerializationSettings
...
Questo errore è probabilmente causato dall'uso di una migrazione incompleta da AzureRM a AZ Modules in Runbook. Questa situazione può far sì che automazione di Azure avvii un processo Runbook usando solo moduli AzureRM e quindi avviare un altro processo usando solo AZ Modules, che causa un arresto anomalo della sandbox.

### <a name="resolution"></a>Soluzione

Non è consigliabile usare i cmdlet AZ e AzureRM nella stessa Runbook. Per ulteriori informazioni sull'utilizzo corretto di questi moduli, vedere la pagina relativa alla [migrazione a AZ Modules](../shared-resources/modules.md#migrating-to-az-modules).

## <a name="scenario-access-denied-when-using-azure-sandbox-for-runbook-or-application"></a><a name="access-denied-azure-sandbox"></a>Scenario: accesso negato quando si usa sandbox di Azure per Runbook o l'applicazione

### <a name="issue"></a>Problema

Quando il Runbook o l'applicazione tenta l'esecuzione in una sandbox di Azure, l'ambiente nega l'accesso.

### <a name="cause"></a>Causa

Questo problema può verificarsi perché sandbox di Azure impedisce l'accesso a tutti i server COM out-of-process. Ad esempio, un'applicazione o un Runbook in modalità sandbox non può chiamare Strumentazione gestione Windows (WMI) o nel servizio di Windows Installer (MSIServer. exe). 

### <a name="resolution"></a>Soluzione

Per informazioni dettagliate sull'uso di sandbox di Azure, vedere [ambiente di esecuzione Runbook](../automation-runbook-execution.md#runbook-execution-environment).

## <a name="scenario-invalid-forbidden-status-code-when-using-key-vault-inside-a-runbook"></a>Scenario: codice di stato non consentito valido quando si usa Key Vault all'interno di un Runbook

### <a name="issue"></a>Problema

Quando si prova ad accedere a Azure Key Vault tramite un Runbook di automazione di Azure, viene ricevuto l'errore seguente:
...
#### <a name="not-using-a-run-as-account"></a>Non usare un account RunAs

Seguire il [passaggio 5: aggiungere l'autenticazione per gestire le risorse di Azure](https://docs.microsoft.com/azure/automation/automation-first-runbook-textual-powershell#add-authentication-to-manage-azure-resources) per assicurarsi di usare un account RunAs per accedere Key Vault.

#### <a name="insufficient-permissions"></a>Autorizzazioni insufficienti

[Aggiungere le autorizzazioni per Key Vault](https://docs.microsoft.com/azure/automation/manage-runas-account#add-permissions-to-key-vault) per assicurarsi che l'account RunAs disponga di autorizzazioni sufficienti per accedere a Key Vault.
...
Se il problema non è stato riscontrato o non si è in grado di risolvere il problema, provare a usare uno dei canali seguenti per ottenere ulteriore supporto:

* Ottieni risposte dagli esperti di Azure tramite i [Forum di Azure](https://azure.microsoft.com/support/forums/).
* Connettersi con [@AzureSupport](https://twitter.com/azuresupport), l'account ufficiale Microsoft Azure per migliorare l'esperienza del cliente. Il supporto tecnico di Azure consente di connettersi alla community di Azure per risposte, supporto ed esperti.
* Se è necessaria un'assistenza maggiore, è possibile inviare una richiesta al supporto tecnico di Azure. Accedere al [sito del supporto tecnico di Azure](https://azure.microsoft.com/support/options/)e selezionare **ottenere supporto**.
