---
title: Uso di tag in formato JSON per creare una pianificazione per l'avvio e l'arresto di una macchina virtuale di Azure | Microsoft Docs
...
1. I tag vengono definiti in base a una coppia chiave/valore. Digitare **Pianificazione** nel campo **Chiave** e quindi incollare la stringa JSON nel campo **Valore**. Fare clic su **Salva**. Il nuovo tag verrà visualizzato nell'elenco dei tag per la risorsa.
...
### Creare un tag di pianificazione con PowerShell
...
* Per altre informazioni su un account RunAs di Azure e come usarlo per eseguire l'autenticazione dei runbook, vedere [Autenticare runbook con l'account RunAs di Azure](automation-sec-configure-azure-runas-account.md).
