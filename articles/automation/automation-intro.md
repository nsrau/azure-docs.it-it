<properties
	pageTitle="Informazioni su Automazione di Azure | Microsoft Azure"
	description="Informazioni sui vantaggi offerti da Automazione di Azure e risposte alle domande più comuni sul modo in cui è possibile iniziare a creare e utilizzare i Runbook e Azure Automation DSC."
	services="automation"
	documentationCenter=""
	authors="bwren"
	manager="stevenka"
...
## Creazione di Runbook con Automazione di Azure 

È possibile [creare Runbook personalizzati](http://msdn.microsoft.com/library/azure/dn643637.aspx) completamente nuovi o modificare i Runbook contenuti nella relativa [Raccolta runbook](http://msdn.microsoft.com/library/azure/dn781422.aspx) in base alle proprie esigenze. Sono disponibili tre diversi [tipi runbook](automation-runbook-types.md) tra cui è possibile scegliere in base alle esigenze e all’utilizzo fatto di PowerShell. Se si preferisce lavorare direttamente con il codice di PowerShell, allora si può utilizzare un [Runbook di PowerShell](automation-runbook-types.md#powershell-runbooks) o [Runbook del flusso di lavoro di PowerShell](automation-runbook-types.md#powershell-workflow-runbooks) che si modifica in modalità offline o [utilizzando l'editor di testo](http://msdn.microsoft.com/library/azure/dn879137.aspx) nel portale di Azure. Se si preferisce modificare un Runbook evitando l'interazione con il codice sottostante, allora si può creare un[Runbook di interfaccia grafica](automation-runbook-types.md#graphical-runbooks) utilizzando l’[editor grafico](automation-graphical-authoring-intro.md) nel portale di anteprima di Azure.


## Automazione della gestione della configurazione con Configurazione dello stato desiderato 

[DSC (Desired State Configuration) di PowerShell](https://technet.microsoft.com/library/dn249912.aspx) è una nuova piattaforma di gestione, che consente di gestire, distribuire e potenziare la configurazione degli host fisici e delle macchine virtuali mediante una sintassi dichiarativa di PowerShell. È possibile definire configurazioni su un server centrale DSC Pull che i computer di destinazione possono recuperare e applicare automaticamente. DSC fornisce un insieme di cmdlet PowerShell che è possibile utilizzare per gestire le configurazioni e le risorse.

[Automation DSC di Azure](automation-dsc-overview.md) è una soluzione basata su cloud per DSC PowerShell che fornisce i servizi necessari per gli ambienti aziendali. È possibile gestire le risorse DSC in Automazione di Azure e applicare le configurazioni alle macchine virtuali o fisiche che le recuperano da un Server Pull DSC nel cloud di Azure. Fornisce inoltre servizi di creazione report che informano di eventi importanti, ad esempio quando i nodi hanno deviato dalla configurazione assegnata e quando una nuova configurazione è stata applicata.


## Creazione di configurazioni personalizzate DSC con Automazione di Azure

Le [Configurazioni DSC](automation-dsc-overview.md#azure-automation-dsc-terms) specificano lo stato desiderato di un nodo. Più nodi possono applicare la stessa configurazione per garantire che essi mantengano uno stato identico. È possibile creare una configurazione utilizzando qualsiasi editor di testo sul computer locale e quindi importarlo in Automazione di Azure dove è possibile compilarlo e applicarvi nodi.


## Recupero di moduli e configurazioni 

È possibile ottenere [moduli PowerShell](automation-runbook-gallery.md#modules-in-powershell-gallery) contenenti i cmdlet che è possibile utilizzare nei runbook e nelle configurazioni DSC dalla [PowerShell Gallery ](http://www.powershellgallery.com/). È possibile avviare questa raccolta dal portale di anteprima di Azure e importare i moduli direttamente in Automazione di Azure oppure è possibile scaricarli e importarli manualmente. Non è possibile installare i moduli direttamente dal portale di Azure, ma è possibile scaricarli e installarli come qualsiasi altro modulo.
...
* La sezione dedicata a **Libreria di automazione di Azure** è il punto in cui ci si trova in questo momento. Gli articoli inclusi in questa sezione forniscono la documentazione completa sulla configurazione e l'amministrazione di Automazione di Azure e sulla creazione di Runbook personalizzati. 
...
* [Gestione Blog](https://azure.microsoft.com/it-IT/blog/tag/azure-automation/) fornisce le ultime informazioni sull'automazione di Azure e le altre tecnologie di gestione di Microsoft. Effettuare la sottoscrizione a questo blog per essere aggiornati sulle ultime novità del team di Automazione di Azure. 
...
* In [Cmdlet di Automazione di Azure](https://msdn.microsoft.com/library/mt244122.aspx) vengono fornite informazioni per l'automazione delle attività di amministrazione. Contiene i cmdlet per gestire account di automazione, asset, runbook, DSC.
