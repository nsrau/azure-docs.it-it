...
1.	Il runbook è attualmente vuoto, digitare *Write-Output "Hello World".*.  
...
1.	Fare clic su **Riquadro di test** per aprire il riquadro corrispondente.  
...
4.	Al termine del processo del runbook, viene visualizzato l'output. In questo caso, verrà visualizzato *Hello World*  
	![Output del Riquadro test](media/automation-first-runbook-textual-powershell/automation-testpane-output.png)  
...
1.	Fare clic su**Pubblica** per pubblicare il runbook e quindi su **Sì** quando richiesto.  
	![Pulsante Publish](media/automation-first-runbook-textual-powershell/automation-publish-button.png)  
...
3.	Scorrere verso destra per visualizzare il riquadro **MyFirstRunbook-PowerShell**.  
	Le opzioni nella parte superiore consentono di avviare il runbook, pianificarlo per l'avvio in un momento futuro o creare un [webhook](automation-webhooks.md) per poterlo avviare con una chiamata HTTP.
4.	Si vuole solo avviare il runbook, quindi fare clic su **Avvia** e poi fare clic su **OK** quando si apre il pannello Avvia runbook.  
	![Pulsante Avvia](media/automation-first-runbook-textual-powershell/automation-start-button.png)  
...
6.	Lo stato del processo è visualizzato in **Riepilogo processi** e corrisponde agli stati osservati durante il test del runbook.  
	![Riepilogo dei processi](media/automation-first-runbook-textual-powershell/automation-job-summary.png)  
7.	Quando lo stato del runbook risulta *Completato* fare clic su **Output**. Viene aperto il riquadro Output dove si può vedere il testo *Hello World*.  
	![Output processo](media/automation-first-runbook-textual-powershell/automation-job-output.png)
8.	Chiudere il riquadro Output.
9.	Fare clic su **Tutti i log** per aprire il riquadro Flussi relativo al processo del runbook. Nel flusso di output sarà visibile solo *Hello World*, ma possono essere visualizzati altri flussi per un processo del runbook, ad esempio Dettagliato ed Errore, se il runbook scrive in questi flussi.  
...
11.	Fare clic su**Processi** per aprire il pannello dei processi per questo runbook. Sono elencati tutti i processi creati da questo runbook. Verrà elencato un solo processo, perché il processo è stato eseguito una sola volta.  
	![Elenco processi](media/automation-first-runbook-textual-powershell/automation-job-list.png)  
...
1.	Aprire l'editor di testo facendo clic su **Modifica** nel riquadro MyFirstRunbook-PowerShell.  
	![Modifica runbook](media/automation-first-runbook-textual-powershell/automation-edit-runbook.png)  
...
1.	Dopo *Add-AzureAccount* digitare *Start-AzureVM -Name 'NomeVM' -ServiceName 'NomeServizioVM'* specificando il nome e il nome del servizio della macchina virtuale da avviare.  
...
1.	Aggiungere al runbook i parametri per *NomeVM* e *NomeServizioVM* e usare queste variabili con il cmdlet **Start-AzureVM** come illustrato nell'immagine seguente.  
	![Aggiungi parametro](media/automation-first-runbook-textual-powershell/automation-add-parameter.png)  
