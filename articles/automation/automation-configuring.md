...
>[AZURE.NOTE] Gli account di automazione e le risorse che contengono, creati con il portale di Azure, non sono accessibili nel portale di Azure classico. Se si vogliono gestire questi account o le relative risorse con Windows PowerShell, è necessario usare i moduli di Gestione risorse di Azure.
>
>Gli account di automazione creati con il portale di Azure classico possono essere gestiti da entrambi i portali e set di cmdlet. Una volta creato l'account, non fa alcuna differenza come si creano e gestiscono le risorse all'interno dell'account. Se si prevede di continuare a usare il portale di Azure classico, è necessario usarlo invece del portale di Azure per creare gli account di automazione.
...
11. Creare un nuovo [asset credenziali di Automazione di Azure](automation-credentials.md) per l'account utente creato. Il **Tipo di credenziali** deve essere **Credenziali per Windows PowerShell**.
...
2. Fare clic su **Nuovo** > **Gestione** > **Account di automazione**

3. Nel pannello **Aggiungi un nuovo account di automazione** configurare i dettagli dell'account di automazione.

>[AZURE.NOTE] Quando si crea un account di automazione tramite il portale di Azure, l'account e tutte le risorse associate non vengono riportati nel portale di Azure classico.
...
>[AZURE.NOTE] Quando un account di automazione creato con il portale di Azure classico, viene [spostato in un gruppo di risorse diverso](../resource-group-move-resources.md) usando il portale di Azure, l'account di automazione non sarà più disponibile nel portale di Azure classico.
