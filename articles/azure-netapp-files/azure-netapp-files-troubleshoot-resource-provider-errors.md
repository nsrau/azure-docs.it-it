...
* Soluzione    
Nessuno. Non è possibile modificare il nome di dominio.
* Soluzione alternativa:   
...
Questo messaggio indica un errore nei campi `RuleIndex`per, `AllowedClients`, `UnixReadOnly`, `UnixReadWrite`, `Nfsv3`e. `Nfsv4`
...
* Soluzione alternativa:   
Nessuno. L'API sottostante è essenziale per la gestione dei volumi.
...
* Soluzione alternativa:   
Nessuno. È consentita una sola Active Directory.
...
* Soluzione   
Nessuno. Tali risorse non dispongono di proprietà che possono essere modificate.
...
* Soluzione alternativa:   
Nessuno.  Vedere la soluzione sopra. 
...
* Soluzione   
Nessuno. La risorsa che si sta tentando di aggiornare non supporta l'operazione di aggiornamento. Pertanto, non può essere modificato.
* Soluzione alternativa:   
