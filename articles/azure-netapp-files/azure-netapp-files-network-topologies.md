...
* Azure NetApp Files non è supportato con la rete WAN virtuale di Azure

Per Azure NetApp Files vengono applicate le restrizioni di rete seguenti:

* Il numero di indirizzi IP in uso in un VNet con Azure NetApp Files (incluso reti virtuali con peering) non può essere maggiore di 1000. Stiamo lavorando per aumentare questo limite per soddisfare le esigenze di scalabilità dei clienti. 
...
* Nell'architettura precedente, se è presente anche un gateway nel VNET spoke, la connettività al volume e da locale che si connette al gateway nell'hub andrà persa. Per impostazione predefinita, la preferenza viene assegnata al gateway nella VNet spoke, quindi solo i computer che si connettono tramite il gateway possono connettersi al volume e.
