...
> Per l'equivalente di App per dispositivi mobili di questo argomento, vedere [Creare un'app per Xamarin.Android](../app-service-mobile/app-service-mobile-xamarin-android-get-started.md).
> 
> 
...
Dopo aver creato il servizio mobile, è possibile seguire una facile guida introduttiva nel portale classico per creare una nuova app o modificare un'app esistente per connettersi al servizio mobile.
...
4. In **Scaricare e pubblicare il servizio nel cloud** selezionare **Android** e fare clic su **Download**.
      ...
      Verrà scaricata una soluzione che contiene i progetti per il servizio mobile e per l'applicazione *To do list* di esempio connessa al servizio mobile. Salvare il file del progetto compresso nel computer locale e prendere nota del percorso.
...
2. Premere il pulsante **Esegui** per compilare il progetto e avviare l'app. Verrà chiesto di selezionare un emulatore o un dispositivo USB collegato.
   ...
   > [!NOTE]
   > Per poter eseguire il progetto nell'emulatore di Android, è necessario definire almeno un dispositivo Android Virtual Device (AVD). Usare AVD Manager per creare e gestire questi dispositivi.
   > 
   > 
