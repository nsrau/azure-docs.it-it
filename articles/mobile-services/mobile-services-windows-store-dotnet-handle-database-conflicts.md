---
title: Gestione di conflitti di scrittura con concorrenza ottimistica (Windows Store) | Microsoft Docs
...
3. In Esplora soluzioni di Visual Studio aprire MainPage.cs nel progetto condiviso. Aggiungere a MainPage il gestore per l'evento `LostFocus` del controllo TextBox, come illustrato di seguito.
...
1. In MainPage.cs per il progetto condiviso aggiungere la definizione per il metodo `UpdateToDoItem()` MainPage a cui viene fatto riferimento nel gestore eventi, come illustrato di seguito.
...
7. Nell'istanza 1 dell'app aggiornare il testo dell'ultimo elemento in **Test Write 1** e quindi fare clic su un'altra casella di testo in modo che il gestore dell'evento `LostFocus` aggiorni il database. Nella schermata seguente viene visualizzato un esempio.
