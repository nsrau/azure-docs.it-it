---
title: Registrare le app per l'autenticazione Facebook | Microsoft Docs
...
5. Nel campo **App Secret** fare clic su **Show**, fornire la password se richiesto, quindi prendere nota dei valori di **App ID** e **App Secret**. Questi verranno usati più avanti per configurare l'applicazione in Azure.
   ...
   > [!NOTE]
   > **Nota sulla sicurezza** Il segreto dell'app è una credenziale di sicurezza importante. Non condividere questo valore con altri e non distribuirlo all'interno di un'applicazione client.
   > 
   > 
6. Sulla barra di spostamento sinistra fare clic su **Settings**, digitare il nome del dominio del servizio mobile in **App Domains**, immettere un valore facoltativo per **Contact Email**, fare clic su **Add Platform** e selezionare **Website**.
   
       ![][3]
7. Digitare l'URL del servizio mobile in **Site URL** e quindi fare clic su **Save Changes**.
8. Fare clic su **Show**, digitare la propria password, se richiesto, quindi annotare i valori di **App ID** e **App Secret**.
   ...
   > 
   > 
9. Fare clic sulla scheda **Avanzate**, digitare uno dei seguenti formati di URL in **URI di reindirizzamento OAuth validi**, quindi fare clic su **Salva modifiche**:
   ...
   * **Back-end JavaScript**: `https://<mobile_service>.azure-mobile.net/login/facebook`
     ...
     > [!NOTE]
     > Assicurarsi di utilizzare il formato del percorso URL di reindirizzamento corretto per il tipo di back-end di servizi mobili usando lo schema *HTTPS*. Quando questo non è corretto, l'autenticazione avrà esito negativo.
     > 
     > 
