---
title: Pianificare le attività di back-end in un servizio mobile back-end di .NET | Microsoft Docs
...
> [!NOTE]
> In questa esercitazione viene usata la libreria LINQ to Twitter di terze parti per semplificare l'accesso OAuth 2.0 alle API di Twitter v1.1 . Per completare questa esercitazione è necessario scaricare e installare il pacchetto NuGet della libreria LINQ to Twitter. Per altre informazioni vedere il [progetto CodePlex di LINQ to Twitter].
> 
> 
...
2. Sostituire il contenuto del file SampleJob.cs con il codice seguente:
    ...
    Nel codice precedente è necessario sostituire le stringhe *todolistService* e *todolistContext* con lo spazio dei nomi e la classe DbContext del progetto scaricato, che sono rispettivamente *nome&#95;servizio&#95;mobile*Service e *nome&#95;servizio&#95;mobile*Contex.
...
2. Fare clic su **Prova**, quindi su **POST jobs/{jobName}**.
    ...
    ![][8]
3. Fare clic su **Prova**, digitare `Sample` come valore del parametro **{jobName}** e quindi fare clic su **Invia**.
...
2. Nel [portale di Azure classico], fare clic su Servizi mobili e quindi sull'app.
3. Fare clic sulla scheda **Utilità di pianificazione** e quindi su **+Crea**.
   ...
   > 
   > 
4. Nella finestra di dialogo dell'utilità di pianificazione immettere *Sample* per **Nome processo**, impostare le unità e l'intervallo di pianificazione e quindi fare clic sul segno di spunta.
...
5. Fare clic sul nuovo processo appena creato e quindi su **Run Once** per testare lo script.
   ...
   > [!NOTE]
   > È comunque possibile usare una richiesta POST per avviare il processo pianificato. Per impostazione predefinita, l'autorizzazione è concessa all'utente, quindi la richiesta dovrà includere la chiave applicazione nell'intestazione.
   > 
   > 
6. (Facoltativo) Nel [portale di Azure classico] fare clic su Gestisci per il database associato al servizio mobile.
   
    ![][6]
7. Nel portale di Azure classico eseguire una query per visualizzare le modifiche apportate dall'app. La query sarà simile a quella riportata di seguito, ma come nome dello schema verrà usato il nome del servizio mobile anziché `todolist`.
