...
Questo argomento descrive come usare Servizi mobili di Azure per inviare notifiche push a un'app per iOS usando APNS (Apple Push Notification Service). Questa esercitazione consente di usare Hub di notifica di Azure per abilitare le notifiche push nel [progetto di guida introduttiva](mobile-services-ios-get-started.md/). Al termine dell'esercitazione, il servizio mobile invierà una notifica push ogni volta che viene inserito un record.
