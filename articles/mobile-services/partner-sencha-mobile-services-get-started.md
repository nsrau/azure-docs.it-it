---
title: Introduzione a Servizi mobili e Sencha Touch
...
Dopo avere creato il servizio mobile, è possibile seguire una facile guida introduttiva nel portale di Azure classico per creare una nuova tabella di database da usare nel servizio mobile.

1. Nel [portale di Azure classico] fare clic su **Servizi mobili** e quindi sul servizio mobile appena creato.
2. Nella scheda Quickstart fare clic su **HTML** in **Choose platform** ed espandere **Create a new HTML app**.
...
3. Fare clic su **Create TodoItems table** per creare una tabella in cui archiviare i dati dell'app.
   ...
   > [!NOTE]
   > NON scaricare l'app HTML dal portale di Azure classico. Verrà invece creata manualmente un'applicazione Sencha Touch nella sezione seguente.
   > 
   > 
4. Prendere nota della **chiave dell'app** e dell'**URL dell'app** nel portale di Azure classico perché verranno utilizzati in altre sezioni di questa esercitazione.
...
> [!NOTE]
> È possibile usare questo metodo solo quando l'applicazione è stata generata con il comando <code>sencha generate app</code>.
> 
> 
...
2. Ricompilare l'applicazione utilizzando **sencha cmd** per recuperare e installare il pacchetto:
   
        $ sencha app build

Sia **sencha app build** che **sencha app refresh** eseguono entrambi la procedura necessaria per integrare il pacchetto nell'applicazione. In genere dopo la modifica dei requisiti del pacchetto è necessario eseguire **sencha app refresh** in modo da aggiornare i metadati necessari per supportare "dev mode".
...
I modelli Sencha Touch forniscono la definizione dei record di dati che verranno utilizzati nell'applicazione. In questo modo è possibile definire non solo i campi di dati, ma anche fornire la configurazione relativa al proxy che gestirà le comunicazioni tra l'applicazione e il servizio mobile di Azure.
...
A questo punto, dopo aver definito la struttura di ogni elemento Todo e aver creato un archivio in cui inserire tutti i record, è opportuno definire in che modo si desidera che tali informazioni vengano visualizzate all'utente dell'app. A tale scopo vengono in genere utilizzate le **visualizzazioni**, ovvero un qualsiasi numero di componenti Touch, sia singoli che combinati con altri.
...
5. Tornare al [portale di Azure classico], fare clic sulla scheda **Dati** e quindi sulla tabella TodoItems.
