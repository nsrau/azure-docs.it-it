---
title: Aggiungere notifiche push all'app di Servizi mobili (Xamarin.Forms) - Servizi mobili
...
3. [Aggiungere notifiche push all'app Xamarin.Forms.iOS](#iOS)
4. [Aggiungere notifiche push all'app Xamarin.Forms.Android](#Android)
5. [Aggiungere notifiche push all'app Xamarin.Forms.Windows](#Windows)
6. [Aggiornare lo script di inserimento tabelle di Azure per inviare notifiche push a tutte le app](#all-apps)
...
> [!IMPORTANT]
> Considerati i requisiti del servizio Apple Push Notification, è necessario distribuire e testare le notifiche push su un dispositivo con iOS (iPhone o iPad) anziché su un emulatore.
> 
> 
...
1. In **AppDelegate** eseguire l'override dell'evento **RegisteredForRemoteNotifications**:
...
2. In **AppDelegate** eseguire l'override dell'evento **ReceivedRemoteNotification**:
...
> [!NOTE]
> Le procedure seguenti descrivono come aggiornare lo script registrato per l'operazione di inserimento nella tabella TodoItem del portale di Azure classico. È anche possibile accedere e modificare questo script del servizio mobile direttamente in Visual Studio, nel nodo Azure di Esplora server.
> 
> 
...
1. In **todoitem** fare clic sulla scheda **Script**, quindi selezionare **Insert**.
...
3. Aprire il nuovo file di progetto MainActivity.cs e aggiungere l'istruzione using seguente alla classe:
...
1. Nel progetto ToDoAzure.Droid creare una nuova classe denominata `GcmService`.
2. Aggiungere le istruzioni using seguenti alla classe **GcmService**:
...
4. Nel file di progetto **GcmService.cs** aggiungere la classe seguente:
...
5. Nel file di progetto GcmService.cs aggiungere il codice seguente che definisce la classe **GcmService**:
...
1. Aggiungere il codice seguente alla classe **GcmService** che sostituisce il gestore di eventi **OnRegistered**.
...
2. Eseguire l'override del metodo **OnMessage** in **GcmService** con il codice seguente:
...
3. Aggiungere il metodo seguente per eseguire l'override di **OnUnRegistered()** e **OnError()**, che sono necessari per la compilazione del progetto.
...
4. Aggiungere una nuova attività nell'app.
5. Scorrere verso il basso dalla parte superiore della schermata per aprire il Notification Center del dispositivo e visualizzare la notifica.
...
5. In Esplora soluzioni espandere **Proprietà**, aprire il file WMAppManifest.xml, fare clic sulla scheda **Funzionalità** e assicurarsi che la funzionalità **ID\_CAP\_PUSH\_NOTIFICATION** sia selezionata.
...
1. Fare clic sulla scheda **Push**, selezionare **Abilita notifiche push non autenticate** e quindi fare clic su **Salva**.
   ...
   > [!NOTE]
   > In questa esercitazione verrà usato il Servizio notifica Push Microsoft in modalità senza autenticazione. In tale modalità, il Servizio notifica Push Microsoft limita il numero di notifiche che è possibile inviare a un canale di dispositivo. Per rimuovere questa restrizione è necessario generare e caricare un certificato facendo clic su **Upload** e selezionando il certificato. Per altre informazioni sulla generazione del certificato, vedere [Configurazione di un servizio Web autenticato per l'invio di notifiche push per Windows Phone].
   > 
   > 
...
1. In Visual Studio premere F5 per eseguire l'app.
   ...
   > 
   > 
2. Nell'app creare una nuova attività con titolo **Hello push**, quindi fare immediatamente clic sul pulsante start o back per uscire dall'app.
