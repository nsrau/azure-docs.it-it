---
title: Archiviare il codice del progetto di back-end JavaScript nel controllo del codice sorgente | Microsoft Docs
...
1. Passare alla cartella `.\service` del repository Git locale e aprire il file package.json in un editor di testo, quindi aggiungere il campo seguente all'oggetto **dependencies**:
   
        "node-uuid": "~1.4.3"
   
   > [!NOTE]
   > Questo aggiornamento del file package.json provocherà un riavvio del servizio mobile dopo il push del commit.
   > 
   > 
2. Passare alla sottocartella .\\service\\table, aprire il file todoitem.insert.js e modificarlo come illustrato di seguito:
...
3. Come nella sezione precedente, al prompt dei comandi di Git digitare il comando seguente:
