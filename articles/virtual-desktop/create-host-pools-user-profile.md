---
title: Configurare una condivisione di profilo utente per un pool di host - Azure
description: Come configurare un contenitore di profilo FSLogix per un pool di host di Desktop virtuale di Windows (anteprima).
...
ms.locfileid: "58336015"
---
# <a name="set-up-a-user-profile-share-for-a-host-pool"></a>Configurare una condivisione di profilo utente per un pool di host

Il servizio di Windows Virtual Desktop (anteprima) offre contenitori profilo FSLogix come la soluzione consigliata per l'utente per un profilo. Non è consigliabile usare la soluzione disco profili utente (UPD) e nelle versioni future di Windows Desktop virtuale verrà deprecato.
