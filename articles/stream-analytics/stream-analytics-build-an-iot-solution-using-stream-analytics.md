...
## Eseguire il provisioning delle risorse di Azure necessarie per l'esercitazione
...
5. Immettere **tolladmin** nel campo **Nome utente** e **123toll!** nel campo **Password**.
...
Per informazioni più dettagliate, vedere gli articoli di MSDN sui costrutti relativi alla [gestione del tempo](https://msdn.microsoft.com/library/azure/mt582045.aspx) e alla [funzione finestra](https://msdn.microsoft.com/library/azure/dn835019.aspx) usati nella query.
...
## Domanda 1: Numero di veicoli che entrano in un casello
...
1. Arrestare il processo corrente, aggiornare la query nella scheda **QUERY** e aprire la scheda **PIANO**.

	Le **UNITÀ DI STREAMING** definiscono la potenza di calcolo che il processo può ricevere.
...
Anche se questa esercitazione offre una buona introduzione, non può ritenersi completa. Per altri modelli di query che usano il linguaggio SAQL, vedere [Esempi di query per modelli di uso comune di Analisi di flusso](stream-analytics-stream-analytics-query-patterns.md). Per altre informazioni su Analisi di flusso di Azure, vedere la [documentazione online](https://azure.microsoft.com/documentation/services/stream-analytics/).

## Eseguire la pulizia dell'account Azure
