...
Integrazione del portale di Azure | Oltre alla presenza costante nel portale di Azure classico, l’analisi di flusso è ora integrata nel [portale di Azure](http://azure.microsoft.com/overview/preview-portal/). Si noti che la funzionalità di analisi di flusso nel portale è attualmente un sottoinsieme delle funzionalità offerte nel portale di Azure classico, senza il supporto per il test di query nel browser, la configurazione di output Power BI e l’esplorazione o la creazione di nuove risorse di input e output nelle sottoscrizioni a cui si ha accesso.
