...
La tabella seguente illustra ogni proprietà nella nuova pagina di **input** nel portale di Azure per trasmettere i dati di input da un hub eventi:
...
| **Chiave dei criteri di accesso condiviso** | Chiave di accesso condiviso usata per autorizzare l'accesso all'hub IoT.  Il valore di questa opzione viene inserito automaticamente, a meno che non si selezioni l'opzione per specificare le impostazioni dell'hub IoT manualmente. |
