...
Normalmente, le dimensioni del batch impostate per le funzioni Machine Learning non saranno esattamente divisibile per il numero di eventi restituiti da ogni processo di analisi di flusso "pull". Quando ciò accade, il servizio Web di Machine Learning viene chiamato con batch parziali. L'uso di batch parziali evita l'overhead della latenza di processi aggiuntiva negli eventi di Unione da pull a pull.
