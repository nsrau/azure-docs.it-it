...
In genere, le dimensioni batch impostate per le funzioni di Machine Learning non sono esattamente divisibili per il numero di eventi restituiti da ogni pull del processo di Analisi di flusso. Quando ciò accade, il servizio Web di Machine Learning viene chiamato con batch parziali. Questo per non causare un sovraccarico della latenza dei processi nell'unione di eventi tra un pull e l'altro.
