...
description: Questo articolo descrive come creare, eseguire il debug e creare analisi di flusso nei processi di Edge IoT usando gli strumenti di Analisi di flusso per Visual Studio.This article describes how to author, debug, and create your Stream Analytics on IoT Edge jobs using the Stream Analytics tools for Visual Studio.
author: su-jie
ms.author: sujie
...
In this tutorial, you learn how to use Stream Analytics tools for Visual Studio. Scopri come creare, eseguire il debug e creare i processi di Stream Analytics Edge. Dopo aver creato e testato il processo, è possibile passare al portale di Azure per distribuirlo ai dispositivi. 
...
* Seguire le istruzioni di installazione per installare gli strumenti di Analisi di flusso per Visual Studio.Follow the [installation instructions](stream-analytics-tools-for-visual-studio-edge-jobs.md) to install Stream Analytics tools for Visual Studio.
...
![Nuovo progetto Stream Analytics Edge in Visual Studio](./media/stream-analytics-tools-for-visual-studio-edge-jobs/new-stream-analytics-edge-project.png)
...
I processi di Analisi di flusso distribuiti negli ambienti IoT Edge di Analisi di flusso supportano la maggior parte delle informazioni di riferimento sul linguaggio di query di [Analisi di flusso.](https://msdn.microsoft.com/azure/stream-analytics/reference/stream-analytics-query-language-reference?f=255&MSPPError=-2147217396) Tuttavia, le operazioni seguenti non sono ancora supportate per i processi di Stream Analytics Edge: 


|**Category**  | **Comando**  |
|---------|---------|
|Altri operatori | <ul><li>PARTITION BY</li><li>TIMESTAMP BY OVER</li><li>JavaScript UDF</li><li>Aggregazioni definite dall'utente (UDA)User-defined aggregates (UDA)</li><li>GetMetadataPropertyValue</li><li>Utilizzo di più di 14 aggregazioni in un unico passaggioUsing more than 14 aggregates in a single step</li></ul>   |

Quando crei un processo di Analisi di flusso Edge nel portale, il compilatore ti avviserà automaticamente se non usi un operatore supportato.
...
4. È possibile eseguirlo in locale o inviarlo ad Azure.You can either run it locally or submit to Azure. Per testare la query, selezionare **Esegui localmente**.  
...
3. Si apre una finestra pop-up. Scegli se aggiornare un processo Stream Analytics Edge esistente o crearne uno nuovo. Quando si aggiorna un processo esistente, verrà sostituita tutta la configurazione del processo, in questo scenario verrà pubblicato un nuovo processo. Selezionare **Create a New Azure Stream Analytics Job** (Crea un nuovo progetto di Analisi di flusso di Azure) > immettere un nome per il processo simile a **MyASAEdgeJob** > scegliere **Sottoscrizione**, **Gruppo di risorse** e **Percorso** > Selezionare **Invia**.
   ...
   Ora il processo Stream Analytics Edge è stato creato. È possibile fare riferimento [all'esercitazione Esegui processi in Edge IoT](stream-analytics-edge.md) per informazioni su come distribuirlo nei dispositivi. 
