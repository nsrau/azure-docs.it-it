...
Analisi di flusso garantisce che i processi nelle aree abbinate vengano aggiornati in batch separati. Di conseguenza, esiste un intervallo di tempo sufficiente tra gli aggiornamenti per identificare potenziali problemi e risolverli.
...
È consigliabile distribuire processi identici in entrambe le aree abbinate. È quindi necessario [monitorare questi processi](https://docs.microsoft.com/azure/stream-analytics/stream-analytics-set-up-alerts#scenarios-to-monitor) per ricevere una notifica quando si verifica un evento imprevisto. Se uno di questi processi finisce in [stato Non riuscito](https://docs.microsoft.com/azure/stream-analytics/job-states) dopo un aggiornamento del servizio Analisi di flusso, è possibile contattare il supporto clienti per identificare la causa principale. È inoltre necessario eseguire il failover di tutti i consumer a valle nell'output del processo integro.
