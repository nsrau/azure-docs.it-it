<properties 
	pageTitle="Creare e caricare un VHD di Red Hat Enterprise Linux per l'utilizzo in Azure" 
...
- Il formato VHDX più recente non è supportato in Azure. È possibile convertire il disco in formato VHD tramite la console di gestione di Hyper-V o il cmdlet powershell convert-vhd.
...
8.	Registrare la propria sottoscrizione Red Hat per abilitare l’installazione dei pacchetti dall’archivio RHEL eseguendo il seguente comando:
...
6.	Registrare la propria sottoscrizione Red Hat per abilitare l’installazione dei pacchetti dall’archivio RHEL eseguendo il seguente comando:
...
18.	Convertire l'immagine qcow2 in formato vhd: prima convertire l'immagine in formato non elaborato:
    ...
    Verificare che le dimensioni dell'immagine non elaborata siano allineate a 1MB; in caso contrario arrotondare la dimensione per l'allineamento a 1MB: # MB=$((1024*1024)) # size=$(qemu-img info -f raw --output json "rhel-6.7.raw" | \\ gawk 'match($0, /"virtual-size": ([0-9]+),/, val) {print val[1]}') # rounded\_size=$((($size/$MB + 1)*$MB))
...
15.	Eliminare la sottoscrizione(se necessario) eseguendo il seguente comando:
...
4.	Registrare la propria sottoscrizione Red Hat per abilitare l’installazione dei pacchetti dall’archivio RHEL eseguendo il seguente comando:
...
3.	Creare una nuova macchina virtuale nella console di gestione Hyper-V. Nella pagina **Connessione disco rigido virtuale** selezionare **Collega un disco rigido virtuale in un secondo momento** e completare la creazione guidata della nuova macchina virtuale.

4.	Aprire le impostazioni della macchina virtuale:

    a. Collegare un nuovo disco rigido virtuale alla macchina virtuale e assicurarsi di selezionare **Formato VHD** e **Dimensione fissa**.
...
Sebbene sia un problema intermittente, si verifica più spesso durante frequenti operazioni di I/O del disco in Hyper-V e Azure.
...
## Passaggi successivi
È ora possibile usare il file con estensione vhd di Red Hat Enterprise Linux per creare nuove macchine virtuali in Azure. Per altre informazioni sui hypervisor certificati per l'esecuzione di Red Hat Enterprise Linux, visitare [il sito Web di Red Hat](https://access.redhat.com/certified-hypervisors).
