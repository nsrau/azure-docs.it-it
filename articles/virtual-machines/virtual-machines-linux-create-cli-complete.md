...
Viene usato lo strumento [jq](https://stedolan.github.io/jq/), ma è possibile usare **jsawk** o qualsiasi libreria del linguaggio preferita per analizzare JSON, insieme all'opzione dell'interfaccia della riga di comando di Azure `--json` per esaminare il gruppo di risorse con il comando `azure group show`.
...
Anche in questo caso osservare come vengono sviluppate le risorse con l'opzione --json di `azure group show` e **jq**. Sono ora disponibili una risorsa `storageAccounts` e una risorsa `virtualNetworks`.
...
Viene quindi creata una subnet all'interno della rete virtuale `TestVnet` in cui verrà distribuita la macchina virtuale. Viene usato il comando `azure network vnet subnet create` insieme alle risorse già create, ovvero il gruppo di risorse `TestRG` e la rete virtuale `TestVNet` e viene aggiunto il nome della subnet `FrontEnd` e il prefisso dell'indirizzo della subnet `192.168.1.0/24`, come indicato di seguito.
...
Poiché la subnet è logicamente all'interno della rete virtuale, vengono cercate le informazioni relative alla subnet con un comando leggermente diverso, `azure network vnet show`, ma viene esaminato comunque l'output JSON restituito con **jq**.
...
> [AZURE.NOTE] La regola in ingresso è un filtro per le connessioni di rete in ingresso. In questo esempio il gruppo di sicurezza di rete verrà associato alla scheda di rete virtuale della VM, di conseguenza qualsiasi richiesta per la porta 22 verrà passata alla scheda di rete nella VM. Poiché si tratta di una regola relativa a una connessione di rete, non di un endpoint come nelle distribuzioni classiche, per aprire una porta è necessario lasciare `--source-port-range` impostato sul valore predefinito '*' per accettare le richieste in ingresso da **tutte** le porte richiedenti, che sono in genere dinamiche.

### Creare l'indirizzo IP pubblico (PIP)

Viene ora creato l'indirizzo IP pubblico (PIP) che consente di connettersi alla macchina virtuale da Internet tramite il comando `azure network public-ip create`. Poiché il valore predefinito è un indirizzo dinamico, viene creata una voce DNS denominata nel dominio **cloudapp.azure.com** usando l'opzione `-d testsubdomain`.
...
Sono state create le risorse di archiviazione e di rete per supportare una VM accessibile da Internet. Ora si creerà la VM che verrà protetta con una chiave SSH senza password. In questo caso, si vuole creare una VM Ubuntu basata sull'LTS più recente. Le informazioni sull'immagini vengono cercate usando `azure vm image list`, come descritto nell'articolo relativo alla [ricerca delle immagini delle macchine virtuali di Azure](virtual-machines-linux-cli-ps-findimage.md). È stata selezionata un'immagine con il comando `azure vm image list westeurope canonical | grep LTS` e in questo caso viene usato `canonical:UbuntuServer:14.04.3-LTS:14.04.201509080`, ma per l'ultimo campo si passa `latest` in modo che in futuro si avrà sempre la build più recente. La stringa usata è `canonical:UbuntuServer:14.04.3-LTS:latest`.
...
Per creare la macchina virtuale riunire tutte le risorse e le informazioni con il comando `azure vm create`.
...
È ora possibile usare il comando `azure vm show testrg testvm` per esaminare gli elementi creati. A questo punto, è disponibile una VM Ubuntu in esecuzione in Azure a cui è possibile accedere solo con la coppia di chiavi SSH disponibile. Le password sono disabilitate.
