---
title: Creare una coppia di chiavi SSH per le macchine virtuali di Linux | Documentazione Microsoft
description: Creare in modo sicuro una coppia di chiavi SSH pubblica e privata per le macchine virtuali di Linux.
...
Per impostazione predefinita, le chiavi SSH vengono conservate nella directory `~/.ssh`.  Se non si dispone di una directory `~/.ssh`, questa viene creata automaticamente dal comando `ssh-keygen` con le autorizzazioni corrette.  Il flag `-N` dell'interfaccia della riga di comando è la password per crittografare la chiave SSH privata e *non* corrisponde alla password dell'utente.
...
Verificare che `ssh-agent` sia in esecuzione:
...
Se non viene richiesta la password della chiave privata SSH o una password di accesso alla macchina virtuale, il protocollo SSH è stato configurato correttamente.
