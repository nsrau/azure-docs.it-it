...
description: Panoramica delle notifiche di manutenzione per le macchine virtuali in esecuzione in Azure.Overview of maintenance notifications for virtual machines running in Azure.
author: shants123
...
Azure esegue periodicamente aggiornamenti per migliorare l'affidabilità, le prestazioni e la sicurezza dell'infrastruttura host per le macchine virtuali. Gli aggiornamenti sono considerati modifiche, ad esempio l'applicazione di patch all'ambiente di hosting o l'aggiornamento e la rimozione di componenti hardware. La maggior parte di questi aggiornamenti viene completata senza alcun impatto sulle macchine virtuali ospitate. Ci sono casi tuttavia in cui gli aggiornamenti hanno conseguenze:

- Se la manutenzione non richiede un riavvio, Azure usa la migrazione sul posto per sospendere la VM mentre l'host viene aggiornato. Queste operazioni di manutenzione di questi tipi vengono applicate dal dominio di errore. L'operazione viene interrotta in caso di ricezione di segnali di integrità di avviso.

- Se la manutenzione richiede un riavvio, si riceve un avviso che informa per quando è pianificata la manutenzione. Ti viene dato un intervallo di tempo di circa 35 giorni in cui puoi iniziare la manutenzione da solo, quando funziona per te.


La manutenzione pianificata che richiede un riavvio viene pianificata in cicli. Ogni ondata ha un ambito diverso (aree),

- e inizia con un avviso ai clienti. Per impostazione predefinita, la notifica viene inviata all'amministratore del servizio e ai coamministratori. È possibile aggiungere altri destinatari e opzioni di messaggistica come e-mail, SMS e webhook, utilizzando [Avvisi log attività](../service-health/alerts-activity-log-service-notifications.md).  
- Una volta che una notifica viene estratta, viene resa disponibile una *finestra self-service.* Durante questa finestra, è possibile eseguire una query su quali macchine virtuali sono interessate e avviare la manutenzione in base alle proprie esigenze di pianificazione. La finestra self-service è in genere di circa 35 giorni.
...
La manutenzione self-service non è consigliata per le distribuzioni che utilizzano set di **disponibilità**. I set di disponibilità vengono già aggiornati un solo dominio di aggiornamento alla volta. 

- Lasciare che Azure attivi la manutenzione. Per la manutenzione che richiede il riavvio, la manutenzione verrà eseguita aggiornando il dominio per dominio di aggiornamento. I domini di aggiornamento non ricevono necessariamente la manutenzione in sequenza e che vi è una pausa di 30 minuti tra i domini di aggiornamento. 
- Se una perdita temporanea di una certa capacità (1 dominio di aggiornamento) è un problema, è possibile aggiungere istanze durante il periodo di manutenzione. 
...
- Sono necessari più di 30 minuti di tempo di ripristino della macchina virtuale tra due domini di aggiornamento (UD). Per controllare l'intervallo tra i domini di aggiornamento, è necessario attivare la manutenzione sulle macchine virtuali di un dominio di aggiornamento alla volta.
...
**D: Se seguo i consigli per la disponibilità elevata utilizzando un set di disponibilità, sono sicuro?**
...
**A:** La manutenzione pianificata è ora disponibile per i set di scalabilità di macchine virtuali. Per istruzioni su come avviare la manutenzione self-service, fare riferimento alla [manutenzione pianificata per](../virtual-machine-scale-sets/virtual-machine-scale-sets-maintenance-notifications.md) i set di scalabilità delle macchine virtuali.
