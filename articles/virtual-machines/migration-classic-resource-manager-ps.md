...
Se si vuole, è anche possibile eseguire la migrazione delle risorse usando l'interfaccia della riga di comando di [Azure](migration-classic-resource-manager-cli.md).
...
Per le istruzioni di installazione, vedere [Come installare e configurare Azure PowerShell](/powershell/azure/).
...
* [Eseguire la migrazione di macchine virtuali in un servizio cloud (non in una rete virtuale)](#step-51-option-1---migrate-virtual-machines-in-a-cloud-service-not-in-a-virtual-network)
...
### <a name="step-51-option-1---migrate-virtual-machines-in-a-cloud-service-not-in-a-virtual-network"></a>Passaggio 5,1: opzione 1-eseguire la migrazione delle macchine virtuali in un servizio cloud (non in una rete virtuale)
...
Dopo aver completato la procedura di preparazione tramite una delle opzioni precedenti, eseguire una query dello stato di migrazione delle VM, Verificare che si trovino nello `Prepared` stato.
...
### <a name="step-51-option-2---migrate-virtual-machines-in-a-virtual-network"></a>Passaggio 5,1: opzione 2-eseguire la migrazione delle macchine virtuali in una rete virtuale
...
> [!NOTE]
> [Eseguire la migrazione di una singola macchina virtuale](./windows/create-vm-specialized-portal.md) creata usando il modello di distribuzione classica creando una nuova macchina virtuale Gestione risorse con Managed disks usando i file VHD (sistema operativo e dati) della macchina virtuale.
<br>
...
### <a name="step-52-migrate-a-storage-account"></a>Passaggio 5,2: eseguire la migrazione di un account di archiviazione
