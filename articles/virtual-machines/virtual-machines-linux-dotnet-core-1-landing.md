<properties
   pageTitle="Macchine virtuali di Azure - Esercitazione 1 DotNet Core | Microsoft Azure"
...
- [**Architettura delle applicazioni**](./virtual-machines-linux-dotnet-core-2-architecture.md): i componenti delle applicazioni, come i siti Web e i database, devono essere ospitati nelle risorse di calcolo di Azure, ad esempio le macchine virtuali e i database SQL di Azure. Questo documento illustra come mappare i requisiti di calcolo alle risorse di Azure e come distribuire queste risorse con un modello di Azure Resource Manager.

- [**Accesso e sicurezza**](./virtual-machines-linux-dotnet-core-3-access-security.md): quando si esegue l'hosting di applicazioni in Azure, è necessario considerare la modalità di accesso all'applicazione e la modalità di accesso reciproco dei diversi componenti. Questo documento descrive come fornire e proteggere l'accesso Internet per un'applicazione e l'accesso tra i relativi componenti.

- [**Disponibilità e scalabilità**](./virtual-machines-linux-dotnet-core-4-avalibility-scale.md): queste due caratteristiche fanno riferimento alla capacità delle applicazioni di rimanere in esecuzione durante il tempo di inattività dell'infrastruttura e di ridimensionare le risorse di calcolo in base alla domanda. Questo documento descrive i componenti necessari per distribuire un'applicazione con carico bilanciato e disponibilità elevata.

- [**Distribuzione di applicazioni**](./virtual-machines-linux-dotnet-core-5-app-deployment.md): quando si distribuiscono applicazioni nelle macchine virtuali di Azure, è necessario considerare il metodo da usare per installare i file binari dell'applicazione nella macchina virtuale. Questo documento descrive come automatizzare l'installazione di applicazioni mediante le estensioni script personalizzate per le macchine virtuali di Azure.
...
## Distribuire l'applicazione Music Store
...
|---|---|
|SSHKEYDATA | Dati della chiave SSH usati per proteggere l'accesso alla macchina virtuale. Per informazioni sulla creazione di una coppia di chiavi SSH, vedere [Creare chiavi SSH in Linux e Mac per le VM Linux in Azure](./ virtual-machines-linux-mac-create-ssh-keys.md). |
...
[Passaggio 1: Architettura delle applicazioni con i modelli di Azure Resource Manager](./virtual-machines-linux-dotnet-core-2-architecture.md)
