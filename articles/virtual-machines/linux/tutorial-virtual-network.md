...
Per poter creare una rete virtuale, creare prima di tutto un gruppo di risorse con [az group create](/cli/azure/group#az_group_create). L'esempio seguente crea un gruppo di risorse denominato *myRGNetwork* nella località eastus.
...
Alla rete virtuale viene aggiunta una nuova subnet con il comando [az network vnet subnet create](/cli/azure/network/vnet/subnet#az_network_vnet_subnet_create). In questo esempio vengono assegnati alla subnet il nome *myBackendSubnet* e il prefisso di indirizzo *10.0.2.0/24*. Questa subnet verrà usata con tutti i servizi back-end.
...
Quando si crea una VM con il comando [az vm create](/cli/azure/vm#az_vm_create), il metodo di allocazione predefinito dell'indirizzo IP pubblico è il metodo dinamico. Quando si crea una macchina virtuale con il comando [az vm create](/cli/azure/vm#az_vm_create), per assegnare un indirizzo IP pubblico statico includere l'argomento `--public-ip-address-allocation static`. Questa operazione non viene illustrata in questa esercitazione, ma nella sezione successiva un indirizzo IP con allocazione dinamica verrà modificato in indirizzo con allocazione statica. 

### <a name="change-allocation-method"></a>Modificare il metodo di allocazione

Il metodo di allocazione degli indirizzi IP può essere modificato con il comando [az network public-ip update](/cli/azure/network/public-ip#az_network_public_ip_update). In questo esempio, il metodo di allocazione dell'indirizzo IP della VM front-end viene modificato in statico.
...
Usare il comando [az network public-ip update](/cli/azure/network/public-ip#az_network_public_ip_update) per aggiornare il metodo di allocazione. In questo caso, si imposta `--allocation-method` su *static*.
...
Usare il comando [az vm create](/cli/azure/vm#az_vm_create) per creare la VM denominata *myFrontendVM* mediante *myPublicIPAddress*.
...
Per creare un gruppo di sicurezza di rete, usare il comando [az network nsg create](/cli/azure/network/nsg#az_network_nsg_create).
...
Per creare una regola per la porta *80*, usare il comando [az network nsg rule create](/cli/azure/network/nsg/rule#az_network_nsg_rule_create).
...
La VM front-end è accessibile solo sulla porta *22* e sulla porta *80*. Tutto il resto del traffico in ingresso viene bloccato in corrispondenza del gruppo di sicurezza di rete. Potrebbe essere utile visualizzare le configurazioni delle regole dei gruppi di sicurezza di rete. Il comando [az network rule list](/cli/azure/network/nsg/rule#az_network_nsg_rule_list) restituisce la configurazione delle regole dei gruppi di sicurezza di rete. 
...
Per creare una regola per la porta 22, usare il comando [az network nsg rule create](/cli/azure/network/nsg/rule#az_network_nsg_rule_create). Si noti che l'argomento `--source-address-prefix` specifica il valore *10.0.1.0/24*. Questa configurazione garantisce che tramite il gruppo di sicurezza di rete sia consentito solo il traffico dalla subnet front-end.
...
La VM back-end è accessibile dalla subnet front-end solo sulla porta *22* e sulla porta *3306*. Tutto il resto del traffico in ingresso viene bloccato in corrispondenza del gruppo di sicurezza di rete. Potrebbe essere utile visualizzare le configurazioni delle regole dei gruppi di sicurezza di rete. Il comando [az network rule list](/cli/azure/network/nsg/rule#az_network_nsg_rule_list) restituisce la configurazione delle regole dei gruppi di sicurezza di rete. 
