...
description: "Attività di post-distribuzione in OpenShift"
...
Dopo aver distribuito il cluster OpenShift, è possibile configurare elementi aggiuntivi. Questo articolo include gli argomenti seguenti:

- Configurare Single Sign-On con Azure Active Directory (AAD)
- Configurare OMS per monitorare OpenShift
- Configurare le metriche e la registrazione

## <a name="single-sign-on-using-aad"></a>Single Sign-On con AAD

Per usare AAD per l'autenticazione, è necessario creare prima di tutto la registrazione di un'app Azure AD. Questo processo include due passaggi, la creazione della registrazione dell'app e quindi la configurazione delle autorizzazioni.

### <a name="create-app-registration"></a>Creare la registrazione dell'app

Si userà l'interfaccia della riga di comando di Azure per creare la registrazione dell'app e l'interfaccia utente grafica (portale) per impostare le autorizzazioni. Per creare la registrazione dell'app, saranno necessarie cinque informazioni.
...
- URI dell'identificatore: URL della console OpenShift, ad esempio https://masterdns343khhde.westus.cloudapp.azure.com:8443/console
...
5.  Nel pannello Autorizzazioni necessarie fare clic su **Aggiungi**

  ![Registrazione delle app](media/openshift-post-deployment/app-registration.png)

6.  Fare clic su Passaggio 1: Seleziona API, su **Windows Azure Active Directory (Microsoft.Azure.ActiveDirectory)** e quindi su **Seleziona** nella parte inferiore
...
**OpenShift Container Platform con più master**
...
## <a name="monitor-openshift-with-oms"></a>Monitorare OpenShift con OMS

Il monitoraggio di OpenShift con OMS può essere eseguito usando due opzioni diverse: l'installazione dell'agente OMS nell'host della macchina virtuale o il contenitore OMS. Questo articolo contiene le istruzioni sulla distribuzione del contenitore OMS.

## <a name="create-an-openshift-project-for-oms-and-set-user-access"></a>Creare un progetto OpenShift per OMS e impostare l'accesso utente
...
## <a name="create-daemon-set-yaml-file"></a>Creare il file YAML dell'oggetto DaemonSet
...
## <a name="create-secret-yaml-file"></a>Creare il file YAML del segreto
...
Sostituire wsid_data with con l'ID area di lavoro OMS con codifica Base64 e key_data con la chiave condivisa dell'area di lavoro OMS con codifica Base64.
...
## <a name="create-secret-and-daemon-set"></a>Creare il segreto e l'oggetto DaemonSet
...
Il modello OpenShift Container Platform (OCP) di Resource Manager fornisce i parametri di input per l'abilitazione delle metriche e della registrazione, mentre l'offerta OpenShift Container Platform del Marketplace e il modello OpenShift Origin di Resource Manager no.
...
SSH nel primo nodo Master usando la porta 2200
...
Modificare il file **/etc/ansible/hosts file** e aggiungere le righe seguenti dopo la sezione del provider di identità (# Enable HTPasswdPasswordIdentityProvider)
...
Sostituire $ROUTING con la stringa usata per l'opzione **openshift_master_default_subdomain** nello stesso file **/etc/ansible/hosts**.
