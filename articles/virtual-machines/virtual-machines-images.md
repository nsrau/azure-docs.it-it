...
È possono utilizzare due tipi di immagini in Azure: *immagine di macchina virtuale* e *immagine del sistema operativo*. Immagine di macchina virtuale include un sistema operativo e tutti i dischi collegati a una macchina virtuale quando viene creata l'immagine. Si tratta del tipo più recente dell'immagine. Prima dell'introduzione immagini di macchina virtuale, un'immagine in Azure potrebbe contenere solo un sistema operativo generalizzato e non altri dischi. Immagine di macchina virtuale che contiene solo un sistema operativo generalizzato è sostanzialmente identico al tipo originale dell'immagine, l'immagine del sistema operativo.

È possibile creare immagini personalizzate, in base a una macchina virtuale in Azure o una macchina virtuale in esecuzione in un' posizione che copiata e caricata. Se si desidera utilizzare un'immagine per creare più di una macchina virtuale, sarà necessario prepararla per l'utilizzo come immagine generalizzandola. Per creare un'immagine di Windows Server, eseguire il comando Sysprep nel server per generalizzarla prima di caricare il file con estensione vhd. Per ulteriori informazioni su Sysprep, vedere [Introduzione: Introduzione](http://go.microsoft.com/fwlink/p/?LinkId=392030). Per creare un'immagine Linux, a seconda della distribuzione software, è necessario eseguire un set di comandi che sono specifici per la distribuzione, nonché di eseguire l'agente Linux di Azure.
...
- Per CLI, vedere "Comandi per gestire le immagini di macchina virtuale di Azure" in [tramite la CLI di Azure per Mac, Linux e Windows Azure gestione dei servizi](virtual-machines-command-line-tools.md)
