<properties
 ...
 description="Creare un cluster Linux di macchine virtuali di dimensione A8 o A9 per usare RDMA per eseguire applicazioni MPI."
...
[AZURE.INCLUDE [learn-about-deployment-models](../../includes/learn-about-deployment-models-include.md)]Questo articolo illustra la configurazione delle risorse con il modello di distribuzione classica.
...
* **Script dell'interfaccia della riga di comando di Azure**: come mostrato nelle procedure del resto di questo articolo, usare l'[interfaccia della riga di comando di Azure](../xplat-cli.md) per Mac, Linux e Windows per creare script per la distribuzione di una rete virtuale e degli altri componenti necessari per creare un cluster Linux. L'interfaccia della riga di comando nella modalità di distribuzione classica (Gestione dei servizi) crea i nodi del cluster in modo seriale, pertanto la distribuzione di molti nodi di calcolo potrebbe richiedere alcuni minuti.
...
*   **Intel MPI**: scaricare e installare il runtime di Intel MPI Library 5 dal sito Intel.com eseguendo comandi simili ai seguenti.
...
*   **Blocco della memoria**: affinché i codici MPI blocchino la memoria disponibile per RDMA, è necessario aggiungere o modificare le impostazioni seguenti nel file /etc/security/limits.conf. Per modificare questo file sarà necessario l'accesso alla radice.
...
*   **Chiavi SSH**: generare chiavi SSH per stabilire una relazione di trust per il proprio account utente tra tutti i nodi di calcolo del cluster durante l'esecuzione di processi MPI. Usare il comando seguente per creare chiavi SSH. È sufficiente premere INVIO per generare le chiavi nel percorso predefinito senza impostare una passphrase.
...
Modificare lo script Bash seguente con i valori appropriati per il proprio ambiente ed eseguirlo dal computer client. Poiché il metodo di distribuzione Gestione dei servizi distribuisce le macchine virtuali in modo seriale, sono necessari alcuni minuti per distribuire le otto macchine virtuali A9 suggerite in questo script.
