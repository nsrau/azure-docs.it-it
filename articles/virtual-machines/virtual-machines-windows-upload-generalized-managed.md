---
title: Creare una macchina virtuale di Azure gestita da dischi virtuali locali generalizzati | Microsoft Docs
description: Creare una macchina virtuale gestita in Azure usando dischi rigidi virtuali caricati in locale e usare i dischi gestiti nel modello di distribuzione di Resource Manager.
...
# <a name="create-a-new-vm-from-a-generalized-vhd-uploaded-to-azure-using-managed-disks"></a>Creare una nuova macchina virtuale da un disco rigido virtuale generalizzato e caricato in Azure usando Managed Disks

È possibile creare una nuova macchina virtuale in Azure caricando un disco rigido virtuale esportato da uno strumento di virtualizzazione locale o da un altro cloud. Usando [Managed Disks](../storage/storage-managed-disks-overview.md) per la nuova macchina virtuale è possibile semplificarne la gestione e ottenere una maggiore disponibilità posizionando la macchina virtuale in un set di disponibilità. Se si sta caricando un disco rigido virtuale che verrà usato per creare più macchine virtuali di Azure, è innanzitutto necessario generalizzare il disco rigido virtuale mediante un'utilità [Sysprep](virtual-machines-windows-generalize-vhd.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json). Sysprep consente di rimuovere eventuali informazioni specifiche e dati personali dell'account sul computer dal disco rigido virtuale. 
...
> [!IMPORTANT]
> Rivedere l'articolo [Plan for the migration to Managed Disks](virtual-machines-windows-on-prem-to-azure.md#plan-for-the-migration-to-managed-disks) (Piano per la migrazione a Managed Disks) prima di avviare la migrazione a [Managed Disks](../storage/storage-managed-disks-overview.md).
>
