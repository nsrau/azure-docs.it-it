---
title: Usare SSH in Windows per connettersi a macchine virtuali Linux | Microsoft Docs
...
> [!NOTE]
> Se si prevede di gestire il servizio distribuito con il modello di distribuzione classica, è inoltre possibile creare un file in formato **cer** da caricare nel portale, anche se ciò non riguarda **ssh** o la connessione a macchine virtuali Linux, ovvero l'oggetto di questo articolo. Per creare tali file in Windows, digitare: <br /> openssl.exe x509 -outform der -in myCert.pem -out myCert.cer
> 
> 

## Ottenere ssh-keygen e openssl in Windows
[In questa sezione](#What-SSH-and-key-creation-programs-do-you-need) precedente sono elencate diverse utilità che includono `ssh-keygen` e `openssl` per Windows. Ecco alcuni esempi:

### Usare Git per Windows
...
2. Eseguire Git Bash dal menu Start > Tutte le app > Git Shell

> [!NOTE]
> Quando si eseguono i comandi `openssl` precedenti potrebbe verificarsi il seguente errore:
> 
> 
