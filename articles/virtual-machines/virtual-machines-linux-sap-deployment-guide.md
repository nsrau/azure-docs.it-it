...
[planning-guide-2.2]: virtual-machines-linux-sap-planning-guide.md#f5b3b18c-302c-4bd8-9ab2-c388f1ab3d10 "Cross-premise: distribuzione di una singola VM o di più VM SAP in Azure che devono essere completamente integrate con la rete locale"
...
* Pagina di [SCN](https://wiki.scn.sap.com/wiki/display/HOME/SAPonLinuxNotes) contenente tutte le note su SAP necessarie per Linux
...
* **osType**: sistema operativo che si vuole distribuire, ad esempio Windows Server 2012 R2 o SLES 12
...
> ![Linux][Logo_Linux] Linux
>
> <https://azure.microsoft.com/it-IT/documentation/articles/virtual-machines-linux-agent-user-guide/>
...
Le impostazioni proxy in /etc/waagent.conf si applicano anche alle estensioni di VM necessarie. Se si vogliono usare i repository di Azure, verificare che il traffico verso i repository non attraversi la Intranet locale. Se sono state create route definite dall'utente per abilitare il tunneling forzato, verificare di aggiungere una route che indirizza il traffico verso 104.45.17.148 direttamente in Internet e non tramite la connessione da sito a sito. È anche possibile aggiungere route per gli indirizzi IP elencati in /etc/regionserverclnt.cfg. Un esempio è riportato nello screenshot seguente. Per altri dettagli sulle route definite dall'utente, vedere [questo articolo][virtual-networks-udr-overview].
...
Per installare l'estensione di monitoraggio avanzato di Azure per SAP in una VM Linux da un PC desktop/portatile Linux, seguire la procedura descritta in [questo articolo][install-extension-cli].
