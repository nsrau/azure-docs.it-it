...
<p>Verrà creato un cluster multi-Master [Galera](http://galeracluster.com/products/) di [MariaDBs](https://mariadb.org/en/about/), una sostituzione pronta per l’uso robusta, scalabile ed affidabile di MySQL, da usare in un ambiente altamente disponibile in Macchine virtuali di Azure.</p>
...
![Architettura](./media/virtual-machines-mariadb-cluster/Setup.png)
...
9.  Ottimizzare le prestazioni del sistema. Per altre informazioni, fare riferimento a questo articolo sulla [strategia di ottimizzazione delle prestazioni]
	...
	> [AZURE.NOTE]**innodb_buffer_pool_size** dovrebbe essere il 70% della memoria della macchina virtuale. Qui è stato impostato su 2,45 GB per la macchina virtuale di Azure di dimensione Media con 3,5 GB di RAM.
...
11. Acquisire la macchina virtuale tramite il portale. (Attualmente il [problema noto n. 1268 nell’interfaccia della riga di comando di Azure] descrive il fatto che le immagini acquisite dagli strumenti di tale interfaccia non acquisiscono i dischi dati collegati.)
    ...
    - Fare clic su Acquisisci e specificare il nome dell'immagine come **mariadb-galera-image**, fornire una descrizione e selezionare "Il comando "waagent -deprovision" è stato eseguito". ![Acquisizione della macchina virtuale](./media/virtual-machines-mariadb-cluster/Capture.png)![Acquisizione della macchina virtuale](./media/virtual-machines-mariadb-cluster/Capture2.PNG)
...
3. Per il passaggio successivo è necessario ottenere l'indirizzo IP interno di ognuna delle 3 macchine virtuali:
	...
	![Recupero dell'indirizzo IP virtuale](./media/virtual-machines-mariadb-cluster/IP.png)
...
![Modificare l'endpoint](./media/virtual-machines-mariadb-cluster/Endpoint.PNG)

fare clic su Riconfigura il set con carico bilanciato e andare avanti

![Riconfigurare il set con bilanciamento del carico](./media/virtual-machines-mariadb-cluster/Endpoint2.PNG)

quindi modificare Intervallo di probe in 5 secondi e salvare

![Modificare l'intervallo di probe](./media/virtual-machines-mariadb-cluster/Endpoint3.PNG)
...
[riferimento all’interfaccia della riga di comando di Azure]: http://azure.microsoft.com/documentation/articles/virtual-machines-command-line-tools/
