...
1. Installare la versione più recente di Python 2.7 o 3.4 dal sito [python.org][].
1. Installare i pacchetti wfastcgi e django usando pip.
...
1. Abilitare WFastCGI. In questo modo si aggiungerà un'applicazione al file applicationhost.config globale che si riferisce all'interprete Python eseguibile e allo script wfastcgi.py.
...
1. Creare un file web.config in *C:\\inetpub\\wwwroot\\helloworld*. Il valore dell'attributo `scriptProcessor` deve corrispondere all'output del passaggio precedente. Vedere la pagina relativa a [wfastcgi][] su pypi per altre informazioni sulle impostazioni wfastcgi.
...
1. Aggiornare la posizione del sito Web predefinito IIS in modo che punti alla cartella del progetto django.
