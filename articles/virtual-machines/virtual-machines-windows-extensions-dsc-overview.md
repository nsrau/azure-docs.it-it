...
L'agente di macchine virtuali di Azure e le relative estensioni associate fanno parte dei servizi di infrastruttura di Microsoft Azure. Le estensioni della VM sono componenti software che ne estendono la funzionalità e ne semplificano varie operazioni di gestione. Ad esempio, l'estensione VMAccess permette di reimpostare la password di un amministratore e l'estensione script personalizzata permette di eseguire uno script nella VM.
...
**Configuration Arguments** (Argomenti di configurazione): se la funzione di configurazione accetta argomenti, immetterli qui nel formato `argumentName1=value1,argumentName2=value2`. Si noti che questo formato è diverso rispetto a quello in cui vengono accettati gli argomenti di configurazione tramite i cmdlet di PowerShell o i modelli di Azure Resource Manager.
...
Per altre informazioni sul passaggio di parametri sensibili nelle configurazioni, vedere l'articolo [Passaggio di credenziali al gestore estensione DSC di Azure](virtual-machines-windows-extensions-dsc-credentials.md).
