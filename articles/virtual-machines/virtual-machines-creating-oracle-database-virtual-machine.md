...
3.	Selezionare una delle immagini Oracle Database disponibili**: versione 11g, versione 12c, Standard Edition, Enterprise Edition oppure uno dei raggruppamenti di opzioni comuni o avanzate.** Esaminare le informazioni sull'immagine selezionata (come le dimensioni minime consigliate), quindi fare clic su **Avanti**.
...
7.	Fare clic su **Start di Windows**, quindi digitare **Assistente configurazione database**. Fare clic sull’icona di **Assistente configurazione database**.
...
8.	All'interno della procedura guidata di **Assistente configurazione database**, fornire i valori in base alle esigenze per ciascun passaggio della finestra di dialogo:
	...
	1.	**Passaggio 1:** fare clic su **Crea database**, quindi su **Avanti**.

		![](media/virtual-machines-creating-oracle-database-virtual-machine/image5.png)

	2. **Passaggio 2:** immettere un valore per **Nome globale del database**. Immettere e confermare un valore per **Password amministrativa**: questa sarà la password per l’utente **SISTEMA** del database Oracle. Deselezionare l'opzione **Crea come database contenitore**. Fare clic su **Avanti**.

		![](media/virtual-machines-creating-oracle-database-virtual-machine/image6.png)

	3. **Passaggio 3:** la verifica dei prerequisiti dovrebbe procedere automaticamente, avanzando al **Passaggio 4**.
	
	4. **Passaggio 4:** esaminare le opzioni di **Crea database - Riepilogo**, quindi fare clic su **Fine**.

		![](media/virtual-machines-creating-oracle-database-virtual-machine/image7.png)
	5. **Passaggio 5:** nella **pagina di avanzamento** verrà segnalato lo stato di creazione del database.

		![](media/virtual-machines-creating-oracle-database-virtual-machine/image8.png)
	6. Dopo aver creato il database, sarà possibile scegliere di utilizzare la finestra di dialogo **Gestione password**. Modificare le impostazioni della password se necessario per le proprie esigenze, quindi chiudere le finestre di dialogo per uscire dalla procedura guidata **Assistente configurazione database**.

##Conferma dell’installazione del database

1.	Dopo aver effettuato l’accesso alla macchina virtuale, avviare un prompt dei comandi SQL Plus: fare clic su **Start di Windows**, quindi digitare **SQL Plus**. Fare clic sull’icona di **SQL Plus**.
...
3.	Nel prompt dei comandi, eseguire:

		lsnrctl start

(È possibile eseguire `lsnrctl status` per verificare lo stato del listener. Quando si desidera interrompere il listener, è possibile eseguire `lsnrctl stop`.)
...
8.	Specificare un nome per l'endpoint.
	...
	2. Utilizzare **1521** per la porta pubblica.
	
	3. Utilizzare **1521** per la porta privata.
