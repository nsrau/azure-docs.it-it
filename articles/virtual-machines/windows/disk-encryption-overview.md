...
Le macchine virtuali Windows sono disponibili in un [intervallo di dimensioni](sizes-general.md). Crittografia dischi di Azure non è disponibile nelle [VM Basic, serie A](/pricing/details/virtual-machines/series/)o nelle macchine virtuali con meno di 2 GB di memoria.
...
Per abilitare crittografia dischi di Azure, le macchine virtuali devono soddisfare i requisiti di configurazione degli endpoint di rete seguenti:
  - Per ottenere un token per la connessione all'insieme di credenziali delle chiavi, la macchina virtuale Windows deve essere in grado di connettersi a un endpoint Azure Active Directory, @no__t -0login. microsoftonline. com @ no__t-1.
...
- [Scenari di crittografia dischi di Azure nelle macchine virtuali Windows](disk-encryption-windows.md)
