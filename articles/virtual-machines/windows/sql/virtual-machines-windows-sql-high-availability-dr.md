...
* [Istanze del cluster di failover AlwaysOnAlways On Failover Cluster Instances](https://technet.microsoft.com/library/ms189134.aspx)
...
| --- | --- |
| **Gruppi di disponibilitàAvailability groups** |Le repliche di disponibilità in esecuzione nelle macchine virtuali di Azure nella stessa area assicurano disponibilità elevata.  È necessario configurare una macchina virtuale da usare come controller di dominio perché il servizio Clustering di failover di Windows richiede un dominio di Active Directory.<br/><br/> Per una maggiore ridondanza e disponibilità, le macchine virtuali di Azure possono essere distribuite in zone di [disponibilità](../../../availability-zones/az-overview.md) diverse, come documentato nella panoramica del gruppo di [disponibilità.](virtual-machines-windows-portal-sql-availability-group-overview.md) Se le macchine virtuali di SQL Server in un gruppo di disponibilità vengono distribuite in zone di disponibilità, usare il servizio di [bilanciamento del carico standard](../../../load-balancer/load-balancer-standard-overview.md) per il listener come documentato in questi articoli, ovvero Modelli di Guida[introduttiva di Azure](virtual-machines-windows-sql-availability-group-quickstart-template.md)VM [CAUTO.](virtual-machines-windows-sql-availability-group-cli.md) & <br/> ![Gruppi di disponibilità](./media/virtual-machines-windows-sql-high-availability-dr/azure-only-ha-always-on.png)<br/>Per altre informazioni, vedere [Configurare manualmente un gruppo di disponibilità Always On nelle macchine virtuali di Azure tramite Resource Manager](virtual-machines-windows-portal-sql-alwayson-availability-groups.md). |
| **Istanze del cluster di failover** |Le istanze del cluster di failover (FCI), che richiedono l'archiviazione condivisa, possono essere create in 4 modi diversi.<br/><br/>1. Un cluster di failover a due nodi in esecuzione nelle macchine virtuali di Azure con archiviazione collegata tramite [Windows Server \(2016 Storage Spaces Direct S2D\) ](virtual-machines-windows-portal-sql-create-failover-cluster.md) per fornire una SAN virtuale basata su software.<br/><br/> 2. Un cluster di failover a due nodi in esecuzione nelle macchine virtuali di Azure usando la condivisione [file Premium](virtual-machines-windows-portal-sql-create-failover-cluster-premium-file-share.md). Le condivisioni file Premium sono condivisioni file con livello SSD con costante e bassa latenza supportate, completamente supportate per l'utilizzo con l'istanza del cluster di failover.<br/><br/>3. Un cluster di failover a due nodi in esecuzione nelle macchine virtuali di Azure con archiviazione supportata da una soluzione di clustering di terze parti. Per un esempio specifico in cui viene utilizzato SIOS DataKeeper, vedere [Disponibilità elevata per una condivisione file tramite clustering](https://azure.microsoft.com/blog/high-availability-for-a-file-share-using-wsfc-ilb-and-3rd-party-software-sios-datakeeper/)di failover e software di terze parti SIOS DataKeeper .<br/><br/>4. Un cluster di failover a due nodi in esecuzione nelle macchine virtuali di Azure con archiviazione a blocchi condivisi di destinazione iSCSI remota tramite ExpressRoute.4. A two-node failover cluster running in Azure VMs with remote iSCSI Target shared block storage via ExpressRoute. Ad esempio, NetApp Private Storage (NPS) espone una destinazione iSCSI tramite ExpressRoute con Equinix a macchine virtuali di Azure.<br/><br/>Per le soluzioni di replica dei dati e di archiviazione condivisa di terze parti, contattare il fornitore per eventuali problemi di accesso ai dati durante il failover.<br/><br/>|
...
| **Mirroring del database** |Tutti i server principali, mirror e di controllo in esecuzione in diversi data center per il ripristino di emergenza. È necessario eseguire la distribuzione utilizzando i certificati server. SQL Server database mirroring is not supported for SQL Server 2008 nor SQL Server 2008 R2 on an Azure VM. <br/>![Mirroring del database](./media/virtual-machines-windows-sql-high-availability-dr/azure-only-dr-dbmirroring.png) |
...
## <a name="free-dr-replica-in-azure"></a>Replica di ripristino di emergenza gratuita in AzureFree DR replica in Azure

Se si dispone di [Software Assurance](https://www.microsoft.com/licensing/licensing-programs/software-assurance-default?rtc=1&activetab=software-assurance-default-pivot:primaryr3), è possibile implementare piani di ripristino di emergenza ibrido con SQL Server senza incorrere in costi di licenza aggiuntivi per l'istanza di ripristino di emergenza passiva.

Nell'immagine seguente, il programma di installazione usa SQL Server in esecuzione in una macchina virtuale di Azure usando 12 core come replica di ripristino di emergenza per una distribuzione di SQL Server locale usando 12 core. In passato, sarebbe necessario concedere in licenza 12 core di SQL Server per la distribuzione di macchine virtuali locali e di Azure.In the past, you would need to license 12 cores of SQL Server for the on-premises and the Azure Virtual Machine deployment. Il nuovo vantaggio offre vantaggi di replica passiva in esecuzione in una macchina virtuale di Azure.The new benefit offers passive replica benefits running on an Azure Virtual Machine. A questo punto è necessario concedere in licenza solo 12 core di SQL Server in esecuzione in locale, purché vengano soddisfatti i criteri di ripristino di emergenza per la replica passiva nella macchina virtuale di Azure.Now you would need to license only 12 cores of SQL Server running on-premises as long as the disaster recovery criteria for the passive replica on Azure Virtual Machine is met.

![Replica di ripristino di emergenza gratuita in AzureFree DR replica in Azure](media/virtual-machines-windows-sql-high-availability-dr/free-dr-replica-azure.png)

Per ulteriori informazioni, vedere le [Condizioni](https://www.microsoft.com/licensing/product-licensing/products)per le licenze del prodotto . 

Per abilitare questo vantaggio, passare alla [risorsa macchina virtuale SQL Server](virtual-machines-windows-sql-manage-portal.md#access-the-sql-virtual-machines-resource), selezionare **Configura** in Impostazioni e quindi scegliere l'opzione Ripristino **di emergenza** in **Licenza SQL Server**. Selezionare la casella di controllo per confermare che questa macchina virtuale di SQL Server verrà usata come replica passiva e quindi selezionare **Applica** per salvare le impostazioni. 

![Configurare la replica di ripristino di emergenza in AzureConfigure DR replica in Azure](media/virtual-machines-windows-sql-high-availability-dr/dr-replica-in-portal.png)
...
* [Utilizzo di stringhe di connessione per il mirroring del database con i gruppi di disponibilitàUsing Database-Mirroring Connection Strings with Availability Groups](https://technet.microsoft.com/library/hh213417.aspx)
