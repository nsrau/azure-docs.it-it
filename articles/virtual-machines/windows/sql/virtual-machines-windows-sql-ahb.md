---
title: Cambiare il modello di licenza per le VM di SQL Server in Azure
description: Informazioni su come cambiare le licenze per una macchina virtuale di SQL Server in Azure dal pagamento in base al consumo al modello Bring Your Own License tramite Vantaggio Azure Hybrid.
...
ms.locfileid: "83771435"
---
# <a name="change-the-license-model-for-a-sql-server-virtual-machine-in-azure"></a>Cambiare il modello di licenza per una macchina virtuale di SQL Server in Azure
Questo articolo descrive come cambiare il modello di licenza per una macchina virtuale di SQL Server in Azure usando il nuovo provider di risorse VM di SQL, **Microsoft.SqlVirtualMachine**.

Sono disponibili tre modelli di licenza per una VM che ospita SQL Server: pagamento in base al consumo, Vantaggio Azure Hybrid e ripristino di emergenza. È possibile modificare il modello di licenza delle VM di SQL Server tramite il portale di Azure, l'interfaccia della riga di comando di Azure o PowerShell. 
...
- Il tipo di licenza **ripristino di emergenza** viene usato per la [replica DR gratuita](virtual-machines-windows-sql-high-availability-dr.md#free-dr-replica-in-azure) in Azure. 
...
- Installare autonomamente SQL Server in una macchina virtuale di Azure, [eseguire manualmente la registrazione con il provider di risorse VM di SQL](virtual-machines-windows-sql-register-with-resource-provider.md) e attivare Vantaggio Azure Hybrid.
...
- Una [VM di SQL Server](https://docs.microsoft.com/azure/virtual-machines/windows/sql/virtual-machines-windows-portal-sql-server-provision) registrata con il [provider di risorse VM di SQL](virtual-machines-windows-sql-register-with-resource-provider.md).
...
1. Aprire il [portale di Azure](https://portal.azure.com) e accedere alle risorse [macchine virtuali di SQL](virtual-machines-windows-sql-manage-portal.md#access-the-sql-virtual-machines-resource) per la propria VM di SQL Server. 
...
È consentito scegliere l'installazione automatica di SQL Server in una macchina virtuale di Azure solo tramite Vantaggio Azure Hybrid. È necessario [registrare queste VM con il provider di risorse VM di SQL](virtual-machines-windows-sql-register-with-resource-provider.md) impostando la licenza di SQL Server come Vantaggio Azure Hybrid per indicare l'utilizzo di Vantaggio Azure Hybrid in base alle condizioni per l'utilizzo di prodotti Microsoft.
...
Sarà necessario registrare la sottoscrizione e quindi [la VM di SQL Server con il provider di risorse](virtual-machines-windows-sql-register-with-resource-provider.md). 
...
* [Domande frequenti su SQL Server in una macchia virtuale Windows](virtual-machines-windows-sql-server-iaas-faq.md)
* [Informazioni sui prezzi di SQL Server in una macchina virtuale Windows](virtual-machines-windows-sql-server-pricing-guidance.md)
* [Note sulla versione di SQL Server in una macchina virtuale Windows](virtual-machines-windows-sql-server-iaas-release-notes.md)
