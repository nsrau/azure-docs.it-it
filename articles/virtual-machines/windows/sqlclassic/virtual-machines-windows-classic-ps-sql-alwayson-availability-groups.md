...
5. Creare un account di archiviazione associato al set di affinità creato e impostarlo come account di archiviazione corrente nella sottoscrizione.
...
4. Al termine del provisioning di ogni macchina virtuale, scaricare il relativo file del desktop remoto nella directory di lavoro. Tramite i cicli for vengono scorse le tre nuove macchine virtuali e vengono eseguiti i comandi all'interno delle parentesi graffe di livello principale per ognuna di esse.
...
9. Abilitare i gruppi di disponibilità AlwaysOn per le istanze predefinite di SQL Server in **ContosoSQL1** e **ContosoSQL2**.
