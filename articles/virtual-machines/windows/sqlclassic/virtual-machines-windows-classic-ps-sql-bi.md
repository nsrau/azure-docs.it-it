...
| --- | --- | --- |
| **Modalità nativa di Reporting Services** |Yes |Installata ma richiede la configurazione, inclusi l'URL di gestione report. Vedere la sezione [Configurare Reporting Services](#configure-reporting-services). |
...
| **Modalità multidimensionale e di Data mining di Analysis Services (OLAP)** |Yes |Installata e configurata come istanza di Analysis Services predefinita |
...
* Arrestare o disinstallare i servizi che non si intende utilizzare. Ad esempio se la macchina virtuale viene utilizzata solo per Reporting Services, arrestare o disinstallare Analysis Services e SQL Server Integration Services. L'immagine seguente è un esempio dei servizi che vengono avviati per impostazione predefinita.
  ...
  > Il motore del database di SQL Server è necessario negli scenari BI supportati. Nella topologia di macchina virtuale con un solo server, il motore del database deve essere in esecuzione nella macchina virtuale stessa.
    ...
    Per ulteriori informazioni, vedere quanto segue: [Disinstallare Reporting Services](https://msdn.microsoft.com/library/hh479745.aspx) e [disinstallare un'istanza di Analysis Services](https://msdn.microsoft.com/library/ms143687.aspx).
...
* Analysis Services, Reporting Services e il motore di Database SQL Server in una singola macchina virtuale. Questa distribuzione include i database del server di report.
* Origine dati è il database SQL di Azure.
...
2. Passare a http: \/ /localhost/report sulla macchina virtuale.
...
* **Generatore report**: La macchina virtuale è inclusa la-versione ClickOnce di Generatore Report di Microsoft SQL Server per SQL 2014 e 2012. Per avviare il Generatore report per la prima volta sulla macchina virtuale con SQL 2016:
...
* **SQL Server Data Tools**: VM:  SQL Server Data Tools è installato nella macchina virtuale e può essere utilizzato per creare **progetti Server di Report** e i report nella macchina virtuale. SQL Server Data Tools è in grado di pubblicare i report nel server di report sulla macchina virtuale.
...
* SQL Server: Vedere la sezione "Passaggi di configurazione completi per connettersi alla macchina virtuale mediante SQL Server Management Studio" del [il Provisioning di una macchina virtuale di SQL Server in Azure](../sql/virtual-machines-windows-portal-sql-server-provision.md).
