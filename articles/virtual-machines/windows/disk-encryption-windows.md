---
title: Scenari di crittografia dischi di Azure nelle macchine virtuali Windows
...
ms.locfileid: "73063997"
---
# <a name="azure-disk-encryption-scenarios-on-windows-vms"></a>Scenari di crittografia dischi di Azure nelle macchine virtuali Windows

Crittografia dischi di Azure usa la protezione con chiave esterna BitLocker per fornire la crittografia del volume per il sistema operativo e i dischi dati delle macchine virtuali (VM) di Azure ed è integrato con Azure Key Vault per facilitare il controllo e la gestione delle chiavi e dei segreti di crittografia del disco. Per una panoramica del servizio, vedere [crittografia dischi di Azure per macchine virtuali Windows](disk-encryption-overview.md).

Esistono molti scenari di crittografia dei dischi e i passaggi possono variare in base allo scenario. Le sezioni seguenti illustrano in modo più dettagliato gli scenari per le macchine virtuali Windows.
...
### <a name="enable-encryption-on-existing-or-running-vms-with-azure-powershell"></a>Abilitare la crittografia nelle macchine virtuali esistenti o in esecuzione con Azure PowerShell 
...
|  keyEncryptionKeyURL | URL della chiave di crittografia della chiave, nel formato https://&lt;nome dell'insieme di credenziali delle chiavi&gt;. vault.azure.net/key/&lt;chiave-Name&gt;. Se non si vuole usare un KEK, lasciare vuoto questo campo. |
...
## <a name="new-iaas-vms-created-from-customer-encrypted-vhd-and-encryption-keys"></a>Nuove VM IaaS create da chiavi di crittografia e VHD crittografati dal cliente
