...
1. Rimuovere qualsiasi route statica persistente nella tabella di routing:
   ...
   * Controllare le `Persistence Routes` sezioni. Se è presente una route persistente, utilizzare `route delete` il comando per rimuoverla.
...
3. Impostare i criteri SAN del disco [`Onlineall`](https://technet.microsoft.com/library/gg252636.aspx)su:
...
4. Imposta l'ora UTC (Coordinated Universal Time) per Windows. Impostare anche il tipo di avvio del servizio ora di Windows`w32time`() `Automatic`su:
...
5. Impostare il profilo di alimentazione su prestazioni elevate:
    ...
    ```
6. Verificare che le variabili `TEMP` di ambiente `TMP` e siano impostate sui rispettivi valori predefiniti:
...
Assicurarsi che ciascuno dei seguenti servizi di Windows sia impostato sui valori predefiniti di Windows, Questi servizi sono i minimi che è necessario configurare per garantire la connettività della macchina virtuale. Eseguire i comandi seguenti per reimpostare le impostazioni di avvio:
...
>[!NOTE] 
>È possibile che venga visualizzato un messaggio di errore `Set-ItemProperty -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\Terminal Services -Name <object name> -Value <value>`quando si esegue. È possibile ignorare questo messaggio. Significa solo che il dominio non esegue il push della configurazione tramite un oggetto Criteri di gruppo.
...
5. Verificare che nessun'altra applicazione usi la porta 3389. Questa porta viene utilizzata per il servizio RDP in Azure. Per visualizzare le porte usate nella macchina virtuale, eseguire `netstat -anob`:
...
Non tutti i ruoli o le applicazioni installate in un computer basato su Windows supportano immagini generalizzate. Quindi, prima di eseguire questa procedura, assicurarsi che Sysprep supporti il ruolo del computer. Per ulteriori informazioni, vedere [supporto Sysprep per i ruoli del server](https://msdn.microsoft.com/windows/hardware/commercialize/manufacture/desktop/sysprep-support-for-server-roles).
