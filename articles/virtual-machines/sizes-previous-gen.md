---
title: 'Dimensioni delle macchine virtuali Linux in Azure: generazioni precedenti | Microsoft Docs'
description: Elenca le generazioni di dimensioni precedenti disponibili per le macchine virtuali Linux in Azure. Elenca informazioni sul numero di vCPU, dei dischi dati e delle schede di rete, nonché sulla velocità effettiva di archiviazione e sulla larghezza di banda della rete per le dimensioni di queste serie.
...
ms.locfileid: "79476570"
---
# <a name="previous-generations-of-virtual-machine-sizes"></a>Generazioni precedenti delle dimensioni delle macchine virtuali

In questa sezione vengono fornite informazioni sulle generazioni precedenti di dimensioni delle macchine virtuali. Queste dimensioni possono ancora essere usate, ma sono disponibili generazioni più recenti.
...
<sup>1</sup> La massima velocità effettiva del disco (IOPS o MBps) possibile con una macchina virtuale serie Fs può essere limitata dal numero, dalle dimensioni e dallo striping dei dischi collegati.  Per informazioni dettagliate, vedere Progettazione per prestazioni elevate per [Windows](windows/premium-storage-performance.md) o [Linux](linux/premium-storage-performance.md).  

## <a name="nvv2-series"></a>Serie NVv2

**Raccomandazione di nuova dimensione**: [Serie NVv3](nvv3-series.md)
...
In questa sezione vengono fornite informazioni sulle generazioni precedenti di dimensioni delle macchine virtuali. Queste dimensioni sono ancora supportate, ma non riceveranno capacità aggiuntiva. Ci sono dimensioni più recenti o alternative che sono generalmente disponibili. Fare riferimento a [Dimensioni per macchine virtuali Linux in Azure](linux/sizes.md) per scegliere le dimensioni delle macchine virtuali più adatte alle proprie esigenze.  

Per altre informazioni sul ridimensionamento di una macchina virtuale Linux, vedere [Ridimensionare una macchina virtuale Linux.For](linux/change-vm-size.md)more information on resizing a Linux VM, see Resize a Linux VM .  

<br>

### <a name="basic-a"></a>Basic A  

**Raccomandazione di nuova dimensione**: [Av2-serie](av2-series.md)
...
| Dimensione | vCPU | Memoria: GiB | Spazio di archiviazione temp (HDD): GiB | Numero massimo di dischi dati | Velocità effettiva del disco di dati max: IOPS | NiCs max/Larghezza di banda di rete prevista (Mbps) |
| --- | --- | --- | --- | --- | --- | --- |
| &nbsp;<sup>Standard_A0 1</sup> | 1 | 0,768 | 20 | 1 | 1x500 | 2/100 |
...
**Raccomandazione di nuova dimensione**: [Serie Dv3](dv3-dsv3-series.md)
...
<sup>1</sup> VM Family può funzionare su una delle seguenti CPU: 2,2 GHz Intel Xeon® E5-2660 v2, 2,4 GHz Intel Xeon® E5-2673 v3 (Haswell) o 2,3 GHz Intel XEON® E5-2673 v4 (Broadwell)  
...
La serie DC utilizza l'ultima generazione di processore Intel XEON E-2176G a 3,7 GHz con tecnologia SGX e con la tecnologia Intel Turbo Boost può raggiungere i 4,7 GHz. 
...
> [!IMPORTANT]
>
> Le macchine virtuali serie DC sono macchine `Gen2` virtuali di generazione [2](./linux/generation-2.md#creating-a-generation-2-vm) e supportano solo immagini.


### <a name="ds-series"></a>Serie DS  

**Raccomandazione di nuova dimensione**: [Serie Dsv3](dv3-dsv3-series.md)
...
<sup>1</sup> La massima velocità effettiva del disco (IOPS o MBps) possibile con una VM serie DS può essere limitata dal numero, dalle dimensioni e dallo striping dei dischi collegati.  Per informazioni dettagliate, vedere Progettazione per prestazioni elevate per [Windows](windows/premium-storage-performance.md) o [Linux](linux/premium-storage-performance.md).
<sup>2</sup> VM Family può funzionare su una delle seguenti CPU: 2,2 GHz Intel Xeon® E5-2660 v2, 2,4 GHz Intel Xeon® E5-2673 v3 (Haswell) o 2,3 GHz Intel XEON® E5-2673 v4 (Broadwell)  
...
| Dimensione | vCPU | Memoria (GiB) | Spazio di archiviazione temp (GiB) | Numero massimo di dischi dati | Velocità effettiva di archiviazione temporanea massima (IOPS/MBps)Max temp storage throughput (IOPS/MBps) | Velocità effettiva massima del disco non memorizzato nella cache (IOPS/MBps)Max uncached disk throughput (IOPS/MBps) | NiCs max/Larghezza di banda di rete prevista (Mbps) |
|---|---|---|---|---|---|---|---|
...
La massima velocità effettiva del disco possibile con le macchine virtuali serie Ls può essere limitata dal numero, dalle dimensioni e dallo striping dei dischi collegati. Per informazioni dettagliate, vedere Progettazione per prestazioni elevate per [Windows](windows/premium-storage-performance.md) o [Linux](linux/premium-storage-performance.md).
...
| Dimensione | vCPU | Memoria: GiB | GiB di archiviazione temp (unità SSD) | Numero massimo di dischi dati | Velocità effettiva massima di archiviazione temporanea e nella cache: IOPS/MBps (dimensioni della cache in GiB) | Velocità effettiva massima disco non memorizzato nella cache: IOPS/MBpsMax uncached disk throughput: IOPS/MBps | NiCs max/Larghezza di banda di rete prevista (Mbps) |
|---|---|---|---|---|---|---|---|
...
<sup>1</sup> La massima velocità effettiva del disco (IOPS o MBps) possibile con una VM serie GS può essere limitata dal numero, dalle dimensioni e dallo striping dei dischi collegati. Per informazioni dettagliate, vedere Progettazione per prestazioni elevate per [Windows](windows/premium-storage-performance.md) o [Linux](linux/premium-storage-performance.md).
