...
Informazioni su come configurare un cluster Linux RDMA in Azure con macchine virtuali di dimensioni A8 e A9 per eseguire applicazioni MPI (Message Passing Interface) in parallelo. Quando le VM di Azure con dimensioni A8 e A9 eseguono una distribuzione Linux HPC supportata e un'implementazione MPI supportata, le applicazioni MPI comunicano in modo efficiente su una rete a bassa latenza e a elevata velocità effettiva basata sulla tecnologia Accesso diretto a memoria remota (RDMA).

>[AZURE.NOTE] Il supporto di Azure Linux RDMA attualmente richiede Intel MPI Library versione 5 su VM create da SUSE Linux Enterprise Server 12 HPC o immagini HPC 6.5 o 7.1 basate su CentOS in Azure Marketplace. Per altre informazioni e considerazioni, vedere [Informazioni sulle istanze A8, A9, A10 e A11](virtual-machines-linux-a8-a9-a10-a11-specs.md).
...
*   **Intel MPI**: per personalizzare un'immagine di VM HPC SLES 12 per il cluster, è necessario scaricare e installare un runtime di Intel MPI Library 5 corrente dal [sito Intel.com](https://software.intel.com/it-IT/intel-mpi-library/). I dettagli vengono forniti più avanti in questo articolo. Per preparare l'installazione, dopo la registrazione con Intel, copiare il collegamento di download per il file.tgz per la versione appropriata di Intel MPI. Questo articolo si riferisce a Intel MPI versione 5.0.3.048.

    >[AZURE.NOTE] Se si usa l'immagine HPC CentOS 6.5 o CentOS 7.1 in Azure Marketplace per creare i nodi del cluster, è preinstallato Intel MPI versione 5.1.3.181.
...
Ora eseguire il provisioning di una VM di dimensioni A9 con un'immagine HPC SLES 12 eseguendo un comando simile al seguente:
...
* il numero della porta SSH esterna (22 in questo esempio, ovvero la porta SSH predefinita) può essere qualsiasi numero di porta valido. il numero della porta SSH interna viene impostato su 22

* Viene creato un nuovo servizio cloud nell'area di Azure specificata dal percorso. Specificare una posizione in cui sono disponibili le istanze A8 e A9.

* Attualmente il nome dell'immagine SLES 12 può essere `b4590d9e3ed742e4a1d46e5424aa335e__suse-sles-12-hpc-v20150708` o `b4590d9e3ed742e4a1d46e5424aa335e__suse-sles-12-hpc-priority-v20150708` per il supporto SUSE prioritario (sono previsti costi aggiuntivi)
...
* **Aggiornamenti**: installare gli aggiornamenti usando **zypper**. È anche possibile decidere di installare le utilità NFS.
    ...
    >[AZURE.IMPORTANT]Se è stata distribuita una VM HPC SLES 12, è consigliabile non applicare aggiornamenti del kernel, che possono causare problemi con i driver Linux RDMA.
    >
...
* **Blocco della memoria**: affinché i codici MPI blocchino la memoria disponibile per RDMA, è necessario aggiungere o modificare le impostazioni seguenti nel file /etc/security/limits.conf. Per modificare questo file è necessario l'accesso alla radice. Se è stato distribuito una VM HPC 6.5 o 7.1 basata su CentOS, le impostazioni sono già aggiunte al file.
