---
title: Cluster Linux RDMA per l'esecuzione di applicazioni MPI | Microsoft Docs
...
* **Blocco della memoria**: affinché i codici MPI blocchino la memoria disponibile per RDMA, aggiungere o modificare le impostazioni seguenti nel file /etc/security/limits.conf. Per modificare questo file è necessario l'accesso alla radice.
  ...
  > [!NOTE]
  > A scopo di test, è inoltre possibile impostare memlock su illimitato. Ad esempio: `<User or group name>    hard    memlock unlimited`. Per altre informazioni, vedere [Best Known Methods for Setting Locked Memory Size](https://software.intel.com/it-IT/blogs/2014/12/16/best-known-methods-for-setting-locked-memory-size) (Metodi noti per l'impostazione delle dimensioni della memoria bloccata).
  > 
  > 
...
3. Non generare chiavi SSH sulla macchina virtuale di cui viene effettuato il provisioning per l'acquisizione. È invece consigliabile impostare l'autenticazione basata sull'utente dopo la distribuzione del cluster. Vedere la sezione seguente.
