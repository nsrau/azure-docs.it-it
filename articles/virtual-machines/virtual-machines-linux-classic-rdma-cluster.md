...
* **HPC Pack**: creare un cluster Microsoft HPC Pack in Azure e aggiungere nodi di calcolo con supporto per RDMA che eseguono una distribuzione Linux supportata per accedere alla rete RDMA. Vedere l'articolo di [introduzione ai nodi di calcolo Linux in un cluster HPC Pack in Azure](virtual-machines-linux-classic-hpcpack-cluster.md).

## Procedura per una distribuzione di esempio nel modello classico

I passaggi seguenti mostrano come usare l'interfaccia della riga di comando di Azure per distribuire una macchina virtuale HPC SUSE Linux Enterprise Server (SLES) 12 SP1 da Azure Marketplace, personalizzarla e creare un'immagine di macchina virtuale personalizzata. Usare quindi l'immagine per lo script della distribuzione di un cluster di macchine virtuali con supporto per RDMA.

>[AZURE.TIP]Usare una procedura simile per distribuire un cluster di macchine virtuali con supporto per RDMA basate su altre immagini HPC supportate in Azure Marketplace. Alcuni passaggi possono essere leggermente diversi. Intel MPI, ad esempio, è incluso e configurato solo in alcune di queste immagini. Inoltre, se si distribuisce una macchina virtuale HPC SLES 12 anziché una macchina virtuale HPC SLES 12 SP1, è necessario aggiornare i driver RDMA. Per dettagli, vedere [Informazioni sulle istanze A8, A9, A10 e A11 a elevato utilizzo di calcolo](virtual-machines-linux-a8-a9-a10-a11-specs.md#rdma-driver-updates-for-sles-12).


### Prerequisiti

*   **Computer client**: è necessario un computer client basato su Mac, Linux o Windows per comunicare con Azure. Nella procedura si presuppone che venga usato un client Linux.
...
### Passaggio 1. Provisioning di una macchina virtuale HPC SLES 12 SP1
...
Eseguire ora il provisioning di una macchina virtuale con supporto per RDMA con un'immagine HPC SLES 12 SP1 eseguendo un comando simile al seguente:
...
* Attualmente il nome dell'immagine SLES 12 SP1 può essere `b4590d9e3ed742e4a1d46e5424aa335e__suse-sles-12-sp1-hpc-v20160824` o `b4590d9e3ed742e4a1d46e5424aa335e__suse-sles-12-sp1-hpc-priority-v20160824` per il supporto SUSE prioritario (sono previsti costi aggiuntivi).

### Passaggio 2. Personalizzazione della macchina virtuale

Quando la macchina virtuale completa il provisioning, assegnare una porta SSH alla macchina virtuale utilizzando l'indirizzo IP esterno della macchina virtuale (o nome DNS) e il numero della porta esterna configurata e personalizzarla. Per informazioni dettagliate sulla connessione, vedere [Come accedere a una macchina virtuale che esegue Linux](virtual-machines-linux-mac-create-ssh-keys.md). Eseguire i comandi come l'utente configurato nella VM, a meno che non sia necessario l'accesso alla radice per eseguire un passaggio.
...
* **Aggiornamenti**: installare gli aggiornamenti usando **zypper**. È anche possibile decidere di installare le utilità NFS.
    ...
    >[AZURE.IMPORTANT]In una macchina virtuale HPC SLES 12 SP1 è consigliabile non applicare gli aggiornamenti del kernel, che possono causare problemi con i driver Linux RDMA.
...
* **Blocco della memoria**: affinché i codici MPI blocchino la memoria disponibile per RDMA, aggiungere o modificare le impostazioni seguenti nel file /etc/security/limits.conf. Per modificare questo file è necessario l'accesso alla radice.
    ...
    >[AZURE.NOTE]A scopo di test, è inoltre possibile impostare memlock su illimitato. Ad esempio: `<User or group name>    hard    memlock unlimited`. Per altre informazioni, vedere [Best Known Methods for Setting Locked Memory Size](https://software.intel.com/it-IT/blogs/2014/12/16/best-known-methods-for-setting-locked-memory-size) (Metodi noti per l'impostazione delle dimensioni della memoria bloccata).

* **Chiavi SSH per VM SLES **: generare chiavi SSH per stabilire una relazione di trust per il proprio account utente tra i nodi di calcolo del cluster SLES quando vengono eseguiti processi MPI. Se è stata distribuita una VM HPC basata su CentOS, non seguire questo passaggio. Vedere le istruzioni più avanti nell'articolo per configurare un trust SSH senza password tra i nodi del cluster dopo aver acquisito l'immagine e distribuito il cluster.
    ...
    >[AZURE.NOTE]La configurazione di `StrictHostKeyChecking no` può creare un potenziale rischio di sicurezza quando non viene specificato un intervallo o un indirizzo IP specifico.
...
### Passaggio 3. Acquisire l'immagine
...
### Passaggio 4. Distribuzione di un cluster con l'immagine
...
## Considerazioni per un cluster HPC CentOS
...
1. Intel MPI è già installato in una macchina virtuale con provisioning da un'immagine HPC basata su CentOS.

2. Le impostazioni di blocco della memoria sono già state aggiunte al file /etc/security/limits.conf della macchina virtuale.

2. Non generare chiavi SSH sulla macchina virtuale di cui viene effettuato il provisioning per l'acquisizione. È invece consigliabile impostare l'autenticazione basata sull'utente dopo la distribuzione del cluster. Vedere la sezione seguente.

### Impostare il trust SSH senza password nel cluster
...
* Crea una coppia di chiavi pubblica e privata per ogni nodo del cluster, incluso il nodo host, e crea una voce nel file authorized\_keys.
...
## Eseguire MPI su un cluster di base a due nodi
