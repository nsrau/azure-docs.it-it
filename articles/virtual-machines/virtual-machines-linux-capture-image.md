---
title: Acquisire una VM Linux da usare come modello | Microsoft Docs
description: 'Informazioni su come acquisire e '
;generalizzare";: ''
l'immagine: ''
di: ''
una: ''
...
Utilizzare l'interfaccia della riga di comando di Azure (CLI) per acquisire e generalizzare una macchina virtuale Azure che esegue Linux. Successivamente, è possibile utilizzarla come modello di Azure Resource Manager per creare altre macchine virtuali. Questo modello specifica il disco del sistema operativo e i dischi dati collegati alla macchina virtuale. Non include le risorse di rete virtuale necessarie per creare una VM di Azure Resource Manager. Pertanto, in genere è necessario configurare tali risorse separatamente prima di creare un'altra macchina virtuale che utilizza il modello.

> [!TIP]
> Se si desidera creare l'immagine di una VM Linux e caricarla in Azure in modo da poterla utilizzare per creare altre VM, vedere l'argomento su [come caricare e creare una VM da un'immagine del disco personalizzata](virtual-machines-linux-upload-vhd.md).
> 
> 

## Prima di iniziare
Questa procedura presuppone che sia già stata creata una VM di Azure nel modello di distribuzione di Gestione risorse e che sia stato configurato il sistema operativo, incluse le connessioni di dischi dati e l'applicazione di altre personalizzazioni, ad esempio l'installazione di applicazioni. È possibile configurare le VM in diversi modi, ad esempio tramite l'interfaccia della riga di comando di Azure. Se non è ancora stato fatto, vedere queste istruzioni per l'uso dell'interfaccia della riga di comando di Azure nella modalità Gestione risorse di Azure:
...
2. Nella finestra di SSH digitare il comando seguente. L'output di **waagent** può variare leggermente, in base alla versione dell'utilità:
   ...
   * Elimina anche l'ultimo account utente (ottenuto da /var/lib/waagent) di cui è stato effettuato il provisioning e i dati associati.
     ...
     > [!NOTE]
     > Il deprovisioning elimina file e dati per "generalizzare" l'immagine. Eseguire questo comando solo su una VM che si intende acquisire come immagine. Ciò non garantisce che dall'immagine vengano cancellate tutte le informazioni sensibili o che l'immagine sia adatta per la ridistribuzione a terze parti.
     > 
     > 
...
4. Digitare **exit** per chiudere il client SSH.
   ...
   > [!NOTE]
   > I passaggi successivi presuppongono che l'[interfaccia della riga di comando di Azure sia stata già installata](../xplat-cli-install.md) nel computer client.
   > 
   > 
...
> [!TIP]
> Per trovare la posizione di un'immagine, aprire il modello di file JSON. In **storageProfile** trovare l'**uri** di **image** nel contenitore **system**. Ad esempio, l'URI dell'immagine del disco del sistema operativo è simile a `https://xxxxxxxxxxxxxx.blob.core.windows.net/system/Microsoft.Compute/Images/vhds/<your-vhd-name-prefix>-osDisk.xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx.vhd`.
> 
> 

## Distribuire una nuova VM dall'immagine acquisita
Usare ora l'immagine con un modello per creare una VM Linux. Questi passaggi mostrano come usare l'interfaccia della riga di comando di Azure e il modello di file JSON creato con il comando `azure vm capture` per creare la VM in una nuova rete virtuale.
...
* Creare una scheda di interfaccia di rete nella stessa rete virtuale o in un'altra
...
In genere si preferisce usare un modello di Gestione risorse per creare una VM dall'immagine. È però possibile creare la VM *in modo imperativo* usando il comando **azure vm create** con il parametro **-Q** (**--image-urn**). Se si utilizza questo metodo, passare anche il parametro**-d** (**--os-disk-vhd**) per specificare il percorso del file con estensione vhd del sistema operativo per la nuova VM. Questo deve trovarsi nel contenitore dei VHD dell'account di archiviazione in cui è archiviato il file VHD dell'immagine. Il comando copia automaticamente il VHD per la nuova VM nel contenitore dei VHD.
