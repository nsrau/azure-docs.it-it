...
[comment]: <> (In teoria anche le VM che eseguono database possono essere sottoposte a backup in modo coerente se il sistema DBMS supporta il servizio Copia Shadow del volume di Windows come, ad esempio, SQL Server. So using Azure VM backup could be a way to get to a restorable backup of a SAP database. However, be aware that based on Azure VM backups point-in-time restores of databases is not possible. È consigliabile, quindi, eseguire backup di database usando la funzionalità DBMS invece del backup delle VM di Azure.)
