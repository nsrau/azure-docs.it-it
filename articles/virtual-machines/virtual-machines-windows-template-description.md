...
Questo articolo descrive gli aspetti di un modello di Azure Resource Manager che si applicano alle macchine virtuali. L'articolo descrive un modello completo per la creazione di una macchina virtuale; a tale scopo sono necessarie definizioni di risorse per gli account di archiviazione, le interfacce di rete, gli indirizzi IP pubblici e le reti virtuali. Per altre informazioni su come queste risorse possono essere definite insieme, vedere [Procedura dettagliata per un modello di Resource Manager](../resource-manager-template-walkthrough.md).
...
- [dimensione](virtual-machines-windows-sizes.md)
- [nome](virtual-machines-linux-infrastructure-naming-guidelines.md) e credenziali
- disco e [impostazioni del sistema operativo](virtual-machines-windows-cli-ps-findimage.md)
...
Sebbene siano una risorsa separata, le [estensioni](virtual-machines-windows-extensions-features.md) sono strettamente legate alle VM. Le estensioni possono essere aggiunte come risorsa figlio della VM o come risorsa separata. L'esempio illustra l'aggiunta dell'[estensione Diagnostica](virtual-machines-windows-extensions-diagnostics-template.md) alle VM:
...
Sono disponibili numerose estensioni che è possibile installare in una VM, ma la più utile è probabilmente l'[estensione dello script personalizzata](virtual-machines-windows-extensions-customscript.md). Nell'esempio, al primo avvio in ogni VM viene eseguito uno script PowerShell denominato start.ps1:
...
![Ottenere informazioni sulla distribuzione](./media/virtual-machines-windows-template-description/virtual-machines-deployment-info.png)
