...
Se non si dispone di un'altra macchina virtuale nella stessa rete virtuale, è possibile crearne facilmente una nuova. Per altre informazioni, vedere [Creare una VM Linux in Azure usando l'interfaccia della riga di comando](../linux/quick-create-cli.md). Dopo aver completato i test, eliminare la macchina virtuale aggiuntiva.
...
Per altre informazioni sulla risoluzione dei problemi di accesso dell'applicazione, vedere [Risoluzione dei problemi di accesso a un'applicazione in esecuzione su una macchina virtuale di Azure](./troubleshoot-app-connection.md)