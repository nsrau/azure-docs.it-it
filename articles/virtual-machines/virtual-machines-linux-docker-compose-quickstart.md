...
Ora è possibile connettersi a WordPress direttamente nella VM dalla porta 80. Se è stato usato un modello di Resource Manager per creare la VM, provare a connettersi a `http://<dnsname>.<region>.cloudapp.azure.com` o, se la VM è stata creata usando il modello di distribuzione classica, provare a connettersi a `http://<cloudservicename>.cloudapp.net`. Viene visualizzata la schermata di avvio di WordPress, in cui è possibile completare l'installazione e iniziare a utilizzare l’applicazione.
...
* Per altri esempi di compilazione e distribuzione di app multi-contenitore, vedere il [Compose command-line reference](http://docs.docker.com/compose/reference/) (Informazioni di riferimento ai comandi di Compose) e la [guida dell'utente](http://docs.docker.com/compose/).
