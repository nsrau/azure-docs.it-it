...
In questa fase della distribuzione di una farm di SharePoint 2013 solo intranet, ai gruppi di disponibilità AlwaysOn di SQL Server in servizi di infrastruttura di Azure, configurare i due computer SQL Server e il computer del nodo principale del cluster, quindi combinarli in un cluster Windows Server.
...
3. Eseguire i comandi seguenti da prompt dei comandi di Windows PowerShell:
...
Per entrambi i computer SQL server e per il nodo di maggioranza cluster, eseguire il comando seguente a un prompt di Windows PowerShell a livello di amministratore:
...
7.	Nella pagina Punto di accesso per l'amministrazione del cluster, nella casella di testo **Nome cluster** digitare il nome del cluster, quindi fare clic su **Avanti**.
...
17.	Per aggiungere altri nodi al cluster, fare clic con il pulsante destro del mouse sul nome del cluster nel riquadro a sinistra, quindi fare clic su **Aggiungi nodo**.
