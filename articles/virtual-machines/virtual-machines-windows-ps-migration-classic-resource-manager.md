<properties
	pageTitle="Eseguire la migrazione di risorse IaaS dal modello classico al modello di Azure Resource Manager tramite Azure PowerShell"
	description="Questo articolo illustra la gestione delle funzionalità del servizio di migrazione supportate dalla piattaforma in Azure Resource Manager usando script di PowerShell"
...
## Passaggio 4: Comandi per eseguire la migrazione delle risorse IaaS
...
Dopo aver completato l'operazione di preparazione, è possibile eseguire query sullo stato di migrazione della VM e assicurarsi che lo stato delle VM sia 'Preparato'.

	$vmName = "<vm-name>"
	$vm = Get-AzureVM -ServiceName $serviceName -Name $vmName
	$migrationState = $vm.VM.MigrationState

Verificare le risorse preparate nella configurazione tramite PowerShell o il portale di Azure. Se non si è pronti per la migrazione e si vuole tornare allo stato precedente, usare il comando seguente.
...
Verificare le macchine virtuali preparate nella configurazione tramite PowerShell o il portale di Azure. Se non si è pronti per la migrazione e si vuole tornare allo stato precedente, usare il comando seguente.
