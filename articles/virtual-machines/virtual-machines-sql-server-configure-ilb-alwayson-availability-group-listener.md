<properties 
	pageTitle="Configurare un listener ILB per gruppi di disponibilità AlwaysOn in Azure"
	description="Questa esercitazione illustra in modo dettagliato la procedura per creare un listener per gruppi di disponibilità AlwaysOn in Azure usando il servizio di bilanciamento del carico interno."
...
>[AZURE.NOTE]Il portale di gestione di Azure non supporta attualmente il servizio di bilanciamento del carico interno, quindi questa funzionalità o gli endpoint non saranno visualizzati nel portale. **Get-AzureEndpoint** restituisce tuttavia un indirizzo IP interno, se nel portale è in esecuzione il servizio di bilanciamento del carico. In caso contrario, restituirà Null.
