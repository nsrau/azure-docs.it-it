...
Questo articolo presenta informazioni generali e considerazioni sull'uso delle istanze A8, A9, A10 e A11 di Azure, note anche come istanze *a elevato utilizzo di calcolo*. Le funzionalità principali di queste istanze includono:
...
>[AZURE.NOTE]Per altre informazioni sui processori, incluse estensioni supportate del set di istruzioni, vedere il sito Web Intel.com. Per informazioni dettagliate sulle capacità di archiviazione e i dischi delle macchine virtuali, vedere [Dimensioni delle macchine virtuali](virtual-machines-size-specs.md).
...
Rete back-end a 32 Gbps che supporta RDMA | Permette le comunicazioni di applicazioni a latenza minima e velocità effettiva elevata tra le istanze presenti all'interno di un singolo servizio cloud o set di disponibilità. Riservata per il solo traffico MPI.


>[AZURE.IMPORTANT]Nelle macchine virtuali A8 e A9 che eseguono Linux in IaaS, l'accesso alla rete RDMA è attualmente abilitato solo tramite applicazioni che usano Azure Linux RDMA e Intel MPI Library 5.0 su SUSE Linux Enterprise Server 12. Nelle istanze A8 e A9 che eseguono Windows Server in IaaS o PaaS, l'accesso alla rete RDMA è attualmente abilitato solo tramite applicazioni che usano l'interfaccia Microsoft Network Direct tramite Microsoft MPI (MS-MPI). Per altre informazioni sui requisiti, vedere la sezione [Accedere alla rete RDMA](#access-the-rdma-network) in questo articolo.
...
* **Gruppo di affinità**: attualmente un gruppo di affinità non è consigliato per la maggior parte delle nuove distribuzioni. Tuttavia, si noti che se si usa un gruppo di affinità che contiene le istanze di dimensioni diverse da A8 - A11, non sarà possibile usarlo anche per le istanze A8 - A11 e viceversa.
...
Per altre informazioni ed elenchi di controllo sulla distribuzione e l'uso delle istanze a elevato utilizzo di calcolo in scenari IaaS e PaaS con HPC Pack in Windows Server, vedere [Istanze con utilizzo intensivo di calcolo A8 e A9: avvio rapido con HPC Pack](https://msdn.microsoft.com/library/azure/dn594431.aspx).

## Accedere alla rete RDMA

### Accedere da macchine virtuali Linux A8 e A9

All'interno di un singolo servizio cloud o set di disponibilità, le istanze A8 e A9 possono accedere alla rete RDMA in Azure quando eseguono applicazioni MPI che usano i driver RDMA Linux per la comunicazione tra le istanze. In questo momento Azure Linux RDMA è supportato solo con [Intel MPI Library 5.0](https://software.intel.com/it-it/intel-mpi-library/).
...
Vedere la tabella seguente per i prerequisiti per applicazioni MPI Linux per l'accesso alla rete RDMA in cluster di nodi di calcolo (IaaS).

Prerequisito | Macchine virtuali (IaaS)
------------ | -------------
Sistema operativo | Immagine HPC SUSE Linux Enterprise Server 12 (SLES 12) da Azure Marketplace
MPI | Intel MPI Library 5.0

### Accedere da istanze A8 e A9 di Windows

All'interno di un singolo servizio cloud o set di disponibilità, le istanze A8 e A9 possono accedere alla rete RDMA in Azure quando eseguono applicazioni MPI che usano l'interfaccia Microsoft Network Direct per la comunicazione tra le istanze. L'interfaccia Network Direct attualmente è supportata solo dall'implementazione Microsoft di MS-MPI per Windows. Le istanze A10 e A11 non includono l'accesso alla rete RDMA.
...
---------- | ------------ | -------------
Sistema operativo | Macchine virtuali Windows Server 2012 R2 o Windows Server 2012 | Windows Server 2012 R2, Windows Server 2012 o la famiglia del sistema operativo guest Windows Server 2008 R2.
...
* Le dimensioni di macchine virtuali A8 - A11 sono disponibili solo per il livello di prezzo Standard.
...
* Per le informazioni di base sulla distribuzione e l'uso delle istanze A8 e A9 con HPC Pack in Windows, vedere [Istanze con utilizzo intensivo di calcolo A8 e A9: avvio rapido con HPC Pack](https://msdn.microsoft.com/library/azure/dn594431.aspx) e [Eseguire applicazioni MPI nelle istanze con utilizzo intensivo di calcolo A8 e A9](https://msdn.microsoft.com/library/azure/dn592104.aspx).
