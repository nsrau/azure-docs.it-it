<properties 
	pageTitle="SQL Server Business Intelligence in Macchine virtuali di Azure"
	description="In questo argomento vengono descritte le funzionalità di Business Intelligence (BI) disponibili per SQL Server in esecuzione su macchine virtuali di Azure (VM)."
...
La raccolta di macchine virtuali di Microsoft Azure include immagini che contengono installazioni SQL Server. Le edizioni di SQL Server supportate nelle immagini della raccolta sono gli stessi file di installazione che è possibile installare in computer locali e macchine virtuali. In questo argomento vengono riepilogate le funzionalità di SQL Server Business Intelligence (BI) installate nelle immagini e i passaggi di configurazione necessari dopo il provisioning di una macchina virtuale. In questo argomento vengono descritte inoltre le topologie di distribuzione supportate per le funzionalità di Business Intelligence e le procedure consigliate.
...
Per ulteriori informazioni sulla gestione delle licenze e le tariffe attuali, vedere [Domande frequenti sulle licenze di macchine virtuali](http://azure.microsoft.com/pricing/licensing-faq/%20).
...
![PowerShell](./media/virtual-machines-sql-server-business-intelligence/IC660119.gif) Il seguente script PowerShell restituisce l'elenco di immagini di Azure contenenti "SQL Server" in ImageName:
...
Per ulteriori informazioni sulle edizioni e sulle funzionalità supportate da SQL Server, vedere gli argomenti seguenti:
...
Nella tabella seguente sono riepilogate le funzionalità di Business Intelligence installate nelle immagini della raccolta comuni di Microsoft Azure Virtual Machine per SQL Server"
...
<sup>1</sup> Per ulteriori informazioni su SharePoint e le macchine virtuali di Azure, vedere [Architetture di Microsoft Azure per SharePoint 2013](https://technet.microsoft.com/library/dn635309.aspx) e [Distribuzione di SharePoint in macchine virtuali di Microsoft Azure](https://www.microsoft.com/download/details.aspx?id=34598).
...
- La dimensione minima consigliata per una macchina virtuale corrisponde ad **A3** quando si usa SQL Server Enterprise Edition. La dimensione**A4** della macchina virtuale è consigliata per distribuzioni di SQL Server Business Intelligence di Analysis Services e Reporting Services.

	Per informazioni sulle dimensioni della macchina virtuale corrente, vedere [Dimensioni delle macchine virtuali per Azure](virtual-machines-size-specs.md).

- Le procedure consigliate per la gestione del disco consistono nell'archiviare dati, log e file di backup su unità diverse da **C**: e **D**:. Ad esempio, creare i dischi dati **E**: e **F**:.

	- Il criterio di memorizzazione nella cache dell’unità per l'unità predefinita **C**: non è ottimale per l'utilizzo di dati.
...
- Arrestare o disinstallare i servizi che non si intende utilizzare. Ad esempio se la macchina virtuale viene utilizzata solo per Reporting Services, arrestare o disinstallare Analysis Services e SQL Server Integration Services. L'immagine seguente è un esempio dei servizi che vengono avviati per impostazione predefinita.
	...
	Per ulteriori informazioni, vedere gli argomenti seguenti: [Disinstallare Reporting Services](https://msdn.microsoft.com/library/hh479745.aspx) e [Disinstallare un'istanza di Analysis Services](https://msdn.microsoft.com/library/ms143687.aspx).

- Controllare **Windows Update** per nuovi "aggiornamenti importanti". Le immagini di macchina virtuale di Microsoft Azure vengono aggiornate di frequente; tuttavia gli aggiornamenti importanti potrebbero diventare disponibili su **Windows Update** dopo l'ultimo aggiornamento dell'immagine di macchina virtuale.
...
L'immagine della raccolta di macchine virtuali per SQL Server include l’istallazione della modalità nativa di Reporting Services, tuttavia, il server di report non è configurato. I passaggi descritti in questa sezione consentono di configurare il server di report di Reporting Services. Per ulteriori informazioni sulla configurazione della modalità nativa di Reporting Services, vedere [Installare il server di report di Reporting Services in modalità nativa (SSRS)](https://msdn.microsoft.com/library/ms143711.aspx).

>[AZURE.NOTE]Per contenuti simili che utilizzano script di Windows PowerShell per configurare il server di report, vedere [Utilizzare PowerShell per creare una macchina virtuale di Azure con un server di report in modalità nativa](virtual-machines-sql-server-create-native-mode-report-server-powershell.md).
...
- Connettersi alla macchina virtuale tramite connessione desktop remoto di Windows. Nell'interfaccia utente del desktop remoto:
	...
	1. Digitare **nome del servizio cloud** come nome del computer.
	
	1. Digitare due punti (:) e il numero di porta pubblica configurato per l'endpoint del desktop remoto di TCP.
		
		Myservice.cloudapp.net:63133
		
		Per altre informazioni, vedere [Informazioni sul servizio cloud](http://www.windowsazure.com/manage/services/cloud-services/what-is-a-cloud-service/).

**Avviare Gestione configurazione Reporting Services.**

1. In **Windows Server 2012**:

1. Dalla schermata **Start** digitare **Reporting Services** per visualizzare un elenco di applicazioni.

1. Fare clic con il pulsante destro del mouse su **Gestione configurazione Reporting Services** e su **Esegui come amministratore**.
...
1. Verificare che il **Nome server** sia il nome del server locale e fare clic su **Connetti**.
...
1. Verificare che lo **Stato del server di report** sia **Avviato**. Se si desidera controllare il servizio in Gestione server di Windows, il servizio è il servizio Windows**SQL Server Reporting Services**.

1. Fare clic su **Account del servizio** e modificare l'account in base alle esigenze. Se la macchina virtuale viene utilizzata in un ambiente non associato a un dominio, l’account **ReportServer** incorporato è sufficiente. Per ulteriori informazioni sull'account del servizio, vedere [Account del servizio](https://msdn.microsoft.com/library/ms189964.aspx).

1. Nel riquadro a sinistra fare clic su **URL servizio Web**.

1. Fare clic su **Applica** per configurare i valori predefiniti.

1. Tenere presente gli **URL del servizio Web del server di report**. Tenere presente che la porta TCP predefinita è 80 e che fa parte dell'URL. Successivamente si creerà un endpoint della macchina virtuale Microsoft Azure per la porta.
...
1. Accertarsi che **Crea un nuovo database del server di report** sia selezionato e quindi fare clic su Avanti.

1. Verificare **Nome server** e fare clic su **Connessione di test**.

1. Se il risultato è **Connessione di test riuscita**, fare clic su **OK** e quindi fare clic su **Avanti**.

1. Si noti che il nome del database è **ReportServer** e la **Modalità del server di report** è **Nativa** fare clic su **Avanti**.

1. Fare clic su **Avanti** sulla pagina **Credenziali**.

1. Fare clic su **Avanti** sulla pagina **Riepilogo**.

1. Fare clic su **Avanti** sulla pagina **Stato e Termine**.
...
1. Fare clic su **Microsoft SQL Server 2014** o **Microsoft SQL Server 2012** e quindi fare clic su **Strumenti di configurazione**.
...
1. Nell'installazione guidata di SQL Server, fare clic su **Installazione** nel riquadro sinistro e quindi fare clic su **Nuova installazione autonoma di SQL server o aggiunta di funzionalità a un'installazione esistente**.
	...
	- Se viene visualizzato **Sfoglia per cartelle**, passare a c:\\SQLServer\_12.0\_full o c:\\SQLServer\_11.0\_full e quindi fare clic su **Ok**.

1. Fare clic su **Avanti** nella pagina degli aggiornamenti del prodotto.

1. Nel **Tipo di installazione** selezionare **Eseguire una nuova installazione di SQL Server** e fare clic su **Avanti**.
...
Un’ **istanza predefinita** di Analysis Services è in ascolto sulla porta TCP **2383**. Aprire la porta nel firewall delle macchine virtuali. Un cluster di un'istanza denominata di Analysis Services è in ascolto sulla porta **2383**.
...
In questa sezione vengono riepilogati gli endpoint della macchina virtuale Microsoft Azure da creare e le porte da aprire nel firewall della macchina virtuale. Nella tabella seguente sono riepilogate le porte **TCP** per creare endpoint e le porte da aprire nel firewall delle macchine virtuali.
...
|---|---|---|
|**80**|TCP|Accesso remoto al server di report (*). | |**1433**|TCP|SQL Server Management Studio (*). | |**1434**|UDP|Browser SQL Server. E’ necessario quando la macchina virtuale viene associata a un dominio.|
...
- Creare endpoint:[Come configurare gli endpoint in una macchina virtuale](virtual-machines-set-up-endpoints.md).

- SQL Server: Vedere la sezione "Passaggi completi di configurazione per la connessione alla macchina virtuale mediante SQL Server Management Studio" di [Provisioning di una macchina virtuale di SQL Server in Azure](virtual-machines-provision-sql-server.md).
