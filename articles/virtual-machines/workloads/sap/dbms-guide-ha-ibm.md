...
![Panoramica della disponibilità elevata IBM Db2](./media/dbms-guide-ha-ibm/ha-db2-hadr-lb.png)
...
![Panoramica sull'ambiente completo di disponibilità elevata di IBM DB2](.//media/dbms-guide-ha-ibm/end-2-end-ha.png)
...
Per distribuire una configurazione di IBM Db2, è necessario seguire questa procedura:
  ...
  + Aggiornare SUSE Linux e configurare i sistemi di file.
...
| Fencing in Azure | Azure isolamento o isolamento tramite SBD (altamente consigliato). Non è possibile metodo per evitare situazioni di split brain. |
...
Per altre informazioni su Linux Pacemaker in Azure, vedere [configurazione di Pacemaker su SUSE Linux Enterprise Server in Azure](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse-pacemaker).

## <a name="deployment-on-suse-linux"></a>Distribuzione su SUSE Linux

L'agente delle risorse per IBM Db2 LUW è incluso in SUSE Linux Enterprise Server for SAP Applications. Per la configurazione descritta in questo documento, è necessario utilizzare Linux Server SUSE per le applicazioni SAP. Azure Marketplace contiene un'immagine per SUSE Enterprise Server for SAP Applications 12 che è possibile usare per distribuire nuove macchine virtuali di Azure. Tenere presente i vari modelli di supporto o servizio che sono offerti da SUSE tramite Azure Marketplace quando si sceglie un'immagine di macchina virtuale nella macchina virtuale di Azure Marketplace. 
...
1. Creare la macchina virtuale 1.
    + Usa SLES per immagini SAP in Azure Marketplace.
...
Per creare un cluster Pacemaker di base per questo server IBM Db2, vedere [configurazione di Pacemaker su SUSE Linux Enterprise Server in Azure][sles-pacemaker]. 
...
1. Consente di impostare HADR per IBM Db2.
   ...
   Quando si usa un dispositivo SBD per Linux Pacemaker, impostare i parametri di HADR Db2 seguenti:
...
A scopo dimostrativo e le procedure descritte in questo articolo, il database SID è **PTR**.
...
1. Modificare l'ambiente shell per db2\<sid > all'utente */bin/ksh*. È consigliabile usare lo strumento di Yast. 
...
È consigliabile configurare una condivisione NFS comune in cui vengono scritti i log da entrambi i nodi. Deve essere a disponibilità elevata nella condivisione NFS. 

È possibile usare le condivisioni NFS a disponibilità elevata esistenti per i trasporti o una directory del profilo. Per altre informazioni, vedere:

- [Disponibilità elevata per NFS in macchine virtuali di Azure su SUSE Linux Enterprise Server][nfs-ha] 
- [Disponibilità elevata per SAP NetWeaver in macchine virtuali di Azure su SUSE Linux Enterprise Server con file di NetApp Azure for SAP Applications](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse-netapp-files)
...
Questa sezione descrive come è possibile testare la configurazione Db2 HADR. *Ogni test presuppone che si è connessi come utente radice* e il database IBM Db2 primario è in esecuzione nel *azibmdb01* macchina virtuale.
...
> * Pacemaker non siano presenti azioni non riuscite (crm status).
...
![DBACockpit - dopo la migrazione](./media/dbms-guide-ha-ibm/hadr-sap-mgr-post.png)
...
Eseguire la migrazione della risorsa al *azibmdb01* e deselezionare i vincoli di posizione
...
In questo caso, viene testata l'isolamento tramite SBD, è consigliabile procedere quando si usa Linux SUSE.
...
Nodo del cluster *azibmdb01* deve essere riavviato. Il ruolo di IBM Db2 primario HADR sta per essere spostato in *azibmdb02*. Quando *azibmdb01* è nuovamente online, Db2 istanza sta per spostare il ruolo di un'istanza del database secondario. 

Se il servizio Pacemaker non viene avviato automaticamente nel database primario precedente riavviato, assicurarsi di avviare manualmente con:
...
L'istanza di Db2 sta esito negativo e Pacemaker segnalerà il seguente stato:
...
### <a name="crash-vm-with-restart-on-the-node-that-runs-the-hadr-primary-database-instance"></a>Arresto anomalo del sistema della macchina virtuale con il riavvio nel nodo che esegue l'istanza di database primario HADR
...
In caso di un "interruzione" del nodo, il nodo in errore deve essere riavviato tramite gli strumenti di gestione di Azure (nel portale di Azure, PowerShell o Azure CLI). Dopo il nodo in errore viene riportato online, viene avviata l'istanza di Db2 nel ruolo secondario.
