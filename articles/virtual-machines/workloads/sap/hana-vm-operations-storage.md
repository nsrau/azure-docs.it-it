...
> [!IMPORTANT]
> Indipendentemente dal tipo di archiviazione di Azure scelto, il file system usato in tale risorsa di archiviazione deve essere supportato da SAP per il sistema operativo e DBMS specifici. [Nota di supporto SAP #405827](https://launchpad.support.sap.com/#/notes/405827) elenca i file system supportati per diversi sistemi operativi e database, incluso SAP Hana. Questo vale per tutti i volumi SAP HANA possibile accedere per la lettura e la scrittura per qualsiasi attività. In particolare, usando NFS in Azure per SAP HANA, le restrizioni aggiuntive delle versioni di NFS si applicano come indicato più avanti in questo articolo. 
...
Linux offre varie modalità di pianificazione I/O diverse. Una raccomandazione comune per i fornitori di Linux e SAP consiste nel riconfigurare la modalità di pianificazione I/O per i volumi del disco dalla modalità **mq-scadenza** o **Kyber** alla modalità **NoOp** (non multicoda) o **None** per la modalità (multicoda). Per informazioni dettagliate, fare riferimento alla [Nota SAP #1984787](https://launchpad.support.sap.com/#/notes/1984787). 
...
> [!IMPORTANT]
> Se è presente una mancata corrispondenza tra l'ID utente per <b>SID</b>ADM e l'ID `sapsys` del gruppo per tra la macchina virtuale e la configurazione di Azure NetApp, le autorizzazioni per i file in volumi NetApp di Azure, montate in `nobody`macchine virtuali, verranno visualizzate come. Assicurarsi di specificare l'ID utente corretto per <b>SID</b>ADM e l'ID gruppo per `sapsys`, durante [l'onboarding di un nuovo sistema](https://forms.office.com/Pages/ResponsePage.aspx?id=v4j5cvGGr0GRqy180BHbRxjSlHBUxkJBjmARn57skvdUQlJaV0ZBOE1PUkhOVk40WjZZQVJXRzI2RC4u) per Azure NetApp files.
...
Quando si progetta l'infrastruttura per SAP in Azure, è necessario conoscere alcuni requisiti minimi di velocità effettiva di archiviazione da parte di SAP, che si traduce in caratteristiche minime di velocità effettiva:
...
> [!TIP]
> È possibile ridimensionare Azure NetApp Files volumi in modo dinamico, senza la necessità di `unmount` utilizzare i volumi, arrestare le macchine virtuali o arrestare SAP Hana. Questo consente la flessibilità di soddisfare le esigenze di velocità effettiva previste e non previste per le applicazioni.
