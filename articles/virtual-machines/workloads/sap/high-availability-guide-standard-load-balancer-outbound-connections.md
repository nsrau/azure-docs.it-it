...
4. Creare una regola del firewall di Azure per consentire la connettività in uscita agli endpoint pubblici specificati. L'esempio illustra come consentire l'accesso all'endpoint pubblico dell'API di gestione di Azure.  
   ...
   1. Servizio: nome **ToAzureAPI**.  Protocollo: selezionare **any**. Indirizzo di origine: immettere l'intervallo per la subnet in cui vengono distribuite le macchine virtuali e Load Balancer Standard ad esempio: **11.97.0.0/24**. Porte di destinazione: <b>*</b>immettere.  
   1. Salvare
