...
Il livello dell'applicazione SAP viene distribuito in un set di disponibilità di Azure.The SAP application layer is deployed across one Azure [availability set](https://docs.microsoft.com/azure/virtual-machines/windows/manage-availability). Per la disponibilità elevata di SAP Central Services è possibile distribuire due macchine virtuali in un set di disponibilità separato. Usare Windows Server Failover Clustering o Pacemaker (Linux) come framework a disponibilità elevata con failover automatico in caso di problemi dell'infrastruttura o del software. Per altre informazioni su queste distribuzioni, vedere:

- [Cluster un'istanza SAP ASCS/SCS in un cluster di failover di Windows utilizzando un disco condiviso del cluster](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-high-availability-guide-wsfc-shared-disk)
...
- Non è possibile distribuire set di disponibilità di Azure all'interno di una zona di disponibilità di Azure a meno che non si usi il gruppo di posizionamento di prossimità di Azure.You can't deploy Azure availability sets within an Azure Availability Zone unless you use [Azure Proximity Placement Group.](https://docs.microsoft.com/azure/virtual-machines/linux/co-location) Il modo in cui è possibile distribuire il livello SAP DBMS e i servizi centrali tra le zone e allo stesso tempo distribuire il livello dell'applicazione SAP usando i set di disponibilità e ottenere comunque una stretta vicinanza delle macchine virtuali è documentato nell'articolo [Azure Proximity Placement Groups for optimal network latency with SAP applications](sap-proximity-placement-scenarios.md). Se non si sfruttano i gruppi di posizionamento di prossimità di Azure, è necessario scegliere uno o l'altro come framework di distribuzione per le macchine virtuali.
- Non è possibile usare un'istanza di [Azure Load Balancer Basic](https://docs.microsoft.com/azure/load-balancer/load-balancer-overview) per creare soluzioni cluster di failover basate su Windows Server Failover Clustering o Linux Pacemaker. Al contrario, è necessario usare lo [SKU](https://docs.microsoft.com/azure/load-balancer/load-balancer-standard-availability-zones)di Azure Standard Load Balancer .
...
Non è necessario eseguire questi test manualmente. È possibile trovare una procedura di PowerShell Test di [latenza](https://github.com/Azure/SAP-on-Azure-Scripts-and-Utilities/tree/master/AvZone-Latency-Test) zona di disponibilità che automatizza i test di latenza descritti. 
...
Questa architettura di distribuzione è denominata attivo/attivo perché si distribuiscono i server applicazioni SAP attivi in due o tre aree. L'istanza di SAP Central Services che usa la replica di accodamento verrà distribuita tra due zone. Lo stesso vale per il livello DBMS, che verrà distribuito tra le stesse zone di SAP Central Services.
...
- Non si usa [Azure Proximity Placement Group,](https://docs.microsoft.com/azure/virtual-machines/linux/co-location)le zone di disponibilità di Azure vengono considerate come domini di errore e di aggiornamento per tutte le macchine virtuali perché i set di disponibilità non possono essere distribuiti nelle zone di disponibilità di Azure.Not using Azure Proximity Placement Group , you treat the Azure Availability zones as fault and update domains for all the VMs because availability sets can't be deployed in Azure Availability zones.
- Se si desidera combinare distribuzioni di zona per il livello DBMS e i servizi centrali, ma si usano i set di disponibilità di Azure per il livello applicazione, è necessario usare i gruppi di prossimità di Azure come descritto nell'articolo Gruppi di posizionamento di prossimità di Azure per una latenza di [rete ottimale con le applicazioni SAP](sap-proximity-placement-scenarios.md).
...
- Per tutte le macchine virtuali distribuite, è necessario usare [Dischi gestiti](https://azure.microsoft.com/services/managed-disks/)di Azure. I dischi non gestiti non sono supportati per le distribuzioni nelle zone.
...
- La terza zona viene utilizzata per ospitare il dispositivo SBD nel caso in cui si crea un [cluster SUSE Linux Pacemaker](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-suse-pacemaker#create-azure-fence-agent-stonith-device) o istanze dell'applicazione aggiuntive.
- Per ottenere la coerenza in fase di esecuzione per i processi aziendali critici, è possibile provare a indirizzare determinati processi batch e utenti alle istanze dell'applicazione che si trovano nella zona con l'istanza DBMS attiva utilizzando gruppi di server batch SAP, gruppi di accesso SAP o gruppi RFC. In caso di failover di zona, è tuttavia necessario spostare manualmente questi gruppi in istanze in esecuzione su macchine virtuali nella stessa zona della macchina virtuale del database attivo.  
...
> [!IMPORTANT]
> In questo scenario attivo/attivo vengono annunciati costi aggiuntivi per la larghezza di banda da parte di Microsoft a partire dal 04/01/2020. Controllare il documento [Dettagli sui prezzi della larghezza di banda](https://azure.microsoft.com/pricing/details/bandwidth/). Il trasferimento dei dati tra il livello dell'applicazione SAP e il livello SAP DBMS è piuttosto intensivo. Pertanto lo scenario attivo/attivo può contribuire un po' ai costi. Continua a controllare questo articolo per ottenere i costi esatti 
