...
description: Come installare SAP HANA in un SAP HANA in Azure (istanze di grandi dimensioni).
...
> [!Note]
> In base ai criteri SAP, l'installazione di SAP HANA deve essere eseguita da una persona che ha superato l'esame di certificazione Certified SAP Technology Associate, SAP HANA Installation o che è un integratore di sistema (SI) certificato SAP.

Quando si prevede di installare HANA 2.0, vedere [SAP support note #2235581 - SAP HANA: Supported operating systems](https://launchpad.support.sap.com/#/notes/2235581/E) (Nota di supporto SAP n. 2235581 - SAP HANA: sistemi operativi supportati) per verificare che il sistema operativo sia supportato con la versione di SAP HANA da installare. Il sistema operativo supportato per HANA 2.0 ha più restrizioni di quello supportato per HANA 1.0. È inoltre necessario verificare se la versione del sistema operativo a cui si è interessati è elencata come supportata per la particolare unità HLI in questo [elenco](https://www.sap.com/dmc/exp/2014-09-02-hana-hardware/enEN/iaas.html#categories=Microsoft%20Azure)pubblicato . Fare clic sull'unità per ottenere tutti i dettagli con l'elenco dei sistemi ostici supportati di tale unità. 
...
Il **primo passaggio** dopo aver ricevuto l'istanza di grandi dimensioni HANA e aver stabilito l'accesso e la connettività alle istanze consiste nel verificare nel portale di Azure se le istanze vengono visualizzate con gli SKU e i servizi ostiche corretti. Leggere [Il controllo delle istanze di grandi dimensioni di Azure tramite](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-li-portal) il portale di Azure per i passaggi necessari per eseguire i controlli.

Il **secondo passaggio** dopo aver ricevuto l'istanza di grandi dimensioni HANA e stabilire l'accesso e la connettività alle istanze, consiste nel registrare il sistema operativo dell'istanza con il provider del sistema operativo. Questo passaggio include la registrazione del sistema operativo Linux SUSE in un'istanza di SMT SUSE che è stata distribuita in una macchina virtuale di Azure. 
...
Il **quinto passaggio** consiste nel controllare l'ora di sistema dell'unità di istanza HANA Large Instance. Le istanze vengono distribuite in base con un fuso orario del sistema che rappresenta la posizione dell'area di Azure in cui si trova il timestamp dell'istanza Large di HANA. È possibile modificare l'ora o il fuso orario del sistema delle istanze di cui si è proprietari. 
...
[Red Hat Enterprise Linux for SAP HANA](https://www.redhat.com/en/resources/red-hat-enterprise-linux-sap-hana) è un'altra offerta per l'esecuzione di SAP HANA in istanze di grandi dimensioni di HANA. Le versioni di RHEL 7.2 e 7.3 sono disponibili e supportate. 
...
- [Nota del supporto SAP #2009879 - Linee guida SAP HANA per il sistema operativo Red Hat Enterprise Linux (RHEL)](https://launchpad.support.sap.com/#/notes/2009879/E)
...
- [Nota del supporto SAP #2002167 - Red Hat Enterprise Linux 7.x: installazione e aggiornamento](https://launchpad.support.sap.com/#/notes/2002167)
...
- [Panoramica e architettura SAP HANA (Istanza di grandi dimensioni) in Azure](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-overview-architecture)
...
Per le versioni sap HANA 1.0 fino a SPS12, questi parametri possono essere impostati durante l'installazione del database SAP HANA, come descritto nella [nota SAP #2267798 - Configurazione del database SAP HANA](https://launchpad.support.sap.com/#/notes/2267798).

È anche possibile configurare i parametri dopo l'installazione del database SAP HANA tramite il framework hdbparam. 

L'archiviazione utilizzata nelle istanze HANA di grandi dimensioni presenta un limite di dimensione del file. Il [limite di dimensione è 16 TB](https://docs.netapp.com/ontap-9/index.jsp?topic=%2Fcom.netapp.doc.dot-cm-vsmg%2FGUID-AA1419CF-50AB-41FF-A73C-C401741C847C.html) per file. A differenza delle limitazioni delle dimensioni dei file nei file system EXT3, HANA non è a conoscenza implicita della limitazione di archiviazione imposta dall'archivio delle istanze di grandi dimensioni HANA. Di conseguenza HANA non creerà automaticamente un nuovo file di dati quando viene raggiunto il limite di dimensione del file di 16 TB. Man mano che HANA tenta di aumentare l'aumento del file oltre i 16 TB, HANA segnalerà errori e il server di indicizzazione si arresterà alla fine.

> [!IMPORTANT]
> Per evitare che HANA tenti di aumentare i file di dati oltre il limite di 16 file TB dell'archiviazione di un'istanza di grandi dimensioni di HANA, è necessario impostare i parametri seguenti nel file di configurazione SAP HANA global.ini
> 
> - datavolume_striping: vero
> - datavolume_striping_size_gb 15000 USD
> - Vedere anche [#2400005](https://launchpad.support.sap.com/#/notes/2400005) nota SAP
> - Prestare attenzione [alle note note note](https://launchpad.support.sap.com/#/notes/2631285) di SAP #2631285
