...
- SQL Server Always On è supportato con il sistema operativo Windows per SQL Server in Azure. Si tratta dell'indicazione predefinita per le istanze di SQL Server di produzione in Azure. I dettagli sono descritti in questi articoli:
    - [Introduzione SQL Server gruppi di disponibilità always on in macchine virtuali di Azure](https://docs.microsoft.com/azure/virtual-machines/windows/sql/virtual-machines-windows-portal-sql-availability-group-overview).
    ...
    - [Configurare un servizio di bilanciamento del carico per un gruppo di disponibilità always on in Azure](https://docs.microsoft.com/azure/virtual-machines/windows/sql/virtual-machines-windows-portal-sql-alwayson-int-listener).
...
- Server del cluster di failover di Windows `Datakeeper`basato sul tipo di base. Anche se documentato da Microsoft, è necessaria una relazione di supporto con un valore di base, in modo che sia possibile interagire con il supporto del valore di base quando si usa questa soluzione. I dettagli sono descritti nell'articolo:
...
- Pacemaker sul sistema operativo SUSE con la creazione di una condivisione NFS a disponibilità elevata con due `drdb` VM SUSE e per la replica di file. I dettagli sono documentati nell'articolo
...
- Pacemaker sul sistema operativo Red Hat con condivisione NFS ospitata in `glusterfs` un cluster. Informazioni dettagliate sono disponibili negli articoli
...
Tra le soluzioni elencate, è necessaria una relazione di supporto con il supporto per `Datakeeper` il prodotto e per coinvolgere direttamente il codice in caso di problemi. A seconda del modo in cui è stato concesso in licenza il sistema operativo Windows, Red Hat e/o SUSE, potrebbe essere necessario disporre di un contratto di supporto con il provider del sistema operativo per ottenere supporto completo delle configurazioni di disponibilità elevata elencate.
...
- Il server del cluster di failover Windows con disco condiviso di `Datakeeper` Windows basato su un tipo di nodo può essere distribuito in tutti i tipi di archiviazione nativi di Azure, ad eccezione Azure NetApp files. Tuttavia, è consigliabile sfruttare l'archiviazione Premium a causa di contratti di servizio di livello superiore in velocità effettiva e IOPS.
...
- Red Hat pacemaker che `glusterfs` USA per fornire la condivisione NFS è supportato con i tipi di archiviazione nativi di Azure, ad eccezione Azure NetApp files. Tuttavia, è consigliabile sfruttare l'archiviazione Premium a causa di contratti di servizio di livello superiore in velocità effettiva e IOPS.
