...
- **Panorama SAP**: Si riferisce all'intero asset SAP nel panorama IT. Il panorama applicativo SAP include tutti gli ambienti di produzione e non produzione.
...
- **Timbro**: Definisce la dimensione di distribuzione interna Microsoft delle istanze di grandi dimensioni HANA. Prima che le unità di istanza di grandi dimensioni HANA possano essere distribuite, è necessario distribuire un timbro istanza di grandi dimensioni HANA costituito da rack di calcolo, rete e archiviazione in una posizione del data center. Tale distribuzione è chiamata timbro di istanza HANA Large o dalla revisione 4 (vedere di seguito) in utilizziamo l'alternativa del termine di **Large Instance Row**
- **Revisione**: Sono presenti due diverse revisioni dei timbri per i timbri HANA Large Instance. Questi differiscono nell'architettura e nella prossimità degli host di macchine virtuali di AzureThese differ in architecture and proximity to Azure virtual machine hosts
    - "Revisione 3" (Rev 3): è il progetto originale che è stato distribuito da metà dell'anno 2016
    - "Revisione 4" (revisione 4): è una nuova progettazione che può fornire una maggiore vicinanza agli host di macchine virtuali di Azure e con quella latenza di rete inferiore tra macchine virtuali di Azure e unità di istanza di grandi dimensioni HANA 
