...
>[!NOTE]
> Se si memorizzano i file di dati di tempdb e il file di log in una cartella nell'unità D:\ creata personalmente, è necessario assicurarsi che la cartella esista dopo un riavvio della VM. Dal momento che il D: l'unità viene appena inizializzata dopo il riavvio di una macchina virtuale di tutti i file e le strutture di directory vengono eliminate. Possibilità di ricreare eventuali strutture di directory in D: prima dell'avvio del servizio SQL Server è documentato in [questo articolo](https://cloudblogs.microsoft.com/sqlserver/2014/09/25/using-ssds-in-azure-vms-to-store-sql-server-tempdb-and-buffer-pool-extensions/).
...
Per evitare che in seguito al ripristino o alla creazione di database i file di dati non vengano inizializzati e il loro contenuto venga azzerato, è necessario assicurarsi che per il contesto utente in cui viene eseguito il servizio SQL Server siano disponibili determinate autorizzazioni. Queste autorizzazioni sono in genere disponibili per gli utenti del gruppo Administrators di Windows. Se il servizio SQL Server viene eseguito nel contesto utente di un utente non amministratore di Windows, è necessario assegnare a tale utente il diritto utente Eseguire attività di **manutenzione volume**.  Per informazioni dettagliate, vedere l'articolo <https://support.microsoft.com/kb/2574695> di Microsoft Knowledge Base
...
- Configurazione del ripristino di emergenza da locale in un'area di AzureDisaster Recovery configuration from on-premises into an Azure region
- Cut-over scenarios from on-premises to Azure. In questi casi, il log shipping viene usato per sincronizzare la nuova distribuzione DBMS in Azure con il sistema di produzione in corso in locale. Al momento del cutover, la produzione viene arrestata e si controlla che gli ultimi backup del log delle transazioni e quelli più recenti siano stati trasferiti nella distribuzione DBMS di Azure. La distribuzione DBMS in Azure viene quindi aperta per la produzione.  
...
Nei casi in cui si esegue una migrazione eterogenea da un altro DBMS, in esecuzione in locale, a Windows/SQL Server in esecuzione in Azure, è necessario creare il database di destinazione vuoto in SQL Server in anticipo. Il passaggio successivo prevede l'applicazione della funzionalità TDE di SQL Server. Mentre il sistema di produzione è ancora in esecuzione in locale. Il motivo per cui conviene attenersi a questa sequenza è che il processo di crittografia del database vuoto può richiedere molto tempo. I processi di importazione SAP importano quindi i dati nel database crittografato durante la fase di inattività. Nel complesso l'importazione in un database crittografato ha un impatto temporale inferiore rispetto alla crittografia del database dopo la fase di esportazione nella fase di inattività. Esperienze negative si sono verificate nel tentativo di applicare la funzionalità TDE con il carico di lavoro SAP in esecuzione sopra al database. È pertanto consigliabile gestire la distribuzione di TDE come un'attività da svolgere senza carichi di lavoro SAP su un database specifico.

Nei casi in cui si spostano i database SAP SQL Server da locale in Azure, è consigliabile testare l'infrastruttura su quale infrastruttura è possibile ottenere la crittografia applicata più rapidamente. Per questo motivo, tenere presenti queste considerazioni:
...
- Nelle versioni precedenti di SQL Server la compressione dei backup non è più efficiente quando si crittografa il database SQL Server. Questo comportamento potrebbe svilupparsi in un problema quando il piano consisteva nel crittografare il database di SQL Server in locale e quindi copiare un backup in Azure per ripristinare il database in Azure.This behavior could develop into an issue when your plan was to encrypt your SQL Server database on-premises and then copy a backup into Azure to restore the database in Azure. La compressione dei backup di SQL Server raggiunge in genere un rapporto di compressione di fattore 4.
...
Trattando l'applicazione della crittografia TDE con solo un carico di lavoro SAP minimo, è consigliabile testare nella configurazione specifica se è preferibile applicare TDE al database SAP in locale o per eseguire questa operazione in Azure.Treat the application of TDE encryption with no to little SAP workload only, you should test in your specific configuration on whether it is better apply TDE to your SAP database on-premises or to do so in Azure. In Azure è senz'altro maggiore la flessibilità in termini di provisioning eccessivo dell'infrastruttura e di riduzione della stessa dopo l'applicazione di TDE.
...
- [Gestione delle chiavi estendibile tramite l'insieme di](https://docs.microsoft.com/sql/relational-databases/security/encryption/extensible-key-management-using-azure-key-vault-sql-server?view=sql-server-2017)credenziali delle chiavi di Azure (SQL Server) .
...
- [Manutenzione del connettore SQL Server & Risoluzione dei problemi](https://docs.microsoft.com/sql/relational-databases/security/encryption/sql-server-connector-maintenance-troubleshooting?view=sql-server-2017).
