...
* È tuttavia consigliabile usare più file di dati di tempdb. Tenere presente che i volumi delle unità D:\ sono diversi in base al tipo di VM. Per le dimensioni esatte dell'unità D:\ delle diverse VM, vedere l'articolo [Dimensioni per le macchine virtuali Windows in Azure](../../windows/sizes.md).
...
- Usare l'unità D:\ per tempdb fintanto che le prestazioni sono sufficientemente buone. Se il carico di lavoro complessivo è limitato nelle prestazioni dal fatto che tempdb si trova nell'unità D:\, potrebbe essere necessario provare a spostare tempdb in dischi di Archiviazione Premium come consigliato in [questo articolo](../../../azure-sql/virtual-machines/windows/performance-guidelines-best-practices.md).
...
I dettagli di questa funzionalità sono disponibili nell'articolo [File di dati di SQL Server in Microsoft Azure](/sql/relational-databases/databases/sql-server-data-files-in-microsoft-azure?view=sql-server-2017)
...
- Con SQL Server 2016, è stata introdotta una nuova funzionalità che consente anche la compressione efficiente dei database crittografati. Per i dettagli, vedere [questi blog](/archive/blogs/sqlcat/sqlsweet16-episode-1-backup-compression-for-tde-enabled-databases).
...
- [Extensible Key Management tramite l'insieme di credenziali delle chiavi di Azure (SQL Server)](/sql/relational-databases/security/encryption/extensible-key-management-using-azure-key-vault-sql-server?view=sql-server-2017).
- [Extensible Key Management TDE di SQL Server con Azure Key Vault - Passaggi di configurazione](/sql/relational-databases/security/encryption/setup-steps-for-extensible-key-management-using-the-azure-key-vault?view=sql-server-2017).
- [Manutenzione e risoluzione dei problemi di Connettore SQL Server](/sql/relational-databases/security/encryption/sql-server-connector-maintenance-troubleshooting?view=sql-server-2017).
