...
* Quando si usa un listener del gruppo di disponibilità, le VM di database devono essere connesse a un Load Balancer dedicato. Per evitare che Azure assegni nuovi indirizzi IP nei casi in cui entrambe le macchine virtuali siano arrestate in modo anomalo, è necessario assegnare indirizzi IP statici alle interfacce di rete di tali VM nella configurazione Always On (la definizione di un indirizzo IP statico è descritta in [questo][virtual-networks-reserved-private-ip] articolo)
