...
Questo articolo descrive come distribuire più sistemi SAP NetWeaver a disponibilità elevata (ovvero multi-SID) in un cluster a due nodi in macchine virtuali di Azure con SUSE Linux Enterprise Server per le applicazioni SAP.  
...
* Configurazione front-end
  ...
  * Indirizzo IP per NW1 10.3.1.17
  * Indirizzo IP per NW1 10.3.1.19
* Configurazione back-end
  * Connessione alle interfacce di rete primarie di tutte le macchine virtuali che devono far parte del cluster (A)SCS/ERS
* Porta probe
...
8. **[1]** creare le risorse del cluster SAP per il sistema SAP appena installato. 
   ...
   L'esempio illustrato di seguito riguarda i sistemi SAP **NW2** e **NW3**, supponendo che usi l'architettura di Accodamento server 1 (ENSA1):
