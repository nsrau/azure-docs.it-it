---
title: Clustering dell'istanza SAP (A)SCS nel cluster di failover Windows con i dischi condivisi del cluster in Azure | Microsoft Docs
description: Clustering dell'istanza SAP (A)SCS nel cluster di failover Windows con i dischi condivisi del cluster
...
# <a name="clustering-sap-ascs-instance-on-windows-failover-cluster-using-cluster-shared-disk-on-azure"></a>Clustering dell'istanza SAP (A)SCS nel cluster di failover Windows con i dischi condivisi del cluster in Azure
...
Rispetto alle distribuzioni bare metal o cloud privato, sono necessari passaggi aggiuntivi per configurare un Windows Server Failover Clustering in Macchine virtuali di Azure. Quando si crea un cluster è necessario impostare diversi indirizzi IP e nomi host virtuali per l'istanza di SAP ASCS/SCS.
...
### <a name="sap-ascs-ha-with-cluster-shared-disks"></a>Disponibilità elevata di SAP (A)SCS con i dischi condivisi del cluster
In **Windows** un'istanza **SAP (A)SCS** contiene non solo **servizi centrali SAP**, **server di messaggistica SAP** e **processi server di accodamento** ma anche file di **SAP GLOBAL HOST** usati per archiviare i file centrali per l'intero sistema SAP.

Pertanto, sono presenti i due componenti seguenti di un'istanza SAP (A)SCS:

* **servizi centrali SAP** con:
    * Due **processi**, server di messaggistica e accodamento e **<(A)SCSVirtualHostName>** usato per accedere a questi due processi
    * **Struttura del file** S:\usr\sap\\&lt;SID&gt;\(A)SCS<InstanceNumber>


* File di **SAP GLOBAL HOST** con **condivisione file sapmnt**:
    * **Struttura del file** S:\usr\sap\\&lt;SID&gt;\SYS\....
    * **condivisione file sapmnt**, che consente l'accesso a questi file globali S:\usr\sap\\&lt;SID&gt;\SYS\.. con un percorso UNC:

     \\\\<(A)SCSVirtualHostName>\sapmnt\\&lt;SID&gt;\SYS\....


![Figura 2: Processi, struttura del file e condivisione file sapmnt host GLOBAL di un'istanza SAP (A)SCS][sap-ha-guide-figure-8001]

_**Figura 2:** Processi, struttura del file e condivisione file sapmnt host GLOBAL di un'istanza SAP (A)SCS_

Nelle impostazioni di disponibilità elevata si esegue il clustering delle istanze SAP (A)SCS. Per inserire i file di SAP (A)SCS e i file di SAP GLOBAL HOST vengono usati i **dischi condivisi del cluster**, unità S:\ nel nostro esempio.

![Figura 3: Architettura a disponibilità elevata di SAP (A)SCS con dischi condivisi][sap-ha-guide-figure-8002]

_**Figura 3:** Architettura a disponibilità elevata di SAP (A)SCS con dischi condivisi_

> [!IMPORTANT]
>Poiché questi due componenti vengono eseguiti nella stessa istanza SAP (A)SCS:
>* Lo stesso **<(A)SCSVirtualHostName>** viene usato per accedere ai processi del server di messaggistica SAP e del server di accodamento, nonché ai file SAP GLOBAL HOST tramite la condivisione file sapmnt.
>* Lo stesso **disco condiviso del cluster S:\** viene condiviso tra loro
>


![Figura 4: Architettura a disponibilità elevata di SAP (A)SCS con dischi condivisi][sap-ha-guide-figure-8003]

_**Figura 4:** Architettura a disponibilità elevata di SAP (A)SCS con dischi condivisi_

### <a name="shared-disk-in-azure-with-sios-datakeeper"></a>Disco condiviso in Azure con SIOS DataKeeper
...
Per creare una risorsa del disco condiviso per un cluster, seguire questa procedura:
...
_**Figura 5:** Configurazione di Windows Server Failover Clustering in Azure con SIOS DataKeeper_
...
* [Installazione della disponibilità elevata di SAP NetWeaver nel cluster di failover Windows e nei dischi condivisi per l'istanza SAP (A)SCS][sap-high-availability-installation-wsfc-shared-disk]
