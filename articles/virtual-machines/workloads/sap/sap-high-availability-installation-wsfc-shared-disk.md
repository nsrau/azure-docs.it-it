---
title: "Installazione della disponibilità elevata di SAP NetWeaver nel cluster di failover Windows e nei dischi condivisi per l'istanza SAP (A)SCS in Azure | Microsoft Docs"
description: "Installazione della disponibilità elevata di SAP NetWeaver nel cluster di failover Windows e nei dischi condivisi per l'istanza SAP (A)SCS"
...
---
# <a name="sap-netweaver-ha-installation-on-windows-failover-cluster-and-shared-disk-for-sap-ascs-instance-on-azure"></a>Installazione della disponibilità elevata di SAP NetWeaver nel cluster di failover Windows e nei dischi condivisi per l'istanza SAP (A)SCS in Azure
...
Questo documento descrive la procedura di installazione e configurazione del sistema SAP a disponibilità elevata tramite il **cluster di failover Windows** e i **dischi condivisi del cluster** per l'istanza di clustering SAP (A)SCS.

## <a name="prerequisites"></a>Prerequisiti

Assicurarsi di esaminare i documenti seguenti prima di iniziare l'installazione:

* [Guida all'architettura - Clustering SAP (A)SCS Instance on Windows Failover Cluster using Cluster Shared Disk][sap-high-availability-guide-wsfc-shared-disk] (Clustering dell'istanza SAP (A)SCS nel cluster di failover Windows con i dischi condivisi del cluster)
...
2.  Per mantenere le porte predefinite per il bilanciamento del carico interno di Azure, selezionare:
  ...
  * Per il **sistema ABAP + Java**: **ASCS** numero di istanza **00** e **SCS** numero di istanza **01**
  ...
  Per usare altri numeri di istanza rispetto a 00 per l'istanza ABAP ASCS e rispetto a 01 per l'istanza Java SCS, è prima necessario modificare le regole predefinite del servizio di bilanciamento del carico interno di Azure, come descritto in [Modificare le regole di bilanciamento del carico predefinite di ASCS/SCS per il servizio di bilanciamento del carico interno di Azure][sap-ha-guide-8.9].
...
1.  Verificare l'impostazione **ProbePort** corrente eseguendo il comando di PowerShell seguente:
  ...
  ```
   ...
   Eseguire il comando all'interno di una delle macchine virtuali della configurazione del cluster.
...
2.  Definire una porta probe. Il numero della porta probe predefinita è **0**. In questo esempio viene usata la porta probe **62000**.
  ...
  Per impostare un nuovo valore di ProbePort per la risorsa cluster **SAP <*SID*> IP**, eseguire lo script di PowerShell seguente per aggiornare le variabili di PowerShell per l'ambiente:
  ...
  ```
  Dopo l'esecuzione dello script, verrà richiesto di riavviare il gruppo di cluster SAP per attivare le modifiche.
...
![Figura 6: Gestione cluster di failover: il gruppo di cluster SAP <SID> è in esecuzione nel nodo A del cluster][sap-ha-guide-figure-5000]

_**Figura 6:** Gestione cluster di failover: il gruppo di cluster SAP <*SID*> è in esecuzione nel nodo A del cluster_
...
4.  Riavviare il nodo A del cluster usando Azure PowerShell. Verrà avviato un failover automatico del gruppo di cluster <*SID*> SAP dal nodo A al nodo B.
  ...
  ![Figura 8: In Gestione cluster di failover il gruppo di cluster SAP <SID> è in esecuzione nel nodo B del cluster][sap-ha-guide-figure-5002]

  _**Figura 8:** In Gestione cluster di failover il gruppo di cluster SAP <*SID*> è in esecuzione nel nodo B del cluster_
