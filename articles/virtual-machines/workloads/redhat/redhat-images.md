...
Di seguito è riportato un esempio di come creare una VM RHEL 7,6.
...
Per le immagini RHEL 7. x, sono disponibili diversi tipi di immagine. La tabella seguente illustra i diversi set di immagini disponibili. Per visualizzare un elenco completo, usare l'interfaccia della riga di comando di Azure `az vm image list --publisher redhat --all`.
...
In questo caso, per impostazione predefinita `RedHat:RHEL:7.6:7.6.2019102813` viene collegato ai repository EUS. Il valore SKU è 7,4. Per impostazione predefinita, `RedHat:RHEL:7-LVM:7.6.2019062414` è associato a repository non EUS. Il valore SKU è 7-LVM.
