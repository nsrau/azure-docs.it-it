<properties 
   pageTitle="Applicazione automatizzata di patch per SQL Server in Macchine virtuali di Azure"
...
È possibile usare il [portale di anteprima di Azure](http://go.microsoft.com/fwlink/?LinkID=525040&clcid=0x409) per configurare l’applicazione automatizzata di patch quando si crea una nuova macchina virtuale di SQL Server. La seguente schermata mostra queste opzioni in **CONFIGURAZIONE FACOLTATIVA** | **APPLICAZIONE AUTOMATICA DELLE PATCH DI SQL**.

![Applicazione automatizzata di patch SQL nel portale di Azure](./media/virtual-machines-sql-server-automated-patching/IC778484.jpg)
...
![Configurazione dell’applicazione automatizzata di patch nel portale di Azure](./media/virtual-machines-sql-server-automated-patching/IC792132.jpg)

>[AZURE.NOTE]Quando si abilita l’applicazione automatizzata di patch per la prima volta, Azure configura l'agente IaaS di SQL Server in background. Durante questo periodo, nel portale non viene visualizzata l'informazione relativa alla configurazione dell’applicazione automatizzata di patch. Attendere alcuni minuti per l'installazione e la configurazione dell'agente. A questo punto, nel portale verranno visualizzate le nuove impostazioni.
...
Una funzionalità correlata per Macchine virtuali di SQL Server in Azure è [Backup automatizzato per SQL Server in Macchine virtuali di Azure](virtual-machines-sql-server-automated-backup.md).
