...
Questa sezione illustra come generalizzare la macchina virtuale di Windows. Questa operazione rimuove tutte le informazioni sull'account personale, oltre ad altre cose. Questa operazione viene solitamente eseguita quando si vuole usare questa immagine di VM per distribuire rapidamente macchine virtuali simili. Per altre informazioni su Sysprep, vedere [Come usare Sysprep: Introduzione](http://technet.microsoft.com/library/bb457073.aspx).
...
3. Nella finestra di dialogo **Utilità preparazione sistema** seguire questa procedura:
	...
	![Avvio di Sysprep](./media/virtual-machines-upload-image-windows-resource-manager/sysprepgeneral.png)
...
4. Se si desidera creare un nuovo account di archiviazione. fare clic su **Aggiungi** e immettere le informazioni seguenti:
	...
	5. Fare clic su **Crea**. L'account verrà visualizzato nel pannello **Account di archiviazione**.
		...
		![Immissione dei dettagli dell'account di archiviazione](./media/virtual-machines-upload-image-windows-resource-manager/portal_create_storage_account.png)
	...
	6. Questo passaggio e quelli successivi mostrano come creare un contenitore BLOB nell'account di archiviazione. La procedura è facoltativa, dal momento che il comando PowerShell per il caricamento dell'immagine consente anche di creare un nuovo contenitore BLOB per l'immagine. Se non si desidera creare il contenitore seguendo la procedura, passare alla sezione [Caricare l'immagine della VM](#uploadvm). In alternativa, fare clic su **BLOB** nel riquadro **Servizi**.
		...
		![Servizio BLOB](./media/virtual-machines-upload-image-windows-resource-manager/portal_create_blob.png)
	...
	7. Quando il pannello viene visualizzato, fare clic su **+ Contenitore** per creare un nuovo contenitore di archiviazione BLOB. Immettere il nome del contenitore e il tipo di accesso.
		...
		![Creazione di un nuovo BLOB](./media/virtual-machines-upload-image-windows-resource-manager/portal_create_container.png)
...
2. Aggiungere il disco rigido virtuale generalizzato di Azure all'account di archiviazione usando il cmdlet [Add-AzureRmVhd](https://msdn.microsoft.com/library/mt603554.aspx):
	...
	Dove: - **StorageAccountURL** è l'URL dell'account di archiviazione. In genere sarà nel formato seguente: `https://YourStorageAccountName.blob.core.windows.net`. Si noti che sarà necessario usare il nome dell'account di archiviazione nuovo o esistente al posto di **YourStorageAccountName**. - **BlobContainer** corrisponde al contenitore BLOB in cui archiviare le immagini. Se il cmdlet non trova un contenitore BLOB esistente con questo nome, ne creerà automaticamente uno nuovo. - **TargetVHDName** è il nome con cui salvare l'immagine. - **LocalPathOfVHDFile** è il nome e il percorso completo del file con estensione vhd sul computer locale.
...
Per gestire la nuova macchina virtuale con Azure PowerShell, leggere [Gestire le macchine virtuali usando Gestione risorse di Azure e PowerShell](virtual-machines-deploy-rmtemplates-powershell.md).
