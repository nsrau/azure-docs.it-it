...
| --- | --- | --- |
| settings.wmfVersion |string |Specifica la versione di Windows Management Framework (WMF) da installare nella macchina virtuale. L'impostazione di questa proprietà su **latest** consente di installare la versione più recente di WMF. Attualmente, gli unici valori possibili per questa proprietà sono **4.0**, **5.0**, **5.1** e **latest**. Questi valori possibili sono soggetti ad aggiornamenti. Il valore predefinito è **latest**. |
...
Salvare la configurazione di IisInstall.ps1, inserirla in un file con estensione zip (ad esempio: `iisinstall.zip`) e quindi caricare il file in un URL accessibile.
...
**Problema:** un valore fornito non è consentito.
...
**Problema:** un URL fornito non è valido.

**Soluzione**: Controlla tutti gli URL forniti.
