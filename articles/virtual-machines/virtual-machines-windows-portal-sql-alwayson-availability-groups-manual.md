---
title: Configurare manualmente il gruppo di disponibilità AlwaysOn in macchine virtuali di Azure - Microsoft Azure
...
---
# <a name="configure-always-on-availability-group-in-azure-vm-manually---resource-manager"></a>Configurare manualmente il gruppo di disponibilità AlwaysOn in macchine virtuali di Azure con Resource Manager
...
In questa esercitazione verranno creati gli elementi seguenti:
...
Per l'esecuzione di questa esercitazione saranno necessarie alcune ore, in quanto si devono creare e configurare diverse macchine virtuali di Azure. È anche possibile compilare automaticamente l'intera soluzione. Nel portale di Azure è disponibile una configurazione raccolta per i gruppi di disponibilità AlwaysOn con un listener. La configurazione raccolta imposta automaticamente tutto il necessario per i gruppi di disponibilità. Per altre informazioni, vedere [Portale - Resource Manager](virtual-machines-windows-portal-sql-alwayson-availability-groups.md). 
...
2. Fare clic su **+Nuovo** e quindi digitare **Gruppo di risorse** nella finestra di ricerca del **Marketplace**.
...
3. Fare clic su **Gruppo di risorse** 
...
4. Fare clic su **Crea**. 
5. Nel pannello **Gruppo di risorse** digitare **SQL-HA-RG** in **Nome gruppo di risorse**
...
4. Nel pannello **Rete virtuale** fare clic sul modello di distribuzione **Resource Manager** e su **Crea**.
   ...
   ![Crea rete virtuale](./media/virtual-machines-windows-portal-sql-alwayson-availability-groups-manual/05-createvirtualnetwork.png)
5. Configurare la rete virtuale nel pannello **Crea rete virtuale** . 
...
### <a name="create-the-second-subnet"></a>Creare la seconda subnet
A questo punto la rete virtuale contiene una subnet, denominata Subnet-1. I controller di dominio usano questa subnet. Le istanze di SQL Server usano una seconda subnet denominata **Subnet-2**. Per configurare Subnet-2:
...
5. Creare una seconda subnet. Fare clic su **+ Subnet**. 
   ...
   Nel pannello **Aggiungi subnet** configurare la subnet digitando **subnet-2** in **Nome**. Azure specifica automaticamente un **Intervallo di indirizzi**valido. Verificare che questo intervallo di indirizzi includa almeno 10 indirizzi. In un ambiente di produzione potrebbero essere necessari più indirizzi. 
6. Fare clic su **OK**.
...
2. Digitare **Windows Server 2012 R2 Datacenter**. 
3. Fare clic su **Server 2012 R2 Datacenter**. Nel pannello **Windows Server 2012 R2 Datacenter** verificare che il modello di distribuzione sia **Resource Manager** e fare clic su **Crea**. Azure apre il pannello **Crea macchina virtuale** . 

Eseguire due volte i passaggi precedenti per creare due macchine virtuali. Assegnare un nome alle due macchine virtuali:
...
> [!NOTE]
> Non è possibile modificare il set di disponibilità in una VM dopo averlo creato.
> 
> 
...
2. Fare clic su **Installa**. La macchina virtuale **ad-primary-dc** viene riavviata automaticamente.

### <a name="configure-the-second-domain-controller"></a>Configurare il secondo controller di dominio
Dopo il riavvio del controller di dominio primario, è possibile configurare il secondo controller di dominio. Questo passaggio facoltativo serve a garantire una disponibilità elevata. Per completare questo passaggio, è necessario conoscere l'indirizzo IP privato del controller di dominio. È possibile ottenerlo dal portale di Azure. Seguire questi passaggi per configurare il secondo controller di dominio.

1. Accedere alla macchina virtuale **ad-primary-dc** . 
2. Aprire Desktop remoto e connettersi al controller di dominio secondario in base all'indirizzo IP. Se non si conosce l'indirizzo IP del controller di dominio secondario, andare al portale di Azure e controllare l'indirizzo assegnato all'interfaccia di rete per il controller di dominio secondario. 
3. Sostituire l'indirizzo del server DNS preferito con l'indirizzo del controller di dominio. 
4. Avviare il file RDP nel controller di dominio primario (**ad-primary-dc**) e accedere alla VM con l'account amministratore (**BUILTIN\DomainAdmin**) e la password (**Contoso!0000**) configurati.
5. Dal controller di dominio primario avviare una sessione di Desktop remoto in **ad-secondary-dc** usando l'indirizzo IP. Usare lo stesso account e la stessa password.
6. Dopo l'accesso, verrà visualizzato il dashboard **Server Manager** . Nel riquadro di sinistra fare clic su **Server locale** .
7. Selezionare il collegamento **Indirizzo IPv4 assegnato da DHCP - Compatibile IPv6** .
8. Nella finestra **Connessioni di rete** selezionare l'icona della rete.
...
9. Sulla barra dei comandi fare clic su **Cambia impostazioni di connessione** (a seconda delle dimensioni della finestra, potrebbe essere necessario fare clic sulla doppia freccia destra per visualizzare questo comando).
...
12. L'indirizzo è quello assegnato a una VM nella subnet subnet-1 nella rete virtuale di Azure e la VM in questione è **ad-primary-dc**. Per verificare l'indirizzo IP di **ad-primary-dc** digitare **nslookup ad-primary-dc** nel prompt dei comandi, come illustrato di seguito.
    ...
    > [!NOTE]
    > Dopo aver impostato il DNS, è possibile che la sessione RDP sul server membro venga persa. In questo caso, riavviare la VM dal portale di Azure.
    > 
    > 
13. Fare clic su **OK** e quindi su **Chiudi** per eseguire il commit delle modifiche. A questo punto è possibile aggiungere la macchina virtuale a **corp.contoso.com**.
14. Ripetere i passaggi seguiti per creare il primo controller di dominio, ma nella **Configurazione guidata Servizi di dominio Active Directory**usare i valori seguenti:
...
1. Accedere di nuovo alla macchina virtuale **ad-primary-dc** .
...
1. Fare clic su **OK** per creare l'utente **Install**. Usare questo account per configurare il cluster di failover e il gruppo di disponibilità.
2. Creare due utenti aggiuntivi con gli stessi passaggi: **CORP\SQLSvc1** e **CORP\SQLSvc2**. I servizi SQL Server usano questi account. Assegnare quindi a **CORP\Install** le autorizzazioni necessarie per configurare WSFC (Windows Service Failover Clustering).
3. Nel **Centro di amministrazione di Active Directory** selezionare **corp (local)** nel riquadro di sinistra. Nel riquadro **Attività** a destra fare quindi clic su **Proprietà**.
   
    ![Proprietà utente CORP](./media/virtual-machines-windows-portal-sql-alwayson-availability-groups-manual/IC784627.png)
4. Selezionare **Estensioni**, quindi fare clic sul pulsante **Avanzate** nella scheda **Sicurezza**.
5. Nella finestra di dialogo **Impostazioni avanzate di sicurezza per corp** seguire questa procedura. Fare clic su **Aggiungi**.
6. Fare clic su **Seleziona un'entità**. Cercare quindi **CORP\Install**. Fare clic su **OK**.
7. Selezionare le autorizzazioni **Leggi tutte le proprietà** e **Create Computer objects** (Crea oggetti computer).
   
    ![Autorizzazioni utente Corp](./media/virtual-machines-windows-portal-sql-alwayson-availability-groups-manual/IC784628.png)
8. Fare clic su **OK** e quindi di nuovo clic su **OK**. Chiudere la finestra delle proprietà di corp.

Dopo aver completato la configurazione di Active Directory e degli oggetti utente, creare due macchine virtuali SQL Server e una macchina virtuale server di controllo. Aggiungere quindi tutte e tre le macchine virtuali al dominio.
...
Per continuare, annotare l'indirizzo IP virtuale di Azure per ogni VM. Ottenere l'indirizzo IP per ogni server. Nel gruppo di risorse Azure SQL-HA-RG fare clic sulla risorsa **autohaVNET** . Il pannello **autohaVNET** mostra gli indirizzi IP per ogni computer della rete.

Registrare gli indirizzi IP per i dispositivi seguenti: 
...
Usare questi indirizzi per configurare il servizio DNS per ogni VM. Seguire questa procedura per ciascuna delle tre macchine virtuali.

1. In primo luogo, modificare l'indirizzo del server DNS preferito per ogni server membro. 
2. Avviare il file RDP nel controller di dominio primario (**ad-primary-dc**) e accedere alla VM con l'account amministratore (**BUILTIN\DomainAdmin**) e la password (**Contoso!0000**) configurati.
3. Dal controller di dominio primario avviare una sessione di Desktop remoto in **sqlserver-0** usando l'indirizzo IP. Usare lo stesso account e la stessa password.
4. Dopo l'accesso, verrà visualizzato il dashboard **Server Manager** . Nel riquadro di sinistra fare clic su **Server locale** .
...
6. Nella finestra **Connessioni di rete** selezionare l'icona della rete.
    ...
    ![Modificare il Server DNS preferito della macchina virtuale](./media/virtual-machines-windows-portal-sql-alwayson-availability-groups-manual/IC784629.png)
7. Sulla barra dei comandi fare clic su **Cambia impostazioni di connessione** (a seconda delle dimensioni della finestra, potrebbe essere necessario fare clic sulla doppia freccia destra per visualizzare questo comando).
...
10. L'indirizzo è quello assegnato a una VM nella subnet subnet-1 nella rete virtuale di Azure e la VM in questione è **ad-primary-dc**. Per verificare l'indirizzo IP di **ad-primary-dc** digitare **nslookup ad-primary-dc** nel prompt dei comandi, come illustrato di seguito.
    ...
    > 
    > 
11. Fare clic su **OK** e quindi su **Chiudi** per eseguire il commit delle modifiche. A questo punto è possibile aggiungere la macchina virtuale a **corp.contoso.com**.
12. Tornando alla finestra **Server locale**, fare clic sul collegamento **WORKGROUP**.
13. Nella sezione **Nome computer** fare clic su **Cambia**.
14. Selezionare la casella di controllo **Dominio** e digitare **corp.contoso.com** nella casella di testo. Fare clic su **OK**.
15. Nella finestra di dialogo popup **Sicurezza di Windows** specificare le credenziali per l'account amministratore di dominio predefinito (**CORP\DomainAdmin**) e la password (**Contoso!0000**).
16. uando viene visualizzato il messaggio di benvenuto nel dominio corp.contoso.com, fare clic su **OK**.
17. Fare clic su **Chiudi** e quindi su **Riavvia ora** nella finestra di dialogo popup.
18. Ripetere questi passaggi per il server di controllo di condivisione file e per ogni SQL Server. 

### <a name="add-the-corp\install-user-as-an-administrator-on-each-cluster-vm:"></a>Aggiungere l'utente Corp\Install come amministratore in ogni VM del cluster:
...
6. Immettere l'utente **CORP\Install** e quindi fare clic su **OK**. Alla richiesta delle credenziali, usare l'account **DomainAdmin** con la password **Contoso!0000**.
...
### <a name="add-the-**failover-clustering**-feature-to-each-cluster-vm."></a>Aggiungere la funzionalità **Clustering di failover** a ogni VM del cluster.
1. Usare RDP per connettersi a **sqlserver-0**.
2. Nel dashboard **Server Manager** fare clic su **Aggiungi ruoli e funzionalità**.
3. In **Aggiunta guidata ruoli e funzionalità** fare clic su **Avanti** fino a visualizzare la pagina **Funzionalità**.
4. Selezionare **Clustering di failover**. Quando richiesto, aggiungere altre eventuali funzionalità dipendenti.
...
5. Fare clic su **Avanti**, quindi su **Installa** nella pagina **Conferma**.
6. Al termine dell'installazione della funzionalità **Clustering di failover** fare clic su **Chiudi**.
7. Disconnettersi dalla macchina virtuale.
8. Ripetere i passaggi di questa sezione in **sqlserver-1** e **cluster-fsw**.
...
In questa sezione si crea il cluster WSFC in cui verrà ospitato il gruppo di disponibilità. A questo punto sono state eseguite le operazioni indicate di seguito per ognuna delle tre macchine virtuali presenti nel cluster WSFC:
...
2. Modificare l'indirizzo IP del cluster in un indirizzo IP non usato in **sqlsubnet**.
3. Portare il nome del cluster online.
4. Aggiungere gli altri nodi (**sqlserver-1** e **cluster-fsw**).

Seguire questa procedura per configurare il cluster.
...
| Selezione dei server |Digitare **sqlserver-0** in **Nome del server** e fare clic su **Aggiungi** |
...
> [!NOTE]
> Se si usa la funzionalità [Spazi di archiviazione](https://technet.microsoft.com/library/hh831739), che raggruppa più dischi in pool di archiviazione, è necessario deselezionare la casella di controllo **Aggiungi tutte le risorse di archiviazione idonee al cluster** nella pagina **Conferma**. Se non si deseleziona questa opzione, Windows disconnette i dischi virtuali durante il processo di clustering. Di conseguenza, tali dischi non vengono visualizzati in Gestione disco o in Esplora risorse fino a quando gli spazi di archiviazione non vengono rimossi dal cluster e ricollegati tramite PowerShell.
> 
> 
...
3. Selezionare **Indirizzo IP statico** e specificare un indirizzo disponibile da subnet-2 nella casella di testo Indirizzo. Fare quindi clic su **OK**.
4. Nella sezione **Risorse principali del cluster** fare clic con il pulsante destro del mouse su **Nome: Cluster1** e scegliere **Porta online**. Attendere finché entrambe le risorse non sono online Quando la risorsa del nome cluster torna online, il server del controller di dominio viene aggiornato con un nuovo account del computer Active Directory. Usare l'account di Active Directory per seguire il servizio del cluster del gruppo di disponibilità in un secondo momento.
5. Infine, si aggiungono i rimanenti nodi al cluster Nella struttura del browser fare clic con il pulsante destro del mouse su **Cluster1.corp.contoso.com** e scegliere **Aggiungi nodo**, come mostrato di seguito.
   
    ![Aggiungere un nodo al cluster](./media/virtual-machines-windows-portal-sql-alwayson-availability-groups-manual/IC784634.png)
6. In **Aggiunta guidata nodi** fare clic su **Avanti**. Nella pagina **Selezione dei server** aggiungere **sqlserver-1** e **cluster-fsw** all'elenco, digitando il nome del server in **Nome del server** e quindi facendo clic su **Aggiungi**. Al termine dell'operazione, scegliere **Avanti**.
7. Nella pagina **Avviso di convalida** fare clic su **No**. In uno scenario di produzione è necessario eseguire i test di convalida. Quindi fare clic su **Next**.
8. Nella pagina **Conferma** fare clic su **Avanti** per aggiungere i nodi.
   ...
   > 
   > 
9. Una volta aggiunti i nodi al cluster, scegliere **Fine**. A questo punto, Gestione cluster di failover visualizza il cluster con i tre nodi elencati nel contenitore **Nodi** .
10. Disconnettersi dalla sessione desktop remoto.

## <a name="configure-availability-groups"></a>Configurare gruppi di disponibilità
...
* Aprire il firewall per l'accesso remoto a SQL Server per la porta probe di SQL Server, dell'endpoint di mirroring del database e di Azure Load Balancer 
...
### <a name="add-installation-account-as-sysadmin-fixed-server-role-on-each-sql-server"></a>Aggiungere l'account di installazione come ruolo predefinito del server sysadmin in ogni istanza di SQL Server
1. Disconnettersi dalla sessione desktop remoto, se non è ancora stata eseguita questa operazione.
2. Avviare i file RDP per **sqlserver-0** e **sqlserver-1** e accedere come **BUILTIN\DomainAdmin**.
3. Avviare **SQL Server Management Studio** e aggiungere **CORP\Install** come ruolo **sysadmin** all'istanza predefinita di SQL Server. In **Esplora oggetti** fare clic con il pulsante destro del mouse su **Account di accesso** e scegliere **Nuovo account di accesso**.
4. Digitare **CORP\Install** in **Nome account di accesso**.
5. Nella pagina **Ruoli del server** selezionare **sysadmin**. Fare quindi clic su **OK**. Una volta creato l'account di accesso, è possibile visualizzarlo espandendo **Account di accesso** in **Esplora oggetti**.

### <a name="open-the-firewall-for-remote-access-to-sql-server-and-the-probe-port-on-each-sql-server"></a>Aprire il firewall per l'accesso remoto a SQL Server e la porta probe in ogni istanza di SQL Server
...
3. Nella pagina **Tipo di regola** selezionare **Programma**, quindi fare clic su **Avanti**.
4. Nella pagina **Programma** selezionare **Percorso programma** e digitare **%ProgramFiles%\Microsoft SQL Server\MSSQL12.MSSQLSERVER\MSSQL\Binn\sqlservr.exe** nella casella di testo. Se si seguono queste istruzioni, ma si usa SQL Server 2012, la directory di SQL Server è **MSSQL11.MSSQLSERVER**. Quindi fare clic su **Next**.
...
7. Nella pagina **Nome** specificare un nome per la regola, ad esempio **SQL Server (Regola di programma)** nella casella di testo **Nome**, quindi fare clic su **Fine**.
8. Creare una regola del firewall in ingresso aggiuntiva per la porta probe. Si tratta di una regola in ingresso alla porta TCP 59999, ai fini di questa esercitazione. Assegnare alla regola il nome **Listener SQL Server**.

Completare tutti i passaggi in entrambe le istanze di SQL Server.

### <a name="enable-availability-groups-feature-on-each-sql-server"></a>Abilitare la funzionalità Gruppi di disponibilità in ogni istanza di SQL Server.
...
3. Fare clic sulla scheda **Disponibilità elevata AlwaysOn**, selezionare **Abilita gruppi di disponibilità AlwaysOn** come mostrato di seguito, quindi scegliere **Applica**. Fare clic su **OK** nella finestra di dialogo popup e non chiudere ancora la finestra delle proprietà. Verrà riavviato il servizio SQL Server dopo la modifica dell'account del servizio.
...
### <a name="set-the-sql-server-service-account-on-each-sql-server"></a>Impostare l'account del servizio SQL Server in ogni istanza di SQL Server
Eseguire questi passaggi in entrambe le istanze di SQL Server.
...
2. Nella finestra popup fare clic su **Sì** per riavviare il servizio SQL Server. Dopo il riavvio del servizio SQL Server, vengono applicate le modifiche apportate nella finestra delle proprietà.
3. Disconnettersi dalle macchine virtuali.
...
### <a name="create-the-mydb1-database-on-sqlserver-0:"></a>Creare il database MyDB1 in sqlserver-0:
1. Disconnettersi dalle sessioni desktop remoto per **sqlserver-0** e **sqlserver-1**, se non lo si è ancora fatto.
2. Avviare il file RDP per **sqlserver-0** e accedere come **CORP\Install**.
...
5. Aggiungere **CORP\SQLSvc1** e assegnargli l'autorizzazione **Lettura/Scrittura**, quindi aggiungere **CORP\SQLSvc2** e assegnargli l'autorizzazione **Lettura/Scrittura** come mostrato di seguito, quindi fare clic su **Condividi**. Una volta completato il processo di condivisione file, fare clic su **Fatto**.
...
### <a name="take-a-full-backup-of-mydb1-and-restore-it-on-sqlserver-1:"></a>Eseguire un backup completo di MyDB1 e ripristinarlo in sqlserver-1:
...
4. Nella casella di testo **Nome file** digitare **\\\\sqlserver-0\backup\MyDB1.bak**. Fare clic su **OK** e quindi fare di nuovo clic su **OK** per eseguire il backup del database. Una volta completata l'operazione di backup, fare di nuovo clic su **OK** per chiudere la finestra di dialogo.
...
7. Si procede quindi al ripristino del backup completo e del log delle transazioni in **sqlserver-1**. Avviare il file RDP per **sqlserver-1** e accedere come **CORP\Install**. Lasciare aperta la sessione desktop remoto per **sqlserver-0** .
...
12. Nel percorso di file di Backup digitare **\\\\sqlserver-0\backup**, quindi fare clic su Aggiorna, selezionare MyDB1.bak, quindi fare clic su OK e nuovamente su OK. A questo punto vengono visualizzati il backup completo e il backup del log nel riquadro.
...
### <a name="create-the-availability-group:"></a>Creare il gruppo di disponibilità:
1. Tornare alla sessione desktop remoto per **sqlserver-0**. In **Esplora oggetti** in SSMS fare clic con il pulsante destro del mouse su **Disponibilità elevata AlwaysOn**, quindi scegliere **Creazione guidata Gruppo di disponibilità**, come mostrato di seguito.
...
5. Viene visualizzata la finestra di dialogo **Connetti al server** . Digitare **sqlserver-1** in **Nome server**, quindi fare clic su **Connetti**.
...
6. Nella pagina **Specifica repliche** verrà ora visualizzato **sqlserver-1** elencato in **Repliche di disponibilità**. Configurare le repliche come mostrato di seguito: 
...
11. In **Esplora oggetti** espandere **Disponibilità elevata AlwaysOn**, quindi espandere **Gruppi di disponibilità**. A questo punto viene visualizzato il nuovo gruppo di disponibilità in questo contenitore. Fare clic con il pulsante destro del mouse su **AG1 (Primary)** e selezionare **Show Dashboard** (Mostra dashboard).
...
12. L'aspetto del **Dashboard AlwaysOn** dovrebbe essere simile a quello riportato di seguito. È possibile visualizzare le repliche, la modalità di failover di ciascuna replica e lo stato di sincronizzazione.
...
## <a name="configure-internal-load-balancer"></a>Configurare il servizio di bilanciamento del carico interno
Per connettersi direttamente al gruppo di disponibilità, è necessario configurare un servizio di bilanciamento del carico. Il servizio di bilanciamento del carico indirizza il traffico client solo alla macchina virtuale associata all'indirizzo IP del listener e alla porta probe. Questa esercitazione usa un servizio di bilanciamento del carico interno. Tale servizio consente al traffico proveniente dalla stessa rete virtuale di connettersi a SQL Server. Le applicazioni che devono connettersi a SQL Server su Internet richiedono un servizio di bilanciamento del carico esterno, ovvero con connessione Internet. Per altre informazioni, vedere [Panoramica di Azure Load Balancer](../load-balancer/load-balancer-overview.md).

> [!NOTE]
> Attualmente il portale di Azure consente di usare una sola volta una specifica porta front-end in un servizio di bilanciamento del carico. Per usare la stessa porta per tutti i listener, usare PowerShell per collegare gli indirizzi IP dei listener al servizio di bilanciamento del carico. Per istruzioni, vedere [Create availability group listener and load balancer | Azure](virtual-machines-windows-portal-sql-ps-alwayson-int-listener.md)(Creare listener del gruppo di disponibilità e bilanciamento del carico | Azure). 
> 
> 

### <a name="create-the-load-balancer-in-azure"></a>Creare il servizio di bilanciamento del carico in Azure
1. Nel portale di Azure passare a **SQL-HA-RG** e fare clic su **+ Aggiungi**.
2. Cercare **Servizio di bilanciamento del carico**. Scegliere il servizio di bilanciamento del carico pubblicato da Microsoft e fare clic su **Crea**.
...
| **Regole di bilanciamento del carico Porta** |1433 * |
...
> * La porta predefinita di SQL Server è la 1433. Usare la porta front-end per l'istanza predefinita. Se sono necessari più gruppi di disponibilità, creare indirizzi IP aggiuntivi per ciascun gruppo di disponibilità. Ciascun gruppo di disponibilità richiede una propria porta front-end. Vedere [Create availability group listener and load balancer | Azure](virtual-machines-windows-portal-sql-ps-alwayson-int-listener.md)(Creare listener del gruppo di disponibilità e bilanciamento del carico | Azure).
> 
> [!NOTE]
> È necessario abilitare Direct Server Return nelle regole di bilanciamento del carico al momento della creazione.
> 
> 
...
2. In Gestione cluster di failover prendere nota del nome della rete di cluster. Per determinare il nome della rete di cluster in **Gestione cluster di failover**, fare clic su **Reti** nel pannello sinistro. Si userà questo nome nella variabile `$ClusterNetworkName` nello script di PowerShell.
3. In Gestione cluster di failover espandere il nome del cluster e fare clic su **Ruoli**.
4. In **Ruoli** fare clic con il pulsante destro del mouse sul nome del gruppo di disponibilità e quindi scegliere **Aggiungi risorsa** > **Punto di accesso client**. 
5. In **Nome** digitare **aglistener**. Fare clic su **Avanti** due volte e quindi su **Fine**. Non portare il listener o la risorsa in linea a questo punto.
6. Scegliere la scheda **Risorse** , quindi espandere il punto di accesso Client appena creato. Fare clic con il pulsante destro del mouse sulla risorsa IP e scegliere Proprietà. Prendere nota del nome dell'indirizzo IP. Usare questo nome nella variabile `$IPResourceName` nello script di PowerShell.
7. In **Indirizzo IP** fare clic su **Indirizzo IP statico** e impostare l'indirizzo IP statico sullo stesso indirizzo usato nel portale di Azure per il servizio di bilanciamento del carico **sqlLB**. Usare questo stesso indirizzo IP nella variabile `$ILBIP` nello script di PowerShell. 
8. Disabilitare NetBIOS per questo indirizzo e fare clic su **OK**. 
9. Nel nodo del cluster che attualmente ospita la replica primaria aprire un'istanza di PowerShell ISE con privilegi elevati e incollare i comandi seguenti in un nuovo script.
    ...
    ```
10. Aggiornare le variabili ed eseguire lo script di PowerShell per configurare l'indirizzo IP e la porta per il nuovo listener.
11. In **Gestione cluster di failover** fare clic con il pulsante destro del mouse sulla risorsa del gruppo di disponibilità e scegliere **Proprietà**. Nella scheda **Dipendenze** impostare il gruppo di risorse come dipendente dal nome della rete del listener. 
12. Impostare la proprietà Porta di attesa su 1433. A questo scopo, aprire SQL Server Management Studio, fare clic con il pulsante destro del mouse sul listener del gruppo di disponibilità e scegliere Proprietà. Impostare **Porta** su 1433. Usare lo stesso numero di porta configurato per SQL Server. 
13. A questo punto è possibile portare online il listener. Fare clic con il pulsante destro del mouse sul nome del listener e scegliere **Porta in linea**.
...
2. Usare l'utilità sqlcmd per testare la connessione. Lo script seguente, ad esempio, stabilisce una connessione sqlcmd alla replica primaria tramite il listener con l'autenticazione di Windows:
   ...
   Se il listener usa un numero di porta successivo a 1433, è necessario specificare tale numero nel test. La query seguente, ad esempio, testa la connettività al nome del listener usando la porta 1435:
