...
Il primo passaggio viene eseguito tramite il [portale di Azure classico][portale di Azure classico]. La schermata seguente illustra la modalità di creazione di una macchina virtuale CentOS denominata "scott-centos":
...
Per le app PHP e Python, questo esempio usa le librerie client Proton di Apache. Queste librerie sono disponibili per il download da [http://qpid.apache.org/download.html](http://qpid.apache.org/download.html). Il file Readme incluso nel pacchetto di distribuzione illustra i passaggi necessari per installare le dipendenze e creare la libreria Proton. Ecco un riepilogo della procedura:
...
2. Installare i pacchetti prerequisiti:
   ...
   yum install epydoc
   ```
3. Scaricare la libreria Proton:
   ...
   ```
4. Estrarre il codice Proton dall'archivio di distribuzione:
   
   ```
   tar xvfz qpid-proton-0.9.tar.gz
   ```
5. Compilare e installare il codice come illustrato nella procedura seguente, tratta dal file Readme:
...
* [Panoramica di AMQP per il bus di servizio][Panoramica di AMQP per il bus di servizio]
