<properties 
	pageTitle="Panoramica di AMQP per il bus di servizio con Java | Microsoft Azure" 
	description="Informazioni sull'uso di Java con il protocollo AMQP (Advanced Message Queuing Protocol) 1.0 in Azure." 
...
Per altre informazioni su come ottenere e usare queste librerie con il bus di servizio, vedere la [Guida per gli sviluppatori di AMQP per il bus di servizio][]. Vedere la sezione [Passaggi successivi](service-bus-java-amqp-overview.md#next-steps) per collegamenti ad altre informazioni.
...
Visitare i collegamenti seguenti per altre informazioni sul supporto per AMQP nel bus di servizio.
...
*    [Guida per sviluppatori di AMQP per il bus di servizio][]
...
[Guida per gli sviluppatori di AMQP per il bus di servizio]: service-bus-amqp-dotnet.md
