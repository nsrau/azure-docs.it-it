...
> [AZURE.NOTE] È possibile usare il metodo `listQueues` negli oggetti `ServiceBusRestProxy` per verificare se in uno spazio dei nomi esiste già una coda con il nome specificato.
...
Il modo migliore per ricevere i messaggi da una coda consiste nell'usare un metodo **ServiceBusRestProxy->receiveQueueMessage**. È possibile ricevere messaggi in due modalità diverse: **ReceiveAndDelete**, la modalità predefinita, e **PeekLock**.
...
Il seguente esempio illustra come ricevere ed elaborare messaggi usando la modalità **PeekLock** (non predefinita).
