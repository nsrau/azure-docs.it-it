...
Creare un'applicazione Node.js vuota. Per istruzioni su come creare un'applicazione Node.js, vedere [Creare e distribuire un'applicazione Node.js in un sito Web Azure][] [Servizio cloud Node.js][] usando Windows PowerShell.
...
In questo callback, e dopo l'elaborazione di **returnObject**, vale a dire la risposta della richiesta al server, il callback deve richiamare `next`, se questo esiste, per continuare a elaborare altri filtri, oppure semplicemente richiamare `finalCallback` per concludere la chiamata al servizio.
