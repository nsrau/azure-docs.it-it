...
Per altre informazioni sugli elementi interni del bus di servizio, vedere l'argomento [Architettura del bus di servizio][Architettura del bus di servizio].
...
È possibile creare code e argomenti del bus di servizio in dimensioni di 1, 2, 3, 4 o 5 GB (il valore predefinito è 1 GB). Con il partizionamento abilitato, il bus di servizio crea 16 partizioni per ogni GB specificato. Di conseguenza, se si crea una coda con dimensioni pari a 5 GB, con 16 partizioni le dimensioni massime della coda diventano di 80 GB (5 \* 16). È possibile visualizzare le dimensioni massime della coda o dell'argomento partizionato esaminando la relativa voce nel [portale di Azure][portale di Azure].
...
Per inviare un messaggio transazionale a un argomento o a una coda che riconosce la sessione, il messaggio deve avere la proprietà [BrokeredMessage.SessionId][BrokeredMessage.SessionId] impostata. Se è specificata anche la proprietà [BrokeredMessage.PartitionKey][BrokeredMessage.PartitionKey], questa deve avere un valore identico a quello della proprietà [SessionId][SessionId]. Se i valori sono diversi, il bus di servizio restituisce un'eccezione **InvalidOperationException**.
...
Per altre informazioni sul partizionamento delle entità di messaggistica, vedere [Supporto di AMQP 1.0 per code e argomenti partizionati del bus di servizio][Supporto di AMQP 1.0 per code e argomenti partizionati del bus di servizio]. 
