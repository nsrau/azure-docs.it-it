...
Questo articolo illustra come usare il portale di Azure per creare una connessione gateway VPN da sito a sito dalla rete locale alla rete virtuale. The steps in this article apply to the classic deployment model and do not apply to the current deployment model, Resource Manager. È anche possibile creare questa configurazione usando strumenti o modelli di distribuzione diversi selezionando un'opzione differente nell'elenco seguente:
...
* PowerShell è necessario per specificare la chiave condivisa e creare la connessione al gateway VPN. [!INCLUDE [vpn-gateway-classic-powershell](../../includes/vpn-gateway-powershell-classic-locally.md)]
...
1. Da un browser passare al portale di Azure e, se necessario, accedere con l'account Azure.From a browser, navigate to the [Azure portal](https://portal.azure.com) and, if necessary, sign in with your Azure account.
...
6. Fare clic su **Gruppo di risorse** e selezionare un gruppo di risorse esistente o crearne uno nuovo digitandone il nome. Per altre informazioni sui gruppi di risorse, vedere Panoramica di Azure Resource Manager.For more information about resource [groups,](../azure-resource-manager/management/overview.md#resource-groups)visit Azure Resource Manager Overview .
...
## <a name="2-add-additional-address-space"></a><a name="additionaladdress"></a>2. Aggiungere ulteriore spazio di indirizzi
...
1. Nella pagina della rete virtuale fare clic su **Diagramma**in **Impostazioni**.
1. Nella pagina **Connessioni VPN** fare clic su Non sono presenti connessioni **VPN esistenti. Clicca qui per iniziare**.
...
1. Selezionare le **dimensioni** del gateway, che rappresentano lo SKU di gateway usato per creare il gateway di rete virtuale. I gateway VPN classici usano gli SKU del gateway legacy. Per altre informazioni sugli SKU del gateway legacy, vedere [Lavorare con gli SKU del gateway di rete virtuale (SKU precedenti)](vpn-gateway-about-skus-legacy.md).
   ...
   ![Selezionare SKUL e il tipo VPN](./media/vpn-gateway-howto-site-to-site-classic-portal/sku.png "Selezionare SKU e tipo VPN")
...
4. Nella pagina **Nuova connessione VPN** fare clic su **OK** nella parte inferiore della pagina per iniziare a distribuire il gateway di rete virtuale. A seconda dello SKU selezionato, possono essere necessari fino a 45 minuti per creare un gateway di rete virtuale.
...
È necessario eseguire questi comandi in locale usando il modulo di gestione del servizio PowerShell.You must run these commands locally using the PowerShell service management module. 
...
Quando si crea una rete virtuale classica nel portale (non si usa PowerShell), Azure aggiunge il nome del gruppo di risorse al nome breve. Ad esempio, in base ad Azure, il nome della rete virtuale creata per questo esercizio è "TestDigruppo TestVNet1", non "TestVNet1". PowerShell richiede il nome completo della rete virtuale, non il nome breve visualizzato nel portale. Il nome lungo non è visibile nel portale. I passaggi seguenti consentono di esportare il file di configurazione di rete per ottenere i valori esatti per il nome della rete virtuale. 
...
1. Creare una directory nel computer ed esportarvi il file di configurazione di rete. In questo esempio il file di configurazione di rete viene esportato in C:\AzureNet.
   ...
   ```
2. Aprire il file di configurazione di rete con un editor xml e controllare i valori di "LocalNetworkSite name" e "VirtualNetworkSite name". Modificare l'esempio per questo esercizio per riflettere i valori nel file xml. Quando si specifica un nome che contiene spazi, racchiudere il valore tra virgolette singole.
