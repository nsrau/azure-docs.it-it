...
È necessario ottenere il file CER per il certificato radice. È possibile usare un certificato radice da una soluzione aziendale oppure è possibile [creare un certificato radice autofirmato usando makecert](vpn-gateway-certificates-point-to-site.md). Anche se è possibile usare PowerShell per creare certificati autofirmati, il certificato generato con PowerShell non contiene i campi necessari per le connessioni P2S.

1. Per ottenere un file CER da un certificato, aprire **certmgr.msc** e individuare il certificato radice. Fare clic con il pulsante destro del mouse sul certificato radice autofirmato, scegliere **Tutte le attività** e quindi fare clic su **Esporta**. Verrà aperta l'**Esportazione guidata certificati**.
...
3. Installare il pacchetto nel computer client. Se viene visualizzato un popup SmartScreen, fare clic su **More info** (Altre informazioni), quindi su **Esegui comunque** per installare il pacchetto.
...
È possibile gestire l'elenco di certificati client revocati nel pannello **Configurazione da punto a sito** che è stato usato per [caricare un certificato radice attendibile](#uploadfile). Aggiungere il nome del certificato e l'identificazione personale, quindi salvare.
