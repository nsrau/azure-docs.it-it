...
Un **endpoint** che implementa un **endpoint** di risorsa "proxy" deve gestire la richiesta e la risposta per la nuova API in Azure. In questo caso, il **ResourceType** genererà una nuova API di risorse di Azure per `PUT`, `GET`e `DELETE` per eseguire CRUD su una singola risorsa, nonché `GET` per recuperare tutte le risorse esistenti.

> [!NOTE]
> I campi `id`, `name`e `type` non sono necessari, ma sono necessari per integrare la risorsa personalizzata con l'ecosistema di Azure esistente.
...
- Documento oggetto JSON valido. Tutte le matrici e le stringhe devono essere annidate in un oggetto top.
- L'intestazione di `Content-Type` deve essere impostata su "Application/JSON; charset = UTF-8 ".
- L'elenco delle risorse deve essere inserito sotto la proprietà `value` di primo livello.
