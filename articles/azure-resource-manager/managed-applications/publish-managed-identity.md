...
Oltre ad aggiornare il CreateUIDefinition, è necessario aggiornare anche il modello principale in modo da accettare l'ID della risorsa collegata passata. Il modello principale può essere aggiornato in modo da accettare il nuovo output aggiungendo un nuovo parametro. Poiché l'output del `managedIdentity` sostituisce il valore nel modello di applicazione gestita generato, non viene passato al modello principale e non deve essere incluso nella sezione Parameters.
