...
Gestione risorse fornisce le funzioni seguenti per ottenere i valori delle risorse nel modello di Azure Resource Manager (ARM):
...
Le funzioni list possono essere utilizzate solo nelle proprietà di una definizione di risorsa e nella sezione Outputs di un modello o di una distribuzione. Quando viene utilizzato con l' [iterazione della proprietà](copy-properties.md), è possibile utilizzare `input` le funzioni elenco per perché l'espressione viene assegnata alla proprietà della risorsa. Non è possibile usarle `count` con perché è necessario determinare il conteggio prima che la funzione elenco venga risolta.
...
In genere, si usa la funzione **Reference** per restituire un valore specifico da un oggetto, ad esempio l'URI dell'endpoint BLOB o il nome di dominio completo.
...
La funzione reference può essere usata solo nelle proprietà di una definizione di risorsa e nella sezione outputs di un modello o una distribuzione. Quando viene usata con l' [iterazione della proprietà](copy-properties.md), è possibile usare `input` la funzione Reference per perché l'espressione viene assegnata alla proprietà della risorsa.

Non è possibile usare la funzione Reference per impostare il valore della `count` proprietà in un ciclo di copia. È possibile utilizzare per impostare altre proprietà del ciclo. Il riferimento è bloccato per la proprietà Count perché tale proprietà deve essere determinata prima che la funzione di riferimento venga risolta.
...
Quando si fa riferimento a una risorsa che non viene distribuita nello stesso modello, fornire `apiVersion`l'ID risorsa e.
...
Per semplificare la creazione di un ID di risorsa, usare `resourceId()` le funzioni descritte in questo documento anziché la `concat()` funzione.
