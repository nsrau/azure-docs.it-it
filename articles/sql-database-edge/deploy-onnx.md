...
Usando `skl2onnx`, convertire il modello LinearRegression nel formato ONNX e salvarlo localmente.
...
Archiviare il modello nel database SQL di Azure Edge, in `models` una tabella di un `onnx`database. Nella stringa di connessione specificare l' **indirizzo del server**, il **nome utente**e la **password**.
...
Infine, usare `sqlalchemy` per inserire i `x_train` frame `y_train` di dataframe e Pandas rispettivamente `features` nelle `target`tabelle e. 
