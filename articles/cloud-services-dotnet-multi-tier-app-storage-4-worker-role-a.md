...
-   Testare con Esplora server un ruolo di lavoro che usa le tabelle di Archiviazione di Azure.

Il progetto di ruolo di lavoro A è già stato creato insieme al progetto di servizio cloud. A questo punto è necessario programmare il ruolo di lavoro.
...
1.  Fare clic con il pulsante destro del mouse sul progetto WorkerRoleA, scegliere **Aggiungi** e quindi **Riferimento**.
...
-   Consentono al ruolo di lavoro A di determinare se sono stati inviati tutti i messaggi di posta elettronica relativi a uno specifico messaggio in modo da contrassegnarlo come `Complete`.
...
1.  Nella cartella *Models* del progetto Web aggiungere il file *SendEmail.cs* dal progetto scaricato.
...
## <a name="addcode"></a><span class="short-header">Aggiunta del codice del ruolo di lavoro</span>Aggiunta del codice da eseguire all'avvio del ruolo di lavoro

Nel progetto WorkerRoleA il modello di progetto ha creato *WorkerRole.cs* con il codice seguente:
...
1.  Eliminare *WorkerRole.cs* nel progetto WorkerRoleA, quindi aggiungere il file *WorkerRoleA.cs* dal progetto scaricato.

### Metodo OnStart

Il metodo `OnStart` inizializza gli oggetti di contesto necessari per gestire le entità di Archiviazione di Azure. Verifica inoltre l'esistenza di tutte le tabelle, le code e i contenitori BLOB che verranno usati nel metodo `Run`. Il codice che esegue queste attività è simile a quello illustrato in precedenza nei costruttori dei controller MVC. In seguito verrà configurata la stringa di connessione utilizzata da questo metodo.
...
Il metodo `OnStop` viene chiamato quando il ruolo di lavoro sta per essere arrestato per uno dei motivi seguenti:
...
-   Analizza la tabella `message` per verificare se sono presenti messaggi pianificati per l'invio nella data attuale o in una data precedente, per cui non sono stati ancora creati elementi di lavoro della coda.

-   Analizza la tabella `message` per verificare se sono presenti messaggi con uno stato indicante che sono stati creati tutti gli elementi di lavoro della coda ma non sono stati ancora inviati tutti i messaggi di posta elettronica. Se trova un messaggio di questo tipo, analizza le relative righe `SendEmail` per verificare se sono stati inviati tutti i messaggi di posta elettronica e, in caso affermativo, aggiorna lo stato impostandolo su `Completed` e archivia la riga `message`.
...
In seguito il codice imposta le righe `message` con lo stato `Pending` su `Queuing`. Di conseguenza, per tali righe, oltre a quelle che sono già con stato `Queuing`, chiama il metodo `ProcessMessage` per creare gli elementi di lavoro della coda e inviare i messaggi di posta elettronica per il messaggio.
...
Esegue quindi una query sulla tabella `mailinglist` per trovare tutte le righe del sottoscrittore per la lista di distribuzione di destinazione.
...
### Metodo CheckAndUpdateStatusIfComplete

Il metodo `CheckAndUpdateStatusIfComplete` controlla tutti i messaggi con stato Processing per verificare se sono stati inviati tutti i messaggi di posta elettronica. Se non rileva messaggi di posta elettronica non inviati, aggiorna lo stato della riga impostandolo su `Completed` e archivia la riga.
...
## <a name="testing"></a>Test del ruolo di lavoro A

1.  Premere F5 per eseguire l'applicazione.

2.  Utilizzare le pagine Web dell'amministratore per creare una lista di distribuzione e i relativi sottoscrittori. Impostare la proprietà `Verified` su `true` per almeno un sottoscrittore e specificare un indirizzo di posta elettronica a cui è possibile ricevere posta.
...
6.  Espandere **Code**, fare clic con il pulsante destro del mouse su **azuremailqueue**, quindi fare clic su **Visualizza coda**.
...
9.  Espandere il nodo **Tabelle**, fare clic con il pulsante destro del mouse sulla tabella **Message**, quindi scegliere **Visualizza tabella**.
...
<div><a href="/it-it/develop/net/tutorials/multi-tier-web-site/5-worker-role-b/" class="site-arrowboxcta download-cta">Esercitazione 5</a></div>

  [prima esercitazione della serie]: /it-it/develop/net/tutorials/multi-tier-web-site/1-overview/
  [Aggiunta di un riferimento al progetto Web]: #addref
  [Aggiunta del modello SendEmail]: #addmodel
  [Aggiunta del codice da eseguire all'avvio del ruolo di lavoro]: #addcode
  [Test del ruolo di lavoro A]: #testing
  ...
  [Tabella message con SendMail]: ./media/cloud-services-dotnet-multi-tier-app-storage-1-worker-role-a/mtas-sendMailTbl.png
  [seconda esercitazione]: /it-it/develop/net/tutorials/multi-tier-web-site/2-download-and-run/
  [ServicePointManager.DefaultConnectionLimit]: http://msdn.microsoft.com/it-it/library/system.net.servicepointmanager.defaultconnectionlimit.aspx
  [ServicePointManager]: http://msdn.microsoft.com/it-it/library/system.net.servicepointmanager.aspx
  ...
  [UnhandledException]: http://msdn.microsoft.com/it-it/library/system.appdomain.unhandledexception.aspx
  ...
  [Riga messaggio]: ./media/cloud-services-dotnet-multi-tier-app-storage-1-worker-role-a/mtas-worker-a-test-ase-message-table.png
  ...
  [esercitazione successiva]: /it-it/develop/net/tutorials/multi-tier-web-site/5-worker-role-b/
