...
![Pagina Criteri - Conformità](media/policy-compliance/compliance-page.png)

Dal momento che un criterio o un'iniziativa può essere assegnata a diversi ambiti, si noti che nella tabella sono riportati l'ambito per ogni assegnazione e il tipo di definizione assegnato a tale ambito. È inoltre indicato il numero di criteri non conformi e di risorse non conformi per ogni assegnazione. Quando si fa clic su un criterio o su un'iniziativa nella tabella, viene fornita un'analisi più approfondita della conformità per quella particolare assegnazione.

![Dettagli della conformità ai criteri](media/policy-compliance/compliance-details.png)

Mentre l'elenco delle risorse nella scheda **Risorse non conformi** riflette lo stato di valutazione delle risorse esistenti per l'assegnazione corrente, gli eventi (Append, Audit, Deny, Deploy) attivati dalla richiesta per creare una risorsa sono visualizzati nella scheda **Eventi**.

![Eventi di conformità ai criteri](media/policy-compliance/compliance-events.png)
...
![Log attività della conformità ai criteri](media/policy-compliance/compliance-activitylog.png)
...
### <a name="azure-powershell-preview"></a>Azure PowerShell (anteprima)

Il modulo Azure PowerShell per Criteri non è ancora finale, ma è attualmente disponibile in PowerShell Gallery come [versione di anteprima](https://www.powershellgallery.com/packages/AzureRM.PolicyInsights).
Se la versione di PowerShellGet è almeno 1.6.0 (necessaria per supportare gli elementi della versione preliminare), è possibile scaricare la versione di anteprima usando `Install-Module` (assicurarsi di avere la versione più recente di [Azure PowerShell](/powershell/azure/install-azurerm-ps) installata):
...
Il modulo di anteprima ha tre cmdlet:
...
![Conformità ai criteri con Log Analytics](media/policy-compliance/compliance-loganalytics.png)

## <a name="next-steps"></a>Passaggi successivi

- Vedere [Struttura delle definizioni di criteri di Azure](policy-definition.md).
- Leggere [Informazioni sugli effetti di Criteri](policy-effects.md).
