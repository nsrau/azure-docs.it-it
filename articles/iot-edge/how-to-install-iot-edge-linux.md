...
## <a name="install-container-runtime-and-iot-edge"></a>Installare il runtime del contenitore e IoT Edge
...
Se la versione che si vuole installare non è elencata, seguire la procedura descritta in [Install Runtime using Release assets](#install-runtime-using-release-assets). In questa sezione viene illustrato come specificare come destinazione qualsiasi versione precedente del daemon di sicurezza IoT Edge o versione finale candidata.

Una volta che IoT Edge è stato installato correttamente in `/etc/iotedge/` , l'output richiede di aggiornare il file di configurazione. Passare alla sezione successiva per completare il provisioning dei dispositivi.
...
Eseguire lo [strumento per la risoluzione dei problemi](troubleshoot.md#run-the-check-command) per verificare la configurazione e gli errori di rete più comuni:
...
Se il dispositivo non è in grado di connettersi all'hub Internet e la rete ha un server proxy, seguire i passaggi descritti in [configurare il dispositivo IOT Edge per la comunicazione tramite un server proxy](how-to-configure-proxy-support.md).
...
## <a name="install-runtime-using-release-assets"></a>Installare il runtime usando gli asset di versione
...
Usando i comandi CURL, è possibile indirizzare i file dei componenti direttamente dal repository GitHub IoT Edge. Usare la procedura seguente per installare libiothsm e il daemon di sicurezza IoT Edge.

1. Preparare il dispositivo con un motore di contenitori installato. Se non è presente un motore di contenitori, seguire i passaggi per registrare il repository Microsoft e installare Moby nella sezione [Install container runtime and IOT Edge](#install-container-runtime-and-iot-edge) di questo articolo.

2. Passare alla [Azure IOT Edge versioni](https://github.com/Azure/azure-iotedge/releases)e individuare la versione di rilascio di destinazione.

3. Espandere la sezione **Asset** per la versione.
...
Una volta che IoT Edge è stato installato correttamente in `/etc/iotedge` , l'output richiede di aggiornare il file di configurazione. Seguire i passaggi nella sezione [configurare il daemon di sicurezza](#configure-the-security-daemon) per completare il provisioning dei dispositivi.
