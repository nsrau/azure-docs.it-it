...
Esistono due modi per eseguire il debug di moduli scritti in C , Node.js o Java nel codice di Visual Studio: è possibile collegare un processo in un contenitore di moduli o avviare il codice del modulo in modalità di debug. Per eseguire il debug di moduli scritti in Python o C, è possibile connettersi solo a un processo in contenitori Linux amd64.To debug modules written in Python or C, you can only attach to a process in Linux amd64 containers.
...
In questo articolo vengono fornite istruzioni per lo sviluppo e il debug di moduli in più linguaggi per più architetture. Attualmente, Visual Studio Code fornisce il supporto per i moduli scritti in C , C , Python, Node.js e Java. Le architetture dei dispositivi supportate sono X64 e ARM32. Per ulteriori informazioni sui sistemi operativi, i linguaggi e le architetture supportati, vedere [Supporto di linguaggi e architetture](module-development.md#language-and-architecture-support).
...
- Estensioni Visual Studio specifiche per il linguaggio usato per lo sviluppo:
  - C, incluse le funzioni di Azure: [estensione di C](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.csharp)
  - Python: [Estensione Python](https://marketplace.visualstudio.com/items?itemName=ms-python.python)
  - Java: [Java Extension Pack per il codice Visual Studio](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-java-pack)
  - C: [Estensione C/C](https://marketplace.visualstudio.com/items?itemName=ms-vscode.cpptools)

Dovrai anche installare alcuni strumenti aggiuntivi specifici per la lingua per sviluppare il tuo modulo:
...
Per compilare e distribuire l'immagine del modulo, è necessario Docker per compilare l'immagine del modulo e un registro contenitori per contenere l'immagine del modulo:To build and deploy your module image, you need Docker to build the module image and a container registry to hold the module image:

- [Docker Community Edition](https://docs.docker.com/install/) nel computer di sviluppo.

- [Registro di](https://docs.microsoft.com/azure/container-registry/) sistema del contenitore di Azure o hub [DockerAzure](https://docs.docker.com/docker-hub/repos/#viewing-repository-tags) Container Registry or Docker Hub

    > [!TIP]
    > Per prototipi e test è possibile usare un registro Docker locale anziché un registro nel cloud.

A meno che il modulo non venga sviluppato in C, è necessario anche lo [strumento di sviluppo Azure IoT EdgeHub](https://pypi.org/project/iotedgehubdev/) basato su Python per configurare l'ambiente di sviluppo locale in modo da eseguire e testare la soluzione IoT Edge ed eseguirne il debug. Se non è già stato fatto, installare [Python (2.7/3.6/3.7) e Pip](https://www.python.org/) e quindi installare **iotedgehubdev** eseguendo questo comando nel terminale.
...
> [!NOTE]
> Attualmente, iotedgehubdev utilizza una libreria docker-py che non è compatibile con Python 3.8.
>
> Se si dispone di più Python tra cui python preinstallato 2.7 (ad esempio, su `pip` Ubuntu o macOS), assicurarsi di utilizzare il corretto o `pip3` di installare **iotedgehubdev**
...
1. Nella tavolozza dei comandi immettere ed eseguire il comando **Azure IoT Edge: New IoT Edge Solution**.
...
- Una cartella **modules** contenente le sottocartelle di ogni modulo.  All'interno della cartella per ogni modulo è presente un file, **module.json**, che controlla la modalità di compilazione e distribuzione dei moduli.  Questo file deve essere modificato per modificare il registro del contenitore di distribuzione del modulo da localhost a un registro remoto. A questo punto, si dispone di un solo modulo.  È possibile tuttavia aggiungere altri elemento nel riquadro comandi con il comando **Azure IoT Edge: Aggiungi modulo Edge IoT**.
...
Per vedere come funziona il modulo di temperatura simulato, visualizzare il [codice sorgente SimulatedTemperatureSensor.csproj](https://github.com/Azure/iotedge/tree/master/edge-modules/SimulatedTemperatureSensor).

## <a name="add-additional-modules"></a>Aggiungere altri moduli

Per aggiungere altri moduli alla soluzione, eseguire il comando **Azure IoT Edge: Add IoT Edge Module** dalla tavolozza dei comandi. È anche possibile fare clic con il pulsante destro del mouse sulla cartella **moduli** o sul file `deployment.template.json` nella visualizzazione Explorer di Visual Studio Code e poi selezionare **Aggiungi modulo di IoT Edge**.
...
- Funzione di Azure (c'è): **moduli > * &lt;il nome&gt;* > *&lt;&gt;*** del modulo nome del modulo .cs
...
1. Preparare l'ambiente per il debug in base ai requisiti del linguaggio di sviluppo, impostare un punto di interruzione nel modulo e selezionare la configurazione di debug da usare:
   - **C #**
     - Nel terminale integrato Visual Studio Code *** &lt;&gt; *** modificare la directory nella cartella del nome del modulo, quindi eseguire il comando seguente per compilare l'applicazione .NET Core.
     ...
     - Passare alla visualizzazione Debug di Visual Studio Code selezionando **Visualizza > Debug**. Selezionare la ** * &lt;&gt; * ** configurazione di debug il nome del modulo Local Debug (.NET Core) dall'elenco a discesa.

        > [!NOTE]
        > Se .NET `TargetFramework` Core non è coerente `launch.json`con il percorso del programma in `launch.json` , sarà `TargetFramework` necessario aggiornare manualmente il percorso del programma in modo che corrisponda al file con estensione csproj in modo che Visual Studio Code possa avviare correttamente il programma.

   - **Node.js**
     - Nel terminale integrato Visual Studio Code, *** &lt;&gt; *** modificare la directory nella cartella del nome del modulo e quindi eseguire il comando seguente per installare i pacchetti di nodo
     ...
     - Passare alla visualizzazione Debug di Visual Studio Code selezionando **Visualizza > Debug**. Selezionare la configurazione ** * &lt;&gt; * di debug il nome del modulo Local Debug (Node.js)** dall'elenco a discesa.
   - **Java**
     - Aprire il file `App.java` e aggiungere un punto di interruzione.

     - Passare alla visualizzazione Debug di Visual Studio Code selezionando **Visualizza > Debug**. Selezionare la ** * &lt;&gt; * ** configurazione di debug il nome del modulo Local Debug (Java) dall'elenco a discesa.
...
1. Per interrompere la sessione di debug, selezionare il pulsante Interrompi o premere **MAIUSC e F5,** quindi eseguire **Azure IoT Edge: Stop IoT Edge Simulator** nella tavolozza dei comandi per arrestare il simulatore e pulire.
...
1. Passare alla visualizzazione Debug di Visual Studio Code e selezionare il file di configurazione del debug per il modulo. Il nome dell'opzione di debug dovrebbe essere simile al ** * &lt;nome&gt; * del modulo Remote Debug**

1. Selezionare **Avvia debug** o premere **F5**. Selezionare il processo a cui collegarsi.

1. Nella visualizzazione Debug di Visual Studio Code è possibile vedere le variabili nel pannello di sinistra.

1. Per interrompere la sessione di debug, selezionare innanzitutto il pulsante Stop o premere **MAIUSC e F5**, quindi selezionare **Azure IoT Edge: Stop IoT Edge Simulator** dalla tavolozza dei comandi.
...
1. Se si sta sviluppando il modulo in Python, seguire questi passaggi prima di procedere:
   ...
   - Aggiungere la singola riga di codice seguente al callback da sottoporre a debug:
     ...
     Ad esempio, se si `receive_message_listener` desidera eseguire il debug della funzione, è necessario inserire tale riga di codice come illustrato di seguito:For example, if you want to debug the function, you would insert that line of code as shown below:
...
1. Nel riquadro comandi di Visual Studio Code:
   1. Eseguire il comando **Azure IoT Edge: Build and Push IoT Edge solution**.
...
1. Nella sezione **Azure IoT Hub Devices** (Dispositivi Azure IoT Hub) della visualizzazione Explorer di Visual Studio Code:
   1. Fare clic con il pulsante destro del mouse su un ID dispositivo IoT Edge e quindi **scegliere Crea distribuzione per dispositivo singolo**.
...
- **C, incluse le funzioni di Azure**
...
- **Node.js**
  ...
  - Nel computer di sviluppo aprire Visual `launch.json` Studio Code e quindi modificarlo in modo che il valore dell'indirizzo ** * &lt;&gt; * ** del profilo Remote Debug (Remote Debug (o il ** * &lt;nome&gt; * del modulo Remote Debug (Node.js nel contenitore di Windows)** sia in esecuzione come contenitore Windows) sia l'indirizzo IP del computer sottoposto a debug.
...
- **Java**
  ...
  - Nel computer di sviluppo aprire Visual Studio Code e modificare il `launch.json` ** * &lt;profilo&gt; * Remote Debug (Java)** del modulo in modo che sia possibile connettersi al computer di destinazione. Per altre informazioni sulla modifica di `launch.json` e sul debug di Java con Visual Studio Code, vedere la sezione sulla [configurazione del debugger](https://code.visualstudio.com/docs/java/java-debugging#_configuration).
...
- **Python**
  ...
  - Nel computer di sviluppo aprire Visual `launch.json` Studio Code `host` e quindi modificarlo in modo che il valore del nome `localhost` ** * &lt;&gt; * del modulo Remote Debug (Python)** del profilo utilizzi l'indirizzo IP del computer di destinazione anziché .

### <a name="debug-your-module"></a>Eseguire il debug del modulo

1. Nella visualizzazione Debug di Visual Studio Code selezionare il file di configurazione del debug per il modulo. Il nome dell'opzione di debug dovrebbe essere simile al ** * &lt;nome&gt; * del modulo Remote Debug**

1. Aprire il file del modulo per il linguaggio di sviluppo e aggiungere un punto di interruzione:

   - **Funzione di Azure (C):** aggiungere `<your module name>.cs`il punto di interruzione al file .
   - **C'è**: Aggiungere il `Program.cs`punto di interruzione al file .
   - **Node.js**: aggiunge il `app.js`punto di interruzione al file .
   - **Java**: aggiunge il `App.java`punto di interruzione al file .
   - **Python**: Aggiungere il `main.py`punto di interruzione al `ptvsd.break_into_debugger()` file nel metodo di callback in cui è stata aggiunta la riga.
   - **C**: Aggiungere il `main.c`punto di interruzione al file .
