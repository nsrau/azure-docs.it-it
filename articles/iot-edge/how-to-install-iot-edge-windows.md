...
Per eseguire l'installazione con componenti offline, usare il parametro `-OfflineInstallationPath` come parte del comando deploy-IoTEdge e fornire il percorso assoluto alla directory di file. Ad esempio,
...
Il comando `Deploy-IoTEdge` installa i componenti IoT Edge, quindi è necessario continuare con il comando `Initialize-IoTEdge` per eseguire il provisioning del dispositivo con l'ID dispositivo e la connessione dell'hub Internet. Eseguire il comando direttamente e specificare una stringa di connessione dall'hub Internet o usare uno dei collegamenti nella sezione precedente per informazioni su come eseguire automaticamente il provisioning dei dispositivi con il servizio Device provisioning.
...
È anche possibile usare il parametro percorso di installazione offline con il comando Update-IoTEdge.
...
| Parametro | Valori accettati | Comments |
| --------- | --------------- | -------- |
...
| Parametro | Valori accettati | Comments |
| --------- | --------------- | -------- |
| **Manuale** | None | **Parametro opzionale**. Se non viene specificato alcun tipo di provisioning, Manual è il valore predefinito.<br><br>Dichiara che si fornirà una stringa di connessione del dispositivo per effettuare il provisioning del dispositivo manualmente. |
| **Dps** | None | **Parametro opzionale**. Se non viene specificato alcun tipo di provisioning, Manual è il valore predefinito.<br><br>Dichiara che si forniranno un ID ambito del servizio Device Provisioning (DPS) e l'ID registrazione del dispositivo per effettuare il provisioning tramite DPS.  |
...
| **RegistrationId** | Un ID registrazione generato dal dispositivo | **Obbligatorio** per l'installazione di DPS se si usa il TPM o l'attestazione della chiave simmetrica. |
