...
Il daemon di sicurezza IoT Edge è responsabile delle operazioni logiche di Gestione sicurezza IoT Edge. Rappresenta una parte significativa della base di elaborazione attendibile del dispositivo IoT Edge.
...
Il daemon di sicurezza IoT Edge opera con la massima integrità possibile all'interno della capacità di difesa di qualsiasi radice di hardware di fiducia. Con l'integrazione corretta, l'hardware della radice di attendibilità misura e monitora il daemon di sicurezza in modo statico e in fase di esecuzione per impedire la manomissione.

L'accesso fisico è sempre una minaccia per i dispositivi IoT. La radice di attendibilità dell'hardware svolge un ruolo importante nella difesa dell'integrità del daemon di sicurezza di IoT Edge.  La radice hardware della fiducia si protrae in due varietà:
...
Esistono due tipi di ambienti di esecuzione per utilizzare la radice hardware di attendibilità:

* Ambiente di esecuzione standard o avanzato (REE) che si basa sull'utilizzo di elementi protetti per proteggere le informazioni riservate.
* L'ambiente di esecuzione attendibile (TEE) che si basa sull'uso della tecnologia enclave sicura per proteggere le informazioni sensibili e offrire protezione all'esecuzione del software.

Per i dispositivi che usano enclavi sicuri come radice hardware di attendibilità, la logica sensibile all'interno del daemon di sicurezza di IoT Edge deve essere all'interno dell'enclave.  Parti non sensibili del daemon di sicurezza possono essere al di fuori del TEE.  In ogni caso, i produttori di design originali (ODM) e i produttori di apparecchiature originali (OEM) dovrebbero estendere la fiducia al loro HSM per misurare e difendere l'integrità del daemon di sicurezza IoT Edge all'avvio e al runtime.

#### <a name="minimize-bloat-and-churn"></a>Ridurre al minimo il software bloat e la varianza

Un altro principio di base per il daemon di sicurezza di IoT Edge è la riduzione della varianza.  Per il massimo livello di attendibilità, il daemon di sicurezza di IoT Edge può essere strettamente associato alla radice di attendibilità dell'hardware del dispositivo e operare come codice nativo.  È comune per questi tipi di realizzazioni aggiornare il software daemon attraverso la radice hardware dei percorsi di aggiornamento sicuro di trust (a differenza dei meccanismi di aggiornamento forniti dal sistema operativo), che può essere difficile in alcuni scenari.  Mentre il rinnovo della sicurezza è consigliato per i dispositivi IoT, requisiti di aggiornamento eccessivi o payload di aggiornamento di grandi dimensioni possono espandere la superficie di minaccia in molti modi.  Gli esempi includono l'omissione di aggiornamenti per ottimizzare la disponibilità operativa o hardware della radice di attendibilità troppo vincolato per elaborare i payload di aggiornamento di grandi dimensioni.  Di conseguenza, la progettazione del daemon di sicurezza IoT Edge è concisa per mantenere l'ingombro e la base di elaborazione affidabile di piccole dimensioni e ridurre al minimo i requisiti di aggiornamento.
...
Il daemon di sicurezza IoT Edge sfrutta qualsiasi radice hardware disponibile della tecnologia di attendibilità per la protezione avanzata.  Consente inoltre il funzionamento split-world tra un ambiente di esecuzione standard/rich (REE) e un ambiente di esecuzione attendibile (TEE) quando le tecnologie hardware offrono ambienti di esecuzione attendibili. Le interfacce specifiche del ruolo consentono ai componenti principali di Edge Edge di garantire l'integrità del dispositivo IoT Edge e delle relative operazioni.

#### <a name="cloud-interface"></a>Interfaccia cloud

L'interfaccia cloud consente al daemon di sicurezza IoT Edge di accedere a servizi cloud come i complimenti cloud per la sicurezza dei dispositivi come il rinnovo della sicurezza.  Ad esempio, il daemon di sicurezza IoT Edge utilizza attualmente questa interfaccia per accedere al servizio di [provisioning dei dispositivi](https://docs.microsoft.com/azure/iot-dps/) hub IoT di Azure per la gestione del ciclo di vita delle identità del dispositivo.  

#### <a name="management-api"></a>API di gestione

Il daemon di sicurezza IoT Edge offre un'API di gestione, che viene chiamata dall'agente IoT Edge durante la creazione/avvio/arresto/rimozione di un modulo IoT Edge. Il daemon di sicurezza memorizza le "registrazioni" per tutti i moduli attivi. Queste registrazioni eseguono il mapping dell'identità di un modulo ad alcune proprietà del modulo. Ad esempio, queste proprietà del modulo includono l'identificatore di processo (pid) del processo in esecuzione nel contenitore e l'hash del contenuto del contenitore docker.
...
L'API di gestione è un'API con privilegi, richiamabile solo dall'agente IoT Edge.The management API is a privileged API, callable only from the IoT Edge agent.  Poiché il daemon di sicurezza IoT Edge viene avviato e avvia l'agente IoT Edge, verifica che l'agente IoT Edge non sia stato manomesso, quindi può creare una registrazione implicita per l'agente IoT Edge. Lo stesso processo di attestazione utilizzato dall'API del carico di lavoro limita anche l'accesso all'API di gestione solo all'agente IoT Edge.The same attestation process that the workload API uses also restricts access to the management API to only the IoT Edge agent.
...
L'API del carico di lavoro è accessibile a tutti i moduli. Fornisce la prova di identità, sia come un token firmato con radice HSM o un certificato X509 e il pacchetto di attendibilità corrispondente a un modulo. Il bundle di attendibilità contiene i certificati della CA per tutti gli altri server che i moduli devono considerare attendibili.

Il daemon di sicurezza IoT Edge utilizza un processo di attestazione per proteggere questa API. Quando un modulo chiama questa API, il daemon di sicurezza tenta di trovare una registrazione per l'identità. Se l'operazione riesce, usa le proprietà della registrazione per misurare il modulo. Se il risultato del processo di misurazione corrisponde alla registrazione, viene generata una nuova prova di identità. I certificati della CA (bundle di attendibilità) corrispondenti vengono restituiti al modulo.  Il modulo usa questo certificato per connettersi all'hub IoT o ad altri moduli oppure per avviare un server. Quando il token o il certificato firmato sta per scadere, è responsabilità del modulo richiedere un nuovo certificato.
...
Il Platform Abstraction Layer del modulo di protezione hardware astrae tutto l'hardware della radice di attendibilità per isolare lo sviluppatore o l'utente di IoT Edge dalle relative complessità.  Include una combinazione di API (Application Programming Interface) e procedure di comunicazione tra domini, ad esempio la comunicazione tra un ambiente di esecuzione standard e un'enclave sicura.  L'implementazione effettiva del Platform Abstraction Layer del modulo di protezione hardware dipende dallo specifico hardware sicuro in uso. La sua esistenza consente di usare praticamente qualsiasi hardware di processore sicuro.

## <a name="secure-silicon-root-of-trust-hardware"></a>Hardware della radice di attendibilità per il processore sicuro

Un processore sicuro è necessario per ancorare l'attendibilità nell'hardware del dispositivo IoT Edge.  I processori sicuri possono essere di versi tipi, come Trusted Platform Module (TPM), embedded Secure Element (eSE), ARM TrustZone, Intel SGX e tecnologie per processori sicuri personalizzate.  L'uso di una radice sicura di silicio di fiducia nei dispositivi è raccomandato date le minacce associate all'accessibilità fisica dei dispositivi IoT.
