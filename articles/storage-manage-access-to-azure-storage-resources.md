...
| Operazione REST   | Metodo .NET   | Autorizzazione con accesso in lettura pubblico completo | Autorizzazione con accesso in lettura pubblico solo per BLOB |
|--------------------------------------------------------|--------------------------------------------------|-----------------------------------------|---------------------------------------------------|
| List Containers   | ListContainers   | Solo proprietario  | Solo proprietario   |
| Create Container   | Creazione   | Solo proprietario  | Solo proprietario   |
| Get Container Properties  | Proprietà   | Tutti   | Solo proprietario   |
| Get Container Metadata   | Metadata   | Tutti   | Solo proprietario   |
| Set Container Metadata   | Metadata   | Solo proprietario  | Solo proprietario   |
| Get Container ACL   | BlobContainerPermissions  | Solo proprietario  | Solo proprietario   |
| Set Container ACL   | BlobContainerPermissions  | Solo proprietario  | Solo proprietario   |
| Delete Container   | Delete   | Solo proprietario  | Solo proprietario   |
| List Blobs   | ListBlobs   | Tutti   | Solo proprietario   |
| Put Blob   | Metodo Create di CloudPageBlob o CloudBlockBlob | Solo proprietario  | Solo proprietario   |
| Get Blob   | DownloadByteArray e altri  | Tutti   | Tutti   |
| Get Blob Properties   | Proprietà   | Tutti   | Tutti   |
| Set Blob Properties   | SetProperties   | Solo proprietario  | Solo proprietario   |
| Get Blob Metadata   | Metadata   | Tutti   | Tutti   |
| Set Blob Metadata   | Metadata   | Solo proprietario  | Solo proprietario   |
| Put Block   | PutBlock   | Solo proprietario  | Solo proprietario   |
| Get Block List (solo blocchi con commit)  | DownloadBlockList  | Tutti   | Tutti   |
| Get Block List (solo blocchi senza commit o tutti i blocchi) | DownloadBlockList  | Solo proprietario  | Solo proprietario   |
| Put Block List   | PutBlockList   | Solo proprietario  | Solo proprietario   |
| Delete Blob   | Delete   | Solo proprietario  | Solo proprietario   |
| Copiare i BLOB   | CopyFromBlob   | Solo proprietario  | Solo proprietario   |
| Snapshot Blob   | CreateSnapshot   | Solo proprietario  | Solo proprietario   |
| Lease Blob   | Lease   | Solo proprietario  | Solo proprietario   |
| Put Page   | WritePages   | Solo proprietario  | Solo proprietario   |
| Get Page Ranges   | GetPageRanges   | Tutti   |    |
...
>[AZURE.NOTE]Per una panoramica dei concetti e un'esercitazione sulle firme di accesso condiviso, vedere [Firme di accesso condiviso](http://azure.microsoft.com/documentation/articles/storage-dotnet-shared-access-signature-part-1/).
...
Un client che riceve una firma di accesso condiviso può usare tale firma dal proprio codice per creare un oggetto di tipo [StorageCredentials](https://msdn.microsoft.com/library/microsoft.windowsazure.storage.auth.storagecredentials.aspx). Queste credenziali possono essere quindi usate per creare un oggetto [CloudStorageAccount](https://msdn.microsoft.com/library/microsoft.windowsazure.storage.cloudstorageaccount.aspx) o un oggetto [CloudBlobClient](https://msdn.microsoft.com/library/microsoft.windowsazure.storage.blob.cloudblobclient.aspx) per l'utilizzo della risorsa, come illustrato nell'esempio seguente:
