...
È importante comprendere la differenza tra i set di record DNS e i singoli record DNS. Un set di record è la raccolta di record con lo stesso nome e tipo in una zona. Per altre informazioni, vedere [Informazioni sui set di record e sui record](dns-getstarted-create-recordset-cli.md).


## DNS e interfaccia della riga di comando multipiattaforma di Azure

DNS di Azure è un servizio solo di Gestione risorse di Azure. Non include un’API ASM. È necessario assicurarsi che l'interfaccia della riga di comando di Azure sia configurata per l'uso della modalità Resource Manager. A tale scopo, usare il comando `azure config mode arm`.<BR>Se viene visualizzato il messaggio "*errore: 'dns' non è un comando di azure*" è probabilmente perché si sta usando l'interfaccia della riga di comando di Azure in modalità ASM, non in modalità Resource Manager.
...
È possibile elencare tutti i record in una zona DNS usando il comando `azure network dns record-set list`. È necessario specificare il nome del gruppo di risorse e il nome di zona.
...
I record vengono aggiunti al set di record utilizzando il `azure network dns record-set add-record`. I parametri per l'aggiunta di record a un set di record variano a seconda del tipo del set di record. Ad esempio, quando si usa un set di record di tipo *A*, si possono specificare record solo con il parametro `-a <IPv4 address>`.

Per creare un set di record, usare `azure network dns record-set create`. Specificare il gruppo di risorse, il nome della zona, il nome relativo del set di record, il tipo di record e la durata (TTL). Se il parametro --ttl non è definito, viene impostato il valore predefinito 4 (in secondi).
	
	azure network dns record-set create myresourcegroup  contoso.com "test-a"  A --ttl 300


Dopo aver creato il set di record A, aggiungere l'indirizzo IPv4 usando `azure network dns record-set add-record`.
...
### Per rimuovere un valore esistente da un set di record, usare `azure network dns record-set delete-record`.
...
Per usare i record DNS inversi, vedere [Come gestire i record DNS inversi](dns-reverse-dns-record-operations-cli.md).
