...
È sconsigliabile tentare di modificare il contenuto dei contenitori BLOB generati da Servizi multimediali senza usare le API di Servizi multimediali.
...
>[AZURE.NOTE]È sconsigliabile tentare di modificare il contenuto dei contenitori BLOB generati da Servizi multimediali senza usare le API di Servizi multimediali.
...
Un [processo](https://msdn.microsoft.com/library/azure/hh974289.aspx) viene in genere usato per elaborare (ad esempio, indicizzare o codificare) una presentazione audio/video. Se si stanno elaborando più video, è necessario creare un processo per ciascun video codificato.

Un processo contiene i metadati relativi all'elaborazione da eseguire. Ogni processo contiene una o più [attività](https://msdn.microsoft.com/library/azure/hh974286.aspx) che specificano un'attività di elaborazione atomica, i relativi asset di input, gli asset di output, un processore di contenuti multimediali e le impostazioni associate. Le attività contenute in un processo possono essere concatenate, in modo che l'asset di output di un'attività sia indicato come asset di input dell'attività successiva. In questo modo un processo può contenere tutte le operazioni di elaborazione necessarie per una presentazione multimediale.
...
Se si prevede di usare la creazione dinamica dei pacchetti e/o la crittografia dinamica, è necessario avere almeno un'unità di streaming per l'endpoint da cui si prevede di trasmettere in streaming. Per altre informazioni, vedere l'articolo su come [scalare le unità di streaming](../media-services-manage-origins#scale_streaming_endpoints).
...
Streaming dei contenuti ai client. Per fornire agli utenti gli URL di streaming, è necessario prima creare un localizzatore OnDemandOrigin. Creando il localizzatore è possibile ottenere il valore Path di base dell'asset che include il contenuto da trasmettere in streaming. Tuttavia, per trasmettere in streaming questo contenuto, è necessario modificare ulteriormente il percorso. Per creare un URL completo per il file manifesto di streaming, si deve concatenare il valore Path del localizzatore e il nome del file manifesto (nomefile.ism), quindi aggiungere /Manifest e un formato appropriato (se necessario) al percorso di origine del localizzatore. 
...
L'elenco seguente include la descrizione dei vari formati di streaming e alcuni esempi:
