...
Il servizio di archiviazione include anche il supporto di altre intestazioni condizionale quali **If-Modified-Since**, **If-Unmodified-Since** e **If-None-Match**, nonché le relative combinazioni. Per altre informazioni, vedere [Specifica di intestazioni condizionali per le operazioni del servizio Blob](http://msdn.microsoft.com/library/azure/dd179371.aspx) in MSDN.  

La seguente tabella riepiloga le operazioni contenitore che accettano intestazioni condizionali come **If-Match** nella richiesta e che restituiscono un valore ETag nella risposta.  
...
La seguente tabella riepiloga le operazioni BLOB che accettano intestazioni condizionali come **If-Match** nella richiesta e che restituiscono un valore ETag nella risposta.  
...
Le seguenti operazioni contenitore possono usare i lease per gestire la concorrenza pessimistica:  
...
La seguente tabella riepiloga le modalità in cui le operazioni delle entità di tabella usano i valori ETag:  
...
Si noti che le operazioni **Insert or Replace Entity** e **Insert or Merge Entity** *not* eseguono alcun controllo di concorrenza perché non inviano un valore ETag al servizio tabelle.  
...
- Architettura di Archiviazione - [Archiviazione di Microsoft Azure: servizio di archiviazione cloud a elevata disponibilità con coerenza assoluta](http://blogs.msdn.com/b/windowsazurestorage/archive/2011/11/20/windows-azure-storage-a-highly-available-cloud-storage-service-with-strong-consistency.aspx)
