<properties title="Gestione della concorrenza nell'archiviazione di Microsoft Azure" pageTitle="obbligatorio" description="Come gestire la concorrenza per i servizi BLOB, di accodamento, di tabelle e file" metaKeywords="Optional" services="Optional" solutions="Optional" documentationCenter="Optional" authors="Tamra Myers" videoId="Optional" scriptId="Optional" />
...
Il frammento C# seguente (che usa Client Storage Library 4.2.0) mostra un esempio semplice di come costruire una **If-Match AccessCondition** in base al valore ETag al quale viene effettuato l'accesso dalle proprietà di un BLOB precedentemente recuperato o inserito. Quindi, usa l'oggetto **AccessCondition** quando aggiorna il BLOB: l'oggetto **AccessCondition** aggiunge l'intestazione **If-Match** alla richiesta. Se un altro processo ha aggiornato il BLOB, il servizio BLOB restituisce un messaggio di stato HTTP 412 (Condizione preliminare non riuscita). È possibile scaricare l'esempio completo [qui][qui].
...
Il servizio di archiviazione include anche il supporto di altre intestazioni condizionale quali **If-Modified-Since**, **If-Unmodified-Since** e **If-None-Match** nonché le relative combinazioni. Per altre informazioni, vedere [Specifica di intestazioni condizionali per le operazioni del servizio Blob][Specifica di intestazioni condizionali per le operazioni del servizio Blob] su MSDN.
...
Lease Blob (\*)| Sì| Sì|
Snapshot Blob| Sì| Sì|
Copy Blob| Sì| Sì (per il BLOB di origine e di destinazione)|
...
-   [Specifica di intestazioni condizionali per le operazioni del servizio BLOB][Specifica di intestazioni condizionali per le operazioni del servizio BLOB]
...
-   [Operazioni sulle entità][Operazioni sulle entità]
...
-   [API REST del servizio di accodamento][API REST del servizio di accodamento]
...
-   [Managing File Locks][Managing File Locks]
...
-   [Gestione della concorrenza con l'archiviazione di Azure - Applicazione di esempio][qui]

Per altre informazioni sull'archiviazione di Azure, vedere:

-   [Home page dell'archiviazione di Microsoft Azure][Home page dell'archiviazione di Microsoft Azure]
-   [Introduzione ad Archiviazione di Azure][Introduzione ad Archiviazione di Azure]
-   Introduzione all'archiviazione per [BLOB][BLOB], [tabelle][tabelle] e [code][code]
-   Architettura di archiviazione: [Archiviazione di Windows Azure: A Highly Available Cloud Storage Service with Strong Consistency][Archiviazione di Windows Azure: A Highly Available Cloud Storage Service with Strong Consistency]

  [qui]: http://code.msdn.microsoft.com/windowsazure/Managing-Concurrency-using-56018114
  [Specifica di intestazioni condizionali per le operazioni del servizio Blob]: http://msdn.microsoft.com/it-it/library/dd179371.aspx
  [lease]: http://msdn.microsoft.com/it-it/library/azure/ee691972.aspx
  ...
  [BLOB]: http://azure.microsoft.com/it-it/documentation/articles/storage-dotnet-how-to-use-blobs/
  [tabelle]: http://azure.microsoft.com/it-it/documentation/articles/storage-dotnet-how-to-use-tables/
  [code]: http://azure.microsoft.com/it-it/documentation/articles/storage-dotnet-how-to-use-queues/
  [Archiviazione di Windows Azure: A Highly Available Cloud Storage Service with Strong Consistency]: http://blogs.msdn.com/b/windowsazurestorage/archive/2011/11/20/windows-azure-storage-a-highly-available-cloud-storage-service-with-strong-consistency.aspx
