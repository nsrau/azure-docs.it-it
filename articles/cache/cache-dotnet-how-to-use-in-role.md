...
## <a name="getting-started-with-in-role-cache"></a>Introduzione a Cache nel ruolo
...
* **ruolo dedicato** : le istanze del ruolo vengono utilizzate esclusivamente per la memorizzazione nella cache.
* **basata su ruolo con risorse condivise** : la cache condivide le risorse della macchina virtuale (larghezza di banda, CPU e memoria) con l'applicazione.
...
## <a name="configure-the-cache-cluster"></a>Configurare il cluster di cache
...
Per configurare un cluster di cache **Ruolo dedicato**, aggiungere un **Ruolo di lavoro della cache** al progetto.
