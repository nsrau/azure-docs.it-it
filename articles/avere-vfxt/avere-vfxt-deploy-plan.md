...
* Crea una nuova sottoscrizione per ogni vFXT per la distribuzione di Azure e Gestisci tutti i componenti in questa sottoscrizione. I vantaggi includono:
...
* Individuare il cluster vFXT e la macchina virtuale del controller cluster insieme. in particolare, devono essere:
  ...
  * Nello stesso account di archiviazione
  ...
  Il modello di creazione automatica del cluster gestisce questa situazione per la maggior parte delle situazioni.
...
I sistemi di archiviazione back-end forniscono i file alla cache del cluster e ricevono anche i dati modificati dalla cache. Decidere se il working set verrà archiviato a lungo termine in un nuovo contenitore BLOB o in un sistema di archiviazione hardware o cloud esistente. Questi sistemi di archiviazione back-end sono detti *Core filer*.

### <a name="hardware-core-filers"></a>Componenti di base hardware

Aggiungere i sistemi di archiviazione hardware al cluster vFXT dopo aver creato il cluster. È possibile usare qualsiasi sistema hardware locale esistente, inclusi i sistemi locali, purché sia possibile raggiungere il sistema di archiviazione dalla subnet del cluster.
...
Due ruoli predefiniti di Azure vengono usati per le macchine virtuali di vFXT per le macchine virtuali di Azure:
...
[Panoramica della distribuzione](avere-vfxt-deploy-overview.md) offre una panoramica dei passaggi necessari per creare un vFXT di ricerca per il sistema di Azure e prepararlo a gestire i dati.
