<properties
	pageTitle="Procedure dettagliate del processo di analisi scientifica dei dati cloud | Azure" 
	description="Procedure dettagliate del processo di analisi scientifica dei dati cloud" 
	metaKeywords="" 
...
# Processo di analisi scientifica dei dati cloud in Azure Machine Learning

Questo articolo illustra la mappa del processo di analisi scientifica dei dati cloud e alcuni scenari di destinazione e origini dati di esempio. Illustra alcune delle opzioni e delle sequenze di elaborazione basate su caratteristiche dei dati, posizione dell'origine e repository di destinazione in Azure. Il flusso del processo di analisi scientifica dei dati cloud per lo scenario di esempio è riepilogato nell'ultima sezione.

[AZURE.INCLUDE [machine-learning-free-trial](../includes/machine-learning-free-trial.md)]

**Per tutti gli scenari è necessario:**
...
**Le sezioni seguenti presentano alcuni scenari di esempio. Per ogni scenario vengono elencati un flusso di dati scientifici possibile e le risorse di Azure di supporto.**

- [Scenario n. 1: Set di dati tabulari medio-piccolo in un file locale](#smalllocal)
- [Scenario n. 2: Set di dati medio-piccolo in un file locale che richiede elaborazione](#smalllocalprocess)
- [Scenario n. 3: Set di dati di grandi dimensioni in file locali, con destinazione BLOB di Azure](#largelocal)
- [Scenario n. 4: Set di dati medio-piccolo in un file locale, con destinazione SQL Server nella macchina virtuale di Azure](#smalllocaltodb)
- [Scenario n. 5: Set di dati di grandi dimensioni in file locali, con destinazione SQL Server in una macchina virtuale di Azure](#largelocaltodb)
- [Scenario n. 6: Set di dati di grandi dimensioni in un database SQL Server locale, con destinazione SQL Server nella macchina virtuale di Azure](#largedbtodb)
- [Scenario n. 7: Set di dati di grandi dimensioni in file locali, con destinazione il database Hive nei cluster Hadoop di Azure HDInsight](#largedbtohive)
...
6.  Caricare i dati nel database SQL Server in esecuzione in una VM di Azure.
    ...
    a. Opzione n. 1: Uso di SQL Server Management Studio
		...
		i.  Accedere alla macchina virtuale di SQL Server.
        ii. Eseguire SQL Server Management Studio
        iii. Creare tabelle di database e di destinazione.
        iv. Utilizzare uno dei metodi di importazione globale per caricare i dati dai file della macchina virtuale locale.

    b. Option n. 2: Uso di IPython Notebook, non è consigliabile per i set di dati di dimensioni medio-grandi

        i.  Utilizzare la stringa di connessione ODBC per accedere a SQL Server sulla macchina virtuale.
        ii. Creare tabelle di database e di destinazione.
        iii. Utilizzare uno dei metodi di importazione globale per caricare i dati dai file della macchina virtuale locale.
...
Riepilogo degli scenari di esempio
------------------------

Il diagramma seguente include un riepilogo degli scenari descritti in precedenza e le scelte relative al processo di analisi scientifica dei dati cloud effettuate in ogni scenario. Tenere presente che l'elaborazione dei dati, l'esplorazione, la progettazione delle funzionalità e il campionamento potrebbero avvenire in uno o più metodi/ambienti, ad esempio nell'ambiente di origine, intermedio e/o di destinazione, e può procedere in modo iterativo, se necessario. Il diagramma viene usato come illustrazione solo di alcuni flussi possibili.
...
Per un esempio della procedura dettagliata end-to-end del processo di analisi scientifica dei dati di Azure usando un set di dati pubblici, vedere [Processo di analisi scientifica dei dati di Azure in azione](machine-learning/machine-learning-data-science-process-sql-walkthrough.md).
