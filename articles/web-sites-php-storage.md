<properties urlDisplayName="Web w/ Storage" pageTitle="Sito Web PHP con archiviazione tabelle - Esercitazione su Azure" metaKeywords="Azure table storage PHP, Azure PHP website, Azure PHP web site, Azure PHP tutorial, Azure PHP example" description="Questa esercitazione illustra come creare un sito Web PHP e usare il servizio di archiviazione tabelle di Azure nel back-end." metaCanonical="" services="web-sites,storage" documentationCenter="PHP" title="Create a PHP Website using Azure Storage" authors="tomfitz" solutions="" manager="wpickett" editor="" />
...
* Usare il metodo factory `ServicesBuilder::createTableService` per creare un'istanza di un wrapper intorno alle chiamate del servizio tabelle.
...
* Una volta ottenuto un oggetto `Entity`, il modello per la lettura dei dati è `Entity->getPropertyValue('[nome]')`:
...
* Sarà quindi possibile passare l'oggetto `$entity` appena creato al metodo `insertEntity`:
...
L'app di elenco attività è in grado di contrassegnare un elemento come completo, ma anche di rimuovere tale contrassegno. La home page passa i valori *RowKey* e *PartitionKey* di un'entità e lo stato di destinazione (marked==1, unmarked==0).
...
* Il primo passaggio dell'aggiornamento di un'entità è il suo recupero dalla tabella:
	...
	Come è possibile notare, il filtro di query passato ha il formato `Key eq 'Value'`. Una descrizione completa della sintassi di query è disponibile [qui][msdn-table-query-syntax].
