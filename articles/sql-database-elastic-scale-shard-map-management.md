...
<table>
   <tr>
    <td>Chiave</td>
     <td>Percorso della partizione</td>
   </tr>
   <tr>
    <td>1</td>
     <td>Database_A</td>
...
<table>
   <tr>
    <td><b>Intervalli di chiavi</b></td>
     <td><b>Percorso della partizione</b></td>
   </tr>
   <tr>
    <td>[1, 50)</td>
     <td>Database_A</td>
...
Il gestore mappe partizioni nelle API di Scalabilità elastica è una raccolta di mappe partizioni. I dati gestiti da un oggetto .NET **ShardMapManager** sono memorizzati in tre posizioni: 

1. **Mappa globale partizioni**: quando si crea un oggetto **ShardMapManager**, si specifica un database da usare come repository per tutte le mappe e i mapping corrispondenti. Vengono create automaticamente tabelle speciali e stored procedure per la gestione delle informazioni. Si tratta in genere di un database di piccole dimensioni e a cui si accede raramente, ma è consigliabile non usarlo per altre esigenze dell'applicazione. Le tabelle si trovano in uno schema speciale denominato **__ShardManagement**. 
...
3. **Cache applicazione**: ogni istanza di applicazione che accede a un oggetto **ShardMapManager** gestisce una cache in memoria locale dei rispettivi mapping, in cui vengono archiviate le informazioni di routing appena recuperate. 
...
È necessario creare istanze di **ShardMapManager** solo una volta per ogni dominio applicazione, nel codice di inizializzazione per un'applicazione. Un oggetto **ShardMapManager** può includere un numero qualsiasi di mappe partizioni. Anche se è possibile che una singola mappa partizioni sia sufficiente per molte applicazioni, in alcune situazioni vengono usati diversi set di database per schemi diversi o per finalità specifiche. In questi casi è preferibile usare più mappe partizioni. 

Nel codice seguente un'applicazione tenta di aprire un oggetto **ShardMapManager** esistente.  Se gli oggetti che rappresentano un oggetto **ShardMapManager** globale non esistono ancora nel database, verranno creati dalla libreria client.
...
I metodi usati per popolare o modificare i dati di **ShardMapManager** non influiscono sui dati utente archiviati nelle partizioni stesse. Ad esempio, i metodi quali **CreateShard**, **DeleteShard**, **UpdateMapping** e così via influiscono solo sui metadati della mappa partizioni e non rimuovono, aggiungono o modificano i dati utente inclusi nelle partizioni. Questi metodi sono stati invece progettati per l'uso insieme a operazioni separate eseguite per creare o rimuovere i database effettivi o per rimuovere righe da una partizione a un'altra, in modo da bilanciare nuovamente un ambiente partizionato.  Il servizio di **suddivisione/unione** incluso nell'anteprima di Scalabilità elastica usa queste API, oltre a orchestrare lo spostamento effettivo dei dati tra le partizioni. 

## Popolamento di una mappa partizioni: esempio 
Di seguito è disponibile una sequenza di esempio delle operazioni necessarie per popolare una mappa partizioni specifica. Il codice esegue i passaggi seguenti: 
...
Per informazioni dettagliate, vedere [Routing dipendente dai dati](./sql-database-elastic-scale-data-dependent-routing.md). 

##Modifica di una mappa partizioni 

È possibile modificare una mappa partizioni in molti modi diversi. Tutti i metodi seguenti modificano i metadati che descrivono le partizioni e i rispettivi mapping, ma non modificano fisicamente i dati nelle partizioni e non creano o eliminano i database effettivi.  Potrebbe essere necessario coordinare alcune operazioni sulla mappa partizioni descritte di seguito con le azioni amministrative che spostano fisicamente i dati o che aggiungono e rimuovono i database che fungono da partizioni.
...
* Per creare o rimuovere punti o intervalli mappati alle partizioni, usare **CreateRangeMapping**, **DeleteMapping**, **CreatePointMapping**. 
