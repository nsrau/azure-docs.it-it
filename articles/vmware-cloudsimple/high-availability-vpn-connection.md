---
title: 'Azure VMware Solutions (AVS): configurare la disponibilità elevata da locale ad AVS gateway VPN'
description: Viene descritto come configurare una connessione a disponibilità elevata dall'ambiente locale a un gateway VPN AVS abilitato per la disponibilità elevata
author: sharaths-cs
ms.author: b-shsury
ms.date: 08/14/2019
ms.topic: article
...
---
# <a name="configure-a-high-availability-connection-from-on-premises-to-an-avs-vpn-gateway"></a>Configurare una connessione a disponibilità elevata da locale a un gateway VPN AVS

Gli amministratori di rete possono configurare una connessione VPN da sito a sito IPsec a disponibilità elevata dall'ambiente locale a un gateway VPN AVS.
...
1. Verificare che l'organizzazione abbia eseguito il [provisioning](create-nodes.md) dei nodi necessari e che sia stato creato almeno un cloud privato AVS.
2. [Configurare un gateway VPN da sito a sito](vpn-gateway.md#set-up-a-site-to-site-vpn-gateway) tra la rete locale e il cloud privato AVS.
...
Per il corretto funzionamento della VPN da sito a sito, è necessario consentire UDP 500/4500 e ESP (protocollo IP 50) dall'IP pubblico di AVS primario e secondario (IP peer) nell'interfaccia esterna del gateway VPN Cisco ASA locale.
...
Per configurare la fase 2 (IPsec), creare un elenco di controllo di accesso (ACL) che definisce il traffico da crittografare e tunneling. Nell'esempio seguente il traffico di interesse è dal tunnel originato dalla subnet locale locale (10.16.1.0/24) alla subnet remota del cloud privato AVS (192.168.0.0/24). Se sono presenti più subnet tra i siti, l'ACL può contenere più voci.
...
#### <a name="define-the-avs-remote-subnet-as-an-object"></a>Definire la subnet AVS remote come oggetto
...
Configurare il set di trasformazione (TS), che deve coinvolgere la parola chiave ```ikev1```. Gli attributi di crittografia e hash specificati nei Servizi terminal devono corrispondere ai parametri elencati nella [configurazione predefinita per i gateway VPN AVS](cloudsimple-vpn-gateways.md#cryptographic-parameters).
...
Per il corretto funzionamento della VPN da sito a sito, è necessario consentire UDP 500/4500 e ESP (protocollo IP 50) dall'IP pubblico di AVS primario e secondario (IP peer) nell'interfaccia esterna del gateway Palo Alto Networks locale.
...
### <a name="2-set-up-static-routes-for-avs-private-cloud-subnets-to-be-reached-over-the-site-to-site-vpn"></a>2. configurare le route statiche per le subnet del cloud privato AVS da raggiungere tramite la VPN da sito a sito

Le route sono necessarie per le subnet locali per raggiungere le subnet del cloud privato AVS.
...
* Destinazione. Specificare le subnet del cloud privato AVS da raggiungere tramite le interfacce del tunnel S2S da locale
...
Ripetere i passaggi precedenti per creare un'altra route per le subnet del cloud privato AVS da usare come route secondaria/di backup tramite l'interfaccia del tunnel secondario. Questa volta selezionare un ID tunnel diverso e una metrica più elevata rispetto alla route primaria.
...
* Name. Immettere il nome del gateway IKE a cui eseguire il peering con il peer VPN AVS primario.
...
* Indirizzo peer. Immettere l'indirizzo IP del peer VPN AVS primario.
* L'autenticazione. Selezionare la **chiave precondivisa**.
* Chiave pre-condivisa/conferma chiave precondivisa. Immettere la chiave precondivisa in modo che corrisponda alla chiave del gateway VPN AVS.
* Identificazione locale. Immettere l'indirizzo IP pubblico del palo alto firewall locale.
* Identificazione peer. Immettere l'indirizzo IP del peer VPN AVS primario.
...
Ripetere i passaggi precedenti per creare un altro profilo di crittografia IPsec, che verrà usato come peer VPN AVS secondario. Lo stesso profilo di crittografia IPSEC può essere usato anche per i tunnel IPsec primari e secondari (vedere la procedura seguente).
...
* Name. Immettere un nome per il tunnel IPSEC primario a cui eseguire il peering con il peer VPN AVS primario.
...
* IP di destinazione. Immettere qualsiasi indirizzo IP appartenente alla subnet AVS private cloud consentita tramite la connessione da sito a sito. Verificare che le interfacce del tunnel, ad esempio tunnel. 20-10.64.5.2/32 e tunnel. 30-10.64.6.2/32, in palo alto, siano autorizzate a raggiungere l'indirizzo IP del cloud privato AVS tramite la VPN da sito a sito. Per gli ID proxy, vedere la configurazione seguente.
...
* Local. Specificare le subnet locali che possono comunicare con le subnet del cloud privato AVS tramite la VPN da sito a sito.
* Remoto. Specificare le subnet remote del cloud privato AVS a cui è consentita la comunicazione con le subnet locali.
* Protocollo. Selezionare **any**.

Ripetere i passaggi precedenti per creare un altro tunnel IPsec da usare per il peer VPN AVS secondario.
