---
title: 'Azure VMware Solutions (AVS): configurare la crittografia rete VSAN per AVS private cloud'
description: Viene descritto come configurare la funzionalità di crittografia del software rete VSAN in modo che il cloud privato AVS possa funzionare con un server di gestione delle chiavi in esecuzione nella rete virtuale di Azure.
author: sharaths-cs
ms.author: b-shsury
ms.date: 08/19/2019
ms.topic: article
...
---
# <a name="configure-vsan-encryption-for-avs-private-cloud"></a>Configurare la crittografia rete VSAN per AVS private cloud

È possibile configurare la funzionalità di crittografia del software rete VSAN in modo che il cloud privato AVS possa funzionare con un server di gestione delle chiavi in esecuzione nella rete virtuale di Azure.
...
* Configurare e gestire la crittografia rete VSAN nel cloud privato AVS usando lo strumento di gestione delle chiavi di terze parti in esecuzione nella rete virtuale di Azure.

## <a name="kms-deployment-scenario"></a>Scenario di distribuzione KMS

Il cluster del server del servizio di gestione delle chiavi viene eseguito nella rete virtuale di Azure ed è raggiungibile da AVS private cloud vCenter sulla connessione Azure ExpressRoute configurata.
...
2. [Portale AVS: ottenere informazioni sul peering ExpressRoute](#avs-portal-obtain-expressroute-peering-information)
3. [Portale di Azure: connettere la rete virtuale al cloud privato AVS](#azure-portal-connect-your-virtual-network-to-the-avs-private-cloud)
...
### <a name="avs-portal-obtain-expressroute-peering-information"></a>Portale AVS: ottenere informazioni sul peering ExpressRoute

Per continuare l'installazione, sono necessari la chiave di autorizzazione e l'URI del circuito peer per ExpressRoute e l'accesso alla sottoscrizione di Azure. Queste informazioni sono disponibili nella pagina connessione della rete virtuale nel portale di AVS. Per istruzioni, vedere [configurare una connessione di rete virtuale al cloud privato AVS](virtual-network-connection.md). Se si verificano problemi durante il recupero delle informazioni, aprire una [richiesta di supporto](https://portal.azure.com/#blade/Microsoft_Azure_Support/HelpAndSupportBlade/newsupportrequest).
...
2. Collegare la rete virtuale al circuito AVS ExpressRoute seguendo le istruzioni riportate in [connettere una rete virtuale a un circuito ExpressRoute usando il portale](../expressroute/expressroute-howto-linkvnet-portal-resource-manager.md).
3. Usare le informazioni sul circuito AVS ExpressRoute ricevute nel messaggio di posta elettronica di benvenuto da AVS per collegare la rete virtuale al circuito AVS ExpressRoute in Azure.
