...
1. Fare clic con il pulsante destro del mouse su una macchina virtuale o su un modello di macchina virtuale che si desidera clonare. Selezionare **Clone** > **Clone to Virtual Machine**.
...
8. Assegnare le autorizzazioni per gli indirizzi IP del client per accedere alla condivisione NFS `/etc/exports` modificando il file.
...
9. Esportare le condivisioni NFS tramite il `sudo exportfs -a` comando.

10. Riavviare il server kernel NFS utilizzando il `sudo systemctl restart nfs-kernel-server` comando.
...
5. Nel passaggio 3 della procedura guidata specificare il nome dell'archivio dati, il percorso e il server.  È possibile usare l'indirizzo IP della macchina virtuale Linux per il server.  Il percorso della cartella sarà nel `/<folder>/<subfolder>/` formato.
