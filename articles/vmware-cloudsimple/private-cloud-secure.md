...
Quando viene creato un cloud privato, viene creato un `CloudOwner@cloudsimple.local` utente predefinito nel dominio vCenter SSO.  L'utente CloudOwner dispone dei privilegi per la gestione di vCenter. Per consentire l'accesso a utenti diversi, vengono aggiunte ulteriori origini di identità a vCenter SSO.  I ruoli e i gruppi predefiniti sono configurati in vCenter che possono essere usati per aggiungere altri utenti.

### <a name="add-new-users-to-vcenter"></a>Aggiungere nuovi utenti a vCenter

1. [Escalation dei privilegi](escalate-private-cloud-privileges.md) per **CloudOwner\@cloudsimple. local** utente nel cloud privato.
