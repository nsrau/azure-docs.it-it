---
title: 'Azure VMware Solutions (AVS): configurare un gateway VPN'
description: Viene descritto come configurare un gateway VPN da punto a sito e un gateway VPN da sito a sito e creare connessioni tra la rete locale e il cloud privato AVS
author: sharaths-cs
ms.author: b-shsury
ms.date: 08/14/2019
ms.topic: article
...
ms.locfileid: "77016766"
---
# <a name="set-up-vpn-gateways-on-avs-network"></a>Configurare i gateway VPN nella rete AVS

I gateway VPN consentono di connettersi alla rete AVS dalla rete locale e da un computer client in remoto. Una connessione VPN tra la rete locale e la rete AVS fornisce l'accesso a vCenter e ai carichi di lavoro nel cloud privato AVS. AVS supporta VPN da sito a sito e gateway VPN da punto a sito.

## <a name="vpn-gateway-types"></a>Tipi di gateway VPN

* La connessione **VPN da sito a sito** consente di configurare i carichi di lavoro del cloud privato AVS per accedere ai servizi locali. È anche possibile usare Active Directory locali come origine di identità per l'autenticazione in AVS private cloud vCenter. Attualmente, è supportato solo il tipo **VPN basato su criteri** .
* La connessione **VPN da punto a sito** è il modo più semplice per connettersi al cloud privato AVS dal computer. Usare la connettività VPN da punto a sito per connettersi in remoto al cloud privato AVS. Per informazioni sull'installazione di un client per una connessione VPN da punto a sito, vedere [configurare una connessione VPN al cloud privato AVS](set-up-vpn.md).
...
I gateway VPN AVS forniscono i criteri per aggiungere VLAN/subnet ai gateway VPN. I criteri consentono di specificare regole diverse per la VLAN di gestione/le subnet e le VLAN/subnet definite dall'utente. Le regole per le subnet/VLAN di gestione si applicano a qualsiasi nuovo cloud privato AVS creato dall'utente. Le regole per le VLAN e le subnet definite dall'utente consentono di aggiungere automaticamente nuove VLAN/subnet a cloud privati AVS nuovi o esistenti per un gateway VPN da sito a sito, definendo i criteri per ogni connessione.
...
Un gateway VPN da punto a sito consente di definire un criterio di aggiunta automatica per i nuovi utenti. Per impostazione predefinita, tutti i proprietari e i collaboratori della sottoscrizione hanno accesso ad AVS Portal. Gli utenti vengono creati solo quando il portale AVS viene avviato per la prima volta. Selezionando **Aggiungi regole automaticamente** si consente a qualsiasi nuovo utente di accedere alla rete AVS usando la connessione VPN da punto a sito.

## <a name="set-up-a-site-to-site-vpn-gateway"></a>Configurare un gateway VPN da sito a sito

1. [Accedere al portale AVS](access-cloudsimple-portal.md) e selezionare **rete**.
...
5. Creare la prima connessione dalla rete locale e fare clic su **Avanti**.
    ...
    * Per i **prefissi locali**, immettere i prefissi CIDR locali che accederanno alla rete AVS. Quando si crea la connessione, è possibile aggiungere più prefissi CIDR.

    ![Creare una connessione gateway VPN da sito a sito](media/create-vpn-gateway-s2s-connection.png)

6. Abilitare la VLAN/subnet nella rete di cloud privato AVS a cui sarà possibile accedere dalla rete locale e fare clic su **Avanti**.

    * Per aggiungere una subnet/VLAN di gestione, abilitare **Aggiungi VLAN di gestione/subnet dei cloud privati AVS**. La subnet di gestione è obbligatoria per le subnet vMotion e rete VSAN.
    * Per aggiungere le subnet vMotion, abilitare **la rete Aggiungi vMotion dei cloud privati AVS**.
...
Per connettersi usando la VPN da punto a sito, vedere [connettersi ad AVS con la VPN da punto a sito](set-up-vpn.md#connect-to-avs-using-point-to-site-vpn).
