---
title: Azure VMware Solutions (AVS)-eseguire il backup di macchine virtuali del carico di lavoro in AVS private cloud con Veeam
description: Viene descritto come eseguire il backup delle macchine virtuali in esecuzione in un cloud privato AVS basato su Azure usando Veeam B & R 9,5
author: sharaths-cs
ms.author: b-shsury
ms.date: 08/16/2019
ms.topic: article
...
---
# <a name="back-up-workload-vms-on-avs-private-cloud-using-veeam-br"></a>Eseguire il backup di macchine virtuali del carico di lavoro nel cloud privato AVS usando Veeam B & R

Questa guida descrive come è possibile eseguire il backup delle macchine virtuali in esecuzione in un cloud privato AVS basato su Azure usando Veeam B & R 9,5.
...
È possibile sfruttare Azure per fornire un repository di backup e una destinazione di archiviazione per il backup e l'archiviazione a lungo termine. Tutto il traffico di rete di backup tra le macchine virtuali nel cloud privato AVS e il repository di backup in Azure si sposta su un collegamento a larghezza di banda elevata e a bassa latenza. Il traffico di replica tra le aree passa attraverso la rete interna del backplane di Azure, che riduce i costi di larghezza di banda per gli utenti.

**Distribuzione di base**

Per gli ambienti con meno di 30 TB per il backup, AVS consiglia la configurazione seguente:

* Server di backup Veeam e server proxy installati nella stessa macchina virtuale nel cloud privato AVS.
...
Per gli ambienti con oltre 30 TB per il backup, AVS consiglia la configurazione seguente:

* Un server proxy per nodo nel cluster rete VSAN, come consigliato da Veeam.
* Archivio di backup primario basato su Windows nel cloud privato AVS per memorizzare nella cache cinque giorni di dati per ripristini rapidi.
...
## <a name="requirements-for-veeam-solution-on-avs"></a>Requisiti per la soluzione Veeam in AVS

Per la soluzione Veeam è necessario eseguire le operazioni seguenti:

* Fornisci le tue licenze Veeam.
* Distribuire e gestire Veeam per eseguire il backup dei carichi di lavoro in esecuzione nel cloud privato AVS.
...
## <a name="install-and-configure-veeam-backups-in-your-avs-private-cloud"></a>Installare e configurare i backup Veeam nel cloud privato AVS

Le sezioni seguenti descrivono come installare e configurare una soluzione di backup Veeam per il cloud privato AVS.

Il processo di distribuzione è costituito dai passaggi seguenti:

1. [interfaccia utente di vCenter: configurare i servizi di infrastruttura nel cloud privato AVS](#vcenter-ui-set-up-infrastructure-services-in-your-avs-private-cloud)
2. [Portale AVS: configurare l'AVS per la rete privata del cloud per Veeam](#avs-private-cloud-set-up-avs-private-cloud-networking-for-veeam)
3. [Portale AVS: escalation dei privilegi](#avs-private-cloud-escalate-privileges-for-cloudowner)
4. [Portale di Azure: connettere la rete virtuale al cloud privato AVS](#azure-portal-connect-your-virtual-network-to-the-avs-private-cloud)
5. [Portale di Azure: creare un repository di backup in Azure](#azure-portal-connect-your-virtual-network-to-the-avs-private-cloud)
...
7. [interfaccia utente di vCenter del cloud privato AVS: installare Veeam B & R](#vcenter-console-of-avs-private-cloud-install-veeam-br)
...
9. [Portale AVS: impostare i privilegi di accesso Veeam e decalation](#avs-portal-set-up-veeam-access-and-de-escalate-privileges)
...
* Un [cloud privato AVS](create-private-cloud.md) creato tramite il portale AVS.  
...
* Media installabile Veeam 9,5 U3 (ISO) caricato nell'archivio dati rete VSAN del cloud privato AVS

### <a name="vcenter-ui-set-up-infrastructure-services-in-your-avs-private-cloud"></a>interfaccia utente di vCenter: configurare i servizi di infrastruttura nel cloud privato AVS
...
* È possibile aggiungere un provider di identità esterno come descritto in [configurare le origini di identità vCenter da usare Active Directory](set-vcenter-identity.md) se si verifica una delle condizioni seguenti:
  * Si vuole identificare gli utenti dal Active Directory locale (AD) nel cloud privato AVS.
  * Si vuole configurare un annuncio nel cloud privato AVS per tutti gli utenti.
...
### <a name="avs-private-cloud-set-up-avs-private-cloud-networking-for-veeam"></a>AVS private cloud: configurare AVS private cloud networking per Veeam

Accedere al portale di AVS per configurare la rete AVS private cloud per la soluzione Veeam.
...
Per impostazione predefinita, AVS fornisce un collegamento 1 Gbps ExpressRoute. Per le dimensioni di ambiente più grandi, potrebbe essere necessario un collegamento con una maggiore larghezza di banda. Per ulteriori informazioni sui collegamenti con larghezza di banda più elevata, contattare il supporto di Azure.

Per continuare l'installazione, sono necessari la chiave di autorizzazione e l'URI del circuito peer e l'accesso alla sottoscrizione di Azure. Queste informazioni sono disponibili nella pagina connessione della rete virtuale nel portale di AVS. Per istruzioni, vedere [ottenere informazioni sul peering per la rete virtuale di Azure per la connessione AVS](virtual-network-connection.md). Se si verificano problemi durante il recupero delle informazioni, [contattare il supporto tecnico](https://portal.azure.com/#blade/Microsoft_Azure_Support/HelpAndSupportBlade/newsupportrequest).

### <a name="avs-private-cloud-escalate-privileges-for-cloudowner"></a>AVS private cloud: escalation dei privilegi per **cloudowner**

L'utente ' cloudowner ' predefinito non dispone di privilegi sufficienti in AVS private cloud vCenter per installare VEEAM, quindi è necessario eseguire l'escalation dei privilegi vCenter dell'utente. Per ulteriori informazioni, vedere [privilegi di escalation](escalate-private-cloud-privileges.md).

### <a name="azure-portal-connect-your-virtual-network-to-the-avs-private-cloud"></a>Portale di Azure: connettere la rete virtuale al cloud privato AVS

Connettere la rete virtuale al cloud privato AVS seguendo le istruzioni riportate in [connessione alla rete virtuale di Azure con ExpressRoute](azure-expressroute-connection.md).
...
### <a name="vcenter-console-of-avs-private-cloud-install-veeam-br"></a>Console vCenter del cloud privato AVS: installare Veeam B & R

Accedere a vCenter dal cloud privato AVS per creare un account del servizio Veeam, installare Veeam B & R 9,5 e configurare Veeam con l'account del servizio.
...
1. Aggiungere VMware vSphere come ambiente server gestito. Quando richiesto, specificare le credenziali dell'account del servizio Veeam creato all'inizio della [console vCenter di AVS private cloud: installare Veeam B & R](#vcenter-console-of-avs-private-cloud-install-veeam-br).
...
### <a name="avs-portal-set-up-veeam-access-and-de-escalate-privileges"></a>Portale AVS: impostare i privilegi di accesso Veeam e decalation
...
### <a name="avs-references"></a>Riferimenti AVS
...
* [Autorizzazioni per cloud privato AVS](learn-private-cloud-permissions.md)
