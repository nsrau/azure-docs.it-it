---
title: 'Azure VMware Solutions (AVS): configurare le origini di identità vCenter in AVS private cloud'
description: Viene descritto come configurare AVS private cloud vCenter per l'autenticazione con Active Directory per gli amministratori VMware per accedere a vCenter
author: sharaths-cs
ms.author: b-shsury
ms.date: 08/15/2019
ms.topic: article
...
VMware vCenter supporta diverse origini di identità per l'autenticazione degli utenti che accedono a vCenter. È possibile configurare AVS private cloud vCenter per l'autenticazione con Active Directory per gli amministratori VMware per accedere a vCenter. Al termine dell'installazione, l'utente **cloudowner** può aggiungere utenti dall'origine identità a vCenter. 
...
* Nuovi controller di dominio e dominio Active Directory in esecuzione nel cloud privato AVS
* Servizio Azure Active Directory

Questa guida illustra le attività per la configurazione di Active Directory controller di dominio e di dominio in esecuzione in locale o come macchine virtuali nelle sottoscrizioni. Se si vuole usare Azure AD come origine di identità, vedere [usare Azure ad come provider di identità per vCenter in AVS private cloud](azure-ad.md) per istruzioni dettagliate sulla configurazione dell'origine di identità.
...
* [Configurare nuovi Active Directory in un cloud privato AVS](#set-up-new-active-directory-on-an-avs-private-cloud)
...
* [Connessione VPN da sito a sito](vpn-gateway.md#set-up-a-site-to-site-vpn-gateway) dal Data Center locale al cloud privato AVS.
...
## <a name="set-up-new-active-directory-on-an-avs-private-cloud"></a>Configurare nuovi Active Directory in un cloud privato AVS

È possibile configurare un nuovo dominio di Active Directory nel cloud privato AVS e usarlo come origine di identità per l'accesso Single Sign-on. Il Active Directory dominio può essere parte di una foresta Active Directory esistente oppure può essere configurato come una foresta indipendente.
...
1. [Escalation dei privilegi](escalate-private-cloud-privileges.md) sul cloud privato AVS.

2. Accedere a vCenter per il cloud privato AVS.
