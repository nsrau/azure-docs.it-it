...
In questa guida verranno fornite le nozioni di base per usare il driver Microsoft per Node.JS per SQL Server per accedere a un database SQL di Azure. Gli scenari presentati includono la **creazione di un database SQL** e la **connessione a un database SQL**. In questa guida viene illustrata la creazione di un database SQL dal [portale di gestione di Azure][portale di gestione di Azure].
...
-   [Considerazioni sulla distribuzione di Azure][Considerazioni sulla distribuzione di Azure]
...
Il Driver Microsoft per Node.JS per SQL Server consente agli sviluppatori di accedere ai dati archiviati in Microsoft SQL Server o nel database SQL di Azure da un'applicazione Node.js. Il driver è attualmente solo una versione di anteprima. Altre funzionalità verranno integrate nel progetto non appena completate. Per altre informazioni sul driver, vedere la pagina [Github][Github] del progetto relativo al driver Microsoft per Node.JS per SQL Server e il [Wiki][Wiki] associato.

<div class="dev-callout">
<b>Nota</b>
<p>Il driver Microsoft per Node.js per SQL Server &egrave; attualmente disponibile come versione di anteprima e si basa su componenti di runtime disponibili solo nei sistemi operativi Microsoft Windows e Azure.</p>
</div>
...
Il driver Microsoft SQL Server per Node.js si basa su SQL Server Native Client. Anche se il client nativo è automaticamente disponibile quando l'applicazione viene distribuita in Azure, è possibile che non sia presente nell'ambiente di sviluppo locale. È possibile installare SQL Server Native Client dalla pagina di download [Feature Pack di Microsoft SQL Server 2012][Feature Pack di Microsoft SQL Server 2012].

<div class="dev-callout">
<b>Nota</b>
<p>SQL Server Native Client &egrave; disponibile solo per il sistema operativo Microsoft Windows. Anche se questo driver &egrave; disponibile in modo nativo in Azure, non sar&agrave; possibile testare l'applicazione localmente usando le informazioni in questo articolo, se si sta eseguendo lo sviluppo in un sistema operativo diverso da Microsoft Windows.</p>
</div>

### Installare Node.js

Node.js può essere installato da [][]<http://nodejs.org/#download></a>. Se un pacchetto di installazione non è disponibile per il sistema operativo, è possibile compilare Node.js dall'origine.
...
Per visualizzare le informazioni sul server e sul database, fare clic su **SQL Databases** nel portale di gestione di Azure. Sarà quindi possibile fare clic su **DATABASES** o su **SERVERS** per visualizzare le informazioni pertinenti.
...
Il driver Microsoft per Node.js per SQL Server è disponibile come modulo nativo node-sqlserver. Una versione binaria di questo modulo è disponibile nell'[Area download][Area download]. Per usare la versione binaria, eseguire la procedura seguente:
...
<div class="dev-callout">
<b>Nota</b>
<p>&Egrave; inoltre possibile installare il modulo node-sqlserver mediante l'utilit&agrave; npm. Questa tuttavia richiamer&agrave; node-gyp per compilare una versione binaria del modulo nel sistema.</p>
</div>

### Specificare la stringa di connessione

Per usare node-sqlserver, è necessario richiederlo nell'applicazione e specificare una stringa di connessione. La stringa di connessione deve essere il valore ODBC restituito nella sezione [Procedura: recuperare le informazioni sulla connessione al database SQL][Procedura: Recuperare le informazioni sulla connessione al database SQL] di questo articolo. Il codice sarà simile al seguente:
...
## <span id="Deploy"></span></a>Considerazioni sulla distribuzione di Azure

<div class="dev-callout">
<b>Nota</b>
<p>Le informazioni seguenti si basano su una versione di anteprima del driver Microsoft per Node.js per SQL Server. &Egrave; possibile che le informazioni contenute in questa sezione non siano le pi&ugrave; recenti sull'utilizzo del modulo node-sqlserver con Azure. Per le informazioni pi&ugrave; recenti, vedere la <a href="https://github.com/WindowsAzure/node-sqlserver">pagina del progetto relativo al driver Microsoft per Node.JS per SQL Server</a> in Github.</p>
</div>
...
Per altre informazioni sulla distribuzione di un'applicazione Node.js in Azure, vedere [Creazione e distribuzione di un'applicazione Node.js in un sito Web di Azure][Creazione e distribuzione di un'applicazione Node.js in un sito Web di Azure] e [Servizio cloud Node.js][Servizio cloud Node.js].

## <span id="NextSteps"></span></a>Passaggi successivi

-   [Presentazione del driver Microsoft per Node.JS per SQL Server][Presentazione del driver Microsoft per Node.JS per SQL Server]
-   [Driver Microsoft per Node.JS per SQL Server in Github.com][Github]
  ...
  [Servizio cloud Node.js]: /it-it/develop/nodejs/tutorials/getting-started/
