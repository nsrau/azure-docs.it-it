---
title: Considerazioni sulla sicurezza per Resource Manager | Documentazione Microsoft
...
ms.openlocfilehash: 5a7a053d856c258bf4fdec339aee904d2f4c4626


---
# <a name="security-considerations-for-azure-resource-manager"></a>Considerazioni sulla sicurezza per Gestione risorse di Azure
...
In questo argomento si presuppone che si abbia familiarità con il controllo degli accessi in base al ruolo (RBAC) di Gestione risorse di Azure. Per altre informazioni, vedere [Controllo degli accessi in base al ruolo di Azure](active-directory/role-based-access-control-configure.md).
...
## <a name="secrets-and-certificates"></a>Segreti e certificati
...
* Contesto completo di tutti gli asset:
  ...
  * Materiale chiave effettive nell'insieme di credenziali.  
    Ogni modello, e l'azione, può essere in diversi ruoli RBAC per la separazione completa dei compiti.
...
### <a name="separation-of-keys-from-deployments"></a>Separazione delle chiavi dalle distribuzioni
...
## <a name="service-principals-for-cross-subscription-interactions"></a>Entità servizio per le interazioni tra sottoscrizioni
...
### <a name="how-network-security-groups-work"></a>Come funzionano i gruppi di sicurezza di rete
...
Un gruppo di sicurezza di rete ha un *nome*, un'*area* associata, ovvero una delle località supportate da Azure, e un'etichetta descrittiva. Contiene due tipi di regole, In ingresso e In uscita. Le regole In ingresso vengono applicate ai pacchetti in ingresso in una macchina virtuale, mentre le regole In uscita vengono applicate ai pacchetti in uscita dalla macchina virtuale.
Le regole vengono applicate al computer server in cui si trova la macchina virtuale. Un pacchetto in ingresso o in uscita deve corrispondere a una regola di assenso per essere autorizzato; in caso contrario, viene eliminato.
...
### <a name="special-infrastructure-rules"></a>Regole speciali per l'infrastruttura
...
### <a name="associating-an-nsg-with-a-vm"></a>Associazione di un gruppo di sicurezza di rete a una macchina virtuale
...
### <a name="associating-an-nsg-with-a-subnet"></a>Associazione di un gruppo di sicurezza di rete a una subnet
...
### <a name="associating-an-nsg-with-a-subnet-and-a-vm"></a>Associazione di un gruppo di sicurezza di rete a una subnet e una macchina virtuale
È possibile associare un gruppo di sicurezza di rete a una macchina virtuale e un altro gruppo di sicurezza di rete alla subnet in cui risiede la macchina virtuale. Questo scenario è supportato per fornire alla macchina virtuale due livelli di protezione.
Sul traffico in ingresso, il pacchetto segue le regole di accesso specificate nella subnet, seguite dalle regole della macchina virtuale. Quando in uscita, il pacchetto segue prima le regole specificate nella macchina virtuale, quindi segue le regole specificate nella subnet, come illustrato di seguito.
...
### <a name="default-routes"></a>Route predefinite
...
Al momento della stesura di questo articolo, [ExpressRoute](expressroute/expressroute-introduction.md) non è ancora supportato nel [provider di risorse di rete](virtual-network/resource-groups-networking.md) per Azure Resource Manager.  Se si dispone di una connessione ExpressRoute tra la rete locale e Azure, è possibile abilitare BGP a propagare le route dalla rete locale in Azure una volta che ExpressRoute è supportato dal provider di risorse di rete. Queste route BGP vengono utilizzate nello stesso modo come route predefinite e le route definite dall'utente in ogni subnet di Azure. Per altre informazioni, vedere [Panoramica tecnica relativa a ExpressRoute](expressroute/expressroute-introduction.md).

> [!NOTE]
> Quando ExpressRoute sarà supportato dal provider di risorse di rete, sarà possibile configurare l'ambiente Azure per usare il tunneling forzato attraverso la rete locale tramite la creazione di una route definita dall’utente per la subnet 0.0.0.0/0 che usa il gateway VPN come hop successivo. Tuttavia, funziona solo se si utilizza un gateway VPN, non ExpressRoute. Per ExpressRoute, il tunneling forzato viene configurato tramite BGP.
> 
> 

### <a name="user-defined-routes"></a>Route definite dall'utente
Non è possibile visualizzare le route predefinite specificate in precedenza nell'ambiente Azure e per la maggior parte degli ambienti, questi sono solo le route che è necessario.
Tuttavia, è necessario creare una tabella di route e aggiungere una o più route in casi specifici, ad esempio:
...
* Per configurare routing e inoltro IP, vedere [Creare route definite dall'utente in Resource Manager tramite un modello](virtual-network/virtual-network-create-udr-arm-template.md)
* Per una panoramica del controllo degli accessi in base al ruolo, vedere [Controllo degli accessi in base al ruolo nel portale di Microsoft Azure](active-directory/role-based-access-control-configure.md)
