...
Cloud Cruiser può usare le informazioni dell'API RateCard in modi diversi. Questo articolo illustra come può essere usata per eseguire la simulazione dei costi e l'analisi del carico di lavoro IaaS.
...
In questo esempio vengono usate le informazioni dell'API RateCard per definire nuovi servizi in Cloud Cruiser. Nello stesso modo, è possibile utilizzare i costi associate ai servizi per creare un nuovo piano tariffario in Cloud Cruiser.
...
Dopo aver acquisito i dati sul consumo e caricato i costi nel database Cloud Cruiser, è possibile usare il modulo Insights di Cloud Cruiser per creare la simulazione dei costi del carico di lavoro desiderato.
...
Il grafico inferiore riporta gli stessi dati suddivisi per reparto. I costi sostenuti da ogni reparto per eseguire il carico di lavoro in WAP e in Azure e le relative differenze vengono visualizzati nella barra dei risparmi (verde).

## <a name="azure-usage-api"></a>API di utilizzo di Azure
### <a name="introduction"></a>Introduzione
Microsoft ha introdotto di recente l'API di utilizzo di Azure, che consente ai sottoscrittori di inserire dati di utilizzo a livello di codice per ottenere informazioni significative sul consumo. I clienti di Cloud Cruiser possono usufruire del set di dati più completo disponibile attraverso questa API.
...
Questa esercitazione presenta un esempio di come Cloud Cruiser possa trarre vantaggio dalle informazioni dell'API di utilizzo. In particolare verrà creato un gruppo di risorse in Azure, verranno associati tag per la struttura dei conti e quindi verrà descritto il processo di estrazione ed elaborazione delle informazioni sui tag in Cloud Cruiser.
...
Viene quindi usata l'API di utilizzo per popolare il foglio *UsageAPI* e correlare le informazioni con i dati di consumo provenienti dall'API di fatturazione nel foglio *PublishData*.
...
Il passaggio successivo è creare una tabella di ricerca associando le informazioni dei tag a *ResourceGroupName*. Questa tabella di ricerca viene usata nel passaggio successivo per migliorare i dati sul consumo con le informazioni sui tag.
...
Ora è necessario solo aggiungere un passaggio per pubblicare i dati di utilizzo. Durante questo passaggio le tariffe appropriate per ogni servizio definito nel piano tariffario vengono applicate alle informazioni sull'utilizzo con l'addebito risultante caricato nel database.
...
* Per altre informazioni su Azure Resource Manager, vedere l'articolo [Panoramica di Azure Resource Manager](../azure-resource-manager/resource-group-overview.md) .
