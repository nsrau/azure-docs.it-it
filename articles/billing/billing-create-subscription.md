...
In Azure è possibile creare sottoscrizioni aggiuntive per l'account. Una sottoscrizione aggiuntiva può essere utile per evitare di raggiungere i limiti della sottoscrizione, per creare ambienti distinti per finalità di sicurezza e fatturazione o per isolare i dati per motivi di conformità.

Se si intende creare le sottoscrizioni di Azure in base al Contratto Enterprise dell'organizzazione, è necessario disporre del ruolo di proprietario dell'account per l'organizzazione.

Se non si ha già una sottoscrizione di Azure, è possibile creare un [account gratuito](https://azure.microsoft.com/free).
...
È anche possibile creare sottoscrizioni aggiuntive a livello di codice. Per altre informazioni, vedere [Creare sottoscrizioni di Azure Enterprise a livello di codice](../azure-resource-manager/programmatically-create-subscription.md).
