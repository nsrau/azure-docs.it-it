<properties 
	pageTitle="Convertire un esperimento di training di Machine Learning in un esperimento di assegnazione dei punteggi | Microsoft Azure" 
	description="Come convertire un esperimento di training Machine Learning, utilizzato per il training del modello di analisi predittiva, in un esperimento di assegnazione di un punteggio che può essere pubblicato come servizio Web." 
...
Il pulsante **Create Scoring Experiment** nella parte inferiore dell'area di disegno dell'esperimento esegue automaticamente i tre passaggi di conversione dell'esperimento di training in un esperimento di assegnazione dei punteggi:
...
Quando si converte l'esperimento di training in un esperimento di assegnazione dei punteggi, alcuni di questi moduli non sono più necessari o hanno uno scopo diverso:

- **Data**: i dati in questo set di dati di esempio non vengono usati durante l'assegnazione di un punteggio. L'utente del servizio Web fornisce i dati da classificare. Tuttavia, i metadati di questo set di dati, ad esempio i tipi di dati, vengono utilizzati dal modello sottoposto a training. Pertanto, è necessario mantenere il set di dati nell'esperimento di classificazione in modo da poter fornire questi metadati.
...
- **Score**: nell'esempio, il modulo Split viene usato per suddividere il flusso di dati in un set di dati di test e dati di training. Questa operazione non è necessaria nell'esperimento di classificazione, pertanto può essere rimosso. In modo analogo, il secondo modulo [Score Model][score-model] e il modulo [Valuta modello][evaluate-model] vengono usati per confrontare i risultati dei dati di test, quindi i moduli non sono necessari nell'esperimento di classificazione. Il modulo rimanente [Score Model][score-model] è però necessario per restituire un risultato relativo all'assegnazione del punteggio tramite il servizio Web.

Di seguito è possibile visualizzare l'aspetto dell'esempio dopo aver selezionato **Create Scoring Experiment**:

![Esperimento di assegnazione dei punteggi convertito][figure3]
...
Se presenti nell'esperimento, è possibile rimuove i moduli di cui si è certi che non saranno utili per la classificazione. Ad esempio, poiché il modulo **Web service input** è stato spostato in un punto successivo ai moduli di elaborazione dei dati, è possibile rimuovere il modulo [Clean Missing Data][clean-missing-data] dall'esperimento di assegnazione di un punteggio.

L'esperimento di classificazione ora appare come segue:
...
Per altre informazioni sul processo di pubblicazione completo, vedere [Pubblicare un servizio Web di Azure Machine Learning][publish]
