---
title: 'Esercitazione: Usare R per creare un modello di Machine Learning (anteprima)'
...
> [!IMPORTANT]
> Il servizio Azure Machine Learning R SDK è attualmente disponibile in anteprima pubblica.
...
> * Clonare https://github.com/Azure/azureml-sdk-for-r per ottenere i file necessari per eseguire questa esercitazione nell'area di lavoro personale
...
Questo esempio usa un'istanza di calcolo nell'area di lavoro per un'esperienza preconfigurata senza installazioni. Se si preferisce avere il controllo sull'ambiente, sui pacchetti e sulle dipendenze nel computer in uso, usare il [proprio ambiente](https://azure.github.io/azureml-sdk-for-r/articles/installation.html).

Per eseguire questa esercitazione, usare RStudio in un'istanza di calcolo di Azure Machine Learning.  

1. Selezionare **Calcolo** a sinistra.

1. Aggiungere una risorsa di calcolo, se non ne esiste già una.
...
* Connettersi a un'area di lavoro per consentire all'istanza di calcolo di comunicare con risorse remote
...
Per l'istanza di calcolo è già installata la versione più recente di R SDK di CRAN. Se si vuole invece installare la versione di sviluppo di GitHub per disporre delle correzioni di bug più recenti, eseguire i comandi seguenti:
...
> [!WARNING]
> Se durante il processo di installazione viene visualizzato il messaggio di richiesta "`Would you like to install Miniconda? [Y/n]:`", rispondere con "`n`" perché nell'istanza di calcolo Anaconda è già presente e l'installazione di Miniconda non è necessaria.

A questo punto, proseguire e importare il pacchetto **azuremlsdk**.
...
Uno script di training denominato `accidents.R` è incluso nella directory *train-and-deploy-first-model*. Notare i dettagli seguenti **all'interno dello script di training** che permettono di sfruttare il servizio Azure Machine Learning per il training:
...
Con il modello è possibile stimare il rischio di morte in seguito a una collisione. Usare Azure ML per distribuire il modello come servizio di stima. In questa esercitazione si distribuirà il servizio Web in [Istanze di Azure Container](../container-instances/index.yml) (ACI).
