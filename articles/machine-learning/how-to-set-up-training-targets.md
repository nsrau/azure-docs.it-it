...
Usare uno di questi modi per specificare un'identità gestita per un cluster di calcolo:
    
* In studio, durante la creazione del cluster di calcolo o quando si modificano i dettagli del cluster di calcolo, impostare **assegna un'identità gestita** e specificare un'identità assegnata dal sistema o un'identità assegnata dall'utente.
    
* Con Python SDK, impostare l' `identity_type` attributo nella configurazione del provisioning.  
...
* Con Python SDK, impostare gli `identity_type` attributi e `identity_id` (se si creano identità gestite assegnate dall'utente) nella configurazione del provisioning.  
...
* Usando l'interfaccia della riga di comando, impostare l' `assign-identity` attributo durante la creazione del cluster:
