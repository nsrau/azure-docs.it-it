...
La macchina virtuale Linux per l'analisi scientifica dei dati è una macchina pre-installata con una raccolta di strumenti usati comunemente per l'analisi dei dati e il Machine Learning. Ecco i componenti software principali:
...
3.	 Le sezioni seguenti forniscono gli **input** per ognuno dei **5 passaggi** (elencati in ordine numerico nella parte destra della figura precedente) della procedura guidata usata per creare la macchina virtuale per l'analisi scientifica dei dati di Microsoft. Di seguito sono riportati gli input necessari per configurare ciascuno di questi passaggi:
  ...
  **a. Informazioni di base**:
   ...
   - **Sottoscrizione**: se si ha più di una sottoscrizione, selezionare quella in cui viene creata e fatturata la macchina virtuale. NOTA: è necessario disporre di privilegi di creazione delle risorse per questa sottoscrizione.
  ...
  **c. Impostazioni**
   ...
   - **Altri parametri**: nella maggior parte dei casi si usano semplicemente i valori predefiniti. Nel caso in cui si desideri usare valori non predefiniti, è possibile passare il puntatore sul collegamento informativo per visualizzare informazioni su campi specifici.
...
#### Postgres
Il database open source **Postgres** è disponibile nella VM con i servizi in esecuzione e initdb già completati. È comunque necessario creare database e utenti. Per altre informazioni, fare riferimento alla [documentazione di Postgres](https://www.postgresql.org/docs/).
...
>[AZURE.NOTE] Esistono alcune differenze in questa utilità tra le piattaforme Linux e Windows. Per informazioni dettagliate, vedere le pagine di documentazione.
...
- **Librerie di Azure**: di seguito sono riportate alcune delle librerie installate, disponibili e pronte all'uso:
...
>[AZURE.NOTE] Queste istruzioni sono state scritte per la versione Windows della macchina virtuale di analisi scientifica dei dati. Tuttavia, le informazioni sulla distribuzione di modelli in Azure ML è applicabile anche alle macchine virtuali Linux.
...
È un sistema di Machine Learning che supera i limiti del MAchine Learning grazie all'uso di tecniche come hash, allreduce, reduction, learning2search, nonché apprendimento online, attivo e interattivo.
...
>[AZURE.NOTE] Se non si dispone di accesso per l'installazione del pacchetto nella directory predefinita, è possibile che venga visualizzata la richiesta sulla console R di Windows per l'installazione dei pacchetti nella libreria personale. Se vengono visualizzate queste richieste, rispondere *y* (Sì).

1. Fare clic su **Execute** (Esegui).
...
6. Fare clic sul pulsante di opzione **Forest** (Foresta) e quindi su **Execute** (Esegui) per creare una foresta casuale.
7. Fare clic sulla scheda **Evaluate** (Valuta).
8. Fare clic sul pulsante di opzione **Risk** (Rischio) e quindi su **Execute** (Esegui) per visualizzare due tracciati delle prestazioni per Risk (Cumulative) (Rischio (Cumulativo)).
9. Fare clic sulla scheda **Log** (Log) per visualizzare il codice R generato per le operazioni precedenti. Nota: esiste un bug nella versione corrente di rattle. Occorre inserire un carattere *#* davanti a *Export this log ...* (Esporta il log...) nel testo del log.
10. Fare clic sul pulsante **Export** (Esporta) per salvare lo script R nel file *weather\_script.R* della home directory.
