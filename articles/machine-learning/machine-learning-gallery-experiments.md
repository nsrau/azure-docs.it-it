...
---
# <a name="discover-experiments-in-the-cortana-intelligence-gallery"></a>Scoprire esperimenti in Cortana Intelligence Gallery
...
> [!NOTE]
> Un ***esperimento*** è costituito da un'area di disegno in Azure Machine Learning Studio che consente di creare un modello di analisi predittiva collegando dati a vari moduli analitici. È possibile provare idee diverse, effettuare esecuzioni di prova e alla fine pubblicare il modello come servizio Web in Azure. Per un esempio di creazione di un esperimento semplice, vedere [Esercitazione di Machine Learning: Creare il primo esperimento in Azure Machine Learning Studio](machine-learning-create-experiment.md). Per una procedura dettagliata più completa della creazione di una soluzione di analisi predittiva, vedere [Procedura dettagliata: Sviluppare una soluzione di analisi predittiva per la valutazione del rischio di credito in Azure Machine Learning](machine-learning-walkthrough-develop-predictive-solution.md).
> 
> 

## <a name="discover"></a>Scoprire
  Per cercare gli esperimenti disponibili in Gallery, aprire [Gallery](http://gallery.cortanaintelligence.com) e fare clic su **Experiments**
 (Esperimenti) nella parte alta della home page di Gallery.
...
Sono disponibili due modi per ottenere una copia dell'esperimento:

* **Da Gallery**: se si trova un esperimento di proprio interesse in Gallery, è possibile scaricarne facilmente una copia nella propria area di lavoro di Machine Learning Studio.
* **Da Machine Learning Studio**: in Studio è possibile usare qualsiasi esperimento presente in Gallery come modello per creare un nuovo esperimento.

### <a name="from-the-gallery"></a>Da Gallery
Per scaricare una copia di un esperimento da Gallery:
...
È possibile aprire gli stessi esperimenti di esempio anche mentre si lavora in Machine Learning Studio:
...
6. Usare la casella Detailed description (Descrizione dettagliata) per descrivere le diverse parti dell'esperimento. Ecco alcuni elementi utili da includere:
   ...
   * Risultati e valutazione delle prestazioni del modello
     ...
     <!-- -->
     > [!TIP]
     > La casella per l'anteprima e la modifica markdown è molto piccola. È consigliabile scrivere la documentazione in un editor markdown e incollare il documento finito nella casella di testo.  Dopo aver pubblicato l'esperimento, è possibile usare strumenti standard basati sul Web con la sintassi markdown per visualizzare in anteprima il modello e apportare eventuali correzioni e modifiche necessarie.
     > 
     > 
...
8. Scegliere se rendere il contenuto disponibile pubblicamente o accessibile solo agli utenti che hanno il collegamento.
   ...
   > [!TIP]
   > Per verificare la documentazione prima della pubblicazione vera e propria, è possibile pubblicarla inizialmente come non in elenco e quindi renderla pubblica dalla pagina dell'elemento.
   > 
   > 
...
Ora è possibile visualizzare l'esperimento in Cortana Intelligence Gallery e condividere il collegamento con altri utenti. Se è stato reso disponibile pubblicamente, l'esperimento verrà visualizzato nelle pagine di Gallery e nei risultati delle ricerche. Quando si è connessi è anche possibile modificare in qualsiasi momento la documentazione relativa alla pagina dell'elemento.
...
È possibile eliminare l'esperimento oppure modificare una delle seguenti informazioni sull'esperimento direttamente da Gallery:

* Nome dell'esperimento
* Riepilogo o testo descrittivo
* Tag specificati
* Immagine usata
* Impostazione di visibilità (pubblica o non in elenco)
* Eliminazione dell'esperimento da Gallery

Queste modifiche possono essere apportate in Gallery dalla pagina dei dettagli dell'esperimento o dalla pagina del profilo personale.
...
* Trattandosi di una soluzione relativamente nuova, non tutti i lettori hanno familiarità con Cortana Intelligence Suite.
  È quindi opportuno fornire informazioni esaurienti e spiegazioni dettagliate per consentire a tutti di svolgere il lavoro correttamente.
* Immagini quali grafici di esperimenti o screenshot di dati possono essere molto utili per permettere ai lettori di interpretare e usare rapidamente i contenuti. Per altre informazioni su come includere immagini nella documentazione, vedere [Publishing Guidelines and Examples collection](https://gallery.cortanaintelligence.com/Collection/Publishing-Guidelines-and-Examples-1) (Pubblicazione di linee guida e raccolta di esempi).
* Se nell'esperimento si include un set di dati non importato tramite il modulo di importazione dati, il set di dati diventa parte integrante dell'esperimento e viene pubblicato in Gallery. Assicurarsi quindi che le condizioni di licenza del set di dati pubblicato consentano la condivisione e il download da parte di chiunque. I contributi pubblicati in Cortana Intelligence Gallery sono coperti dalle [Condizioni per l'utilizzo](https://azure.microsoft.com/support/legal/website-terms-of-use/)di Azure.
...
Le immagini inviate insieme all'esperimento vengono usate per creare il riquadro dell'esperimento con informazioni sull'autore che ha inserito il proprio contributo. È consigliabile inviare immagini di dimensioni inferiori a 500 KB, con proporzioni 3:2 e risoluzione di 960 x 640.

**Come vengono impiegati i set di dati usati nell'esperimento? Vengono anch'essi pubblicati in Gallery?**

Se il set di dati non è stato importato tramite il modulo di importazione dati ma fa parte dell'esperimento, viene pubblicato in Gallery contestualmente all'esperimento. Per questo motivo è necessario accertarsi che i termini del contratto di licenza relativo al set di dati pubblicato con l'esperimento consentano la condivisione e il download da parte di qualsiasi utente.

**Per la realizzazione di un esperimento ho usato le mie credenziali per estrarre i dati da HDInsight o SQL tramite un modulo di importazione dati. Come posso assicurarmi che le mie credenziali non verranno condivise con la pubblicazione dell'esperimento?**

Per il momento non è consentita la pubblicazione di esperimenti che usano credenziali.

**Come posso inserire più tag?**

Dopo aver inserito un tag, premere il tasto TAB per inserire un altro tag.
