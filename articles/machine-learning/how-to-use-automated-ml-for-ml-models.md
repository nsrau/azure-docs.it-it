...
1. Accedere a Azure Machine Learning all'indirizzo https://ml.azure.com. 
...
> [!NOTE]
> Se si prevede di esportare i modelli creati automaticamente in un [modello ONNX](concept-onnx.md), solo le opzioni conteggi indicate con * sono supportate nel formato ONNX. Altre informazioni sulla [conversione di modelli in ONNX](concept-automated-ml.md#use-with-onnx). 

|&nbsp;Passaggi di pre-elaborazione| Descrizione |
| ------------- | ------------- |
|Elimina la cardinalità elevata o nessuna funzionalità di varianza * |Eliminarli dai set di training e di convalida, incluse le funzionalità con tutti i valori mancanti, lo stesso valore in tutte le righe o con cardinalità estremamente elevata (ad esempio, hash, ID o GUID).|
...
|Genera funzionalità aggiuntive * |Per le funzionalità DateTime: anno, mese, giorno, giorno della settimana, giorno dell'anno, trimestre, settimana dell'anno, ora, minuto, secondo.<br/><br/>Per le funzionalità di testo: frequenza dei termini basata su unigrammi, bi-grammi e Tri-caratteri-grammi.|
|Trasforma e codifica *|Le funzionalità numeriche con pochi valori univoci vengono trasformate in funzionalità categoriche.<br/><br/>La codifica One-Hot viene eseguita per una cardinalità categorica bassa; per la cardinalità elevata, codifica One-Hot-hash.|
...
I dati Guardrails vengono applicati quando conteggi automatico è abilitato o la convalida è impostata su auto. I dati Guardrails consentono di identificare i potenziali problemi relativi ai dati (ad esempio, i valori mancanti, lo squilibrio della classe) e di intraprendere azioni correttive per ottenere risultati migliori. 

Gli utenti possono esaminare i dati Guardrails in studio nella scheda **Data Guardrails** di un'esecuzione automatizzata di Machine Learning ```show_output=True``` o impostando quando si invia un esperimento con Python SDK. 

#### <a name="data-guardrail-states"></a>Stati di barriera dati

Guardrails dati visualizzeranno uno dei tre stati seguenti: **superato**, **completato**o con **avviso**.

State| Descrizione
----|----
Passed| Non sono stati rilevati problemi relativi ai dati e non è richiesta alcuna azione da utente. 
Operazione completata| Sono state applicate modifiche ai dati. Si consiglia agli utenti di esaminare le azioni correttive apportate da Machine Learning automatiche per garantire che le modifiche siano allineate con i risultati previsti. 
Avvisi| È stato rilevato un problema di dati che non è stato possibile rimediare. Invitiamo gli utenti a rivedere e risolvere il problema. 

>[!NOTE]
> Nelle versioni precedenti degli esperimenti di Machine Learning automatici è stato visualizzato un quarto stato: **fixed**. Gli esperimenti più recenti non visualizzeranno questo stato e tutti i Guardrails che visualizzano lo stato **fisso** verranno ora visualizzati come **done**.   

La tabella seguente descrive i dati Guardrails attualmente supportati e gli Stati associati che gli utenti possono incontrare durante l'invio dell'esperimento.

Guardrail|Stato|Condizione&nbsp;per&nbsp;il trigger
---|---|---
Imputazione dei valori di funzionalità mancanti |**Passed** <br><br><br> **Operazione completata**| Nei dati di training non sono stati rilevati valori di funzionalità mancanti. Altre informazioni sull' [imputazione del valore mancante.](https://docs.microsoft.com/azure/machine-learning/how-to-use-automated-ml-for-ml-models#advanced-featurization-options) <br><br> I valori di funzionalità mancanti sono stati rilevati nei dati di training e imputati.
Gestione delle funzionalità di cardinalità elevata |**Passed** <br><br><br> **Operazione completata**| Gli input sono stati analizzati e non sono state rilevate funzionalità di cardinalità elevata. Altre informazioni sul [rilevamento della funzionalità di cardinalità elevata.](https://docs.microsoft.com/azure/machine-learning/how-to-use-automated-ml-for-ml-models#advanced-featurization-options) <br><br> Sono state rilevate funzionalità di cardinalità elevata negli input ed è stata gestita.
Gestione suddivisione della convalida |**Operazione completata**| *La configurazione della convalida è stata impostata su' auto ' e i dati di training contengono **meno** di 20.000 righe.* <br> Ogni iterazione del modello sottoposto a training è stata convalidata tramite la convalida incrociata. Altre informazioni sui [dati di convalida.](https://docs.microsoft.com/azure/machine-learning/how-to-configure-auto-train#train-and-validation-data) <br><br> *La configurazione della convalida è stata impostata su' auto ' e i dati di training contengono **più** di 20.000 righe.* <br> I dati di input sono stati suddivisi in un set di dati di training e un set di dati di convalida per la convalida del modello.
Rilevamento del bilanciamento della classe |**Passed** <br><br><br><br> **Avvisi** | Gli input sono stati analizzati e tutte le classi sono bilanciate nei dati di training. Un set di dati è considerato bilanciato se ogni classe presenta una rappresentazione corretta del set di dati, misurata in base al numero e al rapporto degli esempi. <br><br><br> Sono state rilevate classi sbilanciate negli input. Per correggere la distorsione del modello, risolvere il problema di bilanciamento del carico. Altre informazioni sui [dati sbilanciati.](https://docs.microsoft.com/azure/machine-learning/concept-manage-ml-pitfalls#identify-models-with-imbalanced-data)
Rilevamento dei problemi di memoria |**Passed** <br><br><br><br> **Operazione completata** |<br> Sono stati analizzati i valori di {Horizon, lag, finestra in sequenza, ovvero la finestra in sequenza, ma non sono stati rilevati potenziali problemi di memoria insufficiente. Altre informazioni sulle [configurazioni di previsione](https://docs.microsoft.com/azure/machine-learning/how-to-auto-train-forecast#configure-and-run-experiment) delle serie temporali. <br><br><br>Sono stati analizzati i valori di {Horizon, lag, Window in sequenza} selezionati che potrebbero causare un esaurimento della memoria da parte dell'esperimento. Le configurazioni del ritardo o della finestra in sequenza sono state disattivate.
...
[![Dashboard Dettagli esecuzione](media/how-to-use-automated-ml-for-ml-models/run-details.png)](media/how-to-use-automated-ml-for-ml-models/run-details-expanded.png#lightbox)
