...
In questo documento vengono illustrati vari metodi di invio di query Hive ai cluster Hadoop gestiti da un servizio HDInsight in Azure. Le query Hive possono essere inviate utilizzando: 
...
Vengono fornite query Hive generiche che possono essere utilizzate per esplorare i dati o per generare funzionalità che usano le funzioni definite dall'utente di Hive (UDF). 

Esempi di query specifiche per scenari relativi ai [dati delle corse dei taxi di NYC](http://chriswhong.com/open-data/foil_nyc_taxi/) vengono inoltre forniti nell'[archivio di Github](https://github.com/Azure/Azure-MachineLearning-DataScience/tree/master/Misc/DataScienceProcess/DataScienceScripts). Tali query presentano già schemi di dati specifici e sono pronte per essere inviate per l'esecuzione. 
...
- [Argomenti avanzati: Ottimizzazione dei parametri Hive per migliorare la velocità delle query](#tuning)
...
####Invio di query Hive direttamente nella riga di comando di Hadoop. 

Gli utenti possono eseguire comandi come `hive -e "<your hive query>;` per inviare query Hive semplici direttamente nella riga di comando di Hadoop. Ecco un esempio, in cui nella casella rossa viene indicato il comando che invia query Hive e nella casella verde viene indicato l'output della query Hive.

![Create workspace][10]

####Inviare query nei file hql

Quando la query Hive è più complicata e presenta più righe, modificare le query nella riga di comando o nella console dei comandi di Hive non è pratico. Un'alternativa consiste nell'utilizzare un editor di testo nel nodo head del cluster Hadoop per salvare le query Hive in un file .hql in una directory locale del nodo head. Quindi la query Hive nel file hql può essere inviata utilizzando l'argomento `-f` nel modo seguente:
...
Per impostazione predefinita, dopo l'invio della query Hive nella riga di comando di Hadoop, lo stato di avanzamento del processo di mapping e riduzione verrà stampato sullo schermo. Per eliminare la stampa della schermata di avanzamento del processo di mapping e riduzione, è possibile utilizzare un argomento `-S` ("S" in lettere maiuscole) nella riga di comando nel seguente modo:
...
Gli utenti possono innanzitutto immettere la console dei comandi di Hive eseguendo il comando  `hive` nella riga di comando di Hadoop, quindi inviare query Hive nella console dei comandi di Hive. Di seguito è fornito un esempio. In questo esempio, le due caselle di colore rosse evidenziano rispettivamente i comandi utilizzati per immettere la console dei comandi di Hive e la query Hive inviata nella console dei comandi di Hive. La casella verde evidenzia l'output della query Hive. 

![Create workspace][11]

Negli esempi precedenti l'output della query Hive veniva visualizzato direttamente sullo schermo. Gli utenti possono inoltre scrivere l'output in un file locale nel nodo head o in un BLOB di Azure. Quindi, gli utenti possono utilizzare altri strumenti per analizzare ulteriormente l'output delle query Hive.
...
Gli utenti possono inoltre restituire i risultati delle query Hive in un BLOB di Azure, all'interno del contenitore predefinito del cluster di Hadoop. La query Hive deve avere il seguente aspetto:

	`insert overwrite directory wasb:///<directory within the default container> <select clause from ...>`

Nell'esempio seguente, l'output della query Hive viene scritto in una directory di BLOB  `queryoutputdir` all'interno del contenitore predefinito del cluster Hadoop. In questo caso, è sufficiente fornire il nome della directory, senza il nome del BLOB. Verrà generato un errore se vengono specificati entrambi i nomi della directory e del BLOB, ad esempio  `wasb:///queryoutputdir/queryoutput.txt`. 

![Create workspace][13]

Se si apre il contenitore predefinito del cluster Hadoop utilizzando strumenti come Esplora archivi di Azure, verrà visualizzato l'output della query Hive come indicato di seguito. È possibile applicare il filtro (evidenziato da una casella rossa) per recuperare solo il BLOB con le lettere specificate nei nomi.

![Create workspace][14]

###2. Tramite l'Editor Hive o i comandi di Azure PowerShell

Gli utenti possono inoltre utilizzare Console Query (Editor Hive) immettendo l'URL in un browser Web `https://<Hadoop cluster name>.azurehdinsight.net/Home/HiveEditor` (verrà richiesto di immettere le credenziali del cluster Hadoop per accedere,) o [inviare processi Hive tramite PowerShell](../hdinsight/hdinsight-submit-hadoop-jobs-programmatically.md#hive-powershell). 

## <a name="explore"></a>Esplorazione dei dati, progettazione delle funzionalità e ottimizzazione dei parametri Hive

I seguenti dati vengono descritti nella seguente sezione utilizzando Hive nei cluster Hadoop di Azure HDInsight e in un argomento più avanzato sull'ottimizzazione di alcuni parametri Hive per migliorare le prestazioni delle query Hive:
...
> [AZURE.NOTE]
> Dopo aver creato le funzionalità aggiuntive, è possibile aggiungerle come colonne alla tabella esistente o creare una nuova tabella con le funzionalità aggiuntive e la chiave primaria, che possono essere unite con la tabella originale.  
...
Talvolta è utile per calcolare le frequenze dei livelli di una variabile di categoria o le frequenze di combinazioni di livelli di più variabili di categoria. Gli utenti possono utilizzare lo script seguente per calcolare le frequenze:
...
Dopo che viene calcolata la tabella di rischio, gli utenti possono assegnare i valori di rischio a una tabella unendoli alla tabella di rischio. Il join della query Hive è stato descritto nella sezione precedente.
...
In questa query, se il `<datetime field>` ha un modello simile a `03/26/2015 12:04:39`, il `'<pattern of the datetime field>'` dovrebbe essere `"MM/gg/aaaa HH:mm:ss"`. Per eseguire il test, gli utenti possono eseguire
...
####<a name="hive-gpsdistance"></a>Calcolo della distanza tra le coordinate del GPS
...
I campi che vengono utilizzati in questa query sono coordinate GPS relative ai luoghi in cui si sale e scende dal taxi, denominati pickup_longitude, pickup_latitude, dropoff_longitude e dropoff_latitude. Le query per calcolare la distanza diretta tra le coordinate dei luoghi in cui si sale e scende sono:
...
Le equazioni matematiche per calcolare la distanza tra due coordinate GPS si trovano [qui](http://www.movable-type.co.uk/scripts/latlong.html), autore Peter Lapisu. Nel suo Javascript, la funzione toRad() è semplicemente  `lat_or_lon*pi/180`, which converts degrees to radians. Here, `lat_or_lon` è la latitudine o longitudine. Poiché Hive non fornisce la funzione  `atan2`, but provides function `atan`, la funzione  `atan2` function is implemented by `atan` nella query Hive precedente, è basata sulla definizione in [Wikipedia](http://en.wikipedia.org/wiki/Atan2). 
...
Le impostazioni predefinite dei parametri del cluster Hive potrebbero non essere adatte per le query Hive e per i dati che le query elaborano. In questa sezione verranno esaminati alcuni parametri che gli utenti possono ottimizzare per migliorare le prestazioni delle query Hive. Gli utenti devono aggiungere le query di ottimizzazione dei parametri prima delle query di elaborazione dei dati. 

1. Spazio heap di Java: Per query che prevedono il join di grandi set di dati o l'elaborazione di lunghi record, un errore tipico è l'**esaurimento dello spazio heap**. Questo può essere ottimizzato impostando i parametri  `mapreduce.map.java.opts` e  `mapreduce.task.io.sort.mb` sui valori desiderati. Di seguito è fornito un esempio:

		set mapreduce.map.java.opts=-Xmx4096m;
		set mapreduce.task.io.sort.mb=-Xmx1024m;
	

	Questo parametro consente di assegnare la memoria di 4 GB di spazio all'heap Java e rende l'ordinamento più efficiente assegnandogli più memoria. È consigliabile provarlo per vedere se si verificano errori relativi alla non riuscita dei processi correlati allo spazio heap.

2. Dimensione del blocco DFS: Questo parametro consente di impostare la più piccola unità di dati che viene archiviata nel file system. Ad esempio, se la dimensione del blocco è pari a 128 MB, allora qualsiasi dato con dimensioni inferiori o pari a 128 MB viene archiviato in un unico blocco, mentre ai dati più grandi di 128 MB vengono assegnati blocchi aggiuntivi. La scelta di una dimensione del blocco molto piccola può comportare sovraccarichi di grandi dimensioni in Hadoop poiché il nodo del nome deve elaborare molte più richieste per trovare il blocco rilevante riguardante il file. Un'impostazione consigliata per dati con dimensioni in gigabyte (o più grandi) è:
...
4. Specifica del numero di mapper in Hive: Mentre Hadoop consente all'utente di impostare il numero di riduttori, il numero di mapper generalmente non viene impostato dall'utente. Un espediente che offre un certo livello di controllo su questo numero consiste nello scegliere le variabili di hadoop, mapred.min.split.size e mapred.max.split.size. Le dimensioni di ogni attività di mapping sono determinate da:
	...
	Generalmente, il valore predefinito di mapred.min.split.size è 0, quello di mapred.max.split.size è Long.MAX e quello di dfs.block.size è 64 MB. Come si può vedere, date le dimensioni dei dati, ottimizzare questi parametri "impostandoli" consente di ottimizzare il numero di mapper utilizzati. 

5. Alcune altre opzioni più avanzate per l'ottimizzazione delle prestazioni di Hive sono riportate di seguito. Consentono l'impostazione della memoria destinata al mapping e riducono le attività e possono essere utili per regolare le prestazioni. Tenere presente che  `mapreduce.reduce.memory.mb` non può essere superiore della dimensione della memoria fisica di ogni nodo di lavoro nel cluster Hadoop.
