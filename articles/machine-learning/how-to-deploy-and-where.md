...
La `AMLRequest` classe consente solo di accedere ai dati non elaborati registrati nel score.py, non è presente alcun componente lato client. Da un client, si pubblicano i dati come di consueto. Ad esempio, il codice Python seguente legge un file di immagine e invia i dati:For example, the following Python code reads an image file and posts the data:
