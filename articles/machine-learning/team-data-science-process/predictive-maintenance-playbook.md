...
- Al livello successivo, alcune aziende applicano la _manutenzione preventiva_, in cui viene determinato il ciclo di vita utile per una parte, in modo da eseguire la manutenzione o la sostituzione prima che si verifichi un guasto. La manutenzione preventiva consente di evitare i problemi non pianificati e di grave entità. Tuttavia, i costi elevati dei tempi di inattività programmati, del sottoutilizzo del componente durante la durata utile e della manodopera rimangono.
...
|:---------|---------------------------|
|_Ritardi e cancellazioni dei voli_ | Informazioni sulle rotte dei voli sotto forma di tratte dei voli e log di manutenzione. I dati delle leg di volo includono dettagli di partenza/arrivo, ora, aeroporto, soste ecc. Il registro delle pagine include una serie di codici di errore e di manutenzione registrati dal personale addetto alla manutenzione a terra.|
...
|_Guasto di uno sportello bancomat_ | Letture dei sensori per ogni transazione (deposito di contante/assegni) ed erogazione di contante. Informazioni sulla misurazione dello spazio tra note, spessore della nota, distanza di arrivo della nota, attributi di controllo ecc. Record di manutenzione che forniscono codici di errore, informazioni di riparazione, l'ultima volta che il distributore di contanti è stato riempito.|
|_Guasti di turbine eoliche_ | I sensori monitorano le condizioni della turbina come la temperatura, la direzione del vento, l'energia generata, la velocità del generatore ecc. I dati vengono raccolti da più turbine eoliche provenienti da parchi eolici situati in varie regioni. In genere, per ogni turbina verranno inoltrate le misurazioni di più letture a un intervallo di tempo prestabilito.|
|_Guasti di interruttori automatici_ | Log di manutenzione che includono le azioni correttive, preventive e sistematiche. Dati operativi che includono i comandi automatici e manuali inviati agli interruttori, ad esempio per le azioni di apertura e chiusura. Metadati del dispositivo come la data di fabbricazione, la posizione, il modello e così via. Specifiche dell'interruttore di circuito come livelli di tensione, geolocalizzazione, condizioni ambientali.|
...
|_Guasti delle ruote_ | Dati del sensore che misurano l'accelerazione della ruota, le istanze di frenata, la distanza di guida, la velocità ecc. Informazioni statiche su ruote come produttore, data prodotta. Dati sui guasti dedotti dal database degli ordini di parti, che tiene traccia delle date e delle quantità degli ordini.|
...
- _Guasto della parte del motore dell'aeromobile_: mezzi di rotolamento, deviazione standard e somma nell'ultimo giorno, settimana ecc. Questa metrica deve essere determinata insieme all'esperto di domini aziendali.
...
Per ogni registrazione etichettata di un bene, viene definita una finestra di dimensioni _W-<sub>k,</sub> _ dove _k_ è il numero di finestre di dimensione _W_. Le aggregazioni vengono quindi create su _k_ _finestre a cascata_ _W-k, W-<sub>(k-1)</sub>, ..., W-<sub>2</sub>, W-<sub>1</sub> _ per i periodi prima del timestamp di un record. _k_ può essere un numero ridotto per acquisire gli effetti a breve termine o un numero elevato per acquisire schemi di degradazione a lungo termine (vedere la figura 2).
...
> - Dong, G. Liu, H. (Editors), Feature Engineering per Machine Learning e Data Analytics (Chapman & Hall/CRC Data Mining and Knowledge Discovery Series), CRC Press, 2018.
...
La classificazione binaria viene utilizzata per _prevedere la probabilità che un'apparecchiatura fallisca entro un periodo_ di tempo futuro - chiamato il futuro periodo _dell'orizzonte X_. X è determinato dal problema aziendale e dai dati a portata di mano, in consultazione con l'esperto di dominio. Alcuni esempi:
...
- _Ritardi dei voli_: X può essere scelto come un giorno, per prevedere i ritardi nelle prossime 24 ore. A tutti i voli entro 24 ore prima dei guasti viene assegnata l'etichetta 1.
...
![Figura 5. Etichette di previsione del tempo di errore per la classificazione multiclasse](./media/predictive-maintenance-playbook/labelling-for-multiclass-classification-for-failure-time-prediction.png)
...
![Figura 6. Etichette di stima della causa principale per la classificazione multiclasse](./media/predictive-maintenance-playbook/labelling-for-multiclass-classification-for-root-cause-prediction.png)
...
Si supponga di avere un flusso di eventi con timestamp, ad esempio le misurazioni da diversi sensori. Definire le funzioni e le etichette degli esempi di training e test in base a intervalli di tempo che contengono più eventi. Ad esempio, per la classificazione binaria, creare feature basate su eventi passati e creare etichette basate su eventi futuri all'interno di unità di tempo "X" in futuro (vedere le sezioni sulle tecniche di [progettazione](#feature-engineering) e modellazione delle entità geografiche). Quindi, l'intervallo di tempo dell'assegnazione di etichette è successivo all'intervallo di tempo delle relative funzioni.
...
| # | Titolo | Descrizione |
|--:|:------|-------------|
| 2 | [Modello di soluzione per la manutenzione predittiva di Azure](https://github.com/Azure/AI-PredictiveMaintenance) | Modello di soluzione open source che illustra la modellazione di Azure ML e un'infrastruttura completa di Azure in grado di supportare scenari di manutenzione predittiva nel contesto del monitoraggio remoto IoT.An open-source solution template that demonstrates Azure ML modeling and a complete Azure infrastructure capable to support Predictive Maintenance scenarios in the context of IoT remote monitoring. |
...
| 6 | [Azure AI Toolkit per IoT Edge](https://github.com/Azure/ai-toolkit-iot-edge) | AI nel bordo IoT utilizzando TensorFlow; Toolkit crea pacchetti di modelli di deep learning in contenitori Docker compatibili con Azure IoT Edge ed espone tali modelli come API REST.
