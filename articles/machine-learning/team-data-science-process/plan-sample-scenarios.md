...
**L'albero delle decisioni** per la selezione degli scenari di esempio appropriati per i dati e l'obiettivo viene presentato nell'ultima sezione.
...
> <br/>
> 
> * [Creare un account di archiviazioneCreate a storage account](../../storage/common/storage-account-create.md)
>   <br/>
> * [Creare un'area di lavoro di Azure Machine LearningCreate an Azure Machine Learning workspace](../studio/create-workspace.md)
> 
> 

## <a name="scenario-1-small-to-medium-tabular-dataset-in-local-files"></a><a name="smalllocal"></a>Scenario \#1: set di dati tabulari da piccolo e medio nei file localiScenario 1: Small to medium tabular dataset in local files
...
1. Caricare i dati in un contenitore di Archiviazione di Azure.Upload data to an Azure Storage container.
1. Pre-elaborare e pulire i dati in IPython Notebook, l'accesso ai dati dal contenitore di Archiviazione di Azure.Pre-process and clean data in IPython Notebook, accessing data from Azure Storage container.
...
1. Caricare i dati in un contenitore di Archiviazione di Azure.Upload data to an Azure Storage container.
1. Pre-elaborare e pulire i dati nel contenitore di Archiviazione di Azure usando IPython Notebook.Pre-process and clean data in Azure Storage container using IPython Notebook.
1. Trasformare i dati in un formato tabulare pulito, se necessario.
...
1. Caricare i dati nel database SQL Server in esecuzione in una VM di Azure.
   ...
   * Accedere alla macchina virtuale di SQL ServerLog in to SQL Server VM
   ...
   * Creare tabelle di database e di destinazione
   * Utilizzare uno dei metodi di importazione globale per caricare i dati dai file della macchina virtuale locale.
1. Esaminare i dati e creare le funzionalità necessarie. Non è necessario materializzare le feature nelle tabelle di database. Notare solo la query necessaria per crearle.
1. Scegliere le dimensioni del campione di dati, se necessario e/o lo si desidera.
1. Accedere a [Azure Machine Learning Studio](https://studio.azureml.net/).
1. Leggere i dati direttamente da SQL Server usando il modulo [Import Data][import-data] (Importa dati). Incollare la query necessaria che estrae i campi, crea feature e campiona i dati se necessario direttamente nella query [Importa dati.][import-data]
1. Compilare un flusso di esperimento di Azure Machine Learning iniziando con il set di dati inserito.

## <a name="scenario-5-large-dataset-in-local-files-target-sql-server-in-azure-vm"></a><a name="largelocaltodb"></a>Scenario \#5: Set di dati di grandi dimensioni nei file locali, SQL Server di destinazione nella macchina virtuale di AzureScenario 5: Large dataset in local files, target SQL Server in Azure VM
...
1. Caricare i dati nel database SQL Server in esecuzione in una VM di Azure.
    ...
    a.  Accedere alla macchina virtuale di SQL Server.Log in to SQL Server VM.
   
    b.  Se i dati non sono già stati salvati, scaricare i file di dati dal contenitore di archiviazione di Azure nella cartella della macchina virtuale locale.
...
## <a name="scenario-6-large-dataset-in-a-sql-server-database-on-premises-targeting-sql-server-in-an-azure-virtual-machine"></a><a name="largedbtodb"></a>Scenario \#6: Set di dati di grandi dimensioni in un database di SQL Server in locale, destinato a SQL Server in una macchina virtuale di AzureScenario 6: Large dataset in a SQL Server database on premises, targeting SQL Server in an Azure Virtual Machine
...
1. Usare uno dei metodi di esportazione dei dati per esportare i dati da SQL Server in file di dump.
   ...
   > [!NOTE]
   > Se si decide di spostare tutti i dati dal database locale, un metodo alternativo (più veloce) per spostare il database completo nell'istanza di SQL Server in Azure.If you decide to move all data from the on premises database, an alternate (faster) method to move the full database to the SQL Server instance in Azure. Ignorare i passaggi per esportare i dati, creare il database e caricare/importare i dati nel database di destinazione e seguire il metodo alternativo.
   > 
   > 
1. Caricare i file di dump nel contenitore di Archiviazione di Azure.Upload dump files to Azure Storage container.
1. Caricare i dati in un database di SQL Server in esecuzione in una macchina virtuale di Azure.
   ...
   b.  Scaricare i file di dati da un contenitore di Archiviazione di Azure nella cartella della macchina virtuale locale.
...
#### <a name="additional-azure-resources-azure-virtual-machine-sql-server--ipython-notebook-server"></a>Risorse di Azure aggiuntive: macchina virtuale di Azure (SQL Server/server IPython Notebook)
Per replicare l'intero database SQL Server nella macchina virtuale di SQL Server, è necessario copiare un database da un percorso/server a un altro, supponendo che il database possa essere portato temporaneamente offline. È possibile utilizzare Esplora oggetti di SQL Server Management StudioSQL Server Management Studio o i comandi Transact-SQLTransact-SQL equivalenti.

1. Scollegare il database nel percorso di origine. Per ulteriori informazioni, vedere [Scollegare un database](https://technet.microsoft.com/library/ms191491\(v=sql.110\).aspx).
...
1. Caricare i dati nel database di Hive del cluster Hadoop di HDInsight di Azure.
   ...
   > 
   > 
1. Esaminare i dati e creare le funzionalità necessarie nella riga di comando di Hadoop. Non è necessario materializzare le feature nelle tabelle di database. Notare solo la query necessaria per crearle.
...
1. Accedere a [Azure Machine Learning Studio](https://studio.azureml.net/).
1. Leggere i dati direttamente da `Hive Queries` usando il modulo [Import Data][import-data] (Importa dati). Incollare la query necessaria che estrae i campi, crea feature e campiona i dati se necessario direttamente nella query [Importa dati.][import-data]
1. Semplificare il flusso di esperimento di Azure Machine Learning iniziando con il set di dati caricato.

## <a name="decision-tree-for-scenario-selection"></a><a name="decisiontree"></a>Albero delle decisioni per la scelta degli scenari
---
Nel diagramma seguente sono riepilogati gli scenari descritti in precedenza e il processo di analisi avanzata e le scelte di tecnologia effettuate che conducono l’utente a ciascuno degli scenari elencati. L'elaborazione, l'esplorazione, la progettazione delle funzionalità e il campionamento dei dati possono essere eseguiti in uno o più metodi/ambiente, negli ambienti di origine, intermedio e/o di destinazione, e possono procedere in modo iterativo in base alle esigenze. Il diagramma è solo un'illustrazione di alcuni dei possibili flussi e non fornisce un'enumerazione completa.
