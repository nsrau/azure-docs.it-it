...
description: Informazioni su come spostare i dati da o verso Archiviazione BLOB di Azure usando SQL Server Integration Services Integration Services Feature Pack per Azure.Learn how to move Data to or from Azure Blob Storage using SQL Server Integration Services Feature Pack for Azure.
...
Dopo aver spostato i dati locali nel cloud, i clienti possono accedere ai dati da qualsiasi servizio di Azure per sfruttare tutta la potenza della suite di tecnologie di Azure.Once customers have moved on-premises data into the cloud, they can access their data from any Azure service to leverage the all power of the suite of Azure technologies. I dati possono essere usati successivamente, ad esempio, in Azure Machine Learning o in un cluster HDInsight.The data may be subsequently used, for example, in Azure Machine Learning or on an HDInsight cluster.

Esempi per l'uso di queste risorse di Azure sono nelle procedure dettagliate di SQL e [HDInsight.Examples](hive-walkthrough.md) for using these Azure resources are in the [SQL](sql-walkthrough.md) and HDInsight walkthroughs.
...
Per eseguire le attività descritte in questo articolo, è necessario disporre di una sottoscrizione di Azure e di un account di Archiviazione di Azure.To perform the tasks described in this article, you must have an Azure subscription and an Azure Storage account set up. Per caricare o scaricare i dati sono necessari il nome e la chiave dell'account di Archiviazione di Azure.You need the Azure Storage account name and account key to upload or download data.
