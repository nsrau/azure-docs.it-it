...
description: Informazioni su come eseguire la formazione distribuita e a nodo singolo dei modelli tradizionali di apprendimento automatico e deep learning usando la classe Azure Machine Learning Estimator
ms.author: maxluk
author: maxluk
...
Con Azure Machine Learning è possibile inviare facilmente lo script di training a [varie destinazioni](how-to-set-up-training-targets.md#compute-targets-for-training)di calcolo usando un [oggetto RunConfiguration](how-to-set-up-training-targets.md#whats-a-run-configuration) e un [oggetto ScriptRunConfig](how-to-set-up-training-targets.md#submit). Questo modello offre una notevole della flessibilità e il massimo controllo.

Per semplificare il training dei modelli di Deep Learning, Python SDK di Azure Machine Learning fornisce un'astrazione alternativa di più alto livello, la classe Estimator, che consente agli utenti di creare con facilità configurazioni di esecuzione. È possibile creare e usare uno strumento stimato generico per inviare uno script di formazione usando qualsiasi framework di apprendimento scelto (ad esempio scikit-learn) su qualsiasi destinazione di calcolo scelta, sia che si tratti del computer locale, di una singola macchina virtuale in Azure o di un cluster GPU in Azure.You can create and use a generic [Estimator](https://docs.microsoft.com/python/api/azureml-train-core/azureml.train.estimator?view=azure-ml-py) to submit training script using any learning framework you choose (such as scikit-learn) on any compute target you choose, whether it's your local machine, a single VM in Azure, or a GPU cluster in Azure. Per le attività PyTorch, TensorFlow e Chainer, Azure Machine Learning fornisce anche gli estimatori [PyTorch](https://docs.microsoft.com/python/api/azureml-train-core/azureml.train.dnn.pytorch?view=azure-ml-py), [TensorFlow](https://docs.microsoft.com/python/api/azureml-train-core/azureml.train.dnn.tensorflow?view=azure-ml-py)e [Chainer](https://docs.microsoft.com/python/api/azureml-train-core/azureml.train.dnn.chainer?view=azure-ml-py) per semplificare l'utilizzo di questi framework.
...
Usare un `Estimator` per un training su nodo singolo su risorse di calcolo remote in Azure per un modello scikit-learn. È necessario aver già creato `compute_target` l'oggetto `ds`di destinazione del [calcolo](how-to-set-up-training-targets.md#amlcompute) e l'oggetto [FileDataset](how-to-create-register-datasets.md) .
...
--|--
`source_directory`| Directory locale contenente tutto il codice necessario per il processo di training. Questa cartella viene copiata dal computer locale al computer remoto.
`script_params`| Dizionario che specifica gli argomenti della riga `entry_script`di comando `<command-line argument, value>` da passare allo script di training, sotto forma di coppie. Per specificare un flag `script_params`dettagliato `<command-line argument, "">`in , utilizzare .
`compute_target`| Destinazione di calcolo remoto in cui verrà eseguito lo script di training, in questo caso un cluster Azure Machine Learning Compute ([AmlCompute](how-to-set-up-training-targets.md#amlcompute)). Si noti anche se il cluster AmlCompute è la destinazione comunemente usata, è anche possibile scegliere altri tipi di destinazione di calcolo, ad esempio macchine virtuali di Azure o persino computer locale.
`entry_script`| Percorso file (relativo a `source_directory`) dello script di training da eseguire nelle risorse di calcolo remote. Questo file, e tutti i file aggiuntivi da cui dipende, dovrebbe trovarsi in questa cartella.
`conda_packages`| Elenco dei pacchetti Python da installare tramite Conda, necessari per lo script di training.  

Il costruttore dispone `pip_packages` di un altro parametro denominato che si utilizza per tutti i pacchetti pip necessari.
...
--|--|--
`custom_docker_image`| Nome dell'immagine da usare. Specificare solo immagini disponibili in repository Docker pubblici (in questo caso Hub Docker). Per usare un'immagine da un repository Docker privato, usare invece il parametro `environment_definition` del costruttore. [Vedere esempio](https://github.com/Azure/MachineLearningNotebooks/blob/master/how-to-use-azureml/training-with-deep-learning/how-to-use-estimator/how-to-use-estimator.ipynb). | `None`
...
`distributed_training`| [Oggetto MPIConfiguration](https://docs.microsoft.com/python/api/azureml-core/azureml.core.runconfig.mpiconfiguration?view=azure-ml-py) per l'avvio del training distribuito tramite back-end MPI.  | `None`
...
L'esecuzione del codice seguente registrerà il modello nell'area di lavoro e lo renderà disponibile per fare riferimento in base al nome nei contesti di calcolo remoti o negli script di distribuzione. Vedere [`register_model`](https://docs.microsoft.com/python/api/azureml-core/azureml.core.run.run?view=azure-ml-py#register-model-model-name--model-path-none--tags-none--properties-none--model-framework-none--model-framework-version-none--description-none--datasets-none--sample-input-dataset-none--sample-output-dataset-none--resource-configuration-none----kwargs-) nella documentazione di riferimento per ulteriori informazioni e parametri aggiuntivi.
...
Per un blocco appunti che mostra le nozioni di base di un modello di stima, vedere:For a notebook that shows the basics of an estimator pattern, see:
* [how-to-use-azureml/training-with-deep-learning/how-to-use-estimator](https://github.com/Azure/MachineLearningNotebooks/blob/master/how-to-use-azureml/training-with-deep-learning/how-to-use-estimator/how-to-use-estimator.ipynb)

Per un notebook che estrae un modello scikit-learn usando estimator, vedere:
* [tutorials/img-classification-part1-training.ipynb](https://github.com/Azure/MachineLearningNotebooks/blob/master/tutorials/img-classification-part1-training.ipynb)

Per i notebook sui modelli di training utilizzando gli esstimatori specifici del framework di apprendimento profondo, vedere:
...
* [Creare e gestire ambienti per la formazione e la distribuzione](how-to-use-environments.md)