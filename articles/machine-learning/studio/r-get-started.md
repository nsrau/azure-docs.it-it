...
description: Usare questa esercitazione sulla programmazione R per iniziare a usare il linguaggio R con Azure Machine Learning Studio per creare una soluzione di previsione.
...
Questa esercitazione illustra come si inizia a estendere Azure Machine Learning Studio usando il linguaggio di programmazione R. Seguire questa esercitazione sulla programmazione R per creare, testare ed eseguire il codice R in Studio. Usando questa esercitazione, sarà possibile creare una soluzione completa di previsione con il linguaggio R in Studio.  

Microsoft Azure Machine Learning Studio contiene molti moduli validi per Machine Learning e la manipolazione dei dati. Il potente linguaggio R è stato definito come la lingua franca dell'analisi. Fortunatamente le analisi e la manipolazione dei dati in Studio possono essere estese usando R. Questa combinazione fornisce la scalabilità e la facilità di distribuzione di Studio insieme alla flessibilità e l'analisi approfondita di R.
...
In questa guida è verranno utilizzate con produzione casearia in California e dei prezzi dei dati. Questi dati includono informazioni mensili sulla produzione di diversi prodotti caseari e il prezzo del latte intero, un materia prima di riferimento.

I dati usati in questo articolo, insieme agli script R, possono essere scaricati dal [repository MachineLearningSamples-Notebooks/studio-samples](https://github.com/Azure-Samples/MachineLearningSamples-Notebooks/tree/master/studio-samples). I dati nel file `cadairydata.csv` sono stati originariamente sintetizzati da informazioni pubblicate dalla University of Wisconsin all'indirizzo [ https://dairymarkets.com ](https://dairymarkets.com).

### <a name="organization"></a>Organizzazione

La procedura sarà formata da diversi passaggi che consentiranno di apprendere come creare, testare ed eseguire il codice R per le analisi e la manipolazione dei dati nell'ambiente di Azure Machine Learning Studio.  
...
* Quindi, verranno discussi diversi aspetti relativi all'I/O dei dati, del codice R e dei grafici nell'ambiente di Azure Machine Learning Studio.
...
Questa sezione illustra alcune nozioni di base relative all'interazione con il linguaggio di programmazione R nell'ambiente di Machine Learning Studio. Il linguaggio R fornisce uno strumento potente per creare moduli personalizzati di analisi e manipolazione dei dati nell'ambiente di Azure Machine Learning Studio.
...
In Machine Learning Studio gli script R vengono eseguiti nel modulo [Execute R Script][execute-r-script]. Un esempio del modulo [Execute R Script][execute-r-script] in Machine Learning Studio è mostrato nella figura 1.
 ...
 ![Linguaggio di programmazione R: il modulo Execute R Script selezionato in Machine Learning Studio](./media/r-quickstart/fig1.png)
...
* La parte inferiore del riquadro di destra mostra alcune proprietà di [Execute R Script][execute-r-script]. È possibile visualizzare i log di output e degli errori selezionando le rispettive opzioni di questo riquadro.

Naturalmente, discuteremo il [Execute R Script] [ execute-r-script] più dettagliatamente nel resto di questo articolo.
...
Qualsiasi codice R nel [Execute R Script] [ execute-r-script] modulo verrà eseguito quando si esegue l'esperimento, selezionare il **eseguire** pulsante. Al termine dell'esecuzione, accanto all'icona [Execute R Script][execute-r-script] verrà visualizzato un segno di spunta.

#### <a name="defensive-r-coding-for-azure-machine-learning"></a>Codice R difensivo per Azure Machine Learning

Se si sviluppa un codice R, ad esempio per un servizio Web, usando Azure Machine Learning Studio, è necessario pianificare in che modo il codice R gestirà gli input di dati imprevisti e le eccezioni. Per maggiore chiarezza, per gli esempi di codice riportati in questa guida non tratterò in modo approfondito le modalità di verifica e gestione delle eccezioni. Tuttavia, verranno forniti diversi esempi di funzioni che prevedono l'uso della capacità di gestione delle eccezioni di R.  

Se è necessaria un'analisi più completa di gestione delle eccezioni di R, è consigliabile leggere le sezioni rilevanti del libro di Wickham elencate di seguito nel [altre informazioni, leggere](#appendixb).
...
L'output dell'esecuzione del codice R e della piattaforma di Azure Machine Learning Studio è disponibile principalmente in output.log, ma alcune informazioni aggiuntive possono essere presenti in error.log.  

Se si verifica un errore in Machine Learning Studio mentre si esegue il codice R, è necessario innanzitutto analizzare error.log. Questo file può contenere messaggi di errore utili a comprendere e correggere l'errore. Per visualizzare Error, selezionare **Visualizza log degli errori** nel **riquadro proprietà** per il [Execute R Script] [ execute-r-script] che contiene l'errore.
...
L'esecuzione del codice non riesce, generando così una condizione di errore. Selezionando **Visualizza log degli errori** nel **riquadro proprietà** viene visualizzato quanto mostrato nella figura 2.
...
Per visualizzare il messaggio di errore di R, è necessario usare output.log. Selezionare il [Execute R Script] [ execute-r-script] e quindi selezionare il **View output. log** elemento sul **riquadro proprietà** a destra. Viene aperta una nuova finestra del browser e viene visualizzato quanto segue.
...
Se al momento non si riesce a comprendere l'ultima riga del codice, continuare a leggere. Nella parte restante di questo articolo vengono discussi diversi usando R nell'ambiente di Studio.

### <a name="introduction-to-rstudio"></a>Introduzione a RStudio

RStudio è un diffuso IDE per R. Utilizzerò RStudio per la modifica, test e debug di alcuni codici R usati in questa Guida. Dopo il test, quando il codice R è pronto, è possibile tagliarlo e incollarlo dall'editor di RStudio a un modulo [Execute R Script][execute-r-script] di Machine Learning Studio.  
...
Introduzione all'esercitazione su RStudio è disponibile all'indirizzo [usando l'IDE RStudio](https://support.rstudio.com/hc/sections/200107586-Using-RStudio).

Fornire informazioni aggiuntive sull'uso di RStudio nel [Guida alla documentazione RStudio](#appendixa) sotto.  
...
Il codice completo per questa sezione è [repository MachineLearningSamples-Notebooks/studio-samples](https://github.com/Azure-Samples/MachineLearningSamples-Notebooks/tree/master/studio-samples).
...
1. Avviare l'ambiente di Azure Machine Learning Studio.
1. Selezionare **+ nuovo** nell'angolo inferiore sinistro dello schermo e selezionare **Dataset**.
...
1. Dopo aver caricato il set di dati, si dovrebbe essere il nuovo set di dati selezionando il **set di dati** scheda.  
...
Diamo ora un'occhiata ai dati caricati nell'esperimento. Nell'esperimento, selezionare l'output del **set di dati cadairydata** e selezionare **visualizzare**. Dovrebbe apparire una tabella simile a quella riportata nella Figura 4.  
...
> [!NOTE]
> Azure Machine Learning Studio gestisce i file nel pacchetto con estensione zip come se fossero nella directory src/, quindi è necessario inserire questo nome della directory come prefisso nei nomi file. Ad esempio, se il file con estensione zip contiene i file `yourfile.R` e `yourData.rdata` nella radice del file con estensione zip, è necessario risolvere questi come `src/yourfile.R` e `src/yourData.rdata` quando si usano `source` e `load`.

Abbiamo già discusso i set di dati durante il caricamento nel [caricare il set di dati](#loading). Dopo aver creato e testato lo script R mostrato nella sezione precedente, procedere come segue:
...
1. Creare un file con estensione zip e copiare al suo interno lo script creato. In Windows, è possibile fare doppio clic su file e selezionare **inviare**e quindi **cartella compressa**. Verrà creato un nuovo file con estensione zip contenente il file "simpleplot.R".

1. Aggiungere il file ai **set di dati** in Machine Learning Studio, specificando il tipo **zip**. Dovrebbe essere visualizzato il file con estensione zip nei set di dati.

1. Trascinare il file ZIP dai **set di dati** all'**area di disegno di ML Studio**.

1. Connettere l'output dell'icona dei **dati zip** all'input **Script Bundle** del modulo [Execute R Script][execute-r-script].

1. Digitare la funzione `source()` con il nome del file con estensione zip nella finestra del codice per il modulo [Execute R Script][execute-r-script]. In questo caso, digitare `source("src/simpleplot.R")`.  

1. Assicurarsi di selezionare **salvare**.
...
Eseguire l'esperimento, selezionare la **eseguire** pulsante. Al termine dell'esecuzione, selezionare la [Execute R Script] [ execute-r-script] modulo e quindi selezionare **View output log** nel riquadro delle proprietà. Viene visualizzata una nuova pagina del browser che mostra i contenuti del file output.log. Scorrendo la pagina, dovrebbe essere visualizzato quanto segue.
...
Dopo aver eseguito l'esperimento, selezionare la porta di output Result Dataset1, quindi selezionare **Visualize**. I risultati dovrebbero essere simili a quanto visualizzato nella Figura 6.
...
Per visualizzare l'output R Device, selezionare la porta e quindi scegliere **Visualize**. Nella Figura 7 sono riportati l'output standard e i messaggi di errore standard generati dallo script R.
...
Il codice R completo per questa sezione è disponibile nel [repository MachineLearningSamples-Notebooks/studio-samples](https://github.com/Azure-Samples/MachineLearningSamples-Notebooks/tree/master/studio-samples).
...
R è un linguaggio tipizzato dinamicamente, in cui i tipi di dati vengono assegnati l'uno all'altro in base alle esigenze. Nel linguaggio R, i dati atomic possono essere di tipo numerico, logico e Char. Il tipo fattore viene usato invece per archiviare in modo compatto i dati relativi alle categorie. È possibile trovare altre informazioni sui tipi di dati nei riferimenti del [altre informazioni, leggere](#appendixb) sotto.
...
I frame di dati R supportano avanzate funzionalità di filtraggio. I set di dati, infatti, possono essere suddivisi in sottoinsiemi usando filtri logici su righe o colonne. In molto casi, tuttavia, sono necessari complessi criteri di filtro. I riferimenti nel [altre informazioni, leggere](#appendixb) di seguito contengono numerosi esempi di filtraggio di frame di dati.  
...
In questa sezione eseguiremo semplici operazioni di trasformazione sui valori di alcune delle colonne del frame di dati. Il linguaggio R supporta trasformazioni di dati pressoché arbitrarie. I riferimenti nel [altre informazioni, leggere](#appendixb) di seguito contengono numerosi esempi.
...
Il codice R completo per questa sezione è [repository MachineLearningSamples-Notebooks/studio-samples](https://github.com/Azure-Samples/MachineLearningSamples-Notebooks/tree/master/studio-samples).
...
Considerata la scala ridotta dei dati, il modello per la tendenza verrà compilato in RStudio, quindi il modello risultante verrà tagliato e incollato in Azure Machine Learning Studio. RStudio fornisce infatti un ambiente interattivo adatto a questo tipo di analisi interattiva.
...
Dai valori P (`Pr(>|t|)`) in questo output possiamo vedere che il termine al quadrato possa non essere significativo. Verrà usata la funzione `update()` per modificare il modello eliminando il termine al quadrato.
...
Come test di integrità, effettueremo un grafico in serie temporale dei dati della produzione casearia in California con la curva di trend mostrata di seguito. È stato aggiunto il codice seguente nel modello [Execute R Script][execute-r-script] (Esegui script R) di Azure Machine Learning Studio (non in RStudio) per creare il modello ed eseguire un tracciato. Il risultato viene mostrato nella Figura 23.
...
Creiamo adesso un altro grafico in serie temporale dei dati di produzione casearia in California per verificare l'effettivo funzionamento del modello con stagionalità. È stato aggiunto il codice seguente nel modello [Execute R Script][execute-r-script] (Esegui script R) di Azure Machine Learning Studio per creare il modello ed eseguire un tracciato.
...
L'esecuzione di questo codice in Azure Machine Learning Studio produce il tracciato mostrato nella Figura 24.
...
## <a id="appendixa"></a>Guida alla documentazione RStudio

RStudio è abbastanza ben documentata. Di seguito sono riportati alcuni collegamenti alle sezioni principali della documentazione rstudio, utili per iniziare a usare.

* **Creazione di progetti** -è possibile organizzare e gestire il codice R in progetti usando RStudio. Visualizzare [utilizzo di progetti](https://support.rstudio.com/hc/articles/200526207-Using-Projects) per informazioni dettagliate. Consigliabile seguire le istruzioni e creare un progetto degli esempi di codice R in questo articolo.  
* **Modifica e l'esecuzione di codice R** -RStudio offre un ambiente integrato per la modifica e l'esecuzione del codice R. Visualizzare [modifica e l'esecuzione di codice](https://support.rstudio.com/hc/articles/200484448-Editing-and-Executing-Code) per informazioni dettagliate.
* **Debug** -RStudio include avanzate funzionalità di debug. Visualizzare [debug con RStudio](https://support.rstudio.com/hc/articles/200713843-Debugging-with-RStudio) per altre informazioni su queste funzionalità. Per informazioni sulla risoluzione dei problemi di funzionalità punto di interruzione, vedere [risoluzione dei problemi di punto di interruzione](https://support.rstudio.com/hc/articles/200534337-Breakpoint-Troubleshooting).

## <a id="appendixb"></a>Altre informazioni, vedere
...
* [R per principianti](https://cran.r-project.org/doc/contrib/Paradis-rdebuts_en.pdf) Emmanuel Paradis è un buon punto di partenza.  
* [Un'introduzione a R](https://cran.r-project.org/doc/manuals/R-intro.html) dal w. N. Venables et. al. diventa un po' più approfondito.
...
Di seguito sono alcune importanti risorse su internet:

* DataCamp insegna a usare R dal proprio browser con lezioni video ed esercizi sulla codifica. Sono disponibili anche esercitazioni interattive sulle ultime tecniche e pacchetti di R. Richiedere la versione gratuita [esercitazione R interattiva](https://www.datacamp.com/courses/introduction-to-r).
* [Impara a programmare in R, la Guida definitiva](https://www.programiz.com/r-programming) da Programiz.
* Un rapido [esercitazione su R](https://www.cyclismo.org/tutorial/R/) Kelly Black della Clarkson University.
* Sono disponibili su 60 risorse di R elencate al [le risorse della lingua Top R per migliorare le competenze di data](https://www.computerworld.com/article/2497464/business-intelligence-60-r-resources-to-improve-your-data-skills.html).
