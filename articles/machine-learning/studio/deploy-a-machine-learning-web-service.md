...
description: Come convertire un esperimento di training in un esperimento predittivo, prepararlo per la distribuzione, quindi distribuirlo come servizio Web di Azure Machine Learning Studio (classico).
...
ms.locfileid: "79218135"
---
# <a name="deploy-an-azure-machine-learning-studio-classic-web-service"></a>Distribuire un servizio Web di Azure Machine Learning Studio (classico)Deploy an Azure Machine Learning Studio (classic) web service

[!INCLUDE [Notebook deprecation notice](../../../includes/aml-studio-notebook-notice.md)]

Azure Machine Learning Studio (classico) consente di creare e testare una soluzione analitica predittiva. È quindi possibile distribuire la soluzione come servizio Web.

I servizi Web di Machine Learning Studio (classici) forniscono un'interfaccia tra un'applicazione e un modello di punteggio del flusso di lavoro di Machine Learning Studio (classico). Un'applicazione esterna può comunicare con un modello di punteggio del flusso di lavoro di Machine Learning Studio (classico) in tempo reale. Una chiamata a un servizio Web di Machine Learning Studio (classico) restituisce i risultati della stima a un'applicazione esterna. Per effettuare una chiamata a un servizio Web, passare una chiave API creata al momento della distribuzione del servizio Web. Un servizio Web di Machine Learning Studio (classico) è basato su REST, una scelta di architettura popolare per i progetti di programmazione Web.

Azure Machine Learning Studio (classico) include due tipi di servizi Web:Azure Machine Learning Studio (classic) has two types of web services:

* Request-Response Service (RRS): un servizio a bassa latenza e altamente scalabile che assegna un punteggio a un singolo record di dati.
* Servizio di esecuzione batch (BES): servizio asincrono che contrassegna un batch di record di dati.
...
* **[Creare un esperimento]** di training: in Studio (classico), è possibile eseguire il training e testare un modello di analisi predittiva usando i dati di training forniti, usando un set di algoritmi di apprendimento automatico predefiniti.
...
Per eseguire il training di un modello di analisi predittiva, usare Azure Machine Learning Studio (classico) per creare un esperimento di training in cui includere vari moduli per caricare i dati di training, preparare i dati in base alle esigenze, applicare algoritmi di apprendimento automatico e valutare Risultati. È possibile eseguire l'iterazione dell'esperimento e provare diversi algoritmi di apprendimento automatico per confrontare e valutare i risultati.
...
* [Creare un semplice esperimento in Azure Machine Learning Studio (classico)Create a simple experiment in Azure Machine Learning Studio (classic)](create-experiment.md)
* [Sviluppare una soluzione predittiva con Azure Machine Learning Studio (classico)Develop a predictive solution with Azure Machine Learning Studio (classic)](tutorial-part1-credit-risk.md)
* [Importare i dati di training in Azure Machine Learning Studio (classico)Import your training data into Azure Machine Learning Studio (classic)](import-data.md)
* [Gestire le iterazioni degli esperimenti in Azure Machine Learning Studio (classico)Manage experiment iterations in Azure Machine Learning Studio (classic)](manage-experiment-iterations.md)
...
Per altre informazioni su come eseguire questa conversione, vedere [Come preparare il modello per la distribuzione in Azure Machine Learning Studio (classico).](convert-training-experiment-to-scoring-experiment.md)
...
### <a name="web-service-portal-deploy-experiment-page"></a>Pagina Distribuisci esperimento al portale dei servizi Web
...
Fare clic su **Distribuisci** per aprire la pagina **Guida introduttiva** per il servizio Web.
...
![Scarica il tuo set di dati dal canvas di Studio (classico)](./media/publish-a-machine-learning-web-service/figure-7-mls-download.png)
...
![Testare il processo di esecuzione batch con il portale del servizio WebTest your Batch Execution job with the web service portal](./media/publish-a-machine-learning-web-service/figure-6-test-batch-execution.png)
...
La pagina **Consumo** fornisce tutte le informazioni necessarie per accedere al servizio Web. Ad esempio, viene fornita la chiave API per consentire l'accesso autorizzato al servizio.

Per altre informazioni sull'accesso a un servizio Web di Machine Learning Studio (classico), vedere Come usare un servizio Web di [Azure Machine Learning Studio (classico).](consume-web-services.md)

### <a name="manage-your-new-web-service"></a>Gestire il nuovo servizio Web

È possibile gestire i nuovi servizi Web usando il portale dei servizi Web di Machine Learning Studio (classico). Dalla [pagina principale](https://services.azureml.net/)del portale fare clic su **Servizi Web**. Nella pagina dei servizi Web è possibile eliminare o copiare un servizio. Per monitorare un servizio specifico, fare clic sul servizio e quindi su **Dashboard**. Per monitorare i processi batch associati al servizio Web, fare clic su **Batch Request Log**(Log richieste batch).
...
![Distribuire il servizio Web da Studio (classico)Deploy your web service from Studio (classic)](./media/publish-a-machine-learning-web-service/figure-2.png)

### <a name="test-your-classic-web-service"></a>Testare il servizio Web classico

È possibile testare il servizio Web nel portale dei servizi Web di Machine Learning Studio (classico) o in Machine Learning Studio (classico).
...
È possibile fare clic sul collegamento **Test** anteprima per testare il servizio nel portale dei servizi Web di Azure Machine Learning Studio (classico), come illustrato in precedenza nella sezione Nuovo servizio Web.
...
È possibile fornire una descrizione dei dati di input, dei dati di output e dei parametri del servizio Web immettendo una stringa per ogni colonna in **INPUT SCHEMA**, **OUTPUT SCHEMA**e Web **SERVICE PARAMETER**. Queste descrizioni vengono usate nella documentazione del codice di esempio fornita per il servizio Web.

È possibile abilitare la registrazione per diagnosticare eventuali errori che si verificano quando si accede al servizio Web. Per altre informazioni, vedere Abilitare la registrazione per i servizi Web di [Machine Learning Studio (classici).](web-services-logging.md)

![Abilitare la registrazione nel portale dei servizi WebEnable logging in the web services portal](./media/publish-a-machine-learning-web-service/figure-4.png)
...
Per aggiornare il servizio Web, aprire l'esperimento predittivo originale utilizzato per distribuire il servizio Web ed eseguire una copia modificabile facendo clic su **SALVA CON**. Eseguire le modifiche e fare clic su **Distribuisci Servizio Web**.

Poiché questo esperimento è stato distribuito prima, viene chiesto se si desidera sovrascrivere (servizio Web classico) o aggiornare il servizio esistente (nuovo servizio Web). Se si fa clic su **Sì** o **Aggiorna,** il servizio Web esistente viene interrotto e il nuovo esperimento predittivo viene distribuito al suo posto.
...
Un'opzione per l'aggiornamento del servizio Web consiste nel ripetere il training del modello a livello di codice. Per altre informazioni, vedere [Riaddestrare](/azure/machine-learning/studio/retrain-machine-learning-model)i modelli di Machine Learning Studio (classici) a livello di codice.

## <a name="next-steps"></a>Passaggi successivi

* Per ulteriori dettagli tecnici sul funzionamento della distribuzione, vedere Come un modello di [Machine Learning Studio (classico) procede da un esperimento a un servizio Web operativo.](model-progression-experiment-to-web-service.md)
...
* È possibile utilizzare l'API REST e accedere al servizio Web in diversi modi. Vedere Come usare un servizio Web di [Azure Machine Learning Studio (classico).](consume-web-services.md)

<!-- internal links -->
[Creare un esperimento di trainingCreate a training experiment]: #create-a-training-experiment
