---
title: Come un modello diventa un servizio WebHow a model becomes a web service
titleSuffix: ML Studio (classic) - Azure
description: Una panoramica dei meccanismi di avanzamento del modello di Azure Machine Learning Studio (classico) da un esperimento di sviluppo a un servizio Web.
...
---
# <a name="how-a-machine-learning-studio-classic-model-progresses-from-an-experiment-to-a-web-service"></a>Avanzamento di un modello di Machine Learning Studio (classico) da un esperimento a un servizio Web

[!INCLUDE [Notebook deprecation notice](../../../includes/aml-studio-notebook-notice.md)]
Azure Machine Learning Studio (classico) offre un canvas interattivo che consente di sviluppare, eseguire, testare e iterare un ***esperimento*** che rappresenta un modello di analisi predittiva. Sono disponibili diversi moduli che consentono di:
...
Quando si è soddisfatti dell'esperimento, è possibile distribuirlo come ***servizio Web di Azure classico*** o come nuovo servizio Web di Azure Machine ***Learning*** in modo che gli utenti possano inviare nuovi dati e ricevere risultati.
...
> [!NOTE]
> Esistono altri modi per sviluppare e distribuire modelli di apprendimento automatico, ma questo articolo è incentrato su come si usa Machine Learning Studio (classico). Per una descrizione su come creare un servizio Web predittivo classico con R, vedere il post di blog su [compilazione e distribuzione di app Web predittive con RStudio e Azure Machine Learning Studio](https://blogs.technet.com/b/machinelearning/archive/2015/09/25/build-and-deploy-a-predictive-web-app-using-rstudio-and-azure-ml.aspx).
>
>

Mentre Azure Machine Learning Studio (classico) è progettato per aiutarti a sviluppare e distribuire un modello di *analisi predittiva,* è possibile usare Studio (classico) per sviluppare un esperimento che non includa un modello di analisi predittiva. Un esperimento, ad esempio, potrebbe limitarsi a inserire i dati, manipolarli e quindi generare l'output dei risultati. Proprio come un esperimento di analisi predittiva, è possibile distribuire questo esperimento non predittivo come servizio Web, ma è un processo più semplice perché l'esperimento non è il training o il punteggio di un modello di apprendimento automatico. Anche se non è il tipico usare Studio (classico) in questo modo, lo includeremo nella discussione in modo da poter fornire una spiegazione completa di come funziona Studio (classico).

## <a name="developing-and-deploying-a-predictive-web-service"></a>Sviluppo e distribuzione di un servizio Web predittivo
Ecco le fasi che una soluzione tipica segue durante lo sviluppo e la distribuzione tramite Machine Learning Studio (classico):Here are the stages that a typical solution follows as you develop and deploy it using Machine Learning Studio (classic):
...
***L'esperimento*** di formazione è la fase iniziale di sviluppo del servizio Web in Machine Learning Studio (classico). Lo scopo dell'esperimento di training è di fornire un'area in cui sviluppare, testare, eseguire l'iterazione e infine il training di un modello di Machine Learning. Puoi anche eseguire il training di più modelli contemporaneamente mentre cerchi la soluzione migliore, ma una volta che hai finito di sperimentare, selezionerai un singolo modello sottoposto a training ed eliminerai il resto dall'esperimento. Per un esempio di sviluppo di un esperimento di analisi predittiva, vedere Sviluppare una soluzione di analisi predittiva per la valutazione del rischio di [credito in Azure Machine Learning Studio (classico).](tutorial-part1-credit-risk.md)
...
Se l'esperimento non esegue il training di un modello di analisi predittiva, non è necessario creare sia un esperimento di training che un esperimento di assegnazione del punteggio: è disponibile un solo esperimento ed è possibile distribuirlo come servizio Web. Machine Learning Studio (classico) rileva se l'esperimento contiene un modello predittivo analizzando i moduli usati.

Dopo aver eseguito l'iterazione sull'esperimento e averlo soddisfatto:
...
Dopo aver distribuito l'esperimento come servizio Web, cosa succede se è necessario aggiornarlo?

Dipende da che cosa è necessario aggiornare:

**Si desidera modificare l'input o l'output oppure modificare il modo in cui il servizio Web modifica i dati**

Se non si modifica il modello, ma si modifica solo il modo in cui il servizio Web gestisce i dati, è possibile modificare l'esperimento predittivo e quindi fare clic su **Distribuisci servizio Web** e selezionare di nuovo **Distribuisci servizio Web [Classico]** o **Distribuisci servizio Web [Nuovo].** Il servizio Web viene arrestato, l'esperimento predittivo aggiornato viene distribuito e il servizio Web viene riavviato.
...
* conversione dell'esperimento - [Come preparare il modello per la distribuzione in Azure Machine Learning Studio (classico)converting](convert-training-experiment-to-scoring-experiment.md) the experiment - How to prepare your model for deployment in Azure Machine Learning Studio (classic)
...
* [Esercitazione sull'apprendimento automatico: Creare il primo esperimento in Azure Machine Learning Studio (classico)Machine learning tutorial: Create your first experiment in Azure Machine Learning Studio (classic)](create-experiment.md)
* [Procedura dettagliata: Sviluppare una soluzione di analisi predittiva per la valutazione del rischio di credito in Azure Machine LearningWalkthrough: Develop a predictive analytics solution for credit risk assessment in Azure Machine Learning](tutorial-part1-credit-risk.md)
