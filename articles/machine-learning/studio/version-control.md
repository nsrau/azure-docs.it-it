...
description: Applicare le procedure consigliate per la gestione del ciclo di vita delle applicazioni in Azure Machine Learning Studio (classico)Apply Application Lifecycle Management best practices in Azure Machine Learning Studio (classic)
...
ms.locfileid: "79204137"
---
# <a name="application-lifecycle-management-in-azure-machine-learning-studio-classic"></a>Gestione del ciclo di vita delle applicazioni in Azure Machine Learning Studio (classico)Application Lifecycle Management in Azure Machine Learning Studio (classic)

[!INCLUDE [Notebook deprecation notice](../../../includes/aml-studio-notebook-notice.md)]

Azure Machine Learning Studio (classico) è uno strumento per lo sviluppo di esperimenti di apprendimento automatico operativi nella piattaforma cloud di Azure.Azure Machine Learning Studio (classic) is a tool for developing machine learning experiments that are operationalized in the Azure cloud platform. Si tratta di una specie di fusione del servizio cloud scalabile e dell'IDE di Visual Studio in un'unica piattaforma. È possibile incorporare procedure standard di Application Lifecycle Management (ALM) dal controllo delle versioni di varie risorse all'esecuzione e distribuzione automatizzate in Azure Machine Learning Studio (classico). Questo articolo descrive alcune opzioni e approcci.
...
Se si crea un nuovo servizio Web basato su Azure Resource Manager, il costrutto dell'endpoint non è più disponibile. È invece possibile generare file di definizione del servizio Web (WSD), in formato JSON, dall'esperimento predittivo usando il commandlet di PowerShell [Export-AmlWebServiceDefinitionFromExperiment](https://github.com/hning86/azuremlps#export-amlwebservicedefinitionfromexperiment) o il commandlet [*Export-AzMlWebservice*](https://docs.microsoft.com/powershell/module/az.machinelearning/export-azmlwebservice) PowerShell da un servizio Web basato su Resource Manager distribuito.
...
Un aspetto importante di ALM è la possibilità di automatizzare il processo di distribuzione ed esecuzione dell'applicazione. In Azure Machine Learning Studio (classico), è possibile eseguire questa operazione usando il [modulo PowerShell.In](https://aka.ms/amlps)Azure Machine Learning Studio (classic), you can accomplish this by using the PowerShell module . Ecco un esempio di passaggi end-to-end rilevanti per un processo di esecuzione/distribuzione automatica ALM standard tramite il modulo PowerShell di [Azure Machine Learning Studio (classico).](https://aka.ms/amlps) Ogni passaggio è collegato a uno o più commandlet di PowerShell che possono essere usati per portare a termine questa operazione.
...
* Scaricare il modulo PowerShell di [Azure Machine Learning Studio (classico)](https://aka.ms/amlps) e iniziare ad automatizzare le attività ALM.
