...
In questa esercitazione in due parti si apprenderà come usare l'interfaccia visiva di Azure Machine Learning per sviluppare e distribuire una soluzione di analisi predittiva che stima il prezzo di qualsiasi automobile. 
...
1. Selezionare **Visual Interface** (Interfaccia visiva).

    ![Screenshot dell'area di lavoro visiva che mostra come accedere all'interfaccia visiva](./media/ui-tutorial-automobile-price-train-score/launch-visual-interface.png)
...
Nell'interfaccia visiva sono disponibili diversi set di dati di esempio con cui sperimentare. Per questa esercitazione verrà usato il set di dati di esempio **Automobile price data (Raw)** . 
...
> [!TIP]
> La pulizia dei valori mancanti dai dati di input è un prerequisito per l'uso della maggior parte dei moduli nell'interfaccia visiva.
