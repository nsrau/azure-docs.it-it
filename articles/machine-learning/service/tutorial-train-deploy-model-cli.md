...
* `train-sklearn.py`: Script di training. Questo file addestra il modello.
...
Questo comando crea un file `.azureml/config.json`, che contiene le informazioni necessarie per connettersi all'area di lavoro. Il resto dei comandi @no__t 0 usati in questa esercitazione utilizzerà questo file, pertanto non è necessario aggiungere l'area di lavoro e il gruppo di risorse a tutti i comandi.

## <a name="create-the-compute-target-for-training"></a>Creare la destinazione di calcolo per il training

In questo esempio viene utilizzata un'istanza di calcolo Azure Machine Learning per eseguire il training del modello. Per creare una nuova istanza di calcolo, usare il comando seguente:
...
Per avviare un'esecuzione di training nella destinazione di calcolo `cpu`, passare alla directory `model-training`, quindi usare il comando seguente:
...
Il parametro `-c sklearn` specifica il file `.azureml/sklearn.runconfig`. Come indicato in precedenza, questo file contiene le informazioni usate per configurare l'ambiente usato dall'esecuzione del training. Se si esamina questo file, si noterà che fa riferimento alla destinazione di calcolo `cpu` creata in precedenza. Elenca inoltre il numero di nodi da usare durante il training (`"nodeCount": "4"`) e contiene una sezione `"condaDependenciees"` in cui sono elencati i pacchetti Python necessari per eseguire lo script di training.

Per altre informazioni sui file di configurazione di esecuzione, vedere [configurare e usare le destinazioni di calcolo per il training del modello](how-to-set-up-training-targets.md#create-run-configuration-and-submit-run-using-azure-machine-learning-cli).
...
Per distribuire un modello, passare alla directory `model-deployment` e quindi utilizzare il comando seguente:
...
Questo comando restituisce lo stesso documento JSON, incluso il `scoringUri`.
...
Per altre informazioni sull'uso dell'interfaccia della riga di comando, vedere [usare l'estensione CLI per il servizio Azure Machine Learning](reference-azure-machine-learning-cli.md).