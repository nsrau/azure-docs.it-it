<properties 
	pageTitle="App Machine Learning: Servizio di rilevamento delle anomalie | Azure " 
...
2. Aumento dell'intervallo dinamico dei valori: ad esempio, quando vengono monitorate le eccezioni generate da un servizio, ogni aumento dell'intervallo dinamico dei valori può indicare l'instabilità dell'integrità del servizio, e
...
Il servizio offre API basate su un REST tramite HTTPS che possono essere utilizzate in modi diversi come un'applicazione Web o per dispositivi mobili, R, Python, Excel ecc. Si dispone di un'[applicazione Web di Azure](http://anomalydetection-aml.azurewebsites.net/) che consente di eseguire il servizio Web di rilevamento delle anomalie sui dati e visualizzarne i risultati. 
