...
## <a name="a-idstandard-authn-web-appaconsider-using-a-standard-authentication-mechanism-to-authenticate-to-web-application"></a><a id="standard-authn-web-app"></a>Valutare l'uso di un meccanismo di autenticazione standard per l'autenticazione nell'applicazione Web
...
## <a name="a-idhandle-failed-authnaapplications-must-handle-failed-authentication-scenarios-securely"></a><a id="handle-failed-authn"></a>Gestire in modo sicuro scenari di autenticazione non riuscita mediante le applicazioni
...
## <a name="a-idstep-up-adaptive-authnaenable-step-up-or-adaptive-authentication"></a><a id="step-up-adaptive-authn"></a>Abilitare l'autenticazione adattiva o incrementale
...
## <a name="a-idadmin-interface-lockdownaensure-that-administrative-interfaces-are-appropriately-locked-down"></a><a id="admin-interface-lockdown"></a>Assicurarsi che le interfacce amministrative siano bloccate correttamente
...
## <a name="a-idforgot-pword-fxnaimplement-forgot-password-functionalities-securely"></a><a id="forgot-pword-fxn"></a>Implementare in modo sicuro funzionalità per la password dimenticata
...
## <a name="a-idpword-account-policyaensure-that-password-and-account-policy-are-implemented"></a><a id="pword-account-policy"></a>Assicurarsi che vengano implementati criteri di account e password
...
## <a name="a-idcontrols-username-enumaimplement-controls-to-prevent-username-enumeration"></a><a id="controls-username-enum"></a>Implementare controlli per prevenire l'enumerazione del nome utente
...
## <a name="a-idwin-authn-sqlawhen-possible-use-windows-authentication-for-connecting-to-sql-server"></a><a id="win-authn-sql"></a>Usare l'autenticazione di Windows per la connessione a SQL Server, se possibile
...
| Passi | L'autenticazione di Windows usa il protocollo di sicurezza Kerberos, consente l'applicazione dei criteri password per la convalida della complessità delle password e supporta il blocco account e la scadenza delle password.|

## <a name="a-idaad-authn-sqlawhen-possible-use-azure-active-directory-authentication-for-connecting-to-sql-database"></a><a id="aad-authn-sql"></a>Usare l'autenticazione di Azure Active Directory per la connessione al database SQL, se possibile
...
## <a name="a-idauthn-account-pwordawhen-sql-authentication-mode-is-used-ensure-that-account-and-password-policy-are-enforced-on-sql-server"></a><a id="authn-account-pword"></a>Assicurarsi che vengano applicati i criteri di account e password in SQL Server quando viene usata la modalità di autenticazione SQL
...
## <a name="a-idautn-contained-dbado-not-use-sql-authentication-in-contained-databases"></a><a id="autn-contained-db"></a>Non usare l'autenticazione SQL in database indipendenti
...
## <a name="a-idauthn-sas-tokensause-per-device-authentication-credentials-using-sas-tokens"></a><a id="authn-sas-tokens"></a>Usare credenziali di autenticazione per dispositivo con i token di firma di accesso condiviso
...
## <a name="a-idmulti-factor-azure-adminaenable-azure-multi-factor-authentication-for-azure-administrators"></a><a id="multi-factor-azure-admin"></a>Abilitare Azure Multi-Factor Authentication per amministratori di Azure
...
## <a name="a-idanon-access-clusterarestrict-anonymous-access-to-service-fabric-cluster"></a><a id="anon-access-cluster"></a>Limitare l'accesso anonimo a un cluster di Service Fabric
...
## <a name="a-idfabric-cn-nnaensure-that-service-fabric-client-to-node-certificate-is-different-from-node-to-node-certificate"></a><a id="fabric-cn-nn"></a>Assicurarsi che il certificato da client a nodo di Service Fabric sia diverso dal certificato da nodo a nodo
...
## <a name="a-idaad-client-fabricause-aad-to-authenticate-clients-to-service-fabric-clusters"></a><a id="aad-client-fabric"></a>Usare Azure Active Directory per autenticare client in cluster di Service Fabric
...
## <a name="a-idfabric-cert-caaensure-that-service-fabric-certificates-are-obtained-from-an-approved-certificate-authority-ca"></a><a id="fabric-cert-ca"></a>Assicurarsi che i certificati di Service Fabric vengano ottenuti da un'Autorità di certificazione (CA) approvata
...
## <a name="a-idstandard-authn-idause-standard-authentication-scenarios-supported-by-identity-server"></a><a id="standard-authn-id"></a>Usare scenari di autenticazione standard supportati da Identity Server
...
## <a name="a-idoverride-tokenaoverride-the-default-identity-server-token-cache-with-a-scalable-alternative"></a><a id="override-token"></a>Sostituire la cache dei token di Identity Server predefinita con un'alternativa scalabile
...
## <a name="a-idbinaries-signedaensure-that-deployed-applications-binaries-are-digitally-signed"></a><a id="binaries-signed"></a>Assicurarsi che i file binari dell'applicazione distribuita abbiano una firma digitale
...
## <a name="a-idmsmq-queuesaenable-authentication-when-connecting-to-msmq-queues-in-wcf"></a><a id="msmq-queues"></a>Abilitare l'autenticazione nella connessione a code MSMQ in WCF
...
## <a name="a-idmessage-noneawcf-do-not-set-message-clientcredentialtype-to-none"></a><a id="message-none"></a>WCF: non impostare il messaggio clientCredentialType su Nessuno
...
## <a name="a-idtransport-noneawcf-do-not-set-transport-clientcredentialtype-to-none"></a><a id="transport-none"></a>WCF: non impostare il trasporto clientCredentialType su Nessuno
...
## <a name="a-idauthn-secure-apiaensure-that-standard-authentication-techniques-are-used-to-secure-web-apis"></a><a id="authn-secure-api"></a>Assicurarsi che vengano usate tecniche di autenticazione standard per proteggere le API Web
...
## <a name="a-idauthn-aadause-standard-authentication-scenarios-supported-by-azure-active-directory"></a><a id="authn-aad"></a>Usare scenari di autenticazione standard supportati da Azure Active Directory
...
## <a name="a-idadal-scalableaoverride-the-default-adal-token-cache-with-a-scalable-alternative"></a><a id="adal-scalable"></a>Sostituire la cache dei token di ADAL predefinita con un'alternativa scalabile
...
## <a name="a-idtokenreplaycache-adalaensure-that-tokenreplaycache-is-used-to-prevent-the-replay-of-adal-authentication-tokens"></a><a id="tokenreplaycache-adal"></a>Assicurarsi che venga usata la proprietà TokenReplayCache per impedire la riproduzione dei token di autenticazione ADAL
...
## <a name="a-idadal-oauth2ause-adal-libraries-to-manage-token-requests-from-oauth2-clients-to-aad-or-on-premises-ad"></a><a id="adal-oauth2"></a>Usare librerie ADAL per gestire le richieste di token da client OAuth2 in AAD o in Active Directory locale
...
## <a name="a-idauthn-devices-fieldaauthenticate-devices-connecting-to-the-field-gateway"></a><a id="authn-devices-field"></a>Autenticare dispositivi che si connettono al gateway sul campo
...
## <a name="a-idauthn-devices-cloudaensure-that-devices-connecting-to-cloud-gateway-are-authenticated"></a><a id="authn-devices-cloud"></a>Assicurarsi che i dispositivi che si connettono al gateway cloud vengano autenticati
...
## <a name="a-idauthn-credause-per-device-authentication-credentials"></a><a id="authn-cred"></a>Usare credenziali di autenticazione per dispositivo
...
## <a name="a-idreq-containers-anonaensure-that-only-the-required-containers-and-blobs-are-given-anonymous-read-access"></a><a id="req-containers-anon"></a>Assicurarsi che l'accesso in lettura anonimo venga concesso solo ai contenitori e ai BLOB necessari
...
## <a name="a-idlimited-access-sasagrant-limited-access-to-objects-in-azure-storage-using-sas-or-sap"></a><a id="limited-access-sas"></a>Concedere l'accesso limitato agli oggetti in Archiviazione di Azure tramite la firma di accesso condiviso o criteri di accesso archiviati
