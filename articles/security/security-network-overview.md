...
ms.locfileid: "34365628"
---
# <a name="azure-network-security-overview"></a>Panoramica della sicurezza di rete di Azure
Azure dispone di una solida infrastruttura di rete per supportare i requisiti di connettività di applicazioni e servizi. La connettività di rete è possibile tra le risorse residenti in Azure, le risorse locali e quelle ospitate in Azure, nonché da e verso Internet e Azure.

Lo scopo di questo articolo è spiegare i vantaggi che Azure offre nell'area della sicurezza di rete. Oltre alla spiegazioni di base sui concetti fondamentali e sui requisiti della sicurezza di rete, l'articolo illustra anche gli argomenti seguenti:
...
La connettività di rete è indispensabile per le macchine virtuali. Per supportare questo requisito, Azure richiede che le macchine virtuali siano connesse a Rete virtuale di Azure. Una rete virtuale è un costrutto logico basato sull'infrastruttura di rete fisica di Azure. Ogni rete virtuale logica è isolata da tutte le altre reti virtuali. Ciò aiuta a garantire che il traffico di rete nelle distribuzioni di un utente non sia accessibile ad altri clienti di Azure.
...
Questo tipo di connessione usa il protocollo [VPN SSTP](https://technet.microsoft.com/library/cc731352.aspx) che consente di configurare una connessione privata e sicura tra l'utente e la rete virtuale. Quando viene stabilita la connessione VPN, l'utente può usare RDP o SSH tramite il collegamento VPN in una macchina virtuale qualsiasi nella rete virtuale. Si presume che l'utente possa eseguire l'autenticazione e disponga delle autorizzazioni.
...
Le organizzazioni che necessitano del massimo livello di sicurezza e disponibilità per le connessioni cross-premise usano in genere collegamenti WAN dedicati per connettersi a siti remoti. Azure consente di usare un collegamento WAN dedicato per connettere la rete locale a una rete virtuale. Questo collegamento si basa su Azure ExpressRoute.
...
Un'opzione migliore potrebbe essere quella di creare una VPN da sito a sito che connette due reti virtuali. Questa modalità usa lo stesso protocollo in [modalità tunnel IPsec](https://technet.microsoft.com/library/cc786385.aspx) della connessione VPN da sito a sito cross premise citata in precedenza.
...
[Visualizzazione Gruppo di sicurezza](../network-watcher/network-watcher-security-group-view-overview.md) aiuta la conformità di controllo e protezione di Macchine virtuali di Microsoft Azure. Questa funzionalità consente di eseguire controlli a livello di codice, confrontando i criteri di base definiti dalla propria organizzazione con le regole valide per ognuna delle proprie macchine virtuali. Ciò consente di identificare eventuali deviazioni della configurazione.
