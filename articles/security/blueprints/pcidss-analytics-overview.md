...
Azure offre svariati servizi di creazione di report e di analisi per i clienti. Questa soluzione incorpora Azure Machine Learning insieme al database SQL di Azure per scorrere rapidamente i dati e offrire risultati più rapidi grazie alla modellazione più intelligente. Azure Machine Learning aumenta la velocità di query individuando nuove relazioni tra i set di dati. Dopo aver eseguito il training dei dati tramite diverse funzioni statistiche, è possibile sincronizzare fino a 7 pool di query aggiuntivi, 8 con il server del cliente, con gli stessi modelli tabulari per distribuire i carichi di lavoro di query e ridurre i tempi di risposta.
...
![Diagramma dell'architettura di riferimento per PCI DSS Analytics](images/pcidss-analytics-architecture.png "Diagramma dell'architettura di riferimento per PCI DSS Analytics")
...
**Azure Machine Learning**:[Azure Machine Learning](https://docs.microsoft.com/azure/machine-learning/service/) è una tecnica di data science che consente ai computer di usare i dati esistenti per prevedere comportamenti, tendenze e risultati futuri.
...
**Archiviazione di Azure**: per soddisfare i requisiti dei dati crittografati inattivi, [Archiviazione di Azure](https://azure.microsoft.com/services/storage/) usa la [crittografia del servizio di archiviazione](../../storage/common/storage-service-encryption.md).  Ciò consente di proteggere i dati dei possessori di carte, supportando l'impegno a livello di sicurezza dell'organizzazione e i requisiti di conformità definiti da PCI DSS 3.2.
...
Il diagramma di flusso di dati per questa architettura di riferimento è disponibile per il [download](https://aka.ms/PCIAnalyticsTM) o è riportato più avanti. Questo modello può aiutare i clienti a comprendere i punti di rischio potenziale nell'infrastruttura del sistema quando vengono apportate modifiche.

![Diagramma dell'architettura di riferimento per PCI DSS Analytics](images/pcidss-analytics-threat-model.png "Analisi del modello di rischio PCI DSS")
