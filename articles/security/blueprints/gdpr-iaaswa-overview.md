...
Log Analytics offre una registrazione completa delle attività di sistema e degli utenti, nonché dell'integrità del sistema. La soluzione [Log Analytics](https://azure.microsoft.com/services/log-analytics/) raccoglie e analizza i dati generati dalle risorse in Azure e negli ambienti locali.
...
Il diagramma di flusso di dati per questa architettura di riferimento illustrato di seguito è disponibile anche per il [download](https://aka.ms/gdprIaaSdfd). Il modello può aiutare i clienti comprendere i punti di rischio potenziale nell'infrastruttura del sistema quando vengono apportate modifiche.
