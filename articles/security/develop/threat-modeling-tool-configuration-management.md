...
| **Database** | <ul><li>[Configurare Windows Firewall per l'accesso al motore di databaseConfigure a Windows Firewall for Database Engine Access](#firewall-db)</li></ul> |
| **Web API** | <ul><li>[Verificare che siano consentite solo origini attendibili se CORS è abilitato nellASP.NETAPI Web](#cors-api)</li><li>[Crittografare sezioni dei file di configurazione dell'API Web che contengono dati sensibili](#config-sensitive)</li></ul> |
...
| **Passi** | I sistemi firewall contribuiscono a impedire l'accesso non autorizzato alle risorse del computer. Per accedere a un'istanza del motore di database di SQL Server tramite un firewall, è necessario configurare il firewall sul computer che esegue SQL Server per consentire l'accesso. |
...
| **Attributi**              | N/D  |
| **Riferimenti**              | N/D  |
| **Passi** | Assicurarsi che i dispositivi abbiano controlli di sicurezza degli endpoint, ad esempio bit-locker per la crittografia a livello di disco, antivirus con firme aggiornate, firewall basato su host, aggiornamenti del sistema operativo, criteri di gruppo e così via, configurati in base ai criteri di sicurezza dell'organizzazione. |
