...
| --------------- | ------- |
| **Applicazione Web**    | <ul><li>[Prendere in considerazione l'uso di un meccanismo di autenticazione standard per l'autenticazione nell'applicazione Web](#standard-authn-web-app)</li><li>[Gestire in modo sicuro scenari di autenticazione non riuscita mediante le applicazioni](#handle-failed-authn)</li><li>[Abilitare l'autenticazione di step up o Adaptive](#step-up-adaptive-authn)</li><li>[Assicurarsi che le interfacce amministrative siano bloccate correttamente](#admin-interface-lockdown)</li><li>[Implementare in modo sicuro funzionalità per le password dimenticate](#forgot-pword-fxn)</li><li>[Verificare che i criteri password e account siano implementati](#pword-account-policy)</li><li>[Implementare i controlli per impedire l'enumerazione del nome utente](#controls-username-enum)</li></ul> |
| **Database** | <ul><li>[Quando possibile, utilizzare l'autenticazione di Windows per la connessione a SQL Server](#win-authn-sql)</li><li>[Quando possibile, usare l'autenticazione Azure Active Directory per la connessione al database SQL](#aad-authn-sql)</li><li>[Assicurarsi che vengano applicati i criteri di account e password in SQL Server quando viene usata la modalità di autenticazione SQL](#authn-account-pword)</li><li>[Non usare l'autenticazione SQL in database indipendenti](#autn-contained-db)</li></ul> |
...
| **Limite di trust di Azure** | <ul><li>[Abilitare Multi-Factor Authentication Azure AD per gli amministratori di Azure](#multi-factor-azure-admin)</li></ul> |
| **Limite di trust di Service Fabric** | <ul><li>[Limitare l'accesso anonimo al cluster Service Fabric](#anon-access-cluster)</li><li>[Verificare che Service Fabric certificato da client a nodo sia diverso dal certificato da nodo a nodo](#fabric-cn-nn)</li><li>[Usare AAD per autenticare i client nei cluster di Service Fabric](#aad-client-fabric)</li><li>[Assicurarsi che i certificati di Service Fabric vengano ottenuti da un'Autorità di certificazione (CA) approvata](#fabric-cert-ca)</li></ul> |
| **Identity Server** | <ul><li>[Usare scenari di autenticazione standard supportati da Identity Server](#standard-authn-id)</li><li>[Sostituire la cache dei token del server Identity predefinita con un'alternativa scalabile](#override-token)</li></ul> |
...
| **WCF** | <ul><li>[Abilitare l'autenticazione per la connessione a code MSMQ in WCF](#msmq-queues)</li><li>[WCF: non impostare il messaggio clientCredentialType su Nessuno](#message-none)</li><li>[WCF: non impostare il trasporto clientCredentialType su nessuno](#transport-none)</li></ul> |
...
| **Azure AD** | <ul><li>[Usare scenari di autenticazione standard supportati da Azure Active Directory](#authn-aad)</li><li>[Sostituire la cache dei token ADAL predefinita con un'alternativa scalabile](#adal-scalable)</li><li>[Assicurarsi che venga usata la proprietà TokenReplayCache per impedire la riproduzione dei token di autenticazione ADAL](#tokenreplaycache-adal)</li><li>[Usare le librerie ADAL per gestire le richieste di token da client OAuth2 ad AAD (o AD locale)](#adal-oauth2)</li></ul> |
| **Gateway IoT sul campo** | <ul><li>[Autenticare dispositivi che si connettono al gateway sul campo](#authn-devices-field)</li></ul> |
| **Gateway IoT cloud** | <ul><li>[Assicurarsi che i dispositivi che si connettono al gateway cloud vengano autenticati](#authn-devices-cloud)</li><li>[Usare le credenziali di autenticazione per dispositivo](#authn-cred)</li></ul> |
...
| **Riferimenti**              | [SQL Server: scegliere una modalità di autenticazione](/sql/relational-databases/security/choose-an-authentication-mode) |
...
| **Riferimenti**              | [Connettersi al Database SQL utilizzando l’autenticazione di Azure Active Directory](../../azure-sql/database/authentication-aad-overview.md) |
...
| **Tecnologie applicabili** | Generico |
| **Attributes (Attributi)**              | N/D  |
| **Riferimenti**              | [Criteri password SQL Server](/previous-versions/sql/sql-server-2012/ms161959(v=sql.110)) |
...
| **Riferimenti**              | [Procedure consigliate per la sicurezza in database indipendenti](/sql/relational-databases/databases/security-best-practices-with-contained-databases) |
...
## <a name="enable-azure-ad-multi-factor-authentication-for-azure-administrators"></a><a id="multi-factor-azure-admin"></a>Abilitare Multi-Factor Authentication Azure AD per gli amministratori di Azure
...
| **Tecnologie applicabili** | Generico |
| **Attributes (Attributi)**              | N/D  |
| **Riferimenti**              | [Che cos'è Azure AD Multi-Factor Authentication?](../../active-directory/authentication/concept-mfa-howitworks.md) |
| **Passaggi** | <p>Multi-Factor Authentication (MFA) è un metodo di autenticazione che richiede più di un metodo di verifica e con il quale viene aggiunto un secondo livello di sicurezza critico agli accessi e alle transazioni degli utenti. In genere richiede due o più dei metodi di verifica seguenti:</p><ul><li>Un elemento noto, in genere una password</li><li>Un oggetto fisico (un dispositivo attendibile non facilmente duplicabile, come un telefono)</li><li>Una caratteristica fisica dell'utente (biometrica)</li><ul>|
...
| **Riferimenti**              | [Scenari di sicurezza di un cluster di Service Fabric](../../service-fabric/service-fabric-cluster-security.md) |
...
| **Fase SDL**               | Distribuzione |
| **Tecnologie applicabili** | Generico |
| **Attributes (Attributi)**              | Ambiente: Azure, ambiente: autonomo |
| **Riferimenti**              | [Sicurezza basata su certificati da client a nodo in Service Fabric](../../service-fabric/service-fabric-cluster-security.md#client-to-node-certificate-security), [Connettersi a un cluster sicuro tramite il certificato client](../../service-fabric/service-fabric-connect-to-secure-cluster.md) |
...
| **Tecnologie applicabili** | Generico |
| **Attributes (Attributi)**              | Ambiente: Azure |
| **Riferimenti**              | [Scenari di sicurezza per i cluster: raccomandazioni sulla sicurezza](../../service-fabric/service-fabric-cluster-security.md#security-recommendations) |
...
| **Tecnologie applicabili** | Generico |
| **Attributes (Attributi)**              | Ambiente: Azure |
| **Riferimenti**              | [Certificati X.509 e Service Fabric](../../service-fabric/service-fabric-cluster-security.md#x509-certificates-and-service-fabric) |
...
| **Attributes (Attributi)**              | N/D |
| **Riferimenti**              | [MSDN](/previous-versions/msp-n-p/ff648500(v=pandp.10)) |
...
| **Riferimenti**              | [MSDN](/previous-versions/msp-n-p/ff648500(v=pandp.10)), [Fortify](https://community.microfocus.com/t5/UFT-Discussions/UFT-API-Test-with-WCF-wsHttpBinding/m-p/600927) |
...
| **Tecnologie applicabili** | Generico |
| **Attributes (Attributi)**              | N/D  |
| **Riferimenti**              | [Scenari di autenticazione per Azure AD](../../active-directory/develop/authentication-vs-authorization.md), [Esempi di codice di Azure Active Directory](../../active-directory/azuread-dev/sample-v1-code.md), [Guida per gli sviluppatori di Azure Active Directory](../../active-directory/develop/index.yml) |
...
| **Tecnologie applicabili** | Generico |
| **Attributes (Attributi)**              | N/D  |
| **Riferimenti**              | [Modern Authentication with Azure Active Directory for Web Applications](/archive/blogs/microsoft_press/new-book-modern-authentication-with-azure-active-directory-for-web-applications) (Autenticazione moderna con Azure Active Directory per applicazioni Web), [Using Redis as ADAL token cache](https://blogs.msdn.microsoft.com/mrochon/2016/09/19/using-redis-as-adal-token-cache/) (Uso di Redis come cache dei token ADAL)  |
...
| **Tecnologie applicabili** | Generico |
| **Attributes (Attributi)**              | N/D  |
| **Riferimenti**              | [Modern Authentication with Azure Active Directory for Web Applications](/archive/blogs/microsoft_press/new-book-modern-authentication-with-azure-active-directory-for-web-applications) (Autenticazione moderna con Azure Active Directory per applicazioni Web) |
...
| **Tecnologie applicabili** | Generico |
| **Attributes (Attributi)**              | N/D  |
| **Riferimenti**              | [ADAL](../../active-directory/azuread-dev/active-directory-authentication-libraries.md) |
...
| **Attributes (Attributi)**              | N/A, opzione gateway: Hub IoT di Azure |
| **Riferimenti**              | N/A, [Hub Azure Internet con .NET](../../iot-hub/quickstart-send-telemetry-dotnet.md), [Introduzione con hub e node JS](../../iot-hub/quickstart-send-telemetry-node.md)di Azure, [protezione delle cose con SAS e certificati](../../iot-hub/iot-hub-devguide-security.md), [repository git](https://github.com/Azure/azure-iot-sdks/) |
...
* Creare un hub Internet delle cose in Azure
...
| **Riferimenti**              | [Token di sicurezza dell'hub IoT di Azure](../../iot-hub/iot-hub-devguide-security.md) |
...
| **Riferimenti**              | [Gestire l'accesso in lettura anonimo a contenitori e BLOB](../../storage/blobs/anonymous-read-access-configure.md), [Firme di accesso condiviso, parte 1: informazioni sul modello di firma di accesso condiviso](../../storage/common/storage-sas-overview.md) |
...
| **Tecnologie applicabili** | Generico |
| **Attributes (Attributi)**              | N/D |
| **Riferimenti**              | [Firme di accesso condiviso, parte 1: informazioni sul modello di firma di accesso condiviso](../../storage/common/storage-sas-overview.md), [Firme di accesso condiviso, parte 2: creare e usare una firma di accesso condiviso con l'archivio BLOB](../../storage/common/storage-sas-overview.md), [Come delegare l'accesso agli oggetti nell'account usando firme di accesso condiviso e criteri di accesso archiviati](../../storage/blobs/security-recommendations.md#identity-and-access-management) |
