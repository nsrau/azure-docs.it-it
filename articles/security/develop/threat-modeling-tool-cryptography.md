...
ms.locfileid: "68728196"
---
# <a name="security-frame-cryptography--mitigations"></a>Infrastruttura di sicurezza: crittografia - Procedure di mitigazione 

| Prodotto o servizio | Articolo |
| --------------- | ------- |
| **Applicazione Web** | <ul><li>[Utilizzare solo la crittografia a blocchi simmetrica approvata e le lunghezze delle chiavi](#cipher-length)</li><li>[Usare modalità di crittografia a blocchi approvate e vettori di inizializzazione per crittografie simmetriche](#vector-ciphers)</li><li>[Usare riempimenti, lunghezze di chiave e algoritmi asimmetrici approvati](#padding)</li><li>[Usare generatori di numeri casuali approvati](#numgen)</li><li>[Non utilizzare cifrari a flusso simmetrico](#stream-ciphers)</li><li>[Utilizzare algoritmi hash MAC/HMAC/con chiave approvati](#mac-hash)</li><li>[Utilizzare solo funzioni hash crittografiche approvate](#hash-functions)</li></ul> |
| **Database** | <ul><li>[Utilizzare algoritmi di crittografia avanzata per crittografare i dati nel databaseUse strong encryption algorithms to encrypt data in the database](#strong-db)</li><li>[I pacchetti SSIS devono essere crittografati e firmati digitalmente](#ssis-signed)</li><li>[Aggiungere la firma digitale alle entità a protezione diretta dei database criticheAdd digital signature to critical database securables](#securables-db)</li><li>[Usare EKM di SQL Server per proteggere le chiavi di crittografiaUse SQL Server EKM to protect encryption keys](#ekm-keys)</li><li>[Usare la funzionalità AlwaysEncrypted se le chiavi di crittografia non devono essere visualizzate al motore di databaseUse AlwaysEncrypted feature if encryption keys should not be revealed to Database engine](#keys-engine)</li></ul> |
| **Dispositivo IoT** | <ul><li>[Archiviare le chiavi di crittografia in modo sicuro sul dispositivo IoTStore Cryptographic Keys securely on IoT Device](#keys-iot)</li></ul> | 
| **Gateway IoT cloud** | <ul><li>[Generare una chiave simmetrica casuale con lunghezza sufficiente per l'autenticazione nell'hub IoTGenerate a random symmetric key of sufficient length for authentication to IoT Hub](#random-hub)</li></ul> | 
| **Client Dynamics CRM Mobile** | <ul><li>[Verificare che sia presente un criterio di gestione dei dispositivi che richiede un PIN di utilizzo e consenta la cancellazione remota](#pin-remote)</li></ul> | 
| **Client Dynamics CRM per Outlook** | <ul><li>[Verificare che sia presente un criterio di gestione dei dispositivi che richiede un blocco PIN/password/automatico e che crittografa tutti i dati (ad esempio BitLocker)](#bitlocker)</li></ul> | 
| **Identity Server** | <ul><li>[Assicurarsi che venga eseguito il rollover delle chiavi di firma quando si usa Identity Server](#rolled-server)</li><li>[Assicurarsi che id client con crittografia avanzata, segreto client vengano utilizzati in Identity ServerEnsure that cryptographically strong client ID, client secret are used in Identity Server](#client-server)</li></ul> | 
...
| **Attributi**              | N/D  |
| **Riferimenti**              | N/D  |
| **Passi** | Non devono essere usate crittografie di flusso simmetriche, ad esempio RC4. Invece delle crittografie di flusso simmetriche, i prodotti devono usare una crittografia a blocchi, in particolare AES con una lunghezza di chiave di almeno 128 bit. |
...
| **Attributi**              | N/D  |
| **Riferimenti**              | N/D  |
| **Passi** | <p>I prodotti devono usare la famiglia SHA-2 di algoritmi (SHA256, SHA384 e SHA512). Se è necessario un hash più breve, ad esempio una lunghezza di output di 128 bit, per la corrispondenza con una struttura di dati progettata per il più breve hash MD5, i team dei prodotti possono troncare uno degli hash SHA-2 (in genere, SHA256). Si noti che SHA384 è una versione troncata di SHA512. Il troncamento di hash crittografici per motivi di sicurezza a meno di 128 bit non è consentito. Il codice nuovo non deve usare gli algoritmi hash MD2, MD4, MD5, SHA-0, SHA-1 o RIPEMD. Da un punto di vista computazionale, le collisioni di hash sono possibili per questi algoritmi, che li interrompono in modo efficace.</p><p>Algoritmi hash .NET consentiti per la flessibilità crittografica gestita (in ordine di preferenza):</p><ul><li>SHA512Cng (conforme allo standard FIPS)</li><li>SHA384Cng (conforme allo standard FIPS)</li><li>SHA256Cng (conforme allo standard FIPS)</li><li>SHA512Managed (non conforme a FIPS) (utilizzare SHA512 come nome dell'algoritmo nelle chiamate a HashAlgorithm.Create o CryptoConfig.CreateFromName)</li><li>SHA384Managed (non conforme a FIPS) (utilizzare SHA384 come nome dell'algoritmo nelle chiamate a HashAlgorithm.Create o CryptoConfig.CreateFromName)</li><li>SHA256Managed (non conforme a FIPS) (utilizzare SHA256 come nome dell'algoritmo nelle chiamate a HashAlgorithm.Create o CryptoConfig.CreateFromName)</li><li>SHA512CryptoServiceProvider (conforme allo standard FIPS)</li><li>SHA256CryptoServiceProvider (conforme allo standard FIPS)</li><li>SHA384CryptoServiceProvider (conforme allo standard FIPS)</li></ul>| 
...
## <a name="ensure-a-device-management-policy-is-in-place-that-requires-a-pinpasswordauto-lock-and-encrypts-all-data-eg-bitlocker"></a><a id="bitlocker"></a>Verificare che sia presente un criterio di gestione dei dispositivi che richiede un blocco PIN/password/automatico e che crittografa tutti i dati (ad esempio BitLocker)
...
## <a name="ensure-that-cryptographically-strong-client-id-client-secret-are-used-in-identity-server"></a><a id="client-server"></a>Assicurarsi che id client con crittografia avanzata, segreto client vengano utilizzati in Identity ServerEnsure that cryptographically strong client ID, client secret are used in Identity Server
