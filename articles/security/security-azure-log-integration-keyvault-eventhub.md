...
* Versione più recente di PowerShell.
   ...
   Dopo aver creato almeno PowerShell 5.0, è possibile installare la versione più recente seguendo le istruzioni riportate in [installare Azure PowerShell](/powershell/azure/install-az-ps).
...
1. Ottenere tutte le possibili posizioni di Azure e aggiungere i nomi a una variabile che può essere usata in un passaggio successivo:
    ...
    Se si immette `$locations` a questo punto, verranno visualizzati i nomi di località senza le informazioni aggiuntive restituite da Get-AzLocation.
