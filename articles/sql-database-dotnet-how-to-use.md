...
Questa guida mostra come creare un server logico e un'istanza di database sul database SQL di Azure e come effettuare la connessione al
database usando le tecnologie seguenti del provider di dati di .NET Framework:
ADO.NET, ODBC e provider EntityClient.


<h2><a name="Whatis"></a>Informazioni sul database SQL</h2>
...
<h2><a name="PreReq1"></a>Accedere ad Azure</h2>
...
1. Aprire un Web browser e passare al sito [http://www.windowsazure.com](http://www.windowsazure.com). Per iniziare usando un account gratuito, fare clic su Versione di valutazione gratuita nell'angolo in alto a destra e seguire le istruzioni visualizzate.

2. Verrà creato l'account e sarà possibile iniziare.


<h2><a name="PreReq2"></a>Creazione e configurazione di un database SQL</h2>
...
7. Scegliere un'edizione, una dimensione massima e le regole di confronto. Ai fini di questa guida, è possibile usare i valori predefiniti. 
	...
	Il valore MAXSIZE viene specificato alla prima creazione del database e può
essere modificato in seguito tramite ALTER DATABASE. MAXSIZE consente di
limitare le dimensioni del database.

	Per ogni database SQL creato in Azure sono effettivamente disponibili tre
repliche di tale database. Ciò consente di assicurare una disponibilità elevata.
Il failover è trasparente e incluso nel servizio. Il [contratto di
...
<h2><a name="Connect-DB"></a>Connettersi al database SQL</h2>

In questa sezione viene illustrato come effettuare la connessione a un'istanza del database SQL usando diversi provider di dati .NET Framework.

Se si decide di usare Visual Studio e la configurazione non include alcuna applicazione Web di Azure come front-end, non sarà necessario installare strumenti o SDK aggiuntivi nel computer di sviluppo e si potrà iniziare subito a sviluppare l'applicazione.

Per lavorare con un database SQL, è possibile usare in Visual Studio gli stessi strumenti di progettazione disponibili con SQL Server. Esplora server consente di
visualizzare, ma non di modificare, gli oggetti di database. Sono disponibili tutte le funzionalità di Progettazione Entity Data Model di Visual Studio ed è possibile usarlo per creare modelli rispetto al database SQL per l'uso con Entity Framework.

### <a name="using-sql-server"></a>Usare il provider di dati .NET Framework per SQL Server

Lo spazio dei nomi **System.Data.SqlClient** corrisponde al provider di dati .NET Framework
Provider per SQL Server.
...
È possibile usare la classe **SQLConnectionStringBuilder** per creare una stringa di connessione come mostrato nell'esempio di codice seguente:
...
### <a name="using-ODBC"></a>Usare il provider di dati .NET Framework per ODBC
...
Se si vuole creare la stringa di connessione in fase di esecuzione, usare la classe **OdbcConnectionStringBuilder**.

### <a name="using-entity"></a>Usare il provider EntityClient
...
A questo punto, dopo aver appreso le nozioni di base della connessione al database SQL, vedere le risorse seguenti per ottenere altre informazioni sul database SQL.
...
-   [Database SQL][]
  ...
  [Sviluppo: Procedure (Database SQL)]: http://msdn.microsoft.com/it-it/library/windowsazure/ee621787.aspx
