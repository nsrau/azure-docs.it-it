...
I connettori per social network sono "app per le API" che possono connettersi alle applicazioni di social networking ed eseguire diverse *Actions* per conto dell'utente autenticato. La maggior parte dei connettori può anche essere configurata con un *Trigger*. I trigger sono eventi (simili agli eventi in .NET Framework) che possono essere configurati in alcuni connettori. I trigger possono quindi iniziare le azioni che sono state configurate per il connettore (o qualsiasi altro connettore). Ad esempio, un'istanza del connettore Twitter può essere configurata con un trigger *new message*, dove il trigger è definito come qualsiasi nuovo tweet che citi @VisualStudio. Questo trigger potrà quindi essere configurato in modo da iniziare un' *Action* per fare il retweet del tweet. 
...
Di seguito è riportata una breve descrizione di ciascuno dei connettori per social network. 
