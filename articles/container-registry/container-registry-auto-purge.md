...
Il `acr purge` comando è attualmente distribuito in un'immagine del contenitore pubblico disponibile da Microsoft container Registry.
...
* `--filter`: Un repository e un' *espressione regolare* per filtrare i tag nel repository. Esempi: `--filter "hello-world:.*"` `hello-world` corrisponde a tutti i tag nel repository e `--filter "hello-world:^1.*"` corrisponde ai tag che `1`iniziano con. Passare più `--filter` parametri per ripulire più repository.
