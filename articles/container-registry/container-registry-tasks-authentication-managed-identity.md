...
### <a name="1-optional-create-a-user-assigned-identity"></a>1. Opzionale Creare un'identità assegnata dall'utente
...
Per abilitare un'identità assegnata dal sistema, `--assign-identity` passare senza valore o `assign-identity [system]`. Il comando seguente crea un'attività Linux da un repository GitHub pubblico che compila l' `hello-world` immagine con un trigger di commit Git e con un'identità gestita assegnata dal sistema:
...
Per abilitare un'identità assegnata dall'utente, `--assign-identity` passare con un valore dell' *ID risorsa* dell'identità. Il comando seguente crea un'attività Linux da un repository GitHub pubblico che compila l' `hello-world` immagine con un trigger di commit Git e con un'identità gestita assegnata dall'utente:
...
A seconda dei requisiti dell'attività, concedere le autorizzazioni di identità per accedere ad altre risorse di Azure. Ecco alcuni esempi:
...
### <a name="4-optional-add-credentials-to-the-task"></a>4. Opzionale Aggiunta di credenziali all'attività

Se l'attività esegue il pull o il push delle immagini in un altro registro contenitori di Azure, aggiungere le credenziali all'attività per l'autenticazione dell'identità. Eseguire il comando [AZ ACR Task Credential Add][az-acr-task-credential-add] e passare il `--use-identity` parametro per aggiungere le credenziali dell'identità all'attività. 

Ad esempio, per aggiungere le credenziali per un'identità assegnata dal sistema per l'autenticazione con il *targetregistry*del `use-identity [system]`registro di sistema, passare:
