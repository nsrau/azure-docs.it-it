...
- **Ridimensionare in base alle esigenze** : aumentare il numero di istanze in un pool di agenti per le attività a elevato utilizzo di calcolo oppure ridimensionarle a zero. La fatturazione è basata sull'allocazione.
...
Questa funzionalità è disponibile nel livello di servizio del registro contenitori **Premium** . Per informazioni sui limiti e i livelli di servizio del registro di sistema, vedere [sku container Registry di Azure][acr-tiers].
...
- I pool di agenti attività sono disponibili in anteprima nelle aree seguenti: Stati Uniti occidentali 2, Stati Uniti centro-meridionali, Stati Uniti orientali 2 e Stati Uniti orientali.
- Per ogni registro di sistema, la quota predefinita totale vCPU (Core) per tutti i pool di agenti è 16. Aprire una [richiesta di supporto][open-support-ticket] per l'allocazione aggiuntiva.
...
Gli esempi seguenti presuppongono che sia stato impostato il registro di sistema predefinito. In caso contrario, passare `--registry <registryName>` un parametro in `az acr` ogni comando.
...
Il completamento della creazione di un pool di agenti e di altre operazioni di gestione del pool potrebbe richiedere diversi minuti.
...
| Direction | Protocollo | Source (Sorgente)         | Porta di origine | Destination          | Porta dest | Usata    |
|-----------|----------|----------------|-------------|----------------------|-----------|---------|
...
> [!NOTE]
> Se le attività richiedono risorse aggiuntive dalla rete Internet pubblica, aggiungere le regole corrispondenti. Queste regole sono necessarie, ad esempio, se si esegue un'attività di compilazione Docker che deve estrarre le immagini di base dall'hub Docker oppure ripristinare un pacchetto NuGet.
...
### <a name="quick-task-run"></a>Esecuzione rapida attività

Accodare un'esecuzione rapida nel pool di agenti usando il comando [AZ ACR Build][az-acr-build] e passare il `--agent-pool` parametro:
...
Ad esempio, creare un'attività pianificata nel pool di agenti con [AZ ACR task create][az-acr-task-create], passando `--agent-pool` il parametro.
