...
description: Creare un token con autorizzazioni con ambito per repository specifici in un registro per effettuare il pull o il push delle immagini oppure eseguire altre azioni
ms.topic: article
ms.date: 02/13/2020
...
* Questa funzionalità è disponibile nei registri contenitori **Premium**. Per informazioni sui livelli di servizio e sui limiti del registro contenitori, vedere [Livelli di servizio del Registro Azure Container](container-registry-skus.md).
...
* Un **mapping di ambito** raggruppa le autorizzazioni del repository che si applicano a un token e possono essere riapplicate ad altri token. Ogni token è associato a un singolo mapping di ambito. 
  ...
  Registro Azure Container offre anche diversi mapping di ambito definiti dal sistema che è possibile applicare, con autorizzazioni fisse in tutti i repository.
...
Creare un token usando il comando [az acr token create][az-acr-token-create]. Quando si crea un token, è possibile specificare uno o più repository e le azioni associate in ogni repository. Non è necessario che i repository siano già nel registro. Per creare un token specificando un mapping di ambito esistente, vedere la sezione successiva.
...
L'output visualizza i dettagli sul token, incluse due password generate. È consigliabile salvare le password in un luogo sicuro per usarle in un secondo momento per l'autenticazione. Le password non possono essere nuovamente recuperate, ma è possibile generarne di nuove.
...
1. In **Servizi** selezionare **Token (anteprima) > +Aggiungi**.
  ![Creare il token nel portale](media/container-registry-repository-scoped-permissions/portal-token-add.png)
...
1. Configurare il mapping di ambito:
    ...
    ![Creare un mapping di ambito nel portale](media/container-registry-repository-scoped-permissions/portal-scope-map-add.png)
...
Generare una password dopo aver creato un token. Per eseguire l'autenticazione con il registro, il token deve essere abilitato e avere una password valida.
...
1. Nella schermata della password, se necessario impostare una data di scadenza per la password e selezionare **Genera**.
1. Dopo aver generato una password, copiarla e salvarla in una posizione sicura. Non è possibile recuperare una password generata dopo aver chiuso la schermata, ma è possibile generarne una nuova.
    ...
    ![Creare la password del token nel portale](media/container-registry-repository-scoped-permissions/portal-token-password.png)

## <a name="authenticate-with-token"></a>Eseguire l'autenticazione con un token

Quando un utente o un servizio usa un token per l'autenticazione con il registro di destinazione, specifica il nome del token come nome utente e una delle password generate. Il metodo di autenticazione dipende dall'azione o dalle azioni configurate associate al token.
...
Eseguire `docker login` per l'autenticazione con il registro di sistema, specificare il nome del token come nome utente e specificare una delle password. Lo stato del token deve essere `Enabled`.
...
### <a name="change-pushpull-permissions"></a>Modificare le autorizzazioni di push/pull
...
1. In **Servizi** selezionare **Mapping di ambito (anteprima)** e selezionare il mapping di ambito da aggiornare.
...
Per aggiornare il mapping di ambito usando il portale, vedere la sezione precedente.

Usare il seguente comando [az acr repository delete][az-acr-repository-delete] per eliminare il repository `samples/alpine`. Per eliminare immagini o repository, il token non esegue l'autenticazione con `docker login`. Passare il nome e la password del token al comando. L'esempio seguente usa le variabili di ambiente create in precedenza nell'articolo:
...
Per leggere i metadati, il token non esegue l'autenticazione con `docker login`. Passare il nome e la password del token a uno dei comandi. L'esempio seguente usa le variabili di ambiente create in precedenza nell'articolo:
...
L'output indica i mapping di ambito definiti dall'utente e diversi mapping di ambito definiti dal sistema che è possibile usare per configurare i token:
...
### <a name="generate-passwords-for-token"></a>Generare le password per il token
