...
description: Informazioni su come usare i webhook per attivare eventi quando si verificano azioni push o pull nei repository del registro di sistema.
ms.topic: article
ms.date: 05/24/2019
...
* INTERFACCIA della riga di comando di Docker: per configurare il computer locale come host Docker e accedere ai comandi dell'interfaccia della riga di comando di Docker, installare il [motore Docker](https://docs.docker.com/engine/installation/).
...
| Scope | L'ambito in cui viene applicato il webhook. Se non specificato, l'ambito è per tutti gli eventi nel registro di sistema. Può essere specificato per un repository o un tag usando il formato "repository: Tag" o "repository: *" per tutti i tag in un repository. |
