...
[Windows Store C#][Windows Store C#][Windows Store JavaScript][Windows Store JavaScript]
...
5.  Fare clic sulla scheda **Script** e sostituire il codice esistente con quello riportato di seguito:
    ...
    **Nota**
    Lo script di questa API personalizzata usa il [modulo wns][modulo wns] di Node.js, al quale viene fatto riferimento mediante la funzione **require**. Questo modulo è diverso rispetto all'[oggetto wns][oggetto wns] restituito dall'[oggetto push][oggetto push], che viene usato per l'invio di notifiche push da script del server.
...
3.  In Visual Studio espandere la cartella `\js` in Esplora soluzioni, aprire il progetto default.js, quindi aggiungere le righe di codice seguenti dopo il codice che definisce la variabile **client**:
...
<!-- Anchors. --> <!-- Images. --> <!-- URLs. -->
  ...
  [Introduzione ai dati]: /it-it/develop/mobile/tutorials/started-with-data-js
  ...
  [modulo wns]: http://go.microsoft.com/fwlink/p/?LinkId=306750
