...
## Power BI Embedded: che cos'è e a cosa serve
...
## Creare una raccolta di aree di lavoro di Power BI e ottenere la chiave di accesso (provisioning)
Power BI Embedded è uno dei servizi di Azure. Le spese di utilizzo vengono addebitate solo ai fornitori di software indipendenti che usano il portale di Azure (per sessione utente oraria), mentre l'utente che visualizza il report non riceve addebiti e non necessita di una sottoscrizione di Azure. Prima di iniziare a sviluppare applicazioni, è necessario creare la **raccolta di aree di lavoro di Power BI** tramite il portale di Azure.
...
## Importare file con estensione pbix nell'area di lavoro
...
## Connettività dell'origine dati (e multi-tenancy dei dati)
...
## Autenticazione e hosting (incorporamento) dei report nella pagina Web
...
Successivamente, è necessario creare la stringa con codifica base64 del codice HMAC (la firma) con l'algoritmo SHA256. Questo valore di input firmato è la stringa precedente.
...
## Incorporare il report nella pagina Web
...
È possibile incorporare il report nell'applicazione Web usando il token dell'applicazione precedente. Se si esamina il codice di esempio successivo, la prima parte corrisponde all'esempio precedente. Nella seconda parte **embedUrl** (vedere il risultato precedente) si trova nell'iframe e il token dell'applicazione viene pubblicato nell'iframe.
