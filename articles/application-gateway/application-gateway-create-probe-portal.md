...
* **Host** : nome host usato per il probe. Applicabile solo quando vengono configurati più siti nel gateway applicazione. In caso contrario, usare "127.0.0.1". Questo nome è diverso dal nome host della macchina virtuale.
...
* **Soglia di non integrità** : numero di tentativi non riusciti da considerare come uno stato di non integrità. Se la soglia è 0 e un controllo integrità ha esito negativo, il back-end verrà immediatamente considerato non integro.

> [!IMPORTANT]
> Il nome host non è il nome del server. È il nome dell'host virtuale in esecuzione nel server applicazioni. Il probe viene inviato a http://(name host):(porta da impostazioni HTTP)/percorsoURL
> 
> 
