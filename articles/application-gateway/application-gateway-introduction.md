...
- [Routing multisito](application-gateway-multi-site-overview.md)

## Bilanciamento del carico di livello 7 HTTP

Azure fornisce il bilanciamento del carico di livello 4 tramite Azure Load Balancer con funzionamento a livello di trasporto (TCP/UDP) e bilanciamento del carico di tutto il traffico di rete in ingresso al servizio gateway applicazione. Il gateway applicazione applica le regole di routing al traffico HTTP, fornendo il bilanciamento del carico di livello 7 (HTTP). Quando si crea un gateway applicazione, un endpoint (indirizzo VIP) viene associato e usato come IP pubblico per il traffico di rete in ingresso.

Il gateway applicazione instrada il traffico HTTP in base alla relativa configurazione, sia che si tratti di una macchina virtuale, di un servizio cloud, di un'app Web o di un indirizzo IP esterno.
