...
Questo articolo descrive come usare Azure PowerShell per configurare un'istanza di [SKU del gateway applicazione V2](./application-gateway-autoscaling-zone-redundant.md) per riscrivere le intestazioni HTTP in richieste e risposte.
...
- È necessario disporre di un'istanza di SKU del gateway applicazione V2. La riscrittura delle intestazioni non è supportata nello SKU V1. Se non si ha lo SKU V2, creare un'istanza di [SKU del gateway applicazione V2](./tutorial-autoscale-ps.md) prima di iniziare.
