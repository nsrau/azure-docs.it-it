<properties urlDisplayName="" pageTitle="Sviluppare programmi per la creazione di flussi Hadoop in C# per HDInsight | Azure" metaKeywords="hdinsight hdinsight development, hadoop development, hdinsight deployment, development, deployment, tutorial, MapReduce" description="Informazioni sullo sviluppo di programmi MapReduce per la creazione di flussi Hadoop in C# e sulla distribuzione in Azure HDInsight." metaCanonical="" services="hdinsight" documentationCenter="" title="Sviluppare programmi per la creazione di flussi Hadoop in C# per HDInsight" authors="jgao" solutions="" manager="paulettm" editor="cgronlun" />
...
10. Fare clic su **COMPILA** e quindi su **Compila soluzione** per compilare il programma Reducer.
...
Per impostazione predefinita, HDInsight Emulator usa HDFS come file system predefinito. Se si desidera, è possibile configurare HDInsight Emulator per l'uso dell'archivio BLOB di Azure. Per informazioni dettagliate, vedere [Introduzione all'uso di HDInsight Emulator][1]. In questa sezione verrà usato il comando *copyFromLocal* di HDFS per il caricamento dei file. Nella sezione successiva verrà illustrato come caricare i file usando Azure PowerShell. Per informazioni sugli altri metodi disponibili, vedere [Caricare i dati in HDInsight][Caricare i dati in HDInsight].
...
In questa esercitazione vengono usati i file con estensione txt disponibili nella directory %hadoop\_home%.
...
2.  Eseguire i comandi seguenti per ottenere un elenco dei file con estensione txt disponibili nella cartella di file di origine:

        # Get a list of the txt files
        $filesAll = Get-ChildItem $localFolder
        $filesTxt = $filesAll | where {$_.Extension -eq ".txt"}

3.  Eseguire il frammento seguente per copiare i file:
...
4.  Eseguire il comando seguente per ottenere un elenco dei file caricati:
...
8.  Quando richiesto, immettere nome utente e password per il cluster HDInsight. Assicurarsi che la password sia almeno di 10 caratteri, inclusi una lettera maiuscola, una lettera minuscola, un numero e un carattere speciale. Se non si desidera che venga richiesta l'immissione di credenziali, vedere [Uso di password, stringhe sicure e credenziali in Windows PowerShell][Uso di password, stringhe sicure e credenziali in Windows PowerShell]
