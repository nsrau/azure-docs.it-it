...
# Utilizzo dell'archiviazione BLOB di Azure con Hadoop in HDInsight
...
> [WACOM.NOTE] La sintassi *asv://* non è supportata nei cluster HDInsight della versione 3.0 e non sarà supportata nelle versioni successive. Ciò vuol dire che i processi inviati a un cluster HDInsight versione 3.0 che utilizzano esplicitamente la sintassi "asv://" avranno esito negativo. Sarà pertanto necessario utilizzare la sintassi *wasb://*. Avrà inoltre esito negativo qualsiasi processo inviato a un cluster HDInsight versione 3.0 e creato con un metastore esistente che contiene riferimenti espliciti a risorse che utilizzano la sintassi asv://. Sarà necessario ricreare i metastore in modo da fare riferimento alle risorse utilizzando la sintassi wasb://.
...
-   Utilizzare l'archiviazione da un'altra sottoscrizione
...
> [WACOM.NOTE] La sintassi per fare riferimento ai file nell'emulatore di archiviazione eseguito in HDInsight Emulator è *wasb://\<NomeContenitore\>@storageemulator*.
...
Se non viene specificato \<BlobStorageContainerName\> né \<StorageAccountName\>, verrà utilizzato il file system predefinito. Per i file nel file system predefinito è possibile utilizzare un percorso relativo o un percorso assoluto. Ad esempio, è possibile fare riferimento al file hadoop-mapreduce-examples.jar incluso nei cluster HDInsight nei due modi seguenti:
...
Per informazioni sull'installazione e la configurazione di Azure PowerShell nella workstation, vedere [Come installare e configurare Azure PowerShell][È possibile utilizzare]. È possibile utilizzare la finestra della console Azure PowerShell oppurePowerShell\_ISE per l'esecuzione di cmdlet di PowerShell.
...
-   [Utilizzo di Pig con HDInsight][Utilizzo di Pig con HDInsight]
  ...
  [È possibile utilizzare]: ../install-configure-powershell/
  [//\<NomeContenitore\>@storageemulator]: mailto://<ContainerName>@storageemulator
