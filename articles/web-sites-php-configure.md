<properties 
	pageTitle="Come configurare PHP in Siti Web di Azure" 
	description="Informazioni su come configurare l'installazione predefinita di PHP o aggiungere un'installazione personalizzata di PHP in Siti Web di Azure." 
	services="" 
...
1. Nel portale Azure fare clic su **Configura**. sul dashboard del proprio sito Web.
...
1. Aggiungere le impostazioni di configurazione al file  `.user.ini` usando la stessa sintassi che si userebbe in un file  `php.ini`. Ad esempio, se si desidera attivare l'impostazione  `display_errors` e configurare l'impostazione  `upload_max_filesize` su 10M, il file  `.user.ini` conterrà il testo seguente:
...
1. Distribuire l'applicazione.
1. Passare al dashboard del proprio sito Web nel portale Azure e fare clic su **Configura**.
...
1. Nella sezione **Mapping gestori** aggiungere `*.php` a EXTENSION, quindi aggiungere il percorso al file eseguibile  `php-cgi.exe`. Se si inserisce il proprio runtime PHP nella directory  `bin` nella radice dell'applicazione,il relativo percorso sarà  `D:\home\site\wwwroot\bin\php\php-cgi.exe`.
...
[Configurare, monitorare e scalare i siti Web in Azure]: http://azure.microsoft.com/manage/services/web-sites/
[Download di Azure SDK per PHP]: http://azure.microsoft.com/develop/php/common-tasks/download-php-sdk/
