...
__Cosa:__ Consente di fare riferimento a un file remoto pacchetto (zip) che contiene l'app per le funzioni. Il file viene scaricato dall'URL specificato e l'app viene eseguita [Run-da-Package](run-functions-from-deployment-package.md) modalità.
...
__Cosa:__ Consente di eseguire il push di un file zip contenente l'app per le funzioni in Azure. Facoltativamente, è inoltre possibile specificare per avviare l'app [Run-da-Package](run-functions-from-deployment-package.md) modalità.
...
__Cosa:__ Specificare un'immagine del contenitore che verrà pull ed eseguire funzioni di Azure.
...
__Cosa:__ I pacchetti e distribuisce le applicazioni di Windows con un server IIS - tra cui l'App per le funzioni di Azure in esecuzione su Windows.
...
__Cosa:__ Consente di associare l'app per le funzioni fino a un repository git in modo che tutti gli aggiornamenti al codice in tale repository attiva la distribuzione. Per altre informazioni, esaminare i [Kudu Wiki](https://github.com/projectkudu/kudu/wiki/VSTS-vs-Kudu-deployments).
...
__Cosa:__ Consente il push di codice dal computer locale le funzioni di Azure tramite Git.
...
__Cosa:__ Consente di sincronizzare il contenuto da OneDrive e Dropbox a funzioni di Azure.
...
__Cosa:__ È possibile direttamente i file in funzioni di Azure.
...
__Cosa:__ Usando l'editor basato sul portale consente di modificare direttamente i file nella tua app di funzione (distribuisce sostanzialmente ogni volta che si fa clic su **salvare**).
...
#### <a name="portal-editing-availability"></a>Portale di modifica di disponibilità
...
<sup>1</sup> portale modifica è abilitata per solo i trigger di Timer e HTTP per le funzioni in Linux usando il piano dedicato.
...
## <a name="other-relevant-information"></a>Altre informazioni rilevanti
...
> [recapito continuo con Azure DevOps](functions-how-to-azure-devops.md)
> [comprimere le distribuzioni per funzioni di Azure](deployment-zip-push.md) 
