...
| Sezione | Attività |
| ------- | ----- |
| **autenticazione** | <ol><li>Scaricare un profilo di pubblicazione o definire un'entità servizio.</li><li>Creare un segreto GitHub.</li></ol>|
...
## <a name="downloading-and-using-a-publish-profile-as-deployment-credential-recommended"></a>Download e utilizzo di un profilo di pubblicazione come credenziale di distribuzione (scelta consigliata)
...
## <a name="create-a-service-principal-deprecated"></a>Creare un'entità servizio (deprecata)
=======
...
Il metodo consigliato per l'autenticazione con funzioni di Azure per le azioni di GitHub è con un profilo di pubblicazione. È anche possibile eseguire l'autenticazione con un'entità servizio, ma il processo richiede più passaggi. 

## <a name="configure-the-github-secret"></a>Configurare il segreto di GitHub
= = = = = = = Salva le credenziali del profilo di pubblicazione o l'entità servizio come [segreto GitHub](https://docs.github.com/en/actions/reference/encrypted-secrets) per l'autenticazione con Azure. Si accederà al segreto all'interno del flusso di lavoro. 
...
È possibile creare un' [entità servizio](../active-directory/develop/app-objects-and-service-principals.md#service-principal-object) usando il comando [AZ ad SP create-for-RBAC](/cli/azure/ad/sp?view=azure-cli-latest#az-ad-sp-create-for-rbac&preserve-view=true) per l' [interfaccia](/cli/azure/)della riga di comando di Azure. Eseguire questo comando usando [Azure cloud Shell](https://shell.azure.com) nel portale di Azure o selezionando il pulsante **prova** .
...
1. Aggiungere un nuovo segreto.
   ...
   * Se si usa un profilo di pubblicazione, usare `AZURE_FUNCTIONAPP_PUBLISH_PROFILE` come **nome**. Usare quindi il contenuto del file del profilo di pubblicazione per **valore**e selezionare **Aggiungi segreto**.
...
### <a name="publish-profile-deploy"></a>Distribuzione del profilo di pubblicazione

Negli esempi seguenti viene utilizzata la versione 1 di `functions-action` e una `publish profile` per l'autenticazione di:
...
### <a name="service-principal-deploy"></a>Distribuzione dell'entità servizio

Nell'esempio seguente viene utilizzata la versione 1 di `functions-action` e una `service principal` per l'autenticazione. Il flusso di lavoro configura un ambiente Windows .NET. 

# <a name="net"></a>[.NET](#tab/dotnet)

Configurare un flusso di lavoro .NET Linux che usa un'entità servizio.
...
Configurare un flusso di lavoro di Windows .NET che usa un'entità servizio.
...
Configurare un flusso di lavoro Java Linux che usa un'entità servizio.
...
Configurare un flusso di lavoro di Windows Java che usa un'entità servizio.
...
Configurare un flusso di lavoro di Node.JS Linux che usa un'entità servizio.
...
Configurare un flusso di lavoro di Node.JS Windows che usa un'entità servizio.
...
Configurare un flusso di lavoro Python Linux che usa un'entità servizio.
...
> [Altre informazioni sull'integrazione di Azure e GitHub](https://docs.microsoft.com/azure/developer/github/)
