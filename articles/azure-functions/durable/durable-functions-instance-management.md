...
* (facoltativo): input della funzione, inline o tramite un file JSON. ** `input` ** Per i file, aggiungere un prefisso al percorso del file con `@`, ad esempio. `@path/to/file.json`
* (facoltativo): ID dell'istanza dell'orchestrazione. ** `id` ** Se non si specifica questo parametro, il comando usa un GUID casuale.
* (facoltativo): nome dell'impostazione dell'applicazione che contiene la stringa di connessione di archiviazione da usare. ** `connection-string-setting` ** Il valore predefinito è AzureWebJobsStorage.
* (facoltativo): nome del Durable Functions Hub attività da usare. ** `task-hub-name` ** Il valore predefinito è DurableFunctionsHub. È anche possibile impostare questa impostazione in [host. JSON](durable-functions-bindings.md#host-json) usando DurableTask: HubName.
...
* **`showHistory`**: Se è impostato `true`su, la risposta contiene la cronologia di esecuzione.
* **`showHistoryOutput`**: Se è impostato `true`su, la cronologia di esecuzione contiene gli output delle attività.
* **`showInput`**: Se è impostato `false`su, la risposta non conterrà l'input della funzione. Il valore predefinito è `true`.
...
* (facoltativo): se impostato su `true`, la risposta contiene l'input della funzione. ** `show-input` ** Il valore predefinito è `false`.
* (facoltativo): se impostato su `true`, la risposta contiene l'output della funzione. ** `show-output` ** Il valore predefinito è `false`.
* (facoltativo): nome dell'impostazione dell'applicazione che contiene la stringa di connessione di archiviazione da usare. ** `connection-string-setting` ** Il valore predefinito è `AzureWebJobsStorage`.
* (facoltativo): nome del Durable Functions Hub attività da usare. ** `task-hub-name` ** Il valore predefinito è `DurableFunctionsHub`. Può anche essere impostato in [host. JSON](durable-functions-bindings.md#host-json), usando DurableTask: HubName.
...
* (obbligatorio): ID dell'istanza dell'orchestrazione. ** `id` **
* (facoltativo): nome dell'impostazione dell'applicazione che contiene la stringa di connessione di archiviazione da usare. ** `connection-string-setting` ** Il valore predefinito è `AzureWebJobsStorage`.
* (facoltativo): nome del Durable Functions Hub attività da usare. ** `task-hub-name` ** Il valore predefinito è `DurableFunctionsHub`. Può anche essere impostato in host. JSON, usando durableTask: HubName.
...
* (facoltativo): token per indicare la pagina o la sezione di istanze da recuperare. ** `continuation-token` ** Ogni esecuzione `get-instances` restituisce un token per il set successivo di istanze.
* (facoltativo): nome dell'impostazione dell'applicazione che contiene la stringa di connessione di archiviazione da usare. ** `connection-string-setting` ** Il valore predefinito è `AzureWebJobsStorage`.
* (facoltativo): nome del Durable Functions Hub attività da usare. ** `task-hub-name` ** Il valore predefinito è `DurableFunctionsHub`. Può anche essere impostato in [host. JSON](durable-functions-bindings.md#host-json), usando DurableTask: HubName.
...
Nel Azure Functions Core Tools è anche possibile usare il comando con `durable get-instances` i filtri. Oltre ai parametri,, `top`e `continuation-token` `connection-string-setting` `task-hub-name` sopra indicati, è possibile usare tre parametri di filtro (`created-after`, `created-before`e `runtime-status`).

* (facoltativo): recuperare le istanze create dopo questa data/ora (UTC). ** `created-after` ** Datetime in formato ISO 8601 accettati.
* (facoltativo): recuperare le istanze create prima della data/ora (UTC). ** `created-before` ** Datetime in formato ISO 8601 accettati.
* (facoltativo): recuperare le istanze con un determinato stato (ad esempio, in esecuzione o completata). ** `runtime-status` ** Può fornire più stati (separati da spazi).
* (facoltativo): numero di istanze recuperate per ogni richiesta. ** `top` ** Il valore predefinito è 10.
* (facoltativo): token per indicare la pagina o la sezione di istanze da recuperare. ** `continuation-token` ** Ogni esecuzione `get-instances` restituisce un token per il set successivo di istanze.
* (facoltativo): nome dell'impostazione dell'applicazione che contiene la stringa di connessione di archiviazione da usare. ** `connection-string-setting` ** Il valore predefinito è `AzureWebJobsStorage`.
* (facoltativo): nome del Durable Functions Hub attività da usare. ** `task-hub-name` ** Il valore predefinito è `DurableFunctionsHub`. Può anche essere impostato in [host. JSON](durable-functions-bindings.md#host-json), usando DurableTask: HubName.
...
* (obbligatorio): ID dell'istanza di orchestrazione da terminare. ** `id` **
* (facoltativo): motivo della terminazione. ** `reason` **
* (facoltativo): nome dell'impostazione dell'applicazione che contiene la stringa di connessione di archiviazione da usare. ** `connection-string-setting` ** Il valore predefinito è `AzureWebJobsStorage`.
* (facoltativo): nome del Durable Functions Hub attività da usare. ** `task-hub-name` ** Il valore predefinito è `DurableFunctionsHub`. Può anche essere impostato in [host. JSON](durable-functions-bindings.md#host-json), usando DurableTask: HubName.
...
Inviare notifiche degli eventi alle istanze in esecuzione utilizzando `RaiseEventAsync` il metodo (.NET) o `raiseEvent` il metodo (JavaScript) dell' [associazione del client di orchestrazione](durable-functions-bindings.md#orchestration-client). Le istanze in grado di gestire questi eventi sono quelle in attesa di una chiamata `WaitForExternalEvent` a (.NET) o il cedimento `waitForExternalEvent` a una chiamata (JavaScript).
...
* (facoltativo): dati da inviare all'istanza dell'orchestrazione. ** `event-data` ** Questo può essere il percorso di un file JSON oppure è possibile fornire i dati direttamente nella riga di comando.
* (facoltativo): nome dell'impostazione dell'applicazione che contiene la stringa di connessione di archiviazione da usare. ** `connection-string-setting` ** Il valore predefinito è `AzureWebJobsStorage`.
* (facoltativo): nome del Durable Functions Hub attività da usare. ** `task-hub-name` ** Il valore predefinito è `DurableFunctionsHub`. Può anche essere impostato in [host. JSON](durable-functions-bindings.md#host-json), usando DurableTask: HubName.
...
Il `WaitForCompletionOrCreateCheckStatusResponseAsync` metodo (.NET) o `waitForCompletionOrCreateCheckStatusResponse` (JavaScript) può essere utilizzato per ottenere l'output effettivo da un'istanza di orchestrazione in modo sincrono. Per impostazione predefinita, questi metodi utilizzano un valore predefinito di 10 secondi `timeout`per e 1 secondo per `retryInterval`.  
...
* (obbligatorio): ID dell'istanza dell'orchestrazione. ** `id` **
* (facoltativo): motivo per la rimozione dell'istanza di orchestrazione. ** `reason` **
* (facoltativo): nome dell'impostazione dell'applicazione che contiene la stringa di connessione di archiviazione da usare. ** `connection-string-setting` ** Il valore predefinito è `AzureWebJobsStorage`.
* (facoltativo): nome del Durable Functions Hub attività da usare. ** `task-hub-name` ** Per impostazione predefinita, viene usato il nome dell'hub attività nel file [host. JSON](durable-functions-bindings.md#host-json) .
...
* (facoltativo): ripulire la cronologia delle istanze create dopo questa data/ora (UTC). ** `created-after` ** Datetime in formato ISO 8601 accettati.
* (facoltativo): ripulire la cronologia delle istanze create prima della data/ora (UTC). ** `created-before` ** Datetime in formato ISO 8601 accettati.
* (facoltativo): ripulire la cronologia delle istanze con un determinato stato (ad esempio, in esecuzione o completata). ** `runtime-status` ** Può fornire più stati (separati da spazi).
* (facoltativo): nome dell'impostazione dell'applicazione che contiene la stringa di connessione di archiviazione da usare. ** `connection-string-setting` ** Il valore predefinito è `AzureWebJobsStorage`.
* (facoltativo): nome del Durable Functions Hub attività da usare. ** `task-hub-name` ** Per impostazione predefinita, viene usato il nome dell'hub attività nel file [host. JSON](durable-functions-bindings.md#host-json) .
...
Il comando seguente elimina tutti i dati di archiviazione di Azure `UserTest` associati all'hub attività.
