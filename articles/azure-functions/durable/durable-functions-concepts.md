---
title: Modelli e concetti tecnici per Durable Functions - Azure
description: Informazioni dettagliate sul funzionamento di Durable Functions in Azure per ottenere i vantaggi dell'esecuzione del codice con stato nel cloud.
services: functions
author: kashimiz
manager: jeconnoc
keywords: ''
...
ms.locfileid: "56302040"
---
# <a name="durable-functions-patterns-and-technical-concepts"></a>Modelli e concetti tecnici per Durable Functions

*Funzioni permanenti* è un'estensione di [Funzioni di Azure](../functions-overview.md) e [Processi Web di Azure](../../app-service/web-sites-create-web-jobs.md) che consente di scrivere funzioni con stato in un ambiente senza server. L'estensione gestisce automaticamente lo stato, i checkpoint e i riavvii. Questo articolo include informazioni più dettagliate sui comportamenti dell'estensione Durable Functions per Funzioni di Azure e sui modelli comuni di implementazione.
...
Questa sezione descrive alcuni modelli di applicazione tipici che possono trarre vantaggio da Durable Functions.
...
![Diagramma di concatenamento funzioni](./media/durable-functions-concepts/function-chaining.png)
...
Il parametro `context` [DurableOrchestrationContext] \(.NET\) e l'oggetto `context.df` (JavaScript) forniscono metodi per richiamare altre funzioni tramite il nome, il passaggio di parametri e la restituzione dell'output della funzione. Ogni volta che il codice chiama `await` (C#) o `yield` (JavaScript), Durable Functions Framework imposta *checkpoint* sullo stato di avanzamento dell'istanza della funzione corrente. Se il processo o la macchina virtuale vengono riciclati durante l'esecuzione, l'istanza della funzione riprende dalla chiamata `await` o `yield` precedente. Altre informazioni su questo comportamento di riavvio sono disponibili più avanti.

> [!NOTE]
> L'oggetto `context` in JavaScript rappresenta il [contesto funzione](../functions-reference-node.md#context-object) nel suo complesso e non [DurableOrchestrationContext].
...
![Diagramma di fan-out/fan-in](./media/durable-functions-concepts/fan-out-fan-in.png)
...
Le operazioni di fan-out vengono distribuite in più istanze della funzione `F2` e vengono monitorate tramite un elenco dinamico di attività. Viene chiamata l'API .NET `Task.WhenAll` o l'API JavaScript `context.df.Task.all` per attendere il completamento di tutte le funzioni chiamate. Quindi, gli output della funzione `F2` vengono aggregati dall'elenco di attività dinamiche e passati alla funzione `F3`.

L'impostazione automatica di checkpoint che avviene alla chiamata di `await` o `yield` su `Task.WhenAll` o `context.df.Task.all` assicura che qualsiasi riavvio o arresto anomalo del sistema durante l'esecuzione non richieda il riavvio di attività già completate.
...
![Diagramma API HTTP](./media/durable-functions-concepts/async-http-api.png)

Funzioni permanenti integra API incorporate che semplificano il codice da scrivere per l'interazione con l'esecuzione di funzioni a esecuzione prolungata. Gli esempi dell'avvio rapido ([C#](durable-functions-create-first-csharp.md), [JavaScript](quickstart-js-vscode.md)) mostrano un semplice comando REST che può essere usato per avviare nuove istanze di funzione agente di orchestrazione. Dopo l'avvio di un'istanza, l'estensione espone API HTTP webhook che eseguono query sullo stato della funzione di orchestrazione. L'esempio seguente mostra i comandi REST per avviare un agente di orchestrazione e per eseguire una query sul relativo stato. Per maggiore chiarezza, alcuni dettagli sono omessi dall'esempio.
...
![Diagramma di monitoraggio](./media/durable-functions-concepts/monitor.png)
...
![Diagramma di interazione umana](./media/durable-functions-concepts/approval.png)
...
Il timer permanente viene creato chiamando `context.CreateTimer` (.NET) o `context.df.createTimer` (JavaScript). La notifica viene ricevuta da `context.WaitForExternalEvent` (.NET) o `context.df.waitForExternalEvent` (JavaScript). Viene chiamato `Task.WhenAny` (.NET) o `context.df.Task.any` (JavaScript) per decidere se eseguire l'escalation (si verifica prima il timeout) o elaborare l'approvazione (l'approvazione viene ricevuta prima del timeout).
...
Ecco un esempio di come si presentano gli eventi di rilevamento nel portale Application Insights usando [Application Insights Analytics](../../application-insights/app-insights-analytics.md):

![Risultati della query App Insights](./media/durable-functions-concepts/app-insights-1.png)
...
![Campo customDimensions nella query App Insights](./media/durable-functions-concepts/app-insights-2.png)
...
Per altre informazioni sulle Funzioni permanenti, vedere [Panoramica dei tipi di funzioni e funzionalità per Durable Functions (Funzioni di Azure)](durable-functions-types-features-overview.md), oppure
