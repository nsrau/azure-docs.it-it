...
L' `@BlobTrigger` attributo viene usato per fornire l'accesso al BLOB che ha attivato la funzione. Per informazioni dettagliate, vedere l' [esempio di trigger](#trigger---example) .
...
Accedere ai dati BLOB `context.bindings.<name from function.json>`tramite.
...
L' `@BlobInput` attributo consente di accedere al BLOB che ha attivato la funzione. Se si usa una matrice di byte con l'attributo, `dataType` impostare `binary`su. Per informazioni dettagliate, vedere l' [esempio di input](#input---example) .
...
L' `@BlobOutput` attributo consente di accedere al BLOB che ha attivato la funzione. Se si usa una matrice di byte con l'attributo, `dataType` impostare `binary`su. Per informazioni dettagliate, vedere l' [esempio di output](#output---example) .
