...
I modelli di hosting hanno diversi livelli di isolamento della rete disponibili. La scelta di quella corretta ti aiuterà a soddisfare i requisiti di isolamento rete.

È possibile ospitare le app per le funzioni in due modi:You can host function apps in a couple ways:

* È disponibile un set di opzioni di piano che vengono eseguite in un'infrastruttura multi-tenant, con vari livelli di connettività di rete virtuale e opzioni di scalabilità:There's a set of plan options that run on a multi-tenant infrastructure, with various levels of virtual network connectivity and scaling options:
    * Il [piano Consumo](functions-scale.md#consumption-plan), che viene ridimensionato dinamicamente in risposta al carico e offre opzioni di isolamento rete minime.
    * Il [piano Premium](functions-scale.md#premium-plan), che si adatta anche in modo dinamico, offrendo al contempo un isolamento della rete più completo.
    * Il [piano di servizio app](functions-scale.md#app-service-plan)di Azure , che opera su scala fissa e offre un isolamento di rete simile a quello del piano Premium.
...
|----------------|-----------|----------------|---------|-----------------------|  
|[Restrizioni IP in ingresso & l'accesso al sito privato](#inbound-ip-restrictions)|✅Sì|✅Sì|✅Sì|✅Sì|
...
L'integrazione della rete virtuale consente all'app per le funzioni di accedere alle risorse all'interno di una rete virtuale. Funzioni di Azure supporta due tipi di integrazione della rete virtuale:Azure Functions supports two kinds of virtual network integration:
...
## <a name="connecting-to-service-endpoint-secured-resources"></a>Connessione alle risorse protette dell'endpoint del servizioConnecting to service endpoint secured resources
...
[Altre informazioni sugli endpoint del servizio di rete virtuale.](../virtual-network/virtual-network-service-endpoints-overview.md)
...
## <a name="using-key-vault-references"></a>Utilizzo dei riferimenti dell'insieme di credenziali delle chiavi 

I riferimenti dell'insieme di credenziali delle chiavi consentono di usare i segreti dall'insieme di credenziali delle chiavi di Azure nell'applicazione Funzioni di Azure senza richiedere alcuna modifica del codice. Azure Key Vault è un servizio che supporta la gestione centralizzata dei segreti con controllo completo sui criteri di accesso e sulla cronologia di controllo.

Attualmente [i riferimenti dell'insieme](../app-service/app-service-key-vault-references.md) di credenziali delle chiavi non funzionano se l'insieme di credenziali delle chiavi è protetto con endpoint di servizio. Per connettersi a un key Vault utilizzando l'integrazione di rete virtuale è necessario chiamare l'insieme di credenziali delle chiavi nel codice dell'applicazione.
...
Quando si esegue un piano Premium, è possibile connettere funzioni trigger non HTTP ai servizi in esecuzione all'interno di una rete virtuale. A tale scopo, è necessario abilitare il supporto dei trigger di rete virtuale per l'app per le funzioni. L'impostazione **di supporto dei trigger** di rete virtuale si trova nel portale di [Azure](https://portal.azure.com) in Impostazioni **app per**le funzioni.

![VNETToggle (Attiva opzione VNET)](media/functions-networking-options/virtual-network-trigger-toggle.png)

È anche possibile abilitare i trigger di rete virtuale usando il comando dell'interfaccia della riga di comando di Azure seguente:You can also enable virtual network triggers using the following Azure CLI command:
...
> [!IMPORTANT]
> Quando si abilita il supporto dei trigger di rete virtuale, solo i tipi di trigger precedenti vengono ridimensionati in modo dinamico con l'applicazione. È comunque possibile utilizzare trigger non elencati sopra, tuttavia non vengono ridimensionati oltre il numero di istanze preriscaldate. Vedere [trigger e associazioni](./functions-triggers-bindings.md#supported-bindings) per l'elenco completo dei trigger.
...
Si supponga, ad esempio, di voler configurare il database Cosmos di Azure per accettare il traffico solo da una rete virtuale. In questo caso, è necessario distribuire l'app per le funzioni in un piano di servizio app che fornisce l'integrazione della rete virtuale con tale rete virtuale. In questo modo una funzione può essere attivata dalla risorsa di database Cosmos di Azure.This enables a function to be triggered by that Azure Cosmos DB resource. 

## <a name="hybrid-connections"></a>connessioni ibride

[Connessioni ibride](../service-bus-relay/relay-hybrid-connections-protocol.md) è una funzionalità di inoltro di Azure che è possibile usare per accedere alle risorse dell'applicazione in altre reti. Fornisce l'accesso dalla propria app a un endpoint applicazione. Non è possibile utilizzarlo per accedere all'applicazione. Connessioni ibride è disponibile per le funzioni in esecuzione in Windows in tutti i piani di consumo tranne.
...
Quando si integra un'app per le funzioni in un piano Premium o in un piano di servizio app con una rete virtuale, l'app può comunque effettuare chiamate in uscita verso Internet per impostazione predefinita. Aggiungendo un'impostazione `WEBSITE_VNET_ROUTE_ALL=1`dell'applicazione , si forza l'invio di tutto il traffico in uscita nella rete virtuale, in cui è possibile utilizzare le regole dei gruppi di sicurezza di rete per limitare il traffico.
