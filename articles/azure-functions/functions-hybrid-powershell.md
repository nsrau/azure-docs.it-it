...
1. Select **OK** to create the plan, and then configure the remaining **Function App** settings as specified in the table immediately after the following screenshot:
    ...
    | **Gruppo di risorse** |  myResourceGroup | Nome del nuovo gruppo di risorse in cui creare l'app per le funzioni. You can also use the suggested value. |
    | **Sistema operativo** | Preferred OS | Select Windows. |
    | **Stack di runtime** | Lingua preferita | Choose PowerShell Core. |
    | **Archiviazione** |  Nome globalmente univoco |  Creare un account di archiviazione usato dall'app per le funzioni. Storage account names must be from 3 to 24 characters in length and can contain numbers and lowercase letters only. È possibile anche usare un account esistente.
    | **Application Insights** | Predefinito | Crea una risorsa di Application Insights con lo stesso *nome di app* nell'area più vicina supportata. By expanding this setting, you can change the **New resource name** or choose a different **Location** in an [Azure geography](https://azure.microsoft.com/global-infrastructure/geographies/) region where you want to store your data. |
