...
Sebbene sia possibile eseguire un aggiornamento sul posto aggiornando manualmente la configurazione dell'app, passare da 1. x a una versione successiva include alcune modifiche di rilievo. In C#, ad esempio, l'oggetto di debug viene modificato `TraceWriter` da `ILogger`a. Creando un nuovo progetto versione 3. x, si inizia con le funzioni aggiornate basate sui modelli più recenti della versione 3. x.
...
* Le limitazioni di concorrenza HTTP sono implementate per impostazione predefinita per le funzioni del piano a consumo, con un valore predefinito di 100 richieste simultanee per istanza. È possibile modificarlo nell' [`maxConcurrentRequests`](functions-host-json.md#http) impostazione del file host. JSON.
...
* Le associazioni di output assegnate `context.done` tramite o i valori restituiti ora hanno lo stesso comportamento `context.bindings`dell'impostazione in.

* L'oggetto trigger timer è camelCase anziché PascalCase

* Le funzioni attivate da Hub `dataType` eventi con binario riceveranno una `binary` matrice di `string`anziché di.

* Non è più possibile accedere al payload della richiesta HTTP `context.bindingData.req`tramite.  È ancora possibile accedervi come parametro di input, `context.req`e in `context.bindings`.
