...
2. Fare clic sulla scheda **Push**, immettere il valore **Chiave API** ottenuto da GCM nella procedura precedente e quindi fare clic su **Salva**.
...
7. Aprire il file ToDoItem.java, quindi aggiungere il codice seguente alla classe **TodoItem**:
	...
	<p>Se lo schema dinamico è abilitato nel servizio mobile, una nuova colonna <strong>handle</strong> verrà aggiunta automaticamente alla tabella <strong>TodoItem</strong> quando viene inserito un nuovo elemento contenente tale proprietà.</p>
    </div>
...
12. Aggiungere la riga di codice seguente al metodo **addItem**, prima dell'inserimento dell'elemento nella tabella:
	...
	Questo codice imposta la proprietà `handle` dell'elemento sull'ID di registrazione del dispositivo.

13. n Package Explorer fare clic con il pulsante destro del mouse sul pacchetto (nel nodo `src`), quindi scegliere **New** e fare clic su **Class**.
...
17. Sostituire il metodo **onRegistered** con il codice seguente:
...
3. Sostituire la funzione insert con il codice seguente, quindi fare clic su **Salva**:
   	...
   	Viene registrato un nuovo script insert, che usa l'oggetto [gcm] per inviare una notifica push (il testo inserito) al dispositivo specificato nella richiesta insert. 

##<a id="test"></a>Testare le notifiche push nell'app

<div class="dev-callout"><b>Nota</b>
	<p>Quando si esegue l'app nell'emulatore, assicurarsi di usare un emulatore Android Virtual Device (AVD) con il supporto per Google APIs.</p>
</div>
...
4. Nel menu **Run** scegliere **Run** per avviare l'app.

5. Digitare testo significativo nell'app, ad esempio _A new Mobile Services task_ e quindi fare clic sul pulsante **Add**.
