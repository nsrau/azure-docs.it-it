...
servizio tabelle di Microsoft Azure. Gli esempi sono scritti con
l'API Node.js. Gli scenari presentati includono **creazione ed eliminazione di una
...
Il valore di `result` sarà `true` se viene creata una nuova tabella e `false` se la tabella esiste già. `response` contiene informazioni sulla richiesta.
...
dell'entità. Tutte le entità devono contenere **PartitionKey** e **RowKey** che sono gli identificatori univoci dell'entità.
...
Se l'operazione riesce, `result` conterrà il valore [ETag](http://en.wikipedia.org/wiki/HTTP_ETag) del record inserito e `response` conterrà le informazioni sull'operazione.
...
Nell'esempio seguente viene mostrato l'aggiornamento di un'entità con **updateEntity**:
...
>    
> 3. Eseguire l'operazione di aggiornamento. Se l'entità è stata modificata dall'ultimo recupero del valore ETag, ad esempio da un'altra istanza dell'applicazione, viene restituito `error` che indica che la condizione di aggiornamento specificata nella richiesta non è stata soddisfatta.
    
Con **updateEntity** e **mergeEntity**, se l'entità da aggiornare non esiste, l'operazione di aggiornamento non riuscirà. Se pertanto si desidera archiviare un'entità indipendentemente dal fatto che esista o meno, è necessario usare **insertOrReplaceEntity** o **insertOrMergeEntity**.
...
Un'applicazione attendibile, ad esempio un servizio basato sul cloud, genera una firma di accesso condiviso tramite il metodo **generateSharedAccessSignature** di **TableService** e la fornisce a un'applicazione non attendibile o parzialmente attendibile. Si prenda ad esempio un'app per dispositivi mobili. La firma di accesso condiviso viene generata tramite un criterio che indica le date di inizio e di fine del periodo di validità della firma, nonché il livello di accesso concesso al titolare della firma di accesso condiviso.
...
L'applicazione client usa quindi la firma di accesso condiviso con il metodo **TableServiceWithSAS** per eseguire operazioni sulla tabella. Nell'esempio seguente vengono eseguite la connessione alla tabella e una query.
