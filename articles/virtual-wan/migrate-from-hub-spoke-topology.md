...
- Uso del modello hub-spoke in più aree. Uso dei circuiti ExpressRoute Premium per la connettività con una rete WAN privata comune.
...
- Rimuovere la dipendenza dai controller di dominio locali esistenti per la terminazione VPN pur mantenendo i percorsi di connettività seguenti:
...
- Gli hub forniscono anche la VPN destinata agli utenti in roaming tra diversi tipi di client tramite connettività OpenVPN con la rete a mesh globale, consentendo l'accesso non solo alle applicazioni trasferite in Azure ma anche alle risorse rimaste in locale. 
- Connettività Internet per le risorse interne a una rete virtuale fornita dalla rete WAN virtuale di Azure. Connettività Internet per i siti remoti fornita dalla rete WAN virtuale di Azure. Breakout Internet locale supportato tramite l'integrazione di partner per l'accesso ottimizzato a servizi SaaS come Office 365.

## <a name="migrate-to-azure-virtual-wan"></a>Eseguire la migrazione alla rete WAN virtuale di Azure

Questa sezione descrive i vari passaggi della migrazione alla rete WAN virtuale di Azure.
...
- Servizi condivisi (qualsiasi funzione comune richiesta da più spoke); ad esempio a questo scopo Contoso usa controller di dominio di Windows Server IaaS con macchine virtuali IaaS (infrastruttura distribuita come servizio).
...
Il primo passaggio comporta la distribuzione di un hub della rete WAN virtuale in ogni area. Distribuire l'hub della rete WAN virtuale con gateway VPN e gateway ExpressRoute come descritto negli articoli seguenti: 
...
**Figura 5: Migrazione dell'hub-spoke del data center virtuale alla rete WAN virtuale - Passaggio 2**
...
A questo punto connettere l'hub della rete WAN virtuale ai circuiti ExpressRoute della società e configurare le VPN da sito a sito per collegarle tramite Internet alle filiali remote.
...
**Figura 6: Migrazione dell'hub-spoke del data center virtuale alla rete WAN virtuale - Passaggio 3**

A questo punto, le apparecchiature di rete locali inizieranno a ricevere route che riflettono lo spazio degli indirizzi IP assegnato alla rete virtuale hub gestita della rete WAN virtuale. In questa fase le filiali remote connesse alla VPN vedranno due percorsi alle applicazioni esistenti nelle reti virtuali spoke. Questi dispositivi devono essere configurati per continuare a usare il tunnel verso l'hub del data center virtuale per assicurare il routing simmetrico durante la fase di transizione.
...
**Figura 7: Migrazione dell'hub-spoke del data center virtuale alla rete WAN virtuale - Passaggio 4**

### <a name="transition-connectivity-to-virtual-wan-hub"></a>Connettività di transizione all'hub della rete WAN virtuale


![Connettività di transizione all'hub della rete WAN virtuale](./media/migrate-from-hub-spoke-topology/figure8.png)
**Figura 8: Migrazione dell'hub-spoke del data center virtuale alla rete WAN virtuale - Passaggio 5**
...
**d**. I gateway VPN ed ExpressRoute esistenti nell'hub del data center virtuale vengono ora rimossi per consentire il passaggio 5.

**e**. Connettere il precedente hub del data center virtuale (rete virtuale hub) all'hub della rete WAN virtuale tramite una nuova connessione di rete virtuale.
...
**Figura 9: Migrazione dell'hub-spoke del data center virtuale alla rete WAN virtuale - Passaggio 6**

Poiché l'hub della rete WAN virtuale è un'entità gestita e non consente la distribuzione di risorse personalizzate, ad esempio macchine virtuali, il blocco dei servizi condivisi ora esiste come rete virtuale spoke, che ospita funzioni come il traffico Internet in ingresso tramite gateway applicazione di Azure o appliance virtuale di rete. Il traffico tra l'ambiente di servizi condivisi e le macchine virtuali back-end ora transita attraverso l'hub gestito dalla rete WAN virtuale.
...
**Figura 11: Rete WAN virtuale a doppia area**
...
Il percorso 1 descrive il flusso del traffico tra la filiale asiatica connessa alla VPN da sito a sito e la rete virtuale di Azure nell'area Asia sud-orientale.
...
Il percorso 2 descrive il flusso del traffico tra la sede centrale europea connessa a ExpressRoute e la rete virtuale di Azure nell'area Asia sud-orientale.

Il traffico viene instradato come segue:
- La sede centrale europea è connessa tramite circuito ExpressRoute standard all'hub della rete WAN virtuale in Europa occidentale.
- La connettività globale hub-hub della rete WAN virtuale assicura il transito ininterrotto del traffico verso la rete virtuale connessa nell'area remota.

![Flusso 2](./media/migrate-from-hub-spoke-topology/flow2.png)

### <a name="path-3"></a>Percorso 3
Il percorso 3 descrive il flusso del traffico tra il controller di dominio locale in Asia connesso alla rete WAN privata e la filiale europea connessa alla VPN da sito a sito.

Il traffico viene instradato come segue:
- Il controller di dominio in Asia è connesso all'operatore della rete WAN privata locale.
- Il circuito ExpressRoute che in locale termina nella rete WAN privata è connesso all'hub della rete WAN virtuale in Asia sud-orientale.
- La connettività globale hub-hub della rete WAN virtuale assicura il transito ininterrotto del traffico della filiale connessa all'hub remoto in Europa.
...
Il traffico viene instradato come segue:
- La connettività globale hub-hub della rete WAN virtuale consente il transito nativo di tutte le reti virtuali di Azure connesse senza ulteriori configurazioni dell'utente.

![Flusso 4](./media/migrate-from-hub-spoke-topology/flow4.png)

### <a name="path-5"></a>Percorso 5
Il percorso 5 descrive il flusso di traffico tra l'utente della VPN in roaming (da punto a sito) e la rete virtuale di Azure nell'area Europa occidentale.

Il traffico viene instradato come segue:
- Gli utenti di portatili e telefoni utilizzano il client OpenVPN per la connettività trasparente con il gateway VPN da punto a sito in Europa occidentale.
...
Contoso ha ora verificato la connettività tra tutte le filiali e le reti virtuali in linea con i requisiti descritti in precedenza in questo documento. Per soddisfare i requisiti per il controllo di sicurezza e l'isolamento della rete, è necessario continuare a separare e registrare il traffico tramite la rete dell'hub, una funzione in precedenza eseguita da un'appliance virtuale di rete. Contoso vuole anche rimuovere gli attuali servizi proxy e utilizzare servizi nativi di Azure per il filtro di Internet in uscita. 
...
- Collegare i criteri del firewall all'hub della rete WAN virtuale di Azure.
Con il passaggio precedente, l'attuale hub della rete WAN virtuale può fungere da hub virtuale protetto e distribuire le risorse necessarie di Firewall di Azure.
...
Il percorso 6 descrive il flusso del traffico del transito protetto da rete virtuale a rete virtuale all'interno della stessa area.
...
Il percorso 7 descrive il flusso del traffico da rete virtuale a Internet o a un servizio di sicurezza di terze parti.
