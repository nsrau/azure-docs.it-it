<properties umbracoNaviHide="0" pageTitle="Modello applicativo" metaKeywords="Azure, Azure, application model, Azure application model, development model, Azure development model, hosted service, Azure hosted service, web role, worker role" description="Informazioni sul modello applicativo del servizio ospitato di Azure. Comprendere i concetti di base, le considerazioni sulla progettazione, la definizione e la configurazione dell'applicazione e la scalabilit&agrave;." urlDisplayName="Application Model" headerExpose="" footerExpose="" disqusComments="1" title="Modello applicativo" authors="robb" manager="johndaw" />
...
-   [Scelta del modello da usare][Scelta del modello da usare]

## <span id="VMachine"></span></a>Macchine virtuali

Macchine virtuali di Azure consente agli sviluppatori, ai responsabili IT e ad altri utenti di creare e usare macchine virtuali nel cloud. La tecnologia nota come *Infrastruttura distribuita come servizio (IaaS)* può essere usata in diversi modi. [Nella][Nella]figura 1 vengono illustrati i componenti di base di questa tecnologia.
...
2.  Usare i dischi rigidi virtuali forniti da Microsoft e dai partner Microsoft nella raccolta Macchine virtuali di Azure o nel sito Web open source Microsoft [VMDepot][VMDepot].
...
-   **Macchine virtuali per lo sviluppo e il test.** I gruppi di sviluppo necessitano in genere di macchine virtuali con configurazioni specifiche per la creazione di applicazioni. Macchine virtuali di Azure fornisce un modo semplice e conveniente per creare macchine virtuali, usarle e rimuoverle quando non servono più.
-   **Esecuzione di applicazioni nel cloud.** L'esecuzione nel cloud pubblico risulta economicamente vantaggiosa per alcune applicazioni, ad esempio per un'applicazione con picchi elevati a livello di domanda. È sempre possibile acquistare un numero sufficiente di computer per il data center per l'esecuzione di questa applicazione, tuttavia la maggior parte di questi computer resterà per lo più inutilizzata. L'esecuzione di un'applicazione di questo tipo su Azure consente di pagare le macchine virtuali aggiuntive solo quando sono necessarie, arrestandole al termine del picco nella domanda. Oppure si immagini uno scenario in cui una start-up necessita di risorse di elaborazione su richiesta in modo rapido e senza impegno di acquisto. su richiesta e senza impegno.
...
.Le procedure iniziali sono molto semplici. Con Siti Web di Azure gli utenti possono effettuare selezioni da una vasta gamma di applicazioni, framework e modelli e creare un sito Web in pochi secondi. Gli utenti possono usare i propri strumenti di sviluppo preferiti (WebMatrix, Visual Studio, qualsiasi altro editor) e opzioni di controllo del codice sorgente per configurare l'integrazione continua ed eseguire lo sviluppo in team. Le applicazioni basate su un database MySQL possono usare un servizio MySQL fornito da ClearDB, un partner Microsoft, per Azure.
Gli sviluppatori possono creare applicazioni Web scalabili e di grandi dimensioni con Siti Web. Questa tecnologia supporta la creazione di applicazioni tramite ASP.NET, PHP, Node.js e Python. Le applicazioni possono ad esempio usare sessioni permanenti ed è possibile spostare le applicazioni Web esistenti in questa piattaforma cloud, senza modifiche. Le applicazioni basate su Siti Web possono usare liberamente altri componenti di Azure, ad esempio il bus di servizio, il database SQL e l'archiviazione BLOB. È anche possibile eseguire più copie di un'applicazione in macchine virtuali diverse usando Siti Web per le richieste di bilanciamento del carico automatico nelle diverse VM. Inoltre, poiché le nuove istanze di Siti Web vengono create in macchine virtuali già esistenti, una nuova istanza dell'applicazione viene avviata molto rapidamente ed è notevolmente più veloce rispetto al tempo di attesa richiesto per la creazione di una nuova VM.
Come mostrato nella [figura 5][figura 5] il codice e gli altri contenuti Web possono essere pubblicati in diversi modi in Siti Web. È possibile usare FTP, FTPS o la tecnologia WebDeploy Microsoft. Siti Web supporta anche la pubblicazione di codice da sistemi di controllo del codice sorgente, inclusi Git, GitHub, CodePlex, BitBucket, Dropbox, Mercurial, Team Foundation Server e il servizio basato sul cloud Team Foundation Service.
...
## <span id="WhatShouldIUse"></span></a>Scelta del modello da usare
