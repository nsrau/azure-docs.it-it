...
[Abilitare il debug remoto per i servizi cloud](#cloudservice)

[Abilitare il debug remoto per le macchine virtuali](#virtualmachine)

<h2> <a name="cloudservice"></a>Abilitare il debug remoto per i servizi cloud</h2>
...
2. Poiché il pacchetto richiede l'installazione del runtime di debug remoto (msvsmon.exe), installare [Remote Tools per Visual Studio 2013](http://www.microsoft.com/it-it/download/details.aspx?id=40781) o [Remote Tools per Visual Studio 2012 Update 4](http://www.microsoft.com/it-it/download/details.aspx?id=38184) se è installato Visual Studio 2012). In alternativa, copiare i binari di debug remoto da un sistema in cui è installato Visual Studio.
3. Creare un certificato come descritto nell'articolo relativo alla [creazione di un certificato di servizio per Azure](http://msdn.microsoft.com/library/azure/gg432987.aspx). Mantenere il file con estensione pfx e l'identificazione personale del certificato RDP e caricare il certificato nel servizio cloud di destinazione.
4. Nella riga di comando MSBuild usare le seguenti opzioni per eseguire la compilazione e creare un pacchetto con il debug remoto abilitato. Aggiornare il percorso in base al sistema e ai file di progetto in uso.
