...
In questa esercitazione viene illustrato come creare un'applicazione [Node] e distribuirla in un sito Web di Azure tramite [Git]. Le istruzioni di questa esercitazione possono essere eseguite in qualsiasi sistema operativo in grado di eseguire Node.
...
3.  Fare clic su **WEB SITE**, quindi su **QUICK CREATE**. Immettere un valore per **URL** e scegliere il data center per il sito Web nell'elenco a discesa **REGION**. Fare clic sul segno di spunta nella parte inferiore della finestra di dialogo.

    ![Finestra di dialogo Quick Create](./media/web-sites-nodejs-develop-deploy-mac/create-quick-website.png)

4.  Quando lo stato del sito Web diventa **Running**, fare clic sul nome del sito Web per accedere al **Dashboard**.
...
7.  Per abilitare la pubblicazione Git, è necessario specificare un nome utente e una password. Se la pubblicazione è già stata abilitata per un sito Web di Azure, non verrà richiesto di specificare il nome utente e la password. Verrà invece creato un archivio Git con il nome utente e la password indicati in precedenza. Prendere nota del nome utente e della password, in quanto verranno utilizzati per la pubblicazione Git in tutti i siti Web di Azure creati.
...
4.  Aprire il Web browser e passare a http://localhost:1337. Verrà visualizzata una pagina con il messaggio "Hello World", come indicato nella schermata seguente:

    ![Finestra del browser con il messaggio 'Hello World' visualizzato.](./media/web-sites-nodejs-develop-deploy-mac/helloworldlocal.png)
...
1.  Dalla riga di comando passare alla directory **helloworld** e immettere i comandi seguenti per inizializzare un archivio Git locale.
    ...
    **Comando Git non disponibile**
    ...
    [Git](http://git-scm.com/) è un sistema di controllo delle versioni distribuite che è possibile utilizzare per distribuire il sito Web di Azure. Per le istruzioni sull'installazione in una specifica piattaforma, vedere la [pagina di download di Git](http://git-scm.com/download).
...
3.  Aggiungere un sito Git remoto per effettuare il push degli aggiornamenti nel sito Web di Azure creato in precedenza, utilizzando il comando seguente:
...
4.  Effettuare il push delle modifiche in Azure utilizzando il comando seguente:
    ...
    Se si passa alla scheda Deployments del sito Web di Azure nel portale di gestione, verrà visualizzata la prima distribuzione nella cronologia:

    ![Stato della distribuzione Git nel portale](./media/web-sites-nodejs-develop-deploy-mac/git_deployments_first.png)

5.  Passare al sito utilizzando il pulsante **Browse** nella pagina del sito Web di Azure all'interno del portale di gestione.
...
2.  Dalla riga di comando passare alla directory **helloworld** ed eseguire i comandi seguenti.
    ...
    Verrà richiesto di specificare la password creata in precedenza. Se si passa alla scheda Deployments del sito Web di Azure nel portale di gestione, verrà visualizzata la cronologia della distribuzione aggiornata:
...
3.  Passare al sito utilizzando il pulsante **Browse** e notare che gli aggiornamenti sono stati applicati.
    ...
    ![Pagina Web in cui è visualizzato il messaggio 'Hello Azure'](./media/web-sites-nodejs-develop-deploy-mac/helloazure.png)

4.  È possibile ripristinare la distribuzione precedente selezionandola nella scheda "Deployments" del sito Web di Azure all'interno del portale di gestione e utilizzando il pulsante **Redeploy**.
...
-   [Strumenti da riga di comando di Azure per Mac e Linux](/en-us/develop/nodejs/how-to-guides/command-line-tools/)
