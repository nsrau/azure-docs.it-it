...
-   [Iscrizione a Twilio (sconto Microsoft)][Iscrizione a Twilio (sconto Microsoft)]
-   [Creazione e distribuzione di un sito Web Node.js in Azure][Creazione e distribuzione di un sito Web Node.js in Azure]
-   [Configurazione del modulo Twilio][Configurazione del modulo Twilio]
-   [Esecuzione di una chiamata in uscita][Esecuzione di una chiamata in uscita]
-   [Invio di un messaggio SMS][Invio di un messaggio SMS]
...
Twilio consente agli sviluppatori di [acquistare numeri di telefono programmabili][acquistare numeri di telefono programmabili] che è possibile usare per inviare e ricevere chiamate e SMS. Quando un numero Twilio riceve una chiamata o un SMS in ingresso, invia all'applicazione Web una richiesta HTTP di tipo POST o GET, in cui vengono chieste istruzioni per la gestione della chiamata o del messaggio. Il server dell'utente risponderà alla richiesta HTTP di Twilio con [TwiML][TwiML], un semplice set di tag XML che contengono istruzioni per la gestione di una chiamata o un SMS. Più avanti verranno illustrati esempi del linguaggio TwiML.
...
Selezionare il sito Web Node.js e fare clic sul collegamento "CONFIGURE". Scorrendo verso il basso verrà visualizzata un'area in cui è possibile impostare le proprietà di configurazione per l'applicazione. Immettere le credenziali dell'account Twilio ([disponibili nel dashboard di Twilio][disponibili nel dashboard di Twilio]) come illustrato, denominandole rispettivamente "TWILIO\_ACCOUNT\_SID" e "TWILIO\_AUTH\_TOKEN":

![Console di amministrazione di Azure][Console di amministrazione di Azure]
...
Verrà ora creato un file package.json per gestire le dipendenze del modulo Node tramite [npm][npm]. Allo stesso livello del file "server.js" creato nell'esercitazione su Azure e Node.js, creare un file denominato "package.json". All'interno del file inserire il codice seguente:
...
Questo codice dichiara il modulo Twilio come dipendenza, così come il diffuso [framework Web express][framework Web express] e il motore di rendering dei modelli EJS. A questo punto, è possibile iniziare a scrivere il codice.
...
-   [Documentazione ufficiale del modulo][Documentazione ufficiale del modulo]
-   [Esercitazione sull'utilizzo di VoIP con le applicazioni Node.js][blog per sviluppatori di Twilio]
-   [Votr: un'applicazione di voto via SMS in tempo reale con Node.js e CouchDB (tre parti)][Votr: un'applicazione di voto via SMS in tempo reale con Node.js e CouchDB (tre parti)]
-   [Programmazione in coppia nel browser con Node.js][Programmazione in coppia nel browser con Node.js]
...
A questo punto, non resta che sperimentare tutte le possibilità offerte dall'utilizzo di Node.js e Twilio in Azure.
  ...
  [acquistare numeri di telefono programmabili]: https://www.twilio.com/user/account/phone-numbers/available/local
  ...
  [npm]: http://npmjs.org
  [framework Web express]: http://expressjs.com
  [Documentazione ufficiale del modulo]: http://twilio.github.io/twilio-node/
  [Votr: un'applicazione di voto via SMS in tempo reale con Node.js e CouchDB (tre parti)]: http://www.twilio.com/blog/2012/09/building-a-real-time-sms-voting-app-part-1-node-js-couchdb.html
  [Programmazione in coppia nel browser con Node.js]: http://www.twilio.com/blog/2013/06/pair-programming-in-the-browser-with-twilio.html
