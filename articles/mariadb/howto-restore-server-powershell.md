...
Viene eseguito periodicamente il backup del database di Azure per i server MariaDB per abilitare le funzionalità di ripristino. L'uso di questa funzionalità consente di ripristinare il server e tutti i suoi database a un momento precedente nel nuovo server.
...
Per altre informazioni sull'impostazione di questi valori durante la creazione del server, vedere [creare un database di Azure per il server MariaDB con PowerShell](quickstart-create-mariadb-server-database-using-azure-powershell.md).
...
Il set di parametri **PointInTimeRestore** del `Restore-AzMariaDbServer` cmdlet richiede i parametri seguenti:
...
Se il server è stato configurato per i backup con ridondanza geografica, è possibile creare un nuovo server dal backup del server esistente. Questo nuovo server può essere creato in qualsiasi area in cui è disponibile Database di Azure per MariaDB.

Per creare un server usando un backup con ridondanza geografica, usare il `Restore-AzMariaDbServer` comando con il parametro **UseGeoRestore** .
...
Il set di parametri **Georestore** del `Restore-AzMariaDbServer` cmdlet richiede i parametri seguenti:
