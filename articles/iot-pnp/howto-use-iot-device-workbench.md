---
title: Usare il Workbench di Azure per dispositivi per compilare i dispositivi Plug and Play anteprima | Microsoft Docs
description: Usa l'estensione Azure per i dispositivi Workbench di Azure in Visual Studio Code per accelerare la creazione di Plug and Play dei modelli di dispositivo e l'implementazione del codice del dispositivo.
author: liydu
ms.author: liydu
ms.date: 07/25/2019
ms.topic: conceptual
...
ms.locfileid: "69880488"
---
# <a name="use-azure-iot-device-workbench-extension-in-visual-studio-code"></a>Usare l'estensione Azure Azure per i dispositivi Workbench in Visual Studio Code

Il Visual Studio Code estensione Azure per dispositivi Workbench fornisce un ambiente integrato per la creazione di modelli di funzionalità del dispositivo (DCM) e interfacce, la pubblicazione in repository di modelli e la generazione di codice Skeleton C per implementare l'applicazione del dispositivo.
...
Per altre informazioni sull'uso dell'estensione di Device Workbench per lo sviluppo di dispositivi [https://github.com/microsoft/vscode-iot-workbench](https://github.com/microsoft/vscode-iot-workbench)Internet, vedere.

## <a name="prerequisites"></a>Prerequisiti

Installare [Visual Studio Code](https://code.visualstudio.com/).

Per installare l'estensione in VS Code, attenersi alla procedura riportata di seguito.

1. In VS Code selezionare scheda **estensioni** .
1. Cercare e installare Azure Internet Machine **Workbench** dal Marketplace.
...
1. Aprire il repository **Azure-Azure-SDK-c** nel **Team Explorer**, selezionare **Branchs**, cercare il ramo **Public-Preview** ed estrarlo.
    ...
    ![Anteprima pubblica](media/howto-use-iot-device-workbench/vs-public-preview.png)
...
1. Nella barra degli strumenti **generale** trovare l'elenco a discesa **configurazioni** . Selezionare **Gestisci configurazione** per aggiungere l'impostazione CMake per il progetto.
    ...
    ![Gestisci configurazione](media/howto-use-iot-device-workbench/vs-manage-config.png)
...
L'estensione Azure per i dispositivi Workbench è un progetto open source su GitHub. Per eventuali problemi e richieste di funzionalità, è possibile [creare un problema in GitHub](https://github.com/microsoft/vscode-iot-workbench/issues).

## <a name="next-steps"></a>Passaggi successivi

In questo articolo sulle procedure si è appreso come usare il Workbench di dispositivi Azure per creare file di interfaccia e di DCM. Si è inoltre appreso come generare codice Skeleton C per implementare l'applicazione del dispositivo. Un passaggio successivo suggerito consiste nell'apprendere come [installare e usare](./howto-install-iot-explorer.md) lo strumento Azure Internet Explorer.
