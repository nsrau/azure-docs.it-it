...
È possibile inoltre controllare la capacità del servizio endpoint di streaming in modo da poter gestire esigenze di larghezza di banda crescenti modificando le unità di scala (note anche come unità di streaming). Si consiglia di allocare una o più unità di scala per le applicazioni nell'ambiente di produzione. Le unità di scala forniscono capacità di uscita dedicata acquistabile in incrementi di 200 Mbps e una funzionalità aggiuntiva che include: [la creazione dinamica dei pacchetti.](https://msdn.microsoft.com/library/azure/jj889436.aspx), l'integrazione della rete CDN e la configurazione avanzata.
...
5. Fare clic sul nome dell'endpoint di streaming per configurarlo.   
	...
	![Pagina degli endpoint di streaming][streaming-endpoint]
...
![Configurare l'origine][configure-origin]
...
4. È possibile specificare un criterio di accesso tra domini per i client di Adobe Flash. Per altre informazioni, vedere la pagina relativa alle [specifiche del criterio di accesso tra domini](http://www.adobe.com/devnet/articles/crossdomain_policy_file_spec.html). È altresì possibile specificare i criteri di accesso per i client di Microsoft Silverlight. Per altre informazioni, vedere la pagina relativa a [come rendere un servizio disponibile attraverso i limiti del dominio](https://msdn.microsoft.com/library/cc197955(v=vs.95).aspx).  
...
- L'endpoint di streaming deve disporre di almeno una unità di streaming (scala). Se si desidera impostare le unità di scala su 0 in un secondo momento, è innanzitutto necessario disattivare l'integrazione della rete CDN. 
...
L'abilitazione dell'integrazione della rete CDN di Azure può richiedere fino a 90 minuti.   

Una volta abilitata, le seguenti configurazioni vengono disabilitate: **Nomi host personalizzati** e **Controllo di accesso**.

Non tutti i data center supportano l'integrazione della rete CDN di Azure. Se si usa il portale di gestione di Azure, verrà visualizzata l'opzione **ABILITA CDN**, se disponibile nel data center. 

![Abilita CDN per l'endpoint di streaming][streaming-endpoint-enable-cdn]

Se si usa .NET SDK o l'API REST, è necessario verificare se nel data center è disponibile l'integrazione della rete CDN di Azure. 

Per verificarlo, seguire questa procedura:

1. Provare a impostare la proprietà CdnEnabled su true.
1. Nel risultato restituito verificare la presenza del codice di errore HTTP 412 (PreconditionFailed) con il messaggio "Impossibile impostare la proprietà CdnEnabled dell'endpoint di streaming su true, in quanto la funzionalità CDN non è disponibile nell'area corrente". 

	Se viene visualizzato questo errore, il data center non supporta la funzionalità. È consigliabile provare con un altro data center.
