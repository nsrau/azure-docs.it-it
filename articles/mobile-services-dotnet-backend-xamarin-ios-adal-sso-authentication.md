...
>[AZURE.NOTE] Questa esercitazione descrive come usare Servizi mobili per eseguire l'autenticazione tramite Single Sign-On di Azure Active Directory per le app Xamarin.iOS. Se si tratta della prima esperienza con Servizi mobili, iniziare dall'esercitazione [Introduzione a Servizi mobili].
...
6. Nel codice per il metodo `AuthenticateAsync` precedente sostituire **INSERT-AUTHORITY-HERE** con il nome del tenant in cui è stato effettuato il provisioning dell'applicazione, nel formatohttps://login.windows.net/tenant-name.onmicrosoft.com. È possibile copiare questo valore dalla scheda Dominio di Azure Active Directory nel [portale di gestione di Azure].

7. Nel codice per il metodo  `AuthenticateAsync` precedente sostituire **INSERT-RESOURCE-URI-HERE** con l'**App ID URI** del servizio mobile. Se sono state seguite le istruzioni riportate nell'argomento sulla [Come eseguire la registrazione ad Azure Active Directory], l'URI ID app dovrebbe essere simile a https://todolist.azure-mobile.net/login/aad.
...
9. Nel codice per il metodo  `AuthenticateAsync` precedente sostituire **INSERT-REDIRECT-URI-HERE** con l'/login/done endpoint del servizio mobile. Questo valore dovrebbe essere simile a https://todolist.azure-mobile.net/login/done


3. Nel QSTodoListViewController, modificare **ViewDidLoad** aggiungendo il codice seguente immediatamente prima della chiamata a RefreshAsync();
