...
2.  [Codice C# per Hadoop Streaming][]
3.  [Riepilogo][]
...
Nel codice del reducer nel file wc.cs viene utilizzato un oggetto [StreamReader][] per leggere dal flusso di input standard i caratteri che sono stati trasmessi dal mapper cat.exe. Durante la lettura dei caratteri con il metodo [Console.Writeline][] viene effettuato il conteggio delle parole, inclusi gli spazi e caratteri di fine riga alla fine di ogni parola. Il totale viene quindi scritto nel flusso di output standard con il metodo Console.Writeline.
