<properties 
	pageTitle="Autorizzazione degli utenti negli script (Xamarin.iOS) - Servizi mobili di Azure" 
	description="Informazioni su come autorizzare gli utenti mediante script nell'app Servizi mobili di Azure per Xamarin.iOS." 
...
Questo argomento illustra come usare gli script del server per autorizzare gli utenti autenticati ad accedere ai dati in Servizi mobili di Azure da un'app per Xamarin.iOS.  In questa esercitazione verranno registrati gli script con Servizi mobili per filtrare le query in base all'ID utente di un utente autenticato, per garantire che ogni utente possa visualizzare solo i propri dati.
...
2. Scegliere il pulsante **Run** per creare il progetto, avviare l'app nell'emulatore iPhone e quindi accedere con il provider di identità scelto. 
