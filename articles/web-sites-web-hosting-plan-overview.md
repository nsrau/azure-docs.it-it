...
Il nuovo portale Azure Preview consente di associare siti Web nuovi o esistenti a dei piani di hosting Web. Tutti i siti Web sono stati infatti assegnati automaticamente a dei piani di hosting Web in base alla sottoscrizione, all'area geografica e all'attuale livello di prezzo.

Quando si crea un nuovo sito, nel portale verrà richiesto a quale piano di hosting Web si desidera associarlo. È possibile creare un nuovo piano di hosting Web o selezionarne uno esistente. Per utilizzare un piano esistente, il nuovo sito deve trovarsi nella stessa sottoscrizione, area geografica e gruppo di risorse in cui si trova tale piano esistente. Per impostazione predefinita, quando si crea un nuovo sito Web vuoto, Azure proporrà il piano di hosting Web utilizzato per ultimo per consentire un utilizzo ottimale dei piani già creati. Quando si crea un sito Web con un database, l'opzione di riutilizzare un piano esistente non sarà disponibile.

È possibile visualizzare tutti i piani di hosting Web per tutte le sottoscrizioni di cui si dispone utilizzando il pulsante **Browse** nella barra dei menu visualizzata a sinistra e quindi facendo clic su **Everything** nella parte superiore del riquadro attività che viene visualizzato.
...
È inoltre possibile verificare a quale piano di hosting Web è associato ogni sito nella rappresentazione grafica del gruppo di risorse in uso, visualizzata nella parte alta del blade del sito Web.

![](./media/web-sites-web-hosting-plan-overview/web-hosting-plan-resource-map.png)

Quando si fa clic sul piano verrà avviato un blade che consente di gestire il piano di hosting Web. [Ulteriori informazioni sulla gestione dei piani di hosting Web](http://go.microsoft.com/fwlink/?LinkID=394411).
