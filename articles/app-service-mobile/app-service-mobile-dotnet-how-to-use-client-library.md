---
title: Utilizzare la libreria client gestita
...
* **Xamarin Studio** Fare clic con il pulsante destro del mouse `Microsoft.Azure.Mobile.Client` sul progetto, scegliere **Aggiungi** > pacchetti **NuGet**, cercare il pacchetto e quindi fare clic su Aggiungi **pacchetto**.
...
### <a name="how-to-work-with-debug-symbols-in-visual-studio"></a><a name="symbolsource"></a>Procedura: Usare i simboli di debug in Visual Studio
...
## <a name="create-the-mobile-apps-client"></a><a name="create-client"></a>Creare il client di App per dispositivi mobili
...
* [Creare un riferimento alla tabellaCreate a table reference](#instantiating)
...
### <a name="how-to-create-a-table-reference"></a><a name="instantiating"></a>Procedura: Creare un riferimento alla tabella
...
L'oggetto restituito usa il modello di serializzazione tipizzato. Viene supportato anche un modello di serializzazione non tipizzato. Nell'esempio seguente viene creato un riferimento a una [tabella non tipizzato]:
...
### <a name="how-to-query-data-from-your-mobile-app"></a><a name="querying"></a>Procedura: Eseguire query sui dati dall'app per dispositivi mobili
...
### <a name="how-to-filter-returned-data"></a><a name="filtering"></a>Procedura: filtrare i dati restituitiHow to: Filter returned data
...
### <a name="how-to-sort-returned-data"></a><a name="sorting"></a>Procedura: Ordinare i dati restituiti
...
### <a name="how-to-return-data-in-pages"></a><a name="paging"></a>Procedura: Restituire i dati in pagine
...
### <a name="how-to-select-specific-columns"></a><a name="selecting"></a>Procedura: Selezionare colonne specifiche
...
### <a name="how-to-look-up-data-by-id"></a><a name="lookingup"></a>Procedura: Cercare dati in base all'ID
...
### <a name="how-to-execute-untyped-queries"></a><a name="untypedqueries"></a>Procedura: eseguire query non tipizzate
...
### <a name="how-to-insert-data-into-a-mobile-app-backend"></a><a name="inserting"></a>Procedura: Inserire dati in un back-end di App per dispositivi mobili
Tutti i tipi di client devono contenere un membro denominato **Id**, che per impostazione predefinita è una stringa. Questo **ID** è necessario per eseguire operazioni CRUD e per la sincronizzazione offline. Il codice seguente illustra come usare il metodo [InsertAsync] per inserire nuove righe in una tabella. Il parametro contiene i dati da inserire come oggetto .NET.
...
### <a name="how-to-modify-data-in-a-mobile-app-backend"></a><a name="modifying"></a>Procedura: Modificare dati in un back-end di App per dispositivi mobili
...
### <a name="how-to-delete-data-in-a-mobile-app-backend"></a><a name="deleting"></a>Procedura: Eliminare dati in un back-end di App per dispositivi mobili
...
### <a name="how-to-use-optimistic-concurrency-for-conflict-resolution"></a><a name="optimisticconcurrency"></a>Procedura: Usare la concorrenza ottimistica per la risoluzione dei conflitti
È possibile che due o più client scrivano modifiche nello stesso elemento contemporaneamente. Se non viene rilevato un conflitto, l'ultima scrittura sovrascrive tutti gli aggiornamenti precedenti. **Il controllo della concorrenza ottimistica** presuppone che ogni transazione possa eseguire il commit e pertanto non utilizzi alcun blocco delle risorse.  Prima di effettuare il commit di una transazione, il controllo della concorrenza ottimistica verifica che i dati non siano stati modificati da un'altra transazione. Se i dati sono stati modificati, verrà eseguito il rollback di tale transazione.
...
### <a name="how-to-bind-mobile-apps-data-to-a-windows-user-interface"></a><a name="binding"></a>Procedura: Associare dati di App per dispositivi mobili a un'interfaccia utente Windows
...
### <a name="change-the-page-size"></a><a name="pagesize"></a>Modificare le dimensioni di pagina
...
## <a name="work-with-offline-tables"></a><a name="#offlinesync"></a>Usare le tabelle offline
...
3. (Facoltativo). Per i dispositivi Windows, fare clic su **Riferimenti** > **Aggiungi riferimento...**, espandere la cartella **di Windows** > **Estensioni**, quindi abilitare l'oggetto **SQLite** appropriato per Windows SDK insieme al runtime di **Visual C.NET 2013 per Windows** SDK.
...
### <a name="syncing-an-offline-table"></a><a name="syncoffline"></a>Sincronizzazione di una tabella offline
...
## <a name="work-with-a-custom-api"></a><a name="#customapi"></a>Usare un'API personalizzata
...
## <a name="authenticate-users"></a><a name="authentication"></a>Autenticare gli utenti
...
### <a name="client-managed-authentication"></a><a name="clientflow"></a>Autenticazione gestita dal client
...
#### <a name="authenticate-users-with-the-active-directory-authentication-library"></a><a name="adal"></a>Autenticare gli utenti con Active Directory Authentication Library
...
#### <a name="single-sign-on-using-a-token-from-facebook-or-google"></a><a name="client-facebook"></a>Accesso singolo usando un token da Facebook o Google
...
### <a name="server-managed-authentication"></a><a name="serverflow"></a>Autenticazione gestita dal server
...
## <a name="push-notifications"></a><a name="pushnotifications"></a>Notifiche push
...
### <a name="how-to-register-for-push-notifications"></a><a name="register-for-push"></a>Procedura: registrarsi per le notifiche pushHow to: Register for Push Notifications
...
### <a name="how-to-obtain-a-microsoft-store-package-sid"></a><a name="package-sid"></a>Procedura: Ottenere un SID pacchetto di Microsoft Store
...
1. In Esplora soluzioni di Visual Studio fare clic **Store** > con il pulsante destro del mouse sul progetto di app di Microsoft Store e scegliere**Associa app allo Store...**.
...
5. Fare clic su**Identità app** **di gestione** > app e quindi scorrere verso il basso per trovare il **SID del pacchetto**.
...
### <a name="how-to-register-push-templates-to-send-cross-platform-notifications"></a><a name="register-xplat"></a>Procedura: registrare modelli push per inviare notifiche multipiattaforma
...
## <a name="miscellaneous-topics"></a><a name="misc"></a>Argomenti vari
### <a name="how-to-handle-errors"></a><a name="errors"></a>Procedura: Gestire gli errori
...
### <a name="how-to-customize-request-headers"></a><a name="headers"></a>Procedura: Personalizzare le intestazioni delle richieste
...
[Prendere]: https://msdn.microsoft.com/library/azure/dn250574(v=azure.10).aspx
[Seleziona]: https://msdn.microsoft.com/library/azure/dn250569(v=azure.10).aspx
[Saltare]: https://msdn.microsoft.com/library/azure/dn250573(v=azure.10).aspx
...
[Dove]: https://msdn.microsoft.com/library/azure/dn250579(v=azure.10).aspx
