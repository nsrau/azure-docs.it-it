...
In questa esercitazione si aggiornerà il progetto client creato nell'esercitazione [Creare un'app Xamarin Android] per supportare le funzionalità offline di App per dispositivi mobili di Azure.
...
* Il membro `toDoTable` di `ToDoActivity` è di tipo `IMobileServiceSyncTable` invece di `IMobileServiceTable`. Per questo motivo, tutte le operazioni di creazione, lettura, aggiornamento ed eliminazione (CRUD, Create, Read, Update, Delete) sulle tabelle vengono indirizzate al database di archiviazione locale.
...
<!-- URLs. -->
[Creare un'app Xamarin Android]: ../app-service-mobile-dotnet-backend-xamarin-android-get-started-preview.md
