---
title: Abilitare la sincronizzazione offline (Novell. Forms) | Microsoft Docs
description: Informazioni su come usare l'app per dispositivi mobili del servizio app per memorizzare nella cache e sincronizzare i dati offline nell'applicazione Novell. Forms.
...
4. Opzionale In ogni progetto di app di Windows fare clic con il pulsante destro del mouse su **riferimenti** > **Aggiungi riferimento...**, espandere la cartella **Windows** > **estensioni**.
...
5. (Facoltativo) Usare Visual Studio per visualizzare la tabella di database SQL di Azure per verificare che i dati nel database back-end non siano cambiati.
    ...
    In Visual Studio aprire **Esplora server**. Passare al database in**database SQL**di **Azure**->. Fare clic con il pulsante destro del mouse sul database e scegliere **Apri in Esplora oggetti di SQL Server**. È ora possibile passare alla tabella di database SQL e al relativo contenuto.
