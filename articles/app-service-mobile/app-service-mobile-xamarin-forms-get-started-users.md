---
title: Introduzione all'autenticazione per app per dispositivi mobili nell'app Xamarin.Forms | Microsoft Docs
...
2. Ricompilare l'app, eseguirla, quindi accedere con il provider di autenticazione selezionato e verificare se si è in grado di accedere ai dati come utente autenticato.
...
2. (Facoltativo) Nel progetto **WinPhone81** aprire il file App.xaml.cs e quindi aggiungere le istruzioni `using` seguenti:
    ...
    Sostituire `<your_Portable_Class_Library_namespace>` con lo spazio dei nomi per la libreria di classi portabile.
3. Aggiungere l'override del metodo **OnActivated** seguente alla classe **App**:
...
4. Ricompilare l'app, eseguirla, quindi accedere con il provider di autenticazione selezionato e verificare se si è in grado di accedere ai dati come utente autenticato.
