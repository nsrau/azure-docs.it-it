...
Questo argomento descrive come eseguire la migrazione di un'applicazione bi back-end .NET esistente da Servizi mobili di Azure a una nuova app per dispositivi mobili del servizio app. Durante una migrazione, l'applicazione Servizi mobili esistente può continuare a funzionare.

Quando si esegue la migrazione di un back-end per dispositivi mobili al Servizio app di Azure, essa ha accesso a tutte le funzionalità del servizio app e vengono fatturate in base ai [prezzi del servizio app], non ai prezzi di Servizi mobili.
...
L'aggiornamento al nuovo [pacchetto NuGet dell’SDK di App per dispositivi mobili](https://www.nuget.org/packages/Microsoft.Azure.Mobile.Server/) offre i vantaggi seguenti:
...
- Supporto per le nuove funzionalità di autenticazione del Servizio app, che consentono di utilizzare una configurazione di autenticazione comune tra web e app per dispositivi mobili: Le funzionalità di autenticazione sono ora anche nel proprio componente middleware OWIN, il che significa che è possibile utilizzare la stessa impostazione di autenticazione per dispositivi mobili e route web.
...
- Maggiore flessibilità nell’SDK per dispositivi mobili. È possibile controllare in modo esplicito le funzionalità e le route impostate, come autenticazione, API di tabella e endpoint di registrazione push. Per ulteriori informazioni, vedere [Come utilizzare l’SDK del server .NET per App per dispositivi mobili di Azure](app-service-mobile-net-upgrading-from-mobile-services.md#server-project-setup).

- Ulteriori opzioni per framework DI. L’SDK del server .NET di Servizi mobili aveva una dipendenza in autofac, ma l’SDK del server di App per dispositivi mobili consente di utilizzare altri framework, ad esempio [Unity](https://unity.codeplex.com/).

>[AZURE.NOTE]Gli SDK del client di Servizi mobili **non** sono compatibili con il nuovo SDK del server di App per dispositivi mobili. Se si dispone di applicazioni client per dispositivi mobili che si connettono al servizio mobile esistente, è consigliabile mantenere il servizio in esecuzione fino a quando non sono stati aggiornati tutti i client mobili per l’**SDK del client**di App per dispositivi mobili.
> 
> Il servizio mobile esistente può essere automaticamente *migrato* al servizio app senza alcuna modifica alle applicazioni client esistenti. Per altre informazioni sulla migrazione, vedere [Effettuare la migrazione del Servizio mobile esistente al Servizio app].


##<a name="overview"></a>Panoramica sull’aggiornamento di base
Il modo più semplice per eseguire l’aggiornamento consiste nel creare una nuova istanza di un'app per dispositivi mobili del servizio app. In molti casi, l’aggiornamento sarà semplice come passare al nuovo SDK del server di App per dispositivi mobili e ripubblicare il codice in una nuova app per dispositivi mobili. Esistono, tuttavia, alcuni scenari che richiedono alcune configurazioni aggiuntive, ad esempio l'autenticazione avanzata e l'uso dei processi pianificati. Ciascuno di questi scenari viene trattato nelle sezioni seguenti.

>[AZURE.NOTE]Si consiglia di leggere e comprendere integralmente il resto di questo argomento prima di avviare un aggiornamento. Prendere nota delle funzionalità usate che sono indicate di seguito.
...
Il set completo di passaggi per questo aggiornamento è il seguente:
...
Il primo passaggio per l’aggiornamento prevede la creazione della risorsa di app per dispositivi mobili che ospiterà la nuova versione dell'applicazione. È possibile creare una nuova app per dispositivi mobili in [Anteprima del portale di gestione di Azure]. È possibile consultare l'argomento [Creare un'app per dispositivi mobili] per altre informazioni.
...
Le app per dispositivi mobili forniscono un nuovo [Mobile App Server SDK] che offre molte delle funzionalità del runtime di Servizi per dispositivi mobili. È prima di tutto necessario rimuovere il pacchetto NuGet per Servizi mobili dal progetto esistente e includere invece l'SDK del server. Per questo aggiornamento, la maggior parte degli sviluppatori vorrà scaricare e installare il pacchetto `Microsoft.Azure.Mobile.Server.Quickstart`, che permette di ottenere l'intero set necessario. In WebApiConfig.cs è quindi possibile sostituire
...
>[AZURE.NOTE]Alcuni provider, ad esempio Twitter e Account Microsoft, non consentono di specificare più URI di reindirizzamento in domini diversi. Se l'app usa uno di questi provider e stabilisce una dipendenza sulle ID utente, è consigliabile evitare di provare a eseguire l’aggiornamento in questo momento.
...
[Effettuare la migrazione del Servizio mobile esistente al Servizio app]: app-service-mobile-dotnet-backend-migrating-from-mobile-services.md
