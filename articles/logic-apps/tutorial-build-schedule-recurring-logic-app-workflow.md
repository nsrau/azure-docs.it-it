...
* Un account di posta elettronica di un provider supportato da App per la logica, ad esempio un account Office 365 Outlook, Outlook.com o Gmail. Per altri provider, [vedere qui l'elenco dei connettori](/connectors/). In questo argomento di avvio rapido si usa un account aziendale o dell'istituto di istruzione. Se si usa un account di posta elettronica diverso, la procedura generale resta invariata, ma l'interfaccia utente potrebbe essere leggermente diversa.
...
1. In **Crea app per la logica** specificare le informazioni sull'app per la logica come mostrato e descritto di seguito. Al termine, selezionare **Crea**.
   ...
   | **Gruppo di risorse** | LA-TravelTime-RG | Il nome del [gruppo di risorse di Azure](../azure-resource-manager/management/overview.md) usato per organizzare le risorse correlate. In questo esempio viene usato "LA-TravelTime-RG". |
...
![Esempio di flusso di lavoro dell'app per la logica completato](./media/tutorial-build-scheduled-recurring-logic-app-workflow/check-travel-time-finished.png)
