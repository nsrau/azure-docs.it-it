...
Gli utenti accedono con l'account aziendale o dell'istituto di istruzione. Questo è l'account dell'organizzazione. Se si è effettuata l'iscrizione a un'offerta di Office 365 senza fornire l'indirizzo di posta elettronica aziendale effettivo, l'account sarà simile a jeff@contoso.onmicrosoft.com. Nell'ambito di un servizio cloud, l'account è archiviato in un tenant in Azure Active Directory (AAD). Nella maggior parte dei casi, l'UPN dell'account AAD corrisponde all'indirizzo di posta elettronica.
...
* [Creare una connessione locale ad App per la logica](../logic-apps/logic-apps-gateway-connection.md)
