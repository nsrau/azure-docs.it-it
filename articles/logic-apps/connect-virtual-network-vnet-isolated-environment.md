...
1. Specificare questi dettagli per l'ambiente, quindi scegliere **Rivedi e crea**, ad esempio:
   ...
   | **SKU** | Sì | **Premium** o **Developer (nessun contratto di contratto)** | SKU ISE da creare e usare. Per le differenze tra questi SKU, vedere [SKU di ISE](../logic-apps/connect-virtual-network-vnet-isolated-environment-overview.md#ise-level). |
   ...
   **Creare una subnet**
   ...
   Per creare e distribuire le risorse nell'ambiente, ISE necessita di quattro subnet *vuote* che non sono delegate ad alcun servizio. 
   *Non è possibile* modificare questi indirizzi subnet dopo aver creato l'ambiente. Ogni subnet deve soddisfare questi criteri:
   ...
   * USA almeno un `/27` nello spazio degli indirizzi, perché ogni subnet deve avere come *minimo*32 indirizzi. Ad esempio:
...
1. Dopo l'avvenuta convalida da parte di Azure delle informazioni dell'ISE, scegliere **Crea**, ad esempio:
   ...
   > Quando si eliminano le reti virtuali, assicurarsi che non ci siano risorse ancora connesse. Vedere [eliminare la rete virtuale](../virtual-network/manage-virtual-network.md#delete-a-virtual-network).
...
Per creare app per la logica che vengono eseguite nell'ambiente di Integration Services (ISE), [creare le app per la logica nel modo consueto](../logic-apps/quickstart-create-first-logic-app-workflow.md) tranne quando si imposta la proprietà **location** , selezionare ISE dalla sezione **ambienti di Integration Services** per esempio
...
Se si vuole usare un account di integrazione con app per la logica in un ambiente Integration Services (ISE), tale account di integrazione deve usare lo *stesso ambiente* delle app per la logica. Le app per la logica in un ISE possono fare riferimento solo agli account di integrazione che sono nello stesso ISE. In base allo [SKU ISE](../logic-apps/connect-virtual-network-vnet-isolated-environment-overview.md#ise-level) selezionato al momento della creazione, ISE include un utilizzo specifico dell'account di integrazione senza costi aggiuntivi. Per informazioni sul funzionamento dei prezzi e della fatturazione per gli account di integrazione con ISEs, vedere il [modello di prezzi di app](../logic-apps/logic-apps-pricing.md#fixed-pricing)per la logica. Per informazioni sui prezzi, vedere [prezzi di app](https://azure.microsoft.com/pricing/details/logic-apps/)per la logica.

Per creare un account di integrazione che usa ISE, [creare l'account di integrazione nel modo consueto](../logic-apps/logic-apps-enterprise-integration-create-integration-account.md) tranne quando si imposta la proprietà **location** , quindi selezionare ISE dalla sezione **ambienti di Integration Services** , ad esempio:
