...
1. Nel menu dell'app per la logica, in **Impostazioni**, selezionare **Identità**. Selezionare **sistema assegnato** > **su** > **Salva**. Quando Azure chiede di confermare, selezionare **Sì**.
...
* Azure PowerShell
  ...
  * [Aggiungi assegnazione di ruolo](../active-directory/managed-identities-azure-resources/howto-assign-access-powershell.md)
* Interfaccia della riga di comando di Azure
  ...
  * [Aggiungi assegnazione di ruolo](../active-directory/managed-identities-azure-resources/howto-assign-access-cli.md)
* API REST di Azure
  ...
  * [Aggiungi assegnazione di ruolo](../role-based-access-control/role-assignments-rest.md)
...
1. Nella casella di ricerca di una pagina [portale di Azure](https://portal.azure.com), immettere `managed identities`e selezionare **identità gestite**.
...
1. Fornire informazioni sull'identità gestita e quindi selezionare **Crea**, ad esempio:
   ...
   |----------|----------|-------|-------------|
   | **Nome risorsa** | Sì | <*assegnato dall'utente-Identity-name*> | Nome da assegnare all'identità assegnata dall'utente. Questo esempio USA "Fabrikam-user-assigned-Identity". |
...
1. Nella portale di Azure trovare e aprire l'app per la logica in progettazione app per la logica.

1. Nel menu dell'app per la logica, in **Impostazioni**, selezionare **identità**, quindi selezionare **utente assegnato** > **Aggiungi**.
...
* Oggetto `userAssignedIdentities` figlio che specifica l'ID risorsa dell'identità, ovvero un altro oggetto figlio con le proprietà `principalId` e `clientId`

Questo esempio illustra una definizione di risorsa dell'app per la logica per una richiesta HTTP PUT e include un oggetto `identity` senza parametri. La risposta alla richiesta PUT e all'operazione GET successiva ha anche questo oggetto `identity`:
...
| Property (JSON) | Valore | Descrizione |
|-----------------|-------|-------------|
| `principalId` | <*principal-ID*> | Identificatore univoco globale (GUID) per l'identità gestita assegnata dall'utente nel tenant di Azure AD |
| `clientId` | <*client-ID*> | Identificatore univoco globale (GUID) per la nuova identità dell'app per la logica usata per le chiamate durante il runtime |
||||
...
1. Nel menu dell'app per la logica, in **Impostazioni**, selezionare **identità**, quindi seguire la procedura per l'identità:
   ...
   * Selezionare **sistema assegnato** > **su** > **Salva**. Quando Azure chiede di confermare, selezionare **Sì**.
...
Se è stata creata l'identità gestita dell'app per la logica usando un modello di Azure Resource Manager, impostare la proprietà figlio `type` dell'oggetto `identity` su `None`. Per l'identità gestita dal sistema, questa azione Elimina anche l'ID entità da Azure AD.
