...
* Le azioni in un ciclo "foreach" usano il[`@item()`](../logic-apps/workflow-definition-language-functions-reference.md#item) 
...
1. In **Nome** selezionare la variabile **Limite**. Per **valore**immettere "1". 
...
1. Impostare le proprietà dell'azione di posta elettronica. Aggiungere la variabile **Limite** all'oggetto. In questo modo, è possibile verificare che il valore corrente della variabile soddisfi la condizione specificata, ad esempio:
      ...
      | Proprietà | Valore | Descrizione |
      | -------- | ----- | ----------- | 
      | **A** | *\<indirizzo\@di posta elettronica>dominio* | Indirizzo e-mail del destinatario. Per il test è possibile usare l'indirizzo di posta elettronica personale. | 
      ...
      | **Corpo** | <*posta elettronica-contenuto*> | Specificare il contenuto del messaggio di posta elettronica da inviare. Per questo esempio, immettere il testo desiderato. | 
      |||| 
