<properties title="Guide to the Net# Neural Networks Specification Language for Azure ML" pageTitle="Guida al linguaggio di specifica Net# delle reti neurali per Azure ML " description="Sintassi relativa al linguaggio per la specifica delle reti neurali Net# ed esempi sulla creazione di un modello di rete neurale personalizzato in Microsoft Azure ML mediante Net# " metaKeywords="" services="machine-learning" solutions="" documentationCenter="" authors="jeannt" manager="paulettm" editor="cgronlun"  videoId="" scriptId="" />
...
# Guida al linguaggio di specifica Net# delle reti neurali


Net#(c) è un linguaggio sviluppato da Microsoft e usato per definire le architetture di reti neurali per moduli di rete neurale in Microsoft Azure Machine Learning. Contenuto dell'articolo:  

-	Concetti di base correlati alla rete neurale
...
Una struttura di rete neurale è costituita da ***nodi*** organizzati in ***livelli*** e ***connessioni*** ponderate (o ***bordi***) tra i nodi. Le connessioni sono direzionali. Ogni connessione ha un nodo di ***origine*** e un nodo di ***destinazione***.  

Ogni ***livello addestrabile*** (ovvero un livello nascosto o di output) ha una o più ***aggregazioni di connessioni***. Un'aggregazione di connessioni è costituita da un livello di origine e da una specifica delle connessioni provenienti da quel livello di origine. Tutte le connessioni in una determinata aggregazione condividono lo stesso ***livello di origine*** e lo stesso ***livello di destinazione***. In Net# un'aggregazione di connessioni è considerata come appartenente al livello di destinazione dell'aggregazione.  
...
-	**Aggregazioni di pooling** e **aggregazioni di normalizzazione delle risposte**. Sono simili alle aggregazioni convoluzionali, in quanto l'utente definisce piccoli intorni di nodi nel livello di origine. La differenza consiste nel fatto che i pesi dei bordi in queste aggregazioni non sono sottoponibili a training. Una funzione predefinita viene infatti applicata ai valori del nodo di origine per determinare il valore del nodo di destinazione.  

L'uso di Net# per definire la struttura di una rete neurale rende possibile la definizione di strutture complesse quali reti neurali basate su machine deep learning o convoluzioni di dimensioni arbitrarie, che notoriamente migliorano l'apprendimento relativo a dati quali immagini, audio e video.  

###Personalizzazioni supportate
L'architettura dei modelli di rete neurale creati in Microsoft Azure Machine Learning può essere ampiamente personalizzata tramite Net#. È possibile:  
...
-	Definire strutture di connettività speciali, ad esempio convoluzioni e aggregazioni per la condivisione dei pesi.
...
###Requisiti generali
-	Devono essere disponibili esattamente un livello di output, almeno un livello di input e zero o più livelli nascosti. 
...
Una specifica della struttura della rete neurale è costituita da tre sezioni: la **dichiarazione delle costanti**, la **dichiarazione dei livelli**, la **dichiarazione delle connessioni** e una **dichiarazione delle condivisioni** facoltativa. Le sezioni possono essere specificate in qualsiasi ordine.  

###Dichiarazione delle costanti 
La dichiarazione delle costanti è facoltativa e permette di definire i valori usati altrove nella definizione della rete neurale. L'istruzione di dichiarazione è costituita da un identificatore seguito da un segno di uguale e un'espressione valore.   
...
Per definire contemporaneamente due o più costanti, racchiudere i nomi e i valori degli identificatori tra parentesi graffe e separarli usando punti e virgola, come illustrato nell'esempio seguente:  
...
La dichiarazione dei livelli è obbligatoria e definisce la dimensione e l'origine del livello, incluse le rispettive aggregazioni di connessioni e i rispettivi attributi. L'istruzione della dichiarazione inizia con il nome del livello (di input, nascosto o di output), seguito dalle dimensioni del livello (una tupla di valori Integer positivi). ad esempio:  
...
La dichiarazione di livello per un livello addestrabile, ovvero i livelli nascosti o di output, può includere facoltativamente la funzione di output, definita anche funzione di attivazione, che assume il valore predefinito **sigmoid**. Sono supportate le funzioni di output seguenti:  
...
-	**Aggregazioni convoluzionali**, indicate dalla parola chiave **convolve**, seguita dagli attributi di convoluzione.
-	**Aggregazioni di pooling**, indicate dalla parola chiave **max pool** o **mean pool**.
...
-	Nel predicato per ByRow, **s** è un parametro che rappresenta un indice nella matrice rettangolare di nodi del livello di input Pixels, mentre **d** è un parametro che rappresenta un indice nella matrice di nodi del livello nascosto. Il tipo di **s** e **d** è una tupla di valori Integer con lunghezza pari a due. Concettualmente, **s** include tutte le coppie di valori Integer con 0 <= s[0] < 10 e 0 <= s[1] < 20, mentre **d** include tutte le coppie di valori Integer con 0 <= d[0] < 10 e 0 <= d[1] < 12. 
...
Le aggregazioni convoluzionali supportano gli attributi seguenti: **InputShape** definisce la dimensionalità del livello di origine ai fini di questa aggregazione convoluzionale. Il valore deve essere una tupla di valori Integer positivi. Il prodotto dei valori Integer deve essere uguale al numero di nodi del livello di origine, ma non deve corrispondere in altro modo alla dimensionalità dichiarata per il livello di origine. La lunghezza di questa tupla diventa il valore di **arietà** per l'aggregazione convoluzionale. In genere, l'arietà fa riferimento al numero di argomenti oppure operandi che una funzione può accettare.  

Per definire la forma e le posizioni dei kernel, usare gli attributi **KernelShape**, **Stride**, **Padding**, **LowerPad** e **UpperPad**:   

-	**KernelShape**: (obbligatorio) definisce la dimensionalità di ogni kernel per l'aggregazione convoluzionale. Il valore deve essere una tupla di valori Integer positivi la cui lunghezza è uguale al valore di arietà dell'aggregazione. Ogni componente di questa tupla non deve essere maggiore del componente corrispondente di **InputShape**. 
-	**Stride**: (facoltativo) definisce le dimensioni dei piani di inserimento della convoluzione (una dimensione di piano per ogni dimensione), ovvero la distanza tra i nodi centrali. Il valore deve essere una tupla di valori Integer positivi, la cui lunghezza corrisponde al valore di arietà dell'aggregazione. Ogni componente di questa tupla non deve essere maggiore del componente corrispondente di **KernelShape**. Il valore predefinito è una tupla con tutti i componenti uguali a uno. 
-	**Padding**: (facoltativo) determina l'input deve essere riempito usando uno schema di riempimento predefinito. Il valore può essere un singolo valore booleano o una tupla di valori booleani, la cui lunghezza corrisponde al valore di arietà dell'aggregazione. Un singolo valore booleano viene esteso in modo da diventare una tupla di lunghezza corretta con tutti i componenti uguali al valore specificato. Se il valore per una dimensione è true, l'origine sarà riempita in modo logico in quella dimensione con celle a valore zero per supportare altre applicazioni di kernel, in modo che i nodi centrali del primo e dell'ultimo kernel in quella dimensione siano i primi e gli ultimi nodi di quella dimensione nel livello di origine. Il numero di nodi "fittizi" in ogni dimensione viene quindi determinato automaticamente, in modo da inserire esattamente (InputShape[d] - 1) / Stride[d] + 1 nel livello di origine riempito. Se il valore per una dimensione è false, i kernel verranno definiti in modo che il numero di nodi esclusi in ogni lato sia uguale (con una differenza massima di 1). Il valore predefinito di questo attributo è una tupla con tutti i componenti uguali a false.
-	**UpperPad** e **LowerPad**: (facoltativi) permettono di controllare la quantità di riempimento da usare. Questi attributi possono essere definiti se e solo se **Padding** ***non*** è stato definito. I valori devono essere tuple con valori Integer la cui lunghezza corrisponde all'arietà dell'aggregazione. Quando questi attributi sono specificati, i nodi "fittizi" vengono aggiunti alle estremità superiori e inferiori di ogni dimensione del livello di input. Il numero di nodi aggiunti alle estremità inferiori e superiori nella dimensione è determinato rispettivamente da **LowerPad**[i] e **UpperPad**[i]. Per assicurare che i kernel corrispondano solo a nodi "effettivi" e non a nodi "fittizi", è necessario che siano soddisfatte le condizioni seguenti:
...
-	**Sharing**: (facoltativo) definisce la condivisione dei pesi per ogni dimensione della convoluzione. Il valore può essere un singolo valore booleano o una tupla di valori booleani, la cui lunghezza corrisponde al valore di arietà dell'aggregazione. Un singolo valore booleano viene esteso in modo da diventare una tupla di lunghezza corretta con tutti i componenti uguali al valore specificato. Il valore predefinito è una tupla costituita interamente da valori true. 
-	**MapCount**: (facoltativo) definisce il numero di mapping di funzionalità per l'aggregazione convoluzionale. Il valore può essere un singolo valore Integer positivo o una tupla di valori Integer positivi, la cui lunghezza corrisponde al valore di arietà dell'aggregazione. Un singolo valore Integer viene esteso in modo da diventare una tupla di lunghezza corretta quando i primi componenti sono uguali al valore specificato e tutti i componenti rimanenti sono uguali a uno. Il valore predefinito è uno. Il numero totale di mapping di funzionalità è il prodotto dei componenti della tupla. La fattorizzazione di questo numero totale nei componenti determina il modo in cui i valori di mapping di funzionalità vengono raggruppati nei nodi di destinazione. 
-	**Weights**: (facoltativo) definisce i pesi iniziali per l'aggregazione. Il valore deve essere una tupla di valori a virgola mobile, la cui lunghezza corrisponde al numero di kernel per il numero di pesi per ogni kernel, come definito di seguito. I pesi predefiniti sono generati in modo casuale.  
...
-	Il numero di nodi nel livello di destinazione è 5 * 12 * 12 = 1440.  
...
**Avviso:** i nodi del livello di destinazione corrispondono ai neuroni che costituiscono i nodi centrali dei kernel. Ad esempio, se KernelShape[d] è dispari, KernelShape[d]/2 corrisponderà al nodo centrale del kernel. Se KernelShape[d] è pari, il nodo centrale si troverà in KernelShape[d]/2 - 1. Se **Padding**[d] è false, il primo e l'ultimo nodo KernelShape[d]/2 non avranno quindi nodi corrispondenti nel livello di destinazione. Per evitare questa situazione, definire **Padding** come [true, true, ..., true].  

Oltre ai quattro attributi descritti in precedenza, le aggregazioni di normalizzazione delle risposte supportano anche gli attributi seguenti:  

-	**Alpha**: (obbligatorio) specifica un valore a virgola mobile corrispondente a ***α*** nella formula precedente. 
-	**Beta**: (obbligatorio) specifica un valore a virgola mobile corrispondente a ***β*** nella formula precedente. 
-	**Offset**: (obbligatorio) specifica un valore a virgola mobile corrispondente a ***k*** nella formula precedente. Assume il valore predefinito 1.  
...
-	Il livello di origine include cinque mapping, ognuno con dimensioni pari a 12x12, per un totale di 1440 nodi. 
-	Il valore di **KernelShape** indica che si tratta di un livello di normalizzazione nello stesso mapping, in cui l'intorno è un rettangolo 3x3. 
-	Il valore predefinito di **Padding** è false, quindi il livello di destinazione ha solo 10 nodi in ogni dimensione. Per includere un nodo nel livello di destinazione corrispondente a ogni nodo nel livello di origine, aggiungere Padding = [true, true, true] e cambiare le dimensioni di RN1 in [5, 12, 12].  

###Dichiarazione delle condivisioni 
Net# supporta facoltativamente la definizione di più aggregazioni con pesi condivisi. I pesi di qualsiasi aggregazione possono essere condivisi purché le strutture siano uguali. La notazione seguente descrive la sintassi per la definizione di aggregazioni con pesi condivisi:  
...
###Definire una semplice rete neurale personalizzata: Esempio "Hello World"
...
L'esempio illustra alcuni comandi di base e il rispettivo ordine:  
...
-	La terza riga definisce il livello di output, denominato O, e include 10 nodi di output. Per le reti neurali di classificazione è disponibile un nodo di output per ogni classe. La parola chiave **sigmoid** indica la funzione di output applicata al livello di output.   

###Definire più livelli nascosti: esempio di visione artificiale
...
-	Il livello di output, Result, è un livello di destinazione in due aggregazioni di connessioni. Il livello di destinazione di una delle aggregazioni è il secondo livello nascosto (Gather) e il livello di destinazione dell'altra aggregazione è il livello di input MetaData.
-	I livelli nascosti, ByRow e ByCol, specificano la connettività filtrata usando espressioni del predicato. In particolare, il nodo in ByRow in corrispondenza di [x, y] è connesso ai nodi in Pixels che hanno la prima coordinata di indice uguale alla prima coordinata del nodo, x. Analogamente, il nodo in ByCol in corrispondenza di [x, y] è connesso ai nodi in Pixels che hanno la seconda coordinata di indice entro una delle seconde coordinate del nodo, y.  

###Definire una rete convoluzionale per la classificazione a più classi: esempio di riconoscimento delle cifre
La definizione della rete seguente, progettata per riconoscere numeri, illustra alcune tecniche avanzate per la personalizzazione di una rete neurale.  
...
-	Il numero di pesi per ogni kernel è 1 + **KernelShape**\[0] * **KernelShape**\[1] * **KernelShape**\[2] = 1 + 1 * 5 * 5 = 26. Or 26 * 50 = 1300.
