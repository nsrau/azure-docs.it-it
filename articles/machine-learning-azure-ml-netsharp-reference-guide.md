...
Ogni ***livello in training*** (un livello nascosto o di output) dispone di una o più ***aggregazioni di connessione***. Un'aggregazione di connessioni è costituita da un livello di origine e da una specifica delle connessioni provenienti da quel livello di origine. Tutte le connessioni in una determinata aggregazione condividono lo stesso ***livello di origine*** e lo stesso ***livello di destinazione***. In Net# un'aggregazione di connessioni è considerata come appartenente al livello di destinazione dell'aggregazione.  
...
Una specifica struttura di rete neurale è composta da tre sezioni: il **dichiarazione di costante**, il **livello di dichiarazione**, la **dichiarazione di connessione**. È inoltre disponibile una funzione facoltativa **Condivisione dichiarazione**. Le sezioni possono essere specificate in qualsiasi ordine.  
...
-	Il prodotto delle dimensioni è il numero di nodi nel livello. In questo esempio, sono disponibili due dimensioni [5,20], che significa che esistono  100 nodi nel livello.
...
<!-- RIMUOVERE IL CONTENUTO FINO A QUANDO QUESTA FUNZIONALITÀ NON È SUPPORTATA NEL PRODOTTO
Per specificare che il numero di nodi in un livello deve essere determinato automaticamente, usare la parola chiave **auto**. La parola chiave **auto**  ha effetti diversi, in base al livello:  
...
La dichiarazione di livello per un livello sottoponibile a training, i livelli nascosti o di output, può includere facoltativamente la funzione di output, definita anche funzione di attivazione, che assume il valore predefinito **sigmoid**. Sono supportate le funzioni di output seguenti:  
...
Immediatamente dopo aver definito il livello sottoponibile a training, è necessario dichiarare connessioni tra i livelli definiti. La dichiarazione dell'aggregazione di connessione inizia con la parola chiave **da**, seguita dal nome del livello di origine dell'aggregazione e dal tipo di aggregazione di connessioni da creare.   

Sono attualmente supportati cinque tipi di aggregazioni di connessioni:  

-	Aggregazioni **complete**, indicate dalla parola chiave **tutti**
-	Aggregazioni **filtrate**, indicate dalla parola chiave **dove**, seguita da un'espressione del predicato
-	Aggregazioni **convoluzionale** , indicate dalla parola chiave **convolve**, seguita dagli attributi convoluzionali
-	Aggregazioni **pooling**, indicate dalle parole chiave **pool massimo** o **pool medio**
-	Aggregazioni **normalizzazione risposta**, indicate dalla parola chiave **norm risposta**  	
...
-	Nel predicato per ByRow, **s** è un parametro che rappresenta un indice nella matrice rettangolare di nodi del livello di input Pixels, mentre **d** è un parametro che rappresenta un indice nella matrice di nodi del livello nascosto ByRow. Il tipo di **s** e **d** è una tupla di valori Integer con lunghezza pari a due. Concettualmente, gli intervalli **s** su tutte le coppie di numeri interi con 0 <= s[0] < 10 e 0 <= s[1] < 20 e gli intervalli **d** su tutte le coppie di numeri interi con 0 <= d[0] < 10 e 0 <= d[1] < 12. 
...
Per definire la forma e le posizioni dei kernel, utilizzare gli attributi **KernelShape**, **Stride**, **Padding**, **LowerPad** e **UpperPad**:   
...
-	**Spaziatura interna**: (facoltativo) determina l'input deve essere riempito usando uno schema di riempimento predefinito. Il valore può essere un singolo valore booleano o una tupla di valori booleani, con lunghezza corrispondente al valore del grado dell'aggregazione. Un singolo valore booleano viene esteso in modo da diventare una tupla di lunghezza corretta con tutti i componenti uguali al valore specificato. Se il valore per una dimensione è True, l'origine sarà riempita in modo logico in quella dimensione con celle a valore zero per supportare altre applicazioni di kernel, in modo che i nodi centrali del primo e dell'ultimo kernel in quella dimensione siano i primi e gli ultimi nodi di quella dimensione nel livello di origine. Il numero di nodi "fittizi" in ogni dimensione viene quindi determinato automaticamente, in modo da adattarsi esattamente (InputShape[d] - 1) / Stride[d] + 1 nel livello di origine riempito. Se il valore per una dimensione è False, i kernel verranno definiti in modo che il numero di nodi esclusi in ogni lato sia uguale (con una differenza massima di 1). Il valore predefinito di questo attributo è una tupla con tutti i componenti uguali a False.
-	**UpperPad** e **LowerPad**: (facoltativi) permettono di controllare la quantità di riempimento da usare. Questi attributi possono essere definiti se e solo se **Spaziatura interna** non *** è *** stata definita. I valori devono essere tuple con numeri interi con lunghezza corrispondente al grado dell'aggregazione. Quando questi attributi sono specificati, i nodi "fittizi" vengono aggiunti alle estremità superiori e inferiori di ogni dimensione del livello di input. Il numero di nodi aggiunti alle estremità inferiori e superiori in ogni dimensione è determinato rispettivamente da **LowerPad**[i] e **UpperPad**[i]. Per assicurare che i kernel corrispondano solo a nodi "effettivi" e non a nodi "fittizi", è necessario che siano soddisfatte le condizioni seguenti:
	-	Ciascun componente di **LowerPad** deve essere rigorosamente inferiore a KernelShape[d]/2. 
...
-	**Pesi**: (facoltativo) definisce i pesi iniziali per l'aggregazione. Il valore deve essere una tupla di valori a virgola mobile con una lunghezza corrispondente al numero di pesi per kernel, come definito in seguito in questo articolo. I pesi predefiniti sono generati in modo casuale.  
...
La **normalizzazione delle risposte** è uno schema di normalizzazione locale introdotto per la prima volta da Geoffrey Hinton, e altri, in un articolo relativo alla classificazione ImageNet con reti neurali convoluzionali (vedere la sezione 3.3). La normalizzazione delle risposte viene usata per semplificare la generalizzazione nelle reti neurali. Quando un neurone opera a un livello di attivazione molto elevato, un livello di normalizzazione delle risposte locale sopprime il livello di attivazione dei neuroni circostanti. Questa operazione viene eseguita utilizzando i tre parametri (***Î±***, ***Î²*** e ***k***) e una struttura convoluzionle (o a forma di prossimità). Ogni neurone nel livello di destinazione ***y*** corrisponde a un neurone ***x*** nel livello di origine. Il livello di attivazione di ***y*** è dato dalla formula seguente, dove ***f*** è il livello di attivazione di un neurone e ***Nx*** è il kernel (o il set di neuroni in prossimità di ***x***),come definito dalla struttura convoluzionale seguente:  
...
**Avviso**: i nodi del livello di destinazione corrispondono ai neuroni che costituiscono i nodi centrali dei kernel. Ad esempio, se KernelShape[d] è dispari, KernelShape[d]/2 corrisponderà al nodo centrale del kernel. Se KernelShape[d] è pari, il nodo centrale si trova su KernelShape[d]/2 - 1. Pertanto, se **Padding**[d] è False, il primo e l'ultimo nodo KernelShape[d]/2 non dispongono di nodi corrispondenti nel livello di destinazione. Per evitare questa situazione, definire **Padding** come [true, true, â€¦, true].  
...
-	**Alpha**: (obbligatorio) Specifica un valore a virgola mobile che corrisponde a ***Î±*** nella formula precedente. 
-	**Beta**: (obbligatorio) Specifica un valore a virgola mobile che corrisponde a ***Î²*** nella formula precedente. 
...
-	La terza riga definisce il livello di output (denominato O) e contiene 10 nodi di output. Per le reti neurali di classificazione è disponibile un nodo di output per ogni classe. La parola chiave **sigmoid** indica che la funzione di output è applicata al livello di output.   
...
-	I livelli nascosti, ByRow e ByCol, specificano connettività filtrata usando espressioni del predicato. Più precisamente, il nodo in ByRow su [x, y] è connesso ai nodi in pixel con la prima coordinata indice uguale alla prima coordinata del nodo, x. Analogamente, il nodo in ByCol su [x, y] è connesso ai nodi in pixel con la seconda coordinata indice all'interno della seconda coordinata del nodo, y.  
