...
Usare i valori relativi ad autorità emittente e chiave SAS recuperati dal portale di gestione come descritto nella sezione precedente.
...
In questo esempio viene costruito un oggetto **NamespaceManager** mediante la classe **CloudConfigurationManager** di Azure con una stringa di connessione costituita dall'indirizzo di base di uno spazio dei nomi del bus di servizio e dalle credenziali SAS appropriate che dispongono delle autorizzazioni per gestirlo. Il formato della stringa di connessione è il seguente:
...
Per il metodo **CreateQueue** sono disponibili overload che consentono di ottimizzare le proprietà della coda, ad esempio per impostare il valore di durata TTL predefinito da applicare ai messaggi inviati alla coda. Per applicare queste impostazioni, viene usata la classe **QueueDescription**. Il seguente esempio illustra come creare una coda denominata "TestQueue" con una dimensione massima pari a 5 GB e una durata predefinita per i messaggi pari a 1 minuto:
...
Per inviare un messaggio a una coda del bus di servizio, l'applicazione crea un oggetto **QueueClient** usando la stringa di connessione.

Il seguente codice illustra come creare un oggetto **QueueClient** per la coda "TestQueue" creata in precedenza tramite la chiamata API **CreateFromConnectionString**:
...
Il seguente esempio illustra come inviare cinque messaggi di test all'oggetto **QueueClient** di "TestQueue" ottenuto nel frammento di codice riportato sopra:
...
In questo esempio viene configurato il callback **OnMessage** usando un oggetto **OnMessageOptions**. **AutoComplete** è impostato su **false** per abilitare il controllo manuale della chiamata **Complete** sul messaggio ricevuto.
