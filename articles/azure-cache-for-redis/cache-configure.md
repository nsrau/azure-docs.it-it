...
> [!NOTE]
> L'accesso SSL di Cache Redis di Azure supporta TLS 1.0 per impostazione predefinita. Volendo, è possibile versione aumentare la versione minima di TLS supportata fino a TLS 1.2 selezionandola nell'elenco a discesa **Versione minima TLS** del pannello **Impostazioni avanzate** e quindi facendo clic su **Salva**.
...
>[!NOTE]
>Oltre ad archiviare le metriche della cache, è possibile [eseguirne lo streaming a un hub eventi o inviarle a Log Analytics](../azure-monitor/platform/stream-monitoring-data-event-hubs.md).
>
>
...
> [!NOTE]
> Le impostazioni `databases` può essere configurata solo durante la creazione della cache e solo usando PowerShell, l'interfaccia della riga di comando o altri client di gestione. Per un esempio di configurazione di `databases` durante la creazione della cache con PowerShell, vedere [New-AzRedisCache](cache-howto-manage-redis-cache-powershell.md#databases).
> 
> 
