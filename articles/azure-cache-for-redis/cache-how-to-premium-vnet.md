...
- Tutte le connessioni in uscita a internet possono essere effettuate tramite un client del dispositivo di controllo locale.
...
<sup>1</sup> questi indirizzi IP di proprietà di Microsoft vengono utilizzati per risolvere la VM Host che viene usato DNS di Azure.

<sup>3</sup> non necessaria per le subnet con nessun server DNS personalizzato, o versione successiva redis cache che ignorano DNS personalizzato.
...
<sup>2</sup> è possibile usare il Tag di servizio 'AzureLoadBalancer' (Resource Manager) (o 'AZURE_LOADBALANCER' per la distribuzione classica) per la creazione di regole NSG.
