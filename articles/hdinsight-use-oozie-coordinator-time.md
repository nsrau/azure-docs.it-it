...
In questo articolo verrà illustrato come definire i flussi di lavoro e i coordinatori e come attivare i processi del coordinatore in base al tempo. Prima di procedere è utile consultare [Usare Oozie con HDInsight][].
...
HDInsight utilizza l'archivio BLOB di Azure per l'archiviazione dei dati. Questa risorsa è denominata *WASB* o *Archiviazione BLOB di Windows Azure*. WASB è l'implementazione Microsoft di HDFS sull'archivio BLOB di Azure. Per altre informazioni, vedere [Usare l'archivio BLOB di Azure con HDInsight][].
...
Per risolvere i problemi relativi a un flusso di lavoro, consultare il file di log di Oozie in C:\\apps\\dist\\oozie-3.3.2.1.3.2.0-05\\oozie-win-distro\\logs\\Oozie.log dal nodo head del cluster. Per informazioni su RDP vedere [Amministrazione di cluster HDInsight tramite il portale di gestione][].
...
In questa esercitazione si è appreso come definire un flusso di lavoro di Oozie e un coordinatore Oozie e come eseguire un processo di coordinatore Oozie con Azure PowerShell. Per ulteriori informazioni, vedere gli articoli seguenti:
