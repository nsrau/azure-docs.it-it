...
Per analizzare il traffico, è necessario avere un'istanza di Network Watcher esistente o [abilitare un'istanza di Network Watcher](network-watcher-create.md) in ogni area in cui sono presenti gruppi di sicurezza di rete per cui si vuole analizzare il traffico. È possibile abilitare analisi del traffico per gruppi ospitate in una delle [aree supportate](#supported-regions-nsg).
...
Se si prova ad abilitare analisi del traffico per un NSG ospitato in un'area diversa dalle [aree supportate](#supported-regions-nsg), viene visualizzato un errore "non trovato".
...
3. Selezionare un account di archiviazione esistente nel quale archiviare i log dei flussi. Assicurarsi che la risorsa di archiviazione non disponga di "Data Lake Storage Gen2 spazio dei nomi gerarchico abilitato" impostato su true.
...
Per visualizzare Analisi del traffico, cercare **Network Watcher** nella barra di ricerca del portale. Una volta all'interno Network Watcher, per esplorare analisi del traffico e le relative funzionalità, selezionare **analisi del traffico** dal menu a sinistra. 
...
Per ottenere risposte alle domande più frequenti, vedere domande [frequenti su analisi del traffico](traffic-analytics-faq.md).
