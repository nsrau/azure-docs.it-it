...
> [!NOTE]
> Analisi del traffico ora supporta la raccolta di dati dei registri di flusso del gruppo di sicurezza di rete a una frequenza maggiore di 10 min
...
- Registri di flusso del gruppo di sicurezza di rete :consentono di visualizzare informazioni sul traffico IP in ingresso e in uscita tramite un gruppo di sicurezza di **rete.Network security group (NSG) flow logs**: Allow you to view information about ingress and gress IP traffic through a network security group. Sono scritti in formato JSON e mostrano i flussi in ingresso e in uscita per ogni regola, la scheda di rete a cui si applica il flusso, informazioni a cinque tuple relative al flusso (indirizzo IP di origine/destinazione, porta di origine/destinazione e protocollo) e se il traffico è consentito o meno. Per altre informazioni sui log dei flussi dei gruppi di sicurezza di rete, vedere [Log dei flussi per i gruppi di sicurezza di rete](network-watcher-nsg-flow-logging-overview.md).
- **Log Analytics**: servizio di Azure che raccoglie i dati di monitoraggio e li archivia in un repository centrale. Questi dati possono includere eventi, dati sulle prestazioni o dati personalizzati forniti tramite l'API di Azure. Dopo essere stati raccolti, i dati sono disponibili per generare avvisi, per l'analisi e per l'esportazione. Le applicazioni di monitoraggio, ad esempio il monitoraggio delle prestazioni di rete e l'analisi del traffico, vengono create usando i log di Monitoraggio di Azure come base. Per altre informazioni, vedere Log di Monitoraggio di Azure.For more [information,](../log-analytics/log-analytics-overview.md?toc=%2fazure%2fnetwork-watcher%2ftoc.json)see Azure Monitor logs .
- Area di **lavoro di Log Analytics:** un'istanza dei log di Monitoraggio di Azure, in cui vengono archiviati i dati relativi a un account Azure.Log Analytics workspace : An instance of Azure Monitor logs, where the data relativeing to an Azure account, is stored. Per ulteriori informazioni sulle aree di lavoro di Log Analytics, vedere [Creare un'area](../azure-monitor/learn/quick-create-workspace.md?toc=%2fazure%2fnetwork-watcher%2ftoc.json)di lavoro di Log Analytics.
...
## <a name="supported-regions-nsg"></a>Aree geografiche supportate: NSGSupported regions: NSG 
...
## <a name="supported-regions-log-analytics-workspaces"></a>Aree supportate: aree di lavoro di Log AnalyticsSupported regions: Log Analytics Workspaces
...
Per analizzare il traffico, è necessario avere un'istanza di Network Watcher esistente o [abilitare un'istanza di Network Watcher](network-watcher-create.md) in ogni area in cui sono presenti gruppi di sicurezza di rete per cui si vuole analizzare il traffico. L'analisi del traffico può essere abilitata per i gruppi di sicurezza di rete ospitati in una qualsiasi delle [aree supportate.](#supported-regions-nsg)
...
Nel portale di Azure passare a **Network watcher**e quindi selezionare Registri di flusso del gruppo di sicurezza di **rete.** Selezionare il gruppo di sicurezza di rete per cui si vuole abilitare un log del flusso del gruppo di sicurezza di rete, come illustrato nell'immagine seguente:
...
Se si tenta di abilitare l'analisi del traffico per un gruppo di sicurezza di rete ospitato in qualsiasi area diversa da quella [supportata,](#supported-regions-nsg)viene visualizzato un errore "Non trovato".
...
Ripetere i passaggi precedenti per qualsiasi altro gruppo di sicurezza di rete per il quale si vuole abilitare Analisi del traffico. I dati dai log dei flussi vengono inviati all'area di lavoro, quindi assicurarsi che le leggi locali e le normative in vigore nel proprio paese consentano l'archiviazione dei dati nell'area in cui è presente l'area di lavoro. Se sono stati impostati intervalli di elaborazione diversi per gruppi di sicurezza di rete diversi, i dati verranno raccolti a intervalli diversi. Ad esempio: è possibile scegliere di abilitare l'intervallo di elaborazione di 10 min per i VNET critici e di 1 ora per i VNET non critici.

È anche possibile configurare l'analisi del traffico usando il cmdlet Di PowerShell Set-AzNetworkWatcherConfigFlowLog in Azure PowerShell.You can also configure traffic analytics using the [Set-AzNetworkWatcherConfigFlowLog](/powershell/module/az.network/set-aznetworkwatcherconfigflowlog) PowerShell cmdlet in Azure PowerShell. Eseguire `Get-Module -ListAvailable Az` per trovare la versione installata. Se è necessario eseguire l'aggiornamento, vedere [Installare e configurare Azure PowerShell](/powershell/azure/install-Az-ps).

## <a name="view-traffic-analytics"></a>Visualizzare Analisi del traffico

Per visualizzare Analisi del traffico, cerca **Network Watcher** nella barra di ricerca del portale. Una volta all'interno di Network Watcher, per esplorare l'analisi del traffico e le relative funzionalità, seleziona **Analisi del traffico** nel menu a sinistra. 
...
- La mappa geografica mostra la barra multifunzione superiore per la selezione di parametri quali i data center (Distribuito/No-distribuzione/Active/Inactive/Analisi del traffico abilitato/Analisi del traffico non abilitato) e i paesi/aree che contribuiscono al traffico Benign/Malicious alla distribuzione attiva:
...
- La mappa geografica mostra la distribuzione del traffico a un data center da paesi/regioni e continenti che comunicano con esso in linee colorate blu (traffico benigno) e rosso (traffico dannoso):

    ![Vista mappa geografica che mostra la distribuzione del traffico verso paesi/regioni e continenti](./media/traffic-analytics/geo-map-view-showcasing-traffic-distribution-to-countries-and-continents.png)
...
- È possibile filtrare la topologia di rete virtuale basata su sottoscrizioni, aree di lavoro, gruppi di risorse e intervallo di tempo. Filtri aggiuntivi che consentono di comprendere il flusso sono: Tipo di flusso (InterVNet, IntraVNET e così via), Direzione flusso (in ingresso, in uscita), Stato del flusso (Consentito, Bloccato), VNET (mirato e connesso), Tipo di connessione (Peering o Gateway - P2S e S2S) e NSG. Usare questi filtri per concentrarsi sulle reti virtuali che si desidera esaminare in dettaglio.
...
Per ottenere risposte alle domande frequenti, vedere [Domande frequenti sull'analisi del traffico.](traffic-analytics-faq.md)
...
- Per comprendere lo schema e i dettagli di elaborazione di Analisi del traffico, vedere Schema di [analisi del traffico.](traffic-analytics-schema.md)
