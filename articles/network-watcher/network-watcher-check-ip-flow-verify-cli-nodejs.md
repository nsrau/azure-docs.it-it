...
È necessario l'indirizzo IP di una scheda di interfaccia di rete nella macchina virtuale. Recuperare le schede di interfaccia di rete per una macchina virtuale con il comando seguente. Se l'indirizzo IP da testare nella macchina virtuale è già noto, è possibile ignorare questo passaggio.
...
Eseguire il cmdlet `network watcher ip-flow-verify` per testare il traffico. In questo esempio, viene usato il primo indirizzo IP della prima scheda di interfaccia di rete:
