<properties 
	pageTitle="Usare le azioni script in HDInsight per installare Solr nel cluster Hadoop | Azure" 
	description="Informazioni su come personalizzare un cluster HDInsight per installare Solr. Verrà usata un'opzione di configurazione Azione script per installare Solr tramite uno script." 
	services="hdinsight" 
	documentationCenter="" 
	authors="nitinme" 
	manager="paulettm" 
...
È possibile installare Solr in qualsiasi tipo di cluster in Hadoop su HDInsight usando la personalizzazione dei cluster **Azione script**. Azione script consente di eseguire script per personalizzare un cluster solo durante la creazione. Per altre informazioni, vedere [Personalizzare cluster HDInsight mediante Azione script][hdinsight-cluster-customize].

In questo argomento si apprenderà come installare Solr mediante le azioni script. Solr è una piattaforma di ricerca avanzata e offre funzionalità di ricerca di livello aziendale per i dati gestiti da Hadoop. Dopo l'installazione di Solr nel cluster HDInsight, verrà illustrato anche come eseguire ricerche nei dati mediante Solr.
...
- [Che cos'è Solr?](#whatis)
- [Come si installa Solr?](#install)
- [Come si usa Solr in HDInsight?](#usesolr)
- [Installare Solr nei cluster Hadoop di HDInsight mediante PowerShell](#usingPS)
- [Installare Solr nei cluster Hadoop di HDInsight mediante .NET SDK](#usingSDK) 
...
2. Nella pagina **Azioni script** della procedura guidata fare clic su **aggiungi azione script** per specificare i dettagli relativi all'azione script, come descritto di seguito:
	...
	<table border='1'>
		...
		<tr><td>URI script</td>
			<td>Specificare l'URI dello script da richiamare per personalizzare il cluster. Ad esempio, <i>https://hdiconfigactions.blob.core.windows.net/solrconfigactionv01/solr-installer-v01.ps1</i></td></tr>
...
È inoltre possibile usare lo script per installare Solr in HDInsight usando PowerShell o .NET SDK per HDInsight. Le istruzioni relative a queste procedure vengono fornite più avanti in questo argomento.
...
2. **Indicizzare Solr mediante il caricamento di file di dati**. Quando si indicizza Solr, si inseriscono documenti su cui potrebbe essere necessario eseguire ricerche. Per indicizzare Solr, connettersi tramite RDP al cluster, passare al desktop, aprire la riga di comando di Hadoop e quindi passare a **C:\apps\dist\solr-4.7.2\example\exampledocs**. Eseguire il comando seguente: 
	...
	Nella console viene visualizzato il seguente output.
...
4. **Consigliato: backup dei dati indicizzati da Solr nel BLOB di Archiviazione di Azure (WASB) associato al cluster HDInsight**. È consigliabile eseguire il backup dei dati indicizzati dai nodi del cluster Solr in WASB. Eseguire quindi la procedura seguente:

	1. Aprire Internet Explorer dalla sessione RDP, quindi selezionare l'URL seguente:

			http://localhost:8983/solr/replication?command=backup

		Viene visualizzata una risposta analoga alla seguente
	...
	2. Nella sessione remota passare a {SOLR_HOME}\{Collection}\data. Per il cluster creato usando lo script di esempio, sarà uguale a **C:\apps\dist\solr-4.7.2\example\solr\collection1\data**. In questo percorso dovrebbe essere creata una cartella snapshot con nome simile a **snapshot.*timestamp***.
	
	3. Comprimere la cartella snapshot e caricarla in WASB. Dalla riga di comando di Hadoop passare al percorso della cartella snapshot usando il comando seguente:
...
.NET SDK per HDInsight fornisce librerie client .NET che semplificano l'uso di HDInsight da un'applicazione .NET. Questa sezione fornisce istruzioni sull'uso delle azioni script da SDK per eseguire il provisioning di un cluster con Solr installato. È necessario eseguire le procedure seguenti:
...
Specificare un nome per il cluster e premere INVIO per eseguire il provisioning di un cluster con Solr installato.
...
- [Installare R nei cluster HDInsight][hdinsight-install-r]. Usare la personalizzazione dei cluster per installare R nei cluster Hadoop di HDInsight. R è un linguaggio open source e un ambiente per l'elaborazione statistica. R fornisce centinaia di funzioni statistiche integrate e un proprio linguaggio che combina aspetti di programmazione funzionale con aspetti di programmazione orientata agli oggetti. Offre inoltre funzionalità complete di grafica.
