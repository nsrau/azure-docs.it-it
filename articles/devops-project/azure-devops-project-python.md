---
title: Creare una pipeline di CI/CD per Python con il progetto DevOps di Azure | Guida introduttiva
...
ms.locfileid: "47405650"
---
# <a name="create-a-cicd-pipeline-for-python-with-the-azure-devops-project"></a>Creare una pipeline di CI/CD per Python con il progetto DevOps di Azure

Il progetto DevOps di Azure offre un'esperienza semplificata per la creazione di risorse di Azure e la configurazione di una pipeline di integrazione continua (CI, Continuous Integration) e recapito continuo (CD, Continuous Delivery) per l'app Python in Azure DevOps Services.  
...
1. Il framework di esempio predefinito è **Django**. Lasciare l'impostazione predefinita e quindi scegliere **Avanti**.  

1. La destinazione della distribuzione predefinita è **App Web per contenitori**.  Il framework applicazione, scelto nei passaggi precedenti, determina il tipo di destinazione della distribuzione del servizio di Azure disponibile.  Lasciare il servizio predefinito e quindi scegliere **Avanti**.
...
Il progetto DevOps di Azure configura automaticamente un trigger di compilazione e di versione di integrazione continua.  A questo punto, si è pronti per collaborare con un team a un'app Python con un processo di CI/CD che distribuisce automaticamente il lavoro più recente nel sito Web.
...
1. Sul lato sinistro del browser passare al file **app/templates/app/index.html**.
