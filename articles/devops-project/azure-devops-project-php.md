---
title: Creare una pipeline di CI/CD per PHP con il progetto DevOps di Azure | Guida introduttiva
...
ms.locfileid: "47409627"
---
# <a name="create-a-cicd-pipeline-for-php-with-the-azure-devops-project"></a>Creare una pipeline di CI/CD per PHP con il progetto DevOps di Azure

Il progetto DevOps di Azure offre un'esperienza semplificata per la creazione di risorse di Azure e la configurazione di una pipeline di integrazione continua (CI, Continuous Integration) e recapito continuo (CD, Continuous Delivery) per l'app PHP in Azure DevOps Services.  
...
1. Selezionare l'applicazione di esempio **PHP**.  Gli esempi di PHP includono diversi framework applicazione.

1. Il framework di esempio predefinito è **Laravel**. Lasciare l'impostazione predefinita e quindi scegliere **Avanti**.  
...
Il progetto DevOps di Azure configura automaticamente un trigger di compilazione e di versione di integrazione continua.  A questo punto, si è pronti per collaborare con un team a un'app PHP con un processo di CI/CD che distribuisce automaticamente il lavoro più recente nel sito Web.
...
1. Sul lato sinistro del browser, passare al file **resources/views/welcome.blade.php**.
