...
Il progetto DevOps di Azure offre un'esperienza semplificata per la creazione di risorse di Azure e la configurazione di una pipeline di integrazione continua (CI, Continuous Integration) e recapito continuo (CD, Continuous Delivery) per l'app Node.js in Azure DevOps Services.  
...
Il Progetto DevOps di Azure crea una pipeline CI/CD in Azure.  È possibile creare gratuitamente una nuova organizzazione **Azure DevOps Services** oppure usare un'**organizzazione esistente**.  Il progetto DevOps crea anche **risorse di Azure** nella **sottoscrizione di Azure** di propria scelta.
...
1. Nel browser passare al **dashboard del progetto DevOps di Azure**.  Dovrebbe venire visualizzata una compilazione in corso.  Le modifiche appena apportate vengono compilate e distribuite automaticamente tramite una pipeline di CI/CD di Azure.
...
Quando è stato configurato il processo di CI/CD in questa guida introduttiva, sono state create automaticamente pipeline di compilazione e di versione nel progetto DevOps di Azure. È possibile modificare queste pipeline di compilazione e di versione in base alle esigenze del team. Per altre informazioni, vedere questa esercitazione:
