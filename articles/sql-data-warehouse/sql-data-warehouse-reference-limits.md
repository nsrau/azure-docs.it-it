<properties
   pageTitle="Specifiche di capacità di SQL Data Warehouse | Microsoft Azure"
   description="Specifiche di capacità minime e massime per SQL Data Warehouse."
...
# Specifiche di capacità di SQL Data Warehouse

SQL Data Warehouse presenta limiti molto simili a SQL Server per l'elaborazione di query e le viste di sistema. Alcuni limiti sono diversi a causa dell'architettura parallela e scalabile sottostante. Questo è un riepilogo dei limiti per SQL Data Warehouse.
...
| Account di accesso | Accessi per server. | 500\.000 |
...
| Categoria | Descrizione | Massima |
| :---------------- | :------------------------------------------- | :----------------- |
| Risultati SELECT | Colonne per riga | 4096<br/><br/>Non è possibile avere più di 4096 colonne per riga nel risultato SELECT. Non è garantito che si possa avere sempre 4096. Se il piano di query richiede una tabella temporanea, potrebbe venire applicato il valore massimo di 1024 colonne per tabella.|
| Risultati SELECT | Byte per riga | > 8060<br/><br/>Il numero di byte per riga nei risultati SELECT può essere superiore al valore massimo di 8060 byte consentito per le righe della tabella. Se il piano di query per l'istruzione SELECT richiede una tabella temporanea, potrebbe venire applicato il valore massimo di 8060 byte per tabella.|
| Risultati SELECT | Byte per colonna | > 8000<br/><br/>Il numero di byte per colonna nei risultati SELECT può essere superiore al valore massimo di 8000 byte consentito per le colonne della tabella. Se il piano di query per l'istruzione SELECT richiede una tabella temporanea, potrebbe venire applicato il valore massimo di 8000 byte per tabella.|
...
| Identificatori e costanti per istruzione | Numero di identificatori e costanti di riferimento. | 65\.535<br/><br/>SQL Data Warehouse limita il numero di identificatori e costanti che possono essere contenute in una singola espressione di una query. Il limite è 65.535. Il superamento di questo numero genera un errore 8632 di SQL Server. Per altre informazioni, vedere [Errore interno: è stato raggiunto un limite di servizi di espressione](http://support.microsoft.com/kb/913050/).|
