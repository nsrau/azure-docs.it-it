...
| Errore della generazione di script in SSMS                               | La generazione di uno script per SQL Analytics ha esito negativo se l'opzione "genera script per oggetti dipendenti" è impostata su "true". Come soluzione alternativa, gli utenti devono passare manualmente a **strumenti-> Opzioni-> Esplora oggetti di SQL Server-> generare uno script per le opzioni dipendenti e impostare su false** |
...
| Come implementare le procedure consigliate                              | Per informazioni su come migliorare le prestazioni delle query, vedere l'articolo [procedure consigliate per SQL Analytics](sql-data-warehouse-best-practices.md) . |
