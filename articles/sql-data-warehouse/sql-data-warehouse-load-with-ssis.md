<properties
   pageTitle="Caricare dati con SQL Server Integration Services (SSIS) | Microsoft Azure"
...
# Caricare dati con SQL Server Integration Services (SSIS)

Questa esercitazione illustra come creare un pacchetto di SQL Server Integration Services (SSIS) per spostare dati da una vasta gamma di origini dati in SQL Data Warehouse. È possibile, facoltativamente, ristrutturare, trasformare e pulire i dati man mano che passano attraverso il flusso di dati SSIS.

Dopo aver completato l'esercitazione, si sarà in grado di eseguire queste operazioni.
...
4. Selezionare **Progetto di Integration Services**. Specificare i valori per **Nome** e **Percorso**, quindi scegliere **OK**.
...
7. Nella finestra di dialogo **Crea tabella** eseguire queste operazioni:
    ...
    3. Modificare il tipo di dati della colonna **LineTotal** in **money**. Il tipo di dati **decima**l non è supportato in SQL Data Warehouse. Per informazioni sui tipi di dati supportati, vedere [CREATE TABLE (Azure SQL Data Warehouse, Parallel Data Warehouse)][].
