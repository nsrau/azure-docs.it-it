<properties
   pageTitle="Caricamento di Contoso Retail Data Warehouse in SQL Data Warehouse | Microsoft Azure"
   description="Informazioni su come caricare Contoso Retail Data Warehouse in SQL Data Warehouse usando PolyBase per ottenere le massime prestazioni"
...
# Caricamento di Contoso Retail Data Warehouse

In questo articolo si apprenderà come caricare rapidamente dati nello schema di Contoso Retail Data Warehouse.

Questo set di dati fornisce un'origine dati alternativa che è possibile usare per ottenere informazioni dettagliate su Azure SQL Data Warehouse.

Le dimensioni del set di dati di Contoso sono nell'ordine di GB. I dati non elaborati sono conservati in un archivio BLOB di Azure a cui si accede direttamente per caricarli in SQL Data Warehouse.

In questa esercitazione si apprenderà come:

1. Configurare PolyBase per il caricamento da un contenitore di archiviazione BLOB pubblico.
2. Importare dati con [CTAS][].
3. [RICOMPILARE][] le tabelle per assicurare l'ottimizzazione degli indici columnstore.
4. Creare statistiche su colonne chiave.

## Configurazione di PolyBase

Il primo passaggio del caricamento dei dati consiste nel configurare gli script dei prerequisiti di PolyBase:

### Creare un'origine dati esterna.

Poiché i dati sono conservati in un contenitore pubblico, la prima attività consiste nel creare l'origine dati esterna:
...
In questo caso non è necessario creare una chiave master o credenziali con ambito database, perché il contenitore di archiviazione BLOB è configurato come "pubblico" e ciò significa che chiunque può leggerlo ed enumerarlo.

> [AZURE.IMPORTANT] Se si sceglie di rendere pubblici i contenitori di archiviazione BLOB di Azure, tenere presente che quando i dati lasciano il data center vengono applicati costi di uscita per cui l'utente sarà responsabile.

### Creare un oggetto formato di file esterno.

Il formato di file fornisce le informazioni necessarie per analizzare i file di testo nell'archivio BLOB.
...
In questo caso i dati non sono compressi e delimitati da barre verticali.

## Creare le tabelle esterne.

A questo punto la connettività all'archivio BLOB di Azure è configurata ed è possibile creare le tabelle esterne.

Prima di tutto si dovrà creare uno schema:
...
Ora si creeranno le tabelle esterne. Tenere presente che in questo caso si sta semplicemente fornendo il mapping dei tipi di dati, ovvero l'associazione di questa definizione ai metadati per individuare i file.

Come si noterà, lo script è molto lungo.
...
## Caricare i dati con CTAS

Per caricare i dati è consigliabile utilizzare le tabelle esterne fortemente tipizzate appena create. Saranno quindi sufficienti le semplici istruzioni [CTAS][].

Tuttavia, come prima cosa si creerà un nuovo schema per caricare le tabelle.
...
Per caricare i dati si userà l'istruzione [CTAS][]. Si noti che è presente un'istruzione [CTAS][] per ogni tabella.
...
È possibile monitorare lo stato del caricamento con la `[sys].[dm_pdw_exec_requests]` vista a gestione dinamica (DMV). Poiché alle istruzioni CTAS precedenti è associato un oggetto `label` è anche possibile monitorare l'avanzamento con `label`. Per altre informazioni, vedere l'articolo relativo alle [etichette][].

## Ricompilare gli indici

Infine, per garantire che le tabelle siano in formato columnstore compresso, è possibile eseguire un'operazione di ricompilazione della tabella. In questo modo i rowgroup aperti o chiusi vengono convertiti nel formato columnstore compresso.
...
Per altre informazioni su come controllare lo stato del columnstore cluster, vedere l'articolo che descrive come [gestire gli indici columnstore] [].

## Creare alcuni oggetti statistics

Per creare statistiche su tutte le colonne è possibile utilizzare il codice della stored procedure `prc_sqldw_create_stats` descritto nell'articolo relativo alle [statistiche][].

Di seguito è riportato un subset dell'output che illustra tutte le dimensioni e tutte le colonne di join in Contoso Retail Data Warehouse. Questo è un buon punto di partenza.

È possibile aggiungere manualmente altre statistiche ad altre colonne dei fatti in un secondo momento.

Anche in questo caso lo script è piuttosto lungo.
...
Ora tutti i dati caricati in Azure SQL Data Warehouse. Ottimo lavoro.

Si può iniziare a eseguire query sulle tabelle prendendo spunto dalla query seguente:
...
## Passaggi successivi
Per una panoramica sul caricamento, vedere [Caricare i dati in SQL Data Warehouse][]. Per altre informazioni sulla ricerca degli indici columnstore vedere l'articolo che illustra come [gestire gli indici columnstore][].
...
<!--MSDN references-->
[sys.dm_pdw_exec_requests]: https://msdn.microsoft.com/it-IT/library/mt203887.aspx
[RICOMPILARE]: https://msdn.microsoft.com/it-IT/library/ms188388.aspx
