...
Vedere anche [Pause compute resources] [Pause compute resources] (Sospendere l'attività delle risorse di calcolo), [Resume compute resources][Resume compute resources] (Riprendere l'attività delle risorse di calcolo) e [Scale compute resources][Scale compute resources] (Ridimensionare l'attività delle risorse di calcolo)
...
Veder anche [Capire le transazioni][Capire le transazioni] e [Optimizing transactions][Optimizing transactions] (Ottimizzare le transazioni)
...
Vedere anche [Manage table statistics][Manage table statistics] (Gestire le statistiche delle tabelle), [CREATE STATISTICS][CREATE STATISTICS] e [UPDATE STATISTICS][UPDATE STATISTICS]
...
Vedere anche [Caricare i dati][Caricare i dati], [Guide for using PolyBase][Guide for using PolyBase] (Guida all'uso di PolyBase), [Azure SQL Data Warehouse loading patterns and strategies][Azure SQL Data Warehouse loading patterns and strategies] (Strategie e modelli di carico di Azure SQL Data Warehouse), [Load Data with Azure Data Factory][Load Data with Azure Data Factory] (Caricare dati con Azure Data Factory), [Spostare dati mediante Azure Data Factory][Spostare dati mediante Azure Data Factory], [CREATE EXTERNAL FILE FORMAT][CREATE EXTERNAL FILE FORMAT], [Create Table As Select (CTAS)][Create Table As Select (CTAS)]
...
Vedere anche [Guide for using PolyBase][Guide for using PolyBase] (Guida all'uso di PolyBase)
...
Vedere anche [Table overview][Table overview] (Panoramica sulle tabelle), [Table distribution][Table distribution] (Distribuzione di tabelle), [Selecting table distribution][Selecting table distribution] (Selezionare la distribuzione di tabelle), [CREATE TABLE][CREATE TABLE] e [CREATE TABLE AS SELECT][CREATE TABLE AS SELECT]

## <a name="do-not-overpartition"></a>Non creare un numero eccessivo di partizioni
...
Vedere anche [Table partitioning][Table partitioning] (Partizionamento di tabelle)
...
Vedere anche [Capire le transazioni][Capire le transazioni], [Optimizing transactions][Optimizing transactions] (Ottimizzare le transazioni), [Table partitioning][Table partitioning] (Partizionamento di tabelle), [TRUNCATE TABLE][TRUNCATE TABLE], [ALTER TABLE][ALTER TABLE] e [Create Table As Select (CTAS)][Create Table As Select (CTAS)]
...
Vedere anche [Table overview][Table overview] (Panoramica sulle tabelle), [Table data types][Table data types] (Tipo di dati delle tabelle) e [CREATE TABLE][CREATE TABLE]
...
Vedere anche [Table indexes][Table indexes] (Indicizzazione delle tabelle), [Guida agli indici columnstore][Guida agli indici columnstore] e [Rebuilding columnstore indexes][Rebuilding columnstore indexes] (Nuova compilazione degli indici columnstore)
...
Vedere anche [Gestione della concorrenza e del carico di lavoro][Gestione della concorrenza e del carico di lavoro]
...
Vedere anche [Gestione della concorrenza e del carico di lavoro][Gestione della concorrenza e del carico di lavoro] e [sys.dm_pdw_waits][sys.dm_pdw_waits]
...
Vedere anche [Monitoraggio del carico di lavoro mediante DMV][Monitoraggio del carico di lavoro mediante DMV], [LABEL][LABEL], [OPTION][OPTION], [sys.dm_exec_sessions][sys.dm_exec_sessions], [sys.dm_pdw_exec_requests][sys.dm_pdw_exec_requests], [sys.dm_pdw_request_steps][sys.dm_pdw_request_steps], [sys.dm_pdw_sql_requests][sys.dm_pdw_sql_requests], [sys.dm_pdw_dms_workers], [DBCC PDW_SHOWEXECUTIONPLAN][DBCC PDW_SHOWEXECUTIONPLAN] e [sys.dm_pdw_waits][sys.dm_pdw_waits]

## <a name="other-resources"></a>Altre risorse:
Per informazioni sui problemi più comuni e le relative soluzioni, vedere anche l'articolo sulla [risoluzione dei problemi][risoluzione dei problemi].

Se questo articolo non contiene le informazioni necessarie, provare a usare la funzione "Search for docs" (Cerca documenti) sul lato sinistro della pagina per cercare in tutti i documenti relativi ad Azure SQL Data Warehouse.  Il [forum di MSDN su Azure SQL Data Warehouse][forum di MSDN su Azure SQL Data Warehouse] è stato creato per offrire un posto in cui porre domande ad altri utenti e al team che si occupa di SQL Data Warehouse.  Questo forum viene monitorato attivamente per garantire che venga fornita una risposta a tutte le domande, da un altro utente o da Microsoft.  Se si preferisce porre domande in Stack Overflow, è disponibile anche un [forum di Stack Overflow su Azure SQL Data Warehouse][forum di Stack Overflow su Azure SQL Data Warehouse].

Per inviare richieste di funzionalità è possibile usare la pagina dei [commenti e suggerimenti su Azure SQL Data Warehouse][commenti e suggerimenti su Azure SQL Data Warehouse].  Le richieste aggiunte o i voti per altre richieste sono utili per definire le priorità per le funzionalità.
...
[Tipi di dati per le tabelle]: ./sql-data-warehouse-tables-data-types.md
[Distribuzione di tabelle]: ./sql-data-warehouse-tables-distribute.md
[Indicizzazione delle tabelle]: ./sql-data-warehouse-tables-index.md
...
[Gestire le statistiche delle tabelle]: ./sql-data-warehouse-tables-statistics.md
[Tabelle temporanee]: ./sql-data-warehouse-tables-temporary.md
[Guida per l'uso di PolyBase]: ./sql-data-warehouse-load-polybase-guide.md
[Caricare i dati]: ./sql-data-warehouse-overview-load.md
[Spostare dati con Azure Data Factory]: ../data-factory/data-factory-azure-sql-data-warehouse-connector.md
[Caricare dati con Azure Data Factory]: ./sql-data-warehouse-get-started-load-with-azure-data-factory.md
...
[Informazioni sulle transazioni]: ./sql-data-warehouse-develop-transactions.md
...
[INSERT (Transact-SQL) (Transact-SQL)]: https://msdn.microsoft.com/library/ms174335.aspx
...
[celta della modalità di distribuzione delle tabelle]: https://blogs.msdn.microsoft.com/sqlcat/2015/08/11/choosing-hash-distributed-table-vs-round-robin-distributed-table-in-azure-sql-dw-service/
...
[Strategie e modelli di caricamento di Azure SQL Data Warehouse]: https://blogs.msdn.microsoft.com/sqlcat/2016/02/06/azure-sql-data-warehouse-loading-patterns-and-strategies
