...
Vedere anche [Sospendere le risorse di calcolo][Sospendere le risorse di calcolo], [Riavviare le risorse di calcolo][Riavviare le risorse di calcolo] e [Ridimensionare le prestazioni][Ridimensionare le prestazioni].
...
Vedere anche [Transazioni in SQL Data Warehouse][Transazioni in SQL Data Warehouse] e [Ottimizzazione delle transazioni per SQL Data Warehouse][Ottimizzazione delle transazioni per SQL Data Warehouse].
...
Vedere anche [Managing statistics on tables in SQL Data Warehouse][Managing statistics on tables in SQL Data Warehouse] \(Gestione delle statistiche nelle tabelle in SQL Data Warehouse), [CREATE STATISTICS (Transact-SQL)][CREATE STATISTICS (Transact-SQL)] e [UPDATE STATISTICS (Transact-SQL)][UPDATE STATISTICS (Transact-SQL)]
...
Vedere anche [INSERT (Transact-SQL)][INSERT (Transact-SQL)]
...
Vedere anche [Caricare i dati in SQL Data Warehouse][Caricare i dati in SQL Data Warehouse], [Guida per l'uso di PolyBase in SQL Data Warehouse][Guida per l'uso di PolyBase in SQL Data Warehouse], [Azure SQL Data Warehouse loading patterns and strategies][Azure SQL Data Warehouse loading patterns and strategies] \(Strategie e modelli di caricamento di Azure SQL Data Warehouse), [Caricare i dati con Azure Data Factory][Caricare i dati con Azure Data Factory], [Spostare dati da e verso Azure SQL Data Warehouse con Azure Data Factory][Spostare dati da e verso Azure SQL Data Warehouse con Azure Data Factory], [CREATE EXTERNAL FILE FORMAT (Transact-SQL)][CREATE EXTERNAL FILE FORMAT (Transact-SQL)] e [Create Table As Select (CTAS) in SQL Data Warehouse][Create Table As Select (CTAS) in SQL Data Warehouse].
...
Vedere anche [Guida per l'uso di PolyBase][Guida per l'uso di PolyBase]
...
Vedere anche [Panoramica sulle tabelle][Panoramica sulle tabelle], [Distribuzione delle tabelle][Distribuzione delle tabelle], il post di blog relativo alla [scelta della modalità di distribuzione delle tabelle][scelta della modalità di distribuzione delle tabelle], [CREATE TABLE][CREATE TABLE], [CREATE TABLE AS SELECT][CREATE TABLE AS SELECT]
...
Vedere anche [Partizionamento delle tabelle][Partizionamento delle tabelle]
...
Vedere anche [Informazioni sulle transazioni][Informazioni sulle transazioni], [Ottimizzazione delle transazioni][Ottimizzazione delle transazioni], [Partizionamento delle tabelle][Partizionamento delle tabelle], [TRUNCATE TABLE][TRUNCATE TABLE], [ALTER TABLE][ALTER TABLE], [Create table as select (CTAS)][Create table as select (CTAS)]
...
Vedere anche [Panoramica sulle tabelle][Panoramica sulle tabelle], [Tipi di dati per le tabelle][Tipi di dati per le tabelle], [CREATE TABLE][CREATE TABLE]
...
Vedere anche gli argomenti su [indicizzazione di tabelle][indicizzazione di tabelle], [guida agli indici columnstore][guida agli indici columnstore], [ricompilazione degli indici columnstore][ricompilazione degli indici columnstore]
...
Vedere anche [Gestione della concorrenza e del carico di lavoro][Gestione della concorrenza e del carico di lavoro], [sys.dm\_pdw\_waits][sys.dm\_pdw\_waits]
...
[Panoramica sulle tabelle]: ./sql-data-warehouse-tables-overview.md
[Tipi di dati per le tabelle]: ./sql-data-warehouse-tables-data-types.md
[Distribuzione delle tabelle]: ./sql-data-warehouse-tables-distribute.md
...
[Ottimizzazione delle transazioni]: ./sql-data-warehouse-develop-best-practices-transactions.md
