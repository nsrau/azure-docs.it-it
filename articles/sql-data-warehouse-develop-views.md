...
L'esempio seguente aggiunge nuovi record a una dimensione data. Si noti che viene prima creato un nuovo oggetto, DimDate_New, quindi viene rinominato per sostituire la versione originale dell'oggetto. ''' CREATE TABLE dbo.DimDate_New WITH (DISTRIBUTION = REPLICATE, CLUSTERED INDEX (DateKey ASC) ) AS SELECT * FROM dbo.DimDate AS prod UNION ALL SELECT * FROM dbo.DimDate_stg AS stg ;
