...
* `Location`definisce una gerarchia con padre `states` e figlio `cities`. Ogni `location` può avere più `states`di un, che a sua volta `cities`può avere più.
* `ManufactureDate`definisce una gerarchia con padre `year` e figlio `month`. Ogni `ManufactureDate` può avere più `years`di un, che a sua volta `months`può avere più.
...
Si consideri un esempio in `building`cui `floor`la gerarchia `room` **H1** presenta, e come parte della definizione **instanceFieldNames** :
