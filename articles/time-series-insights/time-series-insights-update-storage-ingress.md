...
In entrambi i casi, la proprietà time del file parquet corrisponde all'ora di creazione del BLOB. I dati nella cartella `PT=Time` vengono conservati senza modifiche dopo essere stati scritti nel file. I dati nella cartella `PT=TsId` verranno ottimizzati per la query nel tempo e non sono statici.
