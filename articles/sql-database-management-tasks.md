...
<h1><a id="swap"></a>Come gestire un database SQL</h1>
...
<h2><a id="connect"></a>Procedura: Connettersi al database SQL in Azure mediante Management Studio</h2>
...
<h2><a id="addlogins"></a>Procedura: Aggiungere account di accesso e utenti al database SQL in Azure</h2>
...
È necessario che gli account di accesso creati siano account di accesso con autenticazione di SQL Server. Se sono già disponibili script predefiniti che usano identità utente di Windows o identità basate sulle attestazioni, tale script non funzionerà nel database SQL.
...
2. Usare le istruzioni Transact-SQL seguenti per creare account di accesso. Sostituire il valore di password con una password valida. Selezionare ogni account di accesso singolarmente e quindi fare clic su **Esegui**. Ripetere per gli account di accesso rimanenti.
...
Sono stati ora creati e testati diversi account di accesso. Per altre informazioni, vedere [Gestione di database e account di accesso in database SQL][] e [Monitoraggio di database SQL mediante le viste a gestione dinamica][].
