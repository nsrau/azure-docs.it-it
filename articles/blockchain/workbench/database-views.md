---
title: Viste del database di Azure Blockchain WorkbenchAzure Blockchain Workbench database views
description: Panoramica delle viste del database SQL DB di Azure Blockchain Workbench Preview disponibili.
ms.date: 09/05/2019
ms.topic: article
...
---
# <a name="azure-blockchain-workbench-database-views"></a>Viste del database di Azure Blockchain WorkbenchAzure Blockchain Workbench database views

Azure Blockchain Workbench Preview fornisce i dati dai libri contabili distribuiti a un database DI database SQL *off-chain.* Il database off-chain consente di utilizzare SQL e gli strumenti esistenti, ad esempio [SQL Server Management Studio,](https://docs.microsoft.com/sql/ssms/download-sql-server-management-studio-ssms?view=sql-server-2017)per interagire con i dati blockchain.
...
| UploadedByUserExternalId         | nvarchar(255) | No          | Identificatore esterno dell'utente che ha caricato l'applicazione. Per impostazione predefinita, questo ID è l'utente di Azure Active Directory per il consorzio.                                                                                                |
...
| UserExternalId             | nvarchar(255) | No          | Identificatore esterno dell'utente associato al ruolo. Per impostazione predefinita, questo ID è l'utente di Azure Active Directory per il consorzio.                                                                     |
...
| UserExternalId           | nvarchar(255) | No          | Identificatore esterno dell'utente associato alla connessione. Per impostazione predefinita, questo ID è l'utente di Azure Active Directory per il consorzio. |
| UserProvisioningStatus   | INT           | No          |Identifica lo stato corrente del processo di provisioning per l'utente. I valori possibili sono: <br />0: l'utente è stato creato dall'API<br />1: una chiave è stata associata all'utente nel database<br />2: è stato effettuato il provisioning completo per l'utente |
...
| ContractDeployedByUserFirstName          | nvarchar(50)  | Sì         | Nome dell'utente che ha distribuito il contratto |
| ContractDeployedByUserLastName           | nvarchar(255) | Sì         | Cognome dell'utente che ha distribuito il contratto |
| ContractDeployedByUserExternalId         | nvarchar(255) | No          | Identificatore esterno dell'utente che ha distribuito il contratto. Per impostazione predefinita, questo ID è il guid che rappresenta la propria identità nel consorzio Azure Active Directory.By default, this ID is the guid that represents their identity in the consortium Azure Active Directory.                                                                                                                                                |
| ContractDeployedByUserEmailAddress       | nvarchar(255) | Sì         | Indirizzo di posta elettronica dell'utente che ha distribuito il contratto |
...
| ContractActionExecutedByUserExternalId   | nvarchar(255) | Sì         | Identificatore esterno dell'utente che ha eseguito l'azione del contratto. Per impostazione predefinita, questo ID è il guid che rappresenta la propria identità nel consorzio Azure Active Directory.By default, this ID is the guid that represents their identity in the consortium Azure Active Directory. |
...
| TransactionIsWorkbenchTransaction        | bit           | Sì         | Un bit che identifica se la transazione è una transazione Workbench di Azure Blockchain |
...
| StateName                          | nvarchar(50)  | Sì         | Se questa proprietà contiene lo stato del contratto, è il nome visualizzato per lo stato. Se non è associata allo stato, il valore sarà null. |
...
| StateValue                         | nvarchar(255) | Sì         | Se questa proprietà contiene lo stato, è il valore dello stato. Se non è associata allo stato, il valore sarà null. |
...
Questa vista rappresenta informazioni dettagliate sui metadati del flusso di lavoro di base, insieme alle funzioni e ai parametri del flusso di lavoro. Progettato per la creazione di report, contiene anche metadati sull'applicazione associata al flusso di lavoro. Questa vista contiene dati di più tabelle sottostanti per semplificare la creazione di report sui flussi di lavoro. Per ogni flusso di lavoro, questa vista contiene i dati seguenti:
...
Questa vista rappresenta informazioni dettagliate sui metadati del flusso di lavoro di base, insieme alle funzioni e ai parametri del flusso di lavoro. Progettato per la creazione di report, contiene anche metadati sull'applicazione associata al flusso di lavoro. Questa vista contiene dati di più tabelle sottostanti per semplificare la creazione di report sui flussi di lavoro. Per ogni funzione del flusso di lavoro, questa vista contiene i dati seguenti:
...
| WorkflowFunctionIsConstructor        | bit           | No          | Identifica se la funzione del flusso di lavoro è il costruttore per il flusso di lavoro |
