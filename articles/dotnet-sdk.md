...
Azure SDK per .NET include inoltre librerie client per l'uso di servizi di Azure Tali librerie vanno installate separatamente mediante [NuGet][].

## Sommario

-   [Cosa è incluso nell'installazione di Azure SDK per .NET][]
-   [Cosa non è incluso nell'installazione di Azure SDK per .NET][]
-   [Domande frequenti][]
-   [Risorse][]
...
-   [Microsoft ASP.NET e strumenti Web per Visual Studio][]
-   [Strumenti di Microsoft Azure per Microsoft Visual Studio][]
-   [Strumenti di creazione di Microsoft Azure][]
-   [Emulatore di Microsoft Azure][]
-   [Emulatore di archiviazione di Microsoft Azure][]
-   [Strumenti di archiviazione di Microsoft Azure][]
-   [Librerie di Microsoft Azure per .NET][]
-   [Componente aggiuntivo di pubblicazione Azure LightSwitch per Visual Studio][]
...
-   [Pubblicare progetti Web in Siti Web di Azure][].
-   [Pubblicare progetti di applicazioni console in Azure WebJobs][].
-   [Creare risorse di Siti Web di Azure e di database SQL durante la creazione o la pubblicazione di un nuovo progetto Web][].
-   [Creare script di distribuzione di PowerShell durante la creazione di nuovi siti Web][].
-   [Gestire e risolvere i problemi di Siti Web di Azure in Esplora server][].
-   [Condurre esecuzioni remote in modalità debug per Siti Web e WebJobs][].
...
-   [Creare, aprire e pubblicare progetti di servizi cloud][].
-   [Creare pacchetti di distribuzione per progetti di servizi cloud][].
-   [Creare macchine virtuali di Azure durante la creazione di nuovi progetti Web][].
...
-   [Visualizzare e gestire impostazioni di progetti di servizi cloud nelle finestre delle proprietà dei progetti di Visual Studio][].
-   Visualizzare e gestire [servizi cloud][], [macchine virtuali][] e [bus di servizio][] in Esplora server.
-   [Condurre esecuzioni remote in modalità debug per servizi cloud e macchine virtuali][].

### <span id="auth"></span></a>Strumenti di creazione di Microsoft Azure

Sono inclusi i componenti seguenti:

-   Strumento da riga di comando CSPack[per la creazione di pacchetti di distribuzione.][]
-   Strumento da riga di comando CSEncrypt[per la crittografia di password usate per accedere a istanze del ruolo di servizi cloud tramite una connessione Desktop remoto.][]
...
L'[Emulatore di Azure][] simula l'ambiente del servizio cloud per consentire il test dei progetti di servizi cloud nel computer locale prima di distribuirli in Azure.

### <span id="stgemulator"></span></a>Emulatore di archiviazione di Microsoft Azure

L'[Emulatore di archiviazione di Azure][] usa un'istanza di SQL Server e il file system locale per simulare l'archiviazione di Azure (code, tabelle, BLOB) per consentirne il test in locale.

### <span id="stgtools"></span></a>Strumenti di archiviazione di Microsoft Azure

Viene installato [AzCopy][], uno strumento da riga di comando che può essere usato per trasferire dati da e verso un account di archiviazione di Azure.
...
-   Un plug-in di Visual Studio che consente l'esecuzione locale in Visual Studio dei progetti [Cache nel ruolo][].

### <span id="ls"></span></a>Componente aggiuntivo di pubblicazione Azure LightSwitch per Visual Studio

Consente di [pubblicare progetti LightSwitch in Siti Web di Azure][]. Il componente aggiuntivo LightSwitch è incluso negli aggiornamenti di Visual Studio insieme ad Azure SDK per .NET. L'installazione dell'SDK garantisce che si stia usando la versione più recente del componente aggiuntivo.
...
-   [Azure PowerShell][].
    ...
    Azure PowerShell consente di [automatizzare la creazione e la distribuzione dell'ambiente Azure][].
...
-   [Molte funzionalità di Azure sono già presenti in Visual Studio. È necessario installare Azure SDK per .NET?][]
-   [Occorre una libreria client. È necessario installare Azure SDK per .NET per ottenerla?][]
-   [Dove è possibile trovare versioni precedenti di Azure SDK per .NET?][]
-   [Qual è il criterio relativo al ciclo di vita di Azure SDK per .NET?][]
-   [Con quali versioni di sistema operativo guest è compatibile Azure SDK per .NET?][]
...
-   È stato installato l'[aggiornamento di Visual Studio][] più recente.
...
Per versioni precedenti, vedere la pagina di download di [Azure SDK per .NET][].

### <span id="lifecycle"></span></a>Qual è il criterio relativo al ciclo di vita di Azure SDK per .NET?

Vedere [Ciclo di vita del supporto per i servizi cloud di Microsoft Azure][].

### <span id="guestos"></span></a>Con quali versioni di sistema operativo guest è compatibile Azure SDK per .NET?

Vedere [Rilasci del sistema operativo guest Azure e matrice di compatibilità SDK][].
...
Per il codice sorgente di Azure SDK per .NET, comprese le librerie client, vedere [GitHub.com/Azure][].

Per la documentazione di riferimento sulle librerie client di Azure, vedere [Informazioni di riferimento su .NET per Azure][].

  [pagina dei download di Azure]: /en-us/downloads/
  ...
  [Creare script di distribuzione di PowerShell durante la creazione di nuovi siti Web]: http://msdn.microsoft.com/en-us/library/dn642480.aspx
  ...
  [Creare pacchetti di distribuzione per progetti di servizi cloud]: http://msdn.microsoft.com/en-us/library/ff683672.aspx
  ...
  [Visualizzare e gestire impostazioni di progetti di servizi cloud nelle finestre delle proprietà dei progetti di Visual Studio]: http://msdn.microsoft.com/en-us/library/ee405486.aspx
  [servizi cloud]: http://msdn.microsoft.com/en-us/library/ff683675.aspx
  [macchine virtuali]: http://msdn.microsoft.com/en-us/library/jj131259.aspx
  [bus di servizio]: http://msdn.microsoft.com/en-us/library/jj149828.aspx
  [Condurre esecuzioni remote in modalità debug per servizi cloud e macchine virtuali]: http://msdn.microsoft.com/en-us/library/ff683670.aspx
  [per la creazione di pacchetti di distribuzione.]: http://msdn.microsoft.com/en-us/library/gg432988.aspx
  [per la crittografia di password usate per accedere a istanze del ruolo di servizi cloud tramite una connessione Desktop remoto.]: http://msdn.microsoft.com/en-us/library/hh404001.aspx
  [Emulatore di Azure]: http://msdn.microsoft.com/en-us/library/dn339018.aspx
  [Emulatore di archiviazione di Azure]: http://msdn.microsoft.com/en-us/library/hh403989.aspx
  [AzCopy]: http://aka.ms/AzCopy
  [Cache nel ruolo]: http://msdn.microsoft.com/en-us/library/dn386103.aspx
  [pubblicare progetti LightSwitch in Siti Web di Azure]: http://msdn.microsoft.com/en-us/library/jj131261.aspx
  ...
  [aggiornamento di Visual Studio]: http://www.visualstudio.com/en-us/downloads/download-visual-studio-vs#DownloadFamilies_5
  [Azure SDK per .NET]: /en-us/downloads/archive-net-downloads/
  ...
  [Rilasci del sistema operativo guest Azure e matrice di compatibilità SDK]: http://msdn.microsoft.com/en-us/library/ee924680.aspx
  [GitHub.com/Azure]: https://github.com/azure/
  [Informazioni di riferimento su .NET per Azure]: /en-us/develop/net/reference/
