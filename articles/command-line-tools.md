<properties urlDisplayName="Command-Line Tools" pageTitle="Strumenti da riga di comando di Azure per Mac e Linux" metaKeywords="Azure command-line, Azure tools Mac, Azure tools Linux" description="Learn about using the command-line tool for Mac and Linux in Azure." metaCanonical="" services="web-sites,virtual-machines,mobile-services,cloud-services" documentationCenter="" title="" authors="carolz" solutions="" manager="need to identify contact" editor="" />
...
Per installare lo strumento su Linux, installare l'ultima versione di Node.JS, quindi usare NPM per procedere:
...
* [Comandi per la gestione degli oggetti di archiviazione](#Commands_to_manage_your_Storage_objects)
...
<div class="dev-callout"><b>Nota</b>
   <p>Il file di impostazioni di pubblicazione può contenere dettagli (ovvero nome e ID sottoscrizione) di più di una sottoscrizione. Quando si importa il file di impostazioni di pubblicazione, la prima sottoscrizione viene usata come descrizione predefinita. Per usare una sottoscrizione diversa, eseguire il comando seguente.</p>
<code>~$ azure config set subscription <other-subscription-id></code>
</div>

**account clear [opzioni]**

Questo comando rimuove le impostazioni di pubblicazione che sono state importate. Usare questo comando se si è terminato il lavoro con lo strumento nel computer in uso e si desidera assicurarsi che nessuno possa usarlo successivamente con quell'account.
...
È possibile specificare un percorso tramite il parametro --location o un gruppo di affinità tramite il parametro --affinity-group. Se non viene usato nessuno di questi due parametri, verrà richiesto di fornire un percorso scegliendolo da un elenco di percorsi validi.

La lunghezza della password fornita deve essere compresa tra 8 e 123 caratteri e soddisfare i requisiti di complessità della password del sistema operativo usato per la macchina virtuale.

Se per gestire una macchina virtuale Linux distribuita si prevede la necessità di usare SSH (che è in genere il metodo più diffuso) è necessario abilitare SSH mediante l'opzione -e al momento della creazione della macchina virtuale stessa. Non è possibile abilitare SSH dopo che una macchina virtuale è stata creata.
...
**-c, --connect** Crea la macchina virtuale all'interno di una distribuzione già creata in un servizio di hosting. Se il parametro -vm-name non viene usato con questa opzione, il nome della nuova macchina virtuale verrà generato automaticamente.<br />
**-n, --vm-name** Specifica il nome della macchina virtuale. Per impostazione predefinita, questo parametro prende il nome del servizio di hosting. Se -vmname non viene specificato, il nome per la nuova macchina virtuale viene generato come <service-name><id>, dove <id> è il numero di macchine virtuali esistenti nel servizio più 1. Se ad esempio si usa questo comando per aggiungere una nuova macchina virtuale a un servizio di hosting denominato MyService con una macchina virtuale già esistente, la nuova macchina virtuale viene denominata MyService2.<br /> 
...
**-l, --location** specifica il percorso (ad esempio, "North Central US"). <br />
...
Nell'esempio seguente MSFT__Win2K8R2SP1-120514-1520-141205-01-it-it-30GB è un'immagine fornita dalla piattaforma. Per altre informazioni sulle immagini del sistema operativo, vedere il comando vm image list.
...
Nel diagramma seguente viene illustrata l'architettura di una tipica distribuzione di più istanze di una macchina virtuale. Si noti che, in questo esempio, la porta 3389 è aperta su ogni macchina virtuale (per l'accesso RDP) e che è anche presente un indirizzo IP interno (ad esempio 168.55.11.1) su ogni macchina virtuale. Questo viene usato dal servizio di bilanciamento del carico per l'indirizzamento del traffico alla macchina virtuale. L'indirizzo IP interno può essere usato anche per la comunicazione tra macchine virtuali.
...
Questo comando crea un nuovo endpoint per una macchina virtuale. È anche possibile usare le opzioni -u oppure --enable-direct-server-return per specificare se abilitare Direct Server Return sull'endpoint, che per impostazione predefinita è disabilitato.
...
**vm endpoint create-multiple [opzioni] < <nome-vm> [porta-bc><:[porta-vm><:[protocollo<:[nome-set-bc<:<protocollo-prob>:[porta-prob-bc><:]]]]] ]percorso-prob> {1-*}**

Creare più endpoint per macchine virtuali. È anche possibile usare le opzioni -u oppure --enable-direct-server-return per specificare se abilitare Direct Server Return sull'endpoint, che per impostazione predefinita è disabilitato.
...
I dischi dati sono file con estensione vhd nell'archiviazione BLOB che possono essere usati da una macchina virtuale. Per altre informazioni sulle modalità di distribuzione dei dischi dati nell'archiviazione BLOB, vedere il diagramma tecnico di Azure riportato in precedenza in questo articolo. 
...
<div class="dev-callout"><b>Nota</b>
   <p>Si noti che i dischi dati devono essere sempre scollegati in ordine inverso, iniziando dall'unità con il numero LUN più alto assegnato. Il livello SCSI di Linux non supporta lo scollegamento di un'unità mentre è ancora collegata un'unità con un numero LUN superiore. L'unità con LUN 0, ad esempio, non può essere scollegata se l'unità LUN 1 è ancora collegata.</p>
</div>
...
Questo comando elenca i dischi di Azure o i dischi collegati a una macchina virtuale specificata. Se viene eseguito con un nome di macchina virtuale come parametro, restituisce tutti i dischi collegati alla macchina virtuale. L'unità LUN 1 viene creata insieme alla macchina virtuale ed eventuali altri dischi elencati vengono collegati separatamente.
...
**vm disk create <name> [percorso-origine]**

Questo comando carica e registra un disco di Azure. È necessario specificare --blob-url, --location o --affinity-group. Se si usa questo comando con [percorso-origine], viene caricato il file con estensione vhd specificato e viene creata una nuova immagine. È quindi possibile collegare questa immagine a una macchina virtuale usando il comando vm disk attach.
...
**vm disk upload [opzioni] < <percorso_origine> <url-blob> chiave_account_archiviazione>**
...
**vm disk attach <nome-vm> <nome_immagine_disco>**
...
**vm disk attach-new <nome-vm> <dimensioni_in_gb> [url_blob]**

Questo comando collega un disco dati a una macchina virtuale di Azure. In questo esempio, 20 è la dimensione in gigabyte del nuovo disco da collegare. È possibile usare facoltativamente un URL di BLOB come ultimo argomento, per specificare in modo esplicito il BLOB di destinazione da creare. Se non si specifica un URL di BLOB, verrà generato un oggetto BLOB automaticamente.
...
I servizi cloud di Azure sono applicazioni e servizi ospitati in ruoli Web e ruoli di lavoro. I comandi seguenti possono essere usati per gestire i servizi cloud di Azure.
...
**service cert delete [opzioni] <identificazione_personale>**
...
<div class="dev-callout"><b>Nota</b>
   <p>Il nome del sito deve essere univoco. Non è possibile creare un sito con lo stesso nome DNS di un sito esistente.</p>
</div>
...
**-q o **--quiet**: non chiede alcuna conferma. Usare questa opzione negli script automatici.
...
**site cert add [opzione] < [percorso-certificato> nome]**

**site cert delete [opzioni] < [identificazione_personale> nome]**

**site cert show [opzioni] < [identificazione_personale> nome]**
...
**site connectionstring add [opzioni] < <nome_connessione> <valore> [tipo> nome]**

**site connectionstring delete [opzioni] < [nome_connessione> nome]**

**site connectionstring show [opzioni] < [nome_connessione> nome]**
...
**site deployment show [opzioni] < [id_commit> nome]**

**site deployment redeploy [opzioni] < [id_commit> nome]**

**site deployment github [opzioni] [nome]**

**site deployment user set [opzioni] [ ] [nome_utente pass]**
...
+ **--job-type** < tipo_processo>: facoltativa. Tipo di processo Web. Il valore valido è "triggered" o "continuous". Per impostazione predefinita vengono restituiti
i processi Web di qualsiasi tipo.
+ **--slot** <slot>: nome dello slot da riavviare.

**site job show [opzioni] < <nome_processo> [tipo_processo> nome]**

Questo comando visualizza i dettagli di un processo Web specifico.

Il comando supporta le opzioni aggiuntive seguenti:

+ **--job-name** <nome_processo>: obbligatoria. Nome del processo Web.
+ **--job-type** < tipo_processo>: obbligatoria. Tipo di processo Web. Il valore valido è "triggered" o "continuous".
+ **--slot** <slot>: nome dello slot da riavviare.

**site job delete [opzioni] < <nome_processo> [tipo_processo> nome]**
...
+ **--job-name** <nome_processo>    obbligatoria. Nome del processo Web.
+ **--job-name** < tipo_processo>    obbligatoria. Tipo di processo Web. Il valore valido è "triggered" o "continuous".
+ **-q** o **--quiet**: non chiede alcuna conferma. Usare questa opzione negli script automatici.
+ **--slot** <slot>: nome dello slot da riavviare.

**site job delete [opzioni] < <nome_processo> [tipo_processo> <file_processo> nome]**
...
+ **--job-name** <nome_processo>: obbligatoria. Nome del processo Web.
+ **--job-type** < tipo_processo>: obbligatoria. Tipo di processo Web. Il valore valido è "triggered" o "continuous".
+ **--job-file** <file-processo>: obbligatoria. File del processo.
+ **--slot** <slot>: nome dello slot da riavviare.

**site job start [opzioni] < <nome_processo> [tipo_processo> nome]**
...
**site job stop [opzioni] < <nome_processo> [tipo_processo> nome]**
...
**site job history list [opzioni] [ ] [nome_processo nome]**
...
**site job history show [opzioni] [ ] [nome_processo] [ ID_esecuzione nome]**
...
+ **--job-name** <nome_processo>: obbligatoria. Nome del processo Web.
+ **--run-id** < id_esecuzione>: facoltativa. ID dell'esecuzione. Se non è specificato viene visualizzata l'ultima esecuzione.
+ **--slot** <slot>: nome dello slot da riavviare.
...
+ **-s `<id>`** o **--subscription `<id>`**: Usa una sottoscrizione specifica, specificata come `<id>`.
...
**mobile create [opzioni] [ ] [nome_servizio] [nomeutenteAmminSql passwordAmminSql]**
...
+ **-r `<sqlServer>`**  or **--sqlServer `<sqlServer>`**:  usa un server di database SQL esistente, specificato com `<sqlServer>`.
...
**mobile restart [options] [nome_servizio]**
...
Questo comando restituisce i log del servizio mobile, escludendo dal filtro tutti i tipi ad eccezione di `error`.
...
+ **-t `<type>`** o **--type `<type>`**:  esegue la query nel log specificato `<type>`, che può essere `information`, `warning` o `error`.
+ **-k `<skip>`** o **--skip `<skip>`**: ignora il numero di righe specificato in `<skip>`.
+ **-p `<top>`** o **--top `<top>`**: restituisce un numero specifico di righe, specificato in `<top>`.
...
<div class="dev-callout"><b>Nota</b>
   <p>Quando si rigenerano le chiavi, i client che usano la vecchia chiave potrebbero non essere più in grado di accedere al servizio mobile. Quando si rigenera la chiave applicazione, è necessario aggiornare l'app con il nuovo valore della chiave. </p>
</div> 
...
+ **-k `<skip>`** o **--skip `<skip>`**: ignora il numero di righe specificato in `<skip>`.
+ **-t `<top>`** o **--top `<top>`**: restituisce un numero specifico di righe, specificato in `<top>`.
...
<div class="dev-callout"><b>Nota</b>
   <p>I nuovi processi vengono creati con stato disabilitato perché uno script deve ancora essere caricato. Usare il comando <strong>mobile script upload</strong> per caricare uno script e il comando <strong>mobile job update</strong> per abilitare il processo.</p>
</div> 
...
+ **-i `<count>`** o **--numberOfInstances `<count>`**: il numero di istanze usato quando in esecuzione in modalità riservata.

<div class="dev-callout"><b>Nota</b>
   <p>Se si imposta la modalità di calcolo su `Reserved`, tutti i servizi mobili disponibili nella stessa area verranno eseguiti nella modalità Premium.</p>
</div>  
...
**mobile preview enable [opzioni] [ ] [nome_servizio nome_funzionalità]**
...
**-p** o **--permissions** <   permissions>:<elenco delimitato da virgole di coppie <method>=permission>.
...
+ **-e** o **--label** < label>: etichetta per l'account di archiviazione.
+ **-d** o **--description** <   description>:descrizione dell'account di archiviazione.
+ **-l** o **--location** <nome>: area geografica in cui creare l'account di archiviazione.
+ **-a** r **--affinity-group** <nome>: gruppo di affinità a cui associare l'account di archiviazione.
...
+ **-p** o **-prefix** < prefix>: prefisso del nome del contenitore di archiviazione.
+ **-a** o **--account-name** <nome_account>: nome dell'account di archiviazione.
+ **-k** o **--account-key** < chiave_account>: chiave dell'account di archiviazione.
+ **-c** o **--connection-string** < stringa_connessione>: stringa di connessione di archiviazione.
...
+ **--container** <contenitore>: nome del contenitore di archiviazione da creare.
+ **-b** o **--blob** <nome_blob>: nome del BLOB di archiviazione da eliminare.
...
+ **--container** <contenitore>: nome del contenitore di archiviazione da creare.
+ **-b** o **--blob** <nome_blob>: nome del BLOB di archiviazione da caricare.
+ **-t** o **--blobtype** < blobtype>: tipo di BLOB di archiviazione, Page o Block.
+ **-p** o **--properties** < properties>: proprietà del BLOB di archiviazione per il file caricato. Le proprietà sono costituite da coppie chiave=valore e sono separate da punto e virgola (;). Le proprietà disponibili sono contentType, contentEncoding, contentLanguage e cacheControl.
+ **-m** o **--metadata** < metadata>: metadati del BLOB di archiviazione per il file caricato. I metadati sono costituiti da coppie chiave=valore e sono separati da punto e virgola (;).
+ **--concurrenttaskcount** < numero_attività_simultanee>: numero massimo di richieste di caricamento simultanee.
...
+ **--container** <contenitore>: nome del contenitore di archiviazione da creare.
+ **-b** o **--blob** <nome_blob>: nome del BLOB di archiviazione.
+ **-d** o **--destination** [ ]: percorso di directory o file di destinazione del download.
...
**sql server create <id_accesso_amministratore> <password_amministratore> <ubicazione>**
...
**sql db create [opzioni] < <nome_server> <nome_database> password_amministratore>**
...
**sql db show [opzioni] < <nome_server> <nome_database> password_amministratore>**
...
**sql db list [opzioni] < <nome_server> password_amministratore>**
...
**sql db delete [opzioni] < <nome_server> <nome_database> password_amministratore>**
...
**sql firewallrule create [opzioni] < < <nome_server> <nome_regola> indirizzo_IP_inizio> indirizzo_IP_fine>**
...
**sql firewallrule show [opzioni] < <nome_server> nome_regola>**
...
**sql firewallrule list [opzioni] <nome_server>**
...
**sql firewallrule delete [opzioni] < <nome_server> nome_regola>**
...
**network vnet create [opzioni] < ubicazione>**
...
**network import [percorso_file]**

Questo comando importa una configurazione di rete locale.

**network dnsserver register [opzioni] < IP_DNS>**
...
**network dnsserver unregister [opzioni] < IP_DNS>**
