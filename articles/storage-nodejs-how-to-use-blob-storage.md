<properties urlDisplayName="Blob Service" pageTitle="Come usare il servizio di archiviazione BLOB (Node.js) | Microsoft Azure" metaKeywords="Get started Azure blob, Azure unstructured data, Azure unstructured storage, Azure blob, Azure blob storage, Azure blob Node.js" description="Learn how to use the Azure blob service to upload, download, list, and delete blob content. Samples written in Node.js." metaCanonical="" services="storage" documentationCenter="nodejs" title="How to Use the Blob Service from Node.js" authors="larryfr" solutions="" manager="wpickett" editor="" />
...
servizio BLOB di Azure. Gli esempi sono scritti con
...
3.  È possibile eseguire manualmente il comando **ls** per verificare che sia stata creata una cartella
    **node\_modules**. Nella cartella si trova il pacchetto
    **azure-storage**, che contiene le librerie necessarie per accedere
    all'archiviazione.
...
L'oggetto **BlobService** consente di lavorare con contenitori e BLOB. Il
codice seguente crea un oggetto **BlobService**. Aggiungere il codice seguente vicino
alla parte superiore di **server.js**:

    var blobSvc = azure.createBlobService();

> [WACOM.NOTE] È possibile accedere a un BLOB in modo anonimo usando **createBlobServiceAnonymous** e specificando l'indirizzo dell'host. Ad esempio, `var blobSvc = azure.createBlobService('https://myblob.blob.core.windows.net/');`.
...
Se il contenitore viene creato, `result` sarà true. Se il contenitore esiste già, `result` sarà false. `response` conterrà le informazioni sull'operazione, incluse le informazioni [ETag](http://en.wikipedia.org/wiki/HTTP_ETag) per il contenitore.
...
L'esempio seguente illustra l'impostazione del livello di accesso su **blob**: 
...
In alternativa, è possibile modificare il livello di accesso di un contenitore usando **setContainerAcl** per specificarlo. L'esempio seguente imposta il livello di accesso su container:
...
Il risultato conterrà informazioni sull'operazione, incluse le informazioni **ETag** correnti per il contenitore.
...
Sono inclusi due filtri che implementano la logica di ripetizione dei tentativi con Azure SDK per Node.js, **ExponentialRetryPolicyFilter** e **LinearRetryPolicyFilter**. Il codice seguente crea un oggetto **BlobService** che usa **ExponentialRetryPolicyFilter**:
...
L'esempio seguente carica il contenuto del file **test.txt** in **myblob**.
...
L'esempio seguente carica il contenuto del file **test.txt** in **mypageblob**.
...
> [WACOM.NOTE] I BLOB di pagine sono costituiti da 'pagine' di 512 byte. Potrebbe essere visualizzato un messaggio di errore quando si caricano dati con una dimensione diversa da un multiplo di 512.
...
L'oggetto `result` conterrà una raccolta di `entries`, ovvero una matrice di oggetti che descrivono ogni BLOB. Se non vengono restituiti tutti i BLOB, `result` fornirà anche un oggetto `continuationToken`, che può essere usato come secondo parametro per recuperare altre voci.
...
L'esempio seguente illustra l'uso di **getBlobToStream** per scaricare il contenuto del BLOB **myblob** e archiviarlo nel file **output.txt** con un flusso:
...
L'oggetto `result` conterrà le informazioni sul BLOB, incluse le informazioni sull'**ETag**.

## <a name="delete-blob"> </a>Procedura: Eliminare un BLOB

Infine, per eliminare un BLOB, chiamare **deleteBlob**. L'esempio seguente elimina il BLOB denominato **myblob**.
...
È possibile impostare le condizioni degli ETag tramite il parametro `options.accessConditions` facoltativo. L'esempio seguente caricherà solo il file **test.txt** se il BLOB esiste già e ha il valore ETag contenuto in `etagToMatch`.
...
È possibile acquisire un nuovo lease usando il metodo **acquireLease** e specificando il BLOB o il contenitore per il quale si vuole ottenere il lease. Ad esempio, il codice seguente acquisisce un lease sul BLOB **myblob**..
...
Per rimuovere un lease, usare il metodo **releaseLease**.. Per interrompere un lease e impedire ad altri di ottenerne uno nuovo fino alla scadenza della durata originale, usare il metodo **breakLease**..
...
L'esempio seguente genera un nuovo criterio di accesso condiviso che consentirà al titolare della firma di accesso condiviso di eseguire operazioni di lettura nel BLOB **myblob** e che scadrà 100 minuti dopo la data di creazione.
...
L'applicazione client usa quindi la firma di accesso condiviso con il metodo **BlobServiceWithSAS** per eseguire operazioni sul BLOB. Il codice seguente recupera informazioni sul BLOB **myblob**.
...
Un elenco di controllo di accesso viene implementato usando una matrice di criteri di accesso, con un ID associato a ogni criterio. Nell'esempioseguente vengono definiti due criteri, uno per 'user1' e uno per 'user2':
...
L'esempio seguente recupera l'elenco di controllo di accesso corrente per **mycontainer**, quindi aggiunge i nuovi criteri con **setBlobAcl**. Risultato:
...
A questo punto, dopo aver appreso le nozioni di base dell'archiviazione BLOB, usare i collegamenti seguenti
...
[Creazione e distribuzione di un'applicazione Node.js in un sito Web di Azure]: /it-it/documentation/articles/web-sites-nodejs-develop-deploy-mac/
 ...
 [Sito Web con WebMatrix]: /it-it/documentation/articles/web-sites-nodejs-use-webmatrix/
  [usando l'API REST]: http://msdn.microsoft.com/it-it/library/windowsazure/hh264518.aspx
