...
1. Controllare la configurazione della porta negli asset seguenti:
    * **[Grafico Helm](https://docs.helm.sh):** Specificato dall'`service.port` e `deployment.containerPort` in values. YAML con il comando `azds prep`.
...
1. Modificare il file di `azds.yaml` per impostare il contesto di compilazione sul livello della soluzione.
