...
1. L'applicazione *servicea* nel pod *servicea. azureuser* effettua una chiamata a *serviceB*. L'applicazione *servicea* contiene anche il codice per mantenere l' `azds-route-as` intestazione esistente, che in questo caso `azds-route-as: azureuser`è.
