---
title: Distribuire un'applicazione con un Dockerfile in Kubernetes tramite Azure Dev Spaces
...
ms.date: 07/08/2019
ms.topic: quickstart
description: Distribuire un microservizio con un Dockerfile nel servizio Azure Kubernetes con Azure Dev Spaces
...
---
# <a name="quickstart-develop-an-application-with-a-dockerfile-on-kubernetes-using-azure-dev-spaces"></a>Guida introduttiva: Sviluppare un'applicazione con un Dockerfile in Kubernetes tramite Azure Dev Spaces
...
In questo articolo verrà usata l'[applicazione di esempio Azure Dev Spaces](https://github.com/Azure/dev-spaces) per dimostrare l'uso di Azure Dev Spaces. L'applicazione di esempio è scritta in Go, ma è possibile usare qualsiasi linguaggio purché venga fornito un Dockerfile valido per aggiungere l'applicazione a un contenitore ed eseguirla nel servizio Azure Kubernetes.

Clonare l'applicazione da GitHub e passare alla directory *dev-spaces/samples/golang/getting-started/webfrontend*:
...
Per eseguire l'applicazione in Azure Dev Spaces, è necessario avere un Dockerfile e un grafico Helm. Per alcuni linguaggi, come [Java][java-quickstart], [.NET core][netcore-quickstart] e [Node.js][nodejs-quickstart], gli strumenti client di Azure Dev Spaces consentono di generare tutti gli asset necessari. Per molti altri linguaggi, come Go, PHP e Python, gli strumenti client consentono di generare il grafico Helm purché venga specificato un Dockerfile valido.

L'applicazione di esempio include un Dockerfile valido. Per generare gli asset per grafici Helm e Docker per l'esecuzione dell'applicazione in Kubernetes, usare il comando `azds prep`:

```cmd
azds prep --public
```

Per la corretta generazione degli asset per grafici Helm, è necessario eseguire il comando `prep` dalla directory *dev-spaces/samples/golang/getting-started/webfrontend*.
...
Per vedere il servizio in esecuzione, aprire l'URL pubblico e passare al percorso `/api`. L'URL pubblico viene visualizzato nell'output del comando `azds up`. In questo esempio l'URL pubblico è `http://default.webfrontend.1234567890abcdef1234.eus.azds.io/` e si può passare a `http://default.webfrontend.1234567890abcdef1234.eus.azds.io/api`.
...
1. Aggiornare la [riga 29 di `src/app/main.go`](https://github.com/Azure/dev-spaces/blob/master/samples/golang/getting-started/webfrontend/src/app/main.go#L29) in:
