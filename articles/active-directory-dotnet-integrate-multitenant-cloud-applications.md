...
-   [Parte 1: Ottenimento di un ID client per l'accesso ad Azure AD](#getclientid)
-   [Parte 2: Abilitazione dei clienti a effettuare l'iscrizione con Azure AD](#enablesignup)
-   [Parte 3: Abilitazione dell'accesso Single Sign-On](#enablesso)
-   [Parte 4: Accesso ad Azure AD Graph](#accessgraph)
-   [Parte 5: Pubblicazione dell'applicazione](#publish)
-   [Riepilogo](#summary)

Parte 1: Ottenimento di un ID client per l'accesso ad Azure AD
--------------------------------------------------------------
...
### Passaggio 3: Configurazione dell'applicazione per l'utilizzo dell'ID client e del segreto client

Questo passaggio richiede l'ID client e il segreto client generati durante la registrazione sul dashboard venditori Microsoft. L'ID client viene utilizzato per l'accesso SSO ed entrambi (ID client e segreto client) verranno utilizzati in seguito per ottenere un token di accesso per chiamare l'API Graph di Azure AD.

L'applicazione di esempio è predisposta per l'utilizzo di Azure AD ed esegue il caricamento dell'ID client e del segreto client dal file config. Nel file **Web.config** nell'applicazione di esempio, apportare le modifiche seguenti:
...
Parte 2: Abilitazione dei clienti a effettuare l'iscrizione con Azure AD
------------------------------------------------------------------------

In questa sezione viene descritto come abilitare i clienti a effettuare l'iscrizione all'applicazione con Azure AD. Affinché un cliente possa utilizzare un'applicazione integrata in Azure AD un amministratore tenant deve prima autorizzarla. Il processo di autorizzazione inizia con una richiesta di consenso inviata dall'applicazione ad Azure, la quale genera una risposta che dovrà essere gestita dall'applicazione. Nella procedura seguente viene descritto come generare una richiesta di consenso e gestire la risposta.
...
**RequestedPermissions**: (facoltativo) le autorizzazioni che devono essere concesse all'applicazione dal tenant. Durante lo sviluppo, queste autorizzazioni vengono impiegate per testare le applicazioni non pubblicate. Per le applicazioni pubblicate, questo parametro verrà ignorato. Verranno invece utilizzate le autorizzazioni richieste nella presentazione dell'applicazione. Per ulteriori informazioni sulla presentazione vedere la Parte 5. Sono disponibili tre possibili valori per questo parametro:
...
Di seguito è riportato un esempio di URL valido della richiesta di consenso: *https://activedirectory.windowsazure.com/Consent/AuthorizeApplication.aspx?ApplicationId=33E48BD5-1C3E-4862-BA79-1C0D2B51FB26&RequestedPermissions=DirectoryReader*
...
Di seguito è riportato un esempio di risposta valida alla richiesta di consenso, che indica che l'applicazione è stata autorizzata: *https://app.litware.com/redirect.aspx&TenantId=7F3CE253-66DB-4AEF-980A-D8312D76FDC2&Consent=Granted*
...
Per testare la capacità dell'applicazione di integrarsi in Azure AD sarà necessario disporre di un tenant di Azure AD. È possibile riutilizzare un tenant già utilizzato per il test di un'altra applicazione. È consigliabile ottenere almeno due tenant per assicurarsi che l'applicazione possa essere testata e utilizzata da più tenant. A tale scopo, non è consigliabile utilizzare un tenant di produzione. [Ottenere un tenant di Azure AD](http://g.microsoftonline.com/0AX00en/5).
...
Se i clienti si iscrivono per ottenere un nuovo tenant di Azure AD è possibile che il completamento del provisioning di tale tenant richieda alcuni minuti. Nella pagine del consenso potrebbero essere visualizzati alcuni errori fino al completamento del provisioning.

Parte 3: Abilitazione dell'accesso Single Sign-On
-------------------------------------------------
...
-   L'applicazione può chiedere all'utente di fornire l'indirizzo email oppure il nome di dominio di Azure AD. Questo approccio viene utilizzato nell'applicazione di esempio, in cui l'utente deve immettere il nome di dominio di Azure AD, come illustrato di seguito:
...
Per dimostrare questo processo, nei passaggi seguenti viene utilizzato il nome di dominio contoso.com.
...
3.  È consigliabile archiviare il dominio e il TenantID ad esso associato nell'archivio dati cliente/TenantId dell'applicazione. È possibile utilizzare questi due valori assieme per le future richieste di accesso e per eliminare la necessità di ottenere il file **FederationMetadata.xml** ogni volta. L'applicazione di esempio non presenta questa funzione di ottimizzazione.
...
1.  Utilizzare una classe proxy generata per chiamare Azure AD Graph
2.  Acquisire un token di accesso utilizzando Azure Authentication Library (AAL)
3.  Chiamare Azure AD Graph per ottenere un elenco di utenti tenant

**Nota**

La libreria helper dell'applicazione di esempio Microsoft.IdentityModel.WAAD.Preview contiene già una classe proxy generata automaticamente (creata aggiungendo un riferimento al servizio all'indirizzo https://graph.windows.net/nome-dominio-chiamato GraphService). L'applicazione utilizzerà questa classe proxy per chiamare il servizio Azure AD Graph.

### Passaggio 1: Utilizzare la classe proxy generata per chiamare Azure AD Graph
...
2.  Utilizzare l'endpoint per creare un'istanza del proxy per chiamare Graph
...
### Passaggio 2: Acquisire un token di accesso utilizzando Azure Authentication Library
...
AAL è disponibile come pacchetto NuGet e può essere installato in Visual Studio.
...
Parte 5: Pubblicazione dell'applicazione
----------------------------------------
...
Se viene richiesto di aggiungere informazioni su dati fiscali e pagamento, ignorare questo passaggio in quanto si vende l'applicazione direttamente al cliente e non tramite Microsoft.
