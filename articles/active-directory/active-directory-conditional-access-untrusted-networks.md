...
- [Guida introduttiva: Richiedere MFA per app specifiche con l'accesso condizionale di Azure Active Directory](active-directory-conditional-access-app-based-mfa.md) - per acquisire familiarità con la configurazione dei criteri di accesso condizionale. 
...
La richiesta di verifica di questo scenario consiste nel tradurre *quando viene eseguito un tentativo di accesso da un percorso che non è attendibile* in una condizione di accesso condizionale. In un criterio di accesso condizionale, è possibile configurare la [condizione dei percorsi](active-directory-conditional-access-locations.md) per indirizzare scenari che riguardano i percorsi di rete. La condizione dei percorsi consente di selezionare [posizioni specifiche](active-directory-conditional-access-locations.md#named-locations), che rappresentano raggruppamenti logici di intervalli di indirizzi IP, paesi e regioni.  
...
Con l'approccio descritto in questo articolo, è ora possibile configurare un criterio di accesso condizionale per i percorsi non attendibili. È sempre consigliabile testare i criteri prima di distribuirli nell'ambiente di produzione per assicurarsi che funzionino come previsto. Idealmente, è necessario eseguire i test iniziali in un tenant di test se possibile. Per altre informazioni, vedere [Come distribuire un nuovo criterio](active-directory-conditional-access-best-practices.md#how-should-you-deploy-a-new-policy). 
