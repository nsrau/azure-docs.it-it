...
# Anteprima di Modello app 2.0: Riferimento al token
...
| ----------------------- | ------------------------------- | ------------ | --------------------------------- |
| Audience | `aud` | `49210253-0ba1-4a9a-a424-616999fab620` | Identifica il destinatario del token. Nei token ID il destinatario è l'ID applicazione assegnato all'app nel portale di registrazione delle app. L'app deve convalidare questo valore e rifiutare il token, se il valore non corrisponde. |
| Issuer | `iss` | `https://login.microsoftonline.com/b9410318-09af-49c2-b0c3-653adc1f376e/v2.0/` | Identifica il servizio token di sicurezza (STS) che costruisce e restituisce il token e il tenant di Azure AD in cui l'utente è stato autenticato. L'app deve convalidare l'attestazione Issuer per assicurarsi che il token sia stato fornito dall'endpoint 2.0. Può inoltre usare la parte GUID dell'attestazione per limitare il set di tenant che può accedere all'app. |
| Issued At | `iat` | `1438535543` | Ora in cui il token è stato generato, rappresentata dal valore epoch time. |
...
I token di accesso rilasciati dall'endpoint 2.0 presentano due formati diversi. I token di accesso emessi per conto di account aziendali o dell'istituto di istruzione sono di tipo JWT, in modo analogo a un token ID. I token di accesso emessi per conto di un account personale Microsoft sono in un formato noto come "compact ticket". Per questo motivo, in fase di sviluppo è possibile notare un formato di stringa diverso per i token di accesso emessi dall'endpoint 2.0. Nel corso del tempo, questa differenza nei token di accesso verrà eliminata dall'endpoint 2.0.

Ciò premesso, i token di accesso rilasciati dall'endpoint 2.0 attualmente possono essere usati solo dai servizi Microsoft. Le app non devono eseguire la convalida o l'ispezione dei token di accesso per gli scenari attualmente supportati. È possibile considerare i token di accesso completamente opachi. Si tratta di semplici stringhe che l'app può passare a Microsoft nelle richieste HTTP.
...
Quando l'app riceve un token ID all'accesso dell'utente, deve eseguire anche alcuni controlli a fronte delle attestazioni nel token ID. Ad esempio:

- Attestazione **Audience**: per verificare che il token ID era destinato all'app.
- Attestazioni **Issued At** ed **Expiration Time**: per verificare che il token ID non sia scaduto.
