---
title: Autenticazione del passcode monouso per gli utenti guest B2B - Azure ADOne-time passcode authentication for B2B guest users - Azure AD
...
Grazie all'autenticazione con passcode monouso, l'utente guest può riscattare l'invito facendo clic su un collegamento diretto o tramite l'indirizzo di posta elettronica di invito. In entrambi i casi, un messaggio nel browser indica che verrà inviato un codice all'indirizzo di posta elettronica dell'utente guest. L'utente guest seleziona **Send code** (Invia codice):
   ...
   ![Screenshot del pulsante Invia codice](media/one-time-passcode/otp-send-code.png)
...
Un passcode viene inviato all'indirizzo di posta elettronica dell'utente. L'utente recupera il passcode dal messaggio di posta elettronica e lo immette nella finestra del browser:
   ...
   ![Schermata che mostra la tabella codici Invio](media/one-time-passcode/otp-enter-code.png)
...
È possibile visualizzare gli utenti guest che eseguono l'autenticazione con passcode monoorari nel portale di Azure accedendo a**Relazioni organizzative** > di **Azure Active Directory** > **Utenti di altre organizzazioni.**

![Screenshot che mostra un utente passcode monouso con valore di origine OTP](media/one-time-passcode/otp-users.png)
...
Utente guest alexdoe@gmail.com viene invitato a Fabrikam, che non dispone di federazione Google configurata. Alex non ha un account Microsoft. Riceveranno un passcode monouso per l'autenticazione.
...
#### <a name="prerequisite-install-the-latest-azureadpreview-module"></a>Prerequisito: installare il modulo AzureADPreview più recentePrerequisite: Install the latest AzureADPreview module
...
Installare il modulo AzureADPreview più recente se non è già presente (vedere [Prerequisito: installare il modulo AzureADPreview più recente](#prerequisite-install-the-latest-azureadpreview-module) sopra). Verificare quindi che i criteri di anteprima di passcode monouso siano attualmente esistenti eseguendo il comando seguente:
