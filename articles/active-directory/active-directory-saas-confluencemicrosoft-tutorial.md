...
6. Contattare [Microsoft](mailto:waadpartners@microsoft.com) con le informazioni seguenti per il plug-in Confluence.
    ...
    *   Azure AD Premium: sì/no (il plug-in è disponibile per tutte le SKU per i clienti, ovvero Gratuito, Basic e Premium)
...
10. Caricare manualmente il plug-in fornito da Microsoft. Dopo aver installato il plug-in, questo comparirà nella sezione dei componenti aggiuntivi **User Installed** (Installati dall'utente) della sezione **Manage Add-on** (Gestisci componente aggiuntivo).
...
12. Seguire questa procedura nella pagina di configurazione:
    ...
    > [!Note]
    > La posizione predefinita dell'ID utente SAML è nell'identificatore del nome. È possibile sostituirlo con un elemento Attribute e immettere il nome di attributo appropriato.

    > [!TIP]
    > Assicurarsi che esista un solo certificato mappato all'app in modo che non si verifichino errori durante la risoluzione dei metadati. Se sono presenti più certificati, durante la risoluzione dei metadati, l'amministratore riceve un errore.
    ...
    f. Se si usa un dominio federato (come AD FS) con Azure AD, fare clic sull'opzione **Enable Home Realm Discovery** (Abilita individuazione area di autenticazione principale) e configurare **Domain Name** (Nome dominio).
