...
2. In alto a destra fare clic sull'account e nell'elenco **Directory** scegliere un tenant di Active Directory in cui si dispone delle autorizzazioni di amministratore.
3. Fare clic su **Azure Active Directory** nel pannello di navigazione a sinistra.
4. Fare clic su **App registrations (Registrazioni app)** e scegliere **Aggiungi**.
5. Seguire le istruzioni e creare una nuova **Applicazione client nativa**. Il **Nome** dell'applicazione deve essere una descrizione per gli utenti finali. L' **URI di reindirizzamento** è una combinazione dello schema e della stringa che Azure AD userà per restituire le risposte dei token.  Immettere un valore specifico per l'applicazione, ad esempio `http://DirectorySearcher`. Fare clic su **Crea** per creare l'applicazione.
6. Sempre nel Portale di Azure scegliere l'applicazione, fare clic su **Impostazioni** e scegliere **Proprietà**.
7. Individuare il valore dell'ID applicativa e copiarlo negli Appunti.
8. Configurare le autorizzazioni per l'applicazione: nel menu Impostazioni scegliere la sezione "Autorizzazioni necessarie", fare clic su **Aggiungi**, quindi **Selezionare un'API** e selezionare "Windows Azure Active Directory" (questa è l'API AADGraph). Fare quindi clic su **Selezionare le autorizzazioni** e selezionare "Lettura dati directory". In questo modo l'applicazione potrà cercare gli utenti nell'API Graph.
