...
# Integrare Azure AD in un'app WPF desktop di Windows
...
- È opportuno assicurarsi che, quando l'utente fa clic sul pulsante "Sign Out", la chiamata successiva a `AcquireToken(...)` chiederà all'utente di accedere. Con ADAL, basta cancellare la cache dei token:
...
- Se tuttavia l'utente non fa clic sul pulsante di "disconnessione", è consigliabile mantenere la sessione dell'utente per la prossima esecuzione di DirectorySearcher. Quando viene avviata l'app, è possibile cercare nella cache dei token di ADAL se esiste un token e aggiornare di conseguenza l'interfaccia utente. In `MainWindow()` eseguire un'altra chiamata a `AcquireToken(...)`, questa volta passando il parametro `PromptBehavior.Never`. `PromptBehavior.Never` comunicherà ad ADAL che non dovrà richiedere all'utente di accedere e che ADAL dovrà invece generare un'eccezione se non è in grado di restituire un token.
