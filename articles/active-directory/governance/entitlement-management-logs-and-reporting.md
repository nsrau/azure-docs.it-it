...
**Ruolo prerequisiti**: amministratore globale
...
1. In seguito, per visualizzare l'intervallo di date contenute nell'area di lavoro, è possibile utilizzare la cartella di lavoro relativa all' *intervallo di date del log archiviato* :  
    
    1. Selezionare **Azure Active Directory** quindi fare clic su **cartelle di lavoro**. 
    
    1. Espandere la sezione **Azure Active Directory risoluzione dei problemi**e fare clic sull' **intervallo di date del log archiviato**. 
...
1. Nella portale di Azure selezionare **Azure Active Directory** quindi fare clic su **cartelle di lavoro**. Se si dispone di una sola sottoscrizione, procedere al passaggio 3. 

1. Se si dispone di più sottoscrizioni, selezionare la sottoscrizione che contiene l'area di lavoro.  

1. Selezionare la cartella di lavoro denominata *attività di accesso al pacchetto*. 

1. In tale cartella di lavoro, selezionare un intervallo di tempo **(se non** sicuro) e selezionare un ID pacchetto di accesso nell'elenco a discesa di tutti i pacchetti di accesso con attività durante tale intervallo di tempo. Verranno visualizzati gli eventi correlati al pacchetto di accesso che si è verificato durante l'intervallo di tempo selezionato.  

    ![Visualizza eventi del pacchetto di accesso](./media/entitlement-management-logs-and-reporting/view-events-access-package.png) 
...
È possibile eseguire nuovamente l'autenticazione e associare la sessione di PowerShell a tale sottoscrizione usando un comando `Connect-AzAccount –Subscription $subs[0].id`come. Per altre informazioni su come eseguire l'autenticazione in Azure da PowerShell, incluso in modo non interattivo, vedere [accedere con Azure PowerShell](/powershell/azure/authenticate-azureps?view=azps-3.3.0&viewFallbackFrom=azps-2.5.0
).

Se nella sottoscrizione sono presenti più aree di lavoro Log Analytics, il cmdlet [Get-AzOperationalInsightsWorkspace](/powershell/module/Az.OperationalInsights/Get-AzOperationalInsightsWorkspace) restituisce l'elenco delle aree di lavoro. A questo punto è possibile trovare quello con i log Azure AD. Il `CustomerId` campo restituito da questo cmdlet corrisponde al valore dell'ID dell'area di lavoro visualizzato nella portale di Azure nella panoramica dell'area di lavoro log Analytics.
...
È anche possibile recuperare gli eventi di gestione dei diritti usando una query come la seguente:
