---
title: Creare una strategia di gestione del controllo di accesso resiliente - Azure ADCreate a resilient access control management strategy - Azure AD
...
* Ridurre il blocco utente utilizzando più controlli di accesso condizionale (CA).
...
### <a name="mitigating-user-lockout"></a>Riduzione del blocco dell'utente
 ...
 Per ridurre il rischio di blocco degli utenti, usare criteri di accesso condizionale con più controlli per offrire agli utenti la possibilità di scegliere come accedere alle app e alle risorse. Se si permette a un utente di scegliere tra, ad esempio, accedere con autenticazione a più fattori **o** da un dispositivo gestito **o** dalla rete aziendale, se uno dei controlli di accesso non è disponibile l'utente ha altre opzioni per continuare a lavorare.
...
* Criterio 1: Bloccare l'accesso a persone esterne ai gruppi di destinazione
  ...
  * Condizioni: (nessuna)
  * Controllo Grant: Blocca
* Criterio 2: Concedere l'accesso agli utenti di Appche richiedono l'autenticazione a più fattori o un dispositivo attendibile.
  * Utenti e gruppi: includere AppUsers. Escludere CoreAdmins ed EmergencyAccess
  * App cloud: includi tutte le app
  * Condizioni: (nessuna)
  * Concedi controllo: concedi l'accesso, richiedi l'autenticazione a più fattori, richiedi che il dispositivo sia conforme. Per più controlli: richiede uno dei controlli selezionati.
...
* Assegnare un nome ai criteri per esseri sicuri di trovarli facilmente durante un'interruzione. Includere gli elementi seguenti nel nome dei criteri:
  ...
  * L'*interruzione* a cui si applica il criterio. Ad esempio: **Durante l'interruzione dell'autenticazione a più fattoriFor example: During MFA Disruption**
...
* Criterio 1: Richiedere dispositivi aggiunti al dominio per Exchange e SharePoint
  * Nome: EM001 - ENABLE IN EMERGENCY: Interruzione dell'autenticazione a più fattori[1/4] - Exchange SharePoint - Richiedi aggiunta ad Azure AD ibrido
  ...
  * Stato: Disabilitato
* Criterio 2: Blocca piattaforme diverse da Windows
  * Nome: EM002 - ENABLE IN EMERGENCY: Interruzione dell'autenticazione a più fattori[2/4] - Exchange SharePoint - Blocca accesso ad eccezione di Windows
  * Utenti e gruppi: includere tutti gli utenti. Escludere CoreAdmins ed EmergencyAccess
  * App cloud: Exchange Online e SharePoint Online
  * Condizioni: Piattaforma dispositivo Includere tutte le piattaforme, escludere Windows
  * Controllo Grant: Blocca
  * Stato: Disabilitato
* Criterio 3: Bloccare reti diverse da CorpNetwork
  * Nome: EM003 - ENABLE IN EMERGENCY: Interruzione dell'autenticazione a più fattori[3/4] - Exchange SharePoint - Blocca l'accesso ad eccezione di Rete aziendale
  * Utenti e gruppi: includere tutti gli utenti. Escludere CoreAdmins ed EmergencyAccess
  * App cloud: Exchange Online e SharePoint Online
  * Condizioni: Le posizioni includono qualsiasi posizione, escludi CorpNetwork
  * Controllo Grant: Blocca
  * Stato: Disabilitato
* Criterio 4: Bloccare EAS in modo esplicitoPolicy 4: Block EAS Explicitly
  * Nome: EM004 - ENABLE IN EMERGENCY: Interruzione dell'autenticazione a più fattori[4/4] - Exchange - Blocca EAS per tutti gli utenti
...
2. Abilita criteri 1: verificare che gli utenti nei dispositivi aggiunti al dominio che non fanno parte dei gruppi di esclusione siano in grado di accedere a Exchange Online e SharePoint Online. Verificare che gli utenti nel gruppo Exclude possano accedere a SharePoint Online ed Exchange da qualsiasi dispositivo.
3. Abilita criteri 2: verificare che gli utenti che non fanno parte del gruppo di esclusione non possano accedere a SharePoint Online ed Exchange Online dai propri dispositivi mobili. Verificare che gli utenti nel gruppo Exclude possano accedere a SharePoint ed Exchange da qualsiasi dispositivo (Windows/iOS/Android).
4. Abilita criteri 3: verificare che gli utenti che non fanno parte dei gruppi di esclusione non possano accedere a SharePoint ed Exchange dalla rete aziendale, anche con un computer aggiunto a un dominio. Verificare che gli utenti nel gruppo di esclusione possano accedere a SharePoint ed Exchange da qualsiasi rete.
5. Abilitare il criterio 4: verificare che tutti gli utenti non possano ottenere Exchange Online dalle applicazioni di posta native nei dispositivi mobili.
...
* Criterio 1: bloccare tutti gli utenti che non fanno parte del team SalesContingency
  * Nome: EM001 - ENABLE IN EMERGENCY: Interruzione della conformità del dispositivo[1/2] - Salesforce - Blocca tutti gli utenti ad eccezione di SalesforceContingency
...
* Criterio 2: Bloccare il team di vendita da qualsiasi piattaforma diversa da mobile (per ridurre la superficie di attacco)
  * Nome: EM002 - ENABLE IN EMERGENCY: Interruzione della conformità del dispositivo[2/2] - Salesforce - Blocca tutte le piattaforme ad eccezione di iOS e Android
  * Utenti e gruppi: includere SalesforceContingency. Escludere SalesAdmins
  * App cloud: Salesforce
  * Condizioni: Piattaforma dispositivo Includere tutte le piattaforme, escludere iOS e Android
  * Controllo Grant: Blocca
  * Stato: Disabilitato
...
2. Abilita criteri 1: gli utenti esterni a SalesContingency non possono accedere a Salesforce. Verificare che gli utenti in SalesAdmins e SalesforceContingency possano accedere a Salesforce.
3. Abilita policy 2: verificare che gli utenti del gruppo SalesContingency non possano accedere a Salesforce dai computer portatili Windows/Mac, ma possano comunque accedere dai propri dispositivi mobili. Verificare che SalesAdmin possa comunque accedere a Salesforce da qualsiasi dispositivo.
...
4. [Valutare tutti i rilevamenti dei rischi segnalati](https://docs.microsoft.com/azure/active-directory/reports-monitoring/concept-sign-ins) durante questa finestra.
...
5. Continuare a [valutare tutti i rilevamenti dei rischi segnalati](https://docs.microsoft.com/azure/active-directory/reports-monitoring/concept-sign-ins) dopo l'interruzione per attività sospette.
...
## <a name="emergency-options"></a>Opzioni di emergenza
 ...
 In caso di emergenza e l'organizzazione non ha implementato in precedenza un piano di attenuazione o emergenza, seguire i suggerimenti nella sezione [Contingecies for user lockout](#contingencies-for-user-lockout) se già utilizzano criteri di accesso condizionale per applicare l'autenticazione a più fattori.
...
>[!IMPORTANT]
 > Se si ampliano gli indirizzi IP attendibili per sbloccare l'accesso, i rilevamenti dei rischi associati agli indirizzi IP (ad esempio, viaggi impossibili o posizioni sconosciute) non verranno generati.
...
* [Configurare percorsi denominati in Azure Active DirectoryConfigure named locations in Azure Active Directory](https://docs.microsoft.com/azure/active-directory/reports-monitoring/quickstart-configure-named-locations)
...
* [Quali sono le condizioni in Azure Active Directory Conditional Access?](https://docs.microsoft.com/azure/active-directory/conditional-access/conditions)
* [Che cosa sono i controlli di accesso in Azure Active Directory Conditional Access?](https://docs.microsoft.com/azure/active-directory/conditional-access/controls)
