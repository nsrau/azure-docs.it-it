...
* [In che modo il server Azure Multi-Factor Authentication gestisce i dati utente?](#how-does-azure-multi-factor-authentication-server-handle-user-data)
* [Quali codici brevi SMS vengono utilizzati per l'invio di messaggi SMS ai miei utenti?](#what-sms-short-codes-are-used-for-sending-sms-messages-to-my-users)
...
Negli Stati Uniti, utilizziamo i seguenti codici brevi SMS:
...
In Canada, utilizziamo i seguenti codici brevi SMS:

* *759731*
* *673801*

Non è garantito la consegna coerente dei prompt SMS o di autenticazione a più fattori basata su voce con lo stesso numero. Nell'interesse dei nostri utenti, possiamo aggiungere o rimuovere codici brevi in qualsiasi momento mentre apportiamo modifiche al percorso per migliorare la consegna degli SMS. Non supportiamo codici brevi per paesi o regioni oltre agli Stati Uniti e al Canada.

## <a name="billing"></a>Fatturazione

È possibile rispondere alla maggior parte delle domande di fatturazione facendo riferimento alla [pagina Prezzi dell'autenticazione a più fattori](https://azure.microsoft.com/pricing/details/multi-factor-authentication/) o alla documentazione per le versioni di Azure [Multi-Factor Authentication e](concept-mfa-licensing.md)i piani di consumo .

* [All'organizzazione viene addebitato l'invio delle telefonate e dei messaggi di testo utilizzati per l'autenticazione?](#is-my-organization-charged-for-sending-the-phone-calls-and-text-messages-that-are-used-for-authentication)
* [Il modello di fatturazione per utente mi addebita un costo per tutti gli utenti abilitati o solo per quelli che hanno eseguito la verifica in due passaggi?](#does-the-per-user-billing-model-charge-me-for-all-enabled-users-or-just-the-ones-that-performed-two-step-verification)
* [Come funziona la fatturazione di Multi-Factor Authentication?](#how-does-multi-factor-authentication-billing-work)
* [Esiste una versione gratuita di Azure Multi-Factor Authentication?](#is-there-a-free-version-of-azure-multi-factor-authentication)
* [L'organizzazione può passare da un modello di fatturazione del consumo per utente a uno per autenticazione in qualsiasi momento?](#can-my-organization-switch-between-per-user-and-per-authentication-consumption-billing-models-at-any-time)
* [L'organizzazione può passare dalla fatturazione basata sul consumo agli abbonamenti (un modello basato su licenza) in qualsiasi momento?](#can-my-organization-switch-between-consumption-based-billing-and-subscriptions-a-license-based-model-at-any-time)
* [L'organizzazione deve usare e sincronizzare le identità per usare Azure Multi-Factor Authentication?](#does-my-organization-have-to-use-and-synchronize-identities-to-use-azure-multi-factor-authentication)
...
No, non viene addebitato alcun costo per le singole telefonate effettuate o per i messaggi di testo inviati agli utenti tramite Azure Multi-Factor Authentication. Se si utilizza un provider di autenticazione a più fattori per autenticazione, viene addebitato per ogni autenticazione, ma non per il metodo utilizzato.
...
Le impostazioni predefinite di sicurezza possono essere abilitate nel livello Gratuito di Azure AD. Con le impostazioni predefinite di sicurezza, tutti gli utenti sono abilitati per l'autenticazione a più fattori tramite l'app Microsoft Authenticator.With security defaults, all users are enabled for multi-factor authentication using the Microsoft Authenticator app. Non è possibile utilizzare SMS o verifica telefonica con le impostazioni predefinite di sicurezza, solo l'app Microsoft Authenticator.

Per altre informazioni, vedere [Che cosa sono le impostazioni predefinite di sicurezza?](../fundamentals/concept-fundamentals-security-defaults.md)
...
Se la directory dispone di un provider di Azure Multi-Factor Authentication per *autenticazione,* viene sempre addebitata ogni autenticazione, purché il provider di autenticazione a più fattori sia collegato alla sottoscrizione. È possibile assegnare le licenze MFA agli utenti, ma verrà comunque fatturata ogni richiesta di verifica in due passaggi, che provenga o meno da un utente con assegnata una licenza di autenticazione a più fattori.
...
Se l'organizzazione usa un modello di fatturazione in base al consumo, è possibile usare Azure Active Directory ma non è obbligatorio. Se il provider di autenticazione a più fattori non è collegato a un tenant di Azure AD, è possibile distribuire solo il server Azure Multi-Factor Authentication in locale.
...
* [Cosa devo dire ai miei utenti di fare se non ricevono una risposta sul loro telefono?](#what-should-i-tell-my-users-to-do-if-they-dont-receive-a-response-on-their-phone)
* [Cosa devo fare se uno dei miei utenti non riesce ad accedere al proprio account?](#what-should-i-do-if-one-of-my-users-cant-get-in-to-their-account)
* [Cosa devo fare se uno dei miei utenti perde un telefono che utilizza le password dell'app?](#what-should-i-do-if-one-of-my-users-loses-a-phone-that-is-using-app-passwords)
* [Cosa succede se un utente non riesce ad accedere ad app non basate su browser?](#what-if-a-user-cant-sign-in-to-non-browser-apps)
* [I miei utenti dicono che a volte non ricevono il messaggio di testo o il timeout di verifica.](#my-users-say-that-sometimes-they-dont-receive-the-text-message-or-the-verification-times-out)
* [Posso modificare la quantità di tempo per cui i miei utenti hanno per inserire il codice di verifica da un messaggio di testo prima del timeout del sistema?](#can-i-change-the-amount-of-time-my-users-have-to-enter-the-verification-code-from-a-text-message-before-the-system-times-out)
* [È possibile usare i token hardware con Azure Multi-Factor Authentication Server?](#can-i-use-hardware-tokens-with-azure-multi-factor-authentication-server)
* [È possibile usare Azure Multi-Factor Authentication Server per proteggere Servizi terminal?](#can-i-use-azure-multi-factor-authentication-server-to-secure-terminal-services)
* [Ho configurato l'ID chiamante in MFA Server, ma gli utenti ricevono ancora chiamate di autenticazione a più fattori da un chiamante anonimo.](#i-configured-caller-id-in-mfa-server-but-my-users-still-receive-multi-factor-authentication-calls-from-an-anonymous-caller)
...
### <a name="what-should-i-tell-my-users-to-do-if-they-dont-receive-a-response-on-their-phone"></a>Cosa devo dire ai miei utenti di fare se non ricevono una risposta sul loro telefono?

Chiedi agli utenti di tentare fino a cinque volte in 5 minuti per ricevere una telefonata o un SMS per l'autenticazione. Microsoft usa più provider per la distribuzione di chiamate e messaggi SMS. Se questo approccio non funziona, aprire un caso di supporto per risolvere ulteriori problemi.

Le app di sicurezza di terze parti possono anche bloccare il messaggio di testo o la telefonata del codice di verifica. Se si usa un'app di sicurezza di terze parti, provare a disabilitare la protezione, quindi richiedere l'invio di un altro codice di verifica dell'autenticazione a più fattori.

Se i passaggi precedenti non funzionano, controlla se gli utenti sono configurati per più di un metodo di verifica. Prova ad accedere di nuovo, ma seleziona un metodo di verifica diverso nella pagina di accesso.
...
Se l'organizzazione non dispone di client legacy, non è consigliabile consentire agli utenti di creare password per le app.
...
### <a name="my-users-say-that-sometimes-they-dont-receive-the-text-message-or-the-verification-times-out"></a>I miei utenti dicono che a volte non ricevono il messaggio di testo o il timeout di verifica.

Il recapito dei messaggi SMS non è garantito perché esistono fattori incontrollabili che potrebbero influire sull'affidabilità del servizio. Questi fattori includono il paese o l'area geografica di destinazione, l'operatore di telefonia mobile e la potenza del segnale.

Le app di sicurezza di terze parti possono anche bloccare il messaggio di testo o la telefonata del codice di verifica. Se si usa un'app di sicurezza di terze parti, provare a disabilitare la protezione, quindi richiedere l'invio di un altro codice di verifica dell'autenticazione a più fattori.

Se gli utenti hanno spesso problemi con la ricezione affidabile dei messaggi di testo, chiedi loro di usare l'app Microsoft Authenticator o il metodo di chiamata telefonica. Microsoft Authenticator può ricevere notifiche tramite connessioni cellulari e Wi-Fi. L'app per dispositivi mobili può generare codici di verifica anche in caso di totale assenza di segnale. L'app Microsoft Authenticator è disponibile per [Android,](https://go.microsoft.com/fwlink/?Linkid=825072) [iOS](https://go.microsoft.com/fwlink/?Linkid=825073)e [Windows Phone.](https://www.microsoft.com/p/microsoft-authenticator/9nblgggzmcj6)
...
2. Creare una chiave del Registro di sistema **DWORD** denominata *pfsvc_pendingSmsTimeoutSeconds* e impostare il tempo in secondi in cui si vuole che Azure MFA Server archivi una sola volta.
...
Per gli SMS unidirezionali con Azure MFA nel cloud (inclusa la scheda ADFS o l'estensione Server dei criteri di rete), non è possibile configurare l'impostazione di timeout. Azure AD archivia il codice di verifica per 180 secondi.
...
Se si usa Azure Multi-Factor Authentication Server, è possibile importare token di password monouso e di autenticazione aperta di terze parti e quindi usarli per la verifica in due passaggi.
...
Sì, ma se si utilizza Windows Server 2012 R2 o versione successiva, è possibile proteggere Servizi terminal solo utilizzando Gateway Desktop remoto.
...
Quando vengono effettuate chiamate MFA tramite la rete telefonica pubblica, queste vengono a volte indirizzate su un gestore che non supporta l'ID chiamante. A causa di questo comportamento dell'operatore, l'ID chiamante non è garantito, anche se il sistema Multi-Factor Authentication lo invia sempre.
...
- L'utente sta registrando un dispositivo con Azure AD (incluso l'aggiunta ad Azure AD) e l'organizzazione richiede l'autenticazione a più fattori per la registrazione del dispositivo, ma l'utente non è stato registrato in precedenza per l'autenticazione a più fattori.
...
* [Cosa devono fare gli utenti se viene visualizzato un messaggio di errore "Richiesta di autenticazione non per un account attivato" quando si utilizzano le notifiche delle app per dispositivi mobili?](#what-should-users-do-if-they-see-an-authentication-request-is-not-for-an-activated-account-error-message-when-using-mobile-app-notifications)
* [Cosa devono fare gli utenti se viene visualizzato un messaggio di errore 0x800434D4L quando accedono a un'applicazione non browser?](#what-should-users-do-if-they-see-a-0x800434d4l-error-message-when-signing-in-to-a-non-browser-application)

### <a name="what-should-users-do-if-they-see-an-authentication-request-is-not-for-an-activated-account-error-message-when-using-mobile-app-notifications"></a>Cosa devono fare gli utenti se viene visualizzato un messaggio di errore "Richiesta di autenticazione non per un account attivato" quando si utilizzano le notifiche delle app per dispositivi mobili?

Chiedere all'utente di completare la procedura seguente per rimuovere il proprio account da Microsoft Authenticator, quindi aggiungerlo di nuovo:

1. Passare al [profilo del portale di Azure](https://account.activedirectory.windowsazure.com/profile/) e accedere con un account aziendale.
2. Selezionare **Verifica aggiuntiva di sicurezza**.
3. Rimuovere l'account esistente dall'app Microsoft Authenticator.
4. Fare clic su **Configura**e quindi seguire le istruzioni per riconfigurare Microsoft Authenticator.
...
L'errore *0x800434D4L* si verifica quando si tenta di accedere a un'applicazione non browser, installata in un computer locale, che non funziona con gli account che richiedono la verifica in due passaggi.
...
Se la tua domanda non trova risposta qui, sono disponibili le seguenti opzioni di supporto:
...
* Cercare ed esplorare domande e risposte tecniche dalla community oppure porre una domanda personale nella&A di [Azure Active Directory.](https://docs.microsoft.com/answers/topics/azure-active-directory.html)
* Contattare Microsoft Professional tramite il supporto per [Azure Multi-Factor Authentication Server](https://support.microsoft.com/oas/default.aspx?prid=14947). Quando si contatta Microsoft, è utile includere il maggior numero possibile di informazioni relative al problema. Tali informazioni includono la pagina in cui viene visualizzato l'errore, il codice dell'errore specifico, l'ID della sessione specifico e l'ID dell'utente che visualizza l'errore.
* Se sei un cliente PhoneFactor legacy e hai domande o hai bisogno [phonefactorsupport@microsoft.com](mailto:phonefactorsupport@microsoft.com) di aiuto per reimpostare una password, usa l'indirizzo e-mail per aprire un caso di supporto.
