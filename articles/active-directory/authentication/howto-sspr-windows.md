...
- La combinazione delle tre impostazioni specifiche seguenti può causare un funzionamento non corretto di questa funzionalità.
    ...
    - Lo SKU di Windows non è Home o Professional Edition
...
1. Creare un nuovo profilo di configurazione del dispositivo passando a **configurazione** > dispositivo**profili** > **Crea profilo**
...
Quando gli utenti reimpostano la password dalla schermata di accesso di un dispositivo Windows 10, `defaultuser1` viene creato un account temporaneo con privilegi limitati. Questo account viene usato per proteggere il processo di reimpostazione della password. L'account stesso ha una password generata in modo casuale, non viene visualizzato per l'accesso del dispositivo e verrà rimosso automaticamente dopo la reimpostazione della password da parte dell'utente. Possono `defaultuser` esistere più profili, ma possono essere ignorati.
