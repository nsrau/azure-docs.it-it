...
| User registered all required security info | Esito negativo | User canceled security info registration. | This event occurs when a user cancels registration from interrupt mode.|
...
| User reviewed security info | Esito negativo | User failed to review security info. | This event occurs when a user selects **Looks good** on the security info review page but something fails on the backend.|
...
| User deleted security info | Esito negativo | User failed to delete *method*. | This event occurs when a user tries to delete a method but the attempt fails for some reason. *Method* can be Authenticator app, Phone, Email, Security questions, App password, Alternate phone, and so on.|
...
1. Install the appropriate Azure AD PowerShell modules. In una finestra di PowerShell eseguire questi comandi per installare i moduli:
...
1. Save the list of affected user object IDs to your computer as a text file with one ID per line. Prendere nota del percorso del file.
...
In a PowerShell window, run the following command, providing the script and user file locations. Immettere le credenziali di amministratore globale, quando richieste. Lo script genererà il risultato dell'operazione di aggiornamento di ogni utente.
