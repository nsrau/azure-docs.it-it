...
5. Azure Multi-Factor Authentication comunica con Azure Active Directory, recupera i dettagli dell'utente ed esegue l'autenticazione secondaria usando il metodo configurato dall'utente (chiamata telefono cellulare, messaggio di testo o app per dispositivi mobili).
...
2. Nel menu portale di Azure selezionare **Azure Active Directory**oppure cercare e selezionare **Azure Active Directory** da qualsiasi pagina.

3. Selezionare **Proprietà**.

4. Per copiare l'ID di Azure AD, selezionare il pulsante **Copia**.

    ![ID directory Azure AD nell'portale di Azure](./media/howto-mfa-nps-extension-vpn/azure-active-directory-id-in-azure-portal.png)
...
2. Al prompt dei comandi di PowerShell, immettere **CD "C:\Program c:\programmi\microsoft\azuremfa\config"** e quindi premere INVIO.
...
3. Al prompt dei comandi successivo, immettere **.\AzureMfaNpsExtnConfigSetup.ps1**, quindi premere INVIO. Lo script verifica che il modulo di PowerShell per Azure AD sia installato. Se non è installato, lo installa.
    ...
    Se si riceve un errore di sicurezza causato da TLS, abilitare TLS 1,2 usando `[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12` il comando dal prompt di PowerShell.
