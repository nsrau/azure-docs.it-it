---
title: Usare Multi-Factor Authentication di Azure con NPS-Azure Active Directory
description: Informazioni su come usare le funzionalità di Multi-Factor Authentication di Azure con l'infrastruttura di autenticazione server dei criteri di rete (NPS) esistente
...
L'estensione NPS (Network Policy Server) per Azure Multi-Factor Authentication aggiunge funzionalità di autenticazione a più fattori basate sul cloud all'infrastruttura di autenticazione usando i server esistenti. Con l'estensione di Server dei criteri di rete, è possibile aggiungere la verifica con telefonata, messaggio di testo o app telefonica al flusso di autenticazione esistente senza dover installare, configurare e gestire nuovi server.

L'estensione NPS funge da adattatore tra RADIUS e Azure Multi-Factor Authentication basato sul cloud per fornire un secondo fattore di autenticazione per gli utenti federati o sincronizzati.
...
![Diagramma del flusso di autenticazione per l'autenticazione utente tramite un server VPN al server dei criteri di rete e l'estensione server dei criteri di rete Multi-Factor Authentication di Azure](./media/howto-mfa-nps-extension/auth-flow.png)
...
Il server NPS potrebbe non rispondere alla richiesta originale del server VPN prima che si verifichi il timeout della connessione perché la richiesta di autenticazione a più fattori potrebbe essere ancora in corso di elaborazione. È possibile che l'utente non abbia risposto correttamente alla richiesta di autenticazione a più fattori, quindi l'estensione server dei criteri di Multi-Factor Authentication di Azure è in attesa del completamento dell'evento. In questa situazione, il server dei criteri di rete identifica richieste server VPN aggiuntive come richiesta di duplicazione. Il server dei criteri di rete elimina queste richieste di server VPN duplicate.
...
Se si esaminano i log del server NPS, è possibile che vengano rimosse le seguenti richieste aggiuntive. Questo comportamento è progettato per impedire all'utente finale di ottenere più richieste per un singolo tentativo di autenticazione. Le richieste scartate nel registro eventi del server dei criteri di server non indicano un problema con il server NPS o con l'estensione NPS di Azure Multi-Factor Authentication.
...
A causa di questo comportamento del protocollo UDP, il server NPS può ricevere una richiesta duplicata e inviare un altro prompt di autenticazione a più fattori, anche dopo che l'utente ha già risposto alla richiesta iniziale. Per evitare questa condizione di temporizzazione, l'estensione server dei criteri di rete di Azure Multi-Factor Authentication continua a filtrare ed eliminare richieste duplicate per un massimo di 10 secondi dopo che è stata inviata una risposta con esito positivo al server VPN.
...
Anche in questo caso, è possibile che vengano rilevate richieste ignorate nei registri eventi del server dei criteri di accesso, anche quando la richiesta di Multi-Factor Authentication di Azure è riuscita. Si tratta di un comportamento previsto e non indica un problema con il server NPS o con l'estensione NPS di Azure Multi-Factor Authentication.
...
È possibile creare tutti i server dei criteri di NPS abilitati per Multi-Factor Authentication di Azure necessari. Se si installano più server, è consigliabile usare un certificato client di differenza per ciascuno. La creazione di un certificato per ogni server significa che è possibile aggiornare ogni certificato singolarmente e non preoccuparsi dei tempi di inattività in tutti i server.

I server VPN instradano le richieste di autenticazione, pertanto devono essere a conoscenza dei nuovi server dei criteri di rete abilitati per Multi-Factor Authentication Azure.
...
L'estensione NPS per Azure Multi-Factor Authentication è disponibile per i clienti con [licenze per multi-factor authentication di Azure](multi-factor-authentication.md). Le licenze basate sul consumo per Azure Multi-Factor Authentication, ad esempio per utente o licenze per autenticazione, non sono compatibili con l'estensione NPS.
...
- L'estensione NPS per Azure Multi-Factor Authentication non include strumenti per la migrazione di utenti e impostazioni dal server multi-factor authentication al cloud. Per questo motivo, è consigliabile usare l'estensione per le distribuzioni nuove piuttosto che per quelle esistenti. Se si usano le estensioni in una distribuzione esistente, gli utenti dovranno ripetere il processo di registrazione per popolare i dettagli di Azure MFA nel cloud.  
- L'estensione NPS usa il nome UPN dell'ambiente di servizi di dominio Active Directory locale per identificare l'utente in Azure Multi-Factor Authentication per l'esecuzione dell'autenticazione secondaria. L'estensione può essere configurata in modo da usare un identificatore diverso come ID di accesso alternativo o campo di servizi di dominio Active Directory personalizzato diverso da UPN. Per altre informazioni, vedere l'articolo [Opzioni di configurazione avanzate per l'estensione NPS per Multi-Factor Authentication](howto-mfa-nps-extension-advanced.md).
