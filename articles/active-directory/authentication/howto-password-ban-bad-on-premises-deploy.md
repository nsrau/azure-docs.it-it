...
1. Configurare il servizio proxy per la protezione con password per la comunicazione tramite un proxy HTTP.
   ...
   Se il proxy HTTP è configurato per usare un criterio di autorizzazione, è necessario concedere l'accesso all'account del computer di Active Directory del computer che ospita il servizio proxy per la protezione con password.
