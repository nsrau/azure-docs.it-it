...
* **Supporta la reimpostazione delle password per gli utenti che usano la** [sincronizzazione dell'hash delle password](./../connect/active-directory-aadconnectsync-implement-password-hash-synchronization.md): quando il servizio di reimpostazione della password rileva che un account utente sincronizzato è abilitato per la sincronizzazione dell'hash delle password, vengono reimpostate sia la password locale sia la password cloud di questo account. Il writeback delle password non dipende dalla sincronizzazione dell'hash delle password.
