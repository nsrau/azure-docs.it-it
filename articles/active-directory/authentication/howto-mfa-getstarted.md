...
6. Select whether you are defining the location using IP ranges or Countries/Regions
   1. If using IP Ranges
      1. Decide whether to mark the location as Trusted. L'accesso da una posizione attendibile riduce il rischio di accesso dell'utente. Only mark this location as trusted if you know the IP ranges entered are established and credible in your organization.
...
A mobile app like the Microsoft Authenticator app generates a new OATH verification code every 30 seconds. L'utente immette il codice di verifica nell'interfaccia di accesso. The mobile app option can be used whether or not the phone has a data or cellular signal.
...
L'estensione di Server dei criteri di rete (NPS) per Azure MFA aggiunge funzionalità MFA basate su cloud per l'infrastruttura di autenticazione usando i server esistenti. With the NPS extension, you can add phone call, text message, or phone app verification to your existing authentication flow. This integration has the following limitations:
...
Lo scopo di questa impostazione è stabilire cosa fare quando un utente non è registrato per MFA. The effects of changing this setting are listed in the table below.

| Impostazioni | User MFA Status | Effects |
| --- | --- | --- |
