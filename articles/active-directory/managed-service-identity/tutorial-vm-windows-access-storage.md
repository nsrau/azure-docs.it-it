---
title: Usare un'identità gestita per una macchina virtuale Windows per accedere ad Archiviazione di Azure
description: Questa esercitazione illustra come usare un'identità del gestita per una macchina virtuale Windows per accedere ad Archiviazione di Azure.
...
Questa esercitazione illustra come abilitare l'identità gestita per una macchina virtuale Windows e quindi usare l'identità per accedere ad Archiviazione di Azure.  Si apprenderà come:

> [!div class="checklist"]
> * Creare una macchina virtuale Windows in un nuovo gruppo di risorse 
> * Abilitare l'identità gestita in una macchina virtuale (VM) Windows
> * Creare un contenitore BLOB nell'account di archiviazione
> * Concedere l'accesso dell'identità gestita della macchina virtuale (VM) Windows a un account di archiviazione 
...
In questa sezione, si crea una macchina virtuale Windows a cui, in un secondo momento, viene concessa un'identità gestita.
...
Ecco un esempio di codice .Net per l'apertura di una connessione ad Archiviazione di Azure usando un token di accesso e quindi la lettura del contenuto del file creato in precedenza. Questo codice deve essere eseguito nella macchina virtuale per poter accedere all'endpoint dell'identità gestita per la macchina virtuale. Per usare il metodo del token di accesso è necessario .Net Framework 4.6 o una versione successiva. Sostituire il valore di `<URI to blob file>` di conseguenza. È possibile ottenere questo valore spostandosi sul file creato e caricato in archiviazione BLOB e copiando l'**URL** in **Proprietà** nella pagina **Panoramica**.
...
Questa esercitazione ha illustrato come abilitare un'identità gestita per la macchina virtuale di Windows per accedere ad Archiviazione di Azure.  Per altre informazioni su Archiviazione di Azure, vedere:
