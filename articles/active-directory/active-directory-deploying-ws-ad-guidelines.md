<properties
   pageTitle="Linee guida per la distribuzione di Active Directory di Windows Server in macchine virtuali di Azure| Microsoft Azure"
...
### Per le macchine virtuali di Azure potrebbe essere necessaria la connettività alla rete aziendale locale.
...
## Sicurezza della virtualizzazione dei controller di dominio Active directory di Windows Server
...
> [AZURE.NOTE] È disponibile un'opzione per [creare una VPN da punto a sito](../vpn-gateway/vpn-gateway-point-to-site-create.md) per connettere singoli computer basati su Windows direttamente a una rete virtuale di Azure.
...
## Possibilità di distribuire AD FS di Windows Server in macchine virtuali di Azure
...
| ------ | --------- | ------------ |
| [ACL di rete di Azure](virtual-networks-acl.md) | Configurazione iniziale meno costosa e più semplice. | È necessaria la configurazione di altri ACL di rete se vengono aggiunte nuove VM alla distribuzione. |
...
1. Creare una [rete virtuale con connettività cross-premise](../vpn-gateway/vpn-gateway-cross-premises-options.md) usando una VPN o [ExpressRoute](http://azure.microsoft.com/services/expressroute/).
...
### Alternativa alla distribuzione di AD FS se l'obiettivo è solo SSO di Office 365
...
![Distribuzione di Servizi di dominio Active Directory solo cloud](media/active-directory-deploying-ws-ad-guidelines/ADDS_cloud.png) **Figura 1**
...
#### Considerazioni sullo scenario e modalità di applicazione di aree tecnologiche allo scenario
...
![Federazione con connettività cross-premise](media/active-directory-deploying-ws-ad-guidelines/Federation_xprem.png) **Figura 2**
...
![Distribuzione di Servizi di dominio Active Directory cross-premise](media/active-directory-deploying-ws-ad-guidelines/ADDS_xprem.png) **Figura 3**
...
## Decisioni e fattori relativi alla distribuzione
...
| ---- | ---- | ---- |
| [Topologia di rete](#BKMK_NetworkTopology) | Si crea una rete virtuale? | <li>Requisiti per l'accesso alle risorse aziendali</li> <li>Autenticazione</li> <li>Gestione account</li> |
| [Configurazione della distribuzione di un controller di dominio](#BKMK_DeploymentConfig) | <li>Distribuire una foresta separata senza trust?</li> <li>Distribuire una nuova foresta con federazione?</li> <li>Distribuire una nuova foresta con trust tra foreste di Active Directory di Windows Server o Kerberos?</li> <li>Estendere la foresta aziendale distribuendo una controller di dominio di replica?</li> <li>Estendere la foresta aziendale distribuendo un nuovo dominio figlio o albero di dominio?</li> | <li>Sicurezza</li> <li>Conformità</li> <li>Costo</li> <li>Resilienza e tolleranza di errore</li> <li>Compatibilità delle applicazioni</li> |
| [Topologia del sito Active Directory di Windows Server](#BKMK_ADSiteTopology) | Come si configurano subnet, siti e collegamenti di sito con Rete virtuale di Microsoft Azure per ottimizzare il traffico e ridurre i costi? | <li>Definizioni di sito e subnet</li> <li>Proprietà di collegamenti di sito e notifica di modifiche</li> <li>Compressione della replica</li> |
| [Indirizzamento IP e DNS](#BKMK_IPAddressDNS) | Come configurare gli indirizzi IP e risoluzione dei nomi? | <li>Usare il cmdlet Set-AzureStaticVNetIP per assegnare un indirizzo IP statico</li> <li>Installare il server DNS di Windows Server e configurare le proprietà della rete virtuale con il nome e l'indirizzo IP della VM che ospita i ruoli server DNS e controller di dominio</li> |
...
| [Controller di dominio di sola lettura](#BKMK_RODC) | Usare controller di dominio di sola lettura o scrivibili? | <li>Filtrare attributi HBI/PII</li> <li>Filtrare segreti</li> <li>Limitare il traffico in uscita</li> |
| [Catalogo globale](#BKMK_GC) | Installare il catalogo globale? | <li>Per una foresta a dominio singolo, configurare tutti i controller di dominio come cataloghi globali</li> <li>Per una foresta con più domini, i cataloghi globali sono necessari per l'autenticazione</li> |
| [Metodo di installazione](#BKMK_InstallMethod) | Come installare un controller di dominio in Azure? | È possibile: <li>Installare Servizi di dominio Active Directory tramite Windows PowerShell o Dcpromo</li> <li>Spostare il disco rigido virtuale di un controller di dominio virtuale locale</li> |
...
| [Configurazione del server federativo](#BKMK_FedSrvConfig) | <li>Distribuire una nuova foresta con federazione nel cloud?</li> <li>Distribuire AD FS in locale ed esporre un proxy nel cloud?</li> | <li>Sicurezza</li> <li>Conformità</li> <li>Costo</li> <li>Accesso alle applicazioni da parte di partner commerciali</li> |
...
- È possibile ridurre ancora di più il traffico di rete generato dalla replica tra siti modificando l'algoritmo di compressione della replica. L'algoritmo di compressione è controllato dalla voce del Registro di sistema REG\_DWORD HKEY\_LOCAL\_MACHINE\\SYSTEM\\CurrentControlSet\\Services\\NTDS\\Parameters\\Replicator compression algorithm. Il valore predefinito è 3, che è correlato all'algoritmo di compressione Xpress. È possibile impostare il valore su 2, che modifica l'algoritmo in MSZip. Nella maggior parte dei casi ciò comporta un aumento della compressione, che però incide sull'utilizzo della CPU. Per altre informazioni, vedere l'articolo relativo al [funzionamento della topologia di replica di Active Directory](https://technet.microsoft.com/library/cc755994).
...
> [AZURE.NOTE] Le dimensioni dei dischi dati di Azure sono vincolate a 1 TB.
...
Per stabilire le opzioni di configurazione della distribuzione che meglio soddisfano esigenze particolari, vedere [Considerazioni sulla topologia di distribuzione di AD FS](https://technet.microsoft.com/library/gg982489) nella [Guida alla progettazione di AD FS in Windows Server 2012](https://technet.microsoft.com/library/dd807036).
