---
title: Panoramica del protocollo .NET per Azure AD | Microsoft Docs
...
| Parametro | Descrizione |
| --- | --- |
| id_token |L'`id_token` richiesto dall'app. È possibile usare `id_token` per verificare l'identità dell'utente e avviare una sessione con l'utente. |
...
| Parametro |  | Descrizione |
| --- | --- | --- |
| post_logout_redirect_uri |Consigliato |L'URL di destinazione al quale l’utente deve essere reindirizzato dopo la disconnessione.  Se omesso, all'utente viene visualizzato un messaggio generico. |
