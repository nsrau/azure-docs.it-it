...
## autenticazione
Richiesta di credenziali legittime a una parte, che costituisce la base per la creazione di un'entità di sicurezza da usare per il controllo delle identità e di accesso. Durante una [concessione di autorizzazione](#authorization-grant) OAuth2, ad esempio, la parte che esegue l'autenticazione svolge il ruolo di [proprietario delle risorse](#resource-owner) o di [applicazione client](#client-application) a seconda della concessione usata.
...
"Token" di breve durata fornito a un'[applicazione client](#client-application) dall'[endpoint di autorizzazione](#authorization-endpoint) nell'ambito del flusso del "codice di autorizzazione". È una delle quattro [concessioni di autorizzazione](#authorization-grant) OAuth2. Il codice viene restituito all'applicazione client in risposta all'autenticazione di un [proprietario delle risorse](#resource-owner) e indica che il proprietario ha delegato all'applicazione client l'autorizzazione ad accedere alle risorse per suo conto. Nell'ambito del flusso, il codice viene successivamente riscattato per un [token di accesso](#access-token).
...
Classe di [applicazione client](#client-application) registrata in Azure AD e progettata per consentire l'accesso e il [consenso](#consent) da account utente di cui è stato effettuato il provisioning in qualsiasi [tenant](#tenant) di Azure AD, anche se diverso da quello in cui il client è stato originariamente registrato. Un'applicazione registrata come a tenant singolo, invece, consentirà accessi solo da account utente con provisioning nello stesso tenant in cui l'applicazione è stata registrata. Le applicazioni [client native](#native-client) sono multi-tenant per impostazione predefinita, mentre le applicazioni [client Web](#web-client) possono scegliere tra tenant singolo e multi-tenant.
...
- Autorizzazioni "delegate", che richiedono l'accesso [in base all'ambito](#scopes) con autorizzazione delegata dal [proprietario delle risorse](#resource-owner) connesso e si manifestano in fase di esecuzione sotto forma di [attestazioni "scp"](#claim) nel [token di accesso](#access-token) del client.
- Autorizzazioni "applicazione", che richiedono l'accesso [in base al ruolo](#roles) con credenziali/identità dell'applicazione client e si manifestano in fase di esecuzione sotto forma di [attestazioni "roles"](#claim) nel token di accesso del client.
...
I ruoli sono stringhe definite a livello di risorsa (ad esempio, "Responsabile approvazione spese" o "Sola lettura"), vengono gestiti nel [portale di Azure classico][AZURE-classic-portal] tramite il [manifesto dell'applicazione](#application-manifest) della risorsa e vengono archiviati nella [proprietà appRoles][AAD-Graph-Sp-Entity] della risorsa. Un'applicazione client richiede l'[autorizzazione](#permissions) ad accedere a un ruolo "applicazione", che verrà presentata alla risorsa in fase di esecuzione nell'[attestazione "roles"](#claim) del [token di accesso](#access-token) del client. Gli amministratori di Azure AD possono assegnare utenti ai ruoli "utente" tramite il [portale di Azure classico][AZURE-classic-portal].

Per una descrizione dettagliata dei ruoli applicazione esposti dall'API Graph di Azure AD, vedere [Permission scopes | Graph API concepts][AAD-Graph-Perm-Scopes] \(Ambiti di autorizzazione | Concetti sull'API Graph). Per un esempio dettagliato di implementazione, vedere [Role based access control in cloud applications using Azure AD][Duyshant-Role-Blog] \(Controllo degli accessi in base al ruolo nelle applicazioni cloud con Azure AD).
...
Gli ambiti sono stringhe definite a livello di risorsa (ad esempio, "Mail.Read" o "Directory.ReadWrite.All"), vengono gestiti nel [portale di Azure classico][AZURE-classic-portal] tramite il [manifesto dell'applicazione](#application-manifest) della risorsa e vengono archiviati nella [proprietà oauth2Permissions][AAD-Graph-Sp-Entity] della risorsa. Un'applicazione client richiede l'[autorizzazione](#permissions) ad accedere, che verrà presentata alla risorsa in fase di esecuzione nell'[attestazione "scp"](#claim) del [token di accesso](#access-token) del client.
...
## tenant
Un'istanza di una directory di Azure AD è definita tenant di Azure AD. Offre un'ampia gamma di funzionalità, tra cui un servizio Registro per le applicazioni integrate, l'autenticazione degli account utente e delle applicazioni registrate e gli endpoint REST necessari per supportare vari protocolli come OAuth2 e SAML. Gli endpoint includono l'[endpoint di autorizzazione](#authorization-endpoint), l'[endpoint di token](#token-endpoint) e l'endpoint "comune" usato dalle [applicazioni multi-tenant](#multi-tenant-application).

Per informazioni dettagliate sui vari modi in cui è possibile ottenere l'accesso a un tenant, vedere [Come ottenere un tenant di Azure Active Directory][AAD-How-To-Tenant].

## endpoint di token
Uno degli endpoint implementati dal [server di autorizzazione](#authorization-server) per supportare le [concessioni di autorizzazione](#authorization-grant) OAuth2. A seconda della concessione, può essere usato per fornire un [token di accesso](#access-token) (e potenzialmente un token "di aggiornamento") a un [client](#client-application), nonché un [token ID](#ID-token) quando la concessione viene usata insieme al protocollo [OpenID Connect][OpenIDConnect].
...
## entità utente
Analogamente a un oggetto entità servizio che viene usato per rappresentare un'istanza dell'applicazione, un oggetto entità utente è un altro tipo di entità di sicurezza che rappresenta un utente. L'[entità User][AAD-Graph-User-Entity] di Azure AD Graph definisce lo schema di un oggetto entità utente. L'entità User è costituita da proprietà relative all'utente come nome e cognome, nome dell'entità utente, appartenenza ai ruoli della directory e così via e fornisce la configurazione dell'identità utente necessaria ad Azure AD per definire un'entità utente in fase di esecuzione. L'entità utente viene usata per rappresentare un utente autenticato durante la registrazione della delega del [consenso](#consent), le decisioni di controllo di accesso e così via.

## client Web
Tipo di [applicazione client](#client-application) che esegue tutto il codice su un server Web ed è quindi considerata un client "riservato" perché può eseguire l'archiviazione privata/riservata delle credenziali sul server. Per altri dettagli, vedere [i profili e i tipi di client di OAuth2][OAuth2-Client-Types].
