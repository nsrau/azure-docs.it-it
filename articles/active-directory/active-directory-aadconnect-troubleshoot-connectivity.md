...
Azure AD Connect si basa su due metodi di configurazione diversi per connettersi ad Azure AD. L'Installazione guidata e lo stesso motore di sincronizzazione richiedono machine.config per una corretta configurazione, essendo applicazioni .NET. Esiste anche una dipendenza dall'Assistente per l'accesso che richiede la configurazione di winhttp per funzionare correttamente.
...
> [AZURE.NOTE]In alcuni blog è invece documentato che le modifiche devono essere apportare a miiserver.exe.config. Questo file viene però sovrascritto a ogni aggiornamento, quindi anche se funziona durante l'installazione iniziale, il sistema smetterà di funzionare dopo il primo aggiornamento. Per questo motivo è consigliabile aggiornare invece machine.config.

Quindi occorre assicurarsi che winhttp sia configurato. A questo scopo, è possibile usare [**netsh**](active-directory-aadconnect-prerequisites.md#connectivity). ![netsh](./media/active-directory-aadconnect-troubleshoot-connectivity/netsh.png)
...
- È una password temporanea e deve essere modificata? È effettivamente la password corretta? Provare ad accedere a https://login.microsoftonline.com da un server diverso dal server Azure AD Connect e verificare che l'account sia utilizzabile.
- Per l'utente è abilitata MFA (Multi-Factor Authentication? In questo caso, disabilitarla.
