...
**D: è possibile spostare il database Azure AD Connect dal database locale a un'istanza di SQL Server remota?**   
...
Il modo più semplice per eseguire questa operazione consiste nell'usare SQL Server Management Studio, installato nello stesso computer in cui è installato Azure AD Connect. Connettersi a *(local DB) .\ADSync*, quindi eseguire il backup del database AdSync.
...
Attualmente, non è supportata la modifica degli attributi HTML del campo di input della **password** , incluso il tag di completamento automatico. È attualmente in fase di elaborazione una funzionalità che consente la creazione di codice JavaScript personalizzato che consenta di aggiungere qualsiasi attributo al campo **Password**.
...
Per verificare quale versione di Azure ad Connect è installata nel server, passare al pannello di controllo e cercare la versione installata di Microsoft Azure ad Connetti selezionando **programmi** > **e funzionalità**, come illustrato di seguito:
