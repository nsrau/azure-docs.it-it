...
**D: Con quali metodi di accesso funzionano Seamless SSO**
...
**D: Seamless SSO è una funzione gratuita?**
...
**D: L'sSO senza problemi è disponibile nel [cloud di Microsoft Azure Germania](https://www.microsoft.de/cloud-deutschland) e nel cloud microsoft Azure per enti [pubblici?](https://azure.microsoft.com/features/gov/)**

No. L'accesso Single Sign-On facile è disponibile solo nell'istanza di Azure AD a livello mondiale.

**D: Quali applicazioni `domain_hint` sfruttano o `login_hint` parametrano la capacità di SSO Seamless?**
...
| Portali di Office 365 | https:\//portal.office.com?domain_hint'contoso.com,\/https: /www.office.com?domain_hint |

Inoltre, gli utenti ottengono un'esperienza di accesso invisibile all'utente se un'applicazione invia richieste di\/accesso agli endpoint\/di Azure AD configurate come tenant, ovvero https: /login.microsoftonline.com/contoso.com/<..> o https: /login.microsoftonline.com/<tenant_ID>/<. >, anziché l'endpoint comune di Azure\/AD, ovvero https: /login.microsoftonline.com/common/<...>. Di seguito viene fornito un elenco, non completo, di applicazioni che generano questi tipi di richieste di accesso.
...
**D: Seamless SSO `Alternate ID` supporta il nome `userPrincipalName`utente, anziché ?**
...
**D: Qual è la differenza tra l'esperienza Single Sign-On fornita dall'aggiunta ad [Azure AD](../active-directory-azureadjoin-overview.md) e l'Accesso SSO semplice?**
...
**D: Si desidera registrare i dispositivi non Windows 10 con Azure AD, senza usare ADFS. Posso usare Seamless SSO invece?**
...
**D: Come è possibile eseguire il rollover `AZUREADSSOACC` della chiave di decrittografia Kerberos dell'account computer?**
...
Seguire questa procedura nel server locale in cui si esegue Azure AD Connect:
   ...
   **Passo 1. Ottenere l'elenco delle foreste di Active Directory in cui è stato abilitato SSO senza soluzione di continuitàGet list of AD forests where Seamless SSO has been enabled**
   ...
   **Passo 2. Aggiornare la chiave di decrittografia Kerberos in ogni foresta di Active Directory in cui è stata configurata**
   ...
   2. Chiamare `Update-AzureADSSOForest -OnPremCredentials $creds`. Questo comando aggiorna la chiave di decrittografia di Kerberos per l'account computer `AZUREADSSOACC` in questa foresta di AD specifica e la aggiorna in Azure AD.
...
**D: Come posso disabilitare Seamless SSO?**

   **Passo 1. Disabilitare la funzionalità nel tenant**
   ...
   **Passo 2. Ottenere l'elenco delle foreste di Active Directory in cui è stato abilitato SSO senza soluzione di continuitàGet list of AD forests where Seamless SSO has been enabled**
   ...
   **Passo 3. Eliminare manualmente `AZUREADSSOACCT` l'account computer da ogni foresta di Active Directory visualizzata nell'elenco.**

## <a name="next-steps"></a>Passaggi successivi

- [**Guida introduttiva**](how-to-connect-sso-quick-start.md) - Iniziare e eseguire Azure AD Seamless SSO.
