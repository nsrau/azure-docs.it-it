...
**D: Gestisco più directory di Azure AD. Come si passa a quello con Azure Active Directory Premium?**
...
Ulteriori informazioni sulla [configurazione del proxy HTTP per gli agenti di integrità](how-to-connect-health-agent-install.md#configure-azure-ad-connect-health-agents-to-use-http-proxy).
...
1. Aprire Internet Explorer > **Impostazioni Internet** > **Options** > **Impostazioni** > **LAN Impostazioni**.
2. Selezionare **Usa un server proxy per la LAN**.
3. Selezionare Avanzate se si dispone di porte proxy diverse per HTTP e HTTPS/Secure.Select **Advanced** if you have different proxy ports for HTTP and HTTPS/Secure.
...
**D: Perché nel portale di Azure AD Connect Health vengono visualizzati due server con lo stesso nome?**
...
* L'agente non può comunicare con gli endpoint necessari perché un firewall blocca il traffico. Questa situazione è molto comune nei server proxy applicazione Web. Assicurarsi di aver abilitato la comunicazione in uscita per le porte e gli endpoint obbligatori. Vedere la [sezione requisiti](how-to-connect-health-agent-install.md#requirements) per i dettagli.
...
**D: Viene visualizzato un avviso che indica che i dati del servizio sanitario non sono aggiornati. Come posso risolvere il problema?**

Azure AD Connect Health genera l'avviso quando non riceve tutti i punti dati dal server nelle ultime due ore. [Per saperne](how-to-connect-health-data-freshness.md)di più .
...
**D: Viene visualizzato un avviso che indica che la richiesta di autenticazione di prova (transazione sintetica) non è riuscita a ottenere un token." Come posso risolvere il problema?**
...
**D: Ho ricevuto un'e-mail che indica che le mie macchine NON sono patchati per i recenti attacchi ransomware. Perché ho ricevuto questa e-mail?**
...
* [Operazioni di Azure AD Connect HealthAzure AD Connect Health operations](how-to-connect-health-operations.md)
