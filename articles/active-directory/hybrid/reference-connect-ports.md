...
|WinRM  | 5985 (TCP) |Usato solo se si sta installando AD FS con gMSA by Azure AD Connect procedura guidata|
|Servizi Web di servizi di dominio Active Directory | 9389 (TCP) |Usato solo se si sta installando AD FS con gMSA by Azure AD Connect procedura guidata |
...
| Protocollo | Porte | Descrizione |
| --- | --- | --- |
| HTTP |80 (TCP) |Usato per il download di CRL (Certificate Revocation List) per verificare i certificati SSL. |
| HTTPS |443 (TCP) |Usato per la sincronizzazione con Azure AD. |

Per un elenco degli URL e degli indirizzi IP che è necessario aprire nel firewall, vedere [URL e intervalli di indirizzi IP per Office 365](https://support.office.com/article/Office-365-URLs-and-IP-address-ranges-8548a211-3fe7-47cb-abb1-355ea5aa88a2) e [risoluzione dei problemi Azure ad Connect connettività](tshoot-connect-connectivity.md#troubleshoot-connectivity-issues-in-the-installation-wizard).
...
|Protocollo|Numero porta|Descrizione
| --- | --- | ---
...
| Protocollo | Porte | Descrizione |
| --- | --- | --- |
| HTTPS |443 (TCP) |Inserimento in |
| Bus di servizio di Azure |5671 (TCP) |Inserimento in |
