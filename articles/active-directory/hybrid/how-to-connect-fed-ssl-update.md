---
title: Azure AD Connect - Aggiornare il certificato TLS/SSL per una farm AD FS. Documenti Microsoft
description: Questo documento descrive in dettaglio i passaggi per aggiornare il certificato TLS/SSL di una farm ADFS tramite Azure AD Connect.This document details the steps to update the TLS/SSL certificate of an AD FS farm by using Azure AD Connect.
...
ms.locfileid: "80331747"
---
# <a name="update-the-tlsssl-certificate-for-an-active-directory-federation-services-ad-fs-farm"></a>Aggiornare il certificato TLS/SSL per una farm di Active Directory Federation Services (ADFS)

## <a name="overview"></a>Panoramica
In questo articolo viene descritto come utilizzare Azure AD Connect per aggiornare il certificato TLS/SSL per una farm di Active Directory Federation Services (ADFS). È possibile usare lo strumento Azure AD Connect per aggiornare facilmente il certificato TLS/SSL per la farm ADFS anche se il metodo di accesso utente selezionato non è ADFS.

È possibile eseguire l'intera operazione di aggiornamento del certificato TLS/SSL per la farm ADFS in tutti i server federativi e Web Application Proxy (WAP) in tre semplici passaggi:
...
![Attività di aggiornamento TLS](./media/how-to-connect-fed-ssl-update/updatessltask.png)
...
## <a name="step-2-provide-a-new-tlsssl-certificate"></a>Passaggio 2: Fornire un nuovo certificato TLS/SSLStep 2: Provide a new TLS/SSL certificate

Dopo aver confermato le informazioni sui server della farm ADFS, Azure AD Connect richiede il nuovo certificato TLS/SSL. Fornire un certificato PFX protetto da password per continuare l'installazione.
...
Nel passaggio successivo selezionare i server per i server che devono avere aggiornato il certificato TLS/SSL. I server offline non possono essere selezionati per l'aggiornamento.
...
* **Quale deve essere il nome del soggetto del certificato per il nuovo certificato TLS/SSL di ADFS?**
...
* **Il server viene visualizzato come non in linea. Cosa dovrei fare?**
...
* **È possibile aggiornare un sottoinsieme dei server della farm con il nuovo certificato TLS/SSL?**
...
* **Ho rimosso il server nell'esecuzione precedente, ma è ancora visualizzato come non in linea ed elencato nella pagina Server ADFS. Perché il server offline è ancora presente anche dopo averlo rimosso?**
