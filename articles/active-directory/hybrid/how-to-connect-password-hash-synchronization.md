...
7. L'agente di sincronizzazione hash password accetta l'hash a 32 byte risultante, concatena sia il salt per utente che il numero di iterazioni SHA256 (per l'utilizzo da parte di Azure AD), quindi trasmette la stringa da Azure AD Connect ad Azure AD tramite TLS.</br> 
...
Se sono presenti utenti sincronizzati che interagiscono solo con i servizi integrati di Azure AD e devono rispettare anche i criteri di scadenza delle password, è possibile forzarli a rispettare i criteri di scadenza delle password di Azure AD abilitando la funzionalità *EnforceCloudPasswordPolicyForPasswordSyncedUsers.If* there are synchronized users that only interact with Azure AD integrated services and must also comply with a password expiration policy, you can force them to comply with your Azure AD password expiration policy by enabling the EnforceCloudPasswordPolicyForPasswordSyncedUsers feature.

Quando *EnforceCloudPasswordPolicyForPasswordSyncedUsers* è disabilitato (impostazione predefinita), Azure AD Connect imposta l'attributo PasswordPolicies degli utenti sincronizzati su "DisablePasswordExpiration". Questa operazione viene eseguita ogni volta che viene sincronizzata la password di un utente e indica ad Azure AD di ignorare i criteri di scadenza delle password cloud per tale utente. È possibile controllare il valore dell'attributo usando il modulo PowerShell di Azure AD con il comando seguente:

`(Get-AzureADUser -objectID <User Object ID>).passwordpolicies`


Per abilitare la funzionalità EnforceCloudPasswordPolicyForPasswordSyncedUsers, eseguire il comando seguente utilizzando il modulo MSOnline PowerShell come illustrato di seguito. È necessario digitare yes per il parametro Enable come illustrato di seguito:
...
Una volta abilitato, Azure AD non passa a `DisablePasswordExpiration` ogni utente sincronizzato per rimuovere il valore dall'attributo PasswordPolicies.Once enabled, Azure AD does not go to each synchronized user to remove the value from the PasswordPolicies attribute. Al contrario, il `None` valore viene impostato su durante la sincronizzazione della password successiva per ogni utente quando la modifica della password in Active Directory locale.  

Si consiglia di abilitare EnforceCloudPasswordPolicyForPasswordSyncedUsers, prima di abilitare la sincronizzazione dell'hash `DisablePasswordExpiration` delle password, in modo che la sincronizzazione iniziale degli hash delle password non aggiusquestola il valore all'attributo PasswordPolicies per gli utenti.

Il criterio password predefinito di Azure AD richiede agli utenti di modificare le password ogni 90 giorni. Se anche i criteri in Active Directory sono 90 giorni, i due criteri devono corrispondere. Tuttavia, se i criteri di Active Directory non sono 90 giorni, è possibile aggiornare i criteri password di Azure AD in modo che corrispondano usando il comando PowerShell Set-MsolPasswordPolicy.However, if the AD policy is not 90 days, you can update the Azure AD password policy to match by using the Set-MsolPasswordPolicy PowerShell command.

Azure AD supporta criteri di scadenza delle password separati per ogni dominio registrato.

Avvertenza: se sono presenti account sincronizzati che devono avere password non in scadenza `DisablePasswordExpiration` in Azure AD, è necessario aggiungere in modo esplicito il valore all'attributo PasswordPolicies dell'oggetto utente in Azure AD.  A tale scopo, eseguire il comando seguente.

`Set-AzureADUser -ObjectID <User Object ID> -PasswordPolicies "DisablePasswordExpiration"`

> [!NOTE]
> Questa funzionalità è in anteprima pubblica in questo momento.
> Il comando PowerShell Set-MsolPasswordPolicy non funzionerà sui domini federati. 

#### <a name="public-preview-of-synchronizing-temporary-passwords-and-force-password-change-on-next-logon"></a>Anteprima pubblica della sincronizzazione delle password temporanee e "Forza modifica password all'accesso successivo"

È tipico forzare un utente a modificare la password durante il primo accesso, soprattutto dopo la reimpostazione della password di un amministratore.  È comunemente noto come l'impostazione di una password "temporanea" e viene completato controllando il flag "La password deve cambiare la password all'accesso successivo" su un oggetto utente in Active Directory (AD).
  
La funzionalità della password temporanea consente di garantire che il trasferimento della proprietà della credenziale venga completato al primo utilizzo, per ridurre al minimo il periodo di tempo in cui più persone sono a conoscenza di tale credenziale.

Per supportare le password temporanee in Azure AD per gli utenti sincronizzati, è possibile abilitare la funzionalità *ForcePasswordChangeOnLogOn,* eseguendo il comando seguente nel server Azure AD Connect:To support temporary passwords in Azure AD for synchronized users, you can enable the ForcePasswordChangeOnLogOn feature, by running the following command on your Azure AD Connect server:

`Set-ADSyncAADCompanyFeature  -ForcePasswordChangeOnLogOn $true`

> [!NOTE]
> Forzare un utente a cambiare la password all'accesso successivo richiede una modifica della password contemporaneamente.  Azure AD Connect non verrà scelto il flag di modifica forzata della password da solo. è complementare alla modifica della password rilevata che si verifica durante la sincronizzazione dell'hash della password.

> [!CAUTION]
> È consigliabile utilizzare questa funzionalità solo quando SSPR e Writeback password sono abilitati nel tenant.  In questo modo, se un utente modifica la password tramite SSPR, verrà sincronizzata con Active Directory.

> [!NOTE]
> Questa funzione è in anteprima pubblica in questo momento.
...
Se si usa Servizi di dominio Azure AD per fornire l'autenticazione legacy per le applicazioni e i servizi che devono usare Kerberos, LDAP o NTLM, alcuni processi aggiuntivi fanno parte del flusso di sincronizzazione dell'hash delle password. Azure AD Connect uses the additional following process to synchronize password hashes to Azure AD for use in Azure AD Domain Services:
...
>
> Azure AD Connect sincronizza solo gli assi delle password legacy quando si abilita Servizi di dominio Active Directory di Azure per il tenant di Azure AD. I passaggi seguenti non vengono usati se si usa Azure AD Connect solo per sincronizzare un ambiente DiS di Servizi di dominio Active Directory locale con Azure AD.
>
> Se le applicazioni legacy non usano l'autenticazione NTLM o i binding semplici LDAP, è consigliabile disabilitare la sincronizzazione dell'hash delle password NTLM per Servizi di dominio Azure AD. Per ulteriori informazioni, vedere Disabilitare le suite di [crittografia deboli e la sincronizzazione dell'hash delle credenziali NTLM.](../../active-directory-domain-services/secure-your-domain.md)

1. Azure AD Connect recupera la chiave pubblica per l'istanza del tenant di Servizi di dominio Azure AD.
1. Quando un utente modifica la password, il controller di dominio locale archivia il risultato della modifica della password (hashe) in due attributi:
    * *unicodePwd* per l'hash della password NTLM.
    * *supplementalCredentials* per l'hash della password Kerberos.
1. Azure AD Connect rileva le modifiche delle password tramite il canale di replica della directory (le modifiche agli attributi che devono essere replicate in altri controller di dominio).
1. Per ogni utente la cui password è stata modificata, Azure AD Connect esegue i passaggi seguenti:For each user whose password has changed, Azure AD Connect performs the following steps:
    * Genera una chiave simmetrica a 256 bit AES casuale.
    * Genera un vettore di inizializzazione casuale necessario per il primo ciclo di crittografia.
    * Estrae gli ishe delle password Kerberos dagli attributi *supplementalCredentials.*
    * Controlla l'impostazione *SyncNtlmPasswords* della configurazione di sicurezza di Servizi di dominio Azure AD.
        * Se questa impostazione è disabilitata, genera un hash NTLM casuale ad alta entropia (diverso dalla password dell'utente). Questo hash viene quindi combinato con gli esordi delle password Kerberos dall'attributo *supplementareCrendetials* in un'unica struttura di dati.
        * Se abilitata, combina il valore dell'attributo *unicodePwd* con gli isemi della password Kerberos estratti dall'attributo *supplementalCredentials* in un'unica struttura di dati.
    * Crittografa la singola struttura di dati utilizzando la chiave simmetrica AES.
    * Crittografa la chiave simmetrica AES usando la chiave pubblica di Servizi di dominio Azure AD del tenant.
1. Azure AD Connect trasmette la chiave simmetrica AES crittografata, la struttura di dati crittografati contenente gli hash delle password e il vettore di inizializzazione in Azure AD.
...
1. Azure AD esegue il push della chiave simmetrica AES crittografata, della struttura dei dati crittografati e del vettore di inizializzazione usando un meccanismo di sincronizzazione interno tramite una sessione HTTP crittografata in Servizi di dominio Azure AD.
1. Servizi di dominio Azure AD recupera la chiave privata per l'istanza del tenant dall'insieme di credenziali delle chiavi di Azure.Azure AD Domain Services retrieves the private key for the tenant's instance from Azure Key vault.
1. Per ogni set di dati crittografato (che rappresenta la modifica della password di un singolo utente), Servizi di dominio Azure AD esegue quindi i passaggi seguenti:For each encrypted set of data (representing a single user's password change), then performs the following steps:
    * Utilizza la chiave privata per decrittografare la chiave simmetrica AES.
    * Utilizza la chiave simmetrica AES con il vettore di inizializzazione per decrittografare la struttura di dati crittografati che contiene gli hash delle password.
    * Scrive gli hashe delle password Kerberos ricevuti nel controller di dominio di Servizi di dominio Azure AD. Gli hash vengono salvati nell'attributo *supplementalCredentials* dell'oggetto utente crittografato nella chiave pubblica del controller di dominio di Servizi di dominio Azure AD.
    * Servizi di dominio Azure AD scrive l'hash della password NTLM ricevuto nel controller di dominio di Servizi di dominio Azure AD. L'hash viene salvato nell'attributo *unicodePwd* dell'oggetto utente crittografato nella chiave pubblica del controller di dominio di Servizi di dominio Azure AD.
