...
ms.locfileid: "91578284"
---
# <a name="custom-installation-of-azure-ad-connect"></a>Installazione personalizzata di Azure AD Connect
Azure AD Connect **impostazioni personalizzate** vengono utilizzate quando si desiderano più opzioni per l'installazione.  Ad esempio, se si dispone di più foreste o se si desidera configurare le funzionalità facoltative. Viene usato in tutti i casi in cui l'opzione di [**installazione rapida**](how-to-connect-install-express.md) non soddisfa la distribuzione o la topologia.
...
In questa pagina fare clic su **Personalizza** per avviare un'installazione di Impostazioni personalizzate.  Nella parte restante di questo documento vengono illustrate le varie schermate della procedura guidata per l'installazione personalizzata.  È possibile utilizzare i collegamenti seguenti per passare rapidamente alle informazioni relative a una schermata specifica della procedura guidata.

- [Installare i componenti necessari](#install-required-components)
- [Accesso utente](#user-sign-in)
- [Connessione ad Azure AD](#connect-to-azure-ad)
- [Pagine nella sezione Sincronizzazione](#pages-under-the-sync-section)

### <a name="install-required-components"></a>Installare i componenti necessari
Durante l'installazione dei servizi di sincronizzazione è possibile lasciare deselezionata l'opzione di configurazione facoltativa perché Azure AD Connect esegua la configurazione in modo automatico. Viene configurata un'istanza di SQL Server database locale Express 2012, vengono creati i gruppi appropriati e vengono assegnate le autorizzazioni. Se si desidera modificare le impostazioni predefinite, è possibile utilizzare questa opzione selezionando le caselle appropriate.  La tabella seguente fornisce un riepilogo di queste opzioni e i collegamenti a informazioni aggiuntive. 
...
|Importa impostazioni di sincronizzazione (anteprima)|Consente di importare le impostazioni da un'altra versione di Azure AD Connect.  Per altre informazioni [, vedere Importazione ed esportazione di Azure ad Connect impostazioni di configurazione](how-to-connect-import-export-config.md).|
...
| --- | --- |
| Sincronizzazione dell'hash delle password |Gli utenti possono accedere ai servizi cloud Microsoft, ad esempio Microsoft 365, usando la stessa password usata nella rete locale. Le password degli utenti vengono sincronizzate in Azure AD come hash della password e l'autenticazione viene eseguita sul cloud. Per altre informazioni, vedere [Sincronizzazione dell’hash delle password](how-to-connect-password-hash-synchronization.md) . |
|Autenticazione pass-through|Gli utenti possono accedere ai servizi cloud Microsoft, ad esempio Microsoft 365, usando la stessa password usata nella rete locale.  La password degli utenti viene passata al controller di dominio di Active Directory locale per la convalida.
| Federazione con ADFS |Gli utenti possono accedere ai servizi cloud Microsoft, ad esempio Microsoft 365, usando la stessa password usata nella rete locale.  Gli utenti vengono reindirizzati alla rispettiva istanza locale di AD FS per l'accesso e l'autenticazione viene eseguita in locale. |
| Federazione con PingFederate|Gli utenti possono accedere ai servizi cloud Microsoft, ad esempio Microsoft 365, usando la stessa password usata nella rete locale.  Gli utenti vengono reindirizzati all'istanza locale di PingFederate per l'accesso e l'autenticazione viene eseguita in locale. |
...
#### <a name="enterprise-admin-and-domain-admin-accounts-not-supported"></a>Account Amministratore aziendale e Amministratore di dominio non supportati
A partire dalla versione 1.4.18.0 non è più supportato l'uso di un account di Amministratore aziendale o di Amministratore di dominio come account di AD DS Connector.  Se si tenta di immettere un account amministratore aziendale o di dominio quando si specifica **usa account esistente**, verrà visualizzato l'errore seguente:

  **"L'uso di un account di tipo Amministratore dell'organizzazione o Amministratore di dominio per l'account della foresta di AD non è consentito.  È possibile consentire ad Azure AD Connect di creare automaticamente l'account o specificare un account di sincronizzazione con le autorizzazioni corrette.  &lt;Ulteriori informazioni&gt;"**
...
| Attributo specifico |Questa opzione consente di selezionare un attributo personale. Se si sceglie questa opzione, gli oggetti User il cui attributo (selected) non è popolato non verranno sincronizzati con Azure AD. **Limitazione:** Per questa opzione sono disponibili solo gli attributi che possono essere già presenti nel metaverse. " |
...
>
>Per altre informazioni, sul servizio di controllo di accesso di Azure vedere [Procedura: eseguire la migrazione dal Servizio di controllo di accesso di Azure](../azuread-dev/active-directory-acs-migration.md)
>
>Fare clic [qui](https://www.microsoft.com/download/details.aspx?id=47594) per scaricare la versione più recente di Azure AD Connect.
 ...
 ![Funzionalità facoltative](./media/how-to-connect-install-custom/optional2a.png)
...
| --- | --- |
| Distribuzione ibrida di Exchange |La funzionalità di distribuzione ibrida di Exchange consente la coesistenza di cassette postali di Exchange sia in locale che in Microsoft 365. Azure AD Connect sincronizza un set specifico di [attributi](reference-connect-sync-attributes-synchronized.md#exchange-hybrid-writeback) da Azure AD alla directory locale. |
...
| writeback delle password |Se si abilita il writeback delle password, le modifiche delle password generate da Azure AD vengono riscritte nella directory locale. Per altre informazioni, vedere [Introduzione alla gestione delle password](../authentication/tutorial-enable-sspr.md). |
| Writeback dei gruppi |Se si usa la funzionalità **gruppi di Microsoft 365** , è possibile che questi gruppi siano rappresentati nell'Active Directory locale. Questa opzione è disponibile solo se si dispone di Exchange in Active Directory locale. Per ulteriori informazioni, vedere [Azure ad Connect writeback del gruppo](how-to-connect-group-writeback.md)|
...
Per assicurarsi che il client acceda automaticamente all'area Intranet, è necessario verificare che l'URL faccia parte dell'area Intranet. In questo modo, il computer aggiunto al dominio invierà automaticamente un ticket Kerberos ad Azure AD quando è connesso alla rete aziendale.
...
4.  Abilitare i criteri e immettere il nome del valore `https://autologon.microsoftazuread-sso.com` e il valore di `1` nella finestra di dialogo.
...
>[!NOTE]
>È possibile aggiornare il certificato TLS/SSL della farm AD FS usando Azure AD Connect anche se non si usa la soluzione per gestire la relazione di trust federativa.
...
È possibile usare una farm ADFS esistente oppure scegliere di creare una nuova farm ADFS. Se si sceglie di crearne una nuova, è necessario fornire il certificato TLS/SSL. Se il certificato TLS/SSL è protetto da password, viene richiesta la password.
...
![Errore](./media/how-to-connect-install-custom/error2.png)
