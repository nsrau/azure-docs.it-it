...
-   Si è deciso di passare a una delle due opzioni seguenti:
    - **Opzione di** - *sincronizzazione dell'hash delle password (sincronizzazione)* + *Single Sign-on seamless (SSO)*
    - **Opzione B** -  + *autenticazione pass-through*facile accesso*SSO*
...
1. Abilitare la *sincronizzazione* dell'hash delle password nella pagina [funzionalità](how-to-connect-install-custom.md#optional-features) facoltative in Azure ad Connect. 
...
Abilitare l'accesso *SSO* facile nelle foreste Active Directory usando PowerShell. Se si dispone di più di una foresta Active Directory, abilitarla singolarmente per ogni foresta.  *Seamless SSO* viene attivato solo per gli utenti selezionati per l'implementazione temporanea. Non influisce sulla configurazione della Federazione esistente.
...
7. Chiamare `Enable-AzureADSSOForest -OnPremCredentials $creds`. Questo comando crea l'account computer AZUREADSSOACC dal controller di dominio locale per la foresta Active Directory necessaria per la funzionalità *seamless SSO*.
...
- **Opzione una** - *sincronizzazione* + dell'hash della password*seamless SSO*
- **Opzione B** -  + *autenticazione pass-through*facile accesso*SSO*
- **Not supported** - *password hash sync* + Autenticazione + *pass-through*della sincronizzazione dell'hash delle password non supportata*SSO semplice*
