---
title: "Azure AD Connect: Eseguire la migrazione dalla federazione all'autenticazione pass-through per Azure AD | Microsoft Docs"
description: Informazioni sulla migrazione di un ambiente ibrido di gestione delle identità dalla federazione all'autenticazione pass-through.
...
>[!NOTE]
>Fare clic [qui](https://aka.ms/ADFSTOPTADPDownload) per scaricare una copia di questo documento.

## <a name="prerequisites-for-pass-through-authentication"></a>Prerequisiti per l'autenticazione pass-through
...
> [!IMPORTANT]
> La documentazione, gli strumenti e i blog meno recenti indicano che la conversione degli utenti è un passaggio obbligatorio per la conversione dei domini da federati a gestiti. La **conversione degli utenti** non è più necessaria e Microsoft sta aggiornando la documentazione e gli strumenti in base a questa modifica.
...
L'autenticazione pass-through viene eseguita tramite la distribuzione di agenti leggeri nel server di Azure AD Connect e nei server Windows in locale. Installare gli agenti il più vicino possibile ai controller di dominio di Active Directory per ridurre la latenza.

Per la maggior parte dei clienti sono sufficienti due o tre agenti di autenticazione per ottenere livelli elevati di capacità e disponibilità e inoltre un tenant non può avere più di 12 agenti registrati. Il primo agente viene sempre installato nel server di AAD Connect. Per comprendere le limitazioni e le opzioni di distribuzione degli agenti, vedere le [informazioni sulle stime del traffico di rete e le linee guida sulle prestazioni](https://docs.microsoft.com/azure/active-directory/connect/active-directory-aadconnect-pass-through-authentication-current-limitations).

### <a name="plan-migration-method"></a>Pianificare il metodo di migrazione

Per eseguire la migrazione dall'autenticazione federata all'autenticazione pass-through e all'accesso Single Sign-On facile sono disponibili due metodi. La scelta del metodo dipende dal modo in cui AD FS è stato originariamente configurato.

- **Uso di Azure AD Connect**. Se AD FS è stato originariamente configurato con Azure AD Connect, il passaggio all'autenticazione pass-through *deve* essere eseguito tramite la procedura guidata di Azure AD Connect.
...
‎  
> [!NOTE]
> Al momento, non è possibile evitare l'annullamento della federazione di tutti i domini nel tenant quando si imposta l'autenticazione pass-through come metodo di accesso utente e AAD Connect è stato originariamente usato per la configurazione automatica di AD FS.  
‎
...
Nella sezione **Accesso utente** verificare che lo stato di **Federazione** sia **Abilitato** e quello di **Accesso Single Sign-On facile** e **Autenticazione pass-through** sia **Disabilitato**. 
...
#### <a name="verify-how-federation-was-configured"></a>Verificare come è stata configurata la federazione
   ...
   3. Nella schermata **Verifica della soluzione** scorrere verso il basso fino a visualizzare **Active Directory Federation Services (AD FS)**.</br>
   ‎Se la configurazione di AD FS è presente in questa sezione, si può tranquillamente presupporre che AD FS sia stato originariamente configurato con Azure AD Connect e quindi la conversione di uno o più domini da federati a gestiti può essere controllata tramite l'opzione **Cambia l'accesso utente** di Azure AD Connect. Questa procedura è descritta nella sezione **Opzione A: Configurazione dell'autenticazione pass-through con Azure AD Connect**.  
‎
   4. Se Active Directory Federation Services non è presente nelle impostazioni correnti, è necessario convertire manualmente i domini da federati a gestiti tramite PowerShell, come descritto nella sezione **Opzione B: Passare dalla federazione all'autenticazione pass-through con Azure AD Connect e PowerShell**.
...
L'attestazione InsideCorporateNetwork non sarà più disponibile dopo che i domini saranno convertiti all'autenticazione pass-through. In alternativa a questa funzionalità è possibile usare [Posizioni specifiche in Azure AD](https://docs.microsoft.com/azure/active-directory/active-directory-named-locations).
...
Per assicurarsi che la funzionalità di aggiunta a un ambiente ibrido continui a funzionare per eventuali nuovi dispositivi aggiunti al dominio dopo che i domini sono stati convertiti all'autenticazione pass-through, Azure AD Connect deve essere configurato in modo da sincronizzare con Azure AD gli account computer di Active Directory per i client Windows 10. Per gli account computer di Windows 7 e Windows 8, la funzionalità di aggiunta a un ambiente ibrido userà l'accesso Single Sign-On facile per registrare il computer in Azure AD e non sarà necessario sincronizzare gli account come per i dispositivi Windows 10. Sarà tuttavia necessario distribuire un file workplacejoin.exe aggiornato (tramite un file con estensione msi) a questi client di livello inferiore in modo che possano registrarsi usando l'accesso Single Sign-On facile. [Scaricare il file con estensione msi](https://www.microsoft.com/download/details.aspx?id=53554).
...
## <a name="planning-for-smart-lockout"></a>Pianificazione per il blocco intelligente

La funzionalità di blocco intelligente di Azure AD protegge dagli attacchi di forza bruta alle password e impedisce che l'account di Active Directory locale venga bloccato quando si usa l'autenticazione pass-through e sono impostati criteri di gruppo per il blocco degli account in Active Directory. 

Per altre informazioni sulla funzionalità di blocco intelligente e su come modificarne la configurazione, vedere [Blocco intelligente di Azure Active Directory](https://docs.microsoft.com/azure/active-directory/connect/active-directory-aadconnect-pass-through-authentication-smart-lockout).
...
Ora che la soluzione è stata pianificata, si è pronti per l'implementazione. L'implementazione include i componenti seguenti:
   ...
   1. Preparazione dell'accesso Single Sign-On facile
   2. Conversione del metodo di accesso all'autenticazione pass-through e abilitazione dell'accesso Single Sign-On facile
...
## <a name="step-2--change-sign-in-method-to-pta-and-enable-seamless-sso"></a>Passaggio 2: Convertire il metodo di accesso all'autenticazione pass-through e abilitare l'accesso Single Sign-On facile

### <a name="option-a-configuring-pta-by-using-azure-ad-connect"></a>Opzione A: Configurazione dell'autenticazione pass-through con Azure AD Connect

Usare questo metodo quando AD FS è stato inizialmente configurato tramite Azure AD Connect. Non è possibile usare questo metodo se AD FS non è stato originariamente configurato con Azure AD Connect.

> [!IMPORTANT]
> Tenere presente che, seguendo questa procedura, si convertiranno tutti i domini da federati a gestiti. Per altre informazioni, vedere la sezione Pianificare il metodo di migrazione.[](#_Plan_Migration_Method)

Prima di tutto, è necessario modificare il metodo di accesso:
   ...
   4. Nella schermata **Accesso utente** modificare l'impostazione dei pulsanti di opzione deselezionando **Federazione tramite AD FS** e selezionando **Autenticazione pass-through**, quindi selezionare **Abilita Single Sign-On** e fare clic su **Avanti**.
...
Nella pagina **Autenticazione pass-through** fare clic sul pulsante Scarica. Nella schermata **Scaricare l'agente** fare clic su **Accetta le condizioni e scarica**.

Verrà avviato il download degli agenti di autenticazione aggiuntivi. Installare l'agente di autenticazione secondario in un server aggiunto al dominio. 

> [!NOTE]
> Il primo agente viene sempre installato nel server di Azure AD Connect come parte delle modifiche di configurazione apportate nella sezione Accesso utente dello strumento Azure AD Connect. Eventuali altri agenti di autenticazione devono essere installati in un server separato. È consigliabile avere 2 o 3 agenti di autenticazione aggiuntivi disponibili. 

Eseguire l'installazione dell'agente di autenticazione. Durante l'installazione sarà necessario specificare le credenziali di un account di **amministratore globale**.
...
Dopo aver installato l'agente di autenticazione, è possibile tornare alla pagina relativa all'integrità dell'agente di autenticazione pass-through per controllare lo stato degli agenti aggiuntivi.


Proseguire con la sezione Test e passaggi successivi.

> [!IMPORTANT]
> Ignorare la sezione Opzione B: Passare dalla federazione all'autenticazione pass-through con Azure AD Connect e PowerShell perché i passaggi descritti in questa sezione non sono applicabili al caso specifico.  

### <a name="option-b---switch-from-federation-to-pta-using-azure-ad-connect-and-powershell"></a>Opzione B: Passare dalla federazione all'autenticazione pass-through con Azure AD Connect e PowerShell

Usare questa opzione quando la federazione non è stata inizialmente configurata tramite Azure AD Connect. Prima di tutto è necessario abilitare l'autenticazione pass-through:

   1. Nel server di Azure AD Connect avviare la procedura guidata.
   2. Selezionare **Cambia l'accesso utente** e quindi fare clic su **Avanti**.
   3. Nella schermata **Connessione ad Azure AD** specificare il nome utente e la password di un amministratore globale.
   4. Nella schermata **Accesso utente** modificare l'impostazione dei pulsanti di opzione deselezionando **Non configurare** e selezionando **Autenticazione pass-through**, quindi selezionare **Abilita Single Sign-On** e fare clic su **Avanti**.
   5. Nella schermata **Abilita Single Sign-on** immettere le credenziali dell'account di amministratore di dominio e quindi fare clic su **Avanti**.
   ...
   * Viene installato il primo agente di autenticazione pass-through.
   * Viene abilitata la funzionalità di autenticazione pass-through.
   * Viene abilitato l'accesso Single Sign-On facile.  
   7. Verificare che lo stato di **Federazione** sia ancora **Abilitato** e che quello di **Accesso Single Sign-on facile** e **Autenticazione pass-through** sia ora Abilitato.
   ...
   Se l'agente di autenticazione non è attivo, seguire [questi passaggi per la risoluzione dei problemi](https://docs.microsoft.com/azure/active-directory/connect/active-directory-aadconnect-troubleshoot-pass-through-authentication) prima di procedere alla conversione dei domini nel passaggio successivo. Se si convertono i domini prima di verificare che gli agenti di autenticazione pass-through siano installati correttamente e che il relativo stato risulti "Attivo" nel portale di Azure, si rischia di causare un'interruzione del servizio di autenticazione.  
   9. Distribuire quindi gli agenti di autenticazione aggiuntivi. Aprire il **portale di Azure**, passare ad **Azure Active Directory**, **Azure AD Connect** e fare clic su **Autenticazione pass-through**.
   10. Nella pagina **Autenticazione pass-through** fare clic sul pulsante **Scarica**. Nella schermata **Scaricare l'agente** fare clic su **Accetta le condizioni e scarica**.
  ...
  > Il primo agente viene sempre installato nel server di Azure AD Connect come parte delle modifiche di configurazione apportate nella sezione Accesso utente dello strumento Azure AD Connect. Eventuali altri agenti di autenticazione devono essere installati in un server separato. È consigliabile avere 2 o 3 agenti di autenticazione aggiuntivi disponibili.
   ...
   11. Eseguire l'installazione dell'agente di autenticazione. Durante l'installazione sarà necessario specificare le credenziali di un account di **amministratore globale**.</br>
   ...
   12. Dopo aver installato l'agente di autenticazione, è possibile tornare alla pagina relativa all'integrità dell'agente di autenticazione pass-through per controllare lo stato degli agenti aggiuntivi.

A questo punto, la federazione è ancora abilitata e operativa per i domini. Per continuare con la distribuzione, è necessario convertire ogni dominio da federato a gestito per consentire alla funzionalità di autenticazione pass-through di iniziare a elaborare le richieste di autenticazione per il dominio.
...
### <a name="test-seamless-single-sign-on"></a>Testare l'accesso Single Sign-On facile
   ...
   2. Aprire **Internet Explorer** o **Chrome** e passare a uno degli URL seguenti:   
      ‎  
      ...
      L'utente verrà rapidamente reindirizzato alla pagina di accesso di Azure AD e vedrà il messaggio "Tentativo di accesso per conto dell'utente". Non dovrà quindi ricevere la richiesta di specificare un nome utente o una password.</br>
   ![Figura 24](media/plan-migrate-adfs-pass-through-authentication/migrating-adfs-to-pta_image30.png)</br>
   3. L'utente verrà quindi reindirizzato al pannello di accesso e l'accesso verrà eseguito automaticamente:
...
Per garantire la disponibilità della soluzione è necessario monitorare i server che eseguono gli agenti di autenticazione. Oltre ai contatori delle prestazioni generali del server, gli agenti di autenticazione espongono oggetti prestazioni che possono essere usati per riconoscere gli errori e le statistiche di autenticazione.

Gli agenti di autenticazione registrano le operazioni nei registri eventi di Windows in "Application and Service Logs\Microsoft\AzureAdConnect\AuthenticationAgent\Admin".

È possibile abilitare i log di risoluzione dei problemi, se necessario.

Per altre informazioni sul monitoraggio e la registrazione, vedere [questo articolo](https://docs.microsoft.com/azure/active-directory/connect/active-directory-aadconnect-troubleshoot-Pass-through-authentication).
