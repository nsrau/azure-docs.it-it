---
title: Domande frequenti sull'accesso condizionale di Azure Active Directory Documenti Microsoft
description: Risposte alle domande frequenti sull'accesso condizionale in Azure Active Directory.Get answers to frequently asked questions about Conditional Access in Azure Active Directory.
...
Per informazioni sulle applicazioni che funzionano con i criteri di accesso condizionale, vedere Applicazioni e browser che usano le regole di [accesso condizionale in Azure Active Directory.](concept-conditional-access-cloud-apps.md)

## <a name="are-conditional-access-policies-enforced-for-b2b-collaboration-and-guest-users"></a>Vengono applicati criteri di accesso condizionale per la collaborazione B2B e gli utenti guest?
...
## <a name="why-cant-i-set-a-policy-directly-on-client-apps-like-word-or-outlook"></a>Perché non è possibile impostare un criterio direttamente nelle app client, ad esempio Word o Outlook?
...
## <a name="does-a-conditional-access-policy-apply-to-service-accounts"></a>Ai criteri di accesso condizionale si applicano agli account di servizio?

I criteri di accesso condizionale si applicano a tutti gli account utente. inclusi gli account utente usati come account del servizio. Spesso, un account di servizio che viene eseguito automaticamente non può soddisfare i requisiti di un criterio di accesso condizionale. È possibile, ad esempio, che sia richiesta l'autenticazione a più fattori. Gli account di servizio possono essere esclusi da un criterio usando le impostazioni di gestione dei criteri di accesso condizionale. 

## <a name="are-microsoft-graph-apis-available-for-configuring-conditional-access-policies"></a>Sono disponibili le API di Microsoft Graph per la configurazione dei criteri di accesso condizionale?
...
Attualmente, i criteri di accesso condizionale vengono applicati in modo selettivo agli utenti di dispositivi iOS e Android.Currently, Conditional Access policies are selectively enforced on users of iOS and Android devices. Le applicazioni su altre piattaforme per dispositivi non sono, per impostazione predefinita, interessate dai criteri di accesso condizionale per i dispositivi iOS e Android.Applications on other device platforms are, by default, not affected by the Conditional Access policy for iOS and Android devices. Un amministratore tenant può decidere di ignorare i criteri globali per disabilitare l'accesso per gli utenti su piattaforme non supportate.
...
- Per configurare i criteri di accesso condizionale per l'ambiente, vedere [Procedure consigliate per l'accesso condizionale in Azure Active Directory](best-practices.md). 
