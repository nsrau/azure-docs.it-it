...
4. Nella casella di ricerca digitare **PurelyHR**.

    ![Creazione di un utente test di Azure AD](./media/active-directory-saas-purelyhr-tutorial/tutorial_purelyhr_search.png)

5. Nel pannello dei risultati selezionare **PurelyHR** e quindi fare clic sul pulsante **Aggiungi** per aggiungere l'applicazione.
...
3. **[Creazione di un utente test di PurelyHR](#creating-a-purelyhr-test-user)**: per avere una controparte di Britta Simon in PurelyHR collegata alla relativa rappresentazione in Azure AD dell'utente.
...
3. Nella sezione **URL e dominio PurelyHR**, se si vuole configurare l'applicazione in modalità avviata da **IDP**, seguire questa procedura:
...
7. Nella sezione **Configurazione di PurelyHR** fare clic su **Configura PurelyHR** per aprire la finestra **Configura accesso**. Copiare l'**ID di entità SAML e l'URL del servizio Single Sign-On SAML** dalla sezione **Riferimento rapido.**

    ![Configure Single Sign-On](./media/active-directory-saas-purelyhr-tutorial/tutorial_purelyhr_configure.png) 

8. Per configurare l'accesso Single Sign-On sul lato **PurelyHR**, accedere al sito aziendale come amministratore.

9. Aprire il **dashboard** dalle opzioni nella barra degli strumenti e fare clic su **SSO Settings** (Impostazioni SSO).

10. Incollare i valori nelle caselle come descritto di seguito.
