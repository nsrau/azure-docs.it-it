...
Per configurare l'integrazione di Spotinst in Azure AD, è necessario aggiungere Spotinst dalla raccolta all'elenco di app SaaS gestite.
...
1. Nell'elenco delle applicazioni selezionare **Spotinst**.
...
### <a name="create-spotinst-test-user"></a>Creare l'utente di test di Spotinst

Questa sezione descrive come creare un utente di nome Britta Simon in Spotinst.
...
2. Se l'applicazione è stata configurata nella modalità avviata da **IDP**, non è necessario alcun intervento dell'utente in questa sezione. Spotinst supporta il provisioning utenti JIT, che è abilitato per impostazione predefinita. Se non esiste già, un nuovo utente viene creato durante un tentativo di accesso a Spotinst.

## <a name="test-sso"></a>Testare l'accesso SSO 

In questa sezione viene testata la configurazione dell'accesso Single Sign-On di Azure AD usando il pannello di accesso.

Quando si fa clic sul riquadro di Spotinst nel pannello di accesso, si dovrebbe accedere automaticamente all'istanza di Spotinst per cui si è configurato l'accesso SSO. Per altre informazioni sul pannello di accesso, vedere [Introduzione al Pannello di accesso](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction).
