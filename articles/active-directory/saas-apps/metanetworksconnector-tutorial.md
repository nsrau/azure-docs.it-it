...
1. Nella casella di ricerca digitare **Meta Networks Connector**, selezionare **Meta Networks Connector** nel riquadro dei risultati e quindi fare clic sul pulsante **Aggiungi** per aggiungere l'applicazione.
...
1. **[Creare un utente di test di Meta Networks Connector](#create-a-meta-networks-connector-test-user)**: per avere una controparte di Britta Simon in Meta Networks Connector collegata alla rappresentazione in Azure AD dell'utente.
...
1. Nella sezione **URL e dominio di Meta Networks Connector** completare i passaggi seguenti se si vuole configurare l'applicazione in modalità avviata da **IDP**:
    ...
    1. Nella casella di testo **Identificatore** digitare l'URL adottando il modello seguente: `https://login.nsof.io/v1/<ORGANIZATION-SHORT-NAME>/saml/metadata`
    
    1. Nella casella di testo **URL di risposta** digitare l'URL usando il modello seguente: `https://login.nsof.io/v1/<ORGANIZATION-SHORT-NAME>/sso/saml`
...
1. Selezionare **Mostra impostazioni URL avanzate** e seguire questa procedura se si vuole configurare l'applicazione in modalità avviata da **SP**:
    ...
    1. Nella casella di testo **URL di accesso** digitare l'URL usando il modello seguente: `https://<ORGANIZATION-SHORT-NAME>.metanetworks.com/login`
    
    1. Nella casella di testo **Stato dell'inoltro** digitare un URL usando il modello seguente: `https://<ORGANIZATION-SHORT-NAME>.metanetworks.com/#/`
...
1. L'applicazione Meta Networks Connector prevede un formato specifico per le asserzioni SAML. Configurare le attestazioni seguenti per questa applicazione. È possibile gestire i valori di questi attributi dalla sezione **Attributi utente** nella pagina di integrazione dell'applicazione. La schermata seguente illustra un esempio relativo a questa operazione.
...
1. Nella sezione **Attributi utente** della finestra di dialogo **Single Sign-On** configurare l'attributo del token SAML come indicato nell'immagine precedente e seguire questa procedura:
    ...
    1. Nella casella di testo **Spazio dei nomi**, digitare il valore dello spazio dei nomi indicato per la riga.
...
1. Nella sezione **Configurazione di Meta Networks Connector** fare clic su **Configura Meta Networks Connector** per aprire la finestra **Configura accesso**. Copiare l'**ID di entità SAML e l'URL del servizio Single Sign-On SAML** dalla sezione **Riferimento rapido.**
...
1. Passare ad **Amministratore** e selezionare **Impostazioni**.
    
    ![Configure Single Sign-On](./media/metanetworksconnector-tutorial/configure3.png)
    
1. Assicurarsi che le opzioni **Log Internet Traffic** (Registra traffico Internet) e **Force VPN MFA** (Forza MFA VPN) siano disattivate.
    
    ![Configure Single Sign-On](./media/metanetworksconnector-tutorial/configure1.png)
    
1. Passare ad **Amministratore** e selezionare **SAML**.
    
    ![Configure Single Sign-On](./media/metanetworksconnector-tutorial/configure4.png)
    
1. Nella pagina **DETTAGLI** completare i passaggi seguenti:
    
    ![Configure Single Sign-On](./media/metanetworksconnector-tutorial/configure2.png)
    
    1. Copiare il valore del campo **SSO URL** (URL di Single Sign-On) nella casella di testo **URL di accesso** della sezione **URL e dominio Meta Networks Connector**.
    
    1. Copiare il valore del campo **Recipient URL** (URL destinatario) nella casella di testo **URL di risposta** della sezione **URL e dominio Meta Networks Connector**.
    
    1. Copiare il valore del campo **Audience URI (SP Entity ID)** (URI destinatario - ID entità provider di servizi) nella casella di testo **Identificatore (ID entità)** della sezione **URL e dominio Meta Networks Connector**.
    
    1. Abilitare SAML
    
1. Nella scheda **GENERALE** completare i passaggi seguenti:

    ![Configure Single Sign-On](./media/metanetworksconnector-tutorial/configure5.png)

    1. Nella casella di testo **URL di accesso Single Sign-On del provider di identità** incollare il valore di **SAML Single Sign-On Service URL** (URL servizio Single Sign-On SAML) copiato dal portale di Azure.

    1. Nella casella di testo **Autorità di certificazione del provider di identità** incollare il valore dell'**ID di entità SAML** copiato dal portale di Azure.

    1. Aprire il certificato scaricato dal portale di Azure nel Blocco note e incollarlo nella casella di testo **Certificato X.509**.

    1. Abilitare il **provisioning JIT (Just-in-Time)**.
...
1. Nell'elenco delle applicazioni selezionare **Meta Networks Connector**.
