...
Per configurare l'integrazione di LogicMonitor in Azure AD, è necessario aggiungere LogicMonitor dalla raccolta all'elenco di app SaaS gestite.
...
4. Nella casella di ricerca digitare **LogicMonitor**, selezionare **LogicMonitor** nel pannello dei risultati e quindi fare clic sul pulsante **Aggiungi** per aggiungere l'applicazione.
     ...
     ![LogicMonitor nell'elenco risultati](common/search-new-app.png)
...
Per configurare e testare l'accesso Single Sign-On di Azure AD con LogicMonitor, è necessario completare le procedure di base seguenti:
...
5. **[Creare l'utente di test di LogicMonitor](#create-logicmonitor-test-user)**: per avere una controparte di Britta Simon in LogicMonitor collegata alla rappresentazione dell'utente in Azure AD.
...
4. Nella sezione **Impostazioni Single Sign-on (SSO)** eseguire questa procedura:
    ...
    c. Aprire il file di metadati scaricato in Blocco note, quindi incollare il contenuto del file nella casella di testo **Identity Provider Metadata** (Metadati del provider di identità).
...
In questa sezione si abilita Britta Simon all'uso dell'accesso Single Sign-On di Azure concedendole l'accesso a LogicMonitor.
