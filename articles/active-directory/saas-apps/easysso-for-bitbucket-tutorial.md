...
1. Fare clic su **Impostare URL aggiuntivi** e seguire questa procedura se si vuole configurare l'applicazione in modalità avviata da **SP**:
    ...
    > [!NOTE]
    > Poiché questi non sono i valori reali, aggiornarli con l'identificatore, l'URL di risposta e l'URL di accesso effettivi. Per ottenere questi valori, contattare il [team di supporto clienti di EasySSO for BitBucket](mailto:support@techtime.co.nz). È anche possibile fare riferimento ai modelli mostrati nella sezione **Configurazione SAML di base** del portale di Azure.
...
1. Accedere all'istanza di EasySSO for BitBucket con privilegi di amministratore e passare alla sezione **Manage Apps** (Gestisci app).
...
1. Selezionare la scheda **Certificates** (Certificati) nella parte superiore per visualizzare la schermata seguente. Individuare il **certificato (Base64)** o il **file dei metadati** salvati nei passaggi precedenti della configurazione dell'**accesso Single Sign-On di Azure AD**. Per procedere, è possibile scegliere tra diverse opzioni:
    ...
    ![URL dei metadati](./media/easysso-for-bitbucket-tutorial/jira-admin-4.png)
...
In questa sezione viene creato un utente di nome B.Simon in EasySSO for BitBucket. EasySSO for BitBucket supporta il provisioning utenti JIT, che è **disabilitato** per impostazione predefinita. Per abilitare il provisioning utenti, è necessario selezionare in modo esplicito l'opzione **Create user on successful login** (Crea utente in caso di accesso riuscito) nella sezione General (Generale) della configurazione del plug-in EasySSO. Se non esiste già un utente in EasySSO for BitBucket, ne viene creato uno nuovo dopo l'autenticazione.

Se, però, non si vuole abilitare il provisioning utenti automatico al primo accesso dell'utente, gli utenti devono esistere già nelle directory utente back-end usate dall'istanza di EasySSO for BitBucket, ad esempio LDAP o Atlassian Crowd.

![Provisioning utenti](./media/easysso-for-bitbucket-tutorial/jira-admin-6.png)

## <a name="test-sso"></a>Testare l'accesso SSO

In questa sezione viene testata la configurazione dell'accesso Single Sign-On di Azure AD usando il pannello di accesso.

Quando si fa clic sul riquadro di EasySSO for BitBucket nel pannello di accesso, si dovrebbe accedere automaticamente all'istanza di EasySSO for BitBucket per cui si è configurato l'accesso SSO. Per altre informazioni sul pannello di accesso, vedere [Introduzione al Pannello di accesso](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction).
