...
4. Nella casella di ricerca digitare **iWellnessNow**, selezionare **iWellnessNow** dal pannello dei risultati quindi fare clic sul pulsante **Aggiungi** per aggiungere l'applicazione.
...
3. **[Creare un utente di test di iWellnessNow](#create-an-iwellnessnow-test-user)**: per avere una controparte di Britta Simon in iWellnessNow collegata alla rappresentazione dell'utente in Azure AD.
...
3. Per configurare l'applicazione in modalità avviata da **IDP** se si dispone del **file di metadati del provider di servizi**, nella sezione **URL e dominio iWellnessNow** seguire questa procedura:
...
4. Per configurare l'applicazione in modalità avviata da **IDP** se non si dispone del **file di metadati del provider di servizi**, seguire questa procedura:
...
5. Selezionare **Mostra impostazioni URL avanzate** e seguire questa procedura se si vuole configurare l'applicazione in modalità avviata da **SP**:
...
7. Per configurare l'accesso Single Sign-On sul lato **iWellnessNow**, è necessario inviare il file **XML metadati** scaricato al [team di supporto di iWellnessNow](mailto:info@iwellnessnow.com). La configurazione viene eseguita in modo che la connessione SSO SAML sia impostata correttamente su entrambi i lati.
...
2. Nell'elenco di applicazioni selezionare **iWellnessNow**.
