...
> * [Single Sign-on](./lucidchart-tutorial.md) per LucidChart (scelta consigliata)
...
3. Determinare quali dati eseguire il [mapping tra Azure ad e LucidChart](../app-provisioning/customize-application-attributes.md). 
...
Aggiungere LucidChart dalla raccolta di applicazioni Azure AD per iniziare a gestire il provisioning in LucidChart. Se in precedenza è stato configurato LucidChart per SSO, è possibile usare la stessa applicazione. È tuttavia consigliabile creare un'app separata per il test iniziale dell'integrazione. Per altre informazioni su come aggiungere un'applicazione dalla raccolta, fare clic [qui](../manage-apps/add-application-portal.md). 
...
9. Esaminare gli attributi utente che vengono sincronizzati da Azure AD a LucidChart nella sezione **mapping degli attributi** . Gli attributi selezionati come proprietà **corrispondenti** vengono usati per trovare le corrispondenze con gli account utente in LucidChart per le operazioni di aggiornamento. Se si sceglie di modificare l' [attributo di destinazione corrispondente](../app-provisioning/customize-application-attributes.md), sarà necessario assicurarsi che l'API LucidChart supporti il filtraggio degli utenti in base a tale attributo. Selezionare il pulsante **Salva** per eseguire il commit delle modifiche.
