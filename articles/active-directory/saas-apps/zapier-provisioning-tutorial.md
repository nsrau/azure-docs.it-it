...
Questa esercitazione descrive i passaggi da eseguire sia in Zapier che in Azure Active Directory (Azure AD) per configurare il provisioning utenti automatico. Se configurato, Azure AD esegue automaticamente il provisioning e il deprovisioning di utenti e gruppi in [Zapier](https://zapier.com/pricing) usando il servizio di provisioning Azure ad. Per informazioni dettagliate sul funzionamento di questo servizio e domande frequenti, vedere [Automatizzare il provisioning e il deprovisioning utenti in applicazioni SaaS con Azure Active Directory](../app-provisioning/user-provisioning.md). 
...
3. Determinare quali dati eseguire il [mapping tra Azure ad e Zapier](../app-provisioning/customize-application-attributes.md). 
...
Aggiungere Zapier dalla raccolta di applicazioni Azure AD per iniziare a gestire il provisioning in Zapier. Se in precedenza è stato configurato Zapier per SSO, è possibile usare la stessa applicazione. È tuttavia consigliabile creare un'app separata per il test iniziale dell'integrazione. Per altre informazioni su come aggiungere un'applicazione dalla raccolta, fare clic [qui](../manage-apps/add-application-portal.md). 
...
5. Nella sezione **credenziali amministratore** immettere l' **URL del tenant** di Zapier e il **token segreto**. Fare clic su **Test connessione** per assicurarsi che Azure ad possa connettersi a Zapier. Se la connessione non riesce, verificare che l'account Zapier disponga delle autorizzazioni di amministratore e riprovare.
   ...
   ![Screenshot mostra la finestra di dialogo credenziali amministratore, in cui è possibile immettere il tenant U R L e il token segreto.](./media/zapier-provisioning-tutorial/provisioning.png)
...
9. Esaminare gli attributi utente che vengono sincronizzati da Azure AD a Zapier nella sezione **mapping degli attributi** . Gli attributi selezionati come proprietà **corrispondenti** vengono usati per trovare le corrispondenze con gli account utente in Zapier per le operazioni di aggiornamento. Se si sceglie di modificare l' [attributo di destinazione corrispondente](../app-provisioning/customize-application-attributes.md), sarà necessario assicurarsi che l'API Zapier supporti il filtraggio degli utenti in base a tale attributo. Selezionare il pulsante **Salva** per eseguire il commit delle modifiche.
