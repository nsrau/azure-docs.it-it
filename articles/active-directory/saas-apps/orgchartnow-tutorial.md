...
3. **[Creare un utente di test di OrgChart Now](#create-an-orgchart-now-test-user)**: per avere una controparte di Britta Simon in OrgChart Now collegata alla rappresentazione dell'utente in Azure AD.
...
3. Nella sezione **URL e dominio OrgChart Now** per configurare l'applicazione in modalità avviata da **IDP**:
...
7. Nella sezione **Configurazione di OrgChart Now** fare clic su **Configura OrgChart Now** per aprire la finestra **Configura accesso**. Copiare l'**ID entità SAML** dalla **sezione Riferimento rapido** e usarlo per completare **URL di accesso** nella sezione **URL e dominio OrgChart Now**.
