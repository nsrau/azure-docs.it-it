...
Spesso nelle organizzazioni può capitare che un nome utente e una password debbano essere usati da più persone. Questa situazione si verifica in genere in due casi:
...
## Aspetti negativi del modello di condivisione tradizionale
...
Azure AD fornisce un nuovo approccio all'uso degli account condivisi che consente di eliminare questi svantaggi. L'amministratore di Azure AD configura le app a cui un utente può accedere tramite il pannello di accesso. Per ogni applicazione, l'amministratore sceglie il tipo di accesso Single Sign-On più adatto all'app. Uno di questi tipi, l'accesso Single Sign-On basato su password, consente ad Azure AD di agire come una sorta di broker durante il processo di accesso a tale app. L'amministratore può facilmente concedere o revocare l'accesso a un'applicazione e può inoltre conoscere, in qualsiasi momento, chi può accedere all'account e chi ha avuto accesso in precedenza.

Gli utenti accedono una sola volta mediante il proprio account dell'organizzazione, lo stesso che usano regolarmente per accedere alle postazioni desktop o alla posta elettronica. Possono individuare e accedere solo alle applicazioni a cui sono assegnati. Con gli account condivisi, questo elenco di applicazioni può includere qualsiasi numero di credenziali condivise. L'utente finale non deve ricordare o annotare i vari account in uso.

Gli account condivisi non solo consentono di aumentare la supervisione e migliorare l'usabilità, ma anche di aumentare la sicurezza. Gli utenti con autorizzazioni per l'uso delle credenziali non visualizzano la password condivisa, ma ottengono autorizzazioni per l'uso della password come parte di un flusso di autenticazione orchestrato. Inoltre, con alcune applicazioni Single Sign-On basate su password, è possibile che Azure AD esegua periodicamente il rollover (aggiornamento) della password, usando password lunghe e complesse, per aumentare la sicurezza dell'account.
...
- Aggiungere un'applicazione alla [raccolta di app](https://azure.microsoft.com/it-IT/marketplace/active-directory/) o integrarla con un'[applicazione personalizzata](http://blogs.technet.com/b/ad/archive/2015/06/17/bring-your-own-app-with-azure-ad-self-service-saml-configuration-gt-now-in-preview.aspx)
...
- Facoltativo: in alcune applicazioni, quali Facebook, Twitter o LinkedIn è possibile abilitare l'opzione per l'[esecuzione automatizzata del rollover delle password di Azure AD](http://blogs.technet.com/b/ad/archive/2015/02/20/azure-ad-automated-password-roll-over-for-facebook-twitter-and-linkedin-now-in-preview.aspx).

Mediante Azure AD è possibile rendere l'account condiviso più sicuro sfruttando l'autenticazione a più fattori (altre informazioni sulla [sicurezza delle applicazioni con Azure AD](multi-factor-authentication-get-started.md)) ed è possibile delegare la possibilità di gestire utenti che hanno accesso all'applicazione mediante la gestione dei gruppi in [modalità self-service di Azure AD](active-directory-accessmanagement-self-service-group-management.md).
