...
Per la maggior parte delle organizzazioni che desiderano semplicemente abilitare l'accesso utente a Office 365, alle applicazioni SaaS e ad altre risorse basate su AD Azure, è consigliabile l'opzione di sincronizzazione password predefinita. Le organizzazioni che, per vari motivi, non possono usare questa opzione possono tuttavia scegliere tra un'opzione di accesso federato, ad esempio AD FS, o l'autenticazione pass-through. La tabella seguente offre gli elementi necessari per effettuare la scelta appropriata. 

Esigenza | PHS e SSO| PTA con SSO| AD FS |
 --- | --- | --- | --- |
Sincronizzare automaticamente nel cloud nuovi account utente, di contatto o di gruppo creati in Active Directory locale|x|x|x|
...
Implementare l'accesso SSO usando le credenziali aziendali|x|x|x|
...
È possibile abilitare l'[accesso Single Sign-On](active-directory-aadconnect-sso.md) anche per gli utenti di computer aggiunti al dominio presenti nella rete aziendale. Con l'accesso Single Sign-On, gli utenti abilitati possono accedere in modo sicuro alle risorse cloud immettendo solo un nome utente.
...
Con l'autenticazione pass-through, la password dell'utente viene convalidata rispetto al controller di Active Directory locale e non è necessario che sia presente in Azure AD in alcuna forma. In questo modo, durante l'autenticazione nei servizi cloud possono essere valutati criteri locali come le restrizioni relative all'ora di accesso. L'autenticazione pass-through usa un agente semplice in un computer aggiunto al dominio di Windows Server 2012 R2 nell'ambiente locale, che resta in attesa di richieste di convalida delle password. L'agente non richiede alcuna porta in ingresso aperta a Internet.
...
[Altre informazioni sull'autenticazione pass-through](active-directory-aadconnect-pass-through-authentication.md) e sull'[accesso Single Sign-On](active-directory-aadconnect-sso.md).
...
[Configurazione di SSO con AD FS](active-directory-aadconnect-get-started-custom.md#configuring-federation-with-ad-fs)
...
> [!NOTE]
> L'uso di un ID alternativo non è compatibile con tutti i carichi di lavoro di Office 365 e con l'autenticazione pass-through. Per altre informazioni, vedere [Configurazione dell'ID di accesso alternativo](https://technet.microsoft.com/library/dn659436.aspx).
> 
> 
