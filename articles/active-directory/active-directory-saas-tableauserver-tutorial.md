<properties
	pageTitle="Esercitazione: Integrazione di Azure Active Directory con Tableau Server | Microsoft Azure"
...
4. **[Creazione di un utente test di Tableau Server](#creating-a-tableauserver-test-user)**: per avere una controparte di Britta Simon in Tableau Server collegata alla relativa rappresentazione in Azure AD.
...
1. Nella finestra di dialogo **Attributi token SAML** seguire questa procedura:
...
2. Nella pagina **Stabilire come si desidera che gli utenti accedano a Tableau Server** selezionare **Single Sign-On di Azure AD** e quindi fare clic su **Avanti**.

	![Configura accesso Single Sign-On](./media/active-directory-saas-tableauserver-tutorial/tutorial_tableauserver_03.png)


3. Nella pagina della finestra di dialogo **Configurare le impostazioni dell'app** seguire questa procedura e fare clic su **Avanti**:
...
4. Nella pagina **Configura accesso Single Sign-On in Tableau Server** seguire la procedura in basso e fare clic su **Avanti**:
...
6. Per configurare l'accesso SSO sull'applicazione, è necessario accedere al tenant di Tableau Server come amministratore.

	a. Nella finestra "Tableau Server Configuration" (Configurazione Tableau Server), fare clic sulla scheda **SAML (SAML)**.

	![Configura accesso Single Sign-On](./media/active-directory-saas-tableauserver-tutorial/tutorial_tableauserver_001.png)


	b. Selezionare la casella di controllo di **Use SAML for single sign-on (Usa SAML per Single Sign-On)**.

	c. Cercare il file di metadati federazione scaricato dal portale di gestione di Azure e quindi caricarlo in **SAML Idp metadata file (File metadati Idp SAML)**.

	d. Tableau Server return URL (URL restituito di Tableau Server): l'URL a cui accedono gli utenti di Tableau Server, ad esempio http://tableau_server. È consigliabile non utilizzare http://localhost. Gli URL con barra finale (ad esempio http://tableau_server/) non sono supportati. Copiare **Tableau Server return URL (URL restituito di Tableau Server)** e incollarlo nella casella di testo **URL di accesso** in Azure AD come illustrato nel passaggio 3

	e. SAML entity ID (ID entità SAML): l'ID entità identifica in modo univoco l'installazione di Tableau Server sull'IdP. Se si desidera, qui è possibile immettere di nuovo l'URL di Tableau Server, ma non deve essere l'URL di Tableau Server in uso. Copiare **SAML entity ID (ID entità SAML)** e incollarlo nella casella di testo **IDENTIFICATORE** in Azure AD come illustrato nel passaggio 3.
	...
	g. Fare clic sul pulsante **OK** nella pagina "Tableau Server Configuration" (Configurazione Tableau Server).

	> [AZURE.NOTE] Per assistenza nella configurazione SAML in Tableau Server, fare riferimento a questo articolo: [Configure SAML (Configurazione SAML)](http://onlinehelp.tableau.com/current/server/it-IT/config_saml.htm)
...
> [AZURE.NOTE] Se si deve creare manualmente un utente, è necessario contattare l'amministratore di Tableau Server dell'organizzazione.
...
Quando si fa clic sul riquadro Tableau Server nel pannello di accesso, si dovrebbe accedere automaticamente all'applicazione Tableau Server.
