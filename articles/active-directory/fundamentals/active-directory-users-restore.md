---
title: Ripristinare o rimuovere definitivamente gli utenti eliminati di recente - Azure ADRestore or permanently remove recently deleted user - Azure AD
...
1. Accedere al [portale](https://portal.azure.com/) di Azure usando un account di amministratore globale per l'organizzazione.
...
Quando un account utente viene eliminato dall'organizzazione, l'account si trova in uno stato sospeso e tutte le informazioni sull'organizzazione correlate vengono mantenute. Quando si ripristina un utente, vengono ripristinate anche queste informazioni sull'organizzazione.

> [!Note]
> Una volta ripristinato un utente, vengono ripristinate anche le licenze assegnate all'utente al momento dell'eliminazione, anche se non sono disponibili postazioni per tali licenze. Se si utilizzano quindi più licenze superiori a quelle acquistate, l'organizzazione potrebbe essere temporaneamente non conforme per l'utilizzo delle licenze.
...
È possibile eliminare definitivamente un utente dall'organizzazione senza attendere i 30 giorni per l'eliminazione automatica. Un utente eliminato in modo permanente non può essere ripristinato né dall'utente, né da un altro amministratore, né dal supporto tecnico Microsoft.
...
Per altre informazioni sulle altre attività di gestione degli utenti disponibili, la documentazione relativa alla gestione degli utenti di Azure AD.For more information about other available user management tasks, [Azure AD user management documentation.](../users-groups-roles/index.yml)
