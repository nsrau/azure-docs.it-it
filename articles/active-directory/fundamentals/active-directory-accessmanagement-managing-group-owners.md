...
I gruppi di Azure Active Directory (Azure AD) sono di proprietà e gestiti dai proprietari del gruppo. I proprietari dei gruppi possono essere utenti o entità servizio e sono in grado di gestire il gruppo, inclusa l'appartenenza. Solo i proprietari di gruppi esistenti o gli amministratori che gestiscono i gruppi possono assegnare i proprietari del gruppo. Non è necessario che il proprietario del gruppo sia un membro del gruppo.

Quando un gruppo non ha un proprietario, gli amministratori di gestione del gruppo sono ancora in grado di gestire il gruppo. È consigliabile per ogni gruppo avere almeno un proprietario. Una volta che i proprietari sono assgined a un gruppo, l'ultimo proprietario del gruppo non può essere rimosso. Assicurati di selezionare un altro proprietario prima di rimuovere l'ultimo proprietario dal gruppo.

## <a name="add-an-owner-to-a-group"></a>Aggiungere un proprietario a un gruppo
Di seguito sono riportate le istruzioni per l'aggiunta di un utente come proprietario a un gruppo tramite il portale di Azure AD. Per aggiungere un'entità servizio come proprietario di un gruppo, seguire le istruzioni per eseguire questa operazione tramite [PowerShell](https://docs.microsoft.com/powershell/module/Azuread/Add-AzureADGroupOwner?view=azureadps-2.0).
