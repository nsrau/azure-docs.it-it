...
Consentire agli utenti di connettere l'account aziendale o dell'istituto di istruzione con LinkedIn | Impostando questa opzione su No impedisce agli utenti di connettersi. l'account aziendale o dell'istituto di istruzione tramite il proprio account LinkedIn. Per altre informazioni, vedere [la condivisione di connessioni dati e il consenso dell'account LinkedIn](https://docs.microsoft.com/azure/active-directory/users-groups-roles/linkedin-user-consent).
...
Limita l'accesso al portale di amministrazione di Azure AD | Impostando questa opzione su Sì impedisce agli utenti di accedere ad Azure Active Directory tramite solo portale di Azure.
Possibilità di leggere altri utenti | Questa impostazione è disponibile solo in PowerShell. Impostando questo flag su $false false per gli tutti gli amministratori di leggere le informazioni utente dalla directory. Questo flag non impedisce la lettura delle informazioni utente in altri servizi Microsoft come Exchange Online. Questa impostazione è destinata a circostanze speciali, e non è consigliabile impostare questo flag su $false false per.
...
### <a name="enterprise-application-owner-permissions"></a>Autorizzazioni di proprietario dell'applicazione Enterprise
Quando un utente aggiunge una nuova applicazione aziendale, vengono aggiunti automaticamente come proprietario. In qualità di proprietario può gestire la configurazione di specifici del tenant dell'applicazione, ad esempio l'accesso SSO configuration, provisioning e le assegnazioni utente. Un proprietario può anche aggiungere o rimuovere altri proprietari. A differenza degli amministratori globali, i proprietari possono gestire solo le applicazioni che sono proprietari.
...
Le tabelle seguenti descrivono le autorizzazioni specifiche in Azure Active Directory gli utenti membro hanno su oggetti di proprietà. L'utente dispone solo di queste autorizzazioni per oggetti di che cui sono proprietari.

#### <a name="owned-application-registrations"></a>Registrazioni di applicazioni di proprietà
Gli utenti possono eseguire le seguenti operazioni sulle registrazioni di proprietà dell'applicazione.
...
#### <a name="owned-enterprise-applications"></a>Applicazioni di proprietà aziendale
Gli utenti possono eseguire le azioni seguenti sulle applicazioni aziendali di proprietà. Un'applicazione aziendale è costituita da entità servizio, uno o più criteri di applicazione e in alcuni casi un oggetto di applicazione nello stesso tenant come entità servizio.
...
Gli utenti possono eseguire le azioni seguenti su dispositivi di proprietà.
...
#### <a name="owned-groups"></a>Gruppi con proprietario
Gli utenti possono eseguire le seguenti azioni sui gruppi con proprietario.
