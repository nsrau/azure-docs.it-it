...
## <a name="single-page-apps-(javascript)"></a>App a pagina singola (JavaScript)
...
### <a name="daemons/server-side-apps"></a>App daemon e sul lato server
...
In questo flusso, l'applicazione ottiene i token interagendo direttamente con l'endpoint `/token`:

![Immagine di corsie di app Daemon](../media/active-directory-v2-flows/convergence_scenarios_daemon.png)

Per compilare un'applicazione daemon, vedere la documentazione sulle credenziali client nella sezione [Introduzione](active-directory-appmodel-v2-overview.md#getting-started) o fare riferimento a [questa applicazione di esempio .NET](https://github.com/Azure-Samples/active-directory-dotnet-daemon-v2).
