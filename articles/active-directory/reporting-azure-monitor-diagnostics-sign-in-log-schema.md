---
title: Interpretare lo schema del log di accesso ad Azure Active Directory in Monitoraggio di Azure (anteprima) | Microsoft Docs
description: Descrizione dello schema del log di accesso ad Azure AD per l'uso in Monitoraggio di Azure (anteprima)
...
ms.locfileid: "39240457"
---
# <a name="interpret-the-azure-active-directory-sign-in-logs-schema-in-azure-monitor-preview"></a>Interpretazione dello schema del log di accesso ad Azure Active Directory in Monitoraggio di Azure (anteprima)

Questo articolo illustra lo schema del log di accesso ad Azure AD per l'uso in Monitoraggio di Azure. La maggior parte delle informazioni relative agli accessi viene fornita nell'attributo *Proprietà* dell'oggetto dei record.
...
| OperationVersion | La versione dell'API REST richiesta dal client |
...
| Properties | Elenca tutte le proprietà associate agli accessi. Per altre informazioni, leggere le [Informazioni di riferimento per l'API Graph di MS](https://developer.microsoft.com/en-us/graph/docs/api-reference/beta/resources/signin). Questo schema utilizza i nomi degli attributi come nella risorsa di accesso, per migliorare la leggibilità.
