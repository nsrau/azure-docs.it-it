...
In un ambiente cloud moderno, si porterà l'accesso remoto al livello successivo usando il proxy dell'applicazione in Azure Active Directory. Il proxy dell'applicazione è una funzionalità di Azure AD che viene fornita come un servizio, vale a dire che è facile da distribuire e utilizzare. Si integra con Azure Active Directory, la stessa piattaforma di identità usata da Office 365.
...
Il proxy dell'applicazione funziona con tre passaggi di base. Primo, i connettori vengono distribuiti nella rete locale. Secondo, il connettore si connette al servizio cloud. Terzo, il connettore e il servizio cloud indirizzano il traffico utente alle applicazioni.
...
Il proxy dell’applicazione funziona con l'installazione di un servizio di Windows Server slim, detto connettore, all'interno della rete. Il connettore non deve aprire porte in ingresso e non è necessario inserire niente nella rete perimetrale. Se i volumi di traffico verso le applicazioni sono elevati, è possibile aggiungere altri connettori e il servizio gestirà il bilanciamento del carico. I connettori sono senza stato e prelevano tutto dal cloud in base alle esigenze.

Quando un utente accede alle applicazioni da remoto, da qualsiasi dispositivo, viene autenticato dalla Azure Active Directory e ottiene l'accesso all'applicazione.

### Accesso Single Sign-On
Il proxy dell'applicazione di Azure AD fornisce l'accesso Single Sign-On alle applicazioni che usano l'autenticazione integrata di Windows o alle applicazioni in grado di riconoscere attestazioni. Se l'applicazione usa l'autenticazione integrata di Windows, il proxy dell'applicazione rappresenta l'utente usando la delega vincolata Kerberos per consentire l'accesso Single Sign-On. Se invece si dispone di un'applicazione con riconoscimento delle attestazione che considera attendibile Azure Active Directory, l'accesso Single Sign-On viene concesso perché l'utente era già stato autenticato da Azure AD.
...
## Risorse aggiuntive
- [Indice di articoli per la gestione delle applicazioni in Azure Active Directory](active-directory-apps-index.md)
