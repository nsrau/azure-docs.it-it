<properties
	pageTitle="Esercitazione: Integrazione di Azure Active Directory con Hosted Graphite | Microsoft Azure"
...
# Esercitazione: Integrazione di Azure Active Directory con Hosted Graphite
...
## Aggiunta di Hosted Graphite dalla raccolta
...
7. Nel riquadro dei risultati selezionare **Hosted Graphite**, quindi fare clic su **Completo** per aggiungere l'applicazione.
...
La relazione di collegamento viene stabilita nel momento in cui il valore di **nome utente** in Azure AD corrisponde al valore **Nome utente** in Hosted Graphite.
...
3. **[Creazione di un utente test Hosted Graphite](#creating-a-hosted-graphite-test-user)**: per avere una controparte di Britta Simon in Hosted Graphite collegata alla rappresentazione di Azure AD di quest'ultima.
...
2. Nella pagina **How would you like users to sign on to Hosted Graphite** (Modalità di accesso a Hosted Graphite da parte degli utenti) selezionare **Single Sign-On di Microsoft Azure AD** e quindi fare clic su **Avanti**.
...
3. Nella pagina della finestra di dialogo **Configurare le impostazioni dell'app**, se si desidera configurare l'applicazione in **modalità iniziata da IDP**,seguire la procedura seguente e fare clic su **Avanti**:
    ...
    b. Nella casella di testo **URL di risposta** digitare un URL corrispondente al modello seguente: `https://www.hostedgraphite.com/complete/saml/<user id>`

	c. Fare clic su **Avanti**

4. Se si desidera configurare l'applicazione in **SP initiated mode** (Modalità iniziata dal provider di servizi) nella finestra di dialogo **Configurare le impostazioni dell'app** fare clic su **"Mostra opzioni avanzate (facoltativo)"**, quindi digitare l'**URL accesso** e fare clic su **Avanti**.

	![Configura accesso Single Sign-On](./media/active-directory-saas-hostedgraphite-tutorial/tutorial_hostedgraphite_10.png)

	a. Nella casella di testo **URL accesso** digitare un URL corrispondente al modello seguente: `https://www.hostedgraphite.com/login/saml/<user id>/`

	b. Fare clic su **Avanti**

	> [AZURE.NOTE] Si noti che questi non sono i valori reali. È necessario aggiornare questi valori con l'URL di accesso, l'ID e l'URL di risposta effettivo. Per ottenere questi valori, è possibile passare a Access->SAML setup (Accesso -> Configurazione SAML) nell'applicazione oppure contattare Hosted Graphite.
...
7. Accedere alla **pagina di configurazione SAML** dall'intestazione laterale (**Accesso -> Configurazione SAML**).
...
9. Copiare l'**URL autorità di certificazione** e l'**SAML SSO URL** (URL SSO SAML) da Azure AD in **ID entità o ID autorità emittente** e **SSO Login URL** (URL di accesso SSO) in Hosted Graphite.
...
10. Copiare il contenuto del certificato scaricato, copiarlo e incollarlo nella casella di testo **Certificato X.509**.
...
### Creazione di un utente test di Hosted Graphite
...
> [AZURE.NOTE] Per creare un utente manualmente, è necessario contattare il team di supporto di Hosted Graphite all'indirizzo <mailto:help@hostedgraphite.com>.
