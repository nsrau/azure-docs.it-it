...
- **Precedenza abbinamento**: è possibile impostare più attributi corrispondenti. Quando sono presenti più, vengono valutati nell'ordine definito da questo campo. Quando viene rilevata una corrispondenza la valutazione degli attributi corrispondenti termina. Sebbene sia possibile impostare tutti gli attributi corrispondenti desiderati, valutare se gli attributi utilizzati come attributi corrispondenti sono realmente univoci e devono corrispondere agli attributi corrispondenti. In genere i clienti hanno 1 o 2 attributi corrispondenti nella propria configurazione. 
 ...
 * urn: IETF: params: SCIM: schemas: Extension: CustomExtensionName: 2.0: utente: CustomAttribute 
 * urn: IETF: params: SCIM: schemas: Extension: 2.0: CustomExtensionName: CustomAttribute  
 * urn: IETF: params: SCIM: schemas: estensione: CustomExtensionName: 2.0: User. CustomAttributeName: valore
...
- L'attributo IsSoftDeleted fa spesso parte dei mapping predefiniti per un'applicazione. IsSoftdeleted può essere true in uno dei quattro scenari (l'utente è esterno all'ambito perché non è stato assegnato dall'applicazione, l'utente è esterno all'ambito a causa della mancata riunione di un filtro di ambito, l'utente è stato eliminato temporaneamente in Azure AD o la proprietà AccountEnabled è impostata su false per l'utente). Non è consigliabile rimuovere l'attributo IsSoftDeleted dai mapping degli attributi.
...
- È in genere necessario eseguire il mapping dell'attributo Role usando un'espressione anziché un mapping diretto. Per ulteriori informazioni sul mapping dei ruoli, vedere la sezione precedente. 
