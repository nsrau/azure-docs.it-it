<properties
	pageTitle="Come eseguire un'installazione invisibile all'utente del connettore del proxy di applicazione di Azure AD"
...
2. Eseguire il comando seguente in cui il parametro /q significa installazione non interattiva, ovvero durante l'installazione non verrà richiesto di accettare il contratto di licenza con l'utente finale.

        AADApplicationProxyConnectorInstaller.exe REGISTERCONNECTOR="false" /q

## Passaggio 2: Registrare il connettore in Azure Active Directory
Per questa operazione è possibile usare uno dei metodi seguenti: -Registrare il connettore con un oggetto credenziali di Windows PowerShell - Registrare il connettore con un token creato offline
...
2. 	Passare a **C:\\Programmi\\Microsoft AAD App Proxy Connector** ed eseguire il file **Register Connector.PS1** in Windows PowerShell.
3. Usare l'oggetto credenziali di PowerShell creato per immettere il nome utente e la password di registrazione del connettore nello script, eseguendo il comando seguente, dove $cred è il nome dell'oggetto credenziali di PowerShell creato:
...
1. Creare un token offline con la classe AuthenticationContext, ad esempio:
...
3. Eseguire il comando di Windows PowerShell seguente, dove SecureToken è il nome del token creato in precedenza: <br> `RegisterConnector.ps1 -modulePath "C:\Program Files\Microsoft AAD App Proxy Connector\Modules" -moduleName "AppProxyPSModule" -Authenticationmode Token -Token $SecureToken -TenantId <tenant GUID>`
