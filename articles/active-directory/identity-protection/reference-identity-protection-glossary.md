...
Un utente con uno o più rilevamenti di rischio attivi. 
...
Modulo di sicurezza di Azure Active Directory che offre una visualizzazione consolidata dei rilevamenti dei rischi e delle potenziali vulnerabilità che interessano le identità di un'organizzazione.

### <a name="conditional-access"></a>Accesso condizionale
Criteri per la protezione dell'accesso alle risorse. Le regole di accesso condizionale vengono archiviate in Azure Active Directory e vengono valutate da Azure AD prima di concedere l'accesso alla risorsa.  Le regole includono la limitazione dell'accesso in base alla posizione dell'utente, all'integrità del dispositivo o al metodo di autenticazione dell'utente.
...
### <a name="false-positive-risk-detection"></a>Falso positivo (rilevamento dei rischi)
Uno stato di rilevamento dei rischi impostato manualmente da un utente di Identity Protection, che indica che il rilevamento dei rischi è stato analizzato ed è stato erroneamente contrassegnato come rilevamento dei rischi.

### <a name="identity"></a>Identità
Persona o entità che deve essere verificata tramite autenticazione, in base a criteri quali password o certificato.

### <a name="identity-risk-detection"></a>Rilevamento dei rischi di identità
Evento di Azure AD contrassegnato come anomalo da Identity Protection e che può indicare che un'identità è stata compromessa.

### <a name="ignored-risk-detection"></a>Ignorato (rilevamento dei rischi)
Stato di rilevamento dei rischi impostato manualmente da un utente di Identity Protection, che indica che il rilevamento dei rischi è chiuso senza eseguire un'azione di correzione.

### <a name="impossible-travel-from-atypical-locations"></a>Trasferimento impossibile con posizioni atipiche
Un rilevamento dei rischi attivato quando vengono rilevati due invii per lo stesso utente, in cui almeno uno di essi proviene da una posizione di accesso atipica e in cui il tempo tra gli invii è inferiore al tempo minimo necessario per viaggiare fisicamente tra questi Luoghi.  

### <a name="investigation"></a>Analisi
Processo di revisione delle attività, dei registri e di altre informazioni pertinenti relative a un rilevamento dei rischi per decidere se sono necessarie operazioni di correzione o attenuazione, capire se e come l'identità è stata compromessa e comprendere come la compromessa è stata utilizzata l'identità.

### <a name="leaked-credentials"></a>Credenziali perse
Un rilevamento dei rischi attivato quando le credenziali dell'utente corrente (nome utente e password) vengono trovate pubblicate pubblicamente nel Web scuro dai nostri ricercatori.

### <a name="mitigation"></a>Strategia di riduzione del rischio
Azione che consente di limitare o eliminare la possibilità che un utente malintenzionato sfrutti un'identità o un dispositivo compromesso senza ripristinare l'identità o il dispositivo a uno stato sicuro. Una mitigazione non risolve i rilevamenti dei rischi precedenti associati all'identità o al dispositivo.
...
Parte di un criterio di sicurezza che descrive le circostanze che attiverebbero il criterio e le azioni eseguite quando il criterio viene attivato.
...
Un utente che al momento del rilevamento dei rischi disponeva di autorizzazioni di amministratore permanenti o temporanee per una o più risorse in Azure Active Directory, ad esempio un amministratore globale, un amministratore della fatturazione, un amministratore del servizio, un amministratore utente e una password Amministratore. 
...
### <a name="remediated-risk-detection"></a>Correzione (rilevamento dei rischi)
Uno stato di rilevamento dei rischi impostato automaticamente da Identity Protection, che indica che il rilevamento dei rischi è stato recuperato utilizzando l'azione di correzione standard per questo tipo di rilevamento dei rischi. Ad esempio, quando la password utente viene reimpostata, molti rilevamenti dei rischi che indicano che la password precedente è stata compromessa vengono corretti automaticamente.

### <a name="remediation"></a>Correzione
Azione che consente di proteggere un'identità o un dispositivo che in precedenza è stato ritenuto o sospettato essere compromesso. Un'azione di correzione ripristina l'identità o il dispositivo a uno stato sicuro e risolve i rilevamenti dei rischi precedenti associati all'identità o al dispositivo.

### <a name="resolved-risk-detection"></a>Risolto (rilevamento dei rischi)
Uno stato di rilevamento dei rischi impostato manualmente da un utente di Identity Protection, che indica che l'utente ha eseguito un'azione di correzione appropriata al di fuori di Identity Protection e che il rilevamento dei rischi deve essere considerato chiuso.

### <a name="risk-detection-status"></a>Stato rilevamento dei rischi
Proprietà di un rilevamento dei rischi, che indica se l'evento è attivo e, se chiuso, il motivo della chiusura.

### <a name="risk-detection-type"></a>Tipo di rilevamento dei rischi
Categoria per il rilevamento dei rischi, che indica il tipo di anomalia che ha causato l'evento da considerare rischioso.

### <a name="risk-level-risk-detection"></a>Livello di rischio (rilevamento dei rischi)
Indicazione (Alta, Media o Bassa) della gravità del rilevamento dei rischi per consentire agli utenti di Identity Protection di assegnare priorità alle azioni intraprese per ridurre il rischio per l'organizzazione. 
...
Un rilevamento dei rischi attivato dopo un accesso riuscito dall'indirizzo IP identificato come indirizzo IP proxy anonimo.

### <a name="sign-in-from-infected-device"></a>Accesso da dispositivo infetto
Un rilevamento dei rischi attivato quando un accesso ha origine da un indirizzo IP, noto per essere utilizzato da uno o più dispositivi compromessi, che tentano attivamente di comunicare con un server bot.

### <a name="sign-in-from-ip-address-with-suspicious-activity"></a>Accesso da indirizzo IP con attività sospetta
Un rilevamento dei rischi attivato dopo un accesso riuscito da un indirizzo IP con un numero elevato di tentativi di accesso non riusciti in più account utente in un breve periodo di tempo.

### <a name="sign-in-from-unfamiliar-location"></a>Accesso da posizione non nota
Rilevamento dei rischi attivato quando un utente accede correttamente da una nuova posizione (IP, Latitude/Longitude e ASN).

### <a name="sign-in-risk"></a>Rischio di accesso
Vedere Livello di rischio (accesso)

### <a name="sign-in-risk-policy"></a>Criteri di rischio di accesso
Criteri di accesso condizionale che valutano il rischio per un accesso specifico e applicano attenuazioni in base a condizioni e regole predefinite.
...
Criteri di accesso condizionale che considerano l'accesso e applicano attenuazioni in base a condizioni e regole predefinite.

### <a name="users-flagged-for-risk"></a>Utenti contrassegnati per il rischio
Utenti con rilevamenti dei rischi attivi o corretti
