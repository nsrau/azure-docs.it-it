---
title: Nomi di prodotto e identificatori dei piani di servizio per le licenze - Azure Active Directory | Microsoft Docs
description: Mappa di identificatore per gestire le licenze di Azure Active Directory nel portale di Azure, l'interfaccia di amministrazione Microsoft 365, PowerShell o Microsoft Graph
...
Quando si gestiscono le licenze [portale di Azure](https://portal.azure.com/#blade/Microsoft_AAD_IAM/LicensesMenuBlade/Products) o il [interfaccia di amministrazione di Microsoft 365](https://admin.microsoft.com), verranno visualizzati i nomi di prodotto sono simili *Office 365 Enterprise E3*. Quando si usano i cmdlet di PowerShell v 1.0, lo stesso prodotto viene identificato mediante un nome specifico ma meno descrittivo: *ENTERPRISEPACK*. Quando si usano i cmdlet di PowerShell v 2.0 o Microsoft Graph, lo stesso prodotto viene identificato con un valore GUID: *6fd2c87f-b296-42f0-b197-1e91e994b900*. La tabella seguente elenca i prodotti del servizio online Microsoft usati più di frequente e indica i relativi valori ID. Queste tabelle servono a scopo di riferimento e sono accurate e valide solo alla data dell'ultimo aggiornamento di questo articolo. Microsoft non prevede di aggiornarle periodicamente quando vengono aggiunti nuovi servizi.
...
>[!NOTE]
>Queste informazioni sono accurate e valide al 17 agosto 2018.

| Nome prodotto | ID stringa | GUID | Piani di servizio inclusi |
| --- | --- | --- |--- |
| AUDIOCONFERENZA | MCOMEETADV | 0c266dff-15dd-4b49-8397-2bb16070ed52 |MCOMEETADV (3e26ee1f-8a5f-4d52-aee2-b81ce45c8f40) |
...
| VISIO ONLINE (PIANO 1)   | VISIOONLINE_PLAN1 | 4b244418-9658-4451-a2b8-b5e2b364e9bd  | ONEDRIVE_BASIC (da792a53-cbc0-4184-a10d-e544dd34b3c1)<br/>VISIOONLINE (2bdbaf8f-738f-4ac7-9234-3c3ee2ce7d0f) |
| VISIO Online (Piano 2)   | VISIOCLIENT   | c5928f49-12ba-48f7-ada3-0d743a3601d5  | ONEDRIVE_BASIC (da792a53-cbc0-4184-a10d-e544dd34b3c1)<br/>VISIO_CLIENT_SUBSCRIPTION (663a804f-1c30-4ff0-9915-9db84f0d1cea)<br/>VISIOONLINE (2bdbaf8f-738f-4ac7-9234-3c3ee2ce7d0f) |
...
Quando si usano licenze basate su gruppo, si verifica [questa condizione di errore](licensing-groups-resolve-problems.md#conflicting-service-plans). Quando si usa PowerShell, viene visualizzato l'errore *MutuallyExclusiveViolation*.

Questa sezione elenca i piani di servizio più comuni che si escludono reciprocamente, raggruppati per tipo di servizio. È possibile usare queste informazioni per pianificare la distribuzione di licenze ed evitare errori di assegnazione. Queste tabelle servono a scopo di riferimento e sono accurate e valide solo alla data dell'ultimo aggiornamento di questo articolo. Microsoft non prevede di aggiornarle periodicamente quando vengono aggiunti nuovi servizi.
...
### <a name="service-dynamics-crm"></a>Servizio: *Dynamics CRM*
...
* [Esempi di PowerShell per le licenze basate sui gruppi in Azure AD](licensing-ps-examples.md)
