---
title: Iscrizione self-service per gli utenti verificati tramite posta elettronica - Azure AD Documenti Microsoft
description: Usare l'iscrizione self-service in un tenant di Azure Active Directory (Azure AD)Use self-service sign-up in an Azure Active Directory (Azure AD) tenant
...
ms.locfileid: "74027641"
---
# <a name="what-is-self-service-sign-up-for-azure-active-directory"></a>Che cos'è l'iscrizione self-service per Azure Active Directory?

Questo articolo illustra come usare l'iscrizione self-service per popolare un'organizzazione in Azure Active Directory (Azure AD). Se si desidera assumere il controllo di un nome di dominio da un'organizzazione di Azure AD non gestita, vedere Assumere il controllo di una [directory non gestita come amministratore.](domains-admin-takeover.md)

## <a name="why-use-self-service-sign-up"></a>Perché utilizzare l'iscrizione self-service?
...
* Creare flussi di iscrizione basati sulla posta elettronica che consentono rapidamente agli utenti di creare identità utilizzando gli alias di posta elettronica di lavoro facili da ricordare
...
* Iscrizione **self-service:** questo è il metodo con cui un utente si iscrive a un servizio cloud e ha un'identità creata automaticamente in Azure AD in base al dominio di posta elettronica.
...
* **AllowAdHocSubscriptions** controlla la possibilità per gli utenti di eseguire l'iscrizione self-service. Se si imposta tale parametro su $false, nessun utente può eseguire l'iscrizione self-service.
...
* Utilizzare la collaborazione B2B da una directory diversa peruserdoesnotexist@contoso.cominvitare un utente che non esiste già ( ) nella home directory di contoso.com
...
Le iscrizioni alla versione di valutazione del flusso e di PowerApps non sono controllate dall'impostazione **AllowAdHocSubscriptions.Flow** and PowerApps trial signups are not controlled by the AllowAdHocSubscriptions setting. Per altre informazioni, vedere gli articoli seguenti:
...
Questi due parametri possono essere utilizzati insieme per definire un controllo più preciso sull'iscrizione self-service. Ad esempio, il comando seguente consentirà agli utenti di eseguire l'iscrizione self-service, ma solo se tali utenti dispongono già di un account in Azure AD (in altre parole, gli utenti che devono prima creare un account verificato tramite posta elettronica non possono eseguire l'iscrizione self-service):

```powershell
    Set-MsolCompanySettings -AllowEmailVerifiedUsers $false -AllowAdHocSubscriptions $true
```

Il diagramma di flusso seguente illustra le diverse combinazioni per questi parametri e le condizioni risultanti per la directory e l'iscrizione self-service.
