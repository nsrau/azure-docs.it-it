...
Questo articolo illustra come assegnare licenze di prodotto a un gruppo di utenti e verificare che siano concesse in licenza correttamente in Azure Active Directory (Azure AD).
...
1. Accedere [**all'interfaccia di amministrazione**](https://aad.portal.azure.com) di Azure AD con un account di amministratore di licenza. Per gestire le licenze, l'account deve essere un amministratore di licenza, un amministratore utente o un amministratore globale.
...
1. In **Tutti i prodotti**selezionare sia Office 365 Enterprise E5 che Enterprise Mobility - Security E3 selezionando i nomi dei prodotti. Per avviare l'assegnazione, **selezionare Assegna** nella parte superiore della pagina.

   ![Selezionare i prodotti per assegnare le licenze](./media/licensing-groups-assign/licenses-all-products-assign.png)
  
1. Nella pagina **Assegna licenza** selezionare Utenti **e gruppi** per aprire un elenco di utenti e gruppi.

1. Selezionare un utente o un gruppo, quindi utilizzare il pulsante **Seleziona** nella parte inferiore della pagina per confermare la selezione.

1. Nella pagina **Assegna licenza** fare clic su Opzioni **di assegnazione**per visualizzare tutti i piani di servizio inclusi nei due prodotti selezionati in precedenza. Trovare **Yammer Enterprise** e disattivarlo per disabilitare il servizio dalla licenza del prodotto. **Off** Confermare facendo clic **su OK** nella parte inferiore di **Opzioni licenza**.

   ![selezionare i piani di servizio per le licenze](./media/licensing-groups-assign/assignment-options.png)
  
1. Per completare l'assegnazione, nella pagina **Assegna licenza** fare clic su **Assegna** nella parte inferiore della pagina.
...
1. Passare a**Gruppi**di **Azure Active Directory** > . Selezionare il gruppo a cui sono state assegnate le licenze.

1. Nella pagina del gruppo selezionare **Licenze**. Questo consente di verificare rapidamente se le licenze sono state completamente assegnate agli utenti e se ci sono errori che richiedono attenzione. Sono disponibili le informazioni seguenti:
   ...
   - Aggiornamenti di stato delle modifiche più recenti della licenza, disponibili se le modifiche sono in fase di elaborazione o se l'elaborazione è terminata per tutti i membri dell'utente.

   - Informazioni sulle assegnazioni di licenze utente in stato di errore.

   ![errori di licenza e stato della licenza](./media/licensing-groups-assign/assignment-errors.png)

1. Vedere informazioni più dettagliate sull'elaborazione delle licenze in**Utenti e gruppi** > di **Azure Active Directory** > *nome* > **log**di controllo . Controllare le seguenti attività:
...
1. Passare a**Gruppi**di **Azure Active Directory** > e individuare il gruppo a cui sono state assegnate le licenze.
1. Nella pagina del gruppo selezionare **Licenze**. La notifica nella parte superiore della pagina indica che sono presenti 10 utenti a cui non è stato possibile assegnare licenze. Aprirlo per visualizzare un elenco di tutti gli utenti in uno stato di errore di licenza per questo gruppo.
...
Per altre informazioni sul set di funzionalità per l'assegnazione delle licenze tramite gruppi, vedere gli articoli seguenti:To learn more about the feature set for license assignment using groups, see the following articles:
