...
Si sta cambiando il comportamento di accesso Azure Active Directory (Azure AD) per fare spazio ai nuovi metodi di autenticazione e migliorare l'usabilità. Durante l'accesso, Azure AD determina la posizione in cui un utente deve eseguire l'autenticazione. Azure AD prende decisioni intelligenti leggendo le impostazioni dell'organizzazione e dell'utente per il nome utente immesso nella pagina di accesso. Si tratta di un passaggio per un futuro senza password che consente credenziali aggiuntive come FIDO 2,0.
