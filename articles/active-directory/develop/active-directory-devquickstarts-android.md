...
## <a name="step-2-register-your-web-api-with-your-microsoft-azure-ad-tenant"></a>Passaggio 2: Registrare l'API Web con il tenant di Microsoft Azure AD
...
6. Come URL di accesso immettere l'URL di base per l'esempio, che per impostazione predefinita è `https://localhost:8080`.
7. Come URI ID app immettere `https://<your_tenant_name>/TodoListService`, sostituendo `<your_tenant_name>` con il nome del tenant di Azure AD.  Fare clic su OK per completare la registrazione.
8. Sempre nel portale di Azure, andare alla pagina delle applicazioni, trovare il valore dell'ID applicazione e copiarlo a parte. Questo ID sarà necessario in un secondo momento durante la configurazione dell'applicazione.
...
7. Nella pagina delle applicazioni trovare il valore dell'ID applicazione e copiarlo a parte. Questo ID sarà necessario in un secondo momento durante la configurazione dell'applicazione.
8. Nella pagina **Impostazioni** scegliere **Autorizzazioni necessarie** e quindi scegliere **Aggiungi**.  Individuare e selezionare TodoListService e aggiungere l'autorizzazione di **accesso a TodoListService** in **Autorizzazioni delegate** e scegliere **Fatto**.
...
#### <a name="option-5-jar-package-inside-libs-folder"></a>Opzione 5: Pacchetto JAR nella cartella libs
...
1. Aggiungere un riferimento al progetto e specificarlo come una libreria Android. In caso di dubbi su come eseguire questa operazione, [fare clic qui per altre informazioni](http://developer.android.com/tools/projects/projects-eclipse.html)
...
4. Creare un'istanza di AuthenticationContext nell'attività principale. I dettagli di questa chiamata non rientrano dell'ambito di questo LEGGIMI, ma come punto di partenza è possibile vedere l' [l'esempio di client nativo Android](https://github.com/AzureADSamples/NativeClient-Android). Di seguito è riportato un esempio che utilizza SharedPreferences come cache predefinita e in cui l'autorità si presenta nel formato `https://login.windows.net/yourtenant.onmicrosoft.com`:
   
    `mContext = new AuthenticationContext(MainActivity.this, authority, true); // mContext is a field in your activity`

5. Copiare il blocco di codice per gestire la fine di AuthenticationActivity dopo che l'utente ha immesso le credenziali e ricevuto il codice di autorizzazione:
...
1. **Gestore**: l'app del portale aziendale di Microsoft Intune fornirà il componente gestore. ADAL userà l'account del gestore, se è stato creato un account utente per questo autenticatore e lo sviluppatore ha scelto di non ignorarlo. Lo sviluppatore può ignorare l'utente gestore con:
   ...
   Il manifesto dell'app deve disporre delle autorizzazioni per usare gli account di AccountManager: http://developer.android.com/reference/android/accounts/AccountManager.html
...
### <a name="authority-url-and-adfs"></a>URL dell'autorità e ADFS
...
È possibile ottenere un log nel formato di file logcat come illustrato di seguito:
 ...
 Altri esempi dei comandi adb: https://developer.android.com/tools/debugging/debugging-log.html#startingLogcat
