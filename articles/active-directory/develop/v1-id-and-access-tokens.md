...
I token ID sono un tipo di token di sicurezza di accesso che l'app riceve quando esegue l'autenticazione usando [OpenID Connect](v1-protocols-openid-connect-code.md). Vengono rappresentati come [JWT](#types-of-tokens) e contengono attestazioni che è possibile usare per l'accesso dell'utente all'app. È possibile usare le attestazioni in un token ID in base alle proprie esigenze. In genere, vengono usate per la visualizzazione di informazioni sull'account o per prendere decisioni relative al controllo di accesso in un'app.
...
| `appidacr` |Riferimento alla classe contesto di autenticazione applicazione |Indica la modalità di autenticazione del client. Per un client pubblico, il valore è 0. Se vengono usati l'ID client e il segreto client, il valore è 1. Se venisse usato un certificato client per l'autenticazione, il valore sarebbe pari a 2. <br><br> **Valore token JWT di esempio**: <br> `"appidacr": "0"` |
...
| `ver` |Version |Archivia il numero di versione del token. <br><br> **Valore token JWT di esempio**: <br> `"ver": "1.0"` |

## <a name="access-tokens"></a>Token di accesso
Al termine dell'autenticazione, Azure AD restituisce un token di accesso, che può essere usato per accedere alle risorse protette. Il token di accesso è un token JSON Web (JWT) con codifica Base64 e il suo contenuto può essere controllato eseguendolo tramite un decodificatore.
...
Se l'app è un'API protetta con Azure AD che prevede token di accesso nelle richieste HTTP, è necessario eseguire la convalida e l'ispezione dei token ricevuti. L'applicazione deve eseguire la convalida del token di accesso prima di usarlo per accedere alle risorse. Per altre informazioni sulla convalida, vedere [Convalida dei token](#validating-tokens). Per informazioni dettagliate su come eseguire questa operazione con .NET, vedere [Proteggere un'API Web usando token di connessione di Azure AD](quickstart-v1-dotnet-webapi.md).
...
|---|-----------------------|----------------------|---------------------------|--------------------------|--------------------------|
|Password scade| Rimarrà attivo|Rimarrà attivo|Rimarrà attivo|Rimarrà attivo|Rimarrà attivo|
...
|[Single Sign-Out](https://docs.microsoft.com/azure/active-directory/develop/active-directory-protocols-openid-connect-code#single-sign-out) sul Web | Revocato | Rimarrà attivo |Revocato | Rimarrà attivo |Rimarrà attivo |Rimarrà attivo |
