---
title: Personalizzare le attestazioni del token SAML dell'app Azure AD
...
| **Nome completo del dominio di Windows** | Azure AD utilizzerà WindowsDomainQualifiedName come formato NameID. |
...
3. Per applicare più trasformazioni, fare clic su **Aggiungi trasformazione**. È possibile applicare un massimo di due trasformazioni a un'attestazione. Ad esempio, è possibile estrarre innanzitutto il prefisso di posta elettronica `user.mail`di. Quindi, creare la stringa in lettere maiuscole.
...
In primo luogo, Azure AD verifica se il tipo di utente `All guests`di Brita è. A partire da, questo è vero quindi Azure AD assegna l'origine per l'attestazione a `user.extensionattribute1`. In secondo luogo, Azure AD verifica se il tipo di utente `AAD guests`di Brita è, poiché questo è anche vero, Azure ad assegna l'origine per l' `user.mail`attestazione a. Infine, l'attestazione viene emessa con valore `user.email` per Brita.
