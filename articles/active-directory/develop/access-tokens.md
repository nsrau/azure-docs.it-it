...
> Per assicurarsi che le dimensioni del token non superino i limiti delle dimensioni delle intestazioni HTTP, Azure AD limita il numero di ID oggetto inclusi nell'attestazione dei gruppi. Se un utente è membro di più gruppi rispetto al limite eccedenza (150 per i token SAML, 200 per i token JWT), Azure AD non emette l'attestazione dei gruppi nel token. Include invece un'attestazione in eccedenza nel token che indica all'applicazione di eseguire una query sul API Graph per recuperare l'appartenenza al gruppo dell'utente.
  ```csharp
  {
   ...
   ```
> È possibile usare la `BulkCreateGroups.ps1` disponibile nella cartella [script](https://github.com/Azure-Samples/active-directory-dotnet-webapp-groupclaims/blob/master/AppCreationScripts/) per la creazione di app per testare gli scenari in eccedenza.
...
* Se sono state richieste le attestazioni `roles` o `groups` nel token di accesso, verificare che l'utente sia nel gruppo autorizzato a eseguire questa azione.
