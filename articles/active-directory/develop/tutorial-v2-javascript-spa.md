...
In questa esercitazione verrà creata un'applicazione a pagina singola JavaScript che concede l'accesso agli utenti e chiama Microsoft Graph usando il flusso implicito. L'applicazione a pagina singola creata usa Microsoft Authentication Library (MSAL) per JavaScript v1.0.
...
1. Per prima cosa, creare un file js denominato `graphConfig.js`, in cui archiviare gli endpoint REST. Aggiungere il codice seguente:
   ...
   Dove:
   - *\<Enter_the_Graph_Endpoint_Here>* è l'istanza dell'API Microsoft Graph. Per l'endpoint globale dell'API Microsoft Graph, è sufficiente sostituire questa stringa con `https://graph.microsoft.com`. Per le distribuzioni cloud nazionali, fare riferimento alla [documentazione dell'API Graph](/graph/deployments).
