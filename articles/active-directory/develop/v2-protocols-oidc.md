---
title: Microsoft Identity Platform & protocollo OpenID Connect | Azure
...
> [!IMPORTANT]
> Per richiedere correttamente un token ID dall'endpoint combinazione/Gestione autorizzazioni, è necessario che la registrazione dell'app nel [portale di registrazione](https://portal.azure.com) includa la concessione implicita di id_tokens abilitata nella scheda autenticazione, che imposta il flag di `oauth2AllowIdTokenImplicitFlow` nel [manifesto dell'applicazione](reference-app-manifest.md) per `true`. Se non è abilitato, verrà restituito un errore di `unsupported_response`: "il valore specificato per il parametro di input ' response_type ' non è consentito per questo client. Expected value is 'code'" (Il valore fornito per il parametro di input 'response_type' non è consentito per questo client. Il valore previsto è 'code')
...
A questo punto, all'utente viene chiesto di immettere le credenziali e completare l'autenticazione. L'endpoint della piattaforma Microsoft Identity verifica che l'utente abbia acconsentito alle autorizzazioni indicate nel parametro di query `scope`. Se l'utente non ha acconsentito a nessuna di queste autorizzazioni, l'endpoint della piattaforma Microsoft Identity chiede all'utente di fornire il consenso per le autorizzazioni necessarie. Per altre informazioni, vedere [autorizzazioni, consenso e app multi-tenant](v2-permissions-and-consent.md).

Dopo che l'utente ha eseguito l'autenticazione e concesso il consenso, l'endpoint della piattaforma di identità Microsoft restituisce una risposta all'app nell'URI di reindirizzamento indicato usando il metodo specificato nel parametro `response_mode`.
...
Includendo gli ambiti di autorizzazione nella richiesta e usando `response_type=id_token code`, l'endpoint della piattaforma Microsoft Identity garantisce che l'utente abbia acconsentito alle autorizzazioni indicate nel parametro di query `scope`. Verrà restituito nell'app un codice di autorizzazione da scambiare con un token di accesso.
