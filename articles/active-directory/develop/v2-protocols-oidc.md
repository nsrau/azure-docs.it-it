...
> [!TIP]
> Prova Fare [https://login.microsoftonline.com/common/v2.0/.well-known/openid-configuration](https://login.microsoftonline.com/common/v2.0/.well-known/openid-configuration) clic per visualizzare `common` la configurazione dei tenant.
...
> [!IMPORTANT]
> Per richiedere correttamente un token ID dall'endpoint combinazione/Gestione autorizzazioni, è necessario che la registrazione dell'app nel [portale di registrazione](https://portal.azure.com) includa la concessione implicita di id_tokens abilitata nella scheda autenticazione (che `oauth2AllowIdTokenImplicitFlow` imposta il flag nel [manifesto dell'applicazione](reference-app-manifest.md) su `true`). Se non è abilitato, verrà `unsupported_response` restituito un errore: "il valore fornito per il parametro di input ' response_type ' non è consentito per questo client. Expected value is 'code'" (Il valore fornito per il parametro di input 'response_type' non è consentito per questo client. Il valore previsto è 'code')
...
| --- | --- |
| `id_token` | Token ID richiesto dall'app. È possibile usare il parametro `id_token` per verificare l'identità dell'utente e avviare una sessione con l'utente. Per ulteriori informazioni sui token ID e sul relativo contenuto, vedere il [ `id_tokens` riferimento](id-tokens.md). |
...
Solo la ricezione di un id_token non è sufficiente per autenticare l'utente. è necessario convalidare la firma del id_token e verificare le attestazioni nel token in base ai requisiti dell'app. L'endpoint della piattaforma Microsoft Identity usa i [token Web JSON (token JWT)](https://self-issued.info/docs/draft-ietf-oauth-json-web-token.html) e la crittografia a chiave pubblica per firmare i token e verificare che siano validi.

È possibile scegliere di convalidare `id_token` nel codice client, ma una procedura comune consiste nell'inviare `id_token` a un server back-end ed eseguire la convalida. Dopo aver convalidato la firma del id_token, è necessario verificare alcune attestazioni. Per ulteriori informazioni, tra cui la [convalida dei token](id-tokens.md#validating-an-id_token) e [informazioni importanti sul rollover della chiave di firma](active-directory-signing-key-rollover.md), vedere il [ `id_token` riferimento](id-tokens.md) . È consigliabile usare una libreria per l'analisi e la convalida dei token. È disponibile almeno una libreria per la maggior parte dei linguaggi e delle piattaforme.
...
Dopo aver convalidato il id_token, è possibile avviare una sessione con l'utente e usare le attestazioni nel id_token per ottenere informazioni sull'utente nell'app. Queste informazioni possono essere usate per la visualizzazione, i record, la personalizzazione e così via.
...
Includendo gli ambiti di autorizzazione nella richiesta e usando `response_type=id_token code`, l'endpoint della piattaforma Microsoft Identity garantisce che l'utente abbia acconsentito alle autorizzazioni indicate nel parametro di `scope` query. Verrà restituito nell'app un codice di autorizzazione da scambiare con un token di accesso.
...
| Parametro | Descrizione |
| --- | --- |
| `id_token` | Token ID richiesto dall'app. È possibile usare il token ID per verificare l'identità dell'utente e avviare una sessione con l'utente. Per ulteriori informazioni sui token ID e sul relativo contenuto, vedere il [ `id_tokens` riferimento](id-tokens.md). |
