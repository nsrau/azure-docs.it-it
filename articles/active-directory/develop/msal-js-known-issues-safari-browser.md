...
MSAL. js usa un iframe nascosto per eseguire l'acquisizione e il `acquireTokenSilent` rinnovo del token invisibile all'utente come parte delle chiamate. Le richieste di token invisibile di sistema si basano sull'iframe che ha accesso alla sessione utente autenticata rappresentata dai cookie impostati da Azure AD. Con ITP 2,0 che impedisce l'accesso a questi cookie, MSAL. js non riesce ad acquisire e rinnovare automaticamente i token, `acquireTokenSilent` causando errori.
...
Per evitare gli accessi ripetuti, un approccio che è possibile implementare consiste nel `acquireTokenSilent` gestire l'errore e fornire all'utente un'opzione per disabilitare l'impostazione di ITP in Safari prima di procedere con la chiamata interattiva. Dopo che l'impostazione è stata disabilitata, i rinnovi dei token Silent successivi dovrebbero avere esito positivo.
