...
* Ambito in sostituzione del parametro della risorsa nelle richieste di autenticazione per l'acquisizione di token
    ...
    Solo per l'API MS `user.read` Graph, un\/valore di ambito viene mappato a https: /graph.microsoft.com/User.Read e può essere utilizzato in modo intercambiabile.
