...
description: Utilizzo degli id_tokens generati dagli endpoint v1 e v2 di Azure AD.
...
|`name` |  Stringa | L'attestazione `name` fornisce un valore leggibile che identifica l'oggetto del token. Il valore potrebbe non essere univoco, è modificabile e può essere usato solo per scopi di visualizzazione. Per ricevere questa attestazione, è necessario l'ambito `profile` . |
...
## <a name="validating-idtokens"></a>Convalida degli `id_tokens`
