---
title: Creare app che accedono agli utenti di Azure ADBuild apps that sign in Azure AD users
titleSuffix: Microsoft identity platform
description: Viene illustrato come creare un'applicazione multi-tenant in grado di accedere a un utente da qualsiasi tenant di Azure Active Directory.
...
Per impostazione predefinita, le registrazioni di API o app Web in Azure AD sono single-tenant. È possibile rendere la registrazione multi-tenant individuando l'opzione **Tipi di account supportati** nel riquadro **Autenticazione** della registrazione dell'applicazione nel [portale][AZURE-portal] di Azure e impostandola su Account in qualsiasi **directory dell'organizzazione.**
...
> [!NOTE]
> Le registrazioni dei client nativi e le applicazioni della piattaforma di [identità Microsoft](./active-directory-appmodel-v2-overview.md) sono multi-tenant per impostazione predefinita. Non è necessario intraprendere alcuna azione per rendere multi-tenant queste registrazioni dell'applicazione.
...
Quando la piattaforma di identità Microsoft riceve una richiesta sull'endpoint /common, accede all'utente e, di conseguenza, individua il tenant da cui proviene l'utente. L'endpoint /common funziona con tutti i protocolli di autenticazione supportati da Azure AD: OpenID Connect, OAuth 2.0, SAML 2.0 e WS-Federation.
...
Le applicazioni Web e le API Web ricevono e convalidano i token dalla piattaforma di identità Microsoft.Web applications and web APIs receive and validate tokens from Microsoft identity platform.

> [!NOTE]
> Mentre le applicazioni client native richiedono e ricevono token dalla piattaforma di identità Microsoft, lo fanno per inviarli alle API, dove vengono convalidati. Le applicazioni native non convalidano i token e devono gestirli come opachi.

Esaminiamo il modo in cui un'applicazione convalida i token ricevuti dalla piattaforma di identità Microsoft.Let's look at how an application validates tokens it receives from Microsoft identity platform. Un'applicazione single-tenant prende in genere un valore dell'endpoint come:
...
Per fare in modo che un utente possa accedere a un'applicazione in Azure AD, l'applicazione deve essere rappresentata nel tenant dell'utente. Ciò consente alle organizzazioni di eseguire operazioni come applicare criteri univoci quando gli utenti dal tenant accedono all'applicazione. Per una singola applicazione tenant, questa registrazione è semplice; è quello che si verifica quando si registra l'applicazione nel portale di [Azure][AZURE-portal].

Per un'applicazione multi-tenant, la registrazione iniziale per l'applicazione si trova nel tenant di Azure AD usato dallo sviluppatore. Quando un utente di un tenant diverso accede all'applicazione per la prima volta, Azure AD richiede il consenso alle autorizzazioni richieste dall'applicazione. Se fornisce il consenso, viene creata una rappresentazione dell'applicazione denominata *entità servizio* nel tenant dell'utente ed è possibile procedere con l'accesso. Viene anche creata una delega nella directory che registra il consenso dell'utente all'applicazione. Per informazioni dettagliate sugli oggetti Application e ServicePrincipal dell'applicazione e sulla relativa correlazione tra loro, vedere [Oggetti applicazione e oggetti entità servizio][AAD-App-SP-Objects].

![Illustra il consenso all'app a livello singolo][Consent-Single-Tier]

Questa esperienza di consenso è interessata dalle autorizzazioni richieste dall'applicazione. La piattaforma di identità Microsoft supporta due tipi di autorizzazioni, solo app e delegate.
...
> [!NOTE]
> Alcune applicazioni offrono un'esperienza in cui gli utenti normali sono inizialmente in grado di fornire il consenso e successivamente l'applicazione può coinvolgere l'amministratore e richiedere le autorizzazioni che necessitano del consenso dell'amministratore. Non è possibile eseguire questa operazione con una registrazione dell'applicazione v1.0 in Azure AD oggi. Tuttavia, l'utilizzo dell'endpoint Microsoft Identity Platform (v2.0) consente alle applicazioni di richiedere autorizzazioni in fase di esecuzione anziché in fase di registrazione, che abilita questo scenario. Per ulteriori informazioni, vedere [Endpoint della piattaforma di identità Microsoft][AAD-V2-Dev-Guide].
...
![Illustra il consenso all'app client nota a più livelli][Consent-Multi-Tier-Known-Client]
...
Se si tratta di un'API creata da un'organizzazione diversa da Microsoft, lo sviluppatore dell'API deve fornire ai clienti un modo per consentire ai clienti di acconsentire all'applicazione nei tenant dei clienti. La progettazione consigliata è per lo sviluppatore di terze parti per compilare l'API in modo che possa funzionare anche come client web per implementare l'iscrizione. A tale scopo, effettuare l'operazione seguente:
...
2. Oltre a esporre gli ambiti/ruoli dell'API, assicurarsi che la registrazione includa l'autorizzazione "Accedi e leggi il profilo utente" (fornita per impostazione predefinita).
...
![Illustra il consenso all'app multiparte multi-party][Consent-Multi-Tier-Multi-Party]
...
* Gli amministratori revocano l'accesso alle applicazioni rimuovendole tramite la sezione [Applicazioni aziendali](https://portal.azure.com/#blade/Microsoft_AAD_IAM/StartboardApplicationsMenuBlade/AllApps) del portale di [Azure.][AZURE-portal]
...
* [Panoramica del quadro di consenso][AAD-Consent-Overview]
