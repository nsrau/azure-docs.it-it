---
title: API Web protetta-configurazione del codice dell'app | Azure
...
Analogamente alle app Web, le API Web di ASP.NET/ASP.NET Core sono "protette" perché le azioni del controller sono `[Authorize]` precedute dall'attributo. Quindi, le azioni del controller possono essere chiamate solo se l'API viene chiamata con un'identità autorizzata.
...
| `ValidateLifetime` | Garantisce che il token sia ancora (o già) valido. Il validator controlla se la durata del token (`notbefore` e `expires` delle attestazioni) è compresa nell'intervallo. |
