...
Gli ambiti in genere hanno il formato `resourceURI/scopeName`. Per Microsoft Graph, gli ambiti dispongono di tasti di scelta rapida. Ad esempio, `User.Read` è un collegamento per `https://graph.microsoft.com/user.read`.
...
Per impostazione predefinita, il portale di registrazione delle applicazioni consiglia di usare l'URI di risorsa `api://{clientId}`. Questo URI è univoco ma non leggibile. Se si modifica l'URI, verificare che il nuovo valore sia univoco.
...
1. Per modificare il manifesto, trovare l'impostazione `appRoles` e aggiungere i ruoli applicazione. Le definizioni di ruolo sono disponibili nel blocco JSON di esempio seguente.
1. Lasciare `allowedMemberTypes` impostato solo su `"Application"`.
1. Assicurarsi che `id` sia un GUID univoco.
1. Verificare `displayName` e `value` non contengano spazi.
...
1. Se si desidera Azure AD per consentire l'accesso all'API Web solo da determinati client, impostare **assegnazione utente obbligatoria?** su **Sì**.
   ...
   > [!IMPORTANT]
   >
   > Se si imposta **assegnazione utente obbligatoria?** su **Sì**, Azure ad controlla le assegnazioni di ruolo dell'app di un client quando richiede un token di accesso all'API Web. Se il client non è assegnato ad alcun ruolo app, Azure AD restituirà il messaggio di errore "invalid_client: AADSTS501051: applicazione \<il nome dell'applicazione\> non è assegnato a un ruolo per \<API Web\>".
   >
