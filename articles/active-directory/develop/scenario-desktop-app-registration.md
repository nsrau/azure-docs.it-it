...
- Se si usa l'autenticazione interattiva o il flusso del codice `https://login.microsoftonline.com/common/oauth2/nativeclient`del dispositivo, usare. Per ottenere questa configurazione, selezionare l'URL corrispondente nella sezione **autenticazione** per l'applicazione.
  
  > [!IMPORTANT]
  > Attualmente, MSAL.NET usa un altro URI di reindirizzamento per impostazione predefinita nelle applicazioni desktop eseguite in`urn:ietf:wg:oauth:2.0:oob`Windows (). In futuro, sarà necessario modificare questa impostazione predefinita, quindi è consigliabile usare `https://login.microsoftonline.com/common/oauth2/nativeclient`.
