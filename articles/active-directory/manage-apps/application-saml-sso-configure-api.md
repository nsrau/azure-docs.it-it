...
Per eseguire il test, è possibile usare il comando di PowerShell seguente per ottenere un certificato autofirmato. Usare le procedure di sicurezza consigliate dell'azienda per creare un certificato di firma per l'ambiente di produzione.
...
Verificare che il valore di keyId per la proprietà keyCredential usata per "Sign" corrisponda al valore di keyId per la proprietà passwordCredential.

È possibile generare `customkeyIdentifier` recuperando l'hash dell'identificazione personale del certificato.
...
> [!NOTE]
> Il valore "key" nella proprietà keyCredentials viene abbreviato per migliorare la leggibilità.
...
Per altre informazioni, vedere il tipo di risorsa [appRoleAssignment](https://docs.microsoft.com/graph/api/resources/approleassignment?view=graph-rest-beta).
