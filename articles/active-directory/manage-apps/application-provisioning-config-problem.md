...
Se si imposta lo **stato di provisioning** **su On** nella **Azure Active Directory &gt; app aziendali &gt; \[nome dell'applicazione\] sezione provisioning** del &gt;. ma in seguito a successivi ricaricamenti non vengono visualizzati altri dettagli nella pagina, È probabile che il servizio sia in esecuzione, ma non ha ancora completato un ciclo iniziale. Controllare i **log di provisioning** descritti in precedenza per determinare le operazioni eseguite dal servizio e in caso di errori.
