...
> [!NOTE]
> Questo articolo è incentrato sullo spostare l'autenticazione dell'applicazione da Active Directory locali e Active Directory Federation Services a Azure AD. Per una panoramica della pianificazione di questo spostamento, vedere la white paper [migrazione dell'autenticazione dell'applicazione a Azure ad](https://aka.ms/migrateapps/whitepaper) . Il white paper illustra come pianificare la migrazione, i test e le informazioni dettagliate.
...
Per configurare un'applicazione SaaS per la Single Sign-On basata su SAML, vedere [configurare Single Sign-on basato su SAML](https://docs.microsoft.com/azure/active-directory/manage-apps/configure-single-sign-on-non-gallery-applications). 
...
| **URL di risposta dell'app** <p>URL dell'app dal punto di vista del provider di identità (IdP). Il provider di identità invia l'utente e il token qui dopo che l'utente ha eseguito l'accesso al provider di identità.  Questa operazione è nota anche come **endpoint consumer di asserzione SAML**.| Selezionare la scheda **endpoint**| Apri configurazione SAML di base da accesso basato su SAML| Elemento Destination nel token SAML. Valore di esempio:[https://contoso.my.salesforce.com](https://contoso.my.salesforce.com/) |
...
| | Come configurare l'opzione Consenti in Azure AD?| Come configurare l'opzione except in Azure AD? |
| - | - | - |
...
* Indicare agli utenti di usare il [Pannello di accesso](https://myapps.microsoft.com) per accedere a tutte le applicazioni migrate. 
