...
1. Selezionare una **mapping** configuration per aprire i relativi **Mapping degli attributi** dello schermo. Alcuni mapping di attributi sono necessarie per una corretto funzionamento dell'applicazione SaaS. Per gli attributi obbligatori la funzionalità **Elimina** non è disponibile.
   ...
   ![Usare Mapping degli attributi per configurare i mapping degli attributi per le app](./media/customize-application-attributes/22.png)
...
![Viene illustrata ServiceNow con oggetti Group e User provisioning](./media/customize-application-attributes/24.png)
...
- L'attributo IsSoftDeleted fa spesso parte i mapping predefiniti per un'applicazione. IsSoftdeleted può essere true in uno dei quattro scenari (l'utente non è compreso nell'ambito a causa non assegnate dall'applicazione, l'utente non è compreso nell'ambito a causa non soddisfa un filtro di ambito, l'utente è stato trasferibili eliminato in Azure AD o la proprietà AccountEnabled è impostata su false  l'utente). 
