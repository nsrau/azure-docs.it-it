---
title: Problemi di accesso all'app federata per la raccolta Single Sign-On . Documenti Microsoft
...
Il `AssertionConsumerServiceURL` valore nella richiesta SAML non corrisponde al valore o al modello di URL di risposta configurato in Azure AD. Il `AssertionConsumerServiceURL` valore nella richiesta SAML è l'URL visualizzato nell'errore.

**Risoluzione**

Verificare `AssertionConsumerServiceURL` che il valore nella richiesta SAML corrisponda al valore URL di risposta configurato in Azure AD. Se si usa [l'esperienza](../azuread-dev/howto-v1-debug-saml-sso-issues.md) di test nel portale di Azure con l'estensione My Apps Secure Browser, non è necessario eseguire manualmente questa procedura.
...
1.  Dopo il caricamento dell'applicazione, aprire **Configurazione SAML di base**. Verificare o aggiornare il valore nella `AssertionConsumerServiceURL` casella di testo URL di risposta in modo che corrisponda al valore nella richiesta SAML.    
...
1.  Nell'elenco delle applicazioni selezionare quella a cui si desidera assegnare un utente.

1.  Una volta caricata l'applicazione, selezionare **Utenti e gruppi** dal menu di navigazione a sinistra dell'applicazione.
...
1. Passare il mouse **sull'utente** nell'elenco per visualizzare una **casella di controllo.** Fai clic sulla casella di controllo accanto alla foto o al logo del profilo dell'utente per aggiungere l'utente all'elenco **Selezionato.**

1. **Facoltativo:** Se si desidera **aggiungere più utenti,** digitare un altro nome completo o indirizzo di posta elettronica nella casella di ricerca **Cerca per nome o indirizzo** di posta elettronica e fare clic sulla casella di controllo per aggiungere l'utente all'elenco **Selezionato.**
...
Dopo un breve periodo di tempo, gli utenti selezionati saranno in grado di avviare queste applicazioni utilizzando i metodi descritti nella sezione relativa alla descrizione della soluzione.
...
1. Acquisire la richiesta SAML. Seguire l'esercitazione Come eseguire il [debug dell'accesso Single Sign-On basato su SAML alle applicazioni in Azure AD](../azuread-dev/howto-v1-debug-saml-sso-issues.md) per informazioni su come acquisire la richiesta SAML.
...
Per informazioni su come personalizzare le attestazioni dell'attributo SAML inviate all'applicazione, vedere [Mapping delle attestazioni in Azure Active Directory.](../develop/active-directory-claims-mapping.md)
