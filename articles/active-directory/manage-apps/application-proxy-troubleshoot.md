...
| Non è stato trovato un server con il nome host specificato. | L'utente può visualizzare questo errore quando prova ad accedere all'app pubblicata, se il dominio personalizzato dell'applicazione non è configurato correttamente. Assicurarsi di aver caricato un certificato per il dominio e configurato correttamente il record DNS seguendo i passaggi descritti in [utilizzo di domini personalizzati nel Proxy applicazione Azure AD](application-proxy-configure-custom-domain.md) |
