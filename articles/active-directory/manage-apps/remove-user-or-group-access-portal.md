---
title: Rimuovere assegnazioni di utenti o gruppi da un'app in Azure ADRemove user or group assignments from an app in Azure AD
...
È facile rimuovere un utente o un gruppo dall'accesso assegnato a una delle applicazioni aziendali in Azure Active Directory (Azure AD). Per gestire l'app aziendale sono necessarie le autorizzazioni appropriate. Inoltre, è necessario essere amministratore globale per la directory.
...
1. Nella pagina **Azure Active Directory - nome *directory,* ** ovvero la pagina di Azure AD per la directory che si sta gestendo, selezionare **Applicazioni aziendali**.
...
1. Nella pagina di panoramica del ***nome app,*** ovvero la pagina con il nome dell'app selezionata nel titolo, selezionare **Gruppi & utenti**.
...
1. Utilizzare lo script seguente per rimuovere un utente e un ruolo da un'applicazione:Use the following script to remove a user and role from an application:
