...
Gli utenti possono anche caricare un'immagine sottoposta a sysprep nel portale di gestione di Azure. Per altre informazioni, vedere <a href="/it-it/manage/windows/common-tasks/upload-a-vhd/">Creazione e caricamento di un disco rigido virtuale</a>. Nella figura 3 è illustrata la procedura di base per caricare un'immagine e creare una VM:
...
<li>Se non è disponibile un account Azure, <a href="http://azure.microsoft.com/pricing/free-trial/">iscriversi per una valutazione gratuita di Azure</a>.</li>
</ul>
</li>
...
<ul>
<li>Immettere un <strong>NOME MACCHINA VIRTUALE</strong>.
</li>
<li>Selezionare le appropriate <strong>DIMENSIONI</strong>.
<ul>
...
<li>Per altre informazioni, vedere <a href="/it-it/manage/windows/common-tasks/upload-a-vhd/#createstorage">Creare un account di archiviazione in Azure</a>.</li>
</ul>
</li>
...
<li>Usare il prompt dei comandi per installare SQL Server. Per altre informazioni, vedere <a href="http://msdn.microsoft.com/library/ms144259.aspx#SysPrep">Per altre informazioni, vedere Installare SQL Server 2012 dal prompt dei comandi</a>.</li>
...
<ul>
<li>Per creare ed eseguire il provisioning di una farm di SharePoint, vedere <a href="http://technet.microsoft.com/ library/ee805948.aspx#CreateConfigure">Creare una farm di Microsoft SharePoint Server</a>.</li>
<li>Per aggiungere un server applicazioni o server Web alla farm, vedere l'articolo relativo all' <a href="http://technet.microsoft.com/ library/cc261752.aspx">aggiunta di un server Web o di un server applicazioni a una farm in SharePoint Server 2010</a>.</li>
<li>
<p>Per aggiungere un server di database a una farm esistente, vedere l'articolo relativo all' <a href="http://technet.microsoft.com/ library/cc262781">aggiunta di un server di database a una farm esistente</a>.</p>
<ul>
<li>Per usare SQL Server 2012 per la farm di SharePoint, è necessario scaricare e installare il Service Pack 1 per SharePoint Server 2010 dopo aver installato l'applicazione e aver scelto di non configurare il server. Per altre informazioni, vedere <a href="http://www.microsoft.com/it-it/download/details.aspx?id=26623">Service Pack 1 per Microsoft SharePoint Server 2010</a>.</li>
...
<li>Preparare l'immagine da caricare: prima di caricare l'immagine in Azure, è necessario generalizzarla tramite il comando SysPrep. Per altre informazioni, vedere <a href="http://technet.microsoft.com/ library/bb457073.aspx">Come usare SysPrep: un'introduzione</a>.</li>
...
<li><em>Installare</em>: installare SharePoint Server, Visual Studio e SQL Server nelle macchine virtuali usando una connessione Desktop remoto.
<ul>
<li>Scegliere un'opzione per l'installazione di SharePoint Server:
<ul>
<li>Usare gli strumenti SharePoint 2010 Easy Setup Script per creare un computer SharePoint per lo sviluppo. Per altre informazioni, vedere <a href="http://www.microsoft.com/it-it/download/details.aspx?id=23415">SharePoint 2010 Easy Setup Script</a>.</li>
...
In questo scenario è necessario aggiungere altri server Web front-end per SharePoint in modo da supportare l'aumento del traffico. Questi server richiedono sicurezza avanzata e controller di dominio di Servizi di dominio di Active directory per supportare l'autenticazione e l'autorizzazione degli utenti. Nella figura 5 è illustrato il layout di questo scenario.
...
<ol>
<li><em>Distribuire Active Directory</em>: i requisiti di base per la distribuzione di Active Directory in Macchine virtuali di Azure sono simili, ma non identici, a quelli della distribuzione in macchine virtuali (e, in una certa misura, in computer fisici) in locale. Per altre informazioni sulle differenze e per linee guida e altre considerazioni, vedere <a href="http://msdn.microsoft.com/library/windowsazure/jj156090">Linee guida per la distribuzione di Active Directory in macchine virtuali di Azure</a>. Per distribuire Active Directory in Azure:
<ul>
<li>Definire e creare una rete virtuale in cui è possibile assegnare le macchine virtuali a specifiche subnet. Per altre informazioni, vedere <a href="https://github.com/WindowsAzure-TrainingKit/HOL-DeployingActiveDirectory/blob/master/HOL.md">Configurare la rete virtuale</a>.</li>
...
<li>Per altre informazioni sulla creazione di una nuova foresta Active Directory in una macchina virtuale di Rete virtuale di Azure, vedere <a href="/it-it/manage/services/networking/active-directory-forest/">Installare una nuova foresta Active Directory in Azure</a>.</li>
</ul>
</li>
</ul>
</li>
...
<li><em>Installare</em>: se l'organizzazione non dispone di immagini predefinite di componenti di BI di SharePoint Server o SQL Server, installare SharePoint Server e SQL Server nelle macchine virtuali usando una connessione Desktop remoto.
<ul>
<li>Per altre informazioni sull'installazione di SharePoint, vedere <a href="http://technet.microsoft.com/ library/cc262839.aspx">Installare SharePoint Server 2010 tramite Windows PowerShell</a> oppure <a href="http://autospinstaller.codeplex.com/">CodePlex: AutoSPInstaller</a>.</li>
...
<li>Configurare la macchina virtuale di SharePoint e connetterla alla farm di SharePoint usando il comando <a href="http://technet.microsoft.com/ library/ff607979.aspx">Join-SharePointFarm</a> .</li>
<li>Usare il portale di gestione per configurare il bilanciamento del carico:
<ul>
...
<p><a href="/it-it/manage/downloads/">https://www.windowsazure.com/it-it/manage/downloads/</a></p>
</li>
<li>
<p>Guida alle procedure e documentazione sulle procedure consigliate</p>
<p><a href="/it-it/manage/windows/">https://www.windowsazure.com/it-it/manage/windows/</a></p>
</li>
</ul>
