...
6. Nell'emulatore sul lato sinistro aggiornare il valore  `text` dell'ultimo TodoItem in **Test Write 1**, quindi fare clic su un'altra casella di testo in modo che il gestore eventi  `LostFocus` aggiorni il database. Nella schermata seguente viene visualizzato un esempio. 
...
In questa esercitazione sono state illustrate le procedure per abilitare la gestione dei conflitti di scrittura in un'app Windows Phone 8 quando si usano i dati in Servizi mobili. In seguito, è consigliabile eseguire una delle esercitazioni seguenti della serie relativa ai dati:
