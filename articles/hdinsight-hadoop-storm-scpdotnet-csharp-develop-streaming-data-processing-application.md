<properties title="Sviluppare applicazioni di elaborazione di flussi di dati con SCP.NET e C# tramite Storm in HDInsight" pageTitle="Sviluppare applicazioni di elaborazione di flussi di dati con SCP.NET e C# tramite Storm | Azure" description="Informazioni su come sviluppare applicazioni di elaborazione di flussi di dati con SCP.NET e C# tramite Storm in HDInsight." services="hdinsight" solutions="" documentationCenter="" authors="Qianlin Xia" videoId="" scriptId="" />
...
# Sviluppare applicazioni di elaborazione di flussi di dati con SCP.NET e C# tramite Storm in HDInsight

## Introduzione a SCP

SCP è una piattaforma per la compilazione di applicazioni di elaborazione dati dalle prestazioni elevate, affidabili, coerenti e in tempo reale. È basata su Storm (http://storm.incubator.apache.org/), un sistema di elaborazione dei flussi progettato dalle community di sviluppo di software open source (OSS). Storm è stato progettato da Nathan Marz e reso open source da Twitter. Il sistema si avvale di ZooKeeper (http://zookeeper.apache.org/), un altro progetto Apache per il coordinamento e la gestione dello stato altamente affidabili delle applicazioni distribuite.
...
In Storm, una topologia applicazione definisce un grafo di elaborazione. Ogni nodo in una topologia contiene logica di elaborazione e i collegamenti tra i nodi indicano il flusso dei dati. I nodi per l'inserimento dei dati di input nella topologia sono denominati Spout e possono essere usati per porre in sequenza i dati. I dati di input possono risiedere in log di file, database transazionali, contatori delle prestazioni del sistema e così via. I nodi con flussi di dati sia di input che di output sono denominati Bolt ed eseguono le effettive operazioni di applicazione di filtri, selezione e aggregazione dei dati.

SCP supporta l'elaborazione dati di tipo best effort, at-least-once ed exactly-once. In un'applicazione distribuita di elaborazione di flussi, durante l'elaborazione possono verificarsi diversi errori, ad esempio un'interruzione di rete, l'errore di un computer o del codice utente e così via. L'elaborazione at-least-once assicura che tutti i dati verranno elaborati almeno una volta, riproducendo automaticamente gli stessi dati in caso di errore. L'elaborazione at-least-once è semplice e affidabile e offre risultati positivi in molte applicazioni. In alcuni casi tuttavia, ad esempio quando l'applicazione richiede il conteggio esatto, l'elaborazione at-least-once è insufficiente, perché potenzialmente nella topologia dell'applicazione potrebbero essere riprodotti gli stessi dati. In questi casi si può usare l'elaborazione exactly-once, progettata per garantire che i risultati siano corretti anche se è possibile che i dati vengano riprodotti ed elaborati più volte.

SCP consente agli sviluppatori .NET di sviluppare applicazioni di elaborazione dati in tempo reale usando in modo invisibile Storm in una JVM. .NET e la JVM comunicano tramite un socket locale TCP. Essenzialmente, ogni Spout/Bolt è una coppia di processi .Net/Java, in cui la logica utente viene eseguita nel processo .Net come plug-in.
...
Se l'acknowledgement non è supportato in una topologia non transazionale, Ack() e Fail() possono essere lasciati come funzione vuota.
...
Quando è disponibile una nuova tupla, verrà chiamata la funzione Execute() per l'elaborazione.
...
Come avviene per la sua controparte transazionale, NextTx(), Ack() e Fail() vengono chiamati in un ciclo ridotto in un singolo thread nel processo C#. Quando non ci sono dati da emettere, è utile una breve sospensione del metodo NextTx (ad esempio 10 millisecondi), in modo da limitare il consumo di CPU.
...
Execute() viene chiamato quando al Bolt arriva una nuova tupla. FinishBatch() viene chiamato quando la transazione viene completata. Il parametro di output "parms" è riservato per l'utilizzo futuro.
...
"pluginType" viene usato per indicare il tipo di plug-in del processo C#. Se il processo C# viene eseguito in modalità di test locale (senza Java), il tipo di plug-in è "SCP\_NET\_LOCAL".
...
**StateStore** fornisce servizi metadati, generazione di frequenze monotone e coordinamento senza attesa. **StateStore** consente di creare astrazioni di concorrenza distribuite di alto livello, tra cui blocchi distribuiti, code distribuite e servizi di transazione.
...
| **tx-topolopy**        | topology-name 
                           spout-map     
                          bolt-map       | Consente di definire una topologia transazionale con il nome della topologia, la mappa di definizione degli Spout e la mappa di definizione dei Bolt.     |
| **scp-tx-spout**       | exec-name     
                          args           
                          fields         | Consente di definire uno Spout transazionale. Eseguirà l'applicazione con ***exec-name*** usando ***args***.                                              
                                          Il parametro ***fields*** corrisponde ai campi di output per lo Spout.                                                                                     |
| **scp-tx-batch-bolt**  | exec-name     
                          args           
                          fields         | Consente di definire un Bolt batch transazionale. Eseguirà l'applicazione con ***exec-name*** usando ***args***.                                          
                                          Il parametro fields corrisponde ai campi di output per il Bolt.                                                                                            |
| **scp-tx-commit-bolt** | exec-name     
                          args           
                          fields         | Consente di definire un Bolt di commit transazionale. Eseguirà l'applicazione con ***exec-name*** usando args.                                            
                                          Il parametro ***fields*** corrisponde ai campi di output per il Bolt.                                                                                      |
| **nontx-topolopy**     | topology-name 
                           spout-map     
                          bolt-map       | Consente di definire una topologia non transazionale con il nome della topologia, la mappa di definizione degli Spout e la mappa di definizione dei Bolt. |
| **scp-spout**          | exec-name     
                          args           
                          fields         
                          parameters     | Consente di definire uno Spout non transazionale. Eseguirà l'applicazione con ***exec-name*** usando ***args***.                                          
                                          Il parametro ***fields*** corrisponde ai campi di output per lo Spout.                                                                                     
                                          ***Parameters*** è facoltativo, può essere usato per specificare parametri come ad esempio "nontransactional.ack.enabled".                                 |
| **scp-bolt**           | exec-name     
                          args           
                          fields         
                          parameters     | Consente di definire un Bolt non transazionale. Eseguirà l'applicazione con ***exec-name*** usando ***args***.                                            
                                          Il parametro ***fields*** corrisponde ai campi di output per il Bolt.                                                                                      
                                          ***Parameters*** è opzionale, può essere usato per specificare parametri come ad esempio "nontransactional.ack.enabled".                                   |
...
Il raggruppamento dei campi predefinito di Storm non funziona correttamente in SCP.NET. Sul lato del proxy Java, tutti dati dei campi sono in effetti di tipo byte[] e per il raggruppamento dei campi viene usato il codice hash dell'oggetto byte[]. Il codice hash dell'oggetto byte[] corrisponde all'indirizzo dell'oggetto in memoria. Per questo motivo, il raggruppamento di due oggetti byte[] che condividono lo stesso contenuto, ma non lo stesso indirizzo, sarà errato.
...
Il sistema Storm nativo è scritto in Java e la caratteristica più importante del progetto SCP è che consente di scrivere codice C# per gestire la logica di business. SCP supporta però anche le topologie ibride, che contengono Spout e Bolt sia in linguaggio Java che in C#.
...
Qui "examples\\HybridTopology\\java\\target" è la cartella che contiene il file Jar.

#### Serializzazione e deserializzazione tra Java e C

Nella topologia ibrida è necessario supportare la serializzazione di un oggetto arbitrario sul lato Java e la successiva deserializzazione sul lato C# e viceversa. **Attualmente sono supportati solo gli Spout Java e i Bolt C#** e altri miglioramenti sono riservati a una versione successiva.
...
HelloWorld è un esempio molto semplice di utilizzo di SCP.Net. L'esempio usa una topologia non transazionale con uno Spout denominato "generator" e due Bolt denominati "splitter" e "counter". Lo Spout "generator" genererà delle frasi casuali e le emetterà verso lo "splitter". Il Bolt "splitter" dividerà le frasi in parole ed emetterà le parole verso il Bolt "counter". Il Bolt "counter" usa un dizionario per registrare il numero di occorrenze di ogni parola.
...
In sostanza, questo esempio è uguale a HelloWorld. L'unica differenza è che il codice utente viene compilato come DLL e la topologia viene inviata usando SCPHost.exe. Per na spiegazione più dettagliata, fare riferimento alla sezione "Modalità host di SCP".
