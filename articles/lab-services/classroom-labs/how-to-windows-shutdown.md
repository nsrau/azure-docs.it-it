...
> [!WARNING]
> Dopo aver configurato questa impostazione utilizzando PowerShell per modificare l'impostazione del Registro di sistema direttamente o manualmente utilizzando l'editor Criteri di gruppo, è necessario innanzitutto riavviare la macchina virtuale per applicare le impostazioni.  Inoltre, se si configura l'impostazione utilizzando il Registro di sistema, l'editor Criteri di gruppo non sempre viene aggiornato per riflettere le modifiche apportate all'impostazione del Registro di sistema; Tuttavia, l'impostazione del Registro di sistema ha ancora effetto come previsto e la sessione RDP verrà disconnessa quando è inattiva per il periodo di tempo specificato.
