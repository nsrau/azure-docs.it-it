---
title: Connettere gli ambienti a rete virtuale di lab in Azure DevTest Labs | Microsoft Docs
description: Informazioni su come connettere un ambiente (ad esempio cluster di Service Fabric) a rete virtuale del lab in Azure DevTest Labs
...
ms.locfileid: "59795529"
---
# <a name="connect-an-environment-to-your-labs-virtual-network-in-azure-devtest-labs"></a>Connettere un ambiente di rete virtuale del lab in Azure DevTest Labs
Azure DevTest Labs consente di creare macchine virtuali in un lab con facilmente [networking incorporati](devtest-lab-configure-vnet.md). Ha una notevole flessibilità grazie alla possibilità [creare un ambiente multi-VM](devtest-lab-test-env.md). Questo articolo illustra come connettere le macchine virtuali in un ambiente per la rete virtuale di lab. Uno scenario in cui si usa questa funzionalità è la configurazione di un'app a più livelli con un livello dati SQL Server che è connesso al test che consente di rete virtuale le macchine virtuali di lab nel lab per accedervi.  

## <a name="sample-environment-that-uses-lab-vnet"></a>Ambiente di esempio che usa i lab VNet
Ecco un modello di ambiente semplice che si connette la subnet del lab. In questo esempio, il `DTLSubnetId` parametro rappresenta l'ID della subnet in cui si trova il lab. Viene assegnato a: `$(LabSubnetId)`, che viene risolto automaticamente in DevTest Labs per l'ID della subnet del lab. Il **subnet** proprietà delle **interfaccia di rete** della macchina virtuale in questa definizione è impostato su `DTLSubnetId` in modo che si unisce alla stessa subnet. 
