---
title: Aumentare il grado di ricerca usando i profili di punteggio
titleSuffix: Azure Cognitive Search
description: Aumentare i punteggi di classificazione della ricerca per i risultati di Ricerca cognitiva di Azure aggiungendo profili di punteggio.
manager: nitinme
author: Brjohnstmsft
ms.author: brjohnst
...
```  
 ...
 Questa query cerca il termine 'inn' e passa la posizione attuale. Si noti che questa query `scoringParameter`include altri parametri, ad esempio . I parametri di query sono descritti in [Documenti di ricerca &#40;l'API REST di Ricerca cognitiva di Azure&#41;. ](https://docs.microsoft.com/rest/api/searchservice/Search-Documents)  
...
Un punteggio di ricerca viene calcolato in base alle proprietà statistiche dei dati e della query. Ricerca cognitiva di Azure trova i documenti che includono i `searchMode`termini di ricerca nella stringa di query (alcuni o tutti, a seconda di ), favorendo i documenti che contengono molte istanze del termine di ricerca. Il punteggio di ricerca aumenta ancora di più se il termine è raro nell'indice dei dati, ma comune all'interno del documento. I concetti di base di questo approccio al calcolo della rilevanza sono noti come [TF-IDF](https://en.wikipedia.org/wiki/Tf%E2%80%93idf) (Term Frequency-Inverse Document Frequency).  
...
|`weights`|Facoltativa. Contiene coppie nome-valore, ognuna delle quali specifica un nome di campo e un peso relativo. Il peso relativo deve essere un numero intero o a virgola mobile positivo.<br /><br /> I pesi vengono utilizzati per indicare l'importanza di un campo ricercabile rispetto a un altro.|  
...
|`magnitude`|La funzione di assegnazione di punteggio in base al valore magnitude viene usata per modificare le classificazioni in base all'intervallo di valori per un campo numerico. Alcuni degli esempi d'uso più comuni sono i seguenti.<br /><br /> -   **Classificazioni a stelle:** Modificare il punteggio in base al valore all'interno del campo "Valutazione stella". Quando due elementi sono rilevanti, viene visualizzato per primo l'elemento con la classificazione superiore.<br />-   **Margine:** Quando due documenti sono rilevanti, un rivenditore potrebbe voler mettere in evidenza i documenti con margini più elevati prima.<br />-   **Conteggi dei clic:** Per le applicazioni che tengono traccia delle azioni di click-through per prodotti o pagine, puoi usare la grandezza per aumentare gli elementi che tendono a ottenere la maggior parte del traffico.<br />-   **Il download conta:** Per le applicazioni che tengono traccia dei download, la funzione di grandezza consente di aumentare gli elementi con il maggior numero di download.|  
...
|`distance` &#124; `referencePointParameter`|Parametro da passare nelle query e da usare come posizione di riferimento. `scoringParameter` è un parametro di query. Per le descrizioni dei parametri di query, vedere [Documenti di ricerca &#40;&#41;API REST di Ricerca cognitiva di Azure.See Search Documents &#40;Azure Cognitive Search REST API&#41;](https://docs.microsoft.com/rest/api/searchservice/Search-Documents) for descriptions of query parameters.|  
...
|`tag` &#124; `tagsParameter`|Parametro da passare nelle query per specificare i tag per una particolare richiesta. `scoringParameter` è un parametro di query. Per le descrizioni dei parametri di query, vedere [Documenti di ricerca &#40;&#41;API REST di Ricerca cognitiva di Azure.See Search Documents &#40;Azure Cognitive Search REST API&#41;](https://docs.microsoft.com/rest/api/searchservice/Search-Documents) for descriptions of query parameters.|  
...
|`defaultScoringProfile`|Quando si esegue una richiesta di ricerca, se non viene specificato alcun profilo di punteggio, verrà usato il punteggio predefinito (solo [tf-idf](http://www.tfidf.com/)).<br /><br /> Un nome di profilo di punteggio predefinito può essere impostato qui, facendo sì che Ricerca cognitiva di Azure usi tale profilo quando nella richiesta di ricerca non viene fornito alcun profilo specifico.|  

##  <a name="set-interpolations"></a><a name="bkmk_interpolation"></a>Impostare le interpolazioni  
