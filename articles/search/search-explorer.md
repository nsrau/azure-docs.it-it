...
description: In questa Guida introduttiva portale di Azure usare Esplora ricerche per apprendere la sintassi delle query, testare le espressioni di query o controllare un documento di ricerca. Esplora ricerche esegue query sugli indici in ricerca cognitiva di Azure.
manager: nitinme
author: HeidiSteen
ms.author: heidist
...
**Esplora ricerche** è uno strumento di query incorporato usato per l'esecuzione di query su un indice di ricerca in Azure ricerca cognitiva. Questo strumento semplifica l'apprendimento della sintassi di query, il test di una query o di un'espressione di filtro o la conferma dei risultati di un aggiornamento dell'indice verificando che esistano contenuti più recenti.

Questa Guida introduttiva usa **immobiliare-US-sample-index** per illustrare Esplora ricerche. Le richieste vengono formulate usando l' [API REST di ricerca](https://docs.microsoft.com/rest/api/searchservice/), con risposte restituite come documenti JSON.
...
+ per questa Guida introduttiva viene utilizzato l' **indice immobiliare-US-sample** . Eseguire la procedura guidata di [**importazione dei dati**](search-import-data-portal.md) per generare l'indice dall'origine dati degli esempi incorporati.

## <a name="start-search-explorer"></a>Avviare Esplora ricerche

1. Nella [portale di Azure](https://portal.azure.com)aprire la pagina del servizio di ricerca dal dashboard o [trovare il servizio](https://ms.portal.azure.com/#blade/HubsExtension/BrowseResourceBlade/resourceType/Microsoft.Search%2FsearchServices).
...
## <a name="free-text-search"></a>Ricerca di testo disponibile

Le query in formato libero, con o senza operatori, sono utili per simulare query definite dall'utente inviate da un'app personalizzata ad Azure ricerca cognitiva. Solo i campi attribuiti come **ricercabili** nella definizione dell'indice vengono analizzati per individuare le corrispondenze. 

Si noti che quando si specificano criteri di ricerca, ad esempio termini di query o espressioni, il rango di ricerca entra in gioco. L'esempio seguente illustra una ricerca di testo libero.
...
Aggiungere **$count = true** per ottenere il numero di corrispondenze rilevate in un indice. Nel caso di una ricerca vuota, il conteggio corrisponde al numero totale di documenti nell'indice. Nel caso di una ricerca qualificata, è il numero di documenti corrispondenti all'input della query.
...
## <a name="return-next-batch-of-results"></a>Restituisce il batch successivo di risultati
...
Utilizzare il parametro [**$Filter**](search-query-odata-filter.md) quando si desidera specificare criteri precisi anziché la ricerca di testo libero. Il campo deve essere attribuito come **filtrabile** nell'indice. Questo esempio cerca gli appartamenti con più di tre camere da letto: 
...
Aggiungere [**$OrderBy**](search-query-odata-orderby.md) per ordinare i risultati in base a un altro campo oltre al Punteggio di ricerca. Il campo deve essere attribuito come **ordinabile** nell'indice. Un'espressione di esempio che è possibile usare per testarla è la seguente:
...
In questa Guida introduttiva è stato usato **Esplora ricerche** per eseguire una query su un indice usando l'API REST.
...
Per altre informazioni sulle strutture e la sintassi delle query, usare il post-o uno strumento equivalente per creare espressioni di query che sfruttano più parti dell'API. L' [API REST di ricerca](https://docs.microsoft.com/rest/api/searchservice/) è particolarmente utile per l'apprendimento e l'esplorazione.

> [!div class="nextstepaction"]
> [Creare una query di base in post](search-query-simple-examples.md)