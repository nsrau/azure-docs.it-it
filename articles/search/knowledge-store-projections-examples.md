...
Utilizzando questo insieme di competenze, con la relativa `knowledgeStore` null come base, il primo esempio riempie l'oggetto `knowledgeStore`, configurato con proiezioni che consentono di creare strutture di dati tabulari che è possibile utilizzare in altri scenari. 
...
Si supponga di voler creare un dashboard in cui è possibile visualizzare le frasi chiave estratte dai documenti come un cloud di Word. Per creare la struttura di dati corretta, è possibile aggiungere un'abilità di shaper al skillt per creare una forma personalizzata con i dettagli e le frasi chiave specifici del documento. La forma personalizzata verrà chiamata `pbiShape` sul nodo radice `document`.
...
In questo esempio, la forma personalizzata combina i metadati e le entità identificate e le frasi chiave. L'oggetto viene chiamato `pbiShape` ed è associato a un elemento padre nella `/document`. 
...
>
> Si notino gli ultimi due nodi, `KeyPhrases` e `Entities`. Queste vengono incapsulate in un oggetto JSON valido con il `sourceContext`. Questa operazione è necessaria come `keyphrases` e `entities` sono arricchimenti sulle primitive e devono essere convertiti in JSON valido prima di poter essere proiettati.
>
...
Ora che sono disponibili tutti i dati necessari per proiettare le tabelle, aggiornare l'oggetto knowledgeStore con le definizioni della tabella. In questo esempio sono disponibili tre tabelle, definite impostando le proprietà `tableName`, `source` e `generatedKeyName`.
...
1. Impostare la proprietà ```storageConnectionString``` su una stringa di connessione dell'account di archiviazione per utilizzo generico V2 valida.  
...
Per generare una proiezione di file, si usa la matrice `files` nell'oggetto Projection. Questo esempio proietta tutte le immagini estratte dal documento in un contenitore denominato `samplefile`.
