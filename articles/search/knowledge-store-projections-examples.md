...
Per creare una proiezione, è necessario modellare i dati usando un' [abilità di shaper](cognitive-search-skill-shaper.md) per creare un oggetto personalizzato o usare la sintassi di shaping in linea all'interno di una definizione di proiezione. 

Una forma dati contiene tutti i dati che si intende proiettare, formati come una gerarchia di nodi. Questo articolo illustra diverse tecniche per il data shaping, in modo che possa essere proiettato in strutture fisiche che contribuiscono alla creazione di report, analisi o elaborazione downstream. 
...
Se si ha familiarità con le [proiezioni](knowledge-store-projection-overview.md), si noterà che esistono tre tipi:
...
Utilizzando questo insieme di competenze, con il `knowledgeStore` relativo valore null come base, nel primo esempio viene compilato `knowledgeStore` l'oggetto, configurato con proiezioni che consentono di creare strutture di dati tabulari che è possibile utilizzare in altri scenari. 
...
Si supponga di voler creare un dashboard in cui è possibile visualizzare le frasi chiave estratte dai documenti come un cloud di Word. Per creare la struttura di dati corretta, è possibile aggiungere un'abilità di shaper al skillt per creare una forma personalizzata con i dettagli e le frasi chiave specifici del documento. La forma personalizzata verrà chiamata `pbiShape` sul nodo `document` radice.
...
Creare una forma personalizzata che è possibile proiettare nell'archivio tabelle. Senza una forma personalizzata, una proiezione può fare riferimento solo a un singolo nodo (una proiezione per output). La creazione di una forma personalizzata consente di aggregare vari elementi in un nuovo intero logico che può essere proiettato come una singola tabella oppure sezionato e distribuito in una raccolta di tabelle. 

In questo esempio, la forma personalizzata combina i metadati e le entità identificate e le frasi chiave. L'oggetto viene chiamato `pbiShape` ed è associato a un `/document`elemento padre. 
...
>
> Si notino gli ultimi due `KeyPhrases` nodi `Entities`, e. Questi vengono inclusi in un oggetto JSON valido con `sourceContext`. Questa operazione è necessaria `keyphrases` perché `entities` e sono arricchimenti sulle primitive e devono essere convertiti in JSON valido prima di poter essere proiettati.
>
...
Prima di passare all'esempio successivo, è possibile rivedere gli aspetti della proiezione della tabella per comprendere i meccanismi di sezionamento e di correlazione dei dati.
...
Il sezionamento genera in modo implicito una relazione tra le tabelle padre e figlio ```generatedKeyName``` , usando nella tabella padre per creare una colonna con lo stesso nome nella tabella figlio. 

### <a name="naming-relationships"></a>Relazioni di denominazione

Le ```generatedKeyName``` proprietà ```referenceKeyName``` e vengono usate per correlare i dati tra tabelle o persino tra tipi di proiezione. Ogni riga nella tabella/proiezione figlio ha una proprietà che punta all'elemento padre. Il nome della colonna o della proprietà nell'elemento figlio è ```referenceKeyName``` dall'elemento padre. Quando non ```referenceKeyName``` viene specificato, il servizio ne imposta il valore predefinito su ```generatedKeyName``` dall'elemento padre. 

Power BI si basa su queste chiavi generate per individuare le relazioni all'interno delle tabelle. Se è necessario che la colonna della tabella figlio sia denominata in modo diverso ```referenceKeyName``` , impostare la proprietà nella tabella padre. Un esempio consiste nell'impostare l' ```generatedKeyName``` ID As nella tabella pbiDocument e ```referenceKeyName``` come DocumentID. Ciò comporterebbe la colonna nelle tabelle pbiEntities e pbiKeyPhrases contenente l'ID documento denominato DocumentID.
...
La proprietà proiezioni è una matrice. Per questo esempio viene aggiunta una nuova istanza di proiezione alla matrice, in cui la definizione knowledgeStore contiene proiezioni inline. Quando si usano le proiezioni inline, è possibile omettere la capacità dell'shaper.
...
Per generare una proiezione di file, si usa `files` la matrice nell'oggetto di proiezione. Questo esempio proietta tutte le immagini estratte dal documento in un contenitore `samplefile`denominato.
...
In questo esempio, gli aggiornamenti al skillt includono le seguenti modifiche:
...
Dall'oggetto crossProjection consolidato, è possibile sezionare l'oggetto in più tabelle, acquisire l'output OCR come BLOB e quindi salvare l'immagine come file (anche nell'archivio BLOB).
...
+ Impossibile definire il data shaping in JSON valido. Quando le stringhe sono arricchite, ad esempio `merged_content` arricchite con frasi chiave, la proprietà arricchita viene rappresentata come elemento figlio di all' `merged_content` interno dell'albero di arricchimento. La rappresentazione predefinita non è JSON ben formato. Quindi, in fase di proiezione, assicurarsi di trasformare l'arricchimento in un oggetto JSON valido con un nome e un valore.
