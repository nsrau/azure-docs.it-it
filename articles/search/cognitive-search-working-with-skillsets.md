---
title: Uso di skillsets-ricerca di Azure
description: Skillsets sono i punti in cui si crea una pipeline di arricchimento di intelligenza artificiale in ricerca cognitiva, si comprendono alcuni concetti e il modo in cui il lavoro di skillsets consente di creare skillsets semplici o complessi
manager: eladz
author: vkurpad
services: search
ms.service: search
...
|Contesto|Input|Forma dell'input|Chiamata di competenze|
|---|---|---|---|
...
Il `sourceContext` viene usato solo nelle [proiezioni](knowledge-store-projection-overview.md)e nelle [competenze del shaper](cognitive-search-skill-shaper.md) . Viene utilizzato per costruire oggetti annidati a più livelli. `sourceContext` Consente di creare un oggetto di tipo gerarchico e anonimo, che richiederebbe più competenze se si utilizzasse solo il contesto. L' `sourceContext` uso di è illustrato nella sezione successiva.
...
### <a name="skill-1-split-skill"></a>#1 competenze: Divisione skill 
...
### <a name="skill-2-language-detection"></a>Rilevamento di competenze #2 lingua
 ...
 ### <a name="skill-3-key-phrases-skill"></a>#3 competenze: Abilità frasi chiave 

Dato che il contesto ```/document/reviews_text/pages/*``` delle frasi chiave verrà richiamato una volta per ogni elemento `pages` della raccolta. L'output dell'abilità sarà un nodo sotto l'elemento della pagina associato. 
...
Con il `tableprojection` nodo definito `outputs` nella sezione precedente, è ora possibile usare la funzionalità `tableprojection` di sezionamento per proiettare parti del nodo in tabelle diverse:
