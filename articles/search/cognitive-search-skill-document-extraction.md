...
Questo oggetto di riferimento file può essere generato in uno dei tre modi seguenti:
 ...
 - Impostazione del `imageAction` parametro nella definizione dell'indicizzatore su un valore diverso da `none`.  In questo modo viene creata una matrice di immagini che segue la convenzione necessaria per l'input di questa competenza se viene `/document/normalized_images/*`passato singolarmente (ad esempio).

 - La presenza di un'abilità personalizzata restituisce un oggetto JSON definito esattamente come sopra.  Il `$type` parametro deve essere impostato su exactly `file` e `data` il parametro deve essere costituito dai dati della matrice di byte codificati 64 di base del contenuto del file.
