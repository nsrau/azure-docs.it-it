...
Impostare lo stato su **Sì**, selezionare l'account di archiviazione di Azure e scegliere i dati che si vuole copiare: log, metriche o entrambi. È consigliabile copiare sia i log che le metriche.
È possibile impostare i criteri di conservazione per i dati su un valore compreso tra 1 e 365 giorni. Se non si vogliono conservare i dati a tempo indeterminato, impostare Conservazione (giorni) su 0.
...
| operationVersion |string |"2016-09-01" |api-version usata |
...
| Query |stringa |"?search=AzureSearch&$count=true&api-version=2016-09-01" |Parametri della query |
...
I BLOB delle metriche contengono i valori aggregati per il servizio di ricerca.
In ogni file è presente un oggetto radice denominato **record** contenente una matrice di oggetti di metrica. Questo oggetto radice contiene metriche per ogni minuto per il quale i dati erano disponibili.
...
> [!IMPORTANT]
> La limitazione si verifica quando viene inviato un numero eccessivo di query, esaurendo la capacità delle risorse di provisioning del servizio. È possibile aggiungere più repliche al servizio.
>
>
