...
description: Usare l'API REST e Postman per creare un archivio conoscenze di Ricerca di Azure per salvare in modo permanente gli arricchimenti da una pipeline di ricerca cognitiva.
author: lobrien
services: search
ms.service: search
ms.topic: tutorial
...
ms.locfileid: "72244473"
---
# <a name="create-an-azure-search-knowledge-store-by-using-rest"></a>Creare un archivio conoscenze di Ricerca di Azure con REST
...
- Consigliato: Ottenere l'[app desktop Postman](https://www.getpostman.com/) per inviare richieste a Ricerca di Azure. È possibile usare l'API REST con qualsiasi strumento che può gestire richieste e risposte HTTP. Postman è una scelta valida per esplorare le API REST e verrà usato in questo articolo. Inoltre, il [codice sorgente](https://github.com/Azure-Samples/azure-search-postman-samples/blob/master/knowledge-store/KnowledgeStore.postman_collection.json) di questo articolo include una raccolta di richieste Postman. 
...
|-------------|-----------------|
| `admin-key` | Nella scheda **Keys** del servizio Ricerca di Azure.  |
...
| `search-service-name` | Nome principale del servizio Ricerca di Azure. L'URL è `https://{{search-service-name}}.search.windows.net`. | 
| `skillset-name` | Lasciare **hotel-reviews-ss**. | 
| `storage-account-name` | Nome principale dell'account di archiviazione. | 
...
- **Richiesta PUT per creare l'indice**: Questo indice contiene i dati usati e restituiti da Ricerca di Azure.
- **Richiesta POST per creare l'origine dati**: Questa origine dati connette il comportamento di Ricerca di Azure ai dati e all'account di archiviazione dell'archivio conoscenze. 
