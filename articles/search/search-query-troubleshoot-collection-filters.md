...
> [!NOTE]
> Azure ricerca cognitiva non supporta gli operatori `lt` / `le` / `gt` / `ge` per le stringhe, sia all'interno che all'esterno di un'espressione lambda.
...
Il tipo `Edm.Boolean` supporta solo gli `eq` operatori `ne` e. Di conseguenza, non è molto sensato consentire la combinazione di tali clausole che controllano la stessa variabile `and` / `or` di intervallo con perché questo porterebbe sempre a tautologie o contraddizioni.
...
I valori di `Edm.GeographyPoint` tipo in una raccolta non possono essere confrontati direttamente tra loro. Ma devono essere usati come parametri per le `geo.distance` funzioni e. `geo.intersects` La `geo.distance` funzione deve essere a sua volta confrontata con un valore di distanza utilizzando uno `lt`degli `le`operatori `gt`di confronto `ge`,, o. Queste regole si applicano anche ai campi EDM. GeographyPoint non di raccolta.
...
- Nel corpo di un `any`, le espressioni geospaziali possono essere combinate `or`usando. Nel corpo di un oggetto `all`, le espressioni di questo tipo possono `and`essere combinate usando.
...
- Regole per `all`:
  ...
  - Le espressioni di confronto semplici `ne`che `lt`coinvolgono `le`, `gt`,, `ge` o possono essere combinate con `and` / `or`. Ad esempio:
...
In primo luogo, `search.ismatch` le `search.ismatchscoring` funzioni e non sono supportate nelle espressioni lambda. Per ulteriori informazioni, vedere informazioni sui [filtri di raccolta OData in Azure ricerca cognitiva](search-query-understand-collection-filters.md).
