...
Il segmento video seguente consente di eseguire rapidamente una spiegazione del funzionamento dell'elaborazione del testo in Azure ricerca cognitiva.
...
3. Facoltativamente, anziché una proprietà dell' **analizzatore** , è possibile impostare diversi analizzatori per l'indicizzazione e l'esecuzione di query usando i parametri dei campi **indexAnalyzer** e **searchAnalyzer** . Usare analizzatori diversi per il recupero e la preparazione dei dati se una di tali attività necessitasse di una specifica trasformazione non richiesta dall'altro.
