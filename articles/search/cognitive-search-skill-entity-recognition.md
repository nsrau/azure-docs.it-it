...
description: Estrarre diversi tipi di entità dal testo in una pipeline di arricchimento in Ricerca cognitiva di Azure.Extract different types of entities from text in an enrichment pipeline in Azure Cognitive Search.
manager: nitinme
author: luiscabrer
ms.author: luisca
...
La dimensione massima di un record deve essere di [`String.Length`](https://docs.microsoft.com/dotnet/api/system.string.length)50.000 caratteri misurata da . Se è necessario suddividere i dati prima di inviarli all'estrattore di frasi chiave, è possibile usare la competenza [Divisione del testo](cognitive-search-skill-textsplit.md).
...
|defaultLanguageCode |    Codice lingua del testo di input. Sono supportate le `ar, cs, da, de, en, es, fi, fr, hu, it, ja, ko, nl, no, pl, pt-BR, pt-PT, ru, sv, tr, zh-hans`seguenti lingue: . Non tutte le categorie di entità sono supportate per tutte le lingue. vedere la nota qui sotto.|
|minimumPrecision | Immettere un valore compreso tra 0 e 1. Se il punteggio `namedEntities` di confidenza (nell'output) è inferiore a questo valore, l'entità non viene restituita. Il valore predefinito è 0. |
|includeTypelessEntities | Impostare `true` su se si desidera riconoscere le entità note che non rientrano nelle categorie correnti. Le entità riconosciute vengono `entities` restituite nel campo di output complesso. Ad esempio, "Windows 10" è un'entità nota (un prodotto), ma poiché "Products" non è una categoria supportata, questa entità verrà inclusa nel campo di output delle entità. Il valore predefinito è `false` |
...
> [!NOTE]
> non tutte le categorie di entità sono supportate per tutte le lingue. I `"Person"` `"Location"`tipi `"Organization"` di categoria , , e entity sono supportati per l'elenco completo delle lingue sopra riportate. Solo _de_, _en_, _es_, _fr_ `"Quantity"`e `"Datetime"` `"URL"` _zh-hans_ supportano l'estrazione di tipi , , e `"Email"` . Per altre informazioni, vedere [Supporto di lingue e aree geografiche per l'API Analisi del testo.](https://docs.microsoft.com/azure/cognitive-services/text-analytics/language-support)  
...
| namedEntities | Una matrice di tipi complessi, che contiene i campi seguenti: <ul><li>category</li> <li>value (il nome effettivo dell'entità)</li><li>offset (percorso in cui è stato trovato nel testo)</li><li>fiducia (valore più alto significa che è più di essere un'entità reale)</li></ul> |
