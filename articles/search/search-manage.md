<properties 
	pageTitle="Amministrazione del servizio per Ricerca di Azure nel portale di Azure" 
...
<a id="admin-rights"></a>
## Diritti di amministratore in Ricerca di Azure
...
<a id="sys-info"></a>
## Accesso a Ricerca di Azure e informazioni sul sistema
...
- Usando [PowerShell](search-manage-powershell.md) o l'[API REST di gestione](https://msdn.microsoft.com/library/azure/dn832684.aspx) per [ottenere le proprietà del servizio](https://msdn.microsoft.com/library/azure/dn832694.aspx) oppure lo stato nell'uso della risorsa dell'indice.
...
Ruolo|Attività
---|---
Proprietario|Creare o eliminare il servizio o qualsiasi oggetto nel servizio, tra cui chiavi API, indici, indicizzatori, origini dati degli indicizzatori e pianificazioni degli indicizzatori.<p>Visualizzare lo stato di servizio, inclusi i conteggi e le dimensioni di archiviazione.<p>Aggiungere o eliminare l'appartenenza al ruolo. Solo un proprietario può gestire l'appartenenza al ruolo.<p>Gli amministratori della sottoscrizione e i proprietari del servizio appartengono automaticamente al ruolo Proprietario.
...
1. Nel dashboard del servizio fare clic sull'icona Accesso per aprire il pannello Utenti con effetto scorrimento. ![][7]
...
L'aumento delle query al secondo o il raggiungimento della disponibilità elevata si ottengono mediante l'aggiunta di repliche. Ogni replica presenta una copia di un indice, quindi l'aggiunta di un'altra replica si traduce in un indice aggiuntivo disponibile per la gestione delle richieste di query del servizio. Per la disponibilità elevata, sono necessarie almeno 3 repliche. Per i dettagli, vedere [Pianificazione della capacità](search-capacity-planning.md).
...
## Procedure consigliate su scalabilità e distribuzione (video)
