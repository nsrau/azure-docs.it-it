...
Non è possibile `MoreLikeThis` utilizzare sui sottocampi disponibili per la ricerca in un [tipo complesso](search-howto-complex-data-types.md).
...
La query seguente trova i documenti i cui campi di descrizione sono più simili al campo del documento di origine, come `moreLikeThis` specificato dal parametro:
...
`MoreLikeThis`può essere combinato con altri parametri comuni di query `$filter`come. Ad esempio, la query può essere limitata solo a Alberghi la cui categoria è' budget ' e dove la classificazione è superiore a 3,5:
...
Il `$top` selettore può essere utilizzato per limitare il numero di risultati che devono `MoreLikeThis` essere restituiti in una query. Inoltre, è possibile selezionare i campi `$select`con. I primi tre hotel sono selezionati insieme al relativo ID, nome e classificazione: 
