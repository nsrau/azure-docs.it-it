...
Se prima della registrazione di Monitoraggio di Azure per le macchine virtuali non era abilitata la [raccolta dei contatori delle prestazioni](monitoring-vminsights-onboard.md?toc=/azure/azure-monitor/toc.json#performance-counters-enabled) per le macchine virtuali Windows o Linux dell'area di lavoro, è necessario disabilitare tali regole seguendo i passaggi indicati [qui](../log-analytics/log-analytics-data-sources-performance-counters.md?toc=/azure/azure-monitor/toc.json#configuring-performance-counters) per Windows e per Linux.
