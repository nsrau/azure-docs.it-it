<properties pageTitle="Gestione di conflitti di scrittura con concorrenza ottimistica (Windows Store) | Mobile Dev Center" writer="wesmc" description="Learn how to handle database write conflicts on both the server and in your Windows Store application." documentationCenter="windows" authors="wesmc7777" manager="dwrede" editor="" services=""/>
...
+ Pacchetto Azure Mobile Services NuGet 1.1.5 o versione successiva. Per ottenere l'ultima versione, eseguire la procedura seguente:
	...
	2. Espandere **Online** e fare clic su **Microsoft and .NET**. Nella casella di testo di ricerca immettere **WindowsAzure.MobileServices.WinJS**. Fare clic su **Installa** accanto al pacchetto NuGet **Azure Mobile Services for WinJS**.
...
4. Nel file default.js aggiungere il gestore eventi seguente per l'evento `keydown` in modo che l'elemento venga aggiornato premendo **INVIO**.
...
7. Nell'istanza 1 dell'app aggiornare il testo dell'ultimo elemento in **Test Write 1**, quindi premere **INVIO** per aggiornare il database. Nella schermata seguente viene visualizzato un esempio.
...
8. A questo punto l'ultimo elemento dell'istanza 2 dell'app contiene una versione precedente dell'elemento. In tale istanza dell'app, immettere **Test Write 2** per la proprietà `text` dell'ultimo elemento e premere **INVIO** per aggiornare il database con una proprietà `_version` precedente.
