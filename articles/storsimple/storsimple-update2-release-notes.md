...
* **Miglioramenti dell'aggiornamento** : nell'aggiornamento 1.2 e versioni precedenti, StorSimple serie 8000 veniva aggiornato tramite due canali: Aggiornamento di Windows per il clustering, iSCSI e così via e Microsoft Update per i file binari e del firmware.
...
* **Aggiornamenti del firmware** : sono inclusi gli aggiornamenti del firmware seguenti:
  ...
  * Solo unità SSD (Nessun aggiornamento HDD): XMGG, XGEG, KZ50, F6C2 e VR08
...
| --- | --- | --- | --- | --- |
| 1 |Interfacce di rete |Dopo aver eseguito l'aggiornamento 1, il servizio StorSimple Manager segnalava che le porte Data2 e Data3 restituivano un errore su un controller. Il problema è stato risolto. |Yes |No |
| 2 |Aggiornamenti |Dopo aver eseguito l'aggiornamento 1, si sono attivati degli avvisi acustici nel portale di Azure classico in più dispositivi. Il problema è stato risolto. |Yes |No |
| 3 |Autenticazione Openstack |Quando si usa Openstack come provider di servizi cloud, potrebbe venire visualizzato un errore che informa che la stringa di autenticazione cloud è troppo lunga. Questo problema è stato risolto. |Yes |No |
