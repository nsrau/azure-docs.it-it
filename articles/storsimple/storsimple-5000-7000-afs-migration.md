...
Qui sono disponibili i prerequisiti per la migrazione del dispositivo legacy 5000 o 7000 Series per Sincronizzazione file di Azure. Prima di iniziare, verificare di avere:
...
1.  Eseguire questi passaggi nello stesso host di Windows Server in cui i volumi StorSimple vengono montati o usano un sistema diverso. 
    ...
    - [Installare l'agente di sincronizzazione file di Azure](https://docs.microsoft.com/azure/storage/files/storage-sync-files-deployment-guide#install-the-azure-file-sync-agent).
...
2.  Attendere fino a quando non è stata completata la sincronizzazione del file server. Per ogni server in un gruppo di sincronizzazione specificato, assicurarsi che:
    ...
    Per ulteriori informazioni sul completamento della sincronizzazione del server, vedere la pagina relativa alla [risoluzione dei problemi sincronizzazione file di Azure](https://docs.microsoft.com/azure/storage/files/storage-sync-files-troubleshoot?tabs=portal1%2Cportal#how-do-i-know-if-my-servers-are-in-sync-with-each-other). La sincronizzazione può richiedere diverse ore a giorni, a seconda delle dimensioni dei dati e della larghezza di banda. Una volta completata la sincronizzazione, tutti i dati sono al sicuro nella condivisione File di Azure. 
