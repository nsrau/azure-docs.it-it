<properties 
   pageTitle="Amministrazione del dispositivo tramite Windows PowerShell per StorSimple"
   description="Leggere le informazioni su come gestire il dispositivo StorSimple tramite Windows PowerShell per StorSimple"
   services="storsimple"
...
In questo articolo vengono descritte le funzionalità di Windows PowerShell per StorSimple tra cui come è possibile connettersi a questa interfaccia e i collegamenti alle procedure dettagliate dei flussi di lavoro che possono essere eseguiti utilizzando questa interfaccia. I flussi di lavoro includono come registrare il dispositivo, configurare l'interfaccia di rete sul dispositivo, installare gli aggiornamenti che richiedono che il dispositivo sia in modalità di manutenzione, modificare lo stato del dispositivo e risolvere eventuali problemi che possono verificarsi.

Dopo aver letto l'articolo, l'utente sarà in grado di:


- Connettersi al dispositivo StorSimple tramite Windows PowerShell per StorSimple

- Amministrare il dispositivo StorSimple tramite Windows PowerShell per StorSimple
...
- [Connessione alla console seriale del dispositivo StorSimple](#connect-to-windows-powershell-for-storsimple-via-device-serial-console)
...
### Informazioni sulla console seriale
...
1. **Modifica lingua** Questa opzione consente di modificare la lingua di visualizzazione nell'interfaccia Windows PowerShell. Le lingue supportate sono inglese, giapponese, russo, francese, sudcoreano, spagnolo, italiano, tedesco, cinese e portoghese (Brasile).
...
1. Nella finestra di dialogo Riconfigurazione PuTTY, selezionare **Tastiera** nel riquadro **Categoria**.

1. Assicurarsi che siano selezionate le opzioni seguenti (queste sono le impostazioni predefinite quando si avvia una nuova sessione).
...
1. Nel riquadro **Categoria**, selezionare**Traduzione**.

1. Nella casella di riepilogo **Set di caratteri remoti**, selezionare **UTF-8**.

1. Sotto **Gestione dei caratteri lineette**, selezionare **Utilizza punti di codice lineette Unicode**. Nella figura seguente vengono illustrate le selezioni di PuTTY corrette.
...
È ora possibile utilizzare PuTTY per la connessione alla console seriale del dispositivo attendendosi ai passaggi seguenti:
...
Nella tabella seguente viene illustrato un riepilogo di tutte le attività di gestione comuni e i flussi di lavoro complessi che possono essere eseguiti all'interno dell'interfaccia di Windows PowerShell del dispositivo StorSimple. Per ulteriori informazioni su ciascun flusso di lavoro, fare clic sulla voce appropriata nella tabella.
...
- Per la guida completa, è possibile utilizzare il parametro –Full e, per gli esempi, il parametro `–Examples`.
