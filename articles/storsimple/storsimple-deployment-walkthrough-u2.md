<properties 
   pageTitle="Distribuire il dispositivo StorSimple (Aggiornamento 2) | Microsoft Azure"
...
| [Passaggio 7: Montare, inizializzare e formattare un volume](#step-7-mount-initialize-and-format-a-volume)</br>[ Facoltativo: Configurare MPIO](storsimple-configure-mpio-windows-server.md) | Connettere i server all'archiviazione iSCSI fornita dal dispositivo. Se si desidera configurare MPIO per assicurarsi che i server siano in grado di tollerare errori di collegamento, rete e interfaccia. |
...
> [AZURE.IMPORTANT] StorSimple Manager consente di creare volumi con thin provisioning e con provisioning completo. Tuttavia, non è possibile creare volumi con provisioning parziale.
