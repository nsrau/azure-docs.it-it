...
| Metis |N/D |Logico |Shared |N/D |Visualizza lo stato delle batterie: se devono essere messe in carica e se si stanno avvicinando alla fine del ciclo di vita. |
| Cluster |N/D |Logico |Shared |N/D |Visualizza lo stato del cluster creato tra i due moduli controller integrato. |
| Nodo del cluster |N/D |Logico |Shared |N/D |Indica lo stato del controller come parte del cluster. |
...
| Spazio dati unità disco rigido |N/D |Logico |Shared |N/D |Lo spazio di archiviazione utilizzato per i dati nel pool di archiviazione di unità disco rigido (HDD). |
| Spazio di gestione di unità disco rigido |N/D |Logico |Shared |N/D |Lo spazio riservato nel pool di archiviazione di unità disco rigido per le attività di gestione. |
| Spazio quorum unità disco rigido |N/D |Logico |Shared |N/D |Lo spazio riservato nel pool di archiviazione di unità disco rigido per il quorum del cluster. |
| Spazio sostituzione unità disco rigido |N/D |Logico |Shared |N/D |Lo spazio riservato nel pool di archiviazione di unità disco rigido per la sostituzione del controller. |
| Spazio dati unità SSD |N/D |Logico |Shared |N/D |Lo spazio di archiviazione utilizzato per i dati nel pool di archiviazione di unità SSD. |
| Spazio NVRAM SSD |N/D |Logico |Shared |N/D |Lo spazio di archiviazione nel pool di archiviazione di unità SSD dedicato per la logica NVRAM. |
| Pool di archiviazione di unità disco rigido |N/D |Logico |Shared |N/D |Visualizza lo stato del pool di archiviazione logica creato dal dispositivo HDD. |
| Pool di archiviazione di unità SSD |N/D |Logico |Shared |N/D |Visualizza lo stato del pool di archiviazione logica creato dal dispositivo SSD. |
...
| Alimentazione [0-1] |PCM |Fisico |Shared |Sì |Viene visualizzata una riga per ogni alimentatore nei due PCM presenti sulla parte posteriore del dispositivo. |
| Raffreddamento [0-1] |PCM |Fisico |Shared |Sì |Viene visualizzata una riga per ognuno delle quattro ventole di raffreddamento che risiedono nei due PCM. |
| Archiviazione locale [HDD] |N/D |Logico |Shared |N/D |Visualizza lo stato del pool di archiviazione logica creato dal dispositivo HDD. |
