...
Se si desidera anche creare un nuovo spazio dei nomi per l'hub di notifica, vedere [Gestire il bus di servizio con PowerShell](../service-bus/service-bus-powershell-how-to-provision.md).

La gestione degli hub di notifica non è supportata direttamente tramite i cmdlet inclusi con Azure PowerShell. L'approccio migliore da PowerShell deve fare riferimento all'assembly Microsoft.ServiceBus.dll. L'assembly viene distribuito con il [pacchetto NuGet del Bus di servizio](http://www.nuget.org/packages/WindowsAzure.ServiceBus/).
...
La gestione degli hub di notifica di Azure non è ancora inclusa con i cmdlet PowerShell di Azure PowerShell. Per eseguire il provisioning di hub di notifica e altre entità di servizio non esposte tramite i cmdlet esistenti, è possibile usare il client .NET per Bus di servizio nel [pacchetto NuGet del bus di servizio](http://www.nuget.org/packages/WindowsAzure.ServiceBus/).

Innanzitutto, assicurarsi che lo script sia in grado di individuare l'assembly **Microsoft.ServiceBus.dll**, che viene installato con il pacchetto NuGet in un progetto Visual Studio. Per essere flessibile, lo script esegue i passaggi seguenti:
...
Per eseguire il provisioning di hub di notifica e altre entità di Bus di servizio, creare un'istanza della classe [NamespaceManager](http://msdn.microsoft.com/library/microsoft.servicebus.namespacemanager.aspx) dall’SDK.
...
Per eseguire il provisioning di un nuovo hub di notifica, utilizzare [API .NET per Bus di servizio]. In questo articolo vengono trattati solo gli hub di notifica. Per utilizzare altre entità del bus di servizio, vedere [Gestire Bus di servizio con PowerShell](../service-bus/service-bus-powershell-how-to-provision.md).

In questa parte dello script verranno impostate quattro variabili locali.

1. `$Namespace` : impostare il nome dello spazio dei nomi in cui si desidera creare un hub di notifica.
2. `$Path` : impostare il percorso sul nome del nuovo hub di notifica. Ad esempio, "MyHub".    
3. `$WnsPackageSid` : impostare il SID di pacchetto per Applicazione Windows dal [Windows Developer Center](http://go.microsoft.com/fwlink/p/?linkid=266582&clcid=0x409).
4. `$WnsSecretkey` : impostare il SID di pacchetto per Applicazione Windows dal [Windows Dev Center](http://go.microsoft.com/fwlink/p/?linkid=266582&clcid=0x409).

Queste variabili vengono utilizzate per connettersi allo spazio dei nomi del Bus di servizio e creare un nuovo Hub di notifica configurato per gestire le notifiche dei servizi di notifica di Windows (WNS) con credenziali WNS per un’applicazione Windows. Per informazioni su come ottenere il SID di pacchetto e la chiave segreta, vedere l’esercitazione [Introduzione agli Hub di notifica](notification-hubs-windows-store-dotnet-get-started.md).
...
[API .NET per Bus di servizio]: https://msdn.microsoft.com/library/microsoft.servicebus.aspx
