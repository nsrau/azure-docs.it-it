...
Apple ora richiede che gli sviluppatori identifichino le notifiche come avvisi o notifiche in background `apns-push-type` tramite la nuova intestazione nell'API APNs. Secondo la [documentazione di Apple](https://developer.apple.com/documentation/usernotifications/setting_up_a_remote_notification_server/sending_notification_requests_to_apns): "il valore di questa intestazione deve riflettere accuratamente il contenuto del payload della notifica. Se si verifica una mancata corrispondenza o se l'intestazione non è presente nei sistemi richiesti, APNs può restituire un errore, ritardare il recapito della notifica o eliminarlo completamente. "
...
Un'altra modifica secondaria, ma una che richiede una modifica all'applicazione back-end che invia le notifiche, è il requisito che per `apns-priority` le notifiche in background l'intestazione deve essere impostata su 5. Molte applicazioni impostano l' `apns-priority` intestazione su 10 (che indica il recapito immediato) o non lo impostano e ottengono il valore predefinito (che è anche 10).
