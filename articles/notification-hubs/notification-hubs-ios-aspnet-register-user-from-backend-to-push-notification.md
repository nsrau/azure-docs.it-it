...
description: Informazioni su come chiedere la registrazione per le notifiche push in un'app per iOS con Hub di notifica di Azure quando la registrazione viene eseguita tramite l'API Web ASP.NET.
...
1. Nel file MainStoryboard_iPhone.storyboard aggiungere i componenti seguenti dalla libreria di oggetti:
   ...
   * **Pulsante**: "Login"
     ...
     ![Screenshot dell'app MainStoryboard_iPhone. Storyboard con i componenti aggiunti.][0]
...
2. Nell'assistente dell'editor creare outlet per tutti i controlli con opzioni e chiamarli, collegare i campi di testo per mezzo dell'elemento View Controller (delegato), quindi creare una connessione di tipo **Action** per il pulsante **login**.
    ...
    ![Screenshot dell'editor di assistente nell'app MainStoryboard_iPhone. Storyboard][1]
...
5. In PushToUserAppDelegate.h aggiungere il singleton della proprietà seguente:
    ...
    ```
6. Nel metodo `didFinishLaunchingWithOptions` in PushToUserAppDelegate.m aggiungere il codice seguente:
...
7. In PushToUserAppDelegate.m implementare il metodo `didRegisterForRemoteNotificationsWithDeviceToken` nel delegato dell'app e aggiungere il codice seguente:
   ...
   > [!NOTE]
   > A questo punto, il metodo non dovrebbe contenere altro codice. Se si ha già una chiamata al `registerNativeWithDeviceToken` metodo che è stato aggiunto al termine dell'esercitazione [inviare notifiche push alle app iOS usando hub di notifica di Azure](ios-sdk-get-started.md) , è necessario impostare come commento o rimuovere la chiamata.

8. Nel file `PushToUserAppDelegate.m` aggiungere il metodo del gestore seguente:
...
9. Aprire il file `PushToUserViewController.m` e ripristinare l'implementazione seguente della tastiera:
...
10. Nel metodo `viewDidLoad` nel file `PushToUserViewController.m` inizializzare l'etichetta `installationId` come indicato di seguito:
...
[Introduzione ad Hub di notifica]: ios-sdk-get-started.md
