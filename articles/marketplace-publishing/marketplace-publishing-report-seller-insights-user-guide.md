...
Ricorrere all'amministratore di Seller Insights Portal per aggiungere, modificare ed eliminare utenti.

Un utente di Seller Insights Portal può visualizzare tutti i dati disponibili per l'organizzazione.

Gli amministratori e gli utenti devono eseguire l'autenticazione con un account Microsoft valido (LiveID http://signup.live.com) o un account della società. Assicurarsi che il nuovo ID account sia associato a una cassetta postale monitorata.

### <a name="invitation-to-activate-your-account"></a>Invito ad attivare l'account

Idealmente, prima di accedere a Seller Insights Portal, l'amministratore deve identificare gli account delle persone da configurare come utenti. Gli account possono essere account Microsoft o account della società.
...
>[!NOTE]
>Se l'indirizzo di posta elettronica dell'amministratore è indicato come account Microsoft e non è stato creato un account Microsoft associato all'indirizzo da cui è stato ricevuto l'invito, sarà necessario crearne uno prima di poter effettuare l'accesso. Per creare l'account, andare all'indirizzo https://signup.live.command.


### <a name="sign-in-to-the-seller-insights-portal"></a>Accedere a Seller Insights Portal

1. Accedere a Seller Insights Portal facendo clic sull'URL contenuto nel messaggio di posta elettronica ricevuto.
...
5. Assicurarsi che gli utenti abbiano l'URL per Seller Insights Portal.

![Pagina di accesso][6]

## <a name="sign-in"></a>Accesso

L'amministratore deve inviare a ogni utente un messaggio di posta elettronica per comunicare l'avvenuta creazione dell'account e condividere l'URL di Seller Insights Portal (https://reports.azure.com).
...
È possibile accedere a Seller Insights Portal usando un account Microsoft personale o un account della società. Il tipo di account dipende dalla configurazione eseguita dall'amministratore nel profilo. L'amministratore deve comunicare il tipo di account da usare.
...
3. Per qualsiasi problema, è possibile fare clic su **Contact Us** (Contattaci) in qualunque momento per inviare un messaggio di posta elettronica, con tutte le domande relative a Seller Insights Portal.
...
5. Fare clic sull'emoticon in qualsiasi momento per inviare commenti e suggerimenti relativi al portale.
...
3. Qui vengono visualizzati i dati sulle tendenze per il periodo di tempo di tre mesi selezionato.
...
R. Nel *pannello A* è possibile selezionare la scheda **Orders and Usage** (Ordini e uso).  Nella scheda è possibile selezionare **Orders** (Ordini) o **Usage** (Uso) in **Report Type** (Tipo di report) e quindi usare i dispositivi di scorrimento per selezionare i dati di interesse.

B. Nel *pannello B* è possibile selezionare i dati mensili. Selezionare **Orders** (Ordini) o **Usage** (Uso) a seconda dell'opzione selezionata in **Report Type** (Tipo di report) nel pannello A. I dati visualizzati corrispondono alle date immesse e ai filtri applicati usando i dispositivi di scorrimento nel pannello A.

C. Nel *pannello C* è possibile selezionare i dati sulle tendenze di tre mesi. Selezionare **Orders** (Ordini) o **Usage** (Uso) a seconda dell'opzione selezionata in **Report Type** (Tipo di report) nel pannello A. I dati visualizzati corrispondono alle date immesse e ai filtri applicati usando i dispositivi di scorrimento nel pannello A.

D. Nel *pannello D* sono elencati i dati di sottoscrizione per ogni utente.  I dati possono essere scaricati da questo pannello in formato Excel o CSV.
...
### <a name="customer-data"></a>Dati del cliente
  ...
  5. Usare i filtri nella parte superiore di ogni colonna per filtrare i dati tramite un'ampia gamma di comandi e valori.
...
3. Immettere l'indirizzo e-mail associato all'account Microsoft o all'account della società dell'utente e quindi fare clic su **Continue** (Continua).
...
2. Individuare l'utente di cui si desidera modificare le autorizzazioni.  È anche possibile aggiungere nuovi utenti. Fare clic su **Aggiungi utenti**.
...
3. È anche possibile inviare una richiesta di supporto dal collegamento **Contact Us** (Contattaci) nella parte inferiore di ogni schermata dell'applicazione.
...
1. *Verificare che l'account in uso sia registrato e che la password funzioni.* Se si immette un account registrato in Azure Active Directory, viene automaticamente chiesto di scegliere l'account con cui accedere. È possibile scegliere un account aziendale o dell'istituto di istruzione o un account Windows LiveID personale.
2. Andare all'indirizzo https://reports.azure.com e disconnettere qualsiasi account con cui potrebbe essere stato effettuato l'accesso in precedenza. È anche possibile aprire la pagina in una finestra privata per assicurarsi che non siano connessi altri account.
3. Immettere l'indirizzo e-mail del server di pubblicazione senza specificare una password. Se l'account è registrato in Azure Active Directory, si verrà reindirizzati al *Passaggio 4*.

  >[!NOTE]
  >Gli account amministratore sono registrati. Anche gli account aziendali configurati dall'account amministratore sono registrati.

4. Scegliere **Account aziendale o dell'istituto di istruzione**. Gli account aziendali o dell'istituto di istruzione vengono configurati da società registrate in Azure. Gli account personali (chiamati anche account LiveID) sono account registrati in Microsoft.

  >[!NOTE]
  >Un utente potrebbe aver configurato un account LiveID usando il proprio indirizzo e-mail aziendale. In questo scenario la società esegue successivamente l'onboarding ad Azure e lo stesso indirizzo e-mail viene registrato come account aziendale. Di conseguenza, l'utente può avere un account LiveID con la password usata per creare il proprio account LiveID, nonché un account aziendale con una password separata usata dalla società.

5. Per Seller Insights Portal, quando si seleziona **Account aziendale o dell'istituto di istruzione**, accertarsi di usare la combinazione di account aziendale e password e non la combinazione di account LiveID e password.
6. L'uso di un account aziendale comporta il reindirizzamento alla pagina di accesso del server di pubblicazione,
7. da cui è possibile verificare il funzionamento della password per l'account aziendale. In caso contrario, passare alla sezione "Reimpostazione della password" di questo articolo.



**Reimpostazione della password**

Provare a reimpostare la password usando i collegamenti seguenti.

>[!NOTE]
>Per gli account aziendali configurati tramite l'account amministratore, se l'amministratore ha attivato la funzionalità che permette di reimpostare la password, è possibile reimpostare personalmente la password. In caso contrario, è possibile contattare il proprio amministratore tramite il modulo di reimpostazione della password.

**Account Microsoft (Live ID): https://account.live.com/ResetPassword.aspx**
![Reimpostazione della password di un account Microsoft][3]

**Account aziendale o dell'istituto di istruzione (ID organizzazione): https://passwordreset.microsoftonline.com/**
![Reimpostazione della password di un account aziendale o dell'istituto di istruzione][4]
