...
## <a name="data-box-service-limits"></a>Limiti di servizio Data Box
 ...
 - Il servizio Data Box è disponibile nelle aree di Azure elencate in [Disponibilità area](data-box-disk-overview.md#region-availability).
...
- Non copiare i dati direttamente nei dischi. Copiare i dati nelle cartelle *BlockBlob*,*PageBlob*e *AzureFile* create in precedena.
...
- Se si dispone di un oggetto di Azure esistente (ad esempio un BLOB) nel cloud con lo stesso nome dell'oggetto che viene copiato, Data Box Disk rinominerà il file come file(1) nel cloud.
...
| BLOB di pagine         | 8 TiB <br> (Ogni file caricato in formato BLOB di pagine deve essere allineato a 512 byte, altrimenti il caricamento non riesce. <br> Sia il disco rigido virtuale che il disco rigido virtuale sono allineati 512 byte.) |
|File di Azure        | 1 TiB <br> Max. l'entità della quota è 5 TiB     |
| Dischi gestiti     |4 TiB <br> Per ulteriori informazioni su dimensioni e limiti, vedere: <li>[Obiettivi di scalabilità per i dischi gestitiScalability targets for managed disks](../virtual-machines/windows/disk-scalability-targets.md#managed-virtual-machine-disks)</li>|
...
| Entità | Convenzioni                                             |
|-------------------|-----------------------------------------------------------|
| Nomi dei dischi gestiti       | <li> Il nome deve avere una lunghezza da 1 a 80 caratteri. </li><li> Il nome deve iniziare con una lettera o un numero, terminare con una lettera, un numero o un segno di sottolineatura. </li><li> Il nome può contenere solo lettere, numeri, caratteri di sottolineatura, punti o trattini. </li><li>   Il nome non deve `/`avere spazi o .                                              |

## <a name="next-steps"></a>Passaggi successivi

- Esaminare [i requisiti di sistema di Data Box DiskReview Data Box Disk system requirements](data-box-disk-system-requirements.md)
