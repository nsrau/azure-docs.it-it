...
3. Nella finestra di dialogo **Configura processo e inizia** specificare gli input seguenti.
    ...
    |Nome utente                       |Nome utente per accedere all'origine dati.        |
    ...
    |Ottimizzazione file              |Quando questa opzione è abilitata, i file vengono compressi all'inserimento. Ciò consente di velocizzare la copia dei dati per i file di piccole dimensioni.        |
...
5. Verrà creato un processo con le impostazioni specificate. Selezionando la casella di controllo, è quindi possibile sospendere, riprendere, annullare o riavviare un processo.
    ...
    - Se è attivata l'ottimizzazione dei file, i file di piccole dimensioni vengono compressi all'inserimento per migliorare le prestazioni di copia. In questi casi verrà visualizzato un file compresso, con il GUID come nome, come illustrato nello screenshot seguente.

        ![Esempio di file compresso](media/data-box-deploy-copy-data-via-copy-service/packed-file-on-ingest.png)
