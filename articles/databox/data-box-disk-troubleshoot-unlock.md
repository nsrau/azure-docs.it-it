---
title: Disco Azure Data Box per la risoluzione dei problemi di sblocco del disco Documenti Microsoft
...
---
# <a name="troubleshoot-disk-unlocking-issues-in-azure-data-box-disk"></a>Risolvere i problemi di sblocco del disco nel disco di Azure Data BoxTroubleshoot disk slocking issues in Azure Data Box Disk

Questo articolo si applica al disco di Microsoft Azure Data Box e descrive i flussi di lavoro usati per risolvere eventuali problemi quando si usa lo strumento di sblocco. 
...
## <a name="unlock-issues-for-disks-when-using-a-windows-client"></a>Sbloccare i problemi per i dischi quando si utilizza un client Windows

In questa sezione vengono descritti in dettaglio alcuni dei principali problemi rilevati durante la distribuzione del disco data box quando si utilizza un client Windows per la copia dei dati.

### <a name="issue-could-not-unlock-drive-from-bitlocker"></a>Problema: Impossibile sbloccare l'unità da BitLockerIssue: Could not unlock drive from BitLocker
...
Per sbloccare i dispositivi Data Box disk, è necessario usare lo strumento di sblocco di Data Box Disk e fornire la password dal portale di Azure. Per altre informazioni, vedere [Esercitazione: Decomprimere, connettersi e sbloccare Azure Data Box Disk](data-box-disk-deploy-set-up.md#connect-to-disks-and-get-the-passkey).
