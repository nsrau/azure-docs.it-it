<properties 
	pageTitle="Programmazione con DocumentDB: stored procedure, trigger e UDF | Azure" 
...
L'esecuzione integrata e transazionale di JavaScript con il linguaggio di DocumentDB consente agli sviluppatori di scrivere **stored procedure**, **trigger** e **funzioni definite dall'utente (UDF)** in modo nativo in JavaScript. Ciò consente di scrivere la logica dell'applicazione che potrà essere fornita ed eseguita direttamente sulle partizioni di memoria del database.
...
-   **Transazioni atomiche:** DocumentDB garantisce che le operazioni di database eseguite all'interno di una singola stored procedure o di un unico trigger siano atomiche. Di conseguenza, un'applicazione potrà combinare le operazioni correlate in unico batch, in modo che o tutte o nessuna di esse avranno esito positivo.
...
-   **Prestazioni:** il fatto che JSON sia intrinsecamente mappato al sistema di tipi in linguaggio JavaScript e rappresenti anche l'unità di base di archiviazione in DocumentDB permette di eseguire una serie di ottimizzazioni, tra cui la materializzazione differita dei documenti JSON nel pool di buffer e la loro disponibilità su richiesta per il codice in esecuzione. Vi sono altri vantaggi associati all'integrazione della logica di business nel database:
    ...
    -   Invio in batch: gli sviluppatori possono raggruppare le operazioni, come gli inserimenti, e inviarle in blocco. Ciò comporta una drastica riduzione del costo della latenza del traffico di rete e i costi generali di archiviazione per la creazione di transazioni separate.
...
La creazione e l'esecuzione di trigger, stored procedure e operatori di query personalizzate sono supportate con le API REST e gli SDK del client in molte piattaforme, tra cui .NET, Node.js e JavaScript. **In questa esercitazione viene usato l'SDK di Node.js** per illustrare la sintassi e l'utilizzo di stored procedure, trigger e UDF.
...
Il frammento successivo mostra come usare l'oggetto contesto in modo che interagisca con le entità di DocumentDB.
...
Questa stored procedure accetta come input documentToCreate, il corpo di un documento da creare nella raccolta corrente. Tutte queste operazioni sono asincrone e dipendono dai callback della funzione JavaScript. La funzione di callback ha due parametri: uno per l'oggetto errore in caso di errore dell'operazione e uno per l'oggetto creato. All'interno del callback, gli utenti possono gestire l'eccezione oppure generare un errore. Nel caso in cui non venisse fornito un callback e si verificasse un errore, il runtime di DocumentDB genererà un errore.

Nell'esempio seguente, il callback genera un errore se l'operazione non è riuscita. In caso contrario, viene impostato l'ID del documento creato come corpo della risposta al client. Ecco come viene eseguita questa stored procedure con i parametri di input.
...
L'SDK lato server JavaScript di DocumentDB offre supporto per la maggior parte delle principali funzionalità del linguaggio JavaScript, secondo lo standard [ECMA-262][ECMA-262].
...
In breve, l'atomicità garantisce che tutte le operazioni eseguite nell'ambito di una transazione siano trattate come unità singola, indipendentemente dal fatto che sia stato eseguito il commit di tutte le operazioni o di nessuna di esse. La coerenza garantisce che i dati siano sempre in ottimo stato interno in tutte le transazioni. L'isolamento garantisce che non vi siano transazioni conflittuali; in generale, la maggior parte dei sistemi commerciali offre più livelli di isolamento utilizzabili in base alle esigenze dell'applicazione. La durabilità assicura che qualsiasi modifica di cui sia stato eseguito il commit nel database sia sempre presente.
...
Questa stored procedure usa le transazioni in un'applicazione di gioco per scambiare elementi tra due giocatori in un'unica operazione. La stored procedure prova a leggere due documenti, ciascuno corrispondente all'ID del giocatore passato come argomento. Se vengono trovati i documenti di entrambi i giocatori, la stored procedure aggiorna i documenti scambiando gli elementi. Se si riscontrano errori lungo il percorso, viene generata un'eccezione JavaScript che interrompe implicitamente la transazione.
...
## Trigger e funzioni definite dall'utente
...
Un fattore importante da tenere presente è l'esecuzione **transazionale** dei trigger in DocumentDB. Questo post-trigger viene eseguito come parte della stessa transazione relativa alla creazione del documento originale. Pertanto, se si genera un'eccezione dal post-trigger (ad esempio se non è possibile aggiornare il documento dei metadati), l'intera transazione avrà esito negativo e verrà ripristinata allo stato precedente. Non verrà creato alcun documento e verrà restituita un'eccezione.

## Funzioni definite dall'utente

Le funzioni definite dall'utente, o UDF, consentono di ampliare la grammatica del linguaggio di query SQL di DocumentDB e implementare la logica di business personalizzata. Possono essere richiamate solo dall'interno delle query. Non hanno accesso all'oggetto contesto e vanno usate come JavaScript di solo calcolo. È quindi possibile eseguire le UDF su repliche secondarie del servizio DocumentDB.

Nell'esempio seguente viene creata una funzione definita dall'utente per calcolare l'imposta sul reddito in base ai tassi relativi a varie fasce di reddito. La funzione viene quindi usata all'interno di una query per cercare tutte le persone che hanno pagato oltre 20.000 dollari di imposte.
...
Per semplificare lo sviluppo delle stored procedure e dei trigger per la gestione dei limiti di tempo, tutte le funzioni sottostanti l'oggetto raccolta (per creare, leggere, sostituire ed eliminare i documenti e gli allegati) restituiscono un valore booleano che rappresenta se l'operazione verrà completata. Se questo valore è falso, sarà indice che il tempo limite sta per scadere e che è necessario concludere l'esecuzione della procedura. Il completamento delle operazioni inserite in coda precedentemente alla prima operazione di archiviazione non accettata è garantito se la stored procedure viene completata in tempo e non vengono inserite in coda altre richieste.
...
La stored procedure viene registrata eseguendo una richiesta POST a fronte dell'URI dbs/sehcAA==/colls/sehcAIE2Qy4=/sprocs con il corpo contenente la stored procedure da creare. È possibile registrare trigger e UDF in maniera simile generando una richiesta POST a fronte di /triggers e /udfs rispettivamente.
Questa stored procedure potrà quindi essere eseguita generando una richiesta POST rispetto al relativo collegamento alle risorse:
...
Oltre al client Node.js, DocumentDB supporta gli SDK di .NET, JavaScript e Python. È possibile creare ed eseguire stored procedure, trigger e UDFs usando anche uno qualsiasi di questi SDK. Nell'esempio seguente viene illustrato come creare ed eseguire una stored procedure con il client .NET. Notare il modo in cui i tipi -NET vengono passati nella stored procedure come JSON e poi riletti.
...
In questo esempio viene mostrato come usare .NET SDK per creare un pre-trigger e quindi un documento con il trigger attivato.
...
Nell'esempio seguente viene illustrato come creare una funzione definita dall'utente (UDF) in una query SQL di DocumentDB.
...
Riferimenti degli SDK dei client
...
3.  JavaScript – Tipo di sistema JSON [][2]<http://www.json.org/js.html></a>

4.  Estendibilità dei database sicuri e portabili - [][3]<http://dl.acm.org/citation.cfm?id=276339></a>

5.  Architettura di database orientata ai servizi - [][4]<http://dl.acm.org/citation.cfm?id=1066267&coll=Portal&dl=GUIDE></a>

6.  Hosting del runtime .NET in Microsoft SQL Server - [][5]<http://dl.acm.org/citation.cfm?id=1007669></a>
