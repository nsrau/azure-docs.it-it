...
Per autorizzare l'azione di accesso di Azure, sono necessarie le credenziali dell'entità servizio di Azure. Per ottenere le credenziali di Azure, eseguire i comandi seguenti nel computer locale:
```
az login
...
Il comando `az spring-cloud app create` attualmente non è idempotente.  Questo flusso di lavoro è consigliato nelle app e nelle istanze cloud di Azure Spring esistenti.
...
Creare il file di `.github/workflow/main.yml` nel repository:
...
Il comando AZ `run` userà la versione più recente dell'interfaccia della riga di comando di Azure. In caso di modifiche di rilievo, è anche possibile usare una versione specifica dell'interfaccia della riga di comando di Azure con `action`di Azure/CLI. 
...
Un'altra opzione consiste nell'usare il plug-in [Maven](https://docs.microsoft.com/azure/spring-cloud/spring-cloud-quickstart-launch-app-maven) per la distribuzione del file jar e l'aggiornamento delle impostazioni dell'app. Il `mvn azure-spring-cloud:deploy` di comando è idempotente e creerà automaticamente le app, se necessario. Non è necessario creare in anticipo le applicazioni corrispondenti.
...
Le **azioni** di GitHub devono essere abilitate automaticamente dopo aver eseguito il push di `.github/workflow/main.yml` su GitHub. L'azione verrà attivata quando si esegue il push di un nuovo commit. Se si crea questo file nel browser, l'azione dovrebbe essere già stata eseguita.
...
* [Azioni di GitHub per Azure](https://github.com/Azure/actions/)
