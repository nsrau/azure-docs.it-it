...
1. Se l'app non è ancora stata registrata, accedere al portale di provisioning iOS</a> su Apple Developer Center, eseguire l'accesso con il proprio ID Apple, fare clic su **Identifiers**, quindi su **App IDs** e infine fare clic sul segno **+** per creare un ID per l'app.
...
3. Selezionare quindi l'ID dell'app Guida introduttiva per l'app per dispositivi mobili nell'elenco a discesa **App ID** e fare clic su **Continue**.
...
1. In **QSTodoService** eseguire l'override della dichiarazione client esistente in modo che **AppDelegate** possa acquisire il client mobile:
...
[introduzione alle app per dispositivi mobili del servizio app ]: app-service-mobile-dotnet-backend-xamarin-ios-get-started-preview.md
