...
Per creare nuovi utenti in grado di eseguire l'autenticazione con Azure AD, è necessario `azure_ad_admin` disporre del ruolo nel database. Questo ruolo viene assegnato configurando l'account amministratore Azure AD per uno specifico server di database di Azure per PostgreSQL.
