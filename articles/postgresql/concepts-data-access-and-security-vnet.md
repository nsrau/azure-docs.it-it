---
title: Panoramica degli endpoint di servizio di rete virtuale per il server di Database di Azure per PostgreSQL | Microsoft Docs
description: Questo articolo descrive il funzionamento degli endpoint di servizio di rete virtuale per il server di Database di Azure per PostgreSQL.
services: postgresql
author: mbolz
ms.author: mbolz
manager: jhubbard
...
> [!NOTE]
> Per Database di Azure per PostgreSQL, questa funzionalità è disponibile in anteprima pubblica in tutte le aree del cloud pubblico di Azure in cui viene distribuito Database di Azure per PostgreSQL.
...
Tuttavia, a partire da maggio 2018, il servizio Database di Azure per PostgreSQL non è ancora tra i servizi che possono essere assegnati direttamente a una subnet.
...
Ogni regola di rete virtuale si applica all'intero server di Database di Azure per PostgreSQL, non solo a un determinato database nel server. In altre parole, la regola della rete virtuale si applica a livello di server, non a livello di database.
