---
title: Aumento automatico delle dimensioni archiviazione nel portale di Azure nel Database di Azure per PostgreSQL - Server singolo
description: Questo articolo descrive come abilitare l'auto aumento delle dimensioni di archiviazione tramite il portale di Azure nel Database di Azure per PostegreSQL - Server singolo
author: ambhatna
ms.author: ambhatna
...
Seguire questi passaggi per impostare l'aumento automatico di archiviazione server PostgreSQL:

1. Nel [portale di Azure](https://portal.azure.com/), selezionare il Database di Azure per il server PostgreSQL.

2. Nella pagina del server PostgreSQL, sotto **le impostazioni**, fare clic su **tariffario** per aprire la pagina dei prezzi a livelli.

3. Nel **aumento automatico** sezione, selezionare **Yes** per abilitare l'aumento automatico di archiviazione.
...
5. Una notifica viene visualizzata la conferma l'aumento automatico che è stata abilitata.
    ...
    ![Database di Azure per PostgreSQL - operazione riuscita di aumento automatico](./media/howto-auto-grow-storage-portal/5-auto-grow-successful.png)
