...
description: Informazioni sui backup automatici e sul ripristino del database di Azure per il server PostgreSQL - Server singolo.
author: rachel-msft
ms.author: raagyema
...
Questi file di backup non possono essere esportati. I backup possono essere usati solo per le operazioni di ripristino nel database di Azure per PostgreSQL.The backups can only be used for restore operations in Azure Database for PostgreSQL. È possibile utilizzare [pg_dump](howto-migrate-using-dump-and-restore.md) per copiare un database.
...
- Assicurarsi che siano disponibili regole appropriate per il firewall e la rete virtuale a livello di server per consentire agli utenti di connettersi. Queste regole non vengono copiate dal server originale.
...
- Informazioni su come eseguire il ripristino usando [l'interfaccia della riga di comando](howto-restore-server-cli.md)di Azure.
