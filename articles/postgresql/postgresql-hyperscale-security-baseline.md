...
**Linee guida**: non applicabile; Non è possibile associare database di Azure per PostgreSQL-iperscalabilità (CITUS) a una rete virtuale, una subnet o un gruppo di sicurezza di rete.
...
**Linee guida**: non applicabile; gli endpoint usati da database di Azure per PostgreSQL-iperscalabilità (CITUS) sono tutti gestiti da Microsoft. L'utente è responsabile di eventuali controlli aggiuntivi che si desidera distribuire nei sistemi locali.
...
**Linee guida**: Microsoft gestisce l'origine dell'ora usata per le risorse di Azure, ad esempio database di Azure per PostgreSQL-iperscalabilità (CITUS) per i timestamp nei log.
...
**Linee guida**: non applicabile; Database di Azure per PostgreSQL: la scalabilità iperscalabile (CITUS) non elabora o produce log correlati all'anti-malware.
...
**Linee guida**: l'accesso a database di Azure per PostgreSQL-iperscalabilità (CITUS) è supportato sia con nome utente/password configurato direttamente nel database, sia con un'identità Azure Active Directory (ad) e utilizzando un token Azure ad per la connessione. Quando si usa un token Azure AD, sono supportati metodi diversi, ad esempio un utente Azure AD, un gruppo Azure AD o un'applicazione Azure AD che si connette al database.
...
**Linee guida**: rivedere e riconciliare l'accesso per gli utenti che hanno accesso al database locale, oltre che tramite Azure Active Directory.

Esaminare i registri di Azure Active Directory (AD) per individuare gli account obsoleti che possono includere i ruoli amministrativi con iperscalabilità (CITUS). Inoltre, è possibile usare le verifiche di accesso alle identità di Azure per gestire in modo efficiente l'appartenenza ai gruppi, accedere alle applicazioni aziendali che possono essere usate per accedere a iperscalabilità (CITUS) e assegnazioni di ruolo. È consigliabile verificare regolarmente l'accesso degli utenti, ad esempio ogni 90 giorni, per assicurarsi che solo gli utenti appropriati dispongano di accesso continuo.
...
**Linee guida**: Microsoft gestisce l'infrastruttura sottostante per database di Azure per PostgreSQL-iperscalabilità (CITUS) e ha implementato severi controlli per impedire la perdita o l'esposizione dei dati del cliente.
...
**Indicazioni**: le funzionalità di identificazione, classificazione e prevenzione della perdita dei dati non sono ancora disponibili per l'iperscalabilità (CITUS).
...
**Linee guida**: usare il controllo degli accessi in base al ruolo di Azure (RBAC di Azure) per controllare l'accesso al piano di controllo iperscalabile (CITUS), ad esempio portale di Azure. Il controllo degli accessi in base al ruolo non influisce sulle autorizzazioni utente all'interno del database.
...
Microsoft gestisce l'infrastruttura sottostante per l'iperscalabilità (CITUS) e ha implementato severi controlli per impedire la perdita o l'esposizione dei dati del cliente.
...
**Linee guida**: Microsoft esegue la gestione delle vulnerabilità nei sistemi sottostanti che supportano database di Azure per PostgreSQL-iperscalabilità (CITUS). Per le vulnerabilità correlate alla configurazione del cluster di database e delle risorse correlate, usare un programma comune di assegnazione dei punteggi di rischio (ad esempio, il sistema di valutazione delle vulnerabilità comuni) o le valutazioni di rischio predefinite fornite dallo strumento di analisi di terze parti.
...
**Linee guida**: non applicabile; È possibile gestire in modo sicuro la password di amministratore del database usando Azure Key Vault. Assegnare le autorizzazioni appropriate per l'ambito per il Key Vault segreto che limita l'accesso alla password del database. Evitare di archiviare le stringhe di connessione o i segreti del database nei repository di codice.

* [Come connettersi al database PostgreSQL](https://docs.microsoft.com/azure/postgresql/quickstart-create-hyperscale-portal#connect-to-the-database-using-psql)
...
Microsoft anti-malware è abilitato nell'host sottostante che supporta i servizi di Azure, ad esempio iperscale (CITUS). Tuttavia, non viene eseguito sui contenuti del cliente.
