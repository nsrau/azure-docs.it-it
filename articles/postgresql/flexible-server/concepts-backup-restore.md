...
I backup possono essere utilizzati solo per le operazioni di ripristino all'interno del server flessibile. Se si desidera esportare o importare dati nel server flessibile, utilizzare la metodologia di [dump e ripristino](https://docs.microsoft.com/azure/postgresql/howto-migrate-using-dump-and-restore)   .
...
È possibile utilizzare la metrica di [archiviazione di backup utilizzata](https://docs.microsoft.com/azure/postgresql/concepts-monitoring)   nel portale di Azure per monitorare l'archiviazione di backup utilizzata da un server. La metrica di archiviazione di backup usata rappresenta la somma dello spazio di archiviazione utilizzato da tutti i backup del database e dei backup del log mantenuti in base al periodo di conservazione dei backup impostato per il server.  Un'intensa attività transazionale sul server può causare un aumento dell'uso dell'archivio di backup indipendentemente dalle dimensioni totali del database.
