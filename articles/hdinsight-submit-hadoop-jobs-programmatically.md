...
-   [Inviare processi MapReduce tramite PowerShell][Inviare processi MapReduce tramite PowerShell]
-   [Inviare processi Hive tramite PowerShell][Inviare processi Hive tramite PowerShell]
-   [Inviare processi Sqoop tramite PowerShell][Inviare processi Sqoop tramite PowerShell]
-   [Inviare processi MapReduce tramite HDInsight .NET SDK][Inviare processi MapReduce tramite HDInsight .NET SDK]
-   [Inviare processi MapReduce di flussi Hadoop tramite HDInsight .NET SDK][Inviare processi MapReduce di flussi Hadoop tramite HDInsight .NET SDK]
-   [Inviare processi Hive tramite HDInsight .NET SDK][Inviare processi Hive tramite HDInsight .NET SDK]
...
Apache [hdinsight-use-hive][hdinsight-use-hive] fornisce un metodo per l'esecuzione di processi MapReduce mediante un linguaggio di scripting simile a SQL, denominato *HiveQL*, che può essere applicato per attività di riepilogo, query e analisi di volumi di dati molto elevati.
...
Vedere [Usare Sqoop con HDInsight][Usare Sqoop con HDInsight].
...
10. Nella funzione Main() copiare e incollare il codice seguente:
    ...
    Queste sono tutte le variabili che è necessario impostare per il programma. È possibile ottenere il nome della sottoscrizione di Azure dal [portale di gestione di Azure][portale di gestione di Azure].

    Per informazioni sul certificato, vedere [Creare e caricare un certificato di gestione per Azure][Creare e caricare un certificato di gestione per Azure]. Un modo semplice per configurare il certificato consiste nell'eseguire i cmdlet di PowerShell *Get-AzurePublishSettingsFile* e *Import-AzurePublishSettingsFile*, che creeranno e gestiranno automaticamente il certificato di gestione. Dopo aver eseguito i cmdlet di PowerShell, è possibile aprire *certmgr.msc* dalla workstation e individuare il certificato espandendo *Personal/Certificates*. Il certificato creato dai cmdlet di PowerShell conterrà l'indicazione *Strumenti di Azure* in entrambi in campi *Rilasciato a* e *Rilasciato da*.
...
Per informazioni dettagliate sulla creazione di un'applicazione .NET per l'invio di processi MapReduce, vedere [Inviare processi MapReduce tramite HDInsight .NET SDK][Inviare processi MapReduce tramite HDInsight .NET SDK].

Per altre informazioni sullo sviluppo e sulla distribuzione di processi di flussi di Hadoop, vedere [Sviluppare programmi per la creazione di flussi Hadoop in C# per HDInsight][Sviluppare programmi per la creazione di flussi Hadoop in C# per HDInsight].
