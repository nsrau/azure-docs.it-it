---
title: Set di dati associati in set di macchine virtuali di AzureAzure Virtual Machine Scale Sets Attached Data Disks
description: Informazioni su come usare i dischi dati collegati con i set di scalabilità delle macchine virtuali tramite strutture di casi d'uso specifici.
author: avirishuv
...
- [Interfaccia della riga di comando di AzureAzure](tutorial-use-disks-cli.md)
...
I dischi dati specificati nel modello del set di scalabilità sono sempre vuoti. Tuttavia è possibile collegare un disco dati esistente a una macchina virtuale specifica in un set di scalabilità. Se si desidera propagare i dati in tutte le macchine virtuali nel set di scalabilità, è possibile duplicare il disco dati e collegarlo a ogni macchina virtuale nel set di scalabilità oppure creare un'immagine personalizzata contenente i dati ed eseguire il provisioning del set di scalabilità da questa immagine personalizzata oppure è possibile usare File di Azure o un'offerta di archiviazione dati simile.
...
Il supporto del portale di Azure per i dischi dati collegati nei set di scalabilità è limitato. A seconda dei requisiti è possibile usare modelli di Azure, interfaccia della riga di comando, PowerShell, SDK e API REST per gestire i dischi collegati.
