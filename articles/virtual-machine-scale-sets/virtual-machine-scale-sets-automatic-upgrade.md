---
title: Aggiornamenti automatici delle immagini del sistema operativo con i set di scalabilità delle macchine virtuali di AzureAutomatic OS image upgrades with Azure virtual machine scale sets
description: Informazioni su come aggiornare automaticamente l'immagine del sistema operativo nelle istanze di macchine virtuali in un set di scalabilitàLearn how to automatically upgrade the OS image on VM instances in a scale set
author: mayanknayar
...
- Aggiorna i batch di istanze in modo insequenziabile ogni volta che una nuova immagine viene pubblicata dal server di pubblicazione.
...
- Funziona per tutte le dimensioni di VM e per immagini Windows e Linux.Works for all VM sizes, and both for Windows and Linux images.
...
- L'aggiornamento automatico delle immagini del sistema operativo può essere abilitato su un set di scalabilità di qualsiasi dimensione.
...
2. L'agente di orchestrazione di aggiornamento identifica il batch di istanze di macchine virtuali da aggiornare, con qualsiasi batch con un massimo del 20% del numero totale di istanze, soggetto a una dimensione minima del batch di una macchina virtuale.
3. Il disco del sistema operativo del batch di istanze di vm selezionato viene sostituito con un nuovo disco del sistema operativo creato dall'immagine più recente. Tutte le estensioni e le configurazioni specificate nel modello del set di scalabilità vengono applicate all'istanza aggiornata.
...
Attualmente sono supportate solo alcune immagini della piattaforma del sistema operativo. Il supporto delle immagini personalizzate è disponibile [in anteprima](virtual-machine-scale-sets-automatic-upgrade.md#automatic-os-image-upgrade-for-custom-images-preview) per le immagini personalizzate tramite Shared [Image Gallery](shared-image-galleries.md).

Gli SKU della piattaforma seguenti sono attualmente supportati (e altri vengono aggiunti periodicamente):The following platform SKUs are currently supported (and more are added periodically):
...
- Usare Compute API versione 2018-10-01 o successiva.
...
- Per i set di scalabilità che usano le macchine virtuali di Windows, a partire dalla versione 2019-03-01 dell'API di calcolo, la proprietà *virtualMachineProfile.osProfile.windowsConfiguration.enableAutomaticUpdates* deve essere impostata su *false* nella definizione del modello del set di scalabilità. La proprietà precedente abilita gli aggiornamenti in-VM in cui "Windows Update" applica le patch del sistema operativo senza sostituire il disco del sistema operativo. Con gli aggiornamenti automatici delle immagini del sistema operativo abilitati sul set di scalabilità, non è necessario un aggiornamento aggiuntivo tramite "Windows Update".

### <a name="service-fabric-requirements"></a>Requisiti di Service Fabric

Se si utilizza Service Fabric, assicurarsi che siano soddisfatte le condizioni seguenti:If you are using Service Fabric, ensure the following conditions are met:
-   Il livello di [durabilità](../service-fabric/service-fabric-cluster-capacity.md#the-durability-characteristics-of-the-cluster) di Service Fabric è Argento o Oro e non Bronzo.
...
-   Il livello di durabilità deve essere lo stesso nel cluster di Service Fabric e nell'estensione di Service Fabric nella definizione del modello del set di scalabilità.

Assicurarsi che le impostazioni di durabilità non corrispondano nell'estensione del cluster Service Fabric e dell'infrastruttura del servizio, in quanto una mancata corrispondenza genererà errori di aggiornamento. I livelli di durabilità possono essere modificati in base alle linee guida descritte in [questa pagina.](../service-fabric/service-fabric-cluster-capacity.md#changing-durability-levels)


## <a name="automatic-os-image-upgrade-for-custom-images-preview"></a>Aggiornamento automatico delle immagini del sistema operativo per immagini personalizzate (anteprima)Automatic OS image upgrade for custom images (preview)

> [!IMPORTANT]
> L'aggiornamento automatico delle immagini del sistema operativo per le immagini personalizzate è attualmente in anteprima pubblica. È necessaria una procedura di consenso esplicito per utilizzare la funzionalità di anteprima pubblica descritta di seguito.
> Questa versione di anteprima viene messa a disposizione senza contratto di servizio e non è consigliata per i carichi di lavoro di produzione. Alcune funzionalità potrebbero non essere supportate o potrebbero presentare funzionalità limitate.
> Per altre informazioni, vedere [Condizioni supplementari per l'utilizzo delle anteprime di Microsoft Azure](https://azure.microsoft.com/support/legal/preview-supplemental-terms/).

L'aggiornamento automatico delle immagini del sistema operativo è disponibile in anteprima per le immagini personalizzate distribuite tramite [Shared Image Gallery](shared-image-galleries.md). Altre immagini personalizzate non sono supportate per gli aggiornamenti automatici delle immagini del sistema operativo.

L'abilitazione della funzionalità di anteprima richiede un consenso esplicito una tantera per la funzionalità *AutomaticOSUpgradeWithGalleryImage* per ogni sottoscrizione, come descritto di seguito.

### <a name="rest-api"></a>API REST
Nell'esempio seguente viene descritto come abilitare l'anteprima per la sottoscrizione:The following example describes how to enable the preview for your subscription:
...
Utilizzare il cmdlet [Register-AzProviderFeature](/powershell/module/az.resources/register-azproviderfeature) per abilitare l'anteprima per la sottoscrizione.
...
Usare [il registro delle funzionalità](/cli/azure/feature#az-feature-register) per abilitare l'anteprima dell'abbonamento.
...
- Il processo di consenso esplicito descritto in precedenza deve essere completato una sola volta per ogni sottoscrizione. Dopo il completamento dell'opzione di consenso esplicito, gli aggiornamenti automatici del sistema operativo possono essere abilitati per qualsiasi set di scalabilità in tale sottoscrizione.
- La Raccolta immagini condivise può essere in qualsiasi sottoscrizione e non richiede l'attivazione separata di una raccolta esplicita. Solo la sottoscrizione del set di scalabilità richiede l'consenso esplicito per la funzionalità.
- Il processo di configurazione per l'aggiornamento automatico delle immagini del sistema operativo è lo stesso per tutti i set di scalabilità, come descritto nella sezione relativa alla [configurazione](virtual-machine-scale-sets-automatic-upgrade.md#configure-automatic-os-image-upgrade) di questa pagina.
- Le istanze dei set di scalabilità configurate per gli aggiornamenti automatici delle immagini del sistema operativo verranno aggiornate alla versione più recente dell'immagine della raccolta immagini condivise quando una nuova versione dell'immagine viene pubblicata e [replicata](shared-image-galleries.md#replication) nell'area di tale set di scalabilità. Se la nuova immagine non viene replicata nell'area in cui viene distribuita la scalabilità, le istanze del set di scalabilità non verranno aggiornate alla versione più recente. La replica regionale delle immagini consente di controllare l'implementazione della nuova immagine per i set di scalabilità.
...
### <a name="azure-powershell"></a>Azure PowerShell
Utilizzare il cmdlet [Update-AzVmss](/powershell/module/az.compute/update-azvmss) per configurare gli aggiornamenti automatici delle immagini del sistema operativo per il set di scalabilità. L'esempio seguente configura gli aggiornamenti automatici per il set di scalabilità denominato myScaleSet nel gruppo di risorse denominato *myResourceGroup:The*following example configures automatic upgrades for the scale set named *myScaleSet* in the resource group named myResourceGroup :
...
Se il set di scalabilità usa le credenziali per accedere a risorse esterne, ad esempio un'estensione della macchina virtuale configurata per l'uso di un token di firma di accesso condiviso per l'account di archiviazione, assicurarsi che le credenziali siano aggiornate. Se le credenziali, inclusi certificati e token, sono scadute, l'aggiornamento avrà esito negativo e il primo batch di macchine virtuali verrà lasciato in uno stato di errore.
...
### <a name="azure-powershell"></a>Azure PowerShell
Usare il cmdlet [Get-AzVmss](/powershell/module/az.compute/get-azvmss) per controllare la cronologia di aggiornamento del sistema operativo per il set di scalabilità. L'esempio seguente descrive in dettaglio come esaminare lo stato di aggiornamento del sistema operativo per un set di scalabilità denominato *myScaleSet* nel gruppo di risorse denominato *myResourceGroup*:
...
### <a name="azure-cli-20"></a>Interfaccia della riga di comando di Azure 2.0
Usare [az vmss get-os-upgrade-history](/cli/azure/vmss#az-vmss-get-os-upgrade-history) per controllare la cronologia di aggiornamento del sistema operativo per il set di scalabilità. Usare l'interfaccia della riga di comando di Azure versione 2.0.47 o successiva. L'esempio seguente descrive in dettaglio come esaminare lo stato di aggiornamento del sistema operativo per un set di scalabilità denominato *myScaleSet* nel gruppo di risorse denominato *myResourceGroup*:
...
È possibile ottenere le versioni di immagine disponibili per l'aggiornamento automatico del sistema operativo supportati SKU utilizzando gli esempi seguenti:You can get the available image versions for automatic OS upgrade supported SKUs using the below examples:
...
Con l'aggiornamento automatico delle immagini del sistema operativo abilitato nel set di scalabilità, non è necessario attivare manualmente gli aggiornamenti delle immagini nel set di scalabilità. L'agente di orchestrazione di aggiornamento del sistema operativo applicherà automaticamente l'ultima versione dell'immagine disponibile alle istanze del set di scalabilità senza alcun intervento manuale.

Per i casi specifici in cui non si desidera attendere che l'agente di orchestrazione applichi l'immagine più recente, è possibile attivare manualmente un aggiornamento dell'immagine del sistema operativo usando gli esempi riportati di seguito.
...
### <a name="azure-powershell"></a>Azure PowerShell
Utilizzare il cmdlet [Start-AzVmssRollingOSUpgrade](/powershell/module/az.compute/Start-AzVmssRollingOSUpgrade) per controllare la cronologia degli aggiornamenti del sistema operativo per il set di scalabilità. L'esempio seguente descrive in dettaglio come avviare un aggiornamento del sistema operativo in sequenza in un set di scalabilità denominato *myScaleSet* nel gruppo di risorse denominato *myResourceGroup*:
...
### <a name="azure-cli-20"></a>Interfaccia della riga di comando di Azure 2.0
Utilizzare [az vmss avvio dell'aggiornamento](/cli/azure/vmss/rolling-upgrade#az-vmss-rolling-upgrade-start) in sequenza per controllare la cronologia di aggiornamento del sistema operativo per il set di scalabilità. Usare l'interfaccia della riga di comando di Azure versione 2.0.47 o successiva. L'esempio seguente descrive in dettaglio come avviare un aggiornamento del sistema operativo in sequenza in un set di scalabilità denominato *myScaleSet* nel gruppo di risorse denominato *myResourceGroup*:
