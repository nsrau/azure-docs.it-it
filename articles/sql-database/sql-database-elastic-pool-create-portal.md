---
title: Creare un nuovo pool di database elastici con il portale di Azure | Documentazione Microsoft
description: "Come aggiungere un pool elastico scalabile alla configurazione del database SQL per semplificare l&quot;amministrazione e la condivisione delle risorse tra più database."
...
---
# <a name="create-a-new-elastic-pool-with-the-azure-portal"></a>Creare un nuovo pool elastico con il portale di Azure
...
## <a name="step-1-create-a-new-pool"></a>Passaggio 1: Creare un nuovo pool
...
> [!NOTE]
> È anche possibile creare un nuovo pool cercando **pool elastico SQL** in **Marketplace** o facendo clic su **+Aggiungi** nel pannello **Pool elastici SQL**. Sarà possibile specificare un server nuovo o esistente nel flusso di lavoro dei provisioning del pool.
>
>

1. Nel [portale di Azure](http://portal.azure.com/) sotto l'elenco sul lato sinistro fare clic su **Altri servizi** **>** **SQL Server** e quindi fare clic sul server che contiene i database da aggiungere a un pool.
...
3. Usare i controlli nella pagina **Configura pool** per esaminare le impostazioni e configurare il pool. Vedere la sezione relativa ai [limiti dei pool elastici](sql-database-elastic-pool.md#edtu-and-storage-limits-for-elastic-pools) per altri dettagli sui limiti per ogni livello di servizio e l'articolo [Price and performance considerations for elastic pools](sql-database-elastic-pool-guidance.md) (Considerazioni su prezzi e prestazioni dei pool elastici) per istruzioni dettagliate sul corretto ridimensionamento di un pool. Per altre informazioni sulle impostazioni del pool, vedere [Proprietà del pool elastico](sql-database-elastic-pool.md#elastic-pool-properties).
...
## <a name="understand-pool-recommendations"></a>Informazioni sulle raccomandazioni per i pool
...
### <a name="dynamic-recommendations"></a>Indicazioni dinamiche
