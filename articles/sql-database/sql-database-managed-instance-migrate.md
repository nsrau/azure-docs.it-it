...
- [Valutare la compatibilità delle istanze gestite](#assess-managed-instance-compatibility) in cui è necessario assicurarsi che non ci siano problemi di blocco che possono impedire le migrazioni.
  - Questo passaggio include anche la creazione di [baseline](#create-performance-baseline) per le prestazioni per determinare l'utilizzo delle risorse nell'istanza di SQL Server di origine. Questo passaggio è necessario se si desidera distribuire Istanza gestita dimensionate correttamente e verificare che le prestazioni dopo la migrazione non siano interessate.
