...
1. Specificare l'obiettivo di servizio. L'obiettivo di servizio prevede il livello di servizio, la generazione hardware e il numero massimo di vcore. La tabella seguente mostra le opzioni relative all'obiettivo di servizio:
...
Per modificare il valore massimo o minimo per Vcore e il ritardo di sospensione, viene eseguita usando il comando [set-AzSqlDatabase](/powershell/module/az.sql/set-azsqldatabase) in PowerShell `MaxVcore`usando `MinVcore`gli argomenti `AutoPauseDelayInMinutes` , e.

### <a name="use-azure-cli"></a>Utilizzare l'interfaccia della riga di comando di Azure

Per modificare il vcore massimo o minimo e il ritardo di sospensione, è necessario usare il [comando AZ SQL DB Update](/cli/azure/sql/db#az-sql-db-update) nell'interfaccia della riga di `capacity`comando `min-capacity`di Azure `auto-pause-delay` usando gli argomenti, e.
