<properties 
 pageTitle="Ripristinare un database SQL di Azure mediante Geo-Restore con l'API REST" 
 description="Geo-Restore, Database SQL di Microsoft Azure, ripristinare il database, recuperare il database, API REST" 
...
# Ripristinare un database SQL di Azure mediante Geo-Restore con l'API REST
...
In questa guida viene illustrato come ripristinare un database SQL di Azure mediante l'API REST. Vengono forniti collegamenti a operazioni più dettagliate. Geo-Restore è la protezione da ripristino di emergenza inclusa per tutti i livelli di servizio Basic, Standard e Premium dei database SQL di Azure.
...
2.	Ottenere il database che si desidera ripristinare tramite l'operazione [Get Recoverable Database](http://msdn.microsoft.com/library/azure/dn800985.aspx).
