---
title: Configurare avvisi e notifiche per l'istanza gestita (portale di Azure)Setup alerts and notifications for Managed Instance (Azure portal)
description: Usare il portale di Azure per creare avvisi dell'istanza gestita SQL, che possono attivare notifiche o automazione quando vengono soddisfatte le condizioni specificate.
...
---
# <a name="create-alerts-for-azure-sql-managed-instance-using-the-azure-portal"></a>Creare avvisi per l'istanza gestita sql di Azure usando il portale di AzureCreate alerts for Azure SQL Managed Instance using the Azure portal

## <a name="overview"></a>Panoramica

Questo articolo illustra come configurare gli avvisi per i database nel database dell'istanza gestita SQL di Azure usando il portale di Azure.This article shows you how to set up alerts for databases in Azure SQL Managed Instance Database using the Azure portal. Gli avvisi possono inviare un messaggio di posta elettronica o chiamare un hook Web quando una metrica (ad esempio la dimensione di archiviazione dell'istanza o l'utilizzo della CPU) raggiunge la soglia. Questo articolo include anche le procedure consigliate per impostare i periodi di avviso.
