...
Da un prompt dei `$resourceGroupName`comandi di PowerShell, impostare `$serverName`, e `$databaseName`, quindi eseguire il comando seguente:
...
- Il database SQL alloca automaticamente 32 GB per vCore per il `tempdb` database. `tempdb`si trova nell'archivio SSD locale in tutti i livelli di servizio.
