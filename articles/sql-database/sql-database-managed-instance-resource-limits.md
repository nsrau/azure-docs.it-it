...
Istanza gestita offre due livelli di servizio: Utilizzo generico e Business critical (anteprima pubblica). Questi livelli offrono funzionalità diverse, come descritto nella tabella seguente:

| **Funzionalità** | **Utilizzo generico** | **Business critical (anteprima)** |
| --- | --- | --- |
...
| Numero massimo di file di database per istanza | Fino a 280 | Illimitato |
| Numero massimo di IOPS di archiviazione previste | 500-5.000 ([in base alle dimensioni del file di dati](../virtual-machines/windows/premium-storage-performance.md#premium-storage-disk-sizes)). | Dipende dalla velocità del disco SSD sottostante. |
...
Se servono più istanze gestite nelle aree correnti, è possibile inviare una richiesta di supporto per estendere la quota tramite il portale di Azure. Per avviare il processo di acquisizione di una quota maggiore:
