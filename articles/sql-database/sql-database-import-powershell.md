<properties
    ...
    description="Importare un file BACPAC per creare un nuovo database SQL di Azure tramite PowerShell"
...
# Importare un file BACPAC per creare un nuovo database SQL di Azure tramite PowerShell
...
- Una sottoscrizione di Azure. Se è necessaria una sottoscrizione ad Azure, fare semplicemente clic su **Versione di valutazione gratuita** nella parte superiore della pagina, quindi tornare e proseguire fino alla fine di questo articolo.
- Un file BACPAC del database che si vuole importare. Il file BACPAC deve essere inserito in un contenitore BLOB di un[account di archiviazione di Azure (classico)](../storage/storage-create-storage-account.md).
...
L'esecuzione di questo cmdlet subito dopo la richiesta restituisce in genere **Status : Pending** o **Status : Running**. Eseguire l'operazione più volte finché non viene visualizzato l'output **Status : Completed**.
