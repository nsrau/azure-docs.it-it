---
title: TDE - Bring Your Own Key (BYOK) - Database SQL di Azure | Microsoft Docs
...
>[!IMPORTANT]
>È importante notare che **una volta che una protezione TDE è memorizzata in Azure Key Vault, non si distacca mai da Azure Key Vault**. Il server logico può solo inviare richieste di operazione della chiave al materiale della chiave di protezione TDE in Key Vault e **non accedere mai o memorizzare nella cache la protezione TDE**. L'amministratore di Key Vault ha il diritto di revocare le autorizzazioni di Key Vault del server in qualsiasi momento, in tal caso, tutte le connessioni al server vengono interrotte. 
>


## <a name="guidelines-for-configuring-tde-with-byok"></a>Indicazioni per la configurazione di TDE con BYOK

### <a name="general-guidelines"></a>Linee guida generali
- Verificare che Azure Key Vault e database SQL di Azure siano nello stesso tenant.  Le interazioni di server e insieme di credenziali delle chiavi tra tenant **non sono supportate**.
...
- Configurare Azure Key Vault senza una rete virtuale o un firewall.  Se SQL perde l'accesso all'insieme di credenziali delle chiavi, i database di crittografia TDE vengono eliminati entro 24 ore.
...
### <a name="guidelines-for-configuring-the-tde-protector-asymmetric-key-stored-in-azure-key-vault"></a>Linee guida per la configurazione della protezione TDE (chiave asimmetrica) memorizzate in Azure Key Vault
...
- Usare una chiave senza data di scadenza e non impostare mai una data di scadenza per una chiave già in uso: **una volta scaduta la chiave, i database di crittografia perdono l'accesso alla protezione TDE e vengono eliminati entro 24 ore**.
...
- Creare un backup della chiave di Azure Key Vault prima di usare per la prima volta la chiave in Azure Key Vault. Altre informazioni sul comando [Backup-AzureKeyVaultKey](https://docs.microsoft.com/en-us/powershell/module/azurerm.keyvault/backup-azurekeyvaultkey?view=azurermps-5.1.1).
...
- Creare copie di backup recuperabili di tali chiavi usando [Backup-AzureKeyVaultKey](https://docs.microsoft.com/en-us/powershell/module/azurerm.keyvault/backup-azurekeyvaultkey?view=azurermps-5.1.1).
...
Come configurare la disponibilità elevata con Azure Key Vault dipende dalla configurazione del database e del server logico, di seguito sono presenti le configurazioni consigliate per due casi distinti.  Il primo caso è un database autonomo o un server logico senza alcuna ridondanza geografica configurata.  Il secondo caso è un database o un server logico configurato con i gruppi di failover o di ridondanza geografica, in cui è necessario assicurare che ogni copia con ridondanza geografica abbia un Azure Key Vault locale all'interno del gruppo di failover per garantire il funzionamento di failover geografici. Nel primo caso, se si richiedono un'elevata disponibilità di un database e un server logico senza alcuna ridondanza geografica configurata, si consiglia di configurare il server per usare due diversi insiemi di credenziali delle chiavi in due aree diverse con lo stesso materiale della chiave.  Ciò può essere ottenuto creando una protezione TDE con l'insieme di credenziali delle chiavi primarie posizionato nella stessa area del server logico e clonando la chiave in un insieme di credenziali delle chiavi in un'area di Azure diversa, in modo che il server abbia accesso a un secondo insieme di credenziali delle chiavi se l'insieme di credenziali delle chiavi primarie subisce un'interruzione mentre il database è attivo e in esecuzione. Usare il cmdlet Backup-AzureKeyVaultKey per recuperare la chiave in formato crittografato dall'insieme di credenziali delle chiavi primarie e quindi usare il cmdlet Restore-AzureKeyVaultKey e specificare un insieme di credenziali delle chiavi nella seconda area.
...
- Selezionare il riquadro TDE dei server logici e per ogni server SQL logico:  
   ...
   - Questa operazione nel portale creerà un [AppID](https://docs.microsoft.com/en-us/azure/active-directory/managed-service-identity/overview) per il server SQL logico, che consente di assegnare le autorizzazioni del server SQL logico per accedere a un insieme di credenziali delle chiavi. Non eliminare questa identità. L'accesso potrà essere revocato rimuovendo invece le autorizzazioni in Azure Key Vault per il server SQL logico, che consente di assegnare le autorizzazioni del server SQL logico per accedere all'insieme di credenziali delle chiavi.
- Creare il database primario. 
- Seguire il [materiale sussidiario di replica geografica attiva](https://docs.microsoft.com/en-us/azure/sql-database/sql-database-geo-replication-overview) per completare lo scenario, questo passaggio creerà il database secondario.
...
Poiché i server SQL logici esistono già e i database primari e secondari sono già stati assegnati, i passaggi per configurare Azure Key Vault devono essere eseguiti nell'ordine seguente: 
...
- Andare adesso al server SQL logico che ospita il database primario: 
...
Per risolvere questo problema, eseguire il cmdlet [Get-AzureRmSqlServerKeyVaultKey](/powershell/module/azurerm.sql/get-azurermsqlserverkeyvaultkey) per restituire l'elenco di chiavi da Key Vault aggiunte al server (a meno che non siano state eliminate da un utente). Per garantire che tutti i backup possano essere ripristinati, verificare che il server di destinazione per il backup abbia accesso a tutte queste chiavi.
   ...
   ```
Per altre informazioni sul ripristino dei backup per il database SQL, vedere [Recuperare un database SQL di Azure](https://docs.microsoft.com/azure/sql-database/sql-database-recovery-using-backups). Per altre informazioni sul ripristino dei backup per SQL Data Warehouse, vedere [Recuperare un Azure SQL Data Warehouse](https://docs.microsoft.com/azure/sql-data-warehouse/sql-data-warehouse-restore-database-overview).
