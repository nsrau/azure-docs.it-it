...
Alla fine della giornata, ad esempio alle 23:00 ora locale, le basi di dati attive devono passare alla regione successiva (Europa settentrionale). Questa attività può essere completamente automatizzata usando [App per la logica](../logic-apps/logic-apps-overview.md)di Azure.This task can be fully automated by using Azure Logic Apps . L'attività include i passaggi seguenti:
...
* Per informazioni sulla replica geografica attiva con pool elastici, vedere Strategie di ripristino di emergenza del [pool elastico.](sql-database-disaster-recovery-strategies-for-applications-with-elastic-pool.md)
