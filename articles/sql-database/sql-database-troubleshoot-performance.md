...
2.	Usare [Informazioni dettagliate prestazioni query](sql-database-query-performance.md) per valutare le query con le DTU e quindi usare [Advisor per database SQL](sql-database-index-advisor.md) per visualizzare raccomandazioni relative alla creazione e all'eliminazione di indici, alla parametrizzazione di query e alla correzione di problemi di schema.
