...
| **Pubblicazione**   | **Database di distribuzione** | **Sottoscrittore** |
| :------------   | :-------------- | :------------- |
...
- È necessario aprire la porta 1433 (TCP in uscita) se il server di pubblicazione/distribuzione si trova in un'istanza gestita e il Sottoscrittore non lo è. Potrebbe anche essere necessario modificare la regola di sicurezza in uscita NSG istanza gestita per `allow_linkedserver_outbound` per il tag del **servizio di destinazione** porta `virtualnetwork` 1433 `internet`da a. 
