...
| Crittografia a livello di colonna (Azure Data Services)| Yes | Attraverso [Always Encrypted](sql-database-always-encrypted.md). |
...
| Attributo di sicurezza | Sì/No | Note |
|---|---|--|
| Supporto endpoint del servizio| Yes | Si applica a [singolo database](sql-database-single-index.yml) solo. |
| Supportare l'inserimento di rete virtuale| Yes | Si applica a [istanza gestita](sql-database-managed-instance.md) solo. |
| Isolamento di rete e supporto firewall| Yes | In entrambi database - e a livello di server; del firewall isolamento di rete [istanza gestita](sql-database-managed-instance.md) solo |
| Supporto di tunneling forzato| Yes | [istanza gestita](sql-database-managed-instance.md) tramite [Azure ExpressRoute](../expressroute/index.yml) VPN |

## <a name="detection"></a>Rilevamento

| Attributo di sicurezza | Sì/No | Note|
|---|---|--|
| Monitoraggio di supporto (analitica di Log, Application insights e così via) di Azure| Yes | La soluzione SIEM di terze parti cybersicurezza di Imperva (SecureSphere) è supportata anche, tramite [hub eventi di Azure](../event-hubs/index.yml) integrazione tramite [SQL audit](sql-database-auditing.md). |
...
| Controllo e registrazione di piano dati | Yes | Tramite [SQL audit](sql-database-auditing.md). |
...
| Il rilevamento: rilevamento delle minacce | Yes | Visualizzare [Advanced Threat Protection per il Database SQL di Azure](sql-database-threat-detection-overview.md). |
