...
Il database SQL prevede tre opzioni di ripristino con i [backup automatici del database](sql-database-automated-backups.md). È possibile ripristinare un database durante il relativo [periodo di conservazione](sql-database-service-tiers.md) dei backup per:
...
I [backup automatici del database](sql-database-automated-backups.md) possono anche essere usati per creare una [copia del database](sql-database-copy.md) in qualsiasi server logico di qualsiasi area. Inoltre, è possibile configurare un server di database SQL per archiviare i backup di database SQL di Azure in un insieme di credenziali di Servizi di ripristino di Azure fino a 10 anni. Vedere l'articolo sulla [conservazione del backup a lungo termine](sql-database-long-term-retention.md).
...
* Per configurare, gestire e ripristinare dalla conservazione a lungo termine di backup automatici in un insieme di credenziali di Servizi di ripristino di Azure tramite Powershell, vedere l'articolo sulla [gestione della conservazione a lungo termine dei backup tramite il portale di PowerShell](sql-database-manage-long-term-backup-retention-powershell.md).
