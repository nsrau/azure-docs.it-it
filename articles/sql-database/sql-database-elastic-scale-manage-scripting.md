...
* [Introduzione all'Automazione di Azure ](http://azure.microsoft.com/documentation/articles/automation-create-runbook-from-samples/)
...
[Iscriversi](http://azure.microsoft.com/services/preview/) e [acquisire familiarità](http://azure.microsoft.com/documentation/articles/automation-create-runbook-from-samples/) con il servizio Anteprima Automazione di Microsoft. 
...
2. [Aggiungere riferimenti ai file binari di Elastic Scale mediante NuGet](sql-database-elastic-scale-add-references-visual-studio.md)
...
4. In Automazione di Azure, creare una **credenziale di asset** denominata *ElasticScaleCredential* che includa il nome utente e la password per il server di database SQL di Azure. 
5. Creare una **variabile di asset** denominata *SqlServerName* per il nome completo del server di database SQL di Azure. 
...
1. Accedere alla pagina [Anteprima funzionalità di Azure](http://azure.microsoft.com/services/preview/).
 
2. Fare clic su **Prova**.

 ![Click Try It][8]

2. Accedere al [portale Microsoft Azure](https://manage.windowsazure.com/microsoft.onmicrosoft.com).
...
1. Fare clic su **ASSET** sulla barra multifunzione nella parte superiore dello schermo.
2. Fare clic su **IMPORTA MODULO** nella parte inferiore della pagina. 
3. Fare clic su **CERCA FILE** e individuare il file **ShardElasticityModule.zip** precedente. 
...
2. Nell'account **ShardElasticityExamples** fare clic su **ASSETS** nella barra multifunzione.
3. Fare clic su **AGGIUNGI IMPOSTAZIONE** nella parte inferiore dello schermo. 
4. Fare clic su **AGGIUNGI CREDENZIALE**. 

 ![Add credential][9]
4. Selezionare **Credenziali Windows PowerShell** come **TIPO DI CREDENZIALI** e **ElasticScaleCredential** per il campo Name. La descrizione è facoltativa. 
...
4. Fare clic su **SALVA**, quindi su **TEST**.
5. Ripetere per **ReduceServiceTier**. 
