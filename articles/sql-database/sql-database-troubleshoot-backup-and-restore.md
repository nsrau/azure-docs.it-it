...
4.	Apportare le modifiche necessarie all'app per fare riferimento al nuovo database. Vedere [Ripristinare un database dall’errore dell’utente](sql-database-user-error-recovery.md).
...
5.	Apportare le modifiche necessarie all'app per fare riferimento al nuovo database. Vedere [Ripristinare un database dall’errore dell’utente](sql-database-user-error-recovery.md).

##Per eseguire il ripristino in seguito a un'interruzione del data center regionale
Con i database Standard e Premium, se sono stati configurati database secondari con replica geografica, è possibile usarli per eseguire il ripristino. In questo modo è possibile ripristinare un database con una possibilità inferiore che si verifichi una perdita di dati. Per informazioni dettagliate, vedere [Ripristinare un database SQL di Azure in seguito a un'interruzione del servizio](sql-database-disaster-recovery.md).
