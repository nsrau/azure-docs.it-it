---
title: Come configurare un'istanza gestita
...
- [Configurare il DNS personalizzato](sql-database-managed-instance-custom-dns.md) : è necessario configurare il DNS personalizzato se si vuole accedere alle risorse esterne nei domini personalizzati dall'istanza gestita tramite il server collegato dei profili di posta elettronica database.
...
- [Trova indirizzo IP dell'endpoint di gestione](sql-database-managed-instance-find-management-endpoint-ip-address.md) : istanza gestita usa l'endpoint pubblico per la gestione. È possibile determinare l'indirizzo IP dell'endpoint di gestione tramite lo script descritto qui.
- [Verificare la protezione del firewall incorporata](sql-database-managed-instance-management-endpoint-verify-built-in-firewall.md) : l'istanza gestita è protetta con un firewall incorporato che consente il traffico solo sulle porte necessarie. È possibile controllare e verificare le regole del firewall incorporato usando lo script descritto in questa guida.
- [Connetti applicazioni](sql-database-managed-instance-connect-app.md) : l'istanza gestita viene inserita nella VNet privata di Azure con un indirizzo IP privato. Informazioni su modelli differenti per connettere le applicazioni all'istanza gestita.
...
- La [creazione di avvisi](sql-database-managed-instance-alerts.md) consente di configurare gli avvisi sulle metriche monitorate, ad esempio utilizzo della CPU, consumo dello spazio di archiviazione, IOPS e altre per istanza gestita. In questa guida si apprenderà come abilitare e configurare gli avvisi per l'istanza gestita.

## <a name="next-steps"></a>Passaggi successivi

- Ulteriori informazioni sulle [guide per i database singoli](sql-database-howto-single-database.md)
