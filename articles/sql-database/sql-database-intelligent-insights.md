---
title: Monitorare le prestazioni del database con Intelligent InsightsMonitor database performance with Intelligent Insights
...
---
# <a name="intelligent-insights-using-ai-to-monitor-and-troubleshoot-database-performance-preview"></a>Intelligent Insights che usa l'intelligenza immagine per monitorare e risolvere i problemi relativi alle prestazioni del database (anteprima)Intelligent Insights using AI to monitor and troubleshoot database performance (Preview)

Intelligent Insights per il database SQL di Azure assicura un maggior controllo sulle prestazioni del database.

Intelligent Insights usa le funzionalità di intelligence integrata per monitorare in modo continuativo l'utilizzo del database grazie all'intelligenza artificiale e rilevare gli eventi che possono causare interruzioni del funzionamento e una riduzione delle prestazioni. Una volta rilevata, viene eseguita un'analisi dettagliata che genera un log delle risorse di Intelligent Insights (denominato SQLInsights) con una valutazione intelligente del problema. Questa valutazione è costituita da un'analisi della causa radice del problema di prestazioni del database e, dove possibile, da raccomandazioni per il miglioramento delle prestazioni.
...
Le metriche usate per misurare e rilevare i problemi di prestazioni del database si basano su durata delle query, richieste di timeout, tempi di attesa eccessivi e richieste con errori. Per ulteriori informazioni sulle metriche, vedere [Metriche di rilevamento](#detection-metrics).

Le riduzioni delle prestazioni del database SQL identificate vengono registrate nel log di SQLInsights con voci intelligenti costituite dalle proprietà seguenti:Identified SQL Database performance degradations are recorded in the SQLInsights log with intelligent entries that consist of the following properties:
...
Per una panoramica pratica sull'uso di Intelligent Insights con Azure SQL Analytics e per scenari di utilizzo tipici, vedere questo video:For a hands-on overview on using Intelligent Insights with Azure SQL Analytics and for typical usage scenarios, see this video:

> [!VIDEO https://channel9.msdn.com/Shows/Azure-Friday/Get-Intelligent-Insights-for-Improving-Azure-SQL-Database-Performance/player]
>

Intelligent Insights si distingue per l'individuazione e la risoluzione dei problemi di prestazioni del database SQL. Per usare Intelligent Insights per risolvere i problemi di prestazioni del database, vedere Risolvere i problemi di prestazioni del database SQL di [Azure con Intelligent Insights.](sql-database-intelligent-insights-troubleshoot-performance.md)

## <a name="intelligent-insights-options"></a>Opzioni di Intelligent Insights

Le opzioni di Intelligent Insights disponibili nel database SQL di Azure sono le seguenti:Intelligent Insights options available in Azure SQL Database are:

| Opzione Intelligent Insights | Supporto dei database singoli e in pool | Supporto del database dell'istanza |
| :----------------------------- | ----- | ----- |
| **Configurare Intelligent Insights** : Configurare l'analisi di Intelligent Insights per i database. | Sì | Sì |
| **Approfondimenti di flusso in Analisi SQL** di Azure: analisi del flusso nella soluzione di monitoraggio di Azure SQL Analytics per il database SQL di Azure.Stream insights to Azure SQL Analytics -- Stream insights to Azure SQL Analytics monitoring solution for Azure SQL Database. | Sì | Sì |
| **Informazioni dettagliate sul flusso all'hub eventi:** trasmettele informazioni dettagliate agli hub eventi per ulteriori integrazioni personalizzate. | Sì | Sì |
| **Trasmettere in streaming informazioni dettagliate in Archiviazione di Azure:** trasmettere informazioni dettagliate in Archiviazione di Azure per un'ulteriore analisi e archiviazione a lungo termine. | Sì | Sì |

## <a name="configure-the-export-of-the-intelligent-insights-log"></a>Configurare l'esportazione del log di Intelligent Insights

L'output di Intelligent Insights può essere trasmesso in uno dei diversi punti di destinazione per l'analisi:

- L'output trasmesso in un'area di lavoro di Log Analytics può essere usato con Analisi SQL di Azure per visualizzare le informazioni dettagliate tramite l'interfaccia utente del portale di Azure.Output streamed to a Log Analytics workspace can be used with [Azure SQL Analytics](https://docs.microsoft.com/azure/log-analytics/log-analytics-azure-sql) to view insights through the user interface of the Azure portal. Questa è la soluzione integrata di Azure e il modo più comune per visualizzare informazioni dettagliate.
- L'output trasmesso negli hub eventi di Azure può essere usato per lo sviluppo di scenari di monitoraggio e avvisi personalizzatiOutput streamed to Azure Event Hubs can be used for development of custom monitoring and alerting scenarios
- L'output trasmesso in Archiviazione di Azure può essere usato per lo sviluppo di applicazioni personalizzate, ad esempio la creazione di report personalizzati, l'archiviazione dei dati a lungo termine e così via.

L'integrazione di Azure SQL Analytics, Hub eventi di Azure, Archiviazione di Azure o prodotti di terze parti per l'utilizzo viene eseguita tramite prima l'abilitazione della registrazione di Intelligent Insights (il log "SQLInsights") nel pannello Impostazioni di diagnostica di un database e quindi configurazione dei dati di log di Intelligent Insights da trasmettere in una di queste destinazioni.

Per altre informazioni su come abilitare la registrazione di Intelligent Insights e configurare i dati di log delle metriche e delle risorse in un prodotto consumer, vedere Metriche del database SQL di [Azure e registrazione diagnostica.](sql-database-metrics-diag-logging.md)

### <a name="set-up-with-azure-sql-analytics"></a>Configurazione con Analisi SQL di Azure

La soluzione Analisi SQL di Azure offre funzionalità di interfaccia utente grafica, creazione di report e avvisi sulle prestazioni del database, usando i dati del log delle risorse di Intelligent Insights.Azure SQL Analytics solution provides graphical user interface, reporting and alerting capabilities on database performance, using the Intelligent Insights resource log data.

Aggiungere Analisi SQL di Azure al dashboard del portale di Azure dal marketplace e per creare un'area di lavoro, vedere [configurare Azure SQL Analytics](../azure-monitor/insights/azure-sql.md#configuration)
...
Per usare Intelligent Insights con Hub eventi, configurare i dati del log di Intelligent Insights da trasmettere agli hub eventi, vedere [Metriche del database SQL di Azure e registrazione](sql-database-metrics-diag-logging.md) diagnostica e Eseguire lo streaming dei log di diagnostica di Azure in Eventi [Hub.](../azure-monitor/platform/resource-logs-stream-event-hubs.md)
...
Per usare Intelligent Insights con Archiviazione, configurare i dati del log di Intelligent Insights da trasmettere all'archiviazione, vedere [Metriche e diagnostica](sql-database-metrics-diag-logging.md) del database SQL di Azure e Registrazione diagnostica e [Flusso nell'archiviazione](sql-database-metrics-diag-logging.md#stream-into-azure-storage)di Azure.
