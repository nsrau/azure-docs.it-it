...
2. Nella pagina Database SQL del database fare clic su **Strumenti** sulla barra degli strumenti. Verrà visualizzata la pagina Strumenti.

    ![Menu Strumenti](./media/sql-database-connect-query-portal/tools-menu.png)

3. Fare clic su **Editor di query (anteprima)**, fare clic sulla casella di controllo **Condizioni per l'anteprima** e quindi su **OK**. Si apre la pagina Editor di query.

4. Fare clic su **Accedi** e quindi, quando viene richiesto, selezionare **Autenticazione di SQL Server** e immettere l'account di accesso e la password di amministratore del server specificati durante la creazione del database.
...
Passare al database SQL su cui si vogliono eseguire query, fare clic sul comando **Strumenti** sulla barra degli strumenti e selezionare l'opzione **Editor di query (anteprima)**. Verrà visualizzata la pagina Editor di query e verrà eseguita automaticamente la connessione al database.
