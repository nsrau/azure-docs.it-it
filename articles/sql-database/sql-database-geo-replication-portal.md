---
title: Configurare la replica geografica per il database SQL di Azure con il portale di Azure | Microsoft Docs
...
1. Configurare **Tipo secondario** (**Leggibile** o **Non leggibile**).
2. Selezionare o configurare il server per il database secondario.
   
    ![Creare il database secondario][3]
3. Facoltativamente, è possibile aggiungere un database secondario a un pool di database elastici:
...
4. Fare clic su **Crea** per aggiungere il database secondario.
5. Il database secondario viene creato e viene avviato il processo di seeding.
   
    ![seeding][6]
6. Una volta completato il processo di seeding il database secondario visualizza il relativo stato (non leggibile.
