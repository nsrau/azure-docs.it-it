...
description: Informazioni su come usare Azure Active Directory per l'autenticazione con il database SQL, Istanza gestita e la sinapsi di Azure
...
3. Creare un amministratore di Azure Active Directory per il server di database SQL di Azure, il Istanza gestita o il [pool SQL](https://azure.microsoft.com/services/sql-data-warehouse/).
...
- Azure AD utenti che fanno parte di un gruppo che dispone di `db_owner` ruolo del server non possono usare la sintassi di **[creazione di credenziali con ambito database](/sql/t-sql/statements/create-database-scoped-credential-transact-sql)** per il database SQL di Azure e la sinapsi di Azure. Verrà visualizzato l'errore seguente:
...
- Solo un amministratore Azure AD per SQL Server può connettersi inizialmente al server del database SQL di Azure, Istanza gestita o alla sinapsi SQL di Azure usando un account Azure Active Directory. L'amministratore di Active Directory può configurare gli utenti del database di Azure AD successivi.   
