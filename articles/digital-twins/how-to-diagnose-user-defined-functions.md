---
title: Come eseguire il debug di UDF-dispositivi gemelli digitali di Azure | Microsoft Docs
description: Informazioni sugli approcci consigliati per eseguire il debug di funzioni definite dall'utente nei dispositivi gemelli digitali di Azure.
ms.author: alinast
...
* Vedere l'articolo [raccogliere e usare i dati di log dalle risorse di Azure](../azure-monitor/platform/platform-logs-overview.md) per configurare le impostazioni del log di diagnostica nei dispositivi gemelli digitali di Azure tramite il portale di Azure, l'interfaccia della riga di comando di Azure o PowerShell.
...
Per leggere la query dei log di telemetria recenti:
...
Se si Abilita la registrazione per la funzione definita dall'utente, i log vengono visualizzati nell'istanza di log Analytics con la categoria `UserDefinedFunction`. Per recuperarle, immettere la condizione di query seguente in log Analytics:
...
- Per altre opzioni di registrazione di Azure, vedere l'articolo [Panoramica del log attività di Azure](../azure-monitor/platform/platform-logs-overview.md) .
