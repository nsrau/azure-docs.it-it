...
Come si può osservare, in questo momento non è presente un output che indichi che la chiamata è riuscita. 
...
Per poter creare relazioni, aggiungere un'istruzione `using` per il tipo di base di relazione nell'SDK:
```csharp
using Azure.DigitalTwins.Core.Serialization;
```

Aggiungere quindi un nuovo metodo statico alla classe `Program`, sotto il metodo `Main`:
```csharp
...
* [Procedura: Gestire un modello di gemello](how-to-manage-model.md)
