<properties 
	pageTitle="Creazione di un'app ASP.NET MVC con autenticazione e database SQL e distribuzione in servizio app di Azure" 
...
# Creazione di un'app ASP.NET MVC con autenticazione e database SQL e distribuzione in servizio app di Azure

In questa esercitazione verrà illustrato come creare un'app Web ASP.NET MVC 5 sicura che consente agli utenti di effettuare l'accesso usando credenziali di Facebook o Google. L'applicazione verrà inoltre distribuita nel [servizio app](http://go.microsoft.com/fwlink/?LinkId=529714).

È possibile aprire gratuitamente un account di Azure e, se non si dispone già di Visual Studio 2013, con l'SDK verrà installato automaticamente Visual Studio Express 2013 per il Web. È possibile iniziare a sviluppare per Azure gratuitamente.
...
* Creare un progetto ASP.NET MVC 5 sicuro e pubblicarlo in [servizio app per app Web](http://go.microsoft.com/fwlink/?LinkId=529714) nel servizio app di Azure.
...
>[AZURE.NOTE] Per completare l'esercitazione, è necessario un account Microsoft Azure. Se non si dispone di un account, è possibile attivare <a href="/it-it/pricing/member-offers/msdn-benefits-details/?WT.mc_id=A261C142F" target="_blank">i vantaggi della sottoscrizione a MSDN</a> oppure <a href="/it-it/pricing/free-trial/?WT.mc_id=A261C142F" target="_blank">iscriversi per ottenere una versione di valutazione gratuita</a>.
...
1. Nella finestra di dialogo **Nuovo progetto ASP.NET** selezionare il modello **MVC**. Verificare che **Autenticazione** sia impostato su **Singoli account utente** e che le opzioni **Ospita nel cloud** e **App Web** siano selezionate.
...
1. Abilitare SSL. In Esplora soluzioni fare clic sul progetto **ContactManager**, quindi fare clic su F4 per visualizzare la finestra di dialogo delle proprietà. Modificare il valore di **SSL abilitato** in true. Copiare il valore **URL SSL**. L'URL SSL sarà https://localhost:44300/ a meno che non siano state create app Web SSL in precedenza.
...
2. Nella finestra **Console di Gestione pacchetti** immettere il comando seguente:
	...
	Il comando **add-migration Initial** consente di generare un file denominato **&lt;timbro_data&gt;Initial** nella cartella *Migrations* creata dal database. Il primo parametro (**Initial** ) è arbitrario e viene usato per creare il nome del file. È possibile visualizzare i nuovi file di classe in **Esplora soluzioni**.
...
7. Premere CTRL+F5 per eseguire l'applicazione, quindi fare clic sul link **CM Demo** oppure passare https://localhost:(port#)/Cm. 
...
1. Aprire il file **Controllers\AccountController.cs** e passare il metodo **ExternalLoginConfirmation**.
1. Aggiungere la chiamata **AddToRoleAsync** immediatamente prima di quella **SignInAsync**.
...
In questa sezione, verrà applicato l'attributo [Authorize](http://msdn.microsoft.com/library/system.web.mvc.authorizeattribute.aspx) per limitare l'accesso ai metodi di azione. Gli utenti anonimi saranno in grado di visualizzare il metodo di azione **Index** solo per il controller Home. Gli utenti registrati saranno in grado di visualizzare i dati sui contatti (pagine **Index** e **Details** del controller Cm) e le pagine About e Contact. Soltanto gli utenti appartenenti al ruolo  *canEdit* saranno in grado di accedere ai metodi di azione e di modificare i dati.

1. Aggiungere i filtri [Authorize](http://msdn.microsoft.com/library/system.web.mvc.authorizeattribute.aspx) e [RequireHttps](http://msdn.microsoft.com/library/system.web.mvc.requirehttpsattribute.aspx) all'applicazione. Un approccio alternativo consiste nell'aggiungere gli attributi [Authorize](http://msdn.microsoft.com/library/system.web.mvc.authorizeattribute.aspx) e [RequireHttps](http://msdn.microsoft.com/library/system.web.mvc.requirehttpsattribute.aspx) a ogni controller, ma è consigliabile applicarli all'intera applicazione, per assicurare una protezione migliore. Grazie all'aggiunta a livello globale, ogni nuovo controller e ogni metodo di azione aggiunti saranno protetti automaticamente e non sarà necessario ricordarsi di applicare gli attributi. Per altre informazioni, vedere [Protezione dell'app ASP.NET MVC e del nuovo attributo AllowAnonymous](http://blogs.msdn.com/b/rickandy/archive/2012/03/23/securing-your-asp-net-mvc-4-app-and-the-new-allowanonymous-attribute.aspx). Aprire il file  *App_Start\FilterConfig.cs* e sostituire il metodo *RegisterGlobalFilters* con il seguente (consente di aggiungere due filtri):
		<pre>
        public static void
...
2. Eseguire una ricerca globale di  *AllowAnonymous*; come si può vedere, viene usato nei metodi di accesso e registrazione del controller Account.
1. In  *CmController.cs*, aggiungere `[Authorize(Roles = "canEdit")]` ai metodi HttpGet e HttpPost mche consentono di modificare i dati (Creazione, Modifica, Eliminazione, qualsiasi metodo di azione ad eccezione di Indice e Dettagli) nel controller  *Cm*. Di seguito è riportata una parte del codice completato: 
		<pre>
	// GET: Cm/Create
...
1. Accedere come  *user1@contoso.com* (utilizzando la password "P_assw0rd1") e verificare che sia possibile modificare i dati.
...
4. Fare clic con il pulsante destro del mouse sull'app Web e selezionare **Interrompi**. 
	...
	In alternativa, dal [portale di Azure](http://go.microsoft.com/fwlink/?LinkId=529715), è possibile selezionare l'app Web e fare clic sull'icona d'**interruzione** presente nella parte inferiore della pagina.
...
1. Prendere nota dell'ID dell'account Google usato per la registrazione nel ruolo **canEdit** e dell'ID  *user1@contoso.com*. Questi devono essere gli unici utenti nel ruolo **canEdit**. Questa verifica verrà eseguita nel passaggio successivo.
...
Per informazioni più avanzate sulla programmazione MVC ed EF, vedere la pagina di [introduzione a EF e MVC](http://www.asp.net/mvc/tutorials/getting-started-with-ef-using-mvc/creating-an-entity-framework-data-model-for-an-asp-net-mvc-application) creata da Tom Dykstra.
...
[Informazioni importanti su ASP.NET nelle app Web di Azure]: #aspnetwindowsazureinfo
