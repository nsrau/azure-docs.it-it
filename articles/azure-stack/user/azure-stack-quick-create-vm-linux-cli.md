...
Perché questa macchina virtuale verrà eseguito il server web IIS, è necessario aprire la porta 80 al traffico Internet. Usare il comando [az vm open-port](/cli/azure/vm#open-port) per aprire la porta.
...
Da un computer client con SSH installato, connettersi alla macchina virtuale. Se si lavora in un client Windows, usare [Putty](http://www.putty.org/) per creare la connessione. Per connettersi alla macchina virtuale, usare il comando seguente:
