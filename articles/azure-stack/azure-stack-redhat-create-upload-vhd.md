---
title: Creare e caricare un file di Red Hat Enterprise Linux VHD per l'utilizzo nello Stack di Azure | Documenti Microsoft
...
* Stack di Azure non supporta cloud init. Si macchina virtuale deve essere configurato con una versione supportata di Windows Azure Linux agente (WALA).
...
A questo punto è pronti a utilizzare il disco rigido virtuale Red Hat Enterprise Linux per creare nuove macchine virtuali nello Stack di Azure. Se questa è la prima volta che si sta caricando il file con estensione vhd allo Stack di Azure, vedere [usare il toolkit di Marketplace per creare e pubblicare elementi del marketplace](azure-stack-marketplace-publisher.md).
