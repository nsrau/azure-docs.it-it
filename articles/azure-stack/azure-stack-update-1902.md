...
- Prima di iniziare l'installazione di questo aggiornamento, eseguire [Test-AzureStack](azure-stack-diagnostic-test.md) con i parametri seguenti per convalidare lo stato di Azure Stack e risolvere eventuali problemi operativi trovati, inclusi tutti gli avvisi e gli errori. Anche gli avvisi attivi rivedere e risolvere gli eventuali che richiedono un'azione:
  ...
  Se il `AzsControlPlane` è incluso quando **Test AzureStack** viene eseguita, verrà visualizzato il seguente errore il **Test AzureStack** output: **Riepilogo di siti Web di piano di controllo di FAIL Azure Stack**. È possibile ignorare questo errore specifico.
...
- Miglioramenti all'affidabilità di espansione della capacità durante un'operazione di nodo aggiunta quando si passa lo stato di unità di scala da "Archivio di espansione" su "Running".
