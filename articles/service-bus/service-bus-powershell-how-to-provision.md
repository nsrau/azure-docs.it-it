---
title: Gestione del Bus di servizio con PowerShell | Microsoft Docs
...
Due cmdlet di PowerShell supportano le operazioni di spazio dei nomi del bus di servizio. Anziché le API di .NET SDK, è possibile utilizzare [Get-AzureSBNamespace][Get-AzureSBNamespace] e [New-AzureSBNamespace][New-AzureSBNamespace].
...
Per il provisioning di altre entità del bus di servizio, viene creata un'istanza della classe [NamespaceManager][NamespaceManager] dall'SDK. È possibile usare il cmdlet [Get-AzureSBAuthorizationRule][Get-AzureSBAuthorizationRule] per recuperare una regola di autorizzazione usata per fornire una stringa di connessione. Verrà archiviato un riferimento all'istanza di `NamespaceManager` nella variabile `$NamespaceManager`. `$NamespaceManager` verrà utilizzato in seguito nello script per il provisioning di altre entità.
