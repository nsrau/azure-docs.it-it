<properties pageTitle="Uso dei dati offline in Servizi mobili (Xamarin per iOS) | Mobile Dev Center" description="Learn how to use Azure Mobile Services to cache and sync offline data in your Xamarin iOS application" documentationCenter="xamarin" authors="lindydonna" editor="wesmc" manager="dwrede" services=""/>
...
1. Aprire `QSTodoService.cs`. Modificare i metodi `InsertTodoItemAsync()` e `CompleteItemAsync()` e impostare come commento le chiamate a `SyncAsync()`.
...
3. Per modificare l'inizializzazione dell'interfaccia utente in modo che non esegua la sincronizzazione all'avvio dell'app, modificare il metodo `QSTodoListViewController.ViewDidLoad()`. Rimuovere la chiamata a `RefreshAsync()` alla fine del metodo e sostituirla con una chiamata a `ReloadData()`:
