<properties urlDisplayName="Learn Azure SQL Database and SQL Server in Azure VMs" pageTitle="Informazioni sul database SQL di Azure e su SQL Server in Macchine virtuali di Azure" metaKeywords="SQL, Azure SQL Database, SQL Server IAAS, SQL Server in Azure VM" description="Learn Azure SQL Database and SQL Server in Azure Virtual Machines. Review common business motivators for determining which SQL technology works best for your application." metaCanonical="" services="virtual-machines" documentationCenter="" title="Understanding Azure SQL Database and SQL Server in Azure VMs" authors="selcint" solutions="data-management" manager="jeffreyg" editor="tysonn" />
...
- [Motivi aziendali alla base della scelta del database SQL di Azure o di SQL Server in una macchina virtuale di Azure](#business)	
	- [Costo](#cost)
		...
		- [Calcolo dei costi totali dell'applicazione](appcost)	
...
Nelle seguenti sezioni vengono fornite informazioni sulle ultime due opzioni: il database SQL di Azure e SQL Server in Macchine virtuali di Azure. Inoltre, vengono trattati i vantaggi aziendali più diffusi che consentono di determinare l'opzione ottimale per la propria applicazione.
...
<tr>
   ...
   <td valign="middle">
      ...
      <li type=round>Ripristino di emergenza per applicazioni SQL Server locali tramite il <a href="http://msdn.microsoft.com/library/jj919148.aspx">backup nell'archiviazione di Azure</a> o <a href="http://msdn.microsoft.com/library/azure/jj870962.aspx">repliche AlwaysOn in Macchine virtuali di Azure</a>.
...
<tr>
   ...
   <td valign="middle"><ul>
      <li type=round>Con SQL Server in Macchine virtuali di Azure, le applicazioni possono essere eseguite in parte nel cloud e in parte localmente. Ad esempio, è possibile estendere la rete locale e Active Domain Directory nel cloud tramite i <a href="http://msdn.microsoft.com/library/azure/gg433091.aspx">servizi di rete di Azure</a>. Inoltre è possibile archiviare i file di dati locali nell'archiviazione di Azure usando la <a href="http://msdn.microsoft.com/library/dn385720.aspx">funzionalità File di dati di SQL Server in Azure</a>. Per altre informazioni, vedere <a href="http://msdn.microsoft.com/library/dn606154.aspx">Introduzione al cloud ibrido di SQL Server 2014</a>.
      <li type=round>Supporta il ripristino di emergenza per le applicazioni SQL Server locali  usando il   <a href="http://msdn.microsoft.com/library/jj919148.aspx">backup nell'archiviazione di Azure</a> o le repliche <a href="http://msdn.microsoft.com/library/azure/jj870962.aspx">AlwaysOn in Macchine virtuali di Azure</a>.
      </ul></td>
...
####<a name="cost"></a>Calcolo del costo totale dell'applicazione
...
- [Dettagli prezzi per la macchina virtuale](http://azure.microsoft.com/pricing/details/virtual-machines/)
...
D'altra parte, se si hanno risorse interne esperte, si potrebbe voler mantenere il controllo su tutto il percorso del database fino allo stesso computer. Con **SQL Server in esecuzione in una macchina virtuale di Azure** si ha il pieno controllo del sistema operativo e della configurazione dell'istanza di SQL Server. Con una macchina virtuale, l'utente può decidere quando aggiornare il sistema operativo e il software del database e quando installare software aggiuntivi come gli strumenti anti-virus e di backup. Inoltre, è possibile controllare le dimensioni della macchina virtuale, il numero di dischi e le relative configurazioni di archiviazione. Ad esempio, Azure consente di modificare le dimensioni di una macchina virtuale in esecuzione in base alle esigenze. Per informazioni, vedere [Dimensioni delle macchine virtuali e dei servizi cloud per Azure](http://msdn.microsoft.com/library/azure/dn197896.aspx).

###<a name="sla"></a>Contratto di servizio

Per alcuni di noi, rispettare gli obblighi relativi al tempo di attività di un contratto di servizio è della massima priorità. In questa sezione vengono spiegate le condizioni del contratto di servizio per ciascuna opzione di hosting del database.
...
Il **database SQL di Azure** è la soluzione ideale per le applicazioni progettate per il cloud quando i fattori critici sono la produttività degli sviluppatori e un time-to-market rapido. Con la funzionalità di tipo DBA programmatica, è lo strumento ideale per gli architetti e gli sviluppatori cloud perché riduce la gestione del sistema operativo e del database sottostanti. Aiuta gli sviluppatori a comprendere e configurare le attività relative al database. Ad esempio, è possibile usare i cmdlet [REST API](http://msdn.microsoft.com/library/gg715283.aspx) e [PowerShell](http://msdn.microsoft.com/library/azure/dn546726.aspx) per automatizzare e gestire le operazioni amministrative di migliaia di database. Con la scalabilità elastica nel cloud, è possibile concentrarsi sul livello dell'applicazione e rilasciare più rapidamente l'applicazione sul mercato. 

**SQL Server in esecuzione in una macchina virtuale di Azure** è la soluzione ottimale se le applicazioni esistenti e nuove richiedono l'accesso e il controllo per tutte le funzionalità di un'istanza di SQL Server e se si vuole eseguire la migrazione delle applicazioni e dei database locali esistenti al cloud senza modificarli. Poiché non è necessario modificare la presentazione, l'applicazione e i livelli dati, si risparmia il tempo e i costi legati a una nuova progettazione dell'architettura della soluzione esistente. In questo modo, è possibile concentrarsi sulla migrazione di tutti i pacchetti della soluzione alle macchine virtuali e sull'ottimizzazione delle prestazioni richiesta dalla piattaforma Azure. Per informazioni, vedere [Procedure consigliate per le prestazioni per SQL Server in Macchine virtuali di Azure](http://msdn.microsoft.com/library/azure/dn133149.aspx).

##<a name="summary"></a>Riepilogo

In questo articolo sono stati discussi: il database SQL di Azure e SQL Server in una macchina virtuale di Azure. Inoltre, si è discusso dei vantaggi aziendali più diffusi che possono influire sulla decisione della soluzione da adottare.  
...
- Si stanno compilando nuove applicazioni basate su cloud o si vuole eseguire la migrazione del database SQL Server esistente in Azure e il database non usa una delle funzionalità non supportate nel database SQL di Azure. Per altre informazioni, vedere [Istruzioni Transact-SQL non supportate](http://msdn.microsoft.com/library/azure/ee336253.aspx). Questo approccio offre tutti i vantaggi di un servizio cloud completamente gestito e assicura un rapido time-to-market.

- Si vuole che Microsoft esegua operazioni di gestione comuni sui database e si richiedono contratti di servizio con disponibilità più elevata per i database. Questo approccio consente di ridurre i costi amministrativi e, contemporaneamente, fornisce una disponibilità garantita per il database. 
...
<p><a href="http://msdn.microsoft.com/library/azure/jj823132.aspx">MSDN: SQL Server in Macchine virtuali di Azure</a></p>
