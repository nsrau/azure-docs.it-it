<properties
   pageTitle="Aggiornamento di un'app di infrastruttura di servizi mediante PowerShell | Microsoft Azure"
   description="In questo articolo vengono esaminati l'esperienza di distribuzione di un'applicazione di infrastruttura di servizi, la modifica del codice e l'implementazione di un aggiornamento tramite PowerShell."
...
È anche necessario aggiornare il file *ServiceManifest.xml* (in PackageRoot) del progetto **VisualObjects.ActorService**. Aggiornare *CodePackage* e la versione del servizio alla versione 2.0, nonché le righe corrispondenti nel file *ServiceManifest.xml*. È possibile usare l'opzione *Modifica i file manifesto* di Visual Studio dopo aver fatto clic con il pulsante destro del mouse sulla soluzione per modificare il file manifesto. Vedere [Esercitazione sull'aggiornamento di un'applicazione di infrastruttura di servizi](service-fabric-application-tutorial.md).
...
È ora necessario aggiornare il file *ApplicationManifest.xml* (nel progetto **VisualObjects** della soluzione **VisualObjects**) in modo da usare la versione 2.0 del progetto **VisualObjects.ActorService** e aggiornare anche la versione dell'applicazione a 2.0.0.0 da 1.0.0.0. Le righe corrispondenti nel file *ApplicationManifest.xml* ora dovrebbero essere simili alle seguenti:
