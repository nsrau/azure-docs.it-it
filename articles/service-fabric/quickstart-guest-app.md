...
Prima di iniziare, assicurarsi di avere [configurato l'ambiente di sviluppo](service-fabric-get-started.md). Ciò include l'installazione di Service Fabric SDK e Visual Studio 2019 o 2015.
...
Il progetto di applicazione (**MyGuestApp**) non contiene codice direttamente. Il progetto fa riferimento a un set di progetti di servizio. Contiene inoltre tre altri tipi di contenuto:
...
Aprire il file **ServiceManifest.xml** nel progetto. Nella parte inferiore del manifesto, è presente un `<Resources> \ <Endpoints>` con una voce già definita. Modificare la voce aggiungendo `Port`, `Protocol` e `Type`. 
...
In **Endpoint connessione** selezionare il cluster di Service Fabric in cui eseguire la distribuzione. Se non si dispone di uno, selezionare **&lt;Crea nuovo Cluster... &gt;** verrà aperta una finestra del web browser nel portale di Azure. Per altre informazioni, vedere [Creare un cluster nel portale](service-fabric-cluster-creation-via-portal.md#create-cluster-in-the-azure-portal). 
...
Passa a questo indirizzo in cui sono disponibili le `HELLO WORLD` risposta.

## <a name="delete-the-cluster"></a>Eliminare il cluster

Non dimenticare di eliminare tutte le risorse create per questa Guida introduttiva, in quanto viene addebitato tali risorse.
