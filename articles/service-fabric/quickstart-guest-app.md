...
Se non si dispone di una sottoscrizione di Azure, creare un [account gratuito.][create-account]
...
Il progetto dell'applicazione (**MyGuestApp**) non contiene direttamente codice. Il progetto fa riferimento a un insieme di progetti di servizio. Inoltre, contiene altri tre tipi di contenuto:
...
Aprire il file **ServiceManifest.xml** nel progetto. Nella parte inferiore del manifesto è `<Resources> \ <Endpoints>` presente una voce con una voce già definita. Modificare la voce aggiungendo `Port`, `Protocol` e `Type`. 
...
Se si preme **F5** ed eseguire il progetto, viene distribuito nel cluster locale. Si eseguirà invece la distribuzione in Azure.
...
In **Endpoint connessione** selezionare il cluster di Service Fabric in cui eseguire la distribuzione. Se non ne hai uno, seleziona ** &lt;Crea nuovo cluster... &gt; ** che apre la finestra del Web browser nel portale di Azure.Which opens web browser window to the Azure portal. Per altre informazioni, vedere [Creare un cluster nel portale](service-fabric-cluster-creation-via-portal.md#create-cluster-in-the-azure-portal). 
...
Il completamento della creazione di un nuovo cluster di Service Fabric richiede tempo. Una volta creato, tornare alla finestra di ** &lt;&gt;** dialogo di pubblicazione e selezionare Aggiorna . Il nuovo cluster sarà incluso nella casella di riepilogo a discesa. Selezionare il cluster.
...
Passare a questo indirizzo in `HELLO WORLD` cui verrà visualizzata la risposta.

## <a name="delete-the-cluster"></a>Eliminare il cluster

Non dimenticare di eliminare tutte le risorse che hai creato per questa guida introduttiva, poiché ti vengono addebitati gli addebiti per tali risorse.
