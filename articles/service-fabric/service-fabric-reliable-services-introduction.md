...
* Un modello di comunicazione modulare. Usare il trasporto di propria scelta, ad esempio HTTP con [API Web](service-fabric-reliable-services-communication-webapi.md), WebSockets, protocolli TCP personalizzati e così via. Reliable Services fornisce alcune interessanti opzioni predefinite, ma consente anche di usarne di personalizzate.
...
* **Scalabilità**: i servizi vengono separati dall'hardware specifico e possono essere aumentati o ridotti secondo necessità mediante l'aggiunta o la rimozione di risorse hardware o virtuali. I servizi, in particolare quelli con stato, possono essere partizionati facilmente. In questo modo, le porzioni indipendenti del servizio possono essere scalate e, in caso di errore, possono rispondere in modo indipendente. Service Fabric, infine, permette di ottenere servizi "leggeri" consentendo il provisioning di migliaia di servizi con un unico processo, evitando così di richiedere o dedicare intere istanze del sistema operativo a una singola istanza di un particolare carico di lavoro.
* **Coerenza**: è possibile garantire la coerenza di tutte le informazioni memorizzate in questo servizio (si applica solo ai servizi con stati; altre informazioni su questo argomento saranno fornite più avanti)
...
### <a name="service-shutdown"></a>Arresto del servizio
Quando il servizio viene arrestato (per l'eliminazione, l'aggiornamento o il trasferimento), viene eseguito il mirroring dell'ordine delle chiamate. Innanzitutto viene annullato il token di annullamento di proprietà di `RunAsync()`, in secondo luogo viene chiamato `CloseAsync()` nei listener di comunicazione.

Esistono alcuni aspetti importanti da considerare riguardo al processo di arresto per i servizi con stato:

* Service Fabric non promuoverà un'altra replica del servizio allo stato primario fino alla restituzione di `CloseAsync` e `RunAsync`. Se si usa un listener di comunicazione predefinito, il metodo `CloseAsync` viene gestito automaticamente.
...
> [!NOTE]
> *Funzionalità disponibili con la disponibilità generale dell'SDK.
> 
> 
