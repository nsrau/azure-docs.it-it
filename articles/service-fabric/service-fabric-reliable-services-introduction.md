---
title: Panoramica del modello di programmazione Reliable Service
...
Azure Service Fabric semplifica la scrittura e la gestione dei servizi senza stato e con stato. In questo argomento viene trattato quanto segue:
...
*Reliable Services* è uno dei modelli di programmazione disponibili su Service Fabric. Un altro è il modello di *programmazione Reliable Actor,* che fornisce un framework di applicazione [Attore virtuale](https://research.microsoft.com/en-us/projects/orleans/) sopra il modello Reliable Services. Per altre informazioni su Reliable Actors, vedere [Introduzione a Reliable Actors in Service Fabric](service-fabric-reliable-actors-introduction.md).
...
## <a name="what-are-reliable-services"></a>Che cosa sono i servizi affidabili
...
* Accesso alle API di Service Fabric. A differenza dei servizi di Service Fabric modellati come [eseguibili guest,](service-fabric-guest-executables-introduction.md)Reliable Services può usare direttamente le API di Service Fabric. Questo consente ai servizi di:
  ...
  * Utilizzare le [raccolte affidabiliUse](service-fabric-reliable-services-reliable-collections.md) the Reliable Collections
  * Accedi a molte altre funzionalità, il tutto da un modello di programmazione di prima classe in diversi linguaggi di programmazione.
* Un modello semplice per l'esecuzione di codice personalizzato che si sente come altri modelli di programmazione familiari. Il codice ha un punto di ingresso ben definito e un ciclo di vita facile da gestire.
...
## <a name="what-makes-reliable-services-different"></a>Cosa rende Reliable Services diverso

I servizi Affidabili sono diversi dai servizi che potrebbero essere stati scritti in precedenza, perché Service Fabric fornisce:Reliable Services are different from services you may have written before, because Service Fabric provides:
...
* **Scalabilità**: i servizi vengono separati dall'hardware specifico e possono essere aumentati o ridotti secondo necessità mediante l'aggiunta o la rimozione di risorse hardware o di altro tipo. I servizi, in particolare quelli con stato, possono essere partizionati facilmente. In questo modo, il servizio dispone di scalabilità per gestire gli errori parziali. I servizi possono essere creati ed eliminati dinamicamente tramite codice, consentendo di creare più istanze in base alle esigenze, ad esempio in risposta alle richieste dei clienti. Infine, Service Fabric promuove l'uso di servizi leggeri. Service Fabric permette il provisioning di migliaia di servizi con un unico processo, evitando così di richiedere o dedicare intere istanze o processi del sistema operativo a una singola istanza di un servizio.
* **Coerenza:** tutte le informazioni archiviate in un servizio Affidabile possono essere garantite per essere coerenti. Questo vale anche attraverso più Reliable Collections all'interno di un servizio. È possibile apportare modifiche nelle raccolte all'interno di un servizio con transazioni atomiche.

## <a name="service-lifecycle"></a>Ciclo di vita del servizio

Reliable Services fornisce un ciclo di vita semplice che consente di attivare rapidamente il codice e iniziare a usarlo.  Per eseguire un nuovo servizio è necessario implementare due metodi:Getting a new service up and running requires you to implement two methods:
...
Se è la prima volta che si sente parlare di Reliable Services, è consigliabile continuare a leggere. Se si sta cercando una procedura dettagliata del ciclo di vita dei servizi affidabili, vedere [Panoramica del ciclo](service-fabric-reliable-services-lifecycle.md)di vita di Reliable Services .

## <a name="example-services"></a>Servizi di esempio

Diamo un'occhiata più da vicino come funziona il modello Reliable Services con i servizi senza stato e con stato.

### <a name="stateless-reliable-services"></a>Reliable Services senza stato

Un *servizio senza stato* è uno in cui non vi è alcuno stato mantenuto all'interno del servizio tra le chiamate. Qualsiasi stato presente è interamente eliminabile e non richiede sincronizzazione, replica, persistenza o disponibilità elevata.
...
Si supponga di voler scrivere un servizio di elaborazione immagini. Per l'elaborazione, il servizio accetta un'immagine e la serie di conversioni da eseguire su di essa. Il servizio restituisce un listener di comunicazione (si supponga che sia un'API Web) che espone un'API come `ConvertImage(Image i, IList<Conversion> conversions)`. Quando riceve una richiesta, il servizio `IReliableQueue`la archivia in un oggetto e restituisce un ID al client in modo che possa tenere traccia della richiesta.
...
Sebbene questo servizio sia simile a un tipico servizio .NET, la differenza è che le strutture di dati utilizzate (`IReliableQueue` e `IReliableDictionary`) sono fornite da Service Fabric e sono altamente affidabili, disponibili e coerenti.

## <a name="when-to-use-reliable-services-apis"></a>Quando usare le API Reliable Services

Considerare le API di Reliable Services se:Consider Reliable Services APIs if:

* Si desidera che il codice del servizio (e facoltativamente lo stato) sia altamente disponibile e affidabile.
