...
```
dove `<path to ClusterConfig.json>` e `<path to MicrosoftAzureServiceFabric.cab>` sono rispettivamente i percorsi per la configurazione del cluster e per il file di runtime con estensione cab.
