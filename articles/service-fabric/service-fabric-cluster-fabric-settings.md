...
|ClusterId |string | Dinamica |L'ID univoco del cluster. Viene generato quando viene creato il cluster. |
|ConsumerInstances |string | Dinamica |L'elenco delle istanze di consumer DCA. |
...
|ProducerInstances |string | Dinamica |L'elenco delle istanze di producer DCA. |
...
| CnsNetworkPluginCnmUrlPort | wstring, valore predefinito: L "48080" | Statica | Porta URL dell'API CNM di Azure |
| CnsNetworkPluginCnsUrlPort | wstring, valore predefinito: L "10090" | Statica | Porta URL di Azure CNS |
...
| DoNotInjectLocalDnsServer | bool, valore predefinito: FALSE | Statica | Impedisce al runtime di inserire l'indirizzo IP locale come server DNS per i contenitori. |
...
| **Parametro** | **Valori consentiti** | **Criteri di aggiornamento** | **Indicazioni o breve descrizione** |
| --- | --- | --- | --- |
|Counters |string | Dinamica |Elenco delimitato da virgole dei contatori delle prestazioni da raccogliere. |
...
|AADTokenEndpointFormat|stringa, il valore predefinito è ""|Statica|Endpoint del token AAD, Commercial di Azure predefinito, specificato per un ambiente non predefinito, ad esempio Azure Government "https:\//login.microsoftonline.us/{0}" |
...
| EnforceLinuxMinTlsVersion | bool, valore predefinito: FALSE | Dinamica | Se impostato su true; è supportato solo TLS versione 1.2 +.  Se false; supportano le versioni di TLS precedenti. Si applica solo a Linux |
...
|Elimina |stringa, il valore predefinito è "Admin" |Dinamica| Configurazioni di sicurezza per l'operazione di eliminazione del client dell'archivio immagini. |
...
|ContainerNetworkSetup|bool, valore predefinito: FALSE (Linux) e il valore predefinito è TRUE (Windows)| Statica |Se configurare una rete di contenitori.|
...
| **Parametro** | **Valori consentiti** | **Criteri di aggiornamento** | **Indicazioni o breve descrizione** |
| --- | --- | --- | --- |
|Livello |Int, valore predefinito: 4 | Dinamica |Il livello di traccia ETW può accettare valori 1, 2, 3 e 4. Per assicurare il supporto è necessario mantenere il livello di traccia a 4 |
