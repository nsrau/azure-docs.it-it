...
Un cluster SKU Basic significa che la maggior parte delle configurazioni viene fornita dal provider di risorse Service Fabric. I cluster SKU Basic sono destinati a essere usati per gli ambienti di test e pre-produzione. Un cluster SKU standard consente agli utenti di configurare il cluster in modo specifico per soddisfare le proprie esigenze. Per ulteriori informazioni, vedere [Service Fabric SKU del cluster gestito](https://docs.microsoft.com/azure/service-fabric/overview-managed-cluster#service-fabric-managed-cluster-skus) .
