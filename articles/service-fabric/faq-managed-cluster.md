...
Un cluster SKU Basic significa che la maggior parte delle configurazioni viene fornita dal provider di risorse Service Fabric. I cluster SKU Basic sono destinati a essere usati per gli ambienti di test e pre-produzione. Un cluster SKU standard consente agli utenti di configurare il cluster in modo specifico per soddisfare le proprie esigenze. Per ulteriori informazioni, vedere [Service Fabric SKU del cluster gestito](overview-managed-cluster.md#service-fabric-managed-cluster skus.md) .
...
L'esperienza di sviluppo locale rimane invariata rispetto ai cluster di Service Fabric esistenti. Per ulteriori informazioni sull'esperienza di sviluppo locale, vedere [creare un'applicazione .NET](https://docs.microsoft.com/azure/service-fabric/service-fabric-quickstart-dotnet) .
