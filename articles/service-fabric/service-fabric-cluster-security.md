...
Accedere con l'account Azure. Se per qualche motivo questo codice PowerShell genera un errore, verificare di aver installato correttamente Azure PS.
...
Dal momento che si tratta di un certificato autofirmato, è necessario importarlo nell'archivio "Persone attendibili" del computer prima di poterlo usare per la connessione a un cluster sicuro.
...
**Identificazione personale del certificato**: 64881409F4D86498C88EEC3697310C15F8F1540F **Insieme di credenziali di origine**/ID risorsa dell'insieme di credenziali delle chiavi: /subscriptions/35389201-c0b3-405e-8a23-9f1450994307/resourceGroups/chackdankeyvault4doc/providers/Microsoft.KeyVault/vaults/chackdankeyvault4doc **URL del certificato**/URL del percorso del certificato nell'insieme di credenziali delle chiavi: https://chackdankeyvalut4doc.vault.azure.net:443/secrets/chackdantestcertificate3/fvc8df6300834326a95d05d90e0720ea

### Passaggio 3: Configurare un cluster sicuro

Seguire la procedura descritta nell'articolo [Creazione del cluster di Service Fabric](service-fabric-cluster-creation-via-portal.md) fino a visualizzare la sezione Configurazioni di sicurezza. Passare quindi alle istruzioni illustrate qui per impostare le configurazioni di sicurezza:

>[AZURE.NOTE] 
1\. I certificati da usare vengono specificati a livello di tipo di nodo nelle configurazioni di sicurezza. 2. È necessario specificarli per ogni tipo di nodo presente nel cluster. 3. Questo documento illustra come procedere usando il portale, ma è possibile fare lo stesso usando un modello ARM.
