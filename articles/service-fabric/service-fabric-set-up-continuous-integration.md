<properties
   ...
   description="Panoramica di come configurare l'integrazione continua per un'applicazione Service Fabric tramite Visual Studio Team Services."
...
# Configurare l'integrazione continua per un'applicazione Service Fabric di Azure con Visual Studio Team Services (VSTS)

Questo articolo descrive la configurazione dell'integrazione continua (CI) per un'applicazione di Service Fabric tramite Visual Studio Team Services (VSTS) per compilarla, crearne il pacchetto e distribuirla automaticamente. Questo documento riflette l'esperienza corrente ed è soggetto a modifiche man mano che procede l'attività di sviluppo. Inoltre, queste istruzioni ricreano ogni volta il cluster da zero.

## Prerequisiti

Per iniziare, configurare il progetto in Visual Studio Team Services:

1. Se non è già stato fatto, creare un account Team Services usando il proprio [account Microsoft](http://www.microsoft.com/account).
...
Prima di configurare il computer di compilazione, è necessario creare una "entità servizio" di cui l'automazione si serve per l'autenticazione in Azure. È anche necessario creare un certificato e caricarlo in un insieme di credenziali delle chiavi perché l'insieme di credenziali delle chiavi di Azure non supporta l'autenticazione dell'entità servizio. È possibile eseguire questa procedura da qualsiasi computer. Il computer di sviluppo è un'ottima scelta.
...
2.	Installare e aggiornare il modulo AzureRM.
    1.  Se è installata una versione precedente di Azure PowerShell, rimuoverla. Fare clic con il pulsante destro del mouse sul pulsante Start, quindi selezionare **Installazione applicazioni**. Cercare "Azure PowerShell" e disinstallarlo.
    2.  Avviare un prompt dei comandi di PowerShell.
    3.	Installare il modulo AzureRM con il comando `Install-Module AzureRM`.
    4.	Aggiornare il modulo AzureRM con il comando `Update-AzureRM`.
...
4.	Accedere ad Azure PowerShell.
    ...
    2. Nella finestra di dialogo visualizzata immettere le credenziali di Azure.
    3. Eseguire il comando `Get-AzureRmSubscription`.
    4. Trovare la sottoscrizione che si vuole usare e quindi eseguire il comando `Select-AzureRmSubscription -SubscriptionId <id for your subscription>`."
...
2.	In un prompt dei comandi amministratore di PowerShell passare alla directory in cui è stato estratto ServiceFabricCIManualScripts.zip.
...
Installare [Service Fabric SDK](https://azure.microsoft.com/campaigns/service-fabric/).
...
Per installare Azure PowerShell, seguire la procedura nella sezione precedente **Installare Azure PowerShell e accedere**. Ignorare il passaggio 4, **Accedere ad Azure PowerShell**.

### Registrare i moduli di Azure PowerShell con l'account Servizio locale

1. Premere WIN+R, quindi digitare **regedit** e premere INVIO.
...
>[AZURE.NOTE]Eseguire questa operazione *prima* di avviare l'agente di compilazione o la nuova variabile di ambiente non verrà selezionata.
...
1. Impostare il certificato nel computer di compilazione. A tale scopo, seguire questa procedura:
    ...
    2. Aprire un account amministratore di PowerShell ed eseguire i comandi seguenti con la password passata a GenerateCertificate.ps1 in precedenza:
...
2. Eseguire il gestore di certificati.
    ...
    2. Cercare **certificato**.
    3. Scegliere **Strumenti di amministrazione** > **Gestisci i certificati computer**.
...
3. Concedere all'account Servizio locale l'autorizzazione per usare il certificato di Automazione.
    ...
    2.	Trovare il certificato nell'elenco.
    3.	Fare clic con il pulsante destro del mouse sul certificato e quindi scegliere **Tutte le attività** > **Gestisci chiavi private**.
    4.	Scegliere il pulsante **Aggiungi**, quindi immettere **Servizio locale** e scegliere **Controlla nomi**.
    5.	Fare clic su **OK** e chiudere il gestore di certificati.

![Immagine dell'interfaccia utente di gestione certificati di Windows](media/service-fabric-set-up-continuous-integration/windows-certificate-manager.png)
...
2.	A un prompt dei comandi amministratore di PowerShell, eseguire `C:\agent\ConfigureAgent.ps1`. Lo script richiede i parametri seguenti:

    |Parametro|Valore|
    |---|---|
    |Agent Name|Accettare il valore predefinito, `Agent-[machine name]`.
    |TFS Url|Immettere l'URL del progetto team, ad esempio `https://[your-Team-Services-account-name].visualstudio.com`.
...
4. Verificare che l'agente di compilazione sia registrato. A tale scopo, seguire questa procedura:

    1. Tornare al web browser, normalmente nella pagina `https://[your-Team-Services-account-name].visualstudio.com/_admin/_AgentPool`, quindi aggiornare la pagina.
    1. Scegliere il pool di agenti selezionato prima durante l'esecuzione di ConfigureAgent.ps1.
    1. Verificare che l'agente di compilazione sia visualizzato nell'elenco e sia contrassegnato con uno stato verde. Se lo stato è rosso, l'agente di compilazione non riesce a connettersi a Team Services.

![La scheda **Pool di agenti** del Pannello di controllo](media/service-fabric-set-up-continuous-integration/vso-configured-agent.png)
...
3.	Nella scheda **Trigger** selezionare le opzioni **Integrazione continua** e **Modifiche batch**.
4.	Nella scheda **Generale** scegliere la coda in cui sono stati registrati gli agenti di compilazione.
5.	Salvare la definizione di compilazione e assegnare un nome. È possibile modificare questo nome in seguito.
...
### Aggiungere un passaggio "Effettuare il provisioning e distribuire al cluster sicuro"
...
A questo punto il cluster temporaneo è completato e occorre quindi pulirlo. In caso contrario continueranno a essere addebitati i costi corrispondenti. Questa procedura rimuove il gruppo di risorse, che rimuove il cluster e tutte le altre risorse nel gruppo:
...
Le istruzioni precedenti creano un nuovo cluster per ogni compilazione e lo rimuovono al termine dell'operazione. Se invece si preferisce che ogni compilazione esegua l'aggiornamento di un'applicazione in un cluster esistente, seguire questi passaggi:

1.	Creare manualmente un cluster di test tramite il portale di Azure o Azure PowerShell. Come riferimento, è possibile vedere lo script "ProvisionAndDeploy-SecureCluster.ps1".
