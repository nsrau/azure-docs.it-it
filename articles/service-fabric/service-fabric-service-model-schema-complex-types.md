...
Limite della memoria espresso in MB. Deve essere un numero intero positivo. I pacchetti di codice (contenitori o pacchetti) non sono in grado di allocare una quantità di memoria superiore a questo limite. Un'operazione di questo tipo genererebbe un'eccezione di memoria esaurita.
...
Quantità totale di memoria di scambio che è possibile usare, espressa in MB. Deve essere un numero intero positivo.
...
Limite flessibile (in MB) per la governance della memoria che viene applicato solo quando si verificano problemi di contesa della memoria sul nodo. Deve essere un numero intero positivo.
...
Peso relativo della CPU. Deve essere un numero intero positivo.
...
Percentuale utilizzabile di CPU disponibili (solo Windows). Deve essere un numero intero positivo. Se vengono specificati dei limiti di CPU per il pacchetto del servizio, questo parametro viene ignorato.
...
Velocità massima di I/O (lettura e scrittura) in termini di operazioni di I/O al secondo che possono essere usate. Deve essere un numero intero positivo.
...
Valore massimo di I/O (byte al secondo) che può essere usato (lettura e scrittura). Deve essere un numero intero positivo.
...
Quota del disco per i contenitori.  Deve essere un numero intero positivo.
...
Limiti della memoria kernel in byte.  Deve essere un numero intero positivo.  Questo valore è specifico di Linux e se impostato in Docker in Windows causa un errore.
...
Dimensioni di /dev/shm in byte. Se il valore viene omesso, il sistema usa 64 MB.  Deve essere un numero intero positivo.  Questo valore è specifico di Linux, ma se impostato in Docker non causa un errore.  Viene semplicemente ignorato.
...
Limite della CPU espresso in numero di core logici. Deve essere un numero intero positivo.
...
#### <a name="memoryinmb"></a>MemoryInMB
Limite della memoria espresso in MB. Deve essere un numero intero positivo.
