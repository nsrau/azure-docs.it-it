...
A ogni backup, dovrà copiare 16 GB dei checkpoint oltre a 50 MB di log (configurabili usando **CheckpointThresholdInMB**).
...
Per avviare un backup, il servizio deve richiamare la funzione membro ereditata **BackupAsync**.  
...
La richiesta di eseguire un backup incrementale può non riuscire con **FabricMissingFullBackupException**, il quale indica che
...
* che la replica ha oltrepassato il limite di **MaxAccumulatedBackupLogSizeInMB**.
...
**RestoreDescription** passato alla chiamata **RestoreContext.RestoreAsync** contiene un membro denominato **BackupFolderPath**.
...
La chiamata **RestoreAsync** può generare un'eccezione **FabricMissingFullBackupException** se l'oggetto **BackupFolderPath** specificato non contiene un backup completo.
Può anche generare **ArgumentException** se **BackupFolderPath** ha una catena interrotta di backup incrementali,
...
Il metodo **RestoreAsync** in **RestoreContext** può essere chiamato solo all'interno del metodo **OnDataLossAsync**.
...
**OnDataLossAsync** viene quindi richiamato nella nuova replica primaria.
