...
La procedura seguente consente di creare un cluster di Service Fabric di anteprima a nodo singolo (con una sola macchina virtuale) protetto da un certificato autofirmato posizionato in un insieme di credenziali delle chiavi. I cluster a nodo singolo non possono essere ridimensionati per più di una macchina virtuale e i cluster di anteprima non possono essere aggiornati a versioni più recenti. 
...
3. Aprire una console di PowerShell, accedere ad Azure e selezionare la sottoscrizione in cui si vuole distribuire il cluster:
   ...
   ```
4. Creare e crittografare una password per il certificato usato da Service Fabric.

   ```powershell
   $pwd = "<your password>" | ConvertTo-SecureString -AsPlainText -Force
   ```
5. Creare il cluster eseguendo il comando seguente:
...
6. Fare doppio clic sul certificato per aprire l'Importazione guidata certificati.

7. Usare le impostazioni predefinite, ma assicurarsi di spuntare la casella di testo **Mark this key as exportable.** (Contrassegnare questa chiave come esportabile.) nel passaggio **Protezione della chiave privata**. Visual Studio deve esportare il certificato durante la configurazione del Registro contenitori di Azure per l'autenticazione del cluster di Service Fabric in seguito in questa esercitazione.

8. È ora possibile aprire Service Fabric Explorer in un browser. L'URL è **ManagementEndpoint** nell'output di PowerShell CmdLet, ad esempio *https://mycluster.westeurope.cloudapp.azure.com:19080* 
