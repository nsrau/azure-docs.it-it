...
`InstanceCount` per il servizio indica il numero desiderato di istanze che devono essere in esecuzione. In caso di errore di una o tutte le istanze, Service Fabric risponderà creando automaticamente istanze di sostituzione in altri nodi. Service Fabric continuerà a creare sostituzioni fino a quando il servizio non tornerà al numero di istanze desiderato.
Un altro esempio, se il servizio senza stato ha `InstanceCount` di-1, ovvero un'istanza dovrebbe essere in esecuzione in ogni nodo del cluster. Se alcune di queste istanze dovessero avere esito negativo, Service Fabric rileverà che il servizio non si trova nello stato desiderato e tenterà di creare le istanze nei nodi in cui sono mancanti.

#### <a name="stateful-services"></a>Servizi con stato
Esistono due tipi di servizi con stato:
1. Con stato permanente con stato permanente
2. Con stato non permanente (stato archiviato in memoria)

Il ripristino in caso di errore del servizio con stato dipende dal tipo di servizio con stato e dal numero di repliche del servizio e dal numero di repliche non riuscite.

Che cos'è il quorum?
In un servizio con stato, i dati in ingresso vengono replicati tra le repliche (Primary e ActiveSecondary). Se la maggior parte delle repliche riceve i dati, i dati vengono considerati di cui è stato eseguito il commit del quorum (per 5 repliche, 3 sarà il _quorum_). Ciò significa che in qualsiasi momento si garantisce che sia presente almeno il quorum delle repliche con i dati più recenti. Se le repliche hanno esito negativo (ad eccezione di 2, le repliche non sono riuscite), è possibile usare il valore del quorum per calcolare se è possibile eseguire il ripristino (poiché rimanenti 3 di 5 repliche sono ancora in esecuzione, si garantisce che almeno una replica avrà dati completi).

Che cos'è la perdita del quorum?
Quando il quorum delle repliche ha esito negativo, la partizione viene dichiarata come in uno stato di perdita del quorum. Supponiamo che una partizione includa 5 repliche, il che significa che almeno 3 ha a disposizione dati completi. Se il quorum (3 out 5) delle repliche ha esito negativo, Service Fabric non è in grado di determinare se le repliche rimanenti (2 out 5) hanno dati sufficienti per ripristinare la partizione.

La determinazione di una situazione di emergenza per un servizio con stato e la relativa gestione seguono tre fasi:
...
2. Determinazione se la perdita del quorum è permanente o meno
   - Nella maggior parte dei casi gli errori sono temporanei. I processi, i nodi e le VM vengono riavviati e le partizioni di rete vengono corrette. In alcuni casi invece gli errori sono permanenti. 
     - Per i servizi senza stato persistente, un errore di uno o più quorum di repliche determina _immediatamente_ una perdita del quorum permanente. Quando Service Fabric rileva la perdita del quorum in un servizio non persistente con stato, si procederà immediatamente al passaggio 3 dichiarando (potenziale) la perdita di dati. La perdita di dati è sensata perché Service Fabric sa che non ci sono punti in attesa del ritorno delle repliche, perché anche se sono state ripristinate, a causa della natura non permanente del servizio, i dati sono andati persi.
     - Per i servizi permanenti con stato, un errore di un quorum o di più repliche fa in modo che Service Fabric attenda che le repliche tornino e ripristinino il quorum. Si verifica di conseguenza un'interruzione del servizio per tutte le _operazioni di scrittura_ alla partizione interessata (o "set di repliche") del servizio. Le operazioni di lettura restano invece ancora possibili con una minore garanzia di coerenza. La quantità di tempo predefinita in cui Service Fabric attende il ripristino del quorum è infinita, poiché il passaggio successivo è un evento di perdita di dati (potenziale) e comporta altri rischi.
...
Le repliche potrebbero essere inattive a intermittenza a causa di un errore temporaneo. Attendere un po' di tempo mentre Service Fabric tenterà di riportarli. Se le repliche sono inattive per un periodo di tempo superiore a quello previsto, attenersi alla procedura seguente per la risoluzione dei problemi.
- Le repliche potrebbero essere arrestate in modo anomalo. Controllare i report sull'integrità del livello di replica e i registri dell'applicazione. Raccogliere i dump di arresto anomalo del sistema ed eseguire le azioni necessarie per il ripristino.
- Potrebbe non essere possibile rispondere al processo di replica. Esaminare i registri applicazioni per verificarlo. Raccogliere il dump del processo e quindi terminare il processo che non risponde. Service Fabric creerà il processo di sostituzione e tenterà di ripristinare la replica.
...
Potrebbero verificarsi situazioni in cui non è possibile recuperare le repliche, ad esempio, è possibile che le unità non siano riuscite o che i computer non rispondano fisicamente. In questi casi, è necessario che Service Fabric non attendano il ripristino della replica.
Non usare questi metodi se non è accettabile una potenziale perdita di dati per portare il servizio online, nel qual caso è necessario effettuare tutte le operazioni necessarie per il ripristino dei computer fisici.

I passaggi seguenti possono comportare la perdita di dataloss. Assicurarsi prima di seguirli:
   
> [!NOTE]
> Non è _mai_ sicuro usare questi metodi, tranne che in modo mirato, su partizioni specifiche. 
>

- Usare `Repair-ServiceFabricPartition -PartitionId` o `System.Fabric.FabricClient.ClusterManagementClient.RecoverPartitionAsync(Guid partitionId)` API. Questa API consente di specificare l'ID della partizione da ripristinare dal quorum con potenziale perdita di dati.
- Se il cluster riscontra errori frequenti che causano lo stato di perdita del quorum e la potenziale _perdita di dati è accettabile, è_possibile specificare un valore [QuorumLossWaitDuration](https://docs.microsoft.com/powershell/module/servicefabric/update-servicefabricservice?view=azureservicefabricps) appropriato per consentire il ripristino automatico del servizio. Service Fabric attenderà il QuorumLossWaitDuration specificato (il valore predefinito è infinito) prima di eseguire il ripristino. Questo metodo _non è consigliato_ perché può causare perdite di dati impreviste.
...
In Service Fabric è presente il concetto di nodi di inizializzazione. Si tratta di nodi che gestiscono la disponibilità del cluster sottostante. Questi nodi aiutano a garantire che il cluster rimanga attivo stabilendo lease con altri nodi e servendo da tiebreaker durante determinati tipi di errori di rete. Se gli errori casuali rimuovono la maggior parte dei nodi di inizializzazione nel cluster e non vengono restituiti, l'anello della Federazione del cluster viene compresso, perché si è perso il quorum del nodo di inizializzazione e il cluster ha esito negativo. In Azure Service Fabric provider di risorse gestisce Service Fabric configurazioni del cluster e per impostazione predefinita distribuisce i nodi di inizializzazione nei domini di errore e di aggiornamento del tipo di nodo primario. Se il valore NodeType primario è contrassegnato come durabilità Silver o Gold, quando si rimuove un nodo di inizializzazione, tramite la scalabilità nel NodeType primario o la rimozione manuale di un nodo di inizializzazione, il cluster tenterà di innalzare di livello un altro nodo non di inizializzazione dal NodeType primario disponibile la capacità e avrà esito negativo se la capacità disponibile è inferiore a quella richiesta dal livello di affidabilità del cluster per il tipo di nodo primario.
