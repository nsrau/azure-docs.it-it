...
1. **Numero di istanze delle VM:** per eseguire qualsiasi carico di lavoro di produzione in Azure, è necessario specificare un livello di affidabilità Argento o superiore, che corrisponde una dimensione minima del tipo di nodo primario pari a 5.
2. **SKU di VM:** nel tipo di nodo primario vengono eseguiti i servizi di sistema, quindi lo SKU della VM scelto deve tenere in considerazione il carico massimo totale che si prevede di inserire nel cluster. Per comprendere questo concetto, si pensi all'analogia seguente: il tipo di nodo primario è come i polmoni che forniscono ossigeno al cervello e quindi, se il cervello non riceve abbastanza ossigeno, il corpo ne risente. 

Poiché le esigenze in termini di capacità di un cluster dipendono completamente dal carico di lavoro che si prevede di eseguire nel cluster, non è possibile offrire una guida valida per un carico di lavoro specifico, ma ecco alcune indicazioni generali per poter iniziare.
...
1. **Numero di istanze delle VM:** è consigliabile eseguire i carichi di lavoro di produzione con stato con un numero di repliche di destinazione di almeno 5 istanze. Nello stato stabile verrà quindi creata una replica (da un set di repliche) in ogni dominio di errore e in ogni dominio di aggiornamento. L'intero concetto di livello di affidabilità per i servizi di sistema è in realtà solo un modo per specificare questa impostazione per i servizi di sistema.
...
* [Relazione tra i tipi di nodo e i set di scalabilità di macchine virtuali](service-fabric-cluster-nodetypes.md)
