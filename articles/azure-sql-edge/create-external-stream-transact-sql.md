...
Azure SQL Edge attualmente supporta solo gli input e gli output di flusso seguenti.

**Input di flusso**: definisce le connessioni a un'origine dati da cui leggere il flusso di dati
- Hub di Edge
- Kafka (il supporto per gli input Kafka è attualmente disponibile solo nelle versioni Intel/AMD64 di Azure SQL Edge).

**Output flusso**: definisce le connessioni a un'origine dati in cui scrivere il flusso di dati. 
- Hub di Edge
