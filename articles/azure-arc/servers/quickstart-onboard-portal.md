...
![Una connessione server riuscita](./media/quickstart-onboard/arc-for-servers-successful-onboard.png)
...
1. Per disinstallare l'agente di Windows dalla macchina virtuale, eseguire le operazioni seguenti:
    ...
    Se si vuole creare uno script per la disinstallazione, è possibile usare l'esempio seguente, che recupera il codice prodotto e disinstalla l'agente tramite la riga di comando Msiexec.exe - `msiexec /x {Product Code}`. A tale scopo, procedere come indicato di seguito:  
...
1. Per disinstallare l'agente di Linux, eseguire il comando seguente:
