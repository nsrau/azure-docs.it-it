...
* Se l'agente è già stato installato, eseguire il comando `/opt/azcmagent/bin/hybridrp_proxy add https://{proxy-url}:{proxy-port}`, che configura il proxy e riavvia l'agente.
...
1. Selezionare il computer nel [portale](https://aka.ms/hybridmachineportal), fare clic sui puntini di sospensione (`...`) e selezionare **Elimina**.
