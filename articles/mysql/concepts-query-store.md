...
description: Informazioni sulla funzionalità Archivio query nel database di Azure per MySQL per tenere traccia delle prestazioni nel tempo.
author: ajlam
ms.author: andrela
ms.service: mysql
ms.topic: conceptual
...
---
# <a name="monitor-azure-database-for-mysql-performance-with-query-store"></a>Monitorare le prestazioni del database di Azure per MySQL monitor

**Si applica a:** Database di Azure per MySQL 5.7Azure Database for MySQL 5.7

La funzionalità Archivio query nel database di Azure per MySQL consente di tenere traccia delle prestazioni delle query nel tempo. Query Store semplifica la risoluzione dei problemi di prestazioni consentendo di trovare rapidamente le query con il tempo di esecuzione più lungo e il più elevato utilizzo di risorse. Query Store acquisisce automaticamente una cronologia delle query e le statistiche di runtime e le conserva a scopo di verifica. I dati vengono separati per intervalli di tempo per consentire l'individuazione dei modelli di utilizzo dei database. I dati per tutti gli utenti, i database e le query vengono archiviati nel database dello schema mysql nel database di Azure per l'istanza MySQL.Data for all users, databases, and queries is stored in the **mysql** schema database in the Azure Database for MySQL instance.
...
1. Accedere al portale di Azure e selezionare il database di Azure per il server MySQL.Sign in to the Azure portal and select your Azure Database for MySQL server.
...
- Archivio di statistiche di runtime per rendere persistenti le informazioni statistiche di esecuzione delle query.
- Un archivio di statistiche di attesa per rendere persistenti le informazioni sulle statistiche di attesa.
...
Usare il [portale di](howto-server-parameters.md) Azure o [l'interfaccia della riga di comando](howto-configure-server-parameters-using-cli.md) di Azure per ottenere o impostare un valore diverso per un parametro.

## <a name="views-and-functions"></a>Viste e funzioni

Visualizzare e gestire Query Store usando le viste e le funzioni seguenti. Chiunque nel [ruolo pubblico dei privilegi](howto-create-users.md#how-to-create-additional-admin-users-in-azure-database-for-mysql) di selezione può usare queste visualizzazioni per visualizzare i dati in Query Store. Queste viste sono disponibili solo nel database **mysql.**
...
- Se un server MySQL `default_transaction_read_only` ha il parametro, Query Store non è in grado di acquisire dati.
...
- Le statistiche di attesa usano il campione per acquisire una frazione di eventi. La frequenza può essere `query_store_wait_sampling_frequency`modificata utilizzando il parametro .
