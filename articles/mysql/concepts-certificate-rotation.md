...
*   Sono inclusi un archivio certificati CA combinato con certificati **BaltimoreCyberTrustRoot** e **DigiCertGlobalRootG2** .
    ...
    *   Per gli utenti di .NET (MySQL Connector/NET, MySQLConnector), assicurarsi che **BaltimoreCyberTrustRoot** e **DigiCertGlobalRootG2** siano presenti nell'archivio certificati di Windows, autorità di certificazione radice attendibili. Se non esistono certificati, importare il certificato mancante.
        ...
        ![Certificato .NET per database di Azure per MySQL](media/overview/netconnecter-cert.png)
    ...
    *   Per gli altri utenti (client MySQL/MySQL Workbench/C/C++/Go/Python/Ruby/PHP/NodeJS/Perl/Swift), è possibile unire due file di certificato CA come questo formato</b>
...
Se non si aggiorna il certificato radice prima del 30 novembre 2020, le applicazioni che si connettono tramite SSL/TLS e la verifica per il certificato radice non saranno in grado di comunicare con il server di database MySQL e l'applicazione subirà problemi di connettività al server di database MySQL.
...
###  <a name="6-if-i-create-a-new-server-after-nov-30-2020-will-i-be-impacted"></a>6. Se si crea un nuovo server dopo il 30 novembre 2020, I ripercussioni saranno compromessi?
