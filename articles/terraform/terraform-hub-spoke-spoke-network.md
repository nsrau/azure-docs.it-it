---
title: Creare una rete spoke con Terraform in Azure
description: Informazioni su come implementare due reti virtuali spoke connesse a un hub in una topologia hub-spoke
services: terraform
ms.service: azure
keywords: terraform, hub e spoke, reti, reti ibride, devops, macchina virtuale, azure, peering reti virtuali, spoke, hub-spoke
author: VaijanathB
manager: jeconnoc
ms.author: vaangadi
ms.topic: tutorial
...
ms.locfileid: "71173389"
---
# <a name="tutorial-create-a-spoke-virtual-network-with-terraform-in-azure"></a>Esercitazione: Creare una rete virtuale spoke con Terraform in Azure

In questa esercitazione si implementeranno due reti spoke separate per illustrare la separazione dei carichi di lavoro. Le reti condividono risorse comuni usando la rete virtuale hub. Gli spoke possono essere usati per isolare i carichi di lavoro nelle reti virtuali corrispondenti, gestite separatamente rispetto agli altri spoke. Ogni carico di lavoro può includere più livelli, con più subnet connesse tramite i servizi di bilanciamento del carico di Azure.
...
In questa sezione vengono creati due script di spoke. Ogni script definisce una rete virtuale spoke e una macchina virtuale per il carico di lavoro. Viene quindi creata una rete virtuale con peering da hub a spoke.
...
## <a name="declare-the-two-spoke-networks"></a>Dichiarare le due reti spoke

1. In Cloud Shell aprire un nuovo file denominato `spoke1.tf`.
...
1. Creare un file denominato `spoke2.tf`.
...
> [Convalidare una rete hub-spoke con Terraform in Azure](./terraform-hub-spoke-validation.md)