---
title: Accesso a pagina singola con flusso implicitoSingle-page sign-in using implicit flow
titleSuffix: Azure AD B2C
description: Informazioni su come aggiungere l'accesso a pagina singola usando il flusso implicito OAuth 2.0 con Azure Active Directory B2C.
...
Molte applicazioni moderne hanno un front-end di app a pagina singola che viene scritto principalmente in JavaScript. Spesso, l'app viene scritta usando un framework come React, Angular o Vue.js. Le app a pagina singola e le altre app JavaScript che vengono eseguite soprattutto in un browser presentano alcune problematiche aggiuntive per l'autenticazione:

- Le caratteristiche di sicurezza di queste applicazioni sono diverse dalle tradizionali applicazioni Web basate su server.
...
![Diagramma in stile corsia che mostra il flusso implicito OpenID Connect](./media/implicit-flow-single-page-application/convergence_scenarios_implicit.png)
...
In questa richiesta, il client indica le autorizzazioni che deve `scope` acquisire dall'utente nel parametro e il flusso utente da eseguire. Per avere un'idea del funzionamento della richiesta, provare a incollare la richiesta in un browser ed estrarla. Sostituire `{tenant}` con il nome del tenant di Azure AD B2C. Sostituire `90c0fe63-bcf2-44d5-8fb7-b8bbc0b29dc6` con l'ID app dell'applicazione registrata in precedenza nel tenant. Sostituire `{policy}` con il nome di un criterio creato nel `b2c_1_sign_in`tenant, ad esempio .
...
| prompt | No | Tipo di interazione utente obbligatoria. Al momento l'unico valore valido è `login`. Questo parametro impone all'utente di immettere le proprie credenziali nella richiesta. L'accesso Single Sign-On non ha effetto. |

Viene a questo punto richiesto all'utente di completare il flusso di lavoro dei criteri. L'utente potrebbe dover inserire il proprio nome utente e password, accedere con un'identità sociale, iscriversi alla directory o qualsiasi altro numero di passaggi. Le azioni dell'utente dipendono dal modo in cui è definito il flusso utente.
...
Molte altre convalide che è necessario eseguire sono descritte in dettaglio in [OpenID Connect Core Spec](https://openid.net/specs/openid-connect-core-1_0.html). È anche possibile convalidare attestazioni aggiuntive, a seconda dello scenario. Alcune convalide comuni includono:
...
| --- | --- | --- |
|"tenant"| Obbligatoria | Nome del tenant B2C di Azure AD|
"Criteri"| Obbligatoria| Flusso utente da eseguire. Specificare il nome di un flusso utente creato nel tenant B2C di Azure AD. Ad `b2c_1_sign_in`esempio: `b2c_1_sign_up`, `b2c_1_edit_profile`, o . |
...
| response_mode |Consigliato |Specifica il metodo usato per restituire il token risultante all'app. Per il flusso `fragment`implicito, utilizzare . È possibile specificare `query` altre `form_post`due modalità, e , ma non funzionano nel flusso implicito. |
...
| login_hint |Obbligatoria |Per aggiornare e ottenere i token in un iframe nascosto, includere il nome utente dell'utente in questo hint per distinguere tra le eventuali varie sessioni dell'utente in un determinato momento. È possibile estrarre il nome utente da `preferred_username` un accesso `profile` precedente usando l'attestazione (l'ambito è necessario per ricevere l'attestazione). `preferred_username` |
| domain_hint |Obbligatoria |Può essere `consumers` o `organizations`.  Per aggiornare e ottenere i token in `domain_hint` un iframe nascosto, includi il valore nella richiesta.  Estrarre `tid` l'attestazione dal token ID di un accesso precedente `profile` per determinare il valore `tid` da utilizzare (l'ambito è necessario per ricevere l'attestazione). Se il valore dell'attestazione `tid` è `9188040d-6c67-4c5b-b112-36a304b66dad`, usare `domain_hint=consumers`.  In caso contrario, usare `domain_hint=organizations`. |
...
### <a name="successful-response"></a>Risposta con esito positivo
Una risposta corretta `response_mode=fragment` tramite l'utilizzo è simile all'esempio seguente:A successful response by using looks like this example:
...
### <a name="error-response"></a>Risposta di errore
Anche le risposte di errore possono essere inviate all'URI di reindirizzamento in modo che l'app possa gestirle adeguatamente.  Per `prompt=none`, un errore previsto è simile all'esempio seguente:For , an expected error looks like this example:
...
Quando si vuole disconnettere l'utente dall'app, reindirizzare l'utente ad Azure AD per disconnettersi. Se non si reindirizza l'utente, potrebbe essere in grado di eseguire nuovamente l'autenticazione all'app senza immettere nuovamente le credenziali perché hanno una sessione Single Sign-On valida con Azure AD.
...
### <a name="code-sample-azure-ad-b2c-with-microsoft-authentication-library-for-javascript"></a>Esempio di codice: Azure AD B2C con Microsoft Authentication Library per JavaScriptCode sample: Azure AD B2C with Microsoft Authentication Library for JavaScript

Applicazione a [pagina singola creata con msal.js per Azure AD B2C (GitHub)Single-page application built with msal.js for Azure AD B2C][github-msal-js-example] (GitHub)

Questo esempio su GitHub ha lo scopo di iniziare ad usare Azure AD B2C in una semplice applicazione Web creata con [msal.js][github-msal-js] e con l'uso dell'autenticazione in stile popup.
