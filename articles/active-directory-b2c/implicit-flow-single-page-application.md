...
| ambito |Obbligatoria |Elenco di ambiti separato da spazi.  Per ottenere i token, includere tutti gli ambiti necessari per la risorsa di interesse. |
| response_mode |Consigliato |Specifica il metodo usato per restituire il token risultante all'app. Per il flusso implicito `fragment`, usare. È possibile specificare due altre modalità, `query` e `form_post`, ma non funzionano nel flusso implicito. |
...
### <a name="successful-response"></a>Risposta di esito positivo
Una risposta corretta con ha `response_mode=fragment` un aspetto simile all'esempio seguente:
...
Questo esempio su GitHub è concepito per aiutare a iniziare a Azure AD B2C in una semplice applicazione Web compilata con [MSAL. js][github-msal-js] e usando l'autenticazione popup.
