...
> [!NOTE]
> Se gli account di cui si vuole eseguire la migrazione da un archivio utenti esistente hanno un livello di complessità della password inferiore rispetto al livello [di attendibilità delle password complesse applicato da Azure ad B2C](user-flow-password-complexity.md), è possibile disabilitare il requisito per la password complessa usando il valore `DisableStrongPassword` nella proprietà `passwordPolicies`. È ad esempio possibile modificare la richiesta di creazione utente precedente nel modo seguente: `"passwordPolicies": "DisablePasswordExpiration, DisableStrongPassword"`.
...
Per definire un attributo personalizzato nel tenant di B2C, vedere le [informazioni di riferimento sugli attributi personalizzati di B2C](user-flow-custom-attributes.md).
