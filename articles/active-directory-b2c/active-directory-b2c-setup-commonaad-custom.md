...
Questo articolo illustra come abilitare l'accesso per gli utenti usando l'endpoint comune per Azure Active Directory (Azure AD) tramite l'uso di [criteri personalizzati](active-directory-b2c-overview-custom.md).
...
Per abilitare l'accesso per gli utenti che usano l'endpoint di Azure AD multi-tenant, è necessario disporre di un'applicazione multi-tenant registrata in uno dei tenant di Azure AD. In questo articolo verrà illustrato come creare un'applicazione di Azure AD multi-tenant nel tenant di Azure AD B2C. Sarà quindi possibile consentire l'accesso per gli utenti tramite l'uso di tale applicazione di AD Azure multi-tenant.

>[!NOTE]
> Se si vuole che sia gli utenti di Azure AD **che gli utenti con account Microsoft** possano eseguire l'accesso, ignorare questa sezione e registrare invece un'applicazione nel [portale per sviluppatori Microsoft](https://apps.dev.microsoft.com).
...
2. Fare clic su **Altri servizi** nel riquadro a sinistra e cercare "Registrazioni per l'app".
3. Selezionare **Registrazione nuova applicazione**.
4. Immettere un nome per l'applicazione, ad esempio `Azure AD B2C App`.
5. Selezionare **App Web/API** per il tipo di applicazione.
6. Per **URL accesso** immettere l'URL seguente, dove `yourtenant` viene sostituito dal nome del tenant di Azure AD B2C (`fabrikamb2c.onmicrosoft.com`):
...
6. Confermare di avere creato la chiave `B2C_1A_AADAppSecret`.
...
> [!NOTE]
> L'uso di `https://sts.windows.net` come valore di prefisso consentirà tutti gli utenti di Azure AD di accedere alle app.
...
6. Caricare il file `TrustFrameworkExtensions.xml` e il file RP (ad esempio, `SignUpOrSignInWithAAD.xml`) e verificare che superino la convalida.
