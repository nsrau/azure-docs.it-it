...
[OpenID Connect](openid-connect.md) è un protocollo di autenticazione basato su OAuth 2.0 che può essere utilizzato per l'accesso sicuro degli utenti. La maggior parte dei provider di identità che usano questo protocollo sono supportati in Azure AD B2C. In questo articolo viene illustrato come aggiungere provider di identità OpenID Connect personalizzati nei flussi utente.
...
1. Assicurarsi di usare la directory che contiene il tenant B2C di Azure AD facendo clic sul filtro **Directory e sottoscrizione** nel menu in alto e scegliendo la directory che contiene il tenant.
...
> [!NOTE]
> Il segreto client è facoltativo. Tuttavia, è necessario immettere un segreto client se si desidera utilizzare il flusso del codice di [autorizzazione](https://openid.net/specs/openid-connect-core-1_0.html#CodeFlowAuth), che utilizza il segreto per scambiare il codice per il token.
