...
1. Nell'editor HTML incollare il modello HTML seguente o usare il proprio. I `{{otp}}` parametri `{{email}}` e verranno sostituiti in modo dinamico con il valore della password monouso e con l'indirizzo di posta elettronica dell'utente.
...
Nel criterio aggiungere i tipi di attestazione seguenti all' `<ClaimsSchema>` elemento all'interno `<BuildingBlocks>`di.
...
Aggiungere la trasformazione delle attestazioni seguente `<ClaimsTransformations>` all'elemento `<BuildingBlocks>`all'interno di. Apportare gli aggiornamenti seguenti all'XML di trasformazione delle attestazioni:
...
1. `email` Tornare al profilo tecnico autocertificato che contiene un riferimento a questo controllo di visualizzazione.
...
Nel passaggio finale aggiungere un riferimento al DisplayControl creato. Sostituire il profilo `LocalAccountSignUpWithLogonEmail` tecnico autocertificato esistente con il codice seguente se è stata usata una versione precedente di Azure ad B2C criterio. Questo profilo tecnico USA `DisplayClaims` con un riferimento a DisplayControl.
...
1. Modificare la `GenerateSendGridRequestBody` trasformazione delle attestazioni per utilizzare le attestazioni di input con il frammento di codice XML seguente.
