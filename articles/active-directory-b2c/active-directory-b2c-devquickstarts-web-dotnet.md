...
---
# <a name="azure-ad-b2c-build-a-net-web-app"></a>Azure AD B2C: compilare un'app Web .NET
...
In Azure AD B2C ogni esperienza utente è definita da [criteri](active-directory-b2c-reference-policies.md)specifici. Questo esempio di codice contiene tre esperienze di identità: iscrizione, accesso e modifica del profilo. È necessario creare un criterio per ogni tipo, come descritto nell' [articolo di riferimento sui criteri](active-directory-b2c-reference-policies.md#create-a-sign-up-policy).

> [!NOTE]
> Azure AD B2C supporta anche criteri di registrazione o accesso combinati non illustrati in questa esercitazione.  I criteri di iscrizione o accesso sono illustrati in [questa esercitazione equivalente](active-directory-b2c-devquickstarts-web-dotnet-susi.md).
>
>

Durante la creazione dei tre criteri, assicurarsi di:

* Scegliere **User ID sign-up** (Iscrizione ID utente) o **Email sign-up** (Iscrizione posta elettronica) nel pannello dei provider di identità.
...
Aggiungere quindi al progetto una classe di avvio OWIN denominata `Startup.cs`. Fare clic con il pulsante destro del mouse sul progetto, scegliere **Aggiungi** e quindi **Nuovo elemento** e cercare "OWIN". **Assicurarsi di modificare la dichiarazione di classe in `public partial class Startup`**. Parte di questa classe è stata implementata in un altro file. Il middleware OWIN richiamerà il metodo `Configuration(...)` all'avvio dell'app. In questo metodo effettuare una chiamata a `ConfigureAuth(...)`, in cui viene configurata l'autenticazione per l'app.
