...
La directory B2C Azure Active Directory (AD) viene fornita con un set predefinito di attributi. ad esempio nome, cognome, città, codice postale, e così via. Tuttavia, ogni applicazione per consumatori ha dei requisiti specifici su quali informazioni (attributi) desiderano raccogliere dagli utenti. Azure AD B2C consente di estendere la directory (in particolare, di estendere il set di attributi archiviati in ogni account utente). È possibile creare attributi personalizzati nel [portale di Azure](https://portal.azure.com/) e usarli nei criteri di iscrizione, come illustrato di seguito. È inoltre possibile la lettura e la scrittura di questi attributi utilizzando l'API Azure AD Graph come mostrato [qui](active-directory-b2c-devquickstarts-graph-dotnet.md).
