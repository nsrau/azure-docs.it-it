...
I token di sicurezza ricevuti dall'applicazione da Azure AD B2C possono provenire dagli endpoint `/authorize` o `/token`. Quando i token ID vengono acquisiti dall'endpoint `/authorize`, viene eseguita usando il [flusso implicito](implicit-flow-single-page-application.md), che viene spesso usato per gli utenti che accedono alle applicazioni Web basate su JavaScript. Quando i token ID vengono acquisiti dall'endpoint `/token`, viene eseguita utilizzando il flusso del [codice di autorizzazione](openid-connect.md#get-a-token), che mantiene il token nascosto dal browser.
