---
title: Aggiungere un'applicazione API Web - Azure Active Directory B2C Documenti Microsoft
description: Informazioni su come aggiungere un'applicazione API Web al tenant B2C di Active Directory.
...
# <a name="add-a-web-api-application-to-your-azure-active-directory-b2c-tenant"></a>Aggiungere un'applicazione API Web al tenant di Azure Active Directory B2C
 ...
 Registrare le risorse API Web nel tenant in modo che possano accettare e rispondere alle richieste da parte delle applicazioni client che presentano un token di accesso. Questo articolo illustra come registrare un'API Web in Azure Active Directory B2C (Azure AD B2C).
...
7. Per **URL di risposta**, immettere un endpoint a cui Azure AD B2C deve restituire eventuali token richiesti dall'applicazione. Nell'applicazione di produzione è possibile impostare l'URL di risposta su un valore, ad `https://localhost:44332`esempio . A scopo di test, impostare l'URL della risposta su `https://jwt.ms`.
...
1. In **URI di reindirizzamento ** selezionare **Web** e quindi immettere un endpoint a cui Azure AD B2C deve restituire eventuali token richiesti dall'applicazione. In un'applicazione di produzione è possibile `https://localhost:5000`impostare l'URI di reindirizzamento come un endpoint come . Durante lo sviluppo o il `https://jwt.ms`test, è possibile impostarlo su , un'applicazione Web di proprietà microsoft che visualizza il contenuto decodificato di un token (il contenuto del token non lascia mai il browser). È possibile aggiungere e modificare gli URI di reindirizzamento nelle applicazioni registrate in qualsiasi momento.
1. Selezionare **Registra**.
1. Registrare **l'ID dell'applicazione (client)** da utilizzare nel codice dell'API Web.

Se si dispone di un'applicazione che implementa il flusso di concessione implicita, ad esempio un'applicazione a pagina singola basata su JavaScript,è possibile abilitare il flusso attenendosi alla seguente procedura:
...
Per chiamare un'API Web protetta da un'applicazione, è necessario concedere all'applicazione le autorizzazioni per l'API. In Example, in [Tutorial: Register an application in Azure Active Directory B2C](tutorial-register-applications.md), un'applicazione Web denominata *webapp1* viene registrata in Azure AD B2C. Questa applicazione può essere usata per chiamare l'API Web.
