...
![Parte del file UsersData. JSON che mostra i blocchi JSON di due utenti](./media/user-migration/pre-migration-data-file.png)
...
![Finestra del prompt dei comandi che mostra l'output del comando UserMigration. exe](./media/user-migration/pre-migration-demo.png)
...
- Per recuperare un utente tramite l'indirizzo di posta elettronica di accesso, usare l'applicazione di esempio:
   ...
   1. Aprire il file UserProfile.json in un editor JSON per visualizzare le informazioni dell'utente.
      ...
      ![File UserProfile. JSON aperto nell'editor di Visual Studio Code](./media/user-migration/pre-migration-get-by-email2.png)
...
Per ottenere il collegamento ai criteri di reimpostazione della password, seguire questa procedura. Questa procedura presuppone che sia stato creato in precedenza un [criterio personalizzato](custom-policy-get-started.md)di reimpostazione della password.
...
1. Copiare l'URL visualizzato nella casella di testo **Run Now endpoint** e quindi inviarlo agli utenti.
    ...
    ![Pagina Criteri di reimpostazione password con endpoint Run Now evidenziato](./media/user-migration/pre-migration-policy-uri.png)
...
1. Immettere le credenziali di uno degli utenti migrati e quindi selezionare **Accedi**. L'API REST genera il messaggio di errore seguente:
    ...
    ![Pagina di iscrizione dell'accesso che mostra il messaggio di errore di modifica della password](./media/user-migration/pre-migration-error-message.png)
...
1. Selezionare **Salva**
    ...
    ![Pagina di configurazione dei log di diagnostica in portale di Azure](./media/user-migration/pre-migration-diagnostic-logs.png)
