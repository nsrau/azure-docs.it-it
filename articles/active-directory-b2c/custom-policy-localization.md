...
Il [LocalizedResources](localization.md#localizedresources) dell'elemento `Localization` contiene l'elenco delle stringhe localizzate. L'elemento resources localizzato ha un identificatore usato per identificare in modo univoco le risorse localizzate. Questo identificatore viene usato in un secondo momento nell'elemento di [definizione del contenuto](contentdefinitions.md) .
...
> [!NOTE]
> Nell'esempio seguente è stato aggiunto il simbolo di cancelletto `#` al supplica di ogni riga, quindi è possibile facilmente trovare le etichette localizzate sullo schermo.
...
1. Impostare la lingua predefinita del browser su spagnolo. In alternativa, è possibile aggiungere il parametro della stringa di query `ui_locales` alla richiesta di autorizzazione. Ad esempio: 
