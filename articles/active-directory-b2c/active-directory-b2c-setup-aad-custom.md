...
1. Immettere un **nome** per la chiave dei criteri. Ad esempio `ContosoAppSecret`.  Il prefisso `B2C_1A_` viene aggiunto automaticamente al nome della chiave al momento della creazione, quindi il riferimento nel codice XML nella sezione seguente viene *B2C_1A_ContosoAppSecret*.
