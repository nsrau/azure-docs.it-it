---
title: Definire un profilo tecnico per un'autorità emittente JWT in un criterio personalizzatoDefine a technical profile for a JWT issuer in a custom policy
titleSuffix: Azure AD B2C
description: Definire un profilo tecnico per un'autorità emittente di token Web JSON (JWT) in un criterio personalizzato in Azure Active Directory B2C.Define a technical profile for a JSON web token (JWT) issuer in a custom policy in Azure Active Directory B2C.
...
|RefreshTokenUserJourneyId| No | Identificatore di un percorso utente che deve essere eseguito durante `/token` l'aggiornamento di una richiesta POST del token di [accesso](authorization-code-flow.md#4-refresh-the-token) all'endpoint. |
...
| Attributo | Obbligatoria | Descrizione |
| --------- | -------- | ----------- |
| issuer_secret | Sì | Il certificato X509 (set di chiavi RSA) da usare per firmare il token JWT. Questa è la chiave `B2C_1A_TokenSigningKeyContainer` configurata in [Introduzione ai criteri personalizzati](custom-policy-get-started.md). |
