...
description: Note per gli sviluppatori che configurano e gestiscono B2C con criteri personalizzati
...
2. Integrare servizi abilitati per API REST nei percorsi utente di autenticazione personalizzati
3. Aggiungere la federazione con provider di identità conformi allo standard OpenID Connect
4. Aggiungere la federazione con provider di identità che rispettano il protocollo SAML 2.0



## <a name="terms-of-the-public-preview"></a>Condizioni per l'anteprima pubblica
1. È consigliabile usare le nuove funzionalità solo a scopo di valutazione.
2. Le nuove funzionalità non sono destinate all'uso in un ambiente di produzione.
3. I contratti di servizio non si applicano alle nuove funzionalità. È possibile inviare richieste di supporto tramite i normali canali del supporto.
5. Non è stata fissata una data per la disponibilità generale.
6. A propria discrezione e per qualsiasi motivo, Microsoft potrebbe contrassegnare e rifiutare o limitare gli scenari e i percorsi utente che esulano dagli obiettivi del prodotto Azure AD B2C di offrire una piattaforma di gestione delle identità e degli accessi per i clienti.

## <a name="responsibilities-for-custom-policy-feature-set-developers"></a>Responsabilità degli sviluppatori che usano il set di funzionalità dei criteri personalizzati
La configurazione manuale dei criteri garantisce un accesso di livello inferiore alla piattaforma sottostante di Azure AD B2C e determina la creazione di un framework attendibilità interamente personalizzabile e univoco.  Le permutazioni quasi infinite di provider di identità personalizzati, relazioni di trust, integrazioni con servizi esterni e flussi di lavoro dettagliati risultano particolarmente impegnative per gli sviluppatori esperti da cui vengono utilizzate.
Per sfruttare appieno l'anteprima pubblica, è consigliabile che gli sviluppatori che utilizzano il set di funzionalità dei criteri personalizzati si attengano alle indicazioni seguenti:
...
* Seguire le procedure consigliate di staging e sviluppo software con almeno un ambiente di sviluppo/testing e un ambiente di produzione
* Mantenersi aggiornati sui nuovi sviluppi relativi ai servizi e ai provider di identità con cui viene eseguita l'integrazione, ad esempio tenendo traccia delle modifiche dei segreti, delle modifiche pianificate e non pianificate del servizio e così via, e configurare il monitoraggio attivo e monitorare il tempo di risposta negli ambienti di produzione
