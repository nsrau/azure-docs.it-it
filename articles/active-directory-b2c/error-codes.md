...
| `AADB2C90016` | Il tipo di asserzione client{0}richiesto '' non corrisponde al tipo previsto{1}''. |
...
| `AADB2C90019` | La chiave del contenitore di chiavi{0}con ID ''{1}nel tenant '' non è valida. Motivo: {2}. |
| `AADB2C90021` | Il profilo tecnico '{0}' non esiste nel criterio '{1}' del tenant '{2}'. |
| `AADB2C90022` | Non è possibile restituire i metadati per il{0}criterio '' nel{1}tenant ''. |
...
| `AADB2C90027` | Le credenziali di base specificate{0}per '' non sono valide. Verificare che le credenziali siano corrette e che l'accesso sia stato concesso dalla risorsa. |
...
| `AADB2C90067` | Il formato dell'URI di Reindirizzamento{0}dopo la disconnessione '' non è valido. Specificare un URL basato su HTTPS, adhttps://example.com/returnesempio '', oppure per i client nativi usare l'URI di IETF Native Client ' urn: IETF: WG: OAuth: 2.0: OOB '. |
...
| `AADB2C90075` | Lo scambio{0}di attestazioni '' specificato nel{1}passaggio '' ha restituito la risposta di{2}errore HTTP con codice{3}'' e motivo ''. |
| `AADB2C90077` | L'utente non dispone di una sessione esistente e il parametro del prompt della richiesta ha{0}un valore ''. |
| `AADB2C90079` | Quando si riscatta una concessione riservata, i client devono inviare un client_secret. |
| `AADB2C90080` | La concessione specificata è scaduta. Ripetere l'autenticazione e riprovare. Ora corrente: {0}, concedi ora rilasciata: {1}, concedi la scadenza della {2}finestra temporale scorrevole:. |
...
| `AADB2C90120` | Il parametro di validità massima{0}'' specificato nella richiesta non è valido. La validità massima deve essere un numero intero{1}compreso tra'{2}' è '. |
...
| `AADB2C90149` | Non è{0}stato possibile caricare lo script ''. |
...
| `AADB2C90155` | Una richiesta di verifica a più fattori non è riuscita con{0}motivo ''. |
| `AADB2C90156` | Una richiesta di convalida a più fattori non è riuscita con{0}motivo ''. |
...
| `AADB2C90165` | Impossibile trovare il messaggio di inizializzazione SAML{0}con ID '' nello stato. |
...
| `AADB2C90188` | Il profilo tecnico SAML '{0}' specifica un URL PartnerEntity di '{1}', ma il recupero dei metadati ha esito negativo{2}con motivo ''. |
...
| `AADB2C90220` | Il contenitore di chiavi nel tenant{0}'' con identificatore di{1}archiviazione '' esiste ma non contiene un certificato valido. Il certificato potrebbe essere scaduto o il certificato potrebbe diventare attivo in futuro (NBF). |
...
| `AADB2C90242` | Il profilo tecnico SAML '{0}' specifica il CDATA PartnerEntity che non può essere caricato per{1}il motivo ''. |
...
| `AADB2C90255` | Lo scambio di attestazioni specificato nel profilo{0}tecnico '' non è stato completato come previsto. Potrebbe essere necessario provare a avviare la sessione dall'inizio. |
| `AADB2C90261` | Lo scambio di attestazioni '{0}' specificato nel{1}passaggio '' ha restituito una risposta di errore HTTP che non è stato possibile analizzare. |
...
| `AADB2C90276` | La richiesta non è coerente con l'impostazione di controllo{0}'':{1}'' in technicalProfile{2}'' per il{3}tenant{4}'' del criterio ''. |
| `AADB2C90277` | Il passaggio di orchestrazione '{0}' del percorso{1}utente '' del{2}criterio '' non contiene un riferimento alla definizione del contenuto. |
...
| `AADB2C90288` | UserJourney con ID '{0}' a cui si fa riferimento{1}nel TechnicalProfile '' per il riscatto{2}del token di aggiornamento per il tenant{3}'' non esiste nel criterio '' o in uno dei criteri di base. |
...
| `AADB2C99005` | La richiesta contiene un parametro di ambito non valido che include un carattere{0}non valido ''. |
| `AADB2C99006` | Azure AD B2C non è possibile trovare l'app estensioni con ID{0}app ''. Per ulteriori https://go.microsoft.com/fwlink/?linkid=851224 informazioni, visitare. |
...
| `AADB2C99015` | Nel tenant '' mancano{1}tutti i InputClaims necessari{2}per il flusso di credenziali della password del proprietario della risorsa nel criterio '' nel tenant ''.{0} |
