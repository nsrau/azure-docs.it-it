---
title: Autenticazione, iscrizione e modifica del profilo in Azure Active Directory B2C | Microsoft Docs
...
È possibile usare MSAL anche per tenere traccia dello stato di accesso dell'utente. In questa app si vuole che l'utente rimanga connesso anche dopo aver chiuso e riaperto l'app. Nuovamente all'interno dell'override `OnInitialized`, usare il metodo `AcquireTokenSilent` di MSAL per verificare la disponibilità di token memorizzati nella cache:
