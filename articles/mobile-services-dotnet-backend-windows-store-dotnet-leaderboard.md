...
Questa esercitazione illustra come compilare un'app di Windows Store mediante Servizi mobili di Azure con un back-end .NET. Servizi mobili di Azure fornisce un back-end scalabile e sicuro con autenticazione, monitoraggio, notifiche push e altre funzionalità predefinite, oltre a una libreria client multipiattaforma per la compilazione di app per dispositivi mobili. Il back-end .NET per Servizi mobili è basato su [API Web ASP.NET][API Web ASP.NET] e offre agli sviluppatori .NET un modo straordinario per creare API REST.

-   [Panoramica][Panoramica]
-   [Informazioni sull'app di esempio][Informazioni sull'app di esempio]
-   [Aggiungere modelli di dati][Aggiungere modelli di dati]
-   [Aggiungere controller API Web][Aggiungere controller API Web]
-   [Usare un oggetto DTO per restituire le entità correlate][Usare un oggetto DTO per restituire le entità correlate]
-   [Definire un'API personalizzata per l'invio dei punteggi][Definire un'API personalizzata per l'invio dei punteggi]
-   [Creare l'app di Windows Store][Creare l'app di Windows Store]
-   [Aggiungere classi di modelli][Aggiungere classi di modelli]
-   [Creare un modello di visualizzazione][Creare un modello di visualizzazione]
-   [Aggiungere un'istanza di MobileServiceClient][Aggiungere un'istanza di MobileServiceClient]
-   [Creare la pagina principale][Creare la pagina principale]
-   [Pubblicare il servizio mobile][Pubblicare il servizio mobile]
...
In questa esercitazione viene usato [Visual Studio 2013 Update 2][Visual Studio 2013 Update 2].
...
Scaricare il progetto completo [qui][qui].
...
In Visual Studio 2013 Update 2 il progetto di applicazione Web ASP.NET ora include un modello per il servizio mobile di Microsoft Azure. Selezionare questo modello e fare clic su **OK**.
...
Non sono necessari per questa esercitazioni, quindi possono essere eliminati dal progetto. Rimuovere anche i riferimenti a TodoItem in WebApiConfig.cs e LeaderboardContext.cs.

## Aggiungere modelli di dati

Si userà [Code First di Entity Framework][Code First di Entity Framework] per definire le tabelle di dati. Nella cartella DataObjects aggiungere una classe denominata `Player`.
...
Si noti che entrambe le classi ereditano dalla classe **EntityData**. L'ereditarietà da **EntityData** semplifica l'uso dei dati da parte dell'app tramite la libreria client multipiattaforma per Servizi mobili di Azure. **EntityData** consente anche a un'app di [gestire i conflitti di scrittura nel database][gestire i conflitti di scrittura nel database].

La classe `PlayerRank` include una [proprietà di navigazione][proprietà di navigazione] che punta all'entità `Player` correlata. L'attributo **[ForeignKey]** indica a EF che la proprietà `Player` rappresenta una chiave esterna.

# Aggiungere controller API Web

In seguito si aggiungeranno controller API Web per `Player` e `PlayerRank`. Anziché controller API Web semplici, si aggiungerà un tipo particolare di controller denominato *controller tabella*, progettato espressamente per Servizi mobili di Azure.

Fare clic con il pulsante destro del mouse sulla cartella Controllers, scegliere Add, quindi selezionare New Scaffolded Item.

![][5]

Nella finestra di dialogo **Add Scaffold** espandere **Common** a sinistra e selezionare **Windows Azure Mobile Services**. Selezionare quindi **Windows Azure Mobile Services Table Controller**. Fare clic su **Aggiungi**.

Nella finestra di dialogo **Aggiungi controller**:

1.  In **Classe modello** selezionare Player.
2.  In **Classe contesto di dati** selezionare LeaderboardContext.
3.  Assegnare al controller il nome "PlayerContoller".
...
-   Routing: la route predefinita per un **TableController** è `/tables/{table_name}/{id}`, dove *table\_name* corrisponde al nome dell'entità. Quindi, la route per il controller Player è */tables/player/{id}*. Questa convenzione di routing assicura la coerenza di **TableController** con l'[API REST][API REST] di Servizi mobili.
...
La libreria client di Servizi mobili non supporta le proprietà di navigazione, che non verranno serializzate. Ecco ad esempio la risposta HTTP non elaborata per GET `/tables/PlayerRank`:
...
Un DTO è un oggetto che definisce la modalità di invio dei dati sulla rete. Gli oggetti DTO sono utili quando è preferibile che il formato di trasmissione sia diverso rispetto al modello di database. Per creare un DTO per `PlayerRank`, aggiungere una nuova classe denominata `PlayerRankDto` nella cartella DataObjects.
...
Prima di tutto, aggiungere una classe denominata `PlayerScore` nella cartella DataObjects.
...
Nella classe `PlayerRankController` spostare la variabile `LeaderboardContext` dal costruttore a una variabile di classe:
...
L'attributo **[Route]** per definire una route personalizzata per questo metodo:

    [Route("api/score")]

È anche possibile inserire il metodo in un controller separato. Nessuna delle due opzioni presenta vantaggi particolari; la scelta dipende solo dal modo in cui si vuole organizzare il codice.
Per altre informazioni sull'attributo **[Route]**, vedere l'articolo relativo al [routing mediante attributi in API Web][routing mediante attributi in API Web].

## Creare l'app di Windows Store

In questa sezione verrà descritta l'app di Windows Store che usa il servizio mobile. L'attenzione non sarà focalizzata in particolare sul codice XAML o sull'interfaccia utente, quanto piuttosto sulla logica dell'applicazione. Il progetto completo è disponibile per il download [qui][qui].
...
Nella finestra di dialogo **Pubblica** fare clic su **Servizi mobili di Microsoft Azure**.
...
-   [Altre informazioni su Servizi mobili di Azure][Altre informazioni su Servizi mobili di Azure]
-   [Altre informazioni su API Web][API Web ASP.NET]
-   [Gestire i conflitti di scrittura nel database][Gestire i conflitti di scrittura nel database]
...
<!-- URLs. -->
  ...
  [Panoramica]: #overview
  [Informazioni sull'app di esempio]: #about-the-sample-app
  [Aggiungere modelli di dati]: #add-data-models
  [Aggiungere controller API Web]: #add-web-api-controllers
  [Usare un oggetto DTO per restituire le entità correlate]: #use-a-dto-to-return-related-entities
  [Definire un'API personalizzata per l'invio dei punteggi]: #define-a-custom-api-to-submit-scores
  [Creare l'app di Windows Store]: #create-the-windows-store-app
  [Aggiungere classi di modelli]: #add-model-classes
  [Creare un modello di visualizzazione]: #create-a-view-model
  [Aggiungere un'istanza di MobileServiceClient]: #add-a-mobileserviceclient-instance
  [Creare la pagina principale]: #create-the-main-page
  [Pubblicare il servizio mobile]: #publish-your-mobile-service
  ...
  [qui]: http://code.msdn.microsoft.com/Leaderboard-App-with-Azure-9acf63af
  ...
  [Code First di Entity Framework]: http://msdn.microsoft.com/en-US/data/ee712907#codefirst
  [gestire i conflitti di scrittura nel database]: http://azure.microsoft.com/it-it/documentation/articles/mobile-services-windows-store-dotnet-handle-database-conflicts/
  [proprietà di navigazione]: http://msdn.microsoft.com/en-us/data/jj713564.aspx
  ...
  [mapping tra tipi di database e tipi client nel back-end .NET mediante AutoMapper]: http://blogs.msdn.com/b/azuremobile/archive/2014/05/19/mapping-between-database-types-and-client-type-in-the-net-backend-using-automapper.aspx
  [routing mediante attributi in API Web]: http://www.asp.net/web-api/overview/web-api-routing-and-actions/attribute-routing-in-web-api-2
  ...
  [Altre informazioni su Servizi mobili di Azure]: /en-us/develop/mobile/resources/
