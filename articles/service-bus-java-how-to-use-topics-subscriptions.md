...
Alcuni metodi di **TopicInfo** consentono di ottimizzare le proprietà dell'argomento, ad esempio impostando il valore di durata "TTL" da applicare i messaggi inviati all'argomento. Nell'esempio seguente viene illustrato come creare un argomento denominato "TestTopic" con una dimensione massima pari a 5 GB:
...
Il modo principale per ricevere i messaggi da una sottoscrizione consiste nell'utilizzare un oggetto **ServiceBusContract**. È possibile utilizzare i messaggi ricevuti in due diverse modalità: **ReceiveAndDelete** e **PeekLock**.
...
A questo punto, dopo aver appreso le nozioni di base sulle code del bus di servizio, vedere l'argomento [Code, argomenti e sottoscrizioni del bus di servizio](http://msdn.microsoft.com/library/windowsazure/hh367516.aspx) su MSDN per ulteriori informazioni.
