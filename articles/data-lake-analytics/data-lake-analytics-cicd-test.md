...
Azure Data Lake fornisce il linguaggio [U-SQL](data-lake-analytics-u-sql-get-started.md) . U-SQL combina SQL dichiarativo con C# imperativo per elaborare i dati su qualsiasi scala. In questo documento si apprenderà come creare i test case per U-SQL e il codice dell'operatore definito dall'utente (UDO) C# esteso.
...
È possibile usare un Framework di unit test C# per testare gli operatori C# definiti dall'utente (Udo). Quando si testano gli UDO, è necessario preparare gli oggetti **IRowset** corrispondenti come input.
...
Dopo aver chiamato le funzioni UDO, è possibile verificare i risultati attraverso la verifica dello schema e del valore Rowset usando le funzioni Assert C#. È possibile aggiungere un **progetto di unit test Udo di U-SQL C#** alla soluzione. A tale scopo, selezionare **File > nuovo > progetto** in Visual Studio.
...
Entrambi i **progetti di test di script U-SQL** e i **progetti di test UDO C#** ereditano i progetti di unit test C#. L' [attività di test di Visual Studio](/azure/devops/pipelines/test/getting-started-with-continuous-testing) in Azure Pipelines può eseguire questi test case.
...
### <a name="run-c-udo-test-cases-in-azure-pipelines"></a>Eseguire test case di UDO C# in Azure Pipelines

Per un test UDO in C#, assicurarsi di fare riferimento agli assembly seguenti, necessari per Udo.
...
- [Usare il progetto di database U-SQL per sviluppare un database U-SQL](data-lake-analytics-data-lake-tools-develop-usql-database.md)