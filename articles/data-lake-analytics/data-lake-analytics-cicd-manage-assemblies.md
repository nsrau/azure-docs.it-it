...
1. Aggiungere un riferimento al progetto libreria di classi C# per il progetto di database U-SQL.
   ...
   ![Strumenti di Data Lake per Visual Studio-Aggiungi riferimento](./media/data-lake-analytics-cicd-manage-assemblies/data-lake-tools-add-project-reference.png)

   ![Strumenti di Data Lake per Visual Studio-aggiungere un riferimento al progetto di database U-SQL](./media/data-lake-analytics-cicd-manage-assemblies/data-lake-tools-add-project-reference-wizard.png)

1. Creare uno script di assembly nel progetto di database U-SQL facendo clic con il pulsante destro del mouse sul progetto e selezionando **Aggiungi nuovo elemento**.
...
1. Nella **Distribuzione guidata database U-SQL** selezionare l'**account ADLA** in cui distribuire il database. Gli account locali e gli account ADLA sono entrambi supportati.

1. **Origine database** viene compilato automaticamente. Punta al pacchetto .usqldbpack nella cartella di output della compilazione del progetto.

1. Immettere un nome in **Nome database** per creare un database. Se esiste già un database con lo stesso nome nell'account Azure Data Lake Analytics, tutti gli oggetti definiti nel progetto di database vengono creati senza ricreare il database.

1. Per distribuire il database U-SQL selezionare **Invia**. Tutte le risorse, come ad esempio assembly e file aggiuntivi, vengono caricate. Viene inviato un processo U-SQL che include tutte le istruzioni DDL.
