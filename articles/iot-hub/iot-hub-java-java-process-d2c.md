---
title: Elaborare messaggi da dispositivo a cloud dell'hub IoT (Java) | Microsoft Docs
...
---
# <a name="tutorial:-how-to-process-iot-hub-device-to-cloud-messages-using-java"></a>Esercitazione: elaborare messaggi da dispositivo a cloud dell'hub IoT usando Java
...
> [!NOTE]
> Un'istanza di **EventProcessorHost** è solo uno dei modi possibili per elaborare i messaggi interattivi. Altre opzioni includono [Azure Service Fabric][lnk-service-fabric] e [Analisi di flusso di Azure][lnk-stream-analytics].
> 
> 
...
2. Aggiungere la classe annidata seguente alla classe **App** :
    ...
    Questa classe è simile alla classe **MessageSender** del progetto **simulated-device**. Le uniche differenze sono l'impostazione della proprietà di sistema **MessageId** e una proprietà personalizzata denominata **messageType**.
...
4. Salvare e chiudere il file simulated-device\src\main\java\com\mycompany\app\App.java.
   ...
   > [!NOTE]
   > Per semplicità, questa esercitazione non implementa alcun criterio di ripetizione. Nel codice di produzione è consigliabile implementare criteri di ripetizione dei tentativi, ad esempio un backoff esponenziale, come indicato nell'articolo di MSDN relativo alla [Transient Fault Handling](Gestione degli errori temporanei.md).
   > 
   > 
...
## <a name="process-device-to-cloud-messages"></a>Elaborare i messaggi da dispositivo a cloud
...
### <a name="provision-an-azure-storage-account-and-a-service-bus-queue"></a>Effettuare il provisioning di un account di archiviazione di Azure e di una coda del bus di servizio
...
Per abilitare l'elaborazione affidabile dei messaggi interattivi, è necessaria anche una coda del bus di servizio. È possibile creare una coda a livello di codice con una finestra di deduplicazione di un'ora, come illustrato in [Introduzione alle code del bus di servizio][coda del bus di servizio]. In alternativa, è possibile usare il [portale di Azure classico][lnk-classic-portal], seguendo questa procedura:
...
### <a name="create-the-event-processor"></a>Creare il processore di eventi
...
> [!NOTE]
> Per semplificare il codice, questa esercitazione usa un solo file BLOB per partizione per archiviare i messaggi. Una soluzione vera implementerebbe il rollover dei file, creando file aggiuntivi dopo un determinato intervallo di tempo o al raggiungimento di una certa dimensione. Tenere presente che il BLOB in blocchi di Azure non può superare i 195 GB.
> 
> 
...
6. Aggiungere il codice seguente al metodo **AppendAndCheckPoint** per recuperare il numero attuale di messaggi sequenza e offset nella partizione:
    ...
    ```
7. Nel metodo **AppendAndCheckPoint** usare il valore di offset corrente per creare un'istanza di **BlockEntry** per il blocco successivo da salvare nel BLOB:
...
3. Aggiungere la variabili a livello di classe seguente alla classe **App** . Sostituire **{stringadiconnessioneaccountdiarchiviazione}** con la stringa di connessione all'account di archiviazione di Azure precedentemente annotata nella sezione [Effettuare il provisioning di un account di archiviazione di Azure e di una coda del bus di servizio](#provision-an-azure-storage-account-and-a-service-bus-queue) :
    ...
    ```
4. Aggiungere alla classe **App** le variabili a livello di classe seguenti e sostituire **{yourservicebusnamespace}** con lo spazio dei nomi del bus di servizio e **{yourservicebussendkey}** con la chiave **send** della coda. Con i valori della chiave **listen** e lo spazio dei nomi annotati nella precedente sezione [Effettuare il provisioning di un account di archiviazione di Azure e di una coda del bus di servizio](#provision-an-azure-storage-account-and-a-service-bus-queue) :
    ...
    ```
5. Aggiungere le variabili a livello di classe seguenti alla classe **App** . Sostituire **{youreventhubcompatibleendpoint}** con il nome dell'endpoint compatibile con l'hub eventi. Il nome dell'endpoint è simile a **ihs... namespace**, quindi è necessario rimuovere il prefisso **sb://** e il suffisso **.servicebus.windows.net/**. Sostituire **{nomecompatibileconhubeventi}** con il nome compatibile con l'hub eventi. Sostituire **{youriothubkey}** con la chiave **iothubowner**. Prendere nota di questi valori nella sezione [Creare un hub IoT][lnk-create-an-iot-hub] dell'esercitazione *Introduzione all'hub IoT di Azure per Java*:
...
8. Aggiungere il codice seguente al metodo **main** per ottenere un riferimento al servizio Bus di servizio:
    ...
    ```
9. Nel metodo **main** configurare e creare un'istanza di **EventProcessorHost**. L'opzione **setInvokeProcessorAfterReceiveTimeout** assicura che l'istanza di **EventProcessorHost** richiami il metodo **onEvents** nell'interfaccia **IEventProcessor** anche in assenza di messaggi da elaborare. Il metodo **onEvents** richiama quindi il metodo **AppendAndCheckPoint** a ogni scadere del timeout.
...
10. Nel metodo **main** registrare l'implementazione di **IEventProcessor** con l'istanza **EventProcessorHost**:
    ...
    ```
11. Infine, aggiungere la logica al metodo **main** per chiudere l'istanza di **EventProcessorHost**:
...
La prima attività consiste nell'aggiungere un progetto Maven denominato **process-interactive-messages** che riceve i messaggi inviati nella coda del bus di servizio dalle istanze di **EventProcessor**.
...
2. Aggiungere le istruzioni `import` seguenti al file:
    ...
    ```
3. Aggiungere alla classe **App** le variabili a livello di classe seguenti e sostituire **{yourservicebusnamespace}** con lo spazio dei nomi del bus di servizio e **{yourservicebuslistenkey}** con la chiave **listen** della coda. Con i valori della chiave **listen** e lo spazio dei nomi annotati nella precedente sezione [Effettuare il provisioning di un account di archiviazione di Azure e di una coda del bus di servizio](#provision-an-azure-storage-account-and-a-service-bus-queue) :
...
> [!NOTE]
> Per visualizzare gli aggiornamenti nel file BLOB, potrebbe essere necessario ridurre la costante **MAX_BLOCK_SIZE** nella classe **StoreEventProcessor** a un valore inferiore, ad esempio **1024**. Questa modifica è utile perché raggiungere il limite di dimensione del blocco con i dati inviati dal dispositivo simulato può richiedere tempo. Con una dimensione del blocco ridotta, la creazione e l'aggiornamento del BLOB richiedono meno tempo. Tuttavia, una dimensione del blocco maggiore rende l'applicazione più scalabile.
> 
> 
...
Per visualizzare esempi di soluzioni complete che usano l'hub IoT, vedere [Azure IoT Suite][lnk-suite].
...
[coda del bus di servizio]: ../service-bus-messaging/service-bus-dotnet-get-started-with-queues.md

[Guida per gli sviluppatori dell'hub IoT di Azure - Da dispositivo a cloud]: iot-hub-devguide-messaging.md
...
[BLOB in blocchi di Azure]: https://msdn.microsoft.com/library/azure/ee691964.aspx
