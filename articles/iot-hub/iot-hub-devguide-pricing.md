---
title: Guida per gli sviluppatori - Esempi di prezzi | Documentazione Microsoft
...
| Messaggi da dispositivo a cloud | Per i messaggi inviati correttamente è previsto un addebito in base a blocchi di 4 KB, ad esempio un messaggio di 6 KB viene addebitato come 2 messaggi. |
...
| Letture del dispositivo gemello | Le letture del dispositivo gemello dal dispositivo e dal back-end vengono addebitate come messaggi in blocchi di 512 byte. Ad esempio, la lettura di un messaggio di 6 KB del dispositivo gemello viene addebitata come 12 messaggi. |
...
| Operazioni dei processi per ogni dispositivo | Le operazioni dei processi, ad esempio gli aggiornamenti del dispositivo gemello e i metodi, vengono addebitati come messaggi normali. Ad esempio, un processo che provoca 1000 chiamate del metodo con richieste di 1 KB e risposte con corpo vuoto verrà addebitato come 1000 messaggi. |

> AZURE.NOTE Tutte le dimensioni vengono calcolate considerando le dimensioni del payload in byte. Il frame del protocollo viene ignorato. Nel caso dei messaggi, che hanno proprietà e corpo, le dimensioni vengono calcolate in modo indipendente dal protocollo, come illustrato nella [Guida per gli sviluppatori della messaggistica dell'hub IoT][lnk-message-size].

## <a name="example-1--"></a>Esempio n. 1 

Un dispositivo invia all'hub IoT un messaggio da 1 KB dal dispositivo al cloud ogni minuto e il messaggio viene quindi letto da Analisi di flusso di Azure. Il back-end chiama un metodo, con payload di 512 byte, sul dispositivo ogni dieci minuti per attivare un'azione specifica. Il dispositivo risponde al metodo con un risultato di 200 byte.
...
Un dispositivo invia un messaggio di 100 KB dal dispositivo al cloud ogni ora. Aggiorna anche il rispettivo dispositivo gemello con payload di 1 KB ogni 4 ore. Il back-end legge una volta al giorno il dispositivo gemello di 14 KB e lo aggiorna con payload di 512 byte per modificare le configurazioni.
...
Il back-end utilizza 28 messaggi (14 KB/0,5 KB) per leggere il dispositivo gemello più 1 messaggio per aggiornarlo, per un totale di 29 messaggi.

In totale il dispositivo e il back-end utilizzano 185 messaggi al giorno.
