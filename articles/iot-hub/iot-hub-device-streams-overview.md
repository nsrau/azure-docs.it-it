---
title: Flussi di dispositivi dell'hub Di Azure IoT Documenti Microsoft
description: Panoramica dei flussi di dispositivi dell'hub Di Azure IoT, che facilitano i tunnel TCP bidirezionali sicuri per un'ampia gamma di scenari di comunicazione da cloud a dispositivo.
author: robinsh
services: iot-hub
ms.service: iot-hub
ms.topic: conceptual
...
!["Panoramica dei flussi del dispositivo Dell'hub IoT"](./media/iot-hub-device-streams-overview/iot-hub-device-streams-overview.png )
...
* **Connettività sicura compatibile con** il firewall: I dispositivi IoT possono essere raggiunti dagli endpoint del servizio senza aprire la porta del firewall in ingresso nel dispositivo o nei perimetri di rete (è necessaria solo la connettività in uscita all'hub IoT sulla porta 443).

* **Autenticazione:** Sia i lati del dispositivo che quelli del servizio del tunnel devono eseguire l'autenticazione con l'hub IoT usando le credenziali corrispondenti.

* **Crittografia:** Per impostazione predefinita, i flussi di dispositivi Dell'hub IoT usano connessioni abilitate per TLS.By default, IoT Hub device streams use TLS-enabled connections. Ciò garantisce che il traffico sia sempre crittografato indipendentemente dal fatto che l'applicazione usi o no la crittografia.

* **Semplicità di connettività:** In molti casi, l'uso dei flussi di dispositivi elimina la necessità di una configurazione complessa di reti private virtuali per abilitare la connettività ai dispositivi IoT.

* **Compatibilità con lo stack TCP/IP:** I flussi di dispositivi dell'hub IoT possono supportare il traffico delle applicazioni TCP/IP. Ciò significa che una vasta gamma di protocolli proprietari e di protocolli basati su standard può sfruttare questa funzionalità.

* **Facilità d'uso nelle configurazioni di rete privata:** Il servizio può comunicare con un dispositivo facendo riferimento al relativo ID dispositivo, anziché all'indirizzo IP del dispositivo. Ciò è utile quando un dispositivo si trova all'interno di una rete privata e dispone di un indirizzo IP privato oppure quando l'indirizzo IP del dispositivo viene assegnato in modo dinamico e non è noto al lato servizio.

## <a name="device-stream-workflows"></a>Flussi di lavoro di flusso del dispositivoDevice stream workflows
...
!["Processo di handshake flusso di dispositivi"](./media/iot-hub-device-streams-overview/iot-hub-device-streams-handshake.png)
...
!["Endpoint flusso dispositivi"](./media/iot-hub-device-streams-overview/device-stream-in-portal.png)

In alternativa, le informazioni sugli endpoint possono essere recuperate usando l'interfaccia della riga di comando di Azure nella sezione delle proprietà dell'hub, in particolare e `property.hostname` `property.deviceStreams` le chiavi.
...
> [!NOTE]
> Verificare che sia installata l'interfaccia della riga di comando di Azure versione 2.0.57 o successiva. È possibile scaricare la versione più recente dalla pagina [Installa interfaccia della riga di comando](https://docs.microsoft.com/cli/azure/install-azure-cli?view=azure-cli-latest) di Azure.You can download the latest version from the Install Azure CLI page.
>

## <a name="allow-outbound-connectivity-to-the-device-streaming-endpoints"></a>Consentire la connettività in uscita agli endpoint di streaming del dispositivoAllow outbound connectivity to the device streaming endpoints

Come accennato all'inizio di questo articolo, il dispositivo crea una connessione in uscita all'endpoint di streaming dell'hub IoT durante il processo di avvio dei flussi del dispositivo. I firewall nel dispositivo o nella relativa rete devono consentire la connettività in uscita al gateway di streaming tramite la porta 443 (la comunicazione avviene su una connessione WebSocket crittografata con TLS).

Il nome host dell'endpoint di streaming del dispositivo è disponibile nel ![portale dell'hub IoT di Azure nella scheda Panoramica. "Endpoint flusso dispositivi".](./media/iot-hub-device-streams-overview/device-stream-in-portal.png)
...
È possibile configurare i log di Monitoraggio di Azure per raccogliere il log attività dei flussi di dispositivi nell'hub IoT.You can set up Azure Monitor logs to collect the activity log of device streams in your IoT Hub. Ciò può essere molto utile in scenari di risoluzione dei problemi.

Seguire i passaggi seguenti per configurare i log di Monitoraggio di Azure per le attività del flusso di dispositivi dell'hub IoT:Follow the steps below to configure Azure Monitor logs for your IoT Hub's device stream activities:

1. Passare alla scheda *Impostazioni di diagnostica* nell'hub IoT e fare clic sul collegamento *Attiva diagnostica*.

   !["Attivazione dei log di diagnostica"](./media/iot-hub-device-streams-overview/device-streams-diagnostics-settings-pane.png)

2. Specificare un nome per le impostazioni di diagnostica e scegliere l'opzione *Invia a Log Analytics*. Verrà visualizzato il controllo per scegliere una risorsa dell'area di lavoro di Log Analytics esistente o crearne una nuova. Inoltre, controllare *DeviceStreams* nell'elenco.

    !["Abilita registri flussi di dispositivi"](./media/iot-hub-device-streams-overview/device-streams-configure-diagnostics.png)
...
3. È ora possibile accedere ai log di flussi dispositivo nella scheda*Log* del portale dell'hub IoT. I log attività di flusso dispositivo verranno visualizzati nella tabella `AzureDiagnostics` e avranno `Category=DeviceStreams`.
   ...
   Come illustrato di seguito, l'identità del dispositivo di destinazione e il risultato dell'operazione sono disponibili anche nei registri.

   !["Accesso ai registri di flusso del dispositivo"](./media/iot-hub-device-streams-overview/device-streams-view-logs.png)

## <a name="regional-availability"></a>Disponibilità a livello di area

Durante l'anteprima pubblica, i flussi di dispositivi Dell'Hub IoT sono disponibili nelle aree STATI centrali degli Stati Uniti, stati Uniti centrali EUAP, Nord Europa e Sud-Est asiatico. Assicurarsi quindi che l'hub venga creato in una di queste aree.
...
## <a name="iot-hub-device-stream-samples"></a>Esempi di flusso del dispositivo dell'hub IoTIoT Hub device stream samples

Nella pagina dell'hub IoT sono disponibili due esempi di [guida rapida.](/azure/iot-hub) Questi illustrano l'uso dei flussi di dispositivi da parte delle applicazioni.

* L'esempio *echo* illustra l'uso a livello di codice dei flussi di dispositivo (chiamando direttamente l'API SDK).
...
L'esempio echo illustra l'uso a livello di codice dei flussi dispositivo per inviare e ricevere byte tra le applicazioni di servizio e di dispositivo. Si noti che è possibile utilizzare programmi di servizio e dispositivo in lingue diverse. Ad esempio, è possibile utilizzare il programma per dispositivi C con il programma di servizio C .

Ecco gli esempi di eco:

* [Programma di assistenza e assistenza in C](quickstart-device-streams-echo-csharp.md)

* [Programma di servizio Node.js](quickstart-device-streams-echo-nodejs.md)

* [Programma per dispositivi C](quickstart-device-streams-echo-c.md)
...
!["Configurazione proxy del flusso di dispositivo per SSH/RDP"](./media/iot-hub-device-streams-overview/iot-hub-device-streams-ssh.png)
...
* La scelta della porta 22 dipende dal protocollo e specifica di SSH in questo caso. Nel caso di RDP, è necessario usare la porta 3389. Questa può essere configurata nei programmi di esempio forniti.
...
* [Programma di assistenza e assistenza in C](quickstart-device-streams-proxy-csharp.md)

* [Programma di servizio Node.js](quickstart-device-streams-proxy-nodejs.md)

* [Programma per dispositivi C](quickstart-device-streams-proxy-c.md)

## <a name="next-steps"></a>Passaggi successivi

Usa i link qui sotto per ulteriori informazioni sui flussi di dispositivi.

> [!div class="nextstepaction"]
> [Flussi di dispositivi nello show IoT (Channel 9)](https://nam06.safelinks.protection.outlook.com/?url=https%3A%2F%2Fchannel9.msdn.com%2FShows%2FInternet-of-Things-Show%2FAzure-IoT-Hub-Device-Streams&data=02%7C01%7Crezas%40microsoft.com%7Cc3486254a89a43edea7c08d67a88bcea%7C72f988bf86f141af91ab2d7cd011db47%7C1%7C0%7C636831125031268909&sdata=S6u9qiehBN4tmgII637uJeVubUll0IZ4p2ddtG5pDBc%3D&reserved=0)
