...
* **--config-id**: nome della configurazione che verrà creata nell'hub IoT. Assegnare alla configurazione un nome univoco con un massimo di 128 lettere in minuscolo. Evitare gli spazi e i seguenti caratteri non validi: `& ^ [ ] { } \ | " < > /`.
...
az iot hub configuration show --config-id [configuration id] --hub-name [hub name]
   ```
* **--config-id**: nome della configurazione che si trova nell'hub IoT.
...
È possibile visualizzare un elenco degli ID o degli oggetti dei dispositivi per ognuna delle metriche tramite il comando seguente:
...
* **--metric-id**: nome della metrica per il quale si vuole visualizzare l'elenco di ID, ad esempio `appliedCount`
