...
| ------- | --------------- | ---------- |
| **Messaggi ed eventi del dispositivo** | Sì, il routing dei messaggi può essere usato per i dati di telemetria, le modifiche al dispositivo di report twin, gli eventi del ciclo di vita del dispositivo e gli eventi di modifica gemelli digitali (parte [dell'anteprima pubblica IoT Plug and Play](../iot-pnp/overview-iot-plug-and-play.md)). | Sì, Griglia di eventi può essere usato per i dati di telemetria, ma può anche segnalare quando i dispositivi vengono creati, eliminati, connessi e disconnessi dall'hub IoT |
| **Ordinamento** | Sì, viene mantenuto l'ordinamento degli eventi.  | No, l'ordine degli eventi non è garantito. | 
| **Filtro** | Applicazione di filtri avanzati in proprietà di applicazione del messaggio, proprietà di sistema del messaggio, corpo del messaggio, tag del dispositivo gemello e proprietà del dispositivo gemello. Il filtro non viene applicato agli eventi di modifica a due digitali. Ad esempio, vedere [Sintassi query per il routing dei messaggi di hub IoT](iot-hub-devguide-routing-query-syntax.md). | Filtraggio in base al tipo di evento, al tipo di oggetto e agli attributi in ogni evento. Per alcuni esempi, vedere [Informazioni sul filtro degli eventi nelle sottoscrizioni griglia di](../event-grid/event-filtering.md)eventi . Quando si sottoscrive eventi di telemetria, è possibile applicare filtri aggiuntivi sui dati per filtrare le proprietà del messaggio, il corpo del messaggio e il dispositivo gemello nell'hub IoT, prima della pubblicazione in Griglia di eventi. Vedere [come filtrare gli eventi](../iot-hub/iot-hub-event-grid.md#filter-events). |
| **Endpoint** | <ul><li>Hub eventi</li> <li>Archiviazione BLOB di Azure</li> <li>Coda del bus di servizio</li> <li>Argomenti del bus di servizio</li></ul><br>Gli SKU per hub IoT a pagamento (S1, S2 e S3) sono limitati a 10 endpoint personalizzati. Si possono creare 100 route per ogni hub IoT. | <ul><li>Funzioni di Azure</li> <li>Automazione di Azure</li> <li>Hub eventi</li> <li>App per la logica</li> <li>Archiviazione BLOB</li> <li>Argomenti personalizzati</li> <li>Archiviazione code</li> <li>Microsoft Flow</li> <li>Servizi di terze parti tramite webhook</li></ul><br>Sono supportati 500 endpoint per hub IoT. Per l'elenco più aggiornato di endpoint, vedere [Gestori eventi di Griglia di eventi](../event-grid/overview.md#event-handlers). |
| **Costo** | Non vengono addebitati costi separati per il routing dei messaggi. Vengono addebitati solo i dati di telemetria in ingresso nell'hub IoT. Se ad esempio un messaggio viene instradato a tre endpoint diversi, viene fatturato un solo messaggio. | Non è previsto alcun addebito per l'hub IoT. Griglia di eventi offre le prime 100.000 operazioni al mese gratuitamente e quindi 0,60 USD per milione di operazioni in seguito. |
...
| Funzionalità | Routing dei messaggi dell'hub IoT | Integrazione dell'hub IoT con Griglia di eventi |
| ------- | --------------- | ---------- |
| **Dimensione massima dei messaggi** | 256 KB, da dispositivo a cloud | 256 KB, da dispositivo a cloud |
...
Il routing dei messaggi dell'hub IoT e l'integrazione dell'hub IoT con Griglia di eventi ottengono risultati simili con azioni diverse. Entrambi acquisiscono le informazioni dalla soluzione hub IoT e le passano in modo che altri servizi possano rispondere. Per decidere quale usare, Considera le seguenti domande per aiutarti a prendere in considerazione la tua decisione: 
...
* **Quale tipo di dati si invierà agli endpoint?**
   ...
   L'integrazione dell'hub IoT con Griglia di eventi è indicata per gli eventi che si verificano nel servizio hub IoT. Questi eventi dell'hub IoT includono dati di telemetria, dispositivo creato, eliminato, connesso e disconnesso. Quando si sottoscrive eventi di telemetria, è possibile applicare filtri aggiuntivi sui dati per filtrare le proprietà del messaggio, il corpo del messaggio e il dispositivo gemello nell'hub IoT, prima della pubblicazione in Griglia di eventi. Vedere [come filtrare gli eventi](../iot-hub/iot-hub-event-grid.md#filter-events).

* **Quali endpoint devono ricevere queste informazioni?**

   Il routing dei messaggi dell'hub IoT supporta un numero limitato di endpoint univoci e tipi di endpoint, ma è possibile creare connettori per reindirizzare i dati e gli eventi a endpoint aggiuntivi. Per un elenco completo degli endpoint supportati, vedere la tabella nella sezione precedente. 

   L'integrazione dell'hub IoT con Griglia di eventi supporta 500 endpoint per ogni hub IoT e una maggiore varietà di tipi di endpoint. Si integra in modo nativo con funzioni di Azure, app per la logica, code di archiviazione e bus di servizio e funziona anche con i webhook per estendere l'invio di dati all'esterno dell'ecosistema di servizi di Azure e in applicazioni aziendali di terze parti.
...
* Provare l'integrazione di Griglia di eventi [inviando notifiche di posta elettronica sugli eventi dell'hub IoT di Azure usando app per](../event-grid/publish-iot-hub-events-to-logic-apps.md)la logica .
