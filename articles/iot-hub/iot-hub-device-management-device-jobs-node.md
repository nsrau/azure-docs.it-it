<properties
	pageTitle="Processi del dispositivo in Gestione dei dispositivi dell'hub IoT | Microsoft Azure"
...
Per avviare l'esempio, è necessario eseguire `jobClient_scheduleJob.js`. Verrà avviato il processo di aggiornamento del firmware su tutti i dispositivi simulati. Per avviare l'esempio, seguire questa procedura:
...
2.  Aprire **jobClient\_scheduleJob.js** e sostituire il segnaposto con la stringa di connessione dell'hub IoT.

4.  Eseguire `node jobClient_scheduleJob.js`.
...
In generale, i processi del dispositivo vengono avviati usando diversi metodi **schedule&lt;JobName&gt;** sull'istanza di **JobClient**. Nell'esempio di aggiornamento del firmware viene chiamato il metodo **scheduleFirmwareUpdate**. Poiché si passa una matrice con 6 ID dispositivo, vengono avviati 7 processi del dispositivo, uno per ogni dispositivo da aggiornare e un processo del dispositivo padre usato per tenere traccia degli altri 6.

Nel frammento di codice seguente viene avviato un processo di aggiornamento. La chiamata accetta una matrice di stringhe con valori **deviceId** come parametro, che rappresentano i dispositivi da aggiornare.
