<properties
    pageTitle="Creare un hub IoT mediante l'interfaccia della riga di comando | Microsoft Azure"
...
> [AZURE.NOTE] Azure offre due modelli di distribuzione per creare e usare le risorse: [Gestione risorse e la distribuzione classica](../resource-manager-deployment-model.md). L'interfaccia della riga di comando di Azure deve essere impostata obbligatoriamente sulla modalità Azure Resource Manager:
```
...
> [AZURE.TIP] L'articolo [Usare l'interfaccia della riga di comando di Azure per gestire risorse e gruppi di risorse][lnk-CLI-arm] offre altre informazioni su come usare l'interfaccia della riga di comando di Azure per gestire le risorse di Azure. 
...
> [AZURE.NOTE] Questo comando crea un hub IoT S1 Standard che viene addebitato. Per eliminare l'hub IoT **exampleIoTHubName** eseguire il comando seguente 
```
