...
* Librerie negli [SDK di Azure IoT](https://github.com/Azure/azure-iot-sdks).
* Il protocollo MQTT direttamente.

La porta MQTT (8883) è bloccata in molti ambienti di rete aziendali ed educativi. Se non è possibile aprire la porta 8883 nel firewall, si consiglia di utilizzare MQTT su Web Sockets. MQTT su Web Sockets comunica tramite la porta 443, che è quasi sempre aperta negli ambienti di rete. Per informazioni su come specificare MQTT e MQTT rispetto ai protocolli Web Sockets quando si usano gli SDK di Azure IoT, vedere [Uso degli SDK del dispositivo.](#using-the-device-sdks)
...
La tabella seguente contiene collegamenti a esempi di codice per ogni linguaggio supportato e specifica il parametro da utilizzare per stabilire una connessione all'hub IoT utilizzando MQTT o MQTT tramite il protocollo Web Sockets.

| Linguaggio | Parametro del protocollo MQTT | Parametro del protocollo MQTT su Web Sockets
| --- | --- | --- |
...
Il frammento seguente mostra come specificare il protocollo MQTT su Web Sockets quando si usa Azure IoT Python SDK:
...
Per garantire che una connessione client/hub IoT rimanga attiva, sia il servizio che il client inviano regolarmente un ping *keep-alive* l'uno all'altro. Il client che usa L'SDK IoT invia un keep-alive all'intervallo definito in questa tabella seguente:
...
|Pitone (V2)   | 60 secondi |  No   |

Seguendo [la specifica MQTT](http://docs.oasis-open.org/mqtt/mqtt/v3.1.1/os/mqtt-v3.1.1-os.html#_Toc398718081), l'intervallo di ping keep-alive dell'hub IoT è 1,5 volte il valore keep-alive del client. Tuttavia, l'hub IoT limita il timeout massimo sul lato server a 29,45 minuti (1767 secondi) perché tutti i servizi di Azure sono associati al timeout di inattività TCP del servizio di bilanciamento del carico di Azure, ovvero 29,45 minuti. 

Ad esempio, un dispositivo che utilizza Java SDK invia il ping keep-alive e perde la connettività di rete. 230 secondi dopo, il dispositivo perde il ping keep-alive perché è offline. Tuttavia, l'hub IoT non chiude immediatamente `(230 * 1.5) - 230 = 115` la connessione: attende un altro secondo prima di disconnettere il dispositivo con l'errore [404104 DeviceConnectionClosedRemotely](iot-hub-troubleshoot-error-404104-deviceconnectionclosedremotely.md). 

Il valore keep-alive massimo del `1767 / 1.5 = 1177` client che è possibile impostare è secondi. Qualsiasi traffico azzererà il keep-alive. Ad esempio, un aggiornamento corretto del token di firma di accesso condiviso reimposta il keep-alive.
...
* Per il campo **Password** usare un token di firma di accesso condiviso. Il formato del token di firma di accesso condiviso è identico a quello per i protocolli HTTPS e AMQP:
  ...
  > [!NOTE]
  > Le password di token SAS non sono obbligatorie se si usa l'autenticazione dei certificati X.509. Per altre informazioni, vedere Configurare la [sicurezza X.509 nell'hub IoT](iot-hub-security-x509-get-started.md) di Azure e seguire le istruzioni del codice [riportate](#tlsssl-configuration)di seguito.
...
### <a name="an-example-of-c-code-using-mqtt-without-azure-iot-c-sdk"></a>Un esempio di codice C che usa MQTT senza Azure IoT C SDK
In questo [repository](https://github.com/Azure-Samples/IoTMQTTSample)sono disponibili un paio di progetti dimostrativi C/C, che illustrano come inviare messaggi di telemetria, ricevere eventi con un hub IoT senza usare Azure IoT C SDK. 

Questi esempi usano la libreria Eclipse Mosquitto per inviare un messaggio al broker MQTT implementato nell'hub IoT.

Questo repository contiene:

**Per Windows:**

* TelemetryMQTTWin32: contiene il codice per inviare un messaggio di telemetria a un hub IoT di Azure, compilato ed eseguito in un computer Windows.TelemetryMQTTWin32: contains code to send a telemetry message to an Azure IoT hub, built and run on a Windows machine.

* SubscribeMQTTWin32: contiene il codice per sottoscrivere gli eventi di un determinato hub IoT in un computer Windows.

* DeviceTwinMQTTWin32: contiene il codice per eseguire query e sottoscrivere gli eventi del dispositivo gemello di un dispositivo nell'hub IoT di Azure in un computer Windows.DeviceTwinMQTTWin32: contains code to query and subscribe to the device twin events of a device in the Azure IoT hub on a Windows machine.

* PnPMQTTWin32: contiene il codice per inviare un messaggio di telemetria con funzionalità del dispositivo di anteprima Plug & IoT a un hub IoT di Azure, compilato ed eseguito in un computer Windows.PnPMQTTWin32: contains code to send a telemetry message with IoT Plug & Play preview Device capabilities to an Azure IoT hub, built and run on a Windows machine. Ulteriori informazioni su IoT Plug & Play [qui](https://docs.microsoft.com/azure/iot-pnp/overview-iot-plug-and-play)

**Per Linux:**

* MQTTLinux: contiene codice e script di compilazione da eseguire su Linux (WSL, Ubuntu e Raspbian sono stati testati finora).

* LinuxConsoleVS2019: contiene lo stesso codice ma in un progetto VS2019 destinato a WSL (sottosistema Windows Linux). Questo progetto consente di eseguire il debug del codice in esecuzione su Linux passo dopo passo da Visual Studio.This project allows you to debug the code running on Linux step by step from Visual Studio.

**Per mosquitto_pub:**

Questa cartella contiene due comandi di esempio utilizzati con mosquitto_pub strumento di utilità fornito da Mosquitto.org.

* Mosquitto_sendmessage: per inviare un semplice messaggio di testo a un hub IoT di Azure che funge da dispositivo.

* Mosquitto_subscribe: per visualizzare gli eventi che si verificano in un hub IoT di Azure.Mosquitto_subscribe: to see events occurring in an Azure IoT hub.
...
Per eseguire l'autenticazione tramite un certificato del dispositivo, aggiornare il frammento di codice precedente con le modifiche seguenti (vedere Come ottenere un certificato CA X.509 su come preparare l'autenticazione basata su certificati):To authenticate using a device certificate, update the code snippet above with the following changes (see [How to get an X.509 CA certificate](./iot-hub-x509ca-overview.md#how-to-get-an-x509-ca-certificate) on how to prepare for certificate-based authentication):
...
> [!NOTE]
> Questo `{property_bag}` elemento utilizza la stessa codifica delle stringhe di query nel protocollo HTTPS.
...
Un dispositivo effettua la sottoscrizione a `$iothub/twin/res/#` per ricevere le risposte dell'operazione. Quindi, invia un messaggio `$iothub/twin/GET/?$rid={request id}`vuoto all'argomento , con un valore popolato per **l'ID richiesta.** Il servizio invia quindi un messaggio di `$iothub/twin/res/{status}/?$rid={request id}`risposta contenente i dati del dispositivo gemello sull'argomento , utilizzando lo stesso **ID richiesta** della richiesta.

L'ID richiesta può essere qualsiasi valore valido per un valore della proprietà del messaggio, come per ogni [guida per gli sviluppatori di messaggistica dell'hub IoT](iot-hub-devguide-messaging.md), e lo stato viene convalidato come numero intero.
...
Per quanto riguarda `null` gli aggiornamenti delle proprietà, i valori indicano che il membro dell'oggetto JSON viene eliminato. Si noti inoltre che `$version` indica la nuova versione della sezione delle proprietà desiderate del dispositivo gemello.
