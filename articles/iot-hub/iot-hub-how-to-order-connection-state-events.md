---
title: Ordinare gli eventi di connessione del dispositivo fr Azure IoT Hub w/Azure Cosmos DBOrder device connection events fr Azure IoT Hub w/Azure Cosmos DB
...
* Un account Azure attivo. Se non ne hai uno, puoi [creare un account gratuito.](https://azure.microsoft.com/pricing/free-trial/)
...
* Una raccolta nel database. Vedere la procedura dettagliata [Aggiungere una raccolta](../cosmos-db/create-sql-api-java.md#add-a-container). Quando si crea la `/id` raccolta, usare per la chiave di partizione.
...
1. Nell'API SQL del database Cosmos selezionare**Elementi** > di **Esplora** > dati**Nuova stored procedure**.

   ![Creare la stored procedure](./media/iot-hub-how-to-order-connection-state-events/create-stored-procedure.png)

2. Immettere **LatestDeviceConnectionState** come ID stored procedure e incollare quanto segue nel corpo della **stored procedure.** Si noti che questo codice deve sostituire il codice esistente nel corpo della stored procedure. Questo codice mantiene una riga per ogni ID dispositivo e registra lo stato di connessione più recente di tale ID identificando il numero di sequenza più alto.
...
3. Fare clic su **Crea** per creare l'app per la logica.
   ...
   > [!NOTE]
   > Per trovare e riaprire nuovamente l'app per la logica, selezionare **Gruppi di** risorse e selezionare il gruppo di risorse usato per questa procedura. Selezionare quindi la nuova app per la logica. Verrà visualizzata la finestra di progettazione dell'app per la logica.

4. In Progettazione app per la logica scorrere verso destra fino a visualizzare i trigger comuni. In **Modelli**scegliere **Applicazione per la logica vuota** per creare l'app per la logica da zero.
...
1. Nella barra di ricerca dei connettori e dei trigger digitare **HTTP** e premere INVIO.
...
4. Incollare l'esempio di codice JSON seguente nella casella di testo e quindi selezionare **Fine**:
   ...
   ![Incollare il payload JSON di esempioPaste sample JSON payload](./media/iot-hub-how-to-order-connection-state-events/paste-sample-payload.png)
...
1. Selezionare **Nuovo passaggio,** quindi **Predefinito**, quindi trovare e selezionare **Condizione**. Fare clic su **Scegliere un valore** e verrà visualizzata una casella che mostra il contenuto dinamico, ovvero i campi che è possibile selezionare. Compilare i campi come illustrato di seguito per eseguire questa operazione solo per gli eventi Dispositivo connesso e Disconnesso dispositivo:

   * Scegliere un valore: **eventType** -- selezionare questo dai campi nel contenuto dinamico che viene visualizzato quando si fa clic su questo campo.
   * Modificare "è uguale a" in **termina con**.
...
4. Compilare **cosmosdb-connection** per Nome **connessione** e selezionare la voce nella tabella, quindi selezionare **Crea**. Viene visualizzato il pannello **Esegui stored procedure.** Immettere i valori per i campi:
...
5. Selezionare **Aggiungi nuovo parametro**. Nell'elenco a discesa visualizzato, selezionare le caselle accanto a **Partizione chiave** e **Parametri per la stored procedure**, quindi fare clic in un punto qualsiasi dello schermo. aggiunge un campo per il valore della chiave di partizione e un campo per i parametri per la stored procedure.
...
6. Immettere ora il valore e i parametri della chiave di partizione come illustrato di seguito. Assicurarsi di inserire tra parentesi e virgolette doppie come mostrato. Potrebbe essere necessario fare clic su **Aggiungi contenuto dinamico** per ottenere i valori validi che è possibile utilizzare qui.
...
7. Nella parte superiore del riquadro in cui è indicato **For Each**, in **Selezionare un output dei passaggi precedenti,** assicurarsi che **l'opzione Corpo** sia selezionata.

   ![popolare app logica per ogni](./media/iot-hub-how-to-order-connection-state-events/logicapp-foreach-body.png)
...
3. Selezionare **: Sottoscrizione all'evento**.

   ![Creare una nuova sottoscrizione di eventi](./media/iot-hub-how-to-order-connection-state-events/event-subscription.png)

4. Immettere **Dettagli sottoscrizione evento**: specificare un nome descrittivo e selezionare Schema griglia **eventi**.

5. Compilare i campi **Tipi di evento.** Nell'elenco a discesa, selezionare solo **Dispositivo connesso** e **Dispositivo disconnesso** dal menu. Fare clic in un punto qualsiasi dello schermo per chiudere l'elenco e salvare le selezioni.

   ![Impostare i tipi di evento da cercare](./media/iot-hub-how-to-order-connection-state-events/set-event-types.png)

6. Per **Dettagli endpoint**, selezionare Tipo di endpoint come Web **Hook** e fare clic su seleziona endpoint e incollare l'URL copiato dall'app per la logica e confermare la selezione.

   ![Selezionare l'URL dell'endpoint](./media/iot-hub-how-to-order-connection-state-events/endpoint-select.png)

7. Il modulo dovrebbe essere simile all'esempio seguente:
...
6. Fare di nuovo clic sul dispositivo; ora le stringhe di connessione e le chiavi verranno compilate. Copiare il valore di **Stringa di connessione -- Chiave primaria** per usarlo in seguito.
...
1. Nell'area di codifica sostituire il segnaposto nella riga 15 con la stringa di connessione del dispositivo Hub di Azure IoT salvata alla fine della sezione precedente.
...
Viene visualizzato un messaggio simile all'output seguente che mostra i dati del sensore e i messaggi inviati all'hub IoT.
