...
description: Questo articolo di esercitazione descrive come instradare il traffico agli endpoint ponderati con Gestione traffico.
...
ms.locfileid: "49649435"
---
# <a name="tutorial-control-traffic-routing-with-weighted-endpoints-using-traffic-manager"></a>Esercitazione: Controllare il routing del traffico agli endpoint ponderati con Gestione traffico 

Questa esercitazione descrive come usare Gestione traffico per controllare il routing del traffico utente tra gli endpoint usando il metodo di routing Ponderato. In questo metodo di routing viene assegnato un peso a ogni endpoint come parte della configurazione del profilo di Gestione traffico e il traffico utente viene instradato in base al peso assegnato a ciascun endpoint. Ogni peso è un numero intero compreso tra 1 e 1000. Maggiore sarà il valore del peso assegnato a un endpoint, più elevata sarà la relativa priorità.
...
- Due macchine virtuali (VM) per il test di Gestione traffico: una in **Stati Uniti orientali** e la seconda in **Europa occidentale**. Le VM di test vengono usate per illustrare come Gestione traffico instrada il traffico degli utenti verso un sito Web con peso maggiore assegnato al relativo endpoint.
...
In questa sezione si creano due istanze di sito Web che forniscono due endpoint di servizio per il profilo di Gestione traffico in due aree di Azure. Per creare i due siti Web, seguire questa procedura:
...
In questa sezione viene creata una macchina virtuale, *mVMEastUS*. Questa VM verrà usata per verificare come Gestione traffico instrada il traffico verso l'endpoint del sito Web con il valore di peso maggiore.
...
2. In **Crea profilo di Gestione traffico** immettere o selezionare le informazioni seguenti, accettare le impostazioni predefinite per le impostazioni rimanenti e quindi selezionare **Crea**:
    ...
    | Gruppo di risorse          | Selezionare **Esistente** e *myResourceGroupTM1*. |
    |        |   |
...
Aggiungere le due VM che eseguono i server IIS, *myIISVMEastUS*   &  *myIISVMWEurope* per instradare il traffico degli utenti verso di esse.
...
4. Ripetere i passaggi 2 e 3 per aggiungere un altro endpoint denominato *myWestEuropeEndpoint* per l'indirizzo IP pubblico *myIISVMWEurope-ip* associato alla VM del server IIS denominata *myIISVMWEurope* e per **Peso** immettere **25**. 
...
Per visualizzare Gestione traffico in azione, completare i passaggi seguenti:
...
4. Selezionare **OK**.
5. Durante il processo di accesso potrebbe essere visualizzato un avviso relativo al certificato. Se viene visualizzato l'avviso, selezionare **Sì** o **Continua** per procedere con la connessione. 
6. In un Web browser, nella VM *myVMEastUS*, digitare il nome DNS del profilo di Gestione traffico per visualizzare il sito Web. Si viene indirizzati al sito Web ospitato nel server IIS *myIISVMEastUS* perché a esso è stato assegnato un peso maggiore di **100** rispetto al server IIS *myIISVMWEurope* (a cui è stato assegnato un valore di peso endpoint inferiore, corrispondente a **25**).
...
Eliminare i gruppi di risorse creati in questa esercitazione quando non sono più necessari. A tale scopo, selezionare il gruppo di risorse (**ResourceGroupTM1** o **ResourceGroupTM2**) e quindi selezionare **Elimina**.

## <a name="next-steps"></a>Passaggi successivi

> [!div class="nextstepaction"]
> [Indirizzare il traffico a endpoint specifici basati sulla posizione geografica dell'utente](traffic-manager-configure-geographic-routing-method.md)
