---
title: 'Esercitazione: migliorare la risposta del sito Web-gestione traffico di Azure'
...
- Due istanze di siti Web di base in esecuzione in diverse aree di Azure, **Stati Uniti orientali** ed **Europa occidentale**.
- Due macchine virtuali di test per il test di gestione traffico: una macchina virtuale negli **Stati Uniti orientali** e la seconda macchina virtuale nell' **Europa occidentale**. Le VM di test vengono usate per illustrare come Gestione traffico instrada il traffico degli utenti verso il sito Web in esecuzione nella stessa area poiché offre la latenza più bassa.
...
2. In **Crea macchina virtuale** digitare o selezionare i valori seguenti nella scheda **Nozioni di base**:
   ...
   - **Account amministratore** > **nome**utente: immettere il nome utente desiderato.
   ...
   - **Regole porta in ingresso** > **selezionare porte in ingresso**: selezionare **RDP** e **http** nella casella di riepilogo a discesa.

3. Selezionare la scheda **gestione** oppure selezionare **Avanti: dischi**, quindi **Avanti: rete**, quindi **Avanti: gestione**. In **Monitoraggio** impostare **Diagnostica di avvio** su **Off**.
...
In questa sezione si installa il server IIS nelle due macchine virtuali *myIISVMEastUS* e *myIISVMWestEurope*e quindi si aggiorna la pagina del sito Web predefinito. La pagina del sito Web personalizzata mostra il nome della VM a cui viene connesso l'utente quando visita il sito Web da un Web browser.
...
9. Ripetere i passaggi 1-8 con creando una connessione RDP con la macchina virtuale *myIISVMWestEurope* all'interno del gruppo di risorse *MYRESOURCEGROUPTM2* per installare IIS e personalizzare la pagina Web predefinita.
...
Aggiungere le due macchine virtuali che eseguono i server IIS- *myIISVMEastUS* & *myIISVMWestEurope* per indirizzare il traffico utente all'endpoint più vicino all'utente.
...
4. Ripetere i passaggi 2 e 3 per aggiungere un altro endpoint denominato *myWestEuropeEndpoint* per l'indirizzo IP pubblico *myIISVMWestEurope-IP* associato alla macchina virtuale del server IIS denominata *myIISVMWestEurope*.
...
2. A questo punto, connettersi alla VM *myVMWestEurope* che si trova in **Europa occidentale** eseguendo i passaggi da 1 a 5 e passare al nome di dominio del profilo di Gestione traffico da questa VM. Dal momento che la macchina virtuale si trova in **Europa occidentale**, si viene indirizzati al sito Web ospitato nel server IIS più vicino *myIISVMWestEurope* che si trova in **Europa occidentale**.
