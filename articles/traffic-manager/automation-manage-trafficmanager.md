<properties 
 pageTitle="Gestire Gestione traffico di Azure mediante Automazione di Azure" 
 description="Informazioni su come è possibile usare il servizio Automazione di Azure per gestire Gestione traffico di Azure." 
...
## In che modo Automazione di Azure può facilitare la gestione di Gestione traffico di Azure

È possibile gestire Gestione traffico in Automazione di Azure usando i cmdlet di PowerShell disponibili negli [strumenti di Azure PowerShell](https://msdn.microsoft.com/library/azure/jj156055.aspx). Automazione di Azure può chiamare questi cmdlet di PowerShell per consentire l'esecuzione di tutte le operazioni di gestione di Gestione traffico dall'interno del servizio. È anche possibile abbinare tali cmdlet in Automazione di Azure ai cmdlet per altri servizi di Azure per automatizzare attività complesse in tutti i servizi di Azure e nei sistemi di terze parti.


## Passaggi successivi

A questo punto, dopo aver appreso le nozioni di base di Automazione di Azure e del modo in cui può essere usato per gestire Gestione traffico di Azure, visitare i collegamenti seguenti per altre informazioni su Automazione di Azure.
