...
* Gestione traffico considera un endpoint Online solo se il probe ottiene una risposta HTTP 200 dal percorso probe. Se l'applicazione restituisce qualsiasi altro codice di risposta HTTP, è necessario aggiungere tale codice di risposta agli intervalli di codici di [stato previsti](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-monitoring#configure-endpoint-monitoring) del profilo di Gestione traffico.
* Una risposta di reindirizzamento 30x viene considerata come un errore, a meno che non sia stato specificato come codice di risposta valido negli intervalli di codici di [stato previsti](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-monitoring#configure-endpoint-monitoring) del profilo di Gestione traffico. Gestione traffico non sonda la destinazione del reindirizzamento.
...
Per questo esempio vogliamo vedere la risposta dal\/nostro URL probe: http: /watestsdp2008r2.cloudapp.net:80/Probe. L'esempio di PowerShell seguente illustra il problema.
