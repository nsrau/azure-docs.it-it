<properties title="Federations Migration" pageTitle="Migrazione di federazioni" description="Outlines the steps to migrate an existing app built with Federations feature to Elastic Scale model." metaKeywords="sharding scaling, federations, Azure SQL DB sharding, Elastic Scale" services="sql-database" documentationCenter=""  manager="jhubbard" authors="sidneyh@microsoft.com"/>
...
Con le API di Scalabilità elastica, la connessione a una specifica partizione viene stabilita mediante il [routing dipendente dai dati](./sql-database-elastic-scale-data-dependent-routing.md) usando il metodo **OpenConnectionForKey** sulla classe **RangeShardMap**. 
