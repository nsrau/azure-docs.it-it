...
ms.locfileid: "78200703"
---
# <a name="fetch-shared-access-signature-tokens-in-code"></a>Recuperare i token della firma di accesso condiviso nel codice

È possibile gestire l'account di archiviazione con i token di firma di [accesso condiviso](../storage/common/storage-dotnet-shared-access-signature-part-1.md) nell'insieme di credenziali delle chiavi. In questo articolo vengono forniti esempi di codice in linguaggio C, che recupera un token di firma di accesso condiviso ed esegue operazioni con tale token.  Per informazioni su come creare e archiviare i token di firma di accesso condiviso, vedere [Gestire le chiavi dell'account di archiviazione con Key Vault e l'interfaccia della riga di comando](key-vault-ovw-storage-keys.md) di Azure o Gestire le chiavi dell'account di archiviazione con Key Vault e Azure [PowerShell.](key-vault-overview-storage-keys-powershell.md)
...
- Informazioni su come [gestire le chiavi dell'account di archiviazione con Key Vault e l'interfaccia della riga di comando di Azure](key-vault-ovw-storage-keys.md) o Azure [PowerShell.](key-vault-overview-storage-keys-powershell.md)
