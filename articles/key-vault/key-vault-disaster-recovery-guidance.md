...
L'insieme di credenziali delle chiavi di Azure dispone di più livelli di ridondanza incorporati, per garantire che le chiavi e i segreti rimangano disponibili per l'applicazione anche quando si verificano errori di singoli componenti del servizio.

I contenuti dell'insieme di credenziali delle chiavi vengono replicati sia all'interno dell'area sia a un'area secondaria distante almeno 240 chilometri all'interno della stessa geografia. Questo garantisce un'elevata durabilità delle chiavi e dei segreti.

Se si verificano errori di singoli componenti del servizio dell'insieme di credenziali delle chiavi, per gestire la richiesta subentrano componenti alternativi all'interno dell'area. In questo caso non si verifica alcuna riduzione delle prestazioni delle funzionalità. Non è necessario eseguire alcuna azione, poiché questa operazione viene eseguita in modo automatico e trasparente per l'utente.

Nella rara eventualità che l'intera area di Azure risulti non disponibile, le richieste effettuate all'insieme di credenziali delle chiavi di tale area vengono indirizzate automaticamente a un'area secondaria ("failover"). Quando l'area primaria diventa di nuovo disponibile, le richieste vengono reindirizzate a tale area ("failback"). Anche in questo caso non è necessario eseguire alcuna azione, poiché questa operazione viene eseguita in modo automatico. Esistono alcune limitazioni che è necessario tenere presenti:
