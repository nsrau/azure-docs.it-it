...
Se l'insieme di credenziali delle chiavi è in esecuzione su un account Microsoft diverso da quello che si è connessi a Visual Studio (ad esempio, l'insieme di credenziali delle chiavi è in esecuzione sull'account di lavoro, ma Visual Studio utilizza l'account privato) viene visualizzato un errore nel file di Program.cs , che Visual Studio non può accedere all'insieme di credenziali delle chiavi. Per risolvere il problema:
...
Per ulteriori informazioni sullo sviluppo di Key Vault, leggere la Guida per [gli sviluppatori di Key Vault](key-vault-developers-guide.md).
