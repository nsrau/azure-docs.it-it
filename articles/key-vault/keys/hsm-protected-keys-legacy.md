...
Se si utilizza nCipher nShield Edge, per cambiare la modalità: 1. Usare il pulsante Modalità per evidenziare la modalità richiesta. 2. Entro pochi secondi, premere e tenere premuto per pochi secondi il pulsante Cancella. Se la modalità cambia, il LED della nuova modalità smette di lampeggiare e rimane acceso. È possibile che il LED di stato lampeggi in modo irregolare per pochi secondi e quindi lampeggi in modo regolare quando il dispositivo è pronto. In caso contrario, il dispositivo rimane nella modalità corrente, con il LED della modalità appropriata acceso.
...
Tale programma crea un file di **ambiente di sicurezza** nel percorso %NFAST_KMDATA%\local\world, che corrisponde alla cartella C:\ProgramData\nCipher\Key Management Data\local. È possibile utilizzare valori diversi per il quorum, ma nel nostro esempio, viene richiesto di immettere tre schede vuote e pin per ciascuno di essi. Qualsiasi coppia di schede consente di accedere in modo completo all'ambiente di sicurezza. Tali schede diventano il **set di schede amministrative** per il nuovo ambiente di sicurezza.
...
> [!NOTE]
> Per convalidare il pacchetto scaricato, l'HSM deve essere connesso, acceso e deve avere un mondo di sicurezza su di esso (ad esempio quello appena creato).
...
Ora sei pronto per creare una nuova chiave.
...
### <a name="step-42-encrypt-your-key-by-using-microsofts-key-exchange-key"></a>Passaggio 4.2: Crittografare la chiave utilizzando la chiave di scambio delle chiavi di MicrosoftStep 4.2: Encrypt your key by using Microsoft's Key Exchange Key
