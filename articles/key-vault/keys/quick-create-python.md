...
Nella finestra della console installare la libreria delle chiavi di Azure Key Vault per Python.
...
Creare un criterio di accesso per l'insieme di credenziali delle chiavi che conceda l'autorizzazione all'entità servizio, passando il valore di clientId al comando [az keyvault set-policy](/cli/azure/keyvault?view=azure-cli-latest#az-keyvault-set-policy). Concedere all'entità servizio le autorizzazioni get, list e create per le chiavi.
...
La libreria client di Azure Key Vault per Python consente di gestire le chiavi e le risorse correlate, come i certificati e i segreti. Gli esempi di codice riportati di seguito illustrano come creare un client e come creare, recuperare ed eliminare una chiave.
...
Ora che l'applicazione è autenticata, è possibile inserire una chiave nell'insieme di credenziali delle chiavi. 

```python
rsa_key = client.create_rsa_key(myKey,size=2048)
```

È possibile verificare che la chiave sia stata impostata con il comando [az keyvault key show](/cli/azure/keyvault/key?view=azure-cli-latest#az-keyvault-key-show):
...
È ora possibile recuperare la chiave creata in precedenza.
...
La chiave è ora salvata come `retrieved_key`.

### <a name="delete-a-key"></a>Eliminare una chiave

A questo punto è possibile eliminare la chiave dall'insieme di credenziali delle chiavi.

```python
client.delete_key(keyName)
```

È possibile verificare che la chiave sia stata eliminata con il comando [az keyvault key show](/cli/azure/keyvault/key?view=azure-cli-latest#az-keyvault-key-show):
...
In questa guida di avvio rapido è stato creato un insieme di credenziali delle chiavi, quindi è stata archiviata e recuperata una chiave. Per altre informazioni sul servizio Key Vault e su come integrarlo nelle applicazioni, continuare con gli articoli seguenti.
