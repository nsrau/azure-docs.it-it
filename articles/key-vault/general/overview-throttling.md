...
1. Se si usa Key Vault per archiviare le credenziali per un servizio, controllare se il servizio supporta Autenticazione di Azure AD per l'autenticazione diretta. In questo modo si riduce il carico su Key Vault, aumenta l'affidabilità e si semplifica il codice perché Key Vault ora può usare il token Azure AD.  Molti servizi sono passati a utilizzando l'autenticazione Azure AD.  Vedere l'elenco corrente in [servizi che supportano identità gestite per le risorse di Azure](../../active-directory/managed-identities-azure-resources/services-support-managed-identities.md#azure-services-that-support-managed-identities-for-azure-resources).
