---
title: Novità di Azure Key Vault | Microsoft Docs
...
ms.locfileid: "90069357"
---
# <a name="whats-new-for-azure-key-vault"></a>Novità di Azure Key Vault

> [!WARNING]
> **Luglio 2020**: sono disponibili due aggiornamenti per Key Vault che possono influire sulle implementazioni del servizio, ovvero l'[eliminazione temporanea dell'insieme di credenziali delle chiavi attivata per impostazione predefinita](#soft-delete-on-by-default) e le [modifiche ai certificati TLS di Azure](#azure-tls-certificate-changes). Vedere di seguito per altri dettagli.
...
- La transizione dei servizi di Azure Active Directory (Azure AD) inizia il 7 luglio 2020.
...
### <a name="will-this-affect-me"></a>Chi sarà interessato da questa modifica?
...
- Per sistemi operativi e runtime di linguaggi diversi che comunicano con i servizi di Azure può essere necessario eseguire passaggi aggiuntivi per creare correttamente la catena di certificati con queste nuove radici: 
    - **Linux**: per molte distribuzioni è necessario aggiungere le CA elencate sopra in /etc/ssl/certs. Per istruzioni specifiche, fare riferimento alla documentazione della distribuzione.
    - **Java**: verificare che l'archivio chiavi Java contenga le CA elencate sopra.
    - **Windows in esecuzione in ambienti disconnessi**: per i sistemi in esecuzione in ambienti disconnessi sarà necessario aggiungere le radici elencate sopra all'archivio di autorità di certificazione radice attendibili e le CA intermedie all'archivio di autorità di certificazione intermedie.
