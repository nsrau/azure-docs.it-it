...
description: Informazioni su come configurare un contenitore Python predefinito per l'app. Questo articolo illustra le attività di configurazione più comuni.
ms.topic: quickstart
ms.date: 03/28/2019
...
Il motore di distribuzione del servizio app attiva un ambiente virtuale ed esegue automaticamente `pip install -r requirements.txt` quando si distribuisce un [repository Git](deploy-local-git.md) o un [pacchetto Zip](deploy-zip.md) con processi di compilazione attivati.
...
1. Eseguire `pip install -r requirements.txt`.
1. Se nella radice del repository viene trovato *manage.py*, esegue *manage.py collectstatic*. Se però `DISABLE_COLLECTSTATIC` è impostato su `true`, questo passaggio viene ignorato.
1. Esegue lo script personalizzato se specificato da `POST_BUILD_SCRIPT_PATH`.

`PRE_BUILD_COMMAND`, `POST_BUILD_COMMAND` e `DISABLE_COLLECTSTATIC` sono variabili di ambiente vuote per impostazione predefinita. Per eseguire comandi pre-compilazione, definire `PRE_BUILD_COMMAND`. Per eseguire comandi post-compilazione, definire `POST_BUILD_COMMAND`. Per disabilitare l'esecuzione di collectstatic durante la compilazione di app Django, impostare `DISABLE_COLLECTSTATIC=true`.
...
Per altre informazioni sull'esecuzione del servizio app e sulla compilazione di app Python in Linux, vedere la [documentazione di Oryx relativa al rilevamento e alla compilazione di app Python](https://github.com/microsoft/Oryx/blob/master/doc/runtimes/python.md).
...
Ad esempio, se si ha un'app Flask il cui modulo principale è *hello.py* e l'oggetto app Flask in tale file è denominato `myapp`, *\<custom-command>* sarà simile al seguente:
...
È anche possibile aggiungere altri argomenti per Gunicorn a *\<custom-command>* , ad esempio `--workers=4`. Per altre informazioni, vedere l'articolo relativo all'[esecuzione di Gunicorn](https://docs.gunicorn.org/en/stable/run.html) (docs.gunicorn.org).

Per un server non Gunicorn, ad esempio [aiohttp](https://aiohttp.readthedocs.io/en/stable/web_quickstart.html), è possibile sostituire *\<custom-command>* con un comando simile al seguente:
...
Nel servizio app è possibile [configurare le impostazioni dell'app](configure-common.md#configure-app-settings) al di fuori del codice dell'app. Quindi è possibile accedervi usando il modello standard [os.environ](https://docs.python.org/3/library/os.html#os.environ). Ad esempio, per accedere a un'impostazione dell'app denominata `WEBSITE_SITE_NAME`, usare il codice seguente:
