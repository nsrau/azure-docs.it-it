<properties
   pageTitle="Panoramica della cache locale del servizio app di Azure | Microsoft Azure"
...
* Aggiungere l'impostazione dell'app _permanente_ `WEBSITE_LOCAL_CACHE_OPTION` con il valore `Always` nello slot di **produzione**. Se si usa `WEBSITE_LOCAL_CACHE_SIZEINMB`, aggiungerla anch'essa come impostazione permanente nello slot di produzione.
...
### Come è possibile stabilire se la cache locale si applica all'app Web?
...
### Sono state appena pubblicate nuove modifiche, ma sembra che non siano presenti nell'app Web. Perché?
...
### È stata abilitata la cache locale ma l'app Web viene ancora riavviata. Perché? Pensavo che la cache locale fosse utile in caso di riavvii frequenti dell'app.
