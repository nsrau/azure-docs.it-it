...
description: Informazioni su come clonare l'app di Servizio app in una nuova app tramite PowerShell. Sono descritti diversi scenari di clonazione, inclusa l'integrazione di Gestione traffico.
...
La clonazione delle app è supportata per i piani di servizio delle app Standard, Premium, Premium V2 e Isolated. La nuova funzionalità usa le stesse limitazioni della funzionalità di backup di Servizio app. Vedere [Eseguire il backup di un'app in Servizio app di Azure](manage-backup.md).

## <a name="cloning-an-existing-app"></a>Clonazione di un'app esistente
Scenario: un'app esistente nell'area Stati Uniti centro-meridionali e si desidera clonare il contenuto in una nuova app nell'area Stati Uniti centro-settentrionali. Questa operazione può essere eseguita con la versione Azure Resource Manager del cmdlet PowerShell per creare una nuova app con l'opzione `-SourceWebApp`.
...
Usando `New-AzWebApp` il comando, è possibile creare la nuova app nell'area Stati Uniti centro-settentrionali e lerigli a un piano di servizio app esistente. Inoltre, è possibile usare lo stesso gruppo di risorse dell'app di origine oppure definire un nuovo gruppo di risorse, come illustrato nel comando seguente:
...
Scenario: un'app esistente nell'area Stati Uniti centro-meridionali e si desidera clonare il contenuto in una nuova app in un ambiente del servizio app (ASE) esistente.
