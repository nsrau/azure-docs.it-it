...
Anche se è già stato specificato il nome dell'immagine del contenitore, è comunque necessario specificare l'URL del registro personalizzato e le credenziali utente. In Cloud Shell eseguire il comando [az webapp config container set](/cli/azure/webapp/config/container?view=azure-cli-latest#az-webapp-config-container-set).
