...
Gli ambienti del servizio app sono una piattaforma ideale per l'aumento del numero di istanze.  Una volta selezionata una configurazione di Ambiente del servizio app che possa supportare una frequenza di richieste nota, gli sviluppatori possono distribuire altre istanze di ambienti del servizio app usando modelli di progetto (o "cookiecutter") per ottenere una capacità di carico di picco desiderata.
...
Il modo più semplice per registrare più endpoint del servizio app di Azure in esecuzione nella **stessa** area di Azure consiste nell'usare il [supporto di Azure Resource Manager per Gestione traffico][ARMTrafficManager] con Powershell.  
