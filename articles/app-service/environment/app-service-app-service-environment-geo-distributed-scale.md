---
title: Scala geografica distribuita
...
Gli ambienti del servizio app sono una piattaforma ideale per la scalabilità orizzontale.  Dopo aver selezionato una configurazione dell'ambiente del servizio app in grado di supportare una frequenza di richieste nota, gli sviluppatori possono distribuire ambienti del servizio app aggiuntivi in modalità "cookie cutter" per raggiungere la capacità di carico di picco desiderata.
...
* **Dominio personalizzato per l'app:**  Qual è il nome di dominio personalizzato che i clienti utilizzeranno per accedere all'app?  Per l'app di esempio il nome di dominio personalizzato è`www.scalableasedemo.com`
...
* **Convenzione di denominazione per gli ambienti del servizio app:Naming convention for the App Service Environments:**  Ogni ambiente del servizio app richiede un nome univoco.  Oltre a uno o due ambienti del servizio app, è utile avere una convenzione di denominazione per facilitare l'identificazione di ogni ambiente del servizio app.  Per l'app di esempio è stata usata una convenzione di denominazione semplice.  I nomi dei tre ambienti del servizio app sono *fe1ase*, *fe2ase* e *fe3ase*.
* **Convenzione di denominazione per le app:** poiché saranno distribuite più istanze dell'app, è necessario un nome per ogni istanza dell'app distribuita.  Una funzionalità poco conosciuta ma molto pratica degli ambienti del servizio app è la possibilità di usare lo stesso nome tra più ambienti.  Poiché ogni ambiente del servizio app ha un suffisso univoco, gli sviluppatori possono scegliere di riutilizzare esattamente lo stesso nome dell'app in ogni ambiente.  Ad esempio, uno sviluppatore potrebbe avere app denominate come segue: *myapp.foo1.p.azurewebsites.net*, *myapp.foo2.p.azurewebsites.net*, *myapp.foo3.p.azurewebsites.net,* ecc.  Per l'app di esempio anche se ogni istanza dell'app ha anche un nome univoco.  I nomi delle istanze dell'app usati sono *webfrontend1*, *webfrontend2* e *webfrontend3*.
...
* **webfrontend1.fe1ase.p.azurewebsites.net:**  Un'istanza dell'app di esempio distribuita nel primo ambiente del servizio app.
* **webfrontend2.fe2ase.p.azurewebsites.net:**  Un'istanza dell'app di esempio distribuita nel secondo ambiente del servizio app.
* **webfrontend3.fe3ase.p.azurewebsites.net:**  Un'istanza dell'app di esempio distribuita nel terzo ambiente del servizio app.
...
Come passaggio finale è necessario che il dominio personalizzato dell'app punti al dominio di Gestione traffico.  Per l'app di `www.scalableasedemo.com` esempio `scalable-ase-demo.trafficmanager.net`ciò significa puntare a .  Questo passaggio deve essere completato con il registrar che gestisce il dominio personalizzato.  
...
In questo esempio il `www.scalableasedemo.com`dominio personalizzato è e a ogni istanza dell'applicazione è associato il dominio personalizzato.
...
Il risultato finale di Gestione traffico e `www.scalableasedemo.com` la configurazione DNS è che le richieste per scorreranno attraverso la sequenza seguente:
