...
description: Informazioni su come implementare un'architettura di sicurezza a più livelli nell'ambiente del servizio app. Questo documento è rivolto solo ai clienti che usano l'ambiente del servizio app v1 legacy.
author: stefsch
...
Una volta noto il set di indirizzi IP in uscita, il passaggio successivo consiste nel creare un gruppo di sicurezza di rete.  I gruppi di sicurezza di rete possono essere creati sia per le reti virtuali basate su Resource Manager che per le reti virtuali classiche.  Gli esempi seguenti illustrano la creazione e la configurazione di un NSG in una rete virtuale classica tramite PowerShell.
