---
title: Evento di creazione del pool batch di AzureAzure Batch pool create event
description: Riferimento per l'evento Create del pool Batch, che viene generato dopo la creazione di un pool. Il contenuto del registro espone informazioni generali sul pool.
services: batch
author: LauraBrenner
manager: evansma
ms.assetid: ''
...
# <a name="pool-create-event"></a>Evento di creazione di pool
 ...
 Nell'esempio seguente viene illustrato il corpo di un `CloudServiceConfiguration` evento di creazione del pool per un pool creato utilizzando la proprietà .
...
|`imageType`|string|Metodo di distribuzione per l'immagine. I valori `virtualMachineConfiguration` supportati sono o`cloudServiceConfiguration`|
...
###  <a name="cloudserviceconfiguration"></a><a name="bk_csconf"></a>cloudServiceConfiguration (configurazione di servizio cloudService)
...
###  <a name="imagereference"></a><a name="bk_imgref"></a>imageReference (riferimento immagine)
...
###  <a name="windowsconfiguration"></a><a name="bk_winconf"></a>windowsConfigurazione
...
###  <a name="networkconfiguration"></a><a name="bk_netconf"></a>networkConfigurazione
