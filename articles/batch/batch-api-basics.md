...
Si useranno molte delle risorse e delle funzionalità illustrate di seguito, sia per sviluppare un'applicazione o un servizio di calcolo distribuito che rilascia chiamate API [Batch REST][batch_rest_api] sia quando si usa uno degli [SDK di Batch](batch-technical-overview.md#batch-development-apis).

> [AZURE.TIP] Per un'introduzione più generale al servizio Batch, vedere [Nozioni di base su Azure Batch](batch-technical-overview.md).
...
2. Caricare i **file dell'applicazione** che verranno eseguiti dalle attività. Questi file possono essere binari o script con le relative dipendenze e vengono eseguiti dalle attività nei processi. Questi file possono essere recuperati dall'account di archiviazione e scaricati nei nodi di calcolo dalle attività oppure è possibile usare la funzionalità [Pacchetti dell'applicazione](#application-packages) di Batch per la gestione e la distribuzione di applicazioni.
...
4. Creare un [processo](#job). Un processo gestisce una raccolta di attività e ogni processo viene associato a un pool specifico in cui verranno eseguite le attività del processo.
...
Alcune delle risorse seguenti (account, nodo di calcolo, pool, processo, attività) sono richieste da tutte le soluzioni che usano il servizio Batch. Altre risorse, ad esempio le pianificazioni di processi e i pacchetti dell'applicazione, sono funzionalità utili ma facoltative.
...
- [Accesso remoto](#connecting-to-compute-nodes) sia ai nodi Windows (RDP) che ai nodi Linux (SSH).
...
- **Sistema operativo** e **versione** dei nodi di calcolo
	...
	Sono disponibili due opzioni quando si seleziona un sistema operativo per i nodi nel pool: **Configurazione macchina virtuale** e **Services Configuration** (Configurazione servizi cloud).
...
- **Dimensioni dei nodi**
	...
	È necessario tenere in considerazione le caratteristiche e i requisiti dell'applicazione o delle applicazioni che saranno eseguite nei nodi di calcolo, quando si selezionano le dimensioni di un nodo. Le dimensioni del nodo verranno in genere selezionate presupponendo che nel nodo venga eseguita un'attività alla volta. Per determinare la dimensioni del nodo più appropriate e convenienti, considerare vari aspetti, ad esempio se si tratta di un'applicazione multithreading e la quantità di memoria che utilizza. È possibile che più attività e quindi più istanze dell'applicazione vengano [eseguite in parallelo](batch-parallel-node-tasks.md) e in questo caso si sceglierà in genere un nodo più grande. Per altre informazioni, vedere di seguito "Criteri di pianificazione delle attività".
...
- **Numero di nodi di destinazione**
	...
	È il numero di nodi di calcolo che si vuole distribuire nel pool. Viene detto *di destinazione* perché, in alcune situazioni, il pool potrebbe non raggiungere il numero desiderato di nodi. Tra i motivi per cui un pool non raggiunge il numero desiderato di nodi, c'è il raggiungimento della [quota core](batch-quota-limit.md#batch-account-quotas) per l'account Batch o una formula di scalabilità automatica applicata al pool, che limita il numero massimo di nodi. Vedere più avanti *Criteri di ridimensionamento*.

- **Criteri di ridimensionamento**

	Oltre a specificare un numero statico di nodi, è invece possibile scrivere e applicare una [formula di scalabilità automatica](#scaling-compute-resources) a un pool. Il servizio Batch valuterà periodicamente la formula e rettificherà il numero di nodi nel pool in base a vari parametri relativi a pool, processi e attività specificati.

- **Criteri di pianificazione di attività**

	L'opzione di configurazione [Numero massimo attività per nodo](batch-parallel-node-tasks.md) determina il numero massimo di attività che è possibile eseguire in parallelo in ogni nodo di calcolo del pool. La configurazione predefinita prevede l'esecuzione di una sola attività alla volta in un nodo, ma in alcuni scenari risulta utile eseguire più di un'attività contemporaneamente in un nodo. Vedere lo [scenario di esempio](batch-parallel-node-tasks.md#example-scenario) nell'articolo [Ottimizzare l'utilizzo delle risorse di calcolo di Azure Batch con attività dei nodi simultanee](batch-parallel-node-tasks.md) per informazioni su come sfruttare al meglio più attività per nodo.
...
- **Stato delle comunicazioni** dei nodi di calcolo
	...
	Nella maggior parte degli scenari, il funzionamento delle attività è indipendente e non è necessario che comunichino reciprocamente, mentre alcune applicazioni potrebbero richiedere la comunicazione tra le attività, ad esempio negli [scenari MPI](batch-mpi.md).
	...
	Si noti che l'abilitazione della comunicazione tra nodi ha effetto anche sul posizionamento dei nodi nei cluster e, a causa delle restrizioni di distribuzione, può limitare il numero massimo di nodi in un pool. Se l'applicazione non richiede la comunicazione tra nodi, il servizio Batch può allocare al pool un numero potenzialmente elevato di nodi da diversi cluster e data center per consentire una maggiore capacità di elaborazione parallela.

- **Attività di avvio** per i nodi di calcolo

	L'*attività di avvio* facoltativa verrà eseguita in ogni nodo durante l'aggiunta al pool e a ogni riavvio o ricreazione dell'immagine del nodo. L'attività di avvio è utile soprattutto per preparare i nodi di calcolo per l'esecuzione di attività, ad esempio l'installazione delle applicazioni che verranno eseguite dalle attività.
...
Un processo è una raccolta di attività e gestisce la modalità di esecuzione dei calcoli da parte delle attività nei nodi di calcolo di un pool.
...
- **Vincoli** in base ai quali viene eseguita l'attività. Ad esempio, l'intervallo di tempo massimo in cui l'attività può essere eseguita, il numero massimo di ripetizioni dei tentativi di un'attività non riuscita e l'intervallo di tempo massimo in cui i file vengono mantenuti nella directory di lavoro.
...
#### Attività del gestore di processi
...
- **Attività**: viene creata una directory per ogni attività in esecuzione sul nodo, accessibile facendo riferimento alla variabile di ambiente `AZ_BATCH_TASK_DIR`.
...
Un approccio combinato, usato in genere per gestire un carico variabile, ma continuativo, consiste nell'avere un pool al quale vengono inviati più processi, ma aumentando o riducendo il numero di nodi in base al carico di processi. Vedere [Ridimensionamento delle risorse di calcolo](#scaling-compute-resources) di seguito. Questa operazione può essere eseguita in modo reattivo in base al carico corrente o in modo proattivo se è possibile prevedere il carico.
...
- **Errori delle applicazioni**
	...
	Anche il processo specificato dalla riga di comando dell'attività può non riuscire. Il processo viene considerato non riuscito quando il processo eseguito dall'attività restituisce un codice di uscita diverso da zero. Vedere più avanti *Codici di uscita delle attività*.
...
- **Codici di uscita delle attività**
	...
	Come indicato in precedenza, un'attività viene contrassegnata come non riuscita dal servizio Batch se il processo eseguito dall'attività restituisce un codice di uscita non zero. Quando un'attività esegue un processo, Batch popola la proprietà del codice di uscita dell'attività con il *codice restituito del processo*. È importante notare che il codice di uscita di un'attività **non** è determinato dal servizio Batch, ma dal processo stesso o dal sistema operativo in cui il processo è stato eseguito.
...
Nei casi in cui alcune attività non riescono, il servizio o l'applicazione client Batch può esaminare i metadati delle attività non riuscite per identificare un nodo non correttamente funzionante. A ogni nodo di un pool viene assegnato un ID univoco e il nodo in cui viene eseguita un'attività viene incluso nei metadati dell'attività. Una volta identificato un "nodo problematico", è possibile intervenire in diversi modi:
