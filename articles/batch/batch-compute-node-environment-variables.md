---
title: Variabili di ambiente di runtime dell'attività - Batch di Azure Documenti Microsoft
description: Guida alle variabili di ambiente di runtime delle attività e riferimento per Analisi batch di Azure.Task runtime environment variable guidance and reference for Azure Batch Analytics.
services: batch
author: LauraBrenner
manager: evansma
ms.assetid: ''
...
ms.locfileid: "80053941"
---
# <a name="azure-batch-runtime-environment-variables"></a>Variabili di ambiente di runtime di Azure BatchAzure Batch runtime environment variables
...
Per ottenere il valore corrente di `cmd.exe` una variabile di `/bin/sh` ambiente, avviare in un nodo di calcolo di Windows o in un nodo Linux:To get the current value of an environment variable, launch on a Windows compute node or on a Linux node:
...
| AZ_BATCH_CERTIFICATES_DIR       | Una directory all'interno della [directory di lavoro dell'attività][files_dirs] in cui sono archiviati i certificati i nodi di calcolo Linux. Questa variabile di ambiente non si applica ai nodi di calcolo di Windows.This environment variable does not apply to Windows compute nodes.                                                  | Tutte le attività.   |  /mnt/batch/tasks/workitems/batchjob001/job-1/task001/certs |
...
| AZ_BATCH_NODE_MOUNTS_DIR        | Percorso completo del percorso di [montaggio](virtual-file-mount.md) del file system a livello di nodo in cui risiedono tutte le directory di montaggio. Le condivisioni file di Windows usano una lettera di unità, pertanto per Windows l'unità di montaggio fa parte di dispositivi e unità.  |  Tutte le attività, inclusa l'attività di avvio, hanno accesso all'utente, dato che l'utente è a conoscenza delle autorizzazioni di montaggio per la directory montata. | In Ubuntu, ad esempio, la posizione è:`/mnt/batch/tasks/fsmounts` |
