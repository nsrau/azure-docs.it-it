<properties 
	pageTitle="Introduzione a Ricerca di Azure | Azure" 
	description="Informazioni sui vantaggi della compilazione di applicazioni di ricerca avanzata con Ricerca di Azure." 
	editor="monicar" 
	manager="mblythe" 
	services="search" 
	documentationCenter="" 
	authors="HeidiSteen"/>

<tags 
	ms.service="multiple" 
	ms.workload="multiple" 
	ms.tgt_pltfrm="na" 
	ms.devlang="na" 
	ms.topic="article" 
	ms.date="03/12/2015" 
	ms.author="heidist"/>

# Introduzione a Ricerca di Azure #

**Riepilogo:** altre informazioni sulla Ricerca di Azure, un servizio di ricerca completamente gestito in Azure che supporta esperienze di ricerca avanzata in applicazioni Web e mobili. La ricerca è lo stile di interazione predefinito con i dati sul Web. Viene sempre più usata in tutte le situazioni relative alle applicazioni.

**Sommario:**

Questo articolo illustrerà quanto segue:

* Servizi offerti da Ricerca di Azure
* Introduzione all'uso di un indice
* Esecuzione di ricerche in Ricerca di Azure
* Prezzi e scalabilità

Questo articolo fornisce una panoramica della tecnologia e delle funzionalità e degli scenari supportati. Non è un approfondimento tecnico.

**Autore**: David Chappell, titolare di Chappell & Associates

**Ultimo aggiornamento**: marzo 2015

**Revisione:** 1.1

**Download:**

* <a href="http://go.microsoft.com/fwlink/p/?LinkId=522547" target="_blank">"Introduzione a Ricerca di Azure" (in formato PDF)</a>
* <a href="http://go.microsoft.com/fwlink/p/?LinkId=529011" target="_blank">"Introduzione a Ricerca di Azure" (in Microsoft Word)</a> <!--HONumber=52-->
