...
Prima di iniziare questa esercitazione, [rivedere](migrate-architecture.md) l'architettura della migrazione basata su agente di VMware.
...
Se non si ha già un progetto Azure Migrate, [configurarlo](how-to-add-tool-first-time.md) ora e aggiungere lo strumento Migrazione del server.

Se si ha già un progetto, seguire queste istruzioni per aggiungere lo strumento:
...
12. In **Calcolo** controllare il nome, le dimensioni, il tipo di disco del sistema operativo e il set di disponibilità delle VM. Le VM devono essere conformi ai [requisiti di Azure](migrate-support-matrix-vmware-migration.md#azure-vm-requirements).
