...
Questo articolo riepiloga i prerequisiti e i requisiti di supporto quando si valutano le VM VMware per la migrazione ad Azure, usando lo strumento Azure Migrate: server Assessment] (migrate-Services-Overview. MD # Azure-migrate-server-Assessment-Tool). Per eseguire la migrazione di macchine virtuali VMware in Azure, esaminare la [matrice di supporto](migrate-support-matrix-vmware-migration.md)per la migrazione.
...
- In Azure per enti pubblici è necessario distribuire l'appliance con lo script.
...
**Account di Windows** |  Per l'analisi delle dipendenze, l'appliance Azure Migrate necessita di un account di amministratore locale o di dominio per accedere alle macchine virtuali Windows.
