...
Se non si utilizza Eclipse, è inoltre consigliabile consultare l'argomento [Creazione di un'applicazione Hello World per Azure in Eclipse](http://msdn.microsoft.com/it-it/library/windowsazure/hh690944) o acquisire familiarità con altre tecniche per l'hosting di applicazioni Java in Azure.
...
-   È possibile utilizzare `RoleEnvironment.getConfigurationSettings` per recuperare il nome utente e la password di SendGrid dalle impostazioni di configurazione della distribuzione, anziché utilizzare il modulo Web per recuperare questi valori. Per informazioni sulla classe `RoleEnvironment` vedere [Utilizzo della libreria di runtime del servizio Azure in JSP](http://msdn.microsoft.com/it-it/library/windowsazure/hh690948) e la documentazione sul pacchetto della libreria di runtime del servizio Azure all'indirizzo &lt;http://dl.windowsazure.com/javadoc\>.
