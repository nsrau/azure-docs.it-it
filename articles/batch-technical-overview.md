...
2. Fare clic su **Nuovo**, **Calcolo**, **Servizio Batch**, e quindi fare clic su **Creazione rapida**.
![Create a Batch account][account_portal]

3. Immettere le seguenti informazioni:

	* In **Nome account** immettere un nome univoco sa usare nell'URL dell'account Batch.
...
Un **elemento di lavoro** è un modello che specifica la modalità di esecuzione di un'applicazione sule TVM di un pool. Un **processo** è un'istanza pianificata di un elemento di lavoro e potrebbe verificarsi una sola volta o ripetersi. Un processo è costituito da una raccolta di **attività**. La figura 3 illustra le relazioni di base.    
...
* Se si integra un'applicazione client usando le API e gli strumenti delle app di Batch, è possibile usare componenti che suddividono automaticamente un processo in attività, elaborano le attività e uniscono i risultati delle singole attività ai risultati del processo finale. Quando si invia il carico di lavoro al servizio Batch, il framework delle app di Batch gestisce i processi ed esegue le attività sulle risorse di calcolo sottostanti. 
...
Ancora una volta, a seconda delle API usate con Batch, sarà necessario gestire un numero variabile di dettagli sugli elementi di lavoro, sui processi e sulle attività.. Se si sviluppa con le API Batch di livello inferiore, è necessario specificare queste dipendenze e gli spostamenti dei file in modo esplicito. Con le app di Batch, il framework gestisce la maggior parte di questi dettagli automaticamente. 
...
1.	Caricare i file di input (ad esempio i dati o le immagini di origine) necessari per un processo in un account di archiviazione di Azure. I file devono trovarsi nell'account di archiviazione in modo che il servizio Batch possa accedervi. Il servizio Batch li carica in una TVM quando l'attività viene eseguita.
...
3.	Creare un pool of TVM, specificando la dimensione delle TVM nel pool, il sistema operativo eseguito e altre proprietà. Quando un'attività viene eseguita, a essa viene assegnata una TVM da questo pool.
