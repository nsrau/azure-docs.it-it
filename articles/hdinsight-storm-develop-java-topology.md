<properties
   pageTitle="Sviluppo di topologie basate su Java per Apache Storm in HDInsight | Azure"
...
#Sviluppo di topologie basate su Java per Apache Storm in HDInsight
...
Dalla riga di comando usare il seguente codice per creare un nuovo progetto Maven denominato **WordCount**.
...
La directory **WordCount** conterrà i seguenti elementi:
...
Poiché l'applicazione sarà completamente nuova, eliminare i file dell'applicazione e di test generati:
...
> [AZURE.NOTE] Si noti la riga `<scope>provided</scope>` nella sezione aggiunta. Tale riga indica a Maven di escludere storm-core da qualsiasi file jar creato, poiché storm-core verrà fornito dal sistema. Questo assicura che i pacchetti creati saranno di dimensioni più ridotte e che useranno i bit storm-core inclusi nel cluster Storm in HDInsight.
...
Per ridurre i requisiti relativi all'impostazione di origini dati esterne, il seguente spout genera semplicemente frasi casuali. Si tratta di una versione modificata di uno spout fornito con gli esempi di Storm-Starter (<a href="https://github.com/apache/storm/blob/master/examples/storm-starter/" target="_blank">https://github.com/apache/storm/blob/master/examples/storm-starter/</a>).
...
> [AZURE.NOTE] I bolt eseguono qualsiasi tipo di attività, inclusi calcolo, persistenza, comunicazione con componenti esterni e così via.
...
Si consiglia di leggere con attenzione i commenti del codice per comprendere il funzionamento di ciascun bolt.
...
Dopo aver salvato i file, usare il seguente comando per testare la topologia in locale.
