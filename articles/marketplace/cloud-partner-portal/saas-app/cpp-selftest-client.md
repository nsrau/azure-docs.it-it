---
title: Client di verifica automatica per la preconvalida di un'applicazione SaaS di Azure | Microsoft Docs
description: Come creare un client di verifica automatica per la preconvalida di un'applicazione SaaS di Azure.
...
---
# <a name="create-a-self-test-client-to-pre-validate-an-azure-saas-application"></a>Creare un client di verifica automatica per la convalida preliminare di un'applicazione SaaS di Azure
...
L'API ha la struttura seguente.
...
|          Utente          |          Nome utente per la effettuare la connessione e l'accesso alla macchina virtuale.         |
|           Password         |         Password per effettuare la connessione e l'accesso alla macchina virtuale.          |
|          OS          |         Sistema operativo della macchina virtuale, "Linux" o "Windows".          |
|         PortNo           |         Numero della porta aperta per la connessione alla macchina virtuale. Il numero di porta è in genere "22" per "5986" per Windows e Linux.          |
...
La schermata seguente mostra un esempio di risultati del test JSON in un visualizzatore JSON online,
...
3. Nella barra di spostamento a sinistra selezionare **Tutti i servizi** e quindi selezionare **Azure Active Directory**.
   ...
   Nei passaggi seguenti potrebbe essere necessario il nome del tenant (o della directory) oppure l'ID del tenant (o della directory).
...
3. In **Crea** inserire le informazioni necessarie per i campi seguenti:
   ...
   - **URL di accesso**: digitare "https://isvapp.azurewebsites.net/selftest"
...
Dopo aver testato correttamente l'applicazione SaaS di Azure, è possibile [Pubblicare l'offerta](./cpp-publish-offer.md)