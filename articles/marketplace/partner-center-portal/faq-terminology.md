...
| Nome istanza cloud: globale di Azure| JSON| Microsoft cloud pubblico globale.|| |
| Nome istanza cloud: Azure per enti pubblici | JSON| Cloud Microsoft specifici per enti pubblici per uno dei seguenti enti pubblici: Cina, Germania o il Stati Uniti di America.| |
...
| Marketplace License Type: Bring Your Own License | JSON| La macchina virtuale richiede un codice di licenza fornito dal cliente per la distribuzione. Microsoft non fattura i clienti per elencare le loro offerte in questo modo tramite il Marketplace.||
| Marketplace License Type: Free| JSON| L'offerta è configurata per essere gratuita a tutti gli utenti. Microsoft non addebita ai clienti l'utilizzo di questa offerta.||
...
| Nome offerta  | JSON| Nome dell'offerta del Marketplace.|| |
...
| Tipo di offerta: Dynamics 365 business Central| JSON| Usare questo tipo di offerta quando la soluzione è integrata con Dynamics 365 per Finanza e operazioni| |
| Tipo di offerta: Dynamics 365 per Customer Engagement | JSON| Usare questo tipo di offerta quando la soluzione è integrata con Dynamics 365 per il coinvolgimento dei clienti.||
| Tipo di offerta: modulo IoT Edge | JSON| Azure IoT Edge moduli sono le unità di calcolo più piccole gestite da IoT Edge e possono contenere servizi Microsoft, ad esempio analisi di flusso di Azure, servizi di terze parti o codice specifico della soluzione. |
| Tipo di offerta: applicazione Power BI | JSON| Usare il tipo di offerta di applicazione Power BI quando si distribuisce un'applicazione integrata con Power BI.|  |
...
| Tipo di offerta: estensione Visual Studio Marketplace  | JSON| Tipo di offerta in precedenza disponibile per gli sviluppatori di estensioni DevOps di Azure. In futuro gli sviluppatori di estensioni DevOps di Azure possono vendere la propria estensione direttamente ai clienti. Le offerte di estensione possono essere configurate come a pagamento o inclusa una versione di valutazione. |
| Order Cancel Date| JSON| Data in cui l'ordine del Marketplace è stato eliminato.||
| ID dell'ordine| JSON| Identificatore univoco dell'ordine del cliente per il servizio Marketplace. Le offerte basate sull'utilizzo delle macchine virtuali non sono associate a un ordine.| |
