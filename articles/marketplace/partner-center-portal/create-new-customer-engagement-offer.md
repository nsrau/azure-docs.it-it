---
title: Creare una nuova offerta di PowerApps Dynamics 365 for Customer Engagement & nel Marketplace commerciale
description: Come creare un nuovo'offerta di PowerApps Dynamics 365 for Customer Engagement & per l'inserzione o la vendita in Azure Marketplace, AppSource o tramite il programma Cloud Solution Provider (CSP) tramite il portale Commercial Marketplace nel Centro per i partner Microsoft.
author: dsindona
ms.author: dsindona
...
ms.locfileid: "81869817"
---
# <a name="create-a-new-dynamics-365-for-customer-engagement--powerapps-offer"></a>Creare una nuova offerta di PowerApps Dynamics 365 for Customer Engagement &

In questo argomento viene illustrato come creare una nuova offerta di Dynamics 365 for Customer Engagement & PowerApps. Tutte le app per Dynamics 365 for Customer Engagement (PowerApps, Sales, Service, Project Service e Field Service) devono eseguire il processo di certificazione e supportare un'esperienza di valutazione. Il processo di certificazione verifica che la soluzione soddisfi i requisiti standard, la compatibilità e l'efficienza nelle procedure consigliate. mentre il percorso di valutazione consente agli utenti di distribuire la soluzione in un ambiente Dynamics 365 reale.

Per iniziare a creare Dynamics 365 for Customer Engagement & le offerte PowerApps, assicurati di [creare prima un account del Centro per i partner](./create-account.md) e aprire il dashboard Marketplace [commerciale](https://partner.microsoft.com/dashboard/commercial-marketplace/offers)con la scheda **Panoramica** selezionata.
...
Seleziona il pulsante **Nuova offerta,** quindi seleziona la voce di menu **Dynamics 365 for Customer Engagement & PowerApps.** Verrà visualizzata la finestra di dialogo **Nuova offerta.**

### <a name="offer-id-and-alias"></a>ID e alias dell'offerta

- **ID offerta**: Identificatore univoco per ogni offerta nel tuo account. Questo ID sarà visibile ai clienti nell'indirizzo URL dell'offerta del marketplace e nei modelli di Azure Resource Manager (se applicabile). L'ID offerta deve essere carattere alfanumerico minuscolo (inclusi trattini e caratteri di sottolineatura, ma senza spazi vuoti). L'ID è limitato a 50 caratteri e non può essere modificato dopo aver selezionato **Crea**. Ad esempio, se si immette *test-offer-1* in `https://azuremarketplace.microsoft.com/marketplace/../test-offer-1`questo campo, l'URL dell'offerta sarà .
...
Selezionare un minimo di una e un massimo di tre categorie. La **Categoria** verrà utilizzata per inserire l'offerta nelle aree di ricerca del marketplace appropriate. Assicurati di sapere come la tua offerta supporta queste categorie nella descrizione dell'offerta.
...
Seleziona tutti i prodotti Dynamics 365 a cui si applica questa offerta.

### <a name="app-version"></a>Versione dell'app

Inserisci il numero di versione dell'offerta. I clienti vedranno questa versione elencata nella pagina dei dettagli dell'offerta. Se stai aggiornando il numero di versione solo a causa di modifiche di marketing/descrittive, seleziona la casella **Solo marketing.** Questa opzione consente all'offerta di ignorare le fasi di certificazione e provisioning.
...
Fornisci una breve descrizione dell'offerta (fino a 100 caratteri). Questa descrizione può essere utilizzata nei risultati di ricerca del marketplace.
...
- Descrivere chiaramente la proposta di valore dell'offerta subito nelle prime frasi della descrizione. Includi le seguenti informazioni nella tua proposta di valore:
...
È necessario fornire almeno uno (e fino a tre) documenti di marketing correlati qui, come white paper, brochure, elenchi di controllo o presentazioni. Questi documenti devono essere in formato .pdf.
...
È possibile fornire il logo dell'offerta in tre dimensioni: **Piccolo (48 x 48)**, **Grande (216 x 216)** e **Ampio (255 x 115)**. Le taglie Piccole e Grandi sono obbligatorie.
...
Aggiungi screenshot che mostrano come funziona la tua offerta. È necessaria almeno una schermata ed è possibile sommarne fino a cinque. Tutte le schermate devono essere 1280 x 720 pixel.

#### <a name="videos"></a>Video

Facoltativamente, puoi aggiungere fino a quattro video che dimostrano la tua offerta. Questi video devono essere ospitati su YouTube e/o Vimeo. Per ognuno di essi, inserisci il nome del video, il suo URL e un'immagine di anteprima del video (1280 x 720 pixel)
...
Le tue selezioni qui si applicano solo alle nuove acquisizioni; se qualcuno ha già la tua app in un certo mercato e in seguito rimuovi quel mercato, le persone che hanno già l'offerta in quel mercato possono continuare a usarla, ma nessun nuovo cliente in quel mercato sarà in grado di ottenere la tua offerta.

> [!IMPORTANT]
> È tua responsabilità soddisfare eventuali requisiti legali locali, anche se tali requisiti non sono elencati qui o nel Centro per i partner. Anche se selezioni tutti i mercati, le leggi e le restrizioni locali o altri fattori potrebbero impedire che determinate offerte vengano elencate in alcuni paesi e aree geografiche.
...
Il modello di licenza di base determina la modalità di assegnazione dell'applicazione da parte dei clienti nell'interfaccia di amministrazione di CRM. Selezionare **Risorsa** per licenze basate sull'istanza o **Utente** se le licenze ne sono assegnate una per tenant.

### <a name="requires-s2s-outbound-and-crm-secure-store-access"></a>Richiede l'accesso all'archiviazione sicura S2S in uscita e CRM

Selezionare questa casella per abilitare la configurazione dell'accesso in uscita tra l'archiviazione sicura CRM o l'accesso in uscita da server a server (S2S). Questa funzionalità richiede una considerazione speciale da parte del Team di Dynamics 365 durante la fase di certificazione. Microsoft contatterà l'utente per completare alcuni passaggi aggiuntivi per supportare questa funzionalità.

### <a name="application-configuration-url"></a>URL di configurazione dell'applicazione

Specificare l'URL della pagina Web di configurazione che consente al cliente di configurare l'app.

### <a name="crm-package"></a>Pacchetto CRM

Nel campo **URL del percorso** del pacchetto immettere l'URL di un account di Archiviazione di Azure contenente il file .zip del pacchetto CRM caricato. Questo URL deve includere una chiave di sola accesso condiviso di sola lettura per consentire a Microsoft di prelevare il pacchetto per la verifica.

Seleziona la casella **C'è più di un pacchetto CRM nel mio file**del pacchetto , se applicabile. In tal caso, assicurati di includere tutti i pacchetti nel file .zip.

### <a name="crm-package-availability"></a>Disponibilità del pacchetto CRM

In questa sezione, selezionare **Aggiungi area** per specificare le aree geografiche in cui il pacchetto CRM sarà disponibile per i clienti. La distribuzione nelle seguenti regioni sovrane richiede un'autorizzazione e una convalida speciali durante il processo di certificazione: [Germania,](https://docs.microsoft.com/azure/germany/) [US Government Cloud](https://docs.microsoft.com/azure/azure-government/documentation-government-welcome)e TIP.

Per impostazione predefinita, per ogni area verrà utilizzato **l'URL di configurazione dell'applicazione** immesso in precedenza. Se si preferisce, è possibile immettere un URL di configurazione dell'applicazione separato per una o più aree specifiche. 
...
- **Nome ruolo** (obbligatorio): specificare il nome del ruolo di sicurezza definito nel test drive Dynamics 365 personalizzato. Questo nome del ruolo di sicurezza verrà assegnato all'utente durante il test drive, ad esempio test-drive-role.
...
Per distribuire test Drive per conto dell'utente, creare e fornire una sottoscrizione di Azure separata e univoca (non necessaria per i test drive di Power BI).
...
È necessario caricare un file PDF che elenca gli scenari di utilizzo chiave dell'offerta elencati in un documento (formato .pdf). Tutti gli scenari elencati di seguito possono essere verificati dal nostro team di convalida prima di approvare l'offerta per il marketplace.
