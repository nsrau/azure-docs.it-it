<properties
	pageTitle="Introduzione all'archiviazione BLOB di Azure e ai relativi servizi di Visual Studio"
...
# Introduzione all'archiviazione BLOB di Azure e ai relativi servizi di Visual Studio
...
Per elencare i BLOB in un contenitore, ottenere prima un riferimento al contenitore. Sarà quindi possibile chiamare il metodo `ListBlobsSegmentedAsync` del contenitore per recuperare i BLOB e/o le directory in esso contenuti. Per accedere all'insieme avanzato di proprietà e metodi per un `IListBlobItem` restituito, è necessario eseguirne il cast in un oggetto `CloudBlockBlob`, `CloudPageBlob`, o `CloudBlobDirectory`. Se il tipo del BLOB non è noto, è possibile usare un controllo del tipo per determinare quello in cui viene eseguito il cast. Nel codice seguente viene illustrato come recuperare e visualizzare l'URI di ogni elemento in un contenitore.
