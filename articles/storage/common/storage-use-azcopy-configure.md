...
Per visualizzare le indicazioni dettagliate della Guida per questo comando `azcopy bench -h` , digitare e quindi premere il tasto INVIO.

### <a name="optimize-throughput"></a>Ottimizzare la velocità effettiva

È possibile usare il `cap-mbps` flag nei comandi per posizionare un limite massimo sulla velocità di dati della velocità effettiva. Il comando seguente, ad esempio, riprende un processo e la velocità effettiva `10` delle maiuscole in megabyte (MB) al secondo. 
...
Per impostazione predefinita, i file di log e di piano si `%USERPROFILE%\.azcopy` trovano nella directory di `$HOME$\.azcopy` Windows o nella directory in Mac e Linux, ma è possibile modificare tale percorso se lo si desidera.

L'errore pertinente non è necessariamente il primo errore visualizzato nel file. Per gli errori, ad esempio errori di rete, timeout ed errori di server occupato, AzCopy tenterà un tentativo fino a 20 volte e in genere il processo di ripetizione dei tentativi riesce.  Il primo errore visualizzato potrebbe essere qualcosa di innocuo che è stato eseguito un nuovo tentativo.  Quindi, invece di esaminare il primo errore nel file, cercare gli errori vicini `UPLOADFAILED`a, `COPYFAILED`o. `DOWNLOADFAILED` 
...
Per impostazione predefinita, i file di piano e di log `%USERPROFILE%\.azcopy` si trovano nella directory di Windows o `$HOME$\.azcopy` nella directory in Mac e Linux. È possibile modificare questo percorso.
...
I livelli di log disponibili `NONE`sono `DEBUG`: `INFO`, `WARNING`, `ERROR`, `PANIC`,, `FATAL`e.

## <a name="remove-plan-and-log-files"></a>Rimuovere i file di piano e di log

Se si desidera rimuovere tutti i piani e i file di log dal computer locale per risparmiare spazio su disco, `azcopy jobs clean` utilizzare il comando.

Per rimuovere il piano e i file di log associati a un solo processo `azcopy jobs rm <job-id>`, utilizzare. Sostituire il `<job-id>` segnaposto in questo esempio con l'ID processo del processo.
