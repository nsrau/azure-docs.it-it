...
La funzionalità Crittografia del servizio di archiviazione esegue la crittografia dei dati durante la scrittura in Archiviazione di Azure e può essere usata per Archiviazione BLOB, file, tabelle e code di Azure. È adatta alle operazioni seguenti:

* Archiviazione standard: account di archiviazione di uso generico per i BLOB, account di archiviazione file, tabelle e code e account di archiviazione BLOB
* Archiviazione Premium 
* Abilitata per impostazione predefinita per gli account di archiviazione classici e di Azure Resource Manager 
...
La funzionalità Crittografia del servizio di archiviazione è abilitata a livello di account di archiviazione. Per impostazione predefinita, è abilitata per tutti i servizi. Supporta gli scenari cliente seguenti:

* Crittografia per l'archiviazione BLOB, file, tabelle e code per gli account classici e di Resource Manager.
...
### <a name="step-2-verify-encryption"></a>Passaggio 2: Verificare la crittografia.
È possibile verificare la crittografia usando il [portale di Azure](https://portal.azure.com).

> [!NOTE]
> Se si vuole verificare a livello di codice la funzionalità Crittografia del servizio di archiviazione in un account di archiviazione, è possibile usare l'[API REST del provider di risorse di Archiviazione di Azure](https://msdn.microsoft.com/library/azure/mt163683.aspx), la [libreria client dei provider delle risorse di archiviazione per .NET](https://msdn.microsoft.com/library/azure/mt131037.aspx), [Azure PowerShell](/powershell/azureps-cmdlets-docs) o l'[interfaccia della riga di comando di Azure](../storage-azure-cli.md).
> 
> 

### <a name="step-3-copy-data-to-storage-account"></a>Passaggio 3: Copiare i dati in un account di archiviazione
Dopo che la funzionalità Crittografia del servizio di archiviazione è stata abilitata per un account di archiviazione, i dati scritti in tale account di archiviazione verranno crittografati. Eventuali dati già presenti nell'account di archiviazione non verranno crittografati finché non saranno riscritti. È possibile copiare i dati da un contenitore a un altro per assicurarsi che i dati precedenti vengano crittografati. Per farlo, è possibile usare uno qualsiasi dei seguenti strumenti. La stessa operazione è applicabile ad Archiviazione file, tabelle e code.
...
* La crittografia è abilitata per un account di archiviazione.
...
R: Con la crittografia abilitata per impostazione predefinita, i nuovi dati vengono crittografati automaticamente dal servizio di archiviazione. 

È anche possibile creare un nuovo account di archiviazione di Resource Manager e copiare tutti i dati usando [AzCopy](storage-use-azcopy.md) dall'account di archiviazione classico esistente all'account di archiviazione di Resource Manager appena creato. 

È anche possibile scegliere di eseguire la migrazione dell'account di archiviazione classico a un account di archiviazione di Resource Manager. Questa operazione è istantanea, modifica il tipo di account ma non influisce sui dati esistenti. Eventuali nuovi dati verranno crittografati solo dopo l'attivazione della crittografia. Per altre informazioni, vedere [Platform Supported Migration of IaaS Resources from Classic to Resource Manager](https://azure.microsoft.com/blog/iaas-migration-classic-resource-manager/)(Migrazione supportata dalla piattaforma di risorse IaaS dal modello classico al modello di Azure Resource Manager). Si noti che questa operazione è supportata solo per i servizi BLOB e file.
...
R: La funzionalità Crittografia del servizio di archiviazione è abilitata per impostazione predefinita su tutti gli account di archiviazione di Resource Manager esistenti. Questa funzionalità è supportata per Archiviazione BLOB, tabelle, file e code. 

**D: È possibile crittografare i dati correnti in un account di archiviazione di Resource Manager esistente?**

R: Con la funzionalità Crittografia del servizio di archiviazione abilitata per impostazione predefinita per tutti gli account di archiviazione (classici e di Resource Manager). Tuttavia, i dati che erano già presenti non verranno crittografati. Per crittografare i dati esistenti, è possibile copiarli in un altro nome o in un altro contenitore e quindi rimuoverne le versioni non crittografate. 
...
**D: Se si crea un nuovo account di archiviazione con la funzionalità Crittografia del servizio di archiviazione abilitata e quindi si crea una nuova VM usando tale account di archiviazione, la VM sarà crittografata?**
...
R: La funzionalità Crittografia del servizio di archiviazione è abilitata per impostazione predefinita al momento della creazione di qualsiasi account di archiviazione (classico e di Resource Manager). È possibile verificare le proprietà dell'account con Azure PowerShell e l'interfaccia della riga di comando di Azure.
...
R: Sì, la funzionalità Crittografia del servizio di archiviazione con l'uso delle chiavi gestite da Microsoft è abilitata per impostazione predefinita per tutti gli account di archiviazione (classici e di Azure Resource Manager). È abilitata anche per tutti i servizi (Archiviazione BLOB, tabelle, code e file).
...
**D: Cosa succede se la funzionalità Crittografia del servizio di archiviazione è abilitata e quindi si abilita Crittografia dischi di Azure sui dischi?**

R: Non si verifica alcun problema. I dati verranno crittografati da entrambi i metodi.

**D: L'account di archiviazione è configurato per la replica con ridondanza geografica. Se la funzionalità Crittografia del sistema di archiviazione è abilitata, verrà crittografata anche la copia ridondante?**
...
**D: Non è possibile disabilitare la crittografia sull'account di archiviazione.**
