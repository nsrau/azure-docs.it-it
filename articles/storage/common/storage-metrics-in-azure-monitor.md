...
Monitoraggio di Azure offre [.NET SDK](https://www.nuget.org/packages/Microsoft.Azure.Management.Monitor/) per leggere la definizione e i valori delle metriche. Il [codice di esempio](https://azure.microsoft.com/resources/samples/monitor-dotnet-metrics-api/) illustra come usare l'SDK con parametri diversi. È necessario usare `0.18.0-preview` o versione successiva per le metriche di archiviazione. L'ID risorsa viene utilizzato in .NET SDK. Per altre informazioni, leggere Understanding resource ID for services in Storage (Informazioni sull'ID risorsa per i servizi in Archiviazione).

L'esempio seguente mostra come usare Azure Monitor .NET SDK per leggere le metriche di archiviazione.

#### <a name="list-account-level-metric-definition-with-the-net-sdk"></a>Elencare la definizione della metrica a livello di account con .NET SDKList account level metric definition with the .NET SDK
...
#### <a name="read-metric-values-with-the-net-sdk"></a>Leggere i valori delle metriche con .NET SDKRead metric values with the .NET SDK
...
#### <a name="read-multi-dimensional-metric-values-with-the-net-sdk"></a>Leggere valori metrici multidimensionali con .NET SDKRead multi-dimensional metric values with the .NET SDK
...
| Nome misurazione | Descrizione |
| ------------------- | ----------------- |
| BlobCapacity | Totale di risorse di archiviazione BLOB usato nell'account di archiviazione. <br/><br/> Unità: byte <br/> Tipo di aggregazione: media <br/> Esempio di valore: 1024 <br/> Dimensioni: **BlobType**e **BlobTier** ([Definition](#metrics-dimensions)) |
| BlobCount    | Numero di oggetti BLOB archiviati nell'account di archiviazione. <br/><br/> Unità: conteggio <br/> Tipo di aggregazione: media <br/> Esempio di valore: 1024 <br/> Dimensioni: **BlobType**e **BlobTier** ([Definition](#metrics-dimensions)) |
...
| Egress | Quantità di dati in uscita. Questo numero include l'uscita da un client esterno da Archiviazione di Azure e l'uscita all'interno di Azure.This number includes egress to an external client from Azure Storage as well as gress within Azure. Questo numero non rispecchia quindi dati in uscita fatturabili. <br/><br/> Unità: byte <br/> Tipo di aggregazione: totale <br/> Dimensioni applicabili: GeoType, ApiName e Authentication ([definizione](#metrics-dimensions)) <br/> Esempio di valore: 1024 |
...
| ------------------- | ----------------- |
| **BlobType (Tipo di BLOB)** | Tipo di BLOB solo per le metriche relative ai BLOB. I valori supportati sono **BlockBlob**, **PageBlob**e **Azure Data Lake Storage**. Il BLOB di aggiunta è incluso in BlockBlob. |
| **BlobTier** | Archiviazione di Azure offre diversi livelli di accesso, che consentono di archiviare i dati degli oggetti BLOB nel modo più conveniente. Vedere altre informazioni nel [livello BLOB di Archiviazione di Azure.See](../blobs/storage-blob-storage-tiers.md)more in Azure Storage blob tier . I valori supportati includono: <br/> <li>**Caldo**: Livello caldo</li> <li>**Cool**: Livello Cool</li> <li>**Archivio**: Livello di archiviazione</li> <li>**Premium:** livello Premium per il BLOB in blocchiPremium : Premium tier for block blob</li> <li>**P4/P6/P10/P15/P20/P40/P50/P60**: Tipi di livello per blob di pagine premium</li> <li>**Standard**: Tipo di livello per la pagina standard Blob</li> <li>**Untiered**: tipo di livello per l'account di archiviazione v1 genericoUntiered : Tier type for general purpose v1 storage account</li> |
...
| **ResponseType (Tipo di risposta)** | Tipo di risposta della transazione. I valori disponibili includono: <br/><br/> <li>**ServerOtherError:** tutti gli altri errori sul lato server ad eccezione di quelli descritti </li> <li>**ServerBusyError**: richiesta autenticata che ha restituito un codice di stato HTTP 503.ServerBusyError : Authenticated request that returned an HTTP 503 status code. </li> <li>**ServerTimeoutError:** richiesta autenticata senza timeout che ha restituito un codice di stato HTTP 500.ServerTimeoutError : Timed-out authenticated request that returned an HTTP 500 status code. Il timeout si è verificato a causa di un errore del server. </li> <li>**AuthorizationError:** richiesta autenticata non riuscita a causa di accesso non autorizzato ai dati o di un errore di autorizzazione. </li> <li>**NetworkError:** richiesta autenticata non riuscita a causa di errori di rete. Questo errore si verifica in genere quando un cliente chiude prematuramente una connessione prima della scadenza del timeout. </li>  <li>**ClientAccountBandwidthThrottlingError:** la richiesta viene limitata sulla larghezza di banda per il superamento dei limiti di [scalabilità dell'account di archiviazione.](scalability-targets-standard-account.md)</li><li>**ClientAccountRequestThrottlingError:** la richiesta viene limitata alla frequenza delle richieste per il superamento dei limiti di [scalabilità dell'account](scalability-targets-standard-account.md)di archiviazione.<li>**ClientThrottlingError:** altro errore di limitazione lato client. ClientAccountBandwidthThrottlingError e ClientAccountRequestThrottlingError sono esclusi.</li> <li>**ClientTimeoutError:** richiesta autenticata timeout che ha restituito un codice di stato HTTP 500.ClientTimeoutError : Timed-out authenticated request that returned an HTTP 500 status code. Se il timeout di rete o il timeout della richiesta del client è impostato su un valore inferiore rispetto a quanto previsto dal servizio di archiviazione, si tratta di un timeout previsto. In caso contrario, viene segnalato come ServerTimeoutError.</li> </li> <li>**ClientOtherError:** tutti gli altri errori sul lato client ad eccezione di quelli descritti. </li> <li>**Operazione riuscita**: Richiesta riuscita</li> <li> **SuccessWithThrottling:** richiesta riuscita quando un client SMB viene limitato nei primi tentativi ma ha esito positivo dopo i tentativi.</li> |
| **NomeApiName** | Nome dell'operazione. Ad esempio: <br/> <li>**CreateContainer**</li> <li>**DeleteBlob**</li> <li>**GetBlob**</li> Per i nomi di tutte le operazioni, vedere questo [documento](/rest/api/storageservices/storage-analytics-logged-operations-and-status-messages). |
| **Autenticazione** | Tipo Authentication usato nelle transazioni. I valori disponibili includono: <br/> <li>**AccountKey:** la transazione viene autenticata con la chiave dell'account di archiviazione.</li> <li>**SAS**: la transazione viene autenticata con firme di accesso condiviso.</li> <li>**OAuth**: La transazione viene autenticata con token di accesso OAuth.</li> <li>**Anonimo:** la transazione viene richiesta in modo anonimo. Non include le richieste di verifica preliminare.</li> <li>**AnonymousPreflight:** la transazione è una richiesta di verifica preliminare.</li> |
