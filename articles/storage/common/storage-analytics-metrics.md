---
title: Metriche di analisi di Archiviazione di Azure (classica)Azure Storage analytics metrics (Classic)
description: Informazioni su come usare le metriche in Archiviazione di Azure.Learn how to use metrics in Azure Storage.
author: normesta
ms.service: storage
ms.topic: conceptual
ms.date: 03/11/2019
...
---
# <a name="azure-storage-analytics-metrics-classic"></a>Metriche di analisi di Archiviazione di Azure (classica)Azure Storage analytics metrics (Classic)
...
## <a name="transaction-metrics"></a>Metriche di transazione  
 ...
 Se, ad esempio, si esegue un'operazione **GetBlob** nel servizio BLOB, la metrica Analisi archiviazione registrerà la richiesta e la includerà nei dati aggregati sia per il servizio BLOB, sia per l'operazione **GetBlob**. Tuttavia, se non viene richiesta alcuna operazione **GetBlob** durante l'ora, un'entità non verrà scritta *nel $MetricsTransactionsBlob* per tale operazione.  
...
- **ObjectCount:** numero di BLOB di blocchi o pagine di cui è stato eseguito il commit e di cui non è stato eseguito il commit nel servizio BLOB dell'account di archiviazione.  
...
|Capacità (solo servizio BLOB)|$MetricsCapacityBlob|tutte le versioni.|  
 ...
 Queste tabelle vengono create automaticamente quando Analisi archiviazione è abilitato per un endpoint del servizio di archiviazione. È possibile accedervi tramite lo spazio `https://<accountname>.table.core.windows.net/Tables("$MetricsTransactionsBlob")`dei nomi dell'account di archiviazione, ad esempio: . Le tabelle delle metriche non vengono visualizzate in un'operazione di presentazione e devono essere accessibili direttamente tramite il nome della tabella.  

## <a name="enable-metrics-using-the-azure-portal"></a>Abilitare le metriche usando il portale di AzureEnable metrics using the Azure portal
Seguire questi passaggi per abilitare le metriche nel [portale di Azure](https://portal.azure.com):

1. Passare all'account di archiviazione.
1. Selezionare **Impostazioni di diagnostica (classica)** dal riquadro Menu.Select Diagnostics settings (classic) from the **Menu** pane.
...
## <a name="enable-storage-metrics-using-powershell"></a>Abilitare le metriche di archiviazione tramite PowerShellEnable Storage metrics using PowerShell  
...
* **ServiceType**, il valore possibile sono **Blob**, **Queue**, **Table**e **File**.
* **MetricsType**, i valori possibili sono **Hour** e **Minute**.  
* **MetricsLevel**, i valori possibili sono:
* **Nessuno**: Disattiva il monitoraggio.
* **Servizio:** raccoglie metriche quali ingresso in ingresso/uscita, disponibilità, latenza e percentuali di successo, aggregate per i servizi BLOB, coda, tabella e file.
* **ServiceAndApi:** oltre alle metriche del servizio, raccoglie lo stesso set di metriche per ogni operazione di archiviazione nell'API del servizio di archiviazione di Azure.ServiceAndApi : In addition to the Service metrics, collects the same set of metrics for each storage operation in the Azure Storage service API.

Ad esempio, il comando seguente attiva le metriche dei minuti per il servizio BLOB nell'account di archiviazione con il periodo di conservazione impostato su cinque giorni:For example, the following command switches on minute metrics for the blob service in your storage account with the retention period set to five days: 

> [!NOTE]
> Questo comando presuppone che sia stato eseguito l'accesso alla sottoscrizione di Azure usando il `Connect-AzAccount` comando.
...
## <a name="enable-storage-metrics-programmatically"></a>Abilitare le metriche di archiviazione a livello di codiceEnable Storage metrics programmatically  
Oltre a usare il portale di Azure o i cmdlet di Azure PowerShell per controllare le metriche di archiviazione, è anche possibile usare una delle API di Archiviazione di Azure.In addition to using the Azure portal or the Azure PowerShell cmdlets to control Storage Metrics, you can also use one of the Azure Storage APIs. Se, ad esempio, si usa un linguaggio .NET, è possibile usare la libreria client di archiviazione.  
...
Per ulteriori informazioni sull'utilizzo di un linguaggio .NET per configurare le metriche di archiviazione, vedere [Libreria client di archiviazione per .NET.](https://msdn.microsoft.com/library/azure/mt347887.aspx)  

Per informazioni generali sulla configurazione delle metriche di archiviazione tramite l'API REST, vedere [Abilitazione e configurazione di Analisi archiviazione](/rest/api/storageservices/Enabling-and-Configuring-Storage-Analytics).  
...
1. Selezionare **Metriche (classica)** nel pannello **Menu** per il servizio di cui si desidera visualizzare le metriche.
...
Nella sezione **Monitoraggio (classica)** del pannello di menu dell'account di archiviazione nel portale di Azure è possibile configurare le regole di [avviso,](#metrics-alerts)ad esempio l'invio di avvisi tramite posta elettronica per ricevere una notifica quando una metrica specifica raggiunge un determinato valore.

Se si desidera scaricare le metriche per l'archiviazione a lungo termine o per analizzarle in locale, è necessario utilizzare uno strumento o scrivere codice per leggere le tabelle. È necessario scaricare le metriche al minuto per l'analisi. Se si elencano tutte le tabelle nell'account di archiviazione, le tabelle non vengono visualizzate, ma è possibile accedervi direttamente mediante il nome. Molti strumenti di esplorazione dell'archiviazione sono a conoscenza di queste tabelle e consentono di visualizzarle direttamente (vedere [Strumenti client di Archiviazione](/azure/storage/storage-explorers) di Azure per un elenco degli strumenti disponibili).

||||  
|-|-|-|  
|**Metriche**|**Nomi delle tabelle**|**Note**|  
|Metriche orarie|$MetricsHourPrimaryTransactionsBlob<br /><br /> $MetricsHourPrimaryTransactionsTable<br /><br /> $MetricsHourPrimaryTransactionsQueue<br /><br /> $MetricsHourPrimaryTransactionsFile|Nelle versioni precedenti al 15 agosto 2013, queste tabelle sono note come:<br /><br /> $MetricsTransactionsBlob <br /><br /> $MetricsTransactionsTable<br /><br />  $MetricsTransactionsQueue<br /><br /> Le metriche per il servizio File sono disponibili a partire dalla versione 2015-04-05.Metrics for the File service are available beginning with version 2015-04-05.|  
...
-   Il tipo di accesso è **user** o **system**, dove **user** è riferito a tutte le richieste utente al servizio di archiviazione, mentre **system** è riferito alle richieste eseguite da Analisi archiviazione.  
...
## <a name="billing-on-storage-metrics"></a>Fatturazione sulle metriche di archiviazione  
...
Anche le richieste di lettura ed eliminazione dei dati delle metriche da parte di un client sono fatturabili a tariffe standard. Se è stato configurato un criterio di memorizzazione dati, non è necessario sostenere alcun addebito quando Archiviazione di Azure elimina i vecchi dati di metrica. Tuttavia, se si eliminano dati di analisi, all'account vengono addebitate le operazioni di eliminazione.  

Anche la capacità utilizzata dalle tabelle delle metriche è fatturabile. È possibile utilizzare quanto segue per stimare la quantità di capacità utilizzata per l'archiviazione dei dati delle metriche:  

-   Se ogni ora in cui un servizio utilizza ogni API in ogni servizio, circa 148 KB di dati vengono archiviati ogni ora nelle tabelle delle transazioni delle metriche se è stato abilitato il riepilogo a livello di servizio e di API.  
-   Se entro ogni ora, un servizio utilizza ogni API nel servizio, circa 12 KB di dati vengono archiviati ogni ora nelle tabelle delle transazioni delle metriche se è stato abilitato solo il riepilogo del livello di servizio.  
-   La tabella capacity per i BLOB include due righe aggiunte ogni giorno, a condizione che sia stato scelto per i log. Ciò implica che ogni giorno, la dimensione di questa tabella aumenta fino a circa 300 byte.
