...
:::row:::
    :::column:::
        [![Intervista e demo introduttiva Sincronizzazione file di Azure-fare clic per riprodurre.](./media/storage-sync-files-planning/azure-file-sync-interview-video-snapshot.png)](https://www.youtube.com/watch?v=nfWLO7F52-s)
    :::column-end:::
    :::column:::
        Sincronizzazione file di Azure è un servizio che consente di memorizzare nella cache un numero di condivisioni file di Azure in una macchina virtuale locale di Windows Server o cloud. 
        
        Questo articolo presenta Sincronizzazione file di Azure concetti e funzionalità. Una volta acquisita familiarità con Sincronizzazione file di Azure, è consigliabile seguire la [Guida alla distribuzione di sincronizzazione file di Azure](storage-sync-files-deployment-guide.md) per provare questo servizio.        
    :::column-end:::
:::row-end:::

I file verranno archiviati nel cloud in [condivisioni file di Azure](storage-files-introduction.md). Le condivisioni file di Azure possono essere usate in due modi: montando direttamente le condivisioni file di Azure senza server (SMB) o memorizzando nella cache le condivisioni file di Azure in locale usando Sincronizzazione file di Azure. L'opzione di distribuzione scelta consente di modificare gli aspetti che è necessario tenere in considerazione durante la pianificazione della distribuzione. 
...
\*Al momento non è consigliabile sincronizzare più di 100 milioni file & directory. Si tratta di un limite flessibile in base alle soglie testate. Per ulteriori informazioni, vedere [obiettivi di scalabilità e prestazioni file di Azure](storage-files-scale-targets.md#azure-file-sync-scale-targets).
...
> [!NOTE]
> Sincronizzazione file di Azure servizio eliminerà il supporto per TLS 1.0 e 1,1 nel 2020 agosto. Per impostazione predefinita, tutte le versioni dell'agente Sincronizzazione file di Azure supportate usano già TLS 1.2. L'uso di una versione precedente di TLS potrebbe verificarsi se TLS 1.2 è stato disabilitato nel server o viene usato un proxy. Se si usa un proxy, è consigliabile controllare la configurazione del proxy. Sincronizzazione file di Azure aree del servizio aggiunte dopo il 5/1/2020 supporteranno solo TLS 1.2 e il supporto per TLS 1.0 e 1,1 verrà rimosso dalle aree esistenti nel 2020 agosto.  Per ulteriori informazioni, vedere la [Guida alla risoluzione dei problemi](storage-sync-files-troubleshoot.md#tls-12-required-for-azure-file-sync).
...
|-------------|-------------------|--------------|-------------|
| Public | Asia | Asia orientale | `eastasia` |
| Public | Asia | Asia sud-orientale | `southeastasia` |
| Public | Australia | Australia orientale | `australiaeast` |
| Public | Australia | Australia sud-orientale | `australiasoutheast` |
| Public | Brasile | Brasile meridionale | `brazilsouth` |
| Public | Canada | Canada centrale | `canadacentral` |
| Public | Canada | Canada orientale | `canadaeast` |
| Public | Europa | Europa settentrionale | `northeurope` |
| Public | Europa | Europa occidentale | `westeurope` |
| Public | Francia | Francia centrale | `francecentral` |
| Public | Francia | Francia meridionale * | `francesouth` |
| Public | India | India centrale | `centralindia` |
| Public | India | India meridionale | `southindia` |
| Public | Giappone | Giappone orientale | `japaneast` |
| Public | Giappone | Giappone occidentale | `japanwest` |
| Public | Corea del Sud | Corea centrale | `koreacentral` |
| Public | Corea del Sud | Corea meridionale | `koreasouth` |
| Public | Sud Africa | Sudafrica settentrionale | `southafricanorth` |
| Public | Sud Africa | Sudafrica occidentale * | `southafricawest` |
| Public | Emirati Arabi Uniti | Emirati Arabi Uniti centrali * | `uaecentral` |
| Public | Emirati Arabi Uniti | Emirati Arabi Uniti settentrionali | `uaenorth` |
| Public | Regno Unito | Regno Unito meridionale | `uksouth` |
| Public | Regno Unito | Regno Unito occidentale | `ukwest` |
| Public | Stati Uniti | Stati Uniti centrali | `centralus` |
| Public | Stati Uniti | Stati Uniti orientali | `eastus` |
| Public | Stati Uniti | Stati Uniti orientali 2 | `eastus2` |
| Public | Stati Uniti | Stati Uniti centro-settentrionali | `northcentralus` |
| Public | Stati Uniti | Stati Uniti centro-meridionali | `southcentralus` |
| Public | Stati Uniti | Stati Uniti centro-occidentali | `westcentralus` |
| Public | Stati Uniti | Stati Uniti occidentali | `westus` |
| Public | Stati Uniti | Stati Uniti occidentali 2 | `westus2` |
