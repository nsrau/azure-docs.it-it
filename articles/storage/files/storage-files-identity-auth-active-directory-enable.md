...
> 
> L'autenticazione di Active Directory non supporta l'autenticazione per gli account computer creati in Active Directory. 
> 
...
- Aggiungere un dominio a un computer locale o una macchina virtuale di Azure ad ad (noto anche come AD DS). 

    Per accedere a una condivisione file usando le credenziali di Active Directory da un computer o una macchina virtuale, il dispositivo deve essere aggiunto a un dominio ad. Per informazioni su come aggiungere un dominio ad Active Directory, vedere [aggiungere un computer a un dominio](https://docs.microsoft.com/windows-server/identity/ad-fs/deployment/join-a-computer-to-a-domain). 
...
### <a name="2-execute-ad-enablement-script"></a>2. eseguire lo script di abilitazione di AD
...
La descrizione seguente riepiloga tutte le azioni eseguite quando viene eseguito il `join-AzStorageAccountForAuth` cmdlet. È possibile eseguire questi passaggi manualmente, se si preferisce non usare il comando:

> [!NOTE]
> Se lo script join-AzStorageAccountForAuth è già stato eseguito correttamente, passare alla sezione successiva "3. Verificare che la funzionalità sia abilitata ". Non è necessario eseguire di nuovo le operazioni seguenti.
...
Tenere il SID dell'account appena creato, sarà necessario per il passaggio successivo. L'identità di Active Directory appena creata che rappresenta l'account di archiviazione non deve essere sincronizzata con Azure AD.
...
### <a name="3-confirm-that-the-feature-is-enabled"></a>3. Verificare che la funzionalità sia abilitata
