...
Quando un utente apre un file a livelli, Sincronizzazione file di Azure richiama senza problemi i dati del file da File di Azure senza che l'utente debba sapere che il file è archiviato in Azure.When a user opens a tiered file, Azure File Sync seamlessly recalls the file data from Azure Files without the user needing that the file is stored in Azure. 
 
 > [!Important]  
 > La suddivisione in livelli cloud non è supportata nel volume di sistema di Windows.Cloud tiering is not supported on the Windows system volume.
    
 > [!Important]  
 > Per richiamare i file che sono stati a livelli, la larghezza di banda di rete deve essere di almeno 1 Mbps. Se la larghezza di banda di rete è inferiore a 1 Mbps, i file potrebbero non essere riprodotti con un errore di timeout.
...
### <a name="what-is-the-minimum-file-size-for-a-file-to-tier"></a>Qual è la dimensione minima del file da un file a livello?
Per le versioni 9.x e successive dell'agente, la dimensione minima del file da un file a livello è basata sulla dimensione del cluster del file system (raddoppiare la dimensione del cluster del file system). Ad esempio, se la dimensione del cluster del file system NTFS è 4 KB, la dimensione minima del file risultante per un file a livello è 8 KB. Per le versioni dell'agente 8.x e precedenti, la dimensione minima del file da un file a livello è 64 KB.
...
Esistono diversi modi per verificare se un file è archiviato a livelli in una condivisione file di Azure:
   ...
   * **Usare `fsutil` per verificare la presenza di reparse point in un file.**
       Come descritto per l'opzione precedente, per un file archiviato a più livelli è sempre impostato un reparse point. Un reparse point è un puntatore speciale per il filtro del file system di Sincronizzazione file di Azure (StorageSync.sys). Per verificare se un file contiene un reparse point, eseguire l'utilità `fsutil` in un prompt dei comandi con privilegi elevati o in una sessione di PowerShell:
        ...
        Se il file contiene un reparse point, dovrebbe essere visualizzato **Reparse Tag Value: 0x8000001e** (Valore tag di reparse: 0x8000001e). Questo valore esadecimale è il valore di reparse point di proprietà di Sincronizzazione file di Azure.This hexadecimal value is the reparse point value that is owned by Azure File Sync. L'output contiene anche i dati di tipo reparse che rappresenta il percorso del file nella condivisione file di Azure.The output also contains the reparse data that represents the path to your file on your Azure file share.
...
* `-Order CloudTieringPolicy`ricorderà prima i file modificati più di recente.  
* `-ThreadCount`determina quanti file possono essere richiamati in parallelo.
...
Esplora file di Windows espone due proprietà per rappresentare le dimensioni di un file: **Dimensioni** e **Dimensioni su disco**. Queste proprietà hanno un significato leggermente diverso. **Dimensioni** rappresenta le dimensioni complete del file. **Dimensioni su disco** rappresenta le dimensioni del flusso di file archiviato sul disco. I valori per queste proprietà possono essere diversi per diversi motivi, ad esempio la compressione, l'uso della deduplicazione dei dati o il tiering cloud con Sincronizzazione file di Azure.The values for these properties can differ for a variety reasons, such as compression, use of Data Deduplication, or cloud tiering with Azure File Sync. Se un file è suddiviso in livelli in una condivisione file di Azure, la dimensione sul disco è zero, perché il flusso di file viene archiviato nella condivisione file di Azure e non nel disco. È anche possibile che un file sia parzialmente a livelli (o parzialmente richiamato). In un file parzialmente a livelli, parte del file è su disco. Questa situazione può verificarsi quando i file vengono letti parzialmente da applicazioni come i lettori multimediali o le utilità di compressione. 
...
### <a name="why-are-my-tiered-files-not-showing-thumbnails-or-previews-in-windows-explorer"></a>Perché i file a livelli non mostrano miniature o anteprime in Esplora risorse?
Per i file a livelli, le anteprime e le anteprime non saranno visibili nell'endpoint server. Questo comportamento è previsto poiché la funzionalità di cache delle anteprime in Windows ignora intenzionalmente la lettura dei file con l'attributo offline. Con cloud Tiering abilitato, la lettura di file a livelli causerebbe il download (richiamato).

Questo comportamento non è specifico di Sincronizzazione file di Azure, in Esplora risorse viene visualizzata una "X grigia" per tutti i file per i cui attributi offline sono impostati. Vedrai l'icona X quando accedi ai file tramite SMB. Per una spiegazione dettagliata di questo comportamento, fare riferimento a[https://blogs.msdn.microsoft.com/oldnewthing/20170503-00/?p=96105](https://blogs.msdn.microsoft.com/oldnewthing/20170503-00/?p=96105)
