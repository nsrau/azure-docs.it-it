...
description: In questa guida introduttiva si crea un contenitore nell'archivio oggetti (BLOB), si carica un file, si elencano gli oggetti e si esegue il download con Java Storage SDK.
services: storage
author: roygara
manager: jeconnoc
ms.custom: mvc
...
---
# <a name="quickstart-upload-download-and-list-blobs-using-the-java-sdk-v10-preview"></a>Guida introduttiva: Caricare, scaricare ed elencare BLOB con Java SDK V10 (anteprima)

In questa guida introduttiva viene illustrato come usare il nuovo Java Storage SDK per caricare, scaricare ed elencare BLOB in blocchi in un contenitore di Archiviazione BLOB di Azure. Il nuovo Java SDK usa il modello di programmazione reattiva con RxJava offrendo operazioni asincrone. Per altre informazioni su RxJava, fare clic [qui](https://github.com/ReactiveX/RxJava). 

## <a name="prerequisites"></a>prerequisiti

Per completare questa guida introduttiva:

* Installare e configurare [Maven](http://maven.apache.org/download.cgi) per usarlo dalla riga di comando o l'ambiente IDE Java preferito
...
Questo esempio crea un file di test nella directory predefinita (AppData\Local\Temp per gli utenti di Windows), quindi chiede di immettere i comandi per caricare il file di test nell'archivio BLOB, elenca i BLOB nel contenitore e scarica il file caricato con un nuovo nome, così da poter confrontare il file nuovo e quello vecchio. 

Per eseguire l'esempio usando Maven dalla riga di comando, aprire una shell e passare a **storage-blobs-java-v10-quickstart** all'interno della directory clonata. Immettere quindi `mvn compile exec:java`.
...
Avendo il controllo dell'esempio, immettere i comandi per eseguire il codice. Tenere presente che gli input rispettano la distinzione tra maiuscole e minuscole.
...
Dopo aver verificato i file, premere **INVIO** per terminare la demo ed eliminare i file di test. Ora che si conosce il risultato dell'esempio, aprire il file **Quickstart.java** per esaminare il codice. 
...
* Creare un'istanza dell'oggetto StorageURL che punti all'account di archiviazione.

    L'oggetto [**StorageURL**](https://docs.microsoft.com/en-us/java/api/com.microsoft.azure.storage.blob._storage_u_r_l?view=azure-java-preview) è una rappresentazione dell'account di archiviazione e consente di generare una nuova pipeline. Una pipeline ([pipeline HTTP](https://github.com/Azure/azure-storage-java/wiki/Azure-Storage-Java-V10-Overview#url-types--http-pipeline)) è un set di criteri usati per modificare richieste e risposte con meccanismi di autorizzazione, registrazione e ripetizione dei tentativi. Con la pipeline è possibile creare un'istanza dell'oggetto [**ServiceURL**](https://docs.microsoft.com/en-us/java/api/com.microsoft.azure.storage.blob._service_u_r_l?view=azure-java-preview), che consente di creare un'istanza di [**ContainerURL**](https://docs.microsoft.com/en-us/java/api/com.microsoft.azure.storage.blob._container_u_r_l?view=azure-java-preview), necessaria per eseguire le operazioni sui contenitori BLOB.

* Creare un'istanza dell'oggetto ContainerURL che rappresenta il contenitore a cui si accede. I contenitori vengono usati per organizzare i BLOB, così come si usano le cartelle nel computer per organizzare i file.

    **ContainerURL** fornisce un punto di accesso al servizio contenitore. Con l'oggetto **ContainerURL** è possibile creare un'istanza dell'oggetto [**BlobURL**](https://docs.microsoft.com/en-us/java/api/com.microsoft.azure.storage.blob._container_u_r_l?view=azure-java-preview), necessaria per creare i BLOB.

* Creare un'istanza dell'oggetto [BlobURL](https://docs.microsoft.com/en-us/java/api/com.microsoft.azure.storage.blob._blob_u_r_l?view=azure-java-preview), che punti al BLOB specifico a cui si è interessati.
...
In questa sezione si creano un'istanza di ContainerURL e un nuovo contenitore. Il contenitore dell'esempio è denominato **quickstart**. 

Questo esempio usa [containerURL.create](https://docs.microsoft.com/en-us/java/api/com.microsoft.azure.storage.blob._container_u_r_l.create?view=azure-java-preview) perché si vuole creare un nuovo contenitore ogni volta che si esegue l'esempio. In alternativa, è possibile creare il contenitore in anticipo in modo che non sia necessario crearlo nel codice.
...
I BLOB in blocchi possono essere qualsiasi tipo di file di testo o binario. I BLOB di pagine vengono usati principalmente per i file VHD usati per tornare alle macchine virtuali IaaS. I BLOB di aggiunta servono per aggiungere dati alla fine e spesso vengono usati per la registrazione. La maggior parte degli oggetti presenti nell'archiviazione BLOB è costituita da BLOB in blocchi.

### <a name="list-the-blobs-in-a-container"></a>Elencare i BLOB in un contenitore

È possibile ottenere un elenco di oggetti in un contenitore usando [containerURL.listBlobsFlatSegment](https://docs.microsoft.com/en-us/java/api/com.microsoft.azure.storage.blob._container_u_r_l.listblobsflatsegment?view=azure-java-preview). Questo metodo restituisce fino a 5000 oggetti in una sola volta con un marcatore di continuazione (marcatore next) se nel contenitore sono presenti altri elementi da elencare. A questo scopo, viene creata una funzione helper che chiama se stessa ripetutamente finché il marcatore next è presente nella risposta listBlobsFlatSegment precedente.
...
In questa guida rapida è stato descritto il trasferimento di file tra il disco locale e Archiviazione BLOB di Azure con Java. 
