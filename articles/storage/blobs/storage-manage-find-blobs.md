...
Poiché i set di dati ottengono dimensioni maggiori e maggiori, la ricerca di un oggetto specifico in un mare di dati può risultare difficile e frustrante. L'indice BLOB offre funzionalità di gestione e individuazione dei dati usando attributi di tag di indice chiave-valore che consentono di categorizzare e trovare oggetti all'interno di un singolo contenitore o di tutti i contenitori nell'account di archiviazione. In un secondo momento, in base ai requisiti delle modifiche dei dati, gli oggetti possono essere ricategorizzati dinamicamente aggiornando i tag degli indici rimanendo sul posto con l'organizzazione del contenitore corrente. L'uso di un indice BLOB può semplificare lo sviluppo consolidando i dati BLOB e gli attributi degli indici associati nello stesso servizio. consente di creare applicazioni efficienti e scalabili usando le funzionalità native. 
...
## <a name="blob-index-tags-and-data-management"></a>Tag e Gestione dati di indice BLOB
...
Ai chiamanti che usano un' [identità AAD](../common/storage-auth-aad.md) possono essere concesse le autorizzazioni seguenti per operare sui tag degli indici BLOB. 
...
> [!IMPORTANT]
> Vedere la sezione condizioni di questo articolo. Per iscriversi all'anteprima, vedere la sezione registrare la sottoscrizione di questo articolo. Prima di poter usare l'indice BLOB negli account di archiviazione, è necessario registrare la sottoscrizione.
