---
title: Ripristino temporizzato per BLOB in blocchi (anteprima)
...
ms.locfileid: "83195222"
---
# <a name="point-in-time-restore-for-block-blobs-preview"></a>Ripristino temporizzato per BLOB in blocchi (anteprima)
...
Per avviare un ripristino temporizzato, chiamare l'operazione [Restore Ranges BLOB](/rest/api/storagerp/storageaccounts/restoreblobranges) e specificare un punto di ripristino in ora UTC. È possibile specificare un intervallo di lessicografico di nomi di contenitori e BLOB da ripristinare oppure omettere l'intervallo per ripristinare tutti i contenitori nell'account di archiviazione. L'operazione **Restore Ranges BLOB** restituisce un ID di ripristino che identifica in modo univoco l'operazione.
...
Quando si richiede un'operazione di ripristino, archiviazione di Azure blocca le operazioni sui dati nei BLOB nell'intervallo da ripristinare per la durata dell'operazione. Le operazioni di lettura, scrittura ed eliminazione sono bloccate nella posizione primaria. Quando l'account di archiviazione viene replicato geograficamente, è possibile che le operazioni di lettura dalla posizione secondaria continuino durante l'operazione di ripristino.
...
Per controllare lo stato di un ripristino temporizzato, chiamare l'operazione **Get Restore status** con l'ID di ripristino restituito dall'operazione **Restore Ranges BLOB** .
...
Abilitare queste funzionalità per l'account di archiviazione prima di abilitare il ripristino temporizzato. Assicurarsi di eseguire la registrazione per le anteprime del feed delle modifiche e del controllo delle versioni dei BLOB prima di abilitarle.
...
- Il ripristino dei BLOB nel livello archivio non è supportato. Se, ad esempio, un BLOB nel livello di accesso frequente è stato spostato nel livello di archiviazione due giorni fa e un'operazione di ripristino viene ripristinata a un punto di tre giorni fa, il BLOB non viene ripristinato nel livello critico.
...
Per eseguire la registrazione per l'anteprima, eseguire i comandi seguenti da Azure PowerShell:
...
Per verificare lo stato della registrazione, eseguire i comandi seguenti:
...
## <a name="ask-questions-or-provide-feedback"></a>Porre domande o fornire commenti e suggerimenti
...
- [Supporto del feed delle modifiche nell'archivio BLOB di Azure (anteprima)](storage-blob-change-feed.md)
