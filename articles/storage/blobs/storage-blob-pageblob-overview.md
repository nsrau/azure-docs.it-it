...
Per creare un BLOB di pagine, viene prima creato un oggetto CloudBlobClient, con l'URI di base per l'accesso all'archivio BLOB per l'account di archiviazione (pbaccount nella figura 1) con l'oggetto StorageCredentialsAccountAndKey, come illustrato di seguito.  L'esempio illustra quindi la creazione di un riferimento a un oggetto CloudBlobContainer e quindi la creazione del contenitore (testvhds) se non esiste già.  Usando l'oggetto CloudBlobContainer, è quindi possibile creare un riferimento a un oggetto CloudPageBlob specificando il nome del BLOB di pagine (os4.vhd) a cui si vuole accedere. Per creare il BLOB di pagine, viene quindi chiamato [CloudPageBlob.Create](/dotnet/api/microsoft.windowsazure.storage.blob.cloudpageblob.create?view=azure-dotnet#Microsoft_WindowsAzure_Storage_Blob_CloudPageBlob_Create_System_Int64_Microsoft_WindowsAzure_Storage_AccessCondition_Microsoft_WindowsAzure_Storage_Blob_BlobRequestOptions_Microsoft_WindowsAzure_Storage_OperationContext_) passando le dimensioni massime per il BLOB che si vuole creare.  BlobSize deve essere un multiplo di 512 byte.
...
L'obiettivo di Azure è di offrire un'esperienza ottimale ai clienti. La piattaforma di archiviazione è progettata per garantire la durabilità dei dati a livello aziendale in modo che i clienti possano affidare ad Azure i propri dati critici. Azure, a differenza delle altre piattaforme cloud pubbliche, offre il supporto esclusivo delle API e l'esperienza di sviluppo per i BLOB di pagine. Tutto ciò ha favorito numerose innovazioni di Microsoft e di terze parti, come le migrazioni cloud senza problemi, un'esperienza di backup/ripristino di emergenza superiore, il supporto PaaS e DBaaS, soluzioni di archiviazione distribuite e altre innovazioni per VM/dischi IaaS. In generale, grazie a queste funzionalità esclusive, Azure si distingue da tutte le piattaforme cloud pubbliche e i clienti di Azure ottengono vantaggi significativi da questi valori aggiunti che nessun'altra piattaforma cloud può offrire.
