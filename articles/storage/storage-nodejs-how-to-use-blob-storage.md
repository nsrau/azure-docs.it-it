...
Questa guida illustra diversi scenari d'uso comuni del servizio BLOB di Azure. Gli esempi sono scritti usando l'API Node.js. Gli scenari presentati includono **caricamento**, **visualizzazione dell'elenco**, **download** ed **eliminazione** di BLOB.
...
Il risultato contiene informazioni sull'operazione, incluse le informazioni **ETag** per il contenitore.
...
Nell'esempio di codice seguente viene caricato il contenuto del file **test.txt** nel BLOB **myblob**.
...
Nell'esempio di codice seguente viene caricato il contenuto del file **test.txt** nel BLOB **mypageblob**.
...
Per eliminare un BLOB, infine, chiamare **deleteBlob**. Nell'esempio di codice seguente viene eliminato il BLOB denominato **myblob**.
...
È possibile impostare le condizioni degli ETag usando il parametro `options.accessConditions` facoltativo. Nell'esempio di codice seguente il file **test.txt** viene caricato solo se il BLOB è già presente e include il valore di ETag contenuto in `etagToMatch`.
...
È possibile acquisire un nuovo lease usando il metodo **acquireLease** e specificando il BLOB o il contenitore per il quale si vuole ottenere il lease. Il codice seguente consente ad esempio di acquisire un lease sul BLOB **myblob**.
...
Nell'esempio di codice seguente viene generato un nuovo criterio di accesso condiviso che consente al titolare della firma di accesso condiviso di eseguire operazioni di lettura nel BLOB **myblob** e che scadrà 100 minuti dopo la data di creazione.
...
Un elenco di controllo di accesso viene implementato usando una matrice di criteri di accesso, con un ID associato a ogni criterio. Nell'esempio di codice seguente vengono definiti due criteri, uno per 'user1' e uno per 'user2':
...
Nell'esempio di codice seguente viene recuperato l'elenco di controllo di accesso corrente per **mycontainer** e vengono aggiunti i nuovi criteri tramite **setBlobAcl**. Risultato:
...
Dopo avere impostato l'elenco di controllo di accesso, è possibile creare una firma di accesso condiviso in base all'ID di un criterio. Nell'esempio di codice seguente viene creata una nuova firma di accesso condiviso per 'user2':
...
[Creare e distribuire un'app Web Node.js nel servizio app di Azure]: /develop/nodejs/tutorials/create-a-website-(mac)/
