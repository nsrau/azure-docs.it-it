...
![Azure Databricks con Data Lake Storage Gen2 e SQL Data Warehouse](./media/handle-data-using-databricks/databricks-extract-transform-load-sql-datawarehouse.png "Azure Databricks con Data Lake Storage Gen2 e SQL Data Warehouse")
...
1. Nel portale di Azure selezionare **Crea una risorsa** > **Analisi** > **Azure Databricks**.
    ...
    ![Databricks nel portale di Azure](./media/handle-data-using-databricks/azure-databricks-on-portal.png "Databricks nel portale di Azure")

2. Nella pagina **Servizio Azure Databricks** specificare i valori per creare un'area di lavoro di Databricks.

    ![Creare un'area di lavoro di Azure Databricks](./media/handle-data-using-databricks/create-databricks-workspace.png "Creare un'area di lavoro di Azure Databricks")
...
3. La creazione dell'account richiede alcuni minuti, durante i quali il portale visualizza il riquadro **Invio della distribuzione per Azure Databricks** a destra. Potrebbe essere necessario scorrere verso destra nel dashboard per visualizzare il riquadro. È presente anche un indicatore di stato nella parte superiore della schermata. È possibile esaminare lo stato di avanzamento nelle due aree.
    ...
    ![Riquadro di distribuzione Databricks](./media/handle-data-using-databricks/databricks-deployment-tile.png "Riquadro di distribuzione Databricks")
...
3. Nella pagina **New cluster** (Nuovo cluster) specificare i valori per creare un cluster.
    ...
    ![Creare il cluster Databricks Spark in Azure](./media/handle-data-using-databricks/create-databricks-spark-cluster.png "Creare il cluster Databricks Spark in Azure")
...
4. Immettere il codice seguente nella prima cella ed eseguire il codice:
...
1. Se non si dispone già di un account creato per Data Lake Storage Gen2, seguire la guida introduttiva per creare un account per Data Lake Storage Gen2.
2. I dati di esempio (**small_radio_json.json**) sono disponibili nel repository di [gestione dei problemi ed esempi di U-SQL](https://github.com/Azure/usql/blob/master/Examples/Samples/Data/json/radiowebsite/small_radio_json.json). Scaricare il file JSON e prendere nota del percorso in cui viene salvato.
3. Caricare i dati nell'account di archiviazione. Il metodo usato per caricare i dati nell'account di archiviazione è diverso a seconda che sia abilitato o meno il Servizio dello spazio dei nomi gerarchico (HNS).

    Se il Servizio dello spazio dei nomi gerarchico è abilitato per l'account di ADLS Gen2, è possibile usare Azure Data Factory, distp o AzCopy (versione 10) per gestire il caricamento. AzCopy versione 10 è disponibile solo per i clienti che usano la versione di anteprima. Per usare AzCopy da Cloud Shell:
...
6. Connettersi al database SQL e verificare che sia visualizzata una tabella **SampleTable**.
    ...
    ![Verificare la tabella di esempio](./media/handle-data-using-databricks/verify-sample-table.png "Verificare la tabella di esempio")
...
7. Eseguire una query selezionata per verificare i contenuti della tabella. Dovrebbe includere gli stessi dati del frame di dati **renamedColumnsDF**.
    ...
    ![Verificare i contenuti della tabella di esempio](./media/handle-data-using-databricks/verify-sample-table-content.png "Verificare i contenuti della tabella di esempio")
...
![Arrestare un cluster Databricks](./media/handle-data-using-databricks/terminate-databricks-cluster.png "Arrestare un cluster Databricks")
