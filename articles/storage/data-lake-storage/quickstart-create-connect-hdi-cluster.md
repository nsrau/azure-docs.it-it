...
ms.locfileid: "39525587"
---
# <a name="quickstart-set-up-clusters-in-hdinsight"></a>Avvio rapido: impostazione dei cluster in HDInsight

In questa guida di avvio rapido sono riportate informazioni su come impostare e configurare i cluster di HDInsight con Hadoop, Spark, Kafka, Interactive Query, HBase, R Server o Storm. Si scoprirà anche a personalizzare i cluster, aggiungerli a un dominio e allegarli all’[anteprima di Azure Data Lake Storage Gen2](introduction.md).
...
In questa guida di avvio rapido, Data Lake Storage viene utilizzato come livello dati. Con il suo servizio di spazio dei nomi gerarchico e il [driver Hadoop](abfs-driver.md), Data Lake Storage è ottimizzato per l'elaborazione e l'analisi distribuita. I dati archiviati in Data Lake Storage persistono anche dopo l'eliminazione di un cluster HDInsight.
...
![Opzioni di creazione di HDInsight Creazione rapida personalizzata](media/quickstart-create-connect-hdi-cluster/hdinsight-creation-options.png)
...
Per i tipi di cluster Hadoop, Spark e Interactive Query, è possibile scegliere di abilitare il **Pacchetto di sicurezza aziendale**. Questo pacchetto consente di eseguire una configurazione cluster più sicura usando Apache Ranger e integrando Azure Active Directory. Per altre informazioni, vedere [Enterprise Security Package in Azure HDInsight](../../hdinsight/domain-joined/apache-domain-joined-introduction.md) (Pacchetto di sicurezza aziendale in Azure HDInsight).

![Opzioni di creazione di HDInsight Scegliere il pacchetto di sicurezza aziendale](./media/quickstart-create-connect-hdi-cluster/hdinsight-creation-enterprise-security-package.png)
...
| type | Nodi | Diagramma |
| --- | --- | --- |
| Hadoop |Nodo head (2), nodo dati (1+) |![Nodi del cluster HDInsight Hadoop](media/quickstart-create-connect-hdi-cluster/hdinsight-hadoop-cluster-type-nodes.png) |
| hbase |Server head (2), server di area (1+), nodo master/ZooKeeper (3) |![Nodi del cluster HDInsight HBase](media/quickstart-create-connect-hdi-cluster/hdinsight-hbase-cluster-type-setup.png) |
| Storm |Nodo Nimbus (2), server supervisore (1+), nodo ZooKeeper (3) |![Nodi del cluster HDInsight Storm](media/quickstart-create-connect-hdi-cluster/hdinsight-storm-cluster-type-setup.png) |
| Spark |Nodo head (2), nodo Worker (1+), nodo ZooKeeper (3), gratuito per le macchine virtuali ZooKeeper con dimensioni A1 |![Nodi del cluster HDInsight Spark](media/quickstart-create-connect-hdi-cluster/hdinsight-spark-cluster-type-setup.png) |
...
![Dimensioni dei nodi delle VM di HDInsight](media/quickstart-create-connect-hdi-cluster/hdinsight-node-sizes.png)
...
- [Driver di file System Hadoop ABFS per Archiviazione di Azure Data Lake Gen2](abfs-driver.md)
- [Esercitazione: Estrarre, trasformare e caricare dati usando Apache Hive in Azure HDInsight](tutorial-extract-transform-load-hive.md)
