...
1.  [Diagnostica dei problemi][Diagnostica dei problemi]
2.  [Indicizzazione][Indicizzazione]
3.  [Progettazione dello schema][Progettazione dello schema]
4.  [Progettazione delle query][Progettazione delle query]
5.  [Architettura del servizio][Architettura del servizio]
6.  [Risoluzione dei problemi avanzata][Risoluzione dei problemi avanzata]

<a name="Diagnosing"></a>

## Diagnostica dei problemi

Se si sospetta la presenza di problemi nel servizio mobile in condizioni di carico, il primo punto da esaminare è la scheda **Dashboard** del servizio nel [portale di gestione di Azure][portale di gestione di Azure]. In questa scheda, verificare che:
...
-   Edizione Web e Business
-   Edizione Basic, Standard e Premium (attualmente in anteprima)

Anche se l'edizione Web e Business è completamente supportata, il servizio verrà interrotto entro il 24 aprile 2015, come illustrato in [Domande frequenti sull'interruzione delle edizioni Web e Business][Domande frequenti sull'interruzione delle edizioni Web e Business]. In preparazione per questa modifica, i nuovi clienti sono invitati a iniziare a usare le anteprime Basic, Standard e Premium, se i requisiti dell'applicazione lo consentono.

#### Edizione Web e Business

Attualmente questa è l'edizione predefinita usata da Servizi mobili. Ecco alcuni consigli per la scelta del livello del database:

-   **Database gratuito da 20 MB**: usare solo a scopi di sviluppo
-   **Web e Business**: usare per servizi di sviluppo e produzione. I due livelli offrono lo stesso livello di prestazioni, anche se il livello Web supporta solo database delle dimensioni massime di 5 GB. Per database di dimensioni superiori, usare il livello Business.

#### Edizione Basic, Standard e Premium

Questa nuova edizione offre numerosi nuovi livelli e funzionalità di monitoraggio, che agevolano la comprensione e la risoluzione dei problemi relativi alle prestazione del database. Per usare questa edizione con il servizio mobile:

1.  Passare alla pagina [Anteprima funzionalità][Anteprima funzionalità] e iscriversi per i **Nuovi livelli di servizio per database SQL.**
2.  Dopo l'attivazione della funzionalità di anteprima, avviare il [portale di gestione di Azure][portale di gestione di Azure].
3.  Selezionare **+NUOVO** sulla barra degli strumenti e quindi **Servizi dati**, **Database SQL**, **Creazione rapida**.
4.  Immettere un nome di database e quindi selezionare **Nuovo server di database SQL** nel campo **Server**. Verrà creato un server che usa la nuova edizione Basic, Standard e Premium.
5.  Compilare il resto dei campi e selezionare **Crea database SQL**. Verrà creato un database da 100 MB di livello Basic.
6.  Configurare il servizio mobile per l'uso del database creato

    -   Se si sta creando un nuovo servizio mobile, selezionare **+NUOVO**, **Calcolo**, **Servizio mobile**, **Crea**. Nella schermata successiva compilare i valori, selezionare **Utilizza un database SQL esistente** nel campo **Database** e quindi selezionare **Avanti**. Nella schermata successiva assicurarsi di selezionare il database creato nel passaggio 5, quindi selezionare **OK**.
    -   Se si dispone già di un servizio mobile, passare alla scheda **Configura** del servizio e selezionare **Cambia database** sulla barra degli strumenti. Nella schermata successiva selezionare **Utilizza un database SQL esistente** nel campo **Database SQL** e quindi selezionare **Avanti**. Nella schermata successiva assicurarsi di selezionare il database creato nel passaggio 5, quindi selezionare **OK**.
...
Per altre informazioni sulle situazioni in cui usare ogni livello, vedere [Vantaggi dei nuovi livelli di servizio][Vantaggi dei nuovi livelli di servizio]
...
1.  Avviare il [portale di gestione di Azure][portale di gestione di Azure].
...
-   **Ottimizzare il database.**
     Spesso è possibile ridurre l'utilizzo del database ed evitare la necessità di passare a un livello superiore ottimizzando il database.
...
Per altre informazioni sulla diagnostica dei problemi SQL, vedere [Diagnostica avanzata][Diagnostica avanzata] più avanti in questo documento.
...
Pertanto, se si eseguono spesso istruzioni WHERE o JOIN su specifiche colonne, assicurarsi di indicizzarle. Per altre informazioni, vedere la sezione [Creazione di indici][Creazione di indici].

Ma se gli indici sono così utili e le scansioni di tabella sono da evitare, per prevenire qualsiasi problema è opportuno indicizzare tutte le colonne della tabella? La risposta breve è "probabilmente no". Gli indici occupano spazio e hanno un certo overhead: ogni volta che viene eseguita un'operazione di inserimento in una tabella, è necessario aggiornare le strutture dell'indice per ogni colonna indicizzata. Vedere la sezione seguente per le linee guida sulla scelta degli indici di colonna.
...
1.  Aprire il servizio mobile nel [portale di gestione di Azure][portale di gestione di Azure].
...
Per altre informazioni sugli indici, vedere l'argomento relativo alle [annotazioni di indice in Entity Framework][annotazioni di indice in Entity Framework]. Per altri suggerimenti sull'ottimizzazione degli indici, vedere [Indicizzazione avanzata][Indicizzazione avanzata] più avanti in questo documento.
...
-   **Eseguire sempre le operazioni di join nel database.** Spesso è necessario combinare i record di due o più tabelle in cui i record combinati condividono un campo comune (operazione di *join*). Se eseguita in modo errato, questa operazione può risultare inefficiente in quanto può comportare l'estrazione e la successiva iterazione di tutte le entità di tutte le tabelle. È consigliabile che questa operazione venga eseguita dal database stesso, ma a volte viene erroneamente eseguita nel client o nel codice del servizio mobile.
    ...
    -   Non eseguire join nel codice del servizio mobile. Quando si usa il back-end JavaScript, tenere presente che l'[oggetto tabella][oggetto tabella] non gestisce i join. Usare l'[oggetto mssql][oggetto mssql] per assicurarsi che il join venga eseguito nel database. Per altre informazioni, vedere [Creare un join tra tabelle relazionali][Creare un join tra tabelle relazionali]. Se si usa il back-end .NET e le query vengono eseguite tramite LINQ, i join vengono gestiti automaticamente a livello di database da Entity Framework.
-   **Implementare il paging.** L'esecuzione di query sul database può a volte risultare nella restituzione al client di un numero molto elevato di record. Per minimizzare le dimensioni e la latenza delle operazioni, valutare la possibilità di implementare il paging.
    ...
    -   Per impostazione predefinita, il servizio mobile limiterà le query in arrivo a una pagina di dimensioni pari a 50 righe ed è possibile richiedere manualmente fino a 1.000 record. Per altre informazioni, vedere "Restituire i dati in pagine" per [Windows Store][Windows Store], [iOS][iOS], [Android][Android], [HTML/JavaScript][HTML/JavaScript] e [Xamarin][Xamarin].
    -   Per le query eseguite dal codice del servizio mobile non sono previste dimensioni di pagina predefinite. Se l'app non implementa il paging, o come misura preventiva, valutare l'applicazione di limiti predefiniti alle query. Nel back-end JavaScript usare l'operatore **take** sull'[oggetto query][oggetto query]. Se si usa il back-end .NET, considerare l'uso del [metodo Take][metodo Take] come parte della query LINQ.

Per altre informazioni sul miglioramento della progettazione delle query e sull'analisi dei piani di query, vedere [Progettazione avanzata delle query][Progettazione avanzata delle query] più avanti in questo documento.
...
3.  Selezionare la scheda **Configura**.
4.  Selezionare il nome del **Database SQL** nella sezione **Impostazioni database**. Verrà visualizzata la scheda del database SQL di Azure nel portale.
5.  Selezionare **Imposta regole firewall di Windows Azure per questo indirizzo IP**.
...
1.  Passare a [Edizioni di SQL Server - Express][Edizioni di SQL Server - Express]
...
Il risultato conterrà le utili metriche seguenti: CPU (% del limite del livello), Archiviazione (MB), Letture dati fisiche (% del limite del livello), Scritture nei log (% del limite del livello), Memoria (% del limite del livello), Conteggio Worker, Conteggio sessioni e così via.

#### Eventi di connettività SQL

La vista **[sys.event\_log][sys.event\_log]** contiene i dettagli degli eventi correlati alla connettività.
...
> [WACOM.NOTE]
> Eseguire la query sul database **master** del server, la vista **** è presente solo in quel database.
...
Come analogia con il mondo reale, si può pensare a un libro o a un manuale tecnico. Il contenuto di ogni pagina è un record, il numero di pagina è l'indice cluster e l'indice degli argomenti in fondo al libro è l'indice non cluster. Ogni voce nell'indice degli argomenti punta all'indice cluster, il numero di pagina.

> [WACOM.NOTE]
> Per impostazione predefinita, il back-end JavaScript di Servizi mobili di Azure imposta **\_createdAt** come indice cluster. Se si rimuove questa colonna o si desidera un indice cluster diverso, assicurarsi di seguire le [linee guida per la progettazione di indici cluster][linee guida per la progettazione di indici cluster] di seguito. Nel back-end .NET, la classe `EntityData` definisce `CreatedAt` come indice cluster usando l'annotazione `[Index(IsClustered = true)]`.
...
-   Di dimensioni limitate: che usa un tipo di dati "corto" o è una [chiave composta][chiave composta] di un numero ridotto di colonne di dimensioni limitate
...
-   [Creazione e modifica di vincoli PRIMARY KEY][Creazione e modifica di vincoli PRIMARY KEY]
-   [Creare indici non cluster][Creare indici non cluster]
-   [Creare indici cluster][Creare indici cluster]
-   [Creare indici univoci][Creare indici univoci]

#### Trovare i primi n indici mancanti

È possibile scrivere query SQL sulle DMV che offriranno informazioni più dettagliate circa l'utilizzo delle risorse delle singole query o dati euristici sugli indici di aggiungere. La query seguente determina i 10 indici mancanti in grado di determinare il massimo miglioramento cumulativo previsto, in ordine decrescente, per le query utente.
...
Per altre informazioni, vedere [Monitoraggio di database SQL mediante le viste a gestione dinamica][Monitoraggio di database SQL mediante le viste a gestione dinamica] e [DMV di indici mancanti][DMV di indici mancanti].
...
-   [Documentazione relativa al database SQL di Azure][Documentazione relativa al database SQL di Azure]
-   [Prestazioni e scalabilità del database SQL di Azure][Prestazioni e scalabilità del database SQL di Azure]
-   [Risoluzione dei problemi relativi al database SQL di Azure][Risoluzione dei problemi relativi al database SQL di Azure]

### Indicizzazione

-   [Nozioni di base sugli indici][Nozioni di base sugli indici]
-   [Linee guida generali per la progettazione di indici][Linee guida generali per la progettazione di indici]
-   [Linee guida per la progettazione di indici univoci][Linee guida per la progettazione di indici univoci]
-   [Linee guida per la progettazione di indici cluster][Linee guida per la progettazione di indici cluster]
-   [Vincoli di chiavi primarie ed esterne][chiave composta]
-   [Costo delle chiavi][Costo delle chiavi]

### Entity Framework

-   [Considerazioni sulle prestazioni per Entity Framework 5][Considerazioni sulle prestazioni per Entity Framework 5]
-   [Annotazioni dei dati per Code First][Annotazioni dei dati per Code First]
...
<!-- BLOG LINKS -->
  ...
  [Diagnostica dei problemi]: #Diagnosing
  [Indicizzazione]: #Indexing
  [Progettazione dello schema]: #Schema
  [Progettazione delle query]: #Query
  [Architettura del servizio]: #Architecture
  [Risoluzione dei problemi avanzata]: #Advanced
  [portale di gestione di Azure]: http://manage.windowsazure.com
  [Domande frequenti sull'interruzione delle edizioni Web e Business]: http://msdn.microsoft.com/en-US/library/azure/dn741330.aspx
  [Anteprima funzionalità]: https://account.windowsazure.com/previewfeatures
  [Vantaggi dei nuovi livelli di servizio]: http://msdn.microsoft.com/en-US/library/azure/dn369873.aspx#Reasons
  [Azure Management Portal - SQL Database Metrics]: ./media/mobile-services-sql-scale-guidance/3.png
  [Informazioni sull'utilizzo delle risorse]: http://msdn.microsoft.com/en-US/library/azure/dn369873.aspx#Resource
