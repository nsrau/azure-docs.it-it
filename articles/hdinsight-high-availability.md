...
La creazione e il provisioning di un cluster con PowerShell sono documentati in [Amministrazione di HDInsight tramite PowerShell][Amministrazione di HDInsight tramite PowerShell]. La configurazione di un nodo head molto grande richiede l'aggiunta del parametro `-HeadNodeVMSize ExtraLarge` al cmdlet `New-AzureHDInsightcluster` usato in questo codice.
