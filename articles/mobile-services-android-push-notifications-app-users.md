...
In questo argomento viene estesa l'[esercitazione precedente relativa alle notifiche push](/en-us/develop/mobile/tutorials/get-started-with-push-android) mediante l'aggiunta di una nuova tabella per l'archiviazione degli URI di registrazione di Google Cloud Messaging (GCM) che possono quindi essere utilizzati per inviare le notifiche push a più utenti dell'app per Android. In questa esercitazione un singolo aggiornamento genererà le notifiche push a tutti i dispositivi registrati ogni volta che viene effettuata un'operazione di inserimento nella tabella ToDoList. Nell'esercitazione precedente, la notifica è stata inviata solo al dispositivo che ha effettuato l'operazione di inserimento.

In questa esercitazione vengono descritte le operazioni per aggiornare le notifiche push nell'app:

1.  [Creazione della tabella Registration](#create-table)
...
Questa esercitazione è basata sul progetto di guida introduttiva per Servizi mobili e sull'esercitazione [Introduzione alle notifiche push](/en-us/develop/mobile/tutorials/get-started-with-push-android). Prima di iniziare questa esercitazione, è necessario completare le procedure illustrate in [Introduzione alle notifiche push](/en-us/develop/mobile/tutorials/get-started-with-push-android).
...
1.  In Eclipse, in Package Explorer fare clic con il pulsante destro del mouse sul pacchetto (nel nodo `src`), quindi scegliere **New** e fare clic su **Class**.
...
6.  Aprire il file **ToDoActivity.java**, quindi eliminare le righe seguenti nel metodo `addItem`:
...
2.  In **registration** fare clic sulla scheda **Script** e selezionare **Insert**.
   	...
   	Verrà visualizzata la funzione che viene richiamata quando si verifica un inserimento nella tabella **Registration**.
...
3.  Sostituire la funzione insert con il codice seguente, quindi fare clic su **Save**:
   	...
   	Lo script verifica la tabella **Registration** per individuare una registrazione esistente con lo stesso URI. L'operazione di inserimento viene eseguita solo se non viene trovata alcuna registrazione corrispondente, in modo da evitare l'inserimento di record di registrazione duplicati.
...
1.  In Eclipse, dal menu **Run** scegliere **Run** per avviare il l'app.
...
-   [Come utilizzare la libreria client di Android per Servizi mobili](/en-us/develop/mobile/how-to-guides/work-with-android-client-library)
