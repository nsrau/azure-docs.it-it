...
- Supporto per la registrazione e l'individuazione degli oggetti di SQL Server Analysis Services tramite HTTP/HTTPS. Gli utenti possono ora connettersi ai server SSAS utilizzando un URL (ad esempio https://servername/olap/msmdpump.dll) anziché un nome di server, e possono utilizzare l'autenticazione Basic e le connessioni anonime oltre all'autenticazione di Windows. Per ulteriori informazioni sulle connessioni HTTP/HTTPS a SSAS, vedere [Configurare l'accesso HTTP ad Analysis Services](https://msdn.microsoft.com/library/gg492140.aspx).
- Supporto per le origini di dati Hive in HDInsight. Gli utenti possono ora registrarsi e individuare le tabelle Hive per Apache Hive di Hadoop sulle origini dei dati HDInsight. Per ulteriori informazioni su Hive in HDInsight, vedere il [Centro di documentazione HDInsight](../hdinsight-use-hive/).
- Supporto per la registrazione e l'individuazione dei database Oracle e dei cluster HDFS come contenitori. Quando si registrano le tabelle Oracle e le visualizzazioni o HDFS, **Catalogo dati di Azure** crea una voce per il database, le tabelle e le viste. Il database può essere individuato e annotato utilizzando il portale **Catalogo dati di Azure**. Gli utenti possono inoltre cercare e filtrare il contenuto di un database o cluster, oltre a cercare e filtrare il contenuto del catalogo.
