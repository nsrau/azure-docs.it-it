...
> [AZURE.NOTE]Se si sceglie di specificare per <strong>Bundle Identifier</strong> un valore diverso da <i>MobileServices.Quickstart</i>, è necessario aggiornare anche il valore dell'identificatore del bundle nel progetto Xcode. Si consiglia di usare l'identificatore del bundle esatto già usato nel progetto di guida introduttiva.

2. Digitare un nome per l'app in **Description**, inserire il valore _MobileServices.Quickstart_ in **Bundle Identifier**, selezionare l'opzione "Push Notifications" nella sezione "App Services" e quindi fare clic su **Continue**. In questo esempio viene usato l'ID **MobileServices.Quickstart**, tuttavia non è possibile riutilizzare lo stesso ID, in quanto gli ID app devono essere univoci per tutti gli utenti. È pertanto consigliabile aggiungere il proprio nome completo o le proprie iniziali dopo il nome dell'app.
