...
Per poter eseguire il backup di un server locale (client di Windows o server Windows Server o di Data Protection Manager) in Azure, è possibile autenticare il server con un insieme di credenziali di servizi di ripristino. Utilizzare un file di credenziali dell'insieme di credenziali per l'autenticazione del server con Azure. Insieme di credenziali è simili al concetto di un file "impostazioni di pubblicazione" utilizzato in Azure PowerShell.
