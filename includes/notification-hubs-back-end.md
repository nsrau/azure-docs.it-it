...
Entrambi i back-end inviano notifiche sia a Windows Store sia a dispositivi iOS. È possibile inviare notifiche da qualsiasi back-end tramite l'[interfaccia REST degli Hub di notifica][1].
...
2.  Nel menu principale di Visual Studio fare clic su **Strumenti**, **Gestione pacchetti di librerie** e **Console di Gestione pacchetti**, quindi nella finestra di tipo console digitare quanto segue e premere **INVIO**:
    ...
    Verrà aggiunto un riferimento ad Azure Service Bus SDK con il [pacchetto NuGet WindowsAzure.ServiceBus][2].
...
4.  Nella classe `Program` aggiungere il metodo seguente oppure sostituirlo se esiste già:
	...
	**Nota**
	...
	Questo codice back-end supporta Windows Store, Windows Phone e client iOS. I metodi Send restituiscono una risposta di errore sel
...
È ora possibile procedere con la sezione [Esecuzione dell'app e generazione di notifiche](#test-app).

### <a name="mobile-services"></a>Per inviare notifiche da Servizi mobili

Per inviare una notifica tramite un servizio mobile, eseguire le operazioni seguenti:

1.  Completare l'esercitazione [Introduzione a Servizi mobili](/en-us/develop/mobile/tutorials/get-started/#create-new-service) per creare il servizio mobile.

2.  Accedere al [portale di gestione di Azure][3], fare clic su Servizi mobili e quindi sul servizio mobile.

3.  Fare clic sulla scheda **Scheduler** e quindi su **Create**.
    
       ![][15]

4.  In **Create new job** digitare un nome, selezionare **On demand** e quindi fare clic sul segno di spunta per accettare.
    
       ![][16]

5.  Dopo la creazione del processo, fare clic sul relativo nome e quindi nella scheda **Script** inserire lo script seguente all'interno della funzione del processo pianificato:
...
7.  Aggiungere la funzione di supporto seguente dopo la funzione del processo pianificato, quindi fare clic su **Save**:
