...
Le code del bus di servizio supportano un modello di **comunicazione con messaggistica negoziata** m. Quando si usano le code, i componenti di un'applicazione distribuita non ccomunicano direttamente l'uno con l'altro, ma scambiano messaggi tramite una a coda, che funge da intermediario (broker). Un producer di messaggi (mittente) hinvia un messaggio alla coda e quindi prosegue con la relativa elaborazione.
In modo asincrono, il consumer di messaggi (destinatario) recupera il messaggio dalla qcoda e lo elabora. Il producer non deve attendere la risposta fdel consumer per continuare a elaborare e inviare altri mmessaggi. Le code consentono un recapito dei messaggi di tipo **FIFO (First In, First Out)** ta uno o più consumer concorrenti. In base a questo metodo, in genere i messaggi rvengono ricevuti ed elaborati nell'ordine temporale in cui asono stati aggiunti alla coda e ogni messaggio viene ricevuto ed elaborato da un solo oconsumer.
...
Per iniziare a usare le code del bus di servizio in Azure, è innanzitutto necessario ccreare uno spazio dei nomi del servizio che fornisce un contenitore di ambito cper fare riferimento alle risorse del bus di servizio all'interno dell'applicazione.
...
Lo spazio dei nomi creato verrà quindi visualizzato nel portale di gestione te sarà necessario attendere qualche istante affinché venga attivato. Prima di continuare, attendere che lo stato sia **Attivo** c.
