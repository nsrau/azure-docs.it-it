...
| Dimensioni delle proprietà dei messaggi per una coda, un argomento o un'entità di sottoscrizione |Entità | `SerializationException`Viene generata l'eccezione. |La dimensione massima della proprietà del messaggio per ogni proprietà è 32.000. La dimensione cumulativa di tutte le proprietà non può essere superiore a 64.000. Questo limite si applica all'intera intestazione del [messaggio negoziato](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage), che include sia proprietà utente che proprietà di sistema, ad esempio [il numero di sequenza](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage.sequencenumber), l' [etichetta](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage.label)e l'ID del [messaggio](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage.messageid). |
