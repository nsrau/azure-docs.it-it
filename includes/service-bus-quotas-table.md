...
| Nome quota | `Scope` | Note | Value |
| --- | --- | --- | --- |
| Numero massimo di spazi dei nomi Basic o Standard per ogni sottoscrizione di Azure |Namespace |Le successive richieste di altri spazi dei nomi Basic o Standard vengono rifiutate dal portale di Azure. |100|
| Numero massimo di spazi dei nomi Premium per ogni sottoscrizione di Azure |Namespace |Le successive richieste di altri spazi dei nomi Premium vengono rifiutate dal portale. |25 |
...
| Numero di connessioni simultanee per uno spazio dei nomi |Namespace |Le successive richieste di connessioni aggiuntive vengono rifiutate e il codice chiamante riceve un'eccezione. Operazioni REST non vengono considerati connessioni TCP simultanee. |NetMessaging: 1,000.<br /><br />AMQP: 5,000. |
...
| Numero di messaggi per transazione | Transazione | Altri messaggi in arrivo vengono rifiutati, e l'eccezione seguente "non è possibile inviare più di 100 messaggi in un'unica transazione" viene ricevuto il codice chiamante. | 100 <br /><br /> Per entrambe le operazioni **Send()** e **SendAsync()**. |
