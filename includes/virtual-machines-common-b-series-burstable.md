...
| Dimensioni             | vCPU  | Memoria: GiB | GiB di archiviazione temporanea (unità SSD) | Prestazioni base della CPU della VM | Prestazioni massime della CPU della VM | Crediti iniziali | Crediti guadagnati/ora | Massimo crediti ottenuti | Numero massimo di dischi dati | Velocità effettiva massima memorizzata nella cache e archiviazione temporanea: IOPS/MBps | Max velocità effettiva del disco non memorizzato nella cache: IOPS/MBps | Schede di interfaccia di rete max |          
|---------------|-------------|----------------|----------------------------|-----------------------|--------------------|--------------------|--------------------|----------------|----------------------------------------|-------------------------------------------|-------------------------------------------|----------|
...
![Grafico dei dati relativi al traffico orario](./media/virtual-machines-common-b-series-burstable/office-workload.png)

| Scenario | Durata | Utilizzo CPU (%) | Crediti accumulati<sup>1</sup> | Crediti disponibili |
| --- | --- | --- | --- | --- |
...
**R**: verranno introdotte 2 nuove metriche nelle prossime settimane. la metrica del **credito** consentirà di visualizzare il numero di crediti ottenuti dalla macchina virtuale e la metrica **ConsumedCredit** INDICHERà il numero di crediti CPU che la macchina virtuale ha usato dalla banca.    Sarà possibile visualizzare queste metriche dal riquadro delle metriche nel portale o a livello di programmazione tramite le API di Monitoraggio di Azure.
...
### <a name="q-what-happens-if-i-run-out-of-credits"></a>D: cosa accade se si esauriscono i crediti?
**R** : se la macchina virtuale dedica tutti i crediti disponibili, può usare solo la CPU di base e non sarà in grado di superare quella Baseline. 
