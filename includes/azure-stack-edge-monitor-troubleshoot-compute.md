...
Per ottenere informazioni della Guida per qualsiasi comando disponibile, usare `dkrdbe <command-name> --help` .
...
I moduli di calcolo sono contenitori per i quali è implementata una logica di business. Per verificare se un modulo di calcolo è stato distribuito correttamente, eseguire il `ps` comando e verificare se il contenitore, corrispondente al modulo di calcolo, è in esecuzione.
...
Per monitorare la memoria, l'utilizzo della CPU e l'i/o nel dispositivo, utilizzare il `stats` comando.
