...
Creare una pagina HTML con una personalizzazione personalizzata per gestire il contenuto della pagina personalizzata. Questa pagina può essere una pagina di `*.html` statica o una pagina dinamica, ad esempio .NET, node. js o PHP.

Il contenuto della pagina personalizzata può contenere qualsiasi elemento HTML, incluso CSS e JavaScript, ma non può includere elementi non protetti come gli iframe. L'unico elemento obbligatorio è un elemento div con `id` impostato su `api`, ad esempio questo `<div id="api"></div>` all'interno della pagina HTML.
...
- A causa delle restrizioni di sicurezza, Azure AD B2C non supporta gli elementi HTML `frame`, `iframe`o `form`.
...
1. Passare a [www.test-CORS.org](https://www.test-cors.org/) e incollare l'URL nella casella **Remote URL (URL remoto** ).
1. Selezionare **Invia richiesta**.
    Se viene visualizzato un errore, verificare che le impostazioni CORS siano corrette. Potrebbe anche essere necessario cancellare la cache del browser o aprire una sessione di esplorazione anonima premendo Ctrl+Maiusc+P.
