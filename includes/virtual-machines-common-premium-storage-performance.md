...
| **Limiti relativi al ridimensionamento di VM e dischi** |Il limite di operazioni di I/O al secondo della dimensione della macchina virtuale scelta deve essere maggiore del numero totale di operazioni di I/O al secondo guidate dai dischi di archiviazione collegati. |Il limite di velocità effettiva della dimensione di VM scelta deve essere superiore al totale di velocità effettiva basato sul disco di Archiviazione Premium collegato alla VM. |I limiti di ridimensionamento della dimensione di VM scelta devono essere superiori al totale dei limiti di ridimensionamento dei dischi di Archiviazione Premium collegati. |
...
| **Dimensioni di striping** |Dimensioni di striping ridotte per modelli di I/O casuali presenti in applicazioni OLTP. Ad esempio, utilizzare una dimensione di striping di 64 KB per l'applicazione OLTP di SQL Server. |Dimensioni di striping superiori per modelli di I/O sequenziali di grandi dimensioni presenti in applicazioni di tipo data warehouse. Ad esempio, utilizzare 256 KB di dimensione dello striping per l'applicazione sql Server Data warehouse. | &nbsp; |
