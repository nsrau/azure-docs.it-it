...
In questa sezione il progetto dell'esercitazione [Introduzione a Servizi mobili][Introduzione a Servizi mobili] verrà aggiornato in modo da consentire di scattare foto e caricarle in Archiviazione BLOB di Azure. Per acquisire l'immagine, in questa esercitazione viene usato [CameraCaptureTask][CameraCaptureTask] ricavato dallo spazio dei nomi `Microsoft.Phone.Tasks`. Questa classe avvia l'interfaccia utente della fotocamera sul dispositivo Windows Phone per acquisire la foto e salva automaticamente l'immagine nel rullino sul dispositivo Windows Phone. Se non si desidera salvare le immagini nel rullino, usare la classe [PhotoCamera][PhotoCamera] nello spazio dei nomi `Microsoft.Devices`.
...
2.  Aprire il file MainPage.xaml e sostituire l'elemento **Grid** denominato **ContentPanel** con il codice seguente:
    ...
    Viene aggiunto un nuovo pulsante per avviare [CameraCaptureTask][CameraCaptureTask], viene aggiunta un'immagine a **ItemTemplate** e l'origine dell'associazione viene impostata come URI dell'immagine caricata nel servizio di archiviazione BLOB.
...
5.  Nel file del progetto MainPage.xaml.cs aggiornare la classe MainPage. Aggiungere il codice seguente per dichiarare [CameraCaptureTask][CameraCaptureTask] e un oggetto stream che fa riferimento all'immagine acquisita:
...
8.  Nel file del progetto MainPage.xaml.cs aggiornare la classe MainPage. Sostituire il metodo `InsertTodoItem` con il codice seguente:
...
> [WACOM.NOTE]L'immagine viene scaricata automaticamente dal servizio di archiviazione BLOB quando la proprietà `imageUri` del nuovo elemento viene associata al controllo **Image**.
  ...
  [CameraCaptureTask]: http://msdn.microsoft.com/it-it/library/windowsphone/develop/microsoft.phone.tasks.cameracapturetask(v=vs.105).aspx
  [PhotoCamera]: http://msdn.microsoft.com/it-it/library/windowsphone/develop/microsoft.devices.photocamera(v=vs.105).aspx
