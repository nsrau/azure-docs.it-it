...
3. Nella pagina **Crea un gateway di rete locale** specificare i valori per il gateway di rete locale.
   ...
   :::image type="content" source="./media/vpn-gateway-add-lng-rm-portal-include/create-local-gateway-fqdn.png" alt-text="Creare il gateway di rete locale":::
   
   > [!NOTE]
   >
   > * La VPN di Azure supporta un solo indirizzo IPv4 per ogni FQDN. Se il nome di dominio si risolve in più indirizzi IP, il gateway VPN di Azure userà il primo restituito dai server DNS. Per eliminare l'incertezza, è consigliabile che il nome FQDN venga sempre risolto in un singolo indirizzo IPv4. IPv6 non è supportato.
