...
3. Se necessario, eseguire il cmdlet **New-AzureResourceGroup** per creare un nuovo gruppo di risorse, come illustrato di seguito. Per questo scenario, creare un gruppo di risorse denominato *RG1*.
...
5. Eseguire il cmdlet **Get AzureVirtualNetwork** per archiviare l'oggetto rete virtuale in una variabile, come illustrato di seguito.
	...
	>[AZURE.TIP]È possibile combinare i passaggi 4 e 5 eseguendo **$vnet = New-AzureVirtualNetwork -ResourceGroupName RG1 -Name TestVNet -AddressPrefix 192.168.0.0/16 -Location centralus**.
