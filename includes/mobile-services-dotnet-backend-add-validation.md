...
5. Fare di nuovo clic con il pulsante destro del mouse sul progetto di servizio e scegliere **Pubblica**. Pubblicare il servizio mobile nell'account Microsoft Azure utilizzando le impostazioni di pubblicazione illustrate nell'esercitazione [Attività iniziali] o [Introduzione ai dati](mobile-services-dotnet-backend-windows-store-dotnet-get-started-data.md).
 
     >[AZURE.NOTE] In alternativa, è possibile eseguire il test con il servizio ospitato in locale in IIS Express. Per altre informazioni, vedere l'esercitazione[Introduzione ai dati](mobile-services-dotnet-backend-windows-store-dotnet-get-started-data.md).
