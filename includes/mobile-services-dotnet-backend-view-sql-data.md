...
1.  Nel portale di gestione di Azure fare clic su Manage per il database associato al servizio mobile.
    ...
    ![accesso per gestione database SQL][]
...
2.  Nel portale di gestione eseguire una query per visualizzare le modifiche apportate dall'app di Windows Store. La query da utilizzare sarà simile alla seguente, ma anziché `todolist` conterrà il nome del database.
    ...
    ![query al database SQL per ricerca elementi memorizzati][]

    Si noti che la tabella include le colonne Id, \_\_createdAt, \_\_updatedAt, and \_\_version. Tali colonne supportano la sincronizzazione dei dati offline e sono implementate nella classe di base [EntityData][]. Per ulteriori informazioni, vedere [Get started with offline data sync].
