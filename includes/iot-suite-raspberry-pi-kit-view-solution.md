...
1. Scegliere **Let me define my own Device ID** (Desidero definire il mio ID dispositivo). Immettere un ID dispositivo, ad esempio **rasppi**, fare clic su **Controlla ID** per verificare di non avere già usato il nome nella soluzione e quindi fare clic su **Crea** per effettuare il provisioning del dispositivo.
