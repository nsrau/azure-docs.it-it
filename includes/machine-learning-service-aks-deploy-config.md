...
| ----- | ----- | ----- |
| `computeType` | N/D | La destinazione di calcolo. Per AKS, il valore deve essere `aks`. |
| `autoScaler` | N/D | Contiene gli elementi di configurazione per la scalabilità automatica. Vedere la tabella scalabilità automatica. |
| &emsp;&emsp;`autoscaleEnabled` | `autoscale_enabled` | Indica se abilitare la scalabilità automatica per il servizio Web. `numReplicas`  = Se `0`, `True`; in caso `False`contrario,. |
...
| `dataCollection` | N/D | Contiene elementi di configurazione per la raccolta di dati. |
...
| `authEnabled` | `auth_enabled` | Indica se abilitare o meno l'autenticazione della chiave per il servizio Web. `tokenAuthEnabled` E `authEnabled` non possono essere `True`. Impostazione predefinita `True`. |
| `tokenAuthEnabled` | `token_auth_enabled` | Indica se abilitare o meno l'autenticazione del token per il servizio Web. `tokenAuthEnabled` E `authEnabled` non possono essere `True`. Impostazione predefinita `False`. |
...
| `keys` | N/D | Contiene elementi di configurazione per le chiavi. |
...
| `livenessProbeRequirements` | N/D | Contiene gli elementi di configurazione per i requisiti del probe di liveity. |
