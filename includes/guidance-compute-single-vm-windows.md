...
> [!NOTE]
> Azure offre due diversi modelli di distribuzione, ovvero [Azure Resource Manager](resource-manager-overview.md) e la distribuzione classica. Questo articolo usa Azure Resource Manager, consigliato da Microsoft per le nuove distribuzioni.
> 
> 

Non è consigliabile usare una singola VM per carichi di lavoro di produzione perché non esiste alcun contratto di servizio che disciplini il tempo di attività delle singole VM in Azure. Per ottenere il contratto di servizio, è necessario distribuire più VM in un [set di disponibilità](availability-set.md). Per altre informazioni, vedere [Running multiple Windows VMs on Azure](multi-vm.md) (Esecuzione di più VM Windows in Azure). 
...
* **Gruppo di risorse.** Un [*gruppo di risorse*](resource-manager-overview.md) è un contenitore che contiene risorse correlate. Creare un gruppo di risorse in cui includere le risorse per questa VM.
...
* **Disco del sistema operativo.** Il disco del sistema operativo è un VHD archiviato nell'[Archiviazione di Azure](azure-storage.md). Viene quindi salvato in modo permanente anche se il computer host si arresta.
...
* **Dischi dati.** Un [disco dati](data-disk.md) è un VHD persistente utilizzato per i dati dell'applicazione. I dischi dati vengono archiviati nell'Archiviazione di Azure, come il disco del sistema operativo.
...
* Se sono presenti molti dischi dati, occorre prestare attenzione ai limiti totali di I/O dell'account di archiviazione. Per altre informazioni, vedere [Sottoscrizione di Azure e limiti, quote e vincoli dei servizi](vm-disk-limits.md).
...
* Per reimpostare la password dell'amministratore locale, eseguire il comando `vm reset-access` dell'interfaccia della riga di comando di Azure.
    ...
    ```
* Per verificare le azioni di provisioning e altri eventi della VM, usare i [log di controllo](audit-logs.md).
* Se si necessita di crittografare i dischi del sistema operativo e i dischi dati, usare [Crittografia dischi di Azure](disk-encryption.md). 

## <a name="solution-deployment"></a>Distribuzione della soluzione
La distribuzione di esempio fornita in questa guida usa tre diversi [elementi di base dei modelli](blocchi.md) per creare:
...
Se è necessario modificare la distribuzione per le proprie esigenze, seguire le istruzioni disponibili nella pagina [guidance-single-vm](leggimi.md). 

## <a name="next-steps"></a>Passaggi successivi
Per poter applicare il [Contratto di Servizio per Macchine virtuali][vm-sla], è necessario distribuire due o più istanze in un set di disponibilità. Per altre informazioni, vedere [Running multiple VMs on Azure](multi-vm.md) (Esecuzione di più VM in Azure).
