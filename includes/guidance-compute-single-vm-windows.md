...
* **Gruppo di risorse.** Un [*gruppo di risorse*][resource-manager-overview] è un contenitore che contiene risorse correlate. Creare un gruppo di risorse in cui includere le risorse per questa VM.
...
* **Disco del sistema operativo.** Il disco del sistema operativo è un disco rigido virtuale archiviato nell'[Archiviazione di Azure][azure-storage]. Viene quindi salvato in modo permanente anche se il computer host si arresta.
...
Poiché Azure offre diverse risorse e diversi tipi di risorsa, è possibile distribuire questa architettura di riferimento in più modi. Per distribuire l'architettura nel modo illustrato nella figura precedente, vedere la sezione [Distribuzione della soluzione](#solution-deployment) alla fine del documento per ottenere altre informazioni. Se si preferisce creare la propria architettura di riferimento, è opportuno seguire questi consigli a meno che non esista un requisito specifico non soddisfatto da uno dei consigli.

### <a name="vm-recommendations"></a>Indicazioni per le VM

Azure offre diverse dimensioni per le macchine virtuali, ma è consigliabile scegliere le serie DS e GS, perché supportano l'[Archiviazione Premium][premium-storage]. Selezionare una di queste dimensioni del computer, a meno che non si abbia un carico di lavoro specializzato, ad esempio di high-performance computing. Per informazioni dettagliate vedere [Dimensioni delle macchine virtuali in Azure][virtual-machine-sizes]. 

Se si sposta un carico di lavoro esistente in Azure, per iniziare scegliere le dimensioni di VM più simili a quelle dei server locali. Misurare quindi le prestazioni del carico di lavoro effettivo in relazione agli aspetti di CPU, memoria e operazioni di input/output (IOPS) del disco e regolare le dimensioni secondo necessità. Se sono necessarie più schede di interfaccia di rete per la macchina virtuale, occorre notare che il numero massimo di schede di interfaccia di rete disponibili dipende dalle [dimensioni della VM][vm-size-tables].   

Quando si effettua il provisioning della VM e di altre risorse, è necessario specificare una località. È in genere consigliabile scegliere la località più vicina agli utenti interni o ai clienti. È tuttavia possibile che le dimensioni di VM non siano tutte disponibili in tutte le località. Per informazioni dettagliate, vedere [Servizi in base all'area][services-by-region]. Per visualizzare un elenco delle dimensioni delle VM disponibili in una determinata località, eseguire il comando seguente dell'interfaccia della riga di comando di Azure:
...
Effettuare il provisioning di una quantità di account di archiviazione sufficiente per il numero di macchine virtuali da distribuire. Ogni macchina virtuale necessita di un disco del sistema operativo e un disco temporaneo e, in base ai [limiti del disco dell'account di archiviazione][storage-account-limits], un account di archiviazione Premium può supportare da 1 a 20 VM. Se si sceglie di aggiungere dischi dati alla macchina virtuale, tali dischi non sono formattati quando vengono creati. Sarà necessario accedere alla VM per formattare eventuali dischi dati.
...
* Riservare un [indirizzo IP statico][static-ip] se è necessario un indirizzo IP fisso che non cambierà, ad esempio se è necessario creare un record A in DNS o se è necessario che l'indirizzo IP venga aggiunto all'elenco di elementi attendibili.
...
I dischi rigidi virtuali vengono archiviati nell'[Archiviazione di Azure][azure-storage] e l'Archiviazione di Azure viene replicata per assicurare durabilità e disponibilità. 
...
È disponibile una [distribuzione][github-folder] per un'architettura di riferimento che implementa queste procedure consigliate. Questa architettura di riferimento include una rete virtuale, un gruppo di sicurezza di rete e una singola macchina virtuale.

Esistono diversi modi per distribuire questa architettura di riferimento. Il modo più semplice consiste nel seguire questa procedura: 
