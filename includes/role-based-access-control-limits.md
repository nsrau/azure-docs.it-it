---
title: File di inclusione
description: File di inclusione
services: active-directory
author: rolyon
ms.service: role-based-access-control
ms.topic: include
ms.date: 05/13/2019
ms.author: rolyon
ms.custom: include file
ms.openlocfilehash: f8576c8486fb25d645eaf67448d2d7e0b9f5071f
ms.sourcegitcommit: d4dfbc34a1f03488e1b7bc5e711a11b72c717ada
ms.translationtype: MT
ms.contentlocale: it-IT
ms.lasthandoff: 06/13/2019
ms.locfileid: "66238698"
---
| Resource | Limite |
| --- | --- |
| [Assegnazioni di ruolo per le risorse di Azure per sottoscrizione di Azure](../articles/role-based-access-control/overview.md) | 2\.000 |
| [Ruoli personalizzati per le risorse di Azure per tenant](../articles/role-based-access-control/custom-roles.md) | 5\.000 |
| [Ruoli personalizzati per le risorse di Azure per tenant](../articles/role-based-access-control/custom-roles.md)<br/>(cloud specializzato, ad esempio Azure per enti pubblici, Azure Germania e Azure Cina 21Vianet) | 2\.000 |
