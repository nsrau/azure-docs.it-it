...
## Sommario
**Servizi di base** - [Componenti di Azure](#components) - [Portale di gestione](#portal) - [Calcolo](#compute) - [Gestione dati](#data) - [Rete](#networking) - [Servizi per gli sviluppatori](#DevService)

**Servizi aggiuntivi** - [Identità e accesso](#identity) - [Dispositivi mobili](#mobile) - [Backup](#backup) - [Messaggistica e integrazione](#messaging) - [Assistenza per il calcolo](#ComputeAssist) - [Prestazioni](#Performance) - [Big Compute e dati](#BigStuff) - [Servizi multimediali](#media) - [E-commerce](#commerce)

[Per iniziare](#start)

<h2><a id="components"></a>Componenti di Azure</h2>

Azure raggruppa i servizi in categorie nel portale di gestione e in vari supporti visivi come l'[infografica su Microsoft Azure](http://azure.microsoft.com/documentation/infographics/azure/ "Che cos'è l'infografica poster di Microsoft Azure"). Il portale di gestione è il portale usato per gestire la maggior parte dei servizi in Azure (ma non tutti).
...
![Componenti di Azure](./media/intro-to-azure/AzureComponentsIntroNew780.png) **Figura: Azure fornisce servizi per applicazioni accessibili tramite Internet ed eseguiti nei data center di Azure.**
...
![Macchine virtuali di Azure](./media/intro-to-azure/VirtualMachinesIntroNew.png) **Figura: Macchine virtuali offre il controllo completo sulle istanze della macchina virtuale nel cloud.**
...
### Siti Web
![Siti Web di Azure](./media/intro-to-azure/AzureWebsitesIntroNew.png) **Figura: Siti Web di Azure esegue un'applicazione di sito Web nel cloud senza dover gestire il server Web sottostante.**
...
![Database SQL di archiviazione di Azure](./media/intro-to-azure/StorageAzureSQLDatabaseIntroNew.png) **Figura: il database SQL di Azure fornisce un servizio di database relazionale gestito nel cloud.**
...
### Tables
![Tabelle di archiviazione di Azure](./media/intro-to-azure/StorageTablesIntroNew.png) **Figura: le tabelle di Azure costituiscono una soluzione NoSQL semplice per archiviare i dati.**
...
### Blobs
![BLOB di archiviazione di Azure:](./media/intro-to-azure/StorageBlobsIntroNew.png) **Figura: i blob di Azure forniscono dati binari non strutturati.**
...
![Servizio file di Azure](./media/intro-to-azure/FileServiceIntroNew.png) **Figura: Servizi file di Azure fornisce percorsi SMB \server\share per le applicazioni in esecuzione nel cloud.**
...
### Express Route
![ExpressRoute](./media/intro-to-azure/ExpressRouteIntroNew.png) **Figura: ExpressRoute usa una rete virtuale di Azure, ma reindirizza le connessioni tramite linee più veloci dedicate invece della rete Internet pubblica.**
...
### Gestione traffico
![TrafficManager](./media/intro-to-azure/TrafficManagerIntroNew.png) **Figura: Gestione traffico di Azure consente di instradare il traffico globale al servizio dell'utente in base a regole intelligenti.**
...
![Azure Multi-Factor Authentication](./media/intro-to-azure/MFAIntroNew.png) **Figura: Multi-Factor Authentication è una funzionalità che consente di verificare più forme di identificazione sull'applicazione**
...
### Servizi mobili
![MobileServices](./media/intro-to-azure/MobileServicesIntroNew.png) **Figura: Servizi mobili fornisce funzionalità comunemente richieste dalle applicazioni che si interfacciano con dispositivi mobili.**

Servizi mobili di Azure fornisce molte funzionalità utili che consentono di risparmiare tempo nella creazione di un back-end per un'applicazione mobile. Consente di effettuare semplici attività di provisioning e gestione dei dati archiviati in un database SQL. Con il codice lato server è possibile usare facilmente opzioni aggiuntive di archiviazione di dati come lo storage BLOB o MongoDB. Servizi mobili fornisce il supporto di notifiche, per quanto in determinati casi sia possibile usare hub di notifica come descritto in seguito. Il servizio include inoltre un'API REST che l'applicazione mobile può chiamare per l'esecuzione delle attività. Servizi mobili offre inoltre la possibilità di autenticare gli utenti tramite Microsoft e Active Directory nonché altri provider di identità noti come Facebook, Twitter e Google.
...
### Hub di notifica
![NotificationHubs](./media/intro-to-azure/NotificationHubsIntroNew.png) **Figura: Hub di notifica fornisce funzionalità comunemente richieste dalle applicazioni che si interfacciano con dispositivi mobili.**
...
### Queues
![Inoltro del bus di servizio di Azure](./media/intro-to-azure/QueuesIntroNew.png) **Figura: le code consentono l'accoppiamento di tipo loose tra parti di un'applicazione e facilitano la scalabilità.**
...
![Inoltro del bus di servizio di Azure](./media/intro-to-azure/ServiceBusRelayIntroNew.png) **Figura: l'inoltro del bus di servizio consente la comunicazione tra applicazioni su diversi lati di un firewall.**
...
![Argomenti del bus di servizio di Azure](./media/intro-to-azure/ServiceBusTopicsSubsIntroNew.png) **Figura: Argomenti del bus di servizio consente a più applicazioni di pubblicare messaggi e ad altre applicazioni di sottoscrivere la ricezione di messaggi che soddisfano requisiti specifici.**
...
**Scenari di Inoltro del bus di servizio** Ogni volta che si individuano le aree in cui ci sono molti messaggi tutti importanti, ma vari sistemi di downstream devono solo essere in ascolto di diversi subset di tali comunicazioni, Argomenti e sottoscrizioni del bus di servizio costituisce una scelta appropriata.

### Servizi BizTalk
![Servizi BizTalk](./media/intro-to-azure/BizTalkServicesIntroNew.png) **Figura: i Servizi BizTalk consentono di trasformare i formati dei messaggi XML nel cloud.**
...
![Utilità di pianificazione di Azure](./media/intro-to-azure/SchedulerIntroNew.png) **Figura: l'utilità di pianificazione di Azure consente di pianificare i processi in modo che vengano eseguiti in un momento specifico e per una durata specifica.**
...
![Servizio di memorizzazione nella cache di Azure](./media/intro-to-azure/AzureCacheIntroNew.png) **Figura: un'applicazione Azure può memorizzare i dati nella cache e perfino suddividerli in numerosi ruoli di lavoro**
...
### RETE CDN
![Rete CDN di Azure](./media/intro-to-azure/CDNIntroNew.png) **Figura: è possibile memorizzare nella cache copie di un BLOB nei siti di tutto il mondo.**
...
### HDInsight (Hadoop)
![HDInsight](./media/intro-to-azure/HDInsightIntroNew.png) **Figura: HDInsight è utile per l'elaborazione in blocco di elevate quantità di dati**
...
![Servizi multimediali di Azure](./media/intro-to-azure/MediaServicesIntroNew.png) **Figura: Servizi multimediali è una piattaforma per le applicazioni che forniscono video e altri elementi multimediali a client in tutto il mondo.**
...
![E-commerce di Azure](./media/intro-to-azure/CommerceIntroNew.png) **Figura: Azure Marketplace e Azure Store consentono di trovare e acquistare applicazioni e set di dati commerciali Azure e di usarli come parte delle applicazioni Azure.**
