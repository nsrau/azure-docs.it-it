...
5. Aprire la console di Windows Firewall e creare una nuova regola di porta per consentire le connessioni alla porta. Per ulteriori informazioni sulla creazione di una nuova regola di porta in Windows Firewall, vedere l'argomento relativo all'[aggiunta di una nuova regola porta in Windows Firewall].
   ...
   > [!NOTE]
   > Se il computer di test fa parte di un dominio, è possibile che le eccezioni del firewall siano controllate da un criterio di dominio. In questo caso, contattare l'amministratore del dominio per ottenere un'esenzione per la porta nel computer in uso.
   > 
   > 
   ...
   > [!NOTE]
   > Al termine del test del servizio in locale, eliminare la regola di Windows Firewall creata.
   > 
   > 
