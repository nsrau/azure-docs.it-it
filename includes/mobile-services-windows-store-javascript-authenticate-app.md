...
1.  Aprire il file di progetto default.js e nell'overload del metodo **app.OnActivated** sostituire l'ultima chiamata al metodo **refreshTodoItems** con il codice seguente:
    ...
    Verranno creati una variabile membro per archiviare l'utente corrente e un metodo per gestire il processo di autenticazione. L'utente viene autenticato tramite un account di accesso di Facebook. Se si utilizza un provider di identità diverso da Facebook, sostituire il valore passato al metodo **login** riportato in precedenza con uno dei seguenti: *microsoftaccount*, *twitter*, or *google*.

    > [WACOM.NOTE]Se le informazioni del pacchetto dell'app di Windows Store sono state registrate con Servizi mobili, è necessario chiamare il metodo [login](http://go.microsoft.com/fwlink/p/?LinkId=322050) specificando il valore **true** per il parametro *useSingleSignOn*. In caso contrario, gli utenti dovranno comunque specificare le credenziali di accesso ogni volta che viene chiamato il metodo di accesso.
