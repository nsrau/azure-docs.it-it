...
Questo articolo non fornisce una panoramica dei concetti relativi ad Azure Data Factory. Per una panoramica dei concetti correlati al servizio, vedere [Introduzione al servizio Azure Data Factory](../articles/data-factory/data-factory-introduction.md).
...
2. **Archiviazione di Azure**: in questa esercitazione si usa un account di archiviazione di Azure per archiviare i dati. Se non è disponibile un account di archiviazione di Azure, vedere l'articolo [Creare un account di archiviazione](../articles/storage/storage-create-storage-account.md#create-a-storage-account). Dopo avere creato l'account di archiviazione, è necessario ottenere la chiave dell'account usata per accedere alla risorsa di archiviazione. Vedere [Visualizzare, copiare e rigenerare le chiavi di accesso alle risorse di archiviazione](../articles/storage/storage-create-storage-account.md#view-and-copy-storage-access-keys).
...
1. Caricare il file di query Hive (HQL) nella cartella **script** del contenitore **adfgetstarted**.
2. Caricare il file di input nella cartella **inputdata** del contenitore **adfgetstarted**.
...
1. Per preparare l'archiviazione di Azure per l'esercitazione:
   
   1. Scaricare l'[ultima versione di **AzCopy**](http://aka.ms/downloadazcopy) o l'[ultima versione di anteprima](http://aka.ms/downloadazcopypr). Per istruzioni sull'uso dell'utilità, vedere l'articolo [Come usare AzCopy](../articles/storage/storage-use-azcopy.md).
   2. Aggiungere la posizione di AzCopy al percorso del sistema eseguendo il comando seguente al prompt dei comandi.
   ...
   3. Accedere alla cartella c:\\adfgetstarted ed eseguire il comando seguente. Questo comando carica il file **input.log** nell'account di archiviazione (contenitore **adfgetstarted** e cartella **inputdata**). Sostituire **StorageAccountName** con il nome del proprio account di archiviazione e **Storage Key** con la chiave dell'account di archiviazione.
      ...
      > 
      > 
   4. Al termine del caricamento del file, viene visualizzato un output simile al seguente da AzCopy.
