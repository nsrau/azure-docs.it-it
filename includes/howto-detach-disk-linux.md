...
1. Usare il comando `lsscsi` per trovare l'identificatore disco. `lsscsi` può essere installato da `yum install lsscsi` (su distribuzioni basate su Red Hat) o `apt-get install lsscsi` (su distribuzioni basate su Debian). È possibile trovare l'identificatore disco che si sta cercando grazie al numero LUN. L'ultimo numero della tupla in ogni riga è il LUN. Nell'esempio di seguito LUN 0 mappa a */dev/sdc*
...
4. Smontare ogni partizione elencata per il disco. In questo esempio: `$ sudo umount /dev/sdc1`
5. Usare il comando `blkid` per scoprire gli UUID per tutte le partizioni
...
6. Rimuovere le voci del file **/etc/fstab** associato ai percorsi dei dispositivi oppure agli UUID per scollegare tutte le partizioni del disco. Le voci per questo esempio potrebbero essere:
