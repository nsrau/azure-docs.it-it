...
---
## <a name="ropc-flow-notes"></a>Note di flusso ROPC
In Azure Active Directory B2C (Azure AD B2C) sono supportate le opzioni seguenti:In Azure Active Directory B2C (Azure AD B2C), the following options are supported:

- **Client nativo**: l'interazione dell'utente durante l'autenticazione avviene quando il codice viene eseguito in un dispositivo lato utente. Il dispositivo può essere un'applicazione per dispositivi mobili in esecuzione in un sistema operativo nativo, ad esempio Android e iOS.The device can be a mobile application that's running in a native operating system, such as Android and iOS.
...
- La gestione delle sessioni, inclusa la gestione dell'accesso a i servizi di accesso (KMSI,, Keep MeOn), non è applicabile.
