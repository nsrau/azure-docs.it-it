1. Accedere al [portale di gestione](http://manage.windowsazure.com) di Azure. Se non si dispone ancora di una sottoscrizione, usare la [versione di valutazione gratuita](http://www.windowsazure.com/pricing/free-trial/).
...
6. Nella terza schermata è possibile configurare le risorse per le connessioni di rete, l'archiviazione e la disponibilità. Di seguito sono indicate alcune informazioni utili per agevolare la compilazione: 
	...
	- Per informazioni dettagliate sulla configurazione degli endpoint, vedere la pagina che illustra [come configurare gli endpoint a una macchina virtuale](http://www.windowsazure.com/documentation/articles/virtual-machines-set-up-endpoints/).
...
7. La quarta schermata di configurazione consente di configurare l'agente di macchine virtuali e alcune delle estensioni disponibili. Fare clic sul segno di spunta per creare la macchina virtuale.
	...
	>[AZURE.NOTE] L'agente di macchine virtuali offre l'ambiente per installare le estensioni che consentono di interagire con la macchina virtuale o gestirla. Per informazioni dettagliate, vedere [Estensioni VM e funzionalità di Azure](http://go.microsoft.com/FWLink/p/?LinkID=390493).  
